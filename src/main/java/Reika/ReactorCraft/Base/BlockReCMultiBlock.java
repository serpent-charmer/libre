...
public abstract class BlockReCMultiBlock extends BlockMultiBlock<Boolean> {
    public BlockReCMultiBlock(Material par2Material) {
        ...
        this.setHardness(2);
        this.setCreativeTab(
            ReactorCraft.instance.isLocked() ? null : ReactorCraft.tabRctrMultis
        );
    }
    ...
    getMessages(World world, int x, int y, int z, int side) {
        ...
        return te instanceof TileEntityReactorBase
            ? ((TileEntityReactorBase) te).getMessages(world, x, y, z, side)
            : new ArrayList();
    }

    public final String getName(int meta) {
        return StatCollector.translateToLocal(
            "multiblock." + this.getIconBaseName().toLowerCase(Locale.ENGLISH) + "."
            + (meta & 7)
        );
    }
