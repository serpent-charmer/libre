...
public abstract class TileEntityReactorPiping
    extends TileEntityReactorBase implements RenderableDuct, PumpablePipe, RCPipe {
    ...
    public final void deleteFromAdjacentConnections(World world, int x, int y, int z) {
        for (int i = 0; i < 6; i++) {
            ...
            if (m == ReactorTiles.TEList[this.getIndex()]) {
                TileEntityReactorPiping te
                    = (TileEntityReactorPiping) world.getTileEntity(dx, dy, dz);
                ...
                if (te instanceof TileEntityReactorPiping) {
                    ...
                    if (f != null) {
                        ...
                        if (todrain > 0 && this.canIntakeFluid(f)) {
                            //ReikaJavaLibrary.pConsole("took in "+todrain+", had
                            //"+this.getLevel()+" here and "+amt+" in other");
                            this.setFluid(f);
            ...
            if (this.canInteractWith(world, x, y, z, dir)) {
                ...
                if (te instanceof TileEntityReactorPiping) {
                    ...
                    if (tp.canIntakeFluid(f)) {
                        ...
                        if (toadd > 0) {
                            //ReikaJavaLibrary.pConsole("dumped "+toadd+", had
                            //"+this.getLevel()+" here and "+otherlevel+" in other");
                            tp.addFluid(toadd);
    ...
    public final int getPipeOutput(int max) {
        return Math.min(
            TransferAmount.FORCEDQUARTER.getTransferred(max), this.getLevel() - 5
        );
    }
    ...
    public final boolean canTransferTo(PumpablePipe p, ForgeDirection dir) {
        return p instanceof TileEntityReactorPiping
            && this.getTile() == ((TileEntityReactorPiping) p).getTile();
    }
