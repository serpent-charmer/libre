...
public abstract class TileEntityReactorBase extends TileEntityRegistryBase<ReactorTiles>
    implements RenderFetcher, Transducerable {
    ...
    public boolean shouldRenderInPass(int pass) {
        ...
        return pass == 0
            || ((r.renderInPass1() || this instanceof ShaftMachine) && pass == 1);
    }
    ...
    protected void updateTemperature(World world, int x, int y, int z) {
        ...
        ReikaWorldHelper.temperatureEnvironment(
            world, x, y, z, Math.min(temperature, 1000)
        );
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (r != null) {
                TileEntityReactorBase te
                    = (TileEntityReactorBase) world.getTileEntity(dx, dy, dz);
                if (te instanceof Temperatured) {
                    int Tamb_loc
                        = ReikaWorldHelper.getAmbientTemperatureAt(world, dx, dy, dz);
                    ...
                    if (flag) {
                        ...
                        dT = (T - temperature)
                            - Math.max(
                                0, (Tamb - Tamb_loc)
                            ); //if Tamb here is > Tamb there, subtract that difference to
                               //avoid exploits
                        float f = te.getHeatThroughput(this);
                        //ReikaJavaLibrary.pConsole(te.getMachine()+" >
                        //"+this.getMachine()+" = "+f);
                        dT *= f;
                        if (dT > 0) {
                            ...
                            //"+this.getTEName()+":"+te.getTEName()+"->"+(temperature+dT/4D)+":"+newT,
                            //this instanceof TileEntityWaterCell &&
                            ...
                            if (this instanceof TileEntityReactorBoiler
                                && !(tr instanceof TileEntityReactorBoiler)
                                && tr instanceof TypedReactorCoreTE) {
                                ((TileEntityReactorBoiler) this)
                                    .setReactorType(
                                        ((TypedReactorCoreTE) tr).getReactorType(), add
                                    );
                            } else if (this instanceof TileEntityNuclearBoiler && tr.getClass() == this.getClass()) {
                                ((TileEntityNuclearBoiler) this)
                                    .setReactorType(
                                        ((TileEntityNuclearBoiler) tr).getReactorType(),
                                        add
                                    );
                            }
                        }
    ...
    private int getHeatFraction(TileEntityReactorBase other) {
        return other instanceof TemperaturedReactorTyped
            ? this.getHeatConductionFraction((TemperaturedReactorTyped) other)
            : 4;
    }

    private float getHeatThroughput(TileEntityReactorBase other) {
        return other instanceof TemperaturedReactorTyped
            ? this.getHeatConductionThroughput((TemperaturedReactorTyped) other)
            : 1;
    }

    private float getHeatEfficiency(TileEntityReactorBase other) {
        return other instanceof TemperaturedReactorTyped
            ? this.getHeatConductionEfficiency((TemperaturedReactorTyped) other)
            : 0;
    }
    ...
    protected float getHeatConductionEfficiency(TemperaturedReactorTyped other) {
        ...
        ReactorType r1 = this instanceof ReactorTyped
            ? ((ReactorTyped) this).getReactorType()
            : null;
    ...
    getMessages(World world, int x, int y, int z, int side) {
        ...
        if (this instanceof Temperatured) {
            String s = String.format(
                ...
                ((Temperatured) this).getTemperature()
            );
            li.add(s);
            ...
            String s = String.format(
                ...
                this.getTEName(),
                Variables.TEMPERATURE,
                ((ThermalMachine) this).getTemperature()
            );
            //li.add(s);
            ...
            String s = String.format(
                ...
                this.getTEName(),
                Variables.TEMPERATURE,
                ((TemperatureTE) this).getTemperature()
            );
            li.add(s);
        }
        if (this instanceof TileEntityReactorPiping) {
            ...
            if (rp.getLevel() <= 0) {
                ...
                String s = String.format(
                    ...
                    this.getTEName(),
                    rp.getLevel(),
                    rp.getFluidType().getLocalizedName()
                );
                li.add(s);
            }
        }
        ...
        if (this instanceof TileEntitySteamLine) {
            ...
            String s = String.format(
                "%s contains %d m^3 of steam.", this.getTEName(), sl.getSteam()
            );
            li.add(s);
        ...
        if (this instanceof TileEntityHeatPipe) {
            ...
            String s = String.format(
                "%s contains %.3f%sJ of heat energy.",
                this.getTEName(),
                ReikaMathLibrary.getThousandBase(e),
                ReikaEngLibrary.getSIPrefix(e)
            );
            li.add(s);
        }
        return li;
    ...
    public final Visibility getOCNetworkVisibility() {
        return this.getTile().isPipe() || this.getTile() == ReactorTiles.REFLECTOR
            ? Visibility.None
            : Visibility.Network;
    }
    ...
    public double heatEnergyPerDegree() {
        double base = ReikaThermoHelper.STEEL_HEAT
