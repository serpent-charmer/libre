...
public abstract class TileEntityNuclearCore extends TileEntityInventoriedReactorBase
    implements LinkableReactorCore, Feedable, ChunkLoadingTile, EMPControl {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        if (!world.isRemote && this.isFissile()
            && rand.nextInt(this.getDecayNeutronChance()) == 0)
            world.spawnEntityInWorld(new EntityNeutron(
                world, x, y, z, this.getRandomDirection(false), NeutronType.DECAY
            ));
        ...
        if (temperature > CLADDING) {
            ...
            ReikaParticleHelper.SMOKE.spawnAroundBlockWithOutset(
                world, x, y, z, 9, 0.0625
            );
    ...
    protected final void spawnNeutronBurst(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 3; i++) {
            world.spawnEntityInWorld(new EntityNeutron(
                ...
                y,
                z,
                this.getRandomDirection(ReactorOptions.VERTNEUTRONS.getState()),
                n
            ));
        }
    }
    ...
    protected void onMeltdown(World world, int x, int y, int z) {
        ...
        for (int i = x - r; i <= x + r; i++) {
            for (int j = y - r; j <= y + r; j++) {
                for (int k = z - r; k <= z + r; k++) {
                    ...
                    if (src == other)
                        world.setBlock(
                            i, j, k, ReactorBlocks.CORIUMFLOWING.getBlockInstance()
                        );
                }
            }
        }
        ...
        double scatter = RadiationEffects.instance.contaminateArea(
            world, x, y, z, 32, 8, 2, true, RadiationIntensity.LETHAL
        );
        this.testAndDoHydrogenExplosion(world, x, y, z, scatter);
    }

    private void
    testAndDoHydrogenExplosion(World world, int x, int y, int z, double scatter) {
        if (true || ReikaRandomHelper.doWithChance((double) hydrogen / MAX_HYDROGEN)) {
            HydrogenExplosion ex = new HydrogenExplosion(
                world, null, x + 0.5, y + 0.5, z + 0.5, 7 /*, scatter*/
            );
            ex.doExplosionA();
    ...
    protected void updateTemperature(World world, int x, int y, int z) {
        ...
        if (dT > 0) {
            for (int i = 2; i < 6; i++) {
                ...
                if (r == this.getTile()) {
                    TileEntityNuclearCore te
                        = (TileEntityNuclearCore) world.getTileEntity(dx, dy, dz);
                    int dTemp = temperature - te.temperature;
                    if (dTemp > 0) {
                        int d = this.getSameCoreHeatConductionFraction();
                        temperature -= dTemp / d;
                        te.temperature
                            += dTemp / d * this.getHeatConductionEfficiency(te);
                    }
                }
            }
        }
