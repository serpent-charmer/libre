...
public abstract class TileEntityWasteUnit extends TileEntityInventoriedReactorBase {
    ...
    public static void registerAdjacency() {
        adjacency = TileEntityAdjacencyUpgrade.getOrCreateAdjacencyCheckHandler(
            CrystalElement.LIGHTBLUE,
            "Accelerate waste decay",
            ReactorTiles.STORAGE.getCraftedProduct(),
            ReactorTiles.WASTEDECAYER.getCraftedProduct()
        );
    }
    ...
    private final double getAcceleratorBoost() {
        ...
        if (tier <= 0)
            return 1;
        return Math.sqrt(ChromatiAPI.getAPI().adjacency().getFactor(
            CrystalElementAccessor.getByEnum("LIGHTBLUE"), tier
        ));
    }
    ...
    protected final void decayWaste() {
        ...
        for (int i = 0; i < this.getSizeInventory(); i++) {
            if (inv[i] != null
                && inv[i].getItem() == ReactorItems.WASTE.getItemInstance()) {
                Isotopes atom = Isotopes.getIsotope(inv[i].getItemDamage());
                if (ReikaRandomHelper.doWithChance(
                        mult / this.getBaseDecayRate() * 0.5
                        * ReikaNuclearHelper.getDecayChanceFromHalflife(
                            Math.log(atom.getMCHalfLife())
                        )
                    )) {
                ...
                if (ReikaNuclearHelper.shouldDecay(atom, mult)) {
                    ReikaInventoryHelper.decrStack(
                        i, this, Math.max(1, inv[i].stackSize / 2)
                    );
                    this.onDecayWaste(i);
                }
            }
    ...
    ) { //compensate for lag + make decay effectively run even with MC closed
    ...
    public final boolean isItemValidForSlot(int i, ItemStack is) {
        return is.getItem() == ReactorItems.WASTE.getItemInstance()
            && is.getItemDamage() < 1000
            && this.isValidIsotope(Isotopes.getIsotope(is.getItemDamage()))
            && this.isValidSlot(i, is);
    }
    ...
    public static double getHalfLife(ItemStack is) {
        if (is.getItem() != ReactorItems.WASTE.getItemInstance())
            return 0;
        return Isotopes.getIsotope(is.getItemDamage())
            .getMCHalfLife(
            ); //WasteManager.getWasteList().get(is.getItemDamage()).getMCHalfLife();
    }

    public static boolean isLongLivedWaste(ItemStack is) {
        return is.getItem() == ReactorItems.WASTE.getItemInstance()
            && getHalfLife(is) > 6 * ReikaTimeHelper.YEAR.getMinecraftDuration();
    }
