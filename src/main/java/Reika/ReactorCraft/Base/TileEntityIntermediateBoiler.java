...
public abstract class TileEntityIntermediateBoiler
    extends TileEntityNuclearBoiler implements TemperaturedReactorTyped {
    protected StepTimer timer = new StepTimer(20);

    protected final HybridTank output = new HybridTank(
        this.getName().toLowerCase(Locale.ENGLISH) + "out", this.getCapacity()
    );
    ...
    private void transferFluid(World world, int x, int y, int z) {
        ...
        if (r == this.getTile()) {
            TileEntityIntermediateBoiler te
                = (TileEntityIntermediateBoiler) world.getTileEntity(x, y + 1, z);
            if (!te.tank.isFull() && !tank.isEmpty()) {
                int amt = Math.min(
                    tank.getLevel(),
                    Math.min(100, te.tank.getCapacity() - te.tank.getLevel())
                );
            ...
            if (!te.output.isFull() && !output.isEmpty()) {
                int amt = Math.min(
                    output.getLevel(),
                    Math.min(100, te.output.getCapacity() - te.output.getLevel())
                );
    ...
    public boolean canHeat() {
        return temperature >= this.getMinimumTemperature()
            && tank.getLevel() >= this.getLiquidUsage() && !output.isFull()
            && tank.getActualFluid().equals(this.getInputFluid());
    }
