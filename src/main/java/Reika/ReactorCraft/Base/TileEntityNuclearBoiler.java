...
public abstract class TileEntityNuclearBoiler
    extends TileEntityTankedReactorMachine implements TypedReactorCoreTE {
    protected int steam;
    ...
    protected void balanceFluid(World world, int x, int y, int z) {
        for (int i = 0; i < 2; i++) {
            ...
            if (r == ReactorTiles.TEList[this.getIndex()]) {
                TileEntityNuclearBoiler te
                    = (TileEntityNuclearBoiler) world.getTileEntity(dx, dy, dz);
                if (te.tank.getLevel() < tank.getLevel()
                    && (te.tank.isEmpty()
                        || te.tank.getActualFluid() == tank.getActualFluid())) {
