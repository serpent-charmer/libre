...
public class RadiationHandler {
    ...
    applyPotionEffectToEntity(EntityLivingBase e, RadiationLevel ri) {
        try {
    ...
    public static void
    irradiateBlock(World world, int x, int y, int z, RadiationLevel ri) {
        try {
    ...
    public static void irradiateArea(
        World world,
        int x,
        int y,
        int z,
    ...
    static {
        try {
            intensityClass = Class.forName(
                "Reika.ReactorCraft.Auxiliary.RadiationEffects$RadiationIntensity"
            );
            intensities = (RadiationLevel[]) intensityClass.getEnumConstants();

            radiationHandler
                = Class.forName("Reika.ReactorCraft.Auxiliary.RadiationEffects");

            Field f = radiationHandler.getDeclaredField("instance");
            f.setAccessible(true);
            instance = f.get(null);

            hasSuit = radiationHandler
                          .getDeclaredMethod("hasHazmatSuit", EntityLivingBase.class);
            hasSuit.setAccessible(true);
            applyPotion = radiationHandler.getDeclaredMethod(
                "applyEffects", EntityLivingBase.class, intensityClass
            );
            applyPotion.setAccessible(true);
            applyToBlock = radiationHandler.getDeclaredMethod(
                "transformBlock",
                World.class,
                int.class,
                int.class,
            ...
            fillArea = radiationHandler.getDeclaredMethod(
                "contaminateArea",
                World.class,
                int.class,
                int.class,
