...
public interface NeutronShield {
    /**
       Return a percentage from 0-100. Args: Neutron Type. Types:
        ...
        Note that values may be clamped such that the absorption chance cannot exceed that
       of native materials, usually around 95-99%.
     */
    public double getAbsorptionChance(String type);

    /**
     * Return a double from 0-1 (return is clamped) to modify radiation spawn chances on
     * neutron absorption. 1 is default settings, 0 is no radiation.
     */
    public double
    getRadiationSpawnMultiplier(World world, int x, int y, int z, String type);
}
