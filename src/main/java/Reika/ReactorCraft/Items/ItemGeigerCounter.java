...
public class ItemGeigerCounter extends ItemReactorTool implements ChargeableTool {
    ...
    @Override
    public void
    onUpdate(ItemStack is, World world, Entity e, int slot, boolean currentHeld) {
        if (currentHeld && is.getItemDamage() > 0) {
            int r = 20;
            if (e instanceof EntityLivingBase
                && ((EntityLivingBase) e).isPotionActive(ReactorCraft.radiation))
                r = -1;
            AxisAlignedBB box = r >= 0
                ? AxisAlignedBB
                      .getBoundingBox(e.posX, e.posY, e.posZ, e.posX, e.posY, e.posZ)
                      .expand(r, r, r)
                : null;
            List<EntityRadiation> li = box == null
                ? null
            ...
            if (li == null) {
                ...
                Collections.sort(
                    li, new EntityDistanceComparator(e.posX, e.posY, e.posZ)
                );
                EntityRadiation er = li.get(0);
                double dist = ReikaMathLibrary.py3d(
                    e.posX - er.posX, e.posY - er.posY, e.posZ - er.posZ
                );
