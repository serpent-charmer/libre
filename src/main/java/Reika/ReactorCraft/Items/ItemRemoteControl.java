    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        if (this.canUse(is, world, ep)) {
            ...
            if (te instanceof TileEntityCPU) {
                ...
                if (id == ReactorTiles.CPU.getBlock()
                    && meta == ReactorTiles.CPU.getBlockMetadata()) {
        ...
        if (r == ReactorTiles.CPU) {
            if (te != null) {
                ...
                ReikaChatHelper.sendChatToPlayer(
                    ep,
                    "Linking to " + ReactorTiles.CPU.getName() + " at "
                        + new Coordinate(te)
                );
                return true;
            }
        } else if (r != null && r.isLinkableReactorCore() && this.getLinkedCPU(is) != null) {
            ((TileEntityCPU) this.getLinkedCPU(is))
                .addTemperatureCheck((LinkableReactorCore) te);
            ReikaChatHelper.sendChatToPlayer(
                ep,
                "Linking " + r.getName() + " to " + ReactorTiles.CPU.getName() + " at "
                    + new Coordinate(this.getLinkedCPU(is))
            );
        }
        return false;
    }
