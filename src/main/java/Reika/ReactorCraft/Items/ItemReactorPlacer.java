...
public class ItemReactorPlacer extends Item implements ISize {
    ...
    public boolean onItemUse(
        ...
        if (!ep.capabilities.isCreativeMode
            && !this.checkValidBounds(is, ep, world, x, y, z))
            return false;
        ...
        if (m == ReactorTiles.INJECTOR) {
            ((TileEntityFusionInjector) te)
                .setFacing(
                    ReikaEntityHelper.getDirectionFromEntityLook(ep, false).getOpposite()
                );
        }
        if (m == ReactorTiles.GENERATOR) {
            ((TileEntityReactorGenerator) te)
                .setFacing(ReikaEntityHelper.getDirectionFromEntityLook(ep, false));
        }
    ...
    private void placeFusionReactor(World world, int x, int y, int z, EntityPlayer ep) {
        ...
        world.setBlock(
            ...
            dy,
            dz,
            ReactorTiles.INJECTOR.getBlock(),
            ReactorTiles.INJECTOR.getBlockMetadata(),
            3
        );
        world.setBlock(
            dx - 28,
        ...
        ((TileEntityFusionInjector) world.getTileEntity(dx, dy, dz))
            .setFacing(ForgeDirection.NORTH);
        ((TileEntityFusionInjector) world.getTileEntity(dx - 28, dy, dz))
            .setFacing(ForgeDirection.SOUTH);
        ((TileEntityFusionInjector) world.getTileEntity(dx - 14, dy, dz + 14))
        ...
        for (int k = 0; k < 4; k++) {
            for (int i = 0; i < 9; i++) {
                world.setBlock(
                    ...
                    dy,
                    dz,
                    ReactorTiles.MAGNET.getBlock(),
                    ReactorTiles.MAGNET.getBlockMetadata(),
                    3
                );
        ...
        for (int i = 2; i < 6; i++) {
            for (int k = 2; k < 8; k++) {
                ...
                world.setBlock(x+dir.offsetX*k, y+1, z+dir.offsetZ*k,
        ReactorBlocks.SOLENOIDMULTI.getBlockInstance(), 4, 3);
            }
        }
         */
        ...
        world.setBlock(
            ...
            y + 1,
            z,
            ReactorTiles.SOLENOID.getBlock(),
            ReactorTiles.SOLENOID.getBlockMetadata(),
            3
        );
        ((TileEntitySolenoidMagnet) world.getTileEntity(x, y + 1, z)).setPlacer(ep);
        ((TileEntitySolenoidMagnet) world.getTileEntity(x, y + 1, z))
            .setHasMultiBlock(true);
    }
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        ...
        if (r.isPowerReceiver()) {
            ReactorPowerReceiver te
                = (ReactorPowerReceiver) r.createTEInstanceForRender();
            ...
            if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
                if (pow > 1)
                    li.add(String.format(
                        "Minimum Power: %.3f %sW",
                        ReikaMathLibrary.getThousandBase(pow),
                        ReikaEngLibrary.getSIPrefix(pow)
                    ));
                if (trq > 1)
                    li.add(String.format(
                        "Minimum Torque: %.3f %sNm",
                        ReikaMathLibrary.getThousandBase(trq),
                        ReikaEngLibrary.getSIPrefix(trq)
                    ));
                if (spd > 1)
                    li.add(String.format(
                        "Minimum Speed: %.3f %srad/s",
                        ReikaMathLibrary.getThousandBase(spd),
                        ReikaEngLibrary.getSIPrefix(spd)
                    ));
            } else {
    ...
    public String getItemStackDisplayName(ItemStack is) {
        ...
        return ir.hasMultiValuedName() ? ir.getMultiValuedName(is.getItemDamage())
                                       : ir.getBasicName();
    }
    ...
    public EnumSize getSize(ItemStack is) {
        return ReactorTiles.TEList[is.getItemDamage()].isMultiblock() ? EnumSize.HUGE
                                                                      : EnumSize.LARGE;
    }
