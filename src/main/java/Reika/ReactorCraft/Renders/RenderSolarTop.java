...
public class RenderSolarTop extends ReactorRenderBase {
     ...
     */
    public void renderTileEntitySolarTopAt(
        TileEntitySolarTop tile, double par2, double par4, double par6, float par8
    ) {
        ModelSolarTop var14;
        var14 = TopModel;
        ...
        boolean flip = tile.worldObj != null
            && tile.worldObj.getTileEntity(tile.xCoord, tile.yCoord - 1, tile.zCoord)
                    instanceof TileEntitySolarTop;
        double d = flip ? 0 : 2;
        ...
        int c = tile.isInWorld()
            ? ReikaPhysicsHelper.getColorForTemperature(200 + tile.getTemperature() * 2)
            : 0x000000;
        //ReikaJavaLibrary.pConsole(tile.getTemperature()+" > "+Integer.toHexString(c),
        //tile.isInWorld());
    ...
    private void renderFlare(TileEntitySolarTop te, float par8) {
        ...
        double f2
            = ((TileEntitySolar) te.getTileEntity(te.xCoord, te.yCoord - 1, te.zCoord))
                  .getArrayOverallBrightness();
    ...
    public void renderTileEntityAt(
        ...
        if (te.isInWorld() && te.isActive() && te.getTemperature() > 400
