...
public class RenderTurbine extends ReactorRenderBase {
        ...
        for (int i = 0; i < models.length; i++) {
            ...
            try {
                ...
                throw new RegistrationException(
                    ReactorCraft.instance, "Invalid turbine model registration!"
                );
            }
        }
    }
    ...
    public final void renderTileEntityTurbineCoreAt(
        TileEntityTurbineCore tile, double par2, double par4, double par6, float par8
    ) {
        this.bindTextureByName(
            "/Reika/ReactorCraft/Textures/TileEntity/" + this.getTextureName() + ".png"
        );
        GL11.glPushMatrix();
        ...
        if (tile.isInWorld()) {
            if (tile.hasMultiBlock()) {
                ...
                if (Minecraft.getMinecraft().thePlayer.getCommandSenderName().equals(
                        "waterlubber"
                    )) {
                    if (System.identityHashCode(tile) % 2 == 0
                        && !Minecraft.getMinecraft().isIntegratedServerRunning()) {
                        dmg += 2;
                    }
                }
                models[tile.getStage()].renderAll(
                    tile,
                    dmg > 0 ? ReikaJavaLibrary.makeListFrom(dmg) : null,
                    -tile.phi,
                    0
                );
            } else {
    ...
    public final void renderTileEntityAt(
        ...
        if (this.doRenderModel((TileEntityReactorBase) tile))
            this.renderTileEntityTurbineCoreAt(
                (TileEntityTurbineCore) tile, par2, par4, par6, par8
            );
