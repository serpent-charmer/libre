...
public class RenderSolenoid extends ReactorRenderBase {
    ...
    public void renderTileEntitySolenoidMagnetAt(
        TileEntitySolenoidMagnet tile, double par2, double par4, double par6, float par8
    ) {
        ModelSolenoid var14;
        var14 = SolenoidModel;
        ...
        if (tile.canRenderCoil() || !tile.isInWorld())
            var14.renderAll(
                tile,
                ReikaJavaLibrary.makeListFrom(tile.canRenderCoil() && tile.hasWorldObj()),
                -tile.phi,
                0
            );
        else {
    ...
    public void renderTileEntityAt(
        ...
        if (this.doRenderModel((TileEntityReactorBase) tile))
            this.renderTileEntitySolenoidMagnetAt(
                (TileEntitySolenoidMagnet) tile, par2, par4, par6, par8
            );
    }
