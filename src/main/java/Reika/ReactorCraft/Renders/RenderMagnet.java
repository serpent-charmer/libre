...
public class RenderMagnet extends ReactorRenderBase {
    ...
    public void renderTileEntityMagnetAt(
        TileEntityToroidMagnet tile, double par2, double par4, double par6, float par8
    ) {
        ModelMagnet var14;
        var14 = MagnetModel;
    ...
    public void renderTileEntityAt(
        ...
        if (this.doRenderModel((TileEntityReactorBase) tile))
            this.renderTileEntityMagnetAt(
                (TileEntityToroidMagnet) tile, par2, par4, par6, par8
            );
    ...
    private void
    renderAngleLine(TileEntityToroidMagnet tile, double par2, double par4, double par6) {
        if (tile == null)
            return;
        if (!tile.isInWorld())
            return;
        ...
        if (ep.getEntityData().hasKey(ItemIOGoggles.NBT_KEY)
        ...
        for (int i = 1; i < 4; i++)
            ReikaRenderHelper.renderVCircle(
                i, 0, 0, 0, ReikaColorAPI.RGBtoHex(255, 255, 255, a), 90, 10
            );
        GL11.glRotated(-ang, 0, 1, 0);
        GL11.glTranslated(-par2 - 0.5, -par4, -par6 - 0.5);
