...
public class BasicReactorOreGenerator implements OreGenerator {
    ...
    private void
    generate(ReactorOres ore, World world, Random random, int chunkX, int chunkZ) {
        ...
        for (int i = 0; i < passes; i++) {
            ...
            if (ore.canGenAt(world, posX, posY, posZ)) {
                ...
                if ((new WorldGenMinable(id, meta, size, ore.getReplaceableBlock()))
                        .generate(world, random, posX, posY, posZ))
                    ; //ReikaJavaLibrary.pConsole(ore+" @ "+posX+", "+posY+", "+posZ, ore
                      //== ReactorOres.MAGNETITE);
            }
