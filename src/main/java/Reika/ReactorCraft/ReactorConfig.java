...
public class ReactorConfig extends ControlledConfig {
    ...
    private static final ArrayList<String> entries
        = ReikaJavaLibrary.getEnumEntriesWithoutInitializing(ReactorAchievements.class);
    ...
    public ReactorConfig(DragonAPIMod mod, ConfigList[] option, IDRegistry[] id) {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            achievementIDs[i]
                = this.registerAdditionalOption("Achievement IDs", name, 72000 + i);
        }

        heavyWaterDimensions = this.registerAdditionalOption(
            "Other Options", "Heavy Water Dimensions (Empty for All)", new int[0]
        );
    }
