...
public class ReactorGuiHandler implements IGuiHandler {
        ...
        if (ID == 0) {
            ...
            if (r != null) {
                ...
                switch (r) {
                    case FUEL:
                    case BREEDER:
                        return new ContainerNuclearCore(
                            player, (TileEntityNuclearCore) te
                        );
                    case THORIUM:
                        return new ContainerThoriumCore(
                            player, (TileEntityThoriumCore) te
                        );
                    case WASTECONTAINER:
                        return new ContainerWasteContainer(
                            player, (TileEntityWasteContainer) te
                        );
                    case WASTEDECAYER:
                        return new ContainerWasteDecayer(
                            player, (TileEntityWasteDecayer) te
                        );
                    case PROCESSOR:
                    ...
                    case SYNTHESIZER:
                        return new ContainerSynthesizer(
                            player, (TileEntitySynthesizer) te
                        );
                    case ELECTROLYZER:
                        return new ContainerElectrolyzer(
                            player, (TileEntityElectrolyzer) te
                        );
                    case STORAGE:
                        return new ContainerWasteStorage(
                            player, (TileEntityWasteStorage) te
                        );
                    case PEBBLEBED:
                        return new ContainerPebbleBed(player, (TileEntityPebbleBed) te);
                    case CPU:
                        int slot = ReikaInventoryHelper.locateIDInInventory(
                            ReactorItems.REMOTE.getItemInstance(), player.inventory
                        );
                        return new CoreContainer(player, te)
                            .setAlwaysInteractable()
                            .addSlotRelay(player.inventory, slot);
                    default:
                        return null;
                }
            }
    ...
    getClientGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
        ...
        if (ID == 11) {
            return new GuiReactorBook(
                player, world, ReactorBook.getScreen(r, te), ReactorBook.getPage(r, te)
            );
        }
        if (ID == 12) {
            return new GuiReactorBookPage(
                player, world, ReactorBook.getScreen(r, te), ReactorBook.getPage(r, te)
            );
        }
