...
public class GuiCPU extends ReactorGuiBase {
    ...
    public void initGui() {
        ...
        buttonList.add(
            new ImagedGuiButton(2, j + 7, k + 84, 12, 48, 90, 60, tex, RotaryCraft.class)
        );
        buttonList.add(new ImagedGuiButton(
            3, j + 157, k + 84, 12, 48, 90, 108, tex, RotaryCraft.class
        ));
    }
        ...
        switch (button.id) {
            case 0:
                ReikaPacketHelper.sendUpdatePacket(
                    ...
                    ReactorPackets.CPURAISE.ordinal(),
                    tile,
                    pt
                );
                break;
            case 1:
                ReikaPacketHelper.sendUpdatePacket(
                    ReactorCraft.packetChannel,
                    ReactorPackets.CPULOWER.ordinal(),
                    tile,
                    pt
                );
                break;
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        for (int a = layout.getMinX(); a <= layout.getMaxX(); a++) {
            for (int b = layout.getMinZ(); b <= layout.getMaxZ(); b++) {
                if (a != 0 || b != 0) {
                    int c = layout.getDisplayColorAtRelativePosition(
                        tile.worldObj, a, offsetY, b
                    );
                    int x = ox + a * s;
                    int y = oy + b * s;
    ...
    protected void mouseClicked(int x, int y, int id) {
        ...
        TileEntityControlRod rod
            = layout.getControlRodAtRelativePosition(tile.worldObj, a, offsetY, b);
        //ReikaJavaLibrary.pConsole(a+", "+b+": "+rod);
        if (rod != null) {
            ReikaPacketHelper.sendUpdatePacket(
                ReactorCraft.packetChannel,
                ReactorPackets.CPUTOGGLE.ordinal(),
                rod,
                PacketTarget.server
            );
