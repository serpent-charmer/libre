...
public class GuiReactorBook extends GuiHandbook {
    ...
    public int getMaxScreen() {
        return ReactorBook.RESOURCEDESC.getScreen()
            + ReactorBook.RESOURCEDESC.getNumberChildren() / GuiHandbook.PAGES_PER_SCREEN;
    }
    ...
    protected void drawAuxData(int posX, int posY) {
        ...
        if (h.isMachine()) {
            List<ItemStack> out
                = ReikaJavaLibrary.makeListFrom(h.getMachine().getCraftedProduct());
            if (out == null || out.size() <= 0)
                return;
            ReikaGuiAPI.instance.drawCustomRecipes(
                ri,
                fontRendererObj,
                out,
                HandbookAuxData.getWorktable(),
                posX + 72 - 18,
                posY + 18,
                posX - 1620,
                posY + 32
        ...
        if (h == ReactorBook.MAGNET) {
            ...
            int k = (int
            ) ((System.nanoTime() / 2000000000) % ReactorItems.MAGNET.getNumberMetadatas()
            );
            if (k != 0) {
            ...
            MachineRecipeRenderer.instance.drawCompressor(
                posX + 66, posY + 14, in, posX + 120, posY + 41, out
            );
            ...
            BlastCrafting r
                = RecipesBlastFurnace.getRecipes().getAllCraftingMaking(out).get(0);
            MachineRecipeRenderer.instance.drawBlastFurnaceCrafting(
                posX + 99, posY + 18, posX + 180, posY + 32, r
            );
            ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                fontRendererObj, r.temperature + "C", posX + 56, posY + 66, 0
            );
        }
    }
    ...
    private void drawTally(ReactorStructures s, int j, int k) {
        ItemHashMap<Integer> map
            = s.getStructure(worldObj, 0, 0, 0, ForgeDirection.EAST).tally();
        int i = 0;
        int n = 8;
        ...
        for (ItemStack is : c) {
            ...
            if (b != null && b.isMachine()) {
                is2 = ReactorTiles
                          .getMachineFromIDandMetadata(
                              b.getBlockInstance(), is2.getItemDamage()
                          )
                          .getCraftedProduct();
            }
            ...
            if (b2 != null && b2.isMachine()) {
                is2 = MachineRegistry
                          .getMachineFromIDandMetadata(
                              b2.getBlockInstance(), is2.getItemDamage()
                          )
                          .getCraftedProduct();
            }
            ReikaGuiAPI.instance.drawItemStackWithTooltip(
                itemRender, fontRendererObj, is2, dx, dy
            );
            fontRendererObj.drawString(
                String.valueOf(map.get(is)), dx + 20, dy + 5, 0x000000
            );
            i++;
        }
    }
    ...
    protected void bindTexture() {
        ...
        if (h == ReactorBook.FUSIONINFO) {
            ReikaTextureHelper.bindTexture(
                ReactorCraft.class, "Textures/GUI/Handbook/fusion.png"
            );
        } else if (h == ReactorBook.FISSIONINFO) {
            ReikaTextureHelper.bindTexture(
                ReactorCraft.class, "Textures/GUI/Handbook/fission.png"
            );
        } else
            super.bindTexture();
    }
