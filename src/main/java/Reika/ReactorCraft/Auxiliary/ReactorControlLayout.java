...
public class ReactorControlLayout {
    ...
    public void removeControlRod(TileEntityControlRod rod) {
        controls.remove(rod);
        if (minX == rod.xCoord || maxX == rod.xCoord || minY == rod.yCoord
            || maxY == rod.yCoord || minZ == rod.zCoord || maxZ == rod.zCoord)
            this.recalcLimits();
    }
    ...
    public TileEntityControlRod
    getControlRodAtRelativePosition(World world, int x, int y, int z) {
        return controls.get(new WorldLocation(
            world, x + controller.xCoord, y + controller.yCoord, z + controller.zCoord
        ));
    }

    public TileEntityControlRod
    getControlRodAtAbsolutePosition(World world, int x, int y, int z) {
    ...
    public int getDisplayColorAtRelativePosition(World world, int x, int y, int z) {
        ...
        if (rod != null) {
            if (((TileEntityCPU) controller.getTileEntity(world)).getPower()
                >= this.getMinPower())
