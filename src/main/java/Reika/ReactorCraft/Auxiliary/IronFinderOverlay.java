...
public class IronFinderOverlay {
    ...
    public void renderFinderArrow(RenderGameOverlayEvent evt) {
        if (evt.type == ElementType.HELMET) {
            ...
            boolean render
                = ReactorItems.IRONFINDER.matchWith(ep.getCurrentEquippedItem())
                || (ep.getEntityData().hasKey("ironfinder")
                    && ep.getEntityData().getLong("ironfinder")
            ...
            if (render) {
                ...
                for (Coordinate c : map) {
                    ...
                    if (b instanceof BlockAir)
                        continue;
                    IIcon[] icons
                        = new IIcon[] { b.getIcon(1, c.getBlockMetadata(ep.worldObj)) };
                    if (ModList.MIMICRY.isLoaded()
                        && b == MimicryHandler.getInstance().oreID) {
                        icons = new IIcon[] { icons[0], this.getMimichiteOreOverlay(b) };
                    } else if (b instanceof MagneticOreOverride) {
                        icons = ((MagneticOreOverride) b)
                                    .getRenderIcons(
                                        ep.worldObj, c.xCoord, c.yCoord, c.zCoord
                                    );
                    }
                    ...
                    if (arel < 0)
                        arel += 360;
                    //ReikaJavaLibrary.pConsole(arel, c.zCoord == 1184 && c.xCoord ==
                    //-1047); ReikaJavaLibrary.pConsole(prel, c.zCoord == 1184 && c.xCoord
                    //== -1047);
                    double phi = arel - yaw;
                    double theta = prel - pitch;
                    if (phi < 0)
                        phi += 360;
                    //ReikaJavaLibrary.pConsole(phi, c.zCoord == 1184 && c.xCoord ==
                    //-1047); ReikaJavaLibrary.pConsole(theta, c.zCoord == 1184 &&
                    //c.xCoord == -1047);
                    ...
                    if (phi >= 180 && 360 - fov > phi) {
                        ...
                        //ReikaJavaLibrary.pConsole(cx, c.zCoord == 1184 && c.xCoord ==
                        //-1047);
                ...
                /*
                    ...
                    for (CrystalElement e : right) {
                        ...
                        int y = (int)(h+(i-left.size()/2D)*f.FONT_HEIGHT);
                        f.drawString(s,
                   evt.resolution.getScaledWidth()-24-f.getStringWidth(s), y,
                   e.getColor()); i++;
                    }*/
            }
        }
