...
public final class ReactorDescriptions {
    ...
    private static HashMap<ReactorBook, String> notes
        = new HashMap<ReactorBook, String>();

    private static HashMap<ReactorTiles, Object[]> machineData
        = new HashMap<ReactorTiles, Object[]>();
    private static HashMap<ReactorTiles, Object[]> machineNotes
        = new HashMap<ReactorTiles, Object[]>();
    private static HashMap<ReactorBook, Object[]> miscData
        = new HashMap<ReactorBook, Object[]>();
    ...
    private static boolean hasLocalizedFor(Language language) {
        String lang = language.getLanguageCode();
        try (
            InputStream o = ReactorCraft.class.getResourceAsStream(
    ...
    public static void loadData() {
        ...
        for (int i = 0; i < parenttabs.size(); i++) {
            ...
            String desc = parents.getValueAtNode(
                "categories:" + h.name().toLowerCase(Locale.ENGLISH)
            );
            addEntry(h, desc);
        }
    ...
    private static void loadNumericalData() {
        ...
        addNotes(
            ReactorTiles.ELECTROLYZER,
            /*TileEntityElectrolyzer.SALTPOWER, */ TileEntityElectrolyzer.SALT_MELT
        );
        addNotes(ReactorTiles.SYNTHESIZER, TileEntitySynthesizer.AMMONIATEMP);
        addNotes(
            ReactorTiles.TURBINECORE,
            TileEntityTurbineCore.GEN_OMEGA,
            TileEntityTurbineCore.TORQUE_CAP
        );
        addNotes(ReactorTiles.BIGTURBINE, TileEntityHiPTurbine.GEN_OMEGA);
        addNotes(
            ReactorTiles.PUMP,
            TileEntityReactorPump.MINPOWER,
            TileEntityReactorPump.MINTORQUE
        );
        addNotes(
            ReactorTiles.EXCHANGER,
            TileEntityHeatExchanger.MINPOWER,
            TileEntityHeatExchanger.MINSPEED
        );
        ...
        addNotes(
            ReactorTiles.SOLENOID,
            TileEntitySolenoidMagnet.MINOMEGA,
            TileEntitySolenoidMagnet.MINTORQUE,
            TileEntitySolenoidMagnet.MAX_SPEED
        );
        //addData(ReactorTiles.HEAVYPUMP, TileEntityHeavyPump.MINDEPTH,
        //TileEntityHeavyPump.MAXY);
        addData(ReactorTiles.BOILER, TileEntityReactorBoiler.DETTEMP);
        addData(
            ReactorTiles.FLUIDEXTRACTOR,
            TileEntityHeavyPump.HeavyWaterExtraction.MAXY,
            TileEntityHeavyPump.HeavyWaterExtraction.MINDEPTH
        );
        addNotes(
            ReactorTiles.FLUIDEXTRACTOR,
            TileEntityHeavyPump.MINPOWER,
            TileEntityHeavyPump.MINTORQUE
        );
        addNotes(
            ReactorTiles.TURBINEMETER,
            TileEntityTurbineCore.GEN_OMEGA,
            TileEntityHiPTurbine.GEN_OMEGA
        );
        ...
        addNotes(
            ReactorTiles.SOLAR,
            TileEntitySolarExchanger.MINPOWER,
            TileEntitySolarExchanger.MINSPEED
        );
