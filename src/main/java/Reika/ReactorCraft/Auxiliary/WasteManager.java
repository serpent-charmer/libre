...
public class WasteManager {
    ...
    public static ItemStack getWaste(Isotopes atom) {
        return ReactorItems.WASTE.getStackOfMetadata(
            /*wastes.indexOf(atom)*/ atom.ordinal()
        );
    }
    ...
    private static HashMap<ElementGroup, Float> getChancesByGroup() {
        ...
        for (Isotopes i : thoriumWastes) {
            ...
            if (get == null)
                get = 0F;
            float ch = (float
            ) (100F * thoriumYields.getWeight(i) / thoriumYields.getTotalWeight());
            get += ch;
            total += ch;
    ...
    public static ChancedOutputList getThoriumGroupOutputs(ElementGroup g) {
        ...
        for (Isotopes i : thoriumWastes) {
            if (i.group == g) {
                float ch = (float
                ) (100F * f * thoriumYields.getWeight(i) / thoriumYields.getTotalWeight()
                );
                if (ch > 0) {
                    ItemStack is = getWaste(i);
                    //ReikaJavaLibrary.pConsole("Adding "+i.getDisplayName()+" to
                    //"+g.displayName+" with chance "+ch+" = "+is);
                    c.addItem(is, ch);
                }
            }
        }
