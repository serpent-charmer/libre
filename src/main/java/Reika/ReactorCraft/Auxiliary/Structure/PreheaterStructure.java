...
public class PreheaterStructure extends ReactorStructureBase {
    ...
    public FilledBlockArray getArray(World world, int x, int y, int z) {
            ...
            for (int k = 0; k < 5; k++) {
                for (int h = 0; h < 5; h++) {
                    ...
                    if (h > 0 && !edge) {
                        ...
                        if (i == 2 && k == 2 && h >= 2) {
                            ReactorTiles r
                                = h > 2 ? ReactorTiles.MAGNETPIPE : ReactorTiles.HEATER;
                            array.setBlock(
                                x + i, y + h, z + k, r.getBlock(), r.getBlockMetadata()
                            );
                        }
                    }
                }
        ...
        array.setBlock(
            ...
            y + 5,
            z + 2,
            ReactorTiles.MAGNETPIPE.getBlock(),
            ReactorTiles.MAGNETPIPE.getBlockMetadata()
        );
        array.setBlock(
            x + 2,
            y + 6,
        ...
        for (int i = 0; i < 5; i++) {
            if (i != 2)
                array.setBlock(
                    ...
                    y + 2,
                    z + 2,
                    MachineRegistry.PIPE.getBlock(),
                    MachineRegistry.PIPE.getBlockMetadata()
                );
        }
