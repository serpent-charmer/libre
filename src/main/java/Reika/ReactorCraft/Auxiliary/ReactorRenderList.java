...
public class ReactorRenderList {
    private static HashMap<ReactorTiles, ReactorRenderBase> renders
        = new HashMap<ReactorTiles, ReactorRenderBase>();
    private static HashMap<ReactorTiles, ReactorTiles> overrides
        = new HashMap<ReactorTiles, ReactorTiles>();
    ...
    public static TileEntitySpecialRenderer instantiateRenderer(ReactorTiles m) {
        try {
            ReactorRenderBase r
                = (ReactorRenderBase) Class.forName(m.getRenderer()).newInstance();
            if (addRender(m, r))
                return r;
            else
                return renders.get(overrides.get(m));
