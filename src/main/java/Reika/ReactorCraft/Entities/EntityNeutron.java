...
public class EntityNeutron extends ParticleEntity implements IEntityAdditionalSpawnData {
    ...
    public static void initTransparencyBlocks() {
        botaniaPlatform = ModList.BOTANIA.isLoaded()
            ? GameRegistry.findBlock(ModList.BOTANIA.modLabel, "platform")
            : null;
        ttPlatform = ModList.THAUMICTINKER.isLoaded()
            ? GameRegistry.findBlock(ModList.THAUMICTINKER.modLabel, "platform")
            : null;
    }

    public EntityNeutron(
        World world, int x, int y, int z, ForgeDirection f, NeutronType type
    ) {
        super(world, x, y, z, f);
        height = 1;
        this.type = type;
    ...
    public void applyEntityCollision(Entity e) {
        if (ReikaRandomHelper.doWithChance(12.5)) {
            if (e instanceof EntityLivingBase) {
                RadiationEffects.instance.applyPulseEffects(
                    (EntityLivingBase) e, RadiationIntensity.MODERATE
                );
                this.setDead();
            }
        }
        ...
        if (!this.isNeutronTransparent(id)) {
            if (id.hasTileEntity(meta)) {
                ...
                if (te instanceof NeutronTile) {
                    ...
                    if (tgt != null) {
                        //this.setPosition(tgt.xCoord+0.5, tgt.yCoord+0.5,
                        //tgt.zCoord+0.5);
                        if (rand.nextInt(2) == 0) {
                            //this.setDead(); //negates need for shielding
                        } else {
                            this.setLocationAndAngles(
                                tgt.xCoord + 0.5, tgt.yCoord + 0.5, tgt.zCoord + 0.5, 0, 0
                            );
                            if (tgt.dimensionID != worldObj.provider.dimensionId
                                && !worldObj.isRemote)
                                ReikaEntityHelper.transferEntityToDimension(
                                    this,
                                    tgt.dimensionID,
                                    new BasicTeleporter((WorldServer) tgt.getWorld())
                                );
                        }
                    }
                }
            ...
            if (id instanceof NeutronBlock) {
                ...
                double c = Math.min(
                    ns.getAbsorptionChance(type),
                    RadiationShield.BEDINGOT.neutronAbsorbChance
                );
                boolean flag = ReikaRandomHelper.doWithChance(c);
                if (flag) {
                    double c2 = MathHelper.clamp_double(
                        ns.getRadiationSpawnMultiplier(world, x, y, z, type), 0, 1
                    );
            ...
            if ((id == ReactorBlocks.FLUORITE.getBlockInstance()
                 || id == ReactorBlocks.FLUORITEORE.getBlockInstance())
                && meta < FluoriteTypes.colorList.length) {
            ...
            if (ReikaRandomHelper.doWithChance(speed.getIrradiatedAbsorptionChance())) {
                boolean flag = id.isOpaqueCube()
                    ? (rand.nextBoolean()
                       && id.getExplosionResistance(null, world, x, y, z, x, y, z) >= 12)
                        || ReikaRandomHelper.getSafeRandomInt((int
                           ) (24
                              - id.getExplosionResistance(null, world, x, y, z, x, y, z)))
                            == 0
                    : 255 - id.getLightOpacity(world, x, y, z) == 0
                    ? ReikaRandomHelper.getSafeRandomInt(
                          id.getLightOpacity(world, x, y, z)
                      ) > 0
                    : rand.nextInt(1000) == 0;
                if (flag) {
                    this.spawnRadiationChance(world, x, y, z);
                    if (ReikaRandomHelper.doWithChance(20))
                        RadiationEffects.instance.transformBlock(
                            world, x, y, z, RadiationIntensity.MODERATE
                        );
                    return true;
                }
            }
    ...
    private void spawnRadiationChance(World world, int x, int y, int z) {
        if (ReikaRandomHelper.doWithChance(2)) {
            ...
            if (inbox.size() < 3)
                RadiationEffects.instance.contaminateArea(
                    world, x, y, z, 1, 1, 0, false, RadiationIntensity.LOWLEVEL
                );
        }
    }
    ...
    public static enum NeutronType {
        ...
        public boolean canTriggerFission() {
            return this.isFissionType()
                || (this == WASTE && ReikaRandomHelper.doWithChance(40));
        }
