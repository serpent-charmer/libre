...
public final class EntityNuclearWaste extends EntityItem {
        ...
        if (posY < 0) {
            ...
            if (timer % 256 == 0) {
                ...
                List<EntityRadiation> li
                    = worldObj.getEntitiesWithinAABB(EntityRadiation.class, box);
                if (li.size() < 100) {
    ...
    private void applyRadiation() {
        ...
        //Contaminate the area slightly every 10 min left in the world, after the first 3
        //minutes
        if ((timer - RADIATION_DELAY) % RADIATION_INTERVAL == 0
            && timer >= RADIATION_DELAY) {
            AxisAlignedBB.getBoundingBox(posX, posY, posZ, posX, posY, posZ)
                .expand(12, 8, 12);
            List<EntityRadiation> near
            ...
            if (near.size() < 32) {
                RadiationEffects.instance.contaminateArea(
                    ...
                    false,
                    RadiationIntensity.HIGHLEVEL
                ); //no LOS to simulate groundwater/air particulates
            }
        }
    }
