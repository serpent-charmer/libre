...
public class EntityFusion extends InertEntity {
    ...
    public EntityFusion(World world, double x, double y, double z, String creator) {
        ...
        //world.setBlock(MathHelper.floor_double(x), MathHelper.floor_double(y)+1,
        //MathHelper.floor_double(z), 51); worldObj.playSoundEffect(posX, posY, posZ,
        //"random.explode", 0.2F, 0.1F);
        for (int i = 0; i < 3; i++)
            this.spawnNeutrons(
                worldObj,
        ...
        if (creator != null && !creator.isEmpty())
            ReactorAchievements.FUSION.triggerAchievement(
                world.getPlayerEntityByName(creator)
            );
    }
    ...
    private void spawnNeutrons(World world, int x, int y, int z) {
        EntityNeutron e = new EntityNeutron(
            world, x, y, z, this.getRandomDirection(), NeutronType.FUSION
        );
        if (!world.isRemote)
