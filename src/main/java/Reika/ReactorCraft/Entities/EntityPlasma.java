...
public class EntityPlasma extends ParticleEntity implements CustomFanEntity {
    ...
    private void checkFusion() {
        ...
        if (li.size() >= this.getFusionThreshold() && !li.get(0).hasEscaped()
            && !li.get(li.size() - 1).hasEscaped()) {
            EntityFusion fus
                = new EntityFusion(worldObj, posX, posY, posZ, placerOfInjector);
    ...
    public void applyEntityCollision(Entity e) {
        int dmg = e instanceof EntityLivingBase
                && ((EntityLivingBase) e).isPotionActive(Potion.fireResistance)
            ? 4
            : Integer.MAX_VALUE;
    ...
    protected void onTick() {
        if (ticksExisted > 1200)
            ; //this.setDead();
        if (!worldObj.isRemote && !this.hasEscapedSeverely()
            && rand.nextInt(this.hasEscaped() ? 48 : 12) == 0)
            this.checkFusion();
        motionY = 0;
