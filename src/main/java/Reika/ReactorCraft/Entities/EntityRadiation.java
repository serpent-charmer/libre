...
public class EntityRadiation extends InertEntity implements IEntityAdditionalSpawnData {
        ...
        if (this.rainCleanable()) {
            if (worldObj.isRaining()) {
                if (rand.nextInt(36000) == 0) {
                    if (worldObj.getBiomeGenForCoords(this.getBlockX(), this.getBlockZ())
    ...
    protected void applyRadiation() {
        ...
        int dx = ReikaRandomHelper.getRandomPlusMinus(
            MathHelper.floor_double(x), effectRange
        );
        int dy = ReikaRandomHelper.getRandomPlusMinus(
            MathHelper.floor_double(y), effectRange
        );
        int dz = ReikaRandomHelper.getRandomPlusMinus(
            MathHelper.floor_double(z), effectRange
        );
