...
public enum ReactorFuel {
    ...
    public ItemStack getFissionProduct(ItemStack input) {
        ...
        switch (this) {
            case PLUTONIUM:
                if (input.getItemDamage()
                    == ReactorItems.PLUTONIUM.getNumberMetadatas() - 1)
                    return null;
                else
                    return ReactorItems.PLUTONIUM.getStackOfMetadata(
            ...
            case THORIUM:
                if (input.getItemDamage() == ReactorItems.THORIUM.getNumberMetadatas()-1)
                    ...
                    return
            ReactorItems.THORIUM.getStackOfMetadata(input.getItemDamage()+1);
                 */
            case URANIUM:
                if (input.getItemDamage() == ReactorItems.FUEL.getNumberMetadatas() - 1)
                    return ReactorItems.DEPLETED.getStackOf();
                else
                    return ReactorItems.FUEL.getStackOfMetadata(
                        input.getItemDamage() + 1
                    );
        }
        return null;
