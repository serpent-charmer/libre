...
public enum ReactorStructures implements StructureEnum {
    ...
    public StructureRenderer getRenderer() {
        if (render == null || Keyboard.isKeyDown(Keyboard.KEY_LCONTROL)) {
            FilledBlockArray f = this.getStructure(
                Minecraft.getMinecraft().theWorld, 0, 0, 0, ForgeDirection.EAST
            );
    ...
    private void addOverrides() {
        render.addOverride(
            new ItemStack(tile.getBlock(), 1, tile.getBlockMetadata()),
            tile.getCraftedProduct()
        );

        render.addOverride(
            new ItemStack(
                ReactorTiles.STEAMLINE.getBlock(),
                1,
                ReactorTiles.STEAMLINE.getBlockMetadata()
            ),
            ReactorTiles.STEAMLINE.getCraftedProduct()
        );
    }

    public FilledBlockArray
    ...
    public String getName() {
        return StatCollector.translateToLocal(
            "reactorstruct." + this.name().toLowerCase(Locale.ENGLISH)
        );
    }
