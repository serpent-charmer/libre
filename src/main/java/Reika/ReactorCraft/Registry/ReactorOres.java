...
public enum ReactorOres implements OreEnum {
    ...
    CADMIUM(
        12, 32, 9, 3, 0, 2, 0.7F, "ore.cadmium", ReactorOptions.CADMIUMORE.getState()
    ),
    ...
    CALCITE(
        32, 60, 4, 12, 0, 0, 0.4F, "ore.calcite", ReactorOptions.CALCITEORE.getState()
    ),
    MAGNETITE(
        60, 128, 16, 7, 0, 1, 0.8F, "ore.magnetite", ReactorOptions.MAGNETORE.getState()
    ),
    ...
    private static final EnumMap<ReactorOres, Boolean> equivalents
        = new EnumMap(ReactorOres.class);

    private ReactorOres(
        int min, int max, int size, int count, int dim, int level, float xp, String name
    ) {
    ...
    public String toString() {
        return this.name() + " " + perChunk + "x" + veinSize + " between " + minY
            + " and " + maxY;
    }
    ...
    public ItemStack getProduct() {
        switch (this) {
            case FLUORITE:
                return ReactorItems.FLUORITE.getStackOfMetadata(
                    FluoriteTypes.WHITE.ordinal()
                );
            case ENDBLENDE:
        ...
        switch (this) {
            case FLUORITE:
                return ReikaJavaLibrary.makeListFrom(
                    ReactorItems.FLUORITE.getStackOfMetadata(meta)
                );
            case CALCITE:
                return ReikaJavaLibrary.makeListFrom(ReactorStacks.calcite.copy());
            case AMMONIUM:
                return ReikaJavaLibrary.makeListFrom(
                    ReactorStacks.ammonium.copy(), new ItemStack(Blocks.netherrack)
                );
            case MAGNETITE:
                return ReikaJavaLibrary.makeListFrom(ReactorStacks.lodestone.copy());
            default:
                return ReikaJavaLibrary.makeListFrom(
                    new ItemStack(ReactorBlocks.ORE.getBlockInstance(), 1, meta)
                );
        }
    }
        ...
        switch (this) {
            ...
            default:
                return StatCollector.translateToLocal(
                    "item." + this.name().toLowerCase(Locale.ENGLISH)
                );
        }
    }
        ...
        switch (this) {
            case PITCHBLENDE:
                ...
                if (biome == BiomeGenBase.river || biome == BiomeGenBase.ocean
                    ...
                    || biome == BiomeGenBase.mushroomIsland
    ...
    public boolean canGenerateInChunk(World world, int chunkX, int chunkZ) {
        ...
        if (id == ExtraUtilsHandler.getInstance().darkID)
            return true;
        return this.isValidBiome(world.getBiomeGenForCoords(chunkX << 4, chunkZ << 4))
            || id == ReikaTwilightHelper.getDimensionID();
    }

    private boolean shouldGen() {
        if (shouldGen)
            return true;
        if (ModList.CONDENSEDORES.isLoaded()
            && CondensedOreAPI.instance.doesBlockGenerate(
                this.getBlock(), this.getBlockMetadata()
            ))
            return false;
    ...
    public boolean canGenAt(World world, int x, int y, int z) {
        if (this == AMMONIUM)
            return ReikaWorldHelper.checkForAdjBlock(world, x, y, z, Blocks.lava) != null
                || ReikaWorldHelper.checkForAdjBlock(world, x, y, z, Blocks.flowing_lava)
                != null;
        return true;
    }
    ...
    public boolean dropsSelf(int meta) {
        ...
        return li.size() == 1
            && li.get(0).getItem()
            == Item.getItemFromBlock(ReactorBlocks.ORE.getBlockInstance())
            && li.get(0).getItemDamage() == meta;
    }
