...
public enum ReactorAchievements {
    ...
    FIFTYGW(
        -6, 2, MachineRegistry.DYNAMOMETER.getCraftedProduct(), FUSION, true
    ), //per reactor
    ...
    private ReactorAchievements(
        int x, int y, Item icon, ReactorAchievements preReq, boolean special
    ) {
        this(x, y, new ItemStack(icon), preReq, special);
    }

    private ReactorAchievements(
        int x, int y, Block icon, ReactorAchievements preReq, boolean special
    ) {
        this(x, y, new ItemStack(icon), preReq, special);
    }

    private ReactorAchievements(
        int x, int y, ReactorItems icon, ReactorAchievements preReq, boolean special
    ) {
        this(x, y, icon.getStackOf(), preReq, special);
    }

    private ReactorAchievements(
        int x, int y, ReactorTiles icon, ReactorAchievements preReq, boolean special
    ) {
        this(x, y, icon.getCraftedProduct(), preReq, special);
    }

    private ReactorAchievements(
        int x, int y, ItemStack icon, ReactorAchievements preReq, boolean special
    ) {
        xPosition = x;
        yPosition = y;
        dependency = preReq;
    ...
    public static void registerAchievements() {
        ...
        for (int i = 0; i < list.length; i++) {
            ...
            //	throw new IDConflictException(ReactorCraft.instance, "The mod's
            //achievement IDs are conflicting with another at ID "+id+" ("+a+" is trying
            //to overwrite "+StatList.getOneShotStat(id).statName+").\nCheck the config
        ...
        AchievementPage.registerAchievementPage(
            new ReactorAchievementPage("ReactorCraft", ReactorCraft.achievements)
        );
    }
}
