...
public class ElectrolyzerHandler extends TemplateRecipeHandler {
    public class ElectrolyzerRecipe extends CachedRecipe {
        ...
        public PositionedStack getIngredient() {
            return recipe.hasItemRequirement()
                ? new PositionedStack(recipe.getItemListForDisplay(), 39, 30)
                : null;
        }
    }
    ...
    public void drawExtras(int recipe) {
        ...
        Fluid o1
            = er.recipe.lowerOutput != null ? er.recipe.lowerOutput.getFluid() : null;
        Fluid o2
            = er.recipe.upperOutput != null ? er.recipe.upperOutput.getFluid() : null;

        ReikaTextureHelper.bindTerrainTexture();
        if (o1 != null)
            ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                93, 7, ReikaLiquidRenderer.getFluidIconSafe(o1), 16, 60
            );
        if (o2 != null)
            ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                129, 7, ReikaLiquidRenderer.getFluidIconSafe(o2), 16, 60
            );
        ...
        if (er.recipe.requiredFluid != null) {
            ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                12,
                7,
                ReikaLiquidRenderer.getFluidIconSafe(er.recipe.requiredFluid.getFluid()),
                16,
                60
            );
        }
        ...
        if (er.recipe.consumeItem) {
            ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                Minecraft.getMinecraft().fontRenderer, "(Use)", 48, 51, 0x000000
            );
        } else {
            ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                Minecraft.getMinecraft().fontRenderer, "(Cat)", 48, 51, 0x000000
            );
        }
    }
