...
public class UProcessorHandler extends TemplateRecipeHandler {
    public class UProcessorRecipe extends CachedRecipe {
        ...
        public List<PositionedStack> getIngredients() {
            ...
            meta = (int
            ) ((System.nanoTime() / 1000000000) % FluoriteTypes.colorList.length);
    ...
    public void drawExtras(int recipe) {
        ...
        if (f != null)
            ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                93, 7, ReikaLiquidRenderer.getFluidIconSafe(f), 16, 60
            );
        f = p.intermediateFluid;
        if (f != null)
            ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                93 + 18, 7, ReikaLiquidRenderer.getFluidIconSafe(f), 16, 60
            );
        f = p.outputFluid;
        if (f != null)
            ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                93 + 36, 7, ReikaLiquidRenderer.getFluidIconSafe(f), 16, 60
            );
    }
}
