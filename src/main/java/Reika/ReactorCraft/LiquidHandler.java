...
public class LiquidHandler {
    ...
    public boolean isCorrodable(World world, int x, int y, int z) {
        ...
        if (id == BCPipeHandler.getInstance().pipeID) {
            BCPipeHandler.Materials type
                = BCPipeHandler.getInstance().getPipeType(world.getTileEntity(x, y, z));
            if (type == BCPipeHandler.Materials.GOLD
                || type == BCPipeHandler.Materials.IRON)
                return true;
        }
    ...
    public boolean isExplodable(World world, int x, int y, int z) {
        ...
        if (id == BCPipeHandler.getInstance().pipeID) {
            BCPipeHandler.Materials type
                = BCPipeHandler.getInstance().getPipeType(world.getTileEntity(x, y, z));
            if (type == BCPipeHandler.Materials.DIAMOND
                || type == BCPipeHandler.Materials.EMERALD)
                return true;
        }
        return false;
    }
    ...
    private PipeReactions getReaction(Fluid f, World world, int x, int y, int z) {
        if (f == null)
            ...
            return world.getTileEntity(x, y, z) instanceof TileEntityReactorBase
                ? null
                : PipeReactions.MELT;
            ...
            switch (this) {
                        ...
                        for (int dy = c.yCoord - r; dy <= c.yCoord + r; dy++) {
                            for (int dz = c.zCoord - r; dz <= c.zCoord + r; dz++) {
                                ReikaWorldHelper.temperatureEnvironment(
                                    ...
                                    c.yCoord,
                                    c.zCoord,
                                    TileEntityFusionHeater.PLASMA_TEMP
                                );
                            }
                        }
                    }
                ...
                case CORRODE:
                    ...
                    ReikaParticleHelper.SMOKE.spawnAroundBlock(
                        world, c.xCoord, c.yCoord, c.zCoord, 6
                    );
