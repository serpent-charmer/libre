...
public class BlockReactorTile extends BlockTileEnum<TileEntityReactorBase, ReactorTiles>
    implements IWailaDataProvider {
    protected static final IIcon[][][] icons
        = new IIcon[ReactorTiles.TEList.length][6][16];
        ...
        if (te instanceof TileEntityNuclearCore) {
            ...
            if (tile.isActive()) {
                if (ReikaWorldHelper.isSubmerged(iba, x, y, z)) {
                    return ModList.COLORLIGHT.isLoaded()
                        ? ReikaColorAPI.getPackedIntForColoredLight(0x0077ff, 15)
                        : 15;
                }
            }
                ...
                if (r.hasTextureStates()) {
                    for (int k = 0; k < r.getTextureStates(); k++) {
                        if (r.hasSidedTextures()) {
                            for (int j = 0; j < 6; j++) {
                                icons[i][j][k] = ico.registerIcon(
                                    "ReactorCraft:" + r.name().toLowerCase(Locale.ENGLISH)
                                    + "_" + j + "_#" + k
                                );
                            }
                        } else if (r.isEndTextured()) {
                            for (int j = 0; j < 2; j++) {
                                String s = "ReactorCraft:"
                                    + r.name().toLowerCase(Locale.ENGLISH) + "_top"
                                    + "_#" + k;
                                if (j == 0 && !r.isTopSameTextureAsBottom()) {
                                    s = "ReactorCraft:"
                                        + r.name().toLowerCase(Locale.ENGLISH) + "_bottom"
                                        + "_#" + k;
                                }
                    ...
                    if (r.hasSidedTextures()) {
                        ...
                        for (int j = 0; j < 2; j++) {
                            String s = "ReactorCraft:"
                                + r.name().toLowerCase(Locale.ENGLISH) + "_top";
                            if (j == 0 && !r.isTopSameTextureAsBottom()) {
                                s = "ReactorCraft:" + r.name().toLowerCase(Locale.ENGLISH)
                                    + "_bottom";
                            }
    ...
    public boolean onBlockActivated(
        ...
        if (is != null) {
            ...
            if ((i == ItemRegistry.METER || RotaryAux.isHoldingScrewdriver(ep))
                && r == ReactorTiles.MAGNET)
        ...
        if (r == ReactorTiles.SYNTHESIZER && is != null && is.stackSize == 1) {
            TileEntitySynthesizer te
                = (TileEntitySynthesizer) world.getTileEntity(x, y, z);
            if (is.getItem() == Items.water_bucket) {
        ...
        if (r == ReactorTiles.ELECTROLYZER && is != null && is.stackSize == 1) {
            TileEntityElectrolyzer te
                = (TileEntityElectrolyzer) world.getTileEntity(x, y, z);
        ...
        if (r == ReactorTiles.FLUIDEXTRACTOR && is != null && is.stackSize == 1) {
            ...
            if (te.hasABucket()) {
                if (is.getItem() == Items.bucket
                    && te.getFluid() == FluidRegistry.getFluid("rc heavy water")) {
                    te.subtractBucket();
                    ep.setCurrentItemOrArmor(0, ReactorItems.BUCKET.getStackOf());
                    return true;
                }
        ...
        if (r == ReactorTiles.SODIUMBOILER && is != null && is.stackSize == 1) {
            TileEntitySodiumHeater te
                = (TileEntitySodiumHeater) world.getTileEntity(x, y, z);
            if (te.getLevel() + FluidContainerRegistry.BUCKET_VOLUME
                <= te.getCapacity()) {
                if (ReikaItemHelper.matchStacks(is, ReactorStacks.nacan)) {
                    if (te.getLevel() <= 0
                        || te.getContainedFluid().equals(
                            FluidRegistry.getFluid("rc sodium")
                        )) {
                        te.addLiquid(
        ...
        if (r == ReactorTiles.BOILER && is != null && is.stackSize == 1) {
            TileEntityReactorBoiler te
                = (TileEntityReactorBoiler) world.getTileEntity(x, y, z);
            if (te.getLevel() + FluidContainerRegistry.BUCKET_VOLUME
                <= te.getCapacity()) {
                if (is.getItem() == Items.water_bucket) {
                    if (te.getLevel() <= 0
                        || te.getContainedFluid().equals(FluidRegistry.WATER)) {
                        te.addLiquid(
                            FluidContainerRegistry.BUCKET_VOLUME, FluidRegistry.WATER
                        );
                        if (!ep.capabilities.isCreativeMode)
                            ep.setCurrentItemOrArmor(0, new ItemStack(Items.bucket));
                    }
                ...
                if (ReikaItemHelper.matchStacks(is, ReactorStacks.nh3can)) {
                    if (te.getLevel() <= 0
                        || te.getContainedFluid().equals(
                            FluidRegistry.getFluid("rc ammonia")
                        )) {
                        te.addLiquid(
                            FluidContainerRegistry.BUCKET_VOLUME,
                            FluidRegistry.getFluid("rc ammonia")
        ...
        if (r == ReactorTiles.PROCESSOR && is != null
            && is.getItem() == ReactorItems.CANISTER.getItemInstance()
            && is.stackSize == 1) {
            TileEntityUProcessor te = (TileEntityUProcessor) world.getTileEntity(x, y, z);
            if (is.getItemDamage() == ReactorStacks.emptycan.getItemDamage()
                && te.getOutput() >= FluidContainerRegistry.BUCKET_VOLUME) {
                if (!ep.capabilities.isCreativeMode)
                    ep.setCurrentItemOrArmor(
                        0,
                        /*te.getOutputFluid() == FluidRegistry.getFluid("rc uranium
                           hexafluoride") ? ReactorStacks.uf6can.copy() :
                           ReactorStacks.lifbecan.copy()*/
                        ReikaFluidHelper.getAllContainersFor(te.getOutputFluid())
                            .get(0)
                    );
                ...
                te.addIntermediate(
                    FluidContainerRegistry.BUCKET_VOLUME,
                    FluidRegistry.getFluid("rc hydrofluoric acid")
                );
            }
            return true;
        }
        if (r == ReactorTiles.CENTRIFUGE && is != null
            && is.getItem() == ReactorItems.CANISTER.getItemInstance()
            && is.stackSize == 1) {
            TileEntityCentrifuge te = (TileEntityCentrifuge) world.getTileEntity(x, y, z);
            if (is.getItemDamage() == ReactorStacks.emptycan.getItemDamage()
                && te.getUF6() >= FluidContainerRegistry.BUCKET_VOLUME) {
                if (!ep.capabilities.isCreativeMode)
        ...
        if (r.isTurbine()) {
            if (is != null && is.stackSize == 1
                && ReikaItemHelper.matchStacks(is, ItemStacks.lubebucket)) {
                TileEntityTurbineCore te
                    = (TileEntityTurbineCore) world.getTileEntity(x, y, z);
                int amt = 1000;
    ...
    @Override
    public ItemStack
    getPickBlock(MovingObjectPosition vec, World world, int x, int y, int z) {
        ...
        if (te instanceof TileEntitySolenoidMagnet) {
            ...
            if (id == ReactorBlocks.SOLENOIDMULTI.getBlockInstance()) {
                BlockSolenoidMulti b
                    = (BlockSolenoidMulti) ReactorBlocks.SOLENOIDMULTI.getBlockInstance();
    ...
    @Override
    public boolean
    removedByPlayer(World world, EntityPlayer player, int x, int y, int z, boolean harv) {
        if (!player.capabilities.isCreativeMode
            && this.canHarvest(world, player, x, y, z))
    ...
    harvestBlock(World world, EntityPlayer ep, int x, int y, int z, int meta) {
        ...
        if (world.getBlock(x, y, z) == this)
            ReikaItemHelper.dropItems(
                world, x + 0.5, y + 0.5, z + 0.5, this.getDrops(world, x, y, z, meta, 0)
            );
    }
        ...
        if (r != null) {
            if (r.isTurbine()) {
                ...
                if (te.getDamage() > 0) {
                    ...
                    for (int i = 0;
                         i < ReikaRandomHelper.getSafeRandomInt(24 - te.getDamage());
                         i++) {
                    ...
                    if (te.getLubricant() > 0) {
                        ...
                        int lube = ReikaMathLibrary.roundDownToX(
                            10, te.getLubricantToDrop()
                        ); //to help with stacking
    ...
    getWailaStack(IWailaDataAccessor accessor, IWailaConfigHandler config) {
        return null; //ReactorTiles.getMachineFromIDandMetadata(this,
    ...
    public List<String> getWailaBody(
        ...
        if (te instanceof Temperatured)
            tip.add(
                String.format("Temperature: %dC", ((Temperatured) te).getTemperature())
            );
        else if (te instanceof TemperatureTE)
            tip.add(
                String.format("Temperature: %dC", ((TemperatureTE) te).getTemperature())
            );
        else if (te instanceof ThermalMachine)
            tip.add(
                String.format("Temperature: %dC", ((ThermalMachine) te).getTemperature())
            );
        ...
        if (te instanceof TileEntityToroidMagnet) {
            tip.add(
                String.format("Charge: %dkV", ((TileEntityToroidMagnet) te).getCharge())
            );
            tip.add(
                String.format("Coolant: %dmB", ((TileEntityToroidMagnet) te).getCoolant())
            );
            tip.add(String.format(
                "Direction: %s", ((TileEntityToroidMagnet) te).getAim().toString()
            ));
        }
