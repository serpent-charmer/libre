...
public class BlockThoriumFuel extends BlockFluidFinite {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (dir != ForgeDirection.UP) {
                ...
                if (world.getBlock(dx, dy, dz) == this
                    || this.canOverwrite(world, dx, dy, dz)) {
                    ...
                    if (f > 0) {
                        ...
                        if (rand.nextInt(4) == 0) {
                            world.setBlock(
                                dx, dy, dz, this, world.getBlockMetadata(x, y, z), 3
                            );
                            world.setBlock(x, y, z, Blocks.air);
                            return;
                        }
    ...
    private int
    getFillDifference(World world, int x, int y, int z, int x2, int y2, int z2) {
    ...
    protected void flowIntoBlock(World world, int i, int j, int k, int l) {
        ...
        if (ReikaRandomHelper.doWithChance(0.005))
            RadiationEffects.instance.contaminateArea(world, i,
    j+ReikaRandomHelper.getSafeRandomInt(2), k, 2, 0.25F, 0, false,
    RadiationIntensity.MODERATE);
        //}

        ForgeDirection iceside = ReikaWorldHelper.checkForAdjBlock(world, i, j, k,
    Blocks.ice); ForgeDirection waterside = ReikaWorldHelper.checkForAdjMaterial(world, i,
    j, k, Material.water); if (iceside != null || waterside != null) { if
    (ReikaRandomHelper.doWithChance(15))
                ;//world.setBlock(i, j, k, ReactorBlocks.MATS.getBlock(),
    MatBlocks.SLAG.ordinal(), 3); if (iceside != null) {
                ReikaWorldHelper.changeAdjBlock(world, i, j, k, iceside,
    ...
    private boolean blockBlocksFlow(World world, int x, int y, int z) {
        ...
        if (l != Blocks.wooden_door && l != Blocks.iron_door && l != Blocks.standing_sign
    && l != Blocks.ladder && l != Blocks.reeds) { if (l == Blocks.air) { return false;
            }
            else {
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
        blockIcon = iconRegister.registerIcon("ReactorCraft:fluid/lifbe_fuel"
        ); //icon = new IIcon[]{this.getFluid().getStillIcon(),
    ...
    private void checkForHarden(World world, int x, int y, int z) {
        if (world.getBlock(x, y, z) == this) {
            ...
            if (flag
                || ReikaWorldHelper.getMaterial(world, x, y, z - 1) == Material.water)
                flag = true;
            if (flag
                || ReikaWorldHelper.getMaterial(world, x, y, z + 1) == Material.water)
                flag = true;
            if (flag
                || ReikaWorldHelper.getMaterial(world, x - 1, y, z) == Material.water)
                flag = true;
            if (flag
                || ReikaWorldHelper.getMaterial(world, x + 1, y, z) == Material.water)
                flag = true;
            if (flag
                || ReikaWorldHelper.getMaterial(world, x, y + 1, z) == Material.water)
                flag = true;

            if (flag) {
                ; //world.setBlock(x, y, z, ReactorBlocks.MATS.getBlock(),
                  //MatBlocks.SLAG.ordinal(), 3);
    ...
    public static boolean canOverwrite(World world, int x, int y, int z) {
        ...
        if (n.contains("duct") || n.contains("conduit") || n.contains("cable")
            || n.contains("pipe"))
            return true;
        return false;
    }
