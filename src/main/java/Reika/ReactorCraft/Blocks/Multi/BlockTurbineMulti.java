...
public class BlockTurbineMulti extends BlockReCMultiBlock {
    ...
    public Boolean checkForFullMultiBlock(
        ...
        blocks.recursiveAddWithBounds(
            world, x, y, z, this, x - 12, y - 12, z - 12, x + 12, y + 12, z + 12
        );
        int sx;
        int sz;
        int n = this.checkForTurbines(
            world, x, y, z, dir, blocks
        ); //only accept steam emitter at last turb stage
        if (n <= 0 || n > 7)
            return false;
    ...
    private int checkForTurbines(
        World world, int x, int y, int z, ForgeDirection dir, StructuredBlockArray blocks
    ) {
        int mx = blocks.getMinX() + blocks.getSizeX() / 2;
        int my = blocks.getMinY() + blocks.getSizeY() / 2;
        int mz = blocks.getMinZ() + blocks.getSizeZ() / 2;
        int sx = dir.offsetX == 0 ? mx
            : dir.offsetX < 0     ? blocks.getMaxX()
                                  : blocks.getMinX();
        int sz = dir.offsetZ == 0 ? mz
            : dir.offsetZ < 0     ? blocks.getMaxZ()
                                  : blocks.getMinZ();
        int c = 0;
    ...
    public void breakMultiBlock(World world, int x, int y, int z) {
        ...
        Set<BlockKey> set = ReikaJavaLibrary.getSet(
            new BlockKey(this), new BlockKey(ReactorTiles.BIGTURBINE)
        );
        blocks.recursiveAddMultipleWithBounds(
            world, x, y, z, set, x - 12, y - 12, z - 12, x + 12, y + 12, z + 12
        );
        for (int i = 0; i < blocks.getSize(); i++) {
            ...
            if (b == this) {
                ...
                TileEntityTurbineCore te = (TileEntityTurbineCore
    ...
    protected TileEntity getTileEntityForPosition(World world, int x, int y, int z) {
        ...
        int my = blocks.getMinY() + blocks.getSizeY() / 2;
        int mz = blocks.getMinZ() + blocks.getSizeZ() / 2;
        return ReactorTiles.getTE(world, mx, my, mz) == ReactorTiles.BIGTURBINE
