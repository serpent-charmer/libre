...
public class BlockFluoriteOre extends BlockFluorite {
    ...
    getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        ...
        ReikaWorldHelper.splitAndSpawnXP(
            world, x + 0.5F, y + 0.5F, z + 0.5F, this.droppedXP()
        );
        return li;
    }

    private int getMetaDropped(int metadata) {
        return ReactorOptions.RAINBOW.getState()
            ? rand.nextInt(FluoriteTypes.colorList.length)
            : metadata % 8;
    }

    private int getNumberDrops(int fortune) {
        if (ReactorOptions.RAINBOW.getState()) {
            return FluoriteTypes.colorList.length + fortune * 4
                + rand.nextInt(11 + fortune * 6);
        }
    ...
    private int droppedXP() {
        ...
        return ReikaRandomHelper.doWithChance(ReactorOres.FLUORITE.xpDropped * factor)
            ? 1
            : 0;
    }
    ...
    @Override
    public ItemStack
    getPickBlock(MovingObjectPosition tgt, World world, int x, int y, int z) {
    ...
    ) //may need to call lighting updates on generate
    {
        ...
        return ModList.COLORLIGHT.isLoaded()
            ? ReikaColorAPI.getPackedIntForColoredLight(color, l)
            : l;
    }
