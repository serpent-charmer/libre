...
public class BlockDuct extends BlockReactorTile {
    ...
    public BlockDuct(Material mat) {
        super(mat);
        this.setHardness(
            MathHelper.clamp_float(ConfigRegistry.PIPEHARDNESS.getFloat(), 0, 1)
        );
    ...
    public void onBlockAdded(World world, int x, int y, int z) {
        TileEntityReactorPiping te
            = (TileEntityReactorPiping) world.getTileEntity(x, y, z);
    ...
    getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        AxisAlignedBB box = AxisAlignedBB.getBoundingBox(
            x + dd[4], y + dd[1], z + dd[2], x + 1 - dd[5], y + 1 - dd[0], z + 1 - dd[3]
        );
    ...
    public int getLightValue(IBlockAccess world, int x, int y, int z) {
        TileEntityReactorPiping te
            = (TileEntityReactorPiping) world.getTileEntity(x, y, z);
        return te != null && te.getLevel() > 0 && te.getFluidType() != null
            ? te.getFluidType().getLuminosity(te.worldObj, x, y, z)
            : 0;
    }
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity e) {
        if (TileEntityTurbineCore.canDamageTurbine(e)) {
            if (ReactorTiles.getTE(world, x, y, z) == ReactorTiles.MAGNETPIPE) {
                TileEntityMagneticPipe te
                    = (TileEntityMagneticPipe) world.getTileEntity(x, y, z);
                int charge = te.getCharge();
                if (charge > 0) {
                    double sx = te.getAimX();
                    double sy = te.getAimY();
                    double sz = te.getAimZ();
                    EntityDischarge ed = new EntityDischarge(
                        world,
                        sx,
                        sy,
                        sz,
