...
public class BlockPoisonGas extends BlockFluidClassic {
    ...
    protected void flowIntoBlock(World world, int x, int y, int z, int l) {
        ...
        if (this.liquidCanDisplaceBlock(world, x, y, z)) {
            if (b.getMaterial() == Material.plants || b.getMaterial() == Material.grass
                || b.getMaterial() == Material.vine || b.getMaterial() == Material.wood
                || b.getMaterial() == Material.leaves || b.getMaterial() == Material.cloth
                || b.getMaterial() == Material.gourd) {
    ...
    public void registerBlockIcons(IIconRegister ico) {
        blockIcon = ico.registerIcon(
            "ReactorCraft:fluid/"
            + ReactorBlocks.getBlock(this).name().toLowerCase(Locale.ENGLISH)
        );
    }
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        switch (ReactorBlocks.getBlock(this)) {
            case CHLORINE: {
                if (rand.nextInt(
                        1 + Minecraft.getMinecraft().gameSettings.particleSetting
                    )
                    == 0) {
                    ...
                    ReikaParticleHelper.MOBSPELL.spawnAroundBlock(
                        world, x, y, z, r, g, b, 1
                    );
                }
                break;
            ...
            case HF: {
                ...
                double g = ReikaColorAPI.getGreen(clr) / 255D;
                double b = ReikaColorAPI.getBlue(clr) / 255D;
                ReikaParticleHelper.AMBIENTMOBSPELL.spawnAroundBlock(
                    world, x, y, z, r, g, b, 1
                );
