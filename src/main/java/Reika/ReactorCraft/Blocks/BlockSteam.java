...
public class BlockSteam extends Block {
    ...
    public BlockSteam(Material mat) {
        super(mat);
        this.setCreativeTab(
            ReactorCraft.instance.isLocked() ? null : ReactorCraft.tabRctr
        );
    ...
    private void
    directionalMovement(World world, int x, int y, int z, Random rand, int meta) {
        ForgeDirection dir;
        switch (meta - 4) {
            case 0:
    ...
    private void
    defaultMovement(World world, int x, int y, int z, Random rand, int meta) {
        if (world.getBlock(x, y + 1, z) == ReactorBlocks.MATS.getBlockInstance()
            && world.getBlockMetadata(x, y + 1, z) == MatBlocks.SCRUBBER.ordinal()) {
            world.setBlock(x, y, z, Blocks.air, 0, 2);
            return;
        }
        if (ReactorTiles.getTE(world, x, y + 1, z) == ReactorTiles.TURBINECORE) {
            TileEntityTurbineCore te
                = (TileEntityTurbineCore) world.getTileEntity(x, y + 1, z);
            ...
            if (this.canMoveInto(world, dx, dy, dz)) {
                world.setBlock(
                    dx, dy, dz, this, this.getTransmittedMetadata(meta, dir), 2
                );
                world.setBlock(x, y, z, Blocks.air, 0, 2);
                world.markBlockForUpdate(dx, dy, dz);
            }
                ...
                int ddx = ReikaRandomHelper.getRandomPlusMinus(
                    dx, 1 + 3 * Math.abs(dir.offsetZ)
                );
                int ddz = ReikaRandomHelper.getRandomPlusMinus(
                    dz, 1 + 3 * Math.abs(dir.offsetX)
                ); //only move on Z if was moving along X
                if ((meta & 4) != 0) {
                    int i = 0;
                    while (i <= 20 && !this.canMoveInto(world, dx, dy, dz)) {
                        ddx = ReikaRandomHelper.getRandomPlusMinus(
                            dx, 1 + 3 * Math.abs(dir.offsetZ)
                        );
                        ddz = ReikaRandomHelper.getRandomPlusMinus(
                            dz, 1 + 3 * Math.abs(dir.offsetX)
                        );
                        i++;
                    }
                ...
                if (this.canMoveInto(world, ddx, dy, ddz)) {
                    world.setBlock(
                        ddx, dy, ddz, this, this.getTransmittedMetadata(meta, dir), 2
                    );
                    world.setBlock(x, y, z, Blocks.air, 0, 2);
            ...
            return;
             ...
             TileEntityTurbineCore te = (TileEntityTurbineCore)world.getTileEntity(x+1, y,
         z); ForgeDirection dir = te.getSteamMovement(); int dx = x+dir.offsetX; int dy =
         y+dir.offsetY; int dz = z+dir.offsetZ; if (this.canMoveInto(world, dx, dy, dz)) {
                 world.setBlock(dx, dy, dz, this, this.getTransmittedMetadata(meta, dir),
         3); world.setBlock(x, y, z);
             }
             else if (world.getBlock(dx, dy, dz) ==
         ReactorBlocks.GENERATORMULTI.getBlockInstance() && world.getBlockMetadata(dx, dy,
         dz) == 11) { if (this.canMoveInto(world, te.xCoord+dir.offsetX*2, te.yCoord+3,
         te.zCoord+dir.offsetZ*2)) { world.setBlock(te.xCoord+dir.offsetX*2, te.yCoord+3,
         te.zCoord+dir.offsetZ*2, this, this.getTransmittedMetadata(meta, dir), 3);
         ...
         te.zCoord+dir.offsetZ*2, this, this.getTransmittedMetadata(meta, dir), 3);
             ...
             TileEntityTurbineCore te = (TileEntityTurbineCore)world.getTileEntity(x, y,
         z+1); ForgeDirection dir = te.getSteamMovement(); int dx = x+dir.offsetX; int dy
         = y+dir.offsetY; int dz = z+dir.offsetZ; if (this.canMoveInto(world, dx, dy, dz))
         ...
         ReactorBlocks.GENERATORMULTI.getBlockInstance() && world.getBlockMetadata(dx, dy,
         dz) == 3) { if (this.canMoveInto(world, te.xCoord+dir.offsetX*2, te.yCoord+3,
         te.zCoord+dir.offsetZ*2)) { world.setBlock(te.xCoord+dir.offsetX*2, te.yCoord+3,
         te.zCoord+dir.offsetZ*2, this, this.getTransmittedMetadata(meta, dir), 3);
                     world.setBlock(x, y, z);
                 }
            ...
            //ReikaJavaLibrary.pConsole(meta+":"+this.getTransmittedMetadata(meta,
            //ForgeDirection.UP), Side.SERVER);
            if (((meta & 1) != 0) || ReikaRandomHelper.doWithChance(80))
                world.setBlock(
                    ...
                    z,
                    this,
                    this.getTransmittedMetadata(meta, ForgeDirection.UP),
                    2
                );
            world.setBlock(x, y, z, Blocks.air, 0, 2);
            world.markBlockForUpdate(x, y, z);
            ...
            ForgeDirection[] dir = new ForgeDirection[] { ForgeDirection.EAST,
                                                          ForgeDirection.WEST,
            ...
            for (int i = 0; i < dir.length; i++) {
                ...
                if (this.canMoveInto(world, dx, dy, dz)) {
                    world.setBlock(
                        dx, dy, dz, this, this.getTransmittedMetadata(meta, dir[i]), 2
                    );
                    world.setBlock(x, y, z, Blocks.air, 0, 2);
    ...
    public boolean canMoveInto(World world, int x, int y, int z) {
        ...
        //"+id+":"+world.getBlockMetadata(x, y, z));
        if (id == Blocks.air)
            return true;
        if (id == this)
            return false;
        ...
        if (!(e instanceof EntityItem || e instanceof EntityXPOrb)) {
            ...
            if ((meta & 4) != 0) {
                if (e instanceof EntityLivingBase)
                    ((EntityLivingBase) e)
                        .addPotionEffect(new PotionEffect(Potion.poison.id, 200, 0));
            }
        }
    }
}
