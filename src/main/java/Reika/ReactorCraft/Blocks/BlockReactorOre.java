...
public class BlockReactorOre extends EnumOreBlock {
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        if (ReactorOptions.RADIOORE.getState()) {
            ...
            if (ore.isRadioactive()
                && !RadiationIntensity.LOWLEVEL.hasSufficientShielding(
                    Minecraft.getMinecraft().thePlayer
                )) {
                ReikaPacketHelper.sendUpdatePacket(
                    ReactorCraft.packetChannel,
                    ReactorPackets.ORERADIATION.ordinal(),
                    x,
                    y,
                    z,
                    PacketTarget.server
    ...
    @Override
    protected void onHarvested(
        World world, int x, int y, int z, Block b, int meta, OreEnum ore, EntityPlayer ep
    ) {
    ...
    getPickBlock(MovingObjectPosition tgt, World world, int x, int y, int z) {
        ...
        return new ItemStack(
            ReactorBlocks.ORE.getBlockInstance(), 1, ore.getBlockMetadata()
        );
    }
