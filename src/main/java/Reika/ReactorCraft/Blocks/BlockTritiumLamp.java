...
public class BlockTritiumLamp extends Block {
    ...
    public int getLightValue(IBlockAccess iba, int x, int y, int z) {
        ...
        return ModList.COLORLIGHT.isLoaded()
            ? ReikaColorAPI.getPackedIntForColoredLight(color, 15)
            : 15;
    }
    ...
    private FluoriteTypes getColor(IBlockAccess iba, int x, int y, int z) {
        ...
        return FluoriteTypes
            .colorList[iba.getBlockMetadata(x, y, z) % FluoriteTypes.colorList.length];
    }
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < icons.length; i++) {
            icons[i] = ico.registerIcon(
                "reactorcraft:lamp/" + FluoriteTypes.colorList[i].getColorName()
            );
        }
    ...
    public static class TileEntityTritiumLamp extends TileEntity {
        ...
        public static final long LIFESPAN
            = (long) (ReikaTimeHelper.YEAR.getMinecraftDuration() * 98.4);
        ...
        public void updateEntity() {
            if (ticks == 0
                && worldObj.getBlockMetadata(xCoord, yCoord, zCoord)
                    >= FluoriteTypes.colorList.length) {
                this.onCreate();
            }
        ...
        public void writeToNBT(NBTTagCompound NBT) {
            ...
            //ReikaJavaLibrary.pConsole("NBT write: "+this+" %
            //"+FMLCommonHandler.instance().getEffectiveSide()+"/"+blocks);
                ...
                for (int i = -r; i <= r; i++) {
                    for (int j = -r; j <= r; j++) {
                        for (int k = -r; k <= r; k++) {
                            ...
                            int z = zCoord+k;
                            if (worldObj.getBlock(x, y, z) ==
                BlockRegistry.LIGHT.getBlockInstance()) { worldObj.setBlockToAir(x, y, z);
                            }
                        }
                    }
                }
                        ...
                        for (int j = -r; j <= r; j++) {
                            for (int k = -r; k <= r; k++) {
                                if (ReikaMathLibrary.py3d(i, j, k) <= r) {
                                    ...
                                    int y = yCoord + j;
                                    int z = zCoord + k;
                                    if (worldObj.getBlock(x, y, z).isAir(
                                            worldObj, x, y, z
                                        )) {
                                        worldObj.setBlock(
                                            x,
        ...
        public boolean shouldRefresh(
            ...
            return oldBlock != newBlock
                || oldMeta % FluoriteTypes.colorList.length
                != newMeta % FluoriteTypes.colorList.length;
        }
    }
}
