...
public class BlockCoriumFlowing extends BlockFluidClassic {
            ...
            if (world.getEntitiesWithinAABB(EntityRadiation.class, box).size() < 24) {
                if (ReikaRandomHelper.doWithChance(0.02))
                    RadiationEffects.instance.contaminateArea(
                        world,
                        x,
                        y + ReikaRandomHelper.getSafeRandomInt(3),
                        z,
                        8,
                        1,
                        0,
                        false,
                        RadiationIntensity.LETHAL
                    );
    ...
    private boolean blockBlocksFlow(World world, int x, int y, int z) {
        ...
        if (l != Blocks.wooden_door && l != Blocks.iron_door && l != Blocks.standing_sign
            && l != Blocks.ladder && l != Blocks.reeds) {
            if (l == Blocks.air) {
                return false;
            } else {
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
        icon = new IIcon[] { iconRegister.registerIcon("ReactorCraft:mat/slag"),
                             iconRegister.registerIcon("ReactorCraft:fluid/slag_flow") };
    }
