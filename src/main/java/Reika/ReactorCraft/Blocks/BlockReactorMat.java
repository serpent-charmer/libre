...
public class BlockReactorMat extends Block implements NeutronBlock {
        ...
        if (m == MatBlocks.SLAG.ordinal()) {
            if (ReikaRandomHelper.doWithChance(7.5)) {
                RadiationEffects.instance.contaminateArea(
                    world, x, y, z, 4, 0.5F, 0.05, false, RadiationIntensity.HIGHLEVEL
                );
            }
    ...
    private void
    doLodestoneTick(World world, int x, int y, int z, Random rand, boolean forced) {
        if (world.isBlockIndirectlyGettingPowered(x, y, z)) {
            TileEntity te = world.getTileEntity(x, y + 1, z);
            if (te instanceof IEnergyReceiver) {
                IEnergyReceiver ier = (IEnergyReceiver) te;
                int amt = MathHelper.ceiling_float_int(
                    ReactorOptions.LODESTONERFMULT.getFloat() * (!forced ? 2 : 1)
                );
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < MatBlocks.matList.length; i++) {
            if (MatBlocks.matList[i].isMultiSidedTexture()) {
                for (int j = 0; j < 6; j++) {
                    icons[i][j] = ico.registerIcon(
                        "ReactorCraft:mat/"
                        + MatBlocks.matList[i].name().toLowerCase(Locale.ENGLISH) + "_"
                        + j
                    );
                }
            } else {
                for (int j = 0; j < 6; j++) {
                    icons[i][j] = ico.registerIcon(
                        "ReactorCraft:mat/"
                        + MatBlocks.matList[i].name().toLowerCase(Locale.ENGLISH)
                    );
                }
            }
        }
