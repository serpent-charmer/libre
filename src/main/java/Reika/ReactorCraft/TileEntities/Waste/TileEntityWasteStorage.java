...
public class TileEntityWasteStorage
    extends TileEntityWasteUnit implements RangedEffect, Feedable {
    @Override
        ...
        for (EntityLivingBase e : li) {
            if (!RadiationIntensity.MODERATE.hasSufficientShielding(e)) {
                ...
                if (ReikaWorldHelper.canBlockSee(
                        world, x, y, z, e.posX, e.posY, e.posZ, dd
                    )) {
                    RadiationEffects.instance.applyEffects(
                        e, RadiationIntensity.MODERATE
                    );
                }
            }
        }
    ...
    public boolean isValidIsotope(Isotopes i) {
        ...
        ); //i.getMCHalfLife() > ReikaTimeHelper.YEAR.getMinecraftDuration();
    }
