...
public class TileEntityWasteContainer
    extends TileEntityWasteUnit implements TemperatureTE, Feedable, NeutronTile {
    ...
    private void distributeHeat(World world, int x, int y, int z) {
        ...
        if (temperature > Tamb) {
            ...
            if (side != null) {
                temperature -= ReikaThermoHelper.getTemperatureIncrease(
                    1, 15000, ReikaThermoHelper.WATER_BLOCK_HEAT
                );
                //ReikaJavaLibrary.pConsole(temperature);
                if (temperature > 100)
                    ...
                    ReikaWorldHelper.changeAdjBlock(
                        world, x, y, z, side, Blocks.flowing_water, 6
                    );
            }
        }
        //ReikaJavaLibrary.pConsole(temperature);
        ...
        if (temperature > this.getMaxTemperature()) {
            ...
            world.spawnParticle(
                "smoke", x + rand.nextDouble(), y + 1, z + rand.nextDouble(), 0, 0, 0
            );
    ...
    public void onMeltdown(World world, int x, int y, int z) {
        ...
        RadiationEffects.instance.contaminateArea(
            world, x, y, z, 9, 4, 1.5, true, RadiationIntensity.LETHAL
        );
