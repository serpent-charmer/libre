...
public class TileEntityPebbleBed extends TileEntityInventoriedReactorBase
    implements TemperaturedReactorTyped, Feedable, ReactorBlock, BreakAction {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        if (!world.isRemote && this.isFissile()
            && ReikaRandomHelper.doWithChance(this.getFissionChance() / 100D))
            this.runDecayCycle();
    ...
    protected void updateTemperature(World world, int x, int y, int z) {
        ...
        if (dT > 0) {
            for (int i = 2; i < 6; i++) {
                ...
                if (r == this.getTile()) {
                    TileEntityPebbleBed te
                        = (TileEntityPebbleBed) world.getTileEntity(dx, dy, dz);
    ...
    public boolean isItemValidForSlot(int i, ItemStack is) {
        return is.getItem() == ReactorItems.PELLET.getItemInstance()
            || is.getItem() == ReactorItems.OLDPELLET.getItemInstance();
    }
    ...
    public boolean isFissile() {
        return ReikaInventoryHelper.checkForItem(
            ReactorItems.PELLET.getItemInstance(), inv
        );
    }
