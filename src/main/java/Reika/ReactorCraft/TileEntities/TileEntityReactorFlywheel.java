...
public class TileEntityReactorFlywheel extends TileEntityReactorBase
    implements ShaftPowerEmitter, Screwdriverable, MultiBlockTile {
    private int iotick;
    private long power;
    private int omega;
    private int torque;
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (r != null && r.isTurbine()) {
            ...
            //if (te.getOmega() > omega && omega < MAXSPEED && te.getTorque() >=
            //MINTORQUE) { 	omega++;
            //}
            //else if (omega > 0) {
            //	omega--;
            //}
            //torque = te.getTorque();
            //ReikaJavaLibrary.pConsole(torque+"/"+te.getTorque()+":"+omega+"/"+te.getOmega(),
            //Side.SERVER);
            omega = te.getOmega();
    ...
    public static int clampTorque(TileEntityTurbineCore te) {
        return te instanceof TileEntityHiPTurbine
            ? te.getTorque()
            : Math.min(te.getTorque(), te.isAmmonia() ? MAXTORQUE_AMMONIA : MAXTORQUE);
    }
