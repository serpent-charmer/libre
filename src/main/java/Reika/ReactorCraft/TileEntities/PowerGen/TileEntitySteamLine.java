...
public class TileEntitySteamLine
    extends TileEntityLine implements PumpablePipe, SteamTile, PressureTile {
    private int steam;
    ...
    canConnectToMachine(Block id, int meta, ForgeDirection dir, TileEntity te) {
        if (id == ReactorTiles.BOILER.getBlock()
            && meta == ReactorTiles.BOILER.getBlockMetadata()
            && dir == ForgeDirection.DOWN)
            return true;
        if (id == ReactorTiles.GRATE.getBlock()
            && meta == ReactorTiles.GRATE.getBlockMetadata())
            return true;
        if (id == ReactorTiles.BIGTURBINE.getBlock()
            && meta == ReactorTiles.BIGTURBINE.getBlockMetadata())
            return true;
        if (id == ReactorTiles.DIFFUSER.getBlock()
            && meta == ReactorTiles.DIFFUSER.getBlockMetadata()) {
            return ((TileEntitySteamDiffuser) this.getAdjacentTileEntity(dir))
                       .getFacing()
                       .getOpposite()
                == dir;
        }
        if (id == MachineRegistry.PIPEPUMP.getBlock()
            && meta == MachineRegistry.PIPEPUMP.getBlockMetadata()) {
            return ((TileEntityPipePump) this.getAdjacentTileEntity(dir))
                .canConnectToPipeOnSide(dir);
        }
        return false;
    }
    ...
    private void drawFromBoiler(World world, int x, int y, int z) {
        ...
        if (r == ReactorTiles.BOILER) {
            TileEntityReactorBoiler te
                = (TileEntityReactorBoiler) world.getTileEntity(x, y - 1, z);
            if (te.getTileEntityAge() > 5
                && this.canTakeInWorkingFluid(te.getWorkingFluid())) {
    ...
    public void readFromNBT(NBTTagCompound NBT) {
        ...
        source.readFromNBT(
            NBT.getCompoundTag("sources"),
            (NBTIO<ReactorType>) ReikaNBTHelper.getEnumConverter(ReactorType.class)
        );
        if (source.removeValue(null) > 0) {
            ReactorCraft.logger.logError(
                this + " loaded null-containing steam type map from NBT: " + NBT
            );
        }
    }
    ...
    public void writeToNBT(NBTTagCompound NBT) {
        ...
        source.writeToNBT(
            tag, (NBTIO<ReactorType>) ReikaNBTHelper.getEnumConverter(ReactorType.class)
        );
