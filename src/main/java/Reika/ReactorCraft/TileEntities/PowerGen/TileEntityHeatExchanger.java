...
public class TileEntityHeatExchanger
    extends TankedReactorPowerReceiver implements TemperatureTE {
        ...
        for (int i = 2; i < 6; i++) {
            ...
            if (r == ReactorTiles.BOILER) {
                TileEntityReactorBoiler te
                    = (TileEntityReactorBoiler) world.getTileEntity(dx, dy, dz);
                ...
                if (dT > 0) {
                    ...
                    te.setReactorType(
                        currentRecipe != null ? currentRecipe.type : ReactorType.NONE, add
                    );
                }
            }
        }
    ...
    private void cool() {
        ...
        output.addLiquid(
            COOL_AMOUNT * currentRecipe.expansionRatio, currentRecipe.coldFluid
        );
        double eff = Math.min(
            1,
            Math.max(0.1, 1 - (temperature - 100) / (currentRecipe.maxTemperature - 100D))
        );
    ...
    private boolean canCool() {
        ...
        return temperature < currentRecipe.maxTemperature
            && tank.getLevel() >= COOL_AMOUNT
            && output.getRemainingSpace() >= COOL_AMOUNT * currentRecipe.expansionRatio
            && this.canCoolFluid(tank.getActualFluid());
    }
    ...
    public static enum Exchange {
        ...
        CO2("rc hot co2",
            ...
            TileEntityPebbleBed.MINTEMP,
            ReactorType.HTGR),
        LIFBE(
            "rc hot lifbe",
            "rc lifbe",
            ReikaThermoHelper.LIFBE_HEAT,
            1000,
            ReactorType.THORIUM
        ),
        OXYGEN(
            ...
            "rc oxygen",
            4,
            -ReikaThermoHelper.OXYGEN_HEAT - ReikaThermoHelper.OXYGEN_BOIL_ENTHALPY,
            500,
            ReactorType.NONE
        ),
        SOLARSODIUM(
            ReactorCraft.NA_warm,
            ReactorCraft.NA,
            ReikaThermoHelper.SODIUM_HEAT * 0.375F,
            400,
            ReactorType.SOLAR
        ),
        NITROGEN(
            ...
            "nitrogen",
            12,
            -ReikaThermoHelper.NITROGEN_HEAT - ReikaThermoHelper.NITROGEN_BOIL_ENTHALPY,
            500,
            ReactorType.NONE
        );
        ...
        private Exchange(
            String from, String to, int r, double c, int max, ReactorType t
        ) {
    ...
    public void updateTemperature(World world, int x, int y, int z, int meta) {
        ...
        if (lavaside != null) {
            ...
            if (temperature < 100)
                ReikaWorldHelper.changeAdjBlock(
                    world, x, y, z, lavaside, Blocks.stone, 0
                );
            else {
                world.setBlockToAir(x, y, z);
                world.createExplosion(
    ...
    public ConnectOverride overridePipeConnection(PipeType type, ForgeDirection side) {
        return type == PipeType.FLUID && side != ForgeDirection.DOWN
