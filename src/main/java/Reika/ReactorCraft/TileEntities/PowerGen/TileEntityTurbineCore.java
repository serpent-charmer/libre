...
public class TileEntityTurbineCore extends TileEntityReactorBase
    implements ShaftPowerEmitter, Screwdriverable, IFluidHandler, PipeConnector,
    ...
    protected final HybridTank tank
        = new HybridTank("turbine", this.getLubricantCapacity());
    ...
    private final StepTimer lubeTimer
        = new StepTimer((int) (20 / DifficultyEffects.LUBEUSAGE.getChance()));
    ...
    public final void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!hasMultiBlock) {
            //boolean checkMulti = this.getTicksExisted() < 5 ||
            //world.getTotalWorldTime()%32 == 0; if (!checkMulti &&
            //!this.checkForMultiblock(world, x, y, z, meta)) {
            omega = 0;
            phi = 0;
            steam = 0;
            return;
        ...
        if (ReactorCraft.logger.shouldDebug()) {
            if (world.isRemote)
                ReactorCraft.logger.log(
                    "Clientside " + this + " has " + steam + " steam, spinning @ " + omega
                    + " rad/s. Phi=" + phi
                );
            else
                ReactorCraft.logger.log(
                    "Serverside " + this + " has " + steam + " steam, spinning @ " + omega
                    + " rad/s."
                );
        }
                ...
                if (OldTextureLoader.instance.loadOldTextures()) {
                    if (this.getTicksExisted() % 4 == 0)
                        ReikaSoundHelper.playSoundFromServerAtBlock(
                            world, x, y, z, "mob.villager.idle", 1, 1, true
                        );
    ...
    private void distributeLubricant(World world, int x, int y, int z, int meta) {
        ...
        if (r == this.getTile()) {
            TileEntityTurbineCore te
                = (TileEntityTurbineCore) world.getTileEntity(dx, dy, dz);
    ...
    public final boolean isAtEndOFLine() {
        if (ReactorTiles.getTE(worldObj, readx, ready, readz) == this.getTile()) {
            TileEntityTurbineCore tile
                = (TileEntityTurbineCore) worldObj.getTileEntity(readx, ready, readz);
    ...
    protected final int getGenTorque() {
        int torque
            = steam > 0 ? (int) (steam * 24 * this.getTorqueFactor()) : omega / 16 + 1;
    ...
    protected boolean intakeSteam(World world, int x, int y, int z, int meta) {
        ...
        if (canAccel && world.isRemote
            && world.getClosestPlayer(x + 0.5, y + 0.5, z + 0.5, 64) != null) {
            ...
            for (int i = 0; i < this.getNumberStagesTotal(); i++) {
                ...
                for (int n = 0; n < 2; n++) {
                    ...
                    ReikaParticleHelper.CLOUD.spawnAt(
                        world, dx, dy, dz, dir.offsetX * v, 0, dir.offsetZ * v
                    );
                }
            }
        }
    ...
    private void readSurroundings(World world, int x, int y, int z, int meta) {
        if (this.canCollideCheck()) {
            ...
            for (int i = 0; i < contact.getSize(); i++) {
                ...
                if (ReikaMathLibrary.py3d(x - c.xCoord, y - c.yCoord, z - c.zCoord)
                    <= this.getRadius()) {
                    Block id2 = c.getBlock(world);
                    int meta2 = c.getBlockMetadata(world);
                    if (!ReikaWorldHelper.softBlocks(world, c.xCoord, c.yCoord, c.zCoord)
                        && !c.equals(x, y, z)
                        && id2 != ReactorBlocks.TURBINEMULTI.getBlockInstance()) {
                        phi = 0;
                        omega = 0;
        ...
        if (this.getStage() == 0) {
            boolean accel
                = this.enabled(world, x, y, z) && this.intakeSteam(world, x, y, z, meta);
            if (!world.isRemote)
        ...
        for (EntityLivingBase e : li) {
            if (this.getOmega() > 0
                && ReikaMathLibrary.py3d(
                       e.posX - x - 0.5, e.posY - y - 0.5, e.posZ - z - 0.5
                   ) < r) {
                if (this.canDamageTurbine(e)) {
                    if (!world.isRemote) {
                        Explosion exp = world.createExplosion(
                            null, e.posX, e.posY + e.getEyeHeight() / 1F, e.posZ, 2, false
                        );
    ...
    public final int getNumberStagesTotal() {
        ...
        if (ReactorTiles.getTE(worldObj, writex, writey, writez) == this.getTile()) {
            TileEntityTurbineCore tile
                = (TileEntityTurbineCore) worldObj.getTileEntity(writex, writey, writez);
    ...
    private void followHead(World world, int x, int y, int z, int meta) {
        if (ReactorTiles.getTE(worldObj, readx, ready, readz) == this.getTile()) {
            TileEntityTurbineCore tile
                = (TileEntityTurbineCore) world.getTileEntity(readx, ready, readz);
            if (tile.writex == x && tile.writey == y && tile.writez == z) {
                this.copyDataFrom(tile);
            }
        }
        if (ReactorTiles.getTE(worldObj, writex, writey, writez) == this.getTile()) {
            TileEntityTurbineCore tile = (TileEntityTurbineCore
            ) worldObj.getTileEntity(writex, writey, writez); //write!
    ...
    protected final void animateWithTick(World world, int x, int y, int z) {
        ...
        phi += this.getAnimationSpeed()
            * ReikaMathLibrary.doubpow(ReikaMathLibrary.logbase(omega + 1, 2), 1.05);
    }
    ...
    public final boolean canWriteTo(ForgeDirection from) {
        return xCoord + from.offsetX == writex && yCoord + from.offsetY == writey
            && zCoord + from.offsetZ == writez;
    }
    ...
    onShiftRightClick(World world, int x, int y, int z, ForgeDirection side) {
        return false;
    }

    @Override
    public final boolean
    onRightClick(World world, int x, int y, int z, ForgeDirection side) {
    ...
    public final boolean canFill(ForgeDirection from, Fluid fluid) {
        return from == this.getSteamMovement().getOpposite()
            && fluid.equals(FluidRegistry.getFluid("rc lubricant"));
    }
    ...
    public void breakBlock() {
        ...
        if (m == this.getTile()) {
            TileEntityTurbineCore te
                = (TileEntityTurbineCore) this.getAdjacentTileEntity(dir);
