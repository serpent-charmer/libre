...
public class TileEntityCondenser extends TileEntityTankedReactorMachine {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (world.getBlock(x, y - 1, z) == ReactorBlocks.STEAM.getBlockInstance()
            && !tank.isFull() && temperature < 100 && !world.isRemote) {
    ...
    private Fluid getFluidFromSteamMetadata(int smeta) {
        //ReikaJavaLibrary.pConsole(String.format("%4s",
        //Integer.toBinaryString(smeta)).replace(" ", "0"), Side.SERVER);
        if ((smeta & 4) == 4)
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (rt == ReactorTiles.CONDENSER) {
                TileEntityCondenser te
                    = (TileEntityCondenser) world.getTileEntity(dx, dy, dz);
    ...
    public ConnectOverride overridePipeConnection(PipeType type, ForgeDirection with) {
        return type == PipeType.FLUID
            ? (with == ForgeDirection.UP ? ConnectOverride.CONNECT
