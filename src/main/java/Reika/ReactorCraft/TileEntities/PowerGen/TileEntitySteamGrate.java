...
public class TileEntitySteamGrate
    extends TileEntityReactorBase implements Screwdriverable, SteamTile {
    private int steam;
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote && this.canMakeSteam(world, x, y, z)) {
            ...
            world.setBlock(
                ...
                y + 1,
                z,
                ReactorBlocks.STEAM.getBlockInstance(),
                this.getSteamMetadata(),
                3
            );
        }
    ...
    private boolean canMakeSteam(World world, int x, int y, int z) {
        ...
        if (AtmosphereHandler.isNoAtmo(world, x, y + 1, z, blockType, false))
            return false;
        return ((BlockSteam) ReactorBlocks.STEAM.getBlockInstance())
            .canMoveInto(world, x, y + 1, z);
    }
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (rt == ReactorTiles.STEAMLINE) {
                TileEntitySteamLine te
                    = (TileEntitySteamLine) world.getTileEntity(dx, dy, dz);
