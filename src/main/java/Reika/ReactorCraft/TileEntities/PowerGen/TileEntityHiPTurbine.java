...
public class TileEntityHiPTurbine
    extends TileEntityTurbineCore implements MultiBlockTile {
    public static final int GEN_OMEGA = 131072;
    public static final int FLUID_PER_RESERVOIR
        = TileEntityReactorBoiler.WATER_PER_STEAM * 131 / 20 / 24 * 6 / 10;
    ...
    protected boolean checkForMultiblock(World world, int x, int y, int z, int meta) {
        for (int i = 0; i < 6; i++) {
            ...
            if (dir != this.getSteamMovement()
                && dir.getOpposite() != this.getSteamMovement()) {
        ...
        if (dripBuffer > 0 && this.dumpLiquid(world, x, y, z, meta)) {
                    ...
                    for (int dy = 2; dy < 5; dy++) {
                        ...
                        for (int i = -th; i <= th; i++) {
                            ...
                            if (ReikaMathLibrary.py3d(
                                    dir.offsetX * i, py - y, dir.offsetZ * i
                                )
                                < th)
                                ReikaParticleHelper.DRIPWATER.spawnAroundBlock(
                                    world, x + dir.offsetX * i, py, z + dir.offsetZ * i, 5
                                );
                        }
                        if (dripBuffer <= 0)
                            break;
            ...
            for (int i = 0; i < n; i++) {
                ...
                ReikaParticleHelper.RAIN.spawnAt(
                    world, px + ax, y - th + 1 + rand.nextInt(th * 2), pz + az
                );
            }
        }
    }
    ...
    private boolean dumpLiquid(World world, int x, int y, int z, int meta) {
        if (AtmosphereHandler.isNoAtmo(
                world,
                x + this.getSteamMovement().offsetX,
                y,
                z + this.getSteamMovement().offsetZ,
                blockType,
                false
            ))
            return false;
    ...
    protected boolean intakeSteam(World world, int x, int y, int z, int meta) {
        ...
        if (r == ReactorTiles.STEAMLINE) {
            TileEntitySteamLine te
                = (TileEntitySteamLine) this.getAdjacentTileEntity(dir);
            int s = te.getSteam();
            //ReikaJavaLibrary.pConsole(steam+"/"+this.getMaxSteam()+" from "+s,
            //Side.SERVER);
            if (s > 8 && this.canTakeIn(te.getWorkingFluid())) {
                ...
                if (s > 0) {
                    ...
                    if (steam < this.getMaxSteam()) {
                        ...
                        te.removeSteam(rm2);
                        dripBuffer += rm2 * 1000;
                        //ReikaJavaLibrary.pConsole("Took in "+rm2+" of "+s+" available",
                        //Side.SERVER);
                    }
                    flag = s > rm + 32 && steam >= this.getMaxSteam() / 15;
                    //ReikaJavaLibrary.pConsole("Has "+steam+"/"+this.getMaxSteam()+",
                    //s/rm = "+s+"/"+rm, Side.SERVER);
                }
            }
        } else if (r == this.getTile()) {
            TileEntityHiPTurbine te
                = (TileEntityHiPTurbine) this.getAdjacentTileEntity(dir);
            fluid = te.fluid;
        }
    ...
    protected float getTorqueFactor() {
        ...
        if (steam < this.getMaxSteam() / 2) {
            float f
                = steam / (float) this.getMaxSteam(); //stops at 0.5, aka the peak of cos
