...
@Strippable(
    value = { "cofh.api.energy.IEnergyHandler",
              "ic2.api.energy.tile.IEnergySource",
              "Reika.ElectriCraft.API.WrappableWireSource" }
)
public class TileEntityReactorGenerator extends TileEntityReactorBase
    implements IEnergyHandler, IEnergySource, Screwdriverable, MultiBlockTile,
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (Math.abs(currentAverage - lastAverage)
            <= Math.min(lastAverage, currentAverage) * 0.05) {
        ...
        if (power > 0) {
            ...
            switch (mode) {
                case RF:
                    if (tile instanceof IEnergyReceiver) {
                        ...
                        int used = rc.receiveEnergy(
                            this.getFacing(), (int) this.getGenUnits(), false
                        );
                        //}
                ...
                case EU:
                    if (tile instanceof IEnergySink) {
                        ...
                        if (rc.acceptsEnergyFrom(this, this.getFacing())) {
                            double leftover = rc.injectEnergy(
                                this.getFacing(),
                                (int) this.getGenUnits(),
                                this.getSourceTier()
                            );
                        }
                    }
            ...
            if (rs != null && this.getTicksExisted() % len == 0) {
                ...
                rs.playSoundAtBlock(
                    worldObj,
                    xCoord + this.getFacing().offsetX * l,
                    yCoord,
                    zCoord + this.getFacing().offsetZ * l,
                    2,
                    1
                );
                rs.playSoundAtBlock(
                    worldObj,
                    xCoord + this.getFacing().offsetX * l / 2,
                    yCoord,
                    zCoord + this.getFacing().offsetZ * l / 2,
                    2,
                    1
                );
            }
    ...
    private TileEntityTurbineCore getTurbine(World world, int x, int y, int z) {
        ...
        return r != null && r.isTurbine()
            ? (TileEntityTurbineCore) this.getTileEntity(dx, dy, dz)
            : null;
    }
    ...
    protected void animateWithTick(World world, int x, int y, int z) {
        ...
        phi += 0.5
            * ReikaMathLibrary.doubpow(ReikaMathLibrary.logbase(omegain + 1, 2), 1.05);
    }
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        ...
        return AxisAlignedBB.getBoundingBox(mx, yCoord - 2, mz, mx2, yCoord + 3, mz2)
            .expand(6, 6, 6);
    }
    ...
    public static enum Modes {
        ...
        private String getELCDisplay(TileEntityReactorGenerator te) {
            return te.getTorque() / WireNetwork.TORQUE_PER_AMP + "A @ "
                + te.getOmega() * WireNetwork.TORQUE_PER_AMP + "V";
        }
    ...
    @Override
    protected void
    onInvalidateOrUnload(World world, int x, int y, int z, boolean invalidate) {
        if (!world.isRemote && ModList.IC2.isLoaded())
    ...
    public int getOmega() {
        ...
        int lim = te instanceof TileEntityHiPTurbine ? TileEntityHiPTurbine.GEN_OMEGA
    ...
    public int getTorque() {
        ...
        if (te instanceof TileEntityHiPTurbine) {
            ...
            max = te.isAmmonia() ? TileEntityTurbineCore.TORQUE_CAP * 0.95
                                 : TileEntityTurbineCore.TORQUE_CAP * 1.95;
        }
        /*
        return Math.min(torquein, TileEntityReactorFlywheel.clampTorque(te)); //clamp for
        the same reason*/
