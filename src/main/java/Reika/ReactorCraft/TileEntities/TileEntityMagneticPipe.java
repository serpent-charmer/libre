...
public class TileEntityMagneticPipe
    extends TileEntityReactorPiping implements Shockable, NeutronTile {
    private int charge;
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (charge <= 0 && !world.isRemote) {
            ReactorCraft.logger.debug(
                "Melting magnetic pipe " + this + " with charge " + charge
            );
            charge = 0;
    ...
    private void distributeCharge(World world, int x, int y, int z) {
        for (int i = 0; i < 6; i++) {
            ...
            if (r == ReactorTiles.MAGNETPIPE) {
                TileEntityMagneticPipe tile
                    = (TileEntityMagneticPipe) world.getTileEntity(dx, dy, dz);
    ...
    private void updateCharge(World world, int x, int y, int z) {
        ...
        if (dir != null) {
            EntityDischarge e = new EntityDischarge(
                world,
                x + 0.5,
                y + 0.5,
                z + 0.5,
    ...
    private boolean isPlasmaAcceptingBlock(TileEntity te) {
        return te instanceof TileEntityMagneticPipe
            || te instanceof TileEntityFusionHeater
            || te instanceof TileEntityFusionInjector;
    }

    @Override
    protected boolean isInteractableTile(TileEntity te) {
        return (te instanceof WorldRift || this.isPlasmaAcceptingBlock(te))
            && super.isInteractableTile(te);
    }
