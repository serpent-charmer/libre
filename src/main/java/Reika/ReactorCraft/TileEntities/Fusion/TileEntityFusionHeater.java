...
public class TileEntityFusionHeater extends TileEntityReactorBase
    implements TemperatureTE, Laserable, IFluidHandler, PipeConnector, MultiBlockTile {
    ...
    public boolean hasMultiBlock() {
        return hasMultiBlock
            && !ReikaWorldHelper.isExposedToAir(worldObj, xCoord, yCoord, zCoord);
    }

    public void setHasMultiBlock(boolean has) {
        hasMultiBlock
            = has && !ReikaWorldHelper.isExposedToAir(worldObj, xCoord, yCoord, zCoord);
    }
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        //ReikaJavaLibrary.pConsole(temperature+": "+((float)temperature/PLASMA_TEMP),
        //Side.SERVER); ReikaJavaLibrary.pConsole(h2, Side.SERVER);
    ...
    private boolean canMake() {
        return this.hasMultiBlock() && temperature >= PLASMA_TEMP && h2.getLevel() >= 50
            && h3.getLevel() >= 50 && tank.canTakeIn(100);
    }
    ...
    public boolean canDrain(ForgeDirection from, Fluid fluid) {
        return from == ForgeDirection.UP
            && this.getAdjacentTileEntity(from) instanceof TileEntityMagneticPipe
