...
public class TileEntityToroidMagnet extends TileEntityReactorBase
    implements Screwdriverable, Shockable, MultiBlockTile, FusionReactorToroidPart,
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (m != null && m.isStandardPipe()) {
            ...
            int amt = Math.min(
                tank.getRemainingSpace(), ((TileEntityPiping) te).getFluidLevel()
            );
            if (amt > 0) {
                if (FluidRegistry.getFluid("rc liquid nitrogen")
                        .equals(((TileEntityPiping) te).getFluidType())) {
    ...
    private boolean checkCompleteness(World world, int x, int y, int z) {
        ...
        //ReikaJavaLibrary.pConsole("B:"+(te == world.getTileEntity(x+aim.xOffset, y,
        //z+aim.zOffset))+"   "+te+"/"+this+"@"+aim, Side.SERVER);
        if (te != this)
    ...
    public FusionReactorToroidPart getNextPart(World world, int x, int y, int z) {
        ...
        return te instanceof FusionReactorToroidPart ? (FusionReactorToroidPart) te
                                                     : null;
    }
    ...
    private void checkSurroundingMagnetsAndCopySolenoidState() {
        ...
        if (r == ReactorTiles.MAGNET) {
            TileEntityToroidMagnet te
                = (TileEntityToroidMagnet) worldObj.getTileEntity(dx, yCoord, dz);
    ...
    private void collectCharge(World world, int x, int y, int z) {
        ...
        if (m == MachineRegistry.VANDEGRAFF) {
            TileEntityVanDeGraff te
                = (TileEntityVanDeGraff) world.getTileEntity(x, y - 3, z);
    ...
    private boolean distributeCharge(World world, int x, int y, int z) {
        ...
        if (r == ReactorTiles.MAGNET) {
            TileEntityToroidMagnet te
                = (TileEntityToroidMagnet) world.getTileEntity(dx, y, dz);
            int dC = charge - te.charge;
            if (dC > 0) {
                te.charge += dC / 4;
                charge -= dC / 4;
                EntityDischarge e1 = new EntityDischarge(
                    world,
                    x + 0.5,
                    y + 2.25,
                    z + 0.5,
                ...
                EntityDischarge e2 = new EntityDischarge(
                    world,
                    x + 0.5,
                    y - 1.25,
                    z + 0.5,
                ...
                EntityDischarge e3 = new EntityDischarge(
                    world,
                    x + 0.5 + fx,
                    y + 0.5,
                    z + 0.5 + fz,
                ...
                EntityDischarge e4 = new EntityDischarge(
                    world,
                    x + 0.5 - fx,
                    y + 0.5,
                    z + 0.5 - fz,
    ...
    public int addFluid(Fluid fluid, int amount, boolean doFill) {
        if (fluid != RotaryCraft.nitrogenFluid)
            return 0;
        int add
            = tank.canTakeIn(fluid, 1) ? Math.min(tank.getRemainingSpace(), amount) : 0;
        if (doFill && add > 0) {
