...
public class TileEntitySolenoidMagnet extends TileEntityReactorBase
    implements ReactorPowerReceiver, MultiBlockTile, NeutronTile {
        ...
        if (ReactorCraft.logger.shouldDebug()) {
            if (world.isRemote)
                ReactorCraft.logger.log(
                    "Clientside " + this + " receiving " + torque + " Nm @ " + omega
                    + " rad/s. Phi=" + phi
                );
            else
                ReactorCraft.logger.log(
                    "Serverside " + this + " receiving " + torque + " Nm @ " + omega
                    + " rad/s."
                );
        }

        if (DragonAPICore.debugtest
            || hasMultiBlock && checkForToroids && this.arePowerReqsMet()) {
            this.addToToroids();
        }
        if (!hasMultiBlock || !this.arePowerReqsMet()) {
            this.removeFromToroids();
        }
        if (hasMultiBlock && torque >= MINTORQUE
            && speed > MAX_SAFE_SPEED * 4) { //violently fail
            world.setBlockToAir(x, y, z);
    ...
    private void checkForMultiBlock(World world, int x, int y, int z) {
        ...
        if (id == ReactorBlocks.SOLENOIDMULTI.getBlockInstance()) {
            ...
            if (b.checkForFullMultiBlock(
                    world, x, y - 1, z, ForgeDirection.UNKNOWN, null
                )) {
        ...
        while ((r == ReactorTiles.MAGNET || r == ReactorTiles.INJECTOR) && c <= 38) {
            if (r == ReactorTiles.MAGNET) {
                TileEntityToroidMagnet te
                    = (TileEntityToroidMagnet) world.getTileEntity(x, y, z);
                te.hasSolenoid = true;
                a = te.getAim();
            }
            x += a.xOffset;
