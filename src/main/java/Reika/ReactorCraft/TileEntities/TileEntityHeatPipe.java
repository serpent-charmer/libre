...
public class TileEntityHeatPipe extends TileEntityLine {
    private static final double HEAT_CAPACITY
        = ReikaThermoHelper.COPPER_HEAT * ReikaEngLibrary.rhoiron;
    ...
    public static double getTemperatureForPipe(TileEntityHeatPipe tp, boolean net) {
        return net ? tp.getNetHeatEnergy() / HEAT_CAPACITY
                   : tp.heatEnergy / HEAT_CAPACITY;
    }

    @Override
    protected void onFirstTick(World world, int x, int y, int z) {
        heatEnergy
            = ReikaWorldHelper.getAmbientTemperatureAt(world, x, y, z) * HEAT_CAPACITY;
    }
    ...
    private void balanceHeat(World world, int x, int y, int z) {
        for (int i = 0; i < 6; i++) {
            ...
            if (te instanceof TileEntityHeatPipe) {
                ...
                boolean valid = intake ? hc.allowHeatExtraction() && ourtemp < theirtemp
                                       : hc.allowExternalHeating() && ourtemp > theirtemp;
                //ReikaJavaLibrary.pConsole(our+" vs "+heat+" > "+valid, Side.SERVER);
                //ReikaJavaLibrary.pConsole("our "+ourheat+", their "+theirheat+" (Ts =
                //"+theirtemp+", "+ourtemp+")", Side.SERVER, valid && !intake);
                if (valid) {
                    double diff = ourheat - theirheat; // >0 if applying heat
                    diff /= 4;
                    int put = this.getTemperatureForHeat(diff, hc);
                    //ReikaJavaLibrary.pConsole("Adding "+put+" to "+hc, Side.SERVER,
                    //!intake);
                    ...
                    /*
                    if (diff > 0)
                        ReikaJavaLibrary.pConsole("Taking "+diff+" heat from pipe to put
                    into "+te+" at new temp "+put+" ["+ourtemp+" -> "+theirtemp+"]"); else
                        ReikaJavaLibrary.pConsole("Taking "+(-diff)+" heat from "+te+" to
                    put into pipe @ heat "+heatEnergy+" ["+theirtemp+" -> "+ourtemp+"]");
                     */
                }
            }
        }
    ...
    private void updateBrightness() {
        ...
        renderBrightness = f;
        if (worldObj != null
            && (brightnessDeltaSinceUpdate >= 0.2
                || this.getTicksExisted() - lastUpdateTime > 40)) {
    ...
    @Override
    protected boolean
    canConnectToMachine(Block id, int meta, ForgeDirection dir, TileEntity te) {
