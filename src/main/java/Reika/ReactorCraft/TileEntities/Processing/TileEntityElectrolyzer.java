...
public class TileEntityElectrolyzer extends TileEntityInventoriedReactorBase
    implements IFluidHandler, PipeConnector, TemperatureTE, ThermalMachine, Shockable,
               IHasWork {
    ...
    public boolean hasWork() {
        return recipe != null && tankH.canTakeIn(recipe.lowerOutput)
            && tankL.canTakeIn(recipe.upperOutput);
    }

    public static enum Electrolysis {
        SALT(
            new ItemMatch("salt/dustSalt").addItem(new KeyedItemStack(ItemStacks.salt)),
            false,
            FluidRegistry.getFluid("rc chlorine"),
            100,
        ...
        HEAVYWATER(
            FluidRegistry.getFluid("rc heavy water"),
            100,
            null,
            false,
            FluidRegistry.getFluid("rc deuterium"),
            100,
            FluidRegistry.getFluid("rc oxygen"),
            50
        );
        ...
        private Electrolysis(
            ItemMatch item, boolean cata, Fluid out1, int amt1, Fluid out2, int amt2
        ) {
        ...
        public boolean requirementsMet(TileEntityElectrolyzer te) {
            if (requiredFluid != null) {
                if (te.input.getActualFluid() != requiredFluid.getFluid()
                    || te.input.getLevel() < requiredFluid.amount)
                    return false;
            }
            ...
            if (upperOutput != null) {
                if (te.tankL.isFull()
                    || (!te.tankL.isEmpty()
                        && te.tankL.getActualFluid() != upperOutput.getFluid()))
                    return false;
            }
            if (lowerOutput != null) {
                if (te.tankH.isFull()
                    || (!te.tankH.isEmpty()
                        && te.tankH.getActualFluid() != lowerOutput.getFluid()))
                    return false;
            }
        ...
        public boolean makes(Fluid f) {
            return (upperOutput != null && upperOutput.getFluid() == f)
                || (lowerOutput != null && lowerOutput.getFluid() == f);
        }
    ...
    public static void addRecipe(
        ...
        Class[] types = new Class[] { Fluid.class,   int.class,   ItemMatch.class,
                                      boolean.class, Fluid.class, int.class,
                                      Fluid.class,   int.class,   int.class };
        Object[] args
            = new Object[] { in, amt, item, cata, out1, amt1, out2, amt2, temp };
        Electrolysis c
            = EnumHelper.addEnum(Electrolysis.class, name.toUpperCase(), types, args);
