...
public class TileEntityUProcessor extends TileEntityInventoriedReactorBase
    implements IFluidHandler, PipeConnector, IHasWork {
    ...
    private ParallelTicker timer
        = new ParallelTicker().addTicker("intermediate", 0).addTicker("output", 0);

    public static enum Processes {
        UF6("water",
            "rc hydrofluoric acid",
            "rc uranium hexafluoride",
            250,
            1000,
            250,
            125,
        ...
        private Processes(
            ...
            this(
                ...
                t1,
                t2,
                new ArrayList(OreDictionary.getOres(in))
            );
        }
        ...
        private Processes(
            ...
            this(
                ...
                t1,
                t2,
                ReikaJavaLibrary.makeListFrom(in)
            );
        }
    ...
    public boolean canRunOutput(Processes p) {
        return this.hasInputItem(p)
            && (!p.hasIntermediate()
                || this.getIntermediate() >= p.intermediateFluidConsumed)
            && this.canAcceptMoreOutput(p.outputFluidProduced);
    }
    ...
    private boolean isFluorite(ItemStack is) {
        return ReactorItems.FLUORITE.matchWith(is)
            || ReikaItemHelper.isInOreTag(is, "gemFluorite");
    }

    public boolean canRunIntermediate(Processes p) {
        return this.getInput() > 0
            && this.canAcceptMoreIntermediate(p.intermediateFluidProduced)
            && this.hasFluorite();
    }
    ...
    private void getFluidContainers() {
        if (inv[1] != null) {
            ...
            if (fs != null && Processes.processMap.get(fs.getFluid().getName()) != null
                && this.canAcceptMoreInput(fs.amount)) {
    ...
    public boolean canAcceptMoreInput(int amt) {
        return input.getFluid() == null
            || input.getFluid().amount + amt <= input.getCapacity();
    }

    public boolean canAcceptMoreIntermediate(int amt) {
        return intermediate.getFluid() == null
            || intermediate.getFluid().amount + amt <= intermediate.getCapacity();
    }

    public boolean canAcceptMoreOutput(int amt) {
        return output.getFluid() == null
            || output.getFluid().amount + amt <= output.getCapacity();
    }
    ...
    public boolean canFill(ForgeDirection from, Fluid fluid) {
        for (int i = 0; i < Processes.list.length; i++) {
            if (Processes.list[i].inputFluid.equals(fluid))
                return true;
            if (Processes.list[i].intermediateFluid != null
                && Processes.list[i].intermediateFluid.equals(fluid))
                return true;
        }
        return false;
    }
