...
public class TileEntitySynthesizer extends TileEntityInventoriedReactorBase
    implements IFluidHandler, ThermalMachine, PipeConnector, IHasWork {
    ...
    public static enum FluidSynthesis {
        AMMONIA(
            ...
            50,
            0,
            constructItemMatch("dustQuicklime", ReactorStacks.lime, 1),
            constructItemMatch("dustAmmonium", ReactorStacks.ammonium, 1)
        ),
        HOTLIFBE(
            ReactorCraft.LIFBe_fuel, ReactorCraft.LIFBe_fuel_preheat, 50, 50, 350, 100, 5
        );
        ...
        private FluidSynthesis(
            Fluid in, Fluid out, int amtin, int amtout, int temp, int time, int tc
        ) {
        ...
        private FluidSynthesis(
            ...
            int time,
            int tc,
            FlexibleIngredient a,
            FlexibleIngredient b
        ) {
            input = in;
            output = out;
            fluidConsumed = amtin;
            ...
            if (fluidMap.containsKey(input))
                throw new IllegalArgumentException(
                    "Fluid " + input.getName() + " already mapped to a recipe!"
                );
        ...
        public boolean usesItem(ItemStack item) {
            return (itemA != null && itemA.match(item))
                || (itemB != null && itemB.match(item));
        }
        ...
        private static FlexibleIngredient
        constructItemMatch(String ore, ItemStack is, int amt) {
        ...
        public int getDuration(int temperature) {
            return Math.max(
                5, baseDuration - temperatureSpeedCurve * (temperature - minTemp) / 100
            );
        }
    }
    ...
    public static void addRecipe(
        ...
        Class[] types = new Class[] { Fluid.class,
                                      Fluid.class,
                                      int.class,
                                      int.class,
                                      int.class,
                                      int.class,
                                      int.class,
                                      FlexibleIngredient.class,
                                      FlexibleIngredient.class };
        Object[] args = new Object[] { in, out, amtin, amtout, temp, time, curve, a, b };
        FluidSynthesis c
            = EnumHelper.addEnum(FluidSynthesis.class, name.toUpperCase(), types, args);
    }
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (recipe != null && water.getLevel() >= recipe.fluidConsumed
            && temperature >= recipe.minTemp
            && tank.canTakeIn(recipe.output, recipe.fluidProduced)) {
            steptimer.update();
    ...
    public void updateTemperature(World world, int x, int y, int z, int meta) {
        ...
        ForgeDirection waterside
            = ReikaWorldHelper.checkForAdjMaterial(world, x, y, z, Material.water);
        if (waterside != null) {
            Tamb /= 2;
        }
        ...
        ForgeDirection fireside
            = ReikaWorldHelper.checkForAdjBlock(world, x, y, z, Blocks.fire);
        if (fireside != null) {
            Tamb += 200;
        }
    ...
    private void getWaterBuckets() {
        if (inv[0] != null && inv[0].stackSize == 1
            && inv[0].getItem() == Items.water_bucket
            && water.canTakeIn(
                FluidRegistry.WATER, FluidContainerRegistry.BUCKET_VOLUME
            )) {
