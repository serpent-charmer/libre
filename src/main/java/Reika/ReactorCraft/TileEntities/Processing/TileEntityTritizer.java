...
public class TileEntityTritizer
    extends TileEntityReactorBase implements ReactorCoreTE, PipeConnector, IFluidHandler {
    ...
    public boolean onNeutron(EntityNeutron e, World world, int x, int y, int z) {
        ...
        if (type.canIrradiateMaterials()) {
            ...
            if (!world.isRemote && this.canMake(r)
                && ReikaRandomHelper.doWithChance(r.chance)) {
                this.make(r);
                return true;
            }
        }
    ...
    private boolean canMake(Reactions r) {
        int amt = r.amount;
        return input.getLevel() >= amt && output.canTakeIn(amt)
            && input.getActualFluid().equals(r.input);
    }
    ...
    public static void addRecipe(String name, Fluid in, Fluid out, int chance, int amt) {
        ...
        Reactions c
            = EnumHelper.addEnum(Reactions.class, name.toUpperCase(), types, args);
