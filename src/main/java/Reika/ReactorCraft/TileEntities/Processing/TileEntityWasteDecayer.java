...
public class TileEntityWasteDecayer
    extends TileEntityInventoriedReactorBase implements ReactorCoreTE {
    ...
    public boolean onNeutron(EntityNeutron e, World world, int x, int y, int z) {
        ...
        if (!world.isRemote && type.canIrradiateMaterials()
            && ReikaRandomHelper.doWithChance(50)) {
            if (ReikaRandomHelper.doWithChance(
                    this.getDecayChance()
                    * e.getNeutronSpeed().getWasteConversionMultiplier()
                ))
                this.tryDecay();
            return true;
        }
    ...
    private double getDecayChance() {
        return temperature < BASE_TEMP
            ? ReikaMathLibrary.linterpolate(temperature, 20, BASE_TEMP, 0, 5)
            : ReikaMathLibrary.linterpolate(temperature, BASE_TEMP, OPTIMAL_TEMP, 5, 25);
    }
    ...
    private boolean tryDecay() {
        for (int i = 0; i < inv.length; i++) {
            ...
            if (is != null && TileEntityWasteUnit.isLongLivedWaste(is)) {
                //ReikaJavaLibrary.pConsole("Trying to decay
                //"+Isotopes.getIsotope(is.getItemDamage())+" in slot #"+i);
    ...
    private boolean tryDecay(int i, ItemStack is) { /*
         ...
         if (slot >= 0) {
             if (is.stackSize > 1)
                 is.stackSize--;
             inv[slot] =
         ReikaItemHelper.getSizedItemStack(Isotopes.getIsotope(is.getItemDamage()).getForcedFissionProduct(),
         2); return true;
         }
         return false;*/
    ...
    private ItemStack getItem(DecayData split, int amt) {
        if (split.isotope.getChemicalSymbol().equalsIgnoreCase("pb"))
            return ModOreList.LEAD.existsInGame() ? ModOreList.LEAD.getFirstProduct()
                                                  : null;
        else if (split.isotope instanceof Isotopes)
            return new ItemStack(
                ReactorItems.WASTE.getItemInstance(),
                amt,
                ((Isotopes) split.isotope).ordinal()
            );
        else
            return null;
    }
