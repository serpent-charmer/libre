...
public class TileEntityCentrifuge extends TileEntityInventoriedReactorBase
    implements IFluidHandler, ReactorPowerReceiver, PipeConnector {
    /**
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (powerHandler.getPower() > 0 && powerHandler.getOmega() >= MINSPEED
            ...
            if (recipe != null && tank.getLevel() >= recipe.fluidAmount
                && this.hasInventorySpace(recipe)) {
    ...
    private boolean hasInventorySpace(Centrifuging recipe) {
        if (inv[0] != null && !ReikaItemHelper.matchStacks(inv[0], recipe.outputA))
            return false;
        if (inv[1] != null && recipe.outputB != null
            && !ReikaItemHelper.matchStacks(inv[1], recipe.outputB))
            return false;
    ...
    public boolean canAcceptMoreUF6(int amt) {
        return tank.getFluid() == null
            || tank.getFluid().amount + amt <= tank.getCapacity();
    }
    ...
    public static enum Centrifuging {
        UF6(ReactorCraft.UF6,
            UF6_PER_DUST,
            ReactorStacks.fueldust,
            ReactorStacks.depdust,
            FUEL_CHANCE);
        ...
        private Centrifuging(
            Fluid f, int amt, ItemStack a, ItemStack b, float c, int ms, int sf
        ) {
            input = f;
            fluidAmount = amt;
            outputA = a;
            ...
            if (recipes.containsKey(f))
                throw new IllegalArgumentException(
                    "Fluid " + f.getName() + " already registered to a recipe!"
                );
            recipes.put(f, this);
        }
        ...
        public boolean produces(ItemStack result) {
            return ReikaItemHelper.matchStacks(outputA, result)
                || (outputB != null && ReikaItemHelper.matchStacks(outputB, result));
        }
    }
}
