...
public class TileEntityHeavyPump extends TileEntityReactorBase
    implements ReactorPowerReceiver, IFluidHandler, PipeConnector {
    ...
    public boolean hasABucket() {
        return tank.getFluid() != null
            && tank.getFluid().amount >= FluidContainerRegistry.BUCKET_VOLUME;
    }
    ...
    public static class HeavyWaterExtraction extends Extraction {
        ...
        protected boolean canPerform(World world, int x, int y, int z) {
            return this.isValidWorld(world) && y < MAXY
        ...
        private boolean isValidWorld(World world) {
            return ReactorCraft.config.isDimensionValidForHeavyWater(
    ...
    private static class MoltenLithiumExtraction extends Extraction {
        ...
        @Override
        protected boolean canPerform(World world, int x, int y, int z) {
            return y == this.getSurfaceY(world, x, y, z)
                && this.isLavaSurface(world, x, y, z);
        }
        ...
        private boolean isLavaSurface(World world, int x, int y, int z) {
            ...
            return (b == Blocks.lava || b == Blocks.flowing_lava)
                && (b2 != Blocks.lava && b2 != Blocks.flowing_lava);
        }
        ...
        protected int getExtractedAmount(World world, int x, int y, int z) {
            return world.provider.dimensionId == -1
                ? 10 + rand.nextInt(21) + rand.nextInt(51)
                : 10 + rand.nextInt(31);
        }
    }
