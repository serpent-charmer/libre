...
public class TileEntityWaterCell
    extends TileEntityReactorBase implements ReactorCoreTE, Temperatured {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (ReactorTiles.getTE(world, x, y - 1, z) == this.getTile()) {
            TileEntityWaterCell te
                = (TileEntityWaterCell) world.getTileEntity(x, y - 1, z);
            if (te.getLiquidState() == LiquidStates.EMPTY
                && this.getLiquidState() != LiquidStates.EMPTY) {
        ...
        if (m == MachineRegistry.RESERVOIR) {
            TileEntityReservoir te
                = (TileEntityReservoir) this.getAdjacentTileEntity(ForgeDirection.UP);
            if (te.getLevel() >= 1000) {
                Fluid f = te.getFluid();
        ...
        if (this.getLiquidState() == LiquidStates.EMPTY) {
            ...
            if (te instanceof IFluidHandler) {
                ...
                FluidStack liq = ic.drain(
                    ForgeDirection.DOWN, FluidContainerRegistry.BUCKET_VOLUME, false
                );
                if (liq != null && liq.amount >= FluidContainerRegistry.BUCKET_VOLUME) {
                    ic.drain(
                        ForgeDirection.DOWN, FluidContainerRegistry.BUCKET_VOLUME, true
                    );
    ...
    private boolean canIntakeFluid(Fluid f) {
        return f != null && LiquidStates.getState(f) != null
            && internalLiquid == LiquidStates.EMPTY;
    }
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (te instanceof Temperatured) {
                ...
                if (internalLiquid != LiquidStates.HEAVY
                    && tr.canDumpHeatInto(internalLiquid)) {
    ...
    @Override
    public boolean
    onNeutron(EntityNeutron e, World world, int x, int y, int z) { /*
if (ReikaMathLibrary.doWithChance(this.getChanceToStop())) {
temperature += ReikaThermoHelper.getTemperatureIncrease(ReikaThermoHelper.WATER_HEAT,
1000, ReikaNuclearHelper.getUraniumFissionNeutronE()); storedEnergy +=
ReikaNuclearHelper.getUraniumFissionNeutronE(); //3.8kJ per neutron (kinetic energy)
return true;
}*/
    ...
    public boolean canDumpHeatInto(LiquidStates liq) {
        return liq != LiquidStates.EMPTY
            && (this.getLiquidState().isWater() == liq.isWater());
    }
}
