...
public class TileEntityCPU extends TileEntityReactorBase
    implements ReactorPowerReceiver, TemperaturedReactorTyped, ReactorBlock, NeutronTile {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote) {
            ...
            if (reactor.isEmpty()) {
                layout.clear();
                int r = 12;
                reactor.recursiveAddCallbackWithBounds(
                    world,
                    x,
                    y,
                    z,
                ...
                for (int i = 0; i < reactor.getSize(); i++) {
                    ...
                    if (idx == ReactorTiles.CONTROL.getBlock()
                        && metax == ReactorTiles.CONTROL.getBlockMetadata()) {
                        TileEntityControlRod rod
                            = (TileEntityControlRod) world.getTileEntity(dx, dy, dz);
        ...
        if (layout.getNumberRods() > 0) {
            if ((temperature > this.getMaxTemperature()
                 || (!temperatureChecks.isEmpty()
                     && temperatureChecks.get(rand.nextInt(temperatureChecks.size()))
                             .getTemperature(this)
                         > this.getMaxTemperature()))
                && power >= this.getMinPower() * 4) {
    ...
    public int getRedstoneOverride() {
        return layout.isEmpty() ? 0
                                : 15 * layout.countLoweredRods() / layout.getNumberRods();
    }
    ...
    private static class TemperatureMonitor {
        ...
        public int getTemperature(TileEntityCPU te) {
            return ((LinkableReactorCore) location.getTileEntity(te.worldObj))
                .getTemperature();
        }
        ...
        public boolean equals(Object o) {
            return o instanceof TemperatureMonitor
                && ((TemperatureMonitor) o).location.equals(location);
        }
    }
}
