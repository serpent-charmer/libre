...
public class TileEntityReactorBoiler
    extends TileEntityNuclearBoiler implements SteamTile {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        //ReikaJavaLibrary.pConsole(y+":"+steam+":"+temperature+":"+fluid.name()+":"+tank,
        //Side.SERVER);

        //ReikaJavaLibrary.pConsole("T: "+temperature+"    W: "+tank.getLevel()+"    S:
        //"+steam, Side.SERVER);
    ...
    private void detonateAmmonia(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < pipes.getSize(); i++) {
            ...
            ReikaItemHelper.dropItem(
                world, c.xCoord, c.yCoord, c.zCoord, new ItemStack(Items.netherbrick)
            );
        }
        ...
        ReikaItemHelper.dropItem(
            ...
            y,
            z,
            ReikaItemHelper.getSizedItemStack(ItemStacks.scrap, 8 + rand.nextInt(18))
        );
    ...
    private boolean canBoilTankLiquid() {
        ...
        if (temperature < Tamb + 50)
            return false;
        return fluid == WorkingFluid.EMPTY
            || tank.getActualFluid().equals(fluid.getFluid());
    }
