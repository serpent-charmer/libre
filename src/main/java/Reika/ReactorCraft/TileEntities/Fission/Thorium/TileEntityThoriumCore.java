...
public class TileEntityThoriumCore extends TileEntityNuclearCore
    implements InertIInv, IFluidHandler, PipeConnector, IPipeConnection {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote) {
            ...
            if (timer2.checkCap()) {
                ...
                (TileEntitySodiumHeater)world.getTileEntity(dx, dy, dz); int dTemp =
                temperature-te.getTemperature(); if (dTemp > 0) { temperature -= dTemp/16;
                        te.setTemperature(te.getTemperature()+dTemp/16);
                    }
                }
                     */
                    if (r == this.getTile()) {
                        this.balanceLiquidsWith((TileEntityThoriumCore
                        ) this.getAdjacentTileEntity(dir));
                    }
                }
            }
        }
    ...
    protected int getRestingTemperature(World world, int x, int y, int z) {
        return fuelTank.getActualFluid() == ReactorCraft.LIFBe_fuel_preheat
            ? 250
            : super.getRestingTemperature(world, x, y, z);
    }
    ...
    @Override
    protected int
    getAmbientHeatLossFactor(World world, int x, int y, int z, int base, int Tamb) {
    ...
    protected int getHeatConductionFraction(TemperaturedReactorTyped other) {
        return other.getReactorType() == ReactorType.FISSION
            ? 2
            : super.getHeatConductionFraction(other);
    }

    @Override
    protected float getHeatConductionEfficiency(TemperaturedReactorTyped other) {
        boolean rest
            = temperature - this.getRestingTemperature(worldObj, xCoord, yCoord, zCoord)
            < 50;
        ...
        if (!world.isRemote) {
            if (e.getType().canTriggerFission()
                && ReikaRandomHelper.doWithChance(
                    e.getNeutronSpeed().getInteractionMultiplier()
                )
                && e.getType() != NeutronType.BREEDER
                && ReikaRandomHelper.doWithChance(this.getNeutronInteractionChance())) {
                if (this.checkPoisonedChance())
                    return true;
                if (ReikaRandomHelper.doWithChance(this.getNeutronChance())
                    && this.hasFuel()) {
                    fuelTank.removeLiquid(CYCLE_AMOUNT);
                    fuelTankOut.addLiquid(
                        CYCLE_AMOUNT, FluidRegistry.getFluid("rc hot lifbe")
                    );
                    temperature += 50;
    ...
    private double getNeutronInteractionChance() {
        ...
        return (1 - f)
            + f
            * ReikaMathLibrary.cosInterpolation(
                this.getMinTemperature(), this.getMaxTemperature(), temperature
            );
    }
    ...
    private double getNeutronChance() {
        return 50
            - 40
            * Math.sqrt(
                (temperature - this.getMinTemperature())
                / (double) (this.getMaxTemperature() - this.getMinTemperature())
            );
    }
    ...
    protected boolean checkPoisonedChance() {
        return ReikaRandomHelper.doWithChance(
            0.875 * Math.pow(wasteTank.getLevel() / (double) wasteTank.getCapacity(), 1.6)
        );
    }
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        return this.canFill(from, resource.getFluid()) ? fuelTank.fill(resource, doFill)
                                                       : 0;
    }
    ...
    public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain) {
        if (this.canDrain(from, resource.getFluid())) {
            return from.offsetY == 0 ? this.isWastePipe(from)
                    ? wasteTank.drain(resource.amount, doDrain)
                    : null
                                     : fuelTankOut.drain(resource.amount, doDrain);
        }
        return null;
    }
    ...
    public FluidStack drain(ForgeDirection from, int maxDrain, boolean doDrain) {
        if (this.canDrain(from, null)) {
            return from.offsetY == 0
                ? this.isWastePipe(from) ? wasteTank.drain(maxDrain, doDrain) : null
                : fuelTankOut.drain(maxDrain, doDrain);
        }
        return null;
    }
    ...
    public boolean canFill(ForgeDirection from, Fluid fluid) {
        return from == ForgeDirection.UP
            && (fluid == ReactorCraft.LIFBe_fuel
                || fluid == ReactorCraft.LIFBe_fuel_preheat);
    }

    @Override
    public boolean canDrain(ForgeDirection from, Fluid fluid) {
        return from != ForgeDirection.UP
            && (ReikaFluidHelper.isFluidDrainableFromTank(fluid, wasteTank)
                || ReikaFluidHelper.isFluidDrainableFromTank(fluid, fuelTankOut));
    }
    ...
    public ConnectOverride overridePipeConnection(PipeType type, ForgeDirection with) {
        return type == PipeType.FLUID ? ConnectOverride.CONNECT
    ...
    public boolean canConnectToPipeOnSide(MachineRegistry m, ForgeDirection side) {
        return side == ForgeDirection.UP ? m == MachineRegistry.FUELLINE
                                         : m.isStandardPipe();
    }
