...
public class TileEntityBreederCore extends TileEntityNuclearCore {
            ...
            for (int i = 2; i < 6; i++) {
                ...
                ReactorTiles r = ReactorTiles.getTE(world, dx, dy, dz); /*
                 if (r == ReactorTiles.COOLANT) {
                     TileEntityWaterCell w = (TileEntityWaterCell)world.getTileEntity(dx,
                 dy, dz); int T = w.getTemperature(); int dT = temperature-T; if (dT > 0)
                 { w.setTemperature(T+dT/4); temperature -= dT/4;
                     }
                 }*/
                if (r == ReactorTiles.SODIUMBOILER) {
                    TileEntitySodiumHeater te
                        = (TileEntitySodiumHeater) world.getTileEntity(dx, dy, dz);
    ...
    public boolean isFissile() {
        return ReikaInventoryHelper.locateInInventory(
                   ReactorItems.BREEDERFUEL.getItemInstance(), inv
               )
            != -1;
    }
    ...
    public boolean onNeutron(EntityNeutron e, World world, int x, int y, int z) {
        ...
        if (!world.isRemote) {
            if (this.checkPoisonedChance())
                return true;
            if (ReikaRandomHelper.doWithChance(25 + temperature / 100) && this.isFissile()
                ...
                if (slot != -1) {
                    if (e.getType().canTriggerFuelConversion()
                        && ReikaRandomHelper.doWithChance(
                            5 * e.getNeutronSpeed().getWasteConversionMultiplier()
                        )) {
                        ...
                        if (dmg == ReactorItems.BREEDERFUEL.getNumberMetadatas() - 1) {
                            ...
                            ReactorAchievements.PLUTONIUM.triggerAchievement(
                                this.getPlacer()
                            );
                        } else {
                            inv[slot]
                                = ReactorItems.BREEDERFUEL.getStackOfMetadata(dmg + 1);
                        }
                        temperature += 50;
                    } else {
