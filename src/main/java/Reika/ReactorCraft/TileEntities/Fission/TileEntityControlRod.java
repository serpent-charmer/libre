...
public class TileEntityControlRod
    extends TileEntityReactorBase implements LinkableReactorCore {
    ...
    public void toggle(boolean sound, boolean spread) {
        ...
        if (sound)
            ReactorSounds.CONTROL.playSoundAtBlock(
                worldObj, xCoord, yCoord, zCoord, 1, 1.3F
            );
    }
    ...
    public void drop(boolean sound) {
        if (sound)
            if (rodOffset > MINOFFSET && motion != Motions.SCRAM)
                ReactorSounds.SCRAM.playSoundAtBlock(
                    worldObj, xCoord, yCoord, zCoord, 1, 1F
                );
