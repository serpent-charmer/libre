...
public class LegacyPotionHealth extends PotionHealth {
    ...
    public void performEffect(EntityLivingBase par1EntityLivingBase, int par2) {
        if ((id != heal.id || par1EntityLivingBase.isEntityUndead())
            && (id != harm.id || !par1EntityLivingBase.isEntityUndead())) {
            if (id == harm.id && !par1EntityLivingBase.isEntityUndead()
                || id == heal.id && par1EntityLivingBase.isEntityUndead()) {
                par1EntityLivingBase.attackEntityFrom(
                    DamageSource.magic, this.getAttackAmount() << par2
                );
            }
        } else {
    ...
    public void affectEntity(
        ...
        if ((id != heal.id || par2EntityLivingBase.isEntityUndead())
            && (id != harm.id || !par2EntityLivingBase.isEntityUndead())) {
            if (id == harm.id && !par2EntityLivingBase.isEntityUndead()
                || id == heal.id && par2EntityLivingBase.isEntityUndead()) {
                ...
                if (par1EntityLivingBase == null) {
                    ...
                    par2EntityLivingBase.attackEntityFrom(
                        DamageSource.causeIndirectMagicDamage(
                            par2EntityLivingBase, par1EntityLivingBase
                        ),
                        j
                    );
                }
