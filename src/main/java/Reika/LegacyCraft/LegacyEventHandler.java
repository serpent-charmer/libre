...
public class LegacyEventHandler {
    ...
    public void enforceMobs(LivingUpdateEvent evt) {
        if (evt.entityLiving instanceof EntityMob
            && !(evt.entityLiving instanceof Spitter)) {
            if (!LegacyOptions.MOBPICKUP.getState()) {
                ...
                for (int i = held ? 0 : 1; i <= 4; i++) {
                    ...
                    if (is != null) {
                        ...
                        float f = evt.entityLiving instanceof EntityLiving
                            ? ((EntityLiving) evt.entityLiving).equipmentDropChances[i]
                            : 1;
                        if (f >= 1 || ReikaRandomHelper.doWithChance(f))
                            ReikaItemHelper.dropItem(
                                evt.entityLiving.worldObj,
                ...
                if (!es.getSheared() && !es.isChild() && es.getHealth() > 0) {
                    ...
                    for (int i = 0; i < n; i++) {
                        ReikaItemHelper.dropItem(
                            ...
                            es.posY + 0.5,
                            es.posZ,
                            new ItemStack(Blocks.wool, 1, es.getFleeceColor())
                        );
                    }
                    es.setSheared(true);
                }
            ...
            if (b == Blocks.sapling) {
                ...
                if (crop != null) {
                    ...
                    if (mod == ModCropList.MAGIC) {
                        //mod.makeRipe(evt.world, evt.x, evt.y, evt.z); //maybe want to
                        //specifically exclude magic crops?
        ...
        if (out != null) {
            if (LegacyOptions.GOLDENAPPLE.getState()) {
                if (ReikaItemHelper.matchStacks(
                        new ItemStack(Items.golden_apple, 1, 0), out
                    )) {
                    ReikaRecipeHelper.replaceIngredientInRecipe(
            ...
            if (LegacyOptions.OLDMELON.getState()) {
                if (ir instanceof ShapedRecipes && out.getItem() == Items.speckled_melon
                    && evt.isVanillaPass) {
                    evt.setCanceled(true);
                }
            }
                    ...
                    if (ReikaItemHelper.matchStacks(out, ReikaItemHelper.redDye)) {
                        if (sr.recipeItems.size() == 1) {
                            ...
                            if (o instanceof ItemStack
                                ...
                                sr.getRecipeOutput().stackSize
                                    = Math.max(2, sr.getRecipeOutput().stackSize);
                            }
                        }
                    }
                }
