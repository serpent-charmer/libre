...
public class MobTransformer implements IClassTransformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] data) {
        ...
        for (Patches p : Patches.list) {
            try {
                ...
                ReikaASMHelper.logError(
                    "Unable to patch mob class '" + cn.name + "' ^ '" + cn.superName
                    + "': " + e.toString()
                );
                /*
    ...
    private byte[] write(Patches p, byte[] data) {
        ...
        cn.accept(writer);
        ReikaASMHelper.log(
            "Successfully applied " + p + " ASM handler to " + cn.name + "!"
        );
    ...
    private static void patchAI(ClassNode cn) {
        MethodNode m = LegacyASMHandler.getOrCreateMethod(
            cn, "func_70650_aV", "isAIEnabled", "()Z"
        );
        m.instructions.add(new FieldInsnNode(
            Opcodes.GETSTATIC,
            "Reika/LegacyCraft/LegacyOptions",
            "NEWAI",
            "LReika/LegacyCraft/LegacyOptions;"
        ));
    ...
    private static void patchMoveSpeed(ClassNode cn) {
        MethodNode m = LegacyASMHandler.getOrCreateMethod(
            cn, "func_70689_ay", "getAIMoveSpeed", "()F"
        );
        m.instructions.add(new VarInsnNode(Opcodes.ALOAD, 0));
        m.instructions.add(new MethodInsnNode(
            Opcodes.INVOKESPECIAL, cn.superName, "getAIMoveSpeed", "()F", false
        ));
        m.instructions.add(new MethodInsnNode(
    ...
    private static void patchEntityAttr(ClassNode cn) {
        MethodNode m = LegacyASMHandler.getOrCreateMethod(
            cn, "func_110147_ax", "applyEntityAttributes", "()V", false
        );
        InsnList li = new InsnList();
        li.add(new VarInsnNode(Opcodes.ALOAD, 0));
        li.add(new MethodInsnNode(
        ...
        if (m.instructions.size() == 0) {
            m.instructions.add(new VarInsnNode(Opcodes.ALOAD, 0));
            m.instructions.add(new MethodInsnNode(
                Opcodes.INVOKESPECIAL, cn.superName, m.name, m.desc, false
            ));
            //m.instructions.add(new MethodInsnNode(Opcodes.INVOKESPECIAL,
            //"net/minecraft/entity/monster/EntityMob", m.name, m.desc, false));
            m.instructions.add(li);
    ...
    private static void patchToolEnchant(ClassNode cn) {
        MethodNode m = LegacyASMHandler.getOrCreateMethod(
            cn, "func_82162_bC", "enchantEquipment", "()V"
        );
        ...
        m.instructions.add(new VarInsnNode(Opcodes.ALOAD, 0));
        m.instructions.add(
            new MethodInsnNode(Opcodes.INVOKESPECIAL, cn.superName, m.name, m.desc, false)
        );
    ...
    private static boolean extendsMob(ClassNode cn) {
        return cn.superName != null
            && (cn.superName.equals("net/minecraft/entity/monster/EntityMob")
                || cn.superName.equals("yg"));
    }
