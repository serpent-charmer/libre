...
public class LegacyASMHooks {
    /*
    public static int getSugarcaneColorization(int orig, BlockReed b, IBlockAccess iba,
    int x, int y, int z) { if (b == Blocks.reeds && LegacyOptions.SUGARCANE.getState()) {
            return 0xffffff;
        }
        return orig;
    }
    ...
    public static String getFlintAndSteelSound() {
        return LegacyOptions.FLINTSOUND.getState()
            ? LegacyCraft.getOldFlintAndSteelSound()
            : "fire.ignite";
    }
    ...
    public static EntityZombie.GroupData
    interceptZombieData(EntityZombie.GroupData data, EntityZombie e) {
        ...
        if (e instanceof EntityZombie) {
            ...
            if (LegacyOptions.OLDZOMBIES.getState()) {
                removeModifierByName(
                    e, SharedMonsterAttributes.knockbackResistance, "Random spawn bonus"
                );
                removeModifierByName(
                    e, SharedMonsterAttributes.followRange, "Random zombie-spawn bonus"
                );
                removeModifierByName(
                    e, EntityZombie.field_110186_bp, "Leader zombie bonus"
                );
                removeModifierByName(
                    e, SharedMonsterAttributes.maxHealth, "Leader zombie bonus"
                );
            }
            if (!LegacyOptions.ZOMBIEVILLAGER.getState())
    ...
    public static void applyEntityAttributes(EntityLiving e) {
        if (e instanceof EntityZombie) {
            if (LegacyOptions.OLDRANGE.getState())
                e.getEntityAttribute(SharedMonsterAttributes.followRange)
                    .setBaseValue(16.0D);
    ...
    public static void correctSkeletonType(EntitySkeleton es) {
        ...
        if ((es.getSkeletonType() == 1)
            != hell) { //No normal archer skeletons in the nether
            es.setSkeletonType(hell ? 1 : 0);
            es.setCurrentItemOrArmor(
                0, new ItemStack(hell ? Items.stone_sword : Items.bow)
            );
            es.setCombatTask();
        }
    }

    public static PathEntity getPathEntityToEntity(
        World world,
        Entity src,
        Entity tgt,
        float dist,
        boolean useOpenDoors,
        boolean useClosedDoors,
        boolean avoidWater,
        boolean canSwim
    ) {
        ...
        if (!LegacyOptions.NEWAI.getState() && src instanceof EntityMob
            && !(src instanceof TameHostile) && !(src instanceof Spitter)) {
            return getDumbPath(
                world,
                src,
                MathHelper.floor_double(tgt.posX),
                MathHelper.floor_double(tgt.posY),
                MathHelper.floor_double(tgt.posZ),
                dist
            );
        }
        ...
        PathEntity pathentity
            = (new PathFinder(
                   chunkcache, useOpenDoors, useClosedDoors, avoidWater, canSwim
               ))
                  .createEntityPathTo(src, tgt, dist);
        world.theProfiler.endSection();
        return pathentity;
    }

    public static PathEntity getEntityPathToXYZ(
        World world,
        Entity src,
        int x,
        int y,
        ...
        PathEntity pathentity
            = (new PathFinder(
                  ...
                  .createEntityPathTo(src, x, y, z, dist);
        world.theProfiler.endSection();
        return pathentity;
    }

    private static PathEntity
    getDumbPath(World world, Entity src, int x0, int y0, int z0, double dist) {
        ...
        if (src instanceof EntityLivingBase && src.onGround && src.posY < y0 - 0.5
