...
public class LegacyASMHandler implements IFMLLoadingPlugin {
    ...
    static void
    redirectInstanceFunctionCall(ClassNode cn, MethodInsnNode min, String name) {
    ...
    static void redirectInstanceFunction(ClassNode cn, MethodNode m, String name) {
        ...
        "Reika/LegacyCraft/ASM/LegacyASMHooks", name, sig, false);
         ...
         */
        MethodInsnNode call = ReikaASMHelper.rerouteMethod(
            cn, m, "Reika/LegacyCraft/ASM/LegacyASMHooks", name
        );
        //ReikaASMHelper.log(ReikaASMHelper.clearString(m.instructions));
        ReikaASMHelper.log(
            "Constructed redirect in " + ReikaASMHelper.clearString(m) + " to "
            + ReikaASMHelper.clearString(call).replace("\n", "") + "."
        );
    }

    static void
    redirectInstanceFunctionWithReturnHook(ClassNode cn, MethodNode m, String name) {
        ...
        MethodInsnNode call = new MethodInsnNode(
            Opcodes.INVOKESTATIC, "Reika/LegacyCraft/ASM/LegacyASMHooks", name, sig, false
        );
        calls.add(call);
        ...
        //ReikaASMHelper.log(ReikaASMHelper.clearString(m.instructions));
        ReikaASMHelper.log(
            "Constructed return-preserving redirect in " + ReikaASMHelper.clearString(m)
            + " to " + ReikaASMHelper.clearString(call).replace("\n", "") + "."
        );
    }

    static MethodNode
    getOrCreateMethod(ClassNode cn, String obf, String deobf, String desc) {
        return getOrCreateMethod(cn, obf, deobf, desc, true);
    }

    static MethodNode getOrCreateMethod(
        ClassNode cn, String obf, String deobf, String desc, boolean clear
    ) {
        MethodNode m = null;
        try {
            m = ReikaASMHelper.getMethodByName(cn, obf, deobf, desc);
        } catch (NoSuchASMMethodException e) {
            m = ReikaASMHelper.addMethod(
                cn,
                new InsnList(),
                FMLForgePlugin.RUNTIME_DEOBF ? obf : deobf,
                desc,
                Modifier.PUBLIC
            );
        }
    ...
    private static void
    patchPathCalc(ClassNode cn, String obf, String deobf, String sig) {
    ...
    public static class LegacyTransformer implements IClassTransformer {
        private static final MultiMap<String, ClassPatch> classes
            = new MultiMap().setNullEmpty();

        private static final Configuration config = new Configuration(
            new File((File) FMLInjectionData.data()[6], "config/Reika/LegacyCraft.cfg")
        );
                ...
                switch (this) {
                    case SUGARCANE: {
                        ...
                        if (!getConfig("Disable Biome Colors on Sugarcane", true)) {
                            ReikaASMHelper.log(
                                "Not applying " + this
                                + " ASM handler; disabled in config."
                            );
                            return data;
                        }
                    ...
                    case NETHERLAVA: {
                        ...
                        String gen = FMLForgePlugin.RUNTIME_DEOBF
                            ? "ars"
                        ...
                        while (it.hasNext()) {
                            ...
                            if (ain.getOpcode() == Opcodes.NEW) {
                                ...
                                if (tp.desc.equals(gen)) {
                                    tp.desc
                                        = "Reika/LegacyCraft/Overrides/WorldGenCustomNetherLava";
                                }
                    ...
                    case ENDERPORT: {
                        if (!getConfig(
                                "Disable Random Enderman Teleporting in Daylight", true
                            )) {
                            ReikaASMHelper.log(
                                "Not applying " + this
                                + " ASM handler; disabled in config."
                        ...
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "func_70636_d", "onLivingUpdate", "()V"
                        );
                        boolean prep = false;
                    ...
                    case FOLIAGE: {
                        if (!getConfig(
                                "Alpha Grass and Leaf Color", false
                        ...
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "func_77469_b", "getFoliageColorBirch", "()I"
                        );
                        ...
                        m = ReikaASMHelper.getMethodByName(
                            cn, "func_77466_a", "getFoliageColorPine", "()I"
                        );
                        ...
                        m = ReikaASMHelper.getMethodByName(
                            cn, "func_77468_c", "getFoliageColorBasic", "()I"
                        );
                    ...
                    case PORTAL1: {
                        if (!getConfig(
                                "Disable Entities Travelling Through Portals", false
                            )) {
                            ReikaASMHelper.log(
                                "Not applying " + this
                                + " ASM handler; disabled in config."
                        ...
                        insert.add(new TypeInsnNode(
                            Opcodes.INSTANCEOF, "net/minecraft/entity/player/EntityPlayer"
                        ));
                    ...
                    case ICEBLOCK: {
                        ...
                        String hell = FMLForgePlugin.RUNTIME_DEOBF ? "field_76575_d"
                                                                   : "isHellWorld";
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn,
                            "func_149636_a",
                        ...
                        for (int i = 0; i < m.instructions.size(); i++) {
                            ...
                            if (ain.getOpcode() == Opcodes.GETFIELD) {
                                ...
                                if (fin.name.equals(hell)) {
                                    ...
                                    ReikaASMHelper.log(
                                        "Successfully applied " + this + " ASM handler 1!"
                                    );
                                    break;
                                }
                            }
                        ...
                        for (int i = 0; i < m.instructions.size(); i++) {
                            ...
                            if (ain.getOpcode() == Opcodes.GETFIELD) {
                                ...
                                if (fin.name.equals(hell)) {
                                    ...
                                    m.instructions.insert(fin, new InsnNode(Opcodes.POP));
                                    ReikaASMHelper.log(
                                        "Successfully applied " + this + " ASM handler 2!"
                                    );
                                    break;
                                }
                            }
                    ...
                    case WATERPATCH: {
                        ...
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "func_149671_p", "registerBlocks", "()V"
                        );

                        AbstractInsnNode loc = ReikaASMHelper.getFirstInsnAfter(
                            m.instructions, 0, Opcodes.BIPUSH, 8
                        );
                        loc = ReikaASMHelper.getFirstOpcodeAfter(
                            m.instructions, m.instructions.indexOf(loc), Opcodes.NEW
                        );
                        TypeInsnNode type = (TypeInsnNode) loc;
                        int idx = m.instructions.indexOf(type);
                        type.desc = "net/minecraft/block/BlockDynamicLiquid";
                        MethodInsnNode min
                            = (MethodInsnNode) ReikaASMHelper.getFirstOpcodeAfter(
                                m.instructions, idx, Opcodes.INVOKESPECIAL
                            );
                        min.owner = type.desc;

                        type = (TypeInsnNode) ReikaASMHelper.getFirstOpcodeAfter(
                            m.instructions, idx + 1, Opcodes.NEW
                        );
                        idx = m.instructions.indexOf(type);
                        type.desc = "net/minecraft/block/BlockStaticLiquid";
                        min = (MethodInsnNode) ReikaASMHelper.getFirstOpcodeAfter(
                            m.instructions, idx, Opcodes.INVOKESPECIAL
                        );
                        min.owner = type.desc;
                        ...
                        for (int i = 0; i < m.instructions.size(); i++) {
                            ...
                            if (ain.getOpcode() == Opcodes.LDC) {
                                ...
                                if ("fire.ignite".equals(ldc.cst)) {
                                    ReikaASMHelper.replaceInstruction(
                                        m.instructions,
                                        ain,
                                        new MethodInsnNode(
                                            Opcodes.INVOKESTATIC,
                                            "Reika/LegacyCraft/ASM/LegacyASMHooks",
                                            "getFlintAndSteelSound",
                                            "()Ljava/lang/String;",
                                            false
                                        )
                                    );
                    ...
                    case CREEPERFALL: {
                        ...
                        if (ReikaASMHelper.removeMethod(
                                cn,
                                FMLForgePlugin.RUNTIME_DEOBF ? "func_70069_a" : "fall",
                                "(F)V"
                            )
                            == null)
                    ...
                    case EQUIPDMG: {
                        ...
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "func_82160_b", "dropEquipment", "(ZI)V"
                        );
                        String func = FMLForgePlugin.RUNTIME_DEOBF ? "func_77964_b"
                                                                   : "setItemDamage";
                        for (int i = 0; i < m.instructions.size(); i++) {
                            AbstractInsnNode ain = m.instructions.get(i);
                            if (ain.getOpcode() == Opcodes.INVOKEVIRTUAL) {
                                ...
                                if (func.equals(min.name)) {
                                    m.instructions.insertBefore(
                                        min, new InsnNode(Opcodes.POP)
                                    );
                                    m.instructions.insertBefore(
                                        min, new InsnNode(Opcodes.ICONST_0)
                                    );
                                    i += 3;
                                }
                            }
                    ...
                    case ZOMBIEHOOKS: {
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            ...
                            "func_110161_a",
                            "onSpawnWithEgg",
                            "(Lnet/minecraft/entity/IEntityLivingData;)Lnet/minecraft/entity/IEntityLivingData;"
                        );
                        FieldInsnNode fin = ReikaASMHelper.getFirstFieldCallByName(
                            cn, m, "field_142046_b"
                        );
                        int var = ((VarInsnNode) fin.getPrevious()).var;
                        VarInsnNode vin = (VarInsnNode) ReikaASMHelper.getLastInsnBefore(
                            m.instructions,
                            m.instructions.indexOf(fin),
                            Opcodes.ASTORE,
                            var
                        );
                        m.instructions.insertBefore(
                            vin, new VarInsnNode(Opcodes.ALOAD, 0)
                        );
                        m.instructions.insertBefore(
                            vin,
                            new MethodInsnNode(
                                ...
                                "Reika/LegacyCraft/ASM/LegacyASMHooks",
                                "interceptZombieData",
                                "(Lnet/minecraft/entity/monster/EntityZombie$GroupData;Lnet/minecraft/entity/monster/EntityZombie;)Lnet/minecraft/entity/monster/EntityZombie$GroupData;",
                                false
                            )
                        );
                        ReikaASMHelper.log(
                    ...
                    case ZOMBIETOOLS: {
                        ...
                        if (ReikaASMHelper.removeMethod(
                                cn,
                                FMLForgePlugin.RUNTIME_DEOBF ? "func_82164_bB"
                                                             : "addRandomArmor",
                                "()V"
                            )
                            == null)
                            throw new NoSuchASMMethodException(
                                cn, "addRandomArmor", "()V"
                            );
                        ReikaASMHelper.log(
                            "Successfully applied " + this + " ASM handler!"
                    ...
                    case MOBARMOR: {
                        ...
                        MethodNode m = getOrCreateMethod(
                            cn, "func_82164_bB", "addRandomArmor", "()V"
                        );
                        m.instructions.add(new InsnNode(Opcodes.RETURN));
                        ReikaASMHelper.log(
                            "Successfully applied " + this + " ASM handler!"
                    ...
                    case ZOMBIEFIRE: {
                        ...
                        MethodInsnNode min = ReikaASMHelper.getFirstMethodCallByName(
                            cn,
                            m,
                            FMLForgePlugin.RUNTIME_DEOBF ? "func_70027_ad" : "isBurning"
                        );
                        AbstractInsnNode prev = min.getPrevious();
                        ReikaASMHelper.replaceInstruction(
                            m.instructions, min, new InsnNode(Opcodes.ICONST_0)
                        );
                    ...
                    case SKELLYRATE: {
                        ...
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "<init>", "(Lnet/minecraft/world/World;)V"
                        );
                        IntInsnNode iin = (IntInsnNode) ReikaASMHelper.getFirstInsnAfter(
                            m.instructions, 0, Opcodes.BIPUSH, 60
                        );
                    ...
                    case SKELLYFIRE: {
                        ...
                        FieldInsnNode fin = ReikaASMHelper.getFirstFieldCallByName(
                            cn,
                            m,
                            FMLForgePlugin.RUNTIME_DEOBF ? "field_77343_v" : "flame"
                        );
                        JumpInsnNode jin
                            = (JumpInsnNode) ReikaASMHelper.getFirstOpcodeAfter(
                                m.instructions, m.instructions.indexOf(fin), Opcodes.IFGT
                            );
                        jin.setOpcode(Opcodes.IF_ICMPGT);
                        m.instructions.insertBefore(
                            jin, new IntInsnNode(Opcodes.BIPUSH, 100)
                        );
                        ReikaASMHelper.log(
                            "Successfully applied " + this + " ASM handler!"
                        );
                    ...
                    case SILENTVILLAGERS: {
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "func_70639_aQ", "getLivingSound", "()Ljava/lang/String;"
                        );
                        redirectInstanceFunctionWithReturnHook(cn, m, "getVillagerSound");

                        m = ReikaASMHelper.getMethodByName(
                            cn, "func_70621_aR", "getHurtSound", "()Ljava/lang/String;"
                        );
                        redirectInstanceFunctionWithReturnHook(cn, m, "getVillagerSound");

                        m = ReikaASMHelper.getMethodByName(
                            cn, "func_70673_aS", "getDeathSound", "()Ljava/lang/String;"
                        );
                        redirectInstanceFunctionWithReturnHook(cn, m, "getVillagerSound");
                        ReikaASMHelper.log(
                            "Successfully applied " + this + " ASM handler!"
                    ...
                    case ENDERSOUNDS: {
                        ...
                        LdcInsnNode ldc = (LdcInsnNode) ReikaASMHelper.getFirstInsnAfter(
                            m.instructions, 0, Opcodes.LDC, "mob.endermen.stare"
                        );
                        ReikaASMHelper.replaceInstruction(
                            m.instructions, ldc.getNext(), new InsnNode(Opcodes.FCONST_0)
                        );
                        ReikaASMHelper.log(
                            "Successfully applied " + this + " ASM handler!"
                        );
                    ...
                    case DUMBPATH: {
                        patchPathCalc(
                            ...
                            "(Lnet/minecraft/entity/Entity;Lnet/minecraft/entity/Entity;FZZZZ)Lnet/minecraft/pathfinding/PathEntity;"
                        );
                        patchPathCalc(
                            cn,
                            "func_72844_a",
                            "getEntityPathToXYZ",
                            "(Lnet/minecraft/entity/Entity;IIIFZZZZ)Lnet/minecraft/pathfinding/PathEntity;"
                        );
                        ReikaASMHelper.log(
                            "Successfully applied " + this + " ASM handler!"
                        );
