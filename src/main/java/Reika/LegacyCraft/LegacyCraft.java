...
public class LegacyCraft extends DragonAPIMod {
    ...
    public static final LegacyConfig config
        = new LegacyConfig(instance, LegacyOptions.optionList, null);
    ...
    public void preload(FMLPreInitializationEvent evt) {
        ...
        if (LegacyOptions.NETHERICE.getState()) {
            /*
            BlockLiquid b1 = (BlockLiquid)(new
            BlockLegacyDynamicLiquid(Material.water).disableStats().setHardness(100.0F).setLightOpacity(3).setBlockName("water").setBlockTextureName("water_flow"));
            BlockLiquid b2 = (BlockLiquid)(new
            BlockLegacyStaticLiquid(Material.water).disableStats().setHardness(100.0F).setLightOpacity(3).setBlockName("water").setBlockTextureName("water_still"));
            ...
            try {
                ...
                String f2 = FMLForgePlugin.RUNTIME_DEOBF ? "field_150358_i" :
            "flowing_water"; ReikaReflectionHelper.setFinalField(Blocks.class, f1, null,
            b2); ReikaReflectionHelper.setFinalField(Blocks.class, f2, null, b1);
                logger.log("Patched water block fields to ensure functionality of Nether
            Ice behavior");
            }
            catch (Exception e) {
                e.printStackTrace();
                throw new RegistrationException(this, "Could not patch water block
            fields!");
            }
             */
            if (Blocks.water.getClass() != BlockStaticLiquid.class
                || Blocks.flowing_water.getClass() != BlockDynamicLiquid.class) {
    ...
    public void load(FMLInitializationEvent event) {
        ...
        if (LegacyOptions.SILVERFISH.getState()) {
            if (BiomeGenBase.extremeHills instanceof BiomeGenHills) { //Because BoP
                ...
                BiomeGenHills ex3 = (BiomeGenHills
                ) BiomeGenBase.biomeList[BiomeGenBase.extremeHills.biomeID + 128];
                BiomeGenHills ex4 = (BiomeGenHills) BiomeGenBase.extremeHillsPlus;
                BiomeGenHills ex5 = (BiomeGenHills
                ) BiomeGenBase.biomeList[BiomeGenBase.extremeHillsPlus.biomeID + 128];
        ...
        if (LegacyOptions.OLDBOOK.getState()) {
            GameRegistry.addRecipe(
                new ItemStack(Items.book), "P", "P", "P", 'P', Items.paper
            );
        }

        if (LegacyOptions.OLDMELON.getState()) {
            GameRegistry.addShapelessRecipe(
                new ItemStack(Items.speckled_melon), Items.melon, Items.gold_nugget
            );
        }

        if (LegacyOptions.ROSES.getState()) {
            ItemMultiTexture item
                = (ItemMultiTexture) Item.getItemFromBlock(Blocks.red_flower);
    ...
    public void setIcons(TextureStitchEvent evt) {
        if (evt.map.getTextureType() == 0) {
            if (LegacyOptions.ROSES.getState())
                Blocks.red_flower.field_149861_N[0]
                    = evt.map.registerIcon("legacycraft:rose");

            if (LegacyOptions.ALPHAGRASS.getState()) {
                Blocks.grass.field_149994_N
                    = evt.map.registerIcon("legacycraft:alpha/grass_side");
                Blocks.grass.blockIcon
                    = evt.map.registerIcon("legacycraft:alpha/grass_side");
                Blocks.grass.field_149991_b
                    = evt.map.registerIcon("legacycraft:alpha/grass_top");
                ...
                for (int m = 0; m <= 1; m++) {
                    for (int i = 0; i <= 1; i++) {
                        String alpha = i == 0 ? "trans" : "opq";
                        String s = "legacycraft:alpha/leaf_" + 0 + "_"
                            + alpha; //"legacycraft:alpha/leaf2_"+m+"_"+alpha;
                        Blocks.leaves2.field_150129_M[i][m] = evt.map.registerIcon(s);
                    }
                }

                for (int i = 1; i <= 2; i++) {
                    Blocks.tallgrass.field_149870_b[i] = evt.map.registerIcon(
                        "legacycraft:alpha/" + Blocks.tallgrass.field_149871_a[i]
                    );
                }

                //for (int m = 0; m <= 5; m++) {
                BlockSapling.field_149881_b[0]
                    = evt.map.registerIcon("legacycraft:alpha/sapling");
                //}
                ...
                SimpleReloadableResourceManager mgr
                    = (SimpleReloadableResourceManager) Minecraft.getMinecraft()
                          .getResourceManager();
                Iterator<IResourceManagerReloadListener> it
                    = mgr.reloadListeners.iterator();
                while (it.hasNext()) {
