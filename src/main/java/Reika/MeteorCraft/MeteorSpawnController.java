...
public class MeteorSpawnController implements TickHandler {
            ...
            if (isShowering) {
                ...
                if (ReikaRandomHelper.doWithChance(this.getSpawnChanceDuringShower())) {
                    if (world.playerEntities.size() > 0) {
                        EntityPlayer ep = (EntityPlayer) world.playerEntities.get(
                            rand.nextInt(world.playerEntities.size())
                        );
                        if (ep != null) {
                ...
                if (chance > 0 && MeteorOptions.SHOWER.getState()
                    && ReikaRandomHelper.doWithChance(0.01D / chance)) {
    ...
    public EntityMeteor createMeteor(World world, EntityPlayer ep) {
        ...
        EntityMeteor e
            = new EntityMeteor(world, dx, 1024, dz, MeteorType.getWeightedType());
        ...
        //MeteorType.NETHERRACK); else 	e = new EntityMeteor(world, dx,
        //world.provider.getHeight(), dz, MeteorType.STONE);
        return e;
    }
