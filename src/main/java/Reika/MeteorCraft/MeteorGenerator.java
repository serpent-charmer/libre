...
public class MeteorGenerator {
    ...
    private final HashMap<MeteorType, InvertedWeightedRandom<BlockKey>> viableOres
        = new HashMap();

    private final HashMap<ModOreList, HashMap<MeteorType, ArrayList<Integer>>> metas
        = new HashMap();
    private final HashMap<ModOreList, HashMap<MeteorType, ArrayList<Block>>> ids
        = new HashMap();
    ...
    static {
        softMeteorMats = new HashSet(ReikaJavaLibrary.makeListFrom(
            Material.circuits,
            Material.glass,
            Material.snow,
            Material.ice,
    ...
    private MeteorGenerator() {
        ...
        this.addID(
            ModOreList.ESSENCE, MeteorType.STONE, MagicCropHandler.getInstance().oreID
        );
        this.addID(
            ModOreList.ESSENCE,
            MeteorType.NETHERRACK,
            MagicCropHandler.getInstance().netherOreID
        );
        this.addID(
            ModOreList.ESSENCE, MeteorType.END, MagicCropHandler.getInstance().endOreID
        );
        ...
        for (int k = 0; k < MeteorType.list.length; k++) {
            ...
            for (int i = 0; i < ReikaOreHelper.oreList.length; i++) {
                ...
                if (this.canGenerateOre(ore)) {
                    if (this.isValidOreForType(type, ore)) {
                        this.addOre(
                            type,
                            BlockKey.fromItem(ore.getOreBlock()),
                            MeteorCraft.config.getOreWeight(ore)
                        );
                    }
                }
            }
            for (int i = 0; i < ModOreList.oreList.length; i++) {
                ...
                if (this.canGenerateOre(ore) && this.canGenOreIn(type.blockID, ore)) {
                    ...
                    for (ItemStack is : li) {
                        if (is.getItem() != null
                            && Block.getBlockFromItem(is.getItem()) != null) {
                            BlockKey block = BlockKey.fromItem(is);
                            if (MeteorCraft.config.isItemStackGenerationPermitted(block
                                )) {
                                //ReikaJavaLibrary.pConsole(type.name()+"
                                //INIT:"+ore.name());
                                if (this.isValidOreIDForType(type, ore, block.blockID)) {
                                    //ReikaJavaLibrary.pConsole(type.name()+"
                                    //ID:"+ore.name());
                                    if (this.isValidOreMetaForType(
                                            type, ore, block.metadata
                                        )) {
                                        //ReikaJavaLibrary.pConsole(type.name()+"
                                        //META:"+ore.name());
                                        String mod = ReikaItemHelper.getRegistrantMod(is);
                                        if (mod == null
                                            || mod.equalsIgnoreCase("gregtech")
                                            || mod.equalsIgnoreCase("gt"))
                                            continue;
                                        MeteorCraft.logger.log(
                                            "Registering " + mod + ":" + block + " ("
                                            + ore.displayName + " ore) to meteor type "
                                            + type.name()
                                        );
                                        this.addOre(
                                            type,
    ...
    public BlockKey getBlock(MeteorType type, boolean ore) {
        if (ore && ReikaRandomHelper.doWithChance(MeteorOptions.ORE.getValue())
            && this.hasOresForType(type)) {
    ...
    private void
    generate(World world, int x, int y, int z, MeteorType type, boolean ore) {
        ...
        for (int i = -radius; i <= radius; i++) {
            for (int j = -radius; j <= radius; j++) {
                for (int k = -radius; k <= radius; k++) {
                    ...
                    if (dd <= radius
                        || (j < 0 && ReikaMathLibrary.py3d(i, 0, k) <= radius)) {
    ...
    public static boolean canStopMeteor(World world, int x, int y, int z) {
        ...
        if (ReikaWorldHelper.softBlocks(world, x, y, z))
            return false;
        if (ModList.NATURA.isLoaded()
            && b == GameRegistry.findBlock(ModList.NATURA.modLabel, "Cloud"))
            return false;
    ...
    canStopMeteorRayTrace(World world, int x, int y, int z, EntityMeteor e, int dist) {
        int r = 4;
    ...
    public static enum MeteorType {
        ...
        private void addDrop(ItemStack is, int min, int max) {
            if (is == null || is.getItem() == null) {
                MeteorCraft.logger.logError(
                    "Cannot add drop to meteor type " + this + "! Null item!"
                );
                return;
            }
            if (this.isValid())
        ...
        static {
            ...
            if (ModList.APPENG.isLoaded()) {
                ...
                MeteorType.SKYSTONE.addDrop(
                    AppEngHandler.getInstance().getCertusQuartzDust(), 16, 32
                );
                Collection<ItemStack> c
                    = AppEngHandler.getInstance().getPossibleMeteorChestLoot();
                for (ItemStack is : c) {
