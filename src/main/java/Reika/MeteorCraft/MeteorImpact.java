...
public class MeteorImpact {
    ...
    public void impact(EntityMeteor e) {
                        ...
                        if (dd <= radius) {
                            if (id != Blocks.air) {
                                if (this.canEntitize(world, x2, y2, z2, id, meta)) {
                                    ...
                                    EntityFallingBlock es = new EntityFallingBlock(
                                        world,
                                        x2,
                                        y2 + 4,
                                        z2,
                                        Block.getBlockFromItem(toDrop.getItem()),
                                        toDrop.getItemDamage()
                                    );
                                    es.addVelocity(
                                        vx, vy * rand.nextDouble() * rand.nextDouble(), vz
                                    );
        ...
        AxisAlignedBB box
            = ReikaAABBHelper.getBlockAABB(posX, posY, posZ).expand(16, 16, 16);
        ...
        //ReikaSoundHelper.playSoundAtBlock(world, posX, posY, posZ,
        //"meteorcraft:impact");
        if (world.isRemote)
            ReikaSoundHelper.playClientSound(
                MeteorSounds.IMPACT, posX + 0.5, posY + 0.5, posZ + 0.5, 1, 1
            );
        ...
        for (int i = 0; i < num; i++) {
            ...
            ReikaItemHelper.dropItem(world, dx, dy, dz, new
            ...
            for (ItemDrop drop : drops) {
                ...
                if (is == null) {
                    MeteorCraft.logger.log(
                        "Tried to drop an invalid ItemStack for meteor type "
                        + e.getType() + ": " + drop
                    );
                    continue;
                }
    ...
    private float getDamageFor(EntityLivingBase el) {
        int dd
            = (int) ReikaMathLibrary.py3d(el.posX - posX, el.posY - posY, el.posZ - posZ);
        if (dd <= 9)
