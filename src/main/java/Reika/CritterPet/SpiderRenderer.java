...
public class SpiderRenderer extends RenderSpider {
    ...
    @Override
    protected void func_147906_a(
        Entity par1EntityLivingBase,
        String par2Str,
        double par3,
        double par5,
        double par7,
        ...
        if (d3 <= par9 * par9) {
            ...
            GL11.glTranslatef(
                (float) par3 + 0.0F,
                (float) par5 + par1EntityLivingBase.height + 0.5F,
                (float) par7
            );
            ...
            if (s.length > 1) {
                ...
                fontrenderer.drawString(
                    s[0],
                    -fontrenderer.getStringWidth(s[0]) / 2,
                    b0,
                    0x00ffffff + text_alpha
                );
                fontrenderer.drawString(
                    s[1],
                    -fontrenderer.getStringWidth(s[1]) / 2,
                    b0 + 9,
                    0x00ffffff + text_alpha
                );
            } else
                fontrenderer.drawString(
                    par2Str,
                    -fontrenderer.getStringWidth(par2Str) / 2,
                    b0,
                    0x00ffffff + text_alpha
                );
            ...
            //fontrenderer.drawString(par2Str, -fontrenderer.getStringWidth(par2Str) / 2,
            //b0, -1);
    ...
    protected void renderModel(
        ...
        if (MinecraftForgeClient.getRenderPass() == 0) {
            ...
            if (!elb.isInvisible()) {
                boolean sit = elb instanceof EntitySpiderBase
                    && ((EntitySpiderBase) elb).isSitting();
                if (sit) {
