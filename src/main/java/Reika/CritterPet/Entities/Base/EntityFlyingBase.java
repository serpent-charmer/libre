...
public abstract class EntityFlyingBase
    extends EntityFlying implements TamedMob, TameHostile {
    ...
    public EntityFlyingBase(World par1World, CritterType sp) {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(this.getCritterMaxHealth());
    ...
    public void spawnEffects() {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            ReikaParticleHelper.HEART.spawnAt(
                world, rx, y + this.getScaleFactor() * 0.8, rz, 0, 0, 0
            );
    ...
    public final void onUpdate() {
        ...
        if (!worldObj.isRemote
            && worldObj.difficultySetting
                == EnumDifficulty.PEACEFUL) { //the criteria for mob despawn in peaceful
            preventDespawn = true;
            worldObj.difficultySetting = EnumDifficulty.EASY;
        }
        super.onUpdate();
        int y = MathHelper.floor_double(posY - this.getCoastingHeight());
        int x = MathHelper.floor_double(posX);
        int z = MathHelper.floor_double(posZ);
        if (worldObj.getBlock(x, y, z) != Blocks.air
            && worldObj.getBlock(x, y + 2, z) == Blocks.air) {
            motionY += 0.015;
        } else {
            motionY -= 0.015;
        }
        if (this.isSitting()) {
            ...
            if (ep != null) {
                double[] angs = ReikaPhysicsHelper.cartesianToPolar(
                    ep.posX - posX, ep.posY - posY - 1.5, ep.posZ - posZ
                );
                boolean flag = this.alwaysFaceOwner();
                if (angs[0] > 5.2) {
    ...
    protected abstract double getCoastingHeight();
    ...
    protected final boolean interact(EntityPlayer ep) {
        ...
        if (owner == null || owner.isEmpty()) {
            if (is != null && is.getItem() == base.tamingItem) {
                ...
                //ReikaChatHelper.writeString("This critter had no owner! You are now the
                //owner.");
                return true;
            }
            return false;
    ...
    public void heal(float par1) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            double rz = ReikaRandomHelper.getRandomPlusMinus(z, 1);
            ReikaParticleHelper.HEART.spawnAt(
                worldObj, rx, y + this.getScaleFactor() * 0.8, rz, 0, 0, 0
            );
            //ReikaWorldHelper.splitAndSpawnXP(worldObj, rx, y+0.5, rz,
            //1+rand.nextInt(5));
        }
    }
    ...
    public final boolean attackEntityFrom(DamageSource dsc, float par2) {
        if (this.isEntityInvulnerable()) {
            ...
            return false;
        } else if (dsc == DamageSource.fall || !this.canBeHurtBy(dsc)) {
            return false;
        } else if (super.attackEntityFrom(dsc, par2)) {
            Entity entity = dsc.getEntity();
            if (riddenByEntity != entity && ridingEntity != entity) {
                if (entity != this
                    && !entity.getCommandSenderName().equals(this.getMobOwner()))
    ...
    public final String toString() {
        return this.getCommandSenderName() + " @ "
            + String.format("%.1f, %.1f, %.1f", posX, posY, posZ);
    }
    ...
    protected void collideWithNearbyEntities() {
        List list = worldObj.getEntitiesWithinAABBExcludingEntity(
            this, boundingBox.expand(0.2, 0.0D, 0.2)
        );
                    ...
                    for (int k = 0; k <= 4; ++k) {
                        if ((i < 1 || k < 1 || i > 3 || k > 3)
                            && World.doesBlockHaveSolidTopSurface(
                                worldObj, x + i, y - 1, z + k
                            )
                            && !worldObj.getBlock(x + i, y, z + k).isNormalCube()
                            && !worldObj.getBlock(x + i, y + 1, z + k).isNormalCube()) {
                            this.setLocationAndAngles(
                                x + i + 0.5F, y, z + k + 0.5F, rotationYaw, rotationPitch
                            );
                            return;
                        }
                    }
