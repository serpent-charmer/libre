...
public abstract class EntitySpiderBase
    extends EntitySpider implements TamedMob, TameHostile {
    ...
    public final void onUpdate() {
        ...
        if (entityToAttack != null
            && entityToAttack.getCommandSenderName().equals(this.getMobOwner()))
            entityToAttack = null;
    ...
    protected final boolean interact(EntityPlayer ep) {
        ...
        if (ep.getCommandSenderName().equals(owner)) {
            if (is != null) {
                if (is.getItem() == Items.cooked_beef
    ...
    public final boolean attackEntityFrom(DamageSource dsc, float par2) {
        if (this.isEntityInvulnerable()) {
            ...
            if (riddenByEntity != entity && ridingEntity != entity) {
                //if (entity != this &&
                //!entity.getCommandSenderName().equals(this.getOwner()))
    ...
    protected final void func_145780_a(int par1, int par2, int par3, Block par4) {
        this.playSound(
            "mob.spider.step",
            this.getStepSoundVolume(),
            (float) (1.0F / Math.sqrt(base.size))
        );
    }
    ...
    protected void collideWithEntity(Entity e) {
        ...
        if (e instanceof EntityLivingBase && !(e instanceof TamedMob)) {
            if (ReikaEntityHelper.isHostile((EntityLivingBase) e)
                ...
                e.attackEntityFrom(
                    DamageSource.causeMobDamage(this), this.getAttackDamage()
                );
                this.applyAttackEffects((EntityLivingBase) e);
            }
        }
