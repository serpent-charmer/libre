...
public class TameSpitter extends EntitySpitter implements TamedMob, TameHostile {
        ...
        if (!worldObj.isRemote) {
            ...
            if (tier >= 0) {
                TilePowerSlug.doFX(
                    worldObj,
                    posX,
                    posY + 0.375 + this.getMountedYOffset(),
                    posZ,
                    tier,
                    ForgeDirection.DOWN,
                    this
    ...
    public float getDamageScale(EntityLivingBase tgt) {
        return (float
        ) (super.getDamageScale(tgt) + 0.125 * Math.pow(2, this.getSlugTier() + 1)
        ); //x1.25, x1.5, x2.0
    }
    ...
    public float getFireRateScale(EntityAISpitterFireball ai) {
        return (float
        ) (super.getFireRateScale(ai) + 0.25 * Math.pow(2, this.getSlugTier() + 1)
        ); //x1.5, x2.0, x3.0
    }
    ...
    protected final boolean interact(EntityPlayer ep) {
        ...
        if (ep.getCommandSenderName().equals(owner)) {
            if (is != null) {
                if (ReikaItemHelper.matchStackWithBlock(is, Blocks.brown_mushroom)
