...
public class CritterPet extends DragonAPIMod {
    ...
    public static final ControlledConfig config
        = new ControlledConfig(instance, CritterOptions.optionList, null);
    ...
    @SidedProxy(
        clientSide = "Reika.CritterPet.CritterClient",
        serverSide = "Reika.CritterPet.CritterCommon"
    )
        ...
        for (int i = 0; i < CritterType.critterList.length; i++) {
            ...
            if (type.isAvailable()) {
                ...
                EntityRegistry.registerGlobalEntityID(
                    type.entityClass, "critterpet." + type.name, id
                );
                EntityRegistry.registerModEntity(
                    type.entityClass, type.name, id, instance, 32, 1, true
                );
                type.initializeMapping(id);
                GameRegistry.addShapelessRecipe(
                    new ItemStack(tool, 1, i + 1),
                    new ItemStack(tool, 1, 0),
                    type.tamingItem
                );
