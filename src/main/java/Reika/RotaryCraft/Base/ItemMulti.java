...
public class ItemMulti extends ItemBasic {
    ...
    private boolean
    tryBeltConnection(World world, int x, int y, int z, ItemStack is, EntityPlayer ep) {
        ...
        if (is.stackSize >= dl || ep.capabilities.isCreativeMode) {
            ...
            if (te.tryConnect(world, bb.xCoord, bb.yCoord, bb.zCoord)
    ...
    private boolean isProperBelt(MachineRegistry m, ItemStack is) {
        if ((m == MachineRegistry.BELT || m == MachineRegistry.SPLITBELT)
            && ReikaItemHelper.matchStacks(is, ItemStacks.belt))
            return true;
        if (m == MachineRegistry.CHAIN
            && ReikaItemHelper.matchStacks(is, ItemStacks.chain))
            return true;
        return false;
    }
    ...
    public void onUpdate(ItemStack is, World world, Entity e, int par4, boolean par5) {
        if (ReikaItemHelper.matchStacks(is, ItemStacks.shaftcore)
            || ReikaItemHelper.matchStacks(is, ItemStacks.tungstenshaftcore)) {
            if (is.stackTagCompound != null) {
                ...
                if (mag > 0) {
                    ...
                    for (int i = -6; i <= 6; i++) {
                        for (int j = -6; j <= 6; j++) {
                            for (int k = -6; k <= 6; k++) {
                                if (world.getTileEntity(x + i, y + j, z + k)
                                        instanceof TileEntityACEngine) {
                                    TileEntityACEngine te = (TileEntityACEngine
                                    ) world.getTileEntity(x + i, y + j, z + k);
                                    double dx = x - te.xCoord - 0.5;
                                    double dy = y - te.yCoord - 0.5;
                                    double dz = z - te.zCoord - 0.5;
                                    double dd = ReikaMathLibrary.py3d(dx, dy, dz);
                                    double v = ReikaMathLibrary.py3d(
    ...
    @Override
    public void
    addInformation(ItemStack is, EntityPlayer ep, List par3List, boolean par4) {
        /*
        ...
        if (is.stackTagCompound.hasKey("magnet")) {
            if (is.stackTagCompound.getInteger("magnet") >= 1000000)
                par3List.add(
                    "Magnetized to "
                    + String.format(
                        "%.3f", is.stackTagCompound.getInteger("magnet") / 1000000D
                    )
                    + " T"
                );
                ...
                par3List.add(
                    "Magnetized to "
                    + String.format(
                        "%.3f", is.stackTagCompound.getInteger("magnet") / 1000D
                    )
                    + " mT"
                );
            else
                par3List.add(
                    "Magnetized to " + is.stackTagCompound.getInteger("magnet")
                    + " microTeslas"
                );
        }
    }
        ...
        for (int i = 0; i < j; i++) {
            ...
            if (ItemRegistry.GEARCRAFT.matchItem(item)) {
                ...
                if (ReikaItemHelper.matchStacks(
                        item, GearboxTypes.WOOD.getPart(GearPart.SHAFT)
                    )) //stick
                    continue;
            }
            ...
            if (ReikaItemHelper.matchStacks(item, ItemStacks.shaftcore)
                || ReikaItemHelper.matchStacks(item, ItemStacks.tungstenshaftcore)) {
        ...
        try {
            switch (ItemRegistry.getEntryByID(this)) {
                ...
                case SHAFT:
                    ...
                    break;
                case GEARBOX:
                    //s = super.getUnlocalizedName() + "." +
                    //RotaryNames.getGearboxName(d);
                    break;
                case MODINTERFACE:
    ...
    private boolean offsetDustName(ItemStack is) {
        ...
        return (ep.posY < 15 || ep.worldObj.provider.hasNoSky) &&
    ep.getCommandSenderName().equals("Freyskol");
    }

    @Override
    public String getItemStackDisplayName(ItemStack item) {
        if (ReikaItemHelper.matchStacks(item, ItemStacks.bedrockdust) &&
    FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT &&
    this.offsetDustName(item)) { return
    ((ItemMulti)ItemStacks.salt.getItem()).getItemStackDisplayName(ItemStacks.salt);
        }
    ...
    public int getItemSpriteIndex(ItemStack item) {
        //if (ReikaItemHelper.matchStacks(item, ItemStacks.bedrockdust) &&
        //this.offsetDustName(item)) { 	return
        //((ItemMulti)ItemStacks.salt.getItem()).getItemSpriteIndex(ItemStacks.salt);
        //}
        if (ItemRegistry.GEARCRAFT.matchItem(item)) {
            return /*GearboxTypes.getMaterialFromCraftingItem(item).metaOffset+*/ item
                .getItemDamage();
        }
        if (ItemRegistry.FLYWHEELCRAFT.matchItem(item)) {
            return /*GearboxTypes.getMaterialFromCraftingItem(item).metaOffset+*/ type
                + item.getItemDamage();
        }
