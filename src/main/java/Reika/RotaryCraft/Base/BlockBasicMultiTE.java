...
public abstract class BlockBasicMultiTE
    extends BlockRotaryCraftMachine implements FluidBlockSurrogate {
    /**
     * Icons by metadata 0-15 and side 0-6. Nonmetadata blocks can just set the first
     * index to 0 at all times.
     */
    ...
    public final IIcon getIcon(IBlockAccess iba, int x, int y, int z, int s) {
        ...
        //"+icons[machine][meta][s][te.getTextureStateForSide(s)].getIconName());
        return OldTextureLoader.instance.loadOldTextures()
            ? OldTextureLoader.instance.getOldTexture(this, m.getBlockMetadata(), s)
            : icons[machine][meta][s][te.getTextureStateForSide(s)];
    }
    ...
    public IIcon getIcon(int s, int meta) {
        try {
            return OldTextureLoader.instance.loadOldTextures()
                ? OldTextureLoader.instance.getOldTexture(this, meta, s)
                : icons[meta][0][s][0];
    ...
    public final boolean onBlockActivated(
        ...
        if (ep.isSneaking() && !m.hasSneakActions())
            return false;
        if (is != null && ItemRegistry.isRegistered(is)
            && ItemRegistry.getEntry(is).overridesRightClick(is)) {
            return false;
        }
        ...
        if (m == MachineRegistry.SPLITTER) {
            if (is != null
                && ReikaItemHelper.matchStacks(
                    is, GearboxTypes.BEDROCK.getPart(GearPart.UNIT2)
                )) {
                ...
                if (FluidContainerRegistry.isFilledContainer(is)) {
                    ...
                    if (f != null) {
                        ...
                        if (f.getFluid().equals(FluidRegistry.getFluid("fuel"))) {
                            ...
                            ((TileEntityBase) te).syncAllData(true);
                            return true;
                        } else if (f.getFluid().equals(
                                       FluidRegistry.getFluid("rc lubricant")
                                   )) {
                            tf.addLube(f.amount);
                            if (!ep.capabilities.isCreativeMode)
                                ep.setCurrentItemOrArmor(
                    ...
                    if (tr.getLevel() + size * f.amount <= tr.CAPACITY) {
                        if (tr.isEmpty() || f.getFluid().equals(tr.getFluid())) {
                            ...
                            if (!ep.capabilities.isCreativeMode) {
                                if (bucket)
                                    ep.setCurrentItemOrArmor(
                                        0, new ItemStack(Items.bucket, size, 0)
                                    );
                                else
                                    ep.setCurrentItemOrArmor(0, null);
                            }
                            ((TileEntityBase) te).syncAllData(true);
                            if (!world.isRemote)
                                ReikaPacketHelper.sendTankSyncPacket(
                                    RotaryCraft.packetChannel, tr, "tank"
                                );
                            return true;
                        }
                    }
        ...
        if (m == MachineRegistry.RESERVOIR) {
            ...
            if (is == null) {
                Fluid f = tr.getFluid();
                if (ep.isSneaking() && ReikaXPFluidHelper.fluidsExist()
                    && f == ReikaXPFluidHelper.getFluid().getFluid()) {
                    int amt = Math.min(
                        Math.max(1000, Math.min(tr.getLevel() / 4, 4000)), tr.getLevel()
                    );
                ...
                if (ReikaItemHelper.matchStackWithBlock(is, Blocks.glass_pane)) {
                    if (!tr.isCovered) {
                        ...
                        if (!ep.capabilities.isCreativeMode)
                            ep.setCurrentItemOrArmor(
                                0, ReikaItemHelper.getSizedItemStack(is, is.stackSize - 1)
                            );
                        ((TileEntityBase) te).syncAllData(true);
                        return true;
                    }
                    ...
                    if (f != null) {
                        ...
                        if (tr.getLevel() + (size - 1) * f.amount <= tr.CAPACITY) {
                            if (tr.isEmpty()) {
                                ...
                                if (!ep.capabilities.isCreativeMode) {
                                    ItemStack ret
                                        = FluidContainerRegistry.drainFluidContainer(is);
                                    ep.setCurrentItemOrArmor(
                                        0,
                                        ret != null
                                            ? ReikaItemHelper.getSizedItemStack(ret, size)
                                            : null
                                    );
                                }
                                ((TileEntityBase) te).syncAllData(true);
                    ...
                    if (ret != null) {
                        ...
                        if (tr.getLevel() >= amt) {
                            ...
                            if (!ep.capabilities.isCreativeMode)
                                ep.setCurrentItemOrArmor(
                                    0, ReikaItemHelper.getSizedItemStack(ret, size)
                                );
                            ((TileEntityBase) te).syncAllData(true);
                            if (!world.isRemote)
                                ReikaPacketHelper.sendTankSyncPacket(
                    ...
                    if (tr.getLevel() > 0 && tr.getFluid().equals(FluidRegistry.WATER)) {
                        ep.setCurrentItemOrArmor(
                            0, new ItemStack(Items.potionitem, size, 0)
                        );
                        ((TileEntityBase) te).syncAllData(true);
                        return true;
                    }
        ...
        if (m == MachineRegistry.EXTRACTOR) {
            ...
            if (is != null) {
                if (is.getItem() == Items.water_bucket && is.stackSize == 1
                    && ex.getLevel() + 1000 <= ex.CAPACITY) {
                    ex.addLiquid(1000);
                    if (!ep.capabilities.isCreativeMode) {
        ...
        if (m == MachineRegistry.PULSEJET) {
            ...
            int f = ex.getFuel();
            if (is != null && f + 1000 * is.stackSize <= ex.MAXFUEL
                && ReikaItemHelper.matchStacks(is, ItemStacks.fuelbucket)) {
                ex.addFuel(1000 * is.stackSize);
                if (!ep.capabilities.isCreativeMode) {
                    ep.setCurrentItemOrArmor(
                        0, new ItemStack(Items.bucket, is.stackSize, 0)
                    );
                }
                ((TileEntityBase) te).syncAllData(true);
                return true;
            ...
            if (water + 1000 <= ex.CAPACITY && is != null
        ...
        if (m == MachineRegistry.FERMENTER) {
            ...
            if (fm.getLevel() + 1000 <= fm.CAPACITY && is != null
                && is.getItem() == Items.water_bucket) {
                fm.addLiquid(1000);
                if (!ep.capabilities.isCreativeMode) {
                    ep.setCurrentItemOrArmor(0, new ItemStack(Items.bucket));
        ...
        if (m == MachineRegistry.OBSIDIAN) {
            ...
            if (fm.getWater() + 1000 <= fm.CAPACITY && is != null
                && is.getItem() == Items.water_bucket) {
                fm.addWater(1000);
                if (!ep.capabilities.isCreativeMode) {
                    ep.setCurrentItemOrArmor(0, new ItemStack(Items.bucket));
        ...
        if (m == MachineRegistry.FERTILIZER) {
            ...
            if (fm.getLevel() + 1000 <= fm.getCapacity() && is != null
                && is.getItem() == Items.water_bucket) {
                fm.addLiquid(1000);
                if (!ep.capabilities.isCreativeMode) {
                    ep.setCurrentItemOrArmor(0, new ItemStack(Items.bucket));
        ...
        if (m == MachineRegistry.BIGFURNACE) {
            ...
            if (bf.getLevel() + 1000 <= bf.getCapacity() && is != null
                && is.getItem() == Items.lava_bucket) {
                bf.addLiquid(1000);
                if (!ep.capabilities.isCreativeMode) {
                    ep.setCurrentItemOrArmor(0, new ItemStack(Items.bucket));
                }
            ...
            if (is != null) {
                ...
                if (liq != null) {
                    ...
                    if (amt >= liq.amount || ep.capabilities.isCreativeMode) {
                        ...
                        if (!ep.capabilities.isCreativeMode)
                            ep.setCurrentItemOrArmor(
                                0, FluidContainerRegistry.drainFluidContainer(is)
                            );
                        ((TileEntityBase) te).syncAllData(true);
                    }
                    return true;
                ...
                if (ReikaItemHelper.matchStacks(is, ItemStacks.mirror)) {
                    ...
                    if (!ep.capabilities.isCreativeMode) {
                        ep.setCurrentItemOrArmor(
                            0,
                            new ItemStack(
                                is.getItem(), is.stackSize - 1, is.getItemDamage()
                            )
                        );
                    }
                    ((TileEntityBase) te).syncAllData(true);
        ...
        if (te != null && RotaryAux.hasGui(world, x, y, z, ep)
            && ((RotaryCraftTileEntity) te).isPlayerAccessible(ep)) {
            ep.openGui(
                RotaryCraft.instance, GuiRegistry.MACHINE.ordinal(), world, x, y, z
            );
            return true;
        }
    ...
    getPickBlock(MovingObjectPosition target, World world, int x, int y, int z) {
        ...
        if (m.isEnchantable()) {
            Map<Enchantment, Integer> ench
                = ((EnchantableMachine) tile).getEnchantmentHandler().getEnchantments();
        ...
        if (m == MachineRegistry.PORTALSHAFT) {
            return MachineRegistry.SHAFT.getCraftedMetadataProduct(
                ((TileEntityPortalShaft) tile).material.ordinal()
            );
        }
        return core;
    }
    ...
    harvestBlock(World world, EntityPlayer ep, int x, int y, int z, int meta) {
        ...
        if (m != null) {
            ...
            if (m == MachineRegistry.PORTALSHAFT)
                is = MachineRegistry.SHAFT.getCraftedMetadataProduct(
                    ((TileEntityPortalShaft) te).material.ordinal()
                );
            List li;
            if (m.isEnchantable()) {
                Map<Enchantment, Integer> map
                    = ((EnchantableMachine) te).getEnchantmentHandler().getEnchantments();
            ...
            if (((RotaryCraftTileEntity) te).isUnHarvestable()) {
                li = ReikaJavaLibrary.makeListFrom(ReikaItemHelper.getSizedItemStack(
                    ItemStacks.scrap, 2 + par5Random.nextInt(12)
                ));
    ...
    public final RotaryCraftTileEntity createTileEntity(World world, int meta) {
        return (RotaryCraftTileEntity
        ) MachineRegistry.createTEFromIDAndMetadata(this, meta);
    }
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity e) {
        ...
        if (m == MachineRegistry.CENTRIFUGE) {
            ...
            if (tr.omega > 0 && world.isRemote) {
                e.addVelocity(
                    ReikaRandomHelper.getRandomPlusMinus(0, 1),
                    0.1,
                    ReikaRandomHelper.getRandomPlusMinus(0, 1)
                );
            }
        ...
        if (!(e instanceof EntityItem || e instanceof EntityXPOrb)) {
            if (e instanceof EntityLivingBase
                && ItemBedrockArmor.isWearingFullSuitOf((EntityLivingBase) e))
                return;
    ...
    @Override
    public final void
    onBlockExploded(World world, int x, int y, int z, Explosion explosion) {
        TileEntity te = world.getTileEntity(x, y, z);
    ...
    public int isProvidingWeakPower(IBlockAccess iba, int x, int y, int z, int par5) {
        ...
        if (m == MachineRegistry.PLAYERDETECTOR) {
            TileEntityPlayerDetector tp
                = (TileEntityPlayerDetector) iba.getTileEntity(x, y, z);
            return tp.isActive() ? 15 : 0;
        } else if (m == MachineRegistry.SMOKEDETECTOR) {
            TileEntitySmokeDetector tp
                = (TileEntitySmokeDetector) iba.getTileEntity(x, y, z);
            ...
            return tp.getFluid() == FluidRegistry.getFluid("redstone")
                ? (int) Math.round(tp.getLevel() * 15F / tp.CAPACITY)
                : 0;
        } else {
            return 0;
        }
    ...
    public final int
    drain(World world, int x, int y, int z, Fluid f, int amt, boolean doDrain) {
        MachineRegistry m = MachineRegistry.getMachine(world, x, y, z);
        if (m == MachineRegistry.RESERVOIR) {
            TileEntityReservoir te = (TileEntityReservoir) world.getTileEntity(x, y, z);
    ...
    getWailaStack(IWailaDataAccessor accessor, IWailaConfigHandler config) {
        return null; //MachineRegistry.getMachineFromIDandMetadata(this,
    ...
    public List<String> getWailaBody(
        ...
        if (te instanceof TemperatureTE)
            currenttip.add(
                ...
                + RotaryAux.formatTemperature(((TemperatureTE) te).getTemperature())
            );
        if (te instanceof PressureTE)
            currenttip.add(
                Variables.PRESSURE + ": "
                + RotaryAux.formatPressure(((PressureTE) te).getPressure())
            );
        ...
        if (te instanceof EnergyToPowerBase) {
            ...
            currenttip.add(String.format(
                "Consuming %d %s/t", e.getConsumedUnitsPerTick(), e.getUnitDisplay()
            ));
            currenttip.add(
                String.format("Contains %d %s", e.getStoredPower(), e.getUnitDisplay())
            );
        ...
        if (te instanceof PoweredLiquidIO) {
            ...
            String input = in != null
                ? String.format(
                    "%s of %s",
                    RotaryAux.formatLiquidFillFraction(amtin, liq.getInputCapacity()),
                    in.getLocalizedName()
                )
                : "Empty";
            String output = out != null
                ? String.format(
                    "%s of %s",
                    RotaryAux.formatLiquidFillFraction(amtout, liq.getOutputCapacity()),
                    out.getLocalizedName()
                )
                : "Empty";
            ...
            String input = in != null ? String.format(
                               "%s of %s",
                               RotaryAux.formatLiquidFillFraction(amt, liq.getCapacity()),
                               in.getLocalizedName()
                           )
                                      : "Empty";
            ...
            if (tanks != null) {
                for (int i = 0; i < tanks.length; i++) {
                    ...
                    String input = fs != null
                        ? String.format(
                            "%s of %s",
                            RotaryAux.formatLiquidFillFraction(fs.amount, info.capacity),
        ...
        if (te instanceof TileEntityPiping) {
            ...
            if (f != null) {
                ...
                if (tp instanceof TileEntityPipe) {
                    ...
                    currenttip.add(
                        Variables.PRESSURE + ": " + RotaryAux.formatPressure(p)
                    );
                }
            }
        }
        ...
        if (te instanceof TileEntityAggregator) {
            currenttip.add(String.format(
                "Producing %s per tick",
                RotaryAux.formatLiquidAmount(
                    ((TileEntityAggregator) te)
                        .getProductionPerTick(
                            acc.getWorld().getBiomeGenForCoords(te.xCoord, te.zCoord)
                        )
                )
            ));
        }
        if (te instanceof TileEntityFractionator) {
            currenttip.add(String.format(
                "Efficiency Factor: %.2f%s",
                ((TileEntityFractionator) te).getYieldRatio() * 100D,
                "%"
            ));
        }
        if (te instanceof TileEntityBusController) {
            ...
            if (bus != null) {
                currenttip.add(
                    "Power Bus Receiving " + RotaryAux.formatTorque(bus.getInputTorque())
                    + " @ " + RotaryAux.formatSpeed(bus.getSpeed())
                );
                currenttip.add(
                    RotaryAux.formatPower(bus.getInputPower()) + " is being split to "
                    + bus.getTotalOutputSides() + " devices"
                );
                currenttip.add(
                    "(Power per side is "
                    + RotaryAux.formatPower(
                        bus.getInputPower() / bus.getTotalOutputSides()
                    )
                    + ")"
                );
            }
        ...
        if (te instanceof TileEntitySolar) {
            ...
            currenttip.add(
                "Consuming " + RotaryAux.formatLiquidAmount(sol.getCurrentConsumption())
                + "/t of fluid."
            );
        }
        ...
        if (te instanceof TileEntityPulseFurnace) {
            ...
            if (lvl > 0) {
                ...
                currenttip.add(String.format(
                    "Accelerant: %s of %s",
                    RotaryAux.formatLiquidAmount(lvl),
                    f.getLocalizedName()
                ));
            } else
        ...
        if (te instanceof TileEntityFluidCompressor) {
            ...
            if (!tfc.isEmpty())
                currenttip.add(String.format(
                    "Capacity: %s", RotaryAux.formatLiquidAmountWithSI(tfc.getCapacity())
                ));
        }
