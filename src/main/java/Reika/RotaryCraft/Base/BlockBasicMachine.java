...
public abstract class BlockBasicMachine
    extends BlockRotaryCraftMachine implements SidedTextureIndex {
    ...
    @Override
    public void
    setBlockBoundsBasedOnState(IBlockAccess par1IBlockAccess, int x, int y, int z) {
        ...
        if (te instanceof TileEntityAdvancedGear) {
            ...
            if (tile.getGearType().isLubricated()) {
                if (is != null) {
                    ...
                    if (fs != null && fs.getFluid() == RotaryCraft.lubeFluid
                        && tile.canAcceptMoreLubricant(fs.amount)) {
                        ...
                        GearboxTypes material
                            = GearboxTypes.getMaterialFromCraftingItem(is);
                        if (material.material.ordinal()
                            > tile.getBearingTier().ordinal()) {
    ...
    getPickBlock(MovingObjectPosition target, World world, int x, int y, int z) {
        ...
        if (m == MachineRegistry.ADVANCEDGEARS) {
            ...
            ItemStack is
                = new ItemStack(ItemRegistry.ADVGEAR.getItemInstance(), 1, meta / 4);
        ...
        if (m != null && m.isPipe()) {
            return ((TileEntityPiping) world.getTileEntity(x, y, z))
                .getTile()
                .getCraftedProduct();
        }
    ...
    public final String getTextureFile() {
        return "/Reika/RotaryCraft/Textures/Terrain/textures.png"; //return the block
                                                                   //texture where the
                                                                   //block texture is
                                                                   //saved in
    }
    ...
    public List<String> getWailaBody(
        ...
        if (te instanceof TileEntityGearbox) {
            ...
            if (gbx.getGearboxType().needsLubricant()) {
                String s = gbx.isLiving()
                    ? String.format(
                        "Mana: %d%%", gbx.getLubricant() * 100 / gbx.getMaxLubricant()
                    )
                    : String.format("Lubricant: %d mB", gbx.getLubricant());
                currenttip.add(s);
                ...
                if (gbx.getBearingTier() != gbx.getGearboxType()) {
                    ...
                    if (f < 0)
                        ...
                        s = String.format(
                            "Lubricant Rate: %s%.2fx",
                            (f > 1 ? EnumChatFormatting.RED : EnumChatFormatting.GREEN)
                                .toString(),
                            f
                        );
                    currenttip.add(s);
        ...
        if (te instanceof TileEntityAdvancedGear) {
            ...
            if (adv.getGearType().consumesLubricant()) {
                currenttip.add(String.format(
                    "Lubricant: %s", RotaryAux.formatLiquidAmount(adv.getLubricant())
                ));
            }
        ...
        if (te instanceof TileEntityEngine) {
            ...
            if (eng.getEngineType().requiresLubricant()) {
                currenttip.add(String.format(
                    "Lubricant: %s", RotaryAux.formatLiquidAmount(eng.getLube())
                ));
            }
            if (eng.getEngineType().burnsFuel()) {
                currenttip.add(String.format(
                    "Fuel: %s", RotaryAux.formatLiquidAmount(eng.getFuelLevel())
                ));
            }
            if (eng instanceof TileEntityPerformanceEngine) {
                currenttip.add(String.format(
                    ...
                    RotaryAux.formatLiquidAmount(
                        ((TileEntityPerformanceEngine) eng).additives
                    )
                ));
                currenttip.add(String.format(
                    "Water: %s",
                    RotaryAux.formatLiquidAmount(
                        ((TileEntityPerformanceEngine) eng).getWater()
                    )
                ));
            }
            if (eng.hasTemperature()) {
                currenttip.add(
                    Variables.TEMPERATURE + ": "
                    + RotaryAux.formatTemperature(eng.getTemperature())
                );
            }
        }
        return currenttip;
