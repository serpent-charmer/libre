...
public class ItemBasic extends Item implements IndexedItemSprites {
    ...
    private void checkAchievements(EntityPlayer player, ItemStack item) {
        if (ReikaItemHelper.matchStacks(
                item, MachineRegistry.RAILGUN.getCraftedProduct()
            ))
            RotaryAchievements.MAKERAILGUN.triggerAchievement(player);
        //if (ReikaItemHelper.matchStacks(item, new
        //ItemStack(RotaryCraft.engineitems.itemID, 1, EngineType.JET.ordinal())))
        //	RotaryAchievements.MAKEJET.triggerAchievement(player);
        if (ReikaItemHelper.matchStacks(
                item, MachineRegistry.WORKTABLE.getCraftedProduct()
            ))
            RotaryAchievements.WORKTABLE.triggerAchievement(player);
        if (ItemRegistry.SHAFT.matchItem(item)
            && MaterialRegistry.getMaterialFromShaftItem(item).compareTo(
                   MaterialRegistry.STEEL
               ) >= 0)
        ...
        if (ReikaItemHelper.matchStacks(
                item,
                new ItemStack(
                    ItemRegistry.SHAFT.getItemInstance(),
                    1,
        ...
        if (ReikaItemHelper.matchStacks(
                item, new ItemStack(ItemRegistry.ADVGEAR.getItemInstance(), 1, 1)
            ))
            RotaryAchievements.CVT.triggerAchievement(player);
        //if (ItemRegistry.isRegistered(item) &&
        //ItemRegistry.getEntry(item).isBedrockTool())
        //	RotaryAchievements.BEDROCKTOOLS.triggerAchievement(player);
        for (int i = 0; i < 4; i++) {
            if (ReikaItemHelper.matchStacks(
                    item,
                    new ItemStack(
                        ItemRegistry.GEARBOX.getItemInstance(),
                        1,
                        MaterialRegistry.DIAMOND.ordinal() + i * 5
                    )
                ))
