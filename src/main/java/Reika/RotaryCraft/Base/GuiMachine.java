...
public abstract class GuiMachine extends GuiContainer {
    ...
    protected void actionPerformed(GuiButton b) {
        if (b.id == 24000 || b.id == 24001) {
            ...
            if (ReikaInventoryHelper.checkForItem(
                    ItemRegistry.HANDBOOK.getItemInstance(), ep.inventory.mainInventory
                ))
                ep.openGui(
                    RotaryCraft.instance,
                    GuiRegistry.LOADEDHANDBOOK.ordinal(),
                    tile.worldObj,
                    tile.xCoord,
                    tile.yCoord,
                    tile.zCoord
                ...
                ep.openGui(
                    RotaryCraft.instance,
                    GuiRegistry.HANDBOOKPAGE.ordinal(),
                    tile.worldObj,
                    tile.xCoord,
                    tile.yCoord,
                    tile.zCoord
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        if (ep == null && !(this instanceof GuiOneSlotInv))
            RotaryCraft.logger.logError(
                "Gui for " + tile.getMultiValuedName() + " did not set player entity!"
            );
    ...
    protected final void func_146977_a(Slot slot) {
        ...
        if (Keyboard.isKeyDown(DragonOptions.DEBUGKEY.getValue())
            && DragonOptions.TABNBT.getState()) {
            ReikaTextureHelper.bindFontTexture();
            fontRendererObj.drawString(
                String.format("%d", slot.getSlotIndex()),
