...
public abstract class ItemRotaryArmor
    extends ItemArmor implements IndexedItemSprites, ISpecialArmor, UnbreakableArmor {
    private int index;
    ...
    public final IIcon getIconFromDamage(int dmg) {
        return RotaryCraft.instance.isLocked()
            ? ReikaTextureHelper.getMissingIcon()
            : Items.iron_chestplate.getIconFromDamage(0);
    }
    ...
    public ArmorProperties getProperties(
        ...
        double protection = this.providesProtection() && this.isVulnerableTo(src)
            ? damageReduceAmount / 100D / this.getDamageMultiplier(src)
            : 0;
        ArmorProperties prop
            = new ArmorProperties(Integer.MAX_VALUE, protection, Integer.MAX_VALUE);
        return prop;
    }
