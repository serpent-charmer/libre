...
public abstract class TileEntityIOMachine
    extends RotaryCraftTileEntity implements IOMachine, PowerSourceTracker {
    ...
    /*
    protected final void processTileSimply(TileEntity te, MachineRegistry m, int tgx, int
    tgy, int tgz) { if (m == MachineRegistry.SHAFT) { TileEntityShaft devicein =
    (TileEntityShaft)te; if (devicein.isCross()) { this.readFromCross(devicein); return;
            }
    ...
    protected final boolean matchTile(PowerSourceTracker te, ForgeDirection dir) {
        ...
        if (out == null)
            return false;
        return !out.isInvalid() && out.worldObj.provider.dimensionId == dim
            && out.xCoord == tx && out.yCoord == ty && out.zCoord == tz;
    }
