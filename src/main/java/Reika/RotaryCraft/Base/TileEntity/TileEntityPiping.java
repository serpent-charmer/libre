...
public abstract class TileEntityPiping extends RotaryCraftTileEntity
    implements RenderableDuct, CachedConnection, BreakAction, PumpablePipe, RCPipe {
    ...
    private static final int CAPACITY_LIMIT
        = 1000000000; //1 billion mB to prevent overflow
    ...
    public final int getPressure() {
        ...
        if (f.isGaseous())
            ret = 101300
                + (128
                   * (int
                   ) (amt / 1000D * f.getTemperature() * Math.abs(f.getDensity()) / 1000D)
                );
        else
            ret = 101300 + amt * 24;
    ...
    public final boolean canIntakeFluid(Fluid f) {
        if (f == null)
            return false;
        return this.isValidFluid(f)
            && (this.getFluidType() == null || this.getFluidLevel() == 0
                || this.getFluidType().equals(f));
    }
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (this.getPressure() > this.getMaxPressure()) {
            if (world.isRemote) {
                ReikaPacketHelper.sendUpdatePacket(
                    DragonAPIInit.packetChannel,
                    PacketIDs.EXPLODE.ordinal(),
                    this,
                    PacketTarget.server
                );
            } else {
    ...
    canInteractWith(World world, int x, int y, int z, ForgeDirection side) {
        ...
        if (m != null && m.isPipe())
            return this.hasReciprocalConnectivity(
                (TileEntityPiping) this.getAdjacentTileEntity(side), side
            );
    ...
    private boolean isInteractableTile(TileEntity te, ForgeDirection side) {
        ...
        if (te instanceof IFluidHandler) {
            ...
            if (name.contains("conduit") || name.contains("fluidduct")
                || name.contains("pipe") || name.contains("multipart")) {
    ...
    public final int getPipeIntake(int otherlevel) {
        return Math.min(
            CAPACITY_LIMIT - this.getFluidLevel(),
            TransferAmount.QUARTER.getTransferred(otherlevel)
        );
    }

    public final int getPipeOutput(int max) {
        return Math.min(
            TransferAmount.QUARTER.getTransferred(max), this.getFluidLevel() - 5
        );
    }
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (interaction[i]) {
                ...
                if (te instanceof TileEntityPiping) {
                    ...
                    if (this.hasReciprocalConnectivity(tp, dir)
                        && this.canEmitToPipeOn(dir)
                        && tp.canReceiveFromPipeOn(dir.getOpposite())) {
                        //ReikaJavaLibrary.pConsole(dir, this.getSide() == Side.SERVER &&
                        //this instanceof TileEntitySeparatorPipe);
                    ...
                    if (fl.canFill(dir.getOpposite(), f)) {
                        ...
                        if (toadd > 0) {
                            int added = fl.fill(
                                dir.getOpposite(), new FluidStack(f, toadd), true
                            );
                            if (added > 0) {
                                this.removeLiquid(added);
                            }
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (interaction[i]) {
                ...
                if (te instanceof TileEntityPiping) {
                    TileEntityPiping tp = (TileEntityPiping) te;
                    if (this.hasReciprocalConnectivity(tp, dir)
                        && this.canReceiveFromPipeOn(dir)
                        && tp.canEmitToPipeOn(dir.getOpposite())) {
                        ...
                        if (todrain > 0 && this.canIntakeFluid(f)) {
                            ...
                            //ReikaJavaLibrary.pConsole("Transferring "+todrain+", have
                            //"+this.getFluidLevel(), Side.SERVER,
                            //this.getAdjacentTileEntity(ForgeDirection.UP) == null);
                            this.onIntake(te);
                        }
                    }
                } else if (te instanceof PipeConnector) {
                    ...
                    if (flow.canOutput) {
                        ...
                        FluidStack fs
                            = pc.drain(dir.getOpposite(), Integer.MAX_VALUE, false);
                        if (fs != null) {
                            int level = this.getFluidLevel();
                            ...
                            if (todrain > 0) {
                                if (this.canIntakeFluid(fs.getFluid())) {
                                    ...
                                    this.onIntake(te);
                                    //ReikaJavaLibrary.pConsole("Transferring "+todrain+",
                                    //have "+this.getFluidLevel(), Side.SERVER);
                                }
                            }
                        }
                    }
    ...
    /**
     * Direction is relative to the piping block (so DOWN means the block is below the
     * pipe)
     */
    ...
    public void recomputeConnections(World world, int x, int y, int z) {
        for (int i = 0; i < 6; i++) {
            ...
            world.func_147479_m(
                x + dirs[i].offsetX, y + dirs[i].offsetY, z + dirs[i].offsetZ
            );
        }
        this.syncAllData(true);
        world.markBlockForUpdate(x, y, z);
    ...
    public boolean shouldTryToConnect(ForgeDirection dir) {
        ...
        if (tile instanceof TileEntityPiping)
            ...
            return pc.canConnectToPipe(this.getTile())
                && pc.canConnectToPipeOnSide(this.getTile(), dir.getOpposite());
    ...
    public boolean isConnectedToNonSelf(ForgeDirection dir) {
        ...
        int meta = world.getBlockMetadata(dx, dy, dz);
        return b != this.getTile().getBlock()
            || meta != this.getTile().getBlockMetadata();
    }
    ...
    public final boolean canTransferTo(PumpablePipe p, ForgeDirection dir) {
        return p instanceof TileEntityPiping
            && this.hasReciprocalConnectivity((TileEntityPiping) p, dir);
    }

    public final boolean
    hasReciprocalConnectivity(TileEntityPiping te, ForgeDirection dir) {
        return te != null && this.canConnectToPipe(te.getTile(), dir)
            && te.canConnectToPipe(this.getTile(), dir.getOpposite());
    }
