...
public abstract class TileEntityAimedCannon extends TileEntityPowerReceiver
    implements RangedEffect, DiscreteFunction, ConditionalOperation {
    ...
    protected void printSafeList() {
        for (int i = 0; i < safePlayers.size(); i++) {
            ...
            RotaryCraft.logger.log(
                ...
                + ": Safe Player " + (i + 1) + " of " + safePlayers.size() + ": " + player
            );
        }
    }
    ...
    public boolean isAimingAtTarget(World world, int x, int y, int z, double[] t) {
        ...
        while (phi2 >= 360)
            phi2 -= 360;
        //ReikaJavaLibrary.pConsole("PHI: "+phi2+" THETA: "+theta+" for "+tg[2]+",
        //"+tg[1]);
        if (tg[2] - phi2 > 180)
            tg[2] -= 360;
    ...
    public boolean adjustAim(double ang, double incl) {
        ...
        if (theta > -MAXLOWANGLE && dir == -1)
            theta = MAXLOWANGLE;
        return ReikaMathLibrary.approxr(phi, ang, 3)
            && ReikaMathLibrary.approxr(theta, incl, 3);
    }
    ...
    protected final boolean isMobOrUnlistedPlayer(EntityLivingBase ent) {
        return (
            ReikaEntityHelper.isHostile(ent)
            || (targetPlayers && ent instanceof EntityPlayer
    ...
    public void addPlayerToWhiteList(String name) {
        ReikaChatHelper.write(
            name + " added to " + placer + "'s whitelist for the\n" + this.getName()
    ...
    public final boolean playerIsSafe(EntityPlayer ep) {
        ...
        return safePlayers.contains(name)
            || name.equals(this.getPlacer().getCommandSenderName());
    }
