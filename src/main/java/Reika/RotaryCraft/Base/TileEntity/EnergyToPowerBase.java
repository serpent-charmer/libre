...
public abstract class EnergyToPowerBase extends TileEntityIOMachine
    implements SimpleProvider, PowerGenerator, GuiController, UpgradeableMachine,
    ...
    public final int getGenTorque() {
        return TileEntityEngine.getIntegratedGearTorque(
            getGenTorque(tier), integratedGear
        );
    }
    ...
    public void updateTileEntity() {
        ...
        if (DragonAPICore.debugtest) {
            ...
            tank.setContents(
                tank.getCapacity(), FluidRegistry.getFluid("rc liquid nitrogen")
            );
        }
        if (storedEnergy < 0) {
            storedEnergy = 0;
    ...
    protected final boolean isMuffled() {
        ...
        if (world.getBlock(x, y + 1, z) == Blocks.wool
            && world.getBlock(x, y - 1, z) == Blocks.wool) {
            return true;
        }
        for (int i = 0; i < 6; i++) {
    ...
    public boolean isEmitting() {
        ...
        if (this.isRedstoneControlEnabled()) {
            ...
            return this.hasRedstoneSignal() ? rs == RedstoneState.HI
                                            : rs == RedstoneState.LOW;
        } else
            return true;
    }
    ...
    public final int getMaxSpeed() {
        if (baseomega < 0)
            return 0;
        return TileEntityEngine.getIntegratedGearSpeed(
            ReikaMathLibrary.intpow2(2, baseomega), integratedGear
        );
    }
    ...
    public final int getConsumedUnitsPerTick() {
        return MathHelper.ceiling_double_int(
            this.getIdealConsumedUnitsPerTick() * this.getConsumption()
        );
    }
    ...
    @Override
    public final PowerSourceList
    getPowerSources(PowerSourceTracker io, ShaftMerger caller) {
    ...
    public final void updateTemperature(World world, int x, int y, int z, int meta) {
        int Tamb = tank.isEmpty()
            ? ReikaWorldHelper.getAmbientTemperatureAt(world, x, y, z)
            : 25;
        if (power > 0) {
    ...
    public static String getTiersAsString() {
        ...
        for (int i = 0; i < TIERS; i++) {
            ...
            String s = String.format(
                "Tier %d - Torque: %dNm; Max Speed: %d rad/s; Power: %.3f%sW",
                i,
                tq,
                om,
                base,
