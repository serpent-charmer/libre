...
public abstract class SprinklerBlock
    extends RotaryCraftTileEntity implements PipeConnector, RangedEffect {
    private int liquid;
    ...
    private void getLiq(World world, int x, int y, int z, int metadata) {
        ...
        if (m != null && m.isStandardPipe()) {
            ...
            if (tile != null && tile.contains(FluidRegistry.WATER)
                && tile.getFluidLevel() > 0) {
    ...
    public final void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (this.canPerformEffects()
            && !AtmosphereHandler.isNoAtmo(world, x, y + 1, z, blockType, false)) {
