...
public abstract class TileEntityEngine extends TileEntityInventoryIOMachine
    implements TemperatureTE, SimpleProvider, PipeConnector, PowerGenerator,
               IFluidHandler, PartialInventory, PartialTank, IntegratedGearboxable,
               IPipeConnection {
    /** Water capacity */
    ...
    protected ParallelTicker timer
        = new ParallelTicker().addTicker("fuel").addTicker("sound").addTicker(
            "temperature", ReikaTimeHelper.SECOND.getDuration()
        );
    ...
    getRequirements(World world, int x, int y, int z, int meta);

    protected final boolean hasAir(World world, int x, int y, int z) {
        if (this.isDrowned(world, x, y, z))
            return false;
        if (AtmosphereHandler.isNoAtmo(
                world,
                x - this.getWriteDirection().offsetX,
                y,
                z - this.getWriteDirection().offsetZ,
                blockType,
                true
            ))
            return false;
    ...
    private void setPowerData(World world, int x, int y, int z, int meta) {
        int speed = this.getIntegratedGearSpeed(
            this.getMaxSpeed(world, x, y, z, meta), integratedGear
        );
        this.updateSpeed(speed, speed >= omega && (omega > 0 || this.canStart()));
        torque = this.getIntegratedGearTorque(
            this.getGenTorque(world, x, y, z, meta), integratedGear
        );
    }
    ...
    affectSurroundings(World world, int x, int y, int z, int meta);
    ...
    private void updateSpeed(int maxspeed, boolean revup) {
        ...
        if (revup) {
            if (omega < maxspeed) {
                //ReikaJavaLibrary.pConsole(omega+"->"+(omega+2*(int)(ReikaMathLibrary.logbase(maxspeed,
                //2))), Side.SERVER);
                omega += 4 * ReikaMathLibrary.logbase(maxspeed + 1, 2);
                timer.setCap(
                    "fuel", Math.max(type.getFuelUnitDuration() / 4, 1)
                ); //4x fuel burn while spinning up
                if (omega > maxspeed)
                    omega = maxspeed;
            }
        } else {
    ...
    private boolean hasECU() {
        return this.getMachine(isFlipped ? ForgeDirection.UP : ForgeDirection.DOWN)
            == MachineRegistry.ECU;
    }

    private TileEntityEngineController getECU() {
        return (TileEntityEngineController
        ) this.getAdjacentTileEntity(isFlipped ? ForgeDirection.UP : ForgeDirection.DOWN);
    }

    protected abstract void
    playSounds(World world, int x, int y, int z, float pitchMultiplier, float vol);

    protected final boolean isMuffled(World world, int x, int y, int z) {
        if (RotaryAux.isMufflingBlock(world, x, y + 1, z)
            || this.getMachine(ForgeDirection.UP) == MachineRegistry.ECU) {
            if (RotaryAux.isMufflingBlock(world, x, y - 1, z)
                || this.getMachine(ForgeDirection.DOWN) == MachineRegistry.ECU)
                return true;
        }
        for (int i = 0; i < 6; i++) {
            ForgeDirection dir = dirs[i];
            if (dir != ForgeDirection.DOWN) {
                ...
                int dy = y + dir.offsetY;
                int dz = z + dir.offsetZ;
                if ((dir != write.getOpposite() && dir != write)
    ...
    public final boolean isItemValidForSlot(int i, ItemStack is) {
        ...
        switch (type) {
            ...
            case AC:
                return true;
            case SPORT:
                return (i == 0 && is.getItem() == ItemRegistry.ETHANOL.getItemInstance())
                    || (i == 1 && type.isAdditive(is));
            default:
                return false;
        }
    }
    ...
    public final boolean canExtractItem(int i, ItemStack itemstack, int j) {
        if (type == EngineType.AC) {
            if (ReikaItemHelper.matchStacks(itemstack, ItemStacks.shaftcore)
                || ReikaItemHelper.matchStacks(itemstack, ItemStacks.tungstenshaftcore)) {
    ...
    protected final void animateWithTick(World world, int x, int y, int z) {
        ...
        phi += ReikaMathLibrary.doubpow(
            ReikaMathLibrary.logbase((int) (mult * omega + 1), 2), pow
        );
    }
    ...
    public final int getFuelDuration() {
        ...
        float factor = type.getFuelUnitDuration()
            / (float) timer.getCapOf("fuel"); //to compensate for 4x burn during spinup
        if (factor <= 0)
            return 0;
        return (int
        ) ((fuel * type.getFuelUnitDuration() * (burnprogress)) * 5 / factor / 1000 * 10D
           / this.getConsumedFuel());
    }
    ...
    public final boolean canConnectToPipeOnSide(MachineRegistry p, ForgeDirection side) {
        ...
        if (type.isJetFueled() || type.isEthanolFueled())
            if ((p == MachineRegistry.FUELLINE || p == MachineRegistry.BEDPIPE)
                && side == (isFlipped ? ForgeDirection.UP : ForgeDirection.DOWN))
                return true;
    ...
    public static boolean isAirFluid(Fluid f) {
        return f.equals(FluidRegistry.getFluid("air"))
            || f.equals(FluidRegistry.getFluid("oxygen"))
            || f.equals(FluidRegistry.getFluid("rc oxygen"));
    }
    ...
    public void breakBlock() {
        if (integratedGear != 0) {
            ItemStack is
                = ItemIntegratedGearbox.getIntegratedGearItem(integratedGear, null);
    ...
    overridePipeConnection(PipeType type, ForgeDirection side) {
        if (type == PipeType.FLUID)
            return this.hasATank()
                    && this.canConnectToPipeOnSide(MachineRegistry.BEDPIPE, side)
                ? ConnectOverride.CONNECT
                : ConnectOverride.DISCONNECT;
        else if (type == PipeType.ITEM)
            return this.hasAnInventory() ? ConnectOverride.CONNECT
                                         : ConnectOverride.DISCONNECT;
        else
