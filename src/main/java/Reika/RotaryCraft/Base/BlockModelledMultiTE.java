...
public abstract class BlockModelledMultiTE extends BlockBasicMultiTE {
    ...
    public final int getRenderType() {
        return RotaryCraft.instance.isLocked()
                || OldTextureLoader.instance.loadOldTextures()
            ? 0
            : -1;
    }
    ...
    public int getMixedBrightnessForBlock(IBlockAccess world, int x, int y, int z) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            l = Math.max(l, world.getBlock(dx, dy, dz).getMixedBrightnessForBlock(world,
    dx, dy, dz));
        }
        return l;
    }*/
     ...
     */
    public int
    getMixedBrightnessForBlock(IBlockAccess iba, int x, int y, int z) {
        return iba.getLightBrightnessForSkyBlocks(
            x, y + 1, z, this.getLightValue(iba, x, y + 1, z)
        );
    }
    ...
    public void addCollisionBoxesToList(
        ...
        if (m == MachineRegistry.RESERVOIR) {
            if (e instanceof EntityItem) {
                AxisAlignedBB box
                    = AxisAlignedBB.getBoundingBox(x, y, z, x + 1, y + 0.0625, z + 1);
                if (box.intersectsWith(mask))
                    li.add(box);
            } else {
                Collection<AxisAlignedBB> c
                    = ((TileEntityReservoir) te).getComplexHitbox();
