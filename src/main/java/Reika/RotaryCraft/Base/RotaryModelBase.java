...
public abstract class RotaryModelBase extends ModelBase implements TileModel {
    ...
    private boolean canBeCompiled
        = LODModelPart.allowCompiling && this.calcDefaultCompilability();

    private final ArrayList<LODModelPart> renderGroups = new ArrayList();

    public abstract void
    renderAll(TileEntity te, ArrayList conditions, float phi, float theta);
    ...
    @Override
    public final void
    render(Entity entity, float f, float f1, float f2, float f3, float f4, float f6) {}
    ...
    public final void renderAll(TileEntity te, ArrayList conditions, float phi) {
        if (!allowRendering)
            return;
        if (canBeCompiled && phi == 0 && !GuiScreen.isCtrlKeyDown()
    ...
    protected final void renderList(TileEntity te) {
        if (!LODModelPart.allowCompiling) {
            RotaryCraft.logger.logError(
                "Attempt to call compiled model for tile " + te
                + " without compiling enabled!"
            );
        }
