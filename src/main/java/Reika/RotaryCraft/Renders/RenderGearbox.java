...
public class RenderGearbox extends RotaryTERenderer {
     ...
     */
    public void renderTileEntityGearboxAt(
        TileEntityGearbox tile, double par2, double par4, double par6, float par8
    ) {
        int var9;
        ...
        this.bindTextureByName(
            "/Reika/RotaryCraft/Textures/TileEntityTex/Transmission/Gear/"
            + tile.getGearboxType().getBaseGearboxTexture()
        );
    ...
    public void renderTileEntityAt(
        ...
        if (this.doRenderModel((RotaryCraftTileEntity) tile))
            this.renderTileEntityGearboxAt(
                (TileEntityGearbox) tile, par2, par4, par6, par8
            );
    ...
    private void
    renderMode(TileEntityGearbox tile, double par2, double par4, double par6) {
    ...
    private void renderLiquid(TileEntity tile, double par2, double par4, double par6) {
        ...
        if (tr.getLubricant() > 0) {
            ...
            if (tr.isLiving()) {
                ...
                c = ReikaColorAPI.getModifiedHue(
                    0x0000ff, 192 + (int) (32 * Math.sin(t / 16D))
                );
            }
            float u = ico.getMinU();
            float v = ico.getMinV();
            ...
            double h
                = 0.0625 + (4D / 16D * tr.getLubricant() / tr.getMaxLubricant()) * 0.9;
            if (tr.isFlipped) {
                h = 1 - h;
