...
public class RenderProjector extends RotaryTERenderer {
    ...
    public RenderProjector() {
        for (int i = 0; i < hasImages.length; i++) {
            String name = "/Reika/RotaryCraft/Textures/Projector/image"
    ...
    public void renderTileEntityProjectorAt(
        TileEntityProjector tile, double par2, double par4, double par6, float par8
    ) {
        int var9;
    ...
    public void renderTileEntityAt(
        ...
        if (this.doRenderModel((RotaryCraftTileEntity) tile))
            this.renderTileEntityProjectorAt(
                (TileEntityProjector) tile, par2, par4, par6, par8
            );
    ...
    private void renderScreen(TileEntityProjector te, double p2, double p4, double p6) {
        ...
        if (ConfigRegistry.PROJECTORLINES.getState()) {
            this.drawBeam(
                voffset, te.getBlockMetadata(), v5, a, b, c, p2, p4, p6, te.channel == -3
            );
        }
        if (te.channel == -1) {
        ...
        if (te.channel != -2) {
            if (te.emptySlide || te.channel < 0 || te.channel >= hasImages.length
                || !hasImages[te.channel]) {
        ...
        if (te.channel == -2)
            ...
            this.bindTextureByName(
                "/Reika/RotaryCraft/Textures/Projector/image" + String.valueOf(te.channel)
                + ".png"
            );
    ...
    private void drawClock(
        ...
        if (te.getBlockMetadata() < 2) {
            ...
            ReikaRenderHelper.renderVCircle(
                r,
                0,
                0,
                0,
                ReikaColorAPI.RGBtoHex(frame.getRed(), frame.getGreen(), frame.getBlue()),
                0,
                5
            );
            ReikaRenderHelper.renderVCircle(
            ...
            v5.setColorOpaque(
                minutehand.getRed(), minutehand.getGreen(), minutehand.getBlue()
            );
            v5.addVertex(0, 0, 0);
    ...
    private void renderErrorScreen(
        double a,
        double b,
        double c,
        double voffset,
