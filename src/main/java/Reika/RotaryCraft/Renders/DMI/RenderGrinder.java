...
public class RenderGrinder extends RotaryTERenderer {
     ...
     */
    public void renderTileEntityGrinderAt(
        TileEntityGrinder tile, double par2, double par4, double par6, float par8
    ) {
        int var9;
    ...
    public void renderTileEntityAt(
        ...
        if (this.doRenderModel((RotaryCraftTileEntity) tile))
            this.renderTileEntityGrinderAt(
                (TileEntityGrinder) tile, par2, par4, par6, par8
            );
        if (((RotaryCraftTileEntity) tile).isInWorld()
            && MinecraftForgeClient.getRenderPass() == 1) {
            IORenderer.renderIO(tile, par2, par4, par6);
            if (((TileEntityGrinder) tile).getEnchantmentHandler().hasEnchantments())
                EnchantmentRenderer.renderGlint(
                    tile, GrinderModel, null, par2, par4, par6
                );
