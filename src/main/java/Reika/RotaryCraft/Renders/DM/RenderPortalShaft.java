...
public class RenderPortalShaft extends RenderShaft {
    public void renderTileEntityPortalShaftAt(
        TileEntityPortalShaft tile, double par2, double par4, double par6, float par8
    ) {
        int var9;
        ...
        if (meta == 5)
            dir = -1;
        double s = tile.isInWorld() && (tile.isEnteringPortal() || tile.isExitingPortal())
            ? 1.5
            : 1;
        double d = tile.isInWorld() ? tile.isEnteringPortal() ? 0.25
                : tile.isExitingPortal()                      ? -0.25
                                                              : 0
                                    : 0;
        double ss = 0.5;
        if (meta <= 3) {
    ...
    public void renderTileEntityAt(
        ...
        if (this.doRenderModel((RotaryCraftTileEntity) tile))
            this.renderTileEntityPortalShaftAt(
                (TileEntityPortalShaft) tile, par2, par4, par6, par8
            );
