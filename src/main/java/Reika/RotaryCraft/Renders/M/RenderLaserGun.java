...
public class RenderLaserGun extends RotaryTERenderer {
     ...
     */
    public void renderTileEntityLaserGunAt(
        TileEntityLaserGun tile, double par2, double par4, double par6, float par8
    ) {
        int var9;
    ...
    public void renderTileEntityAt(
        ...
        if (this.doRenderModel((RotaryCraftTileEntity) tile))
            this.renderTileEntityLaserGunAt(
                (TileEntityLaserGun) tile, par2, par4, par6, par8
            );
    ...
    private void
    renderLaser(TileEntityLaserGun tile, double par2, double par4, double par6) {
        if (tile == null)
            return;
        if (!tile.isInWorld())
            return;
        ...
        double dx = r * Math.cos(Math.toRadians(tile.theta))
            * Math.cos(Math.toRadians(-tile.phi + 90));
        double dy = r * Math.sin(Math.toRadians(tile.theta));
        double dz = r * Math.cos(Math.toRadians(tile.theta))
            * Math.sin(Math.toRadians(-tile.phi + 90));
