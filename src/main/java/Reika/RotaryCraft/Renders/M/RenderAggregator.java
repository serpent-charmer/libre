...
public class RenderAggregator extends RotaryTERenderer {
     ...
     */
    public void renderTileEntityAggregatorAt(
        TileEntityAggregator tile, double par2, double par4, double par6, float par8
    ) {
        ModelAggregator var14;
        var14 = AggregatorModel;

        this.bindTextureByName(
            "/Reika/RotaryCraft/Textures/TileEntityTex/aggregatortex.png"
        );
    ...
    public void renderTileEntityAt(
        ...
        if (this.doRenderModel((RotaryCraftTileEntity) tile))
            this.renderTileEntityAggregatorAt(
                (TileEntityAggregator) tile, par2, par4, par6, par8
            );
        if (((RotaryCraftTileEntity) tile).isInWorld()
            && MinecraftForgeClient.getRenderPass() == 1) {
            IORenderer.renderIO(tile, par2, par4, par6);
            this.renderWater((TileEntityAggregator) tile, par2, par4, par6);
        }
    }

    private void
    renderWater(TileEntityAggregator tile, double par2, double par4, double par6) {
