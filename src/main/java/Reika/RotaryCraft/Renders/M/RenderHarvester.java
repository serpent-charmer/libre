...
public class RenderHarvester extends RotaryTERenderer {
     ...
     */
    public void renderTileEntityMobHarvesterAt(
        TileEntityMobHarvester tile, double par2, double par4, double par6, float par8
    ) {
        int var9;
        ...
        this.bindTextureByName(
            "/Reika/RotaryCraft/Textures/TileEntityTex/harvestertex.png"
        );
    ...
    public void renderTileEntityAt(
        ...
        if (this.doRenderModel((RotaryCraftTileEntity) tile))
            this.renderTileEntityMobHarvesterAt(
                (TileEntityMobHarvester) tile, par2, par4, par6, par8
            );
        ...
        if (((RotaryCraftTileEntity) tile).isInWorld()
            && MinecraftForgeClient.getRenderPass() == 1)
            ReikaAABBHelper.renderAABB(
                ((TileEntityMobHarvester) tile).getBox(),
                par2,
                par4,
                par6,
                tile.xCoord,
                tile.yCoord,
                tile.zCoord,
                ((TileEntityIOMachine) tile).iotick,
                255,
                127,
                0,
                true
        ...
        if (((RotaryCraftTileEntity) tile).isInWorld()
            ...
            if (((TileEntityMobHarvester) tile).getEnchantmentHandler().hasEnchantments())
                ...
                EnchantmentRenderer.renderGlint(
                    tile, HarvesterModel, null, par2, par4, par6
                );
    ...
    private void
    renderLaser(TileEntityMobHarvester harv, double par2, double par4, double par6) {
        ...
        if (harv.laser) {
            ...
            ReikaAABBHelper.renderAABB(
                harv.getLaser().expand(0.125, 0.001, 0.125),
                par2,
                par4,
                par6,
                harv.xCoord,
