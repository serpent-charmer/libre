...
public class RenderReservoir extends RotaryTERenderer {
    ...
    public void renderTileEntityReservoirAt(
        TileEntityReservoir tile, double par2, double par4, double par6, float par8
    ) {
        int var9;
        ...
        this.bindTextureByName(
            "/Reika/RotaryCraft/Textures/TileEntityTex/reservoirtex.png"
        );
    ...
    public void renderTileEntityAt(
        ...
        if (MinecraftForgeClient.getRenderPass() == 1
            || !((RotaryCraftTileEntity) tile).isInWorld()) {
    ...
    private void
    renderCover(TileEntityReservoir tr, double par2, double par4, double par6) {
    ...
    private void renderLiquid(TileEntity tile, double par2, double par4, double par6) {
        ...
        if (f != null) {
            ...
            if (f == FluidRegistry.WATER && tile.worldObj != null) {
                ico = BlockIconEvent.fire(
                    Blocks.water, tile.worldObj, tile.xCoord, tile.yCoord, tile.zCoord, 1
                );
            }
            float u = ico.getMinU();
            float v = ico.getMinV();
            ...
            if (tr.hasNearbyReservoir(1)) {
                TileEntity teb
                    = tr.worldObj.getTileEntity(tr.xCoord - 1, tr.yCoord, tr.zCoord + 1);
            ...
            if (tr.hasNearbyReservoir(2)) {
                TileEntity teb
                    = tr.worldObj.getTileEntity(tr.xCoord, tr.yCoord, tr.zCoord + 1);
                if (teb instanceof TileEntityReservoir) {
            ...
            if (tr.hasNearbyReservoir(4)) {
                TileEntity teb
                    = tr.worldObj.getTileEntity(tr.xCoord - 1, tr.yCoord, tr.zCoord);
                if (teb instanceof TileEntityReservoir) {
