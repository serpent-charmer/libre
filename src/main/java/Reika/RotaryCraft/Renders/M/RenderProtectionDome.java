...
public class RenderProtectionDome extends RotaryTERenderer {
    ...
    public void renderTileEntityProtectionDomeAt(
        TileEntityProtectionDome tile, double par2, double par4, double par6, float par8
    ) {
        int var9;
        ...
        this.bindTextureByName(
            "/Reika/RotaryCraft/Textures/TileEntityTex/" + this.getImageFileName(tile)
        );
    ...
    public void renderTileEntityAt(
        ...
        if (this.doRenderModel((RotaryCraftTileEntity) tile))
            this.renderTileEntityProtectionDomeAt(
                (TileEntityProtectionDome) tile, par2, par4, par6, par8
            );
        if (((RotaryCraftTileEntity) tile).isInWorld()
            && MinecraftForgeClient.getRenderPass() == 1) {
            IORenderer.renderIO(tile, par2, par4, par6);
            this.renderField(
                ((TileEntityProtectionDome) tile), par2 + 0.5, par4 + 0.5, par6 + 0.5
            );
        }
    }

    protected void
    renderField(TileEntityProtectionDome te, double x, double y, double z) {
        if (!te.isInWorld())
            return;
        if (te.getRange() <= 0)
            return;
        ...
        for (int k = 0; k < 360; k += 15) {
            ReikaRenderHelper.renderVCircle(0.125, x, y+0.5, z, color,
        (System.nanoTime()/7500000)%360+k, 15); ReikaRenderHelper.renderLine(x, y, z,
        x+r*Math.sin(Math.toRadians(ang)*Math.cos(Math.toRadians(k))),
        y+r-Math.sin(Math.toRadians(ang)),
        z+r*Math.sin(Math.toRadians(ang)*Math.sin(Math.toRadians(k))), color);
        }
