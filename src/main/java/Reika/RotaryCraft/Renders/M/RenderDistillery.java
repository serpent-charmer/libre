...
public class RenderDistillery extends RotaryTERenderer {
     ...
     */
    public void renderTileEntityDistilleryAt(
        TileEntityDistillery tile, double par2, double par4, double par6, float par8
    ) {
        ModelDistillery var14;
        var14 = DistilleryModel;

        this.bindTextureByName(
            "/Reika/RotaryCraft/Textures/TileEntityTex/distillertex.png"
        );
    ...
    public void renderTileEntityAt(
        ...
        if (this.doRenderModel((RotaryCraftTileEntity) tile))
            this.renderTileEntityDistilleryAt(
                (TileEntityDistillery) tile, par2, par4, par6, par8
            );
        if (((RotaryCraftTileEntity) tile).isInWorld()
            ...
            this.renderFuels(
                ...
                par4,
                par6,
                ((TileEntityDistillery) tile).getFluidInInput(),
                false
            );
            this.renderFuels(
                (TileEntityDistillery) tile,
                ...
                par4,
                par6,
                ((TileEntityDistillery) tile).getFluidInOutput(),
                true
            );
        }
    }
    ...
    private void renderFuels(
        ...
        ) (amount / ((double) tile.getCapacity()) * (ReikaLiquidRenderer.LEVELS - 1))]);
