...
public class RenderDisplay extends RotaryTERenderer {
    ...
    public void renderTileEntityDisplayAt(
        TileEntityDisplay tile, double par2, double par4, double par6, float par8
    ) {
        int var9;
    ...
    public void renderTileEntityAt(
        ...
        if (this.doRenderModel((RotaryCraftTileEntity) tile))
            this.renderTileEntityDisplayAt(
                (TileEntityDisplay) tile, par2, par4, par6, par8
            );
        if (((RotaryCraftTileEntity) tile).isInWorld()
            && MinecraftForgeClient.getRenderPass() == 1) {
            if (((TileEntityDisplay) tile).canDisplay()
                && ((TileEntityDisplay) tile).hasSpace()) {
    ...
    private void
    renderScreen(TileEntityDisplay tile, double par2, double par4, double par6) {
        if (tile == null)
            return;
    ...
    private void
    renderText(TileEntityDisplay tile, double par2, double par4, double par6) {
        if (tile == null)
            return;
        if (!tile.hasList())
            return;
        ...
        float scroll = cache.size() > tile.displayHeight
            ? (core * 4) % (180 * cache.size()) / 180F
            : 0;
        int linescroll = scroll - (int) scroll > 0.5F
            ? (int) scroll + 1
            : (int) scroll; //tile.getRoundedScroll();
        ...
        for (int i = linescroll; i < len + 1; i++) {
            ...
            //ReikaJavaLibrary.pConsole("Printing line "+i+" of
            //"+tile.getMessageLength()+": "+tile.getMessageLine(i));
            //f.drawString(tile.getMessageLine(i), 1,
            //-1+(int)((i-scroll)*tile.lineHeight), tile.getTextColor());
            //f.drawSplitString(s, 1, -1+(int)((i-scroll)*tile.lineHeight),
            //tile.displayWidth*f.FONT_HEIGHT, tile.getTextColor());
            f.drawString(
                s2, 1, -1 + (int) ((i - scroll) * tile.lineHeight), tile.getTextColor()
            );
