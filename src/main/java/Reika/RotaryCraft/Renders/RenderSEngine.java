...
public class RenderSEngine extends RotaryTERenderer {
     ...
     */
    public void renderTileEntityEngineAt(
        TileEntityEngine tile, double par2, double par4, double par6, float par8
    ) {
        int var9;
        ...
        switch (tile.getEngineType()) {
            case DC:
                this.bindTextureByName(
                    "/Reika/RotaryCraft/Textures/TileEntityTex/Engine/dc.png"
                );
                break;
            case WIND:
                this.bindTextureByName(
                    "/Reika/RotaryCraft/Textures/TileEntityTex/Engine/windtex.png"
                );
                break;
            case STEAM:
                this.bindTextureByName(
                    "/Reika/RotaryCraft/Textures/TileEntityTex/Engine/steamtex.png"
                );
                break;
            case GAS:
                this.bindTextureByName(
                    "/Reika/RotaryCraft/Textures/TileEntityTex/Engine/combtex.png"
                );
                break;
            case AC:
                this.bindTextureByName(
                    "/Reika/RotaryCraft/Textures/TileEntityTex/Engine/actex.png"
                );
                break;
            case SPORT:
                this.bindTextureByName(
                    "/Reika/RotaryCraft/Textures/TileEntityTex/Engine/perftex.png"
                );
                break;
            case HYDRO:
                TileEntityHydroEngine eng = (TileEntityHydroEngine) tile;
                String sg = "/Reika/RotaryCraft/Textures/TileEntityTex/Engine/"
                    + (eng.isBedrock() ? "bedhydrotex.png" : "hydrotex.png");
                this.bindTextureByName(sg);
                break;
            case MICRO:
                this.bindTextureByName(
                    "/Reika/RotaryCraft/Textures/TileEntityTex/Engine/microtex.png"
                );
                break;
            case JET:
                ...
                if (tile.isInWorld())
                    ; //s = s+"_with_alpha";
                this.bindTextureByName(
                    "/Reika/RotaryCraft/Textures/TileEntityTex/Engine/jettex" + s + ".png"
                );
                break;
        }
        ...
        if (tile.isInWorld()) {
            ...
            switch (tile.getEngineType()) {
                ...
                case HYDRO:
                    ...
                    var21.renderAll(
                        tile,
                        ReikaJavaLibrary.makeListFrom(eng.failed, eng.isBedrock()),
                        0,
                        0
                    );
                    GL11.glScaled(1D / s, 1D / s, 1D / s);
                ...
                case JET:
                    this.bindTextureByName(
                        "/Reika/RotaryCraft/Textures/TileEntityTex/Engine/jettex.png"
                    );
                    GL11.glRotatef(90, 0.0F, 1.0F, 0.0F);
        ...
        switch (tile.getEngineType()) {
            ...
            case HYDRO:
                ...
                var21.renderAll(
                    tile,
                    ReikaJavaLibrary.makeListFrom(eng.failed, eng.isBedrock()),
                    eng.isReversed() ? tile.phi : -tile.phi,
                    0
                );
                break;
            case MICRO:
            ...
            case JET:
                ...
                if (f > 0) {
                    int temp = Math.max(
                        tile.temperature - 600, (tile.temperature - 1000) * 5 / 2
                    );
                    ...
                    this.bindTextureByName(
                        "/Reika/RotaryCraft/Textures/TileEntityTex/Engine/jettex_glow_mask2.png"
                    );
                    ...
                    Tessellator.instance.setColorOpaque_I(
                        ReikaColorAPI.getColorWithBrightnessMultiplier(c, 0.65F)
                    );
    ...
    private void prepareShader(TileEntityEngine tile) {
        if (tile.getEngineType() == EngineType.JET) {
            ...
            double dist = ep.getDistance(
                tile.xCoord + 0.5 + dx, tile.yCoord + 0.5, tile.zCoord + 0.5 + dz
            );
            float f = 0;
            if (te.omega > 0) {
                f = Math.max(
                    f, (float) Math.sqrt(te.omega * 0.5F / EngineType.JET.getSpeed())
                );
            }
            if (te.temperature > 100) {
                f = Math.max(
                    ...
                    Math.min(
                        1,
                        (te.temperature - 100F) / (te.getMaxExhaustTemperature() - 100F)
                    )
                );
            }
            double dd = 0.25;
            ...
            for (double d = 0; d <= 3; d += dd) {
                ...
                HeatRippleRenderer.instance.addHeatRippleEffectIfLOS(
                    tile,
                    tile.xCoord + 0.5 + dx,
                    tile.yCoord + 0.5,
                    tile.zCoord + 0.5 + dz,
    ...
    public void renderTileEntityAt(
        ...
        if (this.doRenderModel((RotaryCraftTileEntity) tile))
            this.renderTileEntityEngineAt(
                (TileEntityEngine) tile, par2, par4, par6, par8
            );
    ...
    private void renderRedstoneFrame(
        TileEntityEngine tile, double par2, double par4, double par6, float par8
    ) {
