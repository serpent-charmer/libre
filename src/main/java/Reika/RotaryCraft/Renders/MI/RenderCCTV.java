...
public class RenderCCTV extends RotaryTERenderer {
     ...
     */
    public void renderTileEntityCCTVAt(
        TileEntityCCTV tile, double par2, double par4, double par6, float par8
    ) {
        int var9;
        ...
        if (!tile.isInWorld()) {
            ...
            {
                //((BlockForceFieldBlock1)var10).unifyAdjacentChests(tile.worldObj,
    ...
    private void
    renderColors(TileEntityCCTV tile, double par2, double par4, double par6) {
        ...
        for (int i = 0; i < 3; i++) {
            if (tile.colors[i] != -1) {
                ...
                int[] rgb
                    = { ReikaDyeHelper.getColorFromDamage(tile.colors[i]).getRed(),
                        ReikaDyeHelper.getColorFromDamage(tile.colors[i]).getGreen(),
                        ReikaDyeHelper.getColorFromDamage(tile.colors[i]).getBlue() };
                v5.setColorOpaque(rgb[0], rgb[1], rgb[2]);
                double[] dd
                    = ReikaPhysicsHelper.polarToCartesian(0.37, tile.theta, tile.phi);
                double[] dd2
                    = ReikaPhysicsHelper.polarToCartesian(-0.37, tile.theta, tile.phi);
