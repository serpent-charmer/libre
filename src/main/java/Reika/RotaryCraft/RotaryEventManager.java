...
public class RotaryEventManager {
    ...
    public void interceptNEI(NEIRecipeCheckEvent evt) {
        if (ItemRegistry.SHAFT.matchItem(evt.getItem())) {
            codechicken.nei.recipe.GuiCraftingRecipe.openRecipeGui(
                "item", MaterialRegistry.WOOD.getShaftItem()
            );
    ...
    public void updateStreamHydros(SetBlockEvent.Pre evt) {
        if (!evt.isWorldgen && evt.newBlock != evt.currentBlock) {
            if (InterfaceCache.STREAM.instanceOf(evt.newBlock)
                || InterfaceCache.STREAM.instanceOf(evt.currentBlock)) {
                for (int i = -1; i <= 1; i++) {
                    for (int k = -1; k <= 1; k++) {
                        for (Object o : evt.world
                                            .getChunkFromChunkCoords(
                                                evt.chunkLocation.chunkXPos + i,
                                                evt.chunkLocation.chunkZPos + k
                                            )
    ...
    public void renderEMPSparks(EntityRenderingLoopEvent evt) {
        if (MinecraftForgeClient.getRenderPass() == 1) {
            ...
            EMPSparkRenderer.instance.render(
                Tessellator.instance, ReikaRenderHelper.getPartialTickTime()
            );
        }
    }
    ...
    public void enderVisor(EnderLookAggroEvent evt) {
        ...
        if (is != null
            && (is.getItem() == ItemRegistry.BEDREVEAL.getItemInstance()
                || is.getItem() == ItemRegistry.BEDHELM.getItemInstance())) {
    ...
    public void chaosProtection(LivingDropsEvent evt) {
        if (evt.entity instanceof EntitySlime) {
            int rounds
                = evt.entity.getEntityData().getInteger(TileEntityMultiCannon.SLIME_NBT);
            if (rounds > 0) {
                ...
                while (drop > 0) {
                    ...
                    ReikaItemHelper.dropItem(
                        evt.entity,
                        ReikaItemHelper.getSizedItemStack(ItemStacks.ballbearing, amt)
                    );
                    drop -= amt;
                }
            }
    ...
    public void chaosProtection(LivingHurtEvent evt) {
        if (evt.entity instanceof EntityPlayer) {
            if (ItemBedrockArmor.isWearingFullSuitOf(evt.entityLiving)) {
                ...
                if (evt.source.damageType.startsWith("chaos")
                    || evt.source.damageType.startsWith("damage.de.")
                    || evt.source.damageType.startsWith("de.")) {
    ...
    public void openConduitGUIWithScrewdriver(PlayerInteractEventClient evt) {
        if (evt.entityPlayer.isSneaking() && evt.action == Action.LEFT_CLICK_BLOCK
            && RotaryAux.isHoldingScrewdriver(evt.entityPlayer)) {
            Block b = evt.world.getBlock(evt.x, evt.y, evt.z);
            if (b.getClass().getName().equals(
                    "crazypants.enderio.conduit.BlockConduitBundle"
                )) {
                try {
                    conduitGui.invoke(
                        null, evt.world, evt.x, evt.y, evt.z, evt.entityPlayer
                    );
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public void openConduitGUIWithScrewdriver(PlayerInteractEvent evt) {
        if (!evt.entityPlayer.worldObj.isRemote && evt.entityPlayer.isSneaking()
    ...
    { ItemStack is = evt.getItem(); if (is != null && ItemRegistry.getEntry(is) != null &&
    ItemRegistry.getEntry(is).isPlacer())
    ...
    public void cleanUpDecoTankCrafting(PlayerEvent.ItemCraftedEvent evt) {
        if (ReikaItemHelper.matchStacks(evt.crafting,
    BlockRegistry.DECOTANK.getStackOf())) { if (evt.crafting.stackTagCompound != null &&
    evt.crafting.stackTagCompound.getBoolean(DecoTankSettingsRecipe.NBT_TAG)) {
    ...
    public void cleanUpReservoirCrafting(PlayerEvent.ItemCraftedEvent evt) {
        if (ReikaItemHelper.matchStacks(
                evt.crafting, MachineRegistry.RESERVOIR.getCraftedProduct()
            )) {
            if (evt.crafting.stackTagCompound != null
                && evt.crafting.stackTagCompound.getBoolean(ReservoirComboRecipe.NBT_TAG
                )) {
                if (!ReikaInventoryHelper.addToIInv(
                        MachineRegistry.RESERVOIR.getCraftedProduct(),
                        evt.player.inventory
                    )) {
                    ReikaItemHelper.dropItem(
                        evt.player, MachineRegistry.RESERVOIR.getCraftedProduct()
                    );
                }
    ...
    public void burnLubricantHose(BlockConsumedByFireEvent evt) {
        if (MachineRegistry.getMachine(evt.world, evt.xCoord, evt.yCoord, evt.zCoord)
            == MachineRegistry.HOSE) {
    ...
    public void harvestSpawner(BlockEvent.BreakEvent evt) {
        if (evt.block == Blocks.mob_spawner
            && ItemRegistry.BEDPICK.matchItem(evt.getPlayer().getCurrentEquippedItem())) {
            evt.setExpToDrop(0);
        }
    }
        ...
        if (evt.getGuiClass() == GuiAutoCrafter.class) {
            if (evt.slotIndex < 18) {
                ...
                if (is != null) {
                    ...
                    ReikaGuiAPI.instance.drawItemStack(new RenderItem(), out, evt.slotX,
    evt.slotY);
                }
            }
        }
        ...
        if (evt.ammount < 1000) {
            if (e instanceof EntityPlayer) {
                if (ItemBedrockArmor.isWearingFullSuitOf(e)) {
                    ...
                    if (evt.ammount <= 1 && evt.source != DamageSource.drown
                        && evt.source != DamageSource.starve
                        && !AtmosphereHandler.isAtmoBreathabilityDamage(evt.source)) {
                        evt.ammount = 0;
                        evt.setCanceled(true);
                    } else {
                        ...
                        if (attacker instanceof EntityPlayer) {
                            ItemStack held
                                = ((EntityPlayer) attacker).getCurrentEquippedItem();
                            if (held != null
                                && held.getItem()
                                       .getClass()
                                       .getSimpleName()
    ...
    public void enforceHarvesterLooting(LivingDropsEvent ev) {
        if (ev.source instanceof HarvesterDamage) {
            ...
            ReikaObfuscationHelper.invoke(
                "dropEquipment", e, true, dmg.hasInfinity() ? 100 : looting * 4
            );
    ...
    public void armorDamageBoost(LivingHurtEvent evt) {
        if (evt.source instanceof MachineDamage) {
            evt.ammount = ((MachineDamage) evt.source)
                              .onDamageDealt(evt.ammount, evt.entityLiving);
        }
    }
        ...
        if (ev.source == RotaryCraft.grind) {
            ...
            if (food != null) {
                ...
                if (md.lastMachine instanceof TileEntityGrinder) {
                    MachineEnchantmentHandler ench
                        = ((TileEntityGrinder) md.lastMachine).getEnchantmentHandler();
                ...
                ReikaItemHelper.dropItem(
                    world,
                    ev.entityLiving.posX,
                    ev.entityLiving.posY,
                    ev.entityLiving.posZ,
                    is
                );
            }
            ev.setCanceled(true);
    ...
    public void buildWorktables(PlayerPlaceBlockEvent evt) {
        if (evt.block == Blocks.crafting_table || evt.block instanceof BlockWorkbench
    ...
    public void buildWorktables(PlayerInteractEvent evt) {
        if (evt.action == Action.RIGHT_CLICK_BLOCK) {
            Block b = evt.world.getBlock(evt.x, evt.y, evt.z);
            if (b == Blocks.crafting_table || b instanceof BlockWorkbench
    ...
    private boolean checkAndBuildWorktable(World world, int x, int y, int z) {
        if (!ReikaWorldHelper.matchWithItemStack(
                world, x, y - 1, z, ItemStacks.steelblock
            ))
            return false;
            ...
            for (int k = -1; k <= 1; k++) {
                if (i != 0 || k != 0) {
                    ...
                    if (!ReikaWorldHelper.matchWithItemStack(
                            ...
                            y - 2,
                            z + k,
                            ReikaItemHelper.stoneDoubleSlab.asItemStack()
                        ))
                        return false;
                }
            }
        ...
        world.setBlock(
            ...
            MachineRegistry.WORKTABLE.getBlockMetadata(),
            3
        );
        return true;
    }
    ...
    public void preventDisallowOfCrucialBlocks(BlockEvent ev) {
        ...
        if (place) { //Bukkit Block Place Event
            if (m != null) {
                if (ConfigRegistry.ALLOWBAN.getState()) {
                    if (m.isCrucial()) {
                        if (m.canBeDisabledInOverworld()
                            && ReikaWorldHelper.otherDimensionsExist()
                            && world.provider.dimensionId == 0) {
                            if (ev.isCanceled())
                                RotaryCraft.logger.log(
                                    "Something successfully cancelled the placement of "
                                    + m + ". This ban applies to the overworld only!"
                                );
                        } else {
                            if (ev.isCanceled())
        ...
        if (ConfigRegistry.LOGBLOCKS.getState()) {
            if (m != null) {
                EntityPlayer ep
                    = (EntityPlayer) ReikaWorldHelper.getClosestLivingEntityOfClass(
                        EntityPlayer.class, world, x + 0.5, y + 0.5, z + 0.5, 6
                    );
                String s = place ? "placed" : "removed";
                String name = ep != null ? ep.getCommandSenderName() : "<No Player>";
                RotaryCraft.logger.log(
                    "A " + m.getName() + " was " + s + " by " + name + " at " + x + ", "
                    + y + ", " + z + " in world dimension " + world.provider.dimensionId
                );
            }
        }
    }
    ...
    private void hardCancel(Event e, boolean cancel, boolean print) {
        if (!e.isCancelable())
            throw new IllegalArgumentException(
                "Event " + e.getClass().getSimpleName() + " cannot be cancelled!"
            );
        try {
