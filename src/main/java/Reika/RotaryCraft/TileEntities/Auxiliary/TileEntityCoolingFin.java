...
public class TileEntityCoolingFin extends RotaryCraftTileEntity implements TemperatureTE {
    ...
    private static final Interpolation reactorTemperatureEfficiency
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote) {
            if (ModList.IC2.isLoaded()
                && (te instanceof IReactor || te instanceof IReactorChamber)) {
    ...
    private void coolIC2Reactor(World world, int x, int y, int z, Object te) {
        ...
        if (h > 0) {
            int rem = Math.max(
                1,
                (int) Math.min(
                    20 * setting.rawMultiplier
                        * reactorTemperatureEfficiency.getValue(temperature),
                    h
                )
            );
