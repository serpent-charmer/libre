...
public class TileEntityScreen extends InventoriedPowerReceiver {
    ...
    public void activate(EntityPlayer ep) {
        ...
        if (!this.isValidCamera(cameraPos))
            return;
        RemoteControlMachine te = (RemoteControlMachine
        ) worldObj.getTileEntity(cameraPos[0], cameraPos[1], cameraPos[2]);
    ...
    private boolean isValidCamera(int[] cameraPos) {
        ...
        return (
            worldObj.getTileEntity(cameraPos[0], cameraPos[1], cameraPos[2])
                instanceof RemoteControlMachine
        );
    }
    ...
    private boolean canRun() {
        ...
        if (inv[0] == null || inv[1] == null || inv[2] == null)
            return false;
        if (inv[0].getItem() != Items.dye || inv[1].getItem() != Items.dye
            || inv[2].getItem() != Items.dye)
            return false;
        return true;
    }
    ...
    private int[] getCameraFromColors(World world) {
        ...
        int[] colors
            = { inv[0].getItemDamage(), inv[1].getItemDamage(), inv[2].getItemDamage() };
        ...
        for (int i = -range; i <= range; i++) {
            for (int j = -range; j <= range; j++) {
                for (int k = -range; k <= range; k++) {
                    TileEntity te
                        = world.getTileEntity(xCoord + i, yCoord + j, zCoord + k);
                    if (te != null) {
                        if (te instanceof RemoteControlMachine) {
                            ...
                            if (camcolor[0] == colors[0] && camcolor[1] == colors[1]
                                && camcolor[2] == colors[2]) {
                                cameras.put(
                                    colors, new int[] { cc.xCoord, cc.yCoord, cc.zCoord }
                                );
                                return true;
                            }
                        }
