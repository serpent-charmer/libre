...
public class TileEntityFillingStation
    extends InventoriedPowerLiquidInOut implements ConditionalOperation {
        ...
        if (this.hasFillable()) {
            ...
            if (is != null && is.getItem() instanceof Fillable) {
                ReikaInventoryHelper.addOrSetStack(
                    ReikaItemHelper.getSizedItemStack(is, 1), inv, FILLING_SLOT
                );
    ...
    private boolean hasContainer() {
        return inv[FILLING_SLOT] != null
            && FluidContainerRegistry.isContainer(inv[FILLING_SLOT]);
    }

    private void fillContainer() {
        int maxadd = this.getFluidToAdd() * 128;
        ItemStack filled = FluidContainerRegistry.fillFluidContainer(
            new FluidStack(tank.getActualFluid(), maxadd), inv[FILLING_SLOT]
        );
        if (filled != null) {
    ...
    public boolean canMakeFuel() {
        ...
        if (fs == null) {
            boolean item
                = inv[FUEL_SLOT].getItem() == ItemRegistry.ETHANOL.getItemInstance();
            boolean space = tank.canTakeIn(FUEL_PER_CRYSTAL)
                && (tank.isEmpty()
                    || tank.getActualFluid().equals(FluidRegistry.getFluid("rc ethanol"))
                );
            return item && space;
        } else {
            if (tank.isEmpty())
                return true;
            return tank.canTakeIn(fs.amount)
                && tank.getActualFluid().equals(fs.getFluid());
        }
    }
    ...
    public boolean isItemValidForSlot(int i, ItemStack itemstack) {
        ...
        if (i == FUEL_SLOT) {
            ...
            return container
                || itemstack.getItem() == ItemRegistry.ETHANOL.getItemInstance();
        }
        return false;
    }
