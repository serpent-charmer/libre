...
public class TileEntityFurnaceHeater
    extends TileEntityPowerReceiver implements TemperatureTE, ConditionalOperation {
    ...
    public void updateTemperature(World world, int x, int y, int z, int meta) {
        if (torque >= MINTORQUE && power >= MINPOWER && omega > 0
            && this.hasHeatableMachine(world)) {
            temperature += 3 * ReikaMathLibrary.logbase(omega, 2)
                * ReikaMathLibrary.logbase(torque, 2);
        }
        int Tamb = power > MINPOWER && torque > MINTORQUE
            ? 30
            : ReikaWorldHelper.getAmbientTemperatureAt(
                world, x, y, z
            ); //prevent nether exploit
        if (temperature > Tamb) {
        ...
        if (temperature > MAXTEMP)
            temperature = MAXTEMP;
        if (temperature >= MAXTEMP)
            if (!world.isRemote && ConfigRegistry.BLOCKDAMAGE.getState()
                && rand.nextInt(DifficultyEffects.FURNACEMELT.getInt()) == 0)
    ...
    private boolean hasHeatableMachine(World world) {
        ...
        MachineRegistry m = MachineRegistry.getMachine(
            world, furnaceLocation.xCoord, furnaceLocation.yCoord, furnaceLocation.zCoord
        );
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (this.hasFurnace()) {
            ...
            if (te instanceof ThermalMachine) {
                ...
                s.fuelLevel = 4000;
                s.meltPower
                    = temperature * 25 / 6; //that allows the friction heater to break
                                            //pyrotheum temperatures at its 1200C (~800kW)
                s.write(te);
    ...
    private void heatMachine(World world, int x, int y, int z, ThermalMachine te) {
        if (te.canBeFrictionHeated()) {
            int tdiff
                = Math.min(te.getMaxTemperature(), temperature) - te.getTemperature();
            ...
            if (te.getTemperature() > te.getMaxTemperature()) {
                te.onOverheat(
                    world,
                    furnaceLocation.xCoord,
                    furnaceLocation.yCoord,
                    furnaceLocation.zCoord
                );
            }
            ...
            if (soundtick > 49) {
                SoundRegistry.FRICTION.playSoundAtBlock(
                    world, x, y, z, RotaryAux.isMuffled(this) ? 0.1F : 0.5F, 1
                );
                soundtick = 0;
            }
        }
    ...
    private boolean canTileMake(TileEntityFurnace tile, ItemStack is) {
        ...
        if (ReikaItemHelper.matchStacks(is, out)
            && is.stackSize + out.stackSize <= is.getMaxStackSize())
            return true;
        return false;
    }
    ...
    private void hijackFurnace(World world, int x, int y, int z, int meta) {
        ...
        if (in != null) {
            ...
            FrictionRecipe special
                = RecipesFrictionHeater.getRecipes().getSmelting(in, temperature);
            ...
            if (smelt != null || special != null) {
                ...
                if (smeltTime >= max) {
                    ...
                    if (smelt != null && tile.canSmelt()) {
                        tile.smeltItem();
                        xp = MathHelper.ceiling_float_int(
                            FurnaceRecipes.smelting().func_151398_b(smelt)
                        );
                    } else if (special != null) {
                        ItemStack out2 = special.getOutput();
                        ReikaInventoryHelper.decrStack(0, tile, 1);
                        int amt = out != null ? out.stackSize + out2.stackSize
                                              : out2.stackSize;
                    ...
                    if (xp > 0 && ConfigRegistry.FRICTIONXP.getState()) {
                        ReikaWorldHelper.splitAndSpawnXP(
                            world, fx + 0.5, fy + 0.6, fz + 0.5, xp, 600
                        );
                    }
                    smeltTime = 0;
                }
    ...
    private void getFurnaceCoordinates(World world, int x, int y, int z, int meta) {
        furnaceLocation
            = new Coordinate(this).offset(this.getReadDirection().getOpposite(), 1);
    }
    ...
    private void meltFurnace(World world) {
        ...
        world.createExplosion(
            null,
            furnaceLocation.xCoord + 0.5,
            furnaceLocation.yCoord + 0.5,
            furnaceLocation.zCoord + 0.5,
            1F,
            false
        );
        ...
        //for (int i = 0; i < 8; i++)
        //	ReikaItemHelper.dropItem(world, fx+par5Random.nextDouble(),
        //fy+par5Random.nextDouble(), fz+par5Random.nextDouble(), cobb);
    }

    public boolean hasFurnace() {
        return furnaceLocation != null
            && furnaceLocation.getTileEntity(worldObj) instanceof TileEntityFurnace;
    }
    ...
    public static boolean isHijacked(TileEntityFurnace furn) {
        for (int i = 2; i < 6; i++) {
            ...
            MachineRegistry m
                = MachineRegistry.getMachine(furn.worldObj, dx, furn.yCoord, dz);
            if (m == MachineRegistry.FRICTION) {
                TileEntityFurnaceHeater te = (TileEntityFurnaceHeater
                ) furn.worldObj.getTileEntity(dx, furn.yCoord, dz);
