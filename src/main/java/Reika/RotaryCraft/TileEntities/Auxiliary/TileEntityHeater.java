...
public class TileEntityHeater
    extends InventoriedPowerReceiver implements TemperatureTE, DiscreteFunction {
    ...
    private void addHeat() {
        ...
        if (temperature >= MAXTEMP * 0.9) {
            RotaryCraft.logger.warn(
                "WARNING: " + this + " is reaching a very high temperature!"
            );
        }
    ...
    public void overheat(World world, int x, int y, int z) {
        ReikaWorldHelper.overheat(
            world, x, y, z, ItemStacks.scrap.copy(), 0, 17, true, 1.2F, true, true, 1F
        );
    ...
    private int findHottestUsefulItemTemp(int maxT, boolean consume) {
        ...
        if (slot != -1 && consume) {
            ...
            if (id == Items.lava_bucket) {
                int leftover = ReikaInventoryHelper.addToInventoryWithLeftover(
                    Items.bucket, -1, 1, inv
                );
                if (leftover > 0) {
                    EntityItem ei = new EntityItem(
                        ...
                        new ItemStack(Items.lava_bucket, leftover, 0)
                    );
