...
public class TileEntityMirror extends RotaryCraftTileEntity implements SolarPlantBlock {
    //2.3 kW/m^2 (392MW/170000) -> 2kW/block; sunlight is 15 kW per m^2, so thus
    //efficiency of 13%
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (world.isRemote
            && (this.getTicksExisted() < 400 || rotatingLarge
                || Math.abs(world.getTotalWorldTime() % 8)
                    == Math.abs(System.identityHashCode(this) % 8))) {
            this.adjustAim(world, x, y, z, meta);
        }

        if (!world.isRemote) {
            AxisAlignedBB above = AxisAlignedBB.getBoundingBox(
                x + 0.25, y + 1, z + 0.25, x + 0.75, y + 1.5, z + 0.75
            );
    ...
    public boolean isFunctional() {
        ...
        //	return false;
        return plant != null
            && /*plant.canSeeTheSky(worldObj, xCoord, yCoord, zCoord)*/ plant
                   .canSeeTheSky(this);
    }
    ...
    private void adjustAim(World world, int x, int y, int z, int meta) {
        ...
        double[] angs = ReikaPhysicsHelper.cartesianToPolar(
            x - target.xCoord, y - target.yCoord, z - target.zCoord
        );
        ...
        //ReikaJavaLibrary.pConsole(targetphi+" clamped to "+this.clampPhi(targetphi,
        //time)+"  :  "+bool);
        if (bool)
        ...
        if (time >= 6000) {
            ...
            finalphi = sunphi
                + (targetphi - sunphi) / 2F; //These are mathematically equivalent...
        }

        float sunangle = time >= 6000
            ? (float) (1 - Math.cos(Math.toRadians((time - 6000) * 90D / 6000D)))
            : (float) Math.cos(Math.toRadians(time * 90D / 6000D));
        ...
        //ReikaJavaLibrary.pConsole(String.format("TIME: %d     SUN: %.3f    TARGET: %.3f
        //FINAL: %.3f", time, sunphi, targetphi, finalphi));
        ...
        float aim = (float) Math.max(
            0, 1 - ReikaMathLibrary.py3d(theta - targetTheta, 0, phi - targetPhi) / 20D
        );
        if (Math.abs(aimFactor - aim) > 0.05) {
            lastAimFactor = aimFactor;
            aimFactor = aim;
            ReikaPacketHelper.sendSyncPacket(
                RotaryCraft.packetChannel, this, "aimFactor", true
            );
        }
    ...
    public void breakMirror(World world, int x, int y, int z) {
        ...
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT) {
            ReikaRenderHelper.addModelledBlockParticles(
                "/Reika/RotaryCraft/Textures/TileEntityTex/",
                world,
                x,
                y,
                z,
                this.getTile().getBlock(),
    ...
    private float adjustPhiForClosestPath(float finalphi) {
        //ReikaJavaLibrary.pConsole(String.format("PHI: %.3f    TARGET: %.3f", phi,
        //finalphi));
