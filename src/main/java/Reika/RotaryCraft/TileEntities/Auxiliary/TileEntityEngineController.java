...
public class TileEntityEngineController
    extends RotaryCraftTileEntity implements PipeConnector, IFluidHandler, IBundledTile {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (redstoneMode && !world.isRemote) {
            ...
            if (ModList.PROJRED.isLoaded() && signalColor != null) {
                signal |= this.getBundledInput(world, x, y, z)[15 - signalColor.ordinal()]
                    > 127;
            }
            setting
                = signal ? EngineSettings.SHUTDOWN : EngineSettings.list[4 - power / 3];
        }
        ...
        if (MachineRegistry.getMachine(world, x, y + 1, z) == MachineRegistry.ENGINE)
            if (this.transferToEngine(
                    (TileEntityEngine) world.getTileEntity(x, y + 1, z), false
                ))
                return;
        if (MachineRegistry.getMachine(world, x, y + 1, z) == MachineRegistry.FUELENGINE)
            if (this.transferToFuelEngine(
                    (TileEntityFuelEngine) world.getTileEntity(x, y + 1, z), false
                ))
                return;

        if (MachineRegistry.getMachine(world, x, y - 1, z) == MachineRegistry.ENGINE)
            if (this.transferToEngine(
                    (TileEntityEngine) world.getTileEntity(x, y - 1, z), true
                ))
                return;
        if (MachineRegistry.getMachine(world, x, y - 1, z) == MachineRegistry.FUELENGINE)
            if (this.transferToFuelEngine(
                    (TileEntityFuelEngine) world.getTileEntity(x, y - 1, z), true
                ))
                return;
    }
    ...
    private boolean transferToEngine(TileEntityEngine te, boolean flip) {
        ...
        if (TileEntityEngine.isAirFluid(liq.getFluid())) {
            ...
            int added
                = te.fill(flip ? ForgeDirection.UP : ForgeDirection.DOWN, move, true);
    ...
    public static String getSettingsAsString() {
        ...
        for (int i = 0; i < EngineSettings.list.length; i++) {
            ...
            sb.append(String.format(
                "%s: %.2f%% Speed, %dx Fuel Efficiency",
                ReikaStringParser.capFirstChar(set.name()),
                set.getSpeedDecimal(),
                set.getEfficiencyFactor()
            ));
    ...
    public boolean canFill(ForgeDirection from, Fluid fluid) {
        ...
        if (te instanceof TileEntityEngine) {
            ...
            return eng.getEngineType() != EngineType.STEAM
                && eng.getEngineType().burnsFuel()
                && fluid.equals(eng.getEngineType().getFuelType());
    ...
    private int getBundledInput(
        World world, int x, int y, int z, ForgeDirection dir, ReikaDyeHelper color
    ) {
        byte[] data = ProjectRedAPI.transmissionAPI.getBundledInput(
            world, x, y, z, dir.ordinal()
        );
