...
public class TileEntityACEngine extends TileEntityEngine implements MagnetizationCore {
    ...
    playSounds(World world, int x, int y, int z, float pitchMultiplier, float volume) {
        ...
        SoundRegistry.ELECTRIC.playSoundAtBlock(
            world, x, y, z, 0.125F * volume, 1F * pitchMultiplier
        );
    }

    public void magneticInterference(int mag, double dd) {
        torque = (int
        ) (0.0625 * ReikaMathLibrary.logbase(mag, 2) * this.getEngineType().getTorque()
           / dd);
        omega = (int
        ) (0.0625 * ReikaMathLibrary.logbase(mag, 2) * this.getEngineType().getSpeed()
           / dd / 4D);
    ...
    public int getCoreMagnetization() {
        return inv[0] != null && inv[0].stackTagCompound != null
            ? inv[0].stackTagCompound.getInteger("magnet")
            : 0;
    }
