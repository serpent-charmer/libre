...
public class TileEntityHydroEngine extends TileEntityEngine {
    ...
    protected boolean getRequirements(World world, int x, int y, int z, int meta) {
        boolean hasLube = !lubricant.isEmpty()
            && lubricant.getActualFluid().equals(FluidRegistry.getFluid("rc lubricant"));
        if (hasLube)
        ...
        for (int d = -1; d <= 1; d++) {
            for (int i = -1; i <= 1; i++) {
                if (i == 0 && d == 0)
                    continue;
                if (this.doesBlockObstructBlades(
                        world, x + (pos[0] - x) * d, y + i, z + (pos[1] - z) * d
                    )) {
                    omega = 0;
                    return false;
                }
        ...
        //ReikaJavaLibrary.pConsole(Block.getIdFromBlock(world.getBlock(x, y-1,
        //z))+":"+world.getBlockMetadata(x, y, z)+" > "+world.getBlock(x, y-1, z));
            ...
            if (fluidType != null) {
                if (fluidType.getTemperature() >= 900) {
                    if (ReikaRandomHelper.doWithChance(2)) {
                        ...
                        world.newExplosion(
                            null, x + 0.5, y + 0.5, z + 0.5, lube ? 3 : 2, lube, true
                        );
                    }
                }
    ...
    private boolean isStreamFall(World world, int dx, int dy, int dz) {
        while (world.getBlock(dx, dy, dz) == Blocks.flowing_water
               || world.getBlock(dx, dy, dz) == Blocks.water) {
            dy--;
        }
    ...
    private void checkForOtherStreamEngines(World world, int x, int y, int z, int meta) {
        ...
        for (int d = 1; d < 4; d++) {
            if (continueForward) {
                ...
                if (MachineRegistry.getMachine(world, dx, y, dz)
                    == MachineRegistry.ENGINE) {
                    ...
                    if (te instanceof TileEntityHydroEngine) {
                        ...
                        if (th.streamData != null
                            && th.streamData.omega > streamData.omega) {
    ...
    private void distributeLubricant(World world, int x, int y, int z) {
        for (int i = 2; i < 6; i++) {
            ...
            if (dir == this.getWriteDirection()
                || dir.getOpposite() == this.getWriteDirection()) {
                int dx = x + dir.offsetX;
                int dy = y + dir.offsetY;
                int dz = z + dir.offsetZ;
                MachineRegistry m = MachineRegistry.getMachine(world, dx, dy, dz);
                if (m == MachineRegistry.ENGINE) {
                    TileEntityEngine eng
                        = (TileEntityEngine) this.getAdjacentTileEntity(dir);
                    if (eng instanceof TileEntityHydroEngine) {
                        ...
                        if (dL > 3) {
                            hy.lubricant.addLiquid(
                                dL / 4, FluidRegistry.getFluid("rc lubricant")
                            );
                    ...
                    TileEntityReservoir te
                        = (TileEntityReservoir) this.getAdjacentTileEntity(dir);
                    if (!lubricant.isEmpty()
                        && te.canAcceptFluid(FluidRegistry.getFluid("rc lubricant"))) {
    ...
    private void getFluidData(World world, int x, int y, int z, int meta, int[] pos) {
        ...
        fluidFallSpeed = 0.92 * Math.sqrt(2 * grav * dy)
            / Math.max(0.25, Math.pow(f.getViscosity() / 1000, 0.375));
    }
    ...
    public int getWaterfallHeightForDisplay() {
        ...
        return MathHelper.ceiling_double_int(
                   ReikaWorldHelper.findFluidSurface(worldObj, pos[0], yCoord, pos[1])
               )
            - yCoord;
    }
    ...
    private int getEffectiveTorque(World world, int x, int y, int z) {
        ...
        double mdot = Math.min(12000, fluidType.getDensity())
            * fluidFallSpeed; //*1 since area is 1m^2
    ...
    private void dealPanelDamage(World world, int x, int y, int z, int meta) {
        ...
        if (meta < 2)
            ...
            a = 1;
        AxisAlignedBB box
            = AxisAlignedBB.getBoundingBox(x, y, z, x + 1, y + 1, z + 1).expand(a, 1, b);
    ...
    public boolean isBackEndOfArray() {
        ...
        if (to == MachineRegistry.ENGINE) {
            ...
            return te.getEngineType() == EngineType.HYDRO
                && !((TileEntityHydroEngine) te).failed;
        }
        return false;
    }
    ...
    public boolean isFrontOfArray() {
        ...
        if (from == MachineRegistry.ENGINE && to != MachineRegistry.ENGINE) {
            TileEntityEngine te
                = (TileEntityEngine) worldObj.getTileEntity(backx, yCoord, backz);
            return te.getEngineType() == EngineType.HYDRO;
        }
    ...
    private int getArrayTorqueMultiplier() {
        ...
        while (te instanceof TileEntityHydroEngine && te != this && !li.contains(te)) {
            ...
            li.add(eng);
            if (eng.getRequirements(
                    worldObj, eng.xCoord, eng.yCoord, eng.zCoord, eng.getBlockMetadata()
                )) {
                if (eng.omega == omega && !eng.failed) {
                    //float fac = eng.getHydroFactor(worldObj, xyz[0], xyz[1], xyz[2],
                    //true);
                    size++;
                    te = eng.getAdjacentTileEntity(eng.write.getOpposite());
                } else {
                    ReikaParticleHelper.CRITICAL.spawnAroundBlock(
                        worldObj, eng.xCoord, eng.yCoord, eng.zCoord, 5
                    );
                    if (rand.nextInt(3) == 0)
                        ReikaSoundHelper.playSoundAtBlock(
                            worldObj, eng.xCoord, eng.yCoord, eng.zCoord, "mob.blaze.hit"
                        );
                    break;
                }
            }
    ...
    playSounds(World world, int x, int y, int z, float pitchMultiplier, float volume) {
        ...
        if (this.isFrontOfArray() || !this.isPartOfArray())
            SoundRegistry.HYDRO.playSoundAtBlock(
                world, x, y, z, 1F * volume, 0.9F * pitchMultiplier
            );
    }
    ...
    protected int getGenTorque(World world, int x, int y, int z, int meta) {
        if (failed)
            return 1;
        int torque
            = this.getEffectiveTorque(world, x, y, z) * this.getArrayTorqueMultiplier();
    ...
    private void fail(World world, int x, int y, int z) {
        ...
        while (te instanceof TileEntityHydroEngine) {
            TileEntityHydroEngine eng = (TileEntityHydroEngine)te;
            eng.getGenTorque(world, eng.xCoord, eng.yCoord, eng.zCoord,
        eng.getBlockMetadata()); te = eng.getAdjacentTileEntity(write.getOpposite());
        }*/
        failed = true;
    }
        ...
        if (failed) {
            ...
            for (int a = -1; a <= 1; a++) {
                for (int h = -1; h <= 1; h++) {
                    ...
                    if (!b.isAir(world, dx, dy, dz) && !ReikaBlockHelper.isLiquid(b)
                        && !InterfaceCache.STREAM.instanceOf(b))
                        ReikaWorldHelper.dropAndDestroyBlockAt(
                            world, dx, dy, dz, null, false, true
                        );
                }
            }
        }
    ...
    private void spawnParticles(World world, int x, int y, int z) {
        ...
        ReikaParticleHelper.RAIN.spawnAroundBlock(
            world, x - (xz[0] - x), y, z - (xz[1] - z), 16
        );
        if (this.isStreamPowered()) {
            ReikaParticleHelper.RAIN.spawnAroundBlock(
                world, x + (xz[0] - x), y, z + (xz[1] - z), 16
        ...
        if (failed) {
            ...
            ReikaParticleHelper.CRITICAL.spawnAroundBlockWithOutset(
                world, x, y, z, 3, 0.25
            );
        }
    }
    ...
    private static class StreamPowerData {
        ...
        private StreamPowerData(
            TileEntityHydroEngine te, int wx, int wy, int wz, double vel, int dX, int dZ
        ) {
            this(new Coordinate(te), new Coordinate(wx, wy, wz), vel, dX, dZ);
        }

        private StreamPowerData(
            Coordinate c1, Coordinate c2, double vel, int dX, int dZ
        ) {
            engineSource = c1;
            waterSource = c2;
            normalVelocity = vel;
        ...
        private void calculateGeography(World world, ForgeDirection front) {
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (dir != ForgeDirection.UP) {
                    for (int d = 1; d < (dir.offsetY == 0 ? localWidth : localDepth);
                         d++) {
        ...
        private boolean valid(World world) {
            return gameInstance == startTime
                && engineSource.getTileEntity(world) instanceof TileEntityHydroEngine
                && waterSource.getBlock(world) instanceof FixedFlowBlock;
        }

        private double getStreamSpeedScale() {
            return Math.sqrt(
                streamDepthFactor.getValue(localDepth + 1)
                * streamWidthFactor.getValue(localWidth + 1) * normalVelocity
            );
        }
        ...
        private static StreamPowerData readFromNBT(NBTTagCompound tag) {
            ...
            StreamPowerData ret = new StreamPowerData(
                eng, water, vel, tag.getInteger("flx"), tag.getInteger("flz")
            );
        ...
        public ArrayList<String> getDataDisplay() {
            ...
            li.add(
                "Local Flow Velocity: " + String.format("%.0f", normalVelocity * 100)
                + "%%"
            );
            li.add(
                "Efficiency: " + String.format("%.2f", powerFactor * powerFactor * 100)
                + "%%"
            );
            return li;
        }
