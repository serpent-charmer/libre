...
public class TileEntityGasEngine extends TileEntityEngine implements UpgradeableMachine {
    ...
    public void upgrade(ItemStack is) {
        ...
        worldObj.setBlock(
            xCoord, yCoord, zCoord, this.getTileEntityBlockID(), type.ordinal(), 3
        );
        TileEntityEngine te
            = (TileEntityEngine) worldObj.getTileEntity(xCoord, yCoord, zCoord);
        te.readFromNBT(NBT);
    ...
    public boolean canUpgradeWith(ItemStack item) {
        return item.getItem() == ItemRegistry.UPGRADE.getItemInstance()
            && item.getItemDamage() == Upgrades.PERFORMANCE.ordinal();
    }
    ...
    protected void internalizeFuel() {
        if (inv[0] != null
            && fuel.getLevel() + FluidContainerRegistry.BUCKET_VOLUME <= FUELCAP) {
    ...
    playSounds(World world, int x, int y, int z, float pitchMultiplier, float volume) {
        ...
        SoundRegistry.CAR.playSoundAtBlock(
            world, x, y, z, 0.33F * volume, 0.9F * pitchMultiplier
        );
    }
