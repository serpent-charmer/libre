...
public class TileEntityWindEngine extends TileEntityEngine {
    ...
    private void dealBladeDamage(World world, int x, int y, int z, int meta) {
        ...
        AxisAlignedBB box
            = AxisAlignedBB.getBoundingBox(x + c, y, z + d, x + 1 + c, y + 1, z + 1 + d)
                  .expand(a, 1, b);
    ...
    private float getWindFactor(World world, int x, int y, int z, int meta) {
        ...
        (IGalacticraftWorldProvider)world.provider; f *= ig.getWindLevel();
            }
            if (f > 1)
                f = 1;
            return f;
    ...
    playSounds(World world, int x, int y, int z, float pitchMultiplier, float volume) {
        ...
        SoundRegistry.WIND.playSoundAtBlock(
            world, x, y, z, 1.1F * volume, 1F * pitchMultiplier
        );
    }
    ...
    protected int getMaxSpeed(World world, int x, int y, int z, int meta) {
        return (int
        ) (EngineType.WIND.getSpeed() * this.getWindFactor(world, x, y, z, meta));
    }
    ...
    private static class WindClearanceCheck {
        ...
        private void scanRow(World world, int step) {
            ...
            //ReikaJavaLibrary.pConsole("Scanning row "+step+", center coord is
            //"+engine.offset(direction, step));
            ...
            for (int i = -r; i <= r; i++) {
                ...
                //blockLocations[step-1][i+r] = !ReikaWorldHelper.softBlocks(world, dx,
                //dy, dz);
