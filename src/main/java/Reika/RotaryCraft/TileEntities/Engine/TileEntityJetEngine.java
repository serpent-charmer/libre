...
public class TileEntityJetEngine
    extends TileEntityEngine implements NBTMachine, UpgradeableMachine {
    ...
    public float getChokedFraction(World world, int x, int y, int z, int meta) {
        ...
        if (b == Blocks.glass_pane)
            return 0.5F;
        ;
        if (b.getBlockBoundsMaxX() > 0.875 && b.getBlockBoundsMaxY() > 0.875
            && b.getBlockBoundsMaxZ() > 0.875)
            if (b.getBlockBoundsMinX() < 0.125 && b.getBlockBoundsMinY() < 0.125
                && b.getBlockBoundsMinZ() < 0.125)
                return 0;
        double frac;
    ...
    private void heatJet(World world, int x, int y, int z, int meta) {
        if (this.isOn() && this.getTicksExisted() % 10 == 0) {
            ...
            if (max > temperature)
                temperature
                    = Math.min(temperature + Math.max(1, (max - temperature) / 16), max);
            else if (!this.isAfterburning()) {
                temperature
                    = Math.max(temperature - Math.max(1, (temperature - max) / 32), max);
            }
        }
        int T = temperature;
        ...
        for (int i = 1; i < r; i++) {
            ...
            if (this.isAfterburning()) {
                ReikaWorldHelper.temperatureEnvironment(
                    world, dx, y, dz, Math.min(1400, T)
                );
            }
        }
    ...
    /**
     * Like BC obsidian pipe - suck in entities in a "funnel" in front of the engine, and
     * deal damage (50 hearts). Items (including players' inventories and mob drops) will
     * be spat out the back. Large mobs (Player, creeper, spider, ghast, etc) will cause
     * foreign object damage, necessitating repair.
     */
        ...
        for (int step = 0; step < 8; step++) {
            ...
            for (Entity caught : inzone) {
                if (this.canSuckTowards(world, x, y, z, caught, px, pz)) {
                    ...
                    if (ReikaMathLibrary.py3d(
                            caught.posX - px, caught.posY - (y + 0.5), caught.posZ - pz
                        )
                        < 1.2) {
    ...
    private boolean
    canSuckTowards(World world, int x, int y, int z, Entity e, double px, double pz) {
        int n = 2;
        for (int i = 0; i <= n; i++) {
            tracer.setOrigins(px, y + 0.5, pz, e.posX, e.posY + e.height * i / n, e.posZ);
            //ReikaJavaLibrary.pConsole(e+":"+tracer.isClearLineOfSight(world)+" of
            //"+e.height, e instanceof EntityChicken);
    ...
    private void ingest(
        World world, int x, int y, int z, Entity e, boolean immune
    ) { // Kill the adjacent entities, except items, which are teleported
        if (e instanceof EntityItem) {
            if (!e.isDead) {
                ...
                int trycount = 0;
                while (trycount < 1
                       && !ReikaWorldHelper.nonSolidBlocks(world, dumpx, y, dumpz)) {
                    if (dumpvx == 1)
                        dumpx++;
                    if (dumpvx == -1)
                        dumpx--;
                ...
                EntityItem item
                    = new EntityItem(world, dumpx + 0.5D, y + 0.375D, dumpz + 0.5D, is);
                if (!world.isRemote)
            ...
            if (!e.isDead) {
                ...
                EntityXPOrb item
                    = new EntityXPOrb(world, dumpx + 0.5D, y + 0.375D, dumpz + 0.5D, xp);
                if (!world.isRemote)
                    world.spawnEntityInWorld(item);
                item.motionX = dumpvx * 1.5D;
                item.motionY = 0.15;
            ...
            e.setFire(2);
            if (!world.isRemote && ((EntityLivingBase) e).getHealth() > 0
                && this.canDamageEngine(e))
                this.damageEngine();
            if (FOD > 8)
                FOD = 8;
            if (e instanceof EntityChicken && !e.isDead
                && ((EntityChicken) e).getHealth() > 0) {
                chickenCount++;
                if (chickenCount >= 50) {
    ...
    private void triggerJetFailing(World world, int x, int y, int z) {
        ...
        AxisAlignedBB box = ReikaAABBHelper.getBlockAABB(x, y, z)
                                .addCoord(write.offsetX * 8, y, write.offsetZ * 8)
                                .expand(3, 3, 3);
        List<EntityLivingBase> li
            = world.getEntitiesWithinAABB(EntityLivingBase.class, box);
    ...
    private boolean itemDestroysEngine(ItemStack is) {
        return is.getItem() == ItemRegistry.SCREWDRIVER.getItemInstance()
            || InterfaceCache.IWRENCH.instanceOf(is.getItem());
    }
    ...
    getSuctionZone(World world, int x, int y, int z, int meta, int step) {
        int minx = 0;
        int miny = 0;
        int minz = 0;
        int maxx = 0;
    ...
    public void repairJet() {
        ...
        temperature = Math.max(
            ReikaWorldHelper.getAmbientTemperatureAt(worldObj, xCoord, yCoord, zCoord),
            temperature / 2
        );
    }
    ...
    private void jetEngineDetonation(World world, int x, int y, int z, int meta) {
        ...
        List<EntityLivingBase> in
            = world.getEntitiesWithinAABB(EntityLivingBase.class, zone);
        ...
        for (int i = 0; i < 16; i++) {
            ...
            world.spawnParticle(
                part,
                x + 0.25 + 0.5 * rand.nextDouble(),
                y + 0.25 + 0.5 * rand.nextDouble(),
                z + 0.25 + 0.5 * rand.nextDouble(),
                vx - 0.1 + 0.2 * rand.nextDouble(),
                -0.1 + 0.2 * rand.nextDouble(),
                vz - 0.1 + 0.2 * rand.nextDouble()
            );
        }
        int dx = x - backx;
        int dz = z - backz;
        ...
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER) {
            if (fuel.getLevel() < FUELCAP / 12 && rand.nextInt(10) == 0) {
                ReikaPacketHelper.sendDataPacketWithRadius(
                    ...
                    PacketRegistry.ENGINEBACKFIRE.ordinal(),
                    this,
                    64
                );
    ...
    public void backFire(World world, int x, int y, int z) {
        ...
        world.createExplosion(
            null, x + 0.5, y + 0.5, z + 0.5, 2 * rand.nextFloat(), false
        );
    ...
    private void launchEntities(World world, int x, int y, int z) {
        AxisAlignedBB box
            = AxisAlignedBB.getBoundingBox(x, y, z, x + 1, y + 1, z + 1).expand(8, 8, 8);
    ...
    playSounds(World world, int x, int y, int z, float pitchMultiplier, float volume) {
        ...
        if (FOD > 0 && rand.nextInt(2 * (9 - FOD)) == 0) {
            world.playSoundEffect(
                x + 0.5, y + 0.5, z + 0.5, "mob.blaze.hit", 1F + rand.nextFloat(), 1F
            );
            world.spawnParticle(
                "crit",
                x + rand.nextFloat(),
        ...
        if (jetstarttimer.getTick() >= jetstarttimer.getCap())
            SoundRegistry.JET.playSoundAtBlock(
    ...
    public int getMaxSpeed(World world, int x, int y, int z, int meta) {
        return (int
        ) (EngineType.JET.getSpeed() * this.getChokedFraction(world, x, y, z, meta));
    }
            ...
            if (worldObj.isRemote) {
                ...
                if (f.getName().toLowerCase(Locale.ENGLISH).contains("fuel")) {
                    ...
                    if (temp >= 2000 || rand.nextInt(1 + (2000 - temp) / 500) == 0) {
                        if (FOD < 8 && world.getTotalWorldTime() % 20 == 0
                            && rand.nextInt(1 + 2 * FOD) == 0) {
                            this.damageEngine();
                        }
                    }
                }
    ...
    private void fluidIngestParticles(World world, int x, int y, int z, Fluid f) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityFX fx = new EntityLiquidParticleFX(
                world,
                x + 0.5 + write.offsetX * 0.25,
                y + 0.5,
                z + 0.5 + write.offsetZ * 0.25,
                vx,
                vy,
                vz,
                f
    ...
    private void afterBurnParticles(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            ReikaParticleHelper.spawnColoredParticleAt(
                world, px, y + 0.75 * rand.nextDouble(), pz, r, g, b
            );
        }
    }
    ...
    private void spawnSmokeParticles(World world, int x, int y, int z, int meta) {
        ...
        double vx = -(x - backx) / 2D;
        double vz = -(z - backz) / 2D;
        ReikaParticleHelper.SMOKE.spawnAt(
            world,
            dx + x + 0.25 + 0.5 * rand.nextDouble(),
            y + 0.5 * rand.nextDouble(),
            dz + z + 0.25 + 0.5 * rand.nextDouble(),
            -vx - 0.1 + 0.2 * rand.nextDouble(),
            -0.1 + 0.2 * rand.nextDouble(),
            -vz - 0.1 + 0.2 * rand.nextDouble()
        );
        ...
        int n = 1 + rand.nextInt(8);
        double w = n / 2D;
        dx = write.offsetX == 0 ? ReikaRandomHelper.getRandomPlusMinus(x + 0.5, w)
                                : x + 0.5 - n * write.offsetX;
        double dy = ReikaRandomHelper.getRandomPlusMinus(y + 0.5, w);
        dz = write.offsetZ == 0 ? ReikaRandomHelper.getRandomPlusMinus(z + 0.5, w)
                                : z + 0.5 - n * write.offsetZ;
        ...
        if (b.isAir(world, bx, by, bz)) {
            ...
            EntityBlockTexFX fx
                = new EntityBlockTexFX(
                      world, dx, dy + 1, dz, vx, vy - 0.03125, vz, b, bmeta
                )
                      .setGravity(0);
    ...
    public boolean canUpgradeWith(ItemStack item) {
        return !canAfterBurn && ItemRegistry.UPGRADE.matchItem(item)
            && item.getItemDamage() == Upgrades.AFTERBURNER.ordinal();
    }
    ...
    public void setBurnerActive(boolean burn) {
        ...
        if (!burn && !isJetFailing) {
            temperature = ReikaWorldHelper.getAmbientTemperatureAt(worldObj, xCoord,
        yCoord, zCoord);
        }*/
    }
    ...
    public void breakBlock() {
        ...
        if (canAfterBurn) {
            ReikaItemHelper.dropItem(
                ...
                yCoord + 0.5,
                zCoord + 0.5,
                ItemRegistry.UPGRADE.getStackOfMetadata(Upgrades.AFTERBURNER.ordinal())
            );
        }
    }
