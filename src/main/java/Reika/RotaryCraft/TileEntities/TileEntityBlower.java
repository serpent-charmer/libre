...
public class TileEntityBlower extends TileEntityPowerReceiver {
        ...
        if (ModList.APPENG.isLoaded()) {
            try {
                ...
                RotaryCraft.logger.logError(
                    "Could not add Item Pump AE pattern interfacing!"
                );
                e.printStackTrace();
        ...
        if (ReikaInventoryHelper.isAutomatableInventory(source)) {
            ...
            if (target != null) {
                ...
                while (target instanceof TileEntityBlower) {
                    ...
                    if (this.equals(target)) { //to prevent stackoverflow from loops,
                                               //because some idiot is going to try
                        return;
                    }
                    if (source.equals(target)) { //to prevent dupe glitch, and because
                                                 //this would be stupid to do
                        return;
                    }
                }
    ...
    private void printTEs(TileEntity source, TileEntity target) {
        if (source == null && target == null)
            ...
            ReikaJavaLibrary.pConsole(
                "null >> " + target.getClass().getSimpleName(), Side.SERVER
            );
        else if (target == null)
            ReikaJavaLibrary.pConsole(
                source.getClass().getSimpleName() + " >> null", Side.SERVER
            );
        else
            ReikaJavaLibrary.pConsole(
    ...
    private void dumpItems(
        TileEntity source, InventoryType src, WorldLocation loc, ForgeDirection from
    ) {
        int max = this.getNumberTransferrableItems();
        if (max <= 0)
            return;
    ...
    private void dropItem(ItemStack is, int amt, WorldLocation loc) {
        for (int i = 0; i < amt; i++)
            loc.dropItem(
                ReikaItemHelper.getSizedItemStack(is, 1), 2 + rand.nextDouble() * 4
            );
    }
    ...
    public boolean isIntake() {
        return this.getAdjacentTileEntity(this.getFacingDir().getOpposite())
    ...
    public static enum InventoryType {
        ...
        public HashMap<Integer, ItemStack>
        getMovableSlots(ForgeDirection dir, TileEntity te) {
            switch (this) {
                case CHEST:
                    return ReikaInventoryHelper.getLocatedTransferrables(
                        dir, (IInventory) te
                    );
                case DSU:
                    return null; //ReikaJavaLibrary.makeMapOf(-1,
                                 //((IDeepStorageUnit)te).getStoredItemType());
            }
            return null;
        }
            ...
            switch (this) {
                case CHEST:
                    ...
                    if (remove) {
                        ...
                        return ii.getStackInSlot(slot) != null
                            ? Math.min(ii.getStackInSlot(slot).stackSize, amt)
                            : 0;
                    }
                case DSU:
        ...
        public int insertItem(TileEntity te, ItemStack is, int amt, ForgeDirection dir) {
            switch (this) {
                case CHEST:
                    ...
                    while (flag && amt > 0) {
                        //ReikaJavaLibrary.pConsole("Attempting to add "+is+" to
                        //"+target);
        ...
        private boolean addToIInv(ItemStack is, IInventory ii, ForgeDirection dir) {
            if (ii instanceof ISidedInventory) {
                for (int k = 0; k < ii.getSizeInventory(); k++) {
                    if (((ISidedInventory) ii)
                            .canInsertItem(k, is, dir.getOpposite().ordinal())) {
