...
public class TileEntityFertilizer extends InventoriedPowerLiquidReceiver
    implements RangedEffect, ConditionalOperation, EnchantableMachine {
    ...
    private final MachineEnchantmentHandler enchantments
        = new MachineEnchantmentHandler()
              ...
              .addFilter(Enchantment.aquaAffinity)
              .addFilter(Enchantment.fortune)
              .addFilter(Enchantment.unbreaking)
              .addFilter(Enchantment.power);
    ...
    private int getUpdatesPerTick() {
        ...
        return 4 * ReikaMathLibrary.logbase2(omega)
            + enchantments.getEnchantment(Enchantment.efficiency) * 3;
    }
        ...
        if (id != Blocks.air && dd <= this.getRange()
            ...
            if (this.didSomething(world, dx, dy, dz)) {
                ReikaPacketHelper.sendUpdatePacket(
                    ...
                    PacketRegistry.FERTILIZER.ordinal(),
                    dx,
                    dy,
                    dz,
                    new PacketTarget.RadiusTarget(world, dx, dy, dz, 32)
                );
    ...
    private boolean canTick(World world, int dx, int dy, int dz) {
        ...
        return !(
            b instanceof BlockRedstoneDiode || b instanceof BlockRedstoneComparator
    ...
    private void consumeItem() {
        tank.removeLiquid(
            Math.max(1, 5 - enchantments.getEnchantment(Enchantment.aquaAffinity) / 2)
        );
    ...
    public int getRange() {
        ...
        int r = 2 * (int) ReikaMathLibrary.logbase(torque, 2)
            + enchantments.getEnchantment(Enchantment.power);
