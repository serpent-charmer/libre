...
public class TileEntitySprinkler extends SprinklerBlock {
    ...
    private void clearRadiation(World world, int x, int y, int z) {
        int r = this.getRange();
        AxisAlignedBB box
            = ReikaAABBHelper.getBlockAABB(x, y, z).offset(0, -4, 0).expand(r, 4, r);
        List<EntityRadiation> li
            = world.getEntitiesWithinAABB(EntityRadiation.class, box);
    ...
    public void hydrate(World world, int x, int y, int z) {
        ...
        for (int i = -range; i <= range; i++) {
            for (int k = -range; k <= range; k++) {
                ...
                float f = 0.5F
                    + 1.5F * (i * i + k * k)
                        / (range * range); //2x rate at center, 0.5x rate at edge
    ...
    private void sendParticle(int dx, int dy, int dz, boolean drip) {
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            PacketRegistry.SPRINKLER.ordinal(),
            this,
            48,
            dx,
            dy,
    ...
    public void doParticle(World world, int dx, int dy, int dz, boolean drip) {
        ...
        if (rand.nextInt(d) == 0)
            ReikaParticleHelper.RAIN.spawnAt(
                world, dx + rand.nextDouble(), dy + 1, dz + rand.nextDouble()
            );
        if (drip) {
            ReikaParticleHelper.DRIPWATER.spawnAt(
                world, dx + rand.nextDouble(), dy, dz + rand.nextDouble()
            );
        }
    }
    ...
    PacketIDs.PARTICLEWITHPOS.ordinal(), new PacketTarget.RadiusTarget(this, 32), li);
    }*/
    ...
    private static enum Effects {
        ...
        private boolean doChance(Random rand, float factor) {
            return randomChance <= 0
                || rand.nextInt(Math.max(1, (int) (randomChance * factor))) == 0;
        }

        private void doEffect(World world, int x, int y, int z, Block b) {
            switch (this) {
                case WETTILE:
                    MachineRegistry m = MachineRegistry.getMachineFromIDandMetadata(
                    ...
                    if (b.getMaterial() == Material.leaves) {
                        if (ModularLogger.instance.isEnabled(LOGGER_ID))
                            ModularLogger.instance.log(
                                LOGGER_ID, "Found leaf @ " + x + ", " + y + ", " + z
                            );
                        Block b2 = world.getBlock(x, y - 1, z);
                        if (b2.isAir(
                                world, x, y - 1, z
                            )) { //space for an apple; only stop moving down if this
                            BlockTickEvent.fire(
                                b,
                                world,
                                x,
                    ...
                    if (crop != null) {
                        ...
                        if (p != null && p.grows()) {
                            ...
                            if (b instanceof BlockSapling
                                || ModWoodList.isModSapling(b, meta)
                                || b instanceof Reedlike) {
                                BlockTickEvent.fire(
                                    b,
                                    world,
                                    x,
                            ...
                            if (p != null && p.grows()) {
                                this.addEffectValue(
                                    ...
                                    new ColumnAction(
                                        ...
                                        Effects.CROPTICK,
                                        false,
                                        p == ReikaPlantHelper.SUGARCANE,
                                        true
                                    )
                                );
                                stopMoving = p == ReikaPlantHelper.CACTUS
                                    || p == ReikaPlantHelper.SUGARCANE;
                            } else if (p == null) {
                                if (b instanceof BlockSapling
                                    || ModWoodList.isModSapling(b, meta)) {
                        ...
                        if (b.getMaterial() == Material.leaves
                            ...
                            if (n.startsWith("growthcraft.apples")) {
                                if (b.getMaterial() == Material.leaves
                                    ...
                                    if (isOpaque(b2)) {
                                        stopMoving = true;
                                                   ...
                                                   world, xCoord, dy - 1, zCoord
                                               )) { //space for an apple; only stop moving
                                                    //down if this
                                        this.addEffectValue(
                                            dy,
                                            new ColumnAction(
                                ...
                                if (ModularLogger.instance.isEnabled(LOGGER_ID))
                                    ModularLogger.instance.log(
                                        LOGGER_ID,
                                        "Read GC block '" + n + "', flag2=" + stopMoving
                                    );
                            }
                        }
                    }
            ...
            for (int y = 0; y < effectMap.length; y++) {
                ...
                for (ColumnAction ca : map) {
                    if (ca.doDripParticles && rand.nextInt(8) == 0) {
                        ReikaParticleHelper.DRIPWATER.spawnAt(
                            world,
                            ca.xCoord + rand.nextDouble(),
                            ca.yCoord,
                            ca.zCoord + rand.nextDouble()
                        );
                    }
    ...
    private static class ColumnAction {
        ...
        private static ColumnAction readFromNBT(NBTTagCompound tag) {
            return new ColumnAction(
                Block.getBlockById(tag.getInteger("block")),
                tag.getInteger("x"),
                tag.getInteger("y"),
                tag.getInteger("z"),
                Effects.list[tag.getInteger("effect")],
                tag.getBoolean("drip"),
                tag.getBoolean("drip2"),
                tag.getBoolean("splash")
            );
        }
    }
}
