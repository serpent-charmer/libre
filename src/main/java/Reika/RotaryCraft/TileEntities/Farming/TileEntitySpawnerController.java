...
public class TileEntitySpawnerController extends TileEntityPowerReceiver
    implements GuiController, DiscreteFunction, ConditionalOperation {
    ...
    private void shutdownSpawner(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            worldObj.spawnParticle(
                ...
                var3,
                var5,
                -0.3 + 0.6 * worldObj.rand.nextFloat(),
                0.4 * worldObj.rand.nextFloat(),
                -0.3 + 0.6 * worldObj.rand.nextFloat()
            );
        }
    }
    ...
    private int getNumberNearbySpawns(World world, int x, int y, int z, Class ent) {
        return world
            .getEntitiesWithinAABB(
                ent, ReikaAABBHelper.getBlockAABB(x, y, z).expand(16, 24, 16)
            )
            .size();
    }
    ...
    public boolean isValidLocation(World world, int x, int y, int z) {
        ...
        return b == Blocks.mob_spawner
            || this.getAdjacentTileEntity(ForgeDirection.DOWN)
                   instanceof ControllableSpawner;
    }
    ...
    private static class SpawnerControl {
        ...
        private int getDelay() {
            return vanilla ? tile.func_145881_a().spawnDelay
                           : proxy.getCurrentSpawnDelay();
        }
        ...
        private void spawnCycle(TileEntitySpawnerController te) {
            ...
            MinecraftForge.EVENT_BUS.post(
                new SpawnerControllerSpawnEvent(te, this.getEntity())
            );
        }

        private Class<? extends EntityLiving> getEntity() {
            return vanilla ? (Class) EntityList.stringToClassMapping.get(
                       tile.func_145881_a().getEntityNameToSpawn()
                   )
                           : proxy.getSpawningEntityClass();
        }
    }
    ...
    private static void
    runVanillaSpawnCycle(TileEntityMobSpawner tile, TileEntitySpawnerController c) {
        ...
        if (world.isRemote) {
            ...
            lgc.field_98287_c
                = (lgc.field_98287_c + 1000D / (lgc.spawnDelay + 200)) % 360;
        } else {
            for (int i = 0; i < lgc.spawnCount; i++) {
                Entity toSpawn
                    = EntityList.createEntityByName(lgc.getEntityNameToSpawn(), world);

                // This is the max-6 code
                //int var4 = world.getEntitiesWithinAABB(var13.getClass(),
                //AxisAlignedBB.getBoundingBox(x, y, z, x+1, y+1,
                //z+1).expand(spawnRange*2, 4, spawnRange*2)).size();
                ...
                if (toSpawn != null) {
                    ...
                    double ez = z
                        + (world.rand.nextDouble() - world.rand.nextDouble())
                            * lgc.spawnRange;
                    EntityLiving livingSpawn
                        = toSpawn instanceof EntityLiving ? (EntityLiving) toSpawn : null;
                    toSpawn.setLocationAndAngles(
                        ex, ey, ez, world.rand.nextFloat() * 360, 0
                    );
