...
public class TileEntityFan extends TileEntityBeamMachine
    implements RangedEffect, UpgradeableMachine, BreakAction {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (AtmosphereHandler.isNoAtmo(
                world,
                x - this.getReadDirection().offsetX,
                y,
                z - this.getReadDirection().offsetZ,
                blockType,
                false
            ))
            return;
        ...
        if (omega > 0) {
            if (sound.checkCap())
                SoundRegistry.FAN.playSoundAtBlock(
                    world, x, y, z, RotaryAux.isMuffled(this) ? 0.05F : 0.5F, 1F
                );
        }
    }
        ...
        if (ReikaWorldHelper.checkForAdjMaterial(world, x, y, z, Material.fire) != null
            ...
            for (int i = 1; i <= range; i++) {
                ...
                if (rand.nextInt(60) == 0
                    && ReikaWorldHelper.softBlocks(world, editx, edity, editz))
                    world.setBlock(editx, edity, editz, Blocks.fire);
                editx = -1 * a + x + i * facing.offsetX;
                edity = y + i * facing.offsetY;
                editz = -1 * b + z + i * facing.offsetZ;
                if (rand.nextInt(60) == 0
                    && ReikaWorldHelper.softBlocks(world, editx, edity, editz))
                    world.setBlock(editx, edity, editz, Blocks.fire);
                editx = -1 * a + x + i * facing.offsetX;
                edity = 1 + y + i * facing.offsetY;
                editz = -1 * b + z + i * facing.offsetZ;
                if (rand.nextInt(60) == 0
                    && ReikaWorldHelper.softBlocks(world, editx, edity, editz))
                    world.setBlock(editx, edity, editz, Blocks.fire);
    ...
    protected void makeBeam(World world, int x, int y, int z, int meta) {
        ...
        AxisAlignedBB zone = wideAreaBlow ? this.getWideBlowZone(meta, range)
                                          : this.getBlowZone(meta, range);
            ...
            if (this.canBlowEntity(caught)) {
                ...
                if (caught.motionX < AXISSPEEDCAP && facing.offsetX != 0) {
                    ...
                    if (multiplier > 1 || multiplier < 0)
                        multiplier = 1;
                    double base
                        = multiplier * power2 * BASESPEED * (wideAreaBlow ? 0.125 : 1);
                    double speedstep = Math.max(
                        Math.abs(Math.abs(caught.motionX) + base / (mass * Math.abs(d))),
                        AXISSPEEDCAP
                    );
                ...
                if (caught.motionY < AXISSPEEDCAP && facing.offsetY != 0) {
                    ...
                    double base
                        = multiplier * power2 * BASESPEED * (wideAreaBlow ? 0.125 : 1);
                    caught.motionY = facing.offsetY
                        * Math.max(
                            Math.abs(
                                Math.abs(caught.motionY) + base / (mass * Math.abs(d))
                            ),
                            AXISSPEEDCAP
                        );
                }
                if (caught.motionZ < AXISSPEEDCAP && facing.offsetZ != 0) {
                    ...
                    double base
                        = multiplier * power2 * BASESPEED * (wideAreaBlow ? 0.125 : 1);
                    double speedstep = Math.max(
                        Math.abs(Math.abs(caught.motionZ) + base / (mass * Math.abs(d))),
                        AXISSPEEDCAP
                    );
    ...
    private boolean canBlowEntity(Entity e) {
        if (e instanceof CustomFanEntity) {
            ...
            if (c.getBlowPower() > power)
                return false;
            double ang = ReikaMathLibrary.py3d(
                Math.signum(e.motionX) - facing.offsetX,
                Math.signum(e.motionY) - facing.offsetY,
                Math.signum(e.motionZ) - facing.offsetZ
            );
    ...
    public void rip2(World world, int x, int y, int z) {
         ...
         */
        //ReikaJavaLibrary.pConsole(id+":"+ModCropList.getModCrop(id, meta), id !=
        //Blocks.air);
        boolean crop = ReikaCropHelper.isCrop(id) || ModCropList.isModCrop(id, meta);
        if (id != Blocks.snow && id != Blocks.web && id != Blocks.leaves
            && id != Blocks.leaves2 && id != Blocks.tallgrass && id != Blocks.fire
            && !crop)
            return;
    ...
    private void harvest(World world, int x, int y, int z, int meta, Block id) {
        ...
        if (crop != null
            && crop.isRipe(
                world, x, y, z
            ) /* && (omega > HARVESTSPEED*4 || !crop.getShape().isSolid())*/) {
    ...
    private void dropBlocks(World world, int x, int y, int z, Block id, int meta) {
        if (id != Blocks.air)
            ReikaItemHelper.dropItems(
                world, x + 0.5, y + 0.5, z + 0.5, id.getDrops(world, x, y, z, meta, 0)
            );
        world.setBlockToAir(x, y, z);
    }

    public AxisAlignedBB getBlowZone(int meta, int range) {
        return ReikaAABBHelper.getBeamBox(
            xCoord, yCoord, zCoord, facing, range
        ); //.expand(0.0, 0.0, 0.0);
    }
    ...
    public boolean canUpgradeWith(ItemStack item) {
        return !wideAreaBlow && ReikaItemHelper.matchStacks(item, ItemStacks.diffuser);
    }
    ...
    public void breakBlock() {
        if (wideAreaBlow) {
            ReikaItemHelper.dropItem(
                worldObj, xCoord, yCoord, zCoord, ItemStacks.diffuser
            );
        }
    }
}
