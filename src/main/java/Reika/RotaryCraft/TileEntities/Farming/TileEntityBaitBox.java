...
public class TileEntityBaitBox extends InventoriedPowerReceiver
    implements RangedEffect, ConditionalOperation, MobAttractor, MobRepellent {
    public static final int FALLOFF = 4096; //4 kW per extra meter

    private EntityPathSpline pathfinder;

    private static final PropagationCondition butterflyPathRule
        = new PropagationCondition() {
              @Override
              public boolean isValidLocation(
                  World world, int x, int y, int z, Coordinate from
              ) {
                  return PassablePropagation.instance.isValidLocation(
                             world, x, y, z, from
                         )
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!inbox.isEmpty() && (world.getTotalWorldTime() & 3) == 0) {
            for (EntityLiving ent : inbox) {
                ...
                if (ModList.FORESTRY.isLoaded() && pathfinder != null
                    && ent instanceof IEntityButterfly) {
                    pathfinder.addEntity(ent, butterflyPathRule);
                }
            }
        }
        if (ModList.FORESTRY.isLoaded()
            && ReikaInventoryHelper.checkForItem(
                ForestryHandler.ItemEntry.POLLEN.getItem(), inv
            )) {
            if (pathfinder != null)
                pathfinder.removeDeadEntities(world);
            ReikaBeeHelper.attractButterflies(
                world, x + 0.5, y + 1.5, z + 0.5, range, pathfinder
            );
            ReikaBeeHelper.collectButterflies(
                world, ReikaAABBHelper.getBlockAABB(this).expand(4, 2, 4), null
            );
        }
    ...
    private void doEggIncubation(World world, int x, int y, int z) {
        ...
        if (slot >= 0) {
            if (ReikaWorldHelper.checkForAdjMaterial(world, x, y, z, Material.fire)
                != null) {
                if (ReikaWorldHelper.checkForAdjMaterial(world, x, y, z, Material.lava)
                    == null) {
                    EntityChicken ec = new EntityChicken(world);
                    ec.setLocationAndAngles(
                        x + rand.nextDouble(),
                    ...
                    ReikaInventoryHelper.addToIInv(
                        new ItemStack(Items.cooked_chicken), this, true
                    );
                }
    ...
    private void dropHeldItem(World world, int x, int y, int z, EntityLivingBase ent) {
        ...
        if (held != null && !world.isRemote) {
            EntityItem ei = new EntityItem(
                world, ent.posX, ent.posY + ent.getEyeHeight(), ent.posZ, held
            );
    ...
    public boolean canRepel(EntityLiving ent) {
        return power >= MINPOWER
            && this.getRange()
            >= ent.getDistance(xCoord + 0.5, yCoord + 0.5, zCoord + 0.5)
            && MobBait.hasRepelItem(ent, inv);
    }
    ...
    public boolean canAttract(EntityLiving ent) {
        return power >= MINPOWER
            && this.getRange()
            >= ent.getDistance(xCoord + 0.5, yCoord + 0.5, zCoord + 0.5)
            && MobBait.hasAttractItem(ent, inv);
    }

    private void
    applyEffect(World world, int x, int y, int z, EntityLiving ent, boolean attract) {
        if (world.isRemote)
            return;
        ...
        if (attract) {
            ReikaEntityHelper.addAITask(
                ent, new AITaskSeekMachine(ent, 1, this), -1000000
            );
        } else {
            ReikaEntityHelper.addAITask(
                ent, new AITaskAvoidMachine(ent, 1, this), -1000000
            );
            this.dropHeldItem(world, x, y, z, ent);
        }

        if (ent instanceof EntityBlaze || ent instanceof EntitySlime
            || ent instanceof EntityMagmaCube || ent instanceof EntityGhast
            || ent instanceof EntitySquid) {
            if (attract) {
            ...
            ent.renderYawOffset += (-((float) Math.atan2(ent.motionX, ent.motionZ))
