...
public class TileEntityLawnSprinkler extends SprinklerBlock {
    ...
    public static final int PRESSURE_TO_HURT = 8000000
        / PRESSURE_MODIFIER; //typical pressure washers range from 1.5 to 4ksi = 10-30MPa
    public static final int PRESSURE_TO_KILL
        = 20000000 / PRESSURE_MODIFIER; //high enough to cause fluid injection injuries
    ...
    public void performEffects(World world, int x, int y, int z) {
        ...
        if (!world.isRemote) {
            ...
            if (this.getPressure() > PRESSURE_TO_HURT)
                this.damageMobs(
                    world, x, y, z, this.getPressure() >= PRESSURE_TO_KILL ? 4 : 1
                );
        }
    ...
    private void clearRadiation(World world, int x, int y, int z) {
        int r = this.getRange();
        AxisAlignedBB box
            = ReikaAABBHelper.getBlockAABB(x, y, z).offset(0, 2, 0).expand(r, 2, r);
        ...
        for (int i = y; i > y - 4; i--) {
            ...
            if (id == Blocks.farmland) {
                ...
                if (this.shouldTick(world, x, y, z, id)) {
                    BlockTickEvent.fire(
                        id, world, rx, i, rz, rand, UpdateFlags.getForcedUnstoppableTick()
                    );
                } else {
    ...
    private void damageMobs(World world, int x, int y, int z, int dmg) {
        int r = this.getRange();
        AxisAlignedBB box
            = ReikaAABBHelper.getBlockAABB(x, y, z).offset(0, 1, 0).expand(r, 1, r);
        List<EntityLivingBase> li
            = world.getEntitiesWithinAABB(EntityLivingBase.class, box);
        for (EntityLivingBase e : li) {
            RayTracer rt
                = new RayTracer(x + 0.5, y + 0.5, z + 0.5, e.posX, e.posY + 0.5, e.posZ);
            rt.airOnly = true;
    ...
    private void createWaterStream(World world, int x, int y, int z, float offset) {
        ...
        for (int i = 0; i < 6 * d; i++) {
            ...
            world.spawnParticle(
                ...
                y + 0.75,
                z + 0.5 + dz,
                dx * v - 0.025 + 0.05 * rand.nextDouble(),
                0,
                dz * v - 0.025 + 0.05 * rand.nextDouble()
            );
        }
    }
