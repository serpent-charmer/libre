...
public class TileEntityAutoBreeder extends InventoriedPowerReceiver
    implements RangedEffect, ConditionalOperation, IdleComparator {
    public static final int FALLOFF = 2048; //2kW per extra meter

    private static final MultiMap<Class, KeyedItemStack> feedItems
    ...
    private void breed(World world, int x, int y, int z, List<EntityAgeable> inroom) {
        ...
        for (EntityAgeable ent : inroom) {
            //ReikaJavaLibrary.pConsole(this.canBreed(ent)+" for
            //"+ent.getCommandSenderName());
            if (this.canBreed(ent)) {
                if (!(ent instanceof EntityTameable)
                    || (ent instanceof EntityTameable
                        && !((EntityTameable) ent).isSitting())) {
                    if (pathing) {
                ...
                if (!ent.isChild() && ent.getGrowingAge() <= 0
                    && ReikaMathLibrary.py3d(x - ent.posX, y - ent.posY, z - ent.posZ)
                        <= 2.4) {
                    if (!(ent instanceof EntityTameable)
                        || (ent instanceof EntityTameable
                            && ((EntityTameable) ent).isTamed())) {
                        this.breed(ent);
                        ...
                        for (int i = 0; i < n; i++) {
                            ...
                            ent.worldObj.spawnParticle(
                                "heart",
                                ent.posX + rand.nextFloat() * ent.width * 2.0F
                                    - ent.width,
                                ent.posY + 0.5D + rand.nextFloat() * ent.height,
                                ent.posZ + rand.nextFloat() * ent.width * 2.0F
                                    - ent.width,
                                var4,
                                var6,
                                var8
    ...
    static {
        ...
        if (ModList.TWILIGHT.isLoaded()) {
            try {
                addFeedItem(
                    Class.forName("twilightforest.entity.passive.EntityTFBighorn"),
                    Items.wheat
                );
                addFeedItem(
                    Class.forName("twilightforest.entity.passive.EntityTFBighorn"),
                ...
                addFeedItem(
                    Class.forName("twilightforest.entity.passive.EntityTFDeer"),
                    Items.wheat
                );
                addFeedItem(
                    Class.forName("twilightforest.entity.passive.EntityTFDeer"),
