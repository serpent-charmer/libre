...
public class TileEntityWoodcutter extends InventoriedPowerReceiver
    implements EnchantableMachine, InertIInv, ProcessingMachine, DamagingContact,
               Wettable, MultiOperational {
    private final MachineEnchantmentHandler enchantments
        = new MachineEnchantmentHandler()
              .addFilter(Enchantment.infinity)
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (tree.isEmpty() && this.hasWood()) {
            ...
            if (type == null) {
                TreeType type2 = this.lookupTree(
                    world, editx - read.offsetX, y, editz - read.offsetZ
                );
                if (type2 != null) {
    ...
    private TreeType lookupTree(World world, int x, int y, int z) {
        TreeType type = ReikaTreeHelper.getTree(
            world.getBlock(x, y, z), world.getBlockMetadata(x, y, z)
        );
        if (type == null)
            type = ModWoodList.getModWood(
                world.getBlock(x, y, z), world.getBlockMetadata(x, y, z)
            );
        return type;
    }
    ...
    private void cutCoord(World world, int x, int y, int z, Coordinate c) {
        ...
        if (drop != Blocks.air) {
            Material mat
                = ReikaWorldHelper.getMaterial(world, c.xCoord, c.yCoord, c.zCoord);
            if (ConfigRegistry.INSTACUT.getState()) {
                //ReikaItemHelper.dropItems(world, dropx, y-0.25, dropz,
                //dropBlocks.getDrops(world, c.xCoord, c.yCoord, c.zCoord, dropmeta, 0));
                ...
                if (c.yCoord == y && drop == tree.getTreeType().getLogID()) {
                    ...
                    if (ReikaPlantHelper.SAPLING.canPlantAt(
                            world, c.xCoord, c.yCoord, c.zCoord
                        )) {
                        BlockKey plant = this.getPlantedSapling();
                        if (plant != null) {
                            if (inv[0] != null
                                && !enchantments.hasEnchantment(Enchantment.infinity))
                ...
                boolean fall
                    = BlockSand.func_149831_e(world, c.xCoord, c.yCoord - 1, c.zCoord);
                if (fall) {
                        ...
                        if (ReikaPlantHelper.SAPLING.canPlantAt(
                            ...
                            if (plant != null) {
                                if (inv[0] != null
                                    ...
                                    ReikaInventoryHelper.decrStack(0, inv);
                                world.setBlock(
                                    c.xCoord, c.yCoord - 1, c.zCoord, Blocks.dirt
                                );
                                plant.place(world, c.xCoord, c.yCoord, c.zCoord);
                            }
                        }
    ...
    private void cutBlock(World world, int x, int y, int z, Coordinate c, Material mat) {
        ...
        if (b instanceof LeafBlockWithExtras) {
            ((LeafBlockWithExtras) b)
                .onPreWoodcutterBreak(world, c.xCoord, c.yCoord, c.zCoord);
        }
        c.setBlock(world, Blocks.air);
    ...
    getDrops(World world, int x, int y, int z, Block b, int meta) {
        ...
        if (ReikaRandomHelper.doWithChance(f)) {
            ...
            MinecraftForge.EVENT_BUS.post(new HarvestDropsEvent(
                x, y, z, world, b, meta, fortune, 1, ret, this.getPlacer(), false
            ));
            if (tree.getTreeType() == ModWoodList.SLIME) {
    ...
    private void checkAndMatchInventory() {
        ...
        if (tree.isDyeTree()) {
            sapling = new BlockKey(
                ChromatiAPI.getAPI().trees().getDyeSapling(), tree.getDyeTreeMeta()
            );
    ...
    private Block dropBlocks(World world, int x, int y, int z) {
        ...
        if (drop instanceof LeafBlockWithExtras) {
            ...
            ArrayList<ItemStack> li
                = ((LeafBlockWithExtras) drop).getExtraDrops(world, x, y, z, fortune);
        ...
        if (drop == logID && logID != null) {
            if (tree.getTreeType() != ModWoodList.SLIME) {
                if (rand.nextInt(3) == 0) {
                    drops.add(ReikaItemHelper.getSizedItemStack(
                        ItemStacks.sawdust.copy(), 1 + rand.nextInt(4)
                    ));
                }
            }
        }
        ...
        for (ItemStack todrop : drops) {
            if (ReikaItemHelper.matchStacks(todrop, sapling)) {
                if (enchantments.hasEnchantment(Enchantment.infinity)
                    || (inv[0] != null && inv[0].stackSize >= inv[0].getMaxStackSize())) {
                    this.chestCheck(todrop);
                    if (todrop.stackSize > 0)
                        ReikaItemHelper.dropItem(
                            world, x + 0.5, yCoord - 0.25, z + 0.5, todrop
                        );
                } else
    ...
    public BlockKey getPlantedSapling() {
        ...
        if (treeCopy.isDyeTree())
            return new BlockKey(
                ChromatiAPI.getAPI().trees().getDyeSapling(), treeCopy.getDyeTreeMeta()
            );
    ...
    private boolean shouldPlantSapling() {
        if (enchantments.hasEnchantment(Enchantment.infinity))
            return true;
        if (treeCopy.isDyeTree()) {
            return inv[0] != null && inv[0].stackSize > 0
                && ReikaItemHelper.matchStackWithBlock(
                    inv[0], ChromatiAPI.getAPI().trees().getDyeSapling()
                )
                && inv[0].getItemDamage() == treeCopy.getDyeTreeMeta();
        } else if (treeCopy.getTreeType() != null) {
            return inv[0] != null && inv[0].stackSize > 0
