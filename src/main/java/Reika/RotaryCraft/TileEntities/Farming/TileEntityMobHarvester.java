...
public class TileEntityMobHarvester
    extends TileEntityPowerReceiver implements EnchantableMachine {
    private final MachineEnchantmentHandler enchantments
        = new MachineEnchantmentHandler()
              ...
              .addFilter(Enchantment.fireAspect)
            ...
            if (!(ent instanceof EntityVillager)) {
                ...
                if (ep != null) {
                    ...
                    if (dmg > 0) {
                        ...
                        if (enchantments.hasEnchantment(Enchantment.silkTouch)
                            && rand.nextInt(20) == 0)
                        ...
                        if (enchantments.hasEnchantment(Enchantment.fireAspect))
                            ent.setFire(
                                enchantments.getEnchantment(Enchantment.fireAspect) * 2
                            );
                    }
                }
                ent.motionY = 0;
    ...
    public int getDamage() {
        ...
        double base = ReikaMathLibrary.logbase(ppdiff, 2)
            + 2 * enchantments.getEnchantment(Enchantment.sharpness);
        if (ModList.CHROMATICRAFT.isLoaded()) {
            //base *= ChromatiAPI.getAPI().adjacency().getFactorSimple(worldObj, xCoord,
            //yCoord, zCoord, "PINK");
        }
        return (int) base;
    }
    ...
    public AxisAlignedBB getBox() {
        //return AxisAlignedBB.getBoundingBox(this.xCoord-4, this.yCoord-4, this.zCoord-4,
        //this.xCoord+5, this.yCoord+5, this.zCoord+5);
        return AxisAlignedBB.getBoundingBox(
            xCoord,
            yCoord + 1,
            zCoord,
            xCoord + 1,
            yCoord + this.getHeight() + 1,
            zCoord + 1
        );
    }
