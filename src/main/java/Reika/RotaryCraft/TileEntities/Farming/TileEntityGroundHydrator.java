...
public class TileEntityGroundHydrator
    extends RotaryCraftTileEntity implements PipeConnector, IFluidHandler {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        if (!world.isRemote && tank.getLevel() >= FLUID_PER_BLOCK
            && rand.nextInt(this.getTickRate(world)) == 0) {
            Coordinate c = coordinateRand.getRandomEntry().offset(x, y, z);
            Block b = c.getBlock(world);
            boolean flag = false;
            if (b == Blocks.farmland) {
                flag = ReikaWorldHelper.hydrateFarmland(
                    world, c.xCoord, c.yCoord, c.zCoord, false
                );
    ...
    private int getTickRate(World world) {
        return ModList.MYSTCRAFT.isLoaded() && ReikaMystcraftHelper.isMystAge(world)
                && ReikaMystcraftHelper.isSymbolPresent(world, "EnvAccel")
            ? 1
            : 2;
    }
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        return ReikaAABBHelper.getBlockAABB(this).expand(
            this.getRange(), 0.5, this.getRange()
        );
    }
