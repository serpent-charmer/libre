...
public class TileEntityGPR
     ...
     */
    private BlockKey[][] blocks
        = new BlockKey[MAX_HEIGHT]
                      [MAX_WIDTH]; //from 0-16 -> centred on 8 (8 above and below)
    ...
    private static int getBlockColor(BlockKey bk) {
        return bk != null
            ? BlockColorMapper.instance.getColorForBlock(bk.blockID, bk.metadata)
            : BlockColorMapper.UNKNOWN_COLOR.getColor();
    }
    ...
    public int getRange() {
        return Math.min(
            this.getMaxRange(), 2 * ReikaMathLibrary.logbase2(power - MINPOWER)
        );
    }
