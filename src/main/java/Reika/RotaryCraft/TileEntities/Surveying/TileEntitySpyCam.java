...
public class TileEntitySpyCam extends RemoteControlMachine implements RangedEffect {
    ...
    private void getMobs(World world, int x, int y, int z) {
        ...
        AxisAlignedBB zone = AxisAlignedBB.getBoundingBox(
            x - range, 0, z - range, x + 1 + range, y + 1, z + 1 + range
        );
        ...
        for (EntityLivingBase ent : inzone) {
            ...
            if (EntityList.getEntityID(ent) > 0 && Math.abs(ex) < range + 1
                && Math.abs(ez) < range + 1
                && ent.posY >= ReikaWorldHelper.findTopBlockBelowY(
                       world, (int) ent.posX, y, (int) ent.posZ
                   )) {
                //ReikaJavaLibrary.pConsole(ent.getCommandSenderName()+" @ "+ex+", "+ez);
                mobs[(ez + range)][ex + range] = EntityList.getEntityID(ent);
                //ReikaJavaLibrary.pConsole(mobs[ex+range][ez+range]+String.format("@ %d,
                //%d  from  %d", ex+range, ez+range, EntityList.getEntityID(ent)));
            }
        }
    }
    ...
    public void activate(World world, EntityPlayer ep, int x, int y, int z) {
        if (on)
            ep.openGui(
                RotaryCraft.instance, GuiRegistry.SPYCAM.ordinal(), world, x, y, z
            );
    }
}
