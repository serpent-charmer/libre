...
public class TileEntityMobRadar
    ...
    private void getMobs(World world, int x, int y, int z) {
        ...
        AxisAlignedBB zone = AxisAlignedBB.getBoundingBox(
            x - range, 0, z - range, x + 1 + range, 255, z + 1 + range
        );
        inzone = world.selectEntitiesWithinAABB(
            EntityLivingBase.class, zone, this.getSelector()
        );
        for (EntityLivingBase ent : inzone) {
            if (ent instanceof RadarJammer
                && ((RadarJammer) ent).jamRadar(worldObj, xCoord, yCoord, zCoord)) {
                isJammed = true;
                break;
            }
    ...
    protected void animateWithTick(World world, int x, int y, int z) {
        ...
        if (power < MINPOWER)
            return;
        //this.phi += ReikaMathLibrary.doubpow(ReikaMathLibrary.logbase(this.omega+1,
        //2), 1.05);
        phi += 4F;
    }
