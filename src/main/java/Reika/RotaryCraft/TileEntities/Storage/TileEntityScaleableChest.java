...
public class TileEntityScaleableChest
    extends InventoriedPowerReceiver implements MultiPageInventory {
    ...
    private boolean testInconsistentPower() {
        ...
        if (numchanges > 10 && !worldObj.isRemote) {
            this.getTileEntityBlockID().dropBlockAsItem(
                worldObj, xCoord, yCoord, zCoord, this.getIndex(), 0
            );
            ...
            if (rand.nextInt(19 - numchanges) == 0)
                worldObj.createExplosion(
                    null, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, 0F, false
                );
            ...
            if (rand.nextInt(11 - numchanges) == 0)
                worldObj.playSoundEffect(
                    xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, "random.fizz", 1F, 1F
                );
        } else
            return false;
        return true;
            ...
            while (iterator.hasNext()) {
                ...
                if (entityplayer.openContainer instanceof ContainerScaleChest) {
                    IInventory iinventory
                        = ((ContainerScaleChest) entityplayer.openContainer)
                              .getLowerScaleChestInventory();
                    if (iinventory == this)
                        ++numUsingPlayers;
                }
            }
    ...
    public void openInventory() {
        ...
        worldObj.addBlockEvent(
            xCoord, yCoord, zCoord, this.getTileEntityBlockID(), 1, numUsingPlayers
        );
        worldObj.notifyBlocksOfNeighborChange(
            xCoord, yCoord, zCoord, this.getTileEntityBlockID()
        );
        worldObj.notifyBlocksOfNeighborChange(
            xCoord, yCoord - 1, zCoord, this.getTileEntityBlockID()
        );
    }
    ...
    public void readInventoryFromItem(ItemStack is) {
        if (is.stackTagCompound != null) {
            NBTTagList nbttaglist
                = is.stackTagCompound.getTagList("Items", NBTTypes.COMPOUND.ID);
            ...
            for (int i = 0; i < nbttaglist.tagCount(); i++) {
                ...
                if (byte0 >= 0 && byte0 < inv.length) {
                    ...
                    RotaryCraft.logger.logError(
                        this + " tried to load an inventory slot " + byte0 + " from NBT!"
                    );
                }
            }
        }
