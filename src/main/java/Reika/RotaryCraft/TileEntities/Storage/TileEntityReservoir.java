...
public class TileEntityReservoir extends RotaryCraftTileEntity
    implements PipeConnector, IFluidHandler, NBTMachine, BreakAction,
    ...
    public void recalculateCompound() {
        ...
        for (int i = 2; i < 6; i++) {
            ...
            if (te instanceof TileEntityReservoir) {
                ...
                if (tr.getFluid() == this.getFluid() || tr.getFluid() == null ||
    this.getFluid() == null) { if (tr.network != null) { network.merge(tr.network);
                    }
                }
            }
        }
    ...
    private void updateSides(World world, int x, int y, int z) {
        adjacent[8]
            = MachineRegistry.getMachine(world, x, y, z - 1) == MachineRegistry.RESERVOIR;
        adjacent[4]
            = MachineRegistry.getMachine(world, x - 1, y, z) == MachineRegistry.RESERVOIR;
        adjacent[6]
            = MachineRegistry.getMachine(world, x + 1, y, z) == MachineRegistry.RESERVOIR;
        adjacent[2]
            = MachineRegistry.getMachine(world, x, y, z + 1) == MachineRegistry.RESERVOIR;

        adjacent[1] = MachineRegistry.getMachine(world, x - 1, y, z + 1)
            == MachineRegistry.RESERVOIR;
        adjacent[3] = MachineRegistry.getMachine(world, x + 1, y, z + 1)
            == MachineRegistry.RESERVOIR;
        adjacent[7] = MachineRegistry.getMachine(world, x - 1, y, z - 1)
            == MachineRegistry.RESERVOIR;
        adjacent[9] = MachineRegistry.getMachine(world, x + 1, y, z - 1)
            == MachineRegistry.RESERVOIR;
        ...
        for (int i = -1; i <= 1; i++) {
            for (int k = -1; k <= 1; k++) {
                if (i != 0 || k != 0) {
                    int dx = xCoord + i;
                    int dz = zCoord + k;
                    if (MachineRegistry.getMachine(worldObj, dx, yCoord, dz)
                        == this.getTile()) {
                        ((TileEntityReservoir) worldObj.getTileEntity(dx, yCoord, dz))
                            .updateSides(worldObj, dx, yCoord, dz);
                    }
                }
            }
        }
        ...
        if (!isCovered) {
            if (!world.isRemote) {
                ...
                if (world.isRaining() && biome.canSpawnLightningBolt()
                    ...
                    if (this.isEmpty()
                        || (this.getFluid().equals(FluidRegistry.WATER)
                            && this.getLevel() < CAPACITY)) {
            ...
            if (!tank.isEmpty()) {
                if (tank.getActualFluid().getDensity(world, x, y, z) < 0
                    && tank.getActualFluid().isGaseous()) {
        ...
        if (!world.isRemote && !this.isEmpty() && tempTimer.checkCap()) {
            if (!this.isSurrounded(false)) {
                ...
                for (int i = -r; i <= r; i++) {
                    for (int j = -r; j <= r; j++) {
                        for (int k = -r; k <= r; k++) {
                            ...
                            //ReikaJavaLibrary.pConsole(dT+" above "+Tamb+" @ "+temp+", h
                            //= "+hiT, Side.SERVER);
                            ReikaWorldHelper.temperatureEnvironment(
                                world, x + i, y + j, z + k, hiT
                            );
                            if (temp > 2500)
                                ReikaSoundHelper.playSoundAtBlock(
                                    world, x + i, y + j, z + k, "random.fizz", 0.2F, 1F
                                );
                        }
                    }
                }
                if (temp > 2500) {
                    ...
                    world.setBlock(x, y, z - 1, Blocks.flowing_lava);
                    ReikaSoundHelper.playSoundAtBlock(
                        world, x, y, z, "random.fizz", 0.4F, 1F
                    );
                }

                boolean hot = Tamb >= 300;
                hot = hot
                    || ReikaWorldHelper.checkForAdjMaterial(world, x, y, z, Material.fire)
                        != null;
                hot = hot
                    || ReikaWorldHelper.checkForAdjMaterial(world, x, y, z, Material.lava)
                        != null;
                if (hot) {
    ...
    private boolean isSurrounded(boolean reservoirOnly) {
        return (adjacent[2] && adjacent[4] && adjacent[6] && adjacent[8])
            || (!reservoirOnly
                && ReikaWorldHelper.isBlockSurroundedBySolid(
                ...
                if (this.adjacentOnSide(dir)) {
                    ...
                    if (this.matchMachine(world, dx, dy, dz)) {
                        TileEntityReservoir tile
                            = (TileEntityReservoir) world.getTileEntity(dx, dy, dz);
    ...
    public boolean canConnectToPipe(MachineRegistry m) {
        return m.isStandardPipe() || m == MachineRegistry.HOSE
            || m == MachineRegistry.FUELLINE || m == MachineRegistry.VALVE;
    }
    ...
    public Collection<AxisAlignedBB> getComplexHitbox() {
        ...
        if (isCovered) {
            li.add(AxisAlignedBB.getBoundingBox(
                0.0625, 0.875, 0.0625, 0.9375, 0.9375, 0.9375
            ));
        }
    ...
    public AxisAlignedBB getHitbox() {
        ...
        return AxisAlignedBB.getBoundingBox(
            xCoord, yCoord, zCoord, xCoord + 1, yCoord + 0.0625, zCoord + 1
        );
    }
    ...
    public boolean isConnectedOnSide(ForgeDirection dir) {
        ...
        if (this.adjacentOnSide(dir)) {
            TileEntityReservoir te
                = (TileEntityReservoir) worldObj.getTileEntity(dx, dy, dz);
            if (te == null)
                return false;
            return te.isEmpty() || this.isEmpty()
                || te.getFluid().equals(this.getFluid());
        }
        return false;
    }
    ...
    public int getFluidRenderColor() {
        ...
        if (fs == null)
            return 0xffffff;
        int clr = fs.tag != null && fs.tag.hasKey("renderColor")
            ? fs.tag.getInteger("renderColor")
            : 0xffffff;
        if (this.isInWorld() && fs.getFluid().canBePlacedInWorld()) {
            clr = fs.getFluid().getBlock().colorMultiplier(
    ...
    public void applyFluidEffectsToEntity(EntityLivingBase e) {
        if (!tank.isEmpty() && !isCovered) {
            ...
            if (f.equals(FluidRegistry.LAVA)
                || f.getTemperature(worldObj, xCoord, yCoord, zCoord) > 500) {
    ...
    public static void addFluidEffect(String f, FluidEffect e) {
        ...
        if (g != null) {
            RotaryCraft.logger.logError(
                "Cannot add effect " + e + " for fluid " + f
                + "; fluid already mapped to " + g
            );
        } else {
