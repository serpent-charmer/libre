...
public class TileEntityFluidCompressor
    extends TileEntityPowerReceiver implements IFluidHandler, PipeConnector, NBTMachine {
        ...
        this.getPowerBelow(); /*
         if (this.getLevel() > this.getCapacity()) {
             ...
             if (timer > 400) {
                 ...
                 world.createExplosion(null, x+0.5, y+0.5, z+0.5, 8F,
         ConfigRegistry.BLOCKDAMAGE.getState());
             ...
             timer = 0;*/
        //ReikaJavaLibrary.pConsole(tank.getLevel()/1000D+"/"+this.getCapacity()/1000D,
        //Side.SERVER);
    }
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        if (resource.getFluid() == null)
            return 0;
        int toadd = Math.min(
            resource.amount, this.getCapacity(resource.getFluid()) - tank.getLevel()
        );
        if (toadd <= 0)
            return 0;
