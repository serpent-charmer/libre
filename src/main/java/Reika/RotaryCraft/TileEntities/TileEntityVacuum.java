...
public class TileEntityVacuum
    extends InventoriedPowerReceiver implements RangedEffect, BreakAction, IFluidHandler {
    ...
    public static final int FALLOFF = Math.min(
        524288,
        ReikaMathLibrary.ceil2exp(Math.max(1024, ConfigRegistry.VACPOWER.getValue()))
    );

    private static final IEntitySelector selector
        = ReikaEntityHelper.combineEntitySelectors(
            false,
            ReikaEntityHelper.itemOrXPSelector,
            new ClassEntitySelector(LizardDoggo.class, false)
        );
                ...
                for (int k = 0; k < size; k++) {
                    ...
                    if (inslot != null) {
                        ...
                        if (te instanceof ISidedInventory)
                            cansuck = ((ISidedInventory) te)
                                          .canExtractItem(
                                              k, inslot, dir.getOpposite().ordinal()
                                          );
                        if (cansuck) {
                            if (this.canSuckStacks()
                                && ReikaInventoryHelper.addToIInv(inslot.copy(), this)) {
                                ii.setInventorySlotContents(k, null);
                            } else {
                                int newsize = inslot.stackSize - 1;
                                ItemStack is2
                                    = ReikaItemHelper.getSizedItemStack(inslot, 1);
                                ItemStack is3
                                    = ReikaItemHelper.getSizedItemStack(inslot, newsize);
                                if (newsize <= 0)
                                    is3 = null;
    ...
    public void spawnXP() {
        ReikaWorldHelper.splitAndSpawnXP(
            worldObj,
            xCoord - 1 + 2 * rand.nextFloat(),
            yCoord + 2 * rand.nextFloat(),
            zCoord - 1 + 2 * rand.nextFloat(),
            experience
        );
        experience = 0;
    }
    ...
    private void suck(World world, int x, int y, int z) {
        ...
        for (Entity ent : inbox) {
            if (ent instanceof LizardDoggo) {
                ...
                if (true
                    || ReikaWorldHelper.canBlockSee(
                        world, x, y, z, ent.posX, ent.posY, ent.posZ, this.getRange() + 2
                    )) {
                    double dx = (x + 0.5 - ent.posX);
                    double dy = (y + 0.5 - ent.posY);
                    double dz = (z + 0.5 - ent.posZ);
                    double ddt = ReikaMathLibrary.py3d(dx, dy, dz);
                    if (ent.ticksExisted > 50 && ddt > 1.5
                        && ent.ticksExisted % 400 < 80) { //For routing around objects
                        double t = this.getTicksExisted() / 25D;
                        double r = 2.875; //+1*ReikaMathLibrary.cosInterpolation(0, 1,
                                          //Math.sin(t/2));
                        dx += r * Math.cos(t);
                        dz += r * Math.sin(t);
                    }
    ...
    private void absorb(World world, int x, int y, int z) {
        if (world.isRemote)
            return;
        boolean suck = false;
        AxisAlignedBB close = AxisAlignedBB.getBoundingBox(x, y, z, x + 1, y + 1, z + 1)
                                  .expand(0.25D, 0.25D, 0.25D);
        List<EntityItem> closeitems
            = world.getEntitiesWithinAABB(EntityItem.class, close);
        for (EntityItem ent : closeitems) {
            if (ent.delayBeforeCanPickup <= 0) {
                ...
                world.playSoundEffect(
                    ...
                    z + 0.5,
                    "random.pop",
                    0.1F + 0.5F * rand.nextFloat(),
                    rand.nextFloat()
                );
                MinecraftForge.EVENT_BUS.post(
                    new VacuumItemAbsorbEvent(this, is != null ? is.copy() : null)
                );
            } else {
                suck = true;
            }
        ...
        isFull = !suck;
        List<EntityXPOrb> closeorbs
            = world.getEntitiesWithinAABB(EntityXPOrb.class, close);
    ...
    private AxisAlignedBB getBox(World world, int x, int y, int z) {
        ...
        return equidistant ? base.expand(expand, expand, expand)
                           : base.expand(expand, 2, expand);
    }

    public int getRange() {
        int r = Math.min(8 + (int) (power / FALLOFF), this.getMaxRange());
        if (ModList.CHROMATICRAFT.isLoaded()) {
            //r *= ChromatiAPI.getAPI().adjacency().getFactorSimple(worldObj, xCoord,
            //yCoord, zCoord, "LIME");
        }
        return r;
    }
    ...
    public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain) {
        return resource.getFluidID() == ReikaXPFluidHelper.getFluid().getFluidID()
    ...
    public boolean canDrain(ForgeDirection from, Fluid fluid) {
        return ReikaXPFluidHelper.fluidsExist() && experience > 0
            && fluid.equals(ReikaXPFluidHelper.getFluidType());
    }
