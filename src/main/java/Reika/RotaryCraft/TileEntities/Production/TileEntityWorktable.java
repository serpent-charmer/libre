...
public class TileEntityWorktable extends InventoriedRCTileEntity
    implements CraftingTile<WorktableRecipe>, TriggerableAction, AlternatingRedstoneUser {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        if (!world.isRemote) {
            ...
            if (matrix.isEmpty())
                return;
            if (hasUpgrade && this.getTicksExisted() % 6 == 0
                && !this.hasRedstoneSignal()) {
                this.onPositiveRedstoneEdge();
                if (this.getTicksExisted() % 6 == 0) {
                    ReikaSoundHelper.playSoundAtBlock(
                        world, x, y, z, "random.click", 0.5F, 0.675F
                    );
                }
            }
    ...
    private void makeHelmetUpgrades() {
        int armorslot = ReikaInventoryHelper.locateInInventory(
            ItemRegistry.BEDHELM.getItemInstance(), inv
        );
        if (armorslot == -1)
            armorslot = ReikaInventoryHelper.locateInInventory(
                ItemRegistry.BEDREVEAL.getItemInstance(), inv
            );
        if (armorslot != -1) {
        ...
        if (is != null) {
            ...
            if (item instanceof ItemJetPack) {
                ...
                if (!PackUpgrades.COOLING.existsOn(is)) {
                    boolean items = ReikaItemHelper.matchStacks(
                        inv[3], MachineRegistry.COOLINGFIN.getCraftedProduct()
                    );
                    items &= ReikaItemHelper.matchStacks(
                        inv[5], MachineRegistry.COOLINGFIN.getCraftedProduct()
                    );
                    if (items) {
            ...
            if (item instanceof ItemJetPack) {
                ...
                if (!PackUpgrades.JET.existsOn(is)) {
                    if (ReikaItemHelper.matchStacks(
                            inv[7], EngineType.JET.getCraftedProduct()
                        )) {
    ...
    private boolean craft() {
        WorktableRecipe wr
            = WorktableRecipes.getInstance().findMatchingRecipe(matrix, worldObj);
        if (wr != null) {
    ...
    public boolean handleCrafting(WorktableRecipe wr, EntityPlayer ep, boolean craftAll) {
        ...
        if (wr.isRecycling()) {
            ...
            for (int i = 0; i < maxCrafts; i++) {
                ...
                if (inv[13] != null
                    && inv[13].stackSize + is.stackSize
                        > Math.min(this.getInventoryStackLimit(), is.getMaxStackSize()))
                    break;
        ...
        if (crafts > 0) {
            ...
            SoundRegistry.CRAFT.playSoundAtBlock(
                worldObj, xCoord, yCoord, zCoord, 0.3F, 1.5F
            );
            return true;
        } else {
            return false;
    ...
    private void makeBedjump() {
        int armorslot = ReikaInventoryHelper.locateInInventory(
            ItemRegistry.BEDBOOTS.getItemInstance(), inv
        );
        int jumpslot = ReikaInventoryHelper.locateInInventory(
            ItemRegistry.JUMP.getItemInstance(), inv
        );
        if (jumpslot != -1 && armorslot != -1
            && ReikaInventoryHelper.hasNEmptyStacks(inv, 17)) {
                                ...
                                if (in[k] != null && !in[k].isEmpty()) {
                                    if (inv[k + 9] != null) {
                                        if (!ReikaItemHelper.collectionContainsItemStack(
                                            ...
                                            flag = false;
                                        if (inv[k + 9].stackSize >= Math.min(
                                                this.getInventoryStackLimit(),
                                                inv[k + 9].getMaxStackSize()
                                            ))
                                            flag = false;
                                    }
                                }
    ...
    private boolean isNotUncraftable(ItemStack is) {
        ...
        if (ItemRegistry.GEARBOX.matchItem(is)) {
            MaterialRegistry matl
                = GearboxTypes.valueOf(is.stackTagCompound.getString("bearing")).material;
    ...
    private void chargeTools() {
        int coilslot = ReikaInventoryHelper.locateInInventory(
            ItemRegistry.SPRING.getItemInstance(), inv
        );
        if (coilslot == -1)
            coilslot = ReikaInventoryHelper.locateInInventory(
                ItemRegistry.STRONGCOIL.getItemInstance(), inv
            );
        Item toolid = this.getTool();
        int toolslot = ReikaInventoryHelper.locateInInventory(toolid, inv);

        if (toolslot != -1 && coilslot != -1
            && ReikaInventoryHelper.hasNEmptyStacks(inv, 17)) {
            ...
            if (toolid instanceof ChargeableTool) {
                int newcoilcharge
                    = ((ChargeableTool) toolid)
                          .setCharged(
                              tool,
                              coilmeta,
                              coilid == ItemRegistry.STRONGCOIL.getItemInstance()
                          );
                ...
                NBTTagCompound tag = tool.stackTagCompound != null
                    ? (NBTTagCompound) tool.stackTagCompound.copy()
                    : null;
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (is != null) {
                if (is.getItem() instanceof ItemChargedTool
                    || is.getItem() instanceof ItemChargedArmor
                    || is.getItem() instanceof ChargeableTool)
    ...
    private void makeJetplate() {
        boolean bed = false;
        int plateslot = ReikaInventoryHelper.locateInInventory(
            ItemRegistry.BEDCHEST.getItemInstance(), inv
        );
        if (plateslot == -1)
            plateslot = ReikaInventoryHelper.locateInInventory(
                ItemRegistry.STEELCHEST.getItemInstance(), inv
            );
        else
            bed = true;
        //ReikaJavaLibrary.pConsole(plateslot, Side.SERVER);
        int jetslot = ReikaInventoryHelper.locateInInventory(
            ItemRegistry.JETPACK.getItemInstance(), inv
        );
        if (jetslot != -1 && plateslot != -1 && plateslot < 9 && jetslot < 9
            && ReikaInventoryHelper.hasNEmptyStacks(inv, 17)) {
            ItemStack jet = inv[jetslot];
            ItemStack plate = inv[plateslot];
            NBTTagCompound tag1 = plate.stackTagCompound != null
                ? (NBTTagCompound) plate.stackTagCompound.copy()
                : null;
            NBTTagCompound tag2 = jet.stackTagCompound != null
                ? (NBTTagCompound) jet.stackTagCompound.copy()
                : null;
            inv[jetslot] = null;
            inv[plateslot] = null;
            ItemStack is = bed ? ItemRegistry.BEDPACK.getEnchantedStack()
                               : ItemRegistry.STEELPACK.getStackOf();
            if (is.stackTagCompound == null)
    ...
    private void uncraftJetplate() {
        ...
        if (combine != null
            && (ItemRegistry.BEDPACK.matchItem(combine)
                || ItemRegistry.STEELPACK.matchItem(combine))) {
            ...
            if (ReikaInventoryHelper.hasNEmptyStacks(inv, 18)) {
                ...
                ((ItemJetPack) jet.getItem())
                    .addFluid(jet, pack.getCurrentFluid(combine), pack.getFuel(combine));
                for (PackUpgrades p : pack.getUpgrades(combine))
                    p.enable(jet, true);
                inv[4] = null;
                ItemStack plate = bed ? ItemRegistry.BEDCHEST.getEnchantedStack()
                                      : ItemRegistry.STEELCHEST.getStackOf();
                inv[9] = plate;
                inv[10] = jet;
            }
        }
    ...
    public boolean isItemValidForSlot(int i, ItemStack itemstack) {
        if (i >= 9)
            return false;
        //return hasProgram ? inv[i+18] != null && ReikaItemHelper.matchStacks(inv[i+18],
        //itemstack) : true;
        return ItemRegistry.CRAFTPATTERN.matchItem(inv[18])
            ? ItemCraftPattern.checkPatternForMatch(
                this, RecipeMode.WORKTABLE, i, i, itemstack, inv[18]
            )
            : true;
    }
    ...
    public boolean canUpgradeWith(ItemStack item) {
        return !this.hasRedstoneUpgrade() && ItemRegistry.UPGRADE.matchItem(item)
            && item.getItemDamage() == Upgrades.REDSTONE.ordinal();
    }
    ...
    public void breakBlock() {
        if (this.hasRedstoneUpgrade()) {
            ReikaItemHelper.dropItem(
                ...
                yCoord + 0.5,
                zCoord + 0.5,
                ItemRegistry.UPGRADE.getStackOfMetadata(Upgrades.REDSTONE.ordinal())
            );
        }
    }
