...
public class TileEntitySpillway
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if ((InterfaceCache.STREAM.instanceOf(b) && metadata == 0)
            || InterfaceCache.STREAM.instanceOf(b2)) {
            liquidPool = null;
            ...
            if (ReikaWorldHelper.isLiquidAColumn(world, dx, dy + 1, dz)) {
                liquidPool = null;
                tank.addLiquid(
                    (int) (50 * ConfigRegistry.getFreeWaterProduction()),
                    FluidRegistry.WATER
                );
                this.setActive();
            } else
        ...
        if (te instanceof CrystalTank) {
            ...
            if (te != null) {
                int add = ((CrystalTank) te)
                              .addFluid(
                                  tank.getActualFluid(), Math.max(1, tank.getLevel() / 32)
                              );
                if (add > 0)
    ...
    private void
    formAndDrainPool(World world, int x, int y, int z, int dx, int dy, int dz, Block id) {
        if (liquidPool == null || liquidPool.isEmpty()) {
            liquidPool = new BlockArray();
            liquidPool.maxDepth = 240;
            liquidPool.clampToChunkLoad = true;
            liquidPool.recursiveAddWithBoundsMetadata(
                world, dx, dy, dz, id, 0, x - 64, y, z - 64, x + 64, y + 24, z + 64
            );
    ...
    public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain) {
        return from == ForgeDirection.DOWN && resource.getFluid() == FluidRegistry.WATER
