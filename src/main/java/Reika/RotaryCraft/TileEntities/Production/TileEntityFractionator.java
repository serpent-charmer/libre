...
public class TileEntityFractionator extends InventoriedPoweredLiquidIO
    implements MultiOperational, ProcessingMachine, PressureTE {
    public int mixTime;
    ...
    private static KeyedItemStack key(ItemStack is) {
        return new KeyedItemStack(is).setSimpleHash(true).setIgnoreMetadata(
            !is.getHasSubtypes()
        );
    }
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        //int operationtime = ReikaMathLibrary.extrema(BASETIME-this.omega, MINTIME,
        //"max");
    ...
    public void updatePressure(World world, int x, int y, int z, int meta) {
        ...
        if (pressure < local) { //ascending
            pressure += Math.max(
                1,
                Math.min(
                    ReikaRandomHelper.getRandomPlusMinus(6, 13), (local - pressure) / 4
                )
            );
        } else {
            pressure = local;
    ...
    private void make() {
        ...
        if (!worldObj.isRemote) {
            float consume
                = ingredients.size() * DifficultyEffects.CONSUMEFRAC.getChance();
        ...
        output.addLiquid(
            (int) (this.getYieldRatio() * DifficultyEffects.PRODUCEFRAC.getInt()),
    ...
    private boolean process() {
        ...
        if (output.getLevel() + (DifficultyEffects.PRODUCEFRAC.getMaxAmount())
            >= CAPACITY)
            return false;
        if (inv[ingredients.size()] == null)
            return false;
        if (inv[ingredients.size()].getItem()
            != Items.ghast_tear) //need a ghast tear as fuel solvent
            return false;
    ...
    private boolean hasAllIngredients() {
        ...
        for (int i = 0; i < ingredients.size(); i++) {
            ...
            //%d", ingredients[i.getItem, ingredients[i].getItemDamage()));
            //ModLoader.getMinecraftInstance().thePlayer.addChatMessage(String.format("%d",
            //i)+String.valueOf(this.haveIngredient(ingredients[i.getItem,
            //ingredients[i].getItemDamage())));
    ...
    public boolean isItemValidForSlot(int slot, ItemStack is) {
        ...
        if (!this.isJetFuelIngredient(is))
            return false;
        HashSet<Integer> slots = ReikaInventoryHelper.getSlotsBetweenWithItemStack(
            is, this, 0, ingredients.size() - 1, false
        );
