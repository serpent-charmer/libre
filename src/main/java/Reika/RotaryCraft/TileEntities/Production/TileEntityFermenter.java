...
public class TileEntityFermenter extends InventoriedPowerLiquidReceiver
    implements TemperatureTE, MultiOperational, ProcessingMachine {
        ...
        if (inv[0].getItem() == Items.sugar) {
            if (this.hasWater())
                if (ReikaItemHelper.matchStackWithBlock(inv[1], Blocks.dirt))
                    return !ConfigRegistry.enableFermenterYeast()
                        ? null
                        : new ItemStack(ItemRegistry.YEAST.getItemInstance(), 1, 0);
        }
            ...
            if (MulchMaterials.instance.isMulchable(inv[1]))
                if (this.hasWater())
                    return ReikaItemHelper.getSizedItemStack(
                        ItemStacks.sludge, MulchMaterials.instance.getPlantValue(inv[1])
                    );
        }
        return null;
    }
    ...
    private void doOperation(boolean multiple) {
        ...
        if (product.getItem() != ItemRegistry.YEAST.getItemInstance()
            && !ReikaItemHelper.matchStacks(product, ItemStacks.sludge))
            return;
    ...
    public void updateTemperature(World world, int x, int y, int z, int meta) {
        ...
        ForgeDirection waterside
            = ReikaWorldHelper.checkForAdjSourceBlock(world, x, y, z, Material.water);
        if (waterside != null) {
            Tamb -= 5;
        }
        ...
        ForgeDirection lavaside
            = ReikaWorldHelper.checkForAdjSourceBlock(world, x, y, z, Material.lava);
        if (lavaside != null) {
            Tamb += 200;
        }
        if (temperature > Tamb)
    ...
    public void testYeastKill() {
        ...
        int slot = ReikaInventoryHelper.locateInInventory(
            ItemRegistry.YEAST.getItemInstance(), inv
        );
        if (slot != -1) {
        ...
        if (this.hasRedstoneSignal() || !ConfigRegistry.enableFermenterYeast()) {
            switch (i) {
                ...
                case 1:
                    ...
                    ); //ReikaItemHelper.matchStacks(is, ItemStacks.mulch);
            }
        } else {
            switch (i) {
                case 0:
    ...
    public int getNumberConsecutiveOperations() {
        return (int) Math.max(
            1,
            this.getFermentRate() * DurationRegistry.FERMENTER.getNumberOperations(omega)
        );
    }
