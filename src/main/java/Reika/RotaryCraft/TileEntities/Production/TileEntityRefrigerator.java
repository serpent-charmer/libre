...
public class TileEntityRefrigerator extends InventoriedPowerLiquidProducer
    implements MultiOperational, ProcessingMachine, BreakAction {
    public int time;
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (this.canProgress()) {
            ...
            if (soundTimer.checkCap()) {
                SoundRegistry.FRIDGE.playSoundAtBlock(
                    world, x, y, z, RotaryAux.isMuffled(this) ? 0.25F : 1, 0.88F
                );
            }
        } else {
            soundTimer.reset();
    ...
    private boolean canProgress() {
        ...
        if (!tank.canTakeIn(this.getProducedLN2()))
            return false;
        return ReikaItemHelper.matchStackWithBlock(inv[0], Blocks.ice)
            && (inv[1] == null || inv[1].stackSize < inv[1].getMaxStackSize());
    }
    ...
    private void cycle() {
        ...
        if (amt > 0) {
            ...
            if (rand.nextInt(4) == 0) {
                ...
                if (inv[1] != null)
                    n = Math.min(
                        n, ItemStacks.dryice.getMaxStackSize() - inv[1].stackSize
                    );
                ReikaInventoryHelper.addOrSetStack(
                    ReikaItemHelper.getSizedItemStack(ItemStacks.dryice, n), inv, 1
                );
            }
        }
    }
    ...
    public void breakBlock() {
        ...
        if (f > 0.1) {
            ReikaSoundHelper.playSoundAtBlock(
                worldObj, xCoord, yCoord, zCoord, "random.fizz", 1.2F, 0.8F
            );
            float hearts = f * 4;
            ...
            ReikaPacketHelper.sendDataPacketWithRadius(
                RotaryCraft.packetChannel, PacketRegistry.FRIDGEBREAK.ordinal(), this, 24
            );
        }
    }
    ...
    public static void doBreakFX(World world, int x, int y, int z) {
        for (int i = 0; i < 15; i++) {
            ...
            EntityBlurFX fx = new EntityBlurFX(
                ...
                dy * v,
                dz * v,
                IconPrefabs.FADE_GENTLE.getIcon()
            );
            fx.setColor(0xBFB2FF)
                .setScale(3 + rand.nextFloat() * 2)
                .setRapidExpand()
                .setAlphaFading()
                .setLife(30 + rand.nextInt(31))
                .setColliding();
