...
public class TileEntityBorer extends TileEntityBeamMachine
    implements EnchantableMachine, GuiController, DiscreteFunction, IHasWork {
    private final MachineEnchantmentHandler enchantments
        = new MachineEnchantmentHandler()
              .addFilter(Enchantment.fortune)
              .addFilter(Enchantment.efficiency)
              .addFilter(Enchantment.silkTouch)
              .addFilter(Enchantment.sharpness);
    ...
    private int durability
        = ConfigRegistry.BORERMAINTAIN.getState() ? 256 : Integer.MAX_VALUE;
    ...
    protected void onFirstTick(World world, int x, int y, int z) {
        if (anticipationDistance < 0)
            anticipationDistance
                = Math.max(2, Math.max(genRange, this.getServerViewDistance()));
    }
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (enchantments.hasEnchantments()) {
            for (int i = 0; i < 6; i++) {
                world.spawnParticle(
                    "portal",
                    -0.5 + x + 2 * rand.nextDouble(),
                    y + rand.nextDouble(),
                    -0.5 + z + 2 * rand.nextDouble(),
                    0,
                    0,
                    0
                );
        ...
        if (hitProtection && notifiedPlayer < 10) {
            if (world.getTotalWorldTime() % 100 == 0) {
                ...
                if (ep != null) {
                    ...
                    String sg = "Your " + this + " has hit a protected area at " + hx
                        + ", " + hz + " and has jammed.";
        ...
        if (durability <= 0) {
            if (tickcount % 5 == 0) {
                world.playSoundEffect(
                    x + 0.5, y + 0.5, z + 0.5, "mob.blaze.hit", 0.75F, 0.05F
                );
                for (int i = 0; i < 6; i++) {
                    world.spawnParticle(
                        "smoke",
                        x + rand.nextDouble(),
                        y + 1 + rand.nextDouble() * 0.2,
                        z + rand.nextDouble(),
                        0,
                        0,
                        0
        ...
        //ReikaJavaLibrary.pConsole(isMiningAir+":"+tickcount+"/"+this.getOperationTime(),
        //Side.SERVER);

        if (soundtick > 0)
            soundtick--;

        if (!world.isRemote && tickcount >= this.getOperationTime()
            || (isMiningAir && tickcount % 5 == 0)) {
            ...
            if (power >= reqpow && reqpow != -1) {
                ...
                if (!world.isRemote) {
                    for (int i = 0; i <= anticipationDistance; i++) {
                        ReikaWorldHelper.forceGenAndPopulate(
                            world,
                            x + (step + 16 * i) * facing.offsetX,
                            z + (step + 16 * i) * facing.offsetZ,
                            genRange
                        );
                    }
    ...
    public String getCurrentRequiredPower() {
        ...
        return String.format(
            "Required Power: %.3f%sW; Required Torque: %.3f%sNm", d1, s1, d2, s2
        );
    }
    ...
    private void safeDig(World world, int x, int y, int z, int meta) {
        try {
            ...
            RotaryCraft.logger.logError(
                this + " triggered an exception mining a chunk, probably during worldgen!"
            );
            e.printStackTrace();
        }
    }
    ...
    private void
    skipMiningPipes(World world, int x, int y, int z, int meta, int stepped, int max) {
        if (stepped >= max)
            return;
        int a = 0;
        if (meta > 1)
        ...
        for (int i = 0; i < 7; i++) {
            for (int j = 0; j < 5; j++) {
                if (cutShape[i][j] || step == 1) {
                    ...
                    if (world.getBlock(xread, yread, zread)
                        == BlockRegistry.MININGPIPE.getBlockInstance()) {
                        haspipe = true;
                        int meta2 = world.getBlockMetadata(xread, yread, zread);
                        ForgeDirection dir = BlockMiningPipe.getDirectionFromMeta(meta2);
                        if (meta2 == 3
                            || Math.abs(dir.offsetX) == Math.abs(facing.offsetX)
                                && Math.abs(dir.offsetZ) == Math.abs(facing.offsetZ)) {
                        } else {
                            allpipe = false;
                        }
                    }
        ...
        if (!this.ignoreBlockExistence(world, xread, yread, zread)) {
            ...
            if (DragonOptions.DEBUGMODE.getState()) {
                RotaryCraft.logger.log(
                    this + " mined block " + id + ":" + meta + " at " + xread + ", "
                    + yread + ", " + zread + "; pow=" + reqpow + ", torq=" + mintorque
                );
            }
        }
    }
    ...
    public static int calculateTorqueForHardness(float hard, int sharp) {
        ...
        if (sharp > 0) {
            add = Math.min(
                add, ReikaMathLibrary.intpow2(2, 10 - sharp / 3)
            ); //clamp to 1024, halve for each three levels
        }
        return add;
    }
    ...
    private boolean dropBlocks(
        ...
        if (ModList.TWILIGHT.isLoaded()
            && id == TwilightForestHandler.BlockEntry.MAZESTONE.getBlock())
        ...
        if (!world.isRemote
            && !ReikaPlayerAPI.playerCanBreakAt(
                (WorldServer) world, xread, yread, zread, id, meta, this.getServerPlacer()
            )) {
            hitProtection = true;
            return false;
        }
        ...
        if (drops && id != Blocks.air) {
            ...
            if (this.isMineableBedrock(world, xread, yread, zread)) {
                ItemStack is =
            ReikaItemHelper.getSizedItemStack(ItemStacks.bedrockdust.copy(),
            DifficultyEffects.BEDROCKDUST.getInt()); if (!this.chestCheck(world, x, y, z,
            is)) { ReikaItemHelper.dropItem(world, x+0.5, y+1.125, z+0.5, is, 3);
                }
                return true;
            }*/
                ...
                if (spw != null) {
                    ...
                    if (!this.chestCheck(world, x, y, z, is))
                        ReikaItemHelper.dropItem(
                            world, x + 0.5, y + 1.125, z + 0.5, is, 3
                        );
                    return true;
                }
            }
            ...
            if (enchantments.getEnchantment(Enchantment.silkTouch) > 0
                && this.canSilk(world, xread, yread, zread)) {
                ItemStack is = ReikaBlockHelper.getSilkTouch(
                    world, xread, yread, zread, id, meta, this.getPlacer(), false
                ); //new ItemStack(id, 1, ReikaBlockHelper.getSilkTouchMetaDropped(id,
                   //meta));
                if (!this.chestCheck(world, x, y, z, is)) {
            ...
            if (ModList.CHROMATICRAFT.isLoaded()) {
                //fortune +=
                //ChromatiAPI.getAPI().adjacency().getAdjacentUpgradeTier(worldObj,
                //xCoord, yCoord, zCoord, CrystalElementAccessor.getByEnum("PURPLE"))/2;
            }
            Collection<ItemStack> items
                = id.getDrops(world, xread, yread, zread, meta, fortune);
            ...
            if (id instanceof BlockTieredResource) {
                ...
                boolean harvest = ep != null
                    && bt.isPlayerSufficientTier(world, xread, yread, zread, ep);
                items = harvest
                    ? bt.getHarvestResources(world, xread, yread, zread, fortune, ep)
                    : bt.getNoHarvestResources(world, xread, yread, zread, fortune, ep);
            } else if (id instanceof BlockTileEnum) {
                items = ReikaJavaLibrary.makeListFrom(
                    ((BlockTileEnum) id)
                        .getMapping(world, xread, yread, zread)
                        .getCraftedProduct()
                );
            }
            if (items != null) {
    ...
    private void dig(World world, int x, int y, int z, int metadata) {
            ...
            for (int j = 0; j < 5; j++) {
                if (cutShape[i][j] || step == 1) {
                    ...
                    if (this.dropBlocks(
                            ...
                            z,
                            bk,
                            world.getBlockMetadata(xread, yread, zread)
                        )) {
        ...
        MinecraftForge.EVENT_BUS.post(new BorerDigEvent(
            this,
            step,
            x + step * facing.offsetX,
            y + step * facing.offsetY,
            z + step * facing.offsetZ,
            enchantments.hasEnchantment(Enchantment.silkTouch)
        ));
        step++;
    }
    ...
    public int getOperationTime() {
        ...
        float ench = ReikaEnchantmentHelper.getEfficiencyMultiplier(
            enchantments.getEnchantment(Enchantment.efficiency)
        );
