...
public class TileEntityPump extends TileEntityPowerReceiver
    implements PipeConnector, IFluidHandler, DiscreteFunction {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (f == null)
            return;
        if (blocks.isEmpty()) {
            blocks.recursiveAddLiquidWithBounds(
                world, x, y - 1, z, x - 16, y - 2, z - 16, x + 16, y - 1, z + 16, f
            );
            blocks.reverseBlockOrder();
        }
        if (damage > 400)
            power = 0;
        //ReikaJavaLibrary.pConsole(FMLCommonHandler.instance().getEffectiveSide()+" for
        //"+blocks.getSize());
        if (blocks.isEmpty())
            return;
        if (power >= MINPOWER && torque >= MINTORQUE && this.getLevel() < CAPACITY
            && tickcount >= this.getOperationTime()) {
            //int loc[] = this.findSourceBlock(world, x, y, z);
            Coordinate loc = blocks.getNextAndMoveOn();
            //ReikaJavaLibrary.pConsole(loc.xCoord+"  "+loc.yCoord+"  "+loc.zCoord+"  for
    ...
    private void breakPump(World world, int x, int y, int z) {
        ...
        //for (int i = 0; i < 8; i++)
        //	ReikaItemHelper.dropItem(world, x+par5Random.nextDouble(),
        //y+par5Random.nextDouble(), z+par5Random.nextDouble(), ItemStacks.scrap);
    }

    public void harvest(World world, int x, int y, int z, Coordinate loc) {
        if (world.isRemote)
            return;
        FluidStack fs = ReikaWorldHelper.getDrainableFluid(
        ...
        //%d  %d", loc.xCoord, loc.yCoord, loc.zCoord, world.getBlock(loc.xCoord,
        //loc.yCoord, loc.zCoord)));
        if (f != FluidRegistry.LAVA
            || !ReikaWorldHelper.is1p9InfiniteLava(
                world, loc.xCoord, loc.yCoord, loc.zCoord
            ))
    ...
    public boolean isSource(World world, int x, int y, int z) {
        //ModLoader.getMinecraftInstance().ingameGUI.addChatMessage(String.format("%d, %d,
        //%d, %d", x,y,z,(int)id)); ReikaWorldHelper.legacySetBlockWithNotify(world, x, y,
        //z, 49);
        Block liqid = world.getBlock(x, y, z);
        if (!(liqid instanceof BlockFluidBase || liqid instanceof BlockLiquid))
            return false;
        boolean srcmeta = liqid instanceof BlockFluidFinite
        ...
        //ModLoader.getMinecraftInstance().ingameGUI.addChatMessage(String.valueOf(liq)+"
        //"+String.valueOf(dmg0));
        return srcmeta && liq;
    }
