...
public class TileEntityBlastFurnace extends InventoriedRCTileEntity
    implements TemperatureTE, XPProducer, FrictionHeatable, ProcessingMachine {
    ...
    private BlastCrafting getCrafting() {
        ...
        BlastCrafting c
            = RecipesBlastFurnace.getRecipes().getCrafting(center, temperature);
    ...
    private BlastRecipe getRecipe() {
        ItemStack[] center = new ItemStack[9];
        System.arraycopy(inv, 1, center, 0, 9);
        BlastRecipe rec = RecipesBlastFurnace.getRecipes().getRecipe(
            inv[CENTER_ADDITIVE],
            inv[LOWER_ADDITIVE],
            inv[UPPER_ADDITIVE],
            center,
    ...
    private boolean canAdd(ItemStack is, int slot) {
        if (inv[slot] == null)
            ...
            return ReikaItemHelper.areStacksCombinable(
                is, in, this.getInventoryStackLimit()
            );
        }
    }
    ...
    private void make(BlastRecipe rec) {
        ...
        if (rec.bonusYield > 0) {
            double chance = DifficultyEffects.BONUSSTEEL.getDouble()
                * (ReikaMathLibrary.intpow(1.005, num * num) - 1);
        ...
        if (ReikaItemHelper.matchStacks(out, ItemStacks.steelblock)) {
            if (!ReikaInventoryHelper.addOrSetStack(
                    ReikaItemHelper.getSizedItemStack(ItemStacks.steelingot, 2 * num),
                    inv,
                    10
                ))
                if (!ReikaInventoryHelper.addOrSetStack(
                    ...
                    if (!ReikaInventoryHelper.addOrSetStack(
                        ...
                        return;
            if (!ReikaInventoryHelper.addOrSetStack(
                    ReikaItemHelper.getSizedItemStack(ReikaItemHelper.charcoal, 3 * num),
                    inv,
                    10
                ))
                if (!ReikaInventoryHelper.addOrSetStack(
                        ReikaItemHelper.getSizedItemStack(
                            ReikaItemHelper.charcoal, 3 * num
                        ),
                        inv,
                        12
                    ))
                    if (!ReikaInventoryHelper.addOrSetStack(
                        ...
                        return;
            if (!ReikaInventoryHelper.addOrSetStack(
                    new ItemStack(Items.iron_ingot, 5 * num, 0), inv, 10
                ))
                if (!ReikaInventoryHelper.addOrSetStack(
                        new ItemStack(Items.iron_ingot, 5 * num, 0), inv, 12
                    ))
                    if (!ReikaInventoryHelper.addOrSetStack(
                            new ItemStack(Items.iron_ingot, 5 * num, 0), inv, 13
                        ))
                        return;
        } else {
            if (!ReikaInventoryHelper.addOrSetStack(
                    out.getItem(), num, out.getItemDamage(), inv, 10
                ))
                if (!ReikaInventoryHelper.addOrSetStack(
                        out.getItem(), num, out.getItemDamage(), inv, 12
                    ))
                    if (!ReikaInventoryHelper.addOrSetStack(
                            out.getItem(), num, out.getItemDamage(), inv, 13
                        ))
        ...
        for (int i = 0; i < rec.primary.numberToUse; i++) {
            if (ReikaRandomHelper.doWithChance(
                    this.getConsumptionFactor(rec.primary.chanceToUse, made)
                ))
                ReikaInventoryHelper.decrStack(0, inv);
        }
        for (int i = 0; i < rec.secondary.numberToUse; i++) {
            if (ReikaRandomHelper.doWithChance(
                    this.getConsumptionFactor(rec.secondary.chanceToUse, made)
                ))
                ReikaInventoryHelper.decrStack(11, inv);
        }
        for (int i = 0; i < rec.tertiary.numberToUse; i++) {
            if (ReikaRandomHelper.doWithChance(
                    this.getConsumptionFactor(rec.tertiary.chanceToUse, made)
                ))
    ...
    private float getConsumptionFactor(float base, int made) {
        return MathHelper.clamp_float(
            base * made * DifficultyEffects.BLASTCONSUME.getChance(), base, 1
        );
    }
    ...
    public void updateTemperature(World world, int x, int y, int z, int meta) {
        ...
        ForgeDirection iceside
            = ReikaWorldHelper.checkForAdjBlock(world, x, y, z, Blocks.ice);
        if (iceside == null)
            iceside
                = ReikaWorldHelper.checkForAdjBlock(world, x, y, z, Blocks.packed_ice);
        if (iceside != null) {
            if (Tamb > 0)
                Tamb /= 4;
            ReikaWorldHelper.changeAdjBlock(
                world, x, y, z, iceside, Blocks.flowing_water, 0
            );
        }
        int Tadd = 0;
        ...
        if (temperature > 100) {
            ...
            if (side == null)
                side = ReikaWorldHelper.checkForAdjBlock(
                    world, x, y, z, Blocks.snow_layer
                );
            if (side != null)
    ...
    protected void readSyncTag(NBTTagCompound NBT) {
        ...
        lockedSlots
            = ReikaArrayHelper.booleanFromBitflags(NBT.getInteger("locks"), inv.length);
    ...
    private boolean getSlotForItem(int slot, ItemStack is) {
        ...
        if (ItemRegistry.CRAFTPATTERN.matchItem(patt) && slot >= 1 && slot <= 9) {
            return ItemCraftPattern.checkPatternForMatch(
                this, RecipeMode.BLASTFURN, slot, slot - 1, is, patt
            );
        }
        ...
        HashSet<Integer> slots
            = ReikaInventoryHelper.getSlotsBetweenWithItemStack(is, this, 1, 9, false);
        if (!slots.isEmpty()) {
        ...
        if (slot >= 1 && slot <= 9)
            ...
            ); //check this last, since there are fewer variants that go in additives
        return false;
    }
    ...
    public String getOperationalStatus() {
        return this.areConditionsMet()
            ? "Operational"
            : "Insufficient Temperature or Invalid or Missing Items";
    }
