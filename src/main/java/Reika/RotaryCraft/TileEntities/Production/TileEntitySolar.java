...
public class TileEntitySolar extends TileEntityIOMachine
    implements MultiBlockMachine, SimpleProvider, PipeConnector, PowerGenerator,
               IFluidHandler, SolarPlantBlock {
    ...
    public boolean canProvidePower() {
        //return this.isMultiBlock(worldObj, xCoord, yCoord, zCoord) &&
        //this.getMultiBlockPosition(worldObj, xCoord, yCoord, zCoord)[1] == 0;
        return true;
    }
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote) {
                ...
                for (int j = -3; j <= 3; j++) {
                    if (ConfigRegistry.BLOCKDAMAGE.getState()) {
                        ReikaWorldHelper.temperatureEnvironment(
                            world, x + i, y + 1, z + j, Math.min(temp, 1750)
                        );
                        if (temp >= 1500) {
                            this.delete();
            ...
            if (temp > 400) {
                AxisAlignedBB above = AxisAlignedBB.getBoundingBox(
                    x - 3, y + 1, z - 3, x + 4, y + 2, z + 4
                );
                List<EntityLivingBase> in
                    = world.getEntitiesWithinAABB(EntityLivingBase.class, above);
        ...
        if (world.getBlock(x, y - 1, z) == Blocks.air
            || MachineRegistry.getMachine(world, x, y - 1, z) != this.getTile()) {
            //ReikaJavaLibrary.pConsole("TOWER: "+this.getTowerHeight()+";  SIZE:
            //"+this.getArraySize());
        ...
        if (world.getBlock(x, y + 1, z) != Blocks.air
            && !(world.getTileEntity(x, y + 1, z) instanceof SodiumSolarUpgrades)) {
            if (MachineRegistry.getMachine(world, x, y + 1, z) == this.getTile())
                temperature
                    = ((TileEntitySolar) world.getTileEntity(x, y + 1, z)).temperature;
            return;
        }
    ...
    private void generatePower(World world, int x, int y, int z) {
        ...
        //omega =
        //1*ReikaMathLibrary.extrema(ReikaMathLibrary.ceil2exp(this.getTowerHeight()), 8,
        //"min")*(this.getArraySize()+1);
        ...
        if (this.getArraySize() <= 0 || torque == 0 || tank.getLevel() < amt
            || (!water && temperature < 800)) {
            omega = 0;
            torque = 0;
        }
        ...
        if (tank.getActualFluid() == FluidRegistry.getFluid("rc sodium")) {
            amt = (int
            ) Math.max(1, amt * power / ((double) GENOMEGA_SODIUM * MAXTORQUE_SODIUM));
        }
    ...
    private int getGenTorque(World world, int x, int y, int z) {
        ...
        return Math.min(
            cap,
            (int
            ) (f * this.getArrayOverallBrightness() * plant.getTowerMultiplier()
               * (Math.pow(this.getArraySize() + 1, p)))
        );
    }
    ...
    public int getTopOfTower() {
        int y = yCoord;
        while (MachineRegistry.getMachine(worldObj, xCoord, y, zCoord)
               == MachineRegistry.SOLARTOWER) {
            y++;
        }
        return y - 1;
    }
    ...
    public boolean canFill(ForgeDirection from, Fluid fluid) {
        return fluid.equals(FluidRegistry.WATER)
            || (fluid.equals(FluidRegistry.getFluid("rc sodium")) && this.canUseSodium());
    }
