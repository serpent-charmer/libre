...
public class TileEntityAggregator
    extends PoweredLiquidProducer implements BasicTemperatureMachine, TemperatureTE {
    ...
    private void dumpLiquid(World world, int x, int y, int z) {
        for (int i = 2; i < 6; i++) {
            ...
            if (te instanceof TileEntityPipe) {
                ...
                if (dL > 0
                    && (p.getFluidType() == null
                        || tank.getActualFluid().equals(p.getFluidType()))) {
    ...
    private int getOriginalOperationTime() {
        return Math.max(
            0, (int) (80 - 5 * ReikaMathLibrary.logbase(omega + 1 /*-MINSPEED*/, 2))
        );
    }

    private int getOriginalWaterProduced(BiomeGenBase biome) {
        return Math.max(
            2, (int) (torque * torque * ReikaBiomeHelper.getBiomeHumidity(biome))
        );
    }
