...
public class TileEntityMusicBox extends TileEntityPowerReceiver
    implements GuiController, BreakAction, TriggerableAction {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        //ReikaJavaLibrary.pConsole(Arrays.toString(playDelay)+":"+Arrays.toString(playIndex),
        //Side.SERVER);
    ...
    private void playNote(int channel, Note n) {
        if (!n.isRest()) {
            ...
            ReikaPacketHelper.sendUpdatePacket(
                ...
                PacketRegistry.MUSICPARTICLE.ordinal(),
                this,
    ...
    private void dispatchTrack(MusicScore mus) {
        ReikaPacketHelper.sendPacketToServer(
            RotaryCraft.packetChannel, PacketRegistry.MUSICCLEAR.ordinal(), this
        );
            ...
            for (Entry<Integer, NoteData> e : track.entryView()) {
                ...
                for (MusicScore.Note n : c.notes()) {
                    if (n != null && n.key != null) {
                        ...
                        this.sendNote(
                            n2.pitch, i, n2.length, n2.voice
                        ); //ReikaPacketHelper.sendDataPacket(RotaryCraft.packetChannel,
                           //PacketRegistry.FIXEDMUSICNOTE.getMinValue(), this, i,
                           //n.key.ordinal(), n.length, n.voice == -1 ? 1 : 0, time);
                        lastNoteTime = time;
    ...
    public void save() {
        ...
        String name = "musicbox@" + String.format("%d,%d,%d", xCoord, yCoord, zCoord)
            + ".rcmusic";
    ...
    private void readFile(String path, boolean internal) {
        ...
        int linecount = -1;
        try (
            InputStream in = internal ? RotaryCraft.class.getResourceAsStream(path)
        ...
        try {
            for (int i = 0; i < 16; i++) {
                if (is.stackTagCompound.hasKey("ch" + i)) {
                    NBTTagList li
                        = is.stackTagCompound.getTagList("ch" + i, NBTTypes.COMPOUND.ID);
    ...
    public static final class Note {
        ...
        public void play(World world, int x, int y, int z) {
            ...
            switch (voice) {
                case GUITAR:
                    SoundRegistry.getNoteFromVoiceAndPitch(SoundRegistry.HARP, pit)
                        .playSoundAtBlock(world, x, y, z, volume, pitch);
                    break;
                case BASS:
                    SoundRegistry.getNoteFromVoiceAndPitch(SoundRegistry.BASS, pit)
                        .playSoundAtBlock(world, x, y, z, volume, pitch);
                    break;
                case PLING:
                    SoundRegistry.getNoteFromVoiceAndPitch(SoundRegistry.PLING, pit)
                        .playSoundAtBlock(world, x, y, z, volume, pitch);
                    break;
                case BASSDRUM:
                    world.playSoundEffect(
                        x + 0.5, y + 0.5, z + 0.5, "note.bd", volume, pitch
                    );
                    break;
                case SNARE:
                    world.playSoundEffect(
                        x + 0.5, y + 0.5, z + 0.5, "note.snare", volume, pitch
                    );
                    break;
                case CLAVE:
                    world.playSoundEffect(
                        x + 0.5, y + 0.5, z + 0.5, "note.hat", volume, pitch
                    );
                    break;
                default:
                    break;
        ...
        public static Note readFromNBT(NBTTagCompound NBT) {
            ...
            return new Note(
                NoteLength.values()[length], pitch, Instrument.values()[voice]
            );
        }
    ...
    public static enum NoteLength {
        ...
        public static NoteLength getByTickLength(int ticks) {
            ...
            if (len == null) {
                len = WHOLE;
                for (NoteLength n : values()) {
                    if (Math.abs(n.tickLength - ticks)
                        < Math.abs(len.tickLength - ticks)) {
                        len = n;
                    }
                }
            }
    ...
    public void sendNote(int pitch, int channel, NoteLength len, Instrument voice) {
        ReikaPacketHelper.sendPacketToServer(
            RotaryCraft.packetChannel,
            PacketRegistry.MUSICNOTE.ordinal(),
            this,
            pitch,
            channel,
            len.ordinal(),
