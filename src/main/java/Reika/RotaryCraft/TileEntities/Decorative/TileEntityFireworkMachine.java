...
public class TileEntityFireworkMachine extends InventoriedPowerReceiver
    implements EnchantableMachine, DiscreteFunction, ConditionalOperation {
    private final MachineEnchantmentHandler enchantments
        = new MachineEnchantmentHandler().addFilter(Enchantment.infinity);
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
      ...
      if (this.consumeChance())
          star = ReikaInventoryHelper.findAndDecrStack2(Items.firework_charge, -1,
      this.inv); else { int slot =
      ReikaInventoryHelper.locateInInventory(Items.firework_charge, this.inv); if (slot !=
      -1) star = this.getStackInSlot(slot);
      }*/
        ...
        EntityFireworkRocket firework
            = new EntityFireworkRocket(world, x + 0.5, y + 1.25, z + 0.5, rocket);
    ...
    private boolean canMakeRocketFromStar() {
        ...
        boolean hasStar = ReikaInventoryHelper.checkForItem(Items.firework_charge, inv);
        //ModLoader.getMinecraftInstance().thePlayer.addChatMessage(String.valueOf(hasPaper)+"
        //"+String.valueOf(hasGunpowder)+" "+String.valueOf(hasStar));
    ...
    private ItemStack starToRocket(ItemStack star) {
        ...
        if (this.getIngredient(
                Items.gunpowder, this.canMakeRocketFromStar() && this.consumeChance()
            ))
            ingredients[1] = gunpowder;
        if (numgunpowder >= 2
            && this.getIngredient(Items.gunpowder, this.consumeChance()))
    ...
    ) { //Returns a random color dye from inv
        int color = -1;
    ...
    private ItemStack randomRecipe() {
        ReikaDyeHelper dyeColor = this.pickRandomColor(
            this.consumeChance()
            && ReikaInventoryHelper.checkForItem(Items.gunpowder, inv)
        ); //Dye metadata to craft with - 0-15
        boolean hasDiamond = false;
        boolean hasGlowstone = false;
        int shape = this.getShape(
        ); //Shape modifiers - Fire charge, gold nugget, feather, head, or nothing
            ...
            if (var4 >= 1 && var3 == 1 && var7 == 0) {
                ...
                if (var6 > 0) {
                    ...
                    for (int var22 = 0; var22 < inputitems.length; ++var22) {
                        ...
                        if (var26 != null && var26.getItem() == Items.firework_charge
                            && var26.hasTagCompound()
                            && var26.getTagCompound().hasKey("Explosion")) {
                            var25.appendTag(
                                var26.getTagCompound().getCompoundTag("Explosion")
                            );
                        }
                    }
                ...
                for (int var13 = 0; var13 < inputitems.length; ++var13) {
                    ...
                    if (var14 != null) {
                        if (var14.getItem() == Items.dye) {
                            var12.add(Integer.valueOf(
                                ItemDye.field_150922_c[var14.getItemDamage()]
                            ));
                ...
                for (int var20 = 0; var20 < inputitems.length; ++var20) {
                    ...
                    if (var11 != null) {
                        if (var11.getItem() == Items.dye) {
                            var16.add(Integer.valueOf(
                                ItemDye.field_150922_c[var11.getItemDamage()]
                            ));
                ...
                if (field_92102_a != null && field_92102_a.hasTagCompound()) {
                    NBTTagCompound var23
                        = field_92102_a.getTagCompound().getCompoundTag("Explosion");
