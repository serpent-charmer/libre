...
public class TileEntityProjector
    extends InventoriedPowerReceiver implements RangedEffect {
    ...
    public int getRange() {
        ...
        switch (this.getBlockMetadata()) {
            case 0:
                x = xCoord - 1;
                while (x >= xCoord - 12
                       && worldObj.getBlock(x, yCoord, zCoord) == Blocks.air)
                    x--;
                return x - xCoord + 1;
            case 1:
                x = xCoord + 1;
                while (x <= xCoord + 12 + 1
                       && (worldObj.getBlock(x, yCoord, zCoord) == Blocks.air
                           || worldObj.getBlock(x, yCoord, zCoord)
                                  .isAir(worldObj, x, yCoord, zCoord)))
                    x++;
                return -(x - xCoord);
            case 2:
                z = zCoord + 1;
                while (z <= zCoord + 1 + 12
                       && worldObj.getBlock(xCoord, yCoord, z) == Blocks.air)
                    z++;
                return -(z - zCoord);
            case 3:
                z = zCoord - 1;
