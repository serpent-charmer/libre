...
public class TileEntityChunkLoader extends TileEntityPowerReceiver
    implements ChunkLoadingTile, BreakAction, RangedEffect {
    ...
    private void doParticles(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            EntityFX fx = new EntityEnderFX(world, px, py, pz, 0, vy, 0, 0xffffff)
                              .setRapidExpand()
                              .setScale(s)
                              .setIcon(Items.nether_star.getIconFromDamage(0));
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);

            a += 180;
            px = x + 0.5 + dr * Math.cos(Math.toRadians(a));
            pz = z + 0.5 + dr * Math.sin(Math.toRadians(a));
            fx = new EntityEnderFX(world, px, py, pz, 0, vy, 0, 0xffffff)
                     .setRapidExpand()
                     .setScale(s);
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
        }
