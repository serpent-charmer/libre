...
public class TileEntityFuelConverter extends InventoriedPoweredLiquidIO {
    ...
    private static final MultiMap<String, FuelConversion> conversionOutputMap
        = new MultiMap();

    public static FuelConversion addRecipe(
        String in,
        String out,
        int speed,
        int fluidRatio,
        double itemConsumeChance,
        ItemMatch... items
    ) {
    ...
    public static final class FuelConversion {
        public static final FuelConversion BCFUEL = new FuelConversion(
            ...
            1,
            4,
            DifficultyEffects.CONSUMEFRAC.getChance() / 100D * 1.5,
            new ItemMatch(Items.blaze_powder),
            new ItemMatch(ItemStacks.netherrackdust),
            new ItemMatch(ItemStacks.tar),
            new ItemMatch(Items.magma_cream),
            new ItemMatch(ReikaItemHelper.pinkDye)
        );
        public static final FuelConversion KEROSENE = new FuelConversion(
            ...
            1,
            4,
            DifficultyEffects.CONSUMEFRAC.getChance() / 100D,
            new ItemMatch(Items.blaze_powder),
            new ItemMatch(ItemStacks.netherrackdust),
            new ItemMatch(ItemStacks.tar),
            new ItemMatch(Items.magma_cream),
        ...
        private FuelConversion(
            String in, String out, int sp, int r, double f, ItemMatch... items
        ) {
        ...
        public String toString() {
            return input + ">" + output + " x " + itemConsumptionChance + " @ "
                + fluidRatio + ":1 %" + itemConsumptionChance;
        }
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        //ReikaJavaLibrary.pConsoleSideOnly("BC: "+this.getBCFuel()+"    JET:
        //"+this.getJetFuel(), Side.CLIENT);
        ...
        if (c != null) {
            ...
            for (FuelConversion fc : c) {
                ...
                if (this.getInputLevel() >= fc.fluidRatio * spd
                    && (fc.condition == null || fc.condition.isUsable(this))
                    && output.canTakeIn(spd) && this.hasItems(fc)) {
    ...
    private Collection<FuelConversion> getConversionOptions() {
        return !input.isEmpty() ? conversionMap.get(input.getActualFluid().getName())
                                : null;
    }
        ...
        for (int i = 0; i < c.ingredients.length; i++) {
            if (ReikaRandomHelper.doWithChance(c.itemConsumptionChance))
                ReikaInventoryHelper.decrStack(
                    ReikaInventoryHelper.locateInInventory(c.ingredients[i], inv), inv
                );
        }
    }
