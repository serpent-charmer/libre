...
public class TileEntityPurifier extends InventoriedPowerReceiver
    implements TemperatureTE, DiscreteFunction, ConditionalOperation {
    ...
    private void smelt() {
        ...
        ReikaInventoryHelper.addOrSetStack(
            ItemStacks.steelingot.getItem(),
            count,
            ItemStacks.steelingot.getItemDamage(),
            inv,
            6
        );
    ...
    private boolean isModSteel(ItemStack is) {
        ...
        for (ItemStack s : steel) {
            if (is.getItem() == s.getItem()
                && (is.getItemDamage() == s.getItemDamage() || !s.getHasSubtypes()))
                return true;
        }
        return false;
    }
    ...
    public void overheat(World world, int x, int y, int z) {
        ...
        ReikaWorldHelper.overheat(
            world, x, y, z, ItemStacks.scrap.copy(), 0, 7, false, 1F, false, true, 2F
        );
    }
