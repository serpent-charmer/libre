...
public class TileEntityExtractor extends InventoriedPowerLiquidReceiver
    implements ConditionalOperation, HiddenInventorySlot, IHasWork {
    ...
    public boolean upgrade() {
        ...
        if (inv[9] != null)
            ReikaItemHelper.dropItem(
                worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, inv[9]
            );
    ...
    private int getSmeltNumber(int stage, OreType ore, ItemStack is) {
        ...
        if (ore != null) {
            ...
            boolean nether
                = ore instanceof ModOreList && ((ModOreList) ore).isNetherOres();
            if (is.getItemDamage() == 1
                && (ore == ModOreList.FORCE || ore == ModOreList.MIMICHITE))
                nether = true;
            if (ReikaItemHelper.matchStackWithBlock(
                    is, MagicCropHandler.getInstance().netherOreID
                ))
                nether = true;
            ...
            if (inv[i + 3] != null) {
                if (inv[i] == null) {
                    ...
                    if (ReikaItemHelper.matchStacks(inv[i], inv[i + 3])) {
                        int amt = Math.min(
                            inv[i + 3].stackSize,
                            inv[i].getMaxStackSize() - inv[i].stackSize
                        );
    ...
    private boolean canProcess(int i) {
        if (power < machine.getMinPower(i) || omega < machine.getMinSpeed(i)
            || torque < machine.getMinTorque(i))
            return false;

        if (i == 0 && !bedrock && drillTime <= 0
        ...
        if (inv[i] == null)
            return false;
        if (inv[i + 4] != null
            && inv[i + 4].stackSize + 1 >= inv[i + 4].getMaxStackSize())
            return false;
        if (inv[8] != null) {
        ...
        if (inv[i + 4].stackSize < this.getInventoryStackLimit()
            && inv[i + 4].stackSize < inv[i + 4].getMaxStackSize())
            return true;
    ...
    private void processItem(int i) {
        ...
        //"+ReikaItemHelper.matchStacks(inv[i+4], itemstack)); ReikaOreHelper ore = i == 0
        //? ReikaOreHelper.getFromVanillaOre(inv[i].getItem()) :
    ...
    private boolean processModOre(int i) {
        if (this.isValidModOre(inv[i])) {
            ...
            if (ModOreList.isModOre(inv[i]) && i == 0) {
                ...
                if (ReikaInventoryHelper.addOrSetStack(is.getItem(),
    this.getSmeltNumber(i, m, inv[0]), is.getItemDamage(), inv, i+4)) {
                ...
                if (ExtractorModOres.isDust(m, inv[i].getItemDamage()) && i == 1) {
                    ...
                    if (ReikaInventoryHelper.addOrSetStack(is.getItem(),
    this.getSmeltNumber(i, m, inv[i]), is.getItemDamage(), inv, i+4)) {
                        ReikaInventoryHelper.decrStack(i, inv);
    ...
    public boolean isItemValidForSlot(int slot, ItemStack is) {
        ...
        if (slot == 9)
            return !bedrock && ConfigRegistry.EXTRACTORMAINTAIN.getState()
