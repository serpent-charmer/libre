...
public class TileEntityDistillery extends PoweredLiquidIO {
    ...
    public boolean canIntakeFromPipe(MachineRegistry p) {
        return p.isStandardPipe() || p == MachineRegistry.HOSE
            || p == MachineRegistry.FUELLINE;
    }

    @Override
    public boolean canOutputToPipe(MachineRegistry p) {
        return p == MachineRegistry.HOSE || p.isStandardPipe()
            || p == MachineRegistry.FUELLINE;
    }
    ...
    private static enum Conversion {
        ...
        public String toString() {
            ...
            return name1 + " (" + this.getRequiredAmount() + " mB) + [" + minTorque
                + " Nm & " + minPower + "W] -> " + name2 + " ("
                + this.getProductionAmount() + " mB)";
        }
    }
