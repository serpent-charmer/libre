...
public class TileEntityAutoCrafter extends InventoriedPowerReceiver implements IActionHost/*, ICraftingRequester*/ {
    ...
    private static final HashMap<KeyedItemStack, CraftingLoopCache> loopCache
        = new HashMap();
    ...
    public TileEntityAutoCrafter() {
        if (ModList.APPENG.isLoaded()) {
            ...
            aeGridNode = FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER
                ? AEApi.instance().createGridNode((IGridBlock) aeGridBlock)
                : null;
    ...
    public static enum CraftingMode {
        ...
        CONTINUOUS(
            "Continuous",
            "Crafts continuously as long as there are ingredients",
            0x00aaff,
            "2"
        ),
        SUSTAIN(
            "Sustain", "Tries to sustain a given number of a certain item", 0xbb22ff, "4"
        );
        ...
        if (ModList.APPENG.isLoaded() && network != null) {
            for (int i = 0; i < SIZE; i++) {
                ...
                if (is != null) {
                    ...
                    if (missing > 0) {
                        //ReikaJavaLibrary.pConsole("Crafting missing "+is+" x "+missing+"
                        //(has "+has+")"); lock[i] = true;
                        //network.triggerCrafting(worldObj, is, missing, null, new
                        //CraftingLock(i));

                        //int num = Math.min(is.getMaxStackSize(), (int)Math.min(missing,
                        //Integer.MAX_VALUE));
    ...
    private void profileCraftingTime(long start) {
        ...
        if (ConfigRegistry.CRAFTERPROFILE.getState() && duration > 1000000 * tickTimer
            && tickTimer < MAX_TICK_DELAY) {
    ...
    public void triggerCrafting(int slot, int amt) {
        if (power >= MINPOWER) {
            ...
            if (out != null) {
                int space =
    inv[slot+OUTPUT_OFFSET].getMaxStackSize()-inv[slot+OUTPUT_OFFSET].stackSize; int
    tocraft = ReikaMathLibrary.multiMin(amt, this.getInventoryStackLimit(),
    out.getMaxStackSize(), space); int cycles = tocraft/out.stackSize; for (int i = 0; i <
    cycles; i++) { boolean flag = this.attemptSlotCrafting(slot); if (!flag) break;
                }
            }
        }
    }
    ...
    private ItemStack[] getIngredients(ItemStack is) {
        if (is.getItem() == ItemRegistry.CRAFTPATTERN.getItemInstance()
    ...
    private ItemStack getOutput(ItemStack is) {
        if (is.getItem() == ItemRegistry.CRAFTPATTERN.getItemInstance()
            && is.stackTagCompound != null
            && ItemCraftPattern.getMode(is) == RecipeMode.CRAFTING) {
    ...
    private boolean tryCrafting(int i, ItemStack out, ItemStack[] items, int d) {
        ...
        if (inv[slot] == null
            || (ReikaItemHelper.matchStacks(out, inv[slot])
                && size + out.stackSize <= out.getMaxStackSize())) {
            if (inv[i + CONTAINER_OFFSET] == null) {
                ...
                for (int k = 0; k < 9; k++) {
                    if (items[k] != null) {
                        ...
                        counts.put(
                            items[k], val + 1
                        ); // items[k].stackSize ? no, recipes take 1 per slot
                    }
                }
                for (ItemStack is : counts.keySet()) {
                    if (!ReikaItemHelper.matchStacks(out, is)) {
                        ...
                        int missing = req - has;
                        //ReikaJavaLibrary.pConsole("need "+req+" / have "+has+" '"+is+"
                        //("+is.getDisplayName()+")'; making '"+out+"
                        //("+out.getDisplayName()+")'");
                        if (missing > 0) {
                            if (d < 40) {
                                //ReikaJavaLibrary.pConsole("Going to attempt intermediate
                                //crafting. Can? "+this.canCraftIntermediates(out,
                                //counts));
                                ...
                                if (!this.tryCraftIntermediates(missing, is, d + 1)) {
                                    //ReikaJavaLibrary.pConsole("Failed to craft
                                    //intermediates ("+is+" x "+missing+")");
                                    //ReikaJavaLibrary.pConsole("missing "+missing+":
                                    //"+options.get(is)+", needed "+req+", had "+has);
                                    return false;
                                }
                            } else {
                                //ReikaJavaLibrary.pConsole("Recursed too deep to craft
                                //intermediates");
                                return false;
                            }
                        }
    ...
    private boolean canCraftIntermediates(ItemStack out, ItemHashMap<Integer> req) {
        ...
        if (this.isLoopable(out, req))
            return false;
        if (out.getItem().getClass().getName().equals("ItemReactorCondensator"
    ...
    private boolean isLoopable(ItemStack out, ItemHashMap<Integer> req) {
        ...
        if (seek == null) {
            ...
            cache.loopingSets.put(
                new HashSet(set), seek
            ); //clone set to avoid it being modified
        }
        return seek.booleanValue();
    }

    private KeyedItemStack key(ItemStack is) {
        return new KeyedItemStack(is)
    ...
    private boolean calculateLoopability(
        ItemStack out, Collection<ItemStack> c
        ...
        for (ItemStack is : c) {
            ...
            for (IRecipe ir : li) {
                if (ReikaItemHelper.collectionContainsItemStack(
                        ReikaRecipeHelper.getAllItemsInRecipe(ir), out
                    ))
                    return true;
            }
        }
    ...
    private int getAvailableIngredients(ItemStack is
        ...
        int count = 0;
        //ItemHashMap<Long> map = ModList.APPENG.isLoaded() && network != null ?
        //network.getMESystemContents() : null; ReikaJavaLibrary.pConsole(map); for
        //(ItemStack is : li) {
        //ReikaJavaLibrary.pConsole(is+":"+ingredients.getItemCount(is)+" >
        //"+ingredients);
        count += ingredients.getItemCount(is);
        if (ModList.APPENG.isLoaded() && network != null) {
            count
                += network.getItemCount(is, false); //network.removeItem(ReikaItemHelper.getSizedItemStack(is,
                                                    //is.getMaxStackSize()), true, false);
        }
        //}
    ...
    private boolean tryCraftIntermediates(int num, ItemStack is, int d) {
        ...
        for (int i = 0; i < SIZE && run < num; i++) {
            ...
            if (out != null && ReikaItemHelper.matchStacks(is, out)) {
                //ReikaJavaLibrary.pConsole("Intermediate crafting: attempting slot "+i+",
                //because "+out+" matches "+is);
                    ...
                    if (dec > 0) {
                        ...
                        if (ModList.APPENG.isLoaded()) {
                            if (diff > 0 && network != null) {
                                ...
                                is2c.stackSize = diff;
                                dec -= network.removeItem(
                                    is2, false, false
                                ); //network.removeFromMESystem(is2, diff);
                            }
                        }
                    }
                    if (dec <= 0)
    ...
    public int getSizeInventory() {
        return SIZE * 3; //18+18+18; //18 for patterns, 18 for output, additional 18 for
                         //container items; last 18 is hidden
    }
    ...
    public boolean isItemValidForSlot(int i, ItemStack is) {
        return i < SIZE && ItemRegistry.CRAFTPATTERN.matchItem(is)
            && ItemCraftPattern.getMode(is) == RecipeMode.CRAFTING
            && ItemCraftPattern.getRecipeOutput(is) != null;
    }
    ...
    @Override
    @ModDependent(ModList.APPENG)
    public IAEItemStack injectCraftedItems(ICraftingLink link, IAEItemStack items,
    Actionable mode) { return items;
    }
