...
public class TileEntityWetter
    extends InventoriedPowerLiquidReceiver implements OneSlotMachine {
    private int tick = 0;
    ...
    private void updateItem() {
        if (item == null && inv[0] == null)
            return;
        if ((item == null && inv[0] != null) || (item != null && inv[0] == null)
            || !ReikaItemHelper.matchStacks(item.getEntityItem(), inv[0])) {
    ...
    public boolean canConnectToPipe(MachineRegistry m) {
        return m == MachineRegistry.HOSE || m == MachineRegistry.FUELLINE
            || m.isStandardPipe();
    }
