...
public class TileEntityGrinder extends InventoriedPowerReceiver
    implements PipeConnector, IFluidHandler, MultiOperational, ProcessingMachine,
               DamagingContact, EnchantableMachine {
    private final MachineEnchantmentHandler enchantments
        = new MachineEnchantmentHandler()
              .addFilter(Enchantment.looting)
              .addFilter(Enchantment.knockback)
              .addFilter(Enchantment.flame)
    ...
    private static final int MIN_LUBE_PRODUCTION
        = DifficultyEffects.CANOLA.getAverageAmount();
    ...
    public void readPower() {
        ...
        //ReikaMathLibrary.extrema(2, 1200-this.omega, "max")));
    }
    ...
    private boolean canGrind() {
        ...
        return inv[1].stackSize + out.stackSize
            <= Math.min(this.getInventoryStackLimit(), out.getMaxStackSize());
    }
    ...
    private void grind() {
        ...
        if (is != null && isGrindableSeed(is)) {
            ...
            tank.addLiquid(
                (int
                ) (DifficultyEffects.CANOLA.getInt() * this.getFortuneLubricantFactor()
                   * num),
    ...
    private float getFortuneLubricantFactor() {
        return 1F
            + (float
            ) (enchantments.getEnchantment(Enchantment.fortune)
               * ReikaRandomHelper.getRandomBetween(0.1, 0.2));
    }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack is) {
        ...
        return is.getItem() == ItemRegistry.CANOLA.getItemInstance()
            || RecipesGrinder.getRecipes().isGrindable(is);
    }
