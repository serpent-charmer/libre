...
public class TileEntityCentrifuge extends InventoriedPowerReceiver
    implements MultiOperational, ProcessingMachine, IFluidHandler, PipeConnector,
               EnchantableMachine {
    ...
    private final MachineEnchantmentHandler enchantments
        = new MachineEnchantmentHandler()
              ...
              .addFilter(Enchantment.field_151370_z);
            ...
            if (multiple || progressTime >= this.getOperationTime()) {
                ...
                for (int i = 0; i < out.maxStack && inv[0] != null; i++) {
                    if (this.canMakeAllOf(items)) {
                        ...
                        double enchantBonus = 1
                            + 0.1
                                * enchantments.getEnchantment(Enchantment.field_151370_z);
                        if (fs != null) {
                            fs = fs.copy();
                        ...
                        if (fs == null || tank.canTakeIn(fs)) {
                            ...
                            for (ItemStack is : c) {
                                ReikaInventoryHelper.addToIInv(
                                    is, this, true, 1, this.getSizeInventory()
                                );
                            }
