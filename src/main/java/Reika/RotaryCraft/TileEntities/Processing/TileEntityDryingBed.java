...
public class TileEntityDryingBed extends InventoriedRCFluidReceiver {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ItemStack is = tank.isEmpty()
            ? null
            : RecipesDryingBed.getRecipes().getDryingResult(tank.getFluid());
        if (this.canMake(is)) {
            timer.update();
            if (timer.checkCap()) {
    ...
    private boolean canMakeMore(ItemStack is) {
        if (inv[0] == null)
            return true;
        return ReikaItemHelper.matchStacks(is, inv[0])
            && inv[0].stackSize + is.stackSize <= inv[0].getMaxStackSize();
    }
