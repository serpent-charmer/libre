...
public class TileEntityCrystallizer extends InventoriedPowerLiquidReceiver
    ...
    private void doOperation(boolean multiple) {
        if (!tank.isEmpty()) {
            ItemStack toMake
                = RecipesCrystallizer.getRecipes().getFreezingResult(tank.getFluid());
    ...
    private boolean canOperate(ItemStack toMake) {
        ...
        return ReikaItemHelper.areStacksCombinable(
            toMake, inv[0], this.getInventoryStackLimit()
        );
    }
    ...
    public int getOperationTime() {
        return DurationRegistry.CRYSTALLIZER.getOperationTime(
            Math.max(0, omega - MINSPEED)
        );
    }

    @Override
    public int getNumberConsecutiveOperations() {
        return DurationRegistry.CRYSTALLIZER.getNumberOperations(
            Math.max(0, omega - MINSPEED)
        );
    }
