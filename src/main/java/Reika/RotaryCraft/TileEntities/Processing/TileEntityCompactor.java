...
public class TileEntityCompactor extends InventoriedPowerReceiver
    implements TemperatureTE, PressureTE, FrictionHeatable, MultiOperational,
               ProcessingMachine {
    /** The number of ticks that the current item has been cooking for */
    public int compactorCookTime;

    public static final int MAXTEMP = 1000;
    public static final int MAXPRESSURE
        = 600000; //All pressures in kPa, steel yield strength = 250Mpa
    ...
    public void readPower() {
        ...
        //Math.max(2, 1200-this.omega, "max")));
        return;
    }
    ...
    public void updatePressure(World world, int x, int y, int z, int meta) {
        ...
        if (pressure >= 0.8 * MAXPRESSURE) {
            RotaryCraft.logger.warn(
                "WARNING: " + this + " is reaching very high pressure!"
            );
        }
    ...
    private boolean canSmelt() {
        ...
        if (pressure < RecipesCompactor.getRecipes().getReqPressure(inv[0])
            || temperature < RecipesCompactor.getRecipes().getReqTemperature(inv[0]))
            return false;
        ...
        if (inv[4] == null)
            return true;
        if (inv[4].stackSize < this.getInventoryStackLimit()
            && inv[4].stackSize < inv[4].getMaxStackSize())
            return true;
        return false;
    }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack is) {
        ...
        if (is.getItem() == Items.coal
            || (ItemRegistry.COMPACTS.matchItem(is)
                && is.getItemDamage() <= ItemStacks.lonsda.getItemDamage()))
            return true;
