...
public class TileEntityDropProcessor extends InventoriedPowerReceiver
    implements ProcessingMachine, MultiOperational, EnchantableMachine {
    ...
    private boolean doOperation(World world, int x, int y, int z, boolean multiple) {
        if (inv[0] != null) {
            if (inv[1] == null || this.canContinueProcessingWithOutput()) {
                ...
                dropProcessTime = Math.max(
                    0, Math.min(dropProcessTime, this.getOperationTime() - 1)
                ); //0;
                return false;
            }
        } else {
    ...
    private boolean canContinueProcessingWithOutput() {
        ...
        if (h.allowsStacking()) {
            ...
            if (ReikaItemHelper.matchStacks(is, inv[1])
                && is.stackSize + inv[1].stackSize
    ...
    private Collection<ItemStack> runHandler(DropProcessing dp, ItemStack in) {
        try {
            ...
            return dp.generateItems(
                worldObj, xCoord, yCoord, zCoord, fortune, ep, rand, in
            );
        } catch (Exception e) {
            e.printStackTrace();
            this.writeError(e);
    ...
    public static Collection<DropProcessing> getAllHandlersProducing(ItemStack is) {
        ...
        for (DropProcessing p : dropHandlers) {
            if (!(p instanceof BlockDropProcessing)) //glitchy
                if (ReikaItemHelper.listContainsItemStack(
                        p.getPotentialOutputsForDisplay(), is, false
                    ))
                    li.add(p);
        }
        return li;
    ...
    public boolean areConditionsMet() {
        return inv[0] != null && ReikaItemHelper.isBlock(inv[0]) && inv[1] == null
            && overflow.isEmpty();
    }
    ...
    public void dropCache() {
        ReikaItemHelper.dropItems(
            worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, overflow
        );
        overflow.clear();
    }
    ...
    public static abstract class DropProcessing {
        ...
        protected abstract Collection<ItemStack> generateItems(
            World world,
            int x,
            int y,
            int z,
    ...
    private static class NoProcessing extends DropProcessing {
        ...
        @Override
        protected Collection<ItemStack> generateItems(
            World world,
            int x,
            int y,
            int z,
    ...
    private static class BlockDropProcessing extends DropProcessing {
        ...
        protected Collection<ItemStack> generateItems(
            ...
            ArrayList<ItemStack> li
                = b.getDrops(world, x, y, z, src.getItemDamage(), fortune);
            return li;
        }
        ...
        public ArrayList<ItemStack> getAllInputsForDisplay() {
            ...
            for (Object o : Block.blockRegistry.getKeys()) {
                ...
                if (!ReikaBlockHelper.alwaysDropsSelf(b)
                    && b.getItemDropped(0, rand, 0) != null
                    && b.getItemDropped(0, rand, 0)
                        != Item.getItemFromBlock(Blocks.fire)) {
        ...
        public List<ItemStack> getOutputsOfInputForDisplay(ItemStack src) {
            ...
            return ReikaJavaLibrary.makeListFrom(new ItemStack(
                b.getItemDropped(src.getItemDamage(), rand, 0),
                b.quantityDropped(src.getItemDamage(), 0, rand),
                b.damageDropped(src.getItemDamage())
            ));
        }
    ...
    private static class ScrapBoxProcessing extends DropProcessing {
        ...
        public List<ItemStack> getOutputsOfInputForDisplay(ItemStack src) {
            ...
            for (ItemStack is : map.keySet()) {
                ...
                is2.stackTagCompound.setString(
                    "dropTooltip", String.valueOf(map.get(is) * 100) + "%"
                );
                c.add(is2);
            }
            return c;
    ...
    private static class HardOreProcessing extends DropProcessing {
        ...
        private ItemStack getItem(ItemStack src) {
            return ReikaItemHelper.getSizedItemStack(
                HardOresHandler.instance.getRootOre(src),
                HardOresHandler.instance.getYield(src)
            );
        }
    ...
    private static class ThaumBagProcessing extends DropProcessing {
        ...
        protected void create() {
            try {
                ...
                getToolLoot
                    = c.getDeclaredMethod("getGearItemForSlot", int.class, int.class);
        ...
        protected Collection<ItemStack> generateItems(
            ...
            for (int i = 0; i < n; i++) {
                ItemStack is
                    = (ItemStack) generateBagLoot.invoke(null, src.getItemDamage(), rand);
                if (is != null) {
                    li.add(is);
                }
            }
                ...
                for (WeightedRandomLoot w : c) {
                    if (w.item == null) {
                        throw new InstallationException(
                            ModList.THAUMCRAFT.getDisplayName(),
                            "You have specified a null item in the TC loot bags! Check your configs or MT scripts!"
                        );
                    }
                    ItemStack is = w.item.copy();
                    if (is.stackTagCompound == null)
                        is.stackTagCompound = new NBTTagCompound();
                    is.stackTagCompound.setString(
                        "dropTooltip",
                        String.valueOf(100F * w.itemWeight / totalWeight) + "%"
                    );
                    li.add(is);
                }
            }
                ...
                if (src.getItemDamage() > 0) {
                    ...
                    for (int slot = 0; slot <= 4; slot++) {
                        for (int quality = 0; quality <= 6; quality++) {
                            ...
                            is.stackTagCompound = new NBTTagCompound();
                            is.stackTagCompound.setString(
                                "dropTooltip", "~" + String.valueOf(100F * ch) + "%"
                            );
                            li.add(is);
                        }
                    }
    ...
    private static class CratedItemProcessing extends DropProcessing {
        ...
        protected void create() {
            try {
                ...
                RotaryCraft.logger.logError(
                    "Could not initialize Foestry crated item processing:"
                );
                e.printStackTrace();
                ReflectiveFailureTracker.instance.logModReflectiveFailure(
                    ModList.FORESTRY, e
        ...
        protected Collection<ItemStack> generateItems(
            ...
            return is != null
                ? ReikaJavaLibrary.makeListFrom(ReikaItemHelper.getSizedItemStack(is, 9))
                : new ArrayList();
        }
        ...
        public List<ItemStack> getOutputsOfInputForDisplay(ItemStack src) {
            try {
                return (ArrayList<ItemStack>) this.generateItems(
                    null, 0, 0, 0, 0, null, rand, src
                );
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
    ...
    private static class MystNotebookProcessing extends MystFolderProcessing {
        ...
        protected void create() {
            try {
                ...
                RotaryCraft.logger.logError(
                    "Could not initialize Mystcraft notebook processing:"
                );
                e.printStackTrace();
                ReflectiveFailureTracker.instance.logModReflectiveFailure(
                    ModList.MYSTCRAFT, e
        ...
        protected Collection<ItemStack> generateItems(
            ...
            ) generate.invoke(null, null, rand, 7, 4, 4, 1); //from internal myst code
            if (MystFolderProcessing.instance.isValidItem(is)) {
                return MystFolderProcessing.instance.generateItems(
                    world, x, y, z, fortune, ep, rand, is
                );
            } else {
        ...
        public ArrayList<ItemStack> getAllInputsForDisplay() {
            return ReikaJavaLibrary.makeListFrom(
                new ItemStack(MystCraftHandler.getInstance().sealedBookID)
            );
        }
        ...
        public Collection<ItemStack> getPotentialOutputsForDisplay() {
            ...
            ); //ReikaJavaLibrary.makeListFrom(new
               //ItemStack(MystCraftHandler.getInstance().folderID));
        }
    }
    ...
    private static class MystFolderProcessing extends DropProcessing {
        ...
        public ArrayList<ItemStack> getAllInputsForDisplay() {
            return ReikaJavaLibrary.makeListFrom(
                new ItemStack(MystCraftHandler.getInstance().folderID)
            );
        }
        ...
        public final List<ItemStack> getOutputsOfInputForDisplay(ItemStack src) {
            try {
                return (List<ItemStack>) this.generateItems(
                    null, 0, 0, 0, 0, Minecraft.getMinecraft().thePlayer, rand, src
                );
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
    ...
    private static class GregCrateProcessing extends DropProcessing {
        ...
        protected void create() {
            ...
            this.addCrate(
                "gregtech:gt.metaitem.03:1031", "gregtech:gt.metaitem.01:11031"
            );
            this.addCrate(
                "gregtech:gt.metaitem.03:1300", "gregtech:gt.metaitem.01:11300"
            );
            this.addCrate(
                "gregtech:gt.metaitem.03:1335", "gregtech:gt.metaitem.01:11335"
            );
            this.addCrate(
                "gregtech:gt.metaitem.03:1305", "gregtech:gt.metaitem.01:11305"
            );
