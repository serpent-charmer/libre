...
public class TileEntityPulseFurnace extends InventoriedPowerReceiver
    implements TemperatureTE, PipeConnector, IFluidHandler, ProcessingMachine,
               TemperatureCallback {
    public int pulseFurnaceCookTime;

    public static final int CAPACITY = 3000;
    public static final int MAXFUEL = 8000;
    public static final int MAXTEMP
        = 1000; //1370C = melting steel, 800C = 90% strength loss
    ...
    public void updateTemperature(World world, int x, int y, int z, int meta) {
        ...
        if (temperature > 915) {
            RotaryCraft.logger.warn(
                "WARNING: " + this + " is reaching very high temperature!"
            );
            world.spawnParticle(
                "lava",
                x + rand.nextFloat(),
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote) {
            int tick = 1;
            if (!fuel.isEmpty() && power > 0 && omega >= MINSPEED
                && accel.getLevel() > 10) {
                tick = 4;
            ...
            //%d  %d", this.power, this.omega, this.torque));
            ...
            if (recipe != null && smelttick >= duration) {
                ...
                //ModLoader.getMinecraftInstance().ingameGUI.addChatMessage(String.format("%d",
                //ReikaMathLibrary.extrema(2, 600-this.omega, "max")));
    ...
    /**
     * Returns true if the furnace can smelt an item, i.e. has a source item, destination
     * stack isn't full, etc.
     */
    private PulseJetRecipe getRecipe() {
        ...
        if (inv[2] != null
            && !ReikaItemHelper.areStacksCombinable(
                inv[2], rec.getOutput(), this.getInventoryStackLimit()
            ))
            return null;
    ...
    /**
     * Turn one item from the furnace source stack into the appropriate smelted item in
     * the furnace result stack
     */
    private void smeltItem(PulseJetRecipe rec) {
     ...
     private void smeltScrap() {
         int size = 1;
         if (inv[0].getItem == ItemStacks.scrap.itemID && inv[0].getItemDamage() ==
     ItemStacks.scrap.getItemDamage()) size = 9; inv[0].stackSize -= size; ItemStack i =
     this.getCraftedScrapIngot(); ReikaInventoryHelper.addOrSetStack(i.itemID, 1,
     i.getItemDamage(), inv, 2); if (inv[0].stackSize <= 0) inv[0] = null;
    ...
    public void overheat(World world, int x, int y, int z) {
        ReikaWorldHelper.overheat(
            world,
            x,
            y,
            z,
            ItemStacks.scrap.copy(),
            0,
            17,
            true,
            1.5F,
    ...
    public String getOperationalStatus() {
        return fuel.isEmpty()         ? "No Fuel"
            : this.areConditionsMet() ? "Operational"
                                      : "Invalid or Missing Items";
    }
