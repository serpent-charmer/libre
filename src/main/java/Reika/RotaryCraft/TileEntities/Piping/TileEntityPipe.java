...
public class TileEntityPipe extends TileEntityPiping implements TemperatureTE {
    ...
    public static final int HORIZLOSS
        = 1 * 0; // all are 1(friction)+g (10m) * delta h (0 or 1m)
    ...
    public final void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        //if (ModList.BCFACTORY.isLoaded() && ModList.REACTORCRAFT.isLoaded()) { //Only
        //if, since need a way to pipe it
        ...
        if (rand.nextInt(60) == 0) {
            ReikaWorldHelper.temperatureEnvironment(
                world, x, y, z, this.getTemperature()
            );
        }
    ...
    public boolean canConnectToPipe(MachineRegistry m, ForgeDirection dir) {
        return m == MachineRegistry.BEDPIPE || m.isStandardPipe()
            || m == MachineRegistry.VALVE || m == MachineRegistry.SPILLER
            || m == MachineRegistry.SEPARATION || m == MachineRegistry.BYPASS
            || m == MachineRegistry.SUCTION;
    }
    ...
    public void overheat(World world, int x, int y, int z) {
        ...
        blocks.recursiveAddWithMetadata(
            world, x, y, z, m.getBlock(), m.getBlockMetadata()
        );
        ...
        for (int i = 0; i < blocks.getSize(); i++) {
            ...
            ReikaParticleHelper.LAVA.spawnAroundBlock(
                world, c.xCoord, c.yCoord, c.zCoord, 36
            );
