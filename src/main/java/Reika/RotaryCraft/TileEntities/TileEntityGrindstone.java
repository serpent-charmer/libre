...
public class TileEntityGrindstone extends InventoriedPowerLiquidReceiver
    implements MultiOperational, ConditionalOperation, OneSlotMachine {
        ...
        return is.stackTagCompound != null && is.stackTagCompound.hasKey(NBT_TAG)
            ? is.getMaxDamage()
                - MathHelper.ceiling_float_int(
                    is.stackTagCompound.getInteger(NBT_TAG) / 2F
                )
            : 0;
    }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack is) {
        return is.isItemStackDamageable()
            && (is.getItem() instanceof ItemShears || is.getItem() instanceof ItemSword
