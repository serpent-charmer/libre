...
public class TileEntityAdvancedGear extends TileEntity1DTransmitter
    implements ISidedInventory, PowerGenerator, PartialInventory, PartialTank,
    ...
    public static long getMaxStorageCapacity(boolean bedrock) {
        return bedrock ? 240L * ReikaMathLibrary.longpow(10, 12)
                       : 720 * ReikaMathLibrary.intpow2(10, 6);
    }
    ...
    public long getMaxStorageCapacity() {
        return this.getGearType().storesEnergy()
            ? this.getMaxStorageCapacity(isBedrockCoil)
            : 0;
    }
    ...
    public int getMaximumEmission() {
        return isCreative ? Integer.MAX_VALUE
                          : getOutputCap(this.isBedrockCoil()
                          ); //still the 16x increase in the coil item
    }
    ...
    public void setReleaseTorque(int torque) {
        releaseTorque
            = Math.min(this.getTorqueCap(), Math.min(this.getMaximumEmission(), torque));
    }
    ...
    @Override
    protected void readFromSplitter(
        World world, int x, int y, int z, TileEntitySplitter spl
        ...
        if (this.getGearType() == GearType.WORM
            || this.getGearType() == GearType.CVT && this.getEffectiveRatio() < 0) {
            if (x == spl.getWriteX() && z == spl.getWriteZ()) { //We are the inline
                omega = Math.abs((int
                ) (spl.omega / this.getEffectiveRatio()
                   * this.getPowerLossFraction(spl.omega))); //omega always constant
                ...
                if (favorbent) {
                    torque = Math.abs((int
                    ) (spl.torque / sratio * this.getEffectiveRatio()));
                } else {
                    torque = Math.abs((int
                    ) (this.getEffectiveRatio()
                       * (int) (spl.torque * ((sratio - 1D) / (sratio)))));
                }
            ...
            if (this.hasLubricant()) {
                if (this.getEffectiveRatio() < 0) {
                    if (x == spl.getWriteX()
                        && z == spl.getWriteZ()) { //We are the inline
                        omega = -(int
                        ) (spl.omega / this.getEffectiveRatio()); //omega always constant
                        if (sratio == 1) { //Even split, favorbent irrelevant
                            torque = Math.abs((int
                            ) (spl.torque / 2 * this.getEffectiveRatio()));
                        } else if (favorbent) {
                            torque = Math.abs((int
                            ) (spl.torque / sratio * this.getEffectiveRatio()));
                        } else {
                    ...
                    } else if (x == spl.getWriteX2() && z == spl.getWriteZ2()) { //We are
                                                                                 //the
                                                                                 //bend
                        omega = -(int
                        ) (spl.omega / this.getEffectiveRatio()); //omega always constant
                    ...
                    if (x == spl.getWriteX()
                        ...
                        if (sratio == 1) { //Even split, favorbent irrelevant
                            ...
                            torque
                                = (int) (spl.torque / sratio / this.getEffectiveRatio());
                        } else {
                            torque = (int
                            ) ((spl.torque * ((sratio - 1D)) / sratio)
                               / (this.getEffectiveRatio()));
                        }
                    } else if (x == spl.getWriteX2() && z == spl.getWriteZ2()) { //We are
                                                                                 //the
                ...
                if (omega > 0 && this.consumeLubricantThisTick()) {
                    lubricant.removeLiquid((int) ReikaMathLibrary.logbase(
                        Math.max(omega, torque),
                        bearingType == MaterialRegistry.DIAMOND ? 1 : 2
                    ));
                }
            } else {
                omega = torque = 0;
            ...
            if (x == spl.getWriteX() && z == spl.getWriteZ()) { //We are the inline
                ...
                if (sratio == 1) { //Even split, favorbent irrelevant
                    ...
                    torque = (int
                    ) (spl.torque * ((sratio - 1D) / (sratio)) / this.getEffectiveRatio()
                    );
                } else {
                    torque = (int) (spl.torque / sratio / this.getEffectiveRatio());
                }
    ...
    public int getCVTRatio() {
        switch (cvtMode) {
            ...
            case REDSTONE:
                ...
                return (int) Math.signum(ratio)
                    * Math.min(Math.abs(ratio), this.getMaxRatio());
        }
        return 1;
    }
    ...
    private void store(World world, int x, int y, int z, int meta) {
        ...
        //ReikaJavaLibrary.pConsole(energy/20+"/"+this.getMaxStorageCapacity(),
        //Side.SERVER);
        if (!isCreative && !world.isRemote
            && energy / 20 >= this.getMaxStorageCapacity()) {
            this.overChargeExplosion(world, x, y, z);
        }
        if (!isReleasing) {
            torque = omega = 0;
            power = 0;
            if (energy + ((long) torquein * (long) omegain) < 0
                || energy + ((long) torquein * (long) omegain) > Long.MAX_VALUE) {
                this.destroy(world, x, y, z);
            } else if (!isCreative) {
                long pwr = (long) torquein * (long) omegain;
                if (torquein >= this.getChargingTorque()
                    && pwr >= this.getChargingPower())
                    energy += pwr;
            }
    ...
    public long getChargingPower() {
        return energy >= 20 ? ReikaMathLibrary.ceil2exp(
                   ReikaMathLibrary.intpow2(ReikaMathLibrary.logbase2(energy / 20), 4)
               )
                            : 1;
    }
    ...
    public int getChargingTorque() {
        int base = energy >= 20
            ? ReikaMathLibrary.ceil2exp(
                  ReikaMathLibrary.intpow2(ReikaMathLibrary.logbase2(energy / 20), 3)
              ) / 2
            : 1;
        if (this.isBedrockCoil()) {
            base = Math.max(
                base,
                energy >= 20 ? 16
                        * ReikaMathLibrary.ceil2exp(ReikaMathLibrary.intpow2(
                            ReikaMathLibrary.logbase2(energy / 80), 3
                        ))
                        / 2
                             : 16
            );
    ...
    private void overChargeExplosion(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            world.createExplosion(
                null, rx, ry, rz, 8, ConfigRegistry.BLOCKDAMAGE.getState()
            );
        }
    ...
    public int getMaxRatio() {
        ...
        for (int i = 0; i < 31; i++) {
            if (belts[i] == null
                || !ReikaItemHelper.matchStacks(belts[i], ItemStacks.belt)) {
                break;
            } else
                beltCount++;
        }
    ...
    private boolean hasRequiredBelt() {
        return belts[31] != null
            && ReikaItemHelper.matchStacks(belts[31], ItemStacks.belt);
    }
        ...
        if (cross.isWritingTo(this)) {
            ...
            if (this.getGearType() == GearType.WORM)
                omega = (int
                     ...
                     * (100 - 4 * ReikaMathLibrary.logbase(omega, 2) + 28)))
                   / 100);
    ...
    protected void transferPower(World world, int x, int y, int z, int meta) {
        ...
        if (this.isProvider(te)) {
            ...
            if (loc != null)
                this.transferPower(
                    loc.getWorld(), loc.xCoord, loc.yCoord, loc.zCoord, meta
                );
        } else {
            omega = 0;
            torque = 0;
        ...
        if (performRatio) {
            switch (this.getGearType()) {
                case WORM:
                    omega = (int
                    ) ((omegain / WORMRATIO) * this.getPowerLossFraction(omegain));
                    if (torquein <= RotaryConfig.torquelimit / WORMRATIO)
                        ...
                        world.playSoundEffect(
                            x + 0.5, y + 0.5, z + 0.5, "mob.blaze.hit", 0.1F, 1F
                        );
                    }
                    break;
                case CVT:
                ...
                case HIGH:
                    if (this.hasLubricant()) {
                        ...
                        if (omega > 0 && (world.getTotalWorldTime() & 4) == 4)
                            lubricant.removeLiquid((int
                            ) (DifficultyEffects.LUBEUSAGE.getChance()
                               * ReikaMathLibrary.logbase(Math.max(omega, torque), 2)));
                    } else {
                        omega = torque = 0;
                    }
                    break;
    ...
    private int updateAutoRatio() {
        if (torquein >= targetTorque
            && torquein < targetTorque * 2) { //already meeting, and cannot do any more
            return 1;
    ...
    public boolean isItemValidForSlot(int i, ItemStack itemstack) {
        return this.getGearType() == GearType.CVT
            && ReikaItemHelper.matchStacks(itemstack, ItemStacks.belt);
    }
    ...
    public int getRedstoneOverride() {
        if (this.getGearType().storesEnergy()) {
            int level = (int
            ) (15L * (energy + power) / 20L / this.getMaxStorageCapacity()
            ); //+power to "anticipate" next tick
            return level;
        }
        return 0;
    }
    ...
    public boolean canFill(ForgeDirection from, Fluid fluid) {
        return this.getGearType().consumesLubricant()
            ? FluidRegistry.getFluid("rc lubricant").equals(fluid)
            : false;
    }
    ...
    public FluidTankInfo[] getTankInfo(ForgeDirection from) {
        return this.getGearType().consumesLubricant()
            ? new FluidTankInfo[] { lubricant.getInfo() }
            : null;
    }
    ...
    public boolean canConnectToPipe(MachineRegistry m) {
        return this.getGearType().consumesLubricant()
            ? m == MachineRegistry.HOSE || m == MachineRegistry.BEDPIPE
            : false;
    }
    ...
    public boolean canConnectToPipeOnSide(MachineRegistry p, ForgeDirection side) {
        return this.getGearType().consumesLubricant()
            ? p == MachineRegistry.HOSE || p == MachineRegistry.BEDPIPE
            : false;
    }

    @Override
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        return this.canFill(from, resource.getFluid()) ? lubricant.fill(resource, doFill)
                                                       : 0;
    }
    ...
    public void setLubricantFromNBT(NBTTagCompound NBT) {
        lubricant.setContents(
            NBT.getInteger("lube"), FluidRegistry.getFluid("rc lubricant")
        );
    }
