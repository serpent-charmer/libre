...
public class TileEntityPowerBus
    extends TileEntityInventoryIOMachine implements InertIInv, BreakAction, ComplexIO {
        ...
        if (bus == null && this.hasHubCoordinates()) {
            ...
            if (m == MachineRegistry.BUSCONTROLLER) {
                TileEntityBusController hub
                    = (TileEntityBusController) world.getTileEntity(hubX, hubY, hubZ);
                if (hub != null) {
    ...
    private boolean hasHubCoordinates() {
        return hubX != Integer.MIN_VALUE && hubY != Integer.MIN_VALUE
    ...
    public boolean insertItem(ItemStack is, ForgeDirection side) {
        if (GearboxTypes.getRatioFromPartItem(is) > 0
            && inv[side.ordinal() - 2] == null) {
    ...
    public boolean canOutputToSide(ForgeDirection dir) {
        return !this.isReceivingFromSide(dir) && this.hasValidItem(dir)
            && !this.isDisabledClutch(dir);
    }

    private boolean isDisabledClutch(ForgeDirection dir) {
        TileEntity te = this.getAdjacentTileEntity(dir);
        return te instanceof TileEntityClutch
            && !((TileEntityClutch) te).isOutputEnabled();
    }
    ...
    private void breakItem(ForgeDirection dir) {
        ...
        for (int i = 0; i < 3; i++)
            ReikaSoundHelper.playSoundAtBlock(
                worldObj, xCoord, yCoord, zCoord, "random.break", 2, 1
            );
    }
    ...
    public void findNetwork(World world, int x, int y, int z) {
        for (int i = ALLOW_VERTICAL_CHAINS ? 0 : 2; i < 6; i++) {
            ...
            if (m == MachineRegistry.BUSCONTROLLER) {
                TileEntityBusController te
                    = (TileEntityBusController) world.getTileEntity(dx, dy, dz);
            ...
            if (m == MachineRegistry.POWERBUS) {
                TileEntityPowerBus te
                    = (TileEntityPowerBus) world.getTileEntity(dx, dy, dz);
                ShaftPowerBus bus = te.getBus();
                if (bus != null) {
                    this.configureBusData(bus);
                    inputSide = dir;
    ...
    public PowerSourceList getPowerSources(PowerSourceTracker io, ShaftMerger caller) {
        return bus != null && bus.getController() != null
            ? bus.getController().getPowerSources(io, caller)
            : new PowerSourceList();
    }
