...
public class TileEntityShaft extends TileEntity1DTransmitter {
    ...
    public void testFailure() {
        if (ReikaEngLibrary.mat_rotfailure(
                type.rho,
                0.0625,
                ReikaMathLibrary.doubpow(omega, type.getSpeedForceExponent()),
                type.tensile
            )) {
    ...
    private void crossReadFromSplitter(
        World world, int x, int y, int z, TileEntitySplitter spl, int dir
    ) {
        reading2Dir = true;
        ...
        if (x == spl.getWriteX() && z == spl.getWriteZ()) { //We are the inline
            ...
            //ModLoader.getMinecraftInstance().thePlayer.addChatMessage(String.format("INLINE!
            //%d  %d  FOR %d", spl.omega, spl.torque, sratio));
            if (sratio == 1) { //Even split, favorbent irrelevant
                ...
                //%d", this.readtorque[dir], this.readomega[dir]));
                return;
            }
            if (favorbent) {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        //ReikaJavaLibrary.pConsole(Arrays.toString(readtorque)+":"+Arrays.toString(readomega),
        //Side.SERVER);
    }
    ...
    private void
    crossTransfer(World world, int x, int y, int z, boolean check1, boolean check2) {
        if (check1 && check2) {
            readomega[0] = 0;
            readomega[1] = 0;
            readtorque[0] = 0;
        ...
        TileEntity te1 = isCentered ? this.getAdjacentTileEntity(read)
                                    : world.getTileEntity(dx, dy, dz);
        MachineRegistry m2 = isCentered
            ? this.getMachine(read2)
            : MachineRegistry.getMachine(world, dx2, dy2, dz2);
        TileEntity te2 = isCentered ? this.getAdjacentTileEntity(read2)
                                    : world.getTileEntity(dx2, dy2, dz2);
        ...
        if (check1) {
            if (this.isProvider(te1)) {
                ...
                if (loc != null)
                    this.crossTransfer(
                        loc.getWorld(), loc.xCoord, loc.yCoord, loc.zCoord, true, false
                    );
            } else {
                readtorque[0] = 0;
                readomega[0] = 0;
        ...
        if (check2) {
            if (this.isProvider(te2)) {
                ...
                if (loc != null)
                    this.crossTransfer(
                        loc.getWorld(), loc.xCoord, loc.yCoord, loc.zCoord, false, true
                    );
            } else {
                readtorque[1] = 0;
                readomega[1] = 0;
    ...
    protected void transferPower(World world, int x, int y, int z, int meta) {
        ...
        if (isCentered
            && ReikaBlockHelper.isPortalBlock(
                world, x + write.offsetX, y + write.offsetY, z + write.offsetZ
            )) {
            this.convertToPortal(
                world, x, y, z, x + write.offsetX, y + write.offsetY, z + write.offsetZ
            );
        }
    ...
    private void
    convertToPortal(World world, int x, int y, int z, int dx, int dy, int dz) {
        world.setBlock(
            x,
            y,
            z,
            MachineRegistry.PORTALSHAFT.getBlock(),
            MachineRegistry.PORTALSHAFT.getBlockMetadata(),
            3
        );
    ...
    protected void animateWithTick(World world, int x, int y, int z) {
        ...
        crossphi1 += ReikaMathLibrary.doubpow(
            ReikaMathLibrary.logbase(readomega[0] + 1, 2), 1.25
        );
        crossphi2 += ReikaMathLibrary.doubpow(
            ReikaMathLibrary.logbase(readomega[1] + 1, 2), 1.25
        );
    }
        ...
        if (this.isCross()) {
            ...
            if (read1) {
                ...
                return PowerSourceList.getAllFrom(
                    worldObj,
                    read2,
                    xCoord + read2.offsetX,
                    yCoord + read2.offsetY,
                    zCoord + read2.offsetZ,
                    this,
                    caller
                );
            }
