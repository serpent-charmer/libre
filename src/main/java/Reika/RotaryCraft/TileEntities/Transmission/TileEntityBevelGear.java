...
public class TileEntityBevelGear
    extends TileEntity1DTransmitter implements GuiController {
    private static final HashBiMap<Integer, ImmutablePair<ForgeDirection, ForgeDirection>>
        directions = HashBiMap.create();
    ...
    private static void registerDirection(ForgeDirection read, ForgeDirection write) {
        ...
        RotaryCraft.logger.log(
            "Registered bevel direction #" + idx + ": " + read + " to " + write
        );
    }

    public static HashBiMap<Integer, ImmutablePair<ForgeDirection, ForgeDirection>>
    getDirectionMap() {
    ...
    private void findRoute(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (te instanceof TileEntityIOMachine) {
                ...
                if (read == null) {
                    if (io.isWritingToCoordinate(x, y, z)
                        || io.isWritingToCoordinate2(x, y, z)) {
                        read = dirs[i];
                    }
                }
                if (write == null) {
                    if (io.isReadingFrom(this) || io.isReadingFrom2(this)
                        || io.isReadingFrom3(this) || io.isReadingFrom4(this)) {
                        write = dirs[i];
                    }
                }
