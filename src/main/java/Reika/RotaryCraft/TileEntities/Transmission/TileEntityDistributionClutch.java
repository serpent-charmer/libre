...
public class TileEntityDistributionClutch extends TileEntityTransmissionMachine
    implements ComplexIO, GuiController, IBundledTile {
    ...
    public PowerSourceList getPowerSources(PowerSourceTracker io, ShaftMerger caller) {
        ...
        return PowerSourceList.getAllFrom(
            worldObj,
            read,
            xCoord + read.offsetX,
            yCoord + read.offsetY,
    ...
    public boolean isOutputtingToSide(ForgeDirection dir) {
        return dir != read && enabledSides[dir.ordinal() - 2]
            && (requestedTorques[dir.ordinal() - 2] > 0
    ...
    private void updateControl(World world, int x, int y, int z) {
        ...
        if (control == ControlMode.REDSTONE) {
            ...
            enabledSides[ForgeDirection.NORTH.ordinal() - 2]
                = this.getBundledInput(world, x, y, z, ReikaDyeHelper.RED) != 0;
            enabledSides[ForgeDirection.EAST.ordinal() - 2]
                = this.getBundledInput(world, x, y, z, ReikaDyeHelper.YELLOW) != 0;
            enabledSides[ForgeDirection.SOUTH.ordinal() - 2]
                = this.getBundledInput(world, x, y, z, ReikaDyeHelper.BLUE) != 0;
            enabledSides[ForgeDirection.WEST.ordinal() - 2]
                = this.getBundledInput(world, x, y, z, ReikaDyeHelper.GREEN) != 0;
        }
    }
    ...
    private void distributePower(World world, int x, int y, int z) {
        ...
        for (int i = 2; i < 6; i++) {
            ...
            outputTorques[dir.ordinal() - 2] = 0;
            //ReikaJavaLibrary.pConsole("Resetting output on "+dir, Side.SERVER, xCoord ==
            //514);
            if (this.isOutputtingToSide(dir)) {
                int amt = this.calculateOutputTorque(dir, leftover);
                leftover -= amt;
                outputTorques[dir.ordinal() - 2] = amt;
                //ReikaJavaLibrary.pConsole("Setting output to "+amt+" on "+dir,
                //Side.SERVER, xCoord == 514);
            }
        }
        outputTorques[write.ordinal() - 2] = leftover;
        //ReikaJavaLibrary.pConsole("Setting output to "+leftover+" on "+write,
        //Side.SERVER, xCoord == 514);
        for (int i = 2; i < 6; i++) {
            ForgeDirection dir = dirs[i];
        ...
        //ReikaJavaLibrary.pConsole("write "+System.identityHashCode(this)+" @
        //"+System.nanoTime()+": "+torquein+" > "+Arrays.toString(enabledSides)+" >
        //"+Arrays.toString(outputTorques)+" &
        //"+this.getTorqueToSide(ForgeDirection.WEST), Side.SERVER, x == 514);
    }
    ...
    private void intakePower(World world, int x, int y, int z, int meta) {
        ...
        if (this.isProvider(te)) {
            ...
            if (te instanceof ComplexIO) {
                ...
                //ReikaJavaLibrary.pConsole(System.identityHashCode(te), Side.SERVER,
                //xCoord == 513); ReikaJavaLibrary.pConsole("read
                //"+System.identityHashCode(te)+" @ "+System.nanoTime()+":
                //"+((TileEntityDistributionClutch)te).torquein+" >
                //"+Arrays.toString(((TileEntityDistributionClutch)te).enabledSides)+" >
                //"+Arrays.toString(((TileEntityDistributionClutch)te).outputTorques)+" &
                //"+((TileEntityDistributionClutch)te).getTorqueToSide(dir), Side.SERVER,
                //x == 513);
            }
        ...
        /*
            ...
            this.processTileSimply(other, MachineRegistry.getMachine(loc.move(read, 1)),
        loc.xCoord, loc.yCoord, loc.zCoord);
        }
        else {
            omega = torque = 0;
        }
    ...
    protected void writeSyncTag(NBTTagCompound NBT) {
        ...
        //ReikaJavaLibrary.pConsole("out "+NBT+" leaves "+Arrays.toString(outputTorques),
        //xCoord == 514);
    }
    ...
    protected void readSyncTag(NBTTagCompound NBT) {
        ...
        //ReikaJavaLibrary.pConsole("in "+NBT+" gives "+Arrays.toString(outputTorques),
        //xCoord == 514);
    ...
    private int getBundledInput(World world, int x, int y, int z, ReikaDyeHelper color) {
        ...
        for (int i = 0; i < 6; i++) {
            byte[] data
                = ProjectRedAPI.transmissionAPI.getBundledInput(world, x, y, z, i);
    ...
    public static enum ControlMode {
        ...
        public boolean isValid() {
            switch (this) {
                ...
                case COMPUTER:
                    return ModList.COMPUTERCRAFT.isLoaded()
                        || ModList.OPENCOMPUTERS.isLoaded();
                default:
                    return true;
            }
        }
