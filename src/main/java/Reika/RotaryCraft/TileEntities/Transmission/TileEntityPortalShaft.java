...
public class TileEntityPortalShaft
    extends TileEntity1DTransmitter implements ChunkLoadingTile {
    ...
    private int getMystCraftTarget() {
        ...
        return ReikaMystcraftHelper.getTargetDimensionIDFromPortalBlock(
            worldObj, x, y, z
        );
    }
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (ReikaBlockHelper.isPortalBlock(
        ...
        if (!world.isRemote && otherShaft != null) {
            if (otherShaft.getTileEntity() instanceof TileEntityPortalShaft) {
                ...
                world.setBlock(
                    ...
                    y,
                    z,
                    MachineRegistry.SHAFT.getBlock(),
                    MachineRegistry.SHAFT.getBlockMetadata(),
                    3
                );
    ...
    private Coordinate
    getScaledCoordinates(int x, int y, int z, World source, World target) {
    ...
    private void emitPower(World world, int x, int y, int z) {
        ...
        //"+age.getBlock(dx, dy, dz), Side.SERVER); ReikaJavaLibrary.pConsole(x+", "+y+",
        //"+z+":"+dx+", "+dy+", "+dz+" >> "+m, Side.SERVER);
        ...
        if (m == MachineRegistry.SHAFT) {
            ...
            if (terx == ax && tery == ay && terz == az) {
                ...
                TileEntityPortalShaft ps = (TileEntityPortalShaft
                ) age.getTileEntity(dx, dy, dz); //new TileEntityPortalShaft();
                ...
                ReikaPacketHelper.sendUpdatePacket(
                    ...
                    PacketIDs.TILEDELETE.ordinal(),
                    ps,
                    new PacketTarget.RadiusTarget(this, 32)
                );
            }
        } else if (m == MachineRegistry.PORTALSHAFT) {
            TileEntityPortalShaft te
                = (TileEntityPortalShaft) age.getTileEntity(dx, dy, dz);
            int terx = te.xCoord + te.getReadDirection().offsetX;
            int tery = te.yCoord + te.getReadDirection().offsetY;
            int terz = te.zCoord + te.getReadDirection().offsetZ;
            if (terx == ax && tery == ay && terz == az) {
    ...
    private Location getOutputLocation(World world, int x, int y, int z) {
        ...
        Coordinate loc = this.getScaledCoordinates(
            x + write.offsetX, y + write.offsetY, z + write.offsetZ, world, age
        );
        if (loc == null)
            return null;
        int ax = loc.xCoord;
        ...
        if (age != null /* && age.checkChunksExist(ax, ay, az, ax, ay, az)*/) {
            ...
            if (tg == world.provider.dimensionId) {
                //ReikaJavaLibrary.pConsole(writex+", "+writey+", "+writez+" >>
                //"+Blocks.blocksList[id], Side.SERVER);
                Coordinate c2 = this.getScaledCoordinates(x, y, z, world, age);
                return new Location(
                    age,
                    loc.xCoord + write.offsetX,
                    loc.yCoord + write.offsetY,
                    loc.zCoord + write.offsetZ,
                    ax,
                    ay,
                    az
                );
    ...
    public boolean isEnteringPortal() {
        if (write == null)
            return false;
        return ReikaBlockHelper.isPortalBlock(
            worldObj,
            xCoord + write.offsetX,
            yCoord + write.offsetY,
            zCoord + write.offsetZ
        );
    }
    ...
    public boolean isExitingPortal() {
        if (read == null)
            return false;
        return ReikaBlockHelper.isPortalBlock(
            worldObj, xCoord + read.offsetX, yCoord + read.offsetY, zCoord + read.offsetZ
        );
    }
