...
public class TileEntitySplitter extends TileEntityTransmissionMachine
    implements GuiController, ShaftMerger, NBTMachine {
    ...
    public void getIOSides(World world, int x, int y, int z, int metadata) {
         ...
         */
        //ReikaWorldHelper.legacySetBlockWithNotify(this.worldObj, this.writex,
        //this.yCoord, this.writez, 44);
        //ReikaWorldHelper.legacySetBlockWithNotify(this.worldObj, this.writex2,
        //this.yCoord, this.writez2, 79);
        //ReikaWorldHelper.legacySetBlockWithNotify(this.worldObj, this.readx,
        //this.yCoord, this.readz, 20);
    }
    ...
    protected void transferPower(
        World world, int x, int y, int z, int meta, boolean check1, boolean check2
    ) {
        ...
        if (!this.isSplitting()) {
            ...
            MachineRegistry m = isCentered
                ? this.getMachine(read)
            ...
            if (read != null && read2 != null) {
                //ReikaJavaLibrary.pConsole("====================", Side.SERVER, xCoord ==
                //-1011);
                in1 = PowerSourceList.getAllFrom(
                    world,
                    read,
                    x + read.offsetX,
                    y + read.offsetY,
                ...
                //ReikaJavaLibrary.pConsole("--------------", Side.SERVER, xCoord ==
                //-1011);
                in2 = PowerSourceList.getAllFrom(
                    world,
                    read2,
                    x + read2.offsetX,
                    y + read2.offsetY,
            ...
            if (!this.canCombine(in1, in2, torquein, torquein2)) {
                ...
                omega = omegain == 0 || omegain2 == 0
                    ? Math.max(omegain, omegain2)
                    : rand.nextInt(Math.max(1 + omegain, 1 + omegain2));
            ...
            //* %d = %d", this.omega, this.torque, this.power));
        }
    }
    ...
    public void fail() {
        worldObj.createExplosion(
            null, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, 4, false
        );
        failed = true;
    }
    ...
    public PowerSourceList getPowerSources(PowerSourceTracker io, ShaftMerger caller) {
        ...
        if (!this.isSplitting()) { //merge
            if (read != null) {
                PowerSourceList in = PowerSourceList.getAllFrom(
                    worldObj,
                    read,
                    xCoord + read.offsetX,
                    yCoord + read.offsetY,
