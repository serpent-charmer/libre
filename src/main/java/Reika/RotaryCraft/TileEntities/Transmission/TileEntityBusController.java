...
public class TileEntityBusController
    extends PoweredLiquidReceiver implements TransmissionReceiver, BreakAction {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (tickcount % 10 == 0)
            bus.update();
        //ReikaJavaLibrary.pConsole(bus.getInputPower()+":"+bus.getTotalOutputSides(),
        //Side.SERVER);
    }
    ...
    private int getLubricantUsed() {
        return Math.max(
            1,
            (int
            ) (DifficultyEffects.LUBEUSAGE.getChance() * 2 * bus.getSize()
               + bus.getTotalOutputSides())
        );
    }
