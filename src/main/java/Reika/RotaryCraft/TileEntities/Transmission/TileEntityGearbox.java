...
@Strippable(
    value = { "vazkii.botania.api.mana.IManaReceiver",
              "Reika.ChromatiCraft.API.Interfaces.Repairable" }
)
public class TileEntityGearbox extends TileEntity1DTransmitter
    implements PipeConnector, IFluidHandler, TemperatureTE, NBTMachine, IManaReceiver {
    ...
    private void getLubeAndApplyDamage(World world, int x, int y, int z, int metadata) {
        ...
        if (type.needsLubricant() && omega > 0
            ...
            if (tank.isEmpty()) {
                ...
                if (tickcount >= 80) {
                    tank.removeLiquid(Math.max(
                        1,
                        (int
                        ) (DifficultyEffects.LUBEUSAGE.getChance()
                           * this.getLubricantConsumptionFactor())
                    ));
                    tickcount = 0;
                }
            }
    ...
    public boolean consumesLubricant() {
        return type.consumesLubricant()
            && this.getBearingTier().material.ordinal()
            < MaterialRegistry.DIAMOND.ordinal();
    }

    private double getLubricantConsumptionFactor() {
        double base = type.getLubricantConsumeRate(omegain)
            * ReikaMathLibrary.logbase(omegain, 2) / 4;
    ...
    protected void transferPower(World world, int x, int y, int z, int meta) {
        ...
        int tempEff = temperature
            + Math.max(0, world.getSavedLightValue(EnumSkyBlock.Block, x, y, z) - 10);
    ...
    public void testFailure() {
        if (ReikaEngLibrary.mat_rotfailure(
                type.material.rho,
                0.0625,
                type.getOmegaForRotFailure(omega, omegain),
                type.material.tensile
            )) {
            this.fail(worldObj, xCoord, yCoord, zCoord);
        } else if (ReikaEngLibrary.mat_twistfailure(
                       Math.max(torque, torquein), 0.0625, type.material.shear / 16D
                   )) {
            this.fail(worldObj, xCoord, yCoord, zCoord);
        }
    }
    ...
    public int getRedstoneOverride() {
        return this.getMaxLubricant() > 0 ? 15 * tank.getLevel() / this.getMaxLubricant()
                                          : 0;
    }
    ...
    public boolean canFill(ForgeDirection from, Fluid fluid) {
        return from != (isFlipped ? ForgeDirection.DOWN : ForgeDirection.UP)
            && fluid.equals(FluidRegistry.getFluid("rc lubricant")) && !this.isLiving();
    }
    ...
    public Flow getFlowForSide(ForgeDirection side) {
        return side != (isFlipped ? ForgeDirection.DOWN : ForgeDirection.UP) ? Flow.INPUT
                                                                             : Flow.NONE;
    }
    ...
    public void updateTemperature(World world, int x, int y, int z, int meta) {
        ...
        if (omega > 0 && type.generatesHeat(omega, Tamb)) {
            ...
            ReikaSoundHelper.playSoundAtBlock(
                world, x, y, z, type.material.getDamageNoise(), 0.67F, 1
            );
        }
        if (temperature > 90 && rand.nextBoolean() && type.takesTemperatureDamage()) {
            damage++;
            ReikaSoundHelper.playSoundAtBlock(
                world, x, y, z, type.material.getDamageNoise(), 1, 1
            );
        }
        if (temperature > Tamb)
    ...
    public boolean isLiving() {
        return ModList.BOTANIA.isLoaded()
            && (type == GearboxTypes.LIVINGROCK || type == GearboxTypes.LIVINGWOOD);
    }
    ...
    public boolean isFull() {
        return this.getLubricant() + 150
            >= this.getMaxLubricant(); //+150 to not have bursts sent and waste 95%
    }

    @Override
    public void recieveMana(int mana) {
        tank.addLiquid(
            Math.min(mana, this.getMaxLubricant() - this.getLubricant()),
    ...
    overridePipeConnection(PipeType type, ForgeDirection side) {
        return type == PipeType.FLUID && this.consumesLubricant()
                && this.canConnectToPipeOnSide(MachineRegistry.HOSE, side)
