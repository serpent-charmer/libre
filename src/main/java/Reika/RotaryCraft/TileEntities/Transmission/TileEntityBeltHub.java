...
public class TileEntityBeltHub extends TileEntityPowerReceiver
    implements PowerGenerator, SimpleProvider, TransmissionReceiver, Wettable,
    ...
    public final void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote && world.isRaining()
            && world.canLightningStrikeAt(x, y + 1, z) && rand.nextInt(900) == 0)
            this.makeWet(1);
    ...
    public final int getDistanceToTarget() {
        return otherEnd == null ? -1
                                : otherEnd.getTaxicabDistanceTo(xCoord, yCoord, zCoord);
    }
    ...
    public PowerSourceList getPowerSources(PowerSourceTracker io, ShaftMerger caller) {
        if (isReceivingEnd) {
            TileEntityBeltHub tile = (TileEntityBeltHub) otherEnd.getTileEntity(worldObj);
            return tile != null ? tile.getPowerSources(io, caller)
                                : new PowerSourceList();
        } else {
    ...
    public final AxisAlignedBB getRenderBoundingBox() { /*
         ...
         box.maxZ -= a*dir.offsetZ;*/
        //return AxisAlignedBB.getBoundingBox(box.minX, box.minY, box.minZ, box.maxX,
        //box.maxY, box.maxZ);
        ...
        if (!worldObj.isRemote) {
            ...
            for (int i = 0; i < num; i++) {
                ReikaItemHelper.dropItem(
                    worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, this.getBeltItem()
                );
            }
            this.resetOther();
        }
