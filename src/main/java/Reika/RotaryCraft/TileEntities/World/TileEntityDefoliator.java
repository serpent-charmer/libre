...
public class TileEntityDefoliator
    extends InventoriedPowerLiquidReceiver implements RangedEffect {
    ...
    private void decay(World world, int x, int y, int z) {
        ...
        if (!world.isRemote
            && !ReikaPlayerAPI.playerCanBreakAt(
                (WorldServer) worldObj, x, y, z, id, meta, this.getServerPlacer()
            ))
            return;
        boolean flag = false;
        if (id != Blocks.air) {
            ...
            if (flag) {
                ...
                if (world.checkChunksExist(x, y, z, x, y, z))
                    ReikaPacketHelper.sendDataPacketWithRadius(
                        ...
                        PacketRegistry.DEFOLIATOR.ordinal(),
                        world,
                        x,
                        y,
                        z,
    ...
    public boolean isItemValidForSlot(int slot, ItemStack is) {
        return is.getItem() == Items.potionitem
            && ReikaPotionHelper.getPotionDamageValue(Potion.poison)
            == is.getItemDamage();
    }
