...
public class TileEntityFlooder
    extends TileEntityAreaFiller implements IFluidHandler, PipeConnector {
    private final HybridTank tank = new HybridTank("flooder", 4000);

    private Block getFluidID() {
        return !tank.isEmpty() && tank.getActualFluid().canBePlacedInWorld()
            ? tank.getActualFluid().getBlock()
            : null;
    }
    ...
    public int getOperationTime() {
        ...
        return MathHelper.clamp_int(
            base * tank.getActualFluid().getViscosity() / 1000, base / 4, base * 4
        );
    }
