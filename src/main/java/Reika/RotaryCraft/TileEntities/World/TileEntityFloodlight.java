...
public class TileEntityFloodlight
    ...
    public static final int MAX_RANGE
        = Math.max(64, ConfigRegistry.FLOODLIGHTRANGE.getValue());
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote) {
            if ((world.getTotalWorldTime() & 8)
                == 8) //almost unnoticeable light lag, but big FPS increase
    ...
    protected void makeBeam(World world, int x, int y, int z, int metadata) {
        ...
        if (lastRange != r) {
            RotaryCraft.logger.debug(
                "Updating " + this + " range from " + lastRange + " to " + r
            );
            ...
            if (r > 0) {
                if (fresnel) {
                    ArrayList<ForgeDirection> ds
                        = ReikaDirectionHelper.getPerpendicularDirections(facing);
                    ...
                    for (int d = 1; d <= r; d++) {
                        ...
                        for (int a = -w; a <= w; a++) {
                            for (int b = -w; b <= w; b++) {
                                int dx = x + facing.offsetX * d + d1.offsetX * a
                                    + d2.offsetX * b;
                                int dy = y + facing.offsetY * d + d1.offsetY * a
                                    + d2.offsetY * b;
                                int dz = z + facing.offsetZ * d + d1.offsetZ * a
                                    + d2.offsetZ * b;
                    ...
                    beam.addLineOfClear(
                        world, x, y, z, r, facing.offsetX, facing.offsetY, facing.offsetZ
                    );
                }
            }
            lastRange = r;
    ...
    private Block getPlacedBlockID() {
        return beammode ? BlockRegistry.BEAM.getBlockInstance()
                        : BlockRegistry.LIGHT.getBlockInstance();
    }

    private boolean isLightBlock(Block id) {
        return id == BlockRegistry.BEAM.getBlockInstance()
            || id == BlockRegistry.LIGHT.getBlockInstance();
    }
    ...
    public void breakBlock() {
        ...
        if (fresnel) {
            ReikaItemHelper.dropItem(
                worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, ItemStacks.lens
            );
        }
    }
}
