...
public class TileEntityBlockFiller
    extends TileEntityAreaFiller implements ISidedInventory, OneSlotMachine {
    ...
    protected void onBlockPlaced() {
        if (ModList.BOTANIA.isLoaded() && inv[0].getItem() instanceof IBlockProvider) {
            BlockKey bk
                = this.getBlockFromBotania((IBlockProvider) inv[0].getItem(), inv[0]);
            ((IBlockProvider) inv[0].getItem())
                .provideBlock(
                    this.getPlacer(), null, inv[0], bk.blockID, bk.metadata, true
                );
        } else {
    ...
    protected boolean hasRemainingBlocks() {
        ...
        if (ModList.BOTANIA.isLoaded() && inv[0].getItem() instanceof IBlockProvider) {
            ...
            return bk != null
                && ((IBlockProvider) inv[0].getItem())
                       .provideBlock(
                           this.getPlacer(), null, inv[0], bk.blockID, bk.metadata, false
                       );
        }
