...
public class TileEntityBeamMirror
    extends RotaryCraftTileEntity implements RangedEffect, BreakAction {
    public float theta;
    ...
    public int getRange() {
        ...
        if (time > 13500 && time < 22500)
            return 0;
        double r = ReikaMathLibrary.doubpow(
            2, 7 * ReikaWorldHelper.getSunIntensity(worldObj, true, 0)
        );
        ...
        for (int i = 1; i < ir; i++) {
            ...
            if (b != Blocks.air) {
                if (b instanceof SemiTransparent) {
                    if (((SemiTransparent) b)
                            .isOpaque(worldObj.getBlockMetadata(dx, dy, dz)))
                        return i;
