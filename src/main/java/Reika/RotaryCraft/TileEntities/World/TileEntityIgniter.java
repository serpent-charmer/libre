...
public class TileEntityIgniter extends InventoriedPowerReceiver
    implements TemperatureTE, RangedEffect, ConditionalOperation {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (temperature >= ANIMALIGNITION) {
            AxisAlignedBB box
                = ReikaAABBHelper.getBlockAABB(this).expand(spread, yspread, spread);
    ...
    private void fire(World world, int x, int y, int z, int fx, int fy, int fz) {
        ...
        int d = this.getRange();
        //ReikaWorldHelper.spawnParticleLine(world, x+0.5, y+0.5, z+0.5, fx+0.5, fy+0.5,
        //fz+0.5, "flame", 0, 0, 0, 20);

        if (world.isRemote) {
            this.fireFX(world, x, y, z, fx, fy, fz, d);
        } else
            ReikaWorldHelper.temperatureEnvironment(
                world, fx, fy, fz, this.getAffectiveTemperature()
            );
    }
    ...
    private int getAffectiveTemperature() {
        return ConfigRegistry.ATTACKBLOCKS.getState() ? temperature
                                                      : Math.min(400, temperature);
    }
