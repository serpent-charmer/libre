...
public class TileEntityAerosolizer extends InventoriedPowerReceiver
    implements RangedEffect, ConditionalOperation, IFluidHandler {
                ...
                if (eff != null) {
                    ...
                    if (this.tryAddPotionToSlot(i, num, eff))
                        this.setInventorySlotContents(
                            i, new ItemStack(Items.glass_bottle, inslot.stackSize, 0)
                        );
                }
            }
        }
    ...
    public int getPotionColor(int slot) {
        return potions[slot] != null ? (0xff000000 | potions[slot].renderColor)
                                     : 0xff000000;
    }
        ...
        if (i instanceof ItemPotion) {
            ...
            if (id != -1) {
                ...
                if (p != null && !p.isInstant()) {
                    ...
                    return new PotionApplication(ReikaJavaLibrary.makeListFrom(new
            PotionEffect(p.id, 0)), extended ? 3 : 1, level2 ? 1 : 0);
                }
            }
             */
            ...
            for (PotionEffect p : li) {
                if (!Potion.potionTypes[p.getPotionID()].isInstant()) {
                    ...
                    return new PotionApplication(
                        ReikaJavaLibrary.makeListFrom(new PotionEffect(p.getPotionID(), 0)
                        ),
                        extended ? 3 : 1,
                        level2 ? 1 : 0
                    );
            ...
            if (p != null && !p.isInstant()) {
                ...
                return new PotionApplication(
                    ReikaJavaLibrary.makeListFrom(new PotionEffect(p.id, 0)),
                    extended ? 3 : 1,
                    level2 ? 1 : 0
                );
            }
    ...
    private void dispense2(
        World world, int x, int y, int z, int meta, AxisAlignedBB room, int i
    ) { // id, duration, amplifier
        if (!worldObj.isRemote) {
            if (potions[i] != null) {
                List<PotionEffect> effects = potions[i].effects;
                if (effects != null && !effects.isEmpty()) {
                    List<EntityLivingBase> inroom
                        = worldObj.getEntitiesWithinAABB(EntityLivingBase.class, room);
                    for (EntityLivingBase mob : inroom) {
                        for (PotionEffect effect : effects) {
                            ...
                            if (effect.getAmplifier() == 1)
                                bonus *= 2;
                            mob.addPotionEffect(
                                new PotionEffect(id, 100, effect.getAmplifier() + bonus)
                            );
                        }
                    }
                }
    ...
    private static class PotionApplication {
        ...
        private boolean matchEffects(PotionApplication p) {
            ...
            for (int i = 0; i < effects.size(); i++) {
                ...
                if (!(p1.getPotionID() == p2.getPotionID()
                      && p1.getAmplifier() == p2.getAmplifier())) {
                    return false;
                }
            }
            return true;
    ...
    public boolean isValidFluid(Fluid f) {
        return f == FluidRegistry.getFluid("poison")
            || f == FluidRegistry.getFluid("rc chlorine");
    }
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        if (doFill && this.canFill(from, resource.getFluid())) {
            for (int i = 0; i < 9; i++) {
                PotionApplication eff = this.getEffectFromItem(
                    ReikaPotionHelper.getPotionItem(Potion.poison, false, false, false)
                );
