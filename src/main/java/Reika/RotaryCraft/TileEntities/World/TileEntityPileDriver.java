...
public class TileEntityPileDriver extends TileEntityPowerReceiver {
    ...
    public static final int BASEPOWER
        = 16384; //16 kW per meter to lift the hammer (P = mg x dh/dt)
    ...
    static {
        ...
        BLOCK_CONVERSION.put(
            BlockRegistry.DECO.getBlockInstance(),
            ItemStacks.shieldblock.getItemDamage(),
            new BlockKey(
                BlockRegistry.DECO.getBlockInstance(),
                ItemStacks.shieldblock.getItemDamage()
            )
        ...
        BLOCK_CONVERSION.put(
            ...
            3,
            new BlockKey(BlockRegistry.MININGPIPE.getBlockInstance(), 3)
        );
                ...
                for (int k = 0; k < RockShapes.shapeList.length; k++) {
                    ...
                    if (s != RockShapes.COBBLE) {
                        BLOCK_CONVERSION.put(
                            b,
                            is.getItemDamage(),
                            BlockKey.fromItem(r.getItem(RockShapes.COBBLE))
                        );
                    }
    ...
    private void bounce(World world, int x, int y, int z) { //bounce entities
        AxisAlignedBB zone
            = AxisAlignedBB.getBoundingBox(x - 2, y, z - 2, x + 3, y + 1, z + 3)
                  .expand(24, 24, 24);
    ...
    private void breakGlass(World world, int x, int y, int z) {
        ...
        AxisAlignedBB nearby = AxisAlignedBB.getBoundingBox(
            x - range, y - range, z - range, x + 1 + range, y + 1 + range, z + 1 + range
        );
        List<EntityHanging> inzone
            = world.getEntitiesWithinAABB(EntityHanging.class, nearby);
    ...
    private void makeFall(World world, int x, int y, int z, BlockFalling id) {
        ...
        if (tofall.func_149831_e(world, x, y - 1, z)) {
            byte var8 = 32;
            if (!tofall.fallInstantly
                && world.checkChunksExist(
                    x - var8, y - var8, z - var8, x + var8, y + var8, z + var8
                )) {
                if (!world.isRemote) {
    ...
    private ArrayList<ItemStack> getDrops(World world, int x, int y, int z) {
        ...
        ; /*
         if (TileEntityBorer.isMineableBedrock(world, x, y, z))
             return
         ReikaJavaLibrary.makeListFrom(ReikaItemHelper.getSizedItemStack(ItemStacks.bedrockdust.copy(),
         DifficultyEffects.BEDROCKDUST.getInt())); else*/
        return b != null ? b.getDrops(world, x, y, z, world.getBlockMetadata(x, y, z), 0)
                         : new ArrayList();
    }

    private BlockKey
    getBlockProduct(World world, int x, int y, int z, Block id, int meta) {
    ...
    private boolean drawPile3(World world, int x, int y, int z, int speed) {
        if (climbing && tickcount > speed) {
            if (world.getBlock(x, y - step2 - 2, z)
                == BlockRegistry.MININGPIPE.getBlockInstance())
        ...
        if (!climbing) {
            if (ReikaWorldHelper.getMaterial(world, x, y - step2 - 1, z)
                == Material.water) {
                ...
                world.playSoundEffect(
                    x + 0.5, y - step2, z + 0.5, "random.splash", 1F, 1F
                );
            }
            if (ReikaWorldHelper.getMaterial(world, x, y - step2 - 1, z)
                == Material.lava) {
        ...
        if (world.getBlock(x, y - step2 - 1, z) == Blocks.air) {
            while (world.getBlock(x, y - step2 - 2, z) == Blocks.air && y - step2 - 2 > 0
                   && step == step2) {
                step++;
                step2 = step;
            }
        }
        //ModLoader.getMinecraftInstance().thePlayer.addChatMessage(String.format("%d  %d
        //%d", step, step2, y-step2-2));
        return (step2 == step);
    }
    ...
    private boolean smash(World world, int x, int y, int z) {
            ...
            for (int j = -2; j < 3; j++) {
                ...
                if (!world.isRemote
                    && ReikaPlayerAPI.playerCanBreakAt(
                        ...
                        id,
                        meta,
                        this.getServerPlacer()
                    )) {
                    if (id != Blocks.air && i * j != 4 && i * j != -4) {
                        if (id == Blocks.mob_spawner) {
                            TileEntityMobSpawner spw = (TileEntityMobSpawner
                            ) world.getTileEntity(x + i, y, z + j);
                            if (spw != null) {
                        ...
                        for (int h = 1; h <= 4; h++) {
                            ...
                            if (hits < 0 && Math.abs(hits) >= h) {
                                this.checkIncrementAndBreak(
                                    world, x + i, y - h, z + j, id2, meta2
                                );
                            }
                        }
        ...
        for (int i = -2; i < 3; i++) {
            for (int j = -2; j < 3; j++) {
                if (i * j != 4 && i * j != -4
                    && world.getBlock(x + i, y, z + j) != Blocks.air
                    && ReikaWorldHelper.getMaterial(world, x + i, y, z + j)
                        != Material.water
                    && ReikaWorldHelper.getMaterial(world, x + i, y, z + j)
                        != Material.lava) {
                    cleared = false;
    ...
    private void
    checkIncrementAndBreak(World world, int x, int y, int z, Block id, int meta) {
    ...
    private void
    spawnSpawner(World world, int x, int y, int z, TileEntityMobSpawner spw) {
        if (world.isRemote)
            return;
