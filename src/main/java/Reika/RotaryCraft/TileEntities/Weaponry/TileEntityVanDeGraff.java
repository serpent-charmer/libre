...
public class TileEntityVanDeGraff
    extends TileEntityPowerReceiver implements RangedEffect, AdjacentUpdateWatcher {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        EntityLivingBase e = ReikaWorldHelper.getClosestLivingEntityNoPlayers(
            world, x + 0.5, y + 0.75, z + 0.5, box, true
        );
        if (e != null) {
            EntityDischarge d = new EntityDischarge(
                world,
                x + 0.5,
                y + 0.75,
                z + 0.5,
    ...
    private void shock(World world, int x, int y, int z, ForgeDirection dir) {
        ...
        if (b.isAir(world, dx, dy, dz))
            return;
        this.dischargeToBlock(
            dx, dy, dz, te instanceof Shockable ? (Shockable) te : null
        );
        if (b == Blocks.tnt) {
            world.setBlockToAir(dx, dy, dz);
            EntityTNTPrimed e
                = new EntityTNTPrimed(world, dx + 0.5D, dy + 0.5D, dz + 0.5D, null);
            if (!world.isRemote)
    ...
    public void dischargeToBlock(int x, int y, int z, Shockable s) {
        ...
        if (s != null) {
            ...
            if (charge < min)
                return;
            s.onDischarge(
                charge, ReikaMathLibrary.py3d(xCoord - x, yCoord - y, zCoord - z)
            );
            dx = s.getAimX();
            dy = s.getAimY();
            dz = s.getAimZ();
