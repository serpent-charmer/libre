...
public class TileEntityMachineGun extends InventoriedPowerReceiver
    implements RangedEffect, EnchantableMachine, DiscreteFunction {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        //ReikaJavaLibrary.pConsole(tickcount+"/"+this.getFireRate()+":"+ReikaInventoryHelper.checkForItem(Items.arrow.itemID,
        //inv));

        if (tickcount >= this.getOperationTime()
            && ReikaInventoryHelper.checkForItem(Items.arrow, inv)) {
    ...
    private double getFirePower() {
        return enchantments.getEnchantment(Enchantment.power) * 0.5
            + ReikaMathLibrary.logbase(torque + 1, 2);
    }
    ...
    private void fire(World world, int x, int y, int z, int meta) {
        ...
        if (ModList.CHROMATICRAFT.isLoaded()) {
            //v *= ChromatiAPI.getAPI().adjacency().getFactorSimple(worldObj, xCoord,
            //yCoord, zCoord, "PINK");
        }
        switch (meta) {
            case 1:
    ...
    private AxisAlignedBB drawAABB(World world, int x, int y, int z, int meta) {
        ...
        switch (meta) {
            case 1:
                box.offset(1, 0, 0);
                box.maxX += Math.min(
                    ReikaWorldHelper.getFreeDistance(
                        world, x, y, z, ForgeDirection.EAST, r
                    ),
                    r
                );
                break;
            case 0:
                ...
                box.minX -= Math.min(
                    ReikaWorldHelper.getFreeDistance(
                        world, x, y, z, ForgeDirection.WEST, r
                    ),
                    r
                );
                break;
            case 2:
                ...
                box.maxZ += Math.min(
                    ReikaWorldHelper.getFreeDistance(
                        world, x, y, z, ForgeDirection.SOUTH, r
                    ),
                    r
                );
                break;
            case 3:
                ...
                box.minZ -= Math.min(
                    ReikaWorldHelper.getFreeDistance(
                        world, x, y, z, ForgeDirection.NORTH, r
                    ),
                    r
                );
                break;
