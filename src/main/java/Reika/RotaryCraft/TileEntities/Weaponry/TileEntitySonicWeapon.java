...
public class TileEntitySonicWeapon
    extends TileEntityPowerReceiver implements GuiController, RangedEffect {
    ...
    public static final long OMINOUS
        = 16; //That almost-infrasonic uneasy-feeling-generating tone; triggers blindness
              //effect
    ...
    public static final long LETHALVOLUME
        = 100000000; //10^8 W/m (~38psi)=200dB for 1-hit kill
    public static final long BRICKDESTROY
        = 1000000; //== 20kPa overpressure -> "Brick walls destroyed"
    ...
    public static final long LUNGDAMAGE
        = 2971000; //5 psi causes pulmonary damage -> cause drowning effect
    ...
    public static final long INTENSITYPERTORQUE
        = 1000000L * LETHALVOLUME / 262144; //262144L*65536L*256L*8L;
    ...
    public void applyEffects(World world, int x, int y, int z) {
        ...
        AxisAlignedBB box
            = ReikaAABBHelper.getBlockAABB(this).expand(range, range, range);
        List<EntityLivingBase> inbox
        ...
        for (EntityLivingBase ent : inbox) {
            ...
            if (ent instanceof EntityPlayer)
                if (!this.isPlayerVulnerable((EntityPlayer) ent))
                    vuln = false;
            //ReikaChatHelper.write(this.EYEDAMAGE-this.fudge*ReikaPhysicsHelper.inverseSquare(ent.posX-x-0.5,
            //ent.posY-y-0.5, ent.posZ-z-0.5, this.getMaxVolume()));
            if (vuln) {
                double vol = this.getVolume();
                for (Effects e : Effects.list) {
                    if (fudge
                            * ReikaPhysicsHelper.inverseSquare(
                                ent.posX - x - 0.5,
                                ent.posY - y - 0.5,
                                ent.posZ - z - 0.5,
                                vol
    ...
    private void killSilverfish(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < range; i++) {
            ...
            //ReikaJavaLibrary.pConsole("Block "+world.getBlock(bx, by, bz)+" @ "+bx+",
            //"+by+", "+bz);
            if (world.getBlock(bx, by, bz) == Blocks.monster_egg) {
                ...
                world.playSoundEffect(
                    bx + 0.5, by + 0.5, bz + 0.5, "mob.silverfish.kill", 1, 1
                );
                ReikaWorldHelper.splitAndSpawnXP(
                    ...
                    y + 0.5F,
                    z + 0.5F,
                    new EntitySilverfish(world).experienceValue
                );
            }
        }
    }
                ...
                case BRAIN:
                    ...
                    if (ent instanceof EntityAnimal) {
                        ...
                        if (ani.getNavigator().noPath()) {
                            ...
                            int randy = ent.worldObj.getTopSolidOrLiquidBlock(
                                (int) randx, (int) randz
                            );
                            PathEntity path
                                = ani.getNavigator().getPathToXYZ(randx, randy, randz);
                            ani.getNavigator().setPath(path, 0.2F);
                        }
                    }
                    if (ent instanceof EntityMob) {
                        AxisAlignedBB nearmob = AxisAlignedBB
                                                    .getBoundingBox(
                                                        ent.posX,
                                                        ent.posY,
                                                        ent.posZ,
                                                    ...
                                                    .expand(10, 10, 10);
                        Entity target = ent.worldObj.findNearestEntityWithinAABB(
                            EntityMob.class, nearmob, ent
                        );
