...
public class TileEntityFreezeGun extends TileEntityInventoriedCannon {
    ...
    private void convertSnow() {
        ...
        if (slot != -1
            && ReikaInventoryHelper.canAcceptMoreOf(Items.snowball, 0, 4, this)) {
        ...
        if (slot != -1
            && ReikaInventoryHelper.canAcceptMoreOf(Items.snowball, 0, 16, this)) {
            ReikaInventoryHelper.decrStack(slot, inv);
    ...
    public boolean hasAmmo() {
        return ReikaInventoryHelper.checkForItem(Blocks.ice, inv)
            || ReikaInventoryHelper.checkForItem(Items.snowball, inv);
    }
    ...
    public void fire(World world, double[] xyz) {
        ...
        if (!world.isRemote) {
            ...
            EntityFreezeGunShot snow = new EntityFreezeGunShot(
                world,
                xCoord + 0.5 + dx,
                y,
                zCoord + 0.5 + dz,
    ...
    public boolean isItemValidForSlot(int slot, ItemStack is) {
        return ReikaItemHelper.matchStackWithBlock(is, Blocks.ice)
            || ReikaItemHelper.matchStackWithBlock(is, Blocks.snow)
            || is.getItem() == Items.snowball;
    }
    ...
    protected boolean isValidTarget(Entity ent) {
        ...
        return elb.getHealth() > 0 && this.isMobOrUnlistedPlayer(elb)
            && elb.getActivePotionEffect(RotaryCraft.freeze) == null;
    }
}
