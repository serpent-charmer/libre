...
public class TileEntityLaserGun extends TileEntityAimedCannon {
    ...
    public void fire(World world, double[] xyz) {
        ...
        for (float i = 0; i <= this.getMaxRange(); i += 0.5F) {
            double dx = i * Math.cos(Math.toRadians(theta))
                * Math.cos(Math.toRadians(-phi + 90));
            double dy = i * Math.sin(Math.toRadians(theta));
            double dz = i * Math.cos(Math.toRadians(theta))
                * Math.sin(Math.toRadians(-phi + 90));
            int r = 1;
            AxisAlignedBB light = AxisAlignedBB
                                      .getBoundingBox(
                                          xCoord + dx,
                                          yCoord + dy,
                                          zCoord + dz,
    ...
    private Block getAffectedID(Block id, int meta) {
        ...
        if (id == Blocks.gravel)
            return Blocks.cobblestone;
        if (id == Blocks.tallgrass || id == Blocks.web || id == Blocks.yellow_flower
            || id == Blocks.snow || id == Blocks.red_flower || id == Blocks.red_mushroom
            || id == Blocks.brown_mushroom || id == Blocks.deadbush || id == Blocks.wheat
            || id == Blocks.carrots || id == Blocks.potatoes || id == Blocks.vine
            || id == Blocks.melon_stem || id == Blocks.pumpkin_stem
            || id == Blocks.waterlily)
            return Blocks.air;
