...
public class TileEntityFlameTurret extends TileEntityFluidCannon {
    ...
    public int getRange() {
        return tank.isEmpty()
            ? 0
            : (int
            ) (32 * damageMultipliers.get(tank.getActualFluid().getName()).rangeMultiplier
            );
    }
    ...
    public void fire(World world, double[] xyz) {
        double dist
            = ReikaMathLibrary.py3d(xyz[0] - xCoord, xyz[1] - yCoord, xyz[2] - zCoord);
        double speed = 0.25 * (Math.pow(dist, 0.7) / 7D)
            * ReikaRandomHelper.getRandomPlusMinus(1, 0.125);
        tank.removeLiquid(1);
        ...
        if (!world.isRemote) {
            ...
            world.spawnEntityInWorld(new EntityFlameTurretShot(
                world,
                xCoord + 0.5 + dx,
                y,
                zCoord + 0.5 + dz,
                ...
                v[2],
                this,
                damageMultipliers.get(tank.getActualFluid().getName())
            ));
        }
        if (this.getTicksExisted() % 34 == 0)
            SoundRegistry.FLAMETURRET.playSoundAtBlock(this, 1, 1);
        //ReikaSoundHelper.playSoundAtBlock(world, xCoord, yCoord, zCoord,
        //"mob.blaze.hit", 0.25F, 0.25F);
    }
