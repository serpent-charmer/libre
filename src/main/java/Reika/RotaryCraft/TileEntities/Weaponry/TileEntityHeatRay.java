...
public class TileEntityHeatRay extends TileEntityBeamMachine implements RangedEffect {
    ...
    private static final LaserEffect meltEffectLava
        = new MeltLaserEffect(FluidRegistry.LAVA);
    private static final LaserEffect meltEffectWater
        = new MeltLaserEffect(FluidRegistry.WATER);
    private static final LaserEffect igniteEffect = new IgnitionLaserEffect();
    private static final LaserEffect glassifyEffect
        = new BlockChangeLaserEffect(Blocks.glass);
    private static final LaserEffect sandifyEffect
        = new BlockChangeLaserEffect(Blocks.sand);
    ...
    static {
        ...
        addBlockEffect(
            Blocks.mossy_cobblestone, new BlockChangeLaserEffect(Blocks.cobblestone)
        );
        ...
        addBlockEffect(Blocks.tnt, new LaserEffect() {
            ...
            public boolean doEffect(
                ...
                int range,
                int tickcount,
                TileEntityHeatRay te
            ) {
                world.setBlockToAir(x, y, z);
                EntityTNTPrimed var6
                    = new EntityTNTPrimed(world, x + 0.5D, y + 0.5D, z + 0.5D, null);
                if (!world.isRemote)
            ...
            public boolean doEffect(
                ...
                if (!world.isRemote) {
                    ...
                    MinecraftForge.EVENT_BUS.post(
                        new HeatRayNetherDetonationEvent(world, x, y, z)
                    );
    ...
    protected void makeBeam(World world, int x, int y, int z, int metadata) {
        ...
        if (power >= MINPOWER) { //2MW+ (real military laser)
            ...
            for (step = 1; step < maxdist
                 && (step < this.getMaxRange() || this.getMaxRange() == -1) && !blocked;
                 step++) {
                ...
                if (id != Blocks.air
                    && id.isFlammable(world, dx, dy, dz, ForgeDirection.UP))
            ...
            for (Entity caught : inzone) {
                if (!(caught instanceof EntityItem)) //Do not burn drops
                    caught.setFire(this.getBurnTime()
                    ); // 1 Hearts worth of fire at min power, +1 heart for every 65kW
                       // extra
                if (caught instanceof EntityTNTPrimed)
                    world.spawnParticle(
                        "lava",
                        caught.posX + rand.nextFloat(),
                        caught.posY + rand.nextFloat(),
                        caught.posZ + rand.nextFloat(),
                        0,
                        0,
                        0
                    );
                if (caught instanceof Laserable) {
                    ((Laserable) caught)
                        .whenInBeam(
                            world,
                            MathHelper.floor_double(caught.posX),
                            MathHelper.floor_double(caught.posY),
                            MathHelper.floor_double(caught.posZ),
                            power,
                            step
                        );
                }
    ...
    private boolean affectBlock(
        World world, int dx, int dy, int dz, int step, Block id, int metadata, int maxdist
    ) {
        if (id == Blocks.air)
            return false;
