...
public class TileEntityForceField
    extends TileEntityProtectionDome implements EnchantableMachine {
    private final MachineEnchantmentHandler enchantments
        = new MachineEnchantmentHandler().addFilter(Enchantment.protection);
    ...
    public double[] getBoundaryCoord(double x, double y, double z) {
        ...
        Vec3 vec = ReikaVectorHelper.getVec2Pt(
            xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, x, y, z
        );
        vec = vec.normalize();
        xyz[0] = vec.xCoord;
        xyz[1] = vec.yCoord;
    ...
    private void protect(World world, Entity threat) {
        ...
        if (this.isAtBorder(x, y, z) || threat instanceof EntityArrow) {
            ...
            if (threat instanceof EntityFireball) {
                ...
                if (((EntityFireball) threat).shootingEntity instanceof EntityBlaze) {
                    for (int k = 0; k < 6 + rand.nextInt(7); k++)
                        world.spawnParticle(
                            "flame",
                            x - 0.2 + 0.4 * rand.nextFloat(),
                            y - 0.2 + 0.4 * rand.nextFloat(),
                            z - 0.2 + 0.4 * rand.nextFloat(),
                            0,
                            0,
                            0
                        );
            ...
            if (threat instanceof EntityPotion) {
                if (!threat.isDead) {
                    List var2 = Items.potionitem.getEffects(
                        ((EntityPotion) threat).getPotionDamage()
                    );
                    if (var2 != null && !var2.isEmpty()) {
                        AxisAlignedBB var3 = ((EntityPotion) threat)
                                                 .boundingBox.expand(4.0D, 2.0D, 4.0D);
                        List<EntityPotion> var4 = ((EntityPotion) threat)
                        ...
                        if (var4 != null && !var4.isEmpty()) {
                            ...
                            while (var5.hasNext()) {
                                ...
                                double var7
                                    = ((EntityPotion) threat).getDistanceSqToEntity(var6);
                                if (var7 < 16.0D) {
                                    ...
                                    while (var11.hasNext()) {
                                        ...
                                        if (Potion.potionTypes[var13].isInstant())
                                            Potion.potionTypes[var13].affectEntity(
                                                ((EntityPotion) threat).getThrower(),
                                                var6,
                                                var12.getAmplifier(),
                                                var9
                                            );
                                            ...
                                            int var14 = (int
                                            ) (var9 * var12.getDuration() + 0.5D);
                                            if (var14 > 20)
                                                var6.addPotionEffect(new PotionEffect(
                                                    var13, var14, var12.getAmplifier()
                                                ));
                                        }
                                    }
                                }
                    ...
                    ((EntityPotion) threat)
                        .worldObj.playAuxSFX(
                            2002,
                            (int) Math.round(((EntityPotion) threat).posX),
                            (int) Math.round(((EntityPotion) threat).posY),
                            (int) Math.round(((EntityPotion) threat).posZ),
                            ((EntityPotion) threat).getPotionDamage()
                        );
            ...
            if (InterfaceCache.IMISSILE.instanceOf(threat)) {
                ((IMissileEntity) threat)
                    .destroyMissile(this, null, 1, false, true, true);
                if (!world.isRemote)
                    world.createExplosion(null, x, y, z, 4F, true);
                tickcount = 0;
            }
            if (threat instanceof EntityMob || threat instanceof EntityGhast
                || threat instanceof EntitySlime) {
                int mult = 1;
