...
public class TileEntityLandmine extends TileEntitySpringPowered {
    ...
    private boolean checkForPlayer(World world, int x, int y, int z) {
        AxisAlignedBB above
            = AxisAlignedBB.getBoundingBox(x, y + 1, z, x + 1, y + 3, z + 1);
    ...
    private boolean checkForArrow(World world, int x, int y, int z) {
        AxisAlignedBB above
            = AxisAlignedBB.getBoundingBox(x, y, z, x + 1, y + 1, z + 1).expand(1, 1, 1);
        ...
        for (int i = 1; i < 6; i++) {
            for (int k = 0; k < 6; k++) {
                ...
                if (b instanceof SemiUnbreakable
                    && ((SemiUnbreakable) b)
                           .isUnbreakable(
                               world, dx, dy, z, world.getBlockMetadata(dx, dy, dz)
                           ))
                    continue;
                ReikaWorldHelper.recursiveBreakWithinSphere(
                    world, dx, dy, dz, world.getBlock(dx, dy, dz), -1, x, y, z, 4, 0
                );
            }
    ...
    public void detonate(World world, int x, int y, int z) {
        ...
        AxisAlignedBB region
            = AxisAlignedBB.getBoundingBox(x, y, z, x + 1, y + 1, z + 1).expand(2, 2, 2);
        ...
        for (int i = 0; i < in.size(); i++) {
            ...
            e.attackEntityFrom(
                DamageSource.setExplosionSource(
                    new Explosion(world, null, e.posX, e.posY, e.posZ, power)
                ),
                (int) power * 4
            );
                 ...
                 for (int k = -8; k <= 8; k++) {
                     if (world.getBlock(x+i, y+j, z+k) == this.getTileEntityBlockID()) {
                         ...
                         if (te instanceof TileEntityLandmine)
                             ((TileEntityLandmine)te).detonate(world, te.xCoord,
         te.yCoord, te.zCoord);
                     }
                 }
             }
