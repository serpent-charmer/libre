...
public class TileEntityEMP extends TileEntityPowerReceiver implements RangedEffect {
    ...
    public static class EMPEffect {
        ...
        public double getRadius(float ptick) {
            ...
            return Math.min(
                MAX_RANGE, 0.25 + (age + ptick) * MAX_RANGE / (double) EXPAND_LIFESPAN
            );
        }

        public float getBrightness() {
            return age <= EXPAND_LIFESPAN
                ? 1
                : 1 - ((age - EXPAND_LIFESPAN) / (float) FADE_LIFESPAN);
        }

        public int getColor1() {
            return ReikaColorAPI.getColorWithBrightnessMultiplier(
                0xffBEFFFF, this.getBrightness()
            );
        }

        public int getColor2() {
            return ReikaColorAPI.getColorWithBrightnessMultiplier(
                0xff47F2E2, this.getBrightness()
            );
        }
    }
    ...
    static { //this list is horribly incomplete
        ...
        addEntry(
            "thermalexpansion.Blocks.conduit.TileConduitRoot", ModList.THERMALEXPANSION
        );

        addEntry("ic2.core.Blocks.wiring.TileEntityCable", ModList.IC2);

        addEntry(
            "codechicken.enderstorage.common.TileFrequencyOwner", ModList.ENDERSTORAGE
        );
    ...
    private void fire(World world, int x, int y, int z) {
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            RotaryCraft.packetChannel, PacketRegistry.EMPEFFECT.ordinal(), this, 128
        );
    ...
    private void destroySelf(World world, int x, int y, int z) {
        ...
        if (ReikaRandomHelper.doWithChance(50)) {
            ...
            items.add(
                ReikaItemHelper.getSizedItemStack(ItemStacks.scrap, rand.nextInt(16))
            );
            ...
            ReikaItemHelper.dropItem(
                ...
                y + 0.5,
                z + 0.5,
                ReikaItemHelper.getSizedItemStack(ItemStacks.scrap, 8 + rand.nextInt(16))
            );
        }
    }
    ...
    private void shutdownTE(TileEntity te) {
        ...
        ReikaPacketHelper.sendDataPacketToEntireServer(
            ...
            PacketRegistry.SPARKLOC.ordinal(),
    ...
    private void shutdownFallback(TileEntity te) {
        ...
        TileEntityDeadMachine dead = (TileEntityDeadMachine)worldObj.getTileEntity(x, y,
        z); dead.setBlock(b, id, meta); dead.setInvSize(inv.length); for (int i = 0; i <
        inv.length; i++) { dead.setInventorySlotContents(i, inv[i]);
        }*/
    }

    public static boolean isShutdown(TileEntity te) {
        return !shutdownLocations.isEmpty()
            && shutdownLocations.contains(new WorldLocation(te));
    }

    public static boolean isShutdown(World world, int x, int y, int z) {
        return !shutdownLocations.isEmpty()
            && shutdownLocations.contains(new WorldLocation(world, x, y, z));
    }
    ...
    private void dropMachine(World world, int x, int y, int z) {
        ...
        ;
        if (b != null) {
            //ReikaItemHelper.dropItems(world, x+0.5, y+0.5, z+0.5, b.getDrops(world, x,
            //y, z, meta, 0));
