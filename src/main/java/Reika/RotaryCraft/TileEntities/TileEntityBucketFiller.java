...
public class TileEntityBucketFiller extends InventoriedPowerReceiver
    implements PipeConnector, IFluidHandler, DiscreteFunction, ConditionalOperation {
                ...
                if (fluid != null) {
                    if (this.canAccept(fluid.getFluid())) {
                        if (tank.getCapacity() >= fluid.amount + tank.getLevel()) {
                            ItemStack is
                                = FluidContainerRegistry.drainFluidContainer(slot);
    ...
    private void fillBuckets() {
        for (int i = 0; i < inv.length; i++) {
            ...
            if (slot != null && FluidContainerRegistry.isEmptyContainer(slot)) {
                ItemStack is
                    = FluidContainerRegistry.fillFluidContainer(tank.getFluid(), slot);
                if (is != null) {
