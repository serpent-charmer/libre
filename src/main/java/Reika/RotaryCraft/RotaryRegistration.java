...
public class RotaryRegistration {
    public static void addTileEntities() {
        for (int i = 0; i < MachineRegistry.machineList.length; i++) {
            String label = "RC"
                + MachineRegistry.machineList.get(i)
                      .name()
            ...
            if (label.equals(aux)) {
                GameRegistry.registerTileEntity(
                    MachineRegistry.machineList.get(i).getTEClass(), label
                );
            } else {
                GameRegistry.registerTileEntityWithAlternatives(
                    MachineRegistry.machineList.get(i).getTEClass(), label, aux
                );
            }
            ReikaJavaLibrary.initClass(
                MachineRegistry.machineList.get(i).getTEClass(), true
            );
        }
        for (int i = 0; i < EngineType.engineList.length; i++) {
            String label = "RC"
                + EngineType.engineList[i]
                      .name()
                      .toLowerCase(Locale.ENGLISH)
                      .replaceAll("\\s", "");
    ...
    public static void addEntities() {
        ReikaRegistryHelper.registerModEntities(
            RotaryCraft.instance, RotaryEntities.entityList
        );
    }

    public static void loadOreDictionary() {
        if (!ModList.GREGTECH.isLoaded(
            )) { //GT unificator causes an exploit, and no mods even use this anyways
            OreDictionary.registerOre(
                "ingotHSLA", ItemStacks.steelingot
            ); //though he has an entry, he does not add an alternative manufacture
            OreDictionary.registerOre(
                "RotaryCraft:ingotTungstenAlloy", ItemStacks.tungsteningot
            ); //though he has an entry, he does not add an alternative manufacture
            ...
            RotaryCraft.logger.log(
                "Gregtech is installed, a few OreDict entries are not being added to prevent interchangeability or unification issues"
            );
        }
        ...
        OreDictionary.registerOre(
            "glassHardened", BlockRegistry.BLASTGLASS.getBlockInstance()
        );
        OreDictionary.registerOre(
            "blockGlassHardened", BlockRegistry.BLASTGLASS.getBlockInstance()
        );
        ...
        OreDictionary.registerOre(
            ModList.GREGTECH.isLoaded() ? "dustSmallWood" : "dustWood", ItemStacks.sawdust
        );
    ...
    public static void auxRegistration() {
        ReikaItemHelper.registerItemMass(
            ItemRegistry.STEELBOOTS.getItemInstance(), ReikaEngLibrary.rhoiron, 4
        );
        ReikaItemHelper.registerItemMass(
            ItemRegistry.STEELCHEST.getItemInstance(), ReikaEngLibrary.rhoiron, 8
        );
        ReikaItemHelper.registerItemMass(
            ItemRegistry.STEELLEGS.getItemInstance(), ReikaEngLibrary.rhoiron, 7
        );
        ReikaItemHelper.registerItemMass(
            ItemRegistry.STEELHELMET.getItemInstance(), ReikaEngLibrary.rhoiron, 5
        );
        ...
        ReikaItemHelper.registerItemMass(
            ItemRegistry.STEELPACK.getItemInstance(),
            ReikaEngLibrary.rhoiron * packFactor,
            8
        );
        ReikaItemHelper.registerItemMass(
            ItemRegistry.JUMP.getItemInstance(), ReikaEngLibrary.rhoiron * springFactor, 4
        );

        double bedFactor = 1.25;
        ReikaItemHelper.registerItemMass(
            ItemRegistry.BEDBOOTS.getItemInstance(),
            ReikaEngLibrary.rhoiron * bedFactor,
            4
        );
        ReikaItemHelper.registerItemMass(
            ItemRegistry.BEDCHEST.getItemInstance(),
            ReikaEngLibrary.rhoiron * bedFactor,
            8
        );
        ReikaItemHelper.registerItemMass(
            ItemRegistry.BEDLEGS.getItemInstance(), ReikaEngLibrary.rhoiron * bedFactor, 7
        );
        ReikaItemHelper.registerItemMass(
            ItemRegistry.BEDHELM.getItemInstance(), ReikaEngLibrary.rhoiron * bedFactor, 5
        );

        ReikaItemHelper.registerItemMass(
            ItemRegistry.BEDPACK.getItemInstance(),
            ReikaEngLibrary.rhoiron * bedFactor * packFactor,
            8
        );
        ReikaItemHelper.registerItemMass(
            ItemRegistry.BEDJUMP.getItemInstance(),
            ReikaEngLibrary.rhoiron * bedFactor * springFactor,
            4
        );
    }
    ...
    public static void setupLiquids() {
        ...
        FluidContainerRegistry.registerFluidContainer(
            new FluidStack(RotaryCraft.lubeFluid, FluidContainerRegistry.BUCKET_VOLUME),
            ItemRegistry.BUCKET.getStackOfMetadata(0),
            new ItemStack(Items.bucket)
        );
        FluidContainerRegistry.registerFluidContainer(
            new FluidStack(
                RotaryCraft.jetFuelFluid, FluidContainerRegistry.BUCKET_VOLUME
            ),
            ItemRegistry.BUCKET.getStackOfMetadata(1),
            new ItemStack(Items.bucket)
        );
        FluidContainerRegistry.registerFluidContainer(
            new FluidStack(
                RotaryCraft.ethanolFluid, FluidContainerRegistry.BUCKET_VOLUME
            ),
        ...
        FluidContainerRegistry.registerFluidContainer(
            new FluidStack(
                RotaryCraft.nitrogenFluid, FluidContainerRegistry.BUCKET_VOLUME
            ),
            ItemRegistry.BUCKET.getStackOfMetadata(3),
            new ItemStack(Items.bucket)
        );
        FluidContainerRegistry.registerFluidContainer(
            new FluidStack(RotaryCraft.hslaFluid, FluidContainerRegistry.BUCKET_VOLUME),
            ItemRegistry.BUCKET.getStackOfMetadata(4),
            new ItemStack(Items.bucket)
        );
    }
    ...
    public static void setupLiquidIcons(TextureStitchEvent.Pre event) {
        ...
        if (event.map.getTextureType() == 0) {
            ...
            IIcon nitrogenicon
                = event.map.registerIcon("RotaryCraft:fluid/nitrogen_anim");
