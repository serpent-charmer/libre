...
public class ContainerCraftingPattern extends Container {
        ...
        for (int i = 0; i < height; i++) {
            for (int k = 0; k < width; k++) {
                this.addSlotToContainer(
                    new Slot(craftMatrix, i * width + k, 30 + k * 18, 17 + i * 18)
                );
            }
        }
    ...
    public void onCraftMatrixChanged(IInventory ii) {
        ...
        ItemStack out = ItemRegistry.CRAFTPATTERN.matchItem(is)
            ? ItemCraftPattern.getMode(is).getRecipe(craftMatrix, player.worldObj)
            : null;
