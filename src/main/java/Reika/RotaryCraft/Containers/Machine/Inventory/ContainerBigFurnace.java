...
public class ContainerBigFurnace extends ContainerIOMachine {
    ...
    public ContainerBigFurnace(
        EntityPlayer player, TileEntityBigFurnace par2TileEntityBigFurnace
    ) {
        ...
        for (int i = 0; i < te.getNumberInputSlots(); i++) {
            int row = i % 9;
            int col = i / 9;
            Slot slot = new SlotFurnace(
                player, te, i + te.getNumberInputSlots(), 8 + row * dx, 72 + col * dy
            );
            this.addSlotToContainer(slot);
        }
