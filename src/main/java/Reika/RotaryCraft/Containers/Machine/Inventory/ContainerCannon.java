...
public class ContainerCannon extends ContainerIOMachine {
    ...
    public ContainerCannon(
        EntityPlayer player, TileEntityLaunchCannon par2TileEntityCannon
    ) {
        ...
        for (int j = 0; j < cannon.getSizeInventory(); j++)
            this.addSlotToContainer(
                new Slot(par2TileEntityCannon, j, dx + 8 + j * 18, dy + 18)
            );
        dy += 40;
        dx = 18;
        for (i = 0; i < 3; i++)
            for (int k = 0; k < 9; k++)
                this.addSlotToContainer(new Slot(
                    player.inventory, k + i * 9 + 9, dx + 8 + k * 18, dy + i * 18
                ));
        dy += 58;
        for (int j = 0; j < 9; j++)
