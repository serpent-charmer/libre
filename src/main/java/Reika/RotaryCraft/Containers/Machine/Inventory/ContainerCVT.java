...
public class ContainerCVT extends ContainerIOMachine {
    ...
    public ContainerCVT(
        EntityPlayer player, TileEntityAdvancedGear par2TileEntityAdvancedGear
    ) {
        ...
        for (int i = 0; i < 3; i++) {
            for (k = 0; k < 9; k++) {
                this.addSlotToContainer(new Slot(
                    player.inventory, k + i * 9 + 9, 8 + k * 18 + dx, 84 + i * 18 + dy
                ));
            }
        }
        dy -= 4;
        for (int j = 0; j < 9; j++) {
            this.addSlotToContainer(
                new Slot(player.inventory, j, 8 + j * 18 + dx, 142 + dy)
            );
        }
    }
}
