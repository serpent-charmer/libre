...
public class ContainerWorktable extends CraftingContainer<WorktableRecipe> {
    ...
    public ContainerWorktable(
        EntityPlayer player, TileEntityWorktable te, World world, boolean gui
    ) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                this.addSlotToContainer(
                    new Slot(te, i * 3 + j, dx + 26 + j * 18, 17 + i * 18)
                );
            }
        }
        dx += 96 - 28 + 4;
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                this.addSlotToContainer(new SlotFurnace(
                    player, te, 9 + i * 3 + j, dx + 26 + j * 18, 17 + i * 18
                ));
            }
        }

        this.addSlotToContainer(new SlotApprovedItems(te, 18, 6, 53)
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                //this.addSlotToContainer(new GhostSlot(te, 18+i*3+j, dx+26+j*18,
        dy+i*18)); this.addSlotToContainer(new SlotXItems(te, 18+i*3+j, dx+26+j*18,
        dy+i*18, 1));
            }
        }
         */
