...
public class ContainerItemCannon extends ContainerIOMachine {
    ...
    public ContainerItemCannon(
        EntityPlayer player, TileEntityItemCannon par2TileEntityItemCannon
    ) {
        ...
        for (int j = 0; j < 9; j++)
            this.addSlotToContainer(new Slot(
                par2TileEntityItemCannon, j + 9 * i, 8 + j * 18, dy + i * 18 + 18
            ));
        dy += 40;
        for (i = 0; i < 3; i++)
            for (int k = 0; k < 9; k++)
                this.addSlotToContainer(
                    new Slot(player.inventory, k + i * 9 + 9, 8 + k * 18, dy + i * 18)
                );
        dy += 58;
        for (int j = 0; j < 9; j++)
