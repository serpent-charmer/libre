...
public class ContainerTerraformer extends ContainerIOMachine {
    ...
    public ContainerTerraformer(EntityPlayer player, TileEntityTerraformer te) {
        ...
        for (int j = 0; j < 3; ++j) {
            for (int k = 0; k < 9; ++k) {
                this.addSlotToContainer(new Slot(
                    player.inventory, k + j * 9 + 9, dx + 8 + k * 18, 103 + j * 18 + dy
                ));
            }
        }
        ...
        for (int j = 0; j < 9; ++j) {
            this.addSlotToContainer(
                new Slot(player.inventory, j, dx + 8 + j * 18, 161 + dy)
            );
        }
    }
}
