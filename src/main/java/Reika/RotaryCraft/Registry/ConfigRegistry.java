...
public enum ConfigRegistry implements SegmentedConfigList, SelectiveConfig, IntegerConfig,
                                      BooleanConfig, DecimalConfig, StringConfig,
                                      MatchingConfig, UserSpecificConfig {
    ENGINEVOLUME("Engine Volume", 1F),
    GPRORES("GPR Renders Ores", true),
    INSTACUT("Instant Woodcutter", true), //Whether the woodcutter cuts blocks instantly
                                          //as opposed to making them fall to the ground
    ...
    DIFFICULTY(
        "Difficulty Control", 2
    ), //1-3 for easy/medium/hard; controls some recipe/fuel costs, failure chances, and
       //other numerical tweaks. Progression is <b>not</b> changed.
    //ALARM("Machine Warning Alarms", false), //Whether to
    BIOMEBLOCKS(
        "Terraformer Block Editing", true
    ), //Whether the terraformer can not only change the biome but set blocks, eg spawning
       //trees
    DYNAMICHANDBOOK("Reload Handbook Data on Open", false),
    TABLEMACHINES(
        "Crafting Table can Make Machines", false
    ...
    HSLADICT(
        "Allow RC steel to be used in other mods", false
    ...
    CONSERVEPACK(
        "Conservative Jetpack Firing", true
    ), //Whether to make the jetpack be a bit less likely to fire from things like
       //jumping, helping reduce fuel usage at the cost of making it a bit less immediate
    ...
    EXTRAIRON(
        "Iron Ore Density", 1F
    ), //If set to >1, this will spawn extra iron ore in the world, to help with the fact
       //RC uses a lot of it. Does nothing at <= 1
    TEGLASS("Allow Blast Glass to be Used as TE Hardened Glass", false),
    CLEARCHAT(
        "Tools Clear Chat", true
    ), //Whether using a chat-writing tool will clear your chat buffer to make it easier
       //to read their output
    KICKFLYING("Jetpack bypasses allow-flight property", true),
    BLOWERSPILL(
        "Item Pump Spills Items If Dumping To Air", true
    ...
    HARDGRAVELGUN(
        "Hardmode Gravel Gun", false
    ), //Adjusts charge-damage curves to require more charge per unit damage and cap the
       //damage at a lower value
    BORERMAINTAIN(
        "Borer Requires Maintenance", false
    ), //Makes the borer's operation require a drill item it slowly consumes
    NOMINERS("Disable Automining Machines", false),
    HARDEU(
        "Hard Mode EU Compatibility", ModList.GREGTECH.isLoaded()
    ), //TypeHelper to Website Generator: boolean; Whether to make EU compat more
       //difficult, suitable for GT packs
    ...
    POWERCLIENT(
        "Run power transfer code on client", false
    ), //Whether to also run power sync code on the client, to help with sync at the cost
       //of additional load on the client tick
    //TUTORIAL("Tutorial Mode", false),
    FRAMES("Allow Frames to move Machines (May cause corruption)", false),
    CONVERTERLOSS(
        "Power Converter Loss Percent", 0
    ), //Efficiency of the RC-to-other-mod converters. 0% loss = 100% efficiency
    FAKEBEDROCK(
        "Allow special bedrock tool abilities in automation", true
    ), //Whether things like the bedrock axe and its tree cutting will apply when used by
       //fake players, eg auto activators
    BORERGEN(
        "Borer Chunk Gen Radius", 0
    ...
    CRAFTERPROFILE(
        "AutoCrafter Lag Profiling And Compensation", true
    ), //Make the autocrafter tick less often to reduce lag
    HSLAHARVEST("Increased Harvest Level for HSLA tools", false),
    LATEDYNAMO(
        "Rotational Dynamo Recipe Difficulty", 0
    ), //0-5 to choose a gating item; see above
    BORERPOW("Borer Power Requirement Factor", 1F),
    BEEYEAST(
        "Use Forestry Bees To Produce Yeast", 0
    ), //0 = disabled, 1 = slippery bee also produces yeast (alternate source); 2 = the
       //bee is the only source of yeast
    HARDCONVERTERS("Harder Converter Unit Recipes", false),
    OREALUDUST("Allow other mods' aluminum dust to make Silicon", false),
    GATEBLAST(
        "Enable Blast Furnace recipe gating", false
    ), //Whether to enable the recipe gate functionality for the blast furnace
    GATEWORK(
        "Enable Worktable recipe gating", false
    ), //Whether to enable the recipe gate functionality for the worktable
    VACPOWER(
        "Item Vacuum Power Per Meter", (int) PowerReceivers.VACUUM.getMinPower() / 4
    ), //TypeHelper to Website Generator: int
    HYDROSTREAMFALLMAX(
        "Streams Waterfall Min Height for Max Hydrokinetic Yield", 8
    ...
    IC2BLAZECOMPRESS(
        "Increase Blaze Powder To Rod Cost In IC2 Compressor (Exploit Fix)", true
    ), //Normally the ic2 compressor requires 5 powder per rod, but as the grinder makes
       //six powder per rod this is a feedback exploit. This option increases the ic2 cost
       //to 8.
    FREEWATER(
        "Free Water Production Factor", 1F
    ), //How much if any free water can be produced by things like the pump power surplus,
       //spillway "scraping" (ie back to a waterfall) and rain in reservoirs
    ...
    public int getValue() {
        if (this == DIFFICULTY) {
            return EnumDifficulty
                .getBoundedDifficulty(
                    (Integer) RotaryCraft.config.getControl(this.ordinal()),
                    RotaryConfig.EASIEST,
                    RotaryConfig.HARDEST
                )
                .ordinal();
        }
