...
public enum GearboxTypes {
        ...
        switch (this) {
            ...
            case TUNGSTEN:
                return Math.min(
                    1,
                    0.5F
                        + Math.max(0, 0.03125F * (ReikaMathLibrary.logbase2(omegain) - 2))
                );
            default:
                return 1;
        }
    ...
    public boolean acceptsBearingUpgrade(GearboxTypes mat) {
        switch (this) {
            ...
            default:
                return mat != WOOD && mat.material.ordinal() <= material.ordinal() + 2
                    && !mat.isSpecialType();
        }
    }
    ...
    public String getLocalizedGearboxName(int ratio) {
        return StatCollector.translateToLocal(
            ...
            + " " + ratio + ":1 " + MachineRegistry.GEARBOX.getName();
    }
    ...
    public double getOmegaForRotFailure(int omega, int omegain) {
        return ReikaMathLibrary.doubpow(
            Math.max(omega, omegain), material.getSpeedForceExponent()
        );
    }
