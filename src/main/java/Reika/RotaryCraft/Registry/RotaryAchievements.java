...
public enum RotaryAchievements {
    ...
    CVT(-2,
        -4,
        MachineRegistry.ADVANCEDGEARS.getCraftedMetadataProduct(1),
        STEELSHAFT,
        false), //make
    BEDROCKSHAFT(
        -4, 6, MaterialRegistry.BEDROCK.getShaftItem(), BEDROCKBREAKER, false
    ), //make
    ...
    LIGHTFALL(
        8, -6, MachineRegistry.LIGHTBRIDGE, JETENGINE, false
    ), //light bridge turns off, drops you to death
    SPRINKLER(-6, -2, MachineRegistry.SPRINKLER, PUMP, false), //turn on
    FLOODLIGHT(
        -1, -1, MachineRegistry.FLOODLIGHT, MAKESTEEL, false
    ), //turn on at Light 15
    DAMAGEGEARS(-4, -2, ItemStacks.gearunit, STEELSHAFT, false),
    DIAMONDGEARS(
        -4, -4, GearboxTypes.DIAMOND.getGearboxItem(8), DAMAGEGEARS, false
    ), //make
    MRADS32(2, -6, ItemRegistry.METER, JETFUEL, true), //transmit power at 32Mrad/s
    GIGAWATT(
        6, 0, Blocks.redstone_block, JETENGINE, true
    ), //transmit 1GW of power in one shaft w/o breaking
    RAILDRAGON(2, 8, Blocks.dragon_egg, MAKERAILGUN, true), //kill dragon with railgun
    RAILKILLED(
        0, 10, new ItemStack(Items.skull, 1, 0), MAKERAILGUN, false
    ), //kill self with railgun
    ...
    NETHERHEATRAY(
        4, -2, MachineRegistry.HEATRAY, JETFUEL, true
    ), //dig 500m with heat ray in nether
    ...
    RAREEXTRACT(
        4, 0, ExtractorModOres.getFlakeProduct(ModOreList.PLATINUM), EXTRACTOR, true
    ),
    ...
    private RotaryAchievements(
        int x, int y, Item icon, RotaryAchievements preReq, boolean special
    ) {
        this(x, y, new ItemStack(icon), preReq, special);
    }

    private RotaryAchievements(
        int x, int y, Block icon, RotaryAchievements preReq, boolean special
    ) {
        this(x, y, new ItemStack(icon), preReq, special);
    }

    private RotaryAchievements(
        int x, int y, ItemRegistry icon, RotaryAchievements preReq, boolean special
    ) {
        this(x, y, icon.getStackOf(), preReq, special);
    }

    private RotaryAchievements(
        int x, int y, MachineRegistry icon, RotaryAchievements preReq, boolean special
    ) {
        this(x, y, icon.getCraftedProduct(), preReq, special);
    }

    private RotaryAchievements(
        int x, int y, ItemStack icon, RotaryAchievements preReq, boolean special
    ) {
        xPosition = x;
        yPosition = y;
        dependency = preReq;
    ...
    public void triggerAchievement(EntityPlayer ep) {
        ...
        if (ep == null) {
            if (FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER) {
                //ReikaChatHelper.write("Player does not exist to receive their
                //achievement \""+this+"\"!"); ReikaJavaLibrary.pConsole("Player does not
                //exist to receive their achievement \""+this+"\"!");
                RotaryCraft.logger.debug(
                    "Player does not exist to receive their achievement \"" + this + "\"!"
                );
            }
        } else {
    ...
    public static void registerAchievements() {
        ...
        for (int i = 0; i < list.length; i++) {
            ...
            Achievement ach = new Achievement(
                a.name().toLowerCase(Locale.ENGLISH),
                a.name().toLowerCase(Locale.ENGLISH),
                a.xPosition,
                a.yPosition,
                a.iconItem,
            ...
            //	throw new IDConflictException(RotaryCraft.instance, "The mod's achievement
            //IDs are conflicting with another at ID "+id+" ("+a+" is trying to overwrite
            //"+StatList.getOneShotStat(id).statName+").\nCheck the config file and change
            //them.");
            if (a.isSpecial)
                ach.setSpecial();
            ...
            RotaryCraft.logger.log(
                "Registering achievement " + a + " with ID " + id + " and ingame name \""
                + a + "\" (slot " + i + ")."
            );
        }
        AchievementPage.registerAchievementPage(
            new RCAchievementPage("RotaryCraft", RotaryCraft.achievements)
        );
    }
}
