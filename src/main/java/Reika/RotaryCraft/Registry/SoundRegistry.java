...
public enum SoundRegistry implements CustomDistanceSound {
    ...
    playSound(World world, double x, double y, double z, float vol, float pitch) {
        if (FMLCommonHandler.instance().getEffectiveSide() != Side.SERVER)
            return;
        //Packet250CustomPayload p = new Packet62LevelSound(s.getPlayableReference(), x,
        //y, z, vol, pitch); PacketDispatcher.sendPacketToAllInDimension(p,
    ...
    public void playSoundNoAttenuation(
        ...
        if (world.isRemote)
            return;
        //ReikaSoundHelper.playSound(this, RotaryCraft.packetChannel, te.worldObj, x, y,
    ...
    public URL getURL() {
        if (this.isNote())
            return RotaryCraft.class.getResource(
                SOUND_DIR + MUSIC_FOLDER + name + SOUND_EXT
            );
        else
    ...
    public boolean isNote() {
        return (
            this.name().contains("HARP") || this.name().contains("BASS")
            || this.name().contains("PLING")
        );
    }

    public static SoundRegistry
    getNoteFromVoiceAndPitch(SoundRegistry voice, String pitch) {
