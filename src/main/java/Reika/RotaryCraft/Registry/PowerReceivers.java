...
public enum PowerReceivers {
    ...
    AUTOBREEDER(16384),
    BAITBOX(32768),
    BEDROCKBREAKER(16384 /*65536*/, 1, 4194304 /*16777216*/), // //cannot need more torque
                                                              // w/o breaking diamond
                                                              // gearboxes
    BORER(),
    LIGHTBRIDGE(33554432),
    CAVESCANNER(131072),
    CHUNKLOADER(1, 2097152, 1),
    COMPACTOR(4096, 1, 262144),
    EXTRACTOR(
        new int[] { 512, 1, 1, 256 },
        new int[] { 1, 2048, 8192, 1 },
        new long[] { 65536, 16384, 32768, 65536 }
    ),
    FAN(1024),
    FERMENTER(1, 32, 1024),
    ...
    PILEDRIVER(
        80000, 1, TileEntityPileDriver.BASEPOWER
    ), //4000kg hammer * 10 m/s^2 / 0.5m spool
    PLAYERDETECTOR(),
    PROJECTOR(512),
    PULSEJET(1, 131072, 1),
    PUMP(8, 1, 1024),
    ...
    public long getMinPower(int i) {
        if (!this.hasMultiValuedPower())
            throw new RuntimeException(
                ...
                + " cannot access mutlivalued power! Use direct power!"
            );
        return powers[i];
    }
    ...
    public int getMinTorque() {
        if (this.hasMultiValuedPower())
            throw new RuntimeException(
                "Machine " + this.getName()
                + " cannot access Direct Power! Use multivalued power!"
            );
        return minT;
    }
        ...
        if (m.isPowerReceiver()) {
            ...
            if (name == null)
                throw new RegistrationException(
                    ...
                    m + " does not correspond to an existing machine Enum!"
                );
            ...
            throw new RegistrationException(
                RotaryCraft.instance,
                "This should not happen! Machine " + name
                    + " is a Power Receiver yet has no Power enum! Machine enum: " + em
            );
            //RotaryCraft.logger.logError("This should not happen! Machine "+name+" is a
            //Power Receiver yet has no Power enum! Machine enum: "+em); return null;
        } else
            return null;
    }
    ...
    public MachineRegistry getMachine() {
        ...
        RotaryCraft.logger.logError(
            "This should not happen! Power Receiver " + this + " has no machine!"
        );
        return null;
    }
