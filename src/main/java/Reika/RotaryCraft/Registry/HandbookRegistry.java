...
public enum HandbookRegistry implements HandbookEntry {
    ...
    private HandbookRegistry(MachineRegistry m, EngineType o) {
        ...
        item = null;
        crafted
            = offset >= 0 ? m.getCraftedMetadataProduct(offset) : m.getCraftedProduct();
    }
    ...
    public int getShiftedOrdinal() {
        return this.getParent().getBaseScreen() + this.ordinal()
            - this.getParent().ordinal();
    }
    ...
    private static int getEnginePage(TileEntity te) {
        ...
        return 1 + e.ordinal()
            - (getEngineScreen(te) - ENGINEDESC.getBaseScreen())
            * GuiHandbook.PAGES_PER_SCREEN;
    }
    ...
    public String getTabImageFile() {
        //return
        //"/Reika/RotaryCraft/Textures/GUI/Handbook/tabs_"+this.getParent().name().toLowerCase()+".png";
        return "/Reika/RotaryCraft/Textures/GUI/Handbook/tabs_"
            + TOC.getParent().name().toLowerCase(Locale.ENGLISH) + ".png";
    }
    ...
    public static void addRelevantButtons(int j, int k, int screen, List<GuiButton> li) {
        ...
        for (int i = 0; i < tabList.length; i++) {
            if (tabList[i].getScreen() == screen /* && !tabList[i].isDummiedOut()*/) {
                li.add(new ImagedGuiButton(
                    id,
                    j - 20,
                    k + tabList[i].getRelativeTabPosn() * 20,
                    20,
                    20,
                    0,
                    0,
                    tabList[i].getTabImageFile(),
                    RotaryCraft.class
                ));
                //ReikaJavaLibrary.pConsole("Adding "+tabList[i]+" with ID "+id+" to
                //screen "+screen);
                id++;
            }
        }
    ...
    public static HandbookRegistry getEntry(int screen, int page) {
        ...
        return h != null ? h : TOC;
        //throw new RuntimeException("Handbook screen "+screen+" and page "+page+" do not
    ...
    public List<ItemStack> getCrafting() {
        ...
        if (this == SOLAR)
            return ReikaJavaLibrary.makeListFrom(
                MachineRegistry.SOLARTOWER.getCraftedProduct()
            );
        if (this.getParent() == ENGINEDESC)
            return ReikaJavaLibrary.makeListFrom(
                EngineType.engineList[offset].getCraftedProduct()
            );
        if (machine == MachineRegistry.ADVANCEDGEARS)
            return ReikaJavaLibrary.makeListFrom(
        ...
        if (this == ALLOYING) {
            ...
            ArrayList<BlastFurnacePattern> li
    ...
    public ItemStack getTabIcon() {
        ...
        if (this == UPGRADES)
            return ItemRegistry.UPGRADE.getStackOfMetadata(
                Upgrades.MAGNETOSTATIC4.ordinal()
            );
        if (this == BEDTOOLS)
    ...
    public boolean hasSubpages() {
        ...
        if (this == PACKMODS)
            return PackModificationTracker.instance.modificationsExist(
    ...
    public boolean isDummiedOut() {
        ...
        if (this == PACKMODS)
            return
    !PackModificationTracker.instance.modificationsExist(RotaryCraft.instance); return
    false;
    }
     */
    ...
    private static Comparator<HandbookRegistry> tabSorter
        = new Comparator<HandbookRegistry>() {
              @Override
