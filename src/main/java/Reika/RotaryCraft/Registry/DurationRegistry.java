...
public enum DurationRegistry {
    ...
    /** 4-stage */
    private DurationRegistry(
        MachineRegistry m, int b1, int s1, int b2, int s2, int b3, int s3, int b4, int s4
    ) {
        this(
            m,
            new DurationFormula(b1, s1),
            new DurationFormula(b2, s2),
            new DurationFormula(b3, s3),
            new DurationFormula(b4, s4)
        );
    }
    ...
    public int getOperationTime(int omega, int stage) {
        ...
        try {
            return (int
            ) Math.max(this.getMinOperationTime(), exps[stage].evaluate(omega));
