...
public enum ItemRegistry implements ItemEnum {
    ...
    GRAFTER(
        37, 1, true, "item.chargedgrafter", ItemChargedGrafter.class, ModList.FORESTRY
    ),
    BEDGRAFTER(
        38, 1, false, "item.bedgrafter", ItemBedrockGrafter.class, ModList.FORESTRY
    ),
    ...
    //BEDKNIFE(41, 1, false,			"item.bedknife",			ItemBedrockKnife.class,
    //ModList.APPENG);
    MATCHFILTER(50, 1, false, "item.matchfilter", ItemMatchFilter.class),
    BEDDRILL(
        134, 1, false, "item.beddrill", ItemBedrockDrillHead.class, ModList.IMMERSIVEENG
    ),
    BEDCHISEL(45, 1, false, "item.bedchisel", ItemBedrockChisel.class, ModList.CHISEL),
    SCREWFOCUS(
        0, 0, false, "item.screwfocus", ItemScrewdriverFocus.class, ModList.THAUMCRAFT
    ),
    ;
    ...
    private ItemRegistry(
        int tex, int sheet, boolean sub, String n, Class<? extends Item> iCl
    ) {
        this(tex, sheet, sub, n, iCl, null);
    }

    private ItemRegistry(
        int tex, int sheet, boolean sub, String n, Class<? extends Item> iCl, ModList api
    ) {
        index = tex;
        hasSubtypes = sub;
        name = n;
    ...
    private ItemRegistry(
        int lotex, int hitex, int sheet, boolean sub, String n, Class<? extends Item> iCl
    ) {
        if (lotex > hitex)
            throw new RegistrationException(
                RotaryCraft.instance,
                "Invalid item sprite registration for " + n
                    + "! Backwards texture bounds?"
            );
        index = lotex;
        maxindex = lotex;
        hasSubtypes = sub;
        ...
        if (this.isArmor()) {
            if (this.isBedrockArmor() || this.isSteelArmor())
                return new Class[] { int.class,
                                     int.class,
                                     int.class }; // Armor render, Sprite index, armor
                                                  // type
            else if (this.isJetpack() || this.isJumpBoots())
                return new Class[] { ArmorMaterial.class,
                                     int.class,
                                     int.class }; // Material, Sprite index, Armor render
            else
    ...
    public Object[] getConstructorParams() {
        if (this.isArmor()) {
            if (this.isBedrockArmor() || this.isSteelArmor())
                ...
                return new Object[] { this.getArmorMaterial(),
                                      this.getTextureIndex(),
                                      this.getArmorRender() };
            else
    ...
    public String getMultiValuedName(int dmg) {
        ...
        switch (this) {
            ...
            case RAILGUN:
                return this.getBasicName() + " ("
                    + String.format("%d", (int) ReikaMathLibrary.intpow(2, dmg)) + " kg)";
            case UPGRADE:
            ...
            case GEARBOX:
                return this.getBasicName();
            case FLYWHEEL:
                return Flywheels.list[dmg].getName() + " "
                    + MachineRegistry.FLYWHEEL.getName();
            case ADVGEAR:
        ...
        throw new RuntimeException(
            "Item " + name + " was called for a multi-name, but it was not registered!"
        );
    }
    ...
    public int getArmorRender() {
        if (!this.isArmor())
            throw new RegistrationException(
                RotaryCraft.instance,
                "Item " + name + " is not an armor yet was called for its render!"
            );
        if (this == IOGOGGLES)
        ...
        throw new RegistrationException(
            RotaryCraft.instance,
            "Item " + name + " is an armor yet has no specified render!"
        );
    }
    ...
    public boolean isSteelTool() {
        return this == STEELPICK || this == STEELAXE || this == STEELSHOVEL
            || this == STEELSWORD || this == STEELSHEARS || this == STEELHOE
            || this == STEELSICKLE;
    }
    ...
    public boolean isCharged() {
        ...
        return ItemChargedTool.class.isAssignableFrom(itemClass)
            || ItemChargedArmor.class.isAssignableFrom(itemClass);
    }

    public boolean isBedrockTool() {
        return this == BEDPICK || this == BEDAXE || this == BEDSHOVEL || this == BEDSWORD
            || this == BEDSHEARS || this == BEDHOE || this == BEDSICKLE
            || this == BEDGRAFTER || this == BEDSAW;
    }
        ...
        switch (this) {
            ...
            case MODINGOTS:
                return ReikaJavaLibrary
                    .getEnumEntriesWithoutInitializing(ModOreList.class)
                    .size();
            case CUSTOMEXTRACT:
            ...
            case MACHINE:
                return ReikaJavaLibrary
                    .getEnumEntriesWithoutInitializing(MachineRegistry.class)
                    .size();
            default:
                throw new RegistrationException(
                    RotaryCraft.instance,
                    "Item " + this + " has subtypes but the number was not specified!"
                );
        }
    }
    ...
    public void addRecipe(Object... params) {
        if (!this.isDummiedOut()) {
            ...
            WorktableRecipes.getInstance().addRecipe(
                this.getStackOf(),
                this.isTool() || this.isArmor() ? RecipeLevel.CORE
    ...
    public void addSizedMetaRecipe(int meta, int num, Object... params) {
        if (!this.isDummiedOut()) {
            ...
            WorktableRecipes.getInstance().addRecipe(
                this.getCraftedMetadataProduct(num, meta),
                this.isTool() || this.isArmor() ? RecipeLevel.CORE
                                                : RecipeLevel.PROTECTED,
                params
            );
    ...
    public void addBlastRecipe(int temperature, int speed, Object... params) {
        if (!this.isDummiedOut()) {
            ...
            RecipesBlastFurnace.getRecipes().add3x3Crafting(
                is, temperature, speed, 0, params
            );
        }
    }

    public void
    addMetaBlastRecipe(int temperature, int speed, int meta, Object... params) {
        if (!this.isDummiedOut()) {
    ...
    public void addShapelessEnchantedRecipe(Object... params) {
        if (!this.isDummiedOut()) {
            ...
            WorktableRecipes.getInstance().addShapelessRecipe(
                is,
                this.isTool() || this.isArmor() ? RecipeLevel.CORE
                                                : RecipeLevel.PROTECTED,
                params
    ...
    public ItemStack getEnchantedStack() {
        ...
        switch (this) {
            ...
            case BEDHELM:
            case BEDLEGS:
                ReikaEnchantmentHelper.applyEnchantments(
                    is, ((ItemBedrockArmor) is.getItem()).getDefaultEnchantments()
                );
                break;
            case BEDPACK:
                ReikaEnchantmentHelper.applyEnchantments(
                    is,
                    ((ItemBedrockArmor) BEDCHEST.getItemInstance())
            ...
            case BEDJUMP:
                ReikaEnchantmentHelper.applyEnchantments(
                    is,
                    ((ItemBedrockArmor) BEDBOOTS.getItemInstance())
                        .getDefaultEnchantments()
                );
                break;
            case BEDREVEAL:
                ReikaEnchantmentHelper.applyEnchantments(
                    is,
                    ((ItemBedrockArmor) BEDHELM.getItemInstance())
                        .getDefaultEnchantments()
                );
                break;
            case BEDSWORD:
    ...
    public void addRecipe(IRecipe ir) {
        if (!this.isDummiedOut()) {
            ...
            WorktableRecipes.getInstance().addRecipe(
                ir,
                this.isTool() || this.isArmor() ? RecipeLevel.CORE : RecipeLevel.PROTECTED
            );
        }
    }
