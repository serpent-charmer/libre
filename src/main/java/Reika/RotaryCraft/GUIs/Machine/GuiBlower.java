...
public class GuiBlower extends GuiPowerOnlyMachine {
    ...
    public void initGui() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            buttonList.add(new ImagedGuiButton(
                i, j + 25 + 36 * i, k + 64, 18, 18, u, 54 + 18 * i, s, RotaryCraft.class
            ));
        }
    }
        ...
        if (b.id < 24000) {
            ...
            if (id != null)
                ReikaPacketHelper.sendPacketToServer(
                    RotaryCraft.packetChannel, id.ordinal(), tile
                );
    ...
    protected void drawGuiContainerForegroundLayer(int a, int b) {
        ...
        for (int i = 0; i < tile.matchingItems.length; i++) {
            ...
            if (is != null) {
                api.drawItemStack(
                    itemRender, fontRendererObj, is, x + i % 9 * 18, y + i / 9 * dy
                );
            }
        }
        ...
        if (api.isMouseInBox(j + 25, j + 43, k + 64, k + 82)) {
            api.drawTooltipAt(
                fontRendererObj,
                controls[0] ? "Whitelist" : "Blacklist",
                api.getMouseRealX() - j + 50,
                api.getMouseRealY() - k
            );
        }
        if (api.isMouseInBox(j + 25 + 36, j + 43 + 36, k + 64, k + 82)) {
            api.drawTooltipAt(
                fontRendererObj,
                controls[1] ? "Use Metadata" : "Ignore Metadata",
                api.getMouseRealX() - j + 80,
                api.getMouseRealY() - k
            );
        }
        ...
        if (api.isMouseInBox(j + 25 + 36 * 3, j + 43 + 36 * 3, k + 64, k + 82)) {
            api.drawTooltipAt(
                fontRendererObj,
                controls[3] ? "Match Exact" : "Use Ore Dictionary",
                api.getMouseRealX() - j,
                api.getMouseRealY() - k
            );
        }
