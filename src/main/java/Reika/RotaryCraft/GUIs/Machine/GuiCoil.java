    ...
    public void updateScreen() {
        ...
        if (!input.getText().isEmpty() && !(input.getText().matches("^[0-9 ]+$"))) {
            ...
            ReikaPacketHelper.sendPacketToServer(
                RotaryCraft.packetChannel, PacketRegistry.COILSPEED.ordinal(), coil, omega
            );
            valid1 = false;
        }
        if (!input2.getText().isEmpty() && !(input2.getText().matches("^[0-9 ]+$"))) {
            ...
            ReikaPacketHelper.sendPacketToServer(
                ...
                PacketRegistry.COILTORQUE.ordinal(),
                coil,
                torque
            );
            valid2 = false;
        ...
        if (valid1) {
            ...
            if (omega >= 0) {
                ...
                ReikaPacketHelper.sendPacketToServer(
                    RotaryCraft.packetChannel,
                    PacketRegistry.COILSPEED.ordinal(),
                    coil,
                    omega
                );
            }
    ...
    protected void drawGuiContainerForegroundLayer(int a, int b) {
        ...
        if (!coil.isCreative())
            fontRendererObj.drawString(
                String.format("(Max %d)", coil.getMaximumEmission()),
                xSize / 2 - 82,
                37,
                4210752
            );
        ...
        if (!input.isFocused())
            fontRendererObj.drawString(
                String.format("%d", coil.getReleaseOmega()), xSize / 2 - 3, 22, 0xffffffff
            );
        if (!input2.isFocused())
            fontRendererObj.drawString(
                String.format("%d", coil.getReleaseTorque()),
                xSize / 2 - 3,
                52,
                0xffffffff
            );
