...
public class GuiDistributionClutch extends GuiNonPoweredMachine {
    ...
    private void addEntryForSide(int j, int k, ForgeDirection dir) {
        ...
        ImagedGuiButton gb
            = new ImagedGuiButton(idx, x, y, 9, 9, u, v, file, RotaryCraft.class);
        gb.enabled = enable;
        buttonList.add(gb);
        selectedAmounts[idx]
            = new GuiTextField(fontRendererObj, x + 15, y - h / 2 + 6, 80, h - 4);
    ...
    public void updateScreen() {
        ...
        if (packet)
            ReikaPacketHelper.sendPacketToServer(
                ...
                PacketRegistry.DISTRIBCLUTCHPOWER.ordinal(),
                clutch,
                selected[0],
                selected[1],
                selected[2],
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        if (button.id < 4) {
            ...
            ReikaPacketHelper.sendPacketToServer(
                RotaryCraft.packetChannel,
                PacketRegistry.DISTRIBCLUTCH.ordinal(),
                clutch,
                button.id,
                enabled[button.id] ? 1 : 0
            );
        }
    }
