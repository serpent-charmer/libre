...
public class GuiPulseFurnace extends GuiMachine {
    ...
    protected void drawGuiContainerForegroundLayer(int a, int b) {
        ...
        if (api.isMouseInBox(j + 90, j + 96, k + 15, k + 68)) {
            ...
            api.drawTooltipAt(
                fontRendererObj,
                String.format("%d/%d", puls.getFuel(), puls.MAXFUEL),
                mx - j,
                my - k
            );
        }
        if (api.isMouseInBox(j + 20, j + 30, k + 15, k + 70)) {
            ...
            int my = api.getMouseRealY();
            api.drawTooltipAt(
                fontRendererObj, String.format("%dC", puls.temperature), mx - j, my - k
            );
        }
        if (api.isMouseInBox(j + 159, j + 165, k + 15, k + 68)) {
            ...
            api.drawTooltipAt(
                fontRendererObj,
                String.format(
                    "%d/%d", puls.getAccelerant(), puls.getAccelerantCapacity()
                ),
                mx - j,
                my - k
            );
    ...
    protected void drawPowerTab(int var5, int var6) {
        ...
        //puls.power, puls.MINPOWER), xSize+var5+16, var6+16, 0xff000000);
    }
