...
public class GuiCannon extends GuiPowerOnlyMachine {
    ...
    public GuiCannon(EntityPlayer p5ep, TileEntityLaunchCannon Cannon) {
        ...
        ep = p5ep;
        fuse = tnt instanceof TileEntityTNTCannon
            ? ((TileEntityTNTCannon) tnt).selectedFuse
            : 0;
    }
    ...
    public void initGui() {
        ...
        //this.buttonList.add(new GuiButton(0, j+xSize/2-48, -1+k+32, 80, 20,
        //"Trajectory"));
        if (targetMode) {
            ...
            input = new GuiTextField(
                fontRendererObj, j + xSize / 2 + 22 + 18, k + 104, 26, 16
            );
            input.setFocused(false);
            input.setMaxStringLength(3);
            input2 = new GuiTextField(
                fontRendererObj, j + xSize / 2 - 65 - 18, k + 104, 26, 16
            );
            input2.setFocused(false);
            input2.setMaxStringLength(3);
            input3 = new GuiTextField(
                fontRendererObj, j + xSize / 2 + 22 + 18, k + 124, 26, 16
            );
            input3.setFocused(false);
            input3.setMaxStringLength(3);
            input4
                = new GuiTextField(fontRendererObj, j + xSize / 2 - 49, k + 124, 26, 16);
            input4.setFocused(false);
    ...
    public void sendPacket() {
        ...
        try {
            ...
            //ReikaJavaLibrary.pConsole("Sending packet number "+b+" Type
            //"+PacketRegistry.getEnum(b));
    ...
    public void updateScreen() {
        ...
        if (input4.getText().isEmpty())
            valid4 = false;
        if (!input.getText().isEmpty()
            && !ReikaJavaLibrary.isValidInteger(input.getText())) {
            if (targetMode)
                target[0] = 0;
            else
                phid = 0;
        ...
        if (!input2.getText().isEmpty()
            && !ReikaJavaLibrary.isValidInteger(input2.getText())) {
            if (targetMode)
                target[1] = 0;
            else
                thetad = 0;
        ...
        if (!input3.getText().isEmpty()
            && !ReikaJavaLibrary.isValidInteger(input3.getText())) {
            if (targetMode)
                target[2] = 0;
            else
                velocity = 0;
        ...
        if (!input4.getText().isEmpty()
            && !ReikaJavaLibrary.isValidInteger(input4.getText())) {
            fuse = 0;
    ...
    protected void drawGuiContainerForegroundLayer(int a, int b) {
        ...
        if (targetMode) {
            ...
            fontRendererObj.drawString(
                "Velocity", xSize / 3 - 26 + 24 + 44, 116, 4210752
            );
            if (tnt instanceof TileEntityTNTCannon)
                fontRendererObj.drawString(
                    "Fuse Time", xSize / 3 - 26 - 32, 116, 4210752
                );
        }
        ...
        if (targetMode) {
            if (!input.isFocused())
                fontRendererObj.drawString(
                    String.format("%d", target[0]), 100, 18, 0xffffffff
                );
            if (!input2.isFocused())
                fontRendererObj.drawString(
                    String.format("%d", target[1]), 100, 34, 0xffffffff
                );
            if (!input3.isFocused())
                fontRendererObj.drawString(
                    String.format("%d", target[2]), 100, 50, 0xffffffff
                );
        } else {
            if (!input.isFocused())
                fontRendererObj.drawString(
                    String.format("%d", tnt.phi), 122 + 36, 96, 0xffffffff
                );
            if (!input2.isFocused())
                fontRendererObj.drawString(
                    String.format("%d", tnt.theta), 35, 96, 0xffffffff
                );
            if (!input3.isFocused())
                fontRendererObj.drawString(
                    String.format("%d", tnt.velocity), 122 + 36, 116, 0xffffffff
                );
            if (tnt instanceof TileEntityTNTCannon)
                if (!input4.isFocused())
                    fontRendererObj.drawString(
                        String.format("%d", fuse), 122 - 54, 116, 0xffffffff
                    );
        }
    }
    ...
    private void drawGrid(int j, int k) {
        ...
        for (int i = 0; i < 8; i++) {
            int size = 30;
            double x4
                = basex3 - 18 - 88 + xSize / 2 + size * Math.cos(Math.toRadians(i * 45));
            double y4
                = basey3 - 118 + ySize / 2 - size * Math.sin(Math.toRadians(i * 45));
            api.drawLine(basex3, basey3, x4, y4, color);
        }
        for (int i = 0; i < 3; i++) {
            int size = 30;
            api.drawCircle(
                basex3, basey3, size * Math.cos(Math.toRadians(i * 30)), color
            );
        }
    }
