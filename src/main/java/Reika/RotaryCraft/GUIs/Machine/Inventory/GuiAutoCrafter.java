...
public class GuiAutoCrafter extends GuiPowerOnlyMachine {
    ...
    public void initGui() {
        ...
        if (crafter.getMode() == CraftingMode.REQUEST) {
            for (int i = 0; i < crafter.SIZE; i++) {
                ...
                buttonList.add(new ImagedGuiButton(
                    i, dx, dy, 18, 4, 176, 6, this.getGuiTexture(), RotaryCraft.class
                ));
            }
        }
        ...
        //buttonList.add(new ImagedGuiButton(-1, var5+xSize-25, var6+4, 18, 9, 176, 25,
        //this.getGuiTexture(), RotaryCraft.class));
        if (crafter.getMode() == CraftingMode.SUSTAIN) {
            for (int i = 0; i < crafter.SIZE; i++) {
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        if (button.id >= 0 && button.id < crafter.SIZE)
            ReikaPacketHelper.sendPacketToServer(
                ...
                PacketRegistry.CRAFTERCRAFT.ordinal(),
                crafter,
                button.id,
                0
            );
            ...
            ReikaPacketHelper.sendPacketToServer(
                RotaryCraft.packetChannel,
                PacketRegistry.CRAFTERMODE.ordinal(),
                crafter,
                button.id,
                0
            );
    ...
    protected void keyTyped(char c, int k) {
        ...
        if (text != null) {
            ...
            //	selectedSlot = this.getSlotClickPosition(api.getMouseRealX(),
            //api.getMouseRealY());
            //}
            if (text.isFocused()) {
                ...
                if (selectedSlot >= 0) {
                    this.dispatch(
                        selectedSlot,
                        Math.max(0, ReikaJavaLibrary.safeIntParse(text.getText()))
                    );
                }
            }
        }
    ...
    private void dispatch(int slot, int thresh) {
        ReikaPacketHelper.sendPacketToServer(
            RotaryCraft.packetChannel,
            PacketRegistry.CRAFTERTHRESH.ordinal(),
            crafter,
            selectedSlot,
            thresh
        );
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        if (text != null) {
            ...
            if (selectedSlot >= 0) {
                ItemStack out = ReikaItemHelper.getSizedItemStack(
                    crafter.getSlotRecipeOutput(selectedSlot), 1
                );
                if (out != null) {
                    if (!text.isFocused()) {
                        fontRendererObj.drawString(
                            String.valueOf(crafter.getThreshold(selectedSlot)),
                            var5 + 12,
                            var6 + 122,
                            0xffffff
                        );
                    ...
                    //fontRendererObj.drawString("("+out.getDisplayName()+")", var5+90,
                    //var6+67, 4210752);
                }
            }
        }
        ...
        this.drawRect(
            var5 + 5, var6 + 5, var5 + 16, var6 + 16, 0xff000000 | crafter.getMode().color
        );
        this.drawRect(
            var5 + 5,
            var6 + 5,
            var5 + 6,
            var6 + 15,
            0xff000000 | ReikaColorAPI.mixColors(crafter.getMode().color, 0xffffff, 0.5F)
        );
        this.drawRect(
            var5 + 5,
            var6 + 5,
        ...
        this.drawRect(
            ...
            var5 + 16,
            var6 + 16,
            0xff000000 | ReikaColorAPI.mixColors(crafter.getMode().color, 0x000000, 0.5F)
        );
        this.drawRect(
            var5 + 15,
            var6 + 6,
        ...
        if (api.isMouseInBox(var5 + 5, var5 + 16, var6 + 5, var6 + 16)) {
            ...
            api.drawTooltipAt(
                fontRendererObj,
                s,
                api.getMouseRealX() + fontRendererObj.getStringWidth(s) + 28,
                api.getMouseRealY() + 20
            );
        }
    }
    ...
    protected String getGuiTexture() {
        return "/Reika/RotaryCraft/Textures/GUI/craftergui"
            + crafter.getMode().imageSuffix + ".png";
    }
}
