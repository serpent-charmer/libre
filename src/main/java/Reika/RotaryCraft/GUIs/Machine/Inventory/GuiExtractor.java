...
public class GuiExtractor extends GuiMachine {
    ...
    protected void drawGuiContainerForegroundLayer(int a, int b) {
        ...
        if (ConfigRegistry.EXTRACTORMAINTAIN.getState()) {
            ...
            if (api.isMouseInBox(dx, dx + 4, dy, dy + 18)) {
                String s
                    = String.format("Drill Status: %d%%", ext.getDrillLifeScaled(100));
                api.drawTooltipAt(
                    fontRendererObj, s, api.getMouseRealX() - 35, api.getMouseRealY() - 15
                );
            }
        ...
        String var4 = ConfigRegistry.EXTRACTORMAINTAIN.getState()
            ? "/Reika/RotaryCraft/Textures/GUI/extractorgui2.png"
            : "/Reika/RotaryCraft/Textures/GUI/extractorgui.png";
        ReikaTextureHelper.bindTexture(RotaryCraft.class, var4);
        ...
        if (api.isMouseInBox(j + dx, j + dx + 7, k + dy, k + dy + 18)) {
            ...
            api.drawTooltipAt(
                fontRendererObj,
                s,
                api.getMouseRealX() - fontRendererObj.getStringWidth(s),
                api.getMouseRealY() - 12
            );
        }
    }
    ...
    protected void drawPowerTab(int var5, int var6) {
        ...
        api.drawCenteredStringNoShadow(
            fontRendererObj, "Power:", xSize + var5 + 20, var6 + 9, 0xff000000
        );
        api.drawCenteredStringNoShadow(
            fontRendererObj, "Speed:", xSize + var5 + 20, var6 + 60, 0xff000000
        );
        api.drawCenteredStringNoShadow(
            fontRendererObj, "Torque:", xSize + var5 + 20, var6 + 111, 0xff000000
        );
        //this.drawCenteredStringNoShadow(fontRendererObj, String.format("%d/%d",
        //ext.power, ext.MINPOWER), xSize+var5+16, var6+16, 0xff000000);
    }
    ...
    protected String getGuiTexture() {
        return ConfigRegistry.EXTRACTORMAINTAIN.getState() ? "extractorgui2"
                                                           : "extractorgui";
    }
}
