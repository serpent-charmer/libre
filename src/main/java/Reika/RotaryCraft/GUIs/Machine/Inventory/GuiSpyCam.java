...
public class GuiSpyCam extends GuiNonPoweredMachine {
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        //this.drawRect(j+xSize/2-1-cam.getRange()*UNIT, k+ySize/2+5+cam.getRange()*UNIT,
        //j+xSize/2+1+cam.getRange()*UNIT, k+ySize/2+6+cam.getRange()*UNIT, 0xffffffff);
        //this.drawRect(j+xSize/2-1-cam.getRange()*UNIT, k+ySize/2+4-cam.getRange()*UNIT,
        //j+xSize/2+1+cam.getRange()*UNIT, k+ySize/2+5-cam.getRange()*UNIT, 0xffffffff);
        //this.drawRect(j+xSize/2-1-cam.getRange()*UNIT, k+ySize/2+4-cam.getRange()*UNIT,
        //j+xSize/2-cam.getRange()*UNIT, k+ySize/2+6+cam.getRange()*UNIT, 0xffffffff);
        //this.drawRect(j+xSize/2+cam.getRange()*UNIT, k+ySize/2+4-cam.getRange()*UNIT,
        //j+xSize/2+1+cam.getRange()*UNIT, k+ySize/2+6+cam.getRange()*UNIT, 0xffffffff);
    ...
    private void drawRadar(int a, int b) {
        ...
        for (int i = cam.getBounds()[0]; i <= cam.getBounds()[1]; i++) {
            for (int j = cam.getBounds()[0]; j <= cam.getBounds()[1]; j++) {
                float br = 1
                    - (cam.yCoord - cam.getHeightAt(i, j)) / (float) cam.yCoord * 1.25F;
                if (br < 0)
                    br = 0;
                this.drawRect(
                    a + 17 + max - (UNIT * j),
                    b + 19 + UNIT * i,
                    a + 17 + max - (UNIT + UNIT * j),
                    b + 19 + UNIT * i + UNIT,
                    ReikaColorAPI.getColorWithBrightnessMultiplier(
                        cam.getTopBlockColorAt(i, j), br
                    )
                );
            }
        }
        for (int i = cam.getBounds()[0]; i <= cam.getBounds()[1]; i++) {
            for (int j = cam.getBounds()[0]; j <= cam.getBounds()[1]; j++) {
                this.drawMobIcon(
                    a + 13, b + 19, max - (UNIT * i), UNIT * j, cam.getMobAt(i, j), i, j
                );
            }
        }
    }
