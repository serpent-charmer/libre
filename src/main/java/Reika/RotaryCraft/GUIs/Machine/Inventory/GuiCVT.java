        ...
        switch (mode) {
            case AUTO:
                input = new GuiTextField(
                    fontRendererObj, j + xSize / 2 + 24, k + 48, 76, 16
                );
            ...
            case MANUAL:
                if (ratio > 0)
                    buttonList.add(
                        new GuiButton(1, j + xSize / 2 - 6, -1 + k + 64, 80, 20, "Speed")
                    );
                else
                    buttonList.add(
                        new GuiButton(1, j + xSize / 2 - 6, -1 + k + 64, 80, 20, "Torque")
                    );
                input = new GuiTextField(
                    fontRendererObj, j + xSize / 2 + 24, k + 39, 26, 16
                );
                input.setFocused(false);
            ...
            case REDSTONE:
                buttonList.add(
                    new GuiButton(1, j + xSize / 2 + 25, -1 + k + 44, 71, 20, "")
                );
                buttonList.add(
                    new GuiButton(2, j + xSize / 2 + 25, -1 + k + 67, 71, 20, "")
                );
                break;
        }
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        if (button.id == 0) {
            ...
            ReikaPacketHelper.sendPacketToServer(
                RotaryCraft.packetChannel, PacketRegistry.CVTMODE.ordinal(), cvt
            );
        }
        ...
        switch (mode) {
            ...
            case MANUAL:
                if (button.id == 1) {
                    ...
                    ReikaPacketHelper.sendPacketToServer(
                        ...
                        PacketRegistry.CVTRATIO.ordinal(),
                        cvt,
                        ratio
                    );
                }
            ...
            case REDSTONE:
                if (button.id == 1) {
                    ...
                    ReikaPacketHelper.sendPacketToServer(
                        RotaryCraft.packetChannel,
                        PacketRegistry.CVTREDSTONESTATE.ordinal(),
                        cvt,
                        1
                    );
                }
    ...
    public void updateScreen() {
        ...
        if (input != null) {
            if (input.getText().isEmpty())
                return;
            PacketRegistry p = mode == CVTMode.AUTO ? PacketRegistry.CVTTARGET
                                                    : PacketRegistry.CVTRATIO;
            int val = 1;
            if (!(input.getText().matches("^[0-9 ]+$"))) {
                input.deleteFromCursor(-1);
                ReikaPacketHelper.sendPacketToServer(
                    RotaryCraft.packetChannel, p.ordinal(), cvt, val
                );
                return;
            }
    ...
    protected void drawGuiContainerForegroundLayer(int a, int b) {
        ...
        if (api.isMouseInBox(j + 185, j + 202, k + 88, k + 149)) {
            ...
            api.drawTooltipAt(
                fontRendererObj,
                s,
                -j + api.getMouseRealX() + 55 - fontRendererObj.getStringWidth(s),
                -k + api.getMouseRealY()
            );
        }
        ...
        switch (mode) {
            case REDSTONE:
                ...
                api.drawItemStack(
                    itemRender,
                    fontRendererObj,
                    new ItemStack(Blocks.unlit_redstone_torch),
                    129,
                    54
                );

                this.drawCenteredString(
                    fontRendererObj, cvt.getCVTString(true), 188, 37, 0xffffff
                );
                this.drawCenteredString(
                    fontRendererObj, cvt.getCVTString(false), 188, 60, 0xffffff
                );

                int dy = 17;
                int dx = -14;
                fontRendererObj.drawString(
                    "Belt Ratio:", xSize / 2 - 32 + dx, 31 + dy, 4210752
                );
            ...
            case MANUAL:
                if (!input.isFocused()) {
                    fontRendererObj.drawString(
                        String.format("%d", Math.abs(cvt.getRatio())),
                        xSize / 2 + 36,
                        31,
                        0xffffffff
                    );
                ...
                this.drawCenteredString(
                    fontRendererObj, "M", xSize / 2 + 102, 12, 0xffffffff
                );
                break;
            case AUTO:
                if (!input.isFocused()) {
                    fontRendererObj.drawString(
                        String.format("%d", Math.abs(cvt.getTargetTorque())),
                        xSize / 2 + 36,
                        40,
                        0xffffffff
                    );
                ...
                fontRendererObj.drawString(
                    String.format("Current Input: %d Nm", cvt.getTorqueIn()),
                    xSize / 2 - 30,
                    60,
                    4210752
                );
                ...
                fontRendererObj.drawString(
                    String.format(
                        "Current Ratio: %dx (%s)", Math.abs(r), r < 0 ? "Torque" : "Speed"
                    ),
                    xSize / 2 - 30,
                    72,
                    4210752
                ...
                api.drawItemStack(
                    itemRender, fontRendererObj, ItemStacks.pcb, xSize / 2 + 94, 8
                );
                break;
        }

        if (api.isMouseInBox(
                j + xSize / 2 + 92, j + xSize / 2 + 112, -1 + k + 7, -1 + k + 27
            )) {
            String s = "Control Mode";
            api.drawTooltipAt(
                fontRendererObj,
                s,
                api.getMouseRealX() - 5 - fontRendererObj.getStringWidth(s),
                api.getMouseRealY()
            );
        }
    }
            ...
            case MANUAL:
                if (ratio > cvt.getMaxRatio())
                    api.drawCenteredStringNoShadow(
                        fontRendererObj,
                        String.format("(%d)", cvt.getMaxRatio()),
                        j + xSize / 2 + 88,
                        k + 31,
                        0xff0000
                    );
                    ...
                    api.drawCenteredStringNoShadow(
                        fontRendererObj, "(1)", j + xSize / 2 + 88, k + 31, 0xff0000
                    );
                else
                    api.drawCenteredStringNoShadow(
                        fontRendererObj,
