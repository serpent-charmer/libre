...
public class GuiMultiClutch extends GuiNonPoweredMachine {
    ...
    protected void actionPerformed(GuiButton b) {
        ...
        if (b.id < 16) {
            ...
            ReikaPacketHelper.sendPacketToServer(
                ...
                PacketRegistry.MULTISIDE.ordinal(),
                multi,
                b.id,
                side
            );
    ...
    protected void drawGuiContainerForegroundLayer(int a, int b) {
        ...
        int pwr
            = multi.worldObj.getBlockPowerInput(multi.xCoord, multi.yCoord, multi.zCoord);
        for (int i = 0; i < 16; i++) {
            int x = 3 + 70 * (i / 8);
            int y = 15 + 16 * (i % 8);
            api.drawItemStack(
                itemRender,
                fontRendererObj,
                new ItemStack(i == pwr ? Items.glowstone_dust : Items.redstone),
                x,
                y
            );
        }
        ...
        for (int i = 0; i < 16; i++) {
            fontRendererObj.drawString(
                String.format("%d", i), 18 + 70 * (i / 8), 20 + 16 * (i % 8), 0
            );
            ...
            String s = ReikaStringParser.capFirstChar(
                ForgeDirection.VALID_DIRECTIONS[idx].name()
            );
            fontRendererObj.drawString(
                s,
                18 + 70 * (i / 8) + 16,
                20 + 16 * (i % 8),
                idx == 0 || idx == 2 ? 0x000000 : 0xffffff
            );
        }
