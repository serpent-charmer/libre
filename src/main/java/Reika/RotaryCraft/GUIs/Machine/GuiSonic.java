...
public class GuiSonic extends GuiPowerOnlyMachine {
    ...
    public void initGui() {
        ...
        input2 = new GuiTextField(
            fontRendererObj, j + xSize / 2 - 62 + xo, k + 43, 86 - xo, 16
        );
    ...
    public void updateScreen() {
        ...
        if (!input.getText().isEmpty() && !(input.getText().matches("^[0-9 ]+$"))) {
            ...
            ReikaPacketHelper.sendLongDataPacket(
                RotaryCraft.packetChannel,
                PacketRegistry.SONICPITCH.ordinal(),
                sonic,
                freq
            );
            valid1 = false;
        ...
        if (!input2.getText().isEmpty() && !(input2.getText().matches("^[0-9 ]+$"))) {
            ...
            ReikaPacketHelper.sendLongDataPacket(
                RotaryCraft.packetChannel,
                PacketRegistry.SONICVOLUME.ordinal(),
                sonic,
                vol
            );
            valid2 = false;
        ...
        if (sonic.ENABLEFREQ) {
            if (!input.isFocused())
                fontRendererObj.drawString(
                    String.format("%d", sonic.setpitch), xSize / 2 - 50, 65, 0xffffffff
                );
        }
        if (!input2.isFocused()) {
            if (sonic.DECIBELMODE)
                fontRendererObj.drawString(
                    String.format("%d", dB), xSize / 2 - 50 + xo, 35, 0xffffffff
                );
            else
                fontRendererObj.drawString(
                    String.format("%d", sonic.setvolume),
                    xSize / 2 - 50 + xo,
                    35,
                    0xffffffff
                );
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        if (vol > sonic.getMaxVolume())
            color = 0xff0000;
        //ImagedGuiButton.drawCenteredStringNoShadow(fontRendererObj,
        //String.format("(%d)", sonic.getVolume()), j+xSize/2+85, k+65, color);
        color = 4210752;
        ...
        if (freq > sonic.getMaxPitch())
            color = 0xff0000;
        //ImagedGuiButton.drawCenteredStringNoShadow(fontRendererObj,
        //String.format("(%d)", sonic.getPitch()), j+xSize/2+85, k+35, color);
    }
