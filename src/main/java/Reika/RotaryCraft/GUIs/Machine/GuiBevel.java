...
public class GuiBevel extends GuiNonPoweredMachine {
    ...
    public void initGui() {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (true
                || TileEntityBevelGear.isValid(ForgeDirection.VALID_DIRECTIONS[i], out)) {
                if (in.ordinal() == i)
    ...
    public void getIOFromDirection() {
        ImmutablePair<ForgeDirection, ForgeDirection> dirs
            = TileEntityBevelGear.getDirectionMap().get(posn);
        if (dirs == null) {
            RotaryCraft.logger.logError(
                "Bevel was set to invalid direction value " + posn + "!"
            );
            return;
        }
        in = dirs.left;
    ...
    public void getDirectionFromIO() {
        if (!TileEntityBevelGear.isValid(in, out)) {
            RotaryCraft.logger.logError(
                "Bevel was set to invalid state " + in + " > " + out + "!"
            );
            return;
        }
        posn = TileEntityBevelGear.getDirectionMap().inverse().get(
            new ImmutablePair(in, out)
        );
    }
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        if (button.id < 6) {
            ...
            if (!TileEntityBevelGear.isValid(
                    in, ForgeDirection.VALID_DIRECTIONS[button.id - 6]
                ))
                return;
        ...
        ReikaPacketHelper.sendPacketToServer(
            RotaryCraft.packetChannel, PacketRegistry.BEVEL.ordinal(), bevel, posn
        );
    }
    ...
    protected void drawGuiContainerForegroundLayer(int a, int b) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (!bk.isAir(bevel.worldObj, dx, dy, dz)) {
                ...
                TileEntity te = bevel.worldObj.getTileEntity(dx, dy,
                dz);//bk.createTileEntity(bevel.worldObj, meta); GL11.glTranslated(20,
                53+i*22, 200); GL11.glScaled(-8, 8, -8); GL11.glRotatef(te == null ?
                -22.5F : 22.5F, 1, 0, 0); GL11.glRotatef(180, 0, 0, 1); GL11.glRotatef(45,
                0, 1, 0); ReikaRenderHelper.enableLighting();
                ...
                if (te != null) {
                    ...
                    GL11.glEnable(GL11.GL_DEPTH_TEST);
                    TileEntityRendererDispatcher.instance.renderTileEntityAt(te, 0, 0, 0,
                ReikaRenderHelper.getPartialTickTime());
                }*/
                ItemStack drop = ReikaRenderHelper.getBlockItem(
                    bk, meta, bevel.worldObj.getTileEntity(dx, dy, dz)
                );
                if (drop != null)
