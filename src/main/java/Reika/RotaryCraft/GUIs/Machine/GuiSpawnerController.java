...
public class GuiSpawnerController extends GuiPowerOnlyMachine {
    ...
    public void initGui() {
        ...
        if (hasPower) {
            buttonList.add(new GuiButton(
                0, j + xSize / 2 - 48, -1 + k + 32, 80, 20, "Disable/Enable"
            ));
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        ReikaPacketHelper.sendPacketToServer(
            ...
            PacketRegistry.SPAWNERTIMER.ordinal(),
            spawnercontroller,
            dat
        );
    }
        ...
        if (hasPower) {
            ...
            if (!input.isFocused() && !disabled) {
                fontRendererObj.drawString(
                    String.format("%d", spawnercontroller.getDelay()),
                    xSize / 2 + 5,
                    51,
                    0xffffffff
                );
        ...
        if (hasPower) {
            ...
            if (disabled) {
                ...
                api.drawCenteredStringNoShadow(
                    fontRendererObj, "Infinity", j + xSize / 2 + 28, k + 51, color
                );
            } else
                api.drawCenteredStringNoShadow(
                    fontRendererObj,
