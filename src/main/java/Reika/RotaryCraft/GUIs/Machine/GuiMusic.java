...
public class GuiMusic extends GuiNonPoweredMachine implements MusicGui {
    ...
    public void initGui() {
        ...
        for (int i = 0; i < 16; i++) {
            ColorButton b = new ColorButton(
                200 + i, j + 9 + i * 15, k + 95, 12, 12, music.getColorForChannel(i)
            );
            if (activeChannel == i)
                b.isSelected = true;
            buttonList.add(b);
        ...
        ItemStack[] items = { new ItemStack(Blocks.grass),  new ItemStack(Blocks.planks),
                              new ItemStack(Blocks.portal), new ItemStack(Blocks.stone),
                              new ItemStack(Blocks.sand),   new ItemStack(Blocks.glass) };
        for (int i = 0; i < 6; i++)
            buttonList.add(
                new ItemIconButton(400 + i, j + 152 + 16 * i, k + 53, 0, items[i])
            );
    }
        ...
        if (button.id < 24000) {
            if (button.id == 100) {
                ReikaPacketHelper.sendPacketToServer(
                    ...
                    PacketRegistry.MUSICSAVE.ordinal() + 1,
                    music
                );
            } else if (button.id == 101) {
                ReikaPacketHelper.sendPacketToServer(
                    RotaryCraft.packetChannel,
                    PacketRegistry.MUSICREAD.ordinal() + 2,
                    music
                );
            } else if (button.id == 102) {
                ReikaPacketHelper.sendPacketToServer(
                    RotaryCraft.packetChannel, PacketRegistry.MUSICDEMO.ordinal(), music
                );
            } else if (button.id == 103) {
                ReikaPacketHelper.sendPacketToServer(
                    RotaryCraft.packetChannel,
                    PacketRegistry.MUSICREST.ordinal(),
                    music,
                    activeChannel,
                    activeType.ordinal(),
                    0,
                ...
                ReikaPacketHelper.sendPacketToServer(
                    RotaryCraft.packetChannel,
                    PacketRegistry.MUSICBKSP.ordinal(),
                    music,
                    activeChannel
                ); //bksp
                ...
                ReikaPacketHelper.sendPacketToServer(
                    RotaryCraft.packetChannel,
                    PacketRegistry.MUSICCLEARCH.ordinal(),
                    music,
                    activeChannel
                ); //ch clr
            } else if (button.id == 106) {
                ReikaPacketHelper.sendPacketToServer(
                    RotaryCraft.packetChannel, PacketRegistry.MUSICCLEAR.ordinal(), music
                ); //clrall
    ...
    protected void drawGuiContainerForegroundLayer(int a, int b) {
        ...
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            fontRendererObj, "Note Length", 51, 42, 0
        );
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            fontRendererObj, "Instrument", 200, 42, 0
        );
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            fontRendererObj, "Channel Select", xSize / 2, 85, 0
        );
    ...
    protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3) {
        ...
        //"/Reika/RotaryCraft/Textures/GUI/musicbuttons.png");
    ...
    public void bindKeyboardTexture() {
        ReikaTextureHelper.bindTexture(
            RotaryCraft.class, "/Reika/RotaryCraft/Textures/GUI/musicbuttons.png"
        );
    }
}
