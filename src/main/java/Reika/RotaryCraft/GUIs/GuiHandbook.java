...
public class GuiHandbook extends GuiScreen {
    ...
    public GuiHandbook(EntityPlayer p5ep, World world, int s, int p) {
        ...
        if (ConfigRegistry.DYNAMICHANDBOOK.getState()
            || (DragonAPICore.isReikasComputer()
    ...
    public final void initGui() {
        ...
        buttonList.add(new GuiButton(12, j + xSize - 27, k + 6, 20, 20, "X")
        ); //Close gui button

        HandbookEntry h = this.getEntry();

        if (h.hasSubpages()
            || ((h instanceof HandbookRegistry)
                && ((HandbookRegistry) h).getBonusSubpages() > 0)) {
    ...
    public int getMaxSubpage() {
        ...
        if (h == HandbookRegistry.PACKMODS)
            return PackModificationTracker.instance.getModifications(RotaryCraft.instance)
                       .size()
                / 3;
    ...
    public final String getBackgroundTexture() {
        ...
        String var4
            = "/Reika/RotaryCraft/Textures/GUI/Handbook/" + type.getFileName() + ".png";
        return var4;
    }
    ...
    private final void drawTabIcons() {
        ...
        for (int i = 0; i < li.size(); i++) {
            ...
            ReikaGuiAPI.instance.drawItemStack(
                ri, fontRendererObj, h.getTabIcon(), posX - 17, posY - 6 + i * 20
            );
        }
    }
    ...
    private final void drawGraphics(float ptick) {
        ...
        if (this.getEntry() == HandbookRegistry.ENCHANTING && subpage > 0) {
            ...
            ((EnchantableMachine) te)
                .getEnchantmentHandler()
                .setEnchantment(map.get(m).iterator().next(), 1);
        ...
        if (!this.isLimitedView()) {
            ...
            //ReikaGuiAPI.instance.drawCenteredStringNoShadow(fontRendererObj, s,
            //posX+xSize+23, posY+5, 0xffffff);
            ReikaGuiAPI.instance.drawTooltipAt(
                fontRendererObj,
                s,
                posX + 24 + xSize + fontRendererObj.getStringWidth(s),
                posY + 20
            );
            if (ReikaGuiAPI.instance.isMouseInBox(
                    posX - 18, posX + 2, posY + 0, posY + 220
                )) {
                String sg = "";
                List<HandbookEntry> li = this.getAllTabsOnScreen();
                int idx = (msy - posY) / 20;
                ...
                if (!sg.isEmpty())
                    ReikaGuiAPI.instance.drawTooltipAt(
                        fontRendererObj,
                        sg,
                        msx + fontRendererObj.getStringWidth(sg) + 30,
                        msy
                    );
            }
        }
        ...
        if (HandbookNotifications.instance.newAlerts()
            ...
            int y = posY + 18;
            int alpha = (int
            ) (155 + 100 * Math.sin(Math.toRadians(System.currentTimeMillis() / 8 % 360))
            );
            ...
            if (ReikaMathLibrary.isValueInsideBoundsIncl(261, 377, dx)
                ...
                if (PackModificationTracker.instance.modificationsExist(
                    ...
                    ReikaGuiAPI.instance.drawTooltip(
                        fontRendererObj,
                        "The modpack has made some changes to the mod.",
                        0,
                        10
                    );
            }
    ...
    protected final void mouseClicked(int x, int y, int a) {
        ...
        if (a == 0) {
            ...
            int dy = y - k;
            if (HandbookNotifications.instance.newAlerts()
                || PackModificationTracker.instance.modificationsExist(
                    RotaryCraft.instance
                )) {
                if (ReikaMathLibrary.isValueInsideBoundsIncl(261, 377, dx)
                    && ReikaMathLibrary.isValueInsideBoundsIncl(22, 36, dy)) {
    ...
    public final void keyTyped(char c, int key) {
        ...
        if (ModList.NEI.isLoaded()
            && key == NEIClientConfig.getKeyBinding("gui.recipe")) {
    ...
    protected void drawAuxGraphics(int posX, int posY, float ptick) {
        HandbookAuxData.drawGraphics(
            (HandbookRegistry) this.getEntry(),
            posX,
            posY,
            xSize,
            ySize,
    ...
    public final void drawScreen(int x, int y, float f) {
        ...
        fontRendererObj.drawString(
            s, posX + xo + 6, posY + yo + 6, disable ? 0xff0000 : 0x000000
        );
        if (h == HandbookRegistry.ENCHANTING && subpage > 0) {
            ...
            Collections.sort(li);
            fontRendererObj.drawString(
                li.get(subpage - 1).getName(),
                posX + xo + 6,
                posY + yo + 18,
                disable ? 0xff0000 : 0x000000
            );
        }
        ...
        if (subpage == 0 || h.sameTextAllSubpages()) {
            fontRendererObj.drawSplitString(
                this.parseHandbookText(h.getData()), px, posY + descY, 242, c
            );
        } else {
            fontRendererObj.drawSplitString(
                this.parseHandbookText(h.getNotes(subpage)), px, posY + descY, 242, c
            );
        }
        if (disable) {
            fontRendererObj.drawSplitString(
                "This machine has been disabled by your server admin or modpack creator.",
                px,
                posY + descY,
                242,
                0xffffff
            ...
            fontRendererObj.drawSplitString(
                "Contact them for further information or to request that they remove this restriction.",
                px,
                posY + descY + 27,
                242,
                0xffffff
            ...
            fontRendererObj.drawSplitString(
                "If you are the server admin or pack creator, use the configuration files to change this setting.",
                px,
                posY + descY + 54,
                242,
                0xffffff
    ...
    doRenderMachine(double x, double y, HandbookRegistry h, MachineRegistry m) {
    ...
    private void drawMachineRender(int posX, int posY) {
        ...
        double x = posX + 167;
        double y = posY + 44;
        //float q = 12.5F + fscale*(float)Math.sin(System.nanoTime()/1000000000D);
        ////wobble
        //ReikaJavaLibrary.pConsole(y-ReikaGuiAPI.instance.getMouseScreenY(height));
        int range = 64;
        boolean rotate = ReikaGuiAPI.instance.isMouseInBox(
