...
public class GuiSafePlayerList extends GuiScreen {
    ...
    public void initGui() {
        ...
        for (int i = 0; i < playerList.clampedSize(); i++) {
            ...
            buttonList.add(new GuiButton(
                i, j + dx, k + dy, width, 20, playerList.getEntryAtRelativeIndex(i)
            ));
        }

        buttonList.add(new GuiButton(1000000, j + dx + width + 6, 11 + k, 20, 20, "^"));
        buttonList.add(new GuiButton(
            1000001, j + dx + width + 6, 11 + k + colsize * 20 - 5, 20, 20, "v"
        ));
    }
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        ReikaPacketHelper.sendStringPacket(
            ...
            PacketRegistry.SAFEPLAYER.ordinal(),
            activePlayer,
            te
        );
    ...
    public void drawScreen(int x, int y, float f) {
        ...
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            fontRendererObj, title, posX + xSize / 2, posY + 6, 4210752
        );
