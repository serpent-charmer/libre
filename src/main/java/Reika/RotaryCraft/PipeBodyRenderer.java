...
public class PipeBodyRenderer extends ISBRH {
            ...
            switch (renderPass) {
                ...
                default:
                    throw new WTFException(
                        RotaryCraft.instance, "Who screwed up the render passes?", true
                    );
            }
        }
                            ...
                            for (int f = 0; f < 6; f++) {
                                ...
                                case 1:
                                    if (tile.isFluidPipe()) {
                                        ...
                                        Tessellator.instance.addVertex(-2*x+3, -2*y+3,
    -2*z+3); Tessellator.instance.addVertex(-x, -y, -z); Tessellator.instance.addVertex(0,
    0, 0); Tessellator.instance.addVertex(x, y, z); Tessellator.instance.addVertex(x+3,
    y+3, z+3); Tessellator.instance.addVertex(2*x+3, 2*y+3, 2*z+3);
    ...
    private void renderLiquid(
        IBlockAccess world, RenderableDuct tile, int x, int y, int z, ForgeDirection dir
    ) {
        ...
        //v5.startDrawingQuads();
        v5.addTranslation(x, y, z);
        int mix = tile.getPipeBlockType().getMixedBrightnessForBlock(
            Minecraft.getMinecraft().theWorld, x, y, z
        );
        ReikaLiquidRenderer.bindFluidTexture(f);
        v5.setColorOpaque(255, 255, 255);
        if (f == FluidRegistry.WATER) {
            //v5.setColorOpaque_I(world.getBiomeGenForCoords(x,
            //z).getWaterColorMultiplier());
        ...
        if (f.canBePlacedInWorld()) {
            clr = f.getBlock().colorMultiplier(
                Minecraft.getMinecraft().theWorld, x, y, z
            );
    ...
    public void
    renderBlockInInventory(RenderableDuct tile, double par2, double par4, double par6) {
    ...
    private void renderFace(
        RenderableDuct tile, IBlockAccess world, int x, int y, int z, ForgeDirection dir
    ) {
        if (tile == null)
            return;
        Tessellator v5 = Tessellator.instance;
    ...
    private void
    doRenderFace(RenderableDuct tile, IBlockAccess world, ForgeDirection dir) {
        Tessellator v5 = Tessellator.instance;
        ...
        if (world != null && tile.isConnectionValidForSide(dir)) {
            switch (dir) {
                case DOWN:
                    ...
                    v5.addVertexWithUV(
                        0.5 + size, 0.5 + size + dd, 0.5 + size - dd, u2 - du, vo
                    );
                    v5.addVertexWithUV(0.5 + size, 0.5 + size + dd, 0.5 + size, u2, vo);
                    v5.addVertexWithUV(0.5 + size, 0.5 + size, 0.5 + size, u2, v);
                    v5.addVertexWithUV(
                        0.5 + size, 0.5 + size, 0.5 + size - dd, u2 - du, v
                    );
                ...
                case UP:
                    ...
                    /*
                v5.addVertexWithUV(0.5-size+dd, 	0.5-size+dd, 	0.5+size,
                gu+dgu*size*2, gv); v5.addVertexWithUV(0.5+size-dd, 	0.5-size+dd,
                0.5+size, 	gu, gv); v5.addVertexWithUV(0.5+size-dd, 	0,
                0.5+size, 	gu, gv+dgv*size); v5.addVertexWithUV(0.5-size+dd, 	0,
                0.5+size, 	gu+dgu*size*2, gv+dgv*size);*/
