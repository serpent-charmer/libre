...
public interface TargetEntity {
    /**
     * Called when a railgun projectile hits the entity. Note well that impacts are
     * incredibly destructive, and will destroy large swaths of terrain and only heavily
     * armored (or otherwise protected) entities should survive, as the impact energy is
     * comparable to having a jet airliner crash on top of you. Return true to prevent
     * further processing.
     */
    public boolean onRailgunImpact(TileEntity source, RailGunAmmoType type);

    /**
     * Used to modify the knockback from a railgun impact. Do not return values less than
     * zero or excessively large.
     */
    public double getKnockbackMultiplier(TileEntity source, RailGunAmmoType type);

    /**
     * Called when the laser gun hits the entity. Usually used to set fire, can also do
     * things like "cut".
     */
