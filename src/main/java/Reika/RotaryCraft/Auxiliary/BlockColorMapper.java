...
public class BlockColorMapper {
    ...
    public static final GPRBlockColor AIR_COLOR
        = new BasicBlockColor(ReikaColorAPI.GStoHex(33));
    ...
    private BlockColorMapper() {
        ...
        this.addBlockColor(
            Blocks.planks, ReikaColorAPI.RGBtoHex(178, 142, 90)
        ); //-----------------------------------?
        ...
        this.addBlockColor(
            Blocks.brown_mushroom_block, ReikaColorAPI.RGBtoHex(148, 113, 90)
        );
        this.addBlockColor(
            Blocks.red_mushroom_block, ReikaColorAPI.RGBtoHex(179, 34, 32)
        );
        ...
        this.addBlockColor(
            Blocks.tripwire, ReikaColorAPI.RGBtoHex(33, 33, 33)
        ); //render tripwires as air
        ...
        this.addBlockColor(
            Blocks.stained_hardened_clay, ReikaColorAPI.RGBtoHex(204, 172, 156)
        );
...
this.addBlockColor(Blocks.packedIce, ReikaColorAPI.RGBtoHex(165, 195, 247)); //meta values
        ...
        boolean renderore
            = ConfigRegistry.GPRORES.getState() || DragonAPICore.isReikasComputer();
        if (!renderore) {
    ...
    private void loadModData() {
        ...
        for (BlockKey key : keys) {
            ...
            RotaryCraft.logger.log(
                "Received mod request for block " + id + ":" + meta
                + " to have color mapping " + Integer.toHexString(color)
            );
    ...
    private void addRotaryCraft() {
        for (int i = 0; i < BlockRegistry.blockList.length; i++) {
            ...
            this.addBlockColor(
                r.getBlockInstance(), ReikaColorAPI.RGBtoHex(200, 200, 200)
            );
        }
    ...
    private void addFluids() {
        for (String s : FluidRegistry.getRegisteredFluids().keySet()) {
            ...
            if (f != null && !f.equals(FluidRegistry.WATER)
                && !f.equals(FluidRegistry.LAVA)) {
    ...
    private void addModWood() {
        for (int i = 0; i < ModWoodList.woodList.length; i++) {
            ...
            if (wood.exists()) {
                ...
                if (saplingID != null)
                    this.addOrSetColorMapping(
                        saplingID,
                        wood.getCorrespondingSapling().getItemDamage(),
                        0x3C9119,
                        true
                    );
    ...
    private void
    addOrSetColorMapping(Block b, int meta, int color, boolean allowOverwrite) {
        if (b == null)
            throw new IllegalArgumentException("Block ID " + b + " does not exist!");
        if (!allowOverwrite && map.containsKey(b, meta))
            throw new IllegalArgumentException(
                "Cannot overwrite color mapping for " + b + ":" + meta + "!"
            );
        this.addBlockColor(b, meta, color);
    }

    private void addSlabs() {
        int[] colors = new int[] { 0xA3A3A3, 0xDCD3A0, 0xBC9862, 0x969696,
                                   0xA55B47, 0x797979, 0x36181E, 0xE8E4DC };
    ...
    private void addFlowers() {
        int[] colors = new int[] { 0xF7070F, 0x29AEFB, 0xBF75FB, 0xF2F29C, 0xD33A17,
                                   0xE17124, 0xF3F3F3, 0xEABEEA, 0xD2C71E };
        for (int i = 0; i < colors.length; i++) {
            this.addBlockColor(Blocks.red_flower, i, colors[i]);
        }
        /*
        BlockColor flowers = new BlockColor(0);
        int[] colors2 = new int[]{0xE2A41F, 0x9F78A4, 0x58864C, 0x58864C, 0xBA050B,
        0xDEA5F7}; for (int i = 0; i < colors2.length; i++) { flowers.addMetaColor(i,
        colors2[i]);
        }
    ...
    public void addBlockColor(BlockKey bk, GPRBlockColor rgb) {
        if (map.containsKey(bk))
            RotaryCraft.logger.logError(
                "GPR Color Mapping - block " + bk + " already mapped to a color!"
            );
        else
            map.put(bk, rgb);
    }
    ...
    private static class BlockColor {
        ...
        public BlockColor addMetaColor(int meta, int rgb) {
            if (meta >= 16) {
                RotaryCraft.logger.logError("Could not assign GPR Color mapping
    "+Integer.toHexString(rgb)+" to "+blockID+" meta "+meta+": invalid metadata."); return
    this;
            }
            metaTag = true;
            colorInts[meta] = rgb;
        ...
        public int getColorInt(int meta) {
            if (meta >= 16) {
                RotaryCraft.logger.logError("Could not fetch GPR Color mapping for
    "+blockID+" meta "+meta+": invalid metadata."); return BlockColorMapper.UNKNOWN_COLOR;
            }
            if (metaTag) {
    ...
    private void parseLine(String s) { //[<lookup>]=0xRRGGBB
        if (!s.startsWith("//")) {
            ...
            if (item.getItem() == null)
                throw new IllegalArgumentException(
                    "Item lookup '" + key + "' returned a null-item stack!"
                );
            Block b = Block.getBlockFromItem(item.getItem());
            if (b == null)
                throw new IllegalArgumentException(
                    "Item lookup '" + key + "' returned non-block item!"
                );
            String color = s.substring(idx + 2, s.length());
            this.addOrSetColorMapping(
                b, item.getItemDamage(), Integer.parseInt(color), false
            );
        }
    }
