...
public final class HandbookAuxData {
    ...
    private static final TreeMultimap<Long, MachineRegistry> powerData
        = TreeMultimap.create();
    private static final TreeMultimap<Integer, MachineRegistry> torqueData
        = TreeMultimap.create();
    private static final TreeMultimap<Integer, MachineRegistry> speedData
        = TreeMultimap.create();
    ...
    public static void addPowerData() {
        for (int i = 0; i < MachineRegistry.machineList.length; i++) {
            ...
            if (!m.isDummiedOut()) {
                if (m.isPowerReceiver() && !m.isModConversionEngine()
                    && !m.isPoweredTransmissionMachine()) {
                    ...
                    if (p != null) {
                        ...
                        throw new RegistrationException(
                            ...
                            "Machine " + m + " is a power receiver but has no power Enum!"
                        );
                }
            }
        }
    ...
    private static void addExtracts() {
        ...
        for (int k = 0; k < num; k++) {
            ...
            if (van) {
                ...
                if (ore == null) {
                    DragonAPICore.logError(
                        "ItemStack " + is.getDisplayName() + " (" + is.getItem() + ":"
                        + is.getItemDamage() + ")"
                    );
                    DragonAPICore.logError(
                        "has no mod ore list entry, yet was registered as such during load!"
                    );
            ...
            extracts.add(obj);
            //RotaryCraft.logger.log("Adding extractor entry to handbook: "+oreName+"
            //"+Arrays.toString(in)+";"+Arrays.toString(out));
        }
    }
    ...
    private static void addPlants() {
        ...
        if (ConfigRegistry.enableFermenterYeast()) {
            ...
            in = new ItemStack[] { new ItemStack(Items.sugar),
    ...
    public static void drawPage(
        FontRenderer f, RenderItem ri, int screen, int page, int subpage, int dx, int dy
    ) {
        HandbookRegistry h = HandbookRegistry.getEntry(screen, page);
        if (h.isMachine() || h.isTrans() || h.isEngine()
            || h.getParent() == HandbookRegistry.CONVERTERDESC) {
            List<ItemStack> out = h.getCrafting();
            if (out == null || out.size() <= 0)
                return;
            if (h.isCustomRecipe()) {
                api.drawCustomRecipes(
                    ri, f, out, getWorktable(), dx + 72 - 18, dy + 18, dx - 1620, dy + 32
                );
            } else {
                api.drawCustomRecipes(
                    ri,
            ...
            if (h.isCustomRecipe()) {
                api.drawCustomRecipes(
                    ri, f, out, getWorktable(), dx + 72, dy + 18, dx + 162, dy + 32
                );
            } else {
                api.drawCustomRecipes(
                    ri,
            ...
            if (h == HandbookRegistry.TUNGSTEN) {
                api.drawItemStackWithTooltip(
                    ri, f, ItemStacks.tungstenflakes, dx + 87, dy + 28
                );
            }
            ...
            MachineRecipeRenderer.instance.drawExtractor(
                dx + 66, dy + 17, in, dx + 66, dy + 59, out
            );
            ...
            for (int i = 0; i < words.length; i++)
                f.drawString(
                    words[i],
                    dx + 194,
                    dy + 60 + f.FONT_HEIGHT * i - words.length * f.FONT_HEIGHT / 2,
                    0
                );
            ...
            for (int k = 0; k < words.length; k++)
                f.drawString(
                    words[k],
                    dx + 168,
                    dy + 36 + f.FONT_HEIGHT * k - words.length * f.FONT_HEIGHT / 2,
                    0
                );
            ...
            MachineRecipeRenderer.instance.drawCompressor(
                dx + 66, dy + 14, in, dx + 120, dy + 41, out
            );
        } else if (h == HandbookRegistry.GLASS) {
            api.drawItemStackWithTooltip(
                ri, f, new ItemStack(Blocks.obsidian), dx + 87, dy + 28
            );
            api.drawItemStackWithTooltip(
                ri, f, BlockRegistry.BLASTGLASS.getStackOf(), dx + 145, dy + 28
            );
            ...
            if (k == 0) {
                ...
                ArrayList li = ReikaRecipeHelper.getAllRecipesByOutput(
                    CraftingManager.getInstance().getRecipeList(), out
                );
                api.drawCustomRecipeList(ri, f, li, dx + 72, dy + 18, dx + 162, dy + 32);
            } else if (k == 1) {
                ItemStack plate = k2 == 0 ? ItemRegistry.STEELCHEST.getStackOf()
                                          : ItemRegistry.BEDCHEST.getEnchantedStack();
                ItemStack out = k2 == 0 ? ItemRegistry.STEELPACK.getStackOf()
                                        : ItemRegistry.BEDPACK.getEnchantedStack();
                api.drawItemStackWithTooltip(ri, f, plate, dx + 72, dy + 10);
                api.drawItemStackWithTooltip(
                    ri, f, ItemRegistry.JETPACK.getStackOf(), dx + 90, dy + 10
                );
                api.drawItemStackWithTooltip(ri, f, out, dx + 166, dy + 28);
            } else if (k == 2) { //wing
                ItemStack ing = k3 != 2 ? ItemStacks.steelingot : ItemStacks.bedingot;
                ItemStack pack = k3 == 0 ? ItemRegistry.JETPACK.getStackOf()
                    : k3 == 1            ? ItemRegistry.STEELPACK.getStackOf()
                                         : ItemRegistry.BEDPACK.getEnchantedStack();
                ItemStack out = k3 == 0 ? ItemRegistry.JETPACK.getStackOf()
                    : k3 == 1           ? ItemRegistry.STEELPACK.getStackOf()
                                        : ItemRegistry.BEDPACK.getEnchantedStack();
                ...
                api.drawItemStackWithTooltip(
                    ri, f, EngineType.JET.getCraftedProduct(), dx + 90, dy + 46
                );
                api.drawItemStackWithTooltip(ri, f, pack, dx + 90, dy + 28);
                api.drawItemStack(ri, f, out, dx + 166, dy + 28);
                api.drawMultilineTooltip(out, dx + 166, dy + 28);
            ...
            if (k == 0) {
                ...
                List li = ReikaRecipeHelper.getAllRecipesByOutput(
                    CraftingManager.getInstance().getRecipeList(), out
                );
                api.drawCustomRecipeList(ri, f, li, dx + 72, dy + 18, dx + 162, dy + 32);
            } else {
                api.drawItemStackWithTooltip(
                    ri, f, ItemRegistry.BEDBOOTS.getEnchantedStack(), dx + 72, dy + 10
                );
                api.drawItemStackWithTooltip(
                    ri, f, ItemRegistry.JUMP.getStackOf(), dx + 90, dy + 10
                );
                api.drawItemStackWithTooltip(
                    ri, f, ItemRegistry.BEDJUMP.getEnchantedStack(), dx + 166, dy + 28
                );
            }
            ...
            MachineRecipeRenderer.instance.drawFermenter(
                dx + 102, dy + 18, in, dx + 159, dy + 32, out
            );
        } else if (h == HandbookRegistry.NETHERDUST) {
            if ((System.nanoTime() / 2000000000) % 2 == 0) {
                api.drawItemStackWithTooltip(
                    ri, f, new ItemStack(Blocks.netherrack), dx + 87, dy + 28
                );
                api.drawItemStackWithTooltip(
                    ri, f, ItemStacks.netherrackdust, dx + 145, dy + 28
                );
            } else {
                api.drawItemStackWithTooltip(
                    ri, f, new ItemStack(Blocks.soul_sand), dx + 87, dy + 28
                );
                api.drawItemStackWithTooltip(ri, f, ItemStacks.tar, dx + 145, dy + 28);
            }
        } else if (h == HandbookRegistry.SILVERINGOT) {
            api.drawItemStackWithTooltip(
                ri, f, ItemStacks.silverflakes, dx + 87, dy + 28
            );
            api.drawItemStackWithTooltip(
                ri, f, ItemStacks.silveringot, dx + 145, dy + 28
            );
        } else if (h == HandbookRegistry.SALT) {
            api.drawItemStackWithTooltip(
                ri, f, new ItemStack(Items.water_bucket), dx + 90, dy + 28
            );
            ...
            switch (k) {
                case 0:
                    api.drawItemStackWithTooltip(
                        ri, f, new ItemStack(Items.water_bucket), dx + 72 + 18, dy + 10
                    );
                    api.drawItemStackWithTooltip(
                        ri, f, ItemStacks.sawdust, dx + 72, dy + 28
                    );
                    api.drawItemStackWithTooltip(
                        ri, f, ItemStacks.sawdust, dx + 72 + 18, dy + 28
                    );
                    api.drawItemStackWithTooltip(
                        ri, f, ItemStacks.sawdust, dx + 72 + 36, dy + 28
                    );
                    api.drawItemStackWithTooltip(
                        ri, f, new ItemStack(Blocks.stone), dx + 72, dy + 46
                    );
                    api.drawItemStackWithTooltip(
                        ri, f, new ItemStack(Blocks.stone), dx + 72 + 18, dy + 46
                    );
                    api.drawItemStackWithTooltip(
                        ri, f, new ItemStack(Blocks.stone), dx + 72 + 36, dy + 46
                    );
                    api.drawItemStackWithTooltip(
                        ri, f, new ItemStack(Items.paper, 8, 0), dx + 166, dy + 28
                    );
                    break;
                case 1:
                    api.drawItemStackWithTooltip(
                        ri, f, ItemStacks.sawdust, dx + 72, dy + 10
                    );
                    api.drawItemStackWithTooltip(
                        ri, f, ItemStacks.sawdust, dx + 72, dy + 28
                    );
                    api.drawItemStackWithTooltip(
                        ri, f, ItemStacks.sawdust, dx + 72 + 18, dy + 10
                    );
                    api.drawItemStackWithTooltip(
                        ri, f, ItemStacks.sawdust, dx + 72 + 18, dy + 28
                    );
                    api.drawItemStackWithTooltip(
                        ri, f, ReikaItemHelper.oakWood.asItemStack(), dx + 166, dy + 28
                    );
                    break;
                case 2:
                    api.drawItemStackWithTooltip(
                        ri, f, ReikaDyeHelper.BLACK.getStackOf(), dx + 72 + 36, dy + 10
                    );
                    api.drawItemStackWithTooltip(
                        ri, f, ItemStacks.sawdust, dx + 72, dy + 10
                    );
                    ...
                    api.drawItemStackWithTooltip(
                        ri, f, ReikaItemHelper.spruceWood.asItemStack(), dx + 166, dy + 28
                    );
                    break;
                case 3:
                    api.drawItemStackWithTooltip(
                        ri, f, ReikaDyeHelper.WHITE.getStackOf(), dx + 72 + 36, dy + 10
                    );
                    api.drawItemStackWithTooltip(
                        ri, f, ItemStacks.sawdust, dx + 72, dy + 10
                    );
                    ...
                    api.drawItemStackWithTooltip(
                        ri, f, ReikaItemHelper.birchWood.asItemStack(), dx + 166, dy + 28
                    );
                    break;
                case 4:
                    api.drawItemStackWithTooltip(
                        ri, f, ReikaDyeHelper.RED.getStackOf(), dx + 72 + 36, dy + 10
                    );
                    api.drawItemStackWithTooltip(
                        ri, f, ItemStacks.sawdust, dx + 72, dy + 10
                    );
                    ...
                    api.drawItemStackWithTooltip(
                        ri, f, ReikaItemHelper.jungleWood.asItemStack(), dx + 166, dy + 28
                    );
                    break;
            }
            ...
            for (int i = 0; i < ItemRegistry.RAILGUN.getNumberMetadatas(); i++) {
                li.addAll(ReikaRecipeHelper.getAllRecipesByOutput(
                    CraftingManager.getInstance().getRecipeList(),
                    ItemRegistry.RAILGUN.getStackOfMetadata(i)
                ));
            }
            api.drawCustomRecipeList(ri, f, li, dx + 72, dy + 18, dx + 162, dy + 32);
            ...
            MachineRecipeRenderer.instance.drawBlastFurnaceCrafting(
                dx + 99, dy + 18, dx + 181, dy + 32, is
            );
        ...
        //	List<BlastRecipe> c =
        //RecipesBlastFurnace.getRecipes().getAllRecipesMaking(is);
        //	MachineRecipeRenderer.instance.drawBlastFurnace(dx+99, dy+18, dx+185, dy+36,
        //c.get(0));
        //}
        else if (h == HandbookRegistry.ALLOYING) {
            List<BlastFurnacePattern> c
                = RecipesBlastFurnace.getRecipes().getAllAlloyingRecipes();
            int index = (int) ((System.currentTimeMillis() / 2000) % c.size());
            BlastFurnacePattern p = c.get(index);
            if (p instanceof BlastRecipe)
                MachineRecipeRenderer.instance.drawBlastFurnace(
                    dx + 99, dy + 18, dx + 185, dy + 36, (BlastRecipe) p
                );
            else if (p instanceof BlastCrafting)
                MachineRecipeRenderer.instance.drawBlastFurnaceCrafting(
                    dx + 99, dy + 18, dx + 181, dy + 32, (BlastCrafting) p
                );
            else
                DragonAPICore.logError(
                    p + " to make " + p.outputItem()
                    + " is an invalid (unrenderable) recipe!"
                );
            api.drawCenteredStringNoShadow(
                f, p.getRequiredTemperature() + "C", dx + 54, dy + 66, 0
            );
        } else if (h == HandbookRegistry.COKE) {
            List<BlastRecipe> c
                = RecipesBlastFurnace.getRecipes().getAllRecipesMaking(ItemStacks.coke);
            int index = (int) ((System.currentTimeMillis() / 2000) % c.size());
            BlastRecipe p = c.get(index);
            MachineRecipeRenderer.instance.drawBlastFurnace(
                dx + 99, dy + 18, dx + 185, dy + 36, p
            );
            ...
            int index = (int) ((System.currentTimeMillis() / 2000) % c.size());
            MachineRecipeRenderer.instance.drawBlastFurnace(
                dx + 99, dy + 18, dx + 185, dy + 36, c.get(index)
            );
        }
    }
    ...
    public static void drawGraphics(
        ...
        try {
            if (h == HandbookRegistry.TERMS) {
                ...
                api.drawLine(
                    xc, yc, (int) (xc + r - 0.459 * r), (int) (yc - 0.841 * r), 0
                ); /*
...
api.drawLine(xc, yc, (int)(xc+Math.cos(i)*r), (int)(yc-Math.sin(i)*r), 0x000000);*/
                ...
                api.drawLine(
                    xc - 2 * r, (int) (yc - 1.4 * r), xc - r, yc - r * 2 - 2, 0x8800ff
                );
                api.drawLine(
                    xc - 2 * r, (int) (yc - 1.4 * r), xc - 2 * r - 2, yc, 0x8800ff
                );
                api.drawLine(
                    xc - 2 * r, (int) (yc + 1.4 * r), xc - 2 * r - 2, yc, 0x8800ff
                );
                ...
                api.drawLine(
                    xc,
                    yc,
                    (int) (xc + Math.cos(Math.toRadians(a)) * r),
                    (int) (yc + Math.sin(Math.toRadians(a)) * r),
                    0xff0000
                );
                api.drawLine(
                    xc,
                    yc,
                    (int) (xc + Math.cos(Math.toRadians(b)) * r),
                    (int) (yc + Math.sin(Math.toRadians(b)) * r),
                    0x0000ff
                );
                api.drawLine(
                    xc,
                    yc,
                    (int) (xc + Math.cos(Math.toRadians(c)) * r),
                    (int) (yc + Math.sin(Math.toRadians(c)) * r),
                    0x00a000
                );
                ...
                font.drawString(
                    n1 + " rad/s", xc + r - 4 + dx, yc + 18 + 10 - dy, 0x0000ff
                );
                font.drawString(
                    n2 + " rad/s", xc + r - 4 + dx, yc + 18 + 20 - dy, 0x00a000
                );
            }
            if (h == HandbookRegistry.BAITBOX && subpage == 1) {
                RenderItem ri = item;
                int k
                    = (int) ((System.nanoTime() / 2000000000) % MobBait.baitList.length);
                ...
                int UNIT = 4;
                api.drawTexturedModalRect(
                    posX + 88 - UNIT / 2, posY + 41 - UNIT / 2, u, v, UNIT * 2, UNIT * 2
                );
                font.drawString("Attractor", posX + 110, posY + 30, 0);
                font.drawString("Repellent", posX + 110, posY + 48, 0);
                //RenderManager.instance.renderEntityWithPosYaw(new
                //EntityCreeper(Minecraft.getMinecraft().theWorld), 120, 60, 0, 0, 0);
            } else if (h == HandbookRegistry.TERRA && subpage == 1) {
                RenderItem ri = item;
                ArrayList<BiomeTransform> transforms
                    = TileEntityTerraformer.getTransformList();
                int time = 2000000000;
                ...
                for (int i = 0; i < words.length; i++) {
                    api.drawCenteredStringNoShadow(
                        font, words[i], posX + 33, posY + 57 + i * font.FONT_HEIGHT, 0
                    );
                }
                ...
                for (int i = 0; i < words2.length; i++) {
                    api.drawCenteredStringNoShadow(
                        font, words2[i], posX + 97, posY + 57 + i * font.FONT_HEIGHT, 0
                    );
                }
                font.drawString(
                    String.format("%.3f kW", data.power / 1000D), posX + 116, posY + 22, 0
                );
                ...
                if (liq != null) {
                    ...
                    api.drawTexturedModelRectFromIcon(
                        posX + 116 + 16, posY + 38, ico, 16, 16
                    );
                    //api.drawItemStack(ri, fontRenderer, liq.asItemStack(), posX+116,
                    //posY+38); api.drawItemStack(ri, fontRenderer, liq.asItemStack(),
                    //posX+116+16, posY+38);
                }
                ...
                for (long key : s) {
                    if (t == subpage) {
                        ...
                        font.drawString(
                            sg,
                            posX + font.getStringWidth("Machine Tiers") + 14,
                            posY + 6,
                            0
                        );
                ...
                for (int i = 0; i < DurationRegistry.durationList.length; i++) {
                    ...
                    if (api.isMouseInBox(x, x + 17, y, y + 17)) {
                        for (int j = 0; j < d.getNumberStages(); j++) {
                            ...
                            font.drawString(
                                d.getDisplayTime(j), posX + 10, posY + 150 + j * 10, c
                            );
                        }
                    }
                    ...
                    for (int i = di; i < max; i++) {
                        ...
                        if (api.isMouseInBox(x, x + 17, y, y + 17)) {
                            ...
                            for (LuaMethod cur : li) {
                                if (cur.isDocumented()
                                    && cur.isClassInstanceOf(m.getTEClass())) {
                                    ReikaRenderHelper.disableLighting();
                                    String s = cur.getReturnType().displayName + " "
                                        + cur.displayName + "(" + cur.getArgsAsString()
                                        + ")";
                                    font.drawString(
                                        s, posX + 11, posY + 88 + k * 10, 0xffffff
                                    );
                                    k++;
                                }
                            }
                ...
                if (li.isEmpty()) {
                    font.drawSplitString(
                        "All config settings are identical to defaults.",
                        posX + 10,
                        posY + 88,
                        245,
                        0xffffff
                    ...
                    font.drawSplitString(
                        "Your gameplay is in line with what has been intended.",
                        posX + 10,
                        posY + 98,
                        245,
                        0xffffff
                    ...
                    for (int i = base; i < max; i++) {
                        ...
                        font.drawSplitString(
                            msg, posX + 10, posY + 88 + dy * 44, 245, 0xffffff
                        );
                        dy++;
                    }
                }
                ...
                String title = "These are changes made to the way the mod works by the creator of the pack. None of these are normal " +
						...
						"the mod developer.";
                font.drawSplitString(title, posX + 8, posY + 20, 220, 0x333333);
                List<PackModification> li
                    = PackModificationTracker.instance.getModifications(
                        RotaryCraft.instance
                    );
