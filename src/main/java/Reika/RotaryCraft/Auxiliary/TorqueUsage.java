...
public class TorqueUsage {
    ...
    private static void
    recursiveFind(TileEntity tile, TileEntityFlywheel reader, double activeRatio) {
        if (tile == null)
            return;
        ...
        if (tile instanceof TileEntityTransmissionMachine) { //true if the considered tile
                                                             //is a Transmission tile and
                                                             //is getting power from an
                                                             //already examined block
            ...
            if (tile instanceof TileEntitySplitter) { //check if splitter
                ...
                if (!spl.isSplitting(
                    )) { //check if merge mode or split mode (true if in merge mode)
                    TileEntity write = spl.getWriteTileEntity();
                    if (isPoweredFrom(write, spl)) {
                        recursiveFind(
                            write, reader, activeRatio
                        ); //records the following tile inside the list
                    }
                } else {
                    TileEntity di = spl.getWriteTileEntity(); //records both outputs
                    TileEntity di2 = spl.getWriteTileEntity2();
                    if (isPoweredFrom(di, spl)) { //calls the recursion first with one
                                                  //output, then with the other
                ...
                requiredTorque
                    += activeRatio * ((TileEntityFlywheel) tile).getOppTorque(reader);
                ...
                if (isPoweredFrom(gbx.getWriteTileEntity(), gbx)) {
                    if (gbx.reduction) {
                        recursiveFind(
                            gbx.getWriteTileEntity(), reader, activeRatio / gbx.getRatio()
                        );
                    } else {
                        recursiveFind(
                            gbx.getWriteTileEntity(), reader, activeRatio * gbx.getRatio()
                ...
                switch (adv.getGearType()) {
                    case WORM:
                        if (!isPoweredFrom(adv.getWriteTileEntity(), adv)) {
                            recursiveFind(
                                adv.getWriteTileEntity(), reader, activeRatio / 16
                            );
                        }
                        break;
                    case CVT:
                        if (isPoweredFrom(adv.getWriteTileEntity(), adv)) {
                            if (adv.getRatio() > 0) {
                                recursiveFind(
                                    adv.getWriteTileEntity(),
                                    reader,
                                    activeRatio * adv.getRatio()
                                );
                            } else {
                                recursiveFind(
                                    adv.getWriteTileEntity(),
                    ...
                    case HIGH:
                        if (isPoweredFrom(adv.getWriteTileEntity(), adv)) {
                            ...
                            recursiveFind(
                                adv.getWriteTileEntity(), reader, activeRatio / d
                            );
                        }
                        break;
                }
                ...
                if (sha.isCross()) {
                    ...
                    if (isPoweredFrom(
                            io.getWriteTileEntity(), io
                        )) { //if it's anything else, it just gets its current output and
                             //then calls the recursion
                ...
                if (isPoweredFrom(
                        io.getWriteTileEntity(), io
                    )) { //if it's anything else, it just gets its current output and then
                         //calls the recursion
                    recursiveFind(io.getWriteTileEntity(), reader, activeRatio);
                }
            }
        } else if (tile instanceof TileEntityPowerReceiver) { //if the tile is a power
                                                              //Receiver, it stores its
            ...
            if (tile instanceof TileEntityBeltHub) {
                ...
                double min = 0;
                if (tile
                    instanceof TileEntityExtractor) { //if it's an extractor, it stores
                                                      //the torque requirement of the most
                                                      //demanding WORKING stage
            ...
            requiredTorque
                += Math.max(((PowerAcceptor) tile).getMinTorque(reader.torque), 1);
        }
    }

    private static void manageBus(
        TileEntityBusController tile, TileEntityFlywheel reader, double activeRatio
    ) {
            ...
            for (int k = 2; k < 6; k++) {
                ...
                if (te.canOutputToSide(dir)) {
                    ...
                    if (out instanceof TileEntityIOMachine) {
                        ...
                        if (io.getReadTileEntity() == te || read == te || read2 == te
                            || read3 == te || read4 == te) {
    ...
    private static boolean isPoweredFrom(TileEntity tile, TileEntityIOMachine caller) {
        ...
        if (tile instanceof TileEntityIOMachine) {
            ...
            return loc.equals(read) || loc.equals(read2) || loc.equals(read3)
                || loc.equals(read4);
        } else if (tile instanceof PowerAcceptor) {
    ...
    canReadFrom(TileEntityIOMachine te, TileEntityIOMachine input) {
        return te.getReadTileEntity() == input || te.getReadTileEntity2() == input
            || te.getReadTileEntity3() == input || te.getReadTileEntity4() == input;
    }
}
