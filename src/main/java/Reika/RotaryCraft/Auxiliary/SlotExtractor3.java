...
public class SlotExtractor3 extends Slot {
    ...
    public SlotExtractor3(IInventory par2IInventory, int par3, int par4, int par5) {
    ...
    public boolean isItemValid(ItemStack is) {
        if (ItemRegistry.EXTRACTS.matchItem(is) && is.getItemDamage() < 16
            && is.getItemDamage() >= 8)
            return true;
        if (ExtractorModOres.isModOreIngredient(is)
            && ExtractorModOres.isSlurry(
