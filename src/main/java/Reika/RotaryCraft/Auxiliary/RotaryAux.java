...
public class RotaryAux {
    ...
    public static final double tungstenDensity
        = ReikaEngLibrary.rhoiron * 0.8 + 0.2 * ReikaEngLibrary.rhotungsten;

    private static Set<Class<? extends TileEntity>> shaftPowerBlacklist
        = new OneWaySet<Class<? extends TileEntity>>();
    ...
    private static void addShaftBlacklist(String name) {
        ...
        try {
            ...
            RotaryCraft.logger.log(
                "Disabling " + name + " for shaft power. Destructive compatibility."
            );
    ...
    hasGui(World world, int x, int y, int z, EntityPlayer ep) {
        ...
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT) {
            Object GUI = GuiHandler.instance.getClientGuiElement(
                GuiRegistry.MACHINE.ordinal(), ep, world, x, y, z
            );
            if (GUI != null)
                return true;
        }
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER) {
            Object GUI = GuiHandler.instance.getServerGuiElement(
                GuiRegistry.MACHINE.ordinal(), ep, world, x, y, z
            );
            if (GUI != null)
                return true;
    ...
    public static boolean canHarvestSteelMachine(EntityPlayer ep) {
        ...
        if (TinkerToolHandler.getInstance().isHammer(eitem))
            return false;
        if (TinkerToolHandler.getInstance().isPick(eitem)
            && TinkerToolHandler.getInstance().isStoneOrBetter(eitem))
            return true;
        if (MekToolHandler.getInstance().isPickTypeTool(eitem)
            && !MekToolHandler.getInstance().isWood(eitem))
            return true;
    ...
    public static boolean isMufflingBlock(World world, int x, int y, int z) {
        ...
        return b.getMaterial() == Material.cloth
            || b == Block.getBlockFromName("Rockwool");
    }
    ...
    public static boolean isNextToIce(World world, int x, int y, int z) {
        ...
        if (b instanceof EnvironmentalHeatSource) {
            ...
            return ehs.isActive(world, x, y, z)
                && ehs.getSourceType(world, x, y, z).isCold();
        }
        return false;
    }
    ...
    public static boolean isNextToWater(World world, int x, int y, int z) {
        ...
        for (int i = 1; i <= 2; i++) {
            ...
            if (b instanceof EnvironmentalHeatSource) {
                EnvironmentalHeatSource ehs = (EnvironmentalHeatSource) b;
                return ehs.isActive(world, x, y - i, z)
                    && ehs.getSourceType(world, x, y - i, z) == SourceType.WATER;
            }
        }
        return false;
    }
    ...
    public static boolean isNextToFire(World world, int x, int y, int z) {
        ...
        for (int i = 1; i <= 2; i++) {
            ...
            if (b instanceof EnvironmentalHeatSource) {
                EnvironmentalHeatSource ehs = (EnvironmentalHeatSource) b;
                return ehs.isActive(world, x, y - i, z)
                    && ehs.getSourceType(world, x, y - i, z) == SourceType.FIRE;
            }
        }
        return false;
    }
    ...
    public static boolean isNextToLava(World world, int x, int y, int z) {
        ...
        for (int i = 1; i <= 2; i++) {
            ...
            if (b instanceof EnvironmentalHeatSource) {
                EnvironmentalHeatSource ehs = (EnvironmentalHeatSource) b;
                return ehs.isActive(world, x, y - i, z)
                    && ehs.getSourceType(world, x, y - i, z) == SourceType.LAVA;
            }
        }
        return false;
    }
    ...
    public static String formatTorqueSpeedPowerForBook(
        String text, double torque, double speed, double power
    ) {
    ...
    formatSingleValueForBook(String text, double value, int torqueSpeedPowerSelector) {
        boolean old = OldTextureLoader.instance.loadOldTextures();
        String unit = null;
    ...
    public static boolean isShaftCross(ItemStack is) {
        return ItemRegistry.SHAFT.matchItem(is) && is.stackTagCompound != null
            && is.stackTagCompound.getBoolean("cross");
    }
    ...
    private static boolean isScrewFocusWand(ItemStack is) {
        return is != null
            ...
            == ItemRegistry.SCREWFOCUS.getItemInstance();
    }
}
