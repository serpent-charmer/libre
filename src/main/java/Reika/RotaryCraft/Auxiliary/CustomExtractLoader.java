...
public class CustomExtractLoader {
    ...
    public static class CustomExtractEntry implements OreType {
            ...
            for (int i = 0; i < ores.length; i++) {
                ...
                if (oreItems.isEmpty())
                    throw new IllegalStateException(
                        "Cannot have entries with no corresponding ores!"
                    );
            }
            rarity = r;
            color1 = c1;
        ...
        public String toString() {
            return displayName + " to " + productName + " with rarity " + rarity + " ("
                + oreNames.size() + " ores found from " + oreNames + ")";
        }
                ...
                try {
                    ...
                    if (entry != null) {
                        ...
                        RotaryCraft.logger.logError(
                            "Malformed custom extract entry: " + line
                        );
                    }
                } catch (Exception e) {
                    RotaryCraft.logger.logError(
                        "Malformed custom extract entry [" + e.getLocalizedMessage()
                        + "]: '" + line + "'"
                    );
                    e.printStackTrace();
                }
    ...
    private boolean createOreFile(File f) {
        ...
        this.writeCommentLine(
            p, "Use this file to add custom ores and extracts to the extractor."
        );
        this.writeCommentLine(
            p,
        ...
        for (OreRarity o : OreRarity.values()) {
            this.writeCommentLine(
                p, "\t" + o.name() + " - " + o.desc + ", like " + o.examples + ""
            );
        }
        this.writeCommentLine(p, "");
        ...
        for (ModOreList o : ModOreList.values()) {
            if (!o.isNetherOres())
                this.writeCommentLine(
                    p, "\t" + o.name() + " - " + o.displayName + " " + o.getTypeName()
                );
        }
        this.writeCommentLine(p, "");
        this.writeCommentLine(
        ...
        this.writeCommentLine(
            p, "\tWhile you can theoretically make anything processable in the Extractor,"
        );
        this.writeCommentLine(
            p,
    ...
    private CustomExtractEntry parseString(String s) throws Exception {
        ...
        return new CustomExtractEntry(
            data.size(), name, rarity, type, prod, smelt, c1, c2, ore, ores
        );
    }
    ...
    private OreType parseOreType(String tag) {
        ...
        if (type == null)
            throw new IllegalArgumentException(
                "Native ore type '" + tag + "' is invalid."
            );
        return type;
    }
