...
public class ShaftPowerBus {
    ...
    public String toString() {
        ...
        sb.append(
            "Power Bus Receiving " + this.getInputTorque() + " Nm @ " + this.getSpeed()
            + " rad/s\n"
        );
        sb.append(
            this.getInputPower() + "W is being split to " + this.getTotalOutputSides()
            + " devices\n"
        );
        sb.append(
            "(Power per side is " + this.getInputPower() / this.getTotalOutputSides()
            + "W)"
        );
        return sb.toString();
    }
    ...
    private void rebuild() {
        ...
        for (int i = 2; i < 6; i++) {
            ...
            if (m == MachineRegistry.POWERBUS) {
                b.recursiveAddWithMetadata(
                    hub.worldObj, dx, dy, dz, m.getBlock(), m.getBlockMetadata()
                );
            }
        }
