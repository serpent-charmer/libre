...
public class RecyclingRecipe implements IRecipe, CustomToStringRecipe {
    ...
    public RecyclingRecipe(int value, ItemStack... recipe) {
        this(
            ReikaRecipeHelper.getShapelessRecipeFor(
                ReikaItemHelper.getSizedItemStack(ItemStacks.scrap, value), recipe
            ),
            value
        );
    }
    ...
    public ItemStack getCraftingResult(InventoryCrafting ics) {
        return ReikaItemHelper.getSizedItemStack(
            ItemStacks.scrap, scrapValue
        ); //ItemStacks.getScrap(scrapValue);
    }
    ...
    public String toDeterministicString() {
        return scrapValue
            + " X scrap = " + ReikaRecipeHelper.toDeterministicString(input);
    }
}
