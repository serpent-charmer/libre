...
public class TemperatureHandler {
    ...
    public int
    getAmbientTemperature(World world, int x, int y, int z, int minAmb, int maxAmb) {
        return MathHelper.clamp_int(
            ReikaWorldHelper.getAmbientTemperatureAt(world, x, y, z), minAmb, maxAmb
        );
    }

    public int getAmbientTemperatureOffset(
        World world,
        int x,
        int y,
        int z,
        int temp,
        boolean allowHeating,
        boolean allowCooling
    ) {
        return this.getAmbientTemperatureOffset(
            world,
            x,
            y,
            z,
            temp,
            allowHeating ? EnumSet.allOf(ForgeDirection.class) : null,
            allowCooling ? EnumSet.allOf(ForgeDirection.class) : null
        );
    }

    public int getAmbientTemperatureOffset(
        TemperatureTE te, Set<ForgeDirection> heatSides, Set<ForgeDirection> coolSides
    ) {
    ...
    public int getAmbientTemperatureOffset(
        ...
        int z,
        int temp,
        Set<ForgeDirection> heatSides,
        Set<ForgeDirection> coolSides
    ) {
        int ret = 0;
        if (heatSides != null) {
            for (ForgeDirection dir : heatSides) {
                int get = this.getBlockTemperature(
    ...
    public void applyTemperature(TemperatureTE te) {
        ...
        this.applyTemperature(
            tile.worldObj, tile.xCoord, tile.yCoord, tile.zCoord, te.getTemperature()
        );
    }
