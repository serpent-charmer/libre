...
public class RotaryRenderList {
    private static HashMap<MachineRegistry, RotaryTERenderer> renders
        = new HashMap<MachineRegistry, RotaryTERenderer>();
    private static HashMap<MachineRegistry, MachineRegistry> overrides
        = new HashMap<MachineRegistry, MachineRegistry>();
    ...
    public static RotaryTERenderer instantiateRenderer(MachineRegistry m) {
        ...
        try {
            RotaryTERenderer r
                = (RotaryTERenderer) Class.forName(m.getRenderer()).newInstance();
            if (addRender(m, r))
                return r;
            else
                return renders.get(overrides.get(m));
            ...
            throw new RuntimeException(
                "Tried to call nonexistent render " + m.getRenderer() + "!"
            );
        } catch (IllegalAccessException e) {
            e.printStackTrace();
            throw new RuntimeException(
                "Tried to call illegal render " + m.getRenderer() + "!"
            );
