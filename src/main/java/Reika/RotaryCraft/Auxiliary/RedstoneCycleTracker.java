...
public class RedstoneCycleTracker {
    ...
    public void update(World world, int x, int y, int z) {
        alternating
            = (this.hasIntegrated() && !world.isBlockIndirectlyGettingPowered(x, y, z))
            || ReikaRedstoneHelper.isGettingACRedstone(world, x, y, z, lastPower);
        if (this.hasIntegrated() && alternating) {
            if (world.getTotalWorldTime() % 6 == 0) {
                ...
                ReikaSoundHelper.playSoundAtBlock(
                    world, x, y, z, "random.click", f, 0.66F
                );
            }
        }
    }
