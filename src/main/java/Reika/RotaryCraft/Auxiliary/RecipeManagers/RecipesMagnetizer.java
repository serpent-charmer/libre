...
public class RecipesMagnetizer extends RecipeHandler implements MagnetizerManager {
    ...
    private RecipesMagnetizer() {
        ...
        this.addRecipe(
            ItemRegistry.UPGRADE.getStackOfMetadata(Upgrades.MAGNETOSTATIC2.ordinal()),
            32768,
            1,
            4,
            true,
    ...
    private void addRecipe(
        ...
        this.addRecipe(
            in, minSpeed, reqSpeedPerMicroTesla, timeFactor, allowStacks, null, rl
        );
    }
    ...
    private void addRecipe(
        ...
        MagnetizerRecipe rec = new MagnetizerRecipe(
            in, timeFactor, minSpeed, reqSpeedPerMicroTesla, allowStacks, a
        );
        recipes.put(in, rec);
    ...
    public static class MagnetizerRecipe implements MachineRecipe {
        ...
        public String getAllInfo() {
            return "Magnetizing " + fullID(item) + " @ speed " + minSpeed + " to "
                + speedPeruT + "/rads @ " + timeFactor + "x time factor";
        }
    }
    ...
    public String getRecipesAsString() {
        ...
        for (MagnetizerRecipe mr : recipes.values()) {
            sb.append(
                mr.item.getDisplayName() + " @ "
                + Math.max(PowerReceivers.MAGNETIZER.getMinSpeed(), mr.minSpeed)
                + " rad/s;\n"
            );
            sb.append(
                mr.timeFactor + "x time cost, " + mr.speedPeruT + "rad/s per uT\n\n"
            );
        }
        return sb.toString();
    }
