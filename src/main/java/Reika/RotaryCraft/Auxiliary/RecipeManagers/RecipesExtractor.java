...
public class RecipesExtractor {
    ...
    private RecipesExtractor() {
        for (int i = 0; i < 24; i++)
            this.addRecipe(
                ItemRegistry.EXTRACTS.getStackOfMetadata(i),
                ItemRegistry.EXTRACTS.getStackOfMetadata(i + 8)
            );
        ...
        this.addRecipe(
            Blocks.redstone_ore, 0, ItemStacks.getDust(ReikaOreHelper.REDSTONE)
        );
        ...
        for (int i = 0; i < ModOreList.oreList.length; i++) {
            ...
            for (ItemStack is : c) {
                if (recipeList.containsKey(is)) {
                    ModOreList mod = (ModOreList
                    ) ExtractorModOres.getOreFromExtract(recipeList.get(is));
                    RotaryCraft.logger.log(
                        "Ore " + is.getDisplayName()
                        + " is being skipped for Extractor registration as " + ore
            ...
            this.addRecipe(
                ExtractorModOres.getDustProduct(ore),
                ExtractorModOres.getSlurryProduct(ore)
            );
            this.addRecipe(
                ExtractorModOres.getSlurryProduct(ore),
                ExtractorModOres.getSolutionProduct(ore)
            );
            this.addRecipe(
                ExtractorModOres.getSolutionProduct(ore),
                ExtractorModOres.getFlakeProduct(ore)
            );
        }
        ...
        for (int i = 0; i < li.size(); i++) {
            ...
            for (ItemStack is : c) {
                if (recipeList.containsKey(is)) {
                    RotaryCraft.logger.log(
                        "Ore " + is.getDisplayName()
                        + " is being skipped for Extractor registration as " + e
                        + " as it is already registered to " + recipeList.get(is)
                    );
                } else {
            ...
            this.addRecipe(
                ItemCustomModOre.getItem(i, ExtractorStage.DUST),
                ItemCustomModOre.getItem(i, ExtractorStage.SLURRY)
            );
            this.addRecipe(
                ItemCustomModOre.getItem(i, ExtractorStage.SLURRY),
                ItemCustomModOre.getItem(i, ExtractorStage.SOLUTION)
            );
            this.addRecipe(
                ItemCustomModOre.getItem(i, ExtractorStage.SOLUTION),
                ItemCustomModOre.getItem(i, ExtractorStage.FLAKES)
            );
        }
    }
    ...
    public static boolean isSlurry(ItemStack is) {
        if (!ItemRegistry.EXTRACTS.matchItem(is))
            return false;
        int dmg = is.getItemDamage();
        return dmg < ReikaOreHelper.oreList.length * 2
            && dmg >= ReikaOreHelper.oreList.length;
    }
    ...
    public static boolean isSolution(ItemStack is) {
        if (!ItemRegistry.EXTRACTS.matchItem(is))
            return false;
        int dmg = is.getItemDamage();
        return dmg < ReikaOreHelper.oreList.length * 3
            && dmg >= ReikaOreHelper.oreList.length * 2;
    }

    public static boolean isFlakes(ItemStack is) {
        if (!ItemRegistry.EXTRACTS.matchItem(is))
            return false;
        if (ReikaItemHelper.matchStacks(is, ItemStacks.silverflakes)
            || ReikaItemHelper.matchStacks(is, ItemStacks.tungstenflakes))
            return true;
        int dmg = is.getItemDamage();
        return dmg < ReikaOreHelper.oreList.length * 4
            && dmg >= ReikaOreHelper.oreList.length * 3;
    }

    public static ReikaOreHelper getOreFromExtract(ItemStack item) {
        return ReikaOreHelper
            .oreList[item.getItemDamage() % ReikaOreHelper.oreList.length];
    }
}
