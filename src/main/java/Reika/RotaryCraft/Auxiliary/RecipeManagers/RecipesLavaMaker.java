...
public class RecipesLavaMaker extends RecipeHandler implements RockMelterManager {
    ...
    private RecipesLavaMaker() {
        ...
        this.addRecipe(
            ...
            1000,
            1000,
            ReikaThermoHelper.ROCK_MELT_ENERGY,
        ...
        this.addRecipe(
            "stone", FluidRegistry.LAVA, 1000, 1000, 5200000, RecipeLevel.PROTECTED
        );
        this.addRecipe(
            "cobblestone", FluidRegistry.LAVA, 500, 1000, 2820000, RecipeLevel.PROTECTED
        );
        ...
        this.addRecipe(
            ItemStacks.cleansludge, "rc ethanol", 1000, 180, 9000, RecipeLevel.CORE
        );
    }
    ...
    private static class MeltingRecipe implements MachineRecipe {
        ...
        public String getUniqueID() {
            return fullID(input) + "@" + temperature + "#" + output.getFluid().getName()
                + ":" + output.amount;
        }

        @Override
        public String getAllInfo() {
            return "Melting " + fullID(input) + "into " + output.amount + " of "
                + output.getLocalizedName() + " @ " + temperature + "C using "
                + requiredEnergy + " J";
        }
    ...
    private void addRecipe(
        String in, String out, int amt, int temperature, long energy, RecipeLevel rl
    ) {
        if (this.validateFluid(out)) {
            ...
            for (int i = 0; i < li.size(); i++)
                this.addRecipe(
                    li.get(i),
                    new FluidStack(FluidRegistry.getFluid(out), amt),
                    temperature,
                    energy,
                    rl
                );
    ...
    private void addRecipe(
        String in, Fluid out, int amt, int temperature, long energy, RecipeLevel rl
    ) {
        ArrayList<ItemStack> li = OreDictionary.getOres(in);
    ...
    private void addRecipe(
        ItemStack in, String out, int amt, int temperature, long energy, RecipeLevel rl
    ) {
        if (this.validateFluid(out))
            this.addRecipe(
                in,
    ...
    private void addRecipe(
        Item in, String out, int amt, int temperature, long energy, RecipeLevel rl
    ) {
        if (this.validateFluid(out))
            this.addRecipe(
                new ItemStack(in),
    ...
    private void addRecipe(
        Block in, String out, int amt, int temperature, long energy, RecipeLevel rl
    ) {
        if (this.validateFluid(out))
            this.addRecipe(
                new ItemStack(in),
    ...
    private void addRecipe(
        Block in, Fluid out, int amt, int temperature, long energy, RecipeLevel rl
    ) {
        this.addRecipe(
            new ItemStack(in), new FluidStack(out, amt), temperature, energy, rl
        );
    }

    private void
    addRecipe(Item in, Fluid out, int amt, int temperature, long energy, RecipeLevel rl) {
        this.addRecipe(
            new ItemStack(in), new FluidStack(out, amt), temperature, energy, rl
        );
    }
    ...
    private void
    addRecipe(Block in, FluidStack out, int temperature, long energy, RecipeLevel rl) {
        this.addRecipe(new ItemStack(in), out, temperature, energy, rl);
    }

    private void
    addRecipe(Item in, FluidStack out, int temperature, long energy, RecipeLevel rl) {
        this.addRecipe(new ItemStack(in), out, temperature, energy, rl);
    }

    private void addRecipe(
        ItemStack in, FluidStack out, int temperature, long energy, RecipeLevel rl
    ) {
        if (in != null) {
    ...
    public void addPostLoadRecipes() {
        this.addRecipe(
            "dustGlowstone", "glowstone", 250, 400, 80000, RecipeLevel.MODINTERACT
        );
        this.addRecipe(
            Blocks.glowstone, "glowstone", 1000, 500, 320000, RecipeLevel.MODINTERACT
        );
        this.addRecipe(
            "dustRedstone", "redstone", 100, 600, 120000, RecipeLevel.MODINTERACT
        );
        this.addRecipe(
            Blocks.redstone_block, "redstone", 900, 750, 1080000, RecipeLevel.MODINTERACT
        );
        this.addRecipe(
            Items.ender_pearl, "ender", 250, 400, 240000, RecipeLevel.MODINTERACT
        );
        this.addRecipe("blockEnder", "ender", 1000, 400, 240000, RecipeLevel.MODINTERACT);
        this.addRecipe(
            "dustCoal",
            "coal",
            ModList.ENDERIO.isLoaded() ? 90 : 100,
            300,
            60000,
        ...
        if (ModList.THERMALFOUNDATION.isLoaded()) {
            ItemStack pyro = GameRegistry.findItemStack(
                ModList.THERMALFOUNDATION.modLabel, "dustPyrotheum", 1
            );
            this.addRecipe(
                pyro, "pyrotheum", 250, 1800, 9000000, RecipeLevel.MODINTERACT
            );

            ItemStack cryo = GameRegistry.findItemStack(
                ModList.THERMALFOUNDATION.modLabel, "dustCryotheum", 1
            );
            this.addRecipe(cryo, "cryotheum", 250, -200, 2000, RecipeLevel.MODINTERACT);

            ItemStack petro = GameRegistry.findItemStack(
                ModList.THERMALFOUNDATION.modLabel, "dustPetrotheum", 1
            );
            this.addRecipe(
                petro, "petrotheum", 250, 800, 12000000, RecipeLevel.MODINTERACT
            );

            ItemStack aero = GameRegistry.findItemStack(
                ModList.THERMALFOUNDATION.modLabel, "dustAerotheum", 1
            );
            this.addRecipe(aero, "aerotheum", 250, 400, 40000, RecipeLevel.MODINTERACT);
        }

        this.addRecipe(
            "shardCrystal", "potion crystal", 8000, 500, 80000, RecipeLevel.MODINTERACT
        );

        if (ModList.MAGICCROPS.isLoaded()
            && MagicCropHandler.EssenceType.XP.getEssence() != null)
            this.addRecipe(
                MagicCropHandler.EssenceType.XP.getEssence(),
                "mobessence",
                200,
                600,
                360000,
    ...
    private void addLavaRock() {
        for (int i = 0; i < 4; i++) {
            ...
            this.addRecipe(
                ...
                300 + 200 * i,
                ReikaThermoHelper.ROCK_MELT_ENERGY / (i + 1),
                RecipeLevel.MODINTERACT
            );
        }
    }
