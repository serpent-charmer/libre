...
public class RecipesDryingBed extends RecipeHandler implements DryingBedManager {
    ...
    private RecipesDryingBed() {
        ...
        this.addRecipe(
            "for.honey", 400, new ItemStack(Items.slime_ball), RecipeLevel.PERIPHERAL
        );
        this.addRecipe(
            "honey", 400, new ItemStack(Items.slime_ball), RecipeLevel.PERIPHERAL
        );

        this.addRecipe(
            "chroma", 2000, new ItemStack(Items.emerald), RecipeLevel.MODINTERACT
        );
    }
    ...
    public static class DryingRecipe implements MachineRecipe {
        ...
        public String getAllInfo() {
            return "Drying " + input.amount + " of " + input.getLocalizedName() + " into "
                + fullID(output);
        }
    ...
    public DryingRecipe getRecipe(ItemStack result) {
        for (DryingRecipe cr : recipeList.values()) {
            ...
            "+ReikaFluidHelper.fluidStackToString(dr)+", despite being in the keyset of
            the map, returned null on get()!"); sb.append("\nMap data:
