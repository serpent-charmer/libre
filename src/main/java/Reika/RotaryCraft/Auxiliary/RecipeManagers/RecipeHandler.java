...
public abstract class RecipeHandler implements IngredientIDHandler {
    ...
    private final MultiMap<RecipeLevel, String> recipesByLevel
    ...
    private String generateKey(MachineRecipe recipe) {
        String s = machine.name() + "$" + recipe.getClass().getSimpleName() + "#("
            + recipe.getUniqueID();
        ...
        if (recipeKeys.containsValue(s)) {
            ...
            if (pre == null || pre.equals(recipe)) {
                ...
                RotaryCraft.logger.logError(
                    "Found duplicate recipe key when adding recipe " + recipe.getAllInfo()
                    + " in place of " + pre.getAllInfo()
                );
                RotaryCraft.logger.log("Original Recipe Items:");
                for (ItemStack is : pre.getAllUsedItems()) {
                    RotaryCraft.logger.log(
                        is + " from mod '" + ReikaItemHelper.getRegistrantMod(is)
                ...
                throw new RegistrationException(
                    RotaryCraft.instance, "Two recipes have the same key: '" + s + "'"
                );
            }
        }
    ...
    public final boolean removeRecipe(String rec) {
        ...
        if (power.canRemove(rl)) {
            ...
            if (rec == null) {
                RotaryCraft.logger.log(
                    "Recipe removal of '" + rec + "' from " + machine
                    + " not possible; No such recipe with that key."
                );
                return false;
            }
            try {
                ...
                RotaryCraft.logger.log(
                    "Recipe removal of '" + rec + "' from " + machine
                    + " failed and threw an exception; Potential code error."
                );
                e.printStackTrace();
                return false;
            }
            ...
            RotaryCraft.logger.log(
                "Recipe removal of '" + rec + "' from " + machine
                + " rejected; This is a '" + rl + "' recipe and cannot be modified with '"
                + power + "' modify power."
            );
            return false;
        }
    ...
    private static RecipeModificationPower getRequiredPowerForOutput(ItemStack is) {
        ...
        if (!ReikaItemHelper.getNamespace(is.getItem()).contains("ElectriCraft"))
            return RecipeModificationPower.DEFAULT;
        return SensitiveItemRegistry.instance.contains(is)
            ? RecipeModificationPower.FULL
            : RecipeModificationPower.NORMAL;
    }

    private static RecipeModificationPower getRequiredPowerForOutput(Fluid f) {
        return SensitiveFluidRegistry.instance.contains(f)
            ? RecipeModificationPower.FULL
            : RecipeModificationPower.DEFAULT;
    }

    public static boolean isOutputPermitted(ItemStack is) {
        return getModificationPower().ordinal()
            <= getRequiredPowerForOutput(is).ordinal();
    }
    ...
    private static RecipeModificationPower getModificationPower() {
        int get = Math.min(
            RecipeModificationPower.DEFAULT.ordinal(),
            Math.max(0, ConfigRegistry.getRecipeModifyPower())
        );
        return RecipeModificationPower
            .list[RecipeModificationPower.DEFAULT.ordinal() - get];
    }
    ...
    public final void loadCustomRecipeFiles() {
        CustomRecipeList crl = new CustomRecipeList(
            RotaryCraft.instance, machine.name().toLowerCase(Locale.ENGLISH)
        );
        if (crl.load()) {
            for (LuaBlock lb : crl.getEntries()) {
                ...
                if (flag) {
                    RotaryCraft.logger.log(
                        "Loaded custom recipe '" + n + "' for " + machine.name() + ""
                    );
                } else {
                    RotaryCraft.logger.logError(
                        "Could not load custom recipe '" + n + "' for " + machine.name()
                        + ""
                    );
                    if (e != null)
                        e.printStackTrace();
    ...
    addCustomRecipe(String n, LuaBlock lb, CustomRecipeList crl) throws Exception;

    protected final void verifyOutputItem(ItemStack is) {
        if (is.getItem() instanceof ItemBlockPlacer
            || is.getItem() == ItemRegistry.ETHANOL.getItemInstance())
            throw new IllegalArgumentException(
                "This item is not permitted as an output (it is a gating item)."
            );
        if (ReikaItemHelper.matchStacks(is, ItemStacks.tungstenflakes)
            || ReikaItemHelper.matchStacks(is, ItemStacks.tungsteningot))
            throw new IllegalArgumentException(
                "This item is not permitted as an output (it is a gating item)."
            );
        if (ReikaItemHelper.matchStacks(is, ItemStacks.silicondust)
            || ReikaItemHelper.matchStacks(is, ItemStacks.silicon))
            throw new IllegalArgumentException(
                "This item is not permitted as an output (it is a gating item)."
            );
        if (ReikaItemHelper.matchStacks(is, ItemStacks.redgoldingot)
            || ReikaItemHelper.matchStacks(is, ItemStacks.silumin))
            throw new IllegalArgumentException(
                "This item is not permitted as an output (it is a gating item)."
            );
        if (ReikaItemHelper.matchStacks(is, ItemStacks.bedrockdust)
            || ReikaItemHelper.matchStacks(is, ItemStacks.bedingot))
            throw new IllegalArgumentException(
                "This item is not permitted as an output (it is a gating item)."
            );
        if (ReikaItemHelper.matchStacks(is, ItemStacks.springingot)
            || ReikaItemHelper.matchStacks(is, ItemStacks.steelingot))
            throw new IllegalArgumentException(
                "This item is not permitted as an output (it is a gating item)."
            );
    }
    ...
    protected final void verifyOutputFluid(Fluid f) {
        if (f.getName().equalsIgnoreCase("rc jet fuel")
            || f.getName().equalsIgnoreCase("rc ethanol")
            || f.getName().equalsIgnoreCase("rc lubricant"))
            throw new IllegalArgumentException(
                "This fluid is not permitted as an output (it is a gating item)."
            );
    }
}
