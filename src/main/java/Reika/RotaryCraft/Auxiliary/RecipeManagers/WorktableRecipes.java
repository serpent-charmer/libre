...
public class WorktableRecipes extends RecipeHandler implements WorktableManager {
    ...
    public ShapedRecipes addRecipe(ItemStack output, RecipeLevel rl, Object... items) {
        ...
        if (items[i] == null
            || ((
                items[i] instanceof ItemStack && ((ItemStack) items[i]).getItem() == null
            ))) {
            throw new IllegalArgumentException(
                "Null item in recipe! Possible mod conflict?"
            );
        ...
        for (hashmap = new HashMap(); i < items.length; i += 2) {
            ...
            if (items[i + 1] == null || items[i + 1] == Blocks.air) {
                ...
                if (Item.getItemFromBlock((Block) items[i + 1]) == null)
                    throw new IllegalArgumentException(
                        "Tried to create a recipe with a block with no item!"
                    );
        ...
        for (int i1 = 0; i1 < j * k; ++i1) {
            ...
            if (hashmap.containsKey(c0)) {
                //ReikaJavaLibrary.spamConsole(c0+":
                //"+(hashmap.get(Character.valueOf(c0))));
    ...
    public IRecipe getInputRecipe(ItemStack is) {
        ...
        for (WorktableRecipe wr : recipes) {
            ...
            if (ReikaItemHelper.matchStacks(is, is2) && is.stackSize >= is2.stackSize) {
                if (is.stackTagCompound == null
                    || ItemStack.areItemStackTagsEqual(is, is2))
                    return ir;
            }
        }
        return null;
    ...
    public static final class WorktableRecipe implements MachineRecipe {
        ...
        public String getUniqueID() {
            return "WORKTABLE/" + recipe.getClass().getName() + "^"
                + ReikaRecipeHelper.toDeterministicString(recipe) + ">" + fullID(output)
                + "?" + (output.getItem() instanceof ItemBlockPlacer);
        }

        @Override
        public String getAllInfo() {
            return "Crafting " + fullID(output) + " from "
                + ReikaRecipeHelper.toString(recipe);
        }
        ...
        public boolean equals(Object o) {
            return o instanceof WorktableRecipe
                && ((WorktableRecipe) o).getUniqueID().equals(this.getUniqueID());
        }
    }
    ...
    private static class RecipeSorter implements Comparator<WorktableRecipe> {
        ...
        public int compare(WorktableRecipe ir, WorktableRecipe ir2) {
            return ir.recipe instanceof ShapelessRecipes
                    && ir2.recipe instanceof ShapedRecipes
                ? 1
                : (ir2.recipe instanceof ShapelessRecipes
                           && ir.recipe instanceof ShapedRecipes
                       ? -1
                       : (ir2.recipe.getRecipeSize() < ir.recipe.getRecipeSize()
                              ? -1
                              : (ir2.recipe.getRecipeSize() > ir.recipe.getRecipeSize()
                                     ? 1
                                     : 0)));
    ...
    protected boolean removeRecipe(MachineRecipe recipe) {
        return recipes.remove(recipe)
            && display.remove(((WorktableRecipe) recipe).recipe);
    }
