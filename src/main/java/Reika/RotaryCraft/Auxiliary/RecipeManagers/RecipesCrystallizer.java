...
public class RecipesCrystallizer extends RecipeHandler implements CrystallizerManager {
    ...
    private RecipesCrystallizer() {
        ...
        this.addRecipe(
            FluidRegistry.WATER, 1000, new ItemStack(Blocks.ice), RecipeLevel.CORE
        );
        this.addRecipe(
            FluidRegistry.WATER, 200, new ItemStack(Items.snowball), RecipeLevel.PROTECTED
        );
        this.addRecipe(
            FluidRegistry.LAVA, 1000, new ItemStack(Blocks.stone), RecipeLevel.PERIPHERAL
        );

        this.addRecipe(
            "rc ethanol", 1000, ItemRegistry.ETHANOL.getStackOf(), RecipeLevel.PROTECTED
        );
    }
    ...
    public static class CrystallizerRecipe implements MachineRecipe {
        ...
        public String getAllInfo() {
            return "Freezing " + input.amount + " of " + input.getLocalizedName()
                + " into " + fullID(output);
        }
        ...
        public Collection<ItemStack> getAllUsedItems() {
            return ReikaJavaLibrary.makeListFrom(
                ReikaFluidHelper.getFluidStackAsItem(input)
            );
        }
    ...
    public CrystallizerRecipe getRecipe(ItemStack result) {
        for (CrystallizerRecipe cr : recipeList.values()) {
            ...
            if (cr == null) {
                ...
                sb.append("Looking up recipe for "+result+":
            "+ReikaFluidHelper.fluidStackToString(f)+", despite being in the keyset of the
            map, returned null on get()!"); sb.append("\nMap data:
            "+recipeList.toString()); sb.append("\nReport this to Reika!"); throw new
    ...
    public void addPostLoadRecipes() {
        this.addRecipe(
            "ender", 250, new ItemStack(Items.ender_pearl), RecipeLevel.MODINTERACT
        );
        this.addRecipe(
            "redstone", 100, new ItemStack(Items.redstone), RecipeLevel.MODINTERACT
        );
        this.addRecipe(
            "glowstone", 250, new ItemStack(Items.glowstone_dust), RecipeLevel.MODINTERACT
        );
            ...
            if (li != null) {
                for (Object o : li) {
                    ...
                    if (ReikaItemHelper.matchStacks(
                            cast, TinkerToolHandler.getInstance().getIngotCast()
                        )) {
                        ...
                        if (ModList.IC2.isLoaded()
                            && fs.getFluid()
                                   ...
                                   .equals("steel.molten"
                                   )) { //prevent steel -> refined iron
                            out = getNonRefinedIronSteel(out);
                        }

                        this.addRecipe(
                            fs.getFluid(), fs.amount, out, RecipeLevel.MODINTERACT
                        );
                    }
                }
            }
    ...
    public static ItemStack getNonRefinedIronSteel(ItemStack out) {
        ...
        if (ReikaItemHelper.matchStacks(refiron, out)
            || ReikaItemHelper.matchStacks(refblock, out)) {
            RotaryCraft.logger.log(
                "Handling steel casting to refined iron, finding alternate."
            );
            String tag = null;
            ...
            if (ingots.size() > 1) {
                for (ItemStack ing : ingots) {
                    if (!ReikaItemHelper.matchStacks(refiron, ing)
                        && !ReikaItemHelper.matchStacks(refblock, ing)) {
                        out = ReikaItemHelper.getSizedItemStack(ing, out.stackSize);
                        RotaryCraft.logger.log(
                            "Converting to " + out + " (" + fullID(out) + ")"
                        );
                        break;
                    }
                }
