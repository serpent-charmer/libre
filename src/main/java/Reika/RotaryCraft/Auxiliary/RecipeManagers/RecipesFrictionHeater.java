...
public class RecipesFrictionHeater
    extends RecipeHandler implements FrictionHeaterManager {
    ...
    private RecipesFrictionHeater() {
        ...
        this.addRecipe(
            ItemStacks.silicondust, ItemStacks.silicon, 800, 200, RecipeLevel.PROTECTED
        );
    }

    private void
    addRecipe(ItemStack in, ItemStack out, int temp, int time, RecipeLevel rl) {
    ...
    public static final class FrictionRecipe implements MachineRecipe {
        ...
        public String getUniqueID() {
            return fullID(input) + ">" + fullID(output) + "@" + requiredTemperature + "#"
                + duration;
        }
        ...
        public String getAllInfo() {
            return "Smelting " + fullID(input) + " into " + fullID(output) + " @ "
                + requiredTemperature + "C over " + duration + " ticks";
        }
    ...
    public void addPostLoadRecipes() {
        if (ModList.IMMERSIVEENG.isLoaded() && ModList.THERMALEXPANSION.isLoaded()) {
            this.addRecipe(
                ReikaItemHelper.lookupItem("ImmersiveEngineering:material:13"),
                ReikaItemHelper.lookupItem("ThermalExpansion:Rockwool:12"),
                600,
                400,
    ...
    protected boolean removeRecipe(MachineRecipe recipe) {
        return recipes.removeValue((FrictionRecipe) recipe)
            && outputs.get(((FrictionRecipe) recipe).output).remove(recipe);
    }
