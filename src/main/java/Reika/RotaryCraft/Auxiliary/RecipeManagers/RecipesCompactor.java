...
public class RecipesCompactor extends RecipeHandler implements CompactorManager {
    ...
    private RecipesCompactor() {
        ...
        this.addRecipe(
            ...
            ItemRegistry.COMPACTS.getCraftedMetadataProduct(this.getNumberPerStep(), 0),
            rp,
            rt,
            RecipeLevel.CORE
        );
        this.addRecipe(
            ReikaItemHelper.charcoal,
            ItemRegistry.COMPACTS.getCraftedMetadataProduct(
                this.getNumberPerStep() * 3 / 2, 0
            ),
            rp,
            rt,
            RecipeLevel.CORE
        ...
        this.addRecipe(
            ItemStacks.anthracite,
            ItemRegistry.COMPACTS.getCraftedMetadataProduct(this.getNumberPerStep(), 1),
            rp,
            rt,
            RecipeLevel.CORE
        );
        this.addRecipe(
            ItemStacks.prismane,
            ItemRegistry.COMPACTS.getCraftedMetadataProduct(this.getNumberPerStep(), 2),
            rp,
            rt,
            RecipeLevel.CORE
        );
        this.addRecipe(
            ItemStacks.lonsda,
            new ItemStack(Items.diamond, this.getNumberPerStep(), 0),
            rp,
            rt,
            RecipeLevel.CORE
        );
        ...
        this.addRecipe(
            ...
            new ItemStack(Blocks.packed_ice, 2, 0),
            press,
            temp,
            RecipeLevel.PERIPHERAL
        );
        if (ModList.GEOSTRATA.isLoaded()) {
            this.addRecipe(
                ...
                new ItemStack(Blocks.packed_ice, 2, 1),
                press * 6 / 5,
                temp,
                RecipeLevel.PERIPHERAL
            );
            this.addRecipe(
                new ItemStack(Blocks.packed_ice, 1, 1),
                new ItemStack(Blocks.packed_ice, 2, 2),
                press * 3 / 2,
                temp,
                RecipeLevel.PERIPHERAL
            );
    ...
    private static class CompactingRecipe implements MachineRecipe {
        ...
        public String getAllInfo() {
            return "Compacting " + fullID(in) + " to " + fullID(out) + " @ " + temperature
                + "C & " + pressure + " kPa";
        }
    ...
    public void
    addAPIRecipe(ItemStack in, ItemStack itemstack, int pressure, int temperature) {
        this.addRecipe(in, itemstack, pressure, temperature, RecipeLevel.API);
    }

    public void
    addRecipe(ItemStack in, ItemStack itemstack, int pressure, int temperature) {
        this.addRecipe(in, itemstack, pressure, temperature, RecipeLevel.CORE);
    }

    private void addRecipe(
        ItemStack in, ItemStack itemstack, int pressure, int temperature, RecipeLevel rl
    ) {
    ...
    public ItemStack getCompactingResult(ItemStack item) {
        ...
        //Items.itemID, item.getItemDamage()));
    ...
    public void addPostLoadRecipes() {
        if (ModList.IC2.isLoaded()) {
            ...
            if (plantball != null) {
                Object[] items = new Object[] { Items.wheat,         Items.carrot,
                                                Blocks.leaves,       Blocks.leaves2,
                                                Blocks.tallgrass,    Blocks.red_flower,
                                                Blocks.yellow_flower };
                for (Object in : items) {
                    this.addRecipe(
                        ReikaItemHelper.parseItem(in, true),
                        ReikaItemHelper.getSizedItemStack(plantball, 1),
                        500,
                        0,
                        RecipeLevel.MODINTERACT
                    ); //4:1 is 2x better than handcraft
                }
            }
        }
    }
    ...
    protected boolean addCustomRecipe(String n, LuaBlock lb, CustomRecipeList crl)
        ...
        this.addRecipe(
            in, out, lb.getInt("pressure"), lb.getInt("temperature"), RecipeLevel.CUSTOM
        );
        return true;
    }
}
