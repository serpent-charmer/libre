...
public class RecipesCentrifuge extends RecipeHandler implements CentrifugeManager {
    ...
    private RecipesCentrifuge() {
        ...
        this.addRecipe(
            ...
            null,
            RecipeLevel.PERIPHERAL,
            new ItemStack(Items.pumpkin_seeds, 12),
            100
        );
        this.addRecipe(
            Items.wheat,
        ...
        this.addRecipe(
            ...
            80,
            ExtractorModOres.getSmeltedIngot(ModOreList.SULFUR),
            40
        );
        this.addRecipe(
            Blocks.dirt,
        ...
        this.addRecipe(
            ...
            RecipeLevel.CORE,
            2,
            new Object[] { ItemStacks.cleansludge,
                           80,
                           ItemStacks.cleansludge,
                           20,
                           ItemStacks.compost,
                           25 }
        );
        ...
        this.addRecipe(
            ItemStacks.slipperyComb,
            new FluidStack(FluidRegistry.getFluid("rc lubricant"), 50),
            60,
            RecipeLevel.PROTECTED,
            ItemStacks.slipperyPropolis,
            80
        );
        this.addRecipe(
            ItemStacks.slipperyPropolis,
            new FluidStack(FluidRegistry.getFluid("rc lubricant"), 150),
            100,
            RecipeLevel.PROTECTED
        );

        int amt = ReikaMathLibrary.roundUpToX(
            10, (int) (DifficultyEffects.CANOLA.getAverageAmount() * 0.75F)
        );
        this.addRecipe(
            ItemStacks.canolaHusks,
            new FluidStack(FluidRegistry.getFluid("rc lubricant"), amt),
            100,
            RecipeLevel.CORE
        );
    }
    ...
    public static class CentrifugeRecipe implements MachineRecipe {
        ...
        private CentrifugeRecipe(
            ItemStack is, ChancedOutputList li, FluidOut fs, int cycles
        ) {
            in = is;
            out = li;
            fluid = fs;
        ...
        public String getUniqueID() {
            return fullID(in) + ">" + out.toString() + "&"
                + (fluid != null ? fluid.toString() : "X") + " {x" + maxStack + "}";
        }

        @Override
        public String getAllInfo() {
            return "Centrifuge " + fullID(in) + " to items[" + out + "] and fluid "
                + fluid + "; can stack up to " + maxStack + "x";
        }
        ...
        public FluidStack rollFluid(ChanceManipulator c) {
            return fluid != null
                ? ReikaRandomHelper.doWithChance(
                      c != null ? c.getChance(fluid.chance) : fluid.chance
                  )
                    ? fluid.fluid.copy()
                    : null
                : null;
    ...
    public void
    addRecipe(ItemStack in, ChancedOutputList out, FluidOut fs, RecipeLevel rl) {
        this.addRecipe(in, out, fs, rl, 1);
    }

    public void addRecipe(
        ItemStack in, ChancedOutputList out, FluidOut fs, RecipeLevel rl, int stack
    ) {
        in = in.copy();
    ...
    private void addDirectStackRecipe(
        ItemStack in, ChancedOutputList out, FluidOut fs, RecipeLevel rl, int stack
    ) {
        if (out.size() > 9)
            throw new IllegalArgumentException(
                "Too many output items for " + in.getDisplayName()
                + " centrifuge recipe; only 9 inventory slots!"
            );
        out.lock();
    ...
    private void
    addRecipe(ItemStack in, FluidOut fs, RecipeLevel rl, int stack, Object... items) {
    ...
    public void addAPIRecipe(ItemStack item, FluidStack fs, float fc, Object... items) {
        this.addRecipe(
            item, fs != null ? new FluidOut(fs, fc) : null, RecipeLevel.API, items
        );
    }

    private void
    addRecipe(ItemStack item, FluidStack fs, float fc, RecipeLevel rl, Object... items) {
    ...
    public void addPostLoadRecipes() {
        if (ModList.FORESTRY.isLoaded()) {
            for (ItemStack in : ForestryRecipeHelper.getInstance().getSqueezerRecipes()) {
                ISqueezerRecipe out
                    = ForestryRecipeHelper.getInstance().getSqueezerOutput(in);
                ...
                this.addRecipe(
                    in,
                    co,
                    fs != null ? new FluidOut(fs, 100) : null,
                    RecipeLevel.MODINTERACT
                );
            }

            for (ItemStack in :
                 ForestryRecipeHelper.getInstance().getCentrifugeRecipes()) {
                ChancedOutputList out
                    = ForestryRecipeHelper.getInstance().getCentrifugeOutput(in);
            ...
            this.addRecipe(
                ...
                out,
                new FluidOut(
                    new FluidStack(FluidRegistry.getFluid("for.honey"), 150), 100
                ),
                RecipeLevel.MODINTERACT
            );
            ...
            this.addRecipe(
                drop,
                new FluidStack(FluidRegistry.getFluid("for.honey"), 150),
                100,
                RecipeLevel.MODINTERACT
            );
            ...
            if (drop != null && ReikaXPFluidHelper.fluidsExist()) {
                ...
                this.addRecipe(
                    drop, fs, 100, RecipeLevel.MODINTERACT, ItemStacks.lapisoreflakes, 2
                );
            }
            ...
            if (drop != null) {
                ...
                if (f != null) {
                    this.addRecipe(
                        drop, new FluidStack(f, 50), 100, RecipeLevel.MODINTERACT
                    );
                }
            }
            ...
            if (drop != null) {
                ...
                if (f != null) {
                    this.addRecipe(drop, new FluidStack(f, 20), 100,
            RecipeLevel.MODINTERACT);
                }
            }
             */
        ...
        if (ReikaItemHelper.oreItemsExist("dustLead", "dustSilver")) {
            ...
            this.addRecipe(
                ExtractorModOres.getSmeltedIngot(ModOreList.GALENA),
                null,
                RecipeLevel.PERIPHERAL,
                lead,
                100,
        ...
        if (ModList.TINKERER.isLoaded()) {
            ...
            ItemStack chaff = ModList.IC2.isLoaded()
                    && IC2Handler.IC2Stacks.BIOCHAFF.getItem() != null
                ? IC2Handler.IC2Stacks.BIOCHAFF.getItem()
                : ReikaItemHelper.tallgrass.asItemStack();
            ItemStack blueslime = ReikaItemHelper.lookupItem("TConstruct:strangeFood");
            this.addRecipe(
                ModWoodList.SLIME.getSaplingID(),
                null,
                RecipeLevel.MODINTERACT,
        ...
        if (ModList.MAGICCROPS.isLoaded()) {
            //ItemStack drop = LegacyMagicCropHandler.getInstance().dropID != null ? new
            //ItemStack(LegacyMagicCropHandler.getInstance().dropID) : null; if (drop ==
            //null)
        ...
        if (Loader.isModLoaded("Automagy")) {
            ...
            if (phial != null && ReikaXPFluidHelper.fluidsExist()) {
                try {
                    ...
                    this.addRecipe(
                        phial, co, new FluidOut(fs, 100), RecipeLevel.MODINTERACT
                    );
                } catch (Exception e) {
                    RotaryCraft.logger.logError(
                        "Could not determine XP value of enchanting phial!"
                    );
                    ReflectiveFailureTracker.instance.logModReflectiveFailure(
                        new BasicModEntry("Automagy"), e
                    );
                }
            }
        }
        ...
        ItemStack is
            = ModList.IC2.isLoaded() && IC2Handler.IC2Stacks.BIOCHAFF.getItem() != null
            ? IC2Handler.IC2Stacks.BIOCHAFF.getItem()
            : ReikaItemHelper.tallgrass.asItemStack();
        this.addRecipe(
            new ItemStack(Blocks.clay),
            ...
            100,
            ItemStacks.silicondust,
            75,
            ItemStacks.ironoreflakes,
            0.5F,
    ...
    private void addGrassToSeeds() {
        ...
        float total = 0;
        List<WeightedRandom.Item> li = (List<net.minecraft.util.WeightedRandom.Item>)
                                           ReikaObfuscationHelper.get("seedList", null);
        for (WeightedRandom.Item wi : li) {
            ItemStack seed = (ItemStack) ReikaObfuscationHelper.get("seed", wi);
            if (HarvestCraftHandler.getInstance().isSeedItem(seed
                )) //pollutes with 113435948745609 seeds
                continue;
            if (weights.size()
                >= (weights.containsKey(ItemRegistry.CANOLA.getItemInstance(), 0) ? 9 : 8
                ))
                continue;
            total += wi.itemWeight;
        ...
        this.addRecipe(
            ReikaItemHelper.tallgrass.asItemStack(), c, null, RecipeLevel.PERIPHERAL
        );
        this.addRecipe(
            ReikaItemHelper.fern.asItemStack(), c, null, RecipeLevel.PERIPHERAL
        );
    }
    ...
    protected boolean addCustomRecipe(String n, LuaBlock lb, CustomRecipeList crl)
        ...
        if (lb.hasChild("output_items")) {
            ...
            for (LuaBlock entry : items.getChildren()) {
                ItemStack is = crl.parseItemString(
                    entry.getString("item"), entry.getChild("nbt"), true
                );
