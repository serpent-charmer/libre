...
public interface PipeConnector {
    ...
    /**
     * Side is relative to the piping block (so DOWN means the block is below the pipe);
     * p is the pipe type
     */
    ...
    /**
     * This is called by fluid removal code. The pipes will attempt to drain from
     * direction 'side'. Return the fluid that was successfully removed. Note that 'amt'
     * may be much larger than the machine's liquid level. It is up to the machine to run
     * all the calculations and fluid distribution. This is called every tick, so be
     * efficient.
     */
    //public FluidStack removeFluid(int amt, ForgeDirection side);

    /**
     * This is called by fluid addition. Return a FluidTankInfo which accurately
     * encapsulates the amount your TE has of fluid 'f' and the maximum amount it can
     * hold.
     *//*
