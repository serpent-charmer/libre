    ...
    public void onUpdate() {
        ...
        if (fadeColor != -1) {
            int c = ReikaColorAPI.mixColors(
                fadeColor, preColor, particleAge / (float) particleMaxAge
            );
            this.setColor(c);
        }
        ...
        if (rapidExpand)
            particleScale = scale
                * (particleMaxAge / particleAge >= 12
                       ? particleAge * 12F / particleMaxAge
                       : 1 - particleAge / (float) particleMaxAge);
        else
            particleScale = scale
                * (float) Math.sin(Math.toRadians(180D * particleAge / particleMaxAge));
        //if (particleAge < 10)
