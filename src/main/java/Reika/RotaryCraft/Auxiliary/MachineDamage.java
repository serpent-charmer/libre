...
public class MachineDamage extends CustomStringDamageSource {
    ...
    public MachineDamage
    setArmorBlocking(float f, int mult, int... types) { //1 is feet, 4 is helmet
        armorFactor = f;
        armorDamage = mult;
        for (int type : types) {
            armorTypes.add(type);
    ...
    public float onDamageDealt(float base, EntityLivingBase e) {
        for (int slot : armorTypes) {
            ...
            if (armor != null && armor.getItem() instanceof ItemArmor) {
                /*
                float prot =
                ((ItemArmor)armor.getItem()).getArmorMaterial().getDamageReductionAmount(4-slot);
                if (armor.getItem() instanceof ISpecialArmor) {
                    ((ISpecialArmor)armor.getItem()).getProperties(e, armor, this, base,
                slot);
                }*/
