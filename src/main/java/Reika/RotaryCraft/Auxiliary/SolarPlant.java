...
public class SolarPlant {
    ...
    public static SolarPlant build(World world, int x, int y, int z) {
        ...
        HashMap<Coordinate, ImmutablePair<Integer, Integer>> towerLocations
            = new HashMap();
        ...
        while (blocks.getSize() > 0) {
            ...
            b.setPlant(p);
            MachineRegistry m
                = MachineRegistry.getMachine(world, c.xCoord, c.yCoord, c.zCoord);
        ...
        for (Coordinate c : towerLocations.keySet()) {
            ...
            int dy = ys.left;
            int h = 0;
            while (MachineRegistry.getMachine(world, c.xCoord, dy, c.zCoord)
                       == MachineRegistry.SOLARTOWER
                   && dy <= ys.left + MAX_TOWER_HEIGHT) {
                dy++;

                if (ReikaWorldHelper.checkForAdjBlock(
                        world,
                        c.xCoord,
                        dy,
                        c.zCoord,
                        MachineRegistry.MIRROR.getBlock(),
                        MachineRegistry.MIRROR.getBlockMetadata()
                    )
                    != null) {
                    h = 0;
                }
    ...
    public float getLightLevel(World world) {
        ...
        double sun = (ReikaWorldHelper.getSunIntensity(world, true, 0) * 0.8F + 0.2F)
            * PlanetDimensionHandler.getSunIntensity(world);
        if (sun > 0.21) {
            return (int) (15 * sun);
        }
    ...
    public void invalidate(World world) {
        ...
        for (SolarTower s : towers.values()) {
            for (int y = s.bottomBlock; y <= s.topBlock; y++) {
                SolarPlantBlock b
                    = (SolarPlantBlock) s.location.offset(0, y, 0).getTileEntity(world);
                if (b != null)
                    b.setPlant(null);
            }
        }
    ...
    private class SolarTower implements Comparable<SolarTower> {
        ...
        public int compareTo(SolarTower o) {
            int ret = -Integer.compare(
                effectiveHeight, o.effectiveHeight
            ); //to put bigger at the beginning
    ...
    public boolean canSeeTheSky(TileEntityMirror te) {
        ...
        return te.worldObj.canBlockSeeTheSky(te.xCoord, te.yCoord + 1, te.zCoord)
            && this.isHighestMirror(te);
    }
}
