...
public class RotaryASMHandler implements IFMLLoadingPlugin {
    ...
    public String[] getASMTransformerClass() {
        return new String[] {
            ASMExecutor.class.getName(), InterfaceVerifier.class.getName(),
    ...
    public static class ModelRedirector implements IClassTransformer {
        ...
        public byte[] transform(String name, String transformedName, byte[] data) {
            ...
            if (cn.name.startsWith("Reika/RotaryCraft/Models")
                && !cn.name.startsWith("Reika/RotaryCraft/Models/Animated")) {
                MethodNode m = ReikaASMHelper.getMethodByName(
                    cn,
                    "renderAll",
                    "(Lnet/minecraft/tileentity/TileEntity;Ljava/util/ArrayList;FF)V"
                );
                for (int i = 0; i < m.instructions.size(); i++) {
                    AbstractInsnNode ain = m.instructions.get(i);
                    if (ain.getOpcode() == Opcodes.INVOKESTATIC
                        && ((MethodInsnNode) ain)
                               .name.startsWith("gl")) { //animation code
                    }
                }
            }
            ...
            ClassWriter writer
                = new ClassWriter(ClassWriter
                                      .COMPUTE_MAXS /* | ClassWriter.COMPUTE_FRAMES*/);
            cn.accept(writer);
    ...
    public static class InterfaceVerifier implements IClassTransformer {
        ...
        public byte[] transform(String name, String transformedName, byte[] data) {
            ...
            ClassReader classReader = new ClassReader(data);
            classReader.accept(cn, 0);
            if ((cn.name.contains(par1) || cn.name.contains(par2)
                 || cn.name.contains(par3))
                && !set.contains(cn.name)) {
                ...
                switch (this) {
                    case FURNACEINTERCEPT: //TileEntityFurnaceHeater.isHijacked(te)
                        ...
                        pre.add(new MethodInsnNode(
                            ...
                            "Reika/RotaryCraft/TileEntities/Auxiliary/TileEntityFurnaceHeater",
                            "isHijacked",
                            "(Lnet/minecraft/tileentity/TileEntityFurnace;)Z",
                            false
                        ));
                        ...
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "func_145845_h", "updateEntity", "()V"
                        );
