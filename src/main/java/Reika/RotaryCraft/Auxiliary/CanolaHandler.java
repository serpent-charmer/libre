...
public final class CanolaHandler implements CustomCropHandler {
    ...
    public boolean isCrop(Block id, int meta) {
        return ModList.ROTARYCRAFT.isLoaded()
            && id == BlockRegistry.CANOLA.getBlockInstance();
    }
    ...
    public boolean isRipeCrop(World world, int x, int y, int z) {
        ...
        return ModList.ROTARYCRAFT.isLoaded()
            && this.isCrop(world.getBlock(x, y, z), meta) && meta == BlockCanola.GROWN;
    }
    ...
    public boolean isSeedItem(ItemStack is) {
        return ModList.ROTARYCRAFT.isLoaded() && ItemRegistry.CANOLA.matchItem(is)
            && is.getItemDamage() == 0;
    }
    ...
    public boolean initializedProperly() {
        return ModList.ROTARYCRAFT.isLoaded()
            && BlockRegistry.CANOLA.getBlockInstance() != null;
    }
