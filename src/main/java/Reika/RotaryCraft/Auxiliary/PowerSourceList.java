...
public class PowerSourceList implements PowerTracker {
    ...
    public static PowerSourceList getAllFrom(
        ...
        if (tile instanceof ShaftMerger) {
            ...
            if (tile instanceof TileEntitySplitter
                && ((TileEntitySplitter) tile).isBedrock()) {
                pwr.isBedrock = true;
            } else {
        ...
        try {
            if (tile instanceof TileEntityIOMachine) {
                ...
                if (te.isReadingFrom(io) || te.isReadingFrom2(io) || te.isReadingFrom3(io)
            ...
            RotaryCraft.logger.logError(
                "PowerSourceList SOE @ " + pwr + ", called from " + caller + "!"
            );
            pwr.errored = true;
            //fail(pwr, caller, world, x, y, z);
            return pwr;
    ...
    private static void
    fail(PowerSourceList pwr, ShaftMerger caller, World world, int x, int y, int z) {
        if (caller != null) {
            caller.fail();
        } else {
            try {
    ...
    public static PowerSourceList
    combine(PowerSourceList in1, PowerSourceList in2, ShaftMerger caller) {
    ...
    private static class PowerWrapper {
        ...
        public boolean equals(Object o) {
            return o instanceof PowerWrapper
                && new WorldLocation((TileEntity) ((PowerWrapper) o).generator)
                       .equals(new WorldLocation((TileEntity) generator));
        }
    }
}
