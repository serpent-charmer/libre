...
public abstract class IORenderer {
    ...
    public static void renderIO(TileEntity teb, double p2, double p4, double p6) {
        ...
        boolean flag = ep.getEntityData().hasKey(ItemIOGoggles.NBT_KEY)
            && ep.getEntityData().getLong(ItemIOGoggles.NBT_KEY)
                >= ep.worldObj.getTotalWorldTime() - 10;
        if (teb instanceof TileEntityIOMachine) {
            ...
            if (teb instanceof TileEntityShaft
                && ((TileEntityShaft) teb).isCross()) { //cross
            ...
            if (te.isOmniSided) {
                if (te.getTile().getMaxY(te) == 1)
                    renderIn(
                        par2 + te.getPointingOffsetX(),
                        par4 + te.getPointingOffsetY() + 1,
                        par6 + te.getPointingOffsetZ(),
                        te.iotick
                    );
                ...
                if (te.getTile().getMaxX(te) == 1)
                    renderIn(
                        par2 + te.getPointingOffsetX() + 1,
                        par4 + te.getPointingOffsetY(),
                        par6 + te.getPointingOffsetZ(),
                        te.iotick
                    );
            ...
            if (teb instanceof ShaftPowerReceiver) {
                ...
                for (int i = 0; i < 6; i++) {
                    ...
                    if (sr.canReadFrom(dir)) {
                        renderIn(
                            par2 + dir.offsetX, par4 + dir.offsetY, par6 + dir.offsetZ, io
                        );
                    }
                }
            }
    ...
    private static void renderBox(double x, double y, double z, int[] color) {
        ...
        var5.addVertex(
            x + 1 + 0.0625 * expand, y + 1 + 0.0625 * expand, z - 0.0625 * expand
        );
        var5.addVertex(x - 0.0625 * expand, y + 1 + 0.0625 * expand, z - 0.0625 * expand);
        var5.addVertex(
            x - 0.0625 * expand, y + 1 + 0.0625 * expand, z + 1 + 0.0625 * expand
        );
        var5.addVertex(
            x + 1 + 0.0625 * expand, y + 1 + 0.0625 * expand, z + 1 + 0.0625 * expand
        );
        var5.draw();
        var5.startDrawing(GL11.GL_LINE_LOOP);
        var5.setColorRGBA(color[0], color[1], color[2], color[3]);
        ...
        var5.addVertex(
            x + 1 + 0.0625 * expand, y - 0.0625 * expand, z + 1 + 0.0625 * expand
        );
        var5.draw();
        var5.startDrawing(GL11.GL_LINE_LOOP);
        var5.setColorRGBA(color[0], color[1], color[2], color[3]);
        ...
        if (filled) {
            ...
            var5.addVertex(x - 0.0625 * expand, y - 0.0625 * expand, z - 0.0625 * expand);
            var5.addVertex(
                x + 1 + 0.0625 * expand, y - 0.0625 * expand, z - 0.0625 * expand
            );
            var5.addVertex(
                x + 1 + 0.0625 * expand, y - 0.0625 * expand, z + 1 + 0.0625 * expand
            );
            var5.addVertex(
                x - 0.0625 * expand, y - 0.0625 * expand, z + 1 + 0.0625 * expand
            );
            ...
            var5.addVertex(
                x - 0.0625 * expand, y + 1 + 0.0625 * expand, z - 0.0625 * expand
            );
            var5.addVertex(x - 0.0625 * expand, y - 0.0625 * expand, z - 0.0625 * expand);
            var5.addVertex(
                x - 0.0625 * expand, y - 0.0625 * expand, z + 1 + 0.0625 * expand
