...
public class EntityFlameTurretShot extends EntityTurretShot {
    ...
    public void onUpdate() {
        ...
        hit = ticksExisted > 5 && m != MachineRegistry.FLAMETURRET && id != Blocks.air
        ...
        if (!worldObj.isRemote
            && (ticksExisted > 240 || shootingEntity != null && shootingEntity.isDead
            ...
            if (var3 != null && var3.typeOfHit == MovingObjectType.BLOCK
                && MachineRegistry.getMachine(
                       world, var3.blockX, var3.blockY, var3.blockZ
                   ) != MachineRegistry.FLAMETURRET)
                this.onImpact(var3);
            posX += motionX;
            posY += motionY;
            posZ += motionZ;
            if (this.isInWater()) {
                if (rand.nextInt(6) == 0)
                    ReikaSoundHelper.playSoundAtEntity(
                        world, this, "random.fizz", 0.4F, rand.nextFloat() * 0.5F + 0.5F
                    );
                this.setDead();
            }
    ...
    protected void applyAttackEffectsToEntity(World world, Entity ent) {
        ent.attackEntityFrom(
            this.getDamageSource().setFireDamage(), this.getAttackDamage()
        );
