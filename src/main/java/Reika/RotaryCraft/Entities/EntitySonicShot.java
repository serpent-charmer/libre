...
public class EntitySonicShot extends EntityFireball {
    ...
    public EntitySonicShot(World world, TileEntitySonicBorer tile, String player) {
        ...
        te = tile;
        this.setPosition(
            tile.xCoord + 0.5 + tile.xstep,
            tile.yCoord + 0.5 + tile.ystep,
            tile.zCoord + 0.5 + tile.zstep
        );
    ...
    protected void onImpact(MovingObjectPosition mov) {
        ...
        //ReikaJavaLibrary.pConsole(mov != null ?
        //(mov.blockX+","+mov.blockY+","+mov.blockZ) : "null");
        if (mov != null) {
            int x = mov.blockX;
            int y = mov.blockY;
            int z = mov.blockZ;
    ...
    public void onUpdate() {
        ...
        if (te != null) {
            ...
            int d = te.xstep * (x - te.xCoord) + te.ystep * (y - te.yCoord)
                + te.zstep * (z - te.zCoord);
            int r = this.getRange();
            if (d >= r) {
                Vec3 vec = Vec3.createVectorHelper(0, 0, 0);
                int[] tg = te.getTargetPosn();
                MovingObjectPosition mov
                    = new MovingObjectPosition(tg[0], tg[1], tg[2], -1, vec);
                this.onImpact(mov);
            }
        }
    ...
    public final AxisAlignedBB getBoundingBox() {
        return AxisAlignedBB.getBoundingBox(
            posX + 0.4, posY + 0.4, posZ + 0.4, posX + 0.6, posY + 0.6, posZ + 0.6
        );
    }
    ...
    private void dropBlockAt(World world, int x, int y, int z) {
        ...
        if (!world.isRemote
            && !ReikaPlayerAPI.playerCanBreakAt(
                (WorldServer) world, x, y, z, b, meta, te.getServerPlacer()
            ))
            return;
