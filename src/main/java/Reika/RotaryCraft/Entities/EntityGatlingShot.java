...
public class EntityGatlingShot extends EntityTurretShot {
    ...
    public void onUpdate() {
        ...
        for (int i = -r; i <= r; i++) {
            for (int j = -r; j <= r; j++) {
                for (int k = -r; k <= r; k++) {
                    ...
                    //with "+power+" on "+FMLCommonHandler.instance().getEffectiveSide());
                    if (ConfigRegistry.ATTACKBLOCKS.getState()) {
                        if (id2.getMaterial() == Material.glass
                            || id2.getMaterial() == Material.ice) {
                            ReikaSoundHelper.playBreakSound(
                                world, x + i, y + j, z + k, Blocks.glass
                            );
        ...
        hit
            = (mobs.size() > 0
               || (m != MachineRegistry.GATLING && id != Blocks.air
    ...
    private void spawnTracerParticles() {
        ...
        for (double d = -l; d <= l; d += 0.0625) {
            ...
            EntityFX fx = new EntityFluidFX(worldObj, px, py, pz, FluidRegistry.LAVA)
                              .setLife(2) /*.setColor(0xffd0a0)*/.setScale(s);
    ...
    public void onImpact(MovingObjectPosition mov) {
        ...
        for (int i = -r; i <= r; i++) {
            for (int j = -r; j <= r; j++) {
                for (int k = -r; k <= r; k++) {
                    ...
                    //with "+power+" on "+FMLCommonHandler.instance().getEffectiveSide());
                    if (ConfigRegistry.ATTACKBLOCKS.getState()) {
                        if (id.getMaterial() == Material.glass
                            || id.getMaterial() == Material.ice) {
                            ReikaSoundHelper.playBreakSound(
                                world, x0 + i, y0 + j, z0 + k, Blocks.glass
                            );
                            world.setBlock(x0 + i, y0 + j, z0 + k, Blocks.air);
                        }
                    }

                    if (world.isRemote && id.getMaterial() != Material.air
                        && MachineRegistry.getMachine(worldObj, x0 + i, y0 + j, z0 + k)
                            != MachineRegistry.GATLING)
        ...
        AxisAlignedBB splash
            = AxisAlignedBB.getBoundingBox(x, y, z, x, y, z).expand(d, d, d);
    ...
    protected void applyAttackEffectsToEntity(World world, Entity ent) {
        if (ent instanceof EntitySlime) {
            ...
            ReikaEntityHelper.knockbackEntityFromPos(
                gun.xCoord + 0.5, gun.yCoord + 0.5, gun.zCoord + 0.5, ent, 1
            );
            ...
            ReikaEntityHelper.knockbackEntityFromPos(
                gun.xCoord + 0.5, gun.yCoord + 0.5, gun.zCoord + 0.5, ent, 0.0625
            );
            ent.hurtResistantTime = 0;
        }
    }
