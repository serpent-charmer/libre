...
public class FrictionTweaker {
    private static final RecipesFrictionHeater recipes
        = RecipesFrictionHeater.getRecipes();

    @ZenMethod
    public static void
    addRecipe(IIngredient input, IIngredient output, int temp, int time) {
    ...
    private static class AddRecipe implements IUndoableAction {
        ...
        public String describe() {
            return "Adding " + inputs.size() + " recipe" + (inputs.size() > 1 ? "s" : "")
                + " to Friction Heater for " + output.getDisplayName() + " @ " + temp
                + "C";
        }

        @Override
        public String describeUndo() {
            return "Removing " + inputs.size() + " recipe"
                + (inputs.size() > 1 ? "s" : "") + " to Friction Heater for "
                + output.getDisplayName() + " @ " + temp + "C";
        }
