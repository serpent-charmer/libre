...
public class GrinderTweaker {
    ...
    private static class AddRecipe implements IUndoableAction {
        ...
        public String describe() {
            return "Adding " + inputs.size() + " recipe" + (inputs.size() > 1 ? "s" : "")
                + " to Grinder for " + output.getDisplayName();
        }

        @Override
        public String describeUndo() {
            return "Removing " + inputs.size() + " recipe"
                + (inputs.size() > 1 ? "s" : "") + " to Grinder for "
    ...
    private static class AddSeed implements IUndoableAction {
        ...
        @Override
        public String describe() {
            return "Adding " + seeds.size() + " seed" + (seeds.size() > 1 ? "s" : "")
                + " to Grinder";
        }

        @Override
        public String describeUndo() {
            return "Removing " + seeds.size() + " seed" + (seeds.size() > 1 ? "s" : "")
                + " to Grinder";
        }
