...
public class ItemScrewdriverFocus extends ItemCustomFocus {
    ...
    public boolean onLeftClick(
        ...
        if (!MinecraftForge.EVENT_BUS.post(new PlayerInteractEvent(
                ep, Action.LEFT_CLICK_BLOCK, x, y, z, side.ordinal(), world
            )))
    ...
    public ItemStack onFocusRightClick(
        ItemStack wand, World world, EntityPlayer player, MovingObjectPosition mov
    ) {
        if (mov == null)
            return null;
        ...
        if (ItemRegistry.SCREWDRIVER.getItemInstance().onItemUse(
                is, player, world, x, y, z, mov.sideHit, a, b, c
            ))
            flag = nativeTile;
        if ((nativeTile || this.canMultiTool(wand))
            && this.fakeScrewclick(world, x, y, z, player, mov, a, b, c, is))
            flag = true;
        if (this.canMultiTool(wand)
            && AppEngHandler.getInstance().tryRightClick(
                is, x, y, z, mov.sideHit, player, world, 0
            ))
            flag = true;
        if (flag) {
            this.playSound(world, x, y, z);
    ...
    private void playSound(World world, int x, int y, int z) {
        ReikaSoundHelper.playSoundFromServer(
            ...
            "thaumcraft:wand",
            0.4F,
            0.7F + world.rand.nextFloat() * 0.6F,
            true
        );
        ReikaSoundHelper.playSoundFromServer(
            world, x + 0.5, y + 0.5, z + 0.5, "mob.blaze.hit", 0.5F, 0.75F, true
        );
    }

    private boolean fakeScrewclick(
        World world,
        int x,
        int y,
        int z,
        ...
        boolean ret = world.getBlock(x, y, z).onBlockActivated(
            world, x, y, z, player, mov.sideHit, a, b, c
        );
        //MinecraftForge.EVENT_BUS.post(new PlayerInteractEvent(player,
        //Action.RIGHT_CLICK_BLOCK, x, y, z, mov.sideHit, world));
