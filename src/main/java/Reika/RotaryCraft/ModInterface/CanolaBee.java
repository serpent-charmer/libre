...
public class CanolaBee extends BeeSpecies {
    ...
    public CanolaBee() { //cultivated + meadows
        super(
            ...
            new BeeBranch(
                ...
                "Mechanica",
                "Mechanica",
                "These bees seem to be made to aid mechanical devices."
            )
        );
    ...
    private final class FlowerProviderCanola extends BasicFlowerProvider {
        ...
        /*
        @Override
        public boolean isAcceptedFlower(World world, IIndividual individual, int x, int y,
        int z) { return super.isAcceptedFlower(world, individual, x, y, z) &&
        BlockCanola.canGrowAt(world, x, y, z);
        }
         */
        @Override
        public boolean growFlower(
            ...
            if (ModularLogger.instance.isEnabled(LOGGER_ID))
                ModularLogger.instance.log(
                    LOGGER_ID,
                    "Canola bee @ " + x + ", " + y + ", " + z + " running growFlower"
                );
            int r = 24;
            int n = 4;
            boolean flag = false;
            for (int i = 0; i < n; i++) {
                ...
                if (dy > 0) {
                    ...
                    if (b == BlockRegistry.CANOLA.getBlockInstance()) {
                        if (meta < 9) {
                            //world.scheduleBlockUpdate(dx, dy, dz, b,
                            //20+rand.nextInt(20)); //was 20+rand(300)
                            BlockTickEvent.fire(
                                b,
                                world,
                                dx,
        ...
        @Override
        public boolean
        isAcceptableFlower(String flowerType, World world, int x, int y, int z) {
            ...
            return AgriCraftHandler.getInstance().isCrop(b, meta)
                && AgriCraftHandler.getInstance().getCropObject(world, x, y, z)
                == AgriCanola.instance;
        }
    }
    ...
    public boolean isJubilant(IBeeGenome ibg, IBeeHousing ibh) {
        ...
        int Tamb
            = ReikaWorldHelper.getAmbientTemperatureAt(world, c.posX, c.posY, c.posZ);
