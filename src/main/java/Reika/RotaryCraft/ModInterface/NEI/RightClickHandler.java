...
public class RightClickHandler extends TemplateRecipeHandler {
    ...
    public class RightClickRecipe extends CachedRecipe {
        ...
        public RightClickRecipe(
            MachineRegistry machine, ItemStack machineitem, ItemStack is, String desc
        ) {
    ...
    public void loadCraftingRecipes(ItemStack result) {
        for (int i = 0; i < Actions.values().length; i++) {
            ...
            if (!a.entry.isDummiedOut()
                && ReikaItemHelper.matchStacks(result, a.getMachine())) {
                arecipes.add(
                    new RightClickRecipe(a.entry, a.getMachine(), a.getItem(), a.desc)
                );
            }
        }
    }
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        for (int i = 0; i < Actions.values().length; i++) {
            Actions a = Actions.values()[i];
            if (!a.entry.isDummiedOut()
                && ReikaItemHelper.matchStacks(ingredient, a.getItem())) {
                arecipes.add(
                    new RightClickRecipe(a.entry, a.getMachine(), a.getItem(), a.desc)
                );
            }
    ...
    private void drawText(RightClickRecipe rec) {
        ...
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            Minecraft.getMinecraft().fontRenderer, rec.description, 120, 160, 0
        );
    }
    ...
    private static enum Actions {
        ...
        REPAIRSHAFT(
            ...
            MaterialRegistry.STEEL.getShaftItem(),
            ItemStacks.shaftitem,
            "Repairs Shaft"
        ),
        REPAIRGEAR(
            MachineRegistry.GEARBOX,
            GearboxTypes.STEEL.getGearboxItem(4),
            ItemStacks.gearunit,
            "Repairs Gearbox"
        ),
        REPAIRJET(
            MachineRegistry.ENGINE,
            EngineType.JET.getCraftedProduct(),
        ...
        MAGNETO1(
            MachineRegistry.MAGNETIC,
            GameRegistry
                .findItemStack(ModList.THERMALEXPANSION.modLabel, "powerCoilGold", 1),
            "Upgrades To Tier 1"
        ),
        MAGNETO2(
            MachineRegistry.MAGNETIC,
            GameRegistry.findItemStack(
                ModList.THERMALEXPANSION.modLabel, "conduitEnergyReinforced", 1
            ),
            "Upgrades To Tier 2"
        ),
        ...
        MAGNETO4(
            MachineRegistry.MAGNETIC,
            GameRegistry
                .findItemStack(ModList.THERMALEXPANSION.modLabel, "cellReinforced", 1),
            "Upgrades To Tier 4"
        ),
        MAGNETO5(
            MachineRegistry.MAGNETIC,
            GameRegistry
                .findItemStack(ModList.THERMALEXPANSION.modLabel, "cellResonant", 1),
            "Upgrades To Tier 5"
        );
