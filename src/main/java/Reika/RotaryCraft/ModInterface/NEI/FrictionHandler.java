...
public class FrictionHandler extends TemplateRecipeHandler {
    public class FrictionHeaterRecipe extends CachedRecipe {
        ...
        public PositionedStack getIngredient() {
            return new PositionedStack(
                ReikaItemHelper.getSizedItemStack(input, 1), 51, 6
            );
        }
    }
    ...
    public void loadTransferRects() {
        transferRects.add(
            new RecipeTransferRect(new Rectangle(74, 23, 23, 18), "rcfriction")
        );
    }
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId != null && outputId.equals("rcfriction")) {
            Collection<ItemStack> li
                = RecipesFrictionHeater.getRecipes().getAllSmeltables();
            for (ItemStack is : li)
    ...
    public void loadCraftingRecipes(ItemStack result) {
        Collection<FrictionRecipe> rec
            = RecipesFrictionHeater.getRecipes().getRecipesByOutput(result);
        if (rec != null) {
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        FrictionRecipe rec
            = RecipesFrictionHeater.getRecipes().getRecipeByInput(ingredient);
        if (rec != null) {
    ...
    public void drawExtras(int recipe) {
        ...
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            Minecraft.getMinecraft().fontRenderer, temp, 83, 65, 0
        );
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            Minecraft.getMinecraft().fontRenderer, time, 83, 80, 0
        );
    }
}
