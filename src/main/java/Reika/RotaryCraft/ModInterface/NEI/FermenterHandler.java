...
public class FermenterHandler extends TemplateRecipeHandler {
    public class FermenterRecipe extends CachedRecipe {
        ...
        public PositionedStack getResult() {
            ...
            ItemStack is = output.getItem() == ItemRegistry.YEAST.getItemInstance()
                ? output
                : ReikaItemHelper.getSizedItemStack(output, amt);
        ...
        private ItemStack getTopSlot() {
            return output.getItem() == ItemRegistry.YEAST.getItemInstance()
                ? new ItemStack(Items.sugar)
                : ItemRegistry.YEAST.getStackOf();
        }
    ...
    public void drawBackground(int recipe) {
        ...
        ReikaGuiAPI.instance.drawTexturedModalRectWithDepth(
            0, 6, 5, 5, 166, 76, ReikaGuiAPI.NEI_DEPTH
        );
    }
    ...
    public void drawForeground(int recipe) {
        ...
        //ReikaGuiAPI.instance.drawTexturedModalRect(0, 0, (int)(ico.getMinU()*16),
        //(int)(ico.getMinV()*16), 16, 16);
    }

    @Override
    public void loadTransferRects() {
        transferRects.add(
            new RecipeTransferRect(new Rectangle(74, 35, 23, 17), "rcferment")
        );
    }
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId != null && outputId.equals("rcferment")) {
            ...
            for (ItemStack is : li)
                arecipes.add(new FermenterRecipe(
                    is,
                    ReikaItemHelper.getSizedItemStack(
                        ItemStacks.sludge, MulchMaterials.instance.getPlantValue(is)
                    )
                ));
        }
    ...
    public void loadCraftingRecipes(ItemStack result) {
        if (ReikaItemHelper.matchStacks(result, ItemStacks.sludge)
            || ReikaItemHelper.matchStacks(result, ItemStacks.ethanolbucket)) {
            Collection<ItemStack> li = MulchMaterials.instance.getAllValidPlants();
            for (ItemStack is : li) {
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        if (ItemRegistry.YEAST.matchItem(ingredient)) {
            ...
            if (this.isYeastIngredient(ingredient)) {
                if (ConfigRegistry.enableFermenterYeast())
                    arecipes.add(
                        new FermenterRecipe(ingredient, ItemRegistry.YEAST.getStackOf())
                    );
            } else
