...
public class ExtractorHandler extends TemplateRecipeHandler {
    public class ExtractorRecipe extends CachedRecipe {
        ...
        public ArrayList<PositionedStack> getIngredients() {
            ...
            for (int i = 0; i < 2; i++) {
                for (int j = 0; j < 4; j++) {
                    PositionedStack pos = new PositionedStack(
                        this.getItem(j, i), dx + vx * j, dy + vy * i
                    );
                    if (!(i == 1 && j == 3))
                        stacks.add(pos);
                }
            ...
            if (true) {
                ExtractorBonus bonus
                    = ExtractorBonus.getBonusForIngredient(this.getSolution());
                if (bonus != null) {
    ...
    public void loadTransferRects() {
        transferRects.add(
            new RecipeTransferRect(new Rectangle(20, 20, 126, 22), "rcextract")
        );
    }
    ...
    public void drawExtras(int recipe) {
        ...
        Minecraft.getMinecraft().fontRenderer.drawString(
            String.format("%d%s duplication chance per stage", chance, "%"),
            -2,
            65,
            0x333333,
            false
        ...
        Minecraft.getMinecraft().fontRenderer.drawString(
            String.format(
                "(Average %.2f units per ore)",
                Math.pow(1 + 0.01 * this.getDupeChance(recipe), 4)
            ),
            9,
            76,
            0x333333,
        ...
        if (bon != null) {
            ...
            if (bonus != null) {
                ...
                ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                    Minecraft.getMinecraft().fontRenderer, s, 157, 34, 0
                );
                //Minecraft.getMinecraft().fontRenderer.drawStringWithShadow(s, 146, 34,
                //0xffffff);
            }
        }
    }
