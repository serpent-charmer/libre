...
public class DropProcessorHandler extends TemplateRecipeHandler {
    public class DropRecipe extends CachedRecipe {
        ...
        public List<PositionedStack> getOtherStacks() {
            ...
            List<ItemStack> li = fixedResult != null
                ? recipe.getOutputsOfInputForDisplay(fixedResult)
                : new ArrayList(recipe.getPotentialOutputsForDisplay());
            ...
            ArrayList<Integer> indices = ReikaJavaLibrary.makeIntListFromArray(
                ReikaArrayHelper.getLinearArray(li.size())
            );
            ...
            for (int idx : indices2) {
                ...
                if (is.stackTagCompound != null
                    && is.stackTagCompound.hasKey("dropTooltip")) {
                    tip = is.stackTagCompound.getString("dropTooltip");
                    is.stackTagCompound.removeTag("dropTooltip");
                }
                pos.add(
                    tip != null ? new PositionedStackWithTooltip(is, x, y, tip)
                                : new PositionedStack(is, x, y)
                );
                i++;
            }
            return pos;
        ...
        public PositionedStack getIngredient() {
            return new PositionedStack(
                ReikaItemHelper.getSizedItemStack(this.getInput(), 1), 5, 25
            );
        }
    ...
    public List<String> handleItemTooltip(
        ...
        if (stack != null) {
            for (PositionedStack is :
                 ((DropRecipe) arecipes.get(recipe)).getOtherStacks()) {
                if (is instanceof PositionedStackWithTooltip
                    && gui.isMouseOver(is, recipe))
    ...
    public void drawBackground(int recipe) {
        ...
        ReikaGuiAPI.instance.drawTexturedModalRectWithDepth(
            0, 1, 5, 11, 166, 70, ReikaGuiAPI.NEI_DEPTH
        );
    }
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId != null && outputId.equals("rcdrops")) {
            Collection<DropProcessing> li
                = TileEntityDropProcessor.getAllProcessors(); //load generic recipes
    ...
    public void loadCraftingRecipes(ItemStack result) {
        Collection<DropProcessing> c
            = TileEntityDropProcessor.getAllHandlersProducing(result);
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        if (TileEntityDropProcessor.isProcessable(ingredient)) {
            arecipes.add(
                new DropRecipe(ingredient, TileEntityDropProcessor.getHandler(ingredient))
            );
        }
    }
