...
public class BlastFurnaceHandler extends TemplateRecipeHandler {
    public class BlastFurnRecipe extends CachedRecipe implements BlastTempRecipe {
        ...
        public PositionedStack getResult() {
            ...
            return new PositionedStack(
                ReikaItemHelper.getSizedItemStack(recipe.outputItem(), num), 143, 24
            );
        }
        ...
        public List<PositionedStack> getOtherStacks() {
            ...
            if (ReikaItemHelper.matchStacks(recipe.outputItem(), ItemStacks.steelblock)) {
                li.add(new PositionedStack(
                    ReikaItemHelper.getSizedItemStack(ItemStacks.steelingot, 2), 143, 24
                ));
                li.add(new PositionedStack(
                    ReikaItemHelper.getSizedItemStack(ReikaItemHelper.charcoal, 3),
                    143,
                    24 - 18
                ));
                li.add(new PositionedStack(
                    new ItemStack(Items.iron_ingot, 5, 1), 143, 24 + 18
                ));
            }
            return li;
        }
        ...
        public ArrayList<PositionedStack> getIngredients() {
                ...
                for (int j = 0; j < 3; j++) {
                    if (i * 3 + j < num) {
                        stacks.add(new PositionedStack(
                            recipe.mainItemForDisplay(), dx + 18 * i, dy + 18 * j
                        ));
                    }
                }
            }
            ...
            if (recipe.tertiary.exists())
                stacks.add(
                    new PositionedStack(recipe.tertiary.getItemForDisplay(true), 21, 5)
                );
            if (recipe.primary.exists())
                stacks.add(
                    new PositionedStack(recipe.primary.getItemForDisplay(true), 21, 24)
                );
            if (recipe.secondary.exists())
                stacks.add(
                    new PositionedStack(recipe.secondary.getItemForDisplay(true), 21, 43)
                );
    ...
    public void loadTransferRects() {
        transferRects.add(
            new RecipeTransferRect(new Rectangle(115, 23, 23, 18), "rcblastf")
        );
    }
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId != null && outputId.equals("rcblastf")) {
            Collection<BlastFurnacePattern> li
                = RecipesBlastFurnace.getRecipes().getAllRecipes();
            for (BlastFurnacePattern p : li) {
                if (p instanceof BlastRecipe)
                    ...
                    RotaryCraft.logger.logError(
                        "Unrenderable recipe, makes " + p.outputItem() + "!"
                    );
            }
        }
    ...
    public void loadCraftingRecipes(ItemStack result) {
        ArrayList<BlastRecipe> li
            = RecipesBlastFurnace.getRecipes().getAllRecipesMaking(result);
        for (int i = 0; i < li.size(); i++)
            arecipes.add(new BlastFurnRecipe(li.get(i)));
        ArrayList<BlastCrafting> li2
            = RecipesBlastFurnace.getRecipes().getAllCraftingMaking(result);
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        ArrayList<BlastRecipe> li
            = RecipesBlastFurnace.getRecipes().getAllRecipesUsing(ingredient);
        for (int i = 0; i < li.size(); i++)
            arecipes.add(new BlastFurnRecipe(li.get(i)));
        ArrayList<BlastCrafting> li2
            = RecipesBlastFurnace.getRecipes().getAllCraftingUsing(ingredient);
        for (int i = 0; i < li2.size(); i++)
            arecipes.add(new BlastFurnCrafting(li2.get(i)));
    }
    ...
    public void drawExtras(int recipe) {
        ...
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            f, s, f.getStringWidth(s) / 2 - 2 * (s.length() / 5), 61, 0
        );
        ...
        if (r instanceof BlastFurnRecipe) {
            ...
            if (in1.exists()) {
                String sg = String.format(
                    "%s: x%d (%.1f%%)",
                    in1.getItemForDisplay(true).getDisplayName(),
                    in1.numberToUse,
                    100 * in1.chanceToUse
                );
            ...
            if (in2.exists()) {
                String sg = String.format(
                    "%s: x%d (%.1f%%)",
                    in2.getItemForDisplay(true).getDisplayName(),
                    in2.numberToUse,
                    100 * in2.chanceToUse
                );
            ...
            if (in3.exists()) {
                String sg = String.format(
                    "%s: x%d (%.1f%%)",
                    in3.getItemForDisplay(true).getDisplayName(),
                    in3.numberToUse,
                    100 * in3.chanceToUse
                );
                f.drawString(sg, 21, 72 + dy, 0);
            ...
            f.drawString(
                "Bonus output: " + (br.bonusYield > 0 ? br.bonusYield + "x" : "None"),
                21,
                72 + dy,
                0
            );
