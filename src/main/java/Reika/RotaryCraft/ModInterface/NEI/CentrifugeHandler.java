...
public class CentrifugeHandler extends TemplateRecipeHandler {
    public class CentrifugeNEIRecipe extends CachedRecipe {
        ...
        public List<PositionedStack> getOtherStacks() {
            ...
            for (ItemWithChance is : recipe.getItems()) {
                ...
                pos.add(new PositionedStackWithTooltip(
                    ReikaItemHelper.getSizedItemStack(is.getItem(), num), x, y, tip
                ));
                i++;
            }
            return pos;
        ...
        public PositionedStack getIngredient() {
            ...
            return new PositionedStack(
                ReikaItemHelper.getSizedItemStack(recipe.getInput(), num), 21, 28
            );
        }
    }
    ...
    public List<String> handleItemTooltip(
        ...
        if (stack != null) {
            for (PositionedStack is :
                 ((CentrifugeNEIRecipe) arecipes.get(recipe)).getOtherStacks()) {
    ...
    public void loadTransferRects() {
        transferRects.add(
            new RecipeTransferRect(new Rectangle(39, 16, 40, 40), "rccentri")
        );
    }
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId != null && outputId.equals("rccentri")) {
            Collection<ItemStack> li
                = RecipesCentrifuge.getRecipes().getAllCentrifugables();
            for (ItemStack is : li)
                arecipes.add(new CentrifugeNEIRecipe(
                    RecipesCentrifuge.getRecipes().getRecipeResult(is)
                ));
        } else if (outputId != null && outputId.equals("liquid")) {
            FluidStack fs = (FluidStack) results[0];
            ArrayList<ItemStack> li
                = RecipesCentrifuge.getRecipes().getSources(fs.getFluid());
            for (ItemStack is : li)
                arecipes.add(new CentrifugeNEIRecipe(
                    RecipesCentrifuge.getRecipes().getRecipeResult(is)
                ));
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        if (RecipesCentrifuge.getRecipes().isCentrifugable(ingredient)) {
            arecipes.add(new CentrifugeNEIRecipe(
                RecipesCentrifuge.getRecipes().getRecipeResult(ingredient)
            ));
        }
    }
