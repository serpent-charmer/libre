...
public class NEITabOccluder extends INEIGuiAdapter {
    ...
    public boolean hideItemPanelSlot(GuiContainer gui, int x, int y, int w, int h) {
        if (gui instanceof GuiMachine) {
            ...
            Rectangle4i help = new Rectangle4i(
                gm.getGuiLeft() - 10, gm.getGuiTop(), 10, gm.getYSize()
            );
    ...
    private Rectangle4i getTabBox(GuiMachine gm) {
        /*if (gm instanceof GuiWorktable) {
            return new Rectangle4i(gm.getGuiLeft()+gm.getXSize(), gm.getGuiTop()+78,
        Math.min(((GuiWorktable)gm).getRollout(), 63), 64);
        }
        else */
        if (gm instanceof GuiNonPoweredMachine) {
            ...
            return new Rectangle4i(
                gm.getGuiLeft() + gm.getXSize(), gm.getGuiTop(), 43, 50
            );
        } else if (gm instanceof GuiPowerOnlyMachine) {
            return new Rectangle4i(
                gm.getGuiLeft() + gm.getXSize(), gm.getGuiTop(), 43, 24
            );
        } else {
            return new Rectangle4i(
                gm.getGuiLeft() + gm.getXSize(), gm.getGuiTop() + 3, 43, gm.getYSize() - 6
            );
        }
    }
}
