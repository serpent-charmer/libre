...
public class WetterHandler extends TemplateRecipeHandler {
    ...
    public void loadTransferRects() {
        transferRects.add(
            new RecipeTransferRect(new Rectangle(103, 15, 44, 22), "rcwetter")
        );
    }
    ...
    public void loadCraftingRecipes(ItemStack result) {
        Collection<WettingRecipe> c
            = RecipesWetter.getRecipes().getRecipesByResult(result);
        if (c != null) {
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        ...
        if (fs != null) {
            Collection<WettingRecipe> c
                = RecipesWetter.getRecipes().getRecipesByFluid(fs.getFluid());
            if (c != null) {
                for (WettingRecipe r : c) {
                    arecipes.add(new WetterRecipe(r));
                }
        ...
        Collection<WettingRecipe> c
            = RecipesWetter.getRecipes().getRecipesUsing(ingredient);
        if (c != null) {
            for (WettingRecipe r : c) {
                arecipes.add(new WetterRecipe(r));
            }
    ...
    private void drawFluids(int recipe) {
        ...
        Minecraft.getMinecraft().fontRenderer.drawString(
            r.recipe.getFluid().amount + " mB", x + 20, y2 - 4, 0
        );
    }
}
