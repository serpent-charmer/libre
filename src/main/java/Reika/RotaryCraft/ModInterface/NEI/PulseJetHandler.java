...
public class PulseJetHandler extends TemplateRecipeHandler {
    ...
    public void loadTransferRects() {
        transferRects.add(
            new RecipeTransferRect(new Rectangle(119, 22, 18, 17), "rcpulsej")
        );
    }
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId != null && outputId.equals("rcpulsej")) {
            Collection<ItemStack> li
                = RecipesPulseFurnace.getRecipes().getAllSmeltables();
            for (ItemStack is : li)
    ...
    public void drawExtras(int recipe) {
        ...
        String sg
            = String.format("Min Temperature: %dC", r.reference.requiredTemperature);
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            Minecraft.getMinecraft().fontRenderer, sg, 83, 61, 0
        );
    }
}
