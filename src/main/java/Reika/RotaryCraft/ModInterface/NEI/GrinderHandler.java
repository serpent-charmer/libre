...
public class GrinderHandler extends TemplateRecipeHandler {
    public class GrinderRecipe extends CachedRecipe {
        ...
        public PositionedStack getResult() {
            ItemStack result
                = RecipesGrinder.getRecipes().getGrindingResult(this.getEntry());
            ItemStack is = result;
        ...
        public ItemStack getEntry() {
            ItemStack is
                = input.get((int) (System.nanoTime() / 1000000000) % input.size());
    ...
    public class SeedRecipe extends CachedRecipe {
        ...
        private ItemStack getInput() {
            ItemStack is
                = inputs.get((int) (System.nanoTime() / 1000000000) % inputs.size());
            return ReikaItemHelper.getSizedItemStack(is, 1);
        }
    }
    ...
    public void drawForeground(int recipe) {
        ...
        if (c.getIngredient() != null
            && TileEntityGrinder.isGrindableSeed(c.getIngredient().item)) {
    ...
    public void loadTransferRects() {
        transferRects.add(
            new RecipeTransferRect(new Rectangle(95, 23, 23, 18), "rcgrinder")
        );
    }
