...
public class FuelEnhancerHandler extends TemplateRecipeHandler {
    ...
    public void drawBackground(int recipe) {
        ...
        ReikaGuiAPI.instance.drawTexturedModalRectWithDepth(
            0, 0, 5, 11, 166, 60, ReikaGuiAPI.NEI_DEPTH
        );
    }
    ...
    public void loadCraftingRecipes(ItemStack is) {
        ...
        if (fs != null) {
            Collection<FuelConversion> li
                = TileEntityFuelConverter.getByOutput(fs.getFluid());
    ...
    public void loadTransferRects() {
        transferRects.add(
            new RecipeTransferRect(new Rectangle(32, -12, 105, 10), "fuelenhance")
        );
        //transferRects.add(new RecipeTransferRect(new Rectangle(23, 23, 46, 18),
        //"fuelenhance"));
    }
    ...
    public void drawExtras(int recipe) {
        ...
        ReikaTextureHelper.bindFinalTexture(
            RotaryCraft.class, "/Reika/RotaryCraft/Textures/GUI/buttons.png"
        );
        ...
        if (mx > 0 && mx < 176 && my > 0 && my < 54) {
            ...
            if (mx >= 3 && mx <= 19 && my <= 34)
                li.add(
                    in.getLocalizedName(new FluidStack(in, r.recipe.fluidRatio)) + " x"
                    + (r.recipe.fluidRatio * r.recipe.speedFactor)
                );
            else if (mx >= 147 && mx <= 147 + 16 && my <= 34)
                li.add(
                    out.getLocalizedName(new FluidStack(out, 1)) + " x"
                    + (r.recipe.speedFactor)
                );
            else if (mx >= 21 && mx <= 145 && my >= 36) {
                li.add(String.format(
                    "%.3f%% Item Consumption Chance", r.recipe.itemConsumptionChance * 100
                ));
