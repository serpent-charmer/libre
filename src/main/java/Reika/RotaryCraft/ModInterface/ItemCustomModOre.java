...
public class ItemCustomModOre extends AutoOreItem implements GradientBlend {
    ...
    public int getItemSpriteIndex(ItemStack item) {
        ...
        return this == ItemRegistry.CUSTOMEXTRACT.getItemInstance()
            ? base + item.getItemDamage() % 4
            : base + this.getOreType(item).type.ordinal();
    }
    ...
    public String getItemStackDisplayName(ItemStack is) {
        ...
        if (ItemRegistry.CUSTOMEXTRACT.matchItem(is)) {
            ...
            return s != null
                ? ore.displayName + " " + ReikaStringParser.capFirstChar(s.name())
                : "null";
        } else {
    ...
    public static int getEntryIndex(ItemStack is) {
        return is.getItem() == ItemRegistry.CUSTOMEXTRACT.getItemInstance()
            ? is.getItemDamage() / 4
            : is.getItemDamage();
    }
