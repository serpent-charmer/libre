...
public class MachineAspectMapper {
    ...
    private MachineAspectMapper() {
        ...
        //this.addAspect(MachineRegistry.REFRESHER, Aspect.TIME, 4); not native to
        //ThaumCraft
    ...
    private AspectList getAspects(MachineRegistry m, int offset) {
        ...
        AspectList al
            = ThaumcraftApiHelper.generateTags(is.getItem(), is.getItemDamage());
        if (al == null)
            al = new AspectList();
        ...
        if (m == MachineRegistry.SHAFT) {
            ...
            if (ModList.SATISFORESTRY.isLoaded()) {
                int amt = MaterialRegistry.matList[offset].ordinal()
                    - MaterialRegistry.STONE.ordinal();
                if (amt > 0)
                    al.merge((Aspect) SFAPI.genericLookups.getAspect(), amt);
            }
        }
    ...
    private ItemStack getItem(MachineRegistry m, int offset) {
        return m.hasSubdivisions() ? m.getCraftedMetadataProduct(offset)
                                   : m.getCraftedProduct();
    }
            ...
            if (m.hasSubdivisions()) {
                ...
                for (int k = 0; k < n; k++) {
                    ...
                    ReikaThaumHelper.addAspectsToBlockMeta(
                        m.getBlock(), k, this.getAspects(m, k)
                    );
                }
            } else {
                ...
                ReikaThaumHelper.addAspectsToBlockMeta(
                    m.getBlock(), m.getBlockMetadata(), this.getAspects(m, 0)
                );
            }
        }
    }
