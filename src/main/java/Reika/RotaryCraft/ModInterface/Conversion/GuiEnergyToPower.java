...
public class GuiEnergyToPower extends GuiNonPoweredMachine {
    ...
    public void initGui() {
        ...
        if (flexible) {
            buttonList.add(new GuiButton(
                0, SHIFT + j + inset - 1 + dx, k + ySize - 30 - 48 + 0, 20, 20, "-"
            ));
            ;
            buttonList.add(new GuiButton(
                1,
                SHIFT + j + xSize - 20 - inset - dx,
                k + ySize - 30 - 48 + 0,
                20,
                20,
                "+"
        ...
        buttonList.add(new GuiButton(
            2, SHIFT + j + inset - 10 + dx, k + ySize - 30 - 48 + 25, 20, 20, "-"
        ));
        ;
        buttonList.add(new GuiButton(
            3, SHIFT + j + xSize - 20 - inset - dx, k + ySize - 30 - 48 + 25, 20, 20, "+"
        ));

        buttonList.add(new GuiButton(
            4, SHIFT + j + xSize - 20 - inset - dx, k + ySize - 30 - 48 + 50, 20, 20, ""
        ));
    }
    ...
    protected void actionPerformed(GuiButton b) {
        ...
        if (b.id == 4) {
            ReikaPacketHelper.sendUpdatePacket(
                ...
                PacketRegistry.CONVERTERREDSTONE.ordinal(),
                engine,
                pt
            );
        } else if (b.id < 24000) {
            ReikaPacketHelper.sendPacketToServer(
                RotaryCraft.packetChannel,
                PacketRegistry.CONVERTERPOWER.ordinal(),
                engine,
                b.id == 3 ? 1 : 0
            );
        }
    ...
    protected void drawGuiContainerForegroundLayer(int a, int b) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            this.drawRect(
                SHIFT + xSize / 2 - w + inset - dx,
                ySize - 30 - 48 + inset + i * dy,
                SHIFT + xSize / 2 + w - inset - dx,
                ySize - 30 - 48 + h - inset + i * dy,
                0xff000000
            );
        }
        ...
        this.drawCenteredString(
            fontRendererObj,
            String.format("Torque: %d Nm", torque),
            SHIFT + xSize / 2 - dx,
            ySize - 30 - 48 + 6,
            0xffffff
        );
        ...
        this.drawCenteredString(
            fontRendererObj,
            String.format("Speed: %d rad/s", omega),
            SHIFT + xSize / 2 - dx,
            ySize - 30 - 48 + 6 + dy,
            0xffffff
        );
        this.drawCenteredString(
            ...
            String.format(
                "Power: %.3f %sW",
                ReikaMathLibrary.getThousandBase(power),
                ReikaEngLibrary.getSIPrefix(power)
            ),
            SHIFT + xSize / 2 - dx,
            ySize - 30 - 48 + 6 + dy * 2,
            0xffffff
        ...
        if (ReikaGuiAPI.instance.isMouseInBox(j + 171, j + 188, k + 21, k + 90)) {
            ...
            String sg = String.format(
                "%d/%d %s", e, engine.getMaxStorage(), engine.getUnitDisplay()
            );
            ReikaGuiAPI.instance.drawTooltipAt(
                fontRendererObj,
                sg,
                ReikaGuiAPI.instance.getMouseRealX() - j
                    + fontRendererObj.getStringWidth(sg) + 24,
                ReikaGuiAPI.instance.getMouseRealY() - k
            );
            //this.drawHoveringText(ReikaJavaLibrary.makeListFrom(sg),
            //ReikaGuiAPI.instance.getMouseRealX()-j,
            //ReikaGuiAPI.instance.getMouseRealY()-k, fontRendererObj);
        }
        ...
        if (ReikaGuiAPI.instance.isMouseInBox(
            ...
            ReikaGuiAPI.instance.drawTooltipAt(
                fontRendererObj,
                sg,
                ReikaGuiAPI.instance.getMouseRealX() - 24
                    - fontRendererObj.getStringWidth(sg),
                ReikaGuiAPI.instance.getMouseRealY() - k
            );
            //this.drawHoveringText(ReikaJavaLibrary.makeListFrom(sg),
            //ReikaGuiAPI.instance.getMouseRealX()-j,
        ...
        api.drawItemStack(
            itemRender, fontRendererObj, engine.getRedstoneStateIcon(), 148, 71 + ddy
        );
        GL11.glPushMatrix();
        double s = 0.75;
        GL11.glScaled(s, s, s);
        api.drawItemStack(
            itemRender,
            fontRendererObj,
            engine.hasRedstoneSignal() ? new ItemStack(Blocks.lit_redstone_lamp)
