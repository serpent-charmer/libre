...
public class TileEntityAirCompressor
    extends TileEntityPowerReceiver implements PressureTE, RCToModConverter {
    ...
    public void overpressure(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 6; i++)
            world.createExplosion(
                null,
                x + 0.5 - 1 + rand.nextDouble() * 2,
                y + 0.5 - 1 + rand.nextDouble() * 2,
                z + 0.5 - 1 + rand.nextDouble() * 2,
                3F,
                ConfigRegistry.BLOCKDAMAGE.getState()
            );
    }
    ...
    public ConnectOverride overridePipeConnection(PipeType type, ForgeDirection with) {
        return this.isPipeConnected(with) ? ConnectOverride.CONNECT :
    ...
    public int getGenAir() {
        return (int
        ) (power / ReikaPneumaticHelper.getWattsPerAir()
