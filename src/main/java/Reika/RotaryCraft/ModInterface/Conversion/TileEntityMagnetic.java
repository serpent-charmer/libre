...
public class TileEntityMagnetic extends EnergyToPowerBase
    implements IEnergyHandler { //Handler because EnderIO uses it
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        //ReikaJavaLibrary.pConsole(storedEnergy+":"+this.getConsumedUnitsPerTick(),
        //Side.SERVER);
        ...
        if (!world.isRemote) {
            ...
            if (power > 0) {
                if (tickcount >= 85) {
                    tickcount = 0;
                    SoundRegistry.DYNAMO.playSoundAtBlock(
                        world, x, y, z, this.isMuffled() ? 0.1F : 0.5F, 1F
                    );
                }
            }
        }
    ...
    public boolean isValidSupplier(TileEntity te) {
        return te instanceof IEnergyHandler || te instanceof IEnergyProvider
    ...
    public boolean canConnectEnergy(ForgeDirection from) {
        return from
            == this.getFacing(
            ); // && this.isValidSupplier(this.getAdjacentTileEntity(from));
    }
    ...
    public int getMaxStorage() {
        return 1
            + this.getMinimumCurrent()
            * 200; //ReikaMathLibrary.intpow2(10,
                   //this.getTier());//TileEntityPneumaticEngine.maxMJ*10;
    }
