...
//@Strippable(value = {"universalelectricity.api.electricity.IVoltageOutput",
//"universalelectricity.api.energy.IEnergyInterface"})
@Strippable(value = { "ic2.api.energy.tile.IEnergySource" })
public class TileEntityGenerator extends PoweredLiquidReceiver
    implements IEnergySource, RCToModConverter, UpgradeableMachine, NBTMachine {
                    ...
                    if (torque > t) {
                        if (ReikaRandomHelper.doWithChance((torque - t) / 20000D)) {
                            if (rand.nextInt(upgraded ? 16 : 4) == 0) {
                                this.delete();
                                world.newExplosion(
                                    null, x + 0.5, y + 0.5, z + 0.5, 3, true, true
                                );
                            } else {
                                ReikaSoundHelper.playSoundAtBlock(
                                    world, x, y, z, "random.fizz"
                                );
                            }
                        }
                    }
    ...
    public double getOfferedEnergy() {
        return (power - this.getPowerLoss()) / ReikaEUHelper.getWattsPerEU()
            * ConfigRegistry.getConverterEfficiency();
    }

    private double getPowerLoss() {
        int t = upgraded ? 512 : 128;
        return torque > t
            ? ReikaMathLibrary.intpow2(torque - t, 2) * (upgraded ? 0.0625 : 0.125)
            : 0;
    }
    ...
    public boolean isValidFluid(Fluid f) {
        return f != null
            && (f.equals(FluidRegistry.getFluid("ic2coolant"))
                || f.equals(FluidRegistry.getFluid("rc liquid nitrogen")));
    }
    ...
    public boolean canUpgradeWith(ItemStack item) {
        return !upgraded && ItemRegistry.UPGRADE.matchItem(item)
            && item.getItemDamage() == Upgrades.FLUX.ordinal();
    }
