...
//@Strippable(value = {"universalelectricity.api.electricity.IVoltageInput",
//"universalelectricity.api.energy.IEnergyInterface"})
@Strippable(value = { "ic2.api.energy.tile.IEnergySink" })
public class TileEntityElectricMotor
    extends EnergyToPowerBase implements PowerGenerator, SimpleProvider, IEnergySink {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote) {
            ...
            if (power > 0) {
                if (tickcount >= 294) {
                    tickcount = 0;
                    SoundRegistry.ELECTRIC.playSoundAtBlock(
                        world, x, y, z, this.isMuffled() ? 0.08F : 0.2F, 0.51F
                    );
                }
            }
        }
    ...
    public int getSinkTier() {
        ...
        return IC2Handler.getInstance().isIC2Classic() ? this.getScaledTier()
                                                       : 5; //this.getScaledTier();
    }
    ...
    private void onWrongVoltage(int tier, int correct) {
        ...
        if (over > 2 && TileEntityGenerator.hardModeEU) {
            worldObj.newExplosion(
                null, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, 9F, true, true
            );
        } else if (over > 1) {
            ReikaParticleHelper.SMOKE.spawnAroundBlock(
                worldObj, xCoord, yCoord, zCoord, 12
            );
            ReikaSoundHelper.playSoundAtBlock(
                worldObj, xCoord, yCoord, zCoord, "random.fizz"
            );
        } else if (over == 1) {
            if (rand.nextInt(5) == 0) {
                ReikaParticleHelper.SMOKE.spawnAroundBlock(
            ...
            if (rand.nextInt(15) == 0)
                SoundRegistry.ELECTRIC.playSoundAtBlock(
                    worldObj, xCoord, yCoord, zCoord, 0.2F, 2F
                );
        }
    }
    ...
    public boolean isValidSupplier(TileEntity te) {
        return te instanceof IEnergySource || te instanceof IEnergyConductor
