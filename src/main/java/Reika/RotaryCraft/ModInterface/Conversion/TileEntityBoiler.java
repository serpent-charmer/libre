...
public class TileEntityBoiler
    extends PoweredLiquidIO implements TemperatureTE, RCToModConverter {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        //ReikaJavaLibrary.pConsole(storedEnergy/ReikaRailCraftHelper.getSteamBucketEnergy(this.getWaterTemp()),
        //Side.SERVER);

        //ReikaJavaLibrary.pConsoleSideOnly(this.getSteam()+":"+storedEnergy+"/"+ReikaRailCraftHelper.getSteamBucketEnergy(),
        //Side.SERVER);
        if (!world.isRemote) {
            ...
            if (space > 0) {
                int mB = Math.min(
                    ...
                    Math.min(
                        input.getLevel(),
                        ReikaRailCraftHelper.getAmountConvertibleSteam(
                            this.getInitTemp(), storedEnergy
                        )
                    )
                );
                if (mB > 0)
