...
public class TileEntitySteam
    extends EnergyToPowerBase implements PowerGenerator, SimpleProvider, IPipeConnection,
    ...
    private void getSteam(World world, int dx, int dy, int dz) {
        ...
        if (storedEnergy <= this.getMaxStorage() - drain) {
            ...
            if (te instanceof IFluidHandler) {
                ...
                if (liq != null && liq.amount > 0
                    && liq.getFluid().equals(FluidRegistry.getFluid("water")))
    ...
    public ConnectOverride overridePipeConnection(PipeType type, ForgeDirection dir) {
        return dir == this.getFacing().getOpposite() && type == PipeType.FLUID
    ...
    public boolean canConnectToPipeOnSide(MachineRegistry p, ForgeDirection side) {
        return this.canConnectToPipe(p) && side == this.getFacing()
            || super.canConnectToPipeOnSide(p, side);
    }
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        ...
        return this.canFill(from, resource.getFluid())
            ? this.addEnergy(resource.amount, doFill)
            : 0;
    }
    ...
    public boolean canFill(ForgeDirection from, Fluid fluid) {
        return super.canFill(from, fluid)
            || from == this.getFacing() && fluid.equals(FluidRegistry.getFluid("steam"));
    }
