...
public class TileEntityFuelEngine extends TileEntityIOMachine
    implements IFluidHandler, PipeConnector, SimpleProvider, PowerGenerator,
               IPipeConnection, TemperatureTE {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (power > 0) {
            ...
            if (soundTick.checkCap()) {
                SoundRegistry.DIESEL.playSoundAtBlock(
                    world, x, y, z, RotaryAux.isMuffled(this) ? 0.3F : 1F, 0.4F
                );
            }
    ...
    public boolean isUsingTurbofuel() {
        return ModList.SATISFORESTRY.isLoaded()
            && tank.getActualFluid() == SFAPI.genericLookups.getTurbofuel();
    }

    private int getGenOmega() {
        return temperature <= 450 ? GEN_OMEGA
                                  : Math.max(16, GEN_OMEGA + 450 - temperature);
    }
        ...
        switch (meta) {
            case 0:
                ReikaParticleHelper.SMOKE.spawnAt(
                    world, x + 0.6875, y + 0.9375, z + 0.0625
                );
                ReikaParticleHelper.SMOKE.spawnAt(
                    world, x + 0.6875, y + 0.9375, z + 0.9375
                );
                break;
            case 1:
                ReikaParticleHelper.SMOKE.spawnAt(
                    world, x + 0.3175, y + 0.9375, z + 0.0625
                );
                ReikaParticleHelper.SMOKE.spawnAt(
                    world, x + 0.3175, y + 0.9375, z + 0.9375
                );
                break;
            case 2:
                ReikaParticleHelper.SMOKE.spawnAt(
                    world, x + 0.0625, y + 0.9375, z + 0.6875
                );
                ReikaParticleHelper.SMOKE.spawnAt(
                    world, x + 0.9375, y + 0.9375, z + 0.6875
                );
                break;
            case 3:
                ReikaParticleHelper.SMOKE.spawnAt(
                    world, x + 0.0625, y + 0.9375, z + 0.3175
                );
                ReikaParticleHelper.SMOKE.spawnAt(
                    world, x + 0.9375, y + 0.9375, z + 0.3175
                );
                break;
        }
    }
    ...
    public static boolean isValidFuel(Fluid f) {
        return f.equals(FluidRegistry.getFluid("fuel"))
            || (ModList.SATISFORESTRY.isLoaded()
                && f == SFAPI.genericLookups.getTurbofuel());
    }
    ...
    public ConnectOverride overridePipeConnection(PipeType type, ForgeDirection with) {
        return type == PipeType.FLUID && with != ForgeDirection.DOWN
    ...
    public Flow getFlowForSide(ForgeDirection side) {
        return side == (isFlipped ? ForgeDirection.DOWN : ForgeDirection.UP) ? Flow.NONE
                                                                             : Flow.INPUT;
    }
}
