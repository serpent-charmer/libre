...
@Strippable(
    value = { "mrtjp.projectred.api.IBundledTile",
              "appeng.api.networking.security.IActionHost" }
)
public class TileEntityBundledBus
    extends TileEntityPowerReceiver implements IBundledTile, IActionHost {
                        ...
                        for (int i = 0; i < Math.min(this.getActiveChannels(), NSLOTS);
                            ...
                            if (f1 != null && input[i] > 0) {
                                ...
                                if (fit > 0) {
                                    ...
                                    this.transferItem(
                                        ReikaItemHelper.getSizedItemStack(
                                            f1, Math.min(fit, f1.getMaxStackSize())
                                        ),
                                        (IInventory) te
                                    );
                                }
