...
public class AgriCanola implements ICropPlant {
    ...
    public ArrayList<ItemStack> getFruitsOnHarvest(int gain, Random rand) {
        ...
        while (n > 0) {
            int rem
                = Math.min(n, ItemRegistry.CANOLA.getItemInstance().getItemStackLimit());
    ...
    @Override
    public boolean
    onAllowedGrowthTick(World world, int x, int y, int z, int oldGrowthStage) {
        return true;
    }
    ...
    public IIcon getPlantIcon(int growthStage) {
        return BlockRegistry.CANOLA.getBlockInstance().getIcon(
            2, METADATA_CONVERSION[growthStage]
        );
    }
    ...
    @SideOnly(Side.CLIENT)
    public void
    renderPlantInCrop(IBlockAccess world, int x, int y, int z, RenderBlocks renderer) {}

    public static void register() {
        //Remove native handler for now
        try {
            Class c
                = Class.forName("com.InfinityRaider.AgriCraft.compatibility.ModHelper");
    ...
    @Override
    public IAdditionalCropData
    getInitialCropData(World world, int x, int y, int z, ICrop crop) {
        return null;
    }
