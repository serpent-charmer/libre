...
public final class BedrockRevealingInfusion extends InfusionRecipe {
    ...
    public BedrockRevealingInfusion(int inst, AspectList al, ItemStack[] recipe) {
        super(
            "BEDREVEAL",
            ItemRegistry.BEDREVEAL.getEnchantedStack(),
            inst,
            al,
            ItemRegistry.BEDHELM.getEnchantedStack(),
            recipe
        );
    ...
    public boolean matches(
        ArrayList<ItemStack> input, ItemStack main, World world, EntityPlayer player
    ) {
        ...
        if (!research.isEmpty()
            && !ThaumcraftApiHelper.isResearchComplete(
                player.getCommandSenderName(), research
            )) {
            return false;
        }
        ...
        if (in.getItem()
            != this.getRecipeInput().getItem(
            )) //instead of the original equality test, since that was NBT sensitive
            return false;
    ...
    public Object getRecipeOutput(ItemStack input) {
        ...
        ReikaNBTHelper.combineNBT(
            is.stackTagCompound, cachedCentralInput.stackTagCompound
        );
        return is;
    }
}
