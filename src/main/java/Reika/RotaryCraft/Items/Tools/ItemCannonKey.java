    ...
    public boolean onItemUse(
        ...
        if (DragonOptions.DEBUGMODE.getState()) {
            ReikaChatHelper.write(
                "Key is held by " + name + "; machine was placed by "
                + can.getPlacerName()
            );
            ReikaChatHelper.write(
                "name.equals(placer): " + name.equals(can.getPlacerName())
            );
            ReikaChatHelper.write(
                "name.compareTo(placer): " + name.compareTo(can.getPlacerName())
            );
        }
        if (name.equals(can.getPlacerName())) {
            ep.openGui(
                RotaryCraft.instance, GuiRegistry.SAFEPLAYERS.ordinal(), world, x, y, z
            );
            return true;
        } else {
            if (!is.hasTagCompound())
            ...
            if (DragonOptions.DEBUGMODE.getState()) {
                ReikaChatHelper.write(
                    "Key is made by " + owner + "; machine was placed by "
                    + can.getPlacerName()
                );
                ReikaChatHelper.write(
                    "owner.equals(placer): " + owner.equals(can.getPlacerName())
                );
                ReikaChatHelper.write(
                    "owner.compareTo(placer): " + owner.compareTo(can.getPlacerName())
                );
            }
            if (!owner.equals(can.getPlacerName())) {
                ReikaChatHelper.write(
                    "The key is for " + owner
                    + "'s machines, but this machine is owned by " + can.getPlacerName()
                    + "!"
                );
                return false;
            }
