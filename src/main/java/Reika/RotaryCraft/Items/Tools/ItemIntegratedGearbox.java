...
public class ItemIntegratedGearbox extends ItemRotaryTool implements Fillable {
    ...
    public static int getRatioFromIntegratedGearItem(ItemStack is, boolean requireFill) {
        ...
        if (requireFill && !i.isFull(is))
            return 0;
        boolean pos = !requireFill
            || i.getCurrentFluid(is)
                == FluidRegistry.getFluid("rc lubricant"); //meta >= 4;
    ...
    public void getSubItems(Item item, CreativeTabs par2CreativeTabs, List par3List) {
        for (int i = 0; i <= 4; i++) {
            ...
            if (i > 0) {
                ...
                this.addFluid(
                    is2, FluidRegistry.getFluid("rc lubricant"), this.getCapacity(is2)
                );
                par3List.add(is2);
                ...
                this.addFluid(
                    is2,
                    FluidRegistry.getFluid("rc liquid nitrogen"),
                    this.getCapacity(is2)
                );
                par3List.add(is2);
            }
        }
        ...
        if (is.getItemDamage() > 0) {
            ...
            if (ratio > 0) {
                ...
                if (is.stackTagCompound != null) {
                    ...
                    li.add(
                        "Is " + (amt * 100F / this.getCapacity(is)) + "% full of "
                        + f.getLocalizedName(new FluidStack(f, amt))
                    );
                }
            }
        } else {
    ...
    public boolean isFull(ItemStack is) {
        return is.stackTagCompound != null
            && this.getCurrentFillLevel(is) >= this.getCapacity(is);
    }
