...
@Strippable(
    value = { "buildcraft.api.tools.IToolWrench",
              "mrtjp.projectred.api.IScrewdriver",
              "binnie.extratrees.api.IToolHammer",
...
public class ItemScrewdriver extends ItemRotaryTool
    implements IToolWrench, IScrewdriver, IToolHammer, IMFRHammer, ICarpentersHammer,
               com.bluepowermod.api.misc.IScrewdriver {
    private static final BlockMap<Integer> maxdamage = new BlockMap(
    ); //Max damage values (or tileentity datas) for the block ids associated
    ...
    @Override
    public boolean
    doesSneakBypassUse(World world, int x, int y, int z, EntityPlayer player) {
    ...
    /* causes issues
    @Override
    public boolean onItemUseFirst(ItemStack is, EntityPlayer ep, World world, int x, int
    y, int z, int s, float par8, float par9, float par10)
    {
                ...
                if (wr.wrenchCanRemove(ep)) {
                    ...
                    if (!world.isRemote) {
                        if (drop != null) {
                            ...
                            ArrayList<ItemStack> li = world.getBlock(x, y,
    z).getDrops(world, x, y, z, world.getBlockMetadata(x, y, z), 0);
        ...
        Block b = world.getBlock(x, y, z);
        if (!world.isRemote && !ep.isSneaking() && b.rotateBlock(world, x, y, z,
    ForgeDirection.VALID_DIRECTIONS[s])) { return true;
        }
        return false;
    }*/
    ...
    public boolean onItemUse(
        ...
        if (te instanceof Screwdriverable) {
            ...
            boolean flag = false;
            if (ep.isSneaking())
                flag = sc.onShiftRightClick(
                    world, x, y, z, ForgeDirection.VALID_DIRECTIONS[s]
                );
            else
                flag
                    = sc.onRightClick(world, x, y, z, ForgeDirection.VALID_DIRECTIONS[s]);
            if (flag)
                return true;
        }
            ...
            if (m == MachineRegistry.ECU) {
                ...
                if (ep.isSneaking()) {
                    ...
                    ReikaChatHelper.writeString(
                        clicked.redstoneMode ? "ECU is now redstone-operated."
                                             : "ECU is now manually controlled."
                    );
                } else {
                    clicked.increment();
                    ReikaChatHelper.writeString(String.format(
                        "ECU set to %.2f%s speed.",
                        100D * clicked.getSpeedMultiplier(),
                        "%%"
                    ));
                }
            }
                ...
                if (ep.isSneaking()) {
                    ...
                    if (!world.isRemote)
                        ReikaChatHelper.sendChatToPlayer(
                            ep, "Mode is now " + clicked.getMode().label
                        );
                }
                return true;
            }
                ...
                if (id == Blocks.end_portal_frame) {
                    if (damage >= 4) {
                        ...
                        ReikaItemHelper.dropItem(
                            world, x + 0.5, y + 1, z + 0.5, new ItemStack(Items.ender_eye)
                        );
                    } else {
    ...
    @Override
    public boolean
    damage(ItemStack stack, int damage, EntityPlayer player, boolean simulated) {
        return true;
    }
}
