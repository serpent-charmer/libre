...
public class ItemJetPack
    extends ItemRotaryArmor implements Fillable, MultiLayerItemSprite, IArmorApiarist {
    ...
    public static enum PackUpgrades {
        ...
        public boolean existsOn(ItemStack is) {
            return is.stackTagCompound != null
                && is.stackTagCompound.getBoolean(this.getNBT());
        }
    ...
    public Fluid getFuelType() {
        return ConfigRegistry.JETFUELPACK.getState() ? FluidRegistry.getFluid("rc jet
    fuel") : FluidRegistry.getFluid("rc ethanol");
    }*/
    ...
    protected final ItemStack onSneakClicked(ItemStack is, EntityPlayer ep) {
        ...
        if (is.stackTagCompound == null)
            is.stackTagCompound = new NBTTagCompound();
        is.stackTagCompound.setBoolean(
            "wingon", !is.stackTagCompound.getBoolean("wingon")
        );
        return is;
    }

    private static final boolean wingEnabled(ItemStack is) {
        return PackUpgrades.WING.existsOn(is)
            && (is.stackTagCompound == null || is.stackTagCompound.getBoolean("wingon"));
    }
    ...
    public void onArmorTick(World world, EntityPlayer player, ItemStack is) {
        ...
        if (flying && fuel && world.difficultySetting != EnumDifficulty.PEACEFUL
    ...
    private boolean useJetpack(EntityPlayer ep, ItemStack is) {
        ...
        boolean horiz = KeyWatcher.instance.isKeyDown(ep, Key.FORWARD)
            || KeyWatcher.instance.isKeyDown(ep, Key.BACK);
        horiz = horiz || KeyWatcher.instance.isKeyDown(ep, Key.LEFT)
            || KeyWatcher.instance.isKeyDown(ep, Key.RIGHT);
        ...
        if (isFlying && canFly) {
            ...
            if (this.getFuel(is) > 0) {
                if (hoverMode) {
                    ...
                    double deltav = ep.motionY > 0
                        ? Math.min(0.2, Math.max(0.05, (maxSpeed - ep.motionY) * 0.25))
                        : 0.2;
                    if (jetbonus && !horiz) {
                        deltav *= 1.5;
                    }
                ...
                if (!ep.worldObj.isRemote) {
                    ...
                    if (ConfigRegistry.KICKFLYING.getState()) {
                        if (ep instanceof EntityPlayerMP) {
                            ((EntityPlayerMP) ep).playerNetServerHandler.floatingTickCount
                                = 0;
                        }
                    }
                }
                ...
                float pitch
                    = 1 + 0.5F * (float) Math.sin((ep.worldObj.getWorldTime() * 2) % 360);
                SoundRegistry.JETPACK.playSound(
                    ep.worldObj, ep.posX, ep.posY, ep.posZ, 0.75F, pitch
                );
                if (propel) {
                    SoundRegistry.SHORTJET.playSound(
                        ep.worldObj, ep.posX, ep.posY, ep.posZ, 0.15F, 1F
                    );
                }
            }
        }
    ...
    private void explode(World world, EntityPlayer player) {
        ItemStack to = this.isBedrock() ? ItemRegistry.BEDCHEST.getEnchantedStack()
            : this.isSteel()            ? ItemRegistry.STEELCHEST.getStackOf()
                                        : null;
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean par4) {
        ...
        li.add(
            ch > 0 ? String.format(
                "Fuel: %d mB of %s", ch, this.getCurrentFluid(is).getLocalizedName()
            )
                   : "No Fuel"
        );
    }
    ...
    ) //Adds the metadata blocks to the creative inventory
    {
        ...
        if (this.isBedrock()) {
            HashMap<Enchantment, Integer> ench
                = ((ItemBedrockArmor) ItemRegistry.BEDCHEST.getItemInstance())
                      .getDefaultEnchantments();
            ReikaEnchantmentHelper.applyEnchantments(is, ench);
        }
        ItemStack is2 = is.copy();
        ItemStack is3 = is.copy();
        Fluid f = ConfigRegistry.JETFUELPACK.getState()
            ? FluidRegistry.getFluid("rc jet fuel")
            : FluidRegistry.getFluid("rc ethanol");
    ...
    public double getDamageMultiplier(DamageSource src) {
        if (this.isBedrock())
            return ((ItemBedrockArmor) ItemRegistry.BEDCHEST.getItemInstance())
                .getDamageMultiplier(src);
        else if (this.isSteel())
            return ((ItemSteelArmor) ItemRegistry.STEELCHEST.getItemInstance())
                .getDamageMultiplier(src);
        else
            return 1;
    }
    ...
    public boolean getIsRepairable(ItemStack tool, ItemStack item) {
        return tool.getItem() == this && this.isSteel()
            && ReikaItemHelper.matchStacks(item, ItemStacks.steelingot);
    }

    @Override
    public boolean protectEntity(
        EntityLivingBase entity, ItemStack armor, String cause, boolean doProtect
    ) {
        ItemStack head = entity.getEquipmentInSlot(4);
        return head != null && head.getItem() instanceof IArmorApiarist
            && ((IArmorApiarist) head.getItem())
                   .protectEntity(entity, head, cause, doProtect);
    }

    @Override
    @Deprecated
    public boolean
    protectPlayer(EntityPlayer player, ItemStack armor, String cause, boolean doProtect) {
