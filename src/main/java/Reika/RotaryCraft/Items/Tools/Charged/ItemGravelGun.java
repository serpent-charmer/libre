...
public class ItemGravelGun extends ItemChargedTool {
        ...
        for (float i = 1; i <= 128; i += 0.5) {
            ...
            for (Entity e : li) {
                if (!this.isFiringPlayer(e, ep)) {
                    if (!ep.equals(e) && this.isEntityAttackable(e)
                        && ReikaWorldHelper.lineOfSight(world, ep, e)) {
                        infov.add(e);
                    }
                }
            }
            for (Entity ent : infov) {
                double dist = ReikaMathLibrary.py3d(
                    ep.posX - ent.posX, ep.posY - ent.posY, ep.posZ - ent.posZ
                );
                ...
                if (!world.isRemote) {
                    ...
                    if (is.getItemDamage()
                        > 4096) { //approx the 1-hit kill of a 10-heart mob
                        //ReikaPacketHelper.sendUpdatePacket(RotaryCraft.packetChannel,
                        //PacketRegistry.GRAVELGUN.getMinValue(), world, (int)ent.posX,
                        //(int)ent.posY, (int)ent.posZ); world.playSoundAtEntity(ep,
                        //"random.explode", 0.25F, 1F);
                    }
                    if (ent instanceof EntityDragon) {
                        EntityDragon ed = (EntityDragon) ent;
                        ed.attackEntityFromPart(
                            ed.dragonPartBody,
                            DamageSource.causePlayerDamage(ep),
                            this.getAttackDamage(is.getItemDamage())
                        );
                        ...
                        if (ent instanceof EntityPlayer) {
                            for (int n = 1; n < 5; n++) {
                                ItemRegistry ir
                                    = ItemRegistry.getEntry(e.getEquipmentInSlot(n));
                                if (ir != null) {
                    ...
                    if (ent instanceof EntityMob
                        && (ent.isDead || ((EntityLivingBase) ent).getHealth() <= 0)
                        && ReikaMathLibrary.py3d(
                               ep.posX - ent.posX, ep.posY - ent.posY, ep.posZ - ent.posZ
                           ) >= 80)
                        RotaryAchievements.GRAVELGUN.triggerAchievement(ep);
                }
                //ReikaWorldHelper.spawnParticleLine(world, x, y, z, ent.posX,
                //ent.posY+ent.height/2, ent.posZ, "crit", 0, 0, 0, 60);
                for (float t = 0; t < 2; t += 0.05F)
                    world.spawnParticle(
                        "crit", x, y, z, dx / dist * t, dy / dist * t, dz / dist * t
                    );
            }
            if (infov.size() > 1) {
            ...
            if (infov.size() > 0) {
                if (!ep.capabilities.isCreativeMode)
                    ReikaInventoryHelper.findAndDecrStack(
                        Blocks.gravel, -1, ep.inventory.mainInventory
                    );
                is.setItemDamage(
                    is.getItemDamage() - this.getChargeConsumed(is.getItemDamage())
                );
                return is;
            }
        }
    ...
    private boolean isFiringPlayer(Entity e, EntityPlayer ep) {
        return e instanceof EntityPlayer
            && (e.getCommandSenderName().equals(ep.getCommandSenderName()));
    }
    ...
    private float getAttackDamage(int charge) {
        ...
        return (float
           ...
           + (ReikaMathLibrary.logbase(pow, 2) / 2)
               * ReikaMathLibrary.doubpow(base, charge));
    }
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean par4) {
        float dmg = this.getAttackDamage(is.getItemDamage());
        String s = dmg > 0
            ? String.format("Dealing %.1f hearts of damage per shot", dmg / 2F)
            : "Unable to fire - requires charging";
        li.add(s);
    }
}
