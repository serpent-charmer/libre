...
public class ItemSpringBoots extends ItemChargedArmor implements IArmorApiarist {
    ...
    public void onArmorTick(World world, EntityPlayer ep, ItemStack is) {
        if (is.getItem() == ItemRegistry.BEDJUMP.getItemInstance()
            ...
            ep.stepHeight
                = ep.isSneaking() ? 0.5F : Math.max(ep.stepHeight, 1.45F); //1.25F
            if (itemRand.nextInt(3200) == 0) {
                if (is.getItem() != ItemRegistry.BEDJUMP.getItemInstance()) {
                    ep.setCurrentItemOrArmor(
                        1,
                        new ItemStack(is.getItem(), is.stackSize, is.getItemDamage() - 1)
                    );
                    this.warnCharge(is);
                }
            }
    ...
    ) //Adds the metadata blocks to the creative inventory
    {
        ...
        if (id == ItemRegistry.BEDJUMP.getItemInstance()) {
            HashMap<Enchantment, Integer> ench
                = ((ItemBedrockArmor) ItemRegistry.BEDBOOTS.getItemInstance())
    ...
    public static boolean isSpringBoots(ItemStack is) {
        return is.getItem() instanceof ItemSpringBoots
            || (ModList.CHROMATICRAFT.isLoaded() && checkFloatstoneBoots(is));
    }
