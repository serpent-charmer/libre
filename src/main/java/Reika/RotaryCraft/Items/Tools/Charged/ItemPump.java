...
public class ItemPump extends ItemChargedTool {
                    ...
                    if (ReikaWorldHelper.isLiquidSourceBlock(world, x, y, z)) {
                        ...
                        if (f != null && !world.isRemote) {
                            ...
                            RotaryCraft.logger.debug(
                                "Null fluid for block " + id + ", yet was marked as such!"
                            );
                        }
                    } else {
    ...
    private void drainLiquid(World world, int x, int y, int z, ItemStack is, Fluid f) {
        Fluid f2 = is.stackTagCompound == null
            ? null
                ...
                for (int i = r; i >= -r; i--) {
                    for (int k = r; k >= -r; k--) {
                        ...
                        if (ReikaWorldHelper.isLiquidSourceBlock(world, dx, dy, dz)) {
                            ...
                            if (is.stackTagCompound.getInteger("lvl")
    ...
    private void
    drainAndFill(World world, int x, int y, int z, ItemStack is, Fluid f, int amt) {
    ...
    public Result getEnchantValidity(Enchantment e, ItemStack is) {
        return e == Enchantment.aquaAffinity ? Result.ALLOW
    ...
    public boolean onItemUse(
        ...
        if (te instanceof IFluidHandler) {
            ...
            int amt
                = is.stackTagCompound != null ? is.stackTagCompound.getInteger("lvl") : 0;
            if (this.getMode(is) == Modes.PLACE) {
                Fluid f = is.stackTagCompound != null
                    ? ReikaNBTHelper.getFluidFromNBT(is.stackTagCompound)
                    : null;
                ...
                if (f2 != null) {
                    if (f == null || f == f2.getFluid()) {
                        ...
                        FluidStack fs
                            = fl.drain(ForgeDirection.VALID_DIRECTIONS[s], space, true);
                        if (fs != null) {
                            if (is.stackTagCompound == null)
                                is.stackTagCompound = new NBTTagCompound();
                            is.stackTagCompound.setInteger("lvl", amt + fs.amount);
                            ReikaNBTHelper.writeFluidToNBT(
                                is.stackTagCompound, fs.getFluid()
                            );
                        }
                    }
                }
                        ...
                        if (world.getBlock(dx, dy, dz).isAir(world, dx, dy, dz)
                            ...
                            for (int i = -1; i <= 1; i++) {
                                for (int k = -1; k <= 1; k++) {
                                    ...
                                    world.getBlock(dx + i, dy, dz + k)
                                        .onNeighborBlockChange(
                                            world, dx + i, dy, dz + k, b
                                        );
