...
public class ItemStunGun extends ItemChargedTool {
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        ...
        SoundRegistry.KNOCKBACK.playSound(
            world,
            ep.posX + norm.xCoord,
            ep.posY + norm.yCoord,
            ep.posZ + norm.zCoord,
            ...
            for (int k = 0; k < infov.size(); k++) {
                ...
                if (!(ent instanceof EntityPlayer) && ep.canEntityBeSeen(ent)) {
                    for (int f = 0; i < 64; i++)
                        world.spawnParticle(
                            "magicCrit",
                            ent.posX - 0.5 + par5Random.nextFloat(),
                            ent.posY - 0.5 + par5Random.nextFloat(),
                            ent.posZ - 0.5 + par5Random.nextFloat(),
                            -0.5 + par5Random.nextFloat(),
                            -0.5 + par5Random.nextFloat(),
                            -0.5 + par5Random.nextFloat()
                        );
                    ReikaEntityHelper.knockbackEntity(
                        ep,
                        ent,
                        2
                            * (1
                               + ReikaEnchantmentHelper.getEnchantmentLevel(
                                     Enchantment.knockback, is
                                 ) * 0.25)
                    );
                }
            }
            if (infov.size() > 0
                && !(infov.size() == 1 && infov.get(0) instanceof EntityPlayer))
    ...
    public boolean onItemUse(
        ...
        if (!world.isRemote) {
            ...
            if (id != Blocks.air
                ...
                && (id == Blocks.web || id == Blocks.red_mushroom || id == Blocks.gravel
                    || id == Blocks.monster_egg || id == Blocks.brown_mushroom
                    || id == Blocks.waterlily || id == Blocks.flower_pot
                    || ReikaBlockHelper.isOre(id, meta)
                    || (ReikaWorldHelper.softBlocks(world, x, y, z) && id != Blocks.snow)
                )) {
                for (int k = 0; k < 64; k++)
                    world.spawnParticle(
                        "magicCrit",
                        x + par5Random.nextFloat(),
                        y + par5Random.nextFloat(),
                ...
                ep.setCurrentItemOrArmor(
                    0, new ItemStack(is.getItem(), is.stackSize, is.getItemDamage() - 2)
                );
            }
            int leafrange = 4;
            if (mat == Material.glass || mat == Material.ice || mat == Material.leaves
                || id == Blocks.sand || id == Blocks.snow || id == Blocks.ice) {
                for (int k = 0; k < 64; k++)
                    world.spawnParticle(
                        "magicCrit",
                        x + par5Random.nextFloat(),
                ...
                ReikaWorldHelper.recursiveBreakWithinSphere(
                    world, x, y, z, id, -1, x, y, z, leafrange, fortune
                );
                ep.setCurrentItemOrArmor(
                    0, new ItemStack(is.getItem(), is.stackSize, is.getItemDamage() - 2)
                );
    ...
    public Result getEnchantValidity(Enchantment e, ItemStack is) {
        return e == Enchantment.fortune || e == Enchantment.knockback
            ? Result.ALLOW
            : super.getEnchantValidity(e, is);
    }
}
