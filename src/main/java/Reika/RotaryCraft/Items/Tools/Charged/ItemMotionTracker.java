...
public class ItemMotionTracker extends ItemChargedTool {
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        ...
        for (float i = 1; i <= 128; i += 0.5) {
            ...
            double dz = ep.posZ;
            //ReikaChatHelper.writeString(String.format("%.3f", look.xCoord)+"
            //"+String.format("%.3f", look.yCoord)+" "+String.format("%.3f",
            //look.zCoord));
            look.xCoord *= i;
            look.yCoord *= i;
            look.zCoord *= i;
            AxisAlignedBB fov = AxisAlignedBB.getBoundingBox(
                dx + look.xCoord - 0.5,
                dy + look.yCoord - 0.5,
                dz + look.zCoord - 0.5,
                dx + look.xCoord + 0.5,
            ...
            if (infov.size() > 0) {
                ...
                for (int k = 0; k < infov.size(); k++) {
                    ...
                    if (!(ent instanceof EntityPlayer)
                        && (dist <= 32 || ent instanceof EntityWither
                            || ent instanceof EntityDragon)
                        && !(lastmobname == mobname && dist == lastdist)) {
                        ReikaChatHelper.write(
                            color + mobname + EnumChatFormatting.WHITE
                            + String.format(" %.2f", dist - 1) + "m away."
                        );
                        if (ent instanceof EntityDragon) {
                            ...
                            if (ReikaMathLibrary.approxr(ed.targetX, ep.posX, 4))
                                if (ReikaMathLibrary.approxr(ed.targetY, ep.posY, 4))
                                    if (ReikaMathLibrary.approxr(ed.targetZ, ep.posZ, 4))
                                        ReikaChatHelper.writeFormattedString(
                                            "Dragon is Attacking!", EnumChatFormatting.RED
                                        );
                            ...
                            if (em.getAttackTarget() == ep || em.getAITarget() == ep)
                                ReikaChatHelper.writeFormattedString(
                                    "Mob is Attacking!", EnumChatFormatting.RED
                                );
                        }
                    }
                    /*double[] vel = ReikaPhysicsHelper.cartesianToPolar(ent.motionX,
                    ent.motionY, ent.motionZ); ReikaChatHelper.write(String.format("
                    %.2f", vel[0])+"  "+String.format(" %.2f", vel[1])+" "+String.format("
                    %.2f", vel[2])); ReikaChatHelper.write("�4"+String.format(" %.2f",
                    ReikaPhysicsHelper.cartesianToPolar(dx, dy, dz)[0])+"
                    "+String.format(" %.2f", ReikaPhysicsHelper.cartesianToPolar(dx, dy,
                    dz)[1])+"  "+String.format(" %.2f",
                    ReikaPhysicsHelper.cartesianToPolar(dx, dy, dz)[2])); if (vel[1] ==
                    ReikaPhysicsHelper.cartesianToPolar(dx, dy, dz)[1]) { if (vel[2] ==
                    ReikaPhysicsHelper.cartesianToPolar(dx, dy, dz)[2]) { if (ent
                    instanceof EntityDragon || ent instanceof EntityWither) color = "�4";
                            else if (ent instanceof EntityMob)
                                color = "�e";
                            else
                                color = "�f";
                            ReikaChatHelper.write(String.format("%s", color)+"Mob is
                    approaching you!");
                        }
                    }*/
