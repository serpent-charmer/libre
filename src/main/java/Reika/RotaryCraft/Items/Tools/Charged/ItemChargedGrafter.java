...
public class ItemChargedGrafter extends ItemChargedTool implements IToolGrafter {
    ...
    /*
    @Override
    public boolean onBlockDestroyed(ItemStack is, World world, Block blockID, int x, int
    y, int z, EntityLivingBase e)
    {
                    ...
                    for (int j = -r; j <= r; j++) {
                        for (int k = -r; k <= r; k++) {
                            ...
                            if (b2 != null && b2.getMaterial() == Material.leaves) {
                                b2.dropBlockAsItem(world, dx, dy, dz,
    world.getBlockMetadata(dx, dy, dz), 0); b2.removedByPlayer(world, ep, dx, dy, dz);
                                is.setItemDamage(is.getItemDamage()-1);
                            }
                        }
                    }
