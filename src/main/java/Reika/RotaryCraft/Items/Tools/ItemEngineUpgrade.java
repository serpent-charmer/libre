...
public class ItemEngineUpgrade extends ItemRotaryTool {
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        li.addAll(Minecraft.getMinecraft().fontRenderer.listFormattedStringToWidth(
            Upgrades.list[is.getItemDamage()].getDescription(), 170
        ));
    ...
    public static enum Upgrades {
        ...
        public String getDescription() {
            switch (this) {
                case PERFORMANCE:
                    return "Upgrades "
                        + RotaryNames.getEngineName(EngineType.GAS.ordinal()) + "s to "
                        + RotaryNames.getEngineName(EngineType.SPORT.ordinal()) + "s";
                case AFTERBURNER:
                    return "Adds afterburner capability to the "
                        + RotaryNames.getEngineName(EngineType.JET.ordinal());
                case FLUX:
                    return "Improves energy efficiency/capacity on shaft-to-electromagnetic converter engines";
                case LODESTONE:
                    return "Increases " + MachineRegistry.MAGNETIZER.getName()
                        + " effectivity";
                case REDSTONE:
                ...
                case MAGNETOSTATIC4:
                case MAGNETOSTATIC5:
                    return "Converter engine upgrade, tier "
