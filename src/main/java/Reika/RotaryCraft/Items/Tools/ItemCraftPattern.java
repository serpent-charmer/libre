...
public class ItemCraftPattern extends ItemRotaryTool implements SpriteRenderCallback {
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        if (ep.isSneaking()) {
            ...
            ep.openGui(
                RotaryCraft.instance, GuiRegistry.PATTERN.ordinal(), world, 0, 0, 0
            );
        }
        return is;
    }
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean par4) {
        ...
        if (is.stackTagCompound == null) {
            ...
            if (item != null) {
                li.add(
                    "Crafts " + item.stackSize + " " + item.getDisplayName()
                ); //+" with:");
            } else {
    ...
    public static ItemStack getRecipeOutput(ItemStack is) {
        ItemStack item = is.stackTagCompound != null
            ? ItemStack.loadItemStackFromNBT(is.stackTagCompound.getCompoundTag("output"))
            : null;
        ...
        if (is.stackTagCompound != null) {
            ...
            for (int i = 0; i < 9; i++) {
                ...
                if (recipe.hasKey(s)) {
                    ...
                    if (in == null && tag != null
                        && !tag.hasNoTags()) { //item no longer exists, clear the pattern
                        is.stackTagCompound = null;
                        return null;
                    }
                    items[i] = in;
    ...
    public static RecipeMode getMode(ItemStack is) {
        return is.stackTagCompound != null
            ? RecipeMode.list[is.stackTagCompound.getInteger("mode")]
            : RecipeMode.CRAFTING;
    }
    ...
    public static void changeStackLimit(ItemStack is, int change) {
        ...
        is.stackTagCompound.setInteger(
            "stacklimit", MathHelper.clamp_int(limit + change, 1, 64)
        );
    }
    ...
    public static enum RecipeMode {
        ...
        BLASTFURN(
            "Blast Furnace Crafting", MachineRegistry.BLASTFURNACE.getCraftedProduct()
        );
        ...
        public ItemStack getRecipe(InventoryCrafting ic, World world) {
            switch (this) {
                case CRAFTING:
                    ...
                    return null;
                case BLASTFURN:
                    for (BlastCrafting ir :
                         RecipesBlastFurnace.getRecipes().getAllCraftingRecipes()) {
                ...
                case WORKTABLE:
                    WorktableRecipe wr
                        = WorktableRecipes.getInstance().findMatchingRecipe(ic, null);
    ...
    public static boolean checkPatternForMatch(
        IInventory te,
        RecipeMode type,
        int invslot,
        int patternslot,
        ...
        return getMode(p) == type
            && checkItemAndSize(patternslot, is, p, in != null ? in.stackSize : 0);
    }

    private static boolean
    checkItemAndSize(int slot, ItemStack is, ItemStack p, int current) {
        return ReikaItemHelper.matchStacks(is, getItems(p)[slot])
            && current + is.stackSize
            <= Math.min(is.getMaxStackSize(), getStackInputLimit(p));
    }
}
