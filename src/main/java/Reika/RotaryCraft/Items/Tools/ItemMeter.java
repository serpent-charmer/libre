...
public class ItemMeter extends ItemRotaryTool {
    ...
    public boolean onItemUse(
        ...
        String name = tile != null
            ? (tile instanceof TileEntityBase ? ((TileEntityBase) tile).getName()
        ...
        if (tile instanceof ThermalTile) {
            ...
            this.sendMessage(
                ...
                String.format(
                    ...
                    RotaryAux.formatTemperature(th.getTemperature())
                )
            );
        }
        ...
        if (tile instanceof PressureTile) {
            ...
            this.sendMessage(
                ...
                String.format(
                    ...
                    name,
                    Variables.PRESSURE,
                    RotaryAux.formatPressure(th.getPressure())
                )
            );
        }
        ...
        if (tile instanceof RangedEffect) {
            ...
            this.sendMessage(
                ep,
                Variables.RANGE + ": " + RotaryAux.formatDistance(te.getRange()) + "; "
                    + "Max Range: " + RotaryAux.formatDistance(te.getMaxRange())
            );
        }
        ...
        if (tile instanceof DiscreteFunction) {
            this.sendMessage(
                ep,
                Variables.OPERATIONTIME + ": "
                    + ((DiscreteFunction) tile).getOperationTime() / 20F + " s"
            );
        }
        ...
        if (tile instanceof IntegratedGearboxable) {
            ...
            if (ratio != 0) {
                this.sendMessage(
                    ep,
                    "Integrated Gearbox: " + Math.abs(ratio) + " x, "
                        + (ratio > 0 ? "Torque" : "Speed")
                );
            }
        }
        ...
        if (tile instanceof PowerSourceTracker) {
            ...
            this.sendMessages(
                ep,
                ((PowerSourceTracker) tile)
                    .getPowerSources((PowerSourceTracker) tile, null)
                    .getMessages()
            );
        }
        ...
        if (tile instanceof TileEntityEngine) {
            ...
            long power = te.power;
            this.sendMessage(
                ep, String.format("%s producing %s", name, RotaryAux.formatPowerIO(te))
            );
            ...
            if (te.getEngineType() == EngineType.JET) {
                if (((TileEntityJetEngine) te)
                        .getChokedFraction(world, x, y, z, te.getBlockMetadata())
                    < 1)
            ...
            if (te.getEngineType().burnsFuel()) {
                ...
                String sg = String.format(
                    "%s: %s", Variables.FUEL, ReikaDateHelper.getSecondsAsClock(time)
                );
            ...
            if (te.getEngineType().requiresLubricant()) {
                int amt = te.getLube();
                String sg
                    = String.format("Lubricant: %s", RotaryAux.formatLiquidAmount(amt));
                this.sendMessage(ep, sg);
            }
            ...
            if (te.getEngineType() == EngineType.HYDRO) {
                if (((TileEntityHydroEngine) te).isStreamPowered()) {
                    ...
                    String sg = String.format(
                        "Detected waterfall height: %s",
                        RotaryAux.formatDistance(
                            ((TileEntityHydroEngine) te).getWaterfallHeightForDisplay()
                        )
                    );
                    this.sendMessage(ep, sg);
                }
            ...
            this.sendMessage(
                ep, String.format("%s producing %s", name, RotaryAux.formatPowerIO(sp))
            );
        }
        ...
        if (tile instanceof TileEntityPowerReceiver) {
            ...
            long power = te.power;
            this.sendMessage(
                ep, String.format("%s receiving %s", name, RotaryAux.formatPowerIO(te))
            );
            ...
            this.sendMessage(
                ep, String.format("%s receiving %s", name, RotaryAux.formatPowerIO(sp))
            );
        }
        ...
        if (m == MachineRegistry.DISTRIBCLUTCH) {
            ...
            if (read != null) {
                ...
                this.sendMessage(
                    ep,
                    String.format(
                        "Input side %s receiving %s",
                        sname,
                        RotaryAux.formatPowerIO(te.omega, pwr)
                    )
                );
            }
            for (int i = 0; i < 4; i++) {
                ...
                if (te.isOutputtingToSide(dir)) {
                    ...
                    if (dir == read.getOpposite()) {
                        this.sendMessage(
                            ep,
                            String.format(
                                "Output to side %s: %s (leftover)",
                                sname,
                                RotaryAux.formatPowerIO(te.omega, out)
                            )
                        );
                    } else {
                        ...
                        this.sendMessage(
                            ep,
                            String.format(
                                "Output to side %s: %s (requested %.3f%sNm)",
                                sname,
                                RotaryAux.formatPowerIO(te.omega, out),
                                base,
                                pre
            ...
            if (te.isSplitting()) {
                String sname
                    = ReikaStringParser.capFirstChar(te.getReadDirection().name());
            ...
            this.sendMessage(
                ep, String.format("%s transmitting %s", name, RotaryAux.formatPowerIO(te))
            );
        }
        ...
        if (tile instanceof EnergyToPowerBase) {
            ...
            String unit = te.getUnitDisplay();
            this.sendMessage(
                ep, String.format("%s Outputting %s", name, RotaryAux.formatPowerIO(te))
            );
        ...
        if (tile instanceof TileEntityPiping) {
            ...
            if (f != null)
                this.sendMessage(
                    ...
                    String.format(
                        "Pipe has %s of %s",
                        RotaryAux.formatLiquidAmount(te.getFluidLevel()),
                        f.getLocalizedName()
                    )
                );
            else
        ...
        if (tile instanceof TileEntitySpringPowered) {
            ...
            this.sendMessage(
                ep,
                String.format("Remaining charge: %.2fs", te.getExpectedCoilLife() / 20D)
            );
        }
        ...
        if (m == MachineRegistry.GPR) {
            ...
            if (ep.isSneaking() && te.power > te.MINPOWER) {
                ...
                this.sendMessage(
                    ep, String.format("The ground is %.3f%s caves here", ratio, "%%")
                );
            }
        }
        ...
        if (m == MachineRegistry.PLAYERDETECTOR) {
            ...
            this.sendMessage(
                ep, String.format("Reaction Time: %.2fs", te.getReactionTime() / 20F)
            );
        }
        if (m == MachineRegistry.SOLARTOWER) {
            TileEntitySolar te = (TileEntitySolar) tile;
            TileEntitySolar top
                = (TileEntitySolar) world.getTileEntity(x, te.getTopOfTower(), z);
            TileEntitySolar bottom
                = (TileEntitySolar) world.getTileEntity(x, te.getBottomOfTower(), z);
            if (bottom.getPlant() == null || bottom.getArraySize() <= 0) {
                ...
                this.sendMessage(
                    ep,
                    String.format(
                        "Solar plant contains %d mirrors and %d active tower pieces (out of %d total)",
                        top.getArraySize(),
                        bottom.getPlant().getEffectiveTowerBlocks(),
                        bottom.getPlant().rawTowerBlocks()
                    )
                );
                this.sendMessage(
                    ep,
                    String.format(
                        "Outputting %s; Efficiency %.1f%s",
                        RotaryAux.formatPowerIO(bottom),
                        bottom.getArrayOverallBrightness() * 100F,
                        "%%"
                    )
                );
            }
        ...
        if (m == MachineRegistry.HEATRAY) {
            ...
            if (te.power >= te.MINPOWER)
                this.sendMessage(
                    ep, String.format("Dealing %ds of burn damage", te.getBurnTime())
                );
        }
        if (m == MachineRegistry.PUMP) {
            ...
            if (te.power >= te.MINPOWER && te.duplicationAmount > 1)
                this.sendMessage(
                    ep,
                    String.format(
                        "Duplicating water with a factor of %dx", te.duplicationAmount
                    )
                );
        }
        if (m == MachineRegistry.ADVANCEDGEARS) {
            TileEntityAdvancedGear te = (TileEntityAdvancedGear) tile;
            if (te.getGearType() == TileEntityAdvancedGear.GearType.WORM
                && te.power > 0) {
                this.sendMessage(
                    ep, String.format("Power Loss: %.2f%s", te.getCurrentLoss(), "%%")
                );
            } else if (te.getGearType().storesEnergy()) {
                long energy = te.getEnergy() / 20;
                this.sendMessage(
                    ep, String.format("Stored Energy: %s", RotaryAux.formatEnergy(energy))
                );
            }
        }
        if (m == MachineRegistry.BEVELGEARS) {
            ...
            this.sendMessage(
                ...
                String.format(
                    "Output side: %s",
                    ReikaStringParser.capFirstChar(te.getWriteDirection().name())
                )
            );
        }
        if (m == MachineRegistry.BORER) {
            ...
            this.sendMessage(
                ep,
                String.format(
                    "%s head at %d, %d", te.getName(), te.getHeadX(), te.getHeadZ()
                )
            );
            if (te.isJammed())
                this.sendMessage(
                    ep,
                    String.format(
                        "%s is jammed, supply more torque or power!", te.getName()
                    )
                );
        }
        ...
        if (m == MachineRegistry.DYNAMO) {
            ...
            if ((te.torque > (te.isUpgraded() ? te.MAXTORQUE_UPGRADE : te.MAXTORQUE)
                 || te.omega > te.MAXOMEGA))
        ...
        if (m == MachineRegistry.GEARBOX) {
            ...
            this.sendMessage(
                ep,
                String.format(
                    "Gearbox %d percent damaged; Lubricant Levels at %s",
                    (int) (100 * (1 - ReikaMathLibrary.doubpow(0.99, te.getDamage()))),
                    RotaryAux.formatLiquidAmount(te.getLubricant())
                )
            );
        }
        if (m == MachineRegistry.FRACTIONATOR) {
            ...
            if (te.power >= te.MINPOWER && te.omega >= te.MINSPEED)
                this.sendMessage(
                    ep, String.format("Efficiency: %.3f%%%%", te.getYieldRatio() * 100)
                );
        }

        if (m == null && tile instanceof IFluidHandler) {
            FluidTankInfo[] info
                = ((IFluidHandler) tile).getTankInfo(ForgeDirection.VALID_DIRECTIONS[s]);
                ...
                for (int i = 0; i < info.length; i++) {
                    if (info[i] != null) {
                        ...
                        if (fs != null && fs.getFluid() != null)
                            sb.append(
                                RotaryAux.formatLiquidAmount(fs.amount) + " of "
                                + fs.getFluid().getLocalizedName() + "/"
                                + RotaryAux.formatLiquidAmount(info[i].capacity)
                                + " Capacity"
                            );
                        else
                            sb.append("Empty");
