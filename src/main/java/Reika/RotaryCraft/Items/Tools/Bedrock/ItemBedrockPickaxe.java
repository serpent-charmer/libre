...
public final class ItemBedrockPickaxe extends ItemPickaxe implements IndexedItemSprites {
    ...
    private void forceSilkTouch(ItemStack is, World world, Entity entity, int slot) {
        if (!ReikaEnchantmentHelper.hasEnchantment(Enchantment.silkTouch, is)
            || ReikaEnchantmentHelper.getEnchantmentLevel(Enchantment.fortune, is) < 5) {
    ...
    public int getHarvestLevel(ItemStack stack, String toolClass) {
        return toolClass == null || toolClass.toLowerCase(Locale.ENGLISH).contains("pick")
            ? Integer.MAX_VALUE
    ...
    public boolean onBlockStartBreak(ItemStack is, int x, int y, int z, EntityPlayer ep) {
        ...
        if (ConfigRegistry.MODORES.getState()) {
            if (ModList.THAUMCRAFT.isLoaded()
                && ThaumOreHandler.getInstance().isThaumOre(block)) {
                this.dropDirectBlock(block, world, x, y, z);
                return true;
            }
            if (ModList.DARTCRAFT.isLoaded()
                && DartOreHandler.getInstance().isDartOre(block)) {
                this.dropDirectBlock(block, world, x, y, z);
                return true;
            }
            if (ModList.TRANSITIONAL.isLoaded()
                && TransitionalOreHandler.getInstance().isMagmaniteOre(block)) {
                this.dropDirectBlock(block, world, x, y, z);
                return true;
            }
            if (ModList.MAGICCROPS.isLoaded()
                && MagicCropHandler.getInstance().isEssenceOre(id)) {
                this.dropDirectBlock(block, world, x, y, z);
                return true;
            }
            if (ModList.HEXCRAFT.isLoaded()
                && HexcraftHandler.getActiveHandler().isMonolith(id)) {
                this.dropDirectBlock(block, world, x, y, z);
                return true;
            }
        }
        ...
        if (ConfigRegistry.FAKEBEDROCK.getState() || !ReikaPlayerAPI.isFake(ep)) {
            if (ConfigRegistry.BEDPICKSPAWNERS.getState() && id == Blocks.mob_spawner) {
                ...
                if (ReikaSpawnerHelper.hasCustomLogic(spw)
                    && this.shouldKeepSpawnerLogic(is))
                    ReikaSpawnerHelper.setSpawnerItemNBT(
                        item, spw.func_145881_a(), true, true
                    );
                ReikaItemHelper.dropItem(
                    world,
                    x + itemRand.nextDouble(),
                    y + itemRand.nextDouble(),
                ...
                if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT) {
                    ; //ReikaRenderHelper.spawnDropParticles(world, x, y, z,
                      //Blocks.mob_spawner, meta);
                }
                return false;
            }
        }
        ...
        if (id == Blocks.monster_egg) {
            ...
            if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT) {
                ReikaRenderHelper.spawnDropParticles(
                    world, x, y, z, Blocks.monster_egg, meta
                );
            }
            ItemStack drop
                = new ItemStack(ReikaBlockHelper.getSilverfishImitatedBlock(meta), 1, 0);
            ReikaItemHelper.dropItem(
                world,
                x + itemRand.nextDouble(),
                y + itemRand.nextDouble(),
            ...
            ReikaWorldHelper.splitAndSpawnXP(
                world, x + 0.5F, y + 0.125F, z + 0.5F, si.experienceValue
            );
            return true;
            ...
            if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT) {
                ReikaRenderHelper.spawnDropParticles(
                    world, x, y, z, Blocks.netherrack, meta
                );
            }
            ReikaItemHelper.dropItem(
                world, x + 0.5, y + 0.5, z + 0.5, new ItemStack(Blocks.netherrack)
            );
            ...
            for (EntityLiving e : li) {
                if (e instanceof EntitySilverfish) {
                    ...
                    ReikaWorldHelper.splitAndSpawnXP(
                        world, x + 0.5F, y + 0.125F, z + 0.5F, e.experienceValue
                    );
                }
            }
        }
    ...
    private boolean hasDataKeep(ItemStack is) {
        return ReikaEnchantmentHelper.hasEnchantment(
            ChromaEnchants.DATAKEEP.getEnchantment(), is
        );
    }
    ...
    public float getDigSpeed(ItemStack is, Block b, int meta) {
        ...
        if (ModOreList.getModOreFromOre(b, meta) == ModOreList.MIMICHITE)
            return 64F;
        if (ModList.HEXCRAFT.isLoaded()
            && HexcraftHandler.getActiveHandler().isMonolith(b))
            return 64;
        if (b.getClass().getSimpleName().equalsIgnoreCase("BlockConduitFacade")
            || b.getClass().getSimpleName().equalsIgnoreCase("BlockConduitBundle"))
            return 24F;
        ...
        if (field_150914_c.contains(b))
            return 12F;
        if (((ItemBedrockShovel) ItemRegistry.BEDSHOVEL.getItemInstance())
                .isAcceleratedOn(b))
            return 6F;
    ...
    public final IIcon getIconFromDamage(int dmg) { //To get around a bug in backtools
        return RotaryCraft.instance.isLocked() ? ReikaTextureHelper.getMissingIcon()
                                               : Items.stone_pickaxe.getIconFromDamage(0);
    }

    @Override
    public int getItemEnchantability() {
        return ConfigRegistry.PREENCHANT.getState()
            ? 0
            : Items.iron_pickaxe.getItemEnchantability();
    }
