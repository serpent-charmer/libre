    ...
    public boolean onItemUse(
        ...
        for (int i = -r; i <= r; i++) {
            for (int j = -r; j <= r; j++) {
                ...
                if (!ep.isSneaking()) {
                    boolean flag2
                        = super.onItemUse(is, ep, world, dx, y, dz, s, ax, bx, cx);
                    flag = flag2 || flag;
                    if (flag2 && world.getBlock(dx, y, dz) == Blocks.farmland) {
                        world.setBlockMetadataWithNotify(dx, y, dz, 2, 3);
                        //ReikaItemHelper.dropItem(world, dx+0.5, y+1.2, dz+0.5, new
                                ...
                                if (id != Blocks.air) {
                                    if (id == Blocks.dirt || id == Blocks.farmland
                                        ...
                                        ReikaSoundHelper.playStepSound(
                                            world, dx, y, dz, Blocks.grass, 0.4F, 1
                                        );
                                    }
                                }
                            }
