...
public class ItemBedrockSword extends ItemSword implements IndexedItemSprites {
    ...
    @Override
    public boolean
    hitEntity(ItemStack is, EntityLivingBase target, EntityLivingBase player) {
        ReikaEntityHelper.damageArmor(target, 100);
        if (player instanceof EntityPlayer
            && (ConfigRegistry.FAKEBEDROCK.getState()
                || !ReikaPlayerAPI.isFake((EntityPlayer) player))) {
            if (target.isDead || target.getHealth() <= 0) {
                if (itemRand.nextInt(5) == 0) {
                    ReikaEntityHelper.dropHead(target);
                }
                if (target instanceof EntityLiving) {
                    int xp = ((EntityLiving) target).experienceValue
                        * ReikaRandomHelper.getRandomBetween(1, 10);
                    ReikaWorldHelper.splitAndSpawnXP(
                        target.worldObj,
                        target.posX,
                        target.posY + itemRand.nextDouble() * target.height,
                        target.posZ,
                        xp
                    );
                }
    ...
    public Multimap getItemAttributeModifiers() {
        ...
        boolean flag = multimap.remove(
            SharedMonsterAttributes.attackDamage.getAttributeUnlocalizedName(), 9
        );
        multimap.put(
            SharedMonsterAttributes.attackDamage.getAttributeUnlocalizedName(),
            new AttributeModifier(
                field_111210_e, "Weapon modifier", this.func_150931_i(), 0
            )
        );
        return multimap;
    }
    ...
    private void forceEnchants(ItemStack is, World world, Entity entity, int slot) {
        if (!ReikaEnchantmentHelper.hasEnchantment(Enchantment.looting, is)
            || !ReikaEnchantmentHelper.hasEnchantment(Enchantment.sharpness, is)) {
    ...
    public final IIcon getIconFromDamage(int dmg) { //To get around a bug in backtools
        return RotaryCraft.instance.isLocked() ? ReikaTextureHelper.getMissingIcon()
                                               : Items.stone_sword.getIconFromDamage(0);
    }
