...
public class ItemBedrockShovel extends ItemSpade implements IndexedItemSprites {
    ...
    public ItemBedrockShovel(int tex) {
        ...
        // par3ToolMaterial.getEfficiencyOnProperMaterial();
        damageVsEntity = 4;
        this.setNoRepair();
    ...
    public int getHarvestLevel(ItemStack stack, String toolClass) {
        return toolClass == null
                || toolClass.toLowerCase(Locale.ENGLISH).contains("shovel")
                || toolClass.toLowerCase(Locale.ENGLISH).contains("spade")
            ? Integer.MAX_VALUE
    ...
    public boolean onBlockStartBreak(ItemStack is, int x, int y, int z, EntityPlayer ep) {
        if (ConfigRegistry.FAKEBEDROCK.getState() || !ReikaPlayerAPI.isFake(ep)) {
            ChancedOutputList co = extraDrops.get(
                ep.worldObj.getBlock(x, y, z), ep.worldObj.getBlockMetadata(x, y, z)
            );
            if (co != null) {
                double mult = Math.sqrt(
                    1
                    + EnchantmentHelper.getEnchantmentLevel(
                        Enchantment.fortune.effectId, is
                    )
                );
                Collection<ItemStack> c = co.calculate(mult);
                for (ItemStack drop : c) {
                    ReikaItemHelper.dropItem(
                        ep.worldObj, x + 0.5, y + 0.5, z + 0.5, drop
                    );
                }
            }
        }
    ...
    public float getDigSpeed(ItemStack is, Block b, int meta) {
        ...
        if (ModList.TINKERER.isLoaded()
            && b == TinkerBlockHandler.getInstance().gravelOreID)
            return 36F;
    ...
    public boolean isAcceleratedOn(Block b) {
        return field_150914_c.contains(b) || b.getMaterial() == Material.grass
            || b.getMaterial() == Material.ground || b.getMaterial() == Material.sand;
    }
    ...
    public final IIcon getIconFromDamage(int dmg) {
        return RotaryCraft.instance.isLocked() ? ReikaTextureHelper.getMissingIcon()
                                               : Items.stone_shovel.getIconFromDamage(0);
    }
