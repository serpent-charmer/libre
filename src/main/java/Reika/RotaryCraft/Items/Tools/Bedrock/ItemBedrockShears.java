...
public class ItemBedrockShears extends ItemRotaryShears {
    ...
    @Override
    public boolean
    onBlockStartBreak(ItemStack is, int x, int y, int z, EntityPlayer player) {
        if (player.worldObj.isRemote)
            ...
            if (drop) {
                ...
                ReikaItemHelper.dropItem(
                    player.worldObj, x + 0.5, y + 0.5, z + 0.5, block
                );
    ...
    public static Result getHarvestResult(
        Block b, int meta, EntityPlayer player, World world, int x, int y, int z
    ) {
        if (b instanceof BlockTieredResource) {
            ...
            ((IShearable) b)
                .onSheared(player.getCurrentEquippedItem(), world, x, y, z, 0);
            return Result.ALLOW;
    ...
    public static int getDroppedMeta(Block id, int meta) {
        ...
        if (ModList.CHROMATICRAFT.isLoaded()
            && id == ChromatiAPI.getAPI().trees().getRainbowLeaf())
            return 0;
        if (id == Blocks.vine)
            return 0;
        ...
        if (id == Blocks.sapling)
            return meta & 3;
        if (id.getClass().getName().equals(
                "vazkii.botania.common.block.BlockModDoubleFlower"
            )) {
            meta &= 7;
        ...
        if (entity instanceof IShearable) {
            ...
            if (target.isShearable(itemstack, entity.worldObj, x, y, z)) {
                ...
                ArrayList<ItemStack> drops
                    = target.onSheared(itemstack, entity.worldObj, x, y, z, fortune);

                if (ConfigRegistry.FAKEBEDROCK.getState()
                    || !ReikaPlayerAPI.isFake(player)) {
                    for (ItemStack stack : drops) {
                        stack.stackSize *= 2;
                    }
                }
                ReikaItemHelper.dropItems(
                    entity.worldObj, x + 0.5, y + 0.8, z + 0.5, drops
                );
            }
            return true;
        }
