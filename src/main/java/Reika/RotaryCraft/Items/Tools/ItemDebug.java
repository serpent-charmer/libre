...
public class ItemDebug extends ItemRotaryTool {
    ...
    public boolean onItemUse(
        ...
        if (!ep.isSneaking()) {
            ...
            if (te instanceof RotaryCraftTileEntity)
                ReikaChatHelper.write(
                    "Tile Entity Direction Data: "
                    + (((RotaryCraftTileEntity) te).getBlockMetadata() + 1) + " of "
                    + ((RotaryCraftTileEntity) te).getTile().getNumberDirections()
                );
            else if (te instanceof TileEntityBase)
                ReikaChatHelper.write(
                    "Tile Entity Direction Data: "
                    + (((TileEntityBase) te).getBlockMetadata())
                );
        ...
        if (m == MachineRegistry.BLASTFURNACE) {
            ...
            if (tile != null) {
                ReikaChatHelper.write(
                    String.format("Temperature: %dC", tile.getTemperature())
                );
                if (ep.isSneaking()) {
        ...
        if (m == MachineRegistry.BELT) {
            ...
            if (tile != null) {
                ReikaChatHelper.write(
                    tile.getDistanceToTarget() + " @ " + tile.getBeltDirection()
                );
            }
        }
        ...
        if (m != null && m.isStandardPipe()) {
            ...
            if (tile != null) {
                if (tile.getFluidType() != null)
                    ReikaChatHelper.write(String.format(
                        "%s  %d  %d",
                        tile.getFluidType().getLocalizedName(),
                        tile.getFluidLevel(),
                        tile.getPressure()
                    ));
                else
        ...
        if (m == MachineRegistry.PUMP) {
            ...
            if (tile != null) {
                ReikaChatHelper.write(String.format(
                    "%s  %d",
                    tile.getLevel() <= 0 ? 0 : tile.getLiquid().getLocalizedName(),
                    tile.getLevel()
                ));
            }
        }
        if (m == MachineRegistry.RESERVOIR) {
            ...
            if (ep.isSneaking())
                ...
                ReikaChatHelper.write(String.format(
                    "%s  %d", tile.getFluid().getLocalizedName(), tile.getLevel()
                ));
            }
        }
        ...
        if (m == MachineRegistry.SPRINKLER) {
            ...
            if (tile != null) {
                ReikaChatHelper.write(
                    String.format("%d  %d", tile.getWater(), tile.getPressure())
                );
            }
        }
        if (m == MachineRegistry.OBSIDIAN) {
            ...
            if (tile != null) {
                ReikaChatHelper.write(String.format(
                    "%d  %d  %d", tile.getWater(), tile.getLava(), tile.temperature
                ));
            }
            if (ep.isSneaking()) {
        ...
        if (m == MachineRegistry.PULSEJET) {
            ...
            if (tile != null) {
                ReikaChatHelper.write(String.format(
                    "%d  %d  %d", tile.getWater(), tile.temperature, tile.getFuel()
                ));
                if (ep.isSneaking()) {
        ...
        if (m == MachineRegistry.ENGINE) {
            ...
            if (tile != null) {
                ReikaChatHelper.write(
                    String.format("%d  %d", tile.getWater(), tile.temperature)
                );
            }
            if (ep.isSneaking()) {
