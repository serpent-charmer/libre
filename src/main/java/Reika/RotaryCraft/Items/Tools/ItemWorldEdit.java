...
public class ItemWorldEdit extends ItemRotaryTool {
    ...
    public boolean onItemUse(
        ...
        MovingObjectPosition mov
            = new MovingObjectPosition(x, y, z, side, ep.getLookVec());
        if (!WorldEditHelper.hasPlayer(ep)) {
            ReikaChatHelper.write(
                "Player " + ep.getCommandSenderName() + " has no chosen block!"
            );
            return false;
        }
        ...
        if (ep.isSneaking()) {
            ...
            ReikaChatHelper.write(
                "Position 2 set to " + is.stackTagCompound.getInteger("ex") + ", "
            ...
            ReikaChatHelper.write(
                "Position 1 set to " + is.stackTagCompound.getInteger("sx") + ", "
        ...
        int[] s = { is.stackTagCompound.getInteger("sx"),
                    is.stackTagCompound.getInteger("sy"),
                    is.stackTagCompound.getInteger("sz") };
        int[] e = { is.stackTagCompound.getInteger("ex"),
                    is.stackTagCompound.getInteger("ey"),
                    is.stackTagCompound.getInteger("ez") };
        if (s[0] != Integer.MIN_VALUE && s[1] != Integer.MIN_VALUE
            && s[2] != Integer.MIN_VALUE) {
            if (e[0] != Integer.MIN_VALUE && e[1] != Integer.MIN_VALUE
                ...
                if (s[0] != e[0] || s[1] != e[1] || s[2] != e[2]) {
                    ...
                    ReikaChatHelper.write(
                        ...
                        + " blocks being changed to " + name + " (ID " + id
                        + ") with Metadata " + meta
                    );
                    for (int m = 0; m < 3; m++) {
                        if (s[m] > e[m]) {
