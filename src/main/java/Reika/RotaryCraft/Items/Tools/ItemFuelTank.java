...
public class ItemFuelTank extends ItemRotaryTool implements Fillable {
    ...
    public boolean isValidFluid(Fluid f, ItemStack is) {
        return is.stackTagCompound != null
            ? f.equals(ReikaNBTHelper.getFluidFromNBT(is.stackTagCompound))
            : true;
    }
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        if (is.stackTagCompound != null) {
            ...
            int slot = ReikaInventoryHelper.locateIDInInventory(
                ItemRegistry.JETPACK.getItemInstance(), ep.inventory
            );
            if (slot == -1) {
                slot = ReikaInventoryHelper.locateIDInInventory(
                    ItemRegistry.BEDPACK.getItemInstance(), ep.inventory
                );
            }
            if (slot == -1) {
                slot = ReikaInventoryHelper.locateIDInInventory(
                    ItemRegistry.STEELPACK.getItemInstance(), ep.inventory
                );
            }
            if (slot != -1) {
    ...
    public boolean onItemUse(
        ...
        if (m == MachineRegistry.ENGINE) {
            ...
            if (f != null) {
                int amt = Math.min(
                    this.getCurrentFillLevel(is), te.getFuelCapacity() - te.getFuelLevel()
                );
                if (amt > 0) {
                    boolean flag = false;
                    if (eng.isJetFueled()
                        && f.equals(FluidRegistry.getFluid("rc jet fuel"))) {
                        te.addFuel(amt);
                        flag = true;
                    ...
                    if (flag) {
                        ReikaPacketHelper.sendTankSyncPacket(
                            RotaryCraft.packetChannel, te, "fuel"
                        );
        ...
        if (m == MachineRegistry.FUELENGINE) {
            ...
            if (f == null) {
                ...
                int amt = Math.min(
                    this.getCurrentFillLevel(is), te.CAPACITY - te.getFuelLevel()
                );
                if (amt > 0) {
                    te.addFuel(amt, f);
                    ReikaPacketHelper.sendTankSyncPacket(
                        RotaryCraft.packetChannel, te, "tank"
                    );
                    this.removeFuel(is, amt);
                    return true;
                }
        ...
        if (m == MachineRegistry.PULSEJET) {
            ...
            if (f == null) {
                ...
                int amt
                    = Math.min(this.getCurrentFillLevel(is), te.MAXFUEL - te.getFuel());
                if (amt > 0) {
                    te.addFuel(amt);
                    ReikaPacketHelper.sendTankSyncPacket(
                        RotaryCraft.packetChannel, te, "fuel"
        ...
        if (m == MachineRegistry.RESERVOIR) {
            ...
            if (f != null) {
                int amt
                    = Math.min(this.getCurrentFillLevel(is), te.CAPACITY - te.getLevel());
    ...
    public Fluid getCurrentFluid(ItemStack is) {
        return is.stackTagCompound != null
            ? ReikaNBTHelper.getFluidFromNBT(is.stackTagCompound)
            : null;
    }
}
