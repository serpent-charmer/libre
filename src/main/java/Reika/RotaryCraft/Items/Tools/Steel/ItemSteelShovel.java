...
public class ItemSteelShovel extends ItemSpade implements IndexedItemSprites {
    ...
    public ItemSteelShovel(int tex) {
    ...
    public boolean canHarvestBlock(Block b, ItemStack is) {
        if (ConfigRegistry.HSLAHARVEST.getState() && b.blockHardness < 20
            && this.getDigSpeed(is, b, 0) > 1) {
            return true;
        } else
            return Items.iron_shovel.canHarvestBlock(b, is);
    }
