...
public class ItemCanolaSeed extends ItemBasic implements IPlantable {
    ...
    public boolean onItemUse(
        ...
        boolean flag = false;
        if (ReikaWorldHelper.softBlocks(world, x, y, z)
            && BlockCanola.isValidFarmBlock(world, x, y - 1, z)) {
            ...
            for (int xi = minx; xi <= maxx; xi++) {
                for (int zi = minz; zi <= maxz; zi++) {
                    if ((!ReikaWorldHelper.softBlocks(world, xi, y, zi))
                        || !BlockCanola.isValidFarmBlock(world, xi, y - 1, zi)) {
                        ReikaItemHelper.dropItem(
                            world, xi + 0.5, y + 0.5, zi + 0.5, ItemStacks.canolaSeeds
                        );
                        ...
                        world.setBlock(
                            xi, y, zi, BlockRegistry.CANOLA.getBlockInstance()
                        );
                        flag = true;
                    }
                }
