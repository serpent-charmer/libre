...
public class ItemGearPlacer extends ItemBlockPlacer {
    ...
    public boolean onItemUse(
        ...
        if (!ep.canPlayerEdit(x, y, z, 0, is))
            ...
            if (!ep.capabilities.isCreativeMode)
                --is.stackSize;
            world.setBlock(
                x, y, z, MachineRegistry.GEARBOX.getBlock(), is.getItemDamage() % 5, 3
            );
            ...
            if (gbx != null) {
                ...
                gbx.setBlockMetadata(
                    is.getItemDamage() * 4 + RotaryAux.get4SidedMetadataFromPlayerLook(ep)
                );
    ...
    addInformation(ItemStack is, EntityPlayer ep, List par3List, boolean par4) {
        ...
        if (is.stackTagCompound.hasKey("damage") && mat.isDamageableGear()) {
            int dmg = TileEntityGearbox.getDamagePercent(
                is.stackTagCompound.getInteger("damage")
            );
        ...
        if (is.stackTagCompound.hasKey("lube") && mat.needsLubricant()) {
            ...
            String s = mat == GearboxTypes.LIVINGROCK
                    || is.stackTagCompound.getBoolean("living")
                ? String.format("Mana: %d%%", amt * 100 / mat.getMaxLubricant())
                : "Lubricant: " + amt + " mB";
            par3List.add(s);
        }
        ...
        if (is.stackTagCompound.hasKey("bearing")) {
            ...
            String name = StatCollector.translateToLocal(
                "material." + matl.name().toLowerCase(Locale.ENGLISH)
            );
            EnumChatFormatting ec = EnumChatFormatting.WHITE;
            if (matl.ordinal() != mat.material.ordinal()) {
                ec = matl.ordinal() < mat.material.ordinal() ? EnumChatFormatting.RED
    ...
    public String getItemStackDisplayName(ItemStack is) {
        try {
            ...
            return type != null ? type.getLocalizedGearboxName(
                       ReikaMathLibrary.intpow2(2, is.getItemDamage() + 1)
                   )
                                : "Gearbox";
        } catch (Exception e) {
