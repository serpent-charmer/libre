...
public class ItemShaftPlacer extends ItemBlockPlacer {
    ...
    public boolean onItemUse(
        ...
        if (!ep.canPlayerEdit(x, y, z, 0, is))
            ...
            if (!ep.capabilities.isCreativeMode)
                --is.stackSize;
            world.setBlock(
                x, y, z, MachineRegistry.SHAFT.getBlock(), is.getItemDamage(), 3
            );
            ...
            if (sha != null) {
                ...
                if (cross) {
                    sha.setBlockMetadata(
                        6 + RotaryAux.get4SidedMetadataFromPlayerLook(ep)
                    );
                } else {
    ...
    public String getItemStackDisplayName(ItemStack is) {
        ...
        return type != null ? StatCollector.translateToLocal(type.getShaftUnlocName())
                + " " + MachineRegistry.SHAFT.getName()
                            : "Shaft";
    }
}
