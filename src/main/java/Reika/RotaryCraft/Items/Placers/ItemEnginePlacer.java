...
public class ItemEnginePlacer extends ItemBlockPlacer {
    ...
    public boolean onItemUse(
        ...
        if (!ep.canPlayerEdit(x, y, z, 0, is)) {
            ...
            if (!ep.capabilities.isCreativeMode)
                --is.stackSize;
            world.setBlock(
                x, y, z, MachineRegistry.ENGINE.getBlock(), is.getItemDamage(), 3
            );
    ...
    @Override
    protected void
    checkAndBreakAdjacent(World world, int x, int y, int z, EntityPlayer ep) {
        TileEntityEngine eng = (TileEntityEngine) world.getTileEntity(x, y, z);
        if (eng != null) {
    ...
    public void getSubItems(Item id, CreativeTabs tab, List list) {
        if (MachineRegistry.ENGINE.isAvailableInCreativeInventory()) {
            for (int i = 0; i < EngineType.engineList.length; i++) {
                ...
                list.add(item);
                TileEntityEngine te = (TileEntityEngine
                ) MachineRegistry.ENGINE.createTEInstanceForRender(i);
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean verbose) {
        ...
        if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
            ...
            li.add(String.format(
                "Torque: %.3f %sNm",
                ReikaMathLibrary.getThousandBase(torque),
                ReikaEngLibrary.getSIPrefix(torque)
            ));
            li.add(String.format(
                "Speed: %.3f %srad/s",
                ReikaMathLibrary.getThousandBase(speed),
                ReikaEngLibrary.getSIPrefix(speed)
            ));
        } else {
        ...
        TileEntityEngine te
            = (TileEntityEngine) MachineRegistry.ENGINE.createTEInstanceForRender(i);
