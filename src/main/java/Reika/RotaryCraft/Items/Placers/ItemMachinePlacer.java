    ...
    public boolean onItemUse(
        ...
        if (m == MachineRegistry.GPR) {
            ...
            world.setBlockMetadataWithNotify(
                x, y, z, BlockGPR.getBiomeDesign(world, x, y, z), 3
            );
            return true;
        }
        if (m.isCannon())
        ...
        if (m == MachineRegistry.PNEUENGINE || m == MachineRegistry.STEAMTURBINE
            || m == MachineRegistry.GENERATOR || m == MachineRegistry.ELECTRICMOTOR
            || m == MachineRegistry.MAGNETIC) {
        ...
        if (m.isPipe()) {
            ((TileEntityPiping) world.getTileEntity(x, y, z))
                .onPlacedAgainst(ForgeDirection.VALID_DIRECTIONS[side].getOpposite());
        }
        ...
        for (int i = 0; i < MachineRegistry.machineList.length; i++) {
            ...
            if (!m.hasCustomPlacerItem() && m.isAvailableInCreativeInventory()) {
                ...
                if (m.isCreativeTabValid(par2CreativeTabs)) {
                    if (m.hasNBTVariants()) {
                        ArrayList<NBTTagCompound> li
                            = ((NBTMachine) te).getCreativeModeVariants();
                        if (li.isEmpty())
        ...
        if (m == MachineRegistry.FUELENGINE) {
            if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
                ...
                li.add(String.format(
                    "Power: %.3f %sW",
                    ReikaMathLibrary.getThousandBase(t * o),
                    ReikaEngLibrary.getSIPrefix(t * o)
                ));
                li.add(String.format(
                    "Torque: %.3f %sNm",
                    ReikaMathLibrary.getThousandBase(t),
                    ReikaEngLibrary.getSIPrefix(t)
                ));
                li.add(String.format(
                    "Speed: %.3f %srad/s",
                    ReikaMathLibrary.getThousandBase(o),
                    ReikaEngLibrary.getSIPrefix(o)
                ));
            } else {
