...
public class BlockBedrockSlice extends BlockBasic {
    ...
    /**
     * Returns a bounding box from the pool of bounding boxes (this means this box can
     * change after the pool has been cleared to be reused)
     */
    @Override
    ...
    getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        if (var5 != 0)
            return AxisAlignedBB.getBoundingBox(x + minX, y + minY, z + minZ, x + maxX, y
        + (15-var5)/16F, z + maxZ); else return AxisAlignedBB.getBoundingBox(x + minX, y +
        minY, z + minZ, x + maxX, y + maxY, z + maxZ);
         */
        TileEntity te = world.getTileEntity(x, y, z);
        return te instanceof TileEntityBedrockSlice
            ? ((TileEntityBedrockSlice) te).getBounds().asAABB(x, y, z)
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess iba, int x, int y, int z) {
         ...
         */
        ((TileEntityBedrockSlice) iba.getTileEntity(x, y, z))
            .getBounds()
            .copyToBlock(this);
    }
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        ...
        return var5 != Blocks.air
                && (var5 == Blocks.leaves || var5 == Blocks.leaves2 || var5.isOpaqueCube()
                )
            ? ReikaWorldHelper.getMaterial(world, x, y - 1, z).blocksMovement()
            : false;
    }
    ...
    public boolean shouldSideBeRendered(IBlockAccess iba, int x, int y, int z, int par5) {
        return super.shouldSideBeRendered(
            iba, x, y, z, par5
        ); //par5 == 1 ? true : super.shouldSideBeRendered(iba, x, y, z, par5);
    }
    ...
    public static class TileEntityBedrockSlice extends TileEntity {
        ...
        private BlockBounds getBounds() {
            return BlockBounds.block().cut(
                machineDirection, this.getBlockMetadata() / 16D
            );
        }
