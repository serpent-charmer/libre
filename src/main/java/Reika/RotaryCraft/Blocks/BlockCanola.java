...
public final class BlockCanola
    extends BlockBasic implements IPlantable, IGrowable, IWailaDataProvider {
    ...
    /*
    @Override
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer ep, int
    s, float f1, float f2, float f3) { ItemStack is = ep.getCurrentEquippedItem(); if
    (!world.isRemote && ReikaItemHelper.matchStacks(is, ReikaItemHelper.bonemeal)) { if
    (world.getBlockMetadata(x, y, z) < GROWN) { int to = Math.min(GROWN,
    world.getBlockMetadata(x, y, z)+1+rand.nextInt(4)); if (ModList.LEGACYCRAFT.isLoaded()
    && LegacyOptions.BONEMEAL.getState()) to = GROWN; world.setBlockMetadataWithNotify(x,
    y, z, to, 3); for (int i = 0; i < 16; i++) world.spawnParticle("happyVillager",
    x+rand.nextDouble(), y+rand.nextDouble(), z+rand.nextDouble(), 0, 0, 0); if
    (!ep.capabilities.isCreativeMode) ep.getCurrentEquippedItem().stackSize--; return
    true;
            }
        }
        return false;
    ...
    getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        return AxisAlignedBB.getBoundingBox(
            x + minX, y + minY, z + minZ, x + maxX, meta / (float) GROWN, z + maxZ
        );
    }
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess iba, int x, int y, int z) {
        this.setBlockBounds(
            0.0F, 0.0F, 0.0F, 1.0F, getPlantHeight(iba.getBlockMetadata(x, y, z)), 1.0F
        );
    }
    ...
    public static boolean isValidFarmBlock(World world, int x, int y, int z) {
        ...
        if (id == Blocks.grass) {
            GrassSustainCropEvent evt = new GrassSustainCropEvent(
                world,
                x,
                y,
                z,
                ForgeDirection.UP,
                (IPlantable) BlockRegistry.CANOLA.getBlockInstance()
            );
    ...
    public static ItemStack getDrops(int metadata, int fortune, Random rand) {
        ...
        ItemStack items
            = ReikaItemHelper.getSizedItemStack(ItemStacks.canolaSeeds, ndrops);
        return items;
    }

    public static int getDrops(int fortune, Random rand) {
        int ndrops = Math.max(
            fortune * 2, (1 + rand.nextInt(2)) * (2 + rand.nextInt(8) + rand.nextInt(5))
        );
        if (fortune > 0) {
