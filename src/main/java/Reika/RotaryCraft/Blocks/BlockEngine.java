...
public class BlockEngine extends BlockModelledMachine {
        ...
        if (te instanceof TileEntityEngine) {
            ...
            if (is != null && ReikaItemHelper.matchStacks(is, ItemStacks.turbine)) {
                if (tile.getEngineType() == EngineType.JET
                    && ((TileEntityJetEngine) tile).FOD > 0) {
            ...
            if (is != null && ReikaItemHelper.matchStacks(is, ItemStacks.bedrockshaft)) {
                if (tile.getEngineType() == EngineType.HYDRO
                    && !((TileEntityHydroEngine) tile).isBedrock()) {
                    ...
                    if (tile.getEngineType().isEthanolFueled()) {
                        if (tile.getFuelLevel() >= 1000) {
                            ...
                            ReikaChatHelper.write(
                                "Engine does not have enough fuel to extract!"
                            );
                        }
                        return true;
                    }
    ...
    setBlockBoundsBasedOnState(IBlockAccess par1IBlockAccess, int x, int y, int z) {
        ...
        TileEntityEngine tile
            = (TileEntityEngine) par1IBlockAccess.getTileEntity(x, y, z);
        if (tile == null)
            return;
    ...
    harvestBlock(World world, EntityPlayer ep, int x, int y, int z, int meta) {
        ...
        if (eng != null) {
            if (eng.getEngineType() == EngineType.JET
                && ((TileEntityJetEngine) eng).FOD >= 8) {
                ItemStack todrop = ReikaItemHelper.getSizedItemStack(
                    ItemStacks.steelgear, 1 + par5Random.nextInt(5)
                ); //drop gears
                EntityItem item
                ...
                todrop = ReikaItemHelper.getSizedItemStack(
                    ItemStacks.scrap, 16 + par5Random.nextInt(17)
                ); //drop scrap
                ...
                ItemStack todrop
                    = eng.getEngineType().getCraftedProduct(); //drop engine item
                if (eng.getEngineType() == EngineType.JET) {
                    ...
                    if (todrop.stackTagCompound == null)
                        todrop.stackTagCompound = new NBTTagCompound();
                    todrop.stackTagCompound.setBoolean(
                        "bed", ((TileEntityHydroEngine) eng).isBedrock()
                    );
                }
