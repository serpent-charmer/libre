...
public class BlockGearbox extends BlockModelledMachine {
    ...
    public float getExplosionResistance(
        Entity ent, World world, int x, int y, int z, double eX, double eY, double eZ
    ) {
    ...
    getPlayerRelativeBlockHardness(EntityPlayer ep, World world, int x, int y, int z) {
        ...
        if (ep.inventory.getCurrentItem() != null) {
            if (ep.inventory.getCurrentItem().getItem()
                == ItemRegistry.BEDPICK.getItemInstance())
                mult = 2;
        }
    ...
    harvestBlock(World world, EntityPlayer ep, int x, int y, int z, int meta) {
        ...
        if (gbx != null) {
            ...
            if (gbx.isUnHarvestable()) {
                todrop = ReikaItemHelper.getSizedItemStack(
    ...
    public boolean onBlockActivated(
        ...
        if (ep.getCurrentEquippedItem() != null
            ...
            && ep.getCurrentEquippedItem().getItem() == Items.bucket) {
            tile.clearLubricant();
            return true;
        }
        //}
        ...
        if (ep.getCurrentEquippedItem() != null
            && (ep.getCurrentEquippedItem().getItem() instanceof ItemScrewdriver
                || ep.getCurrentEquippedItem().getItem() instanceof ItemMeter
                || ep.getCurrentEquippedItem().getItem() instanceof ItemDebug)) {
            return false;
        }
        if (tile != null) {
            ...
            if (held != null) {
                if ((ReikaItemHelper.matchStacks(fix, held))) {
                    boolean flag = tile.repair(
                        1 + 20 * tile.getRandom().nextInt(18 - tile.getRatio())
                    );
                    if (flag && !ep.capabilities.isCreativeMode) {
                        ...
                        if (num > 1)
                            ep.inventory.setInventorySlotContents(
                                ...
                                ReikaItemHelper.getSizedItemStack(fix, num - 1)
                            );
                        else
                            ep.inventory.setInventorySlotContents(
                                ep.inventory.currentItem, null
                            );
                    }
                    return true;
                        ...
                        if (tile.canTakeLubricant(amt)) {
                            ...
                            if (!ep.capabilities.isCreativeMode)
                                ep.setCurrentItemOrArmor(
                                    0, new ItemStack(Items.bucket, held.stackSize, 0)
                                );
                        }
                    }
                    return true;
                    ...
                    GearboxTypes material
                        = GearboxTypes.getMaterialFromCraftingItem(held);
                    if (tile.getGearboxType().acceptsBearingUpgrade(material)
                        && tile.getBearingTier() != material) {
                        if (tile.getBearingTier() != tile.getGearboxType())
                            ReikaItemHelper.dropItem(
                                ...
                                y + 0.5,
                                z + 0.5,
                                tile.getBearingTier().getPart(GearPart.BEARING)
                            );
                        ...
                        if (!ep.capabilities.isCreativeMode) {
                            ...
                            if (num > 1)
                                ep.inventory.setInventorySlotContents(
                                    ep.inventory.currentItem,
                                    ReikaItemHelper.getSizedItemStack(held, num - 1)
                                );
                            else
                                ep.inventory.setInventorySlotContents(
                                    ep.inventory.currentItem, null
