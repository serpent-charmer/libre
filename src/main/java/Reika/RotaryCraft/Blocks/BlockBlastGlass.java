...
public class BlockBlastGlass extends BlockBasic implements ConnectedTextureGlass {
    ...
    public boolean canHarvestBlock(EntityPlayer ep, int meta) {
        ...
        if (item.getItem() == Items.diamond_pickaxe
            || item.getItem() == ItemRegistry.BEDPICK.getItemInstance())
            return true;
        return item.getItem().canHarvestBlock(Blocks.obsidian, item);
    }

    /**
     * This block can only be destroyed by the wither explosions - this in effect makes
     * it witherproof
     */
    @Override
    public void
    onBlockDestroyedByExplosion(World world, int x, int y, int z, Explosion ex) {
