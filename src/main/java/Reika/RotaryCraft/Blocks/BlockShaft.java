...
public class BlockShaft extends BlockModelledMachine {
    ...
    public RotaryCraftTileEntity createTileEntity(World world, int meta) {
        return new TileEntityShaft(
            meta < 5 ? MaterialRegistry.matList[meta] : MaterialRegistry.STEEL
        );
    }
    ...
    public boolean onBlockActivated(
        ...
        if (tile != null && tile.getShaftType() != null) {
            ...
            if (ep.getCurrentEquippedItem() != null
                && ReikaItemHelper.matchStacks(fix, ep.getCurrentEquippedItem())) {
                tile.repair();
        ...
        if (sha != null) {
            if (sha.failed()) {
                ...
                if (par5Random.nextInt(8) == 0) {
                    switch (sha.getShaftType()) {
                        ...
                        case STONE:
                            todrop = ReikaItemHelper.getSizedItemStack(
                                ReikaItemHelper.cobbleSlab.asItemStack(), 5
                            );
                            break;
                        case STEEL:
                    ...
                    EntityItem item
                        = new EntityItem(world, x + 0.5F, y + 0.5F, z + 0.5F, todrop);
                ...
                if (!world.isRemote && !ep.capabilities.isCreativeMode)
                   ...
                   ItemStack todrop = new ItemStack(MachineRegistry.SHAFT.getBlock(), 1,
                   6); //drop shaft block (cross) EntityItem item = new EntityItem(world,
                   x + 0.5F, y + 0.5F, z + 0.5F, todrop); Items.delayBeforeCanPickup = 10;
                   if (!world.isRemote && !ep.capabilities.isCreativeMode)
                       world.spawnEntityInWorld(item);*/
    ...
    public boolean canHarvest(World world, EntityPlayer player, int x, int y, int z) {
        ...
        return type != null ? type.isHarvestablePickaxe(player.inventory.getCurrentItem())
                            : false;
    }
