...
public class BlockDecoTank extends Block implements ConnectedTextureGlass {
    ...
    public BlockDecoTank() {
        ...
        this.setCreativeTab(
            RotaryCraft.instance.isLocked() ? null : RotaryCraft.tabRotary
        );
    ...
    public IIcon getIconForEdge(IBlockAccess world, int x, int y, int z, int edge) {
        return TankFlags.RESISTANT.apply(world, x, y, z)
            ? ((BlockBlastGlass) BlockRegistry.BLASTGLASS.getBlockInstance())
                  .getIconForEdge(world, x, y, z, edge)
            : icons[edge];
    }

    public IIcon getIconForEdge(int itemMeta, int edge) {
        return TankFlags.RESISTANT.applyItem(itemMeta)
            ? ((BlockBlastGlass) BlockRegistry.BLASTGLASS.getBlockInstance())
                  .getIconForEdge(itemMeta, edge)
            : icons[edge];
    }
    ...
    getPickBlock(MovingObjectPosition target, World world, int x, int y, int z) {
        TileEntityDecoTank te = (TileEntityDecoTank) world.getTileEntity(x, y, z);
        ItemStack is
            = this.getDrops(world, x, y, z, world.getBlockMetadata(x, y, z), 0).get(0);
        return is;
    }
