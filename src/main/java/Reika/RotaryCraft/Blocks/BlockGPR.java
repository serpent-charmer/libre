...
public class BlockGPR extends BlockBasicMachine {
    ...
    public void onBlockPlacedBy(
        World world, int x, int y, int z, EntityLivingBase par5EntityLiving, ItemStack is
    ) { //Directional code{
        super.onBlockPlacedBy(world, x, y, z, par5EntityLiving, is);
        //if (MathHelper.abs(par5EntityLiving.rotationPitch) < 45) {
        int i
            = MathHelper.floor_double((par5EntityLiving.rotationYaw * 4F) / 360F + 0.5D);
        while (i > 3)
            i -= 4;
        while (i < 0)
            i += 4;
    ...
    public void registerBlockIcons(IIconRegister par1IconRegister) {
        if (RotaryCraft.instance.isLocked())
            return;
        String[] biome = { "grass", "mushroom", "jungle", "hills", "ocean", "desert",
                           "snow",  "nether",   "end",    "swamp", "ice" };
        for (int i = 0; i < 6; i++)
            for (int j = 0; j < biome.length; j++) {
                icons[j][i] = par1IconRegister.registerIcon("RotaryCraft:steel");
                icons[j][1] = par1IconRegister.registerIcon(
                    "RotaryCraft:gpr/gpr_top_" + biome[j]
                );
            }
    }
