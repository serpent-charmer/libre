...
public class VoidMonsterEvents {
    ...
    public void damageGhostMonsters(SplashPotionEvent evt) {
        if (evt.entityLiving instanceof EntityVoidMonster
            && evt.potion.id == Potion.heal.id) {
            if (((EntityVoidMonster) evt.entityLiving).isGhost()) {
                ((EntityVoidMonster) evt.entityLiving)
                    .doGhostDamage(evt.thrower, evt.potionAmplifier, evt.effectFactor);
    ...
    public void dynamicFog(FogDistanceEvent evt) {
        evt.fogDistance = Math.min(
            evt.fogDistance,
            Math.min(MonsterFX.rampFog(evt.fogDistance), VoidFogManager.getFogDistance())
        );
    }
    ...
    public void dynamicFog(FarClippingPlaneEvent evt) {
        evt.farClippingPlaneDistance = Math.min(
            ...
            Math.min(
                MonsterFX.rampFog(evt.farClippingPlaneDistance),
                VoidFogManager.getFogDistance()
            )
        );
    }
    ...
    public void dynamicFog(BossColorEvent evt) {
        ...
        //}
        //int c0 = ReikaColorAPI.RGBtoHex((int)(evt.red*255), (int)(evt.green*255),
        //(int)(evt.blue*255)); int c1 = evt.isLightmap ? 0x101010 : 0x000000; int c =
        //MonsterFX.rampColor(c1, c0);//ReikaColorAPI.mixColors(c1, c0,
        //MathHelper.clamp_float(Minecraft.getMinecraft().entityRenderer.bossColorModifier,
        //0, 1)); evt.red = ReikaColorAPI.getRed(c)/255F; evt.green =
        //ReikaColorAPI.getGreen(c)/255F; evt.blue = ReikaColorAPI.getBlue(c)/255F;
    }
