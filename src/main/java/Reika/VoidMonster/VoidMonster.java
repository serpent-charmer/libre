...
@Mod(
    ...
    dependencies = "required-after:DragonAPI;before:Morph"
)
public class VoidMonster extends DragonAPIMod {
    ...
    @SidedProxy(
        clientSide = "Reika.VoidMonster.VoidClient",
        serverSide = "Reika.VoidMonster.VoidCommon"
    )
    public static VoidCommon proxy;

    public static final SimpleConfig config = new SimpleConfig(instance);

    private static final MultiMap<Integer, Integer> monsterList
        = new MultiMap(CollectionType.CONCURRENTSET);
    ...
    public void preload(FMLPreInitializationEvent evt) {
        ...
        monsterDifficulty
            = config.getFloat("Control Setup", "Void Monster Difficulty Factor", 1F);
        ...
        monsterSoundDelay = Math.max(
            0,
            Math.min(
                1200,
                VoidMonster.config.getInteger(
                    "Control Setup", "Sound Interval in Ticks", 80
                )
            )
        );
    ...
    public void load(FMLInitializationEvent event) {
        ...
        //	EntityList.addMapping(EntityVoidMonster.class, "Void Monster", id, 0x000000,
        //0x555555);
        EntityRegistry.registerGlobalEntityID(
            EntityVoidMonster.class, "Void Monster", id
        );
        EntityRegistry.registerModEntity(
            EntityVoidMonster.class, "Void Monster", id, instance, 256, 10, true
        );
    ...
    public void postload(FMLPostInitializationEvent evt) {
        ...
        if (ModList.THAUMCRAFT.isLoaded()) {
            Object[] asp = { Aspect.ELDRITCH, 40, Aspect.VOID,  40, Aspect.DARKNESS, 40,
                             Aspect.BEAST,    25, Aspect.DEATH, 25, Aspect.ENTROPY,  20,
                             Aspect.AURA,     5,  Aspect.ARMOR, 10, Aspect.CRYSTAL,  10 };
    ...
    public static Collection<EntityVoidMonster> getCurrentMonsterList(World world) {
        ...
        ThreadSafeSet<Integer> set
            = (ThreadSafeSet<Integer>) monsterList.get(world.provider.dimensionId);
