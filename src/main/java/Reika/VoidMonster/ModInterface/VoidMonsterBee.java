...
public class VoidMonsterBee extends BeeSpecies {
    ...
    public VoidMonsterBee() {
        super(
            EnumChatFormatting.OBFUSCATED + "VoidMonster",
            "bee.voidmonster",
            "Mali Sub Orbis Terrarum",
            "Reika",
            new BeeBranch(
                "branch.voidmonster",
                "Void",
                "Void",
                "Is breeding these even slightly advisable?"
            )
        );
        ...
        if (ModList.MAGICBEES.isLoaded()) {
            this.addBreeding(
                "Draconic", ModList.MAGICBEES, "Withering", ModList.MAGICBEES, 4
            );
    ...
    private static void addAlvearyEffect() {
        voidMonsterAlvearyEffect = new LumenAlvearyEffect(
            "voidmonster", CrystalElement.PINK, 200
        ) {
            @Override
            ...
            protected void clientTick(TileEntityLumenAlveary te) {
                if (te.getSpecies() == beeType && te.canQueenWork()) {
                    ChunkCoordinates c
                        = te.getMultiblockLogic().getController().getCoordinates();
                    ...
                    for (int i = 0; i < n; i++) {
                        ...
                        double[] xyz = ReikaPhysicsHelper.polarToCartesian(
                            v, 0, te.worldObj.rand.nextDouble() * 360
                        );
                        fx.motionX = xyz[0];
                        fx.motionZ = xyz[2];
        ...
        ((LumenAlvearyEffect) voidMonsterAlvearyEffect)
            .setXMLText("Forces the alveary to function as if near the void");
    }

    @SideOnly(Side.CLIENT)
    private static void loadColorData() {
        beeColor = new ColorBlendList(
            18F,
            0x000000,
            0x6000e0,
            0x000000,
    ...
    private final class FlowerProviderVoid extends BasicFlowerProvider {
        ...
        @Override
        public ItemStack[] affectProducts(
            World world, IIndividual ii, int x, int y, int z, ItemStack[] products
        ) {
            int n = 1;
            if (ii.getGenome().getActiveAllele(EnumBeeChromosome.FLOWER_PROVIDER)
                == voidflower)
                n *= 2;
            if (ii.getGenome().getInactiveAllele(EnumBeeChromosome.FLOWER_PROVIDER)
                == voidflower)
                n *= 2;
            if (n > 1) {
        ...
        public boolean isAcceptableFlower(String type, World world, int x, int y, int z) {
            return y < 0 || (world.provider.isHellWorld && y >= 128)
                || (ModList.CHROMATICRAFT.isLoaded()
                    && this.isLumenAlveary(world, x, y, z));
        }
        ...
        private boolean isLumenAlveary(World world, int x, int y, int z) {
            if (ChromaTiles.getTile(world, x, y, z) == ChromaTiles.ALVEARY) {
                TileEntityLumenAlveary te
                    = (TileEntityLumenAlveary) world.getTileEntity(x, y, z);
                return te != null
                    && te.getActiveEffects().contains(voidMonsterAlvearyEffect);
            }
            return false;
        }
    }
            ...
            if (c != null && !c.isEmpty()) {
                for (EntityVoidMonster e : c) {
                    if (world.provider.isHellWorld) {
                        ...
                        if (e.getDistanceSq(cc.posX + 0.5, cc.posY + 0.5, cc.posZ + 0.5)
                            < 256) {
    ...
    private static String getRandomString(int maxlen) {
        ...
        while (sb.length() < maxlen) {
            int len = Math.min(
                maxlen - sb.length(), ReikaRandomHelper.getRandomBetween(4, 8)
            );
    ...
    public int getOutlineColor() {
        return beeColor != null
            ? beeColor.getColor(DragonAPICore.getSystemTimeAsInt() / 25D)
            : 0x000000;
    }
    ...
    public boolean isJubilant(IBeeGenome ibg, IBeeHousing ibh) {
        World world = ibh.getWorld();
        ChunkCoordinates c = ibh.getCoordinates();
        return (c.posY <= 1 || (c.posY >= 128 && world.provider.isHellWorld))
            || (ModList.CHROMATICRAFT.isLoaded() && this.isLumenAlveary(ibh, world, c));
    }
