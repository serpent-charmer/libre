...
public class VoidMonsterDrops {
    ...
    public static void loadCustomDrops() {
        ...
        if (crl.load()) {
            for (LuaBlock lb : crl.getEntries()) {
                ...
                if (flag) {
                    VoidMonster.logger.log(
                        "Loaded custom monster drop '" + lb.getString("type") + "'"
                    );
                } else {
                    VoidMonster.logger.logError(
                        "Could not load custom monster drop '" + lb.getString("type")
                        + "'"
                    );
                    if (e != null)
                        e.printStackTrace();
    ...
    private static void dropEnchantedBooks(EntityVoidMonster e) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int max
                = Math.max(1, (int) (en.getMaxLevel() * Math.min(1, e.getDifficulty())));
        ...
        for (ImmutablePair<Enchantment, Integer> p : li) {
            ReikaItemHelper.dropItem(
                ...
                e.posY + 0.25,
                e.posZ,
                ReikaEnchantmentHelper.getEnchantedBook(p.left, p.right)
            );
        }
    }
