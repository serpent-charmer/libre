...
@Strippable(
    value = { "com.carpentersblocks.api.IWrappableBlock",
              "mcp.mobius.waila.api.IWailaDataProvider" }
)
public abstract class RockBlock extends Block
    implements CraftableStone, Laserable, IWrappableBlock, IWailaDataProvider {
    ...
    /*
    @Override
    public final float getPlayerRelativeBlockHardness(EntityPlayer ep, World world, int x,
    int y, int z) { ItemStack is = ep.getCurrentEquippedItem(); int meta =
    world.getBlockMetadata(x, y, z); float buff = ModList.ROTARYCRAFT.isLoaded() &&
    ItemFetcher.isPlayerHoldingBedrockPick(ep) ? 1.875F : 1; float eff = 1; if (is !=
    null) { int level = ReikaEnchantmentHelper.getEnchantmentLevel(Enchantment.efficiency,
    is); eff = ReikaEnchantmentHelper.getEfficiencyMultiplier(level);
        }
        ...
        if (TinkerToolHandler.getInstance().isPick(is) ||
    TinkerToolHandler.getInstance().isHammer(is)) { return
    0.05F/RockTypes.getTypeAtCoords(world, x, y, z).blockHardness*6*eff;
        }
        return 0.05F/RockTypes.getTypeAtCoords(world, x, y,
    z).blockHardness*is.getItem().getDigSpeed(is, this, meta)*eff*buff;
    }

    @Override
    public final float getExplosionResistance(Entity e, World world, int x, int y, int z,
    double eX, double eY, double eZ) { return RockTypes.getTypeAtCoords(world, x, y,
    z).blastResistance/4F; // /5F is in vanilla code
    }*/
    ...
    public final boolean canHarvestBlock(EntityPlayer player, int meta) {
        if (player.capabilities.isCreativeMode)
            return false;
        return
    RockTypes.getTypeFromID(this).isHarvestable(player.getCurrentEquippedItem());
    }*/
    ...
    public final int colorMultiplier(IBlockAccess iba, int x, int y, int z) {
        ...
        ); //not coords, to account for being used as a cover
    ...
    public final int getRenderColor(int dmg) {
        RockTypes rock = RockTypes.getTypeFromID(this);
        if (rock == RockTypes.OPAL) {
            return GeoStrata.getOpalPositionColor(
    ...
    getWailaStack(IWailaDataAccessor acc, IWailaConfigHandler cfg) {
        return null;
    }
    ...
    public List<String> getWailaBody(
        ...
        if (GeoOptions.WAILA.getState()) {
            ...
            String tag = String.format(
                "%.2fR / %.2fH", type.blastResistance, type.blockHardness
            );
            currenttip.add(tag);
        }
        return currenttip;
