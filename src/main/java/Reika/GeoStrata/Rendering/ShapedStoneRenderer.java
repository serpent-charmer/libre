...
public class ShapedStoneRenderer extends BaseBlockRenderer {
    ...
    renderInventoryBlock(Block block, int metadata, int modelID, RenderBlocks renderer) {
        ...
        IIcon ico2 = Blocks.glass.getIcon(
            0, 0
        ); //RockShapes.getShape(block).getIcon();//b.getIconForEdge(0,
           //RockTypes.getTypeFromID(Blocks.blockID, metadata));
    ...
    private void drawInventoryBlock(
        BlockShapedRock b, int metadata, RenderBlocks renderer, IIcon ico
    ) {
        Tessellator v5 = Tessellator.instance;
