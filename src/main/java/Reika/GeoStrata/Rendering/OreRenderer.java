...
public class OreRenderer extends ISBRH {
    //private static final EnumMap<ModOreList, ArrayList<IIcon>> modOreIcons = new
    //EnumMap(ModOreList.class); private static final EnumMap<ReikaOreHelper,
    //ArrayList<IIcon>> oreIcons = new EnumMap(ReikaOreHelper.class);
            ...
            if (ore.canGenerateIn(Blocks.stone)) {
                ...
                for (ItemStack is : c) {
                    try {
                        ...
                        String n = "geostrata:ore_"
                            + ore.name().toLowerCase(Locale.ENGLISH) + "_" + k;
                        k++;
    ...
    renderInventoryBlock(Block block, int metadata, int modelId, RenderBlocks rb) {
        ...
        ItemStack ore
            = BlockOreTile.getOreByItemMetadata(Item.getItemFromBlock(block), metadata);
        ...
        IIcon[] ico = new IIcon[] { this.getOreTexture(b, meta),
                                    ((BlockOreTile) block).getRockIcon(r) };
    ...
    public boolean renderWorldBlock(
        ...
        rb.renderStandardBlockWithAmbientOcclusion(
            b, x, y, z, red / 255, green / 255, blue / 255
        );
        /*
    ...
    private IIcon getOreTexture(Block b, int meta) {
        return b.getIcon(
            0, meta
        ); //icons.get(b, meta); comment out dynamic system for now
    }
