...
public class DecoGenRenderer extends ISBRH {
    ...
    public static RotatedQuad getCrystalShape(int x, int y, int z) {
        int i
            = ((x % crystalShapes.length) + crystalShapes.length) % crystalShapes.length;
        int j = ((y % crystalShapes[i].length) + crystalShapes[i].length)
            % crystalShapes[i].length;
        int k = ((z % crystalShapes[i][j].length) + crystalShapes[i][j].length)
            % crystalShapes[i][j].length;
        ...
        switch (Types.list[meta]) {
            case CRYSTALSPIKE:
                ...
                if (renderPass == 0) {
                    ...
                    int n = 0;
                    while (world.getBlock(x, y + 1 + n, z) == block
                           && world.getBlockMetadata(x, y + 1 + n, z) == meta)
                        n++;
                    v5.setColorOpaque_I(ReikaColorAPI.GStoHex(Math.max(
                        32 + (int) (16 * Math.sin((x + y * 8 + z * 2) / 8D)),
            ...
            case ICICLE:
                if (renderPass == 0) {
                    for (int i = 0; i < 3; i++) {
                        ...
                        double dx
                            = ReikaRandomHelper.getRandomPlusMinus(0, 8 - w, renderRand);
                        double dz
                            = ReikaRandomHelper.getRandomPlusMinus(0, 8 - w, renderRand);
                        double h = (float
                        ) ReikaRandomHelper.getRandomBetween(7D, 24D, renderRand);
                        double dy = 16 - h;
                        while (w >= 1) {
                            ...
                            w -= ReikaRandomHelper.getRandomBetween(
                                0.5D, 1.25D, renderRand
                            );
    ...
    private void renderWater(
        ...
        if (above != Blocks.water && above != Blocks.flowing_water
            && (above != block || world.getBlockMetadata(x, y + 1, z) != meta)) {
