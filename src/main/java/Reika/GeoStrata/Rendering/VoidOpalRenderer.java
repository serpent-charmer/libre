...
public class VoidOpalRenderer extends ISBRH {
    ...
    public boolean renderWorldBlock(
        ...
        if (renderPass == 1) {
            ...
            tv5.addVertexWithUVColor(x+dx[0], y+1-co, z+dz[0], ico.getMinU(),
            ico.getMaxV(), clr); tv5.addVertexWithUVColor(x+dx[1], y+1-co, z+dz[1],
            ico.getMaxU(), ico.getMaxV(), clr); tv5.addVertexWithUVColor(x+dx[2], y+1-co,
            z+dz[2], ico.getMaxU(), ico.getMinV(), clr); tv5.addVertexWithUVColor(x+dx[3],
            y+1-co, z+dz[3], ico.getMinU(), ico.getMinV(), clr); tv5.render();*/
        }
        return flag2;
    }
    ...
    public void renderFlecks(RenderWorldEvent.Post evt) {
        if (!flecksToRender.isEmpty() && evt.pass == 1) {
            ...
            Tessellator.instance.setTranslation(
                -evt.renderer.posX, -evt.renderer.posY, -evt.renderer.posZ
            );
    ...
    private void
    renderFlecksAt(IBlockAccess world, int x, int y, int z, Block b, Tessellator v5) {
        v5.setBrightness(240);
        ...
        for (int i = 0; i < 6; i++) {
            ...
            for (int k = 0; k < n; k++) {
                //int clr = ReikaColorAPI.getModifiedHue(0xff0000,
                //ReikaRandomHelper.getRandomBetween(/*190*/128, 285, rand));
                ...
                double x0 = ReikaRandomHelper.getRandomBetween(
                    nx, mx - minsz, rand
                ); //rand.nextDouble()*(mx-minsz)+0.01;
                double y0 = ReikaRandomHelper.getRandomBetween(
                    ny, my - minsz, rand
                ); //rand.nextDouble()*(my-minsz)+0.01;
                ...
                /*
                double sx = ReikaRandomHelper.getRandomBetween(minsz, Math.min(mx-nx,
                mx-x0), rand); double sy = ReikaRandomHelper.getRandomBetween(minsz,
                Math.min(my-ny, my-y0), rand); double f = sx/sy; while (f > 2 || f < 0.5)
                { if (f > 1) { sx *= 0.9; sy /= 0.9;
                    }
                    else {
                        sy *= 0.9;
                        sx /= 0.9;
                ...
                this.drawSide(
                    world, x, y, z, fo, b, ico, v5, side, x0, y0, sx, sy, nx, ny, mx, my
                );
            }
        }
    }
    ...
    private boolean drawSide(
        ...
        boolean draw = o < 1
            ? b.shouldSideBeRendered(world, x, y, z, side.ordinal())
            : world.getBlock(x - side.offsetX, y - side.offsetY, z - side.offsetZ) != b;
        if (!draw)
            return false;
        double rnx = minX + o;
        double rpx = maxX - o;
