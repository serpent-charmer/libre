...
public class GeoEvents implements BlockRenderWatcher {
    ...
    private final Simplex3DGenerator iceGlowNoise
        = (Simplex3DGenerator) new Simplex3DGenerator(45872187).setFrequency(0.25);
    ...
    public void applyArctic(PlayerTickEvent evt) {
        if (evt.phase == Phase.START
            && ReikaEntityHelper.isInBiome(evt.player, GeoStrata.arcticSpires)
            && ReikaEntityHelper.getSkyLightAt(evt.player) > 0) {
            if (evt.player.worldObj.isRemote) {
                if ((lastIceWormSoundTick <= 0 && evt.player.ticksExisted >= 60)
                    || DragonAPICore.rand.nextInt(2400) == 0) {
                    ...
                    if (time - lastIceWormSoundTick > 20 * 90) {
                        ...
                        ReikaSoundHelper.playClientSound(
                            GeoStrata.proxy.iceWormTheme, evt.player, 1, 1, false
                        );
                    }
                }
            } else {
                if (!evt.player.isInsideOfMaterial(Material.water)) {
                    ...
                    if (evt.player.getAir()
                        < (snow ? 150 : 50)) //1/6 or 1/2 the meter if sun or snow
    ...
    public void changeArcticSpiresAmbience(PlaySoundEvent17 evt) {
        ...
        if (world != null) {
            ...
            if (b != null && b == GeoStrata.arcticSpires
                && ReikaEntityHelper.getSkyLightAt(Minecraft.getMinecraft().thePlayer)
                    > 0) {
                if (evt.name.contains("ambient.cave.cave")) {
                    evt.result
                        = new EnumSound(GeoStrata.proxy.iceWormTheme, evt.sound, false);
        ...
        if (evt.block == Blocks.packed_ice && evt.blockMetadata > 0) {
            for (int i = 0; i < evt.drops.size(); i++) {
                ...
                if (ReikaItemHelper.matchStackWithBlock(is, Blocks.packed_ice))
                    evt.drops.set(
                        i,
                        new ItemStack(Blocks.packed_ice, is.stackSize, evt.blockMetadata)
                    );
            }
        }
    }
    ...
    public void correctPackedIcePlacement(PlayerPlaceBlockEvent evt) {
        if (evt.block == Blocks.packed_ice && evt.getItem().getItemDamage() > 0) {
            evt.world.setBlock(
                ...
                evt.zCoord,
                evt.block,
                evt.getItem().getItemDamage(),
                3
            );
            ReikaSoundHelper.playPlaceSound(
                evt.world, evt.xCoord, evt.yCoord, evt.zCoord, evt.block
            );
        ...
        if (evt.type == ElementType.AIR) {
            ...
            if (mc.thePlayer.getAir() < 300) { //instead of in water
                ...
                for (int i = 0; i < full + partial; ++i) {
                    ReikaGuiAPI.instance.drawTexturedModalRect(
                        left - i * 8 - 9, top, (i < full ? 16 : 25), 18, 9, 9
                    );
                }
    ...
    public void smokeVentAir(EntityDecreaseAirEvent evt) {
        ...
        int y =
        MathHelper.floor_double(evt.entityLiving.posY-(evt.entityLiving.worldObj.isRemote
        ? 1.75 : 0.5)); int z = MathHelper.floor_double(evt.entityLiving.posZ);
        //ReikaJavaLibrary.pConsole(evt.entityLiving.worldObj.getBlock(x, y, z)+" @ "+x+",
        "+z, Side.CLIENT, evt.entityLiving instanceof EntityPlayer); if
        (evt.entityLiving.worldObj.getBlock(x, y, z) == GeoBlocks.VENT.getBlockInstance())
        { TileEntityVent te = (TileEntityVent)evt.entityLiving.worldObj.getTileEntity(x,
        y, z); if (te.getType() == VentType.SMOKE && te.isActive()) {
                evt.setResult(Result.ALLOW);
                return;
            }
        }*/
        if (evt.entityLiving instanceof EntityPlayer
            && ReikaEntityHelper.isInBiome(evt.entityLiving, GeoStrata.arcticSpires)
            && ReikaEntityHelper.getSkyLightAt(evt.entityLiving) >= 10) {
    ...
    public boolean onBlockTriedRender(
        ...
        if (block instanceof BlockPartialBounds && renderPass == 1) {
            TilePartialBounds te
                = (TilePartialBounds) render.blockAccess.getTileEntity(x, y, z);
            if (te.isFence()) {
                ...
                Tessellator.instance.setColorRGBA_I(
                    block.colorMultiplier(access, x, y, z), 96
                );
                Tessellator.instance.setBrightness(
                    block.getMixedBrightnessForBlock(access, x, y, z)
                );
                ...
                if (block.shouldSideBeRendered(
                        access, x - 1, y, z, ForgeDirection.WEST.ordinal()
                    ))
                    render.renderFaceXNeg(
                        block, x - o, y, z, BlockPartialBounds.fenceOverlay
                    );
                if (block.shouldSideBeRendered(
                        access, x + 1, y, z, ForgeDirection.EAST.ordinal()
                    ))
                    render.renderFaceXPos(
                        block, x + o, y, z, BlockPartialBounds.fenceOverlay
                    );
                if (block.shouldSideBeRendered(
                        access, x, y, z - 1, ForgeDirection.NORTH.ordinal()
                    ))
                    render.renderFaceZNeg(
                        block, x, y, z - o, BlockPartialBounds.fenceOverlay
                    );
                if (block.shouldSideBeRendered(
                        access, x, y, z + 1, ForgeDirection.SOUTH.ordinal()
                    ))
            ...
            for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
                ...
                if (block.shouldSideBeRendered(access, dx, dy, dz, dir.ordinal())) {
                    ...
                    int i1 = Minecraft.getMinecraft().theWorld.getSkyBlockTypeBrightness(
                        EnumSkyBlock.Sky, dx, dy, dz
                    );
                    //int j1 =
                    //Minecraft.getMinecraft().theWorld.getSkyBlockTypeBrightness(EnumSkyBlock.Block,
                    //dx, dy, dz);
                    int lv = (int) ReikaMathLibrary.normalizeToBounds(
                        iceGlowNoise.getValue(dx, dy, dz), 1, 14
                    );
                    ...
                    switch (dir) {
                        case DOWN:
                            Tessellator.instance.setColorOpaque_I(
                                ReikaColorAPI.getColorWithBrightnessMultiplier(c, 0.5F)
                            );
                        ...
                        case WEST:
                            Tessellator.instance.setColorOpaque_I(
                                ReikaColorAPI.getColorWithBrightnessMultiplier(c, 0.6F)
                            );
                        ...
                        case NORTH:
                            Tessellator.instance.setColorOpaque_I(
                                ReikaColorAPI.getColorWithBrightnessMultiplier(c, 0.8F)
                            );
    ...
    public void partialBoundsTooltips(ItemTooltipEvent evt) {
        if (this.isPartialBoundsBook(evt.itemStack)) {
            ...
            evt.toolTip.addAll(
                BlockBounds.readFromNBT("partialbounds", evt.itemStack.stackTagCompound)
                    .toClearString()
            );
        }
    }
    ...
    private boolean isPartialBoundsBook(ItemStack is) {
        return is != null && is.getItem() == Items.book && is.stackTagCompound != null
            && is.stackTagCompound.hasKey("partialbounds");
    }
    ...
    public void spikyFall(LivingFallEvent evt) {
        ...
        if (b == GeoBlocks.DECOGEN.getBlockInstance()
            && meta == Types.CRYSTALSPIKE.ordinal())
            evt.distance *= 1.5F;
    }
}
