...
/*
    private void checkAndConvertOre(World world, int x, int y, int z, Block b, int meta,
RockTypes r) { int count = 0; for (int i = 0; i < 6; i++) { ForgeDirection dir =
ForgeDirection.VALID_DIRECTIONS[i]; int dx = x+dir.offsetX; int dy = y+dir.offsetY; int dz
= z+dir.offsetZ; Block b2 = world.getBlock(dx, dy, dz); if (b2 instanceof BlockSmooth) {
                count++;
            }
        }
        if (count >= 2) {
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < RockTypes.rockList.length; i++) {
            ...
            icons[i] = ico.registerIcon(
                "GeoStrata:rock/" + r.getName().toLowerCase(Locale.ENGLISH)
            );
            GeoStrata.logger.debug(
                "Adding " + r.getName() + " rock icon " + icons[i].getIconName()
            );
        }
    }
