...
public class BlockCreepvine
    extends Block implements Submergeable, IPlantable /*, IShearable*/ {
    ...
    public static enum Pieces {
        ...
        public boolean canGrowSeeds() {
            return this.ordinal() >= CORE_EMPTY.ordinal()
                && this.ordinal() <= CORE_4.ordinal();
        }

        public boolean canBeHarvested() {
            return this.ordinal() > CORE_EMPTY.ordinal()
                && this.ordinal() <= CORE_5.ordinal();
        }
    ...
    public int getLightValue(IBlockAccess world, int x, int y, int z) {
        ...
        return ModList.COLORLIGHT.isLoaded()
            ? ReikaColorAPI.getPackedIntForColoredLight(0xffd620, lvl)
            : lvl;
    }
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        if (!isCreepvineOrWater(world, x, y + 1, z, true)
            || countAdjacentWater(world, x, y, z, true) < 2)
            return false;
        Block b = world.getBlock(x, y - 1, z);
        int m = world.getBlockMetadata(x, y - 1, z);
        Pieces p = world.getBlock(x, y, z) == this
            ? Pieces.list[world.getBlockMetadata(x, y, z)]
            : Pieces.ROOT;
        switch (p) {
            case CORE_EMPTY:
            case CORE_1:
            ...
            case TOP:
                return b == this
                    && (Pieces.list[m].isCore() || m == Pieces.TOP.ordinal());
            case TOP_YOUNG:
    ...
    /*
    @Override
    public ArrayList<ItemStack> onSheared(ItemStack item, IBlockAccess world, int x, int
    y, int z, int fortune) { return ReikaJavaLibrary.makeListFrom(new ItemStack(this));
    }*//*
    ...
    public static boolean canGrowOn(World world, int x, int y, int z) {
        ...
        return b == Blocks.dirt || b == Blocks.grass || b == Blocks.sand
            || b == Blocks.gravel || b == Blocks.clay;
    }

    private static boolean
    isCreepvineOrWater(World world, int x, int y, int z, boolean allowCreepvine) {
        Block b = world.getBlock(x, y, z);
        return ((b == Blocks.water || b == Blocks.flowing_water)
                && world.getBlockMetadata(x, y, z) == 0)
            || (allowCreepvine && b == GeoBlocks.CREEPVINE.getBlockInstance());
    }

    private static int
    countAdjacentWater(World world, int x, int y, int z, boolean allowCreepvine) {
        int c = 0;
    ...
    hasSurroundingWater(World world, int x, int y, int z, boolean strict) {
        for (int i = 0; i < 6; i++) {
