...
public class BlockGlowCrystal extends Block implements IWrappableBlock {
    ...
    private final SimplexNoiseGenerator hueNoiseMix
        = new SimplexNoiseGenerator(2 * System.currentTimeMillis());
    ...
    public BlockGlowCrystal(Material mat) {
        ...
        hueRanges[3]
            = new ImmutablePair(283, 27); //256 (deep purple) to 310 (hot magenta)
    ...
    getLightValue(IBlockAccess iba, int x, int y, int z) { //this should be interesting
        return (int
        ) ReikaMathLibrary.normalizeToBounds(lightNoise.getValue(x / 8D, z / 8D), 7, 15);
    }
    ...
    private int getColor(int meta, double x, double y, double z) {
        ...
        double f = 0.5
            + 0.5
                * Math.sin(Math.toRadians(y * 360 / 12D)
                ); //y%16 >= 8 ? y%8/8D : 1-(((y-8)%8)/8D);
        double n = f * n0 + (1 - f) * n1;
        int hue = hueRange.left
            + (int) (hueRange.right * n * 1); //hueNoiseY.getValue(0, y/4D));
    ...
    public boolean shouldSideBeRendered(IBlockAccess iba, int x, int y, int z, int s) {
        ...
        return super.shouldSideBeRendered(iba, x, y, z, s)
            && (b != this
                && !(
                    CarpenterBlockHandler.getInstance().isCarpenterBlock(b)
                    && b.isSideSolid(
                        iba, x, y, z, ForgeDirection.VALID_DIRECTIONS[s].getOpposite()
                    )
                ));
    }
