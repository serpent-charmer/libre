...
public class BlockGlowingVines extends BlockContainer implements ShearablePlant {
    ...
    //private final SimplexNoiseGenerator hueNoise = new
    //SimplexNoiseGenerator(System.currentTimeMillis()); private final
    //SimplexNoiseGenerator hueNoise2 = new
    //SimplexNoiseGenerator(-System.currentTimeMillis());
    ...
    @Override
    public int
    getLightValue(IBlockAccess iba, int x, int y, int z) { //this should be interesting
        return (int
        ) ReikaMathLibrary.normalizeToBounds(lightNoise.getValue(x / 8D, z / 8D), 12, 15);
    }
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        ForgeDirection dir
            = ReikaJavaLibrary.getRandomCollectionEntry(rand, te.filledSides);
        if (dir != null) {
            double dx = x + 0.5;
            double dy = y + 0.5;
            double dz = z + 0.5;
            ...
            //EntityFX fx = new EntityBlurFX(world, dx, dy,
            //dz).setScale(f).setColor(0x22aaff).setLife(ReikaRandomHelper.getRandomBetween(6,
            //15)); Minecraft.getMinecraft().effectRenderer.addEffect(fx);
            int c = 0x22aaff;
    ...
    /*
    @Override
    public MovingObjectPosition collisionRayTrace(World world, int x, int y, int z, Vec3
    from, Vec3 to) { return null;
    }
     */
    ...
    public void shearAll(World world, int x, int y, int z, EntityPlayer ep) {
        ...
        if (!ep.capabilities.isCreativeMode)
            ReikaItemHelper.dropItem(
                ...
                y + 0.5,
                z + 0.5,
                new ItemStack(this, te.filledSides.size(), 0)
            );
        world.setBlock(x, y, z, Blocks.air);
    }
    ...
    @Override
    public void
    shearSide(World world, int x, int y, int z, ForgeDirection dir, EntityPlayer ep) {
        TileGlowingVines te = (TileGlowingVines) world.getTileEntity(x, y, z);
    ...
    public int getRenderColor(int meta) {
        ...
        return this.getColor(meta, RenderManager.renderPosX+d, RenderManager.renderPosY+d,
    RenderManager.renderPosZ+d);
    }
    ...
    public static class TileGlowingVines extends TileEntity {
            ...
            while (it.hasNext()) {
                ...
                if (this.isValidSide(world, x, y, z, dir)) {
                    ...
                    ReikaItemHelper.dropItem(
                        ...
                        y + 0.5,
                        z + 0.5,
                        new ItemStack(GeoBlocks.GLOWVINE.getBlockInstance())
                    );
                    flag = true;
                }
            }
        ...
        isValidSide(World world, int x, int y, int z, ForgeDirection dir) {
            int dx = x + dir.offsetX;
            int dy = y + dir.offsetY;
            int dz = z + dir.offsetZ;
            Block b = world.getBlock(dx, dy, dz);
        ...
        public void readFromNBT(NBTTagCompound NBT) {
            ...
            for (Object o : li.tagList) {
                filledSides.add(
                    ForgeDirection.VALID_DIRECTIONS[((NBTTagInt) o).func_150287_d()]
                );
            }
