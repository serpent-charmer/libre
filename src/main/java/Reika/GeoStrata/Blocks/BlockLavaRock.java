...
public class BlockLavaRock
    extends Block implements EnvironmentalHeatSource, CustomExcavationStarBehavior {
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity e) {
        ...
        if (doEffect) {
            e.attackEntityFrom(
                meta == 0 ? DamageSource.lava : DamageSource.inFire, 3 - meta
            );
    ...
    public boolean isReplaceableOreGen(World world, int x, int y, int z, Block target) {
        ...
        return target == this || target == Blocks.stone
            || target.isReplaceableOreGen(world, x, y, z, Blocks.stone);
    }
    ...
    public void onBlockAdded(World world, int x, int y, int z) {
        if (LavaRockGenerator.instance.doingLavaRockGen
            || !ReikaWorldHelper.isChunkPastCompletelyFinishedGenerating(
                world, x >> 4, z >> 4
            ))
            return;
        int meta = world.getBlockMetadata(x, y, z) % 4;
                ...
                if (ReikaBlockHelper.matchMaterialsLoosely(Material.water, mat2)) {
                    ...
                    world.setBlock(
                        ...
                        obsidian ? Blocks.obsidian
                                 : (meta <= 1 ? Blocks.cobblestone : Blocks.stone)
                    );
                } else {
                }
            }
        ...
        ReikaWorldHelper.temperatureEnvironment(
            world, x, y, z, this.getEffectiveTemperature(meta)
        );
    }
