...
public class BlockRockDeco extends Block {
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < DecoBlocks.list.length; i++) {
            ...
            GeoStrata.logger.debug(
                "Adding " + DecoBlocks.list[i].getName() + " decorative block icon "
                + icons[i].getIconName()
            );
        }
    }
    ...
    getPlayerRelativeBlockHardness(EntityPlayer ep, World world, int x, int y, int z) {
        ...
        return 0.025F / DecoBlocks.getTypeAtCoords(world, x, y, z).getHardness()
            * is.getItem().func_150893_a(is, this);
    }
    ...
    public final float getExplosionResistance(
        ...
        return DecoBlocks.getTypeAtCoords(world, x, y, z).getResistance()
            / 5F; // /5F is in vanilla code
    }
    ...
    public final boolean canHarvestBlock(EntityPlayer player, int meta) {
        if (player.capabilities.isCreativeMode)
            return false;
        return DecoBlocks.getTypeFromMetadata(meta).isHarvestable(
