...
public class BlockOreTile extends Block implements SpecialOreBlock {
    ...
    private static void initSubs() {
        ...
        init = true;
        //ReikaJavaLibrary.pConsole(metaMap.get(ModOreList.PLATINUM, 0,
        //RockTypes.BASALT));
    }
    ...
    private static void
    setMappings(OreType ore, int index, RockTypes r, int meta, ItemStack is) {
        oreMap.put(meta, is);
    ...
    getDrops(World world, int x, int y, int z, int meta, int fortune) {
        TileEntityGeoOre te = (TileEntityGeoOre) world.getTileEntity(x, y, z);
        return te != null && te.getOreBlock() != this
            ? te.getOreBlock().getDrops(world, x, y, z, te.getOreMeta(), fortune)
            : new ArrayList();
    }

    @Override
    public void dropBlockAsItemWithChance(
        World world, int x, int y, int z, int meta, float chance, int fortune
    ) {
    ...
    canSilkHarvest(World world, EntityPlayer player, int x, int y, int z, int metadata) {
        TileEntityGeoOre te = tileCache.get(world, x, y, z);
        return te != null && te.getOreBlock() != this
            && te.getOreBlock().canSilkHarvest(world, player, x, y, z, metadata);
    }
    ...
    @Override
    public void
    onBlockHarvested(World world, int x, int y, int z, int meta, EntityPlayer ep) {
    ...
    harvestBlock(World world, EntityPlayer ep, int x, int y, int z, int meta) {
        ...
        TileEntityGeoOre te = tileCache.get(world, x, y, z);
        if (this.canSilkHarvest(world, ep, x, y, z, meta)
            && EnchantmentHelper.getSilkTouchModifier(ep)) {
            ArrayList<ItemStack> items = new ArrayList();
            items.add(new ItemStack(te.getOreBlock(), 1, te.getOreMeta()));
            //ForgeEventFactory.fireBlockHarvesting(items, world, this, x, y, z, meta,
            //0, 1.0f, true, ep);
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < RockTypes.rockList.length; i++) {
            ...
            icons[i] = ico.registerIcon(
                "geostrata:ore/" + r.name().toLowerCase(Locale.ENGLISH)
            );
        }
    }
    ...
    @Override
    public ItemStack
    getPickBlock(MovingObjectPosition mov, World world, int x, int y, int z) {
        TileEntityGeoOre te = (TileEntityGeoOre) world.getTileEntity(x, y, z);
        if (te == null)
            return null;
    ...
    @Override
    public boolean
    shouldSideBeRendered(IBlockAccess world, int dx, int dy, int dz, int s) {
    ...
    public final int colorMultiplier(IBlockAccess iba, int x, int y, int z) {
        ...
        if (tile instanceof TileEntityGeoOre) {
            ...
            /*
            String s1 = "Ore block @ "+x+", "+y+", "+z+" had its TileEntity overwritten by
            "+tile.getClass().getName()+"!"; String s2 = "This is caused by that tile's
            mod setting the TileEntity without also setting the block type!"; String s3 =
            "This is very dangerous and is a bug on the part of that mod, NOT GeoStrata!";
