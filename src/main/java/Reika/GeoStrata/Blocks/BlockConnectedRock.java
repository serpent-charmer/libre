...
public class BlockConnectedRock extends RockBlock {
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < RockTypes.rockList.length; i++) {
            ...
            icons[r.ordinal()]
                = ico.registerIcon("GeoStrata:rock/" + name.toLowerCase(Locale.ENGLISH));
            GeoStrata.logger.debug(
                "Adding " + name + " rock icon " + icons[r.ordinal()].getIconName()
            );

            for (int k = 0; k < 10; k++) {
                String e = ReikaStringParser.stripSpaces(
                    "GeoStrata:"
                    + RockShapes.getShape(this, 0).name.toLowerCase(Locale.ENGLISH) + "/"
                    + k
                );
                String s = ReikaStringParser.stripSpaces(
                    "GeoStrata:"
                    + RockShapes.getShape(this, 0).name.toLowerCase(Locale.ENGLISH) + "/"
                    + k + "_sec"
                );
    ...
    public boolean shouldSideBeRendered(IBlockAccess iba, int x, int y, int z, int side) {
        ...
        //ReikaJavaLibrary.pConsole(dir+" % "+iba.getBlock(x, y,
        //z)+":"+iba.getBlockMetadata(x, y, z)+" != "+id+":"+meta);
        return id != this
            || meta
     ...
     * computer number pad.
     */
    public ArrayList<Integer> getEdgesForFace(
        IBlockAccess world, int x, int y, int z, ForgeDirection face, RockTypes rock
    ) {
        ...
        if (face.offsetX != 0) { //test YZ
            ...
            if (RockTypes.getTypeAtCoords(world, x, y, z + 1) == rock
                && RockShapes.getShape(world, x, y, z + 1)
                    == RockShapes.getShape(this, 0))
            ...
            if (RockTypes.getTypeAtCoords(world, x, y + 1, z) == rock
                && RockShapes.getShape(world, x, y + 1, z)
                    == RockShapes.getShape(this, 0))
            ...
            if (RockTypes.getTypeAtCoords(world, x, y + 1, z + 1) == rock
                && RockShapes.getShape(world, x, y + 1, z + 1)
                    == RockShapes.getShape(this, 0)
                && !li.contains(4) && !li.contains(2))
                li.remove(new Integer(1));
            if (RockTypes.getTypeAtCoords(world, x, y - 1, z - 1) == rock
                && RockShapes.getShape(world, x, y - 1, z - 1)
                    == RockShapes.getShape(this, 0)
                && !li.contains(6) && !li.contains(8))
                li.remove(new Integer(9));
            if (RockTypes.getTypeAtCoords(world, x, y + 1, z - 1) == rock
                && RockShapes.getShape(world, x, y + 1, z - 1)
                    == RockShapes.getShape(this, 0)
                && !li.contains(4) && !li.contains(8))
                li.remove(new Integer(7));
            if (RockTypes.getTypeAtCoords(world, x, y - 1, z + 1) == rock
                && RockShapes.getShape(world, x, y - 1, z + 1)
                    == RockShapes.getShape(this, 0)
                && !li.contains(2) && !li.contains(6))
        ...
        if (face.offsetY != 0) { //test XZ
            ...
            if (RockTypes.getTypeAtCoords(world, x + 1, y, z) == rock
                && RockShapes.getShape(world, x + 1, y, z)
                    == RockShapes.getShape(this, 0))
                li.remove(new Integer(4));
            if (RockTypes.getTypeAtCoords(world, x - 1, y, z) == rock
                && RockShapes.getShape(world, x - 1, y, z)
            ...
            //Corners; only removed if have adjacent on side AND corner
            if (RockTypes.getTypeAtCoords(world, x + 1, y, z + 1) == rock
                && RockShapes.getShape(world, x + 1, y, z + 1)
                    == RockShapes.getShape(this, 0)
                && !li.contains(4) && !li.contains(2))
                li.remove(new Integer(1));
            if (RockTypes.getTypeAtCoords(world, x - 1, y, z - 1) == rock
        ...
        if (face.offsetZ != 0) { //test XY
            ...
            //Corners; only removed if have adjacent on side AND corner
            if (RockTypes.getTypeAtCoords(world, x + 1, y + 1, z) == rock
                && RockShapes.getShape(world, x + 1, y + 1, z)
                    == RockShapes.getShape(this, 0)
                && !li.contains(2) && !li.contains(4))
                li.remove(new Integer(1));
            if (RockTypes.getTypeAtCoords(world, x - 1, y - 1, z) == rock
                && RockShapes.getShape(world, x - 1, y - 1, z)
                    == RockShapes.getShape(this, 0)
                && !li.contains(8) && !li.contains(6))
                li.remove(new Integer(9));
            if (RockTypes.getTypeAtCoords(world, x + 1, y - 1, z) == rock
                && RockShapes.getShape(world, x + 1, y - 1, z)
                    == RockShapes.getShape(this, 0)
    ...
    public ArrayList<Integer> getSectionsForTexture(
        IBlockAccess world, int x, int y, int z, ForgeDirection face, RockTypes rock
    ) {
        ArrayList<Integer> li = new ArrayList();
        li.addAll(allDirs);
