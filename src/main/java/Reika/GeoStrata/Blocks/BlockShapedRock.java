...
public class BlockShapedRock extends RockBlock {
    ...
    private static final IIcon[][] textures
        = new IIcon[RockTypes.rockList.length][RockShapes.shapeList.length];
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < RockTypes.rockList.length; i++) {
            for (int k = 0; k < RockShapes.shapeList.length; k++) {
                ...
                //int b = type.ordinal();
                String suff = k + "_" + i;
                if (RockShapes.shapeList[k]
                        .name.toLowerCase(Locale.ENGLISH)
                        .contains("connected"))
                    suff = "0_" + i;
                textures[i][k]
                    = ico.registerIcon("GeoStrata:shaped/unstitched/tile" + suff);
                //GeoStrata.logger.debug("Adding "+type.getName()+" "+shapeType+" icon
                //"+icons[i].getIconName()); texturesLoaded = true;
            }
        }
        /*
        for (int i = 0; i < blends.length; i++) {
            blendicons[i] =
        ico.registerIcon("GeoStrata:shaped/"+blends[i]+"/"+shapeType+"base");
        }*/
        /*
        int cols = RockShapes.shapeList.length;
        int rows = RockTypes.rockList.length;
        ReikaImageLoader.unstitchIconsFromSheet(textures, ico, "geostrata:shaped/sheet",
        cols, rows);
         */
    }
