...
public class BlockPartialBounds extends BlockContainer {
    ...
    public int getLightValue(IBlockAccess world, int x, int y, int z) {
        ...
        if (te == null || te.renderBlock == null
            || te.renderBlock.blockID instanceof BlockPartialBounds)
            return 0;
        return te.renderBlock.blockID.getLightValue(
            new IconDelegateAccess(world, x, y, z), x, y, z
        ); //delegate since calls getBlock()
    }
    ...
    public int colorMultiplier(IBlockAccess world, int x, int y, int z) {
        ...
        return b instanceof RockBlock
            ? ((RockBlock) b).getColor(call, x, y, z, RockTypes.getTypeFromID(b))
            : b.colorMultiplier(call, x, y, z);
    }
    ...
    public IIcon getIcon(IBlockAccess world, int x, int y, int z, int s) {
        ...
        return te.renderBlock.blockID.getIcon(
            new IconDelegateAccess(world, x, y, z), x, y, z, s
        );
    }
    ...
    public float getExplosionResistance(
        ...
        TilePartialBounds te = (TilePartialBounds) world.getTileEntity(x, y, z);
        if (te == null || te.renderBlock == null)
            return Blocks.cobblestone.getExplosionResistance(
                e, world, x, y, z, ex, ey, ez
            );
        return te.renderBlock.blockID.getExplosionResistance(
            e, world, x, y, z, ex, ey, ez
        );
    }
    ...
    public boolean shouldSideBeRendered(IBlockAccess iba, int x, int y, int z, int s) {
        ...
        TilePartialBounds te = (TilePartialBounds
        ) iba.getTileEntity(x - dir.offsetX, y - dir.offsetY, z - dir.offsetZ);
        if (te == null)
            return true;
        ...
        switch (dir) {
            case UP:
                return us.positiveY < 1 || at.getBlockBoundsMinY() > 0
                    || at.getBlockBoundsMinX() > us.negativeX
                    || at.getBlockBoundsMaxX() < us.positiveX
                    || us.positiveZ > at.getBlockBoundsMaxZ()
                    || us.negativeZ < at.getBlockBoundsMinZ();
            case DOWN:
                return us.negativeY > 0 || at.getBlockBoundsMaxY() < 1
                    || at.getBlockBoundsMinX() > us.negativeX
                    || at.getBlockBoundsMaxX() < us.positiveX
                    || us.positiveY > at.getBlockBoundsMaxZ()
                    || us.negativeY < at.getBlockBoundsMinZ();
            case EAST:
                return us.positiveX < 1 || at.getBlockBoundsMinX() > 0
                    || at.getBlockBoundsMinZ() > us.negativeZ
                    || at.getBlockBoundsMaxZ() < us.positiveZ
                    || us.positiveY > at.getBlockBoundsMaxY()
                    || us.negativeY < at.getBlockBoundsMinY();
            case WEST:
                return us.negativeX > 0 || at.getBlockBoundsMaxX() < 1
                    || at.getBlockBoundsMinZ() > us.negativeZ
                    || at.getBlockBoundsMaxZ() < us.positiveZ
                    || us.positiveY > at.getBlockBoundsMaxY()
                    || us.negativeY < at.getBlockBoundsMinY();
            case SOUTH:
                return us.positiveZ < 1 || at.getBlockBoundsMinZ() > 0
                    || at.getBlockBoundsMinX() > us.negativeX
                    || at.getBlockBoundsMaxX() < us.positiveX
                    || us.positiveY > at.getBlockBoundsMaxY()
                    || us.negativeY < at.getBlockBoundsMinY();
            case NORTH:
                return us.negativeZ > 0 || at.getBlockBoundsMaxZ() < 1
                    || at.getBlockBoundsMinX() > us.negativeX
                    || at.getBlockBoundsMaxX() < us.positiveX
                    || us.positiveY > at.getBlockBoundsMaxY()
                    || us.negativeY < at.getBlockBoundsMinY();
    ...
    public boolean onBlockActivated(
        ...
        if (InterfaceCache.IWRENCH.instanceOf(is) || n.contains("wrench")
            || n.contains("screwdriver") || n.contains("hammer")) {
            double d = 0.03125;
            te.bounds = te.bounds.cut(
                ForgeDirection.VALID_DIRECTIONS[s], ep.isSneaking() ? -d : d
            );
            //te.bounds.writeToNBT("partialbounds",
            //ReikaPlayerAPI.getDeathPersistentNBT(ep));
            world.markBlockForUpdate(x, y, z);
    ...
    private void
    changeCover(TilePartialBounds te, World world, int x, int y, int z, ItemStack is) {
        ...
        if (te.renderBlock != null) {
            //ReikaItemHelper.dropItem(world, x+0.5, y+0.5, z+0.5,
            //te.renderBlock.asItemStack());
        }
    ...
    public void setBounds(
        ...
        ((TilePartialBounds) world.getTileEntity(x, y, z)).bounds
            = new BlockBounds(nx, ny, nz, px, py, pz);
        world.markBlockForUpdate(x, y, z);
    }
    ...
    private static class IconDelegateAccess implements IBlockAccess {
        ...
        private final BlockKey[][][] data
            = new BlockKey[range * 2 + 1][range * 2 + 1][range * 2 + 1];
        ...
        public Block getBlock(int x, int y, int z) {
            ...
            int dy = y - lowerY;
            int dz = z - lowerZ;
            if (dx < 0 || dy < 0 || dz < 0 || dx >= data.length || dy >= data.length
                || dz >= data.length)
