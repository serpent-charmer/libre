...
public class BlockOreVein extends BlockContainer implements IWailaDataProvider {
    ...
    public static enum VeinType {
        ...
        private void addDefaultItem(ItemStack b, float wt) {
            defaultItems.put(
                Item.itemRegistry.getNameForObject(b.getItem()) + ":" + b.getItemDamage(),
                wt
            );
        }
    }
    ...
    public int getLightValue(IBlockAccess world, int x, int y, int z) {
        return isRenderCenter && VeinType.list[world.getBlockMetadata(x, y, z)].glow()
            ? 12
            : 0;
    }
    ...
    public void registerBlockIcons(IIconRegister ico) {
        ...
        for (VeinType v : VeinType.list) {
            v.itemIcon = ico.registerIcon(
                "geostrata:orevein/" + v.name().toLowerCase(Locale.ENGLISH)
            );
        }
    }
            ...
            if (left == 0 || v.ores.isEmpty()) {
                ...
                for (HarvestableOre ore : v.ores.getValues()) {
                    tip.add(String.format(
                        "%s: %3.2f%%",
                        ore.item.getDisplayName(),
                        v.ores.getProbability(ore) * 100
                    ));
                }
                if (tv.isInfinite())
    ...
    public static void loadConfigs() {
        ...
        File f
            = new File(GeoStrata.config.getConfigFolder(), "GeoStrataOreVeinConfig.lua");
        if (f.exists()) {
            try {
                ...
                if (e instanceof UserErrorException)
                    throw new InstallationException(
                        GeoStrata.instance,
                        "Configs could not be loaded! Correct them and try again.",
                        e
                    );
                else
            ...
            for (VeinType type : VeinType.list) {
                ...
                bk.putData(
                    "harvestLimit", type == VeinType.NETHER ? 15 : 8 - type.ordinal() * 2
                );
                bk.setComment(
                    "harvestLimit",
                    "How many items to allow harvesting of before a vein block is depleted."
                );
                bk.putData("type", id);
                ...
                bk.setComment(
                    "innerIcon",
                    "The namespaced registry name of the block type to use as the inner-material icon. Optional."
                );
        ...
        VeinType.ICE.ores.addEntry(
            new HarvestableOre(new ItemStack(GeoStrata.lowTempDiamonds), 20), 20
        );
    }
    ...
    private static void parseOreEntry(String type, LuaBlock b)
        ...
        if (!Strings.isNullOrEmpty(s)) {
            ...
            if (bk == null) {
                GeoStrata.logger.logError(
                    "No such block for item '" + s + "', skipping icon"
                );
                return;
            }
    ...
    public static class TileOreVein extends TileEntity {
        ...
        public float getRichness() {
            return 1
                - Math.max(0, harvestsUsed) / (float) this.getType().maximumHarvestCycles;
        }
