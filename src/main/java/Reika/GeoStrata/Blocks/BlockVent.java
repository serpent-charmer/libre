...
public class BlockVent extends Block implements MinerBlock, EnvironmentalHeatSource {
    ...
    public IIcon getIcon(int s, int meta) {
        return this.fetchIcon(
            s,
            meta,
            meta == VentType.ENDER.ordinal()      ? 1
                : meta == VentType.PYRO.ordinal() ? -1
                                                  : 0
        );
    }
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < icons.length; i++) {
            ...
            icons[i] = ico.registerIcon(
                "geostrata:vent/" + v.name().toLowerCase(Locale.ENGLISH) + "_top"
            );
            iconsNether[i] = ico.registerIcon(
                "geostrata:vent/nether/" + v.name().toLowerCase(Locale.ENGLISH) + "_top"
            );
            internal[i] = ico.registerIcon(
                "geostrata:vent/" + v.name().toLowerCase(Locale.ENGLISH) + "_inside"
            );
        }
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) { /*
         ...
         List<EntityLivingBase> li = world.getEntitiesWithinAABB(EntityLivingBase.class,
         box); for (int i = 0; i < li.size(); i++) { EntityLivingBase e = li.get(i);
        ...
        world.scheduleBlockUpdate(
            x, y, z, this, this.tickRate(world) + rand.nextInt(2400)
        );
    }
    ...
    public void harvestBlock(World world, EntityPlayer ep, int x, int y, int z, int meta)
    {
        if (EnchantmentHelper.getSilkTouchModifier(ep)) {
            ...
            ReikaItemHelper.dropItem(world, x+0.5, y+0.5, z+0.5, new ItemStack(this, 1,
    meta/2));
        }
        else {
    ...
    getHarvestItems(World world, int x, int y, int z, int meta, int fortune) {
    ...
    public static class TileEntityVent extends TileEntity {
        ...
        private void activate() {
            ...
            ReikaSoundHelper.playSoundAtBlock(
                worldObj, xCoord, yCoord, zCoord, "fire.ignite"
            );
            if (this.getType() == VentType.GAS) {
                if (worldObj.getBlock(xCoord, yCoord + 1, zCoord) == Blocks.fire
                    || worldObj.getBlock(xCoord, yCoord + 1, zCoord) == Blocks.lava) {
                    this.explode(1.5F);
                }
            }
        }
        ...
        private void explode(float factor) {
            ...
            boolean fire
                = type == VentType.FIRE || type == VentType.LAVA || type == VentType.PYRO;
            worldObj.newExplosion(
                null,
                xCoord,
                yCoord,
                zCoord,
                factor * type.getExplosionSizeFactor(),
                fire,
                true
            );
        }
        ...
        public boolean canFire() {
            return this.canTick(worldObj, xCoord, zCoord)
                && !this.isBlocking(worldObj, xCoord, yCoord + 1, zCoord);
        }
        ...
        private void onTick() {
            if (activeTimer % type.getSoundInterval() == 0) {
                switch (type) {
                    ...
                    case GAS:
                        ReikaSoundHelper.playSoundAtBlock(
                            worldObj, xCoord, yCoord, zCoord, "random.fizz", 0.25F, 1.5F
                        );
                        break;
                    case SMOKE:
                        ReikaSoundHelper.playSoundAtBlock(
                            worldObj, xCoord, yCoord, zCoord, "random.fizz", 0.25F, 0.25F
                        );
                        break;
                    case WATER:
                        if (activeTimer % 32 == 0) {
                            ReikaSoundHelper.playSoundAtBlock(
                                worldObj, xCoord, yCoord, zCoord, "liquid.water", 2F, 1F
                            );
                            ReikaSoundHelper.playSoundAtBlock(
                                worldObj, xCoord, yCoord, zCoord, "liquid.water", 2F, 1F
                            );
            ...
            if (!worldObj.isRemote) {
                ...
                AxisAlignedBB box = this.getEffectBox();
                List<EntityLivingBase> li =
            worldObj.getEntitiesWithinAABB(EntityLivingBase.class, box); for
            (EntityLivingBase e : li) { e.setAir(Math.max(0, e.getAir()-1));
                }
            }*/
                ...
                if (type.appliesWorldTemperature() && rand.nextInt(20) == 0) {
                    ...
                    for (int i = 1; i < 5; i++)
                        ReikaWorldHelper.temperatureEnvironment(
                            worldObj, xCoord, yCoord + i, zCoord, temp, true, null
                        );
                }
            }
            ...
            if (p != null) {
                int n = p == ReikaParticleHelper.FLAME ? 3
                    : p == ReikaParticleHelper.RAIN    ? 8
                                                       : 1;
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if (p == ReikaParticleHelper.MOBSPELL || p == ReikaParticleHelper.LAVA
                        || p == ReikaParticleHelper.RAIN)
                    ...
                    worldObj.spawnParticle(
                        p.name, px, py + type.getParticleYOffset(), pz, vx, vy, vz
                    );
                }
            }
        }
        ...
        private AxisAlignedBB getEffectBox() {
            ...
            return AxisAlignedBB.getBoundingBox(
                xCoord, yCoord + 1, zCoord, xCoord + 1, yCoord + i + 1, zCoord + 1
            );
        }
    ...
    public static enum VentType {
        ...
        private AxisAlignedBB getEffectBox(TileEntityVent te) {
            switch (this) {
                ...
                case GAS:
                case PYRO:
                    return ReikaAABBHelper.getBlockAABB(te).expand(3, 3, 3).offset(
                        0, 2, 0
                    );
                case ENDER:
                    return ReikaAABBHelper.getBlockAABB(te).expand(2, 2, 2).offset(
                        0, 1, 0
                    );
                default:
                    return null;
        ...
        private void applyEntityEffect(EntityLivingBase e, Random rand) {
            switch (this) {
                case WATER:
                    ...
                    break;
                case SMOKE:
                    e.getEntityData().setLong(
                        SMOKE_VENT_TAG, e.worldObj.getTotalWorldTime()
                    );
                    break;
                case GAS:
                ...
                case ENDER:
                    ...
                    while (flag && tries < 40) {
                        ...
                        flag = !e.worldObj.getCollidingBoundingBoxes(e, e.boundingBox)
                                    .isEmpty()
                    ...
                    if (!flag) {
                        ...
                        e.worldObj.playSoundEffect(
                            ox, oy, oz, "mob.endermen.portal", 1.0F, 1.0F
                        );
                    }
                    break;
                case PYRO:
                    ...
                    if (rand.nextInt(20) == 0) {
                        ...
                        if (rand.nextInt(3) == 0) {
                            BlockTickEvent.fire(
                                ...
                                rz,
                                world.rand,
                                UpdateFlags.getForcedUnstoppableTick()
                                    + UpdateFlags.NATURAL.flag
                            );
                        }
                    }
                    break;
