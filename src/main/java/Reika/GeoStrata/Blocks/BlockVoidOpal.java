...
public class BlockVoidOpal extends Block implements RockProofStone, IWrappableBlock {
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            z,
            256,
            ReikaParticleHelper.BONEMEAL.ordinal(),
            3
        );
    }
    ...
    @Override
    public boolean
    canEntityDestroy(IBlockAccess world, int x, int y, int z, Entity entity) {
            ...
            for (int i = -1; i <= 1; i++) {
                for (int k = -1; k <= 1; k++) {
                    ...
                    if (world.getBlock(dx, dy, dz) == this
                        && !this.isBlockedOnSide(world, dx, dy, dz, side)) {
                        return true;
                    }
                }
            }
    ...
    private boolean
    isBlockedOnSide(IBlockAccess world, int x, int y, int z, ForgeDirection side) {
