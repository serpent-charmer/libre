...
public class RockGetter {
    ...
    static {
        try {
            modClass = Class.forName(
                "Reika.GeoStrata.GeoStrata", false, RockGetter.class.getClassLoader()
            );
    ...
    /**
     * Returns an itemstack of the specified rock type and shape. Refer to the GeoStrata
     * "RockTypes" and "RockShapes" classes for valid options. Many, but not all, shapes
     * of the same rock are submetadatas of that parent block type. No block type
     * represents two rock types.
     */
    public static ItemStack getRockItem(String type, String shape) {
        ...
        try {
            ...
            System.out.println(
                "GeoStrata rock type " + type + " and shape " + shape
                + " threw exception while fetching!"
            );
            e.printStackTrace();
            return null;
        }
    ...
    /**
     * Returns the block of the specified rock type and shape. Same result, but faster,
     * as calling Block.getBlockFromItem(getRockItem().getItem()). Args: String names of
     * the two enum constants.
     */
    ...
    public static Collection<Block> getAllBlocksFor(String type) {
        ...
        for (int i = 0; i < shapes.length; i++) {
            try {
                ...
                System.out.println(
                    "GeoStrata rock type " + type + " and shape " + shapes[i]
                    + " threw exception while fetching its blocks!"
                );
                e.printStackTrace();
                return null;
            }
