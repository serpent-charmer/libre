...
@Mod(
    modid = GeoStrata.MOD_NAME,
    name = GeoStrata.MOD_NAME,
...
public class GeoStrata extends DragonAPIMod {
    ...
    public static final GeoConfig config
        = new GeoConfig(instance, GeoOptions.optionList, null);
    ...
    public static CreativeTabs tabGeoStairs
        = new GeoTabStairs(GeoStrata.MOD_NAME + " Stairs");
    public static CreativeTabs tabGeoSlabs
        = new GeoTabSlab(GeoStrata.MOD_NAME + " Slabs");
    ...
    @SidedProxy(
        clientSide = "Reika.GeoStrata.GeoClient", serverSide = "Reika.GeoStrata.GeoCommon"
    )
    ...
    public static final CustomStringDamageSource coldDamage
        = new CustomStringDamageSource("froze to death");
    ...
    public void preload(FMLPreInitializationEvent evt) {
        ...
        if (ModList.CHISEL.isLoaded()
            && ReikaJavaLibrary.doesClassExist(
                "com.cricketcraft.chisel.api.carving.ICarvingGroup"
            )) {
    ...
    public void load(FMLInitializationEvent event) {
        ...
        RockGeneratorTypes type
            = RockGeneratorTypes.getType(GeoOptions.ROCKGEN.getString());
        RockGenerator.instance.registerGenerationPattern(type.getGenerator());

        RetroGenController.instance.addHybridGenerator(
            RockGenerator.instance, Integer.MIN_VALUE
        );
        RetroGenController.instance.addHybridGenerator(VentGenerator.instance, 0);
        RetroGenController.instance.addHybridGenerator(
            LavaRockGeneratorRedesign.instance, 0
        );
        RetroGenController.instance.addHybridGenerator(DecoGenerator.instance, 0);
        RetroGenController.instance.addHybridGenerator(
            CreepvineGenerator.instance, Integer.MIN_VALUE
        );
        RetroGenController.instance.addHybridGenerator(
            ArcticSpiresGenerator.instance, Integer.MIN_VALUE
        );
        RetroGenController.instance.addHybridGenerator(
            GlowCrystalGenerator.instance, Integer.MIN_VALUE
        );
        ...
        OreDictionary.registerOre(
            ReikaOreHelper.DIAMOND.getDropOreDictName(), lowTempDiamonds
        );
        ...
        if (ModList.THERMALEXPANSION.isLoaded()) {
            for (int i = 0; i < RockTypes.rockList.length; i++) {
                ...
                ThermalRecipeHelper.addPulverizerRecipe(
                    smooth, cobble, energy
                ); //make proportional to hardness
                ThermalRecipeHelper.addPulverizerRecipe(
                    cobble,
        ...
        if (ModList.FORESTRY.isLoaded()) {
            if (BackpackManager.backpackItems != null
                && BackpackManager.backpackItems.length > 0) {
                for (int i = 0; i < RockTypes.rockList.length; i++) {
                    RockTypes r = RockTypes.rockList[i];
        ...
        for (int i = 0; i < RockTypes.rockList.length; i++) {
            for (int k = 0; k < RockShapes.shapeList.length; k++) {
                ...
                FMLInterModComms.sendMessage(
                    ModList.BCTRANSPORT.modLabel, "add-facade", is
                );
            }
        }
    ...
    private void addIC2Recipes() {
        ...
        ReikaRecipeHelper.addSmelting(
            in,
            ReikaItemHelper.getSizedItemStack(IC2Handler.IC2Stacks.RUBBER.getItem(), 2),
            0
        );
        Recipes.extractor.addRecipe(
            new RecipeInputItemStack(in),
            null,
            ReikaItemHelper.getSizedItemStack(IC2Handler.IC2Stacks.RUBBER.getItem(), 4)
        );
    }
    ...
    public void postload(FMLPostInitializationEvent evt) {
        ...
        if (ModList.ROTARYCRAFT.isLoaded()) {
            for (int i = 0; i < RockTypes.rockList.length; i++) {
                ...
                RecipeInterface.grinder.addAPIRecipe(
                    cobble, new ItemStack(Blocks.gravel)
                );

                for (int k = 0; k < RockShapes.shapeList.length; k++) {
                    RockShapes shape = RockShapes.shapeList[k];
                    BlockColorInterface.addGPRBlockColor(
                        rock.getID(shape), rock.rockColor
                    );
                }
            }
        }

        WorldGenInterceptionRegistry.instance.addException(new InterceptionException(
        ) { //performance
            @Override
        ...
        if (ModList.THAUMCRAFT.isLoaded()) {
            for (int i = 0; i < RockTypes.rockList.length; i++) {
                ...
                //ReikaThaumHelper.addAspects(rockblock, Aspect.STONE,
                //(int)(rock.blockHardness/5));
            }

            ReikaThaumHelper.addAspectsToBlockMeta(
                GeoBlocks.VENT.getBlockInstance(),
                VentType.WATER.ordinal(),
                Aspect.WATER,
                2,
                Aspect.EARTH,
                2
            ...
            if (Aspect.getAspect("ira") != null)
                ReikaThaumHelper.addAspectsToBlockMeta(
                    GeoBlocks.VENT.getBlockInstance(),
                    VentType.PYRO.ordinal(),
                    Aspect.getAspect("ira"),
                    5
                );
            ReikaThaumHelper.addAspectsToBlockMeta(
                GeoBlocks.VENT.getBlockInstance(),
                VentType.CRYO.ordinal(),
                Aspect.EARTH,
                4,
                Aspect.COLD,
                5
            ...
            ReikaThaumHelper.addAspectsToBlock(
                GeoBlocks.GLOWCRYS.getBlockInstance(),
                Aspect.EARTH,
                1,
                Aspect.CRYSTAL,
                2,
            ...
            ReikaThaumHelper.addAspectsToBlock(
                GeoBlocks.GLOWVINE.getBlockInstance(), Aspect.PLANT, 2, Aspect.LIGHT, 2
            );

            ReikaThaumHelper.addAspectsToBlockMeta(
                GeoBlocks.DECOGEN.getBlockInstance(),
                0,
                Aspect.CRYSTAL,
                2,
                Aspect.WATER,
            ...
            ReikaThaumHelper.addAspectsToBlock(
                GeoBlocks.VOIDOPAL.getBlockInstance(),
                Aspect.EARTH,
                2,
                Aspect.CRYSTAL,
                1,
            ...
            if (Aspect.getAspect("principia") != null)
                ReikaThaumHelper.addAspectsToBlock(
                    GeoBlocks.VOIDOPAL.getBlockInstance(),
                    Aspect.getAspect("principia"),
                    1
                );

            ReikaThaumHelper.addAspectsToBlockMeta(
                GeoBlocks.LAVAROCK.getBlockInstance(), 0, Aspect.EARTH, 2, Aspect.FIRE, 4
            );
            ReikaThaumHelper.addAspectsToBlockMeta(
                GeoBlocks.LAVAROCK.getBlockInstance(), 1, Aspect.EARTH, 2, Aspect.FIRE, 3
            );
            ReikaThaumHelper.addAspectsToBlockMeta(
                GeoBlocks.LAVAROCK.getBlockInstance(), 2, Aspect.EARTH, 2, Aspect.FIRE, 2
            );
            ReikaThaumHelper.addAspectsToBlockMeta(
                GeoBlocks.LAVAROCK.getBlockInstance(), 3, Aspect.EARTH, 2, Aspect.FIRE, 1
            );
        }
        ...
        if (ModList.EXTRAUTILS.isLoaded()) {
            ...
            if (id != null) {
                ItemStack burned
                    = new ItemStack(id, 1, ExtraUtilsHandler.getInstance().burntQuartz);
                ReikaRecipeHelper.addSmelting(
                    RockTypes.QUARTZ.getItem(RockShapes.SMOOTH), burned, 0.05F
                );
            }
        }
    ...
    private static void loadClasses() {
        ReikaRegistryHelper.instantiateAndRegisterBlocks(
            instance, GeoBlocks.blockList, blocks
        );
        ...
        //GameRegistry.registerTileEntity(TileEntityOreConverter.class,
        //"geostrataoreconvert");
    ...
    public static void loadDictionary() {
        for (int i = 0; i < RockTypes.rockList.length; i++) {
            ...
            ItemStack slab = new ItemStack(
                GeoBlocks.SLAB.getBlockInstance(),
                1,
                ItemBlockAnyGeoVariant.getStack(type, RockShapes.COBBLE)
            );
            ...
            if (type != RockTypes.QUARTZ) //Nether quartz is "quartz"
                OreDictionary.registerOre(
                    type.getName().toLowerCase(Locale.ENGLISH), rock
                );
        }
        OreDictionary.registerOre(
            "sandstone", RockTypes.SANDSTONE.getItem(RockShapes.SMOOTH)
        );
    ...
    public static int
    getOpalPositionColor(IBlockAccess iba, double x, double y, double z) {
        //int sc = 48;
        ...
        float hue1 = (float
        ) (y / (n * 2) + (d * z / (n * 6) + 0.5 + 0.5 * Math.sin((d * x) / (n * 2))));
