...
public abstract class GeoEDTrade implements EDCommodityHook {
    ...
    private static final double REDUCTION_EXPONENT
        = 0.998; //value raised to this power for every surplus item over thresh until
                 //thresh 2
    private static final int REDUCTION_THRESHOLD_2 = 1024;
    private static final double REDUCTION_EXPONENT_2
        = 0.985; //value raised to this power for every surplus item over thresh 2
    ...
    public void onPriceReceived(int average, int upper, int max) {
        ...
        double fac = Math.pow(price / baselinePrice, 2)
            * Math.pow((double) max / peakPrice, 6); //was ^2*
        emeraldValue = Math.min(
            64, MathHelper.ceiling_double_int(128 * fac)
        ); //used to be 32 and have *inputStack.stackSize/inputStack.getMaxStackSize()
        GeoStrata.logger.log(
            "Received " + this.getCommodityID() + " price data: " + average + "/" + upper
            + "/" + max + " -> " + price + "/" + fac + " -> " + emeraldValue
            + "x emeralds per " + inputStack.stackSize
        );
    ...
    private class Trade extends MerchantRecipe implements PlayerSpecificTrade {
        ...
        public boolean isValid(EntityPlayer ep) {
            ...
            int amt = ReikaInventoryHelper.countItem(
                          inputStack.getItem(), ep.inventory.mainInventory
                      )
                + Math.max(0, 2 * (baseValue - 1));
            ...
            this.getItemToBuy().stackSize
                = Math.min(this.getItemToBuy().getMaxStackSize(), currentPrice);
            return true;
        }
    }
}
