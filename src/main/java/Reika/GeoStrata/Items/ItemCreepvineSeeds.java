...
public class ItemCreepvineSeeds extends Item {
    ...
    public boolean onItemUse(
        ...
        y += dir.offsetY;
        z += dir.offsetZ;
        if (!world.isRemote && BlockCreepvine.canGrowOn(world, x, y - 1, z)
            && BlockCreepvine.hasSurroundingWater(world, x, y, z, true)) {
            boolean flag = false;
            int tries = 0;
            while (!flag && tries < 25) {
                flag = CreepvineGenerator.instance.generate(
                    world, x, y, z, itemRand, 8, 9, 0.8F, false
                );
                tries++;
            }
            if (flag) {
                ReikaSoundHelper.playPlaceSound(
                    world, x, y, z, GeoBlocks.CREEPVINE.getBlockInstance()
                );
                is.stackSize--;
                return true;
            }
