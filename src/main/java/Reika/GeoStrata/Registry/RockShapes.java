...
public enum RockShapes {
    ...
    private static final EnumMap<RockShapes, EnumMap<RockTypes, Block>> blockMap
        = new EnumMap(RockShapes.class);
    private static final HashMap<Integer, ArrayList<RockShapes>> offsetMap
        = new HashMap();
    ...
    public Block register(RockTypes r) {
            ...
            if (!needsOwnBlock) {
                ...
                if (li == null)
                    throw new RegistrationException(
                        GeoStrata.instance,
                        "Rock shape " + this + " has no pair mappings!"
                    );
        ...
        if (map.containsKey(r)) {
            throw new RegistrationException(
                GeoStrata.instance,
                "Block type for " + r + " " + this + " was created twice!"
            );
        } else {
            try {
                ...
                String name = "geostrata_rock_"
                    + (r.name() + "_" + this.name()).toLowerCase(Locale.ENGLISH);
                b.setBlockName(name);
                ...
                throw new RegistrationException(
                    GeoStrata.instance,
                    "Block type for " + r + " " + this
                        + " could not be created: " + e.getLocalizedMessage()
                );
            }
        }
    }
    ...
    public Block getBlock(RockTypes r) {
        ...
        if (map == null || !map.containsKey(r))
            return null; //throw new RegistrationException(GeoStrata.instance, "Rock shape
                         //"+this+" has no block for "+r+"!");
        else
            return map.get(r);
    }
