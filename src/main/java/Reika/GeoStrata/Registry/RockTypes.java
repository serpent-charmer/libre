...
public enum RockTypes {
    ...
    public boolean isHarvestable(ItemStack held) {
        ...
        if (TinkerToolHandler.getInstance().isPick(held)
            || TinkerToolHandler.getInstance().isHammer(held)) {
            switch (harvestTool) {
                case WOOD:
                    return true;
                case STONE:
        ...
        if (held.getItem() == RedstoneArsenalHandler.getInstance().pickID) {
            return RedstoneArsenalHandler.getInstance().pickLevel
                >= harvestTool.getHarvestLevel();
        }
        Item i = held.getItem();
        switch (harvestTool) {
        ...
        switch (this) {
            ...
            case SLATE:
                if (BiomeDictionary.isBiomeOfType(
                        world.getBiomeGenForCoords(x, z), Type.SANDY
                    ))
                    return false;
                if (BiomeDictionary.isBiomeOfType(
                        world.getBiomeGenForCoords(x, z), Type.DRY
                    ))
                    return false;
                if (BiomeDictionary.isBiomeOfType(
                        world.getBiomeGenForCoords(x, z), Type.SAVANNA
                    ))
                    return false;
                return true;
            case ONYX:
    ...
    public boolean canGenerateAtSkipXZ(World world, int x, int y, int z, Random r) {
        ...
        switch (this) {
            ...
            case SLATE:
                return true;
            case ONYX:
                return ReikaWorldHelper.checkForAdjMaterial(world, x, y, z, Material.lava)
                    != null;
            case SCHIST:
                break;
            case OPAL:
