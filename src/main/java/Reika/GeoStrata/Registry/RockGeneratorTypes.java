    ...
    public static RockGeneratorTypes getType(String config) {
        try {
            ...
            throw new InstallationException(
                GeoStrata.instance,
                "Invalid selected rock generation pattern '" + config
                    + "'; choose one of the following: " + Arrays.toString(values())
            );
        }
    }
    ...
    public RockGenerationPattern getGenerator() {
        try {
            ...
            throw new RegistrationException(
                GeoStrata.instance,
                "Could not create rock generator for type " + this + "!",
                e
            );
        } catch (IllegalAccessException e) {
            throw new RegistrationException(
                GeoStrata.instance,
                "Could not access constructor for rock generator for type " + this + "!",
                e
            );
        }
    }
