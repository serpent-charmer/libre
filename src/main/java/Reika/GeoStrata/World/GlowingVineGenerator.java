...
public class GlowingVineGenerator implements RetroactiveGenerator {
    ...
    private static final int PER_CHUNK
        = getVineAttemptsPerChunk(); //calls per chunk; vast majority fail
    ...
    public static boolean canGenerateAt(World world, int x, int y, int z) {
        return isValidBiome(world, x, z) && world.getBlock(x, y, z).isAir(world, x, y, z)
    ...
    private static boolean isValidBiome(World world, int x, int z) {
        ...
        return b instanceof BiomeGenForest || b instanceof BiomeGenTaiga
            || b instanceof BiomeGenJungle
            || BiomeDictionary.isBiomeOfType(b, Type.FOREST)
            || BiomeDictionary.isBiomeOfType(b, Type.JUNGLE);
    }
