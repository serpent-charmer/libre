...
public class LavaRockGeneratorRedesign implements RetroactiveGenerator {
    public static final LavaRockGeneratorRedesign instance
        = new LavaRockGeneratorRedesign();
            ...
            for (int x = 0; x < 16; x++) {
                for (int z = 0; z < 16; z++) {
                    for (int y = 1; y <= 14; y++) {
                        ...
                        if (b.isReplaceableOreGen(
                                world, x + chunkX * 16, y, z + chunkZ * 16, Blocks.stone
                            )
    ...
    private void placeBlock(Chunk c, int x, int y, int z, int meta, Block at) {
        if (meta > 3)
            return;
        if (at == GeoBlocks.LAVAROCK.getBlockInstance()
            && meta >= c.getBlockMetadata(x, y, z)) {
            return;
        }
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (dy >= 0 && dx >= 0 && dz >= 0 && dx < 16 && dz < 16) {
                    ...
                    if (b.isReplaceableOreGen(
                        ...
                        || b == GeoBlocks.LAVAROCK.getBlockInstance()) {
                        //ReikaJavaLibrary.pConsole("Placing "+(meta+1)+" from "+meta+" at
                        //"+new Coordinate(dx, dy, dz));
                        this.placeBlock(
                            c, dx, dy, dz, meta + this.getMetaStep(c, dx, dy, dz), b
                        );
                    }
                }
            }
    ...
    private int getMetaStep(Chunk c, int cx, int y, int cz) {
        ...
        return (int) MathHelper.clamp_double(
                ...
                if (dy > 0 && dy <= 11 && dx >= 0 && dz >= 0 && dx < 16 && dz < 16) {
                    ...
                    if ((b == Blocks.lava || b == Blocks.flowing_lava) && meta == 0) {
                        return d; //ret = Math.min(ret, d);
                         ...
                         if (ret != Integer.MAX_VALUE && ret > meta+2) //+2 because one is
                     subtracted again above ReikaJavaLibrary.pConsole("Found lava rock
                     with meta "+meta+", setting ret from "+ret+" to "+(meta+2)); ret =
                     Math.min(ret, meta+2);
                     }*/
                }
            }
    ...
    private void generate(World world, int x, int y, int z) {
        ...
        for (int i = 3; i >= 0; i--) {
            for (Coordinate c : arrays[i].keySet()) {
                ...
                if (bk.isReplaceableOreGen(
                        world, c.xCoord, c.yCoord, c.zCoord, Blocks.stone
                    )
                    || bk == GeoBlocks.LAVAROCK.getBlockInstance()) {
