...
public class VentGenerator implements RetroactiveGenerator {
    ...
    private static final int PER_CHUNK
        = getVentAttemptsPerChunk(); //calls per chunk; vast majority fail
    ...
    public void generate(
        ...
        if (world.getWorldInfo().getTerrainType() != WorldType.FLAT) {
            ...
            for (int i = 0; i < PER_CHUNK; i++) {
                ...
                int maxy = world.provider.dimensionId == -1
                    ? 128
                    : (world.provider.dimensionId == 1 ? 72 : 64);
    ...
    private VentType
    getVentTypeFor(World world, int posX, int posY, int posZ, Random random) {
        if (world.provider.dimensionId == 1) {
            return VentType.ENDER;
        }

        WeightedRandom<VentGen> wr
            = world.provider.dimensionId == -1 ? ventTypesNether : ventTypes;
        wr.setRNG(random);
        for (VentGen gr : wr.getValues()) {
    ...
    private static class VentGen implements DynamicWeight {
        ...
        private void calcWeight(World world, int x, int y, int z) {
            float f = Math.min(
                1, Math.max(0.25F, world.provider.getAverageGroundLevel() / 64F)
            );
            weight = type.getSpawnWeight((int) (y / f), world.provider.dimensionId == -1);
            if (type == VentType.CRYO
                && !ReikaBiomeHelper.isSnowBiome(world.getBiomeGenForCoords(x, z))) {
                weight = 0;
            }
        }
