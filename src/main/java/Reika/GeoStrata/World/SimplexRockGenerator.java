...
public class SimplexRockGenerator implements RockGenerationPattern {
    ...
    @Override
    public void
    generateRockType(RockTypes geo, World world, Random rand, int chunkX, int chunkZ) {
            ...
            for (int z = chunkZ; z < chunkZ + 16; z++) {
                if (geo.canGenerateAtXZ(world, x, z, rand)) {
                    for (int y = geo.minY; y <= geo.maxY; y++) {
                        ...
                        if (val > gen.noiseThreshold
                            && geo.canGenerateAtSkipXZ(world, x, y, z, rand)) {
                            Block b = world.getBlock(x, y, z);
                            int meta = world.getBlockMetadata(x, y, z);
                            if (this.canGenerateIn(world, x, y, z, b, meta)) {
                                world.setBlock(x, y, z, gen.blockID, 0, 2);
                            } /*
                             else if (RockGenerator.instance.generateOres() &&
                             ReikaBlockHelper.isOre(b, meta)) { TileEntityGeoOre te = new
                             TileEntityGeoOre(); te.initialize(geo, b, meta);
                                 world.setBlock(x, y, z,
                             GeoBlocks.ORETILE.getBlockInstance()); world.setTileEntity(x,
                             y, z, te);
                             }*/
                        }
                    }
    ...
    private boolean canGenerateIn(World world, int x, int y, int z, Block b, int meta) {
        if (b instanceof RockProofStone
            && ((RockProofStone) b).blockRockGeneration(world, x, y, z, b, meta))
            return false;
