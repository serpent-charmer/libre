                            ...
                            for (int dz = k1; dz <= j2; dz++) {
                                ...
                                if (d12 * d12 + d13 * d13 + d14 * d14 < 1.0D) {
                                    ...
                                    if (this.canGenerateIn(world, dx, dy, dz, b, meta)) {
                                        ...
                                        //long dur = time2-time;
                                        //ReikaJavaLibrary.pConsole(rock+" block set time
                                        //for "+dx+", "+dy+", "+dz+": "+dur+" nanos:
                                        //"+this.isInChunk(x, z, dx, dz));
                                    } /*
                                     else if (RockGenerator.instance.generateOres() &&
                                     ReikaBlockHelper.isOre(b, meta)) { TileEntityGeoOre
                                     te = new TileEntityGeoOre(); te.initialize(rock, b,
                                     meta); world.setBlock(dx, dy, dz,
                                     GeoBlocks.ORETILE.getBlockInstance());
