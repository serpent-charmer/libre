    ...
    public void generate(
        ...
        if (this.generateIn(world)) {
            for (int i = 0; i < Decorations.list.length; i++) {
                ...
                if (random.nextInt(Math.max(
                        1, (int) (p.getGenerationChance() / GeoOptions.getDecoDensity())
                    ))
                    == 0) {
            ...
            switch (this) {
                ...
                case OCEANSPIKES:
                    return ReikaBiomeHelper.isOcean(b)
                        && ReikaWorldHelper.getDepthFromBelow(
                               world, x, y - 1, z, FluidRegistry.WATER
                           )
                        > 2;
                default:
                    return true;
        ...
        private boolean generate(World world, int x, int y, int z, Random rand) {
            switch (this) {
                case OCEANSPIKE:
                    ...
                    while (h < 15
                           && (world.getBlock(x, y + h + d, z) == Blocks.water
                               || (h < min
                                   && world.getBlock(x, y + h + 1, z) == Blocks.water))) {
                        world.setBlock(
                            x, y + h, z, GeoBlocks.DECOGEN.getBlockInstance(), 0, 3
                        );
                        h++;
                    }
                    return true;
                ...
                case OREVEINS:
                    ...
                    for (int i = 0; i < amt; i++) {
                        ...
                        if (world.getBlock(dx, dy, dz)
                            == vein.template) { //exact block since texture match
                            int adj = ReikaWorldHelper.countAdjacentBlocks(
                                world, dx, dy, dz, Blocks.air, false
                            );
                            if (adj > 0 && adj < 3)
                                world.setBlock(
                                    dx,
