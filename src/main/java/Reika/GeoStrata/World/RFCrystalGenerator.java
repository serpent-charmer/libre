...
public class RFCrystalGenerator implements RetroactiveGenerator {
    ...
    private static final int PER_CHUNK
        = getCrystalAttemptsPerChunk(); //calls per chunk; vast majority fail
        ...
        if (world.getWorldInfo().getTerrainType() != WorldType.FLAT) {
            ...
            for (int i = 0; i < PER_CHUNK; i++) {
                ...
                if (this.canGenerateAt(world, posX, posY, posZ)) {
                    world.setBlock(
                        posX, posY, posZ, GeoBlocks.RFCRYSTALSEED.getBlockInstance(), 1, 2
                    );
                }
            }
        }
    ...
    public static boolean canGenerateAt(World world, int x, int y, int z) {
        return world.getBlock(x, y, z) == Blocks.redstone_ore
            && ReikaWorldHelper.checkForAdjMaterial(world, x, y, z, Material.air) == null;
    }
