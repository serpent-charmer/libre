...
public class ArcticSpiresGenerator implements RetroactiveGenerator {
    ...
    public void generate(
        ...
        if (world.getWorldInfo().getTerrainType() != WorldType.FLAT
            && Math.abs(world.provider.dimensionId) != 1) {
            chunkX *= 16;
            chunkZ *= 16;
            ...
            if (biome == BiomeGenBase.icePlains || biome == BiomeGenBase.iceMountains) {
                ...
                for (int dx = chunkX; dx < chunkX+16; dx++) {
                    for (int dz = chunkZ; dz < chunkZ+16; dz++) {
                        ...
                        while (y > 0 && (ReikaWorldHelper.softBlocks(world, dx, y, dz) ||
                world.getBlock(dx, y, dz).isWood(world, dx, y, dz) || world.getBlock(dx,
                y, dz).isLeaves(world, dx, y, dz))) y--; setBlock(world, dx, y, dz,
                Blocks.wool, m, 2);/* setBlock(world, dx, 91, dz, Blocks.standing_sign);
                        TileEntitySign te = (TileEntitySign)world.getTileEntity(dx, 91,
                dz); te.signText[0] = String.format("%.1f", pos.getDistanceTo(x,
                pos.yCoord, z));*//*
			}
		}*/
            }
    ...
    private boolean isGennableZone(int x, int z) {
        ...
        return currentClosestZone != null
            && currentClosestZone.getDistanceTo(x, currentClosestZone.yCoord, z) <= 120;
    }
    ...
    public void setSeed(World world) {
        ...
        if (seed != s || mainNoise == null) {
            ...
            zoneNoise
                = (VoronoiNoiseGenerator) new VoronoiNoiseGenerator(seed).setFrequency(
                    0.0024D
                );
            iceLayerNoiseLarge
                = (SimplexNoiseGenerator) new SimplexNoiseGenerator(-seed).setFrequency(
    ...
    private int generateCluster(World world, int x0, int z0, Random random, int amt) {
        double baseTilt
            = /*random.nextDouble()*360;*/ (currentClosestZone.hashCode() * 2387.183)
            % 360D;
        return this.generateCluster(world, x0, z0, random, amt, baseTilt);
    }

    public int generateCluster(
        World world, int x0, int z0, Random random, int amt, double baseTilt
    ) {
        int count = 0;
        ...
        for (int i = 0; i < amt; i++) {
            ...
            BiomeGenBase biome = world.getBiomeGenForCoords(x, z);
            while (!ignoreBiome && biome != BiomeGenBase.icePlains
                   && biome != BiomeGenBase.iceMountains
                   && biome != GeoStrata.arcticSpires) {
            ...
            if (sp.generate(world, x, y, z, tilt, random)) {
                ...
                if (GeoStrata.arcticSpires != null) {
                    ...
                    for (double d = 0; d < 360; d += 0.33) {
                        ...
                        for (double dr = 0; dr <= r; dr += 0.5) {
                            ...
                            if (at == BiomeGenBase.icePlains
                                || at == BiomeGenBase.iceMountains) {
                                ReikaWorldHelper.setBiomeForXZ(
                                    world, dx, dz, GeoStrata.arcticSpires, false
                                );
                            }
                        }
                    }
                ...
                if (VeinType.ICE.isEnabled()) {
                    ...
                    while (veins < max && !veinAttempts.isEmpty()) {
                        Coordinate c = ReikaJavaLibrary.getAndRemoveRandomCollectionEntry(
                            random, veinAttempts
                        );
                        if (c.yCoord == 0 || c.yCoord >= sp.lipYBottom
                            || c.yCoord >= Math.max(sp.lipYBottom - 2, y + 3))
                            continue;
                        boolean flag2 = false;
                        int air = 0;
                        int self = 0;
                        ...
                        if (flag2 && air == 1) {
                            this.setBlock(
                                ...
                                c.yCoord,
                                c.zCoord,
                                GeoBlocks.OREVEIN.getBlockInstance(),
                                VeinType.ICE.ordinal(),
                                2
                            );
                            veins++;
                        }
                ...
                for (Entry<Coordinate, Integer> e : sp.columns.entrySet()) {
                    ...
                    if (ty <= 82) {
                        this.setBlock(
                            world, c.xCoord, ty, c.zCoord, Blocks.packed_ice, 1, 2
                        );
                    }
                    if (sp.core.contains(c.to2D())) {
                        for (int dy = 50; dy < sp.lipYBottom; dy++) {
                            Block at = world.getBlock(c.xCoord, dy, c.zCoord);
                            if (at == Blocks.grass || at == Blocks.sand
                                || at == Blocks.dirt) {
                                this.setBlock(
                                    world, c.xCoord, dy, c.zCoord, Blocks.packed_ice, 1, 2
                                );
                            }
                        }
                    } else {
                        for (int dy = ty - 1; dy >= 60; dy--) {
                            Block at = world.getBlock(c.xCoord, dy, c.zCoord);
                            if (at.isWood(world, c.xCoord, dy, c.zCoord)
                                || at.isLeaves(world, c.xCoord, dy, c.zCoord)) {
                                ...
                                if (ab == Blocks.snow_layer
                                    || ab.isAir(world, c.xCoord, dy + 1, c.zCoord)) {
                                    if (ab != Blocks.snow_layer)
                                        this.setBlock(
                                            world,
                                            c.xCoord,
                            ...
                            if (at != GeoBlocks.DECOGEN.getBlockInstance()
                                && at != Blocks.packed_ice
                                && !ReikaWorldHelper.softBlocks(
                                    world, c.xCoord, dy, c.zCoord
                                ))
                                break;
                        }
                    }
                }
                MinecraftForge.EVENT_BUS.post(new ArcticSpireGenerationEvent(
                    world,
                    x,
                    y,
                    z,
                    random,
                    Collections.unmodifiableSet(sp.core),
                    Collections.unmodifiableSet(snowCover)
                ));
            }
        }
        return count;
    ...
    private void
    setBlock(World world, int x, int y, int z, Block b, int meta, int flags) {
    ...
    private class ArcticSpire {
        ...
        private boolean isValidGroundBlock(World world, int x, int y, int z) {
            ...
            return b == Blocks.grass || b == Blocks.dirt || b == Blocks.sand
                || b == Blocks.stone || b == Blocks.gravel
        ...
        private boolean isValidAirBlock(World world, int x, int y, int z) {
            ...
            return ReikaWorldHelper.softBlocks(world, x, y, z)
                || b.getMaterial() == Material.plants || b.isWood(world, x, y, z)
        ...
        private boolean
        generate(World world, int x, int y0, int z, double tilt, Random rand) {
            int minY = 999;
            int maxY = 0;
            ...
            for (int i = -r; i <= r; i++) {
                for (int k = -r; k <= r; k++) {
                    ...
                    int y = world.getTopSolidOrLiquidBlock(dx, dz) - 1;
                    while (y > 0
                           && (ReikaWorldHelper.softBlocks(world, dx, y, dz)
                               || world.getBlock(dx, y, dz).isWood(world, dx, y, dz)
                               || world.getBlock(dx, y, dz).isLeaves(world, dx, y, dz)))
                        y--;
            ...
            double r1 = Math.min(
                Math.max(r0, rLip - 4.5),
                ReikaRandomHelper.getRandomBetween(8D, 12D, rand)
            );
            ...
            lipYBottom = dy;
            LobulatedCurve lb
                = LobulatedCurve.fromMinMaxRadii(rLip - 3, rLip + 2, 4, true);
            lb.generate(rand);
            LobulatedCurve slopeFactor
                = LobulatedCurve.fromMinMaxRadii(0.7, 1.25, 3, true);
            ...
            for (int i = -r; i <= r; i++) {
                for (int k = -r; k <= r; k++) {
                    ...
                    for (int j = 0; j < h2; j++) {
                        double rL = ReikaMathLibrary.linterpolate(
                            j * slopeAt, 0, h2 - 1, rAt, r1
                        );
                        if (d <= rL) {
                            int dx = x + i;
                            int dz = z + k;
                            ...
                            if (j == 0 && d >= rL - 0.5 && rand.nextInt(2) == 0) {
                                if (world.getBlock(dx, dy - 1, dz)
                                        .isAir(world, dx, dy - 1, dz))
                                    this.setBlock(
                                        world,
                                        dx,
                                        dy - 1,
            ...
            for (int j = 0; j < h3; j++) {
                for (int i = -r; i <= r; i++) {
                    for (int k = -r; k <= r; k++) {
                        ...
                        int dz = z + k;
                        boolean ring
                            = Math.abs(dy + j - this.getIceLayerHeight(dx, dz)) < 2;
                        ...
                        if (d <= rL) {
                            this.setBlock(
                                world,
                                dx + MathHelper.floor_double(tiltX * j),
                                dy + j,
                                dz + MathHelper.floor_double(tiltZ * j),
                                Blocks.packed_ice,
                                d <= rL - 1.75 || ring ? 2 : 1
                            );
                        }
