...
public class BiomeArcticSpires
    extends BiomeGenSnow implements CustomMapColorBiome, CustomTemperatureBiome {
    ...
    @Override
    public int getSurfaceTemperatureModifier(
        World world, int x, int y, int z, float temp, float sun
    ) {
        int ret = (int
        ) (-25 + sun * 20); //-25 at night, -5 in day -> -50C at night, -30 in day
        if (world.isRaining())
            ret -= 10;
        return ret;
    }
    ...
    @Override
    public int
    getAltitudeTemperatureModifier(World world, int x, int y, int z, float temp, int dy) {
