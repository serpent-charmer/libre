...
public class VoidOpalGenerator implements RetroactiveGenerator {
    ...
    public boolean canGenerateAt(World world, int chunkX, int chunkZ) {
        return world.provider.dimensionId == 1
            && this.isAppropriateChunk(world, chunkX, chunkZ);
    }

    private boolean isAppropriateChunk(World world, int chunkX, int chunkZ) {
        if (ModList.CHROMATICRAFT.isLoaded()
            && ChromatiAPI.getAPI().worldgen().getEndIslandBias(world, chunkX, chunkZ)
                > 0)
            return false;
        return true;
    }
            ...
            for (int i = -rxi; i <= rxi; i++) {
                for (int j = -ryi; j <= ryi; j++) {
                    for (int k = -rzi; k <= rzi; k++) {
                        if (ReikaMathLibrary.isPointInsideEllipse(i+0.5, j+0.5, k+0.5, rx,
            ry, rz)) { int dx = x+i; int dy = y+j; int dz = z+k; blocks.add(new
            Coordinate(dx, dy, dz));
                        }
                    }
                }
            ...
            for (int i = 0; i < nspikes; i++) {
                ...
                for (double d = 0; d <= len; d += 0.25) {
                    ...
                    this.generateBallAt(
                        ...
                        dy,
                        dz,
                        ReikaMathLibrary.linterpolate(d, 0, len, r1, r0)
                    );
                }
            }
        }
