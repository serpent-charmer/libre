...
public class SFBees {
    ...
    private static final BeeBranch branch = new BeeBranch(
        "branch.pinkforest",
        "Pink Birch",
        "Silva Roseus",
        "These bees seem to be native to a strangely colored and elevated forest."
    );
    private static final TreeBranch treeBranch = new TreeBranch(
        "branch.pinkforesttree",
        "Pink Birch",
        "Silva Roseus",
        "These trees populate a rare elevated forest, and are far more durable than they look."
    );
    private static final ButterflyBranch butterflyBranch = new ButterflyBranch(
        "branch.pinkforestfly",
        "Pink Birch",
        "Silva Roseus",
        "These butterflies can rarely be found flitting among giant pink trees."
    );
    ...
    private static final Reika.DragonAPI.ModInteract.Bees.ButterflyAlleleRegistry
        .Life blinkLife
        = Reika.DragonAPI.ModInteract.Bees.ButterflyAlleleRegistry.Life.createNew(
            "sprite", 1, false
        );
    ...
    public static void register() {
        ...
        if (Loader.isModLoaded("ExtraBees")) {
            paleberrySpecies.addBreeding(
                (IAlleleBeeSpecies
                ) AlleleManager.alleleRegistry.getAllele("extrabees.species.fruit"),
                baseSpecies,
                18
            );
        } else {
        ...
        if (ModList.MAGICBEES.isLoaded()) {
            slugSpecies.addBreeding(
                (IAlleleBeeSpecies
                ) AlleleManager.alleleRegistry.getAllele("magicbees.speciesDoctoral"),
                baseSpecies,
                5
            );
            ...
            slugSpecies.addBreeding(
                (IAlleleBeeSpecies
                ) AlleleManager.alleleRegistry.getAllele("extrabees.species.unusual"),
                baseSpecies,
                4
            );
        } else {
    ...
    private static void loadColorData() {
        paleberryColor
            = new ColorBlendList(18F, 0xFF6672, 0xFF4032, 0xFF668C, 0xFF7266, 0xFF4C4C);
    ...
    static abstract class SFBee extends BeeSpecies {
        ...
        @Override
        protected BeeBreeding
        createBreeding(IAlleleBeeSpecies p1, IAlleleBeeSpecies p2, int chance) {
    ...
    private static boolean isPinkForest(IBeeHousing ibh) {
        ...
        return Satisforestry.isPinkForest(world, c.posX, c.posZ)
            || (ModList.CHROMATICRAFT.isLoaded()
                && isLumenAlveary(ibh, world, c, false, true));
    }

    @ModDependent(ModList.CHROMATICRAFT)
    private static boolean isLumenAlveary(
        IBeeHousing ibh, World world, ChunkCoordinates cc, boolean omni, boolean infi
    ) {
        TileEntityLumenAlveary te
            = ChromaBeeHelpers.getLumenAlvearyController(ibh, world, cc);
        return te != null && (!omni || te.hasOmnipresence())
            && (!infi || te.hasInfiniteAwareness());
    }

    private static class SFBeeBreeding extends BeeBreeding {
        private SFBeeBreeding(
            IAlleleBeeSpecies p1, IAlleleBeeSpecies p2, int chance, SFBee bee
        ) {
    ...
    static class BaseSFBee extends SFBee {
        private BaseSFBee() {
            ...
            this.addProduct(
                SFBlocks.GRASS.getStackOfMetadata(GrassTypes.PEACH_FRINGE.ordinal()), 2
            );
            this.addSpecialty(
                SFBlocks.GRASS.getStackOfMetadata(GrassTypes.BLUE_MUSHROOM_TOP.ordinal()),
                1
            );
        }
    ...
    static class PaleberryBee extends SFBee {
        ...
        @Override
        public int getOutlineColor() {
            return paleberryColor != null
                ? paleberryColor.getColor(DragonAPICore.getSystemTimeAsInt() / 40D)
                : 0xFF6672;
        }
    ...
    static class SlugBee extends SFBee {
        private SlugBee() {
            ...
            this.addSpecialty(
                SFBlocks.TERRAIN.getStackOfMetadata(TerrainType.OUTCROP.ordinal()), 1
            );
            this.addSpecialty(
                SFBlocks.TERRAIN.getStackOfMetadata(TerrainType.POISONROCK.ordinal()), 1
            );
            this.addSpecialty(
                SFBlocks.TERRAIN.getStackOfMetadata(TerrainType.PONDROCK.ordinal()), 1
            );
        }
        ...
        @Override
        public int getOutlineColor() {
            return slugColor != null
                ? slugColor.getColor(DragonAPICore.getSystemTimeAsInt() / 15D)
                : 0xFFFFFF;
        }
    ...
    static final class AlleleSlugEffect extends BasicGene implements IAlleleBeeEffect {
        ...
        public IEffectData doEffect(IBeeGenome ibg, IEffectData ied, IBeeHousing ibh) {
            ...
            if (world.rand.nextInt(2000) == 0) {
                ...
                EntityPlayer ep = world.getClosestPlayer(
                    cc.posX + 0.5, cc.posY + 0.5, cc.posZ + 0.5, 64
                );
                if (ep != null) {
                    AxisAlignedBB box
                        = ReikaAABBHelper.getBlockAABB(cc.posX, cc.posY, cc.posZ)
                              .expand(12, 4, 12);
                    List<EntityMob> li
                        = world.getEntitiesWithinAABB(EntityMob.class, box);
                    if (li.size() <= 2) {
                        int n = ReikaRandomHelper.getRandomBetween(1, 4);
                        this.spawnMobsAround(
                            world, cc.posX + 0.5, cc.posY + 0.5, cc.posZ + 0.5, 9, 4, n
                        );
                    }
                }
            }
        ...
        private void
        trySpawnPowerSlug(World world, ChunkCoordinates cc, int[] r, IBeeHousing ibh) {
            for (int x = cc.posX - r[0]; x <= cc.posX + r[0]; x++) {
                for (int y = cc.posY - r[1]; y <= cc.posY + r[1]; y++) {
                    for (int z = cc.posZ - r[2]; z <= cc.posZ + r[2]; z++) {
                        if (Math.abs(x - cc.posX) + Math.abs(y - cc.posY)
                                + Math.abs(z - cc.posZ)
                            <= 5)
                            continue;
                ...
                for (int i = 0; i < 6; i++) {
                    ...
                    if (BlockPowerSlug.canExistOn(
                        ...
                        BlockPowerSlug.generatePowerSlugAt(
                            world, x, y, z, world.rand, tier, false, -1, true, 6, dir
                        );
                        break;
                    }
                }
        ...
        private void spawnMobsAround(
            World world, double x, double y, double z, double rxz, double ry, int n
        ) {
            Class<? extends EntityMob> c
                = world.rand.nextBoolean() ? EntityCaveSpider.class : EntitySpider.class;
    ...
    private static final class AllelePaleberry
        extends BasicGene implements IAlleleFlowers {
        private final FlowerProviderPaleberry flowers = new FlowerProviderPaleberry();

        public AllelePaleberry() {
            super(
                "flower.paleberry", "Ripe Paleberry", EnumBeeChromosome.FLOWER_PROVIDER
            );
        }
    ...
    private static final class FlowerProviderPaleberry extends BasicFlowerProvider {
        private FlowerProviderPaleberry() {
            super(
                ...
                GrassTypes.PALEBERRY_NEW.ordinal(),
                "paleberry"
            );
            FlowerManager.flowerRegistry.registerAcceptableFlowerRule(
                this, this.getFlowerType()
            );
        }
    ...
    private static class PinkTree
        extends BasicTreeSpecies implements PinkTreeBlockCallback {
        ...
        public IIcon getLeafIcon(boolean pollinated, boolean fancy) {
            return ((BlockPinkLeaves) SFBlocks.LEAVES.getBlockInstance())
                .getIcon(PinkTreeTypes.TREE.getBaseLeaf().getItemDamage(), fancy);
        }
        ...
        @Override
        protected BlockKey getLogBlock(
            ITreeGenome genes,
            World world,
            int x,
            int y,
        ...
        @Override
        protected boolean
        generate(World world, int x, int y, int z, Random rand, ITreeGenData data) {
            int girth = data.getGirth(world, x, y, z);
            world.setBlock(x, y, z, Blocks.air);
            treeData = data;
            PinkTreeGenerator gen
                = (PinkTreeGenerator) PinkTreeTypes.TREE.constructTreeGenerator();
        ...
        public boolean placeLog(World world, int x, int y, int z, Block b, int meta) {
            treeData.setLogBlock(
                world, x, y, z, ReikaBlockHelper.getColumnBlockDirection(meta)
            );
            return true;
        }
    ...
    private static class PaleberryFruit implements IAlleleFruit {
        ...
        public String getUnlocalizedName() {
            return this.getName(
            ); //"chromaberry."+color.name().toLowerCase(Locale.ENGLISH);
        }
    ...
    private static class BerryProvider
        implements IFruitProvider, NoLocaleDescriptionFruit {
        ...
        public int getColour(
            ITreeGenome genome, IBlockAccess world, int x, int y, int z, int ripeningTime
        ) {
        ...
        @Override
        public boolean
        markAsFruitLeaf(ITreeGenome genome, World world, int x, int y, int z) {
            return genome != null && world != null
                && world.rand.nextFloat() < genome.getYield();
        }
        ...
        @Override
        public ItemStack[] getFruits(
            ITreeGenome genome, World world, int x, int y, int z, int ripeningTime
        ) {
            float f = ripeningTime / (float) this.getRipeningPeriod();
        ...
        @Override
        public boolean
        trySpawnFruitBlock(ITreeGenome genome, World world, int x, int y, int z) {
            return false;
        }
    ...
    private static abstract class SFButterfly extends BasicButterflySpecies {
        ...
        public final String getEntityTexture() {
            return DirectResourceManager
                .getResource(
                    "Reika/Satisforestry/Textures/Butterflies/"
                    + this.getTextureName().toLowerCase(Locale.ENGLISH) + ".png"
                )
                .toString();
        }
        ...
        public final EnumSet<Type> getSpawnBiomes() {
            return EnumSet.of(
                Type.FOREST, BiomeDictionary.getTypesForBiome(Satisforestry.pinkforest)
            );
        }
    ...
    private static class BasicSFButterfly extends SFButterfly {
        ...
        @Override
        public Reika.DragonAPI.ModInteract.Bees.ButterflyAlleleRegistry.Speeds
        getSpeed() {
        ...
        @Override
        public Reika.DragonAPI.ModInteract.Bees.ButterflyAlleleRegistry.Fertility
        getFertility() {
            return Reika.DragonAPI.ModInteract.Bees.ButterflyAlleleRegistry.Fertility
                .NORMAL;
        }

        @Override
        public Reika.DragonAPI.ModInteract.Bees.ButterflyAlleleRegistry.Territory
        getTerritorySize() {
            return Reika.DragonAPI.ModInteract.Bees.ButterflyAlleleRegistry.Territory
                .DEFAULT;
        }

        @Override
        public Reika.DragonAPI.ModInteract.Bees.ButterflyAlleleRegistry.Life
        getLifespan() {
            return Reika.DragonAPI.ModInteract.Bees.ButterflyAlleleRegistry.Life
                .SHORTENED;
        }
    }
    ...
    private static class GrassSpriteButterfly extends SFButterfly {
        ...
        public Reika.DragonAPI.ModInteract.Bees.ButterflyAlleleRegistry.Speeds
        getSpeed() {
            return Reika.DragonAPI.ModInteract.Bees.ButterflyAlleleRegistry.Speeds
                .FASTEST;
        }
    ...
    private static class PaleberryBoostEffect
        extends BasicGene implements IAlleleButterflyEffect {
        public PaleberryBoostEffect() {
            super(
                "sfbutterfly.paleberryboost",
                "Paleberry Fertilization",
    ...
    public static void
    tickPaleberryButterflyEffect(EntityLivingBase e, IEntityButterfly b) {
        if (e.ticksExisted % 32 == 0) {
            EntityCreature ec = b.getEntity();
            boolean flag1 = false;
            boolean flag2 = false;
            for (EntityAITaskEntry task :
                 ((List<EntityAITaskEntry>) ec.tasks.taskEntries)) {
        ...
        if (EntityAIButterflyPollinatePaleberries.isValidTarget(e.worldObj, x, y, z)) {
            e.worldObj.setBlockMetadataWithNotify(
                x, y, z, GrassTypes.PALEBERRY_NEW.ordinal(), 2
            );
            ReikaSoundHelper.playBreakSound(
                e.worldObj, x, y, z, Blocks.leaves, 0.7F, 0.25F
            );
