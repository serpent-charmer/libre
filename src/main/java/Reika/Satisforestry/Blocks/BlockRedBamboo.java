...
public class BlockRedBamboo extends Block implements Reedlike, IPlantable, IShearable {
    ...
    public BlockRedBamboo() {
        ...
        growthLimit = (SimplexNoiseGenerator
        ) new SimplexNoiseGenerator(this.getClass().getName().hashCode())
                          .setFrequency(1 / 2D);
    }
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        /*
        if (world.getBlock(x, y-1, z) == Blocks.reeds || this.checkStability(world, x, y,
        z)) { if (world.isAirBlock(x, y+1, z)) { int l;
    ...
    private boolean tryGrow(World world, int x, int y, int z, Random rand) {
        ...
        if (meta < 8) {
            if (meta == 0) {
                int limit = (int
                ) ReikaMathLibrary.normalizeToBounds(growthLimit.getValue(x, z), 3, 8);
                if (h < limit) {
                    world.setBlock(x, y + h, z, this, 7, 3);
                    ReikaSoundHelper.playPlaceSound(
                        world, x, y + h, z, Blocks.leaves, 0.85F, 0.6F
                    );
                    return true;
                } else {
                    return false;
    ...
    public boolean canPlaceBlockAt(World world, int x, int y, int z) {
        ...
        return b == this || b == Blocks.sand
            || b.canSustainPlant(world, x, y - 1, z, ForgeDirection.UP, this);
    }
