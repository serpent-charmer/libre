...
public class BlockResourceNode extends BlockContainer
    implements PointSpawnBlock, IWailaDataProvider, IMoveCheck, ILaputaImmobile {
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        if (rand.nextInt(n) == 0) {
            double px = x + 0.5
                + rand.nextGaussian(); //ReikaRandomHelper.getRandomBetween(x-1.5, x+2.5);
            double pz = z + 0.5
                + rand.nextGaussian(); //ReikaRandomHelper.getRandomBetween(z-1.5, z+2.5);
            double py = ReikaRandomHelper.getRandomBetween(y + 1.0625, y + 1.375);
            EntityBlurFX fx
                = new EntityBlurFX(world, px, py, pz, IconPrefabs.FADE.getIcon());
            fx.setScale((float) ReikaRandomHelper.getRandomBetween(0.6, 1.2))
                .setLife(ReikaRandomHelper.getRandomBetween(3, 6));
    ...
    public static abstract class ResourceNode<R extends NodeResource>
        extends TileCaveSpawner {
    ...
    public static class TileResourceNode extends ResourceNode<ResourceItem> {
        ...
        public int getOverlayColor() {
            ...
            if (r == null)
                return 0xffffff;
            float l = worldObj == null
                ? 0
                : Math.max(
                    worldObj.getSavedLightValue(
                        EnumSkyBlock.Block, xCoord, yCoord + 1, zCoord
                    ),
                    worldObj.getSavedLightValue(
                        EnumSkyBlock.Sky, xCoord, yCoord + 1, zCoord
                    ) * worldObj.getSunBrightnessFactor(0)
                );
            float a = 1 - l / 24F;
            int c = r.color;
            if (a < 1) {
                ...
                if (resource == null) {
                 ...
                 if (SFOptions.SIMPLEAUTO.getState()) {
                     ...
                     if (simpleAutoOutputTimer == 0) {
                         ...
                         if (te instanceof IInventory && !(te instanceof
                 TileNodeHarvester)) { ItemStack is = this.getRandomNodeItem(); if (is !=
                 null) { if (ReikaInventoryHelper.addToIInv(is, (IInventory)te)) {
                                     this.resetTimer();
                                 }
                             }
                         }
                ...
                if (manualMiningCycle >= MINING_TIME) {
                    ...
                    if (is != null)
                        ReikaItemHelper.dropItem(
                            worldObj, xCoord + 0.5, yCoord + 1, zCoord + 0.5, is
                        );
                    manualMiningCycle = 0;
                }
            }
        ...
        private ItemStack getRandomNodeItem(int tier, boolean manual) {
            ...
            return ReikaItemHelper.getSizedItemStack(
                resource.getItem(ri),
                ri.getAmount(purity, tier, manual, peaceful, DragonAPICore.rand)
            );
        }
