...
public class BlockPowerSlug
    extends BlockContainer implements PointSpawnBlock, Submergeable {
    ...
    public BlockPowerSlug(Material mat) {
        ...
        this.setBlockBounds(
            0.5F - max / 2F, 0, 0.5F - max / 2F, 0.5F + max / 2F, height, 0.5F + max / 2F
        );
    ...
    public static TilePowerSlug generatePowerSlugAt(
        World world,
        int x,
        int y,
        int z,
    ...
    public static TilePowerSlug generatePowerSlugAt(
        ...
        while (dir.offsetY != 0 && y > 0 && y < 256
               && ReikaWorldHelper.softBlocks(
                   world, x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ
               ))
            y--;
        if (y < 256 && canReplace(world, x, y, z)
            && canExistOn(world, x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ)) {
                ...
                switch (tier) {
                    case 0:
                        if (gas || reachDifficulty > 0 || rand.nextBoolean())
                            ...
                            te.setDefaultSpawn(
                                rand.nextInt(4) == 0 ? EntityCaveSpider.class
                                                     : EntitySpider.class
                            );
                        break;
                    case 1:
                        if (gas) {
                            ...
                            if (reachDifficulty >= 2) {
                                ...
                                if (f <= 0.45) {
                                    te.setDefaultSpawn(
                                        rand.nextBoolean() ? EntityCaveSpider.class
                                                           : EntitySpider.class
                                    );
                                    ...
                                    te.spawnCallback
                                        = new SpitterDistribution(SpitterType.BASIC);
                                } else {
                                    te.setDefaultSpawn(EntitySpitter.class, 1);
                                    te.spawnCallback
                                        = new SpitterDistribution(SpitterType.RED);
                                }
                            }
                        } else {
                            double f = rand.nextDouble();
                            if (f <= (reachDifficulty < 0 ? 0.15 : 0.1)) {
                                boolean stinger
                                    = rand.nextInt(reachDifficulty < 0 ? 3 : 5) == 0;
                                te.setSingleStrongEnemy(
                                    stinger ? EntityEliteStinger.class
                                            : EntitySpider.class,
                                    stinger ? 1 : 3
                                );
                            } else if (f < 0.3) {
                                ...
                                te.setDefaultSpawn(EntitySpitter.class, 1);
                                te.spawnCallback = new SpitterDistribution(
                                    f >= 0.95 ? SpitterType.GREEN : SpitterType.RED
                                );
                            }
                        }
                        break;
                        ...
                        switch (power) {
                            ...
                            case 4:
                                if (f < 0.3) {
                                    ...
                                    te.spawnCallback = new SpitterDistribution(
                                        rand.nextDouble() < 0.4 ? SpitterType.RED
                                                                : SpitterType.GREEN,
                                        1,
                                        1,
                                        SpitterType.BASIC,
                                    ...
                                    te.setDefaultSpawn(
                                        EntitySpitter.class,
                                        ((SpitterDistribution) te.spawnCallback)
                                            .totalCount()
                                    );
                                } else {
                                    //te.setDefaultSpawn(EntitySpitter.class, 2,
                                    //rand.nextBoolean() ? SpitterType.GREEN :
                                    //SpitterType.RED);
                                    te.spawnCallback = new SpitterDistribution(
                                        rand.nextDouble() < 0.6 ? SpitterType.RED
                                                                : SpitterType.GREEN,
                                        2,
                                        1,
                                        SpitterType.BASIC,
                            ...
                            case 2:
                                if (f < 0.2) {
                                    ...
                                    te.spawnCallback = new SpitterDistribution(
                                        rand.nextDouble() < 0.5 ? SpitterType.RED
                                                                : SpitterType.GREEN,
                                        1,
                                        1,
                                        SpitterType.BASIC,
                            ...
                            case 1:
                                if (f < 0.15) {
                                    ...
                                    te.spawnCallback = new SpitterDistribution(
                                        rand.nextDouble() < 0.55 ? SpitterType.RED
                                                                 : SpitterType.GREEN,
                                        1,
                                        1,
                                        SpitterType.BASIC,
                                    ...
                                    te.setDefaultSpawn(EntitySpitter.class, 1);
                                    te.spawnCallback = new SpitterDistribution(
                                        rand.nextDouble() < 0.7 ? SpitterType.GREEN
                                                                : SpitterType.RED
                                    );
                                }
                                break;
    ...
    public static boolean canReplace(World world, int x, int y, int z) {
        ...
        return b.isAir(world, x, y, z) || ReikaBlockHelper.isLiquid(b)
            || b == SFBlocks.BAMBOO.getBlockInstance() || b.getMaterial() == Material.vine
    ...
    public static boolean canExistOn(World world, int x, int y, int z) {
        ...
        if (b == Blocks.grass || b == Blocks.dirt || b == Blocks.sand
            || b == Blocks.gravel || b == Blocks.stone || b == Blocks.cobblestone)
            return true;
        if (b == SFBlocks.LOG.getBlockInstance()
            || b == SFBlocks.LEAVES.getBlockInstance())
            return true;
        if (b.isReplaceableOreGen(world, x, y, z, Blocks.stone)
            || b == SFBlocks.TERRAIN.getBlockInstance()
            || ReikaBlockHelper.isOre(b, world.getBlockMetadata(x, y, z)))
            return true;
        if (ModList.CHISEL.isLoaded()
            && ChiselBlockHandler.isWorldgenBlock(b, world.getBlockMetadata(x, y, z)))
            return true;
        return false;
    }
    ...
    static class SpitterDistribution extends SpawnCallback {
        private static final NBTIO<SpitterType> enu
            = (NBTIO<SpitterType>) ReikaNBTHelper.getEnumConverter(SpitterType.class);
        ...
        private SpitterDistribution(
            SpitterType t1, int amt1, float b1, SpitterType t2, int amt2, float b2
        ) {
            this(
                new SpitterType[] { t1, t2 },
                new int[] { amt1, amt2 },
                new float[] { b1, b2 }
            );
        }
    ...
    public static class TilePowerSlug extends TileCaveSpawner {
        ...
        private static final UUID healthBonus
            = UUID.fromString("cea3577b-784d-46e2-ae4c-3de297a10b66");
        ...
        public void updateEntity() {
            ...
            if (!worldObj.isRemote) {
                ...
                this.doFX(
                    worldObj, xCoord + 0.5, yCoord, zCoord + 0.5, tier, mounting, null
                );
            }
        }
        ...
        public static void doFX(
            ...
            if (dist <= 128 && DragonAPICore.rand.nextInt(4 + (int) (dist / 32)) == 0) {
                ...
                if (DragonAPICore.rand.nextInt(2) == 0) {
                    ...
                    EntityBlurFX fx
                        = new EntityBlurFX(
                              world, px, py + 0.5, pz, IconPrefabs.FADE_GENTLE.getIcon()
                        )
                              .setColor(c)
                              .setScale(s)
                              .setLife(l)
                ...
                if (dist <= 64) {
                    ...
                    EntitySlugStreak fx = new EntitySlugStreak(
                        world,
                        px,
                        py + 0.5,
                        pz,
        ...
        setSingleStrongEnemy(Class<? extends EntityMob> c, float boost) {
        ...
        private static void buffHealth(EntityMob e, float buff) {
            AttributeModifier m = e.getEntityAttribute(SharedMonsterAttributes.maxHealth)
                                      .getModifier(healthBonus);
            if (m == null) {
        ...
        public void readFromNBT(NBTTagCompound NBT) {
            ...
            if (NBT.hasKey("callback")) {
                try {
                    ...
                    spawnCallback
                        = (SpawnCallback) Class.forName(tag.getString("classType"))
                              .newInstance();
                    spawnCallback.readFromNBT(tag);
                } catch (Exception e) {
                    e.printStackTrace();
                    Satisforestry.logger.logError(
                        "Could not reconstruct slug spawn callback: " + e.toString()
                    );
                }
            }
        }
