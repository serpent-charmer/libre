...
public class ItemBlockPowerSlug
    extends MetadataItemBlock implements CrystalEffectBoostArmor {
    private static final UUID moveSpeedBonusID
        = UUID.fromString("31bbbe31-4838-44ca-b6a7-2777dcc87375");
    private static final UUID unknownBonusID
        = UUID.fromString("05e61bc2-1be8-415a-9045-f8b6f25bdbc4");
    private static final UUID unknownBonusID2
        = UUID.fromString("622a4a6a-31f4-448d-92bc-fc96c003828d");
    ...
    public boolean placeBlockAt(
        ...
        boolean flag = super.placeBlockAt(
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        ArrayList<ItemStack> li2
            = UpgradeHandler.instance.getSlugEquivalents(is.getItemDamage() % 3);
        if (!li2.isEmpty()) {
            li.add("Usable as upgrades:");
            for (ItemStack is2 : li2) {
                li.add(
                    "  " + is2.getDisplayName() + " ("
                    + ReikaItemHelper.getRegistrantMod(is2) + ")"
                );
            }
        }
    }
    ...
    public static void addBonuses(EntityPlayer ep, ItemStack is) {
        IAttributeInstance speed
            = ep.getEntityAttribute(SharedMonsterAttributes.movementSpeed);
    ...
    /*
        for (iterator1 = this.getModifiersByOperation(0).iterator(); iterator1.hasNext();
       d0 += attributemodifier.getAmount()) for (iterator1 =
       this.getModifiersByOperation(1).iterator(); iterator1.hasNext(); d1 += d0 *
       attributemodifier1.getAmount()) for (iterator1 =
       this.getModifiersByOperation(2).iterator(); iterator1.hasNext(); d1 *= 1.0D +
       attributemodifier1.getAmount())
     */
