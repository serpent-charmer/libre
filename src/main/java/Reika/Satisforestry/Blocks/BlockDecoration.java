...
public class BlockDecoration extends Block {
    ...
    getDrops(World world, int x, int y, int z, int meta, int fortune) {
        ...
        return override != null ? ReikaJavaLibrary.makeListFrom(override)
                                : new ArrayList();
    }
    ...
    public static enum DecorationType {
        ...
        private static final SimplexNoiseGenerator renderNoise2D
            = new SimplexNoiseGenerator(0);
        ...
        public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
            switch (this) {
                case TENDRILS:
                    if (rand.nextInt(4) == 0
                        && !ReikaWorldHelper.isAirBlock(world, x + 1, y, z)
                        && !ReikaWorldHelper.isAirBlock(world, x - 1, y, z)
                        && !ReikaWorldHelper.isAirBlock(world, x, y, z + 1)
                        && !ReikaWorldHelper.isAirBlock(world, x, y, z - 1)) {
                        ...
                        EntityBlurFX fx = new EntityBlurFX(
                            world, px, py, pz, IconPrefabs.FADE.getIcon()
                        );
        ...
        public float width(int x, int y, int z) {
            switch (this) {
                case STALAGMITE:
                case STALACTITE:
                    return (float) ReikaRandomHelper.getRandomBetween(5D, 8D, renderRand)
                        / 16F;
                default:
                    return 1;
            }
        ...
        public boolean render(
            ...
            switch (this) {
                ...
                case STALACTITE:
                    ...
                    while (w >= 1
                           && (world.getBlock(x, y0, z).isAir(world, x, y0, z)
                               || world.getBlock(x, y0, z) == b)) {
                    ...
                    for (int i = 0; i < div; i++) {
                        for (int j = 0; j < div; j++) {
                            for (int k = 0; k < div; k++) {
                                ...
                                double noise
                                    = renderNoise3D.getValue(x1 * f, y1 * f, z1 * f);
