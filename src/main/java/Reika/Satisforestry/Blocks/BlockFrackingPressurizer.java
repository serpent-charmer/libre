...
public class BlockFrackingPressurizer extends BlockSFHarvester {
    ...
    onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase e, ItemStack is) {
        if (world.getBlockMetadata(x, y, z) == 3) {
            TileFrackingExtractor te
                = (TileFrackingExtractor) world.getTileEntity(x, y, z);
            te.facing = ReikaDirectionHelper.getFromLookDirection(e, false).getOpposite();
        }
    }

    public static class TileFrackingExtractor
        extends TileEntityBase implements IFluidHandler, BreakAction {
                    ...
                    if (node != null) {
                        ...
                        if (max != null && max.amount > 0) {
                            if (te instanceof IFluidHandler) {
                                ...
                                int fit = ifl.canFill(dir, max.getFluid())
                                    ? ifl.fill(dir, max, false)
                                    : 0;
                                if (fit > 0) {
        ...
        public static FilledBlockArray getStructure(World world, int x, int y, int z) {
            ...
            for (Coordinate c : arr.keySet()) {
                arr.addBlock(
                    ...
                    c.yCoord,
                    c.zCoord,
                    arr.getBlockAt(c.xCoord, c.yCoord, c.zCoord),
                    arr.getMetaAt(c.xCoord, c.yCoord, c.zCoord) + 8
                );
            }
