...
public class BlockCrashSite
    extends BlockContainer implements IWailaDataProvider, IMoveCheck, ILaputaImmobile {
    ...
    getTileEntityForPosition(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < blocks.getSize(); i++) {
            ...
            if (c.getBlock(world) == SFBlocks.CRASHSITE.getBlockInstance()
                && c.getBlockMetadata(world) == 0)
    ...
    public static class TileCrashSite extends TileEntity
        implements InertIInv, IEnergyReceiver, IEnergySink, ShaftPowerReceiver {
        ...
        public void updateEntity() {
            ...
            if (worldObj.isRemote) {
                ...
                if (recipe.unlockPower != null) {
                    ...
                    progressFactor
                        = ticksEnoughPower / (float) recipe.unlockPower.ticksToHold;
                }
                powerAmount = 0;
        ...
        public boolean tryOpen(EntityPlayerMP ep) {
            ...
            if (this.isRequirementMet()
                && !recipe.playerHas(ep.worldObj, ep.getUniqueID())) {
        ...
        public boolean isRequirementMet() {
            return recipe != null && recipe.matchesItem(currentItem)
                && (recipe.unlockPower == null
                    || ticksEnoughPower >= recipe.unlockPower.ticksToHold);
        }
        ...
        public boolean canConnectEnergy(ForgeDirection from) {
            return recipe != null && recipe.unlockPower != null
                && recipe.unlockPower.type == PowerTypes.RF;
        }

        @Override
        public boolean acceptsEnergyFrom(TileEntity emitter, ForgeDirection direction) {
            return recipe != null && recipe.unlockPower != null
                && recipe.unlockPower.type == PowerTypes.EU;
        }
        ...
        public boolean isReceiving() {
            return recipe != null && recipe.unlockPower != null
                && recipe.unlockPower.type == PowerTypes.ROTARYCRAFT;
        }
        ...
        public static void reactToLockGuiStatus(boolean success) {
            if (success)
                ReikaSoundHelper.playClientSound(
                    SFSounds.CRASHOPEN, Minecraft.getMinecraft().thePlayer, 1, 1
                );
            else
                ReikaSoundHelper.playClientSound(
                    SFSounds.CRASHFAIL, Minecraft.getMinecraft().thePlayer, 1, 1
                );
        }
    }
}
