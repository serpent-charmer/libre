...
public class BlockTerrain extends Block {
    ...
    getDrops(World world, int x, int y, int z, int meta, int fortune) {
        ...
        return override != null ? ReikaJavaLibrary.makeListFrom(override)
                                : super.getDrops(world, x, y, z, meta, fortune);
    }
        ...
        switch(TerrainType.list[world.getBlockMetadata(x, y, z)]) {
            case CRACKS:
                ...
                for (int i = 0; i < 6; i++) {
                    ...
                    if (!world.getBlock(dx, dy, dz).isSideSolid(world, dx, dy, dz,
    dir.getOpposite())) { switch(dir) { case DOWN: y0 += d; break; case UP: y1 -= d;
                                break;
                            case WEST:
                                x0 += d;
                                break;
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        if (world.getBlockMetadata(x, y, z) == TerrainType.CRACKS.ordinal()
            ...
            for (int i = 0; i < n; i++) {
                ...
                ReikaModelledBreakFX fx = new ReikaModelledBreakFX(
                    world,
                    x + rand.nextDouble(),
                    y,
                    z + rand.nextDouble(),
    ...
    canSilkHarvest(World world, EntityPlayer player, int x, int y, int z, int metadata) {
        return TerrainType.list[metadata].canSilkTouch();
    }
    ...
    public void breakBlock(World world, int x, int y, int z, Block old, int oldmeta) {
        ...
        if (y <= 96) { //not on the surface
            for (int i = 0; i < 6; i++) {
                ...
                if ((b == SFBlocks.CAVESHIELD.getBlockInstance()
                     && world.getBlockMetadata(dx, dy, dz) == 0)
