...
public class ItemBlockNodeHarvester extends MetadataItemBlock {
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        TileEntity tile
            = field_150939_a.createTileEntity(ep.worldObj, is.getItemDamage());
        if (tile instanceof TileResourceHarvesterBase) {
            TileResourceHarvesterBase te = (TileResourceHarvesterBase) tile;
            li.add(
                "Requires " + EnumChatFormatting.WHITE + te.getPowerType()
                + EnumChatFormatting.GRAY + " power."
            );
            if (GuiScreen.isShiftKeyDown()) {
                li.add(String.format(
                    "Efficiency: %s%.1f%s",
                    EnumChatFormatting.WHITE.toString(),
                    te.getSpeedFactor() * 100,
                    "%"
                ));
                for (int i = 0; i <= 3; i++) {
                    te.overclockDisplay = i;
                    li.add(String.format(
                        "Power Cost Per Cycle (%.0f%s speed): %s%s",
                        te.getOverclockingLevel(true) * 100,
                        "%",
                        EnumChatFormatting.WHITE.toString(),
                        te.getOperationPowerCost(true)
                    ));
                }
            } else {
