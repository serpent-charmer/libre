...
public class BlockCaveSpawner extends BlockContainer implements PointSpawnBlock {
    ...
    public IIcon getIcon(IBlockAccess iba, int x, int y, int z, int s) {
        return s == 1 ? blockIcon
                      : SFBlocks.CAVESHIELD.getBlockInstance().getIcon(iba, x, y, z, s);
    }
    ...
    private static final class TileSpawnPoint extends SpawnPoint {
        ...
        getSpawn(World world, int cx, int cy, int cz, Random rand) {
            double x = ReikaRandomHelper.getRandomBetween(
                cx + 0.5 - tile.spawnRadius, cx + 0.5 + tile.spawnRadius, rand
            );
            double z = ReikaRandomHelper.getRandomBetween(
                cz + 0.5 - tile.spawnRadius, cz + 0.5 + tile.spawnRadius, rand
            );
    ...
    public static class TileCaveSpawner extends TileEntity implements PointSpawnTile {
        ...
        public final void
        setSpawnParameters(Class<? extends EntityMob> c, int n, double ar, double sr) {
            this.setSpawnParameters(c, n, ar, sr, -1);
        }

        public final void setSpawnParameters(
            Class<? extends EntityMob> c, int n, double ar, double sr, int fl
        ) {
        ...
        private final Class<? extends EntityMob> getSpawnType() {
            return spawner != null ? (Class<? extends EntityMob>) spawner.getSpawnType()
                                   : null;
        }
