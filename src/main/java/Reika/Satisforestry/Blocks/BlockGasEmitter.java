...
public class BlockGasEmitter extends BlockContainer {
    ...
    public IIcon getIcon(IBlockAccess iba, int x, int y, int z, int s) {
        if (s == 1)
            return blockIcon;
        SFBlocks sf
            = iba.getBlockMetadata(x, y, z) == 0 ? SFBlocks.TERRAIN : SFBlocks.CAVESHIELD;
        int meta = 0;
    ...
    public static class TileGasVent extends TileEntity {
        ...
        public void updateEntity() {
            activeArea = ReikaAABBHelper.getBlockAABB(this)
                             .expand(activeRadius, 0, activeRadius)
                             .addCoord(0, activeHeight, 0)
                             .offset(0, yOffset, 0);
            if (worldObj.isRemote) {
                this.doFX();
            } else {
                List<EntityPlayer> li
                    = worldObj.getEntitiesWithinAABB(EntityPlayer.class, activeArea);
                for (EntityPlayer e : li) {
                    if (!e.isPotionActive(Potion.poison)
                        && !RadiationHandler.hasHazmatSuit(e))
        ...
        private void doFX() {
            ...
            int n = 1;
            boolean cave
                = yCoord < 70 && Satisforestry.isPinkForest(worldObj, xCoord, zCoord);
            if (cave) {
                if (ep.posY > 70)
                    dd = 1000;
                else
            ...
            if (dd <= 256 && DragonAPICore.rand.nextInt(n) == 0) {
                double px = ReikaRandomHelper.getRandomBetween(
                    activeArea.minX, activeArea.maxX
                );
                double py = ReikaRandomHelper.getRandomBetween(
                    activeArea.minY, activeArea.maxY
                );
                double pz = ReikaRandomHelper.getRandomBetween(
                    activeArea.minZ, activeArea.maxZ
                );
                int c = 0xBAFF21; //0xC9FF21;
                EntityFloatingSeedsFX fx = new EntityFloatingSeedsFX(
                    worldObj, px, py, pz, 0, 90, IconPrefabs.FADE.getIcon()
                );
                ...
                if (!cave || DragonAPICore.rand.nextInt(8) == 0) {
                    EntityFloatingSeedsFX fx2 = new EntityFloatingSeedsFX(
                        worldObj, px, py, pz, 0, 90, IconPrefabs.FADE_GENTLE.getIcon()
                    );
