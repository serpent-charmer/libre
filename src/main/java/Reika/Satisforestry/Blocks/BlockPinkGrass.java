...
public class BlockPinkGrass extends BlockTallGrass {
    public static enum GrassTypes {
        ...
        private static final Simplex3DGenerator renderNoise
            = new Simplex3DGenerator(DragonAPICore.getLaunchTime());
        ...
        public IIcon getIcon(IBlockAccess iba, int x, int y, int z) {
            switch (this) {
                case STALKS:
                    return this.matchAt(iba, x, y + 1, z)
                        ? icons[0]
                        : icons[1 + renderRand.nextInt(icons.length - 1)];
                default:
            ...
            switch (this) {
                ...
                case STALKS:
                    return this.matchAt(world, x, y + 1, z)
                        ? 1
                        : (float) ReikaRandomHelper.getRandomBetween(1.25, 2, renderRand);
                case FERN:
                    return 0.5F;
                default:
                    return 1;
        ...
        public int getColor(int base) {
            switch (this) {
                ...
                case TREE_VINE:
                    return ReikaColorAPI.getColorWithBrightnessMultiplier(
                        ReikaColorAPI.getModifiedSat(base, 0.5F), 0.67F
                    );
                default:
                    return base;
            }
        ...
        public boolean canExistAt(World world, int x, int y, int z) {
            ...
            switch (this) {
                case TREE_VINE:
                    return (at == SFBlocks.LEAVES.getBlockInstance()
                            && PinkTreeTypes.getLeafType(world, dx, dy, dz)
                                == PinkTreeTypes.JUNGLE)
                        || this.matchAt(world, dx, dy, dz);
                case VINE:
                case BLUE_MUSHROOM_STALK:
                case STALKS:
                    return at.isSideSolid(world, dx, dy, dz, side)
                        || this.matchAt(world, dx, dy, dz);
                case BLUE_MUSHROOM_TOP:
            ...
            switch (this) {
                case STALKS:
                    ReikaRenderHelper.renderCropTypeTex(
                        world, x, y, z, ico, v5, rb, 0.1875, h
                    );
                    break;
                case FERN:
                    v5.setColorOpaque_I(ReikaColorAPI.mixColors(
                        b.colorMultiplier(world, x, y, z), 0xa00000, 0.75F
                    ));
                    int n = ReikaRandomHelper.getRandomBetween(3, 7, renderRand);
                    double da = 360D / n;
                    for (int i = 0; i < n; i++) {
                        double ang = Math.toRadians(ReikaRandomHelper.getRandomPlusMinus(
                            i * da, da / 2.5, renderRand
                        ));
                        double r
                            = ReikaRandomHelper.getRandomBetween(0.25, 0.5, renderRand);
                        double dr
                            = ReikaRandomHelper.getRandomBetween(0, 0.1875, renderRand);
                        ...
                        double h2
                            = ReikaRandomHelper.getRandomPlusMinus(h, 0.125, renderRand);
                        ...
                        v5.addVertexWithUV(
                            x + 0.5 + px + px2 * dr, y, z + 0.5 + pz + pz2 * dr, du, dv
                        );
                        v5.addVertexWithUV(
                            x + 0.5 - px + px2 * dr, y, z + 0.5 - pz + pz2 * dr, u, dv
                        );
                ...
                case PALEBERRY_NEW:
                    ...
                    float light
                        = rawSun * w.getSavedLightValue(EnumSkyBlock.Sky, x, y, z) / 15F;
                    if (light >= 4)
                        v5.setBrightness(240);
                    ReikaRenderHelper.renderCrossTex(
                        world, x, y, z, berryIcon, v5, rb, 1
                    );
                    break;
                case TREE_VINE:
                    ...
                    ReikaRenderHelper.renderCropTypeTex(
                        ...
                        v5,
                        rb,
                        ReikaMathLibrary.normalizeToBounds(
                            renderNoise.getValue(x, y, z), 0.03125, 0.375
                        ),
                        1
                    );
                    break;
                ...
                case BLUE_MUSHROOM_TOP:
                    ...
                    this.prepareRandom(world, x, y, z);
                    double ax
                        = ReikaRandomHelper.getRandomPlusMinus(0D, maxWiggle, renderRand);
                    double az
                        = ReikaRandomHelper.getRandomPlusMinus(0D, maxWiggle, renderRand);
                    //ReikaJavaLibrary.pConsole(x+", "+y+", "+z+" > "+ax+", "+az+" @ "+y+"
                    //0");
                    ...
                    for (int i = y - 1; i >= 0; i--) {
                        ...
                        if (below == SFBlocks.GRASS.getBlockInstance()
                            && (mb == BLUE_MUSHROOM_STALK.ordinal()
                                || mb == BLUE_MUSHROOM_TOP.ordinal())) {
                            this.prepareRandom(world, x, i, z);
                            ax = ReikaRandomHelper.getRandomPlusMinus(
                                0D, maxWiggle, renderRand
                            );
                            az = ReikaRandomHelper.getRandomPlusMinus(
                                0D, maxWiggle, renderRand
                            );
                            //ReikaJavaLibrary.pConsole(x+", "+i+", "+z+" > "+ax+", "+az+"
                            //@ "+y+" "+(y-i));
                ...
                default:
                    ReikaRenderHelper.renderCrossTex(
                        world, x, y, z, ico, v5, rb, 1
                    ); //not h, since icon is already part of a block
                    break;
            }
        }
            ...
            switch (this) {
                case PALEBERRY_EMPTY:
                    ...
                    ReikaSoundHelper.playBreakSound(
                        world, x, y, z, Blocks.leaves, 0.7F, 0.25F
                    );
                    break;
                default:
                    break;
        ...
        private boolean matchAt(IBlockAccess world, int x, int y, int z) {
            return world.getBlock(x, y, z) == SFBlocks.GRASS.getBlockInstance()
            ...
            switch (this) {
                case PALEBERRY_NEW:
                    ...
                    world.setBlockMetadataWithNotify(
                        x, y, z, PALEBERRY_EMPTY.ordinal(), 3
                    );
                    return true;
                default:
                    return false;
        ...
        getDrops(World world, int x, int y, int z, int fortune) {
            switch (this) {
                case BLUE_MUSHROOM_STALK:
                case BLUE_MUSHROOM_TOP:
                ...
                case TREE_VINE:
                    ...
                    if (world.rand.nextInt(Math.max(1, 24 - fortune)) == 0) {
                        add = world.rand.nextInt(6) == 0
                            ? ForgeHooks.getGrassSeed(world)
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < GrassTypes.list.length; i++) {
            ...
            for (int k = 0; k < gr.icons.length; k++) {
                String s
                    = "Satisforestry:foliage/" + gr.name().toLowerCase(Locale.ENGLISH);
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess world, int x, int y, int z) {
        ...
        this.setBlockBounds(
            0.5F - f, 0.0F, 0.5F - f, 0.5F + f, gr.getHeight(world, x, y, z), 0.5F + f
        );
    }
    ...
    @Override
    protected void checkAndDropBlock(
        World world, int x, int y, int z
    ) { //identical except it propagates block updates
    ...
    public boolean canReplace(World world, int x, int y, int z, int side, ItemStack is) {
        return world.getBlock(x, y, z).isReplaceable(world, x, y, z)
            && GrassTypes.list[is.getItemDamage()].canExistAt(world, x, y, z);
    }
    ...
    getDrops(World world, int x, int y, int z, int meta, int fortune) {
        ArrayList<ItemStack> ret
            = GrassTypes.list[meta].getDrops(world, x, y, z, fortune);
        if (ret == null)
