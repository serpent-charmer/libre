...
public abstract class BlockSFMultiBase<S>
    extends BlockMultiBlock<S> implements IWailaDataProvider {
    ...
    getMessages(World world, int x, int y, int z, int side) {
        TileEntity te = this.getTileEntityForPosition(world, x, y, z);
        return te instanceof TileResourceHarvesterBase
            ? ((TileResourceHarvesterBase) te).getMessages(world, x, y, z, side)
            : new ArrayList();
    }

    public final String getName(int meta) {
        return StatCollector.translateToLocal(
            "multiblock." + this.getLocaleKeyName() + "." + (meta & 7)
        );
    }
    ...
    public final boolean onBlockActivated(
        ...
        if (meta >= 8) {
            ...
            if (te instanceof TileResourceHarvesterBase) {
                ep.openGui(
                    Satisforestry.instance, 0, world, te.xCoord, te.yCoord, te.zCoord
                );
                return true;
            }
        }
    ...
    public final void setBlockBoundsBasedOnState(IBlockAccess iba, int x, int y, int z) {
        if (this.shouldTurnToSlab(iba.getBlockMetadata(x, y, z))) {
            if (iba.getBlock(x, y + 1, z).isAir(iba, x, y + 1, z)
                && iba.getBlock(x, y - 1, z) != this) {
                if (!this.match(iba, x, y, z, x, y + 1, z + 1)
                    && !this.match(iba, x, y, z, x, y + 1, z - 1)
                    && !this.match(iba, x, y, z, x + 1, y + 1, z)
                    && !this.match(iba, x, y, z, x - 1, y + 1, z)) {
    ...
    match(IBlockAccess iba, int x, int y, int z, int dx, int dy, int dz) {
        return iba.getBlock(dx, dy, dz) == this
            && iba.getBlockMetadata(dx, dy, dz) == iba.getBlockMetadata(x, y, z);
    }
    ...
    public final void breakMultiBlock(World world, int x, int y, int z) {
        ...
        blocks.recursiveMultiAddWithBounds(
            world,
            x,
            y,
            z,
            ...
            z + s,
            this,
            this.getTileBlockType().getBlockInstance()
        );
        for (int i = 0; i < 6; i++) {
    ...
    @Override
    public final boolean
    canTriggerMultiBlockCheck(World world, int x, int y, int z, int meta) {
        return true;
    }

    @Override
    protected final TileEntity
    getTileEntityForPosition(World world, int x, int y, int z) {
        StructuredBlockArray blocks = new StructuredBlockArray(world);
        blocks.extraSpread = true;
        int s = this.getBlockSearchXZ();
        int sy = this.getBlockSearchY();
    ...
    getWailaStack(IWailaDataAccessor acc, IWailaConfigHandler config) {
        return null;
    }
    ...
    public final List<String> getWailaBody(
        ...
        TileEntity te = this.getTileEntityForPosition(
            acc.getWorld(), pos.blockX, pos.blockY, pos.blockZ
        );
        if (te instanceof TileResourceHarvesterBase) {
    ...
    public static abstract class TileMinerConnection<T extends TileResourceHarvesterBase>
        extends TileEntity {
    ...
    public static class TileShaftConnection
        extends TileMinerConnection implements ShaftPowerReceiver {
        @Override
        ...
        public boolean isReceiving() {
            return this.getRoot() instanceof ShaftPowerReceiver
                && this.getRoot().hasStructure();
        }
    ...
    public static class TilePowerConnection
        extends TileMinerConnection implements IEnergyReceiver, IEnergySink {
        //RF
        @Override
        ...
        public int receiveEnergy(ForgeDirection from, int maxReceive, boolean simulate) {
            return this.getRoot() instanceof IEnergyReceiver
                ? ((IEnergyReceiver) this.getRoot())
                      .receiveEnergy(from, maxReceive, simulate)
                : 0;
        }
        ...
        public int getEnergyStored(ForgeDirection from) {
            return this.getRoot() instanceof IEnergyReceiver
                ? ((IEnergyReceiver) this.getRoot()).getEnergyStored(from)
                : 0;
        }
        ...
        public int getMaxEnergyStored(ForgeDirection from) {
            return this.getRoot() instanceof IEnergyReceiver
                ? ((IEnergyReceiver) this.getRoot()).getMaxEnergyStored(from)
                : 0;
        }
        ...
        public double getDemandedEnergy() {
            return this.getRoot() instanceof IEnergySink
                ? ((IEnergySink) this.getRoot()).getDemandedEnergy()
                : 0;
        }
        ...
        public int getSinkTier() {
            return this.getRoot() instanceof IEnergySink
                ? ((IEnergySink) this.getRoot()).getSinkTier()
                : 0;
        }
