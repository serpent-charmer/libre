...
public class ResourceNodeHandler extends TemplateRecipeHandler {
    private final Comparator<CachedRecipe> displaySorter
        = new Comparator<CachedRecipe>() {
              @Override
    ...
    public class ResourceEntry extends CachedRecipe {
        ...
        private final Comparator<PositionedStack> sorter
            = new Comparator<PositionedStack>() {
                  @Override
    ...
    public void drawBackground(int recipe) {
        ...
        //ResourceLocation("NotEnoughItems", "textures/gui/recipebg"));
        GL11.glDisable(GL11.GL_DEPTH_TEST);
        //ReikaGuiAPI.instance.drawTexturedModalRectWithDepth(0, 1, 5, 11, 166, 70,
    ...
    public void loadTransferRects() {
        transferRects.add(
            new RecipeTransferRect(new Rectangle(0, 3, 165, 52), "ResourceItems")
        );
    }
    ...
    public void drawExtras(int recipe) {
        ...
        if (r instanceof ResourceEntry) {
                ...
                for (int i = -2; i <= 2; i++) {
                    for (int k = -2; k <= 2; k++) {
                        if (Math.abs(i) != 2 || Math.abs(k) != 2)
                            arr.setBlock(
                                i, 0, k, SFBlocks.CAVESHIELD.getBlockInstance(), 0
                            );
                    }
                }
            ...
            api.drawCenteredStringNoShadow(
                fr, re.purity.getDisplayName() + " Nodes", 82, 15, 0x000000
            );
            ...
            Collections.sort(c, new Comparator<ResourceItemView>() {
                ...
                public int compare(ResourceItemView o1, ResourceItemView o2) {
                    return ReikaItemHelper.comparator.compare(
                        wc.getItem(o1), wc.getItem(o2)
                    );
                }
            });
            for (ResourceItemView ri : c) {
                ...
                String counts = ri.minAmount == ri.maxAmount
                    ? ri.minAmount + "x"
                    : ri.minAmount + "x - " + ri.maxAmount + "x";
                if (GuiScreen.isShiftKeyDown()) {
                    fr.drawString("Stack Size: " + counts, 26, 48 + dy, 0x000000);
                    if (ri.manualWeightScale != 1) {
                        fr.drawString(
                            "Manual Weight Factor: " + ri.manualWeightScale,
                            26,
                            58 + dy,
                            0x000000
                        );
                    ...
                    if (ri.manualYieldScale != 1) {
                        fr.drawString(
                            "Manual Yield Factor: " + ri.manualYieldScale,
                            26,
                            58 + dy,
                            0x000000
                        );
            ...
            if (!GuiScreen.isShiftKeyDown())
                api.drawCenteredStringNoShadow(
                    fr, "Hold LSHIFT for detailed info", 82, dy + 28, 0x000000
                );
        }
    }
}
