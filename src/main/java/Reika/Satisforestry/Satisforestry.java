...
@Mod(
    ...
    dependencies
    = "required-after:DragonAPI;after:CritterPet;after:RotaryCraft;after:IC2;after:ThermalExpansion;before:climatecontrol"
)
...
public class Satisforestry extends DragonAPIMod {
    ...
    public static final ControlledConfig config
        = new ControlledConfig(instance, SFOptions.optionList, null);
    ...
    @SidedProxy(
        clientSide = "Reika.Satisforestry.SFClient",
        serverSide = "Reika.Satisforestry.SFCommon"
    )
    ...
    public void preload(FMLPreInitializationEvent evt) {
        ...
        ReikaPacketHelper.registerPacketHandler(
            instance, packetChannel, new SFPacketHandler()
        );

        ReikaRegistryHelper.instantiateAndRegisterBlocks(
            instance, SFBlocks.blockList, blocks
        );
    ...
    public void load(FMLInitializationEvent event) {
        ...
        if (f != null) {
            turbofuel = new Fluid("turbofuel")
                            .setDensity((int) Math.min(f.getDensity() * 1.1, 800))
                            .setViscosity(f.getViscosity());
        ...
        FMLInterModComms.sendMessage(
            ...
            "bsb",
            "EntityDryad|" + SFOptions.BIOMEID.getValue()
        );
    ...
    private void addRecipes() {
        FurnaceFuelRegistry.instance.registerItem(
            new ItemStack(compactedCoal),
            TileEntityFurnace.getItemBurnTime(new ItemStack(Items.coal)) * 4
        );
        ...
        for (PinkTreeTypes type : PinkTreeTypes.list) {
            ...
            ReikaRecipeHelper.addSmelting(
                log,
                ReikaItemHelper.getSizedItemStack(ReikaItemHelper.charcoal, yield),
                xp
            );
            ...
            FurnaceFuelRegistry.instance.registerItem(
                ...
                yield
                    * TileEntityFurnace.getItemBurnTime(
                        ReikaTreeHelper.OAK.getItem().asItemStack()
                    )
            );
            FurnaceFuelRegistry.instance.registerItem(
                type.getSapling(),
                yield
                    * TileEntityFurnace.getItemBurnTime(
                        ReikaTreeHelper.OAK.getSapling().asItemStack()
                    )
            );
            FurnaceFuelRegistry.instance.registerItem(
                type.getBaseLeaf(),
                TileEntityFurnace.getItemBurnTime(
                    ReikaTreeHelper.OAK.getBasicLeaf().asItemStack()
                )
            );
        }
        ...
        GameRegistry.addShapelessRecipe(
            new ItemStack(multiblockPage, 1, 0),
            Items.paper,
            Items.paper,
            Items.paper,
        ...
        GameRegistry.addShapelessRecipe(
            new ItemStack(multiblockPage, 1, 1),
            Items.paper,
            Items.paper,
            Items.paper,
        ...
        GameRegistry.addShapelessRecipe(
            new ItemStack(multiblockPage, 1, 2),
            Items.paper,
            Items.paper,
            Items.paper,
    ...
    public void postload(FMLPostInitializationEvent evt) {
        ...
        if (turbofuel != null) { //generates same power as fuel, but lasts 2.5x as long
            ...
            if (ModList.BCENERGY.isLoaded()) {
                FuelManager.INSTANCE.addFuel(
                    turbofuel,
                    ReikaBuildCraftHelper.getFuelRFPerTick(),
                    ReikaBuildCraftHelper.getFuelBucketDuration() * 5 / 2
                );
            }
            if (ModList.IC2.isLoaded()
                && !Recipes.semiFluidGenerator.getBurnProperties().isEmpty()) {
                BurnProperty prop
                    = Recipes.semiFluidGenerator.getBurnProperties().get(ref.getName());
                if (prop != null) {
                    Recipes.semiFluidGenerator.addFluid(
                        turbofuel.getName(), prop.amount * 2 / 5, prop.power
                    );
                }
            }
            if (ModList.IMMERSIVEENG.isLoaded()) {
                DieselHandler.registerFuel(
                    turbofuel, DieselHandler.getBurnTime(ref) * 5 / 2
                );
            }
            if (ModList.RAILCRAFT.isLoaded()) {
                mods.railcraft.api.fuel.FuelManager.addBoilerFuel(
                    turbofuel,
                    mods.railcraft.api.fuel.FuelManager.getBoilerFuelValue(ref) * 5 / 2
                );
            }
        }
        ...
        if (ModList.BOTANIA.isLoaded()) {
            ...
            RecipeManaInfusion rec = BotaniaAPI.registerManaInfusionRecipe(
                unpacked, PinkTreeTypes.JUNGLE.getBaseLog(), 10
            );
            rec.setAlchemy(true);
        }

        if (ModList.FORESTRY.isLoaded()) {
            ISqueezerRecipe rec = ForestryRecipeHelper.getInstance().getSqueezerOutput(
            ...
            if (rec != null) {
                ...
                RecipeManagers.squeezerManager.addRecipe(
                    rec.getProcessingTime(),
                    new ItemStack[] { new ItemStack(paleberry) },
                    fs.copy(),
                    rec.getRemnants(),
                    (int) (chance * 1.5)
                );
                ...
                RecipeManagers.squeezerManager.addRecipe(
                    rec.getProcessingTime(),
                    new ItemStack[] { new ItemStack(paleberry, 1, 1) },
                    fs.copy(),
                    rec.getRemnants(),
                    (int) (chance * 0.25)
                );
            }
            IFermenterRecipe rec2 = ForestryRecipeHelper.getInstance().getFermenterOutput(
                new ItemStack(Blocks.sapling)
            );
            if (rec2 != null) {
                for (PinkTreeTypes type : PinkTreeTypes.list) {
                    float f = Math.max(type.getBerryModifier() * 1.2F, 0.2F);
                    ForestryRecipeHelper.getInstance().addStandardFermenterRecipes(
                        type.getSapling(), (int) (rec2.getFermentationValue() * f)
                    );
                }
            }
            ...
            try {
                ...
                e.printStackTrace();
                logger.logError(
                    "Could not add custom bee species. Check your versions; if you are up-to-date with both mods, notify Reika."
                );
            } catch (Exception e) {
                e.printStackTrace();
                logger.logError(
    ...
    public static boolean isPinkForest(World world, int x, int z) {
        return isPinkForest(
            world.isRemote ? world.getBiomeGenForCoords(x, z)
                           : world.getWorldChunkManager().getBiomeGenAt(x, z)
        );
    }
