...
public class UpgradeHandler {
    ...
    private UpgradeHandler() {
        if (ModList.IC2.isLoaded()) {
            ic2Upgradeable
                = this.loadClass(ModList.IC2, "ic2.core.upgrade.IUpgradableBlock");
        }
        if (ModList.ENDERIO.isLoaded()) {
            eioUpgradeable = this.loadClass(
                ModList.ENDERIO, "crazypants.enderio.machine.AbstractPoweredMachineEntity"
            );
        }
        if (ModList.THERMALEXPANSION.isLoaded()) {
            teUpgradeable = this.loadClass(
                ModList.THERMALEXPANSION, "cofh.lib.gui.container.IAugmentableContainer"
            );
            Class c2 = this.loadClass(
                ModList.THERMALEXPANSION,
                "cofh.thermalexpansion.gui.container.ContainerTEBase"
            );
            if (c2 == null) //older version
                c2 = this.loadClass(
                    ModList.THERMALEXPANSION,
                    "thermalexpansion.gui.container.ContainerTEBase"
                );
            if (c2 != null) {
                try {
                    ...
                    Satisforestry.logger.logError(
                        "Could not find TE container tile reference!"
                    );
    ...
    private Class loadClass(ModList mod, String s) {
        try {
            ...
            Satisforestry.logger.logError(
                "Could not find class for upgrade handler: " + s
            );
    ...
    public void addSlugs(InitialClickEvent evt) {
        //ReikaJavaLibrary.pConsole(evt.modifiers+" @ "+evt.slotID+" with
        //"+evt.player.inventory.getItemStack());
        if (((evt.modifiers == 1
              && this.getSlugNBT(evt.player.inventory.getItemStack()) != null)
             || evt.modifiers == 6)
            && (this.isSlug(evt.player.inventory.getItemStack())
                || this.isSlug(evt.container.getSlot(evt.slotID).getStack()))) {
    ...
    private ItemStack getSlugNBT(ItemStack is) {
        if (is != null && is.stackTagCompound != null
            && is.stackTagCompound.hasKey(NBT_KEY)) {
            return ItemStack.loadItemStackFromNBT(
                is.stackTagCompound.getCompoundTag(NBT_KEY)
            );
        }
        return null;
    }
    ...
    private boolean addToSlot(IInventory ii, int slot, Container c, EntityPlayer ep) {
        ...
        if (SFBlocks.SLUG.matchWith(is) && is.stackSize == 1 && slot >= 0
            && slot < c.inventorySlots.size() && !c.getSlot(slot).getHasStack()) {
    ...
    private ItemStack getSlugUpgrade(Object ii, int slot, Container c, int tier) {
        if (ii != null && ic2Upgradeable != null
            && ic2Upgradeable.isAssignableFrom(ii.getClass())) {
            if (slot >= 36) //ic2 containers put player inv first
                slot -= 36;
            if (this.containerAccepts(
                    c, slot, IC2Handler.IC2Stacks.OVERCLOCK.getItem()
                )) {
                return ReikaItemHelper.getSizedItemStack(
                    IC2Handler.IC2Stacks.OVERCLOCK.getItem(), UPGRADE_COUNTS[tier]
                );
            }
        }
        if (ii instanceof IAugmentable
            || (teUpgradeable != null && teUpgradeable.isAssignableFrom(c.getClass()))) {
            if (ii == null)
                ii = this.getTileForTEContainer(c);
            int base = ii instanceof IEnergyProvider ? 80 : 128;
            base += tier;
            ItemStack item
                = ReikaItemHelper.lookupItem("ThermalExpansion:augment:" + base);
            if (item != null && this.containerAccepts(c, slot, item)) {
                return item;
            }
        }
        if (ii != null && eioUpgradeable != null
            && eioUpgradeable.isAssignableFrom(ii.getClass())) {
            ItemStack item
                = ReikaItemHelper.lookupItem("EnderIO:itemBasicCapacitor:" + tier);
            if (item != null && this.containerAccepts(c, slot, item)) {
                return item;
            }
        }
    ...
    public ArrayList<ItemStack> getSlugEquivalents(int tier) {
        ...
        if (ModList.IC2.isLoaded())
            li.add(ReikaItemHelper.getSizedItemStack(
                IC2Handler.IC2Stacks.OVERCLOCK.getItem(), UPGRADE_COUNTS[tier]
            ));
        if (ModList.THERMALEXPANSION.isLoaded()) {
    ...
    private boolean containerAccepts(Container c, int slot, ItemStack item) {
        return slot >= 0 && slot < c.inventorySlots.size()
            && c.getSlot(slot).isItemValid(item);
    }
