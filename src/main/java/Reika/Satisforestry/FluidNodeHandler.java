...
public class FluidNodeHandler extends TemplateRecipeHandler {
    ...
    public void loadTransferRects() {
        transferRects.add(
            new RecipeTransferRect(new Rectangle(0, 3, 165, 52), "ResourceFluids")
        );
    }
    ...
    public void drawExtras(int recipe) {
        ...
        if (r instanceof ResourceEntry) {
            ...
            if (renderer == null) {
                ...
                for (int i = -9; i <= 9; i++) {
                    for (int k = -9; k <= 9; k++) {
                        if (Math.abs(i) + Math.abs(k) < 12 && Math.abs(i) < 9
                            && Math.abs(k) < 9)
                            arr.setBlock(i, 0, k, SFBlocks.CAVESHIELD.getBlockInstance());
                    }
                }
                arr.setTile(
                    0, 0, 0, SFBlocks.FRACKNODE.getBlockInstance(), 0, tileDelegate
                );
                ...
                for (int i = -6; i <= 6; i += 4) {
                    ...
                    if (DragonAPICore.rand.nextInt(4) > 0) {
                        ...
                        arr.setTile(
                            i, 0, k, SFBlocks.FRACKNODEAUX.getBlockInstance(), 0, te2
                        );
                    }
                    if (DragonAPICore.rand.nextInt(4) > 0) {
                        TileFrackingAux te2 = new TileFrackingAux();
            ...
            String counts = ri.minAmount == ri.maxAmount
                ? ri.minAmount + "mB"
                : ri.minAmount + "mB - " + ri.maxAmount + "mB";
            ...
            if (GuiScreen.isShiftKeyDown()) {
                if (wc.requiredInput != null) {
                    n = wc.requiredInput.getLocalizedName(
                        new FluidStack(wc.requiredInput, wc.requiredInputAmount)
                    );
                    fr.drawString("Required input: ", 5, 28 + dy, 0x000000);
                    fr.drawString(
                        n + " (" + wc.requiredInputAmount + " mB)",
                        5,
                        28 + dy + fr.FONT_HEIGHT,
                        0x000000
                    );
