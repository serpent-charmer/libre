...
public class GuiSFBlueprint extends GuiScreen implements CustomSoundGui {
    ...
    private final class ModeButton extends CustomSoundImagedGuiButton {
        ...
        protected void renderButton() {
            ...
            int c = mode == id
                ? 0x666666
                : ReikaColorAPI.mixColors(0x404040, 0x262626, this.getHoverFade());
            ...
            this.drawTexturedModalRect(
                xPosition + width / 2 - 16,
                yPosition + height / 2 - 16,
                iconU,
                220,
                32,
                32
    ...
    public GuiSFBlueprint(EntityPlayer ep, int struct) {
        ...
        switch (struct) {
            case 0:
                array = MinerStructure.getMinerStructure(
                    ep.worldObj, 0, 0, 0, ForgeDirection.EAST
                );
        ...
        render.addBlockHook(
            SFBlocks.HARVESTER.getBlockInstance(),
            new PowerTypeChoice(SFBlocks.HARVESTER.getBlockInstance())
        );
        render.addBlockHook(
            SFBlocks.FRACKER.getBlockInstance(),
            new PowerTypeChoice(SFBlocks.FRACKER.getBlockInstance())
        );
    ...
    public void initGui() {
        ...
        buttonList.add(new CustomSoundImagedGuiButton(
            5, j + 219, k + 0, 29, 15, 219, 0, TEXTURE, Satisforestry.class, this
        ) {
            @Override
            protected void renderButton() {
                super.renderButton();
                ...
                if (a > 0) {
                    ...
                    BlendMode.DEFAULT.apply();
                    this.drawTexturedModalRect(
                        xPosition, yPosition, 209, 240, width, height, 0xffffff, a
                    );
                    GL11.glPopAttrib();
                }
            }
        });
        //buttonList.add(new CustomSoundImagedGuiButton(4, mode == 1 ? j+125 : j+165, k+8,
        //20, 20, "N#"));
                    ...
                    protected void renderButton() {
                        ...
                        int c = sel ? 0xCBCBCB
                                    : ReikaColorAPI.mixColors(
                                        0x909090, 0x747474, this.getHoverFade()
                                    );
                        Tessellator v5 = Tessellator.instance;
                        v5.startDrawingQuads();
                        v5.setColorRGBA_I(c, 255);
                        ...
                        c = sel ? 0xffffff
                                : ReikaColorAPI.GStoHex(
                                    0x57 * 3 / 4
                                ); //why is there a *1.333 in the effective color in
                                   //GuiAPI in this window?!
                        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                            renderer,
                            String.format("%d", layer + 1),
                            this.getLabelX(),
                            this.getLabelY(),
                            c
                        );
    ...
    public final void drawScreen(int mx, int my, float f) {
        ...
        fontRendererObj.drawString(
            title + " (" + array.getSizeX() + "x" + array.getSizeY() + "x"
                + array.getSizeZ() + ")",
            j + 6,
            k + 4,
            0xffffff
    ...
    private void drawSlice(int j, int k) {
        ...
        render.drawSlice(
            miner ? j : (fracker ? j - 3 : j - 12),
            k + (miner ? 21 : (fracker ? 25 : 13)),
            fontRendererObj,
            miner ? 1 : (fracker ? 0.5 : 2)
        );
    }

    private void drawTally(int j, int k) {
        ItemHashMap<Integer> map = array.tally(
            mode == 1 ? c -> c.yCoord - array.getMinY() == render.getCurrentSlice() : null
        );
        int i = 0;
        int n = 8;
        ...
        for (ItemStack is : c) {
            ...
            ReikaGuiAPI.instance.drawItemStack(
                itemRender, fontRendererObj, is2, x0, y0, true
            );
            i++;
        }
        GL11.glPopMatrix();
    ...
    private void draw3d(int j, int k, float ptick) {
        ...
        if (renderTESR) {
            ...
            OpenGlHelper.setLightmapTextureCoords(
                OpenGlHelper.lightmapTexUnit, i1 / 1.0F, k1 / 1.0F
            );
            GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        }
