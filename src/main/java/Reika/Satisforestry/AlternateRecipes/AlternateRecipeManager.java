...
public class AlternateRecipeManager implements AltRecipeHandler {
    ...
    public void
    setRecipeStatus(EntityPlayerMP ep, AlternateRecipe s, boolean set, boolean notify) {
        if (set && notify) {
            ReikaChatHelper.sendChatToAllOnServer(
                EnumChatFormatting.GOLD + ep.getCommandSenderName()
                + EnumChatFormatting.RESET + " just unlocked "
                + EnumChatFormatting.LIGHT_PURPLE + s.getDisplayName()
            );
            TickScheduler.instance.scheduleEvent(
                new ScheduledTickEvent(new ScheduledStringPacket(
                    Satisforestry.packetChannel,
                    SFPackets.ALTRECIPEUNLOCK.ordinal(),
        ...
        while (it.hasNext()) {
            ...
            if (p == null) {
                Satisforestry.logger.logError(
                    "Could not load alternate recipe from NBT String " + val
                    + "; was it removed?"
                );
                it.remove();
            } else {
    ...
    private NBTTagList getNBTList(EntityPlayer ep) {
        ...
        if (nbt == null) {
            Satisforestry.logger.logError(
                "Looking for recipe data on player " + ep.getCommandSenderName()
                + ", with no NBT?!"
            );
    ...
    public NBTTagCompound getRootNBTTag(EntityPlayer ep) {
        ...
        if (tag == null || tag.hasNoTags()) {
            NBTTagCompound repl
                = AltRecipeBackupLoadHandler.instance.attemptToLoadBackup(ep);
            if (repl != null) {
                if (tag == null)
    ...
    public AltRecipe addAltRecipe(
        ...
        if (recipe == null)
            throw new IllegalArgumentException(
                "Could not accept alt recipe '" + id + "': Recipe may not be null"
            );
        AlternateRecipe added = BiomeConfig.instance.addAlternateRecipe(
            id,
            displayName,
            spawnWeight,
            recipe,
