...
public class AlternateRecipeNEIHandler extends TemplateRecipeHandler {
    public class AltRecipe extends CachedRecipe {
        ...
        public PositionedStack getResult() {
            ...
            return new PositionedStack(
                rec == null ? recipe.getRecipeOutput() : rec.getRecipeOutput(), 119, 24
            );
        }
    ...
    public void drawBackground(int recipe) {
        ...
        Minecraft.getMinecraft().renderEngine.bindTexture(
            new ResourceLocation(this.getGuiTexture())
        );
    ...
    public void loadTransferRects() {
        transferRects.add(new RecipeTransferRect(
            new Rectangle(84, 23, 24, 18), "altrecipes", new Object[0]
        ));
    }
        ...
        if (a instanceof AltRecipe) {
            ...
            if (sp != null) {
                ...
                ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                    fr, r.getDisplayName(), 166, 123, 0xFA9549
                );
                ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                    fr, sp.getDescription(), 166, 123 + fr.FONT_HEIGHT + 4, 0xFA9549
                );
                GL11.glScaled(2, 2, 2);
            } else {
                ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                    fr, r.getDisplayName(), 83, 63, 0xFA9549
                );
            }
            ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                fr, "Requires unlock in a crash site", 83, 80, 0x646464
            );
            ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                fr, "provided with the following:", 83, 92, 0x646464
            );
            if (r.unlockPower != null)
                ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                    fr, "Power: " + r.unlockPower.getDisplayString(), 83, 104, 0x646464
                );
            ItemStack is = r.getRequiredItem();
            if (is != null)
                ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                    fr,
                    "Items: " + is.stackSize + " " + is.getDisplayName(),
                    83,
                    116,
                    0x646464
                );
