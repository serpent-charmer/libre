...
public class BiomeConfig {
    ...
    private BiomeConfig() {
        {
            ...
            example.setComment(
                "block", "single block type, mutually exclusive with 'blocks'"
            );
            example.setComment(
                "blocks",
                "optional, multiple block shorthand; mutually exclusive with 'block'"
            );
            spawns.setComment(
                null,
                "where this type can spawn, valid locations: "
                    + ReikaJavaLibrary.getEnumNameList(OreSpawnLocation.class)
            );
...

        {
            ...
            ResourceItemLuaBlock example2
                = new ResourceItemLuaBlock("example", null, itemData);
            ...
            ResourceItemLuaBlock levels
                = new ResourceItemLuaBlock("purityLevels", example2, itemData);
            for (Purity p : Purity.list) {
                levels.putData(p.name(), p == Purity.NORMAL ? 25 : 10);
            }
            ResourceItemLuaBlock items
                = new ResourceItemLuaBlock("outputItems", example2, itemData);
            ...
            ResourceItemLuaBlock scales
                = new ResourceItemLuaBlock("weightModifiers", item, itemData);
            for (Purity p : Purity.list) {
            ...
            ResourceItemLuaBlock effects
                = new ResourceItemLuaBlock("effects", example2, itemData);
            ...
            item.setComment(
                "effectType",
                "type of effect, valid values: "
                    + ReikaJavaLibrary.getEnumNameList(EffectTypes.class)
            );
...

        {
            ...
            ResourceFluidLuaBlock example2b
                = new ResourceFluidLuaBlock("example", null, fluidData);
            ...
            ResourceFluidLuaBlock levels2
                = new ResourceFluidLuaBlock("purityLevels", example2b, fluidData);
            for (Purity p : Purity.list) {
                levels2.putData(p.name(), p == Purity.NORMAL ? 25 : 10);
            }
            ResourceFluidLuaBlock fluid
                = new ResourceFluidLuaBlock("outputFluids", example2b, fluidData);
            fluid.putData("key", "lava");
            fluid.putData("minAmount", 100);
            fluid.putData("maxAmount", 500);
            ResourceFluidLuaBlock scales2
                = new ResourceFluidLuaBlock("amountModifiers", fluid, fluidData);
            for (Purity p : Purity.list) {
                scales2.putData(p.name(), (p.ordinal() + 1) * 0.5F);
            }
            ResourceFluidLuaBlock effects2
                = new ResourceFluidLuaBlock("effects", example2b, fluidData);
...

        {
            ...
            AltRecipeLuaBlock example3
                = new AltRecipeLuaBlock("example", null, recipeData);
            example3.putData("type", "example_alt_recipe");
            example3.putData("spawnWeight", 40);
            //example3.putData("generate", "true");
            AltRecipeLuaBlock recipe
                = new AltRecipeLuaBlock("recipe", example3, recipeData);
            ...
            AltRecipeLuaBlock pwr
                = new AltRecipeLuaBlock("requiredPower", example3, recipeData);
            pwr.putData("format", "RF");
            pwr.putData("amount", 6000);
            pwr.putData("time", 30);
            pwr.putData("timeUnit", "second");

            AltRecipeLuaBlock req
                = new AltRecipeLuaBlock("requiredItem", example3, recipeData);
            ...
            try {
                ...
                if (e instanceof UserErrorException)
                    throw new InstallationException(
                        ...
                        "Configs could not be loaded! Delete them and try again.",
                        e
                    );
                else
    ...
    private void createDefaultFiles(File folder) throws IOException {
        ...
        ReikaFileReader.writeLinesToFile(
            f1, oreData.getBlock("example").writeToStrings(), true, Charsets.UTF_8
        );

        File f2 = new File(folder, "resources.lua");
        f2.createNewFile();
        ReikaFileReader.writeLinesToFile(
            f2, itemData.getBlock("example").writeToStrings(), true, Charsets.UTF_8
        );

        File f4 = new File(folder, "fluids.lua");
        f4.createNewFile();
        ReikaFileReader.writeLinesToFile(
            f4, fluidData.getBlock("example").writeToStrings(), true, Charsets.UTF_8
        );

        File f3 = new File(folder, "doggo.lua");
        f3.createNewFile();
        ReikaFileReader.writeLinesToFile(
            f3, doggoData.getBlock("example").writeToStrings(), true, Charsets.UTF_8
        );

        File f5 = new File(folder, "recipes.lua");
        f5.createNewFile();
        ReikaFileReader.writeLinesToFile(
            f5, recipeData.getBlock("example").writeToStrings(), true, Charsets.UTF_8
        );
    }
    ...
    private void loadFiles(File parent) throws IOException {
        ...
        if (f2 == null || !f2.exists()) {
            Satisforestry.logger.logError(
                "No resource config file found! Generating default!"
            );
        ...
        if (f4 == null || !f4.exists()) {
            Satisforestry.logger.logError(
                "No fluid config file found! Generating default!"
            );
        ...
        if (f1 != null && f1.exists())
            ...
            Satisforestry.logger.log(
                "No ore config file found; no ore clusters will generate."
            );
        ...
        if (f3 != null && f3.exists())
            ...
            Satisforestry.logger.log(
                "No doggo config file found; the only items doggos will find will be the ones hardcoded to Satisforestry."
            );
        ...
        if (f5 != null && f5.exists())
            ...
            Satisforestry.logger.log(
                "No custom recipe config file found; your only alternate recipes will be the ones hardcoded to Satisforestry."
            );
    }
    ...
    private void parseConfigs() {
        ...
        for (LuaBlock b : root.getChildren()) {
            try {
                ...
                Satisforestry.logger.logError(
                    "Could not parse config section " + b.getString("type") + ": "
                );
        ...
        Satisforestry.logger.log(
            "All ore config entries parsed; files contained " + definitionCount
            + " definitions, for a total of " + entryAttemptsCount + " entries, of which "
            + entryCount + " loaded."
        );
        ...
        Satisforestry.logger.log(
            "All resource config entries parsed; files contained " + definitionCount
            + " definitions, for a total of " + entryAttemptsCount + " entries, of which "
            + entryCount + " loaded."
        );
        if (resourceEntries.isEmpty()) {
            throw new InstallationException(
                Satisforestry.instance,
                "No resource entries were loaded; at least one must be defined!"
            );
        }
        ...
        Satisforestry.logger.log(
            "All fluid config entries parsed; files contained " + definitionCount
            + " definitions, for a total of " + entryAttemptsCount + " entries, of which "
            + entryCount + " loaded."
        );
        if (fluidEntries.isEmpty()) {
            throw new InstallationException(
                Satisforestry.instance,
                "No fluid entries were loaded; at least one must be defined!"
            );
        }
        ...
        Satisforestry.logger.log(
            "All doggo-item config entries parsed; files contained " + definitionCount
            + " definitions, for a total of " + entryAttemptsCount + " entries, of which "
            + entryCount + " loaded."
        );
        ...
        Satisforestry.logger.log(
            "All alternate recipe config entries parsed; files contained "
            + definitionCount + " definitions, for a total of " + entryAttemptsCount
            + " entries, of which " + entryCount + " loaded."
        );
    ...
    private void addHardcodedEntries() {
        for (int i = 0; i < 3; i++)
            doggoEntries.add(new DoggoDrop(
                SFBlocks.SLUG.getStackOfMetadata(i),
                1,
                1,
                9 / ReikaMathLibrary.intpow2(3, i)
            ));
        ...
        for (Entry<OreType, Integer> e : doggoOres.entrySet()) {
            ...
            if (ore.existsInGame()) {
                ...
                doggoEntries.add(
                    new DoggoDrop(ore.getFirstOreBlock(), 1, amtMax, e.getValue())
                );
            }
        }
        ...
        if (ReikaItemHelper.oreItemExists(sulf)) {
            Object coal
                = ReikaItemHelper.oreItemExists("dustCoal") ? "dustCoal" : Items.coal;
            ItemStack in = CustomRecipeList.parseItemString(
                SFOptions.COMPACTCOALITEM.getString(), null, true
            );
            ...
            if (parts.length != 3 || Strings.isNullOrEmpty(parts[0])) {
                this.addAlternateRecipe(
                    COMPACTED_COAL_ID,
                    null,
                    50,
                    new ShapelessOreRecipe(out, coal, coal, sulf, sulf),
                    in,
                    null,
                    0,
                    0
                ...
                try {
                    ...
                    throw new InstallationException(
                        Satisforestry.instance,
                        "Invalid compacted coal alternate recipe parameters specified",
                        e
                    );
                }
            }
        ...
        if (Satisforestry.turbofuel != null) { //the recipe will not be craftable without
                                               //another mod to add it from their side
            String s = SFOptions.TURBOFUELITEM.getString();
            ItemStack in
                = Strings.isNullOrEmpty(s) ? null : ReikaItemHelper.lookupItem(s);
    ...
    public AlternateRecipe addAlternateRecipe(
        String id,
        String disp,
        int wt,
        IRecipe rec,
        ...
        if (recipeEntries.containsKey(id))
            throw new IllegalArgumentException(
                "Recipe ID '" + id + "' is already in use: " + recipeEntries.get(id)
            );
        AlternateRecipe alt
            = new AlternateRecipe(id, wt, rec, need, powerType, powerAmount, ticksFor);
        alt.displayName = disp;
        if (Strings.isNullOrEmpty(disp) && rec.getRecipeOutput() == null)
            throw new IllegalArgumentException(
                "You cannot add a no-item-output recipe without setting a display name!"
            );
    ...
    private void parseOreEntry(String type, LuaBlock b)
        ...
        for (String s : blocks) {
            ...
            try {
                ...
                Satisforestry.logger.logError(
                    "Threw exception parsing block ID '" + s + "':"
                );
                e.printStackTrace();
            }
            if (bk == null) {
                Satisforestry.logger.logError(
                    "Could not load block type '" + s + "' for ore type '" + type
                    + "'; skipping."
                );
                continue;
            } else {
        ...
        for (Entry<OreSpawnLocation, LuaBlock> e : sections.entrySet()) {
            ...
            OreClusterType ore
                = new OreClusterType(id, cs, data.getInt("spawnWeight"), types);
            ...
            Satisforestry.logger.log(
                "Registered ore type '" + type + "' with " + types.size() + " blocks '"
                + types + "' for area " + cs + ", wt=" + ore.spawnWeight
                + " s=" + ore.sizeScale + " d=" + ore.maxDepth
            );
            entryCount++;
        }
    }

    private void parseResourceEntry(String type, LuaBlock b)
        throws NumberFormatException, IllegalArgumentException, IllegalStateException {
        ...
        ResourceItem ore = new ResourceItem(
            type,
            b.getString("displayName"),
            b.getInt("spawnWeight"),
            b.getInt("renderColor"),
            map
        );
        ...
        for (LuaBlock s : items) {
            ...
            if (is == null) {
                Satisforestry.logger.logError(
                    "Could not load item type '" + sk + "' for resource type '" + type
                    + "' - no item found. Skipping."
                );
                continue;
            }
            ...
            float man = s.containsKey("manualModifier")
                ? (float) s.getDouble("manualModifier")
        ...
        if (ore.hasNoItems())
            throw new IllegalArgumentException(
                "Resource type found no items for any of its definitions"
            );
        ...
        Satisforestry.logger.log(
            "Registered resource type '" + type
            + "': " + ore.toString().replaceAll("\\\\n", " ")
        );
        entryCount++;
    }

    private void parseFluidEntry(String type, LuaBlock b)
        throws NumberFormatException, IllegalArgumentException, IllegalStateException {
        ...
        FluidStack fs = in == null
            ? null
            : new FluidStack(
                FluidRegistry.getFluid(in.getString("key")), in.getInt("amount")
            );
        ResourceFluid ore = new ResourceFluid(
            type,
            b.getInt("spawnWeight"),
            b.getInt("renderColor"),
            nodes,
            b.getBoolean("glowAtNight"),
            fs,
            b.getInt("rounding"),
            map
        );
        ...
        if (is == null)
            throw new IllegalArgumentException(
                "Could not load fluid type '" + sk + "' for resource type '" + type
                + "' - no fluid found."
            );
        ...
        if (ore.hasNoItems())
            throw new IllegalArgumentException(
                "Resource type found no fluids for any of its definitions"
            );
    ...
    private void parseDoggoEntry(String type, LuaBlock b)
        throws NumberFormatException, IllegalArgumentException, IllegalStateException {
        ArrayList<LuaBlock> items = new ArrayList();
        ...
        for (LuaBlock s : items) {
            entryAttemptsCount++;
            ItemStack is = CustomRecipeList.parseItemString(
                s.getString("key"), s.getChild("nbt"), true
            );
            if (is == null) {
                Satisforestry.logger.logError(
                    "Could not load item type '" + s + "' for doggo drop '" + type
                    + "'; skipping."
                );
                continue;
            }
            KeyedItemStack ks = new KeyedItemStack(is)
                                    .setIgnoreMetadata(false)
                                    .setIgnoreNBT(false)
                                    .setSized(false)
                                    .setSimpleHash(true);
            if (doggoItems.contains(ks)) {
                Satisforestry.logger.logError(
                    "Doggo drop for item '" + CustomRecipeList.fullID(is)
                    + "' already loaded. Skipping duplicate in '" + type + "'."
                );
                continue;
            }
            doggoItems.add(ks);
            ...
            Satisforestry.logger.log(
                "Registered doggo drop type '" + type
                + "': " + drop.toString().replaceAll("\\\\n", " ")
            );
            entryCount++;
        }
    }

    private void parseAltRecipeEntry(String type, LuaBlock b)
        throws NumberFormatException, IllegalArgumentException, IllegalStateException {
        entryAttemptsCount++;
        if (recipeEntries.containsKey(type))
            throw new IllegalArgumentException(
                "Recipe ID '" + type + "' is already in use: " + recipeEntries.get(type)
            );
        AlternateRecipe rec = new AlternateRecipe(
            type,
            b.getInt("spawnWeight"),
            b.getChild("recipe"),
            b.getChild("requiredItem"),
            b.getChild("requiredPower")
        );
        rec.displayName
            = b.containsKey("displayName") ? b.getString("displayName") : null;
        recipeEntries.put(rec.id, rec);
        Satisforestry.logger.log(
            "Registered alternate recipe '" + type
            + "': " + rec.toString().replaceAll("\\\\n", " ")
        );
        entryCount++;
    }
    ...
    private Collection<BlockKey> parseBlocks(LuaBlock b) {
        ...
        if (block != null) {
            LuaBlock metas = b.getChild("metadata"
            ); //do not use inherit, use direct call so will return null if unspecified
