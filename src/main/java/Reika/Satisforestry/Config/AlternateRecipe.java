...
public class AlternateRecipe implements AltRecipe {
    ...
    public AlternateRecipe(
        ...
        if (Strings.isNullOrEmpty(powerType)) {
            ...
            if (type != PowerTypes.RF && type != PowerTypes.EU
                && type != PowerTypes.ROTARYCRAFT)
    ...
    public AlternateRecipe(
        String id, double wt, LuaBlock recipe, LuaBlock needItem, LuaBlock power
    ) {
        this.id = id;
        spawnWeight = wt;
        ...
        this.recipe = CustomRecipeList.parseCraftingRecipe(
            recipe,
            CustomRecipeList.parseItemString(
                output.getString("item"), output.getChild("nbt"), false
            )
        );

        if (power != null) {
            PowerTypes type
                = PowerTypes.valueOf(power.getString("format").toUpperCase(Locale.ENGLISH)
                );
            if (type != PowerTypes.RF && type != PowerTypes.EU
                && type != PowerTypes.ROTARYCRAFT)
                throw new IllegalArgumentException("Unsupported power type " + type);
            ...
            if (power.containsKey("timeUnit")) {
                ...
                unit = val.equals("SECONDS") ? ReikaTimeHelper.SECOND
                                             : ReikaTimeHelper.valueOf(val);
            }
        ...
        unlockItem = needItem == null
            ? null
            : CustomRecipeList.parseItemString(
                needItem.getString("item"), needItem.getChild("nbt"), true
            );
    ...
    public boolean giveToPlayer(EntityPlayerMP ep) {
        if (ep == null || ReikaPlayerAPI.isFake(ep)) {
            Satisforestry.logger.logError(
                "Tried to give alt recipe '" + this + "' to null or fake player???"
            );
            return false;
        }
    ...
    public boolean playerHas(World world, UUID id) {
        ...
        if (ep == null && world instanceof WorldServer) {
            ep = ReikaPlayerAPI.getFakePlayerByNameAndUUID(
                (WorldServer) world, "AltRecipe Backup", id
            );
        }
        return ep != null
            && AlternateRecipeManager.instance.getPlayerRecipeData(ep).contains(this);
    }
    ...
    public boolean usesItem(ItemStack ingredient) {
        return this.isUncraftableWithNEI()
            ? ((UncraftableAltRecipeWithNEI) recipe).usesItem(ingredient)
            : ReikaRecipeHelper.recipeContains(recipe, ingredient);
    }

    public boolean crafts(ItemStack is) {
        return this.isUncraftableWithNEI()
            ? ((UncraftableAltRecipeWithNEI) recipe).crafts(is)
            : ReikaItemHelper.matchStacks(this.getRecipeOutput(), is);
    }
    ...
    public CachedRecipe createNEIDelegate() {
        if (recipe instanceof ShapedRecipes) {
            ...
            return srh.new CachedShapelessRecipe(
                sr.recipeItems, sr.getRecipeOutput()
            ); //ew
        } else if (recipe instanceof ShapedOreRecipe) {
            return new ShapedRecipeHandler().forgeShapedRecipe((ShapedOreRecipe) recipe);
        } else if (recipe instanceof ShapelessOreRecipe) {
            return new ShapelessRecipeHandler().forgeShapelessRecipe((ShapelessOreRecipe
            ) recipe);
        } else {
            return null;
        }
    ...
    public static class PowerRequirement {
        ...
        public String getDisplayString() {
            switch (type) {
                case RF:
                case EU:
                    return amount + " " + type.getDisplayName() + "/t for "
                        + ticksToHold / 20 + "s";
                case ROTARYCRAFT:
