...
public abstract class NodeResource<I> {
    ...
    public final void
    addItem(Purity p, I is, int weight, int min, int max, LuaBlock data) {
        if (items.size() >= this.maxItemsPerType())
            throw new IllegalStateException(
                "Max item type limit exceeded when trying to add " + is
            );
    ...
    public final String toString() {
        return "W=" + spawnWeight + ", C=" + Integer.toHexString(color)
            + ", L=" + levels.toString() + ", I=" + items.toString();
    }
    ...
    public static enum Purity {
        ...
        private static final int MINING_COOLDOWN
            = 10; //default resource node is 120/min = 2/s = 10t
    ...
    public class NodeItem implements DynamicWeight {
        ...
        public final int[] getAmountRange(
            Purity p, int tier, boolean manual, boolean peaceful
        ) {
            float sc0 = 1;
            if (peaceful)
            ...
            if (sc != null)
                sc0 *= sc.floatValue();
            return new int[] { (int) Math.max(1, this.minAmount * sc0),
                               (int) Math.max(1, this.maxAmount * sc0) };
        }

        public final int
        getAmount(Purity p, int tier, boolean manual, boolean peaceful, Random rand) {
            double amt = ReikaRandomHelper.getRandomBetween(
                minAmount, (double) this.maxAmount, rand
            );
            if (peaceful)
    ...
    public static enum EffectTypes {
        ...
        private static final HashMap<String, NodeEffectCallback> customEffects
            = new HashMap();
                ...
                case REFLECTIVE:
                    ...
                    if (cn == null)
                        throw new IllegalArgumentException(
                            "Invalid reflective definition - no class specified"
                        );
                    Class c;
                    try {
                        c = Class.forName(cn);
                        ...
                        throw new IllegalArgumentException(
                            "Invalid reflective definition - no such class"
                        );
                    }
                    map.put("class", c);
                    ...
                    if (argC == null)
                        throw new IllegalArgumentException(
                            "Invalid reflective definition - no arguments specified"
                        );
                    ...
                    if (mn == null)
                        throw new IllegalArgumentException(
                            "Invalid reflective definition - no method name specified"
                        );
                    Method m;
                    try {
                        ...
                        throw new IllegalArgumentException(
                            "Invalid reflective effect definition - no such method", ex
                        );
                    }
                ...
                case POTION:
                    ...
                    if (!ep.isPotionActive(p))
                        ep.addPotionEffect(new PotionEffect(
                            p.id, (int) data.get("duration"), (int) data.get("level") - 1
                        ));
                    break;
                case REFLECTIVE:
                    try {
                        ...
                        m.invoke(
                            inst != null ? inst.getValue(te, ep) : null,
                            this.unwrapArgs(te, ep, args)
                        );
                    } catch (Exception e) {
                        Satisforestry.logger.logError(
                            "Error applying reflectively-defined resource effect!"
                        );
                        e.printStackTrace();
                    }
                    break;
        ...
        private Object[] unwrapArgs(
            TileEntity te, EntityPlayer ep, MethodArgument[] args
        ) {
        ...
        public static void addCustomCallback(String name, NodeEffectCallback call) {
            ...
            EffectTypes c = EnumHelper.addEnum(EffectTypes.class, name.toUpperCase(),
            types, args); customEffects.put(c.name(), call); return c;
             */
    ...
    private static class MethodArgument {
        ...
        private static MethodArgument parse(String s) {
            try {
                ...
                MethodArgumentType type
                    = MethodArgumentType.valueOf(s.toUpperCase(Locale.ENGLISH));
                return new MethodArgument(type, type.parseLiteralValue(val));
            } catch (IllegalArgumentException e) {
                throw new IllegalArgumentException(
                    "Invalid argument type. Valid types: "
                    + ReikaJavaLibrary.getEnumNameList(MethodArgumentType.class)
                );
            }
        }
    }
