...
public class FrackingNodeAuxRenderer extends FrackingNodeRenderer {
    ...
    public boolean renderWorldBlock(
        ...
        LightningBolt b = new LightningBolt(
            new DecimalPosition(root), new DecimalPosition(x + 0.5, y + 0.5, z + 0.5), n
        );
        b.setRandom(rand).setVariance(1).maximize();

        List<DecimalPosition> li = b.spline(
            SplineType.CENTRIPETAL,
            (int) (ReikaMathLibrary.py3d(root.xCoord - x, 0, root.zCoord - z) * 1)
        );
        for (DecimalPosition d : li) {
            ...
            int idx = renderPass == 0
                ? 0
                : (int
                ) ((Double.doubleToRawLongBits(dx) ^ Double.doubleToRawLongBits(dz)) % 5);
            IIcon ico = renderPass == 0
                ? block.blockIcon
                : BlockFrackingNode.getOverlay(((idx % 5 + 5) % 5) + 5);
            this.renderWedgePie(
                dx,
                y + renderPass * 0.005,
                dz,
