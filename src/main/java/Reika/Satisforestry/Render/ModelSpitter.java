...
public class ModelSpitter extends ModelBase {
    ...
    public final void setRotationAngles(
        ...
        for (ModelPartEntry head : headParts) {
            //head.model.rotateAngleX = (float)Math.toRadians(head.originalX) + f4 / (180F
            /// (float)Math.PI); head.model.rotateAngleY =
            //(float)Math.toRadians(head.originalY) + f3 / (180F / (float)Math.PI);
        }
        headRotation = f3;
