...
public class SFFrackerRenderer extends TileEntityRenderBase {
            ...
            if (this.doRenderModel(te)) {
                ...
                GL11.glPopMatrix(); /*
                 if (MinecraftForgeClient.getRenderPass() == 1 || te.forceRenderer) {
                     ...
                     ReikaAABBHelper.renderAABB(box, par2, par4, par6, te.xCoord,
                 te.yCoord, te.zCoord, 255, 64, 64, 32, true); GL11.glDepthMask(false);
                     GL11.glTranslated(par2, par4, par6);
                     double s = 0.2;
                     ...
                     for (int i = 0; i < 4; i++) {
                         ...
                         ReikaGuiAPI.instance.drawCenteredStringNoShadow(this.getFontRenderer(),
                 "Incomplete", 0, 0, 0xffffff); GL11.glPopMatrix();
                     }

                     GL11.glPopMatrix();
                 }*/
            } else {
                SFMinerRenderer.renderOverlaidCube(
                    te,
                    SFBlocks.FRACKERMULTI.getBlockInstance().getIcon(
                        1, FrackerBlocks.TUBE.ordinal()
                    )
                );
                GL11.glPopMatrix();
            }
    ...
    protected boolean doRenderModel(TileEntityBase te) {
        ((TileResourceHarvesterBase) te).forceRenderer
            = StructureRenderer.isRenderingTiles() && GuiSFBlueprint.renderTESR;
