...
public class ModelFlyingManta extends ModelBase {
    ...
    render(Entity entity, float f, float f1, float f2, float f3, float f4, float f5) {
        ...
        for (ModelPart p : wingParts) {
            ...
            double ft = o / 70D;
            p.offsetY = (float
            ) (((EntityFlyingManta) entity).getWingDeflection() * ft * Math.sin(t));
            p.render(f5);
            ...
            if (!isHidden) {
                if (showModel) {
                    ...
                    if (rotateAngleX == 0.0F && rotateAngleY == 0.0F
                        ...
                        GL11.glPushMatrix();
                        GL11.glTranslatef(
                            rotationPointX * f5, rotationPointY * f5, rotationPointZ * f5
                        );

                        if (rotateAngleZ != 0.0F)
                            GL11.glRotatef(
                                rotateAngleZ * (180F / (float) Math.PI), 0.0F, 0.0F, 1.0F
                            );
                        if (rotateAngleY != 0.0F)
                            GL11.glRotatef(
                                rotateAngleY * (180F / (float) Math.PI), 0.0F, 1.0F, 0.0F
                            );
                        if (rotateAngleX != 0.0F)
                            GL11.glRotatef(
                                rotateAngleX * (180F / (float) Math.PI), 1.0F, 0.0F, 0.0F
                            );
