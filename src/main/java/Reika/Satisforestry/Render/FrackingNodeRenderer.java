...
public class FrackingNodeRenderer extends ISBRH {
    ...
    public boolean renderWorldBlock(
        ...
        int val = rand.nextInt(5); //do not inline because it needs to be called either
                                   //renderPass for the rands to align
        ...
        this.renderWedgePie(
            x, y + renderPass * 0.005, z, n, r1, r2, minh, maxh, oo, f, v5, ico
        );

        if (renderPass == 0) {
            v5.setColorOpaque_I(0x777777);
            this.renderWedgePie(
                x, y, z, n, 0, r2 - 0.0625, 0, maxh - 0.125, 0, 1, v5, block.blockIcon
            );
            /*
            double da = 30;
    ...
    protected void renderWedgePie(
        ...
        for (int i = 0; i < n; i++) {
            ...
            double x1
                = r1 * Math.cos(a1) + ReikaRandomHelper.getRandomPlusMinus(0, oo, rand);
            double x2
                = r1 * Math.cos(a2) + ReikaRandomHelper.getRandomPlusMinus(0, oo, rand);
            double x3
                = r2 * Math.cos(a2) + ReikaRandomHelper.getRandomPlusMinus(0, oo, rand);
            double x4
                = r2 * Math.cos(a1) + ReikaRandomHelper.getRandomPlusMinus(0, oo, rand);
            double z1
                = r1 * Math.sin(a1) + ReikaRandomHelper.getRandomPlusMinus(0, oo, rand);
            double z2
                = r1 * Math.sin(a2) + ReikaRandomHelper.getRandomPlusMinus(0, oo, rand);
            double z3
                = r2 * Math.sin(a2) + ReikaRandomHelper.getRandomPlusMinus(0, oo, rand);
            double z4
                = r2 * Math.sin(a1) + ReikaRandomHelper.getRandomPlusMinus(0, oo, rand);
    ...
    private void addVertexAt(
        ...
        double fx = MathHelper.clamp_double(
            (x2 - x0) / wr, 0, 1
        ); //ReikaMathLibrary.getDecimalPart(x2);
        double fz = MathHelper.clamp_double(
            (z2 - z0) / wr, 0, 1
        ); //ReikaMathLibrary.getDecimalPart(z2);
        double dr = ReikaMathLibrary.py3d(dx, 0, dz);
        double u = ico.getInterpolatedU(fx * 16);
        double v = ico.getInterpolatedV(fz * 16);
        double dy = maxh <= 0.03
            ? y + 1
            : y + 1 + ReikaMathLibrary.linterpolate(dr, 0, maxr, maxh, minh);
