...
public class RenderCullingSystem implements ChunkWorldRenderWatcher {
    ...
    public boolean interceptChunkRender(WorldRenderer wr, int renderPass, int GLListID) {
        ...
        if (Satisforestry.isPinkForest(ep.worldObj, x, z)) {
            if (ep.posY < 74) { //lowest ground level in the biome
                ...
                if (wr.posY <= 56
                    && Satisforestry.isPinkForest(ep.worldObj, wr.posX + 8, wr.posZ + 8))
                    return true;
    ...
    private boolean shouldCullCanopy(WorldRenderer wr, EntityPlayer ep) {
        ...
        if (ep.getDistanceSq(wr.posX + 8, ep.posY, wr.posZ + 8) < 1024)
            return false;
        if (ep.getDistanceSq(wr.posX + 8, wr.posY * 0.25 + 0.75 * ep.posY, wr.posZ + 8)
            < 1024 * Minecraft.getMinecraft().gameSettings.renderDistanceChunks)
            return false;
        //wr.posY >= 172 && wr.posY >= ep.posY+40 && ep.getDistanceSq(wr.posX+8, ep.posY,
        //wr.posZ+8) >= 1024
        /*
        for (int dx : offsets) {
            for (int dy : offsets) {
                for (int dz : offsets) {
                    ray.setOrigins(wr.posX+dx, wr.posY+dy, wr.posZ+dz, ep.posX, ep.posY,
        ep.posZ); if (ray.isClearLineOfSight(ep.worldObj)) return false;
                }
            }
        }*/
        return true;
