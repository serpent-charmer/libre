...
public class RedBambooRenderer extends ISBRH {
    ...
    public boolean renderWorldBlock(
        ...
        int colorBottom = below
            ? ReikaColorAPI.mixColors(0xE99396, 0xB06A6A, randY.nextFloat())
            : ReikaColorAPI.mixColors(0xD6C8C7, 0xAF9199, randY.nextFloat());
        ...
        for (int i = 0; i < dr; i++) {
            ...
            colorBottom
                = ReikaColorAPI.getColorWithBrightnessMultiplier(colorBottom, 0.9F);
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int i0 = 0; i0 < nl; i0++) {
                double ang = ReikaRandomHelper.getRandomPlusMinus(
                    i0 * 360D / nl, 90D / nl, randY
                ); //Math.toRadians(randY.nextDouble()*360);
                ...
                for (int i2 = 0; i2 < n2; i2++) {
                    int leafColor
                        = ReikaColorAPI.mixColors(0xFF9D9B, 0xAB3B47, randY.nextFloat());
                    dr = ReikaRandomHelper.getRandomBetween(0, 3, randY);
                    for (int di = 0; di < dr; di++) {
                        leafColor = ReikaColorAPI.getColorWithBrightnessMultiplier(
                            leafColor, 0.8F
                        );
            ...
            if (renderPass == 0) {
                double hs = above
                    ? 1
                    : ReikaRandomHelper.getRandomBetween(maxLeaf + 0.125, 1, randY);
                ...
                if (!above) {
                    ...
                    rb.renderFaceYPos(
                        b, x + dx, y, z + dz, rb.getBlockIcon(b, world, x, y, z, 1)
                    );
                }
                if (!below) {
                    v5.setColorOpaque_I(colorBottom);
                    rb.renderFaceYNeg(
                        b, x + dx, y, z + dz, rb.getBlockIcon(b, world, x, y, z, 0)
                    );
                }

                IIcon bico
                    = rb.getBlockIcon(b, world, x, y, z, ForgeDirection.WEST.ordinal());
                ...
                //rb.renderFaceXNeg(b, x+dx, y, z+dz, rb.getBlockIcon(b, world, x, y, z,
