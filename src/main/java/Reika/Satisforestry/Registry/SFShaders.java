...
public enum SFShaders implements ShaderHook {
    ...
    public void create() {
        try {
            ...
            if (this == POISONGAS && SFOptions.ALTSHADER.getState())
                id = id + "_alt";
            shader = ShaderRegistry.createShader(
                Satisforestry.instance, id, Satisforestry.class, path, domain
            );
            shader.setHook(this);
        } catch (Exception e) {
            throw new RegistrationException(
                Satisforestry.instance, "Could not create shader " + this + "!", e
            );
        }
    }
    ...
    public static void registerAll() {
        if (registered)
            return;
        ShaderLibrary.registerComputedLibrary(
            Satisforestry.instance, BlurWithRedBleed.ID, BlurWithRedBleed.class
        );
