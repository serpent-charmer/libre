...
public enum SFBlocks implements BlockEnum {
    ...
    public String getMultiValuedName(int meta) {
        switch (this) {
            ...
            case SAPLING:
            case LOG:
                String tree = StatCollector.translateToLocal(
                    "pinkforest.tree."
                    + PinkTreeTypes.getLeafType(meta).name().toLowerCase(Locale.ENGLISH)
                );
                String block = StatCollector.translateToLocal(
                    "pinkforest.tree." + this.name().toLowerCase(Locale.ENGLISH)
                );
                return tree + " " + block;
            case HARVESTER:
            case FRACKER:
                ...
                return StatCollector.translateToLocal("sfmachine.type." + meta) + " "
                    + this.getBasicName();
            case MINERMULTI:
                return StatCollector.translateToLocal(
                    "multiblock.sfminer."
                    + MinerBlocks.list[meta & 7].name().toLowerCase(Locale.ENGLISH)
                );
            case FRACKERMULTI:
                return StatCollector.translateToLocal(
                    "multiblock.sffracker."
                    + FrackerBlocks.list[meta & 7].name().toLowerCase(Locale.ENGLISH)
                );
            case SLUG:
                String type = String.valueOf(meta % 3);
                if ((meta & 3) == 0 && SFOptions.BLUEGREENSLUGS.getState())
                    type = type + "b";
                return StatCollector.translateToLocal("powerslug." + type) + " "
                    + this.getBasicName();
            default:
                return "";
        }
    ...
    public boolean matchWith(ItemStack is) {
        return is != null
            && ReikaItemHelper.matchStackWithBlock(is, this.getBlockInstance());
    }
