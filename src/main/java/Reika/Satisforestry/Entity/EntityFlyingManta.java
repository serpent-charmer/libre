...
public class EntityFlyingManta extends EntityFlying implements ChunkLoadingEntity {
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote) {
            ...
            if (pathSpline == null) {
                MantaPath path = BiomewideFeatureGenerator.instance.getPathAround(
                    worldObj, pathRoot
                );
                if (pathRoot != null && path == null)
                    Satisforestry.logger.logError(
                        "Could not reload manta path for " + this
                    );
                this.setPath(path);
                return;
            }
    ...
    private void doFX() {
        ...
        if (ticksExisted % 15 == 0)
            ReikaSoundHelper.playClientSound(
                SFSounds.MANTAFLY, posX, posY, posZ, 2, 1.25F, true
            );
        /*
        Vec3 left = this.getLookVec();
        Vec3 look = ReikaVectorHelper.rotateVector(left, -90, 0, 0);
        double dl = 3.5;
        EntityBlurFX fx = new EntityBlurFX(worldObj, posX+look.xCoord*dl, posY-2,
        posZ+look.zCoord*dl, IconPrefabs.FADE.getIcon()); int c1 = 0x52B6FF; int c2 =
        0xA8DAFF; int c = ReikaColorAPI.mixColors(c1, c2, rand.nextFloat()); int l =
        ReikaRandomHelper.getRandomBetween(20, 90); float s =
        (float)ReikaRandomHelper.getRandomBetween(2.5, 5);
    ...
    public Collection<ChunkCoordIntPair> getChunksToLoad() {
        return ChunkManager.instance.getChunkSquare(
            MathHelper.floor_double(posX), MathHelper.floor_double(posZ), 2
        );
    }
