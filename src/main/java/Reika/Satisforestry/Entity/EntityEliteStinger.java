...
public class EntityEliteStinger extends EntitySpider implements SpawnPointEntity {
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote) {
            if (entityToAttack != null && poisonGasTick == 0 && poisonGasCooldown == 0
                && rand.nextInt(150) == 0) {
    ...
    private void jumpAt(Entity e) {
        ...
        //ReikaSoundHelper.playSoundAtEntity(worldObj, this, "mob.cat.hiss",
        //0.8F, 1.9F+rand.nextFloat()*0.1F);
    ...
    protected void fall(float amt) {
        ...
        if (isLeaping) {
            AxisAlignedBB box
                = ReikaAABBHelper.getEntityCenteredAABB(this, 1).expand(3, 0, 3);
            List<EntityLivingBase> li = worldObj.getEntitiesWithinAABBExcludingEntity(
                this, box, new ClassEntitySelector(EntityLivingBase.class, false)
            );
    ...
    private void doParticleTrail() {
        Vec3 vec = Vec3.createVectorHelper(
            dataWatcher.getWatchableObjectFloat(18),
            0,
        ...
        for (int i = 0; i < 6; i++) {
            ...
            double y = posY + this.getEyeHeight() - 0.125
                + ReikaRandomHelper.getRandomPlusMinus(0, 0.03125);
            EntityBlurFX fx
                = new EntityBlurFX(worldObj, x, y, z, IconPrefabs.FADE.getIcon());
    ...
    private void startPoisonCloud(double lenFactor) {
        ...
        //ReikaSoundHelper.playSoundAtEntity(worldObj, this, "mob.chicken.plop", 2F,
        //0.8F); ReikaSoundHelper.playSoundAtEntity(worldObj, this, "mob.cat.hiss", 2,
        //0.5F);
        SFSounds.STINGERGAS.playSound(this);
        EntityStingerPoison e
            = new EntityStingerPoison(this, (int) (POISON_DURATION * lenFactor));
    ...
    protected void applyEntityAttributes() {
        ...
        this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .setBaseValue(30); //15 hearts
        this.getEntityAttribute(SharedMonsterAttributes.movementSpeed).setBaseValue(1.2);
    }

    @Override
    protected Entity findPlayerToAttack() {
        //return worldObj.getClosestVulnerablePlayerToEntity(this, 24); //no light limit,
        //24 range instead of 16
    ...
    public boolean attackEntityAsMob(Entity e) {
        ...
        if (flag && e instanceof EntityLivingBase) {
            ((EntityLivingBase) e)
                .addPotionEffect(new PotionEffect(Potion.poison.id, 20, 1));
            if (e instanceof EntityPlayerMP) {
                ReikaPacketHelper.sendDataPacket(
                    Satisforestry.packetChannel,
                    SFPackets.MOBDAMAGE.ordinal(),
                    (EntityPlayerMP) e,
                    this.getEntityId()
                );
            }
