...
public class EntitySplittingSpitterFireball extends EntitySpitterFireball {
    ...
    public EntitySplittingSpitterFireball(
        World world,
        EntitySpitter e,
    ...
    public EntityFX spawnLifeParticle(double x, double y, double z) {
        ...
        SpitterFireParticle fx
            = new SpitterFireParticle(worldObj, x, y, z, this.getSpitterType());
    ...
    public void onUpdate() {
        super.onUpdate();
        //ReikaJavaLibrary.pConsole(target != null ? this.getDistanceSqToEntity(target) :
        //-1, Side.SERVER);
        if (!worldObj.isRemote
            && (ticksExisted >= 40
                || (target != null && this.getDistanceSqToEntity(target) <= 36))) {
            this.split();
        }
        if (ticksExisted >= 200)
            this.setDead();
    ...
    private void split() {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            double dy = EntityAISpitterFireball.getYTarget(
                target, (EntitySpitter) shootingEntity
            );
