...
public class EntityLizardDoggo
    extends EntityTameable implements SpawnPointEntity, LizardDoggo {
    ...
    public EntityLizardDoggo(World w) {
        ...
        EntityAIFollowOwner seek = new EntityAIDoggoFollow(
            this, 0.4, 4, 2.5F
        ); //args: speed, dist to start follow, dist to consider "reached them"
        tasks.addTask(6, seek);
        tasks.addTask(5, new EntityAIDoggoFollow(this, 0.6, 12, 2.5F));
        EntityAIWatchClosest look
            = new EntityAIWatchClosest(this, EntityPlayer.class, 30);
        look.setMutexBits(4);
    ...
    public boolean getLeashed() {
        return super.getLeashed() || this.hasItem() || this.justDepositedItem(false)
            || this.isSitting(); //Stay!
    }
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote && this.isTamed()) {
            if (foundItem == null && ticksExisted >= 900) {
                ...
                if (last > 20 * 15
                    && (rand.nextInt(10000) == 0 || last >= 20 * 60 * 15)) {
                    this.generateItem();
                }
            } else if (tick % 100 == 0)
                ReikaPacketHelper.sendEntitySyncPacket(
                    DragonAPIInit.packetChannel, this, 128
                );
        ...
        if (worldObj.isRemote) {
            ...
            if (needsItemUpdate) {
                renderItem
                    = foundItem != null ? new InertItem(worldObj, foundItem) : null;
    ...
    protected void jump() {
        ...
        if (this.isPotionActive(Potion.jump)) {
            motionY
                += (this.getActivePotionEffect(Potion.jump).getAmplifier() + 1) * 0.1F;
        }
    ...
    public float getSneezeTick2() {
        return Math.min(
            1, dataWatcher.getWatchableObjectInt(19) * 1.5F / SNEEZE_LENGTH_2
        );
    }
    ...
    private ItemStack getRandomDrop() {
        ...
        return dropTable.isEmpty() ? null
                                   : dropTable.getRandomEntry().drop.generateItem(rand);
    }
    ...
    public final boolean interact(EntityPlayer ep) {
        ...
        if (!this.isTamed()) {
            ...
            if (is != null && is.getItem() == Satisforestry.paleberry
                && is.stackSize > 0) {
                this.setTamed(true);
        ...
        if (!worldObj.isRemote && !ep.isSneaking() && this.isTamed()) {
            ...
            if (is != null && is.getItem() == Satisforestry.paleberry
                ...
                if (healTick == 0) {
                    ...
                    for (int i = 0; i < 7; i++) {
                        int[] x = ReikaJavaLibrary.splitDoubleToInts(
                            ReikaRandomHelper.getRandomPlusMinus(posX, width)
                        );
                        int[] y = ReikaJavaLibrary.splitDoubleToInts(
                            ReikaRandomHelper.getRandomBetween(posY, posY + height)
                        );
                        int[] z = ReikaJavaLibrary.splitDoubleToInts(
                            ReikaRandomHelper.getRandomPlusMinus(posZ, width)
                        );
                        ArrayList<Integer> li = ReikaJavaLibrary.makeIntListFromArray(
                            ReikaParticleHelper.HEART.ordinal(),
                            1,
                            x[0],
                            x[1],
                            y[0],
                        ...
                        ReikaPacketHelper.sendDataPacket(
                            ...
                            PacketIDs.PARTICLEWITHPOS.ordinal(),
                            new PacketTarget.RadiusTarget(this, 64),
                            li
                        );
                    }
                    healTick = 50;
                    ...
                    for (int i = 0; i < line.size(); i++) {
                        ...
                        TickScheduler.instance.scheduleEvent(
                            new ScheduledTickEvent(
                                new ScheduledSoundEvent(SFSounds.DOGGOSING, this, 1, f)
                            ),
                            t
                        );
    ...
    private SFSounds getRandomLivingSound(boolean allowHurt) {
        if (allowHurt && this.getHealth() < this.getMaxHealth() * 0.33
            && rand.nextBoolean())
