...
public class EntitySpitterFireball
    extends EntitySmallFireball implements IEntityAdditionalSpawnData, DestroyOnUnload {
    private SpitterType type;
    private float damageAmount;

    public EntitySpitterFireball(
        World world,
        EntitySpitter e,
        double vx,
        double vy,
        ...
        this.setLocationAndAngles(
            e.posX + vec.xCoord * d,
            e.posY + e.getEyeHeight() + vec.yCoord * d,
            e.posZ + vec.zCoord * d,
            0,
            0
        );
    ...
    protected final void onImpact(MovingObjectPosition mov) {
        if (!worldObj.isRemote) {
            if (shootingEntity != null && mov.entityHit instanceof EntityLivingBase) {
                ...
                if (sc > 0)
                    SpitterDamage.doDamage(
                        (EntitySpitter) shootingEntity,
                        this,
                        (EntityLivingBase) mov.entityHit,
                        damageAmount * sc
                    );
            }
            SFSounds.SPITTERBALLHIT.playSound(this, 1, 1.2F + rand.nextFloat() * 0.5F);
            int id
                = mov.entityHit != null ? mov.entityHit.getEntityId() : Integer.MIN_VALUE;
            ReikaPacketHelper.sendDataPacketWithRadius(
                Satisforestry.packetChannel,
                SFPackets.SPITTERFIREHIT.ordinal(),
                this,
                32,
                this.getEntityId(),
                mov.typeOfHit == MovingObjectType.BLOCK ? 1 : 0,
                mov.blockX,
                mov.blockY,
                mov.blockZ,
                mov.sideHit,
    ...
    public EntityFX spawnLifeParticle(double x, double y, double z) {
        float s = (float) ReikaRandomHelper.getRandomBetween(
            type.isAlpha() ? 1.5 : 1, type.isAlpha() ? 2 : 1.5
        );
    ...
    public final void doHitFX(MovingObjectPosition mov) {
        for (int i = 0; i < 6; i++) {
            ...
            if (mov.typeOfHit == MovingObjectType.BLOCK && mov.sideHit >= 0
                ...
                v = ReikaPhysicsHelper.polarToCartesian(
                    v0,
                    10 + rand.nextDouble() * 40,
                    ReikaRandomHelper.getRandomPlusMinus(ang, 15)
                );
                //v = new double[3];
                ...
                double da = mov.typeOfHit == MovingObjectType.ENTITY
                    ? 90 - rand.nextDouble() * 60
                    : 90 - rand.nextDouble() * 30;
                v = ReikaPhysicsHelper.polarToCartesian(v0, da, rand.nextDouble() * 360);
            }
            double px = mov.typeOfHit == MovingObjectType.ENTITY
                ? mov.entityHit.posX
                : mov.blockX; //posX-motionX
            double py = mov.typeOfHit == MovingObjectType.ENTITY
                ? mov.entityHit.posY
                : Math.max(posY, mov.blockY); //posY-motionY
            double pz = mov.typeOfHit == MovingObjectType.ENTITY
                ? mov.entityHit.posZ
                : mov.blockZ; //posZ-motionZ
            if (mov.typeOfHit == MovingObjectType.BLOCK && mov.sideHit >= 0) {
            ...
            SpitterFireParticle fx = new SpitterFireParticle(
                worldObj,
                px - motionX * 0.25,
                py - motionY * 0.25,
                pz - motionZ * 0.25,
            ...
            fx.setGravity(0.25F)
                .setScale(s)
                .setLife(l + 10)
                .setColliding(Double.NaN, Integer.MIN_VALUE)
                .setRapidExpand();
    ...
    public void readEntityFromNBT(NBTTagCompound tag) {
        ...
        //	shootingEntity =
        //(EntityLivingBase)worldObj.getEntityByID(tag.getInteger("entity"));
