...
public class EntitySpitter extends EntityMob implements Spitter {
    ...
    private EntityAISpitterFireball basicFireball
        = new EntityAISpitterFireball(this, 40, 2, 15, 1, 4);
    ...
    private EntityAISpitterFireball fastFireball
        = new EntityAISpitterFireball(this, 50, 3, 10, 1.85, 7);
    private EntityAISpitterFireball clusterFireball
        = new EntityAISpitterClusterFireball(this, 150, 10, 50, 4, 3);

    private EntityAISpitterFireball basicFireballForAlpha
        = new EntityAISpitterFireball(this, 40, 3, 7.5, 1, 8);
    private EntityAISpitterFireball splittingFireball
        = new EntityAISpitterSplittingFireball(this, 150, 7.5, 50, 1, 5);
    ...
    protected Entity findPlayerToAttack() {
        ...
        EntityPlayer ep = this.getSpitterType().isAlpha()
            ? EntityEliteStinger.getTarget(this, d, 0.7)
            : worldObj.getClosestVulnerablePlayerToEntity(this, d);
    ...
    public void onLivingUpdate() {
        ...
        if (!worldObj.isRemote) {
            ...
            if (tgt != null && riddenByEntity == null && tgt.posY > posY + 0.5
                && rand.nextInt(40) == 0)
                this.jump();
            ...
            if (headshake > 250) {
                rotationYawHead
                    = (float) (rotationYaw + 30 * Math.sin(this.getHeadshake() / 20D));
                headshake--;
            }
            if (reposition > 0)
                reposition--;
    ...
    public final EntityLivingBase findNearTarget() {
        AxisAlignedBB box
            = ReikaAABBHelper.getEntityCenteredAABB(this, this.getSightDistance());
        List<EntityLivingBase> li = worldObj.selectEntitiesWithinAABB(
    ...
    private void setSpeed(double d) {
        this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
            .setBaseValue(d * this.getSpitterType().movementSpeed);
    }

    @Override
    public float getAIMoveSpeed() {
        return (float) this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
            .getAttributeValue();
    }
    ...
    public boolean canEntityBeSeen(Entity e) {
        ...
        return (p == null
                || e.getDistanceSq(
                       p.getLocation().xCoord + 0.5,
                       p.getLocation().yCoord + 0.5,
                       p.getLocation().zCoord + 0.5
                   ) <= p.getAutoClearRadius())
            && this.isLOS(e);
    }

    private boolean isLOS(Entity e) {
        tracer.setOrigins(
            posX, posY + height / 2, posZ, e.posX, e.posY + e.height / 2, e.posZ
        );
    ...
    public void playLivingSound() {
        SFSounds s = rand.nextBoolean()
            ? SFSounds.SPITTER3
            : (rand.nextBoolean() ? SFSounds.SPITTER1 : SFSounds.SPITTER2);
        SpitterType t = this.getSpitterType();
        float f = (float) ReikaRandomHelper.getRandomBetween(
            t.isAlpha() ? 0.5 : 0.9, t.isAlpha() ? 0.8 : 1.3
        );
    ...
    protected void dropFewItems(boolean player, int looting) {
        int n
            = this.getSpitterType().isAlpha() ? 2 + rand.nextInt(4) : 1 + rand.nextInt(3);
        for (int i = 0; i < n; i++) {
    ...
    public final void fireFireballAt(EntityLivingBase e) {
        ...
        switch (this.getSpitterType()) {
            ...
            case RED:
                ...
                break;
            case GREEN:
                if (d < clusterFireball.minDistSq
                    || ReikaMathLibrary.py3d(motionX, motionY, motionZ) >= 0.25)
                    ai = fastFireball;
                else
                    ai = clusterFireball;
                break;
    ...
    public final void doKnockbackBlast(EntityLivingBase target) {
        if (this.isBlastReady()) {
            EntityAISpitterBlast ai
                = this.getSpitterType().isAlpha() ? knockbackBlastBig : knockbackBlast;
            ai.trigger(target);
        }
    }
    ...
    public void setSpitterType(SpitterType type) {
        ...
        if (old != type || ticksExisted < 5) {
            ...
            this.getEntityAttribute(SharedMonsterAttributes.followRange)
                .setBaseValue(type.isAlpha() ? 45 : 35);
    ...
    public static enum SpitterType {
        ...
        public String getName() {
            return StatCollector.translateToLocal(
                "spitter.type." + this.name().toLowerCase(Locale.ENGLISH)
            );
        }
    }
