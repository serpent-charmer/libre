...
public class EntityAIButterflyPollinatePaleberries extends EntityAIButterflyPaleberry {
    ...
    public boolean continueExecuting() {
        if (target != null && this.isValidToStart()
            && this.isValidTarget(world, target.xCoord, target.yCoord, target.zCoord)) {
    ...
    public static boolean isValidTarget(World world, int x, int y, int z) {
        return world.getBlock(x, y, z) == SFBlocks.GRASS.getBlockInstance()
            && world.getBlockMetadata(x, y, z) == GrassTypes.PALEBERRY_EMPTY.ordinal();
    }
