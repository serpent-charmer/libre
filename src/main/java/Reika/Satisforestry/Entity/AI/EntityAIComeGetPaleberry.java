...
public class EntityAIComeGetPaleberry extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        if (closestPlayer.isJumping || !closestPlayer.onGround
    ...
    public boolean continueExecuting() {
        return !pathfinder.noPath() && EntityAIRunFromPlayer.hasPaleberry(closestPlayer)
            && closestPlayer.getDistanceSqToEntity(doggo) > MIN_DIST * MIN_DIST;
    }
    ...
    public void updateTask() {
        doggo.getLookHelper().setLookPositionWithEntity(
            closestPlayer, 10.0F, doggo.getVerticalFaceSpeed()
        );
        doggo.setLured(true);
                ...
                for (int l = 0; l <= 4; ++l) {
                    for (int i1 = 0; i1 <= 4; ++i1) {
                        if ((l < 1 || i1 < 1 || l > 3 || i1 > 3)
                            ...
                            && !world.getBlock(i + l, k, j + i1).isNormalCube()
                            && !world.getBlock(i + l, k + 1, j + i1).isNormalCube()) {
                            doggo.setLocationAndAngles(
                                i + l + 0.5F,
                                k,
                                j + i1 + 0.5F,
                                doggo.rotationYaw,
