...
public class EntityAIRunFromPlayer extends EntityAIBase {
    ...
    public EntityAIRunFromPlayer(
        EntityLizardDoggo e, double dd, double sfar, double sclose
    ) {
        doggo = e;
    ...
    public boolean shouldExecute() {
        ...
        closestPlayer
            = doggo.worldObj.getClosestPlayerToEntity(doggo, distanceToDetectPlayer);
        ...
        Vec3 vec3 = RandomPositionGenerator.findRandomTargetBlockAwayFrom(
            ...
            Vec3.createVectorHelper(
                closestPlayer.posX, closestPlayer.posY, closestPlayer.posZ
            )
        );
    ...
    static boolean isThreatening(EntityLizardDoggo e, EntityPlayer ep) {
        ...
        if (hasPaleberry(ep))
            return false;
        double vel = ReikaMathLibrary.py3d(
            ep.lastTickPosX - ep.posX, 0, ep.lastTickPosZ - ep.posZ
        ); //always zero speed no matter how it is measured!?
    ...
    public boolean continueExecuting() {
        return !pathfinder.noPath()
            && (isThreatening(doggo, closestPlayer) || scaredTick > 0);
    }
