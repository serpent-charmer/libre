...
public class EntityAIDepositItem extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        if (doggo.getDistanceSq(
                closestChest.xCoord + 0.5,
                closestChest.yCoord + 0.5,
                closestChest.zCoord + 0.5
            )
            > distanceToDetectChest * distanceToDetectChest)
            return false;

        Vec3 vec3 = Vec3.createVectorHelper(
            closestChest.xCoord + 0.5, closestChest.yCoord + 1, closestChest.zCoord + 0.5
        );
        ...
        for (int i = -r; i <= r; i++) {
            for (int k = -r; k <= r; k++) {
                for (int j = -1; j <= 1; j++) {
                    ...
                    if (te instanceof TileEntityChest || te instanceof LootChest
                        || InterfaceCache.MEINTERFACE.instanceOf(te)) {
                        if (ret == null
                            || doggo.getDistanceSq(
                                   ret.xCoord + 0.5, ret.yCoord + 0.5, ret.zCoord + 0.5
                               )
                                > doggo.getDistanceSq(
    ...
    public boolean continueExecuting() {
        return !pathfinder.noPath() && closestChest != null && !closestChest.isInvalid()
            && (doggo.hasItem() || doggo.justDepositedItem(true)) && entityPath != null;
    }
    ...
    public void updateTask() {
        Vec3 vec0 = Vec3.createVectorHelper(
            closestChest.xCoord + 0.5,
            closestChest.yCoord + 0.5,
            closestChest.zCoord + 0.5
        );
        if (closestChest != null && doggo.hasItem()
            && (doggo.getDistanceSq(vec0.xCoord, vec0.yCoord, vec0.zCoord) <= 2.5
                || this.isTooFarToBother())) {
            ...
            if (!doggo.hasItem()) {
                if (wasSitting) {
                    ...
                    if (vec3 != null) {
                        entityPath = pathfinder.getPathToXYZ(
