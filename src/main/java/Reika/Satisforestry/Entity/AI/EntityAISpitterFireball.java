...
public class EntityAISpitterFireball extends EntityAIDistanceDependent {
    ...
    public EntityAISpitterFireball(
        EntitySpitter e, int maxTime, double mind, double maxd, double fs, float fd
    ) {
        super(mind, maxd);
        entityHost = e;
    ...
    public final int getCooldown() {
        ...
        if (entityHost.riddenByEntity instanceof EntityPlayer) {
            int slug
                = SFAux.getSlugHelmetTier((EntityLivingBase) entityHost.riddenByEntity);
            if (slug > 0) {
                cool *= 1 - 0.25 * slug;
            }
        }
    ...
    public final void updateTask() {
        if (attackTarget == null)
            return;
        //ReikaJavaLibrary.pConsole(entityHost.getSpitterType()+" "+entityHost+" executing
        //"+this+" "+fireballSpeed+"/"+fireballDamage);
        entityHost.getLookHelper().setLookPositionWithEntity(
            attackTarget, 10.0F, entityHost.getVerticalFaceSpeed()
        );
        ...
        //ReikaJavaLibrary.pConsole(entityHost.getSpitterType()+" "+new
        //DecimalPosition(entityHost)+" ticking "+this);
    }
    ...
    protected final void fireFireball() {
        //ReikaJavaLibrary.pConsole("Firing "+this.getClass().getSimpleName()+"
        //d="+Math.sqrt(currentDistSq)+" from "+entityHost.getSpitterType()+" @ "+new
        //DecimalPosition(entityHost));
        World world = entityHost.worldObj;
        this.updateTargeting(attackTarget);
        this.doFireFireball(
            world, entityHost, attackTarget, fireballSpeed, fireballDamage
        );
    }
    ...
    protected void doFireFireball(
        World world, EntitySpitter src, EntityLivingBase tgt, double sp, float dmg
    ) {
        EntitySpitterFireball esf = new EntitySpitterFireball(
            world,
            src,
            targetVelocity.xCoord,
            targetVelocity.yCoord,
            targetVelocity.zCoord,
            sp,
            dmg
        );
    ...
    public static class EntityAISpitterClusterFireball extends EntityAISpitterFireball {
        public EntityAISpitterClusterFireball(
            EntitySpitter e, int maxTime, double mind, double maxd, double fs, float fd
        ) {
        ...
        @Override
        protected void doFireFireball(
            World world, EntitySpitter src, EntityLivingBase e, double sp, float dmg
        ) {
            for (int i = 0; i < 18; i++) {
                TickScheduler.instance.scheduleEvent(
                    new ScheduledTickEvent(new ScheduledEvent() {
                        @Override
                        public void fire() {
                            EntityLivingBase tgt = e;
                            if ((tgt.isDead || tgt.getHealth() <= 0)
                                && !(tgt instanceof EntityPlayer)) {
                                tgt = src.findNearTarget();
                                if (tgt == null)
                                    return;
                            }
                            EntityAISpitterClusterFireball.this.updateTargeting(tgt);
                            double vx2 = ReikaRandomHelper.getRandomPlusMinus(
                                targetVelocity.xCoord, 0.3
                            );
                            double vy2 = ReikaRandomHelper.getRandomPlusMinus(
                                targetVelocity.yCoord, 0.15
                            );
                            double vz2 = ReikaRandomHelper.getRandomPlusMinus(
                                targetVelocity.zCoord, 0.3
                            );
                            EntitySpitterFireball esf = new EntitySpitterFireball(
                                world, src, vx2, vy2, vz2, sp, dmg
                            );
                            esf.posY = src.posY + src.height / 2.0F + 0.5D;
                            double px
                                = ReikaRandomHelper.getRandomPlusMinus(src.posX, 0.5);
                            double pz
                                = ReikaRandomHelper.getRandomPlusMinus(src.posZ, 0.5);
    ...
    public static class EntityAISpitterSplittingFireball extends EntityAISpitterFireball {
        public EntityAISpitterSplittingFireball(
            EntitySpitter e, int maxTime, double mind, double maxd, double fs, float fd
        ) {
            super(e, maxTime, mind, maxd, fs, fd);
        }
        ...
        protected void doFireFireball(
            ...
            EntitySplittingSpitterFireball esf = new EntitySplittingSpitterFireball(
                world,
                src,
                tgt,
                targetVelocity.xCoord,
    ...
    public static double getYTarget(EntityLivingBase e, EntitySpitter src) {
        return e.boundingBox.minY + e.height / 2.0F
            - (src.posY + src.height / 2.0F); //+(true ? -1.5 : 0);
    }
}