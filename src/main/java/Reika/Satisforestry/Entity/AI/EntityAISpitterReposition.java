...
public abstract class EntityAISpitterReposition extends EntityAIDistanceDependent {
    ...
    protected final boolean isTargetValid() {
        ...
        if (attackTarget == null) {
            //ReikaJavaLibrary.pConsole(entity.getSpitterType()+" "+new
            //DecimalPosition(entity)+" ending "+this+" becayse no target");
            return false;
        }
        this.fetchDistance(entity, attackTarget);
        //ReikaJavaLibrary.pConsole(entity.getSpitterType()+" "+new
        //DecimalPosition(entity)+" ending "+this+" because wrong dist @
        //"+Math.sqrt(currentDistSq), !this.isDistanceAppropriate());
        return this.isDistanceAppropriate();
    }

    @Override
    public final boolean continueExecuting() {
        return this.isTargetValid() && !entity.getNavigator().noPath()
            && this.getDistanceToTarget()
            > this.getStoppingDistance() * this.getStoppingDistance();
    }

    protected final double getDistanceToTarget() {
        return this.needsTargetPosition()
            ? entity.getDistanceSq(xPosition, yPosition, zPosition)
            : entity.getDistanceSqToEntity(attackTarget);
    }
    ...
    public final void startExecuting() {
        //ReikaJavaLibrary.pConsole(entity.getSpitterType()+" "+new
        //DecimalPosition(entity)+" executing "+this);
        entity.setRunning(this.isRunning());
        if (Double.isFinite(xPosition)
            && entity.worldObj.func_147447_a(
                   Vec3.createVectorHelper(xPosition, yPosition + 0.5, zPosition),
            ...
            if (entity.getNavigator().tryMoveToXYZ(
                    xPosition, yPosition, zPosition, entity.getAIMoveSpeed() * 2
                ))
                entity.setRepositioned();
        } else {
            entity.getNavigator().tryMoveToEntityLiving(
                attackTarget, entity.getAIMoveSpeed() * 3
            );
        }
        //ReikaJavaLibrary.pConsole("Repositioning to "+(Double.isFinite(xPosition) ?
        //Vec3.createVectorHelper(xPosition, yPosition, zPosition) : attackTarget));
    }
    ...
    public void updateTask() {
        entity.setRunning(this.isRunning());
        //ReikaJavaLibrary.pConsole(entity.getSpitterType()+" "+new
        //DecimalPosition(entity)+" ticking "+this);
    }
