...
public abstract class EntityAIButterflyPaleberry extends EntityAIBase {
    ...
    public final boolean shouldExecute() {
        if (SFOptions.PALEBERRYPOLLEN.getState() && this.needsPollen()
            && !this.checkPollen()) {
            return false;
        }
    ...
    protected final Coordinate findTarget(World world) {
        if (target != null
            && this.isValidTarget(target.xCoord, target.yCoord, target.zCoord))
            return target;
    ...
    public final void startExecuting() {
        ...
        //if (butterfly.getEntity().getNavigator().tryMoveToXYZ(target.xCoord,
        //target.yCoord+0.25, target.zCoord, butterfly.getEntity().getAIMoveSpeed()*1.5F))
        //ReikaJavaLibrary.pConsole(world.getTotalWorldTime()+": Pathing to "+target+" @
        //"+butterfly);
        ReikaBeeHelper.setButterflyTarget(
            butterfly,
            Vec3.createVectorHelper(
                target.xCoord + 0.5, target.yCoord + 1, target.zCoord + 0.5
            )
        );
        //else
    ...
    public void updateTask() {
        ...
        e.rotationYaw = e.rotationYawHead
            = (float) Math.toDegrees(Math.atan2(e.motionZ, e.motionX));
    }

    @Override
    public final void resetTask() {
        super.resetTask();
        //ReikaJavaLibrary.pConsole(world.getTotalWorldTime()+": Resetting from "+target+"
        //@ "+butterfly);
