...
public class EntityAIButterflyHarvestPaleberries extends EntityAIButterflyPaleberry {
    ...
    protected boolean isValidTarget(int x, int y, int z) {
        return world.getBlock(x, y, z) == SFBlocks.GRASS.getBlockInstance()
            && world.getBlockMetadata(x, y, z) == GrassTypes.PALEBERRY_NEW.ordinal();
    }
    ...
    public void updateTask() {
        ...
        if (this.isValidTarget(x, y, z)) {
            ...
            world.setBlockMetadataWithNotify(
                x, y, z, GrassTypes.PALEBERRY_EMPTY.ordinal(), 2
            );
            ReikaItemHelper.dropItem(
                butterfly.getEntity(), new ItemStack(Satisforestry.paleberry)
            );
            target = null;
        }
    }
