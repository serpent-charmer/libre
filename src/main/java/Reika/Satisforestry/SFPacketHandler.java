    ...
    public void handleData(PacketObj packet, World world, EntityPlayer ep) {
        ...
        try {
            switch (pack) {
                ...
                case SPITTERFIREHIT:
                    if (world.isRemote) {
                        ...
                        if (e instanceof EntitySpitterFireball) {
                            MovingObjectPosition mov = new MovingObjectPosition(
                                data[2],
                                data[3],
                                data[4],
                                data[5],
                            ...
                            mov.typeOfHit = data[1] > 0 ? MovingObjectType.BLOCK
                                                        : MovingObjectType.ENTITY;
                            mov.entityHit = data[6] == Integer.MIN_VALUE
                                ? null
                                : world.getEntityByID(data[6]);
                ...
                case CRASHUNLOCK:
                    if (!world.isRemote) {
                        boolean ret = ((TileCrashSite) world.getTileEntity(x, y, z))
                                          .tryOpen((EntityPlayerMP) ep);
                        ReikaPacketHelper.sendDataPacket(
                            Satisforestry.packetChannel,
                            SFPackets.CRASHUNLOCKRETURN.ordinal(),
                            (EntityPlayerMP) ep,
                            ret ? 1 : 0
                        );
                    }
