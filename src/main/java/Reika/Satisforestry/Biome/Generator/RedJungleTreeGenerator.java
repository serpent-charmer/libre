...
public class RedJungleTreeGenerator extends PinkTreeGeneratorBase {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        if (!ReikaPlantHelper.SAPLING.canPlantAt(world, x, y, z))
            return false;
        int h0 = ReikaRandomHelper.getRandomBetween(
            isSaplingGrowth ? 6 : 8, isSaplingGrowth ? 14 : 12, rand
        );
        for (int d = 2; d < 6; d++) {
            ...
            while (ReikaWorldHelper.softBlocks(world, dx, y + i, dz)
                   || (world.getBlock(dx, y + i, dz) == SFBlocks.LOG.getBlockInstance()
                       && world.getBlockMetadata(dx, y + i, dz) % 4
                           == PinkTreeTypes.JUNGLE.ordinal())) {
                //ReikaJavaLibrary.pConsole(dir+"@"+i+":"+BlockKey.getAt(world, dx, y+i,
                //dz)+"="+BlockKey.getAt(world, dx, y+i, dz).getLocalized(), dir ==
                //ForgeDirection.EAST);
                if (i >= -3)
                    this.setBlockAndNotifyAdequately(
                        world, dx, y + i, dz, SFBlocks.LOG.getBlockInstance(), 2
                    );
                else if (!forceGen)
                    this.setBlockAndNotifyAdequately(
                        world, dx, y + i, dz, Blocks.dirt, 0
                    );
                i--;
            }
            //ReikaJavaLibrary.pConsole(dir+"@"+i+":"+BlockKey.getAt(world, dx, y+i,
            //dz)+"="+BlockKey.getAt(world, dx, y+i, dz).getLocalized());
        }
        for (int i = 0; i < h0; i++) {
            this.setBlockAndNotifyAdequately(
                world, x, y + i, z, SFBlocks.LOG.getBlockInstance(), 2
            );
        }
        int dy = y + h0;
        ...
        for (int i = 0; i < rf.length; i++) {
            rf[i]
                = ReikaRandomHelper.getRandomBetween(factors[i][0], factors[i][1], rand);
            if (i > 0) {
                rf[i] = MathHelper.clamp_double(rf[i], factors[i][0], rf[i - 1] * 0.9);
            }
        }
        double r0
            = ReikaRandomHelper.getRandomBetween(isSaplingGrowth ? 4.5 : 4.75, 6.5, rand);
                        ...
                        if (i != 0 || k != 0 || h >= 0) {
                            ...
                            if (c.canBeReplacedByLeaves(world)) {
                                this.setBlockAndNotifyAdequately(
                                    ...
                                    c.yCoord,
                                    c.zCoord,
                                    SFBlocks.LEAVES.getBlockInstance(),
                                    PinkTreeTypes.JUNGLE.ordinal()
                                );
                                if (i != 0 || k != 0) {
                                    leaves.put(c, dd);
    ...
    private void generateVines(
        ...
        Random rand,
        int h0,
        HashMap<Coordinate, Double> leaves
    ) {
        ...
        int n = ReikaRandomHelper.getRandomBetween(
            isSaplingGrowth ? 6 : 15, isSaplingGrowth ? 12 : 20, rand
        );
        for (int i = 0; i < n; i++) {
    ...
    private void
    tryPlaceVine(World world, Random rand, int x, int y, int z, ForgeDirection... sides) {
        if (rand.nextDouble() <= (isSaplingGrowth ? 0.75 : 0.9)
            && world.getBlock(x, y, z).isAir(world, x, y, z))
            this.setBlockAndNotifyAdequately(
                world, x, y, z, Blocks.vine, ReikaBlockHelper.getVineMetadatasFor(sides)
            );
    }
    ...
    protected int getSlugByHeight(int y, int dy, Random rand) {
        return rand.nextInt(4) == 0
                && this.getHeightFraction(y) >= 0.8 + rand.nextDouble() * 0.2
            ? 1
            : 0;
    }
