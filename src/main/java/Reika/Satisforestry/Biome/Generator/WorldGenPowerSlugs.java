...
public class WorldGenPowerSlugs {
    ...
    public int generate(World world, Random rand, int chunkX, int chunkZ) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            while (!li.isEmpty()) {
                ...
                if (rand.nextInt(4) > 0) {
                    ImmutablePair<Coordinate, ForgeDirection> c2
                        = this.tryFindCaveSpace(world, c.xCoord, c.zCoord, rand, dy);
                    if (c2 != null) {
                        c = c2.left;
                        dy = c.yCoord;
                ...
                if (cave == -1 && rand.nextInt(3) == 0) {
                    Coordinate c2
                        = this.tryGenerateOutcrop(world, c.xCoord, dy, c.zCoord, rand);
                    if (c2 != null) {
                        //successes.add(c2);
                        c = c2;
                        dy = c2.yCoord;
                ...
                if (BlockPowerSlug.canReplace(world, c.xCoord, dy, c.zCoord)) {
                    TilePowerSlug te = BlockPowerSlug.generatePowerSlugAt(
                        world,
                        c.xCoord,
                        dy,
                        c.zCoord,
                        ...
                        if (cave >= 0) {
                            for (int n = 0; n < 32; n++) {
                                int dx = ReikaRandomHelper.getRandomPlusMinus(
                                    c.xCoord, rand.nextInt(3) == 0 ? 9 : 5, rand
                                );
                                int dz = ReikaRandomHelper.getRandomPlusMinus(
                                    c.zCoord, rand.nextInt(3) == 0 ? 9 : 5, rand
                                );
                                if (!world.getBlock(dx, dy, dz).isAir(world, dx, dy, dz))
                                    continue;
                                int dy2 = dy;
                                while (world.getBlock(dx, dy2 - 1, dz)
                                           .isAir(world, dx, dy2 - 1, dz)) {
                                    dy2--;
                                }
                                if (rand.nextInt(4) == 0)
                                    UraniumCave.instance.generateMushroom(
                                        world, dx, dy2, dz, rand
                                    );
                                else
                                    UraniumCave.instance.generateStalks(
                                        world, dx, dy2, dz, rand
                                    );
                            }
                        }
    ...
    private int getHeight(World world, int x, int y, int z) {
        for (int i = 1; i <= y; i++) {
            ...
            if (b.getMaterial().blocksMovement()
                && b.getCollisionBoundingBoxFromPool(world, x, dy, z) != null)
                return i - 1;
        }
        return 256;
    }

    private ImmutablePair<Coordinate, ForgeDirection>
    tryFindCaveSpace(World world, int x, int z, Random rand, int maxY) {
        ...
        return new ImmutablePair(
            new Coordinate(x, y.intValue(), z), coords.get(y).getRandomEntry()
        );
    }
    ...
    private Coordinate tryGenerateOutcrop(World world, int x, int y, int z, Random rand) {
        ...
        switch (rand.nextInt(6)) {
            ...
            case 2:
                type = new BlockKey(
                    SFBlocks.TERRAIN.getBlockInstance(), TerrainType.OUTCROP.ordinal()
                );
                break;
            case 3:
            case 4:
                type = new BlockKey(
                    SFBlocks.TERRAIN.getBlockInstance(), TerrainType.PONDROCK.ordinal()
                );
                break;
            case 5:
        ...
        for (int j = -3; j < h; j++) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (j >= 0) {
                    ...
                    widths[i]
                        = MathHelper.clamp_float(widths[i], 0.2F, j <= lim2 ? 2 : 1);
                }
            }
        }
    ...
    private boolean isOverwritableByOutcrop(World world, int x, int y, int z) {
        if (ReikaWorldHelper.softBlocks(world, x, y, z)
            || DecoratorPinkForest.isTerrain(world, x, y, z))
            return true;
        Block b = world.getBlock(x, y, z);
        return b == SFBlocks.BAMBOO.getBlockInstance() || b.getMaterial() == Material.vine
            || b.getMaterial() == Material.plants || b == Blocks.gravel
            || b == Blocks.dirt;
    }
