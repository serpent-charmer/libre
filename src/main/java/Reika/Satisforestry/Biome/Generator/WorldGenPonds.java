...
public class WorldGenPonds extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x0, int y0, int z0) {
        if (!forceGeneration
            && Satisforestry.pinkforest.getSubBiome(world, x0, z0) != BiomeSection.SWAMP)
            return false;
        ...
        for (int i = 0; i < 2; i++) {
            ...
            PropagationCondition prop = new PropagationCondition() {
                ...
                public boolean isValidLocation(
                    ...
                    return isValidBlock(world, x, y, z)
                        && ReikaMathLibrary.isPointInsideEllipse(
                            x - root.xCoord, 0, z - root.zCoord, rx, 1, rz
                        );
                }
            };

            BlockArray arr = new BlockArray();
            arr.iterativeAddCallbackWithBounds(
                world, x0, y, z0, x - rx, y - 2, z - rz, x + rx, y + 20, z + rz, prop
            );
            ...
            if (!blocks.isEmpty() && arr.getMaxY() <= y + 7) {
                ...
                for (Coordinate c2 : adj) {
                    ...
                    if (floorAt == null || floorAt.intValue() != c2.yCoord) {
                        if (c2.softBlock(world)
                            || !DecoratorPinkForest.isTerrain(
                                world, c2.xCoord, c2.yCoord, c2.zCoord
                            ))
                            continue;
                    }
                    int dy = c2.yCoord;
                    ...
                    boolean flag = false;
                    while (ReikaWorldHelper.softBlocks(
                        world, c2.xCoord, c2.yCoord - 1, c2.zCoord
                    )) {
                        c2 = c2.offset(0, -1, 0);
                        c2.setBlock(
                            world,
                    ...
                    if (flag) {
                        c2 = c2.to2D();
                        if (top.containsKey(c2.offset(0, 0, 1))
                            && top.containsKey(c2.offset(0, 0, -1))
                            && top.containsKey(c2.offset(1, 0, 0))
                            && top.containsKey(c2.offset(-1, 0, 0))) {
                ...
                for (int i2 = 0; i2 < seams.size() / 4; i2++) {
                    Coordinate c2
                        = ReikaJavaLibrary.getRandomCollectionEntry(rand, seams);
                    c2.setBlock(world, Blocks.flowing_water);
                    channels.add(c2);
                    for (Coordinate c3 : c2.getAdjacentCoordinates()) {
                        if (c3.yCoord == c2.yCoord
                            && DecoratorPinkForest.isTerrain(
                                world, c3.xCoord, c3.yCoord, c3.zCoord
                            )) {
                            c3.setBlock(
                                world,
                                SFBlocks.TERRAIN.getBlockInstance(),
                ...
                for (Coordinate c2 : channels) {
                    for (Coordinate c3 : c2.getAdjacentCoordinates()) {
                        if (c3.getBlock(world) == SFBlocks.TERRAIN.getBlockInstance()) {
                            if (c3.offset(1, 0, 0).getBlock(world).getMaterial()
                                    == Material.water
                                && c3.offset(-1, 0, 0).getBlock(world).getMaterial()
                                    == Material.water) {
                                if (c3.offset(0, 0, 1).getBlock(world).getMaterial()
                                        == Material.water
                                    && c3.offset(0, 0, -1).getBlock(world).getMaterial()
                                        == Material.water) {
                ...
                for (int i2 = 0; i2 < n; i2++) {
                    double ang = da * i2;
                    double rang = Math.toRadians(
                        ReikaRandomHelper.getRandomPlusMinus(ang, da / 2.4D, rand)
                    );
                    ...
                    for (int tries = 0; tries < 3; tries++) {
                        ...
                        Coordinate c2 = new Coordinate(
                            ctr.xCoord + dx * dr, 0, ctr.zCoord + dz * dr
                        );
    ...
    private static boolean isValidBlock(World world, int x, int y, int z) {
        return DecoratorPinkForest.isTerrain(world, x, y, z)
            && !Satisforestry.pinkforest.isRoad(world, x, z)
            && hasSolidOnAllSides(world, x, y, z);
    }
