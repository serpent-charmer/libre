...
public class WorldGenCrashSite extends WorldGenerator {
    ...
    private static final TileEntityCache<AlternateRecipe> generatedCrashes
        = new TileEntityCache();
    ...
    private boolean tryPlace(World world, int x, int z, Random rand, boolean inBiome) {
        int minY = 999;
        int maxY = 0;
        ForgeDirection dir = rand.nextInt(15) == 0
            ? ForgeDirection.UP
        ...
        generatedCrashes.put(
            te,
            te.generate(
                rand, generatedCrashes.getAllValuesNear(new WorldLocation(te), 256)
            )
        );
        return true;
    }
    ...
    public static Collection<WorldLocation>
    getCrashSitesNear(WorldLocation loc, double r) {
