...
public class WorldGenUraniumCave extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (bf.calculate(world, x, z)) {
            ...
            CentralCave cc = new CentralCave(
                x, ReikaRandomHelper.getRandomBetween(40, top - 50, rand), z
            );
            ...
            while (Satisforestry.isPinkForest(
                world, MathHelper.floor_double(dx), MathHelper.floor_double(dz)
            )) {
                dx += c1 * d;
                dz += s1 * d;
            }
    ...
    private static class Tunnel extends UraniumCavePiece {
        ...
        private void calculate(World world, Random rand) {
            ...
            double dd
                = ReikaMathLibrary.py3d(endX - center.xCoord, 0, endZ - center.zCoord);
            int n = (int) Math.max(4, dd / 16);
            LightningBolt b = new LightningBolt(
                new DecimalPosition(center.xCoord, center.yCoord, center.zCoord),
            ...
            for (Coordinate c : carve.keySet()) {
                ...
                for (Coordinate c2 : c.getAdjacentCoordinates()) {
                    if (this.skipCarve(c2))
                        continue;
                    if (c2.yCoord <= 58 && c2.softBlock(world)
                        && !carve.containsKey(c2)) {
    ...
    private static class CentralCave extends UraniumCavePiece {
        ...
        private void calculate(World world, Random rand) {
            /*
            LobulatedCurve inner = LobulatedCurve.fromMinMaxRadii(9, 15, 3, true); //was
            6,10,3 outer.generate(rand); inner.generate(rand); int h0 = 3; int h = 8; for
            (int i = -rmax; i <= rmax; i++) { for (int k = -rmax; k <= rmax; k++) { if
            (outer.isPointInsideCurve(i, k)) { footprint.add(center.offset(i, 0,
            k).to2D()); if (!inner.isPointInsideCurve(i, k)) { for (int j = -h0; j <= h;
            j++) { double ry = 1; if (j < 0) { ry *= 1+j*0.4/h0;
                                }
                                else if (h-j <= 4) {
            ...
            for (double a = 0; a < 360; a += da) {
                double ra
                    = Math.toRadians(ReikaRandomHelper.getRandomPlusMinus(a, da / 3, rand)
                    );
                ...
                double dy
                    = ReikaRandomHelper.getRandomPlusMinus(center.yCoord + 0.5, 6, rand);
                double dd = 2;
                DecimalPosition p = new DecimalPosition(
                    ReikaRandomHelper.getRandomPlusMinus(dx, dd),
                    dy,
                    ReikaRandomHelper.getRandomPlusMinus(dz, dd)
                );
                s.addPoint(new BasicSplinePoint(p));
                dd = 5; //4;
                rooms.add(new CaveRoom(p.offset(
                    ReikaRandomHelper.getRandomPlusMinus(0, dd),
                    0,
                    ReikaRandomHelper.getRandomPlusMinus(0, dd)
                )));
            }
