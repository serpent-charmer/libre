...
public class WorldGenPoisonRocks extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int yUnused, int z) {
        ...
        for (int i = 0; i < generationRate; i++) {
            ...
            if (forceGen
                || (this.isReplaceable(world, dx, dy, dz)
    ...
    private boolean tryPlace(World world, int x, int y, int z, Random rand) {
        ...
        for (int i = 0; i < h; i++) {
            ...
            if (bulge[i]) {
                if (!this.isReplaceable(world, x - 1, y + i, z)
                    || !this.isReplaceable(world, x + 1, y + i, z)
                    || !this.isReplaceable(world, x, y + i, z - 1)
                    || !this.isReplaceable(world, x, y + i, z + 1)) {
                    return false;
                }
            }
        ...
        if (dy < 0 || dy >= 256) {
            Satisforestry.logger.logError(
                "Tried to place poison rocks out of bounds (" + dy + ") @ " + x + ", " + z
                + "!"
            );
            return false;
        }
