...
public class WorldGenPinkGrass extends WorldGenerator {
    private static final GrassType BASE = new GrassType(
        Blocks.tallgrass,
        1,
        new Interpolation(false)
            .addPoint(60, 50)
            .addPoint(90, 25)
            .addPoint(110, 5)
            .addPoint(120, 0)
    ...
    public WorldGenPinkGrass() {
        ...
        grassTypes.addDynamicEntry(new GrassType(
            Blocks.tallgrass,
            2,
            new Interpolation(false).addPoint(80, 0).addPoint(100, 20).addPoint(128, 5)
        ));
        Block b = SFBlocks.GRASS.getBlockInstance();
        grassTypes.addDynamicEntry(new GrassType(
            b,
            GrassTypes.FERN.ordinal(),
            new Interpolation(false).addPoint(60, 40).addPoint(100, 15).addPoint(120, 5)
        ));
        grassTypes.addDynamicEntry(new GrassType(
            b,
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int i = 0; i < 128; i++) {
            ...
            boolean paleberry = dy >= 105 && world.isAirBlock(dx, dy + 1, dz)
                && rand.nextInt(2000) < this.paleberryChance(world, dx, dy, dz);
            if (paleberry) {
                place = new BlockKey(
                    SFBlocks.GRASS.getBlockInstance(),
                    GrassTypes.PALEBERRY_STALK.ordinal()
                );
            }

            if (world.isAirBlock(dx, dy, dz)
                && place.blockID.canBlockStay(world, dx, dy, dz)) {
    ...
    private void initNoise(World world) {
        if (noise == null || noise.seed != world.getSeed()) {
            noise = (SimplexNoiseGenerator) new SimplexNoiseGenerator(-world.getSeed())
                        .setFrequency(1 / 3.5D);
        }
    }

    private int paleberryChance(
        World world, int x, int y, int z
    ) { //old 1/200 = 10, >= 2000 for a guarantee
    ...
    private BlockKey getBlockToPlace(World world, int dx, int dy, int dz, Random rand) {
        ...
        if (type == null) {
            Satisforestry.logger.logError(
                "Null grass type calculated @ " + new Coordinate(dx, dy, dz) + "="
                + grassTypes.getTotalWeight()
            );
            type = BASE;
        }
        return type.block;
