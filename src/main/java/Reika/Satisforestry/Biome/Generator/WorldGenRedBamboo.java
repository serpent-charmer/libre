...
public class WorldGenRedBamboo {
    ...
    public boolean generate(World world, Random rand, int chunkX, int chunkZ) {
        ...
        for (int dx = chunkX; dx < chunkX + 16; dx++) {
            for (int dz = chunkZ; dz < chunkZ + 16; dz++) {
                ...
                if (this.isReplaceable(world, dx, dy, dz)
                    && !Satisforestry.pinkforest.isRoad(world, dx, dz)
                    && SFBlocks.BAMBOO.getBlockInstance().canBlockStay(
                        world, dx, dy, dz
                    )) {
                    int h = ReikaRandomHelper.getRandomBetween(
                        3, 7, rand
                    ); //TODO noisemap for heights?
                    for (int d = 0; d < h; d++) {
                        if (this.isReplaceable(world, dx, dy + d, dz)) {
                            world.setBlock(
                                dx, dy + d, dz, SFBlocks.BAMBOO.getBlockInstance(), 15, 2
                            );
                        } else {
                            break;
                        }
    ...
    private boolean isReplaceable(World world, int x, int y, int z) {
        return ReikaWorldHelper.softBlocks(
            world, x, y, z
