...
public abstract class PinkTreeGeneratorBase extends ModifiableBigTree {
    //public static final SpillingGenerationDelegationSystem blockPlacer = new
    //SpillingGenerationDelegationSystem();
    ...
    protected void postGenerate(World world, Random rand, int x, int y, int z) {
        if (allowSlugs) {
                ...
                if (rand.nextFloat() < this.getTreeTopSlugChance()) {
                    ...
                    while (te == null && !set.isEmpty()) {
                        ...
                        te = BlockPowerSlug.generatePowerSlugAt(
                            ...
                            tier,
                            false,
                            this.getDifficultyByHeight(dy, ddy, rand),
                            this.canSpawnLeaftopMobs(),
                            3,
                            ForgeDirection.DOWN
                        );
                    }
            ...
            for (Entry<Coordinate, Integer> e : logs.entrySet()) {
                if (e.getValue() < 4) {
                    Coordinate c = e.getKey();
                    if (c.yCoord >= trunkBottom
                        && c.yCoord <= Math.min(trunkTop, bottomLeaf - 1)) {
                        if (rand.nextFloat() < this.getTrunkSlugChancePerBlock()) {
                            //blockPlacer.placeChunkBlockCoords(world, c.xCoord,
                            //c.zCoord);
                            ...
                            for (ForgeDirection dir : li) {
                                ...
                                TilePowerSlug te = BlockPowerSlug.generatePowerSlugAt(
                                    ...
                                    tier,
                                    false,
                                    this.getDifficultyByHeight(
                                        c2.yCoord - generationOriginY, dy, rand
                                    ),
                                    false,
                                    1,
                                    dir
                    ...
                    if (c.isEmpty(world)) {
                        ...
                        int diff = this.getDifficultyByHeight(
                                       c.yCoord - generationOriginY, dy, rand
                                   )
                            + 1; //+1 since hard to find
    ...
    setBlockAndNotifyAdequately(World world, int x, int y, int z, Block b, int meta) {
        ...
        //blockPlacer.setBlock((WorldServer)world, x+globalOffset[0], y+globalOffset[1],
        //z+globalOffset[2], b, meta, doUpdates ? 3 : 2);
        super.setBlockAndNotifyAdequately(world, x, y, z, b, meta);
        if (log) {
            logs.put(
                new Coordinate(
                    x + globalOffset[0], y + globalOffset[1], z + globalOffset[2]
                ),
                meta
            );
        } else if (leaf) {
    ...
    public static enum PinkTreeTypes implements PinkTreeType {
        ...
        private PinkTreeTypes() {
            String base
                = "satisforestry:pinkleaf"; //_"+this.name().toLowerCase(Locale.ENGLISH);
        ...
        public static PinkTreeTypes getLeafType(int meta) {
            return meta == OreDictionary.WILDCARD_VALUE ? PinkTreeTypes.TREE
                                                        : PinkTreeTypes.list[meta % 4];
        }
