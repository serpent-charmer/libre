...
public class BiomePinkForest
    extends BiomeGenBase implements DyeTreeBlocker, NonconvertibleBiome,
                                    WinterBiomeStrengthControl, CustomMapColorBiome {
    ...
    private static final NoiseGeneratorBase leafColorMix
        = new Simplex3DGenerator(-System.currentTimeMillis()).setFrequency(1 / 1.85D);
    private static final NoiseGeneratorBase waterColorMix
        = new SimplexNoiseGenerator(~System.currentTimeMillis()).setFrequency(1 / 3.5D);
    private static final NoiseGeneratorBase waterBrightnessMix
        = new SimplexNoiseGenerator(~System.currentTimeMillis()).setFrequency(1.5D);
    ...
    public BiomePinkForest(int id) {
        ...
        enableRain = true;
        enableSnow = true; //but melt the snow when it is daytime and sunny, and disallow
                           //all water freezing to ice
        ...
        //base vanilla mobs, most with 1/4th spawn rates (halved for endermen)
        spawnableMonsterList.add(
            new BiomeGenBase.SpawnListEntry(EntityZombie.class, 25, 2, 3)
        );
        spawnableMonsterList.add(
            new BiomeGenBase.SpawnListEntry(EntitySkeleton.class, 25, 2, 3)
        );
        spawnableMonsterList.add(
            new BiomeGenBase.SpawnListEntry(EntityCreeper.class, 25, 1, 3)
        );
        spawnableMonsterList.add(
            new BiomeGenBase.SpawnListEntry(EntityEnderman.class, 5, 1, 2)
        );
        spawnableMonsterList.add(
            new BiomeGenBase.SpawnListEntry(EntitySlime.class, 100, 4, 4)
        );

        //spawnableMonsterList.add(new
        //BiomeGenBase.SpawnListEntry(SpawnReducingPlaceholder.class, 50, 1, 1));
        spawnableMonsterList.add(
            new BiomeGenBase.SpawnListEntry(EntitySpider.class, 30, 1, 3)
        ); //large stinger stand-in
        spawnableMonsterList.add(
            new BiomeGenBase.SpawnListEntry(EntityCaveSpider.class, 25, 2, 4)
        ); //basic (small) stinger stand-in
        //removed to add dynamically spawnableMonsterList.add(new
        //BiomeGenBase.SpawnListEntry(EntityEliteStinger.class, 15, 1, 2)); //was 50 then
        //25

        spawnableCreatureList.add(
            new BiomeGenBase.SpawnListEntry(EntityOcelot.class, 10, 2, 2)
        ); //arachnophobia mode
        ...
        //ReikaReflectionHelper.makeFieldFinal(this.getClass(), "spawnableMonsterList",
        //"field_76761_J"); ReikaReflectionHelper.makeFieldFinal(this.getClass(),
        //"spawnableCreatureList", "field_76762_K");
        //ReikaReflectionHelper.makeFieldFinal(this.getClass(),
        //"spawnableCaveCreatureList", "field_82914_M");
    }
    ...
    public float getFloatTemperature(int x, int y, int z) {
        return (float) ReikaMathLibrary.normalizeToBounds(
            super.getFloatTemperature(x, y, z), 0.1, 0.14, 0, 1
        );
    }
    ...
    public WorldGenAbstractTree func_150567_a(Random rand) {
        return treeSelection; //rand.nextInt(4) == 0 ? new GiantPinkTreeGenerator(false,
                              //true) : new PinkTreeGenerator(false);
    }
    ...
    public int getBiomeGrassColor(int x, int y, int z) { //less intense with Y
        ...
        return ReikaColorAPI.mixColors(
            c1, 0xCD8988, f
        ); //this.isRoad(world, x, z) ? ReikaColorAPI.getModifiedHue(0xff0000,
           //this.getSubBiome(world, x, z).ordinal()*120) : 0;//
        //return this.getRoadFactor(world, x, z) >
        //Math.abs(noise.roadEdgeNoise.getValue(x, z)*0.75) ? 0xffffff : 0; return
        //ReikaColorAPI.mixColors(0, 0xffffff,
        //(float)ReikaMathLibrary.normalizeToBounds(noise.riverNoise.getValue(x, z), 0,
        //1)); return ReikaColorAPI.getModifiedHue(0xff0000,
        //noise.riverNoise.getEdgeRatio(x, z) < 5 ? 0 :
        //(int)((noise.riverNoise.getEdgeRatio(x, z)-5)*5));
        /*
        double df = 1/36D;
        double sc = 15;
        SimplexNoiseGenerator s1 = (SimplexNoiseGenerator)new
        SimplexNoiseGenerator(world.getSeed()-34589).setFrequency(df).addOctave(4.1,
        0.17).addOctave(0.08, 15.7); SimplexNoiseGenerator s2 = (SimplexNoiseGenerator)new
        SimplexNoiseGenerator(world.getSeed()-34589).setFrequency(df).addOctave(4.1,
        0.17).addOctave(0.08, 15.7); s1.clampEdge = true; s2.clampEdge = true; return
        ReikaColorAPI.mixColors(0, 0xffffff,
        noise.riverNoise.getEdgeRatio(x+s1.getValue(x, z)*sc, z+s2.getValue(x, z)*sc) < 5
        ? 0 : 1);
         */
    }
    ...
    public int getBiomeFoliageColor(int x, int y, int z) { //less intense with Y
        ...
        return ReikaColorAPI.getModifiedSat(
            ReikaColorAPI.mixColors(0xFFB3D1, 0xE95F84, f), 1.125F + f / 4F
        );
    }
    ...
    public int getWaterColor(IBlockAccess world, int x, int y, int z, int l) {
        ...
        int ret = ReikaColorAPI.mixColors(
            0xA2E2EE, 0x4F99AA, f
        ); //this.getWaterColorMultiplier();
        f = (float) ReikaMathLibrary.normalizeToBounds(
            waterBrightnessMix.getValue(x, z), 0, 1.35
        ); //was 1.5F
        f = Math.max(1, f);
    ...
    public static void updateRenderFactor(AbstractClientPlayer ep) {
        ...
        } else if (Satisforestry.isPinkForest(ep.worldObj.getBiomeGenForCoords(
    ...
    int getUpthrust(World world, int x, int z) {
        ...
        return (int) Math.round(ReikaMathLibrary.normalizeToBounds(
            noise.upthrustNoise.getValue(x, z), delta, min + delta
        ));
    }
    ...
    int getDirtThickness(World world, int x, int z) {
        return (int) Math.round(ReikaMathLibrary.normalizeToBounds(
            noise.dirtThicknessNoise.getValue(x, z), 1, 3.6
        ));
    }
    ...
    int getOutcropValue(World world, int x, int z, BiomeSection b) {
        ...
        switch (b) {
            case FOREST: {
                if (noise.outcropBooleanNoise.getValue(x, z)
                    <= 0.45) //was 0 then 0.25 then 0.66 then 0.5
                    return -50;
    ...
    public float getWinterSkyStrength(World world, EntityPlayer ep) {
        float sun = ReikaWorldHelper.getSunIntensity(
    ...
    public int getMapColor(World world, int x, int z) {
        return this.getBiomeFoliageColor(
            x,
            (int) ReikaMathLibrary.normalizeToBounds(
                waterColorMix.getValue(x / 5D, z / 5D), 120, 170
            ),
            z
        );
    }
