...
public class BiomeFootprint {
    ...
    public boolean calculate(World world, int x, int z) {
        ...
        while (!next.isEmpty()) {
            ...
            for (Coordinate c : next) {
                if (!this.searchFrom(
                        world, c, newNext
                    )) { //c was not in the biome, its neighbors are edges
    ...
    private boolean isSingleBiomeWorld(World world) {
        return ReikaMystcraftHelper.isMystAge(world)
            && ReikaMystcraftHelper.isSymbolPresent(
                world, BasicPages.BiomeControllerSingle
            );
    }
    ...
    public double getAngleAt(
        Coordinate c, int searchRadius
    ) { //linear regression https://i.imgur.com/La8ge8z.png
        ...
        ReikaJavaLibrary.pConsole(c+" > "+li2+" > "+slope+" >
    ...
    public void exportToImage(File folder) {
        ...
        File f = new File(
            folder,
            "biomefootprint " + MathHelper.floor_double(center.xCoord) + ", "
                + MathHelper.floor_double(center.zCoord) + ".png"
        );
        int r = 20;
        int x0 = minX - r;
        int x1 = maxX + r;
        ...
        int z1 = maxZ + r;
        BufferedImage buf
            = new BufferedImage(x1 - x0 + 1, z1 - z0 + 1, BufferedImage.TYPE_INT_ARGB);
        for (int x = x0; x <= x1; x++) {
            for (int z = z0; z <= z1; z++) {
                int i = x - x0;
                int k = z - z0;
