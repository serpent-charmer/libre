...
public class PinkForestPersistentData extends WorldSavedData {
    ...
    public void readFromNBT(NBTTagCompound NBT) {
        try {
            BiomewideFeatureGenerator.instance.readFromNBT(
                NBT.getCompoundTag("biomewideFeatures")
            );
            //PinkTreeGeneratorBase.blockPlacer.readFromNBT(NBT.getCompoundTag("queuedTreeBlocks"));
        } catch (Exception e) {
            Satisforestry.logger.logError(
                "Failed reading persistent NBT data for biome. It is recommended you repair or delete /data/"
                + IDENTIFIER + ".dat"
            );
            e.printStackTrace();
        }
    ...
    public static PinkForestPersistentData initNetworkData(World world) {
        PinkForestPersistentData data = (PinkForestPersistentData
        ) world.loadItemData(PinkForestPersistentData.class, IDENTIFIER);
        if (data == null) {
