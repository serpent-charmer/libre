...
public class MantaGenerator {
    ...
    public MantaPath generateLoopAround(WorldLocation loc) {
        ...
        for (double a = 0; a < 360; a += 22.5) {
            ...
            s.addPoint(new BasicSplinePoint(
                dx, loc.yCoord + ReikaRandomHelper.getRandomBetween(3, 6), dz
            ));
        }
    ...
    public MantaPath
    generatePathAroundBiome(World world, BiomeFootprint bf, Random rand) {
        ...
        prof.sort(new Comparator<Coordinate>() {
            ...
            public int compare(Coordinate o1, Coordinate o2) {
                return -Double.compare(
                    o1.getDistanceTo(cc.xCoord, o1.yCoord, cc.zCoord),
                    o2.getDistanceTo(cc.xCoord, o2.yCoord, cc.zCoord)
                );
            }
        });
        for (double a = 0; a < 360; a += ANGLE_STEP) {
            ...
            dz /= ddd;
            edge = new Coordinate(
                MathHelper.floor_double(edge.xCoord + dx * out),
                0,
                MathHelper.floor_double(edge.zCoord + dz * out)
            );
            misses = 0;
            ...
            for (int i = -1; i <= 1; i++) {
                for (int k = -1; k <= 1; k++) {
                    int top2 = ReikaWorldHelper.getTopNonAirBlock(
                        world, edge.xCoord + i, edge.zCoord + k, true
                    );
                    if (top2 > top) {
                        top = top2;
            ...
            //ReikaJavaLibrary.pConsole(lastY+" - "+edge.getBiome(world).biomeName+" @
            //"+edge+" > "+top+" = "+at+" >> "+dy+" / "+(lastY+MAX_RISE_STEP*df));
            double spiral = 0;
            double minY = lastY - MAX_DROP_STEP * df;
            if (lastY >= 0) {
                if (lastY - dy >= 18 && s.length() > 10
                    && rand.nextInt(5) > 0) { //was 100%
                    spiral = lastY - dy;
            ...
            if (spiral > 0) {
                ...
                double vx2 = -vz;
                double vz2 = vx;
                double rs = ReikaRandomHelper.getRandomBetween(15D, 25D, rand)
                    * spiralDir; //was 8/15
                spiralPath.add(
                    new DecimalPosition(edge.xCoord + 0.5, lastY, edge.zCoord + 0.5)
                );
                spiralPath.add(new DecimalPosition(
                    edge.xCoord + 0.5 + vx * rs,
                    lastY - spiral / 4D,
                    edge.zCoord + 0.5 + vz * rs
                ));
                spiralPath.add(new DecimalPosition(
                    edge.xCoord + 0.5 + vx * rs + vx2 * rs,
                    lastY - spiral / 2D,
                    edge.zCoord + 0.5 + vz * rs + vz2 * rs
                ));
                spiralPath.add(new DecimalPosition(
                    edge.xCoord + 0.5 + vx2 * rs,
                    lastY - spiral * 3 / 4D,
                    edge.zCoord + 0.5 + vz2 * rs
                ));
            }
            boolean flag = true;
                        ...
                        for (int k = -r; k <= r; k++) {
                            if (ReikaMathLibrary.py3d(i, j, k) <= r + 0.5) {
                                Coordinate c = new Coordinate(
                                    p.xCoord + i, p.yCoord + j, p.zCoord + k
                                );
                                clear.add(c);
                            }
                        }
