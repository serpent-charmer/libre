...
public class PinkRivers {
    ...
    public Collection<Coordinate>
    generateRivers(World world, int x, int z, Random rand, BiomeFootprint bf) {
        ...
        for (River r : riversToGenerate) {
            for (Coordinate r2 : riversGenerated) {
                if (!r.rootPosition.equals(r2)
                    && r.rootPosition.getTaxicabDistanceTo(r2) <= 48) {
                    r = null;
                    break;
                }
            }
    ...
    private River prepareRiverAt(World world, Coordinate c, RiverMouth mouth) {
        ...
        for (double i = -r; i <= r; i += 0.5) {
            for (double k = -2; k <= 18; k += 0.5) {
                int dx =
        MathHelper.floor_double(c.xCoord-k*river.cosNormal+i*river.cosTangent); int dz =
        MathHelper.floor_double(c.zCoord-k*river.sinNormal+i*river.sinTangent); if
        (Satisforestry.isPinkForest(world, dx, dz)) {
    ...
    private RiverMouth
    isValidRiverEndpoint(World world, int x, int z, Coordinate c, BiomeFootprint bf) {
        ...
        if (!Satisforestry.isPinkForest(world, x0, z0))
            return null;
        ...
        for (int dl = -1; dl <= 6; dl += 2) {
            ...
            if (!Satisforestry.isPinkForest(world, x1, z1))
                return null;
            if (!Satisforestry.isPinkForest(world, x2, z2))
                return null;
            double h1 = DecoratorPinkForest.getAverageHeight(world, x1, z1, 2);
            double h2 = DecoratorPinkForest.getAverageHeight(world, x1, z1, 2);
            if (Math.abs(h2 - h1) >= 3 || h2 < h - 2 || h1 < h - 2 || h < h1 - 10
                || h < h2 - 10)
                return null;
        }
        boolean flag = false;
    ...
    /*
    @Deprecated
    private ImmutablePair<Coordinate, Double> findNextTarget(World world, Coordinate root,
    double step, double pa, Coordinate from, RiverMouth river) { double ra = 30; double da
    = 2.5; double angmin = pa-ra; double angmax = pa+ra; final int h = from.yCoord;
        ArrayList<ImmutablePair<Coordinate, Double>> li = new ArrayList();
        for (double ang = angmin; ang <= angmax; ang += da) {
            int dx =
    MathHelper.floor_double(from.xCoord+step*Math.cos(Math.toRadians(ang))); int dz =
    MathHelper.floor_double(from.zCoord+step*Math.sin(Math.toRadians(ang))); if
    (Satisforestry.isPinkForest(world, dx, dz)) { double top =
    DecoratorPinkForest.getAverageHeight(world, dx, dz, 4); if (true || top >= h-6) {
                    /*
...
return li.get(0);
    ...
    private static class River {
        ...
        private void calculate(World world) {
            ...
            for (double k = 0; k <= 10; k += 0.5) {
                int dx
                    = MathHelper.floor_double(rootPosition.xCoord - k * mouth.cosNormal);
                int dz
                    = MathHelper.floor_double(rootPosition.zCoord - k * mouth.sinNormal);
            ...
            for (double step = 0; step < 240; step += 0.5) {
                int dx = MathHelper.floor_double(
                    rootPosition.xCoord - step * mouth.cosNormal
                );
                int dz = MathHelper.floor_double(
                    rootPosition.zCoord - step * mouth.sinNormal
                );
                int top = DecoratorPinkForest.getTrueTopAt(world, dx, dz);
    ...
    while (c2 != null && path.size() < 4) {
        path.add(c2);
        ImmutablePair<Coordinate, Double> pair = this.findNextTarget(c, step, pa, c2,
    river); c2 = pair != null ? pair.left : null; pa = pair != null ? pair.right : pa;
    }*/
            Spline s = new Spline(SplineType.CENTRIPETAL);
            for (Coordinate c3 : path) {
                s.addPoint(new BasicSplinePoint(
                    c3.xCoord + 0.5, c3.yCoord + 0.5, c3.zCoord + 0.5
                ));
            }
                ...
                for (int i = -r; i <= r; i++) {
                    for (int k = -r; k <= r; k++) {
                        if (i * i + k * k <= r * r) {
                            ...
                            if (Satisforestry.isPinkForest(world, ddx, ddz)
                                && DecoratorPinkForest.isTerrain(world, ddx, top, ddz)
                                && this.countSolidSides(world, ddx, top, ddz) >= 2) {
        ...
        private void carve(
            HashMap<Coordinate, BlockCandidate> map, World world, Block put, boolean force
        ) {
            for (Entry<Coordinate, BlockCandidate> e : map.entrySet()) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    if (pos == Blocks.grass || pos == Blocks.dirt || pos == Blocks.stone
                        || pos == Blocks.gravel) {
                ...
                int dy = y + 2;
                if (world.getBlock(c2.xCoord, dy, c2.zCoord) == Satisforestry.log
                    && world.getBlockMetadata(c2.xCoord, dy, c2.zCoord) <= 3) {
                    while (
                        dy >= 0
                        && (ReikaWorldHelper.softBlocks(world, c2.xCoord, dy, c2.zCoord)
                            || world.getBlock(c2.xCoord, dy, c2.zCoord)
                                == Satisforestry.log)
                    ) {
                        world.setBlock(c2.xCoord, dy, c2.zCoord, Satisforestry.log, 0, 2);
                        dy--;
                    }
    ...
    private static class BlockCandidate {
        ...
        public boolean isLeftover(World world) {
            for (int i = 0; i < 4; i++) {
                ...
                if (at == Blocks.water || at == Blocks.flowing_water
                    || !ReikaWorldHelper.softBlocks(world, dx, averageY, dz))
                    continue;
                return false;
            }
            return true;
        ...
        private boolean isValidWater(World world, int y) {
            return isValidWater(
                world, location.yCoord, location.xCoord, y, location.zCoord
            );
        }
        ...
        private static boolean isBarrier(World world, int dx, int dy, int dz) {
            Block at = world.getBlock(dx, dy, dz);
            return !ReikaWorldHelper.softBlocks(world, dx, dy, dz) || at == Blocks.water
                || at == Blocks.flowing_water;
        }
    }
    ...
    private static class SandFinder extends BreadthFirstSearch {
        ...
        private final PropagationCondition propagationCondition
            = new PropagationCondition() {
                  @Override
                  public boolean isValidLocation(
                      World world, int x, int y, int z, Coordinate from
                  ) {
                      return (y >= from.yCoord - 1 || root.yCoord - y <= 2)
                          && y >= highestY - 2 && Satisforestry.isPinkForest(world, x, z)
        ...
        getNextSearchCoordsFor(World world, Coordinate c) {
            highestY = Math.max(
                DecoratorPinkForest.getAverageHeight(
                    world,
                    c.xCoord,
                    c.zCoord,
                    2,
                    (w, x, y, z)
                        -> w.getBlock(x, y, z) == Blocks.sand
                        && Satisforestry.isPinkForest(w, x, z)
                ),
                highestY
            );
