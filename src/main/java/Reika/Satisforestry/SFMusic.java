...
public class SFMusic {
    ...
    private static final String potentialSupportMsg
        = "This file type can be supported with the use of the NotEnoughCodecs Mod. ";
    private static final String officialSiteMsg
        = "The mod can be downloaded from its official CurseForge page: https://www.curseforge.com/minecraft/mc-mods/notenoughcodecs/files";
    private static final String backportMsg
        = "To support this file type, you will need a backported version of a modern version for new filetype support: https://gregtech.overminddl1.com/openmods/codecs/NotEnoughCodecs/1.7.10-0.6/NotEnoughCodecs-1.7.10-0.6.jar";
    ...
    public void playCustomMusic(ClientTickEvent evt) {
        ...
        if (world != null && ep != null
            && Satisforestry.isPinkForest(
                world, MathHelper.floor_double(ep.posX), MathHelper.floor_double(ep.posZ)
            ))
    ...
    private void loadCodecs() {
        try {
            ...
            throw new RegistrationException(Satisforestry.instance, "Could not register
    music codecs!", e);
        }
    }*/
    ...
    public void loadMusic(String path) {
        ...
        if (folder.exists() && folder.isDirectory()) {
            ...
            throw new InstallationException(
                Satisforestry.instance, "Specified OST folder does not exist @ " + path
            );
        }
    }
        ...
        for (File f : f0.listFiles()) {
            ...
            if (type == MusicSupport.SUPPORTED) {
                ...
                Satisforestry.logger.logError(
                    "Track " + f + " is not a recognized audio file!"
                );
                if (type == MusicSupport.MP3 || type == MusicSupport.FLAC) {
                    potentiallySupportedFiles++;
                    boolean official = type == MusicSupport.MP3
                        || !MinecraftForge.MC_VERSION.startsWith("1.7");
                    if (!official)
    ...
    public void tickMusicEngine(World world) {
        ...
        if (currentMusic != null && ReikaObfuscationHelper.isDeObfEnvironment()
        ...
        //ReikaJavaLibrary.pConsole("Selected at time "+world.getWorldTime()%24000+" =
        //"+DayQuadrant.getByTime(world.getWorldTime())+" : "+currentMusic);
    ...
    private SFMusicEntry selectTrack(World world) {
        ...
        return dq != null
            ? ReikaJavaLibrary.getRandomListEntry(DragonAPICore.rand, dq.tracks)
            : null;
    }
    ...
    public static enum DayQuadrant {
        ...
        public void addTrack(String path) throws IOException {
            ...
            DirectResourceManager.getInstance().registerCustomPath(
                mus.path, SFClient.sfCategory, true
            );
            tracks.add(mus);
        }
    }
