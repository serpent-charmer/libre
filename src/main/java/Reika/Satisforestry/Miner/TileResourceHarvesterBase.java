...
public abstract class TileResourceHarvesterBase<N extends ResourceNode, S>
    extends TileEntityBase implements ChunkLoadingTile {
    private int tier = 0;
    ...
    public final int getLightbarColorForRender() {
        if (StructureRenderer.isRenderingTiles()) {
            int idx = (int
            ) ((System.currentTimeMillis() / 1000) % (MachineState.list.length + 1));
            return idx == MachineState.list.length ? 0xb0d0ff
                                                   : MachineState.list[idx].color;
        }
        int c = state.color;
        ...
        if (world.isRemote) {
            ...
            if (this.hasStructure() && te != null && !this.isShutdown(world, x, y, z)) {
                if (this.hasEnergy(false)) {
                    ...
                    if (this.isReady(te)) {
                        stepTime = (int
                        ) (te.getHarvestInterval() / this.getNetSpeedFactor(false));
    ...
    public final float getNetSpeedFactor(boolean display) {
        return (1 + this.getTier()) * this.getOverclockingLevel(display)
            * this.getSpeedFactor();
    }
    ...
    public final int getOverclockingStep(boolean display) {
        return FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT && display
            ? overclockDisplay
            : overclock.getOverclockingLevel();
    }
