...
public class GuiSFFracker extends GuiSFHarvesterBase<TileFrackingPressurizer> {
    ...
    public GuiSFFracker(EntityPlayer ep, TileFrackingPressurizer te) {
        ...
        if (n != null) {
            ...
            Collections.sort(
                satellites, (n1, n2) -> n1.getPurity().compareTo(n2.getPurity())
            );
        }
    }
    ...
    protected void drawExtraGuiElements(ResourceNode te, int j, int k) {
        if (te != null) {
            ...
            //ReikaGuiAPI.instance.drawCenteredStringNoShadow(fontRendererObj, min"x",
            //j+104, k+55, 0x646464);
            //ReikaGuiAPI.instance.drawCenteredStringNoShadow(fontRendererObj, max"x",
            //j+104, k+75, 0x646464);
            ...
            this.drawTexturedModelRectFromIcon(
                j + 80, k + 55, fl.getIcon(new FluidStack(fl, 1000)), 16, 16
            );
        }
    }
    ...
    protected float getNodeBaseYieldPerTick(NodeResource ri, ResourceNode te) {
        int[] minMax = ((ResourceFluid) ri)
                           .getBaseMinMax(
                               te.getPurity(),
                               te.worldObj.difficultySetting == EnumDifficulty.PEACEFUL
                           );
