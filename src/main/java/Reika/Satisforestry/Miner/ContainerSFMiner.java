...
public class ContainerSFMiner extends CoreContainer {
    ...
    public ContainerSFMiner(EntityPlayer player, TileResourceHarvesterBase te) {
        super(
            player,
            te,
            te instanceof TileNodeHarvester ? ((TileNodeHarvester) te).getOutput() : null
        );
        tile = te;
    ...
    public void updateProgressBar(int par1, int par2) {
        switch (par1) {
            case 0:
                tile.progressFactor = par2 / 1000F;
                break;
            case 1:
                tile.powerBar = par2 / 1000F;
                break;
        }
    }
    ...
    public ItemStack slotClick(int ID, int par2, int par3, EntityPlayer ep) {
        ...
        if (ID >= 0 && ID < inventorySlots.size() && !ep.worldObj.isRemote &&
    ((Slot)inventorySlots.get(ID)).inventory == clock) { clock.setOverclockLevels();
        }
        return is;
    }*/
    ...
    private static class OverclockSlot extends Slot {
        ...
        public boolean canTakeStack(EntityPlayer ep) {
            return slotIndex
                == reference.getLastOccupiedSlot(
                ); //slotNumber == 2 || reference.getItems()[slotNumber+1] == null;
        }
    }
}
