...
public abstract class TileFrackingPressurizer
    extends TileResourceHarvesterBase<TileFrackingNode, Boolean> {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (world.isRemote) {
            ...
            activeState
                = this.hasStructure() && this.getState() == MachineState.OPERATING;
        }
    }
    ...
    private void spawnVentParticles() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            EntityBlurFX fx = new EntityBlurFX(
                worldObj,
                px + xCoord + 0.5,
                py + yCoord + 0.5,
                pz + zCoord + 0.5,
                ...
                vy,
                vz,
                IconPrefabs.FADE_BASICBLEND.getIcon()
            );
            fx.setBasicBlend().setAlphaFading();
            TileFrackingNode tf = this.getResourceNode();
            if (tf != null)
                fx.setColor(ReikaColorAPI.mixColors(
                    tf.getResource().color, 0xffffff, rand.nextFloat()
                ));
            fx.setLife(ReikaRandomHelper.getRandomBetween(20, 80))
                .setScale((float) ReikaRandomHelper.getRandomBetween(5D, 12D));
    ...
    public class Thumper { //worth noting that SF fracker has a lot of random in the
                           //"wait" times
        ...
        private void setStage(ThumperStage put) {
            ...
            switch (stage) {
                case RISING: {
                    ...
                    if (Minecraft.getMinecraft().thePlayer.getDistanceSq(
                            xCoord, yCoord, zCoord
                        )
                        <= 2000)
                        ReikaSoundHelper.playClientSound(
                            SFSounds.FRACKHISS, TileFrackingPressurizer.this, v, p, false
                        );
                    break;
                }
                case PAUSE:
                ...
                case RECOVERING:
                    ...
                    if (Minecraft.getMinecraft().thePlayer.getDistanceSq(
                        ...
                        <= 2000)
                        ReikaSoundHelper.playClientSound(
                            SFSounds.FRACKTHUMP, TileFrackingPressurizer.this, v, p, false
                        );
        ...
        public double getRenderPosition() {
            return ReikaMathLibrary.linterpolate(
                ReikaRenderHelper.getPartialTickTime(), 0, 1, lastPosition, position
            );
        }
    }
    ...
    protected boolean isReady(TileFrackingNode te) {
        ...
        return rf.requiredInput == null
            || (rf.requiredInputAmount <= inputFluid.getLevel()
                && rf.requiredInput == inputFluid.getActualFluid());
    }
    ...
    protected final TilePowerConnection getWirePowerConnection() {
        return this.hasStructure() ? (TilePowerConnection
               ) worldObj.getTileEntity(xCoord, yCoord + 10, zCoord - 3)
                                   : null;
    }
    ...
    protected final TileShaftConnection getShaftPowerConnection() {
        return this.hasStructure()
            ? (TileShaftConnection) worldObj.getTileEntity(xCoord, yCoord + 8, zCoord + 2)
            : null;
    }
    ...
    public final int addFrackingFluid(FluidStack resource, boolean doFill) {
        return this.canAccept(resource.getFluid()) ? inputFluid.fill(resource, doFill)
                                                   : 0;
    }

    private static abstract class TileFrackingPressurizerBasicEnergy
        extends TileFrackingPressurizer {
    ...
    public static class TileFrackingPressurizerRF
        extends TileFrackingPressurizerBasicEnergy implements IEnergyReceiver {
    ...
    public static class TileFrackingPressurizerEU
        extends TileFrackingPressurizerBasicEnergy implements IEnergySink {
    ...
    public static class TileFrackingPressurizerRC
        extends TileFrackingPressurizer implements ShaftPowerReceiver {
