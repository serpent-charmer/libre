...
public class FrackerStructure {
    private static final BlockCheck AREA_CHECK = new BlockCheck() {
        ...
        public boolean matchInWorld(World world, int x, int y, int z) {
            ...
            if (ModList.BCTRANSPORT.isLoaded() && b == BCPipeHandler.getInstance().pipeID)
                return true;
            if (ModList.THERMALDYNAMICS.isLoaded()
                && ThermalDuctHandler.getInstance().isDuct(b))
                return true;
            if (ModList.ROTARYCRAFT.isLoaded() && this.isValidRCMachine(b))
                return true;
            if (world.getBlock(x, y - 1, z) == SFBlocks.FRACKERMULTI.getBlockInstance()
                && world.getBlockMetadata(x, y - 1, z) == FrackerBlocks.POWER.ordinal())
                return true;
            return false;
        }
    ...
    private static void setBlock(
        FilledBlockArray array, int x0, int y0, int z0, int dx, int dy, int dz, Block b
    ) {
...
return;
        ...
        if (b == Blocks.air) {
            if (dy == y0
                || (dy == y0 + 1 && Math.abs(dx - x0) > 1 && Math.abs(dz - z0) > 1)
                || (Math.abs(x0 - dx) < 2 && Math.abs(z0 - dz) < 2 && dy <= y0 + 9))
