...
public abstract class TileNodeHarvester
    extends TileResourceHarvesterBase<TileResourceNode, ForgeDirection> {
    ...
    public static enum SpoolingStates {
        ...
        public void playEntrySound(TileNodeHarvester te, SpoolingStates last) {
            switch (this) {
                ...
                case LOWER1:
                    if (last == LOCKING)
                        SFSounds.DRILLLOCK.playSoundNoAttenuation(
                            te.worldObj,
                            te.xCoord + 0.5,
                            te.yCoord + 0.5,
                            te.zCoord + 0.5,
                ...
                case SPINUP:
                    if (last == LOWER1)
                        SFSounds.DRILLSPINUP.playSoundNoAttenuation(
                            te.worldObj,
                            te.xCoord + 0.5,
                            te.yCoord + 0.5,
                            te.zCoord + 0.5,
                        ...
                        SFSounds.DRILLSPINDOWN.playSoundNoAttenuation(
                            te.worldObj,
                            te.xCoord + 0.5,
                            te.yCoord + 0.5,
                            te.zCoord + 0.5,
        ...
        if (world.isRemote) {
            ...
            if (spoolState.ordinal() > SpoolingStates.SPINUP.ordinal()) {
                if (runSoundTick > 0) {
                    ...
                    SFSounds.DRILLRUN.playSoundNoAttenuation(
                        worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, 1, 1, 64
                    );
                    runSoundTick = 148;
                }
            } else {
    ...
    private void spawnMiningFX(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            double[] xyz = ReikaPhysicsHelper.polarToCartesianFast(
                0.15, ReikaRandomHelper.getRandomBetween(45, 72), phi
            );
            EntityMinerFX fx
                = new EntityMinerFX(world, px, py, pz, xyz[0], xyz[1] + 0.04, xyz[2], te);
    ...
    public boolean isShutdown(World world, int x, int y, int z) {
        ...
        if (te != null && te.hasRedstone())
            return true;
        if (te != null
            && world.isBlockIndirectlyGettingPowered(te.xCoord, te.yCoord - 1, te.zCoord))
            return true;
        return false;
    }
    ...
    @SideOnly(Side.CLIENT)
    private double
    getDrillVerticalOffsetScale(double phase1L, double phase2L, boolean rev) {
        if (!this.isInWorld())
            return 0;
        switch (spoolState) {
            case IDLE:
            case LOCKING:
                return 0;
            case LOWER1:
                return phase1L * spoolTime
                    / (rev ? spoolState.durationReverse : spoolState.duration);
            case SPINUP:
                return phase1L;
            case LOWER2:
                return phase1L
    ...
    private double getDrillSpeed(double max) {
        if (spoolState.ordinal() < SpoolingStates.SPINUP.ordinal()) {
            ...
            throw new IllegalStateException(
                "Spoolstate was neither less, equal, or greater than spinup?!"
            );
        }
    }
    ...
    public double getDrillAngle(float ptick) {
        return ReikaMathLibrary.linterpolate(
            ptick, 0, 1, lastDrillSpinAngle, drillSpinAngle
        );
    }
    ...
    private boolean trySpawnItem(ItemStack is) {
        ...
        if (te != null) {
            ...
            if (has != null) {
                fit = ReikaItemHelper.areStacksCombinable(is, has, is.getMaxStackSize())
                    ? Math.min(has.getMaxStackSize() - has.stackSize, is.stackSize)
                    : 0;
            }
            if (fit > 0)
                return ReikaInventoryHelper.addToIInv(
                    ReikaItemHelper.getSizedItemStack(is, fit), te, true
                );
        }
        return false;
    }
    ...
    protected final TilePowerConnection getWirePowerConnection() {
        ...
        return (TilePowerConnection) worldObj.getTileEntity(
            xCoord + right.offsetX * 2, yCoord + 13, zCoord + right.offsetZ * 2
        );
    }
    ...
    protected final TileShaftConnection getShaftPowerConnection() {
        return this.hasStructure()
            ? (TileShaftConnection) worldObj.getTileEntity(
                xCoord - structureDir.offsetX, yCoord + 12, zCoord - structureDir.offsetZ
            )
            : null;
    }

    public final TileMinerConveyorPort getOutput() {
        return this.hasStructure() ? (TileMinerConveyorPort) worldObj.getTileEntity(
                   xCoord + structureDir.offsetX * 8,
                   yCoord + 1,
                   zCoord + structureDir.offsetZ * 8
               )
                                   : null;
    }
    ...
    protected void updateStructureState(ForgeDirection flag) {
        boolean change = (structureDir == null && flag != null)
            || (structureDir != null && flag == null);
        if (change)
            MinerStructure.toggleRSLamps(
                this, flag != null ? flag : structureDir, flag == null
            );
        structureDir = flag;
    }
    ...
    public void addWaila(List<String> tip) {
        tip.add(
            tip.indexOf(this.getState().getDisplayName()) + 1, spoolState.getDisplayName()
        );
        super.addWaila(tip);
    }
    ...
    private static abstract class TileNodeHarvesterBasicEnergy extends TileNodeHarvester {
        ...
        protected TileNodeHarvesterBasicEnergy(long c, long m, long f, long s) {
            ...
            maxFlowRate = f;
            standbyCost = s;
            if (maxFlowRate * Purity.PURE.getCountdown()
                < energyPerCycle * this.getSpeedFactor())
                throw new RegistrationException(
                    Satisforestry.instance,
                    this + " max energy flow rate (" + maxFlowRate + "/" + energyPerCycle
                        + ") too low to sustain continuous operation!"
                );
        }
        ...
        protected final long getActualMaxFlow() {
            return (long
            ) (maxFlowRate * (1 + this.getTier()) * this.getOverclockingPowerFactor(false)
            );
        }
        ...
        public final ArrayList getMessages(World world, int x, int y, int z, int side) {
            ...
            li.add(
                "Contains " + energy + " of " + maxEnergy + " " + this.getEnergyUnit()
            );
            return li;
        }
        ...
        protected final long getActualOperationCost(boolean operation) {
            return (long
            ) ((operation ? energyPerCycle : standbyCost)
               * this.getOverclockingPowerFactor(false));
        }

        protected final float getOverclockingPowerFactor(boolean display) {
            return (float) ReikaMathLibrary.roundToNearestFraction(
                Math.pow(this.getOverclockingLevel(display), 1.82), 0.25
            ); //SF uses 1.6
        }
        ...
        public void addWaila(List<String> tip) {
            ...
            tip.add(String.format(
                "Requires %d %s per cycle",
                (int) (energyPerCycle * this.getOverclockingPowerFactor(true)),
                this.getPowerType()
            ));
        }
    }

    public static class TileNodeHarvesterRF
        extends TileNodeHarvesterBasicEnergy implements IEnergyReceiver {
        ...
        public ItemStack getOverclockingItem() {
            ...
            ); //Overclocked Modular Gearbox = T2 speed booster
        }
    }

    @Strippable(value = { "ic2.api.energy.tile.IEnergySink" })
    public static class TileNodeHarvesterEU
        extends TileNodeHarvesterBasicEnergy implements IEnergySink {
        ...
        @ModDependent(ModList.IC2)
        public double
        injectEnergy(ForgeDirection directionFrom, double amount, double voltage) {
    ...
    public static class TileNodeHarvesterRC
        extends TileNodeHarvester implements ShaftPowerReceiver {
        ...
        public void updateEntity(World world, int x, int y, int z, int meta) {
            ...
            if (!PowerTransferHelper.checkPowerFrom(te, ForgeDirection.DOWN)
                && !PowerTransferHelper.checkPowerFrom(te, ForgeDirection.UP)) {
                this.noInputMachine();
            }
            super.updateEntity(world, x, y, z, meta);
        }
        ...
        protected boolean hasEnergy(boolean operation) {
            return operation ? power >= this.getMinPowerCost(false) && torque >= MINTORQUE
                    && omega >= this.getMinSpeed()
                             : (power >= this.getStandbyPowerCost());
        }
        ...
        public float getOperationEnergyFraction() {
            return ReikaMathLibrary.multiMin(
                1F,
                power / (float) this.getMinPowerCost(false),
                torque / (float) MINTORQUE,
                omega / (float) this.getMinSpeed()
            );
        }
        ...
        public ItemStack getOverclockingItem() {
            return new ItemStack(
                ItemFetcher.getItemByOrdinal(55), 1, 2
            ); //t2 magneto upgrade
        }
    }
