...
public final class SpitterDamage extends CustomStringDamageSource {
    private static final SpitterDamage spitterBlast
        = (SpitterDamage) new SpitterDamage().setExplosion();
    private static final SpitterDamage spitterBall
        = (SpitterDamage) new SpitterDamage().setFireDamage().setProjectile();
    private static final SpitterDamage spitterBallFallback
        = (SpitterDamage) new SpitterDamage().setProjectile();
    ...
    protected String getMessage() {
        return spitter == null || spitter.getSpitterType() == null
            ? "[ERROR]"
            : super.getMessage() + spitter.getSpitterType().getName();
    }
    ...
    public static void
    doDamage(EntitySpitter es, EntitySpitterFireball f, EntityLivingBase tgt, float amt) {
        boolean resist
            = tgt.isImmuneToFire() || tgt.isPotionActive(Potion.fireResistance);
        boolean spitter = tgt instanceof EntitySpitter;
        SpitterDamage dmg
            = f != null ? (resist ? spitterBallFallback : spitterBall) : spitterBlast;
        ...
        if (spitter && ((EntitySpitter) tgt).getSpitterType().isAlpha()
            && !es.getSpitterType().isAlpha())
            amt *= 0.75;
        if (es.isPotionActive(Potion.damageBoost)) {
            amt *= 1
                + 0.333
                    * (es.getActivePotionEffect(Potion.damageBoost).getAmplifier() + 1);
        }
        if (es.riddenByEntity instanceof EntityPlayer) {
            ...
            if (ep.isPotionActive(Potion.damageBoost)) {
                amt *= 1
                    + 0.1667
                        * (ep.getActivePotionEffect(Potion.damageBoost).getAmplifier() + 1
                        );
            }
