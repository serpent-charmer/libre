...
public class ChromaClientEventController
    implements ProfileEventWatcher, ChunkWorldRenderWatcher, AdvancedBlockRenderWatcher,
               EntityRenderWatcher, TileRenderWatcher {
    public static final ChromaClientEventController instance
        = new ChromaClientEventController();
    ...
    public void loadTreeOverlays(IIconRegister ico) {
        for (int i = 0; i < rainbowForestTreeGlowOverlays.length; i++) {
            rainbowForestTreeGlowOverlays[i]
                = ico.registerIcon("chromaticraft:dye/tree_overlay/" + i);
        }
    }
    ...
    public void overenchantedTools(ItemStackUpdateEvent evt) {
        if (evt.held && evt.holder == Minecraft.getMinecraft().thePlayer
            && evt.item.stackTagCompound != null
            && evt.item.stackTagCompound.getBoolean(ChromaEnchants.OVERENCHANT_TAG)) {
            if (Minecraft.getMinecraft().gameSettings.thirdPersonView == 0) {
                Vec3 vec = evt.holder.getLookVec();
                Vec3 vec2 = ReikaVectorHelper.rotateVector(vec, 0, 53, 0);
                double d = 0.3;
                double px = ReikaRandomHelper.getRandomPlusMinus(
                    evt.holder.posX + vec2.xCoord * d, 0.03125
                );
                double py = ReikaRandomHelper.getRandomPlusMinus(
                    evt.holder.posY, evt.holder.height / 4
                );
                double pz = ReikaRandomHelper.getRandomPlusMinus(
                    evt.holder.posZ + vec2.zCoord * d, 0.03125
                );
                EntityCCBlurFX fx = new EntityCCBlurFX(evt.holder.worldObj, px, py, pz);
                fx.setIcon(ChromaIcons.HEXFLARE)
                    .setColor(0x00A7A7)
                    .setLife(10)
                    .setScale(0.25F)
                    .setRapidExpand();
    ...
    public void particleProgramTooltips(ItemTooltipEvent evt) {
        if (this.isParticleProgramBook(evt.itemStack)) {
            ...
            //evt.toolTip.addAll(BlockBounds.readFromNBT("particleprogram",
            //evt.itemStack.stackTagCompound).toClearString());
        }
    }

    private boolean isParticleProgramBook(ItemStack is) {
        return is != null && is.getItem() == Items.book && is.stackTagCompound != null
            && is.stackTagCompound.hasKey("particleprogram");
    }
            ...
            if (ep != null) {
                ...
                if (f >= 0.25) {
                    ...
                    if (r < 1 || rand.nextInt(r) == 0) {
                        ...
                        if (ep.worldObj.getSavedLightValue(EnumSkyBlock.Sky, x, y, z)
                            > 5) {
                            double px
                                = ReikaRandomHelper.getRandomPlusMinus(ep.posX, 3.5);
                            double py
                                = ReikaRandomHelper.getRandomPlusMinus(ep.posY + 1.5, 1);
                            double pz
                                = ReikaRandomHelper.getRandomPlusMinus(ep.posZ, 3.5);
                            float g = (float
                            ) ReikaRandomHelper.getRandomBetween(0.03125, 0.125);
                            int l = ReikaRandomHelper.getRandomBetween(10, 50);
                            float s
                                = (float) ReikaRandomHelper.getRandomBetween(0.25, 0.75);
                            double vm = 0.03125;
                            double vx = ReikaRandomHelper.getRandomPlusMinus(0, vm);
                            double vz = ReikaRandomHelper.getRandomPlusMinus(0, vm);
                            double ax
                                = ReikaRandomHelper.getRandomPlusMinus(0, 0.0625 / 8)
                                * ReikaRandomHelper.getRandomBetween(0.25, 1);
                            double az
                                = ReikaRandomHelper.getRandomPlusMinus(0, 0.0625 / 8)
                                * ReikaRandomHelper.getRandomBetween(0.25, 1);
                            EntityShaderFX fx = new EntityShaderFX(
                                ep.worldObj,
                                px,
                                py,
                                pz,
                                ...
                                vz,
                                1,
                                ChromaShaders.CRYSTALLIZEPARTICLE
                            );
                            int c0
                                = ReikaJavaLibrary.getRandomListEntry(rand, snowColors);
                            int c = ReikaColorAPI.mixColors(
                                c0,
                                0xffffff,
                                (float) ReikaRandomHelper.getRandomBetween(0, 0.25)
                            );
                            fx.setRendering(true).setClip(0.5F).setAcceleration(
                                ax, 0, az
                            );
    ...
    public void adjustLightMap(LightmapEvent evt) {
        if (Minecraft.getMinecraft().theWorld.provider.dimensionId
    ...
    public boolean onBlockTriedRender(
        Block b,
        int xCoord,
        int yCoord,
        int zCoord,
        ...
        if (ChromaticEventManager.instance.isCrystallineRedstoneTorch(
                access, xCoord, yCoord, zCoord, b, meta
            )) {
            Tessellator.instance.setBrightness(
                b.getMixedBrightnessForBlock(access, xCoord, yCoord, zCoord)
            );
            Tessellator.instance.setColorOpaque_I(0xffffff);
            RenderBlockAtPosEvent.continueRendering = true;
            ReikaRenderHelper.renderTorch(
                access,
                xCoord,
                yCoord,
                zCoord,
                BlockPylonStructure.getIconOverride(b),
        ...
        //ReikaJavaLibrary.pConsole("CLIENT RENDER NOCLIP
        //"+AbilityHelper.instance.isNoClipEnabled);
        if (AbilityHelper.instance.isLOSViewEnabled()) {
            if (b != Blocks.air && renderPass == 0 && b.canRenderInPass(0)) {
                boolean flag = render.enableAO;
                render.enableAO = false;
                World world = Minecraft.getMinecraft().theWorld;
                boolean passable = AbilityHelper.instance.losViewMode == 1
                    ? PylonFinder.isBlockPassable(world, xCoord, yCoord, zCoord)
                    : RelayNetworker.instance.isRelayPassable(
                        world, xCoord, yCoord, zCoord
                    );
                int c = passable
                    ? 0x77ff77
                    : 0xff7777; //b.colorMultiplier(access, xCoord, yCoord, zCoord);
                ...
                for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
                    ...
                    if (b.shouldSideBeRendered(access, dx, dy, dz, dir.ordinal())) {
                        ...
                        ); //render.getIconSafe(b.getIcon(access, xCoord, yCoord, zCoord,
                           //dir.ordinal()));
                        //Tessellator.instance.setBrightness(b.getMixedBrightnessForBlock(access,
                        //xCoord, yCoord+(b.isOpaqueCube() ? 1 : 0), zCoord));
                        ...
                        switch (dir) {
                            ...
                            case UP:
                                Tessellator.instance.setColorOpaque_I(
                                    ReikaColorAPI.getColorWithBrightnessMultiplier(c, 1)
                                );
                                Tessellator.instance.addVertexWithUV(
                                    xCoord - d,
                                    yCoord + 1 + d,
            ...
            //ChromatiCraft.logger.debug("Checking render of "+b+":"+meta+" @
            //"+xCoord+"."+yCoord+","+zCoord);
            if (!AbilityHelper.instance.isBlockOreclippable(
                    Minecraft.getMinecraft().theWorld, xCoord, yCoord, zCoord, b, meta
                )) {
                return true;
            } else {
                //render.setRenderAllFaces(true);
                //ChromatiCraft.logger.debug("Checking render pass of "+b+":"+meta+" @
                //"+xCoord+"."+yCoord+","+zCoord+" for pass "+renderPass+":
                //"+b.canRenderInPass(renderPass));
                if (b.canRenderInPass(renderPass)) {
                    int type = b.getRenderType();
                    if (type == 0 || type == ChromaISBRH.ore.getRenderID()
                        || ReikaBlockHelper.isOre(b, meta) || b.renderAsNormalBlock()
                        || b.isOpaqueCube() || ReikaBlockHelper.isLiquid(b)) {
                        //ChromatiCraft.logger.debug("Rendering "+b+":"+meta+" @
                        //"+xCoord+"."+yCoord+","+zCoord);
                        render.enableAO = false;
                        Tessellator.instance.setBrightness(240);
                        ...
                        for (int i = 0; i < 6; i++) {
                            ...
                            Tessellator.instance.setBrightness(240);
                            IIcon ico = render.getIconSafe(
                                b.getIcon(access, xCoord, yCoord, zCoord, i)
                            );
                            int dx = xCoord + dir.offsetX;
                            int dy = yCoord + dir.offsetY;
                            int dz = zCoord + dir.offsetZ;
                            boolean side = ReikaBlockHelper.isLiquid(b)
                                ? access.getBlock(dx, dy, dz) != b
                                : b.shouldSideBeRendered(access, dx, dy, dz, i);
                            double o = side ? 0.001 : 0;
                            if ((side || (dir == ForgeDirection.UP && yCoord == 0))
                                || b != Blocks.bedrock) {
                                ...
                                switch (dir) {
                                    case DOWN:
                                        ...
                                        render.renderFaceYNeg(
                                            b, xCoord, yCoord - o, zCoord, ico
                                        );
                                        break;
                                    case UP:
                                        Tessellator.instance.setColorOpaque_F(1, 1, 1);
                                        render.renderFaceYPos(
                                            b, xCoord, yCoord + o, zCoord, ico
                                        );
                                        break;
                                    case WEST:
                                        ...
                                        render.renderFaceXNeg(
                                            b, xCoord - o, yCoord, zCoord, ico
                                        );
                                        break;
                                    case EAST:
                                        Tessellator.instance.setColorOpaque_F(
                                    ...
                                    case NORTH:
                                        ...
                                        render.renderFaceZNeg(
                                            b, xCoord, yCoord, zCoord - o, ico
                                        );
                                        break;
                                    case SOUTH:
                                        Tessellator.instance.setColorOpaque_F(
            ...
            if (tree != null) {
                ...
                while (tree == tree2 && y < 255) {
                    y++;
                    tree2 = this.getTreeType(
                        access.getBlock(xCoord, y, zCoord),
                        access.getBlockMetadata(xCoord, y, zCoord)
                    );
                }
                if (access.getBlock(xCoord, y, zCoord)
                    == ChromaBlocks.DECAY.getBlockInstance()) {
                    render.enableAO = false;
                    Tessellator.instance.setBrightness(240);
                    Tessellator.instance.setColorRGBA_I(
                        ReikaDyeHelper.dyes[access.getBlockMetadata(xCoord, y, zCoord)]
                            .color,
                        255
                    );
                    IIcon ico = render.getIconSafe(RainbowForestGenerator.getWoodGlow(
                        xCoord, yCoord, zCoord, rainbowForestTreeGlowOverlays
                    ));
                    for (int i = 2; i < 6; i++) {
                        ForgeDirection dir = ForgeDirection.VALID_DIRECTIONS[i];
                        int dx = xCoord + dir.offsetX;
    ...
    public void addLumenBurnButton(GuiScreenEvent.InitGuiEvent.Post evt) {
        EntityPlayer ep = Minecraft.getMinecraft().thePlayer;
        if (evt.gui instanceof GuiInventory
            && ChromaResearchManager.instance.playerHasFragment(
                ep, ChromaResearch.ITEMBURNER
            )) {
            GuiContainer gui = (GuiContainer) evt.gui;
            evt.buttonList.add(new ButtonItemBurner(
                gui,
                gui.guiLeft + (ep.getActivePotionEffects().isEmpty() ? 142 : 82),
                gui.guiTop + 56
            ));
        }
    }
    ...
    public void handleLumenBurnButton(GuiScreenEvent.ActionPerformedEvent.Post evt) {
        if (evt.button.id == ButtonItemBurner.BUTTON_ID
            && evt.gui instanceof GuiInventory) {
            ReikaPacketHelper.sendDataPacket(
                ChromatiCraft.packetChannel,
                ChromaPackets.BURNERINV.ordinal(),
                PacketTarget.server,
                1
            );
        }
    ...
    public void floatBootParticlesForOthers(PlayerTickEvent evt) {
        if (evt.side == Side.CLIENT && evt.phase == Phase.START
            && evt.player instanceof EntityOtherPlayerMP) { //other players only
    ...
    public void textureHook(TextureStitchEvent.Pre event) {
        if (event.map.getTextureType() == 0) {
            biomeWaterIconFlow
                = event.map.registerIcon("chromaticraft:fluid/glowcliffwater_flow");
            biomeWaterIcon
                = event.map.registerIcon("chromaticraft:fluid/glowcliffwater_still");
        }
    }

    @SubscribeEvent
    public void retextureGlowCliffWater(BlockIconEvent evt) {
        if (evt.access != null && BiomeGlowingCliffs.isGlowingCliffs(evt.getBiome())
            && BiomeGlowingCliffs.isClearWater(
                evt.access, evt.xCoord, evt.yCoord, evt.zCoord
            )) {
            if (evt.originalIcon == FluidRegistry.WATER.getFlowingIcon())
    ...
    public void endFog(FogDistanceEvent evt) {
        evt.fogDistance = Math.min(
            evt.fogDistance, EndOverhaulManager.instance.rampFog(evt.fogDistance)
        );
    }

    @SubscribeEvent
    public void endFog(EntityViewRenderEvent.FogColors evt) {
        int c0 = ReikaColorAPI.RGBtoHex(
            (int) (evt.red * 255), (int) (evt.green * 255), (int) (evt.blue * 255)
        );

        int c = ReikaColorAPI.mixColors(
            0xA2A59B, c0, EndOverhaulManager.instance.getRenderFactor()
        );
    ...
    public void dynamicFog(FarClippingPlaneEvent evt) {
        evt.farClippingPlaneDistance = Math.min(
            evt.farClippingPlaneDistance,
            EndOverhaulManager.instance.rampFog(evt.farClippingPlaneDistance)
        );
    }
    ...
    public void preventCliffWeatherSky(WeatherSkyStrengthEvent evt) {
        ...
        if (ep != null) {
            ...
            if
        (BiomeGlowingCliffs.isGlowingCliffs(ep.worldObj.getBiomeGenForCoords(MathHelper.floor_double(ep.posX),
        MathHelper.floor_double(ep.posZ)))) {
         */
        evt.returnValue
            *= 1 - BiomeGlowingCliffs.renderFactor; /*
}
}
*/
    }
    ...
    public void preventCliffWeatherSky(FogColors evt) {
        ...
        if (Minecraft.getMinecraft().thePlayer.isInsideOfMaterial(Material.lava)
            || Minecraft.getMinecraft().thePlayer.isInsideOfMaterial(Material.water))
            return;
        float f = BiomeGlowingCliffs.renderFactor
            * (float) ReikaMathLibrary.normalizeToBounds(
                ReikaWorldHelper.getSunIntensity(
                    Minecraft.getMinecraft().theWorld,
                    false,
                    (float) evt.renderPartialTicks
                ),
                0,
                1,
                0.2,
    ...
    public void preventCliffThunder(PlaySoundEvent17 evt) {
        EntityPlayer ep = Minecraft.getMinecraft().thePlayer;
        if (ep != null
            && BiomeGlowingCliffs.isGlowingCliffs(ep.worldObj.getBiomeGenForCoords(
                MathHelper.floor_double(ep.posX), MathHelper.floor_double(ep.posZ)
            ))) {
    ...
    public void preventCliffAmbience(PlaySoundEvent17 evt) {
        ...
        if (Minecraft.getMinecraft().theWorld != null
            && BiomeGlowingCliffs.isGlowingCliffs(
                Minecraft.getMinecraft().theWorld.getBiomeGenForCoords(
                    MathHelper.floor_float(evt.sound.getXPosF()),
                    MathHelper.floor_float(evt.sound.getZPosF())
                )
            )) {
    ...
    public void noLumaWaterFog(RenderBlockOverlayEvent evt) {
        ...
        if (evt.overlayType == OverlayType.WATER
            && (evt.player.worldObj.getBlock(evt.blockX, evt.blockY, evt.blockZ)
                    == ChromaBlocks.LUMA.getBlockInstance()
                || evt.player.worldObj.getBlock(evt.blockX, evt.blockY + 1, evt.blockZ)
                    == ChromaBlocks.LUMA.getBlockInstance())) {
    ...
    public void glowingCliffsFog(EntityViewRenderEvent.FogColors evt) {
        //ReikaJavaLibrary.pConsole(evt.density);
        Entity e = evt.entity;
        BiomeGenBase b = e.worldObj.getBiomeGenForCoords(MathHelper.floor_double(e.posX),
    MathHelper.floor_double(e.posZ)); if (BiomeGlowingCliffs.isGlowingCliffs(b)) evt.red =
    evt.green = evt.blue = 1;
    }
    ...
    public void glowingCliffsFog(FogDistanceEvent evt) {
        ...
        Entity e = Minecraft.getMinecraft().thePlayer;
        BiomeGenBase b = e.worldObj.getBiomeGenForCoords(MathHelper.floor_double(e.posX),
    MathHelper.floor_double(e.posZ)); float mind = 10; if
    (BiomeGlowingCliffs.isGlowingCliffs(b)) { float f =
    ChromatiCraft.glowingcliffs.getFogDensity(e.posX, e.posY, e.posZ); if (f > 0)
    ...
    public void biomeWaterColor(WaterColorEvent evt) {
        ...
        if (BiomeGlowingCliffs.isGlowingCliffs(b)) {
            evt.color = ChromatiCraft.glowingcliffs.getWaterColor(
                evt.access, evt.xCoord, evt.yCoord, evt.zCoord, evt.getLightLevel()
            );
        } else if (b instanceof BiomeRainbowForest) {
            evt.color = ((BiomeRainbowForest) b)
                            .getWaterColor(
                                evt.access,
                                evt.xCoord,
                                evt.yCoord,
    ...
    public void cancelBeeGlintForBookRender(ItemEffectRenderEvent evt) {
        if (Minecraft.getMinecraft().currentScreen instanceof GuiFragmentSelect
            || Minecraft.getMinecraft().currentScreen instanceof ChromaBookGui)
    ...
    public void createCaveParticles(ClientTickEvent evt) {
        EntityPlayer ep = Minecraft.getMinecraft().thePlayer;
        if (!Minecraft.getMinecraft().isGamePaused() && ep != null
            && VoidGazeLevels.CAVEPARTICLES.isActiveOnPlayer(ep)) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (ep.worldObj.getBlock(mdx, mdy, mdz).isAir(ep.worldObj, mdx, mdy, mdz)
                    && ep.worldObj.getSavedLightValue(EnumSkyBlock.Sky, mdx, mdy, mdz)
                        == 0) {
                    EntityFX fx = new EntityCCBlurFX(ep.worldObj, dx, dy, dz)
                                      .setIcon(ChromaIcons.FLARE)
                                      .setLife(8)
                                      .setAlphaFading()
                                      .setScale(1.5F);
                    Minecraft.getMinecraft().effectRenderer.addEffect(fx);
    ...
    evt.newLogin) { AbilityHelper.instance.onNoClipEnable();
        }
    }
    ...
    public void applyAntFarmLighting(NightVisionBrightnessEvent evt) {
        ...
        if (p.getAmplifier() == 3) {
            ...
            float t = p.getDuration()
                - evt.partialTickTime; //fade-evt.player.ticksExisted+evt.partialTickTime;
    ...
    public void applyOreXRay(GetMouseoverEvent evt) {
        ...
        if (AbilityHelper.instance.isNoClipEnabled) {
            Vec3 vec = Vec3.createVectorHelper(
                ep.posX, (ep.posY + 1.62) - ep.yOffset, ep.posZ
            );
            Vec3 vec2 = ep.getLook(1.0F);
            double reach
                = Minecraft.getMinecraft().playerController.getBlockReachDistance();
            Vec3 vec3 = vec.addVector(
                vec2.xCoord * reach, vec2.yCoord * reach, vec2.zCoord * reach
            );
            MovingObjectPosition hit = AbilityHelper.instance.doOreClipRayTrace(
                Minecraft.getMinecraft().theWorld, vec, vec3, false
            );
    ...
    { long dur = 0; while (!textureLoadingComplete) { Thread.sleep(100); dur += 100; if
    (dur%5000 == 0) ChromatiCraft.logger.log("Waiting for texture loading to
    complete...");
        }
    }
     */
    ...
    public void ensureMusic(SoundVolumeEvent evt) {
        if (evt.sound instanceof CustomMusic) {
            ...
            if (cm.path.toLowerCase(Locale.ENGLISH).contains("chromaticraft") &&
    cm.path.contains(MusicLoader.instance.musicPath)) { evt.volume = Math.max(0.05F,
    Minecraft.getMinecraft().gameSettings.getSoundLevel(ChromaClient.chromaCategory));
            }
        }
    }
     */
                ...
                if (c.hasWiderPitchRange()) {
                    if (es.getPitch() > 2) {
                        if (c.getUpshiftedPitch() != null)
                            evt.result = new EnumSound(
                                c.getUpshiftedPitch(),
                                es.posX,
                                es.posY,
                                es.posZ,
                                es.volume,
                                es.pitch / c.getRangeInterval(),
                                es.attenuate
                            );
                    } else if (es.getPitch() < 0.5) {
                        if (c.getDownshiftedPitch() != null)
                            evt.result = new EnumSound(
                                c.getDownshiftedPitch(),
                                es.posX,
                                es.posY,
                                es.posZ,
                                es.volume,
    ...
    public void hideStructures(InventoryTrackEvent evt) {
        if (evt.getTileEntity() instanceof TileEntityLootChest) {
            ...
            evt.setCanceled(true);
        } else if (ReikaInventoryHelper.checkForItem(
                       ChromaItems.FRAGMENT.getItemInstance(), evt.getInventory()
                   )) {
            evt.setCanceled(true);
        }
    }
    ...
    public void customCreativeTab(CreativeTabGuiRenderEvent evt) {
        if (evt.tab instanceof TabChromatiCraft || evt.tab instanceof FragmentTab) {
            ...
            String s = evt.tab.hasSearchBar() ? "Textures/GUIs/creativetab_search.png"
                                              : "Textures/GUIs/creativetab.png";
            ReikaTextureHelper.bindTexture(ChromatiCraft.class, s);
            evt.gui.drawTexturedModalRect(
                evt.gui.guiLeft, evt.gui.guiTop, 0, 0, evt.guiXSize, evt.guiYSize
            );
            BlendMode.DEFAULT.apply();
            evt.gui.drawTexturedModalRect(
                evt.gui.guiLeft, evt.gui.guiTop, 0, 0, evt.guiXSize, evt.guiYSize
            );
            ChromaFontRenderer.FontType.GUI.drawString(
                evt.tab.getTabLabel(), evt.gui.guiLeft + 4, evt.gui.guiTop + 5, 0xffffff
            );
    ...
    @SideOnly(Side.CLIENT)
    public boolean tryRenderEntity(
        Render r,
        Entity e,
        double renderPosX,
        double renderPosY,
        ...
        if (e.worldObj != null && e instanceof EntityPlayer) {
            ...
            if (Chromabilities.CHESTCLEAR.enabledOn(ep)) {
                AbilityHelper.instance.renderChestCollectionFX(
                    ep, ReikaRenderHelper.getPartialTickTime()
                );
            }
        }
        EntityPlayer ep = Minecraft.getMinecraft().thePlayer;
        if (e.worldObj != null && Chromabilities.MOBSEEK.enabledOn(ep)) {
            if (AbilityHelper.instance.canRenderEntityXRay(e)) {
                ...
                RenderManager.instance.getEntityRenderObject(e).doRender(
                    e, 0, 0, 0, par8, 0
                );
                ...
                BlendMode.DEFAULT.apply();
                int a = (int
                ) (96 + 64 * Math.sin(System.currentTimeMillis() / 400D + e.getEntityId())
                );

                if (ReikaEntityHelper.isInWorld(e)) {
                    if (visualLOS == null) //lazyload
                        visualLOS = RayTracer.getVisualLOS();
                    visualLOS.setOrigins(
                        ep.posX, ep.posY, ep.posZ, e.posX, e.posY + e.height / 2, e.posZ
                    );
                    int c = visualLOS.isClearLineOfSight(ep.worldObj)
                        ? 0x00ff00
                        : 0xff0000; //ReikaEntityHelper.mobToColor((EntityLivingBase)evt.entity);

                    AxisAlignedBB box = e.boundingBox.copy().offset(
                        -e.posX, -e.posY, -e.posZ
                    ); //AxisAlignedBB.getBoundingBox(0, 0, 0, 1, 1, 1).expand(te.width/4,
                       //te.height/4, te.width/4).offset(-te.width/2, -te.height/2,
                       //-te.width/2);
                    box.maxY = Math.min(box.maxY, box.minY + e.height);
                    if (e instanceof EntityGlowCloud) {
                        box = AxisAlignedBB.getBoundingBox(0, 0, 0, 0, 0, 0)
            ...
            if (tx != null) {
                ...
                AxisAlignedBB box
                    = AxisAlignedBB.getBoundingBox(0, 0, 0, 1, 1, 1)
                          .expand(e.width / 4, e.height / 4, e.width / 4)
                          .offset(-e.width / 2, -e.height / 2, -e.width / 2);
    ...
    public void postTileRender(
        ...
        if (te.worldObj != null
            && Chromabilities.SPAWNERSEE.enabledOn(Minecraft.getMinecraft().thePlayer)) {
            ...
            if (tx != null) {
                ...
                TileEntityRendererDispatcher.instance.getSpecialRenderer(te)
                    .renderTileEntityAt(te, 0, 0, 0, par8);
    ...
    public void addCustomThaumGraphics(DrawScreenEvent.Post evt) {
        if (evt.gui != null
            && evt.gui.getClass().getSimpleName().equals("GuiResearchRecipe")) {
            try {
                ...
                if (item.key.equals("WARPPROOF")) { //Aura cleaner scribbles
                    ReikaTextureHelper.bindFinalTexture(
                        ChromatiCraft.class, "Textures/eldritch_s.png"
                    );
                    //Tessellator v5 = Tessellator.instance;
                    //v5.startDrawingQuads();
                    int x2 = j - 20;
                    ...
                    ResourceLocation loc = new ResourceLocation(
                        "thaumcraft:textures/misc/eldritchajor2.png"
                    ); //yes, eldritchajor
                    ...
                    loc = new ResourceLocation(
                        "thaumcraft:textures/misc/eldritchajor1.png"
                    );
                    Minecraft.getMinecraft().renderEngine.bindTexture(loc);
                    ...
                    //v5.draw();
                } else if (item.key.endsWith("PYLONWANDING") && page == 2) { //"Very
                                                                             //technical
                                                                             //diagram"
                    ReikaTextureHelper.bindFinalTexture(
                        ChromatiCraft.class, "Textures/pylon_wanding.png"
                    );
    ...
    public void interceptNEI(NEIRecipeCheckEvent evt) {
        if (!Keyboard.isKeyDown(Keyboard.KEY_LMENU)
            && this.loadLexiconRecipe(evt.gui, evt.getItem()))
            evt.setCanceled(true);
    }
    ...
    private boolean loadLexiconRecipe(GuiContainer gui, ItemStack is) {
        if (is != null && is.getItem() != null) {
            ...
            if (is.stackTagCompound != null && is.stackTagCompound.getBoolean("boosted")
                && r.getMachine() != null && r.getMachine().isRepeater()) {
                r = ChromaResearch.TURBOREPEATER;
            }
            if (r != null && r.playerCanSee(ep) && r.isCrafting() && r.isCraftable()
                && !r.isVanillaRecipe() && r.playerCanSeeRecipe(is, ep) && r.crafts(is)) {
                ep.openGui(
                    ChromatiCraft.instance,
                    r.getCraftingType().ordinal(),
                    null,
                    r.ordinal(),
                    r.getRecipeIndex(is, ep),
                    1
                );
                return true;
            } else if (ReikaItemHelper.collectionContainsItemStack(
                           ChromaResearch.APIRECIPES.getItemStacks(), is
                       )) {
                ep.openGui(
                    ChromatiCraft.instance,
                    ChromaResearch.APIRECIPES.getCraftingType().ordinal(),
                    null,
                    ChromaResearch.APIRECIPES.ordinal(),
                    ChromaResearch.APIRECIPES.getRecipeIndex(is, ep),
                    1
                );
                return true;
            }
    ...
    public void openAbilityGui(KeyPressEvent evt) {
        if (ChromaOptions.KEYBINDABILITY.getState() && evt.key == ChromaClient.key_ability
            && !evt.lastPressed) {
            Minecraft mc = Minecraft.getMinecraft();
            mc.thePlayer.openGui(
                ChromatiCraft.instance, ChromaGuis.ABILITY.ordinal(), mc.theWorld, 0, 0, 0
            );
        }
    }
    ...
    public void heldMobFirstPerson(RenderFirstPersonItemEvent evt) {
        ...
        if (ChromaItems.CAPTURE.matchWith(ep.getCurrentEquippedItem())) {
            EntityLiving e
                = ItemCaptureWand.getMob(ep.getCurrentEquippedItem(), ep.worldObj);
            if (e != null) {
                GL11.glPushMatrix();
                ...
                if (r == null)
                    ; //ChromatiCraft.logger.logError("Cannot render "+e+", has no
                      //renderer!");
                else
    ...
    public void drawBuilderFrame(DrawBlockHighlightEvent evt) {
        if (evt.target != null && evt.target.typeOfHit == MovingObjectType.BLOCK) {
            if (evt.currentItem != null
                && ChromaItems.BUILDER.matchWith(evt.currentItem)) {
                ForgeDirection dir = evt.target.sideHit >= 0
                    ? ForgeDirection.VALID_DIRECTIONS[evt.target.sideHit]
                    : null;
                if (dir != null) {
                    ...
                    ArrayList<Coordinate> li = ItemBuilderWand.getCoordinatesFor(
                        world, x, y, z, dir, Minecraft.getMinecraft().thePlayer
                    );
                    for (Coordinate c : li) {
                        int dx = c.xCoord - x;
                        int dy = c.yCoord - y;
    ...
    public void drawPlacerHighlight(DrawBlockHighlightEvent evt) {
        if (evt.target != null && evt.target.typeOfHit == MovingObjectType.BLOCK) {
            if (evt.currentItem != null
                && ChromaItems.DUPLICATOR.matchWith(evt.currentItem)) {
                FilledBlockArray blocks = ItemDuplicationWand.getStructureFor(
                ...
                if (blocks != null) {
                    ...
                    for (int i = 0; i < blocks.getSize(); i++) {
                        ...
                        if (bk != null && bk != Blocks.air
                            && bk.getMaterial() != Material.air) {
                            ...
                            if (!blocks.hasNonAirBlock(
                                    c.xCoord, c.yCoord - 1, c.zCoord
                                )) {
                                IIcon ico = RenderBlocks.getInstance().getIconSafe(
                                    bk.getIcon(ForgeDirection.DOWN.ordinal(), meta)
                                );
                                float u = ico.getMinU();
                                float du = ico.getMaxU();
                                float v = ico.getMinV();
                                float dv = ico.getMaxV();
                                double en = blocks.hasNonAirBlock(
                                                c.xCoord, c.yCoord, c.zCoord - 1
                                            )
                                    ? 0
                                    : 0 - o;
                                double es = blocks.hasNonAirBlock(
                                                c.xCoord, c.yCoord, c.zCoord + 1
                                            )
                                    ? 1
                                    : 1 + o;
                                double ew = blocks.hasNonAirBlock(
                                                c.xCoord - 1, c.yCoord, c.zCoord
                                            )
                                    ? 0
                                    : 0 - o;
                                double ee = blocks.hasNonAirBlock(
                                                c.xCoord + 1, c.yCoord, c.zCoord
                                            )
                                    ? 1
                                    : 1 + o;
                            ...
                            if (!blocks.hasNonAirBlock(
                                ...
                                IIcon ico = RenderBlocks.getInstance().getIconSafe(
                                    bk.getIcon(ForgeDirection.UP.ordinal(), meta)
                                );
                                float u = ico.getMinU();
                                float du = ico.getMaxU();
                                float v = ico.getMinV();
                            ...
                            if (!blocks.hasNonAirBlock(
                                ...
                                IIcon ico = RenderBlocks.getInstance().getIconSafe(
                                    bk.getIcon(ForgeDirection.EAST.ordinal(), meta)
                                );
                                float u = ico.getMinU();
                                float du = ico.getMaxU();
                                float v = ico.getMinV();
                                ...
                                double es = blocks.hasNonAirBlock(
                                    ...
                                    ? 1
                                    : 1 + o;
                                double ed = blocks.hasNonAirBlock(
                                                c.xCoord, c.yCoord - 1, c.zCoord
                                            )
                                    ? 0
                                    : 0 - o;
                                double eu = blocks.hasNonAirBlock(
                                                c.xCoord, c.yCoord + 1, c.zCoord
                                            )
                                    ? 1
                                    : 1 + o;
                            ...
                            if (!blocks.hasNonAirBlock(
                                ...
                                IIcon ico = RenderBlocks.getInstance().getIconSafe(
                                    bk.getIcon(ForgeDirection.WEST.ordinal(), meta)
                                );
                                float u = ico.getMinU();
                                float du = ico.getMaxU();
                                float v = ico.getMinV();
                            ...
                            if (!blocks.hasNonAirBlock(
                                ...
                                IIcon ico = RenderBlocks.getInstance().getIconSafe(
                                    bk.getIcon(ForgeDirection.NORTH.ordinal(), meta)
                                );
                                float u = ico.getMinU();
                                float du = ico.getMaxU();
                                float v = ico.getMinV();
                            ...
                            if (!blocks.hasNonAirBlock(
                                ...
                                IIcon ico = RenderBlocks.getInstance().getIconSafe(
                                    bk.getIcon(ForgeDirection.SOUTH.ordinal(), meta)
                                );
                                float u = ico.getMinU();
                                float du = ico.getMaxU();
                                float v = ico.getMinV();
    ...
    public void stopPylonBoxHighlight(DrawBlockHighlightEvent evt) {
        if (evt.target != null && evt.target.typeOfHit == MovingObjectType.BLOCK) {
            ...
            if (ChromaTiles.getTile(world, x, y, z) == ChromaTiles.PYLON
                || ChromaTiles.getTile(world, x, y, z) == ChromaTiles.SKYPEATER)
                evt.setCanceled(true);
        }
    }
        ...
        if (evt.type == ElementType.HELMET) {
            ...
            if (Chromabilities.REACH.enabledOn(ep)) {
                MovingObjectPosition mov
                    = ReikaPlayerAPI.getLookedAtBlock(ep, 192, false);
                if (mov != null) {
                    int x = mov.blockX;
                    int y = mov.blockY;
                    int z = mov.blockZ;
                    if (AbilityHelper.instance.canReachBoostSelect(
                            ep.worldObj, x, y, z, ep
                        )) {
                        double dd = ReikaMathLibrary.py3d(
                            x + 0.5 - ep.posX, y + 0.5 - ep.posY, z + 0.5 - ep.posZ
                        );
                        GL11.glPushMatrix();
                        GL11.glPushAttrib(GL11.GL_ALL_ATTRIB_BITS);
                        double s = 1.5;
                        ...
                        f.drawString(
                            sg,
                            evt.resolution.getScaledWidth() / 3 + 4,
                            evt.resolution.getScaledHeight() / 3 - 9,
                            0xffffff
                        );
                        GL11.glPopMatrix();
                        GL11.glPopAttrib();
                        ...
                        if (b != null && b != Blocks.air) {
                            ...
                            if (back)
                                GL11.glFrontFace(GL11.GL_CW);
                            int meta = Minecraft.getMinecraft().theWorld.getBlockMetadata(
                                x, y, z
                            );
                            ReikaTextureHelper.bindTerrainTexture();
                            if (b instanceof BlockTileEnum) {
                                ...
                                try {
                                    ArrayList<ItemStack> li = b.getDrops(
                                        Minecraft.getMinecraft().theWorld,
                                        x,
                                        y,
                                        z,
                                    ...
                                    if (!li.isEmpty()) {
                                        ItemStack is = li.get((int
                                        ) ((System.currentTimeMillis() / 1000) % li.size()
                                        ));
                                        if (is.getItem() != null)
                                            ReikaGuiAPI.instance.drawItemStack(
                                                itemRender, f, is, dx, dy
                                ...
                                ) { //because some mods are derps and make getDrops crash
                                    //on the client or use it to set states
                                    ItemStack is = new ItemStack(b, 1, meta);
                                    if (is.getItem() != null)
                                        ReikaGuiAPI.instance.drawItemStack(
                                            itemRender, f, is, dx, dy
        ...
        if (evt.target != null && evt.target.typeOfHit == MovingObjectType.BLOCK) {
            ...
            if (Chromabilities.REACH.enabledOn(ep)) {
                ...
                if (AbilityHelper.instance.canReachBoostSelect(world, x, y, z, ep)) {
                    ...
                    int c = ReikaColorAPI.mixColors(
                        CrystalElement.LIME.getColor(),
                        CrystalElement.PURPLE.getColor(),
                        r
                    );
    ...
    public void drawExcavatorHighlight(DrawBlockHighlightEvent evt) {
        if (evt.target != null && evt.target.typeOfHit == MovingObjectType.BLOCK) {
            if (evt.currentItem != null
                && evt.currentItem.getItem() instanceof ItemBlockChangingWand) {
                World world = Minecraft.getMinecraft().theWorld;
                int x = evt.target.blockX;
                int y = evt.target.blockY;
                int z = evt.target.blockZ;
                BlockKey bk = BlockKey.getAt(world, x, y, z);
                ItemBlockChangingWand i
                    = (ItemBlockChangingWand) evt.currentItem.getItem();
                ...
                wandBlockCache.updateAndVerifyCache(
                    world, x, y, z, bk, ep, evt.currentItem
                );
                ...
                if (bk.blockID != Blocks.air) {
                    ...
                    double p6 = z - TileEntityRendererDispatcher.staticPlayerZ;
                    GL11.glTranslated(p2, p4, p6);
                    BlockArray blocks = wandBlockCache.getCachedOverlay(
                        world, x, y, z, bk.blockID, bk.metadata, ep, evt.currentItem
                    );
                    ReikaRenderHelper.prepareGeoDraw(true);
                    GL11.glDepthMask(false);
                    BlendMode.DEFAULT.apply();
    ...
    public void renderItemTags(RenderItemInSlotEvent.Pre evt) {
        if (GuiScreen.isCtrlKeyDown()) {
            if (evt.hasItem() && evt.isHovered()) {
                if (ProgressStage.ALLCOLORS.isPlayerAtStage(
                        Minecraft.getMinecraft().thePlayer
                    )) {
                    ItemStack is = evt.getItem();
                    ElementTagCompound tag
                        = ItemElementCalculator.instance.getValueForItem(is);
                    if (tag != null && !tag.isEmpty()) {
                        ...
                        for (CrystalElement e : tag.elementSet()) {
                         ...
                         if (Keyboard.isKeyDown(Keyboard.KEY_LCONTROL)) {
                             ...
                             double sc = 0.5;
                             GL11.glScaled(sc, sc, sc);
                             String s = Integer.toString(tag.getValue(e),
                         10).toUpperCase();//String.format("%d", tag.getValue(e)); int
                         color = e.getColor() | 0xff000000; FontRenderer f =
                         Minecraft.getMinecraft().fontRenderer;
                             ReikaGuiAPI.instance.drawCenteredStringNoShadow(f, s,
                         (int)((x+w-0)/sc), (int)((y+w-6)/sc), color);
                             GL11.glTranslated(1, 0, 0);
                             ReikaGuiAPI.instance.drawCenteredStringNoShadow(f, s,
                         (int)((x+w-0)/sc), (int)((y+w-6)/sc), color); GL11.glPopMatrix();
                         }
                         else {*/
                            ReikaTextureHelper.bindTerrainTexture();
                            v5.startDrawingQuads();
    ...
    public static void
    doDimensionSpellFailParticles(World world, double x, double y, double z) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            double[] vp = ReikaPhysicsHelper.polarToCartesian(
                v, world.rand.nextDouble() * 360, world.rand.nextDouble() * 360
            );
            float s = 5F + world.rand.nextFloat() * 10;
            int l = 20 + world.rand.nextInt(20);
            CrystalElement c = CrystalElement.randomElement();
            int clr = ReikaColorAPI.getModifiedSat(
                ReikaColorAPI.getColorWithBrightnessMultiplier(c.getColor(), 0.4F), 2
            );
            EntityCCBlurFX fx
                = new EntityCCBlurFX(world, rx, ry, rz, vp[0], vp[1], vp[2]);
            fx.setRapidExpand()
                .setAlphaFading()
                .setScale(s)
                .setLife(l)
