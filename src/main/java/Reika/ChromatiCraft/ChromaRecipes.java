...
public class ChromaRecipes {
    ...
    public static void addRecipes() {
        for (int i = 0; i < ReikaDyeHelper.dyes.length; i++) {
            ...
            ItemStack dye = ChromaOptions.isVanillaDyeMoreCommon(i)
                ? new ItemStack(Items.dye, 2, i)
                : ChromaItems.DYE.getCraftedMetadataProduct(2, i);
            GameRegistry.addShapelessRecipe(dye, berry);
        }

        ChromaItems.TOOL.addRecipe(
            "  s",
            " S ",
            "S  ",
            'S',
        ...
        GameRegistry.addRecipe(
            ...
            's',
            new ItemStack(
                ChromaItems.SHARD.getItemInstance(), 1, OreDictionary.WILDCARD_VALUE
            ),
            'C',
            Blocks.crafting_table
        );
        ChromaItems.HELP.addRecipe(
            "abc",
            "gBg",
            "def",
            'B',
            Items.book,
            'a',
            getShard(CrystalElement.BLACK),
            'b',
            getShard(CrystalElement.BLUE),
            'c',
            getShard(CrystalElement.GREEN),
            'd',
            getShard(CrystalElement.YELLOW),
            'e',
            getShard(CrystalElement.RED),
            'f',
            getShard(CrystalElement.WHITE),
            'g',
        ...
        for (ChromaResearch r : ChromaResearch.getAllNonParents()) {
            ...
            GameRegistry.addRecipe(new ShapelessNBTRecipe(
                ReikaItemHelper.getSizedItemStack(output, 2),
                output,
                Items.paper /*, ReikaItemHelper.inksac*/
            ));
        }

        //GameRegistry.addRecipe(ChromaTiles.BREWER.getCraftedProduct(), "NNN", "NBN",
        //"SSS", 'N', Items.quartz, 'S', Blocks.stone, 'B', Items.brewing_stand);
        //GameRegistry.addRecipe(ChromaItems.ENDERCRYSTAL.getStackOf(), "ISI", "SCS",
        //"ISI", 'I', Items.iron_ingot, 'S', getShard(ReikaDyeHelper.WHITE), 'C',
        ...
        if (ModList.THERMALEXPANSION.isLoaded()) {
            ...
            for (int i = 0; i < 16; i++) {
                ...
                ThermalRecipeHelper.addPulverizerRecipe(
                    shard, ChromaStacks.crystalPowder, 1000
                );
            }
        }
        ...
        GameRegistry.addShapelessRecipe(
            ChromaItems.ENDERBUCKET.getStackOf(),
            ChromaItems.ENDERBUCKET.getAnyMetaStack()
        );
    ...
    private static void loadSmelting() {
        ...
        for (int i = 0; i < metas.length; i++) {
            ...
            ); //new ItemStack(ChromaBlocks.PYLONSTRUCT.getBlockInstance(), n, meta);
            ReikaRecipeHelper.addSmelting(is, basic, 0);
        }

        ReikaRecipeHelper.addSmelting(
            BlockSparkle.BlockTypes.CLAY.getItem(),
            BlockSparkle.BlockTypes.BRICKS.getItem(),
            0.2F
        );
        ReikaRecipeHelper.addSmelting(
            BlockSparkle.BlockTypes.COBBLE.getItem(),
            BlockSparkle.BlockTypes.STONE.getItem(),
            0
        );
    }
        ...
        if (ModList.ROTARYCRAFT.isLoaded()) {
            for (int i = 0; i < CrystalElement.elements.length; i++) {
                ...
                BlockColorInterface.addGPRBlockColor(
                    ChromaBlocks.CRYSTAL.getBlockInstance(), i, color.getColor()
                );
                BlockColorInterface.addGPRBlockColor(
                    ChromaBlocks.LAMP.getBlockInstance(), i, color.getColor()
                );
                BlockColorInterface.addGPRBlockColor(
                    ChromaBlocks.SUPER.getBlockInstance(), i, color.getColor()
                );
                RecipeInterface.grinder.addAPIRecipe(
                    new ItemStack(ChromaBlocks.CRYSTAL.getBlockInstance(), 1, i),
                    ReikaItemHelper.getSizedItemStack(shard, GRINDER_SHARDS)
                );
                RecipeInterface.grinder.addAPIRecipe(
                    new ItemStack(ChromaBlocks.LAMP.getBlockInstance(), 1, i),
                    ReikaItemHelper.getSizedItemStack(shard, 4)
                );
            ...
            for (int i = 0; i < Flowers.list.length; i++) {
                ...
                RecipeInterface.grinder.addAPIRecipe(
                    ChromaBlocks.DECOFLOWER.getStackOfMetadata(i),
                    ReikaItemHelper.getSizedItemStack(is, 3)
                );
            }
        }

        if (ModList.THERMALEXPANSION.isLoaded()) {
            ThermalRecipeHelper.addSawmillRecipe(
                ChromaBlocks.GLOWLOG.getStackOfMetadata(OreDictionary.WILDCARD_VALUE),
                ReikaItemHelper.getSizedItemStack(
                    ReikaItemHelper.oakWood.asItemStack(), 6
                ),
