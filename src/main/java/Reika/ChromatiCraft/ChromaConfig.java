...
public class ChromaConfig extends ControlledConfig {
    ...
    private final DataElement<Boolean>[] trees
        = new DataElement[treeLength + vanillaTreeCount];
    private final DataElement<Float>[] dyeChances
        = new DataElement[ReikaJavaLibrary
                              .getEnumLengthWithoutInitializing(CrystalElement.class)];
    ...
    public ChromaConfig(DragonAPIMod mod, ConfigList[] option, IDRegistry[] id) {
        ...
        for (int i = 0; i < treeLength; i++) {
            ...
            trees[i + vanillaTreeCount]
                = this.registerAdditionalOption("Generate Mod Logs", name, true);
        }

        ArrayList<String> elements
            = ReikaJavaLibrary.getEnumEntriesWithoutInitializing(CrystalElement.class);
        for (int i = 0; i < dyeChances.length; i++) {
            dyeChances[i] = this.registerAdditionalOption(
                "Vanilla Dye Drop Chances", elements.get(i), 100F
            );
        }

        this.registerProperty(
            "t2ConfigModel",
            ReikaJVMParser.isArgumentPresent("-ChromaTrustingConfigModel")
        );

        guardianExceptions = this.registerAdditionalOption(
            "Other Options",
            "Guardian Stone Exceptions",
            this.getDefaultGuardstoneExceptions()
        );
        customMinerBlocks = this.registerAdditionalOption(
            "Other Options",
            "Custom Mineral Extractor Mappings",
            this.getDefaultMinerblockConfig()
        );

        structureDimensionBlacklist = this.registerAdditionalOption(
            "Other Options", "Structure Dimension Blacklist", new int[0]
        );
    }
    ...
    private String[] getDefaultGuardstoneExceptions() {
        ...
        for (Action a : Action.values())
            li.add("none#" + a.name());
        li.add(
            ModList.EXTRAUTILS.modLabel + ":dark_portal:2#"
            + Action.RIGHT_CLICK_BLOCK.name()
        ); //last millenium portal
    ...
    private static ArrayList<String> getModTrees() {
        ArrayList<String> base
            = ReikaJavaLibrary.getEnumEntriesWithoutInitializing(ModWoodList.class);
        ArrayList<String> li = new ArrayList();
