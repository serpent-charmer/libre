...
/**
 * Use this to blacklist biomes from the ChromatiCraft Biome changer; to do this, specify
 * "biome connections"; that is, a piece of code saying that "biome X must only come from
 * this set of biomes" or "biome Y must not come from this set of biomes". A few prefabs
 * are provided for convenience, including basic blacklisting/whitelisting features.
 */
public class BiomeBlacklist {
    ...
    public static void addBlacklist(BiomeConnection bc) {
        try {
            ...
            ReikaJavaLibrary.pConsole(
                "Error adding Biome Changer Blacklist: Class not loaded properly!"
            );
            e.printStackTrace();
        }
    }
    ...
    public static abstract class BlacklistBiomeConnection implements BiomeConnection {
        protected final HashMap<BiomeGenBase, Collection<BiomeGenBase>> blacklist
            = new HashMap();

        public BlacklistBiomeConnection() {}

        public BlacklistBiomeConnection(
            HashMap<BiomeGenBase, Collection<BiomeGenBase>> list
        ) {
