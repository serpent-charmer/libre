...
public enum ChromaItems implements ItemEnum {
    ...
    //REMOTETERM(11, false,	"chroma.terminal",		ItemRemoteTerminal.class,
    //ModList.APPENG),
    ...
    MANIPFOCUS(
        0, false, "chroma.manipfocus", ItemManipulatorFocus.class, ModList.THAUMCRAFT
    ),
    NETHERKEY(67, false, "chroma.netherkey", ItemNetherKey.class),
    ABILITYFOCUS(
        0, false, "chroma.abilityfocus", ItemAbilityFocus.class, ModList.THAUMCRAFT
    ),
    RECIPECACHE(69, false, "chroma.recipecapsule", ItemRecipeCacheCapsule.class),
    MULTIPATTERN(
        71, false, "chroma.patterncrystal", ItemPatternCrystal.class, ModList.APPENG
    ),
    ...
    private ChromaItems(
        int tex, boolean sub, String n, Class<? extends Item> iCl, ModList api
    ) {
        index = tex;
        hasSubtypes = sub;
        name = n;
    ...
    public String getMultiValuedName(int meta) {
        if (!this.hasMultiValuedName())
            throw new RuntimeException(
                "Item " + name + " was called for a multi-name, yet does not have one!"
            );
        ...
        switch (this) {
            ...
            case BUCKET:
                //if (meta >= 2)
                //	meta = 2;
                return StatCollector.translateToLocal(ChromaNames.fluidNames[meta]) + " "
                    + this.getBasicName();
            case SHARD:
                String s = meta >= 16 ? /*EnumChatFormatting.GREEN.toString()*/ "Boosted "
                                      : "";
                return s + CrystalElement.elements[meta % 16].displayName + " "
                    + this.getBasicName();
            case POTION:
            case PENDANT:
            case PENDANT3:
            ...
            case ELEMENTAL:
            case THROWGEM:
                return CrystalElement.elements[meta].displayName + " "
                    + this.getBasicName();
            case CLUSTER:
            ...
            case COLOREDMOD:
                return CrystalElement.elements[meta % 16].displayName + " "
                    + StatCollector.translateToLocal(
                        ChromaNames.coloredModInteractNames[meta / 16]
                    );
            case ENDERCRYSTAL:
            ...
            case DIMGEN:
                ...
                //	return this.getBasicName();
            case MAGICBRANCH:
                return ItemMagicBranch.BranchTypes.list[meta].name().toLowerCase(
                           Locale.ENGLISH
                       )
                    + " " + this.getBasicName();
            case ARTEFACT:
                return StatCollector.translateToLocal(
                    "chromaunknownartefact."
                    + ItemUnknownArtefact.ArtefactTypes.list[meta].name().toLowerCase(
                        Locale.ENGLISH
                    )
                );
            case FERTILITYSEED:
                return this.getBasicName();
            case ENDERBUCKET:
                return this.getBasicName() + " (" + BucketMode.list[meta].displayName
                    + " Mode)";
            default:
                break;
        }
    ...
    public int getNumberMetadatas() {
        ...
        switch (this) {
            ...
            case COLOREDMOD:
                return ChromaNames.coloredModInteractNames.length
                    * CrystalElement.elements.length;
            case SEED:
                return ReikaMathLibrary.intpow2(
                    2, Modifier.list.length + 4
                ); //16; //was 32
            case ENDERCRYSTAL:
                return 2;
            case CRAFTING:
            ...
            case FRAGMENT:
                return ReikaJavaLibrary
                    .getEnumLengthWithoutInitializing(ChromaResearch.class);
            case WARP:
                return 2;
            case OREPICK:
                return 720;
            ...
            default:
                throw new RegistrationException(
                    ChromatiCraft.instance,
                    "Item " + name + " has subtypes but the number was not specified!"
                );
        }
    }
    ...
    public void addSizedRecipe(int num, Object... params) {
        if (!this.isDummiedOut()) {
            ...
            //WorktableRecipes.getInstance().addRecipe(this.getCraftedProduct(num),
            //params);
        }
    }
    ...
    public void addMetaRecipe(int meta, Object... params) {
        if (!this.isDummiedOut()) {
            ...
            //WorktableRecipes.getInstance().addRecipe(this.getStackOfMetadata(meta),
            //params);
        }
    }
    ...
    public void addSizedMetaRecipe(int meta, int num, Object... params) {
        if (!this.isDummiedOut()) {
            ...
            //WorktableRecipes.getInstance().addRecipe(this.getCraftedMetadataProduct(num,
            //meta), params);
        }
    }
    ...
    public void addShapelessRecipe(Object... params) {
        if (!this.isDummiedOut()) {
            ...
            //WorktableRecipes.getInstance().addShapelessRecipe(this.getStackOf(),
            //params);
        }
    }
