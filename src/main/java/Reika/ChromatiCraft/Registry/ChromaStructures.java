...
public enum ChromaStructures implements StructureEnum<ChromaStructureBase> {
    ...
    private void construct() {
        try {
            ...
            throw new RegistrationException(
                ...
                "Could not instantiate structure type " + this + "!",
                e
            );
        }
    }
    ...
    public synchronized FilledBlockArray
    getArray(World world, int x, int y, int z, Random r) {
    ...
    public synchronized FilledBlockArray
    getArray(World world, int x, int y, int z, Random r, CrystalElement e) {
        if (e == null)
    ...
    public String getDisplayName() {
        return StatCollector.translateToLocal(
            "chromastruct." + this.name().toLowerCase(Locale.ENGLISH)
        );
    }
    ...
    public WeightedRandomChestContent[] getModifiedLootSet(
        ...
        if (structureInstance instanceof FragmentStructureBase) {
            ArrayList<WeightedRandomChestContent> li
                = ReikaJavaLibrary.makeListFromArray(items);
