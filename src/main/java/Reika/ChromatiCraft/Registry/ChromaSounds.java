...
public enum ChromaSounds implements ChromaSound, VariableSound {
    ...
    public void
    playSound(World world, double x, double y, double z, float vol, float pitch) {
        if (world.isRemote)
            return;
        ReikaSoundHelper.playSound(
            this, world, x, y, z, vol /* *this.getModulatedVolume()*/, pitch
        );
    }
    ...
    public void playSound(
        ...
        if (world.isRemote)
            return;
        ReikaSoundHelper.playSound(
            this, world, x, y, z, vol /* *this.getModulatedVolume()*/, pitch, attenuate
        );
    }

    public void
    playSoundAtBlock(World world, int x, int y, int z, float vol, float pitch) {
    ...
    public void
    playSoundAtBlockNoAttenuation(TileEntity te, float vol, float pitch, int broadcast) {
        this.playSoundNoAttenuation(
            te.worldObj,
            te.xCoord + 0.5,
            te.yCoord + 0.5,
            te.zCoord + 0.5,
    ...
    public void playSoundNoAttenuation(
        ...
        if (world.isRemote)
            return;
        //ReikaSoundHelper.playSound(this, ChromatiCraft.packetChannel, te.worldObj, x, y,
        //z, vol/* *this.getModulatedVolume()*/, pitch, false);
    ...
    public boolean canOverlap() {
        return this == RIFT || this == CAST || this == USE || this == ERROR
            || this == INFUSE || this == DING || this == DRONE || this == ITEMSTAND
            || this == KILLAURA_CHARGE || this == DASH || this == ORB;
    }

    @Override
    public boolean attenuate() {
        return this != GOTODIM && this != PYLONTURBO && this != PYLONFLASH
            && this != PYLONBOOSTRITUAL && this != PYLONBOOSTSTART
            && this != REPEATERSURGE && this != MONUMENT && this != MONUMENTRAY
            && this != GAINPROGRESS && this != LORECOMPLETE;
    }
    ...
    ) { // the steps need to be TWO octaves apart when using the default of 4!
        //if (this == FLUTE)
        //	return 2;
        return 4;
    }
    ...
    private static class ChromaSoundVariant
        extends SoundVariant<ChromaSounds> implements ChromaSound {
        protected ChromaSoundVariant(ChromaSounds s, String k) {
            super(
                s,
                k,
                PREFIX + SOUND_FOLDER + s.relative + "_" + k.toLowerCase(Locale.ENGLISH)
                    + SOUND_EXT
            );
        }
        ...
        @Override
        public String getRelativePath() {
            return SOUND_FOLDER + root.relative + "_" + key.toLowerCase(Locale.ENGLISH)
                + SOUND_EXT;
        }
        ...
        public void playSoundAtBlock(TileEntity te, float vol, float pitch) {
            this.playSoundAtBlock(
                te.worldObj, te.xCoord, te.yCoord, te.zCoord, vol, pitch
            );
        }

        public void playSoundAtBlockNoAttenuation(
            TileEntity te, float vol, float pitch, int broadcast
        ) {
            this.playSoundNoAttenuation(
                te.worldObj,
                te.xCoord + 0.5,
