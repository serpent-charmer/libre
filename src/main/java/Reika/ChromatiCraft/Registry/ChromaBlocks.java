...
public enum ChromaBlocks implements BlockEnum {
    ...
    TILEMODELLED(
        BlockModelledChromaTile.class,
        ItemBlockTileRegistry.class,
        "Modelled Chromatic Tile"
    ),
    ...
    CRYSTAL(
        BlockCaveCrystal.class, ItemBlockCrystal.class, "crystal.cave"
    ), //Cave Crystal
    ...
    TILECRYSTALNONCUBE(
        BlockCrystalTileNonCube.class,
        ItemBlockTileRegistry.class,
        "Crystal Tile Non-Cube"
    ),
    ...
    RAINBOWSAPLING(
        BlockRainbowSapling.class, ItemBlockRainbowSapling.class, "rainbow.sapling"
    ),
    ...
    TIEREDPLANT(
        BlockTieredPlant.class, ItemBlockChromaTiered.class, "chroma.tieredplant"
    ),
    ...
    STRUCTSHIELD(
        BlockStructureShield.class, ItemBlockStructShield.class, "chroma.shield"
    ),
    ...
    SPECIALSHIELD(
        BlockSpecialShield.class, ItemBlockStructShield.class, "chroma.specialshield"
    ),
    ...
    BRIDGECONTROL(
        BlockBridgeControl.class, ItemBlockMultiType.class, "chroma.bridgecontrol"
    ),
    ...
    //LOREREADER(BlockLoreReader.class,
    //"chroma.lorereader");
    ...
    REDSTONEPOD(
        BlockRedstonePod.class, ItemRedstonePodPlacer.class, "chroma.redstonepod"
    ),
    POLYCRYSTAL(BlockPolyCrystal.class, "chroma.polycrystal"),
    MOLTENLUMEN(BlockLiquidLumen.class, ChromatiCraft.lumen, "Molten Lumen"),
    METAALLOYLAMP(
        BlockMetaAlloyLamp.class, ItemBlockSidePlaced.class, "chroma.metaalloy"
    ),
    ...
    //SPAWNERCONTROL(BlockSpawnerShutdown.class,	ItemBlockSidePlaced.class,
    //"chroma.spawnershutdown"),
    ...
    //LIFEWATER(BlockTempleLifewater.class,		ChromatiCraft.lifewater,
    //"chroma.lifewater"),
    ...
    private ChromaBlocks(
        Class<? extends Block> cl, Class<? extends ItemBlock> ib, Fluid f, String n
    ) {
        blockClass = cl;
        blockName = n;
        itemBlock = ib;
    ...
    private ChromaBlocks(
        Class<? extends Block> cl, Class<? extends ItemBlock> ib, String n
    ) {
        this(cl, ib, null, n);
    }
    ...
    public boolean isDyePlant() {
        return this == DYESAPLING || this == DYEGRASS || this == DYEFLOWER
            || this == DYEVINE || this == FERTILEDYEVINE;
    }
    ...
    public boolean isTechnical() {
        return BlockChromaTile.class.isAssignableFrom(blockClass)
            || ReikaBlockHelper.isLiquid(this.getBlockInstance());
    }
    ...
    public String getMultiValuedName(int meta) {
        ...
        if (this == GLOW) {
            return Bases.baseList[meta / 16].displayName + "-Based "
                + CrystalElement.elements[meta % 16].displayName + " "
                + this.getBasicName();
        }
        ...
        switch (this) {
            ...
            case POWERTREE:
            case VOIDRIFT:
                return CrystalElement.elements[meta % 16].displayName + " "
                    + this.getBasicName();
            case HIVE:
            ...
            case STRUCTSHIELD:
                return this.getBasicName() + " "
                    + BlockStructureShield.BlockType.list[meta % 8].name;
            case SPECIALSHIELD:
                return this.getBasicName();
            case RELAY:
                return (meta == 16 ? "Omni" : CrystalElement.elements[meta].displayName)
                    + " " + this.getBasicName();
            case DIMGEN:
                return StatCollector.translateToLocal(
                    "chromablock.dimgen."
                    + BlockDimensionDeco.DimDecoTypes.list[meta].name().toLowerCase(
                        Locale.ENGLISH
                    )
                );
            case DIMGENTILE:
                return StatCollector.translateToLocal(
                    "chromablock.dimgen."
                    + BlockDimensionDecoTile.DimDecoTileTypes.list[meta]
                          .name()
            ...
            case DECOFLOWER:
                return StatCollector.translateToLocal(
                    "chroma.flower."
                    + BlockDecoFlower.Flowers.list[meta].name().toLowerCase(Locale.ENGLISH
                    )
                );
            case LASEREFFECT:
                return StatCollector.translateToLocal(
                    "chromablock.laser."
                    + BlockLaserEffector.LaserEffectType.list[meta].name().toLowerCase(
                        Locale.ENGLISH
                    )
                );
            case PINBALL:
                return StatCollector.translateToLocal(
                    "chromablock.pinball."
                    + BlockPinballTile.PinballRerouteType.list[meta].name().toLowerCase(
                        Locale.ENGLISH
                    )
                );
            case GRAVITY:
                return StatCollector.translateToLocal(
                    "chromablock.gravity."
                    + BlockGravityTile.GravityTiles.list[meta].name().toLowerCase(
                        Locale.ENGLISH
                    )
                );
            case ANTKEY:
            ...
            case SPARKLE:
                return "Sparkling "
                    + new ItemStack(BlockSparkle.BlockTypes.list[meta].getBlockProxy())
                          .getDisplayName();
            case ROUTERNODE:
                return StatCollector.translateToLocal("chromablock.routernode." + meta);
            case CLIFFSTONE:
                return "Cliff "
                    + Variants.getVariant(meta).getBlockProxy().getLocalizedName();
            case REPEATERLAMP:
                return BlockRepeaterLight.MODELS[meta].getName() + " Lamp";
            case HEATLAMP:
                return meta >= 8 ? "Freeze Lamp" : "Heat Lamp";
            case SHIFTLOCK:
                return ReikaObfuscationHelper.isDeObfEnvironment()
                    ? this.getBasicName() + " [" + Passability.list[meta] + "]"
                    : this.getBasicName();
            case PAD:
                return meta == 0 ? this.getBasicName()
                                 : this.getBasicName() + " (Decoration)";
            case PISTONTARGET:
            case PISTONCONTROL:
                return StatCollector.translateToLocal(this.getBasicName() + "." + meta);
            case PISTONBIT:
                return this.getBasicName() + " ("
                    + ReikaStringParser.capFirstChar(
                        BlockPistonTapeBit.getColor(meta).getName()
                    )
                    + ")";
            default:
                return "";
    ...
    public boolean match(ItemStack is) {
        return is != null
            && is.getItem() == Item.getItemFromBlock(this.getBlockInstance());
    }
    ...
    public boolean hasModel() {
        if (this == TILEMODELLED || this == TILEMODELLED2 || this == PYLON
            || this == TILECRYSTALNONCUBE)
            return true;
        return false;
    }

    public boolean isDimensionStructureBlock() {
        return blockClass.getName().startsWith(
            "Reika.ChromatiCraft.Block.Dimension.Structure"
        );
    }
