    ...
    public TileEntityAdjacencyUpgrade createTileEntity() {
        ...
        try {
            ...
            throw new RegistrationException(
                ...
                "Could not instantiate an adjacency upgrade TileEntity " + this,
                e
            );
        }
    }
        ...
        switch (this) {
            case BLACK:
                return String.format(
                    "Reduces lumen consumption by %sx",
                    ReikaStringParser.getAutoDecimal(1 - this.getFactor(tier))
                );
            case RED:
                return String.format(
                    "Protects against explosions up to %sx TNT",
                    ReikaStringParser.getAutoDecimal(this.getFactor(tier) / 4D)
                );
            case GREEN:
                return String.format(
                    "Ticks blocks around it, %d times per 5 seconds",
                    (int) (this.getFactor(tier) * 50)
                );
            case BROWN:
                return String.format(
                    "Transmutes stone into mineral, %s per second, up to rarity '%s'",
                    ReikaStringParser.getAutoDecimal(this.getFactor(tier) * 20),
                    TileEntityOreCreator.getMaxSpawnableRarity(tier)
                );
            case BLUE:
                break;
            case PURPLE:
                return String.format(
                    "Improves the performance of adjacent constructs, Tier %d", tier
                );
            case CYAN:
                return String.format(
                    "Increases fluid capacity %sx",
                    ReikaStringParser.getAutoDecimal(this.getFactor(tier))
                );
            case GRAY:
                return String.format(
                    "Shuffles blocks around it %s times a second",
                    ReikaStringParser.getAutoDecimal(this.getFactor(tier))
                );
            case LIGHTGRAY:
            ...
            case PINK:
                return String.format(
                    "Increases construct damage %sx",
                    ReikaStringParser.getAutoDecimal(this.getFactor(tier))
                );
            case LIME:
                return String.format(
                    "Increases construct range %sx",
                    ReikaStringParser.getAutoDecimal(this.getFactor(tier))
                );
            case YELLOW:
                return String.format(
                    "Multiplies energy output %sx",
                    ReikaStringParser.getAutoDecimal(this.getFactor(tier))
                );
            case LIGHTBLUE:
                return String.format(
                    "Accelerates time by %dx for TileEntities adjacent to it.",
                    (int) this.getFactor(tier)
                );
            case MAGENTA:
                return String.format("Repairs adjacent blocks, Tier %d", tier);
            case ORANGE:
                return String.format(
                    "Exchanges and balances heat, %s%s per tick",
                    ReikaStringParser.getAutoDecimal(this.getFactor(tier)),
                    "%"
                );
            case WHITE:
