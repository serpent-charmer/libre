...
public enum Chromabilities implements Ability {
    ...
    public static final UUID HEALTH_UUID
        = UUID.fromString("71d6a916-a54b-11e7-abc4-cec278b6b50a");
    ...
    private static final MultiMap<Phase, Ability> tickAbilities
    ...
    public String getDisplayName() {
        String s = StatCollector.translateToLocal(
            "chromability." + this.name().toLowerCase(Locale.ENGLISH)
        );
    ...
    private String deobfuscateIf(String s) {
        if (!ChromaResearchManager.instance.playerHasFragment(
                Minecraft.getMinecraft().thePlayer, ChromaResearch.getPageFor(this)
            )) {
    ...
    public void apply(EntityPlayer ep) {
        switch (this) {
            case MAGNET:
                AbilityCalls.attractItemsAndXP(
                    ep, 24, AbilityHelper.instance.isMagnetNoClip(ep)
                );
                break;
            case SHIELD:
    ...
    public boolean trigger(EntityPlayer ep, int data) {
        switch (this) {
            case REACH:
                AbilityCalls.setReachDistance(
                    ep, this.enabledOn(ep) ? AbilityHelper.REACH_SCALE[data] : -1
                );
                return true;
            case SONIC:
            ...
            case SHIFT:
                if (this.enabledOn(ep)) {
                    ...
                    AbilityHelper.instance.shifts.put(
                        ep,
                        new ScaledDirection(
                            ReikaEntityHelper.getDirectionFromEntityLook(ep, true), data
                        )
                    );
                } else {
            ...
            case TELEPORT:
                ...
                return true;
            case BREADCRUMB:
                AbilityHelper.instance.setPathLength(
                    ep, this.enabledOn(ep) ? ReikaMathLibrary.intpow2(2, data) : 0
                );
                return true;
            case DIMPING:
    ...
    public static void
    triggerAbility(EntityPlayer ep, Ability a, int data, float costScale) {
        if (ep.worldObj.isRemote) {
            ReikaPacketHelper.sendPacketToServer(
                ChromatiCraft.packetChannel,
                ChromaPackets.ABILITY.ordinal(),
                getAbilityInt(a),
                data
            );
            ...
            ReikaPacketHelper.sendDataPacket(
                ChromatiCraft.packetChannel,
                ChromaPackets.ABILITYSEND.ordinal(),
        ...
        if (ep == null) {
            if (System.currentTimeMillis() - lastNullPlayerDump > 5000) {
                ChromatiCraft.logger.logError(
                    "Tried to get ability status of null player!?"
                );
                Thread.dumpStack();
    ...
    private static void
    setToPlayer(EntityPlayer ep, boolean set, Ability a, boolean force) {
        NBTTagCompound nbt = ep.getEntityData();
        NBTTagCompound abilities = nbt.getCompoundTag(NBT_TAG);
        if (abilities == null) {
    ...
    public static boolean canPlayerExecuteAt(EntityPlayer ep, Ability a) {
        if (ModList.MYSTCRAFT.isLoaded()
            && MystPages.Pages.ABILITYBLOCK.existsInWorld(ep.worldObj))
            return false;
        ElementTagCompound use = AbilityHelper.instance.getUsageElementsFor(a, ep);
        return PlayerElementBuffer.instance.playerHas(ep, use)
            && a.canPlayerExecuteAt(ep);
    }
    ...
    public static void addAbility(Ability c) {
        ...
        ChromatiCraft.logger.log(
            "Added ability '" + c.getDisplayName() + "', assigned IDs '" + id + "' and #"
            + maxID
        );
    ...
    public String getPowerDesc(int level) {
        switch (this) {
            case SONIC:
                return level + "m radius\nMax explosion resist "
                    + String.format("%.0f", AbilityHelper.SONIC_EXPLO_FACTOR * level);
            case SHIFT:
            ...
            case GROWAURA:
                ...
                for (GrowAuraEffect g :
                     AbilityHelper.instance.getGrowAuraEffects(level)) {
