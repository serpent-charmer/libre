...
public class ItemMagicRegistry {
    ...
    private void addFluids() {
        ...
        this.addElement(
            FluidRegistry.getFluid("liquidessence"), CrystalElement.PURPLE, 2
        );
    }
    ...
    public void addPostload() {
        ...
        if (ModList.BOP.isLoaded()) {
            String[] arr = {
                "plants/0-8,12-15", "mushrooms/0-5", "willow",   "ivy",
                "treeMoss",         "flowerVine",    "wisteria", "lilyBop/0-2",
                "foliage/0-5,7-15", "coral1/11-15",  "coral2/8", "moss",
            };
            for (int i = 0; i < arr.length; i++) {
                for (ItemStack is :
                     ReikaItemHelper.parseMultiRangedMeta(ModList.BOP, arr[i])) {
        ...
        if (ModList.TWILIGHT.isLoaded()) {
            String[] arr = {
                "tile.TFPlant/3,4,8-11,13,14", "tile.HugeLilyPad",
                "tile.HugeWaterLily",          "tile.TrollVidr",
                "tile.UnripeTrollBer",         "tile.TrollBer",
            };
            for (int i = 0; i < arr.length; i++) {
                for (ItemStack is :
                     ReikaItemHelper.parseMultiRangedMeta(ModList.TWILIGHT, arr[i])) {
                    this.addElement(is, CrystalElement.GREEN, 1);
                }
            }
        }
        ...
        if (ModList.THAUMCRAFT.isLoaded()) {
            this.addElement(
                ThaumItemHelper.ItemEntry.ZOMBIEBRAIN.getItem(), CrystalElement.PINK, 4
            );
            this.addElement(
                ThaumItemHelper.ItemEntry.ZOMBIEBRAIN.getItem(), CrystalElement.GREEN, 1
            );
            this.addElement(
                ThaumItemHelper.ItemEntry.ZOMBIEBRAIN.getItem(),
                CrystalElement.LIGHTGRAY,
                1
            );

            this.addElement(
                ThaumItemHelper.ItemEntry.GOO.getItem(), CrystalElement.LIGHTGRAY, 3
            );
            this.addElement(
                ThaumItemHelper.ItemEntry.GOO.getItem(), CrystalElement.GREEN, 1
            );
            this.addElement(
                ThaumItemHelper.ItemEntry.GOO.getItem(), CrystalElement.CYAN, 1
            );
            this.addElement(
                ThaumItemHelper.ItemEntry.GOO.getItem(), CrystalElement.BLACK, 1
            );

            ItemStack shimmer
                = ReikaItemHelper.lookupItem("Thaumcraft:blockCustomPlant:2");
            ...
            for (BasicHexColors hex : BasicHexColors.values()) {
                ...
                if (i == null) {
                    ChromatiCraft.logger.logError(
                        "Could not find hex crystal item for color " + hex + "!"
                    );
                    continue;
                }
                ...
                this.addElement(
                    i, CrystalElement.BLACK, r == CrystalElement.BLACK ? 5 : 2
                );
                this.addElement(
                    i, CrystalElement.WHITE, r == CrystalElement.WHITE ? 4 : 1
                );
            }
        }
    ...
    private void addElement(ItemStack is, CrystalElement e, int amt) {
        KeyedItemStack ks
            = new KeyedItemStack(is).setSimpleHash(true).lock(); //.setSized(true);
    ...
    public ElementTagCompound getItemValue(ItemStack is) {
        if (!finishedLoading)
            throw new MisuseException(
                "Loading is not complete, cannot load the elemental registry yet!"
            );
