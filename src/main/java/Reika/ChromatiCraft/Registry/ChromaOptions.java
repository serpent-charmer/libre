...
public enum ChromaOptions implements SegmentedConfigList, SelectiveConfig, IntegerConfig,
                                     BooleanConfig, DecimalConfig, StringConfig,
                                     MatchingConfig, CustomCategoryConfig,
                                     UserSpecificConfig, Dependency {
    NOISE("Lamp Noises", true),
    //NETHER("Nether Crystals", true),
    NOPARTICLES(
        "Disable Pendant Particles", true
    ), //Whether to disable potion spirals from pendant-given effects
    CRYSTALFARM("Crystal Plants May Drop Shards", true),
    TILELAG(
        "Max Accelerator Lag in NanoSeconds", 1000000
    ), //How many ns can be spent before an accelerator will stop ticking a TileEntity
       //that tick (set negative to disable throttling)
    ...
    ANIMALSPAWN(
        "Rainbow Forest Animal Density", 6
    ), //Note that when reducing this, animals in rainbow forests drop more, to keep total
       //yields the same
    RAINBOWSPREAD("Rainbow Trees Spread Rainbow Forests", true),
    ENDERCOLORING(
        "Ender Forest Coloring", false
    ...
    ENDEREFFECT(
        "Liquid Ender Effect", true
    ), //Whether CC liquid ender should propel entities around (meaningless if TE is
       //installed as Resonant Ender is used instead)
    ...
    HARDTHAUM(
        "More difficult ThaumCraft integration", true
    ), //Whether CC/TC integration requires more progress in both mods
    PIELOC(
        "Energy Buffer Overlay Location", 0
    ), //Where to place the lumen buffer HUD element on screen, 0-3 for the corners, 0
       //being top-left, 1 bottom-left, 2 top-right, 3 bottom-right
    ...
    HOSTILEFOREST(
        "Allow Danger in Rainbow Forests", false
    ), //Whether ball lightnings can do damage, becoming the only potentially-dangerous
       //(if not aggressive) mobs in the biome
    RELAYRANGE("Lumen Relay Range", 16),
    REDRAGON(
        "Always Respawn EnderDragon", false
    ), //Respawn the EnderDragon when it is killed and the end is reloaded
    DELEND("Delete End on Unload", false),
    EASYFRAG(
        "Auxiliary Fragment Acquisition", false
    ), //Whether certain progress flags will also give an associated fragment, instead of
       //you needing to unlock it like the others
    COPYSIZE("Duplication Wand Max Volume", 1000),
    SHIFTTILES("World Shift Can Move TileEntities", false),
    //SMALLAURA("Use reduced-size Pylon Aura image; only enable this if you get a
    //full-screen color washout", false),
    STRUCTDIFFICULTY(
        "Dimension Structure Difficulty", 3
    ...
    CAVELIGHTERSIZE(
        "Cave Lighter Zone Size", 8
    ), //The subterranean illuminator works by dividing the world into NxNxN blocks and
       //attempting to place a light in each one. Smaller zones take longer to process the
       //volume but have more thorough lighting.
    ...
    BIOMEBLEND(
        "Blend CC Biome Edges", true
    ), //Whether to attempt to blend the edges of the Luminous Cliffs to the surroundings.
       //Expensive and imperfect
    MIDISIZE(
        "Orchestra MIDI Size Limit (KB)", 80
    ), //How large a MIDI can be loaded into the crystal orchestra, as superlarge MIDIs
       //will overflow packet limits
    //STRUCTPASSLEVEL("Structure Bypass Usability (0 = None, 1 = Admins Only, 2 = All)",
    //2), //Who can use the bypass dimension structures
    SUPERBUILDKEYBIND(
        "Superbuild Ability Activation", Key.LCTRL.name()
    ), //TypeHelper to Website Generator: String; What key to hold to build in superbuild
    ...
    EPILEPSY(
        "Epilepsy Mode", false
    ), //Whether to reduce the dynamism of some visuals, such as by making them only show
       //when holding a manipulator
    ...
    STRUCTTRIES(
        ...
        20
    ), //How forcefully should CC attempt to generate structures in the planned locations.
       //More attempts means more computation time, but fewer means a lower success rate
       //and thus lower total structure frequency
    PROGSHADER(
        "Use Alternate Progress Shader", false
    ), //Whether to replace the pixelation progress shader with a colorful overlay
    POWEREDPENDANTS(
        "Enhanced Pendants Require Charging", false
    ...
    LUMCLIFFSEEDSHIFT(
        "Luminous Cliffs Terrain Shape Seed Shift", 0
    ), //A modifier for the seed used to generate luminous cliffs terrain. Allows you to
       //try a new terrain layout for a biome while keeping the overall world seed
    ;
    ...
    public static boolean canPlayerUseStructureBypass(EntityPlayer ep) {
        ...
        switch(l) {
            ...
            default:
                return false;
            case 1:
                return ep instanceof EntityPlayerMP &&
    ReikaPlayerAPI.isAdmin((EntityPlayerMP)ep); case 2: return true;
        }
    }
     */
    ...
    public String getCategory() {
        if (this.isString()
            && this.name().toLowerCase(Locale.ENGLISH).contains("keybind"))
            return "Keybinds";
        return null;
    }
    ...
    public static int getMaxFenceSections() {
        return ReikaMathLibrary.ceilPseudo2Exp(
            MathHelper.clamp_int(FENCEHOPS.getValue(), 16, 256)
        );
    }
