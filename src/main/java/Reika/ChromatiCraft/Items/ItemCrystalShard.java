...
public class ItemCrystalShard extends ItemCrystalBasic
    implements AnimatedSpritesheet, TieredItem, IFlowerComponent {
                ...
                if (dmg < 16 && this.canCharge(ei)) {
                    ...
                    if (te instanceof TileEntityChroma) {
                        ...
                        if (tc.isFullyActive() && tc.getElement().ordinal() == dmg) {
                            boolean done = tickShardCharging(
                                ei,
                                CrystalElement.elements[dmg],
                                tc.getEtherCount(),
                                x,
                                y,
                                z
    ...
    tickShardCharging(EntityItem ei, CrystalElement e, int ether, int x, int y, int z) {
        ei.lifespan = Integer.MAX_VALUE;
        ...
        if (age > ei.age) { //items were combined
            ...
            ItemStack is = ChromaItems.SHARD.getCraftedMetadataProduct(
                ei.getEntityItem().stackSize, 16 + e.ordinal()
            );
            ...
            if (!ei.worldObj.isRemote) {
                ...
                ReikaPacketHelper.sendDataPacket(
                    ...
                    ChromaPackets.SHARDBOOST.ordinal(),
                    new PacketTarget.RadiusTarget(ei, 64),
                    ei.getEntityId()
                );
    ...
    public String getPotionEffect(ItemStack is) {
        ...
        switch (dye) {
            case BLACK:
                ret += PotionHelper
                           .redstoneEffect; //PotionHelper.fermentedSpiderEyeEffect;
                break;
            case BLUE:
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean verbose) {
        ...
        if (is.getItemDamage() >= 16)
            li.add(
                ...
                + "Gives level II enhanced potions"
            );
        else
    ...
    public String getTexture(ItemStack is) {
        return is.getItemDamage() >= 16
            ? "/Reika/ChromatiCraft/Textures/Items/shardglow.png"
            : super.getTexture(is);
    }
