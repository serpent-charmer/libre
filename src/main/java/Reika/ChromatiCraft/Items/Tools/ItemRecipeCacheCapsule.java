...
public class ItemRecipeCacheCapsule extends ItemChromaTool {
    ...
    public boolean onItemUseFirst(
        ...
        if (ChromaTiles.getTile(world, x, y, z) == ChromaTiles.TABLE) {
            TileEntityCastingTable te
                = (TileEntityCastingTable) world.getTileEntity(x, y, z);
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        ...
        if (pos != null) {
            li.add(
                "Storing recipe: "
                + (pos.recipe != null ? pos.recipe.getDisplayName() : "Unknown")
                + " at table " + pos.tableLoc
            );
        } else {
    ...
    private static CachedRecipe getRecipe(ItemStack is) {
        return is.stackTagCompound != null && is.stackTagCompound.hasKey("recipe")
            ? CachedRecipe.loadRecipe(is.stackTagCompound.getCompoundTag("recipe"))
            : null;
    }
    ...
    private static int
    getRecipeDifference(TileEntityCastingTable te, MultiBlockCastingRecipe r) {
        int ret = 0;
        for (Entry<List<Integer>, TileEntityItemStand> e :
             te.getOtherStands().entrySet()) {
    ...
    private static class CachedRecipe {
        ...
        private CachedRecipe(String id, Coordinate c) {
            recipe = Strings.isNullOrEmpty(id)
                ? null
                : (MultiBlockCastingRecipe
                ) RecipesCastingTable.instance.getRecipeByStringID(id);
            tableLoc = c;
        }
        ...
        public boolean applyToTable(TileEntityCastingTable te) {
            ...
            for (Coordinate c : standContents.keySet()) {
                if (ChromaTiles.getTile(te.worldObj, c.xCoord, c.yCoord, c.zCoord)
                    != ChromaTiles.STAND) {
                    return false;
                }
            }
            ...
            for (Entry<Coordinate, ItemStack> e : standContents.entrySet()) {
                TileEntityItemStand te2
                    = (TileEntityItemStand) e.getKey().getTileEntity(te.worldObj);
