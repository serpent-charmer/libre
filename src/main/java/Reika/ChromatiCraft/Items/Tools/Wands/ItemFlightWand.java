...
public class ItemFlightWand extends ItemWandBase {
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        ...
        if (this.canUse(ep)) {
            flag = this.trigger(
                world, is, ep, ReikaPlayerAPI.getLookedAtBlock(ep, 8, true)
            );
        }

        if (!flag) {
            ep.openGui(
                ChromatiCraft.instance, ChromaGuis.HOVER.ordinal(), world, 0, 0, 0
            );
        }
        return is;
    }
    ...
    public boolean onItemUse(
        ...
        return this.canUse(ep)
            && this.trigger(
                world,
                is,
                ep,
                new MovingObjectPosition(x, y, z, s, Vec3.createVectorHelper(a, b, c))
            );
    }
    ...
    private boolean
    trigger(World world, ItemStack is, EntityPlayer ep, MovingObjectPosition mov) {
        if (mov != null) {
            if (mov.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                ...
                if (world.getBlock(x, y + 1, z).isAir(world, x, y + 1, z)) {
                    ...
                    for (double d = 0; d <= 1; d += 0.125) {
                        ...
                        ReikaParticleHelper.ENCHANTMENT.spawnAt(
                            world, dx, dy, dz, vx, vy, vz
                        );
                    }
    ...
    public static HoverType getMode(ItemStack is) {
        return is.stackTagCompound != null
            ? HoverType.list[is.stackTagCompound.getInteger("mode")]
            : HoverType.ELEVATE;
    }
