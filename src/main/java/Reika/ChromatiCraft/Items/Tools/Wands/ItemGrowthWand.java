...
public class ItemGrowthWand extends ItemWandBase {
    ...
    public boolean onItemUse(
        ...
        if (this.sufficientEnergy(ep)) {
            ...
            ticker.addLocation(
                ...
                r,
                r / 2,
                ReikaEnchantmentHelper.getEnchantmentLevel(Enchantment.power, is) > 0
            );
    ...
    public static class GrowthTicker implements TickHandler {
        ...
        private void addLocation(
            EntityPlayer ep, World world, int x, int y, int z, int w, int h, boolean e
        ) {
    ...
    private static class GrowthLocation {
        ...
        private GrowthLocation(
            EntityPlayer ep, World world, int x, int y, int z, int r1, int r2, boolean e
        ) {
        ...
        private void
        removeLocation(Coordinate c, int idx, ArrayList<Coordinate> li, int d) {
            li.remove(idx);
            if (li.isEmpty()) {
                locations.remove(d);
            }
        ...
        private void tick(World world) {
            ...
            if (c != null) {
                ...
                int f = enhanced ? 16 : 1;
                ((ItemGrowthWand) ChromaItems.GROWTH.getItemInstance())
                    .drainPlayer(player, f);
            }
        }
    }
}
