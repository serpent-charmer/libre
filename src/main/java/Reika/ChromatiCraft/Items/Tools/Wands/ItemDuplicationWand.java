...
public class ItemDuplicationWand extends ItemWandBase {
        ...
        if (second) {
                    ...
                    for (int dz = z1; dz <= z2; dz++) {
                        if (dy >= 0) {
                            ...
                            if (b instanceof CustomCopyBehavior
                                && !((CustomCopyBehavior) b).allowCopy(meta)) {
            ...
            if (add.getSize() < MAX_SIZE
                ...
                ChromatiCraft.logger.debug(
                    "Added " + ct + " blocks to region, from " + cx + " to " + last
                    + ", for " + s + ". Now has " + all.getSize() + " blocks."
                );
                region.put(s, false);
                lastClick.remove(s);
            }
    ...
    private static void
    triggerPlacement(World world, int x, int y, int z, ForgeDirection dir, String s) {
        ...
        for (int i = 0; i < struct.getSize(); i++) {
            ...
            if (!struct.getBlockAt(c.xCoord, c.yCoord, c.zCoord)
                     .isAir(world, c.xCoord, c.yCoord, c.zCoord))
                ls.add(new PositionedBlock(
                    dx,
                    dy,
                    dz,
                    struct.getBlockAt(c.xCoord, c.yCoord, c.zCoord),
                    struct.getMetaAt(c.xCoord, c.yCoord, c.zCoord)
                ));
        }
    ...
    public static void loadMappings() {
        ...
        for (int i = 0; i < 16; i++) {
            addMapping(
                ...
                i,
                ChromaItems.SEED.getStackOfMetadata(i)
            );
        }
        ...
        if (ModList.ROTARYCRAFT.isLoaded()) {
            addMapping(
                ...
                ItemRegistry.CANOLA.getItemInstance()
            );
        }
        ...
        public void tick(TickType type, Object... tickData) {
            ...
            for (String s : placing.keySet()) {
                ...
                if (o.dimension == world.provider.dimensionId) {
                    ...
                    //ReikaJavaLibrary.pConsole(o.index+"/"+li.size()+": last placed
                    //"+b+", overwriting "+Block.getIdFromBlock(bf)+":"+mf);
                }
            }
            for (String s : end)
                placing.remove(s);
    ...
    private static class PositionedBlock {
        ...
        private void place(World world) {
            if (block == Blocks.air || block instanceof BlockAir) {
                ...
                if (b2 == Blocks.air) {
                    ChromaSounds.ERROR.playSoundAtBlock(
                        world, coord.xCoord, coord.yCoord, coord.zCoord, 0.25F, 2F
                    );
                } else
                    ReikaSoundHelper.playBreakSound(
                        world, coord.xCoord, coord.yCoord, coord.zCoord, b2
                    );
            } else
                ReikaSoundHelper.playPlaceSound(
                    world, coord.xCoord, coord.yCoord, coord.zCoord, block
                );
        ...
        public String toString() {
            return /*"["+block.getLocalizedName()+"x"+block.hashCode()+"] > "+*/ Block
    ...
    private static class PrimacySorter implements Comparator<PositionedBlock> {
      ...
      */
        ...
        private static int getIndex(Block b) {
            if (b.isOpaqueCube())
                return 0;
            if (b instanceof BlockFarmland || b instanceof BlockSlab
                || b instanceof BlockGlass)
                return 1;
            if (b instanceof BlockLeavesBase || b instanceof BlockStairs)
                return 1;
            if (b instanceof BlockBush || b.getMaterial() == Material.cactus
                || b.getMaterial() == Material.plants)
                return 2;
            if (b.getMaterial() == Material.circuits || b.getMaterial() == Material.carpet
                || b.getMaterial() == Material.portal)
                return 2;
            if (b.getMaterial() == Material.vine || b.getMaterial() == Material.web
                || b.getMaterial() == Material.snow)
                return 2;
            if (b == Blocks.torch || b == Blocks.redstone_torch
                || b == Blocks.unlit_redstone_torch || b instanceof BlockDoor)
                return 2;
            if (b instanceof BlockLiquid || b instanceof BlockFluidBase
                || b.getMaterial() == Material.water || b.getMaterial() == Material.lava)
                return 3;
            if (b == Blocks.air || b.getMaterial() == Material.air
