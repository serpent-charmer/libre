...
public class ItemExcavationWand extends ItemBlockChangingWand {
    ...
    public boolean canSpreadOn(World world, int x, int y, int z, Block b, int meta) {
        ...
        if (world.provider.dimensionId == 1
            && (b == Blocks.end_stone || b == Blocks.obsidian))
            return false;
        if (b == ChromaBlocks.BEDROCKCRACK.getBlockInstance())
            return false;
        TileEntity te = world.getTileEntity(x, y, z);
        return !(te instanceof CrystalNetworkTile)
            && !(te instanceof TileEntityStructControl);
    }

    @Override
    public boolean
    onBlockStartBreak(ItemStack itemstack, int x, int y, int z, EntityPlayer ep) {
        World world = ep.worldObj;
        if (!this.canSpreadOn(
                world, x, y, z, world.getBlock(x, y, z), world.getBlockMetadata(x, y, z)
            ))
            return false;
        if (!world.isRemote) {
            ProgressiveBreaker b
                = ProgressiveRecursiveBreaker.instance.addCoordinateWithReturn(
                    world, x, y, z, this.getDepth(ep, world, x, y, z)
                );
            //b.looseMatches.put(Blocks.redstone_ore, new
            //BlockKey(Blocks.lit_redstone_ore));
            b.call = this;
            b.silkTouch = ReikaEnchantmentHelper.getEnchantmentLevel(
                              Enchantment.silkTouch, itemstack
                          )
                > 0;
            b.drops = !ep.capabilities.isCreativeMode;
            b.fortune = ReikaEnchantmentHelper.getEnchantmentLevel(
                Enchantment.fortune, itemstack
            );
            if (ep.isSneaking())
                b.extraSpread = true;
            if (ReikaEnchantmentHelper.getEnchantmentLevel(
                    ChromaEnchants.AUTOCOLLECT.getEnchantment(), itemstack
                )
                > 0) {
    ...
    private HashSet<BlockKey> getSpreadBlocks(World world, int x, int y, int z) {
        ...
        if (bk == Blocks.lit_redstone_ore)
            ...
            Collection<BlockKey> c
                = ((CustomExcavationStarBehavior) bk).getSpreadBlocks(world, x, y, z);
            if (c != null && !c.isEmpty())
                set.addAll(c);
        }
        ReikaTreeHelper tree
            = ReikaTreeHelper.getTree(bk, world.getBlockMetadata(x, y, z));
        if (tree != null) {
    ...
    public int getDepth(EntityPlayer ep, World world, int x, int y, int z) {
        ...
        if (b instanceof CustomExcavationStarBehavior) {
            base = Math.max(
                base, ((CustomExcavationStarBehavior) b).getRange(world, x, y, z, ep)
            );
        }
        return base;
    }
    ...
    private void placeSomeLight(World world, int x, int y, int z) {
        if (world.getBlockLightValue(x, y, z) < 8 && !world.canBlockSeeTheSky(x, y, z)) {
            world.setBlock(
                ...
                y,
                z,
                ChromaBlocks.LIGHT.getBlockInstance(),
                Flags.PARTICLES.getFlag(),
                3
            );
        }
    }
        ...
        if (p != null) {
            ...
            if (ep != null) {
                return this.sufficientEnergy(ep)
                    && this.canBreakBlock(world, x, y, z, id, meta, ep);
            }
        }
        return false;
    }
    ...
    private boolean
    canBreakBlock(World world, int x, int y, int z, Block id, int meta, EntityPlayer ep) {
        return (world.isRemote
                || ReikaPlayerAPI.playerCanBreakAt(
                    (WorldServer) world, x, y, z, (EntityPlayerMP) ep
                ))
            && this.canSpreadOn(world, x, y, z, id, meta);
    }
    ...
    public void getSpreadBlocks(
        ...
        arr.recursiveAddMultipleWithBounds(
            world, x, y, z, set, x - 32, y - 32, z - 32, x + 32, y + 32, z + 32
        );
    }
}
