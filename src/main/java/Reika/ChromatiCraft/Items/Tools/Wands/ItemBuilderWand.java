...
public class ItemBuilderWand extends ItemWandBase {
    ...
    public static ArrayList<Coordinate> getCoordinatesFor(
        World world, int x, int y, int z, ForgeDirection dir, EntityPlayer ep
    ) {
        int dx = x + dir.offsetX;
        int dy = y + dir.offsetY;
        int dz = z + dir.offsetZ;
        ArrayList<ForgeDirection> rights
            = ReikaDirectionHelper.getPerpendicularDirections(dir);
        ...
        base.maxDepth = 2 * r;
        base.recursiveAddWithBoundsMetadata(
            world, x, y, z, b, m, x - rx, y - ry, z - rz, x + rx, y + ry, z + rz
        );
        ...
        for (int i = -d1.offsetX * rx - d2.offsetX * r;
             i <= d1.offsetX * rx + d2.offsetX * rx;
             i++) {
            for (int j = -d1.offsetY * ry - d2.offsetY * ry;
                 j <= d1.offsetY * ry + d2.offsetY * ry;
                 j++) {
                for (int k = -d1.offsetZ * rz - d2.offsetZ * rz;
                     k <= d1.offsetZ * rz + d2.offsetZ * rz;
                     k++) {
                    int ddx = dx + i;
                    int ddy = dy + j;
                    int ddz = dz + k;
                    ...
                    if (db == b && dm == m
                        && ReikaWorldHelper.softBlocks(world, ddx, ddy, ddz)) {
                        if (base.hasBlock(dmx, dmy, dmz)) {
                            if (ReikaWorldHelper.lineOfSight(
                                    world,
                                    ddx + 0.5,
                                    ddy + 0.5,
                                    ddz + 0.5,
        ...
        if (!fill || !fill2) {
            ...
            while (it.hasNext()) {
                ...
                int n = 0;
                for (int i = -d1.offsetX - d2.offsetX; i <= d1.offsetX + d2.offsetX;
                     i++) {
                    for (int j = -d1.offsetY - d2.offsetY; j <= d1.offsetY + d2.offsetY;
                         j++) {
                        for (int k = -d1.offsetZ - d2.offsetZ;
                             k <= d1.offsetZ + d2.offsetZ;
                             k++) {
    ...
    private boolean
    placeBlockAt(World world, int x, int y, int z, Block b, int m, EntityPlayer ep) {
    ...
    static enum BuildControl {
        ...
        public boolean isInUse(EntityPlayer ep) {
            return this == FLAT ? ep.isSneaking()
                                : KeyWatcher.instance.isKeyDown(ep, key);
        }
    }
    ...
    private static class ProximitySorter implements Comparator<Coordinate> {
        ...
        public int compare(Coordinate o1, Coordinate o2) {
            return (int) Math.signum(
                o1.getDistanceTo(posX, posY, posZ) - o2.getDistanceTo(posX, posY, posZ)
            );
        }
    }
}
