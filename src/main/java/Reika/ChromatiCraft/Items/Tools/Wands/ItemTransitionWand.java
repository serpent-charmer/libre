...
public class ItemTransitionWand extends ItemBlockChangingWand {
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        if (ep.isSneaking()) {
            ...
            ep.openGui(
                ChromatiCraft.instance, ChromaGuis.TRANSITION.ordinal(), world, 0, 0, 0
            );
        }
        return is;
    }
        ...
        if (!world.isRemote) {
            if (ep.isSneaking()) {
                ...
                if (id == world.getBlock(x, y, z)
                    ...
                    && mode != TransitionMode.VOLUMETRIC)
                    return false;
                ...
                int depth
                    = mode == TransitionMode.VOLUMETRIC || mode == TransitionMode.COLUMN
                    ? Integer.MAX_VALUE
                    : this.getDepth(ep, world, x, y, z);
    ...
    public void onPreBreak(
        ...
        if (r != null) {
            boolean exists
                = world.getPlayerEntityByName(r.player.getCommandSenderName()) != null;
            if (exists) {
                if (this.isUpgradingTank(world, x, y, z, id, meta, r)) {
                    CrystalTankAuxTile te
                        = (CrystalTankAuxTile) world.getTileEntity(x, y, z);
                ...
                if (r.silkTouch) {
                    ItemStack is = ReikaBlockHelper.getSilkTouch(
                        world, x, y, z, id, meta, r.player, true
                    );
                    if (is != null) {
                        li.clear();
                        li.add(is);
                ...
                if (!r.player.capabilities.isCreativeMode) {
                    if (!ReikaPlayerAPI.findAndDecrItem(r.player, r.place, r.placeM)) {
                        if (ModList.CHISEL.isLoaded()) {
                            int ret = ReikaChiselHandler.getChiselableSource(
                                r.player.inventory, r.place, r.placeM
                            );
                            if (ret != -1) {
                                ReikaInventoryHelper.decrStack(
                                    ret, r.player.inventory, 1
                                );
                            }
                        }
                    }
    ...
    @Override
    public boolean
    canBreak(ProgressiveBreaker b, World world, int x, int y, int z, Block id, int meta) {
        BlockReplace r = breakers.get(b.hashCode());
        if (world.getTileEntity(x, y, z) != null) {
            if (this.isUpgradingTank(world, x, y, z, id, meta, r)
                || (ModList.TINKERER.isLoaded()
                    && TinkerBlockHandler.getInstance().isSmelteryBlock(id)))
                ;
            else
                return false;
        }
        ...
        if (r != null) {
            boolean exists
                = world.getPlayerEntityByName(r.player.getCommandSenderName()) != null;
            if (exists) {
                if (this.sufficientEnergy(r.player)
                    && this.playerHas(r.player, r.place, r.placeM)) {
                    boolean perm = world.isRemote
                        || (ReikaPlayerAPI.playerCanBreakAt(
                            (WorldServer) world, x, y, z, (EntityPlayerMP) r.player
                        ));
                    switch (r.mode) {
                        case CONTIGUOUS:
                            return perm;
                        case AIRONLY:
                            return perm
                                && ReikaWorldHelper.checkForAdjNonCube(world, x, y, z)
                                != null;
                        case VOLUMETRIC:
                            return perm;
                        case COLUMN:
    ...
    private boolean isUpgradingTank(
        World world, int x, int y, int z, Block id, int meta, BlockReplace r
    ) {
        return id == ChromaBlocks.TANK.getBlockInstance() && r != null && r.place == id
            && id.damageDropped(meta) != id.damageDropped(r.placeM);
    }
    ...
    @Override
    public void getSpreadBlocks(
        World world, int x, int y, int z, BlockArray arr, EntityPlayer ep, ItemStack is
    ) {
        final BlockKey bk = BlockKey.getAt(world, x, y, z);
        switch (this.getMode(is)) {
            case CONTIGUOUS:
                arr.recursiveAddWithBoundsMetadata(
                    world,
                    x,
                    y,
                    z,
            ...
            case AIRONLY:
                PropagationCondition pc = new PropagationCondition() {
                    ...
                    public boolean isValidLocation(
                        ...
                        return bk.matchInWorld(world, x, y, z)
                            && ReikaWorldHelper.isExposedToAir(world, x, y, z);
                    }
                };
                arr.recursiveAddCallbackWithBounds(
                    world, x, y, z, x - 32, y - 32, z - 32, x + 32, y + 32, z + 32, pc
                );
                break;
            case VOLUMETRIC:
