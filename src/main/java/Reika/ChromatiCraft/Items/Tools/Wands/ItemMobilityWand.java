...
public class ItemMobilityWand extends ItemWandBase {
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        ...
        if (mov != null) {
            ...
            if (Chromabilities.ORECLIP.enabledOn(ep)
                || this.isValidTarget(world, x, y, z)) {
                ...
                if (this.sufficientEnergy(ep, f)) {
                    ...
                    for (int i = 0; i < 128; i++) {
                        ...
                        double ry
                            = ReikaRandomHelper.getRandomPlusMinus(ep.posY - 1, 0.5);
    ...
    private float
    getUsageFactor(double posX, double posY, double posZ, int x, int y, int z) {
        float num = 1;
        if (y >= posY) {
            num *= 1.5F;
        }
    ...
    private boolean isValidTarget(World world, int x, int y, int z) {
        return ReikaWorldHelper.softBlocks(world, x, y, z)
            && ReikaWorldHelper.softBlocks(world, x, y + 1, z);
    }
}
