...
public class ItemMobSonar extends ItemWandBase {
    ...
    private void prepareNBT(ItemStack is) {
        ...
        if (!is.stackTagCompound.hasKey("typeFlags")) {
            is.stackTagCompound.setInteger(
                "typeFlags", MobSonarRenderer.EntitySonarType.getAllFlags()
            );
        }
    }
    ...
    private void findLizardDoggo(World world, EntityPlayer ep) {
        PointSpawnLocation s = SFAPI.spawningHandler.getNearestSpawnPointOfType(
            ep, 512, SFAPI.genericLookups.getDoggoClass()
        );
        if (s != null) {
            ReikaPacketHelper.sendDataPacket(
                ChromatiCraft.packetChannel,
                ChromaPackets.DOGGOSONAR.ordinal(),
                (EntityPlayerMP) ep,
                s.getX(),
                s.getY(),
                s.getZ()
