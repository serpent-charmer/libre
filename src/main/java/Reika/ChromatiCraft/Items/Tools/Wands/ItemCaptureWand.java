...
public class ItemCaptureWand extends ItemWandBase implements EntityCapturingItem {
    ...
    @Override
    public boolean
    itemInteractionForEntity(ItemStack is, EntityPlayer ep, EntityLivingBase elb) {
        if (!hasMob(is) && elb instanceof EntityLiving
            && isEntityCapturable((EntityLiving) elb) && this.sufficientEnergy(ep)) {
    ...
    private boolean
    releaseMob(ItemStack is, EntityPlayer ep, World world, int x, int y, int z) {
    ...
    public boolean onRender(RenderItem ri, ItemStack is, ItemRenderType type) {
        if (type == ItemRenderType.EQUIPPED_FIRST_PERSON || type ==
    ItemRenderType.EQUIPPED) { EntityLiving e = this.getMob(is,
    Minecraft.getMinecraft().theWorld); if (e != null) { GL11.glPushMatrix();
                //GL11.glScaled(0.2, 0.2, 0.2);
                //GL11.glRotated(30, 0, 1, 0);
                //GL11.glRotated(-45, 1, 0, 0);
                //GL11.glTranslated(0, 1.5, -1);
                //GL11.glTranslated(type == ItemRenderType.EQUIPPED_FIRST_PERSON ? 1.5 :
    1, type == ItemRenderType.EQUIPPED_FIRST_PERSON ? 0.25 : 1, 1);
    ...
    renderer!"); else r.doRender(e, 0, 0, 0, 0, 0); GL11.glPopMatrix();
            }
        }
        return false;
    }
