...
public class ItemFluidWand extends ItemWandBase implements BreakerCallback {
        ...
        if (mov != null && !world.isRemote) {
            ...
            if (id != Blocks.air) {
                ...
                if (f != null) {
                    ...
                    b.blockValidity = bk
                        -> ReikaBlockHelper.isLiquid(bk.blockID)
                        && ReikaFluidHelper.lookupFluidForBlock(id)
                            == ReikaFluidHelper.lookupFluidForBlock(bk.blockID);
    ...
    public void onPostBreak(
        ...
        if (ep != null) {
            boolean exists
                = world.getPlayerEntityByName(ep.getCommandSenderName()) != null;
            if (exists) {
                this.drainPlayer(ep);
            } else {
                b.terminate();
