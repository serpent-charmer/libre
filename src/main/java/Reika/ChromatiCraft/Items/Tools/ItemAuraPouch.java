...
public class ItemAuraPouch extends ItemChromaTool implements ActivatedInventoryItem {
    ...
    private static final TreeMap<String, AuraPouchEffectDescription> specialEffects
        = new TreeMap();
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        ep.openGui(
            ChromatiCraft.instance, ChromaGuis.AURAPOUCH.ordinal(), world, 0, 0, 0
        );
        return is;
    }
    ...
    public ItemStack[] getInventory(ItemStack is) {
        ...
        if (is.getItem() == this && is.stackTagCompound != null) {
            ...
            for (int i = 0; i < inv.length; i++) {
                inv[i] = tag.hasKey("slot_" + i)
                    ? ItemStack.loadItemStackFromNBT(tag.getCompoundTag("slot_" + i))
                    : null;
            }
        }
        return inv;
    ...
    public ItemStack getItem(ItemStack is, int slot) {
        if (is.getItem() == this && is.stackTagCompound != null) {
            NBTTagCompound tag = is.stackTagCompound.getCompoundTag("inventory");
            ItemStack ret = tag.hasKey("slot_" + slot)
                ? ItemStack.loadItemStackFromNBT(tag.getCompoundTag("slot_" + slot))
                : null;
            return ret;
        }
        return null;
    ...
    public void setSlotActive(ItemStack is, int slot, boolean active) {
        if (is.stackTagCompound == null || !is.stackTagCompound.hasKey("inventory")
            || !is.stackTagCompound.hasKey("active")) {
            this.initNBT(is);
        }
