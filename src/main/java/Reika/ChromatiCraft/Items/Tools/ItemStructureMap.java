...
public class ItemStructureMap extends ItemChromaTool implements SpriteRenderCallback {
            ...
            if (ep.isSneaking()) {
                if (activeSearches.containsKey(ep)) {
                    ...
                    ReikaChatHelper.sendChatToPlayer(
                        ep, "This map is already scanning the world."
                    );
                } else {
                    ...
                    is.stackTagCompound.setString(
                        "searcher", ep.getUniqueID().toString()
                    );
                ...
                while (flag) {
                    is.setItemDamage(
                        (is.getItemDamage() + 1) % StructureSearchType.list.length
                    );
    ...
    public void onUpdate(ItemStack is, World world, Entity e, int slot, boolean held) {
        if (!world.isRemote && held && is.stackTagCompound != null
            && is.stackTagCompound.getBoolean("scanning")
            && is.stackTagCompound.hasKey("searcher")) {
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        ...
        if (is.stackTagCompound != null && is.stackTagCompound.getBoolean("scanning")) {
            li.add(String.format(
                "Map is scanning: %.1f%% complete.",
                activeSearches
                    .directGet(UUID.fromString(is.stackTagCompound.getString("searcher")))
                    .getProgress()
            ));
        } else if (is.stackTagCompound != null && is.stackTagCompound.hasKey(TAG_NAME)) {
            if (GuiScreen.isCtrlKeyDown()) {
                StructureMapData map = StructureMapData.readFromNBT(
                    is.stackTagCompound.getCompoundTag(TAG_NAME)
                );
                map.addTooltip(li);
            } else {
    ...
    public static void renderMap(ItemStack item, float ptick, EntityPlayer ep) {
        if (item.stackTagCompound == null || !item.stackTagCompound.hasKey(TAG_NAME))
            return;
        StructureMapData data
            = StructureMapData.readFromNBT(item.stackTagCompound.getCompoundTag(TAG_NAME)
            );
        ...
        ReikaTextureHelper.bindTexture(
            ChromatiCraft.class, "Textures/StructureMap/overlay_typed.png"
        );
        ...
        if (max == -1) {
            ...
            Coordinate far
                = data.bounds.getFarthestPointFrom(px, data.bounds.getCenterY(), pz);
        ...
        ReikaTextureHelper.bindTexture(
            ChromatiCraft.class, "Textures/StructureMap/marker.png"
        );
        ...
        private StructureMapData tick(WorldServer world) {
            ...
            if (type.structure != null) {
                for (int i = 0; i < 24; i++) {
                    ...
                    StructureGenStatus at = DungeonGenerator.instance.getGenStatus(
                        type.structure, world, rx, rz
                    );
                    if (at.hasStructure()) {
                        ...
                        foundStructures.put(
                            c,
                            cf ? StructurePresence.CONFIRMED : StructurePresence.EXPECTED
                        );
                    }
                    scannedChunks++;
                    if (this.updateScanChunk())
                        return new StructureMapData(
                            foundStructures, originX, originZ, cc, bounds
                        );
                }
                return null;
            } else {
                switch (type) {
                    case STRONGHOLD:
                        MapGenStronghold mg
                            = ((ChunkProviderGenerate
                               ) world.theChunkProviderServer.currentChunkProvider)
                                  .strongholdGenerator;
                        Field f = ReikaReflectionHelper.getProtectedInheritedField(
                            mg,
                            FMLForgePlugin.RUNTIME_DEOBF ? "field_75039_c" : "worldObj"
                        );
                        ...
                        ReikaObfuscationHelper.invoke(
                            "canSpawnStructureAtCoords", mg, 0, 0
                        ); //populate the map
                        List li0
                            = (List) ReikaObfuscationHelper.invoke("getCoordList", mg);
                        if (li0 != null) {
                            for (Object o : li0) {
                                Coordinate c = new Coordinate((ChunkPosition) o).to2D();
                                bounds
                                    = bounds.addCoordinate(c.xCoord, c.yCoord, c.zCoord);
                                foundStructures.put(c, StructurePresence.EXPECTED);
                            }
                        }
                        for (StructureStart ss :
                             ((Map<Long, StructureStart>) mg.structureMap).values()) {
                            if (ss.isSizeableStructure()) {
                                StructureBoundingBox sbb = ss.getBoundingBox();
                                if (sbb != null) {
                                    Coordinate c = new Coordinate(
                                        sbb.getCenterX(), 0, sbb.getCenterZ()
                                    );
                                    bounds = bounds.addCoordinate(
                                        c.xCoord, c.yCoord, c.zCoord
                                    );
                    ...
                    case TOWERS:
                        for (Towers t : Towers.towerList) {
                            ...
                            if (c == null)
                                c = new Coordinate(
                                    t.getRootPosition().chunkXPos,
                                    0,
                                    t.getRootPosition().chunkZPos
                                );
                            bounds = bounds.addCoordinate(c.xCoord, c.yCoord, c.zCoord);
                            boolean cf = ReikaWorldHelper.isChunkGenerated(
                                world, c.xCoord, c.zCoord
                            );
                            if (cf)
                                cc++;
                            foundStructures.put(
                                c,
                                cf ? StructurePresence.CONFIRMED
                                   : StructurePresence.EXPECTED
                            );
                        }
                        break;
                    default:
    ...
    private static class StructureMapData {
        ...
        private StructureMapData(
            HashMap<Coordinate, StructurePresence> data,
            int x,
            int z,
            int cc,
            BlockBox box
        ...
        public static StructureMapData readFromNBT(NBTTagCompound data) {
            ...
            return new StructureMapData(
                values,
                meta.getInteger("rootX"),
                meta.getInteger("rootZ"),
                cc,
                BlockBox.readFromNBT(meta)
            );
        }
