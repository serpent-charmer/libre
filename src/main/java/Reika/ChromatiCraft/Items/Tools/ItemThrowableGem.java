...
public class ItemThrowableGem
    extends ItemCrystalBasic implements AnimatedSpritesheet, TieredItem {
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        world.playSoundAtEntity(
            ep, "random.bow", 0.5F, 0.4F / (itemRand.nextFloat() * 0.4F + 0.8F)
        );

        if (!world.isRemote) {
            world.spawnEntityInWorld(new EntityThrownGem(
                world, ep, CrystalElement.elements[is.getItemDamage()]
            ));
        }
