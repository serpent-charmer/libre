...
public class ItemChromaBook extends ItemChromaTool {
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        if (ep.isSneaking()) {
            if (!this.isCreative(is))
                ep.openGui(
                    ChromatiCraft.instance, ChromaGuis.BOOKPAGES.ordinal(), null, 0, 0, 0
                );
        } else {
            ...
            ep.openGui(
                ChromatiCraft.instance, ChromaGuis.BOOKNAV.ordinal(), world, 0, 0, 0
            );
        }
        return is;
    }
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        if (this.isCreative(is)) {
            ...
            li.add(String.format(
                "Has %d of %d pages.",
                this.getNumberPages(is),
                ChromaResearch.getAllObtainableFragments().size()
            ));
            int blanks = this.getBlanksStored(is);
            li.add(String.format(
                "Has %d extra blank fragment%s.", blanks, blanks == 1 ? "" : "s"
            ));
        }
    }

    public static boolean isCreative(ItemStack is) {
        return ChromaItems.HELP.matchWith(is) && is.stackTagCompound != null
            && is.stackTagCompound.getBoolean("creative");
    }

    public static int getBlanksStored(ItemStack is) {
        return ChromaItems.HELP.matchWith(is) && is.stackTagCompound != null
            ? is.stackTagCompound.getInteger("blanks")
            : 0;
    }
    ...
    public ArrayList<ChromaResearch> getItemList(ItemStack tool) {
        ...
        if (tool.stackTagCompound != null) {
            NBTTagList list
                = tool.stackTagCompound.getTagList("pages", NBTTypes.STRING.ID);
            ...
            while (it.hasNext()) {
                ...
                if (r == null) {
                    ChromatiCraft.logger.logError(
                        "Null research item {" + s + "} in the book?!"
                    );
                    it.remove();
                    continue;
                }
                ...
                if (idx < 0) {
                    ...
                    throw new WTFException(
                        "How did you get a parent (OR NONEXISTENT) fragment '" + r
                            + "' in the book!?!",
                        true
                    );
                }
    ...
    public static boolean hasPage(ItemStack is, ChromaResearch b) {
        ...
        if (is.stackTagCompound == null || !is.stackTagCompound.hasKey("pages"))
            return false;
        return is.stackTagCompound.getTagList("pages", NBTTypes.STRING.ID)
            .tagList.contains(new NBTTagString(b.name()));
    }
    ...
    public static void
    recoverFragment(EntityPlayer ep, ChromaResearch r, ItemStack book) {
        ReikaInventoryHelper.findAndDecrStack(
            Items.paper, -1, ep.inventory.mainInventory
        );
        ReikaInventoryHelper.findAndDecrStack(
            ReikaItemHelper.inksac, ep.inventory.mainInventory
        );
