...
public class ItemBulkMover extends ItemChromaTool {
    ...
    public static int getStoredItems(ItemStack tool) {
        return tool.stackTagCompound != null ? tool.stackTagCompound.getInteger("count")
                                             : 0;
    }

    public static ItemStack getStoredItem(ItemStack tool) {
        return tool.stackTagCompound != null
            ? ItemStack.loadItemStackFromNBT(tool.stackTagCompound.getCompoundTag("item"))
            : null;
    }
    ...
    public static int getNumberToCarry(ItemStack tool) {
        return tool.stackTagCompound != null ? tool.stackTagCompound.getInteger("space")
                                             : 0;
    }
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        if (!this.hasItems(is))
            ep.openGui(
                ChromatiCraft.instance, ChromaGuis.BULKMOVER.ordinal(), world, 0, 0, 0
            );
        return is;
    }
    ...
    public boolean onItemUse(
        ...
        if (this.getStoredItem(is) != null && getNumberToCarry(is) > 0) {
            ...
            if (ii != null) {
                if (this.hasItems(is)) {
                    int added
                        = ii.addItems(this.getStoredItem(is), this.getStoredItems(is));
                    removeItems(is, added);
                } else {
                    int rem
                        = ii.drawItems(this.getStoredItem(is), this.getNumberToCarry(is));
                    addItems(is, rem);
                }
            }
        }
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        ...
        if (has != null) {
            String s = String.format(
                "Has %d of %s; set to move up to %d",
                amt,
                has.getDisplayName(),
                this.getNumberToCarry(is)
            );
            li.add(s);
        } else {
    ...
    getInteraction(World world, int x, int y, int z, int side, EntityPlayer ep) {
        TileEntity te = world.getTileEntity(x, y, z);
        if (InterfaceCache.GRIDHOST.instanceOf(te)) {
            return new AEInteraction(
                ((IGridHost) te).getGridNode(ForgeDirection.VALID_DIRECTIONS[side]), ep
            );
    ...
    private static class AEInteraction implements InventoryInteraction {
        ...
        public int addItems(ItemStack is, int max) {
            return (int
            ) (max - me.addItem(ReikaItemHelper.getSizedItemStack(is, max), false));
        }

        @Override
        public int drawItems(ItemStack is, int max) {
            return (int
            ) me.removeItem(ReikaItemHelper.getSizedItemStack(is, max), false, true);
        }
    }
    ...
    private static class IInvInteraction implements InventoryInteraction {
        ...
        public int addItems(ItemStack is, int max) {
            return max
                - ReikaInventoryHelper.addToInventoryWithLeftover(
                    ReikaItemHelper.getSizedItemStack(is, max), inventory, false
                );
        }
