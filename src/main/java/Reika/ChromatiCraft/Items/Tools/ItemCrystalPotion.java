...
public class ItemCrystalPotion extends ItemPotion {
    public ItemCrystalPotion(int tex) {
        ...
        this.setMaxDamage(0);
        this.setCreativeTab(
            this.isAvailableInCreativeMode() ? ChromatiCraft.tabChromaTools : null
        );
    }
    ...
    public List getEffects(ItemStack is) {
        PotionEffect pot = CrystalPotionController.instance.getEffectFromColor(
            CrystalElement.elements[this.getUsableDamage(is)], 3600, 0, false
        );
        if (pot == null)
        ...
        for (int i = 0; i < CrystalElement.elements.length; i++) {
            ...
            if (CrystalPotionController.instance.requiresCustomPotion(color)) {
                ...
                li.add(new ItemStack(
                    ID,
                    1,
                    ReikaPotionHelper.EXTENDED_BIT + ReikaPotionHelper.SPLASH_BIT + i
                ));
                li.add(new ItemStack(
                    ID, 1, ReikaPotionHelper.BOOST_BIT + ReikaPotionHelper.SPLASH_BIT + i
                ));

                li.add(new ItemStack(
                    ID,
                    1,
                    ReikaPotionHelper.EXTENDED_BIT + ReikaPotionHelper.BOOST_BIT + i
                ));
            }
        }
    }
