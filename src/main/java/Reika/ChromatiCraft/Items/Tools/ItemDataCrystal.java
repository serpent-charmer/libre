...
public class ItemDataCrystal extends ItemChromaTool {
    ...
    public void onUpdate(ItemStack is, World world, Entity e, int slot, boolean held) {
        if (is.stackTagCompound != null
            && world.getTotalWorldTime() - is.stackTagCompound.getLong("last") >= 10) {
    ...
    public boolean onItemUse(
        ...
        /*
        if (LoreManager.instance.hasPlayerCompletedBoard(ep) && world.getBlock(x, y, z) ==
        ChromaBlocks.LOREREADER.getBlockInstance() &&
        ChromaStructures.getLoreReaderStructure(world, x, y, z).matchInWorld()) {
        ...
        InscriptionRecipe ir
            = InscriptionRecipes.instance.getInscriptionRecipe(world, x, y, z);
        if (ir == null) {
            is.stackTagCompound = null;
            return true;
        }
        if (is.stackTagCompound != null && is.stackTagCompound.hasKey("recipe")
            && ir.referenceIndex != is.stackTagCompound.getInteger("recipe")) {
            is.stackTagCompound = null;
            return true;
        }
        if (is.stackTagCompound == null)
            ...
            if (!world.isRemote) {
                ...
                if (tick >= ir.duration) {
                    ...
                    ReikaPacketHelper.sendDataPacketWithRadius(
                        ...
                        ChromaPackets.INSCRIBE.ordinal(),
                        world,
                        x,
                        y,
                        z,
                ...
                ReikaRenderHelper.spawnDropParticles(
                    world, x, y, z, ir.input.blockID, ir.input.metadata
                );
            }
        } else {
    ...
    public static class EntityDataCrystal extends EntityItem {
        ...
        public void onUpdate() {
            ...
            if (this.handleWaterMovement()
                && new Coordinate(this).getBlock(worldObj)
    ...
    public double getDurabilityForDisplay(ItemStack is) {
        return 1
            - (double) is.stackTagCompound.getInteger("carve")
            / InscriptionRecipes.instance
                  .getRecipeByID(is.stackTagCompound.getInteger("recipe"))
                  .duration;
    }
}
