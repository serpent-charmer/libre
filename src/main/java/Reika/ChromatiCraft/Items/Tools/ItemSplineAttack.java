...
public class ItemSplineAttack extends ItemChromaTool implements ProjectileFiringTool {
    ...
    private static class PathFinder extends OpenPathFinder {
        ...
        protected boolean isValidBlock(World world, int x, int y, int z) {
            return super.isValidBlock(world, x, y, z)
                || this.isPassableBlock(world, x, y, z);
        }
    ...
    public void fire(ItemStack is, World world, EntityPlayer ep, boolean randomVec) {
        ...
        while (from != null) {
            li.add(from);
            List<EntityTarget> next = li.size() < max
                ? this.getAttackableEntitiesFrom(world, from.entity, ep, li)
                : null;
            from = next == null || next.isEmpty()
                ? null
                : ReikaJavaLibrary.getRandomListEntry(DragonAPICore.rand, next);
        }
        //List<Note> n = KeySignature.D.getScale();
        //float f =
        //0.5F*(float)MusicKey.D4.getInterval(n.get(ReikaRandomHelper.getSafeRandomInt(n.size())).ordinal()).getRatio(MusicKey.D4);
        float f = 0.5F
            * (float
            ) (CrystalMusicManager.instance.getRandomScaledDing(CrystalElement.BLACK)
               / CrystalMusicManager.instance.getDingPitchScale(CrystalElement.BLACK));
        int maxdur = -1;
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            ChromaPackets.SPLINEATTACKTARGETS.ordinal(),
            ep,
            64,
            data
        );
        for (EntityTarget e : li) {
            ...
            TickScheduler.instance.scheduleEvent(
                new ScheduledTickEvent(new DelayedAttack(
                    e.entity, DamageSource.causeIndirectMagicDamage(ep, ep), 6
                )),
                tick + 4
            );
            ScheduledSoundEvent ev = new ScheduledSoundEvent(
                ChromaSounds.ORB.getDownshiftedPitch(), ep, 0.5F, f
            );
            ev.attenuate = false;
    ...
    public static void
    receiveTargetListForTrail(World world, EntityPlayer ep, int[] data) {
        ...
        if (world.isRemote) {
            DecimalPosition pos
                = new DecimalPosition(ep.posX, ep.posY + 1.62 - 0.375, ep.posZ);
        ...
        for (int i = 2; i < data.length; i++) {
            ...
            s.addPoint(new BasicSplinePoint(
                new DecimalPosition(e.posX, e.posY + e.height / 2, e.posZ)
            ));
        }
    ...
    private static void doTrail(World world, Spline s, int offset) {
        ...
        ColorBlendList cbl = new ColorBlendList(
            12,
            0x000000,
            0xffffff,
            0x2288ff,
        ...
        ColorBlendList cbl2 = new ColorBlendList(
            12,
            0x000000,
            0x555555,
            0x004187,
        ...
        for (DecimalPosition pos : trail) {
            EntityBlurFX fx
                = new EntityCCBlurFX(world, pos.xCoord, pos.yCoord, pos.zCoord)
                      .setIcon(ChromaIcons.CENTER)
                      .setLife(l)
                      .setScale(sc * 0.15F);
            fx.setColorController(clr)
                .freezeLife((int) (i / 205F) - offset)
                .forceIgnoreLimits()
                .setAlphaFading();
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
            if (i % 4 == 0) {
                fx = new EntityCCBlurFX(world, pos.xCoord, pos.yCoord, pos.zCoord)
                         .setIcon(ChromaIcons.CENTER)
                         .setLife(l)
                         .setScale(sc);
                fx.setColorController(clr2)
                    .freezeLife((int) (i / 205F) - offset)
                    .forceIgnoreLimits()
                    .setAlphaFading();
                Minecraft.getMinecraft().effectRenderer.addEffect(fx);
    ...
    getFirstEntityTarget(World world, EntityPlayer ep, boolean randomVec) {
        ...
        for (double d = 0; d <= MAX_RANGE; d += 1) {
            ...
            double r = 1 + d / 4D;
            AxisAlignedBB box
                = AxisAlignedBB.getBoundingBox(dx, dy, dz, dx, dy, dz).expand(r, r, r);
            List<EntityLivingBase> li
                = world.getEntitiesWithinAABBExcludingEntity(ep, box, attackableSelector);
    ...
    private LinkedList<Coordinate> getPath(EntityLivingBase from, EntityLivingBase to) {
        ...
        return BreadthFirstSearch
            .getOpenPathBetween(
                ...
                16,
                box,
                EnumSet.allOf(PassRules.class)
            )
            .getPath();
    }

    private List<EntityTarget> getAttackableEntitiesFrom(
        World world, EntityLivingBase from, EntityPlayer owner, List<EntityTarget> path
    ) {
        for (double d = 1; d <= MAX_RANGE; d += Math.sqrt(d)) {
            d = Math.ceil(d);
            AxisAlignedBB box = ReikaAABBHelper.getEntityCenteredAABB(from, d);
            List<EntityLivingBase> li = world.getEntitiesWithinAABBExcludingEntity(
                from, box, attackableSelector
            );
            ArrayList<EntityTarget> li2 = new ArrayList();
            for (EntityLivingBase e : li) {
                if (e == owner)
