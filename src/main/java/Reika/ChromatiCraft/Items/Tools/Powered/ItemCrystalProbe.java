...
public class ItemCrystalProbe extends ItemPoweredChromaTool {
    ...
    @Override
    public void
    onPlayerStoppedUsing(ItemStack is, World world, EntityPlayer ep, int count) {
        count = MathHelper.clamp_int(
            this.getMaxItemUseDuration(is) - count, 0, CHARGE_TIME
        );
    ...
    private void fire(ItemStack is, World world, EntityPlayer ep) {
        ...
        if (mov != null) {
            ...
            if (i != null && i.isValid(world, x, y, z)
                && ToolChargingSystem.instance.getCharge(is) >= i.energyCost) {
    ...
    private int getActionType(ItemStack is) {
        ...
        int type
            = is.stackTagCompound != null ? is.stackTagCompound.getInteger("type") : 0;
        return type;
    }
    ...
    public void onUsingTick(ItemStack is, EntityPlayer ep, int count) {
        ...
        if (ep.worldObj.isRemote) {
            ...
            ChromaSounds.USE.playSound(
                ep,
                0.25F + 2F * count / CHARGE_TIME,
                MathHelper.clamp_float(0.5F, 2F * count / CHARGE_TIME, 2F)
            );
        }
    }
    ...
    public boolean hasEffect(ItemStack is) {
        return ToolChargingSystem.instance.getCharge(is) / (float) this.getMaxCharge()
            > 0.8;
    }
    ...
    public int getChargeState(float frac) {
        return frac >= Inspections.getMostExpensiveOperation().energyCost
                    / (float) this.getMaxCharge()
            ? Math.max(1, super.getChargeState(frac))
            : 0;
    }
    ...
    protected void renderExtraIcons(RenderItem ri, ItemStack is, ItemRenderType type) {
        if (type == ItemRenderType.INVENTORY
            && (GuiScreen.isCtrlKeyDown()
                || RenderItemInSlotEvent.isRenderingStackHovered(is))) {
            ...
            GL11.glTranslated(
                0.5 / sc, GuiScreen.isCtrlKeyDown() ? -0.5 / sc : 0.5 / sc, 0
            );
    ...
    private static enum Inspections {
        ...
        private boolean
        doEffect(World world, int x, int y, int z, int s, EntityPlayer ep, ItemStack is) {
            switch (this) {
                case REPEATER_CONNECTIVITY:
                    for (CrystalElement e : CrystalElement.elements) {
                        CrystalReceiver te
                            = (CrystalReceiver) world.getTileEntity(x, y, z);
                        boolean can = te.isConductingElement(e);
                        boolean flag
                            = can && CrystalNetworker.instance.checkConnectivity(e, te);
                ...
                case STRUCTURE_CHECK: {
                    MultiBlockChromaTile te
                        = (MultiBlockChromaTile) world.getTileEntity(x, y, z);
                    if (te.canStructureBeInspected()) {
                        ...
                        if (str != null) {
                            ...
                            switch (str) {
                                case RITUAL:
                                    ((RitualStructure) str.getStructure())
                                        .initializeEnhance(true, false);
                                    break;
                                default:
                                    break;
                            }
                            CrystalElement e = str.requiresColor
                                ? ((ColoredMultiBlockChromaTile) te).getColor()
                                : null;
                            FilledBlockArray arr = str.requiresColor
                                ? str.getArray(world, x, y, z, e)
                                : str.getArray(
                                    world, x + c.xCoord, y + c.yCoord, z + c.zCoord
                                );
                            if (arr.matchInWorld(StructureErrorOverlays.instance)) {
                                ChromaSounds.CAST.playSoundAtBlock(
                                    ...
                                    z,
                                    1,
                                    (float) MusicKey.G4.getRatio(MusicKey.C5)
                                );
                                ...
                                TickScheduler.instance.scheduleEvent(
                                    new ScheduledTickEvent(new ScheduledSoundEvent(
                                        ...
                                        z + 0.5,
                                        1,
                                        (float) MusicKey.G5.getRatio(MusicKey.C5)
                                    )),
                                    10
                                );
                            }
                            ...
                            if (te.hasStructure()) {
                                ...
                                ((SpecialStructureTile) te)
                                    .inspectStructure(StructureErrorOverlays.instance);
                        }
                    }
                    return false;
                }
                case MULTIBLOCK: {
                    ...
                    b.checkForFullMultiBlock(
                        world,
                        x,
                        y,
                        z,
                        ForgeDirection.VALID_DIRECTIONS[s].getOpposite(),
                        StructureErrorOverlays.instance
                    );
                    return true;
                } /*
                 case IMMERSIVE: {

                     b.checkForFullMultiBlock(world, x, y, z,
                 ForgeDirection.VALID_DIRECTIONS[s].getOpposite(),
                 StructureErrorOverlays.instance); return true;
                 }*/
                default:
                    return false;
            }
