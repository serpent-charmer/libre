...
public class ItemStructureFinder extends ItemPoweredChromaTool {
    ...
    private static final EnumMap<ChromaStructures, Integer> COLORS
    ...
    public static void doTickFX(EntityPlayer ep, ChromaStructures s) {
        ...
        double[] xyz = ReikaPhysicsHelper.polarToCartesian(
            0.0625, -ep.rotationPitch, ep.rotationYawHead + 90 + 60
        );
    ...
    @SideOnly(Side.CLIENT)
    public static void doTriggerFX(
        EntityPlayer ep, ChromaStructures s, int sx, int sz, boolean close, boolean genned
    ) {
        ...
        for (double a = -da; a <= da; a += 1) {
            ...
            for (double r = r1; r <= r2; r += 0.25) {
                ...
                EntityCCBlurFX fx
                    = new EntityCCBlurFX(ep.worldObj, px, ep.posY - 1.62, pz)
                          .setIcon(ChromaIcons.FADE_BASICBLEND);
                int c2 = c;
                if (r > rt)
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        if (!world.isRemote
            ...
            && ToolChargingSystem.instance.getCharge(is) >= PING_COST) {
            EnumMap<ChromaStructures, StructureSeekData> findData
                = new EnumMap(ChromaStructures.class);
            boolean debug = ReikaObfuscationHelper.isDeObfEnvironment() && ep.isSneaking()
                && ReikaPlayerAPI.isReika(ep);
            for (Entry<ChromaStructures, Integer> e : COLORS.entrySet()) {
                ...
                if (debug)
                    r = 5000;
                StructureSeekData loc = DungeonGenerator.instance.getNearestRealStructure(
                    s, (WorldServer) world, ep.posX, ep.posZ, r, false
                );
                if (loc != null)
                    findData.put(s, loc);
            ...
            for (Entry<ChromaStructures, StructureSeekData> e : findData.entrySet()) {
                ...
                if (flag) {
                    x = ReikaRandomHelper.getRandomPlusMinus(
                        MathHelper.floor_double(ep.posX), fz
                    );
                    z = ReikaRandomHelper.getRandomPlusMinus(
                        MathHelper.floor_double(ep.posZ), fz
                    );
                }
                if (debug) {
                    ReikaJavaLibrary.pConsole(
                        s + ">" + loc.location + "/" + loc.isKnownSuccess + "&" + flag
                    );
                }
                ReikaPacketHelper.sendDataPacket(
                    ChromatiCraft.packetChannel,
                    ChromaPackets.STRUCTFIND.ordinal(),
                    new PacketTarget.RadiusTarget(ep, 32),
                    s.ordinal(),
                    x,
                    z,
                    flag ? 1 : 0,
                    loc.isKnownSuccess ? 1 : 0
                );
            }
    ...
    protected boolean doTick(
        ItemStack is, World world, EntityPlayer ep, boolean held
    ) { //only called if isActivated is true anyway - do not need to recheck
        if (!world.isRemote
            && world.provider.dimensionId != ExtraChromaIDs.DIMID.getValue()) {
            ...
            for (Object key : li.func_150296_c()) {
                ...
                if (loc != null) {
                    ...
                    if (world.getTotalWorldTime() % tick == 0)
                        ReikaPacketHelper.sendDataPacket(
                            ChromatiCraft.packetChannel,
                            ChromaPackets.STRUCTFINDTICK.ordinal(),
                            new PacketTarget.RadiusTarget(ep, 16),
                            s.ordinal()
                        );
                }
            }
    ...
    protected boolean isActivated(EntityPlayer e, ItemStack is, boolean held) {
        if (is.stackTagCompound == null
            || !(is.stackTagCompound.getTag("points") instanceof NBTTagCompound))
            return false;
        NBTTagCompound li = is.stackTagCompound.getCompoundTag("points");
        ...
        while (it.hasNext()) {
            ...
            WorldLocation loc = WorldLocation.readFromNBT("loc", tag);
            if (loc == null || loc.dimensionID != e.worldObj.provider.dimensionId
                || e.worldObj.getTotalWorldTime() - tag.getLong(NBT_TAG) >= 20)
                it.remove();
        }
        return !li.hasNoTags();
    }
    ...
    public static void
    activate(ItemStack is, EntityPlayer ep, TileEntityStructControl te) {
