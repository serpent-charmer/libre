...
public class ItemOrePick
    extends ItemPoweredChromaTool implements ToolSprite, EnchantableItem {
    ...
    protected boolean doTick(ItemStack is, World world, EntityPlayer ep, boolean held) {
        ...
        for (float i = 0; i <= r; i += 0.2) {
            ...
            OrePingDelegate at = this.getOreType(
                world,
                MathHelper.floor_double(xyz.xCoord),
                MathHelper.floor_double(xyz.yCoord),
                MathHelper.floor_double(xyz.zCoord)
            );
            if (at != null) {
                type = at;
                break;
        ...
        return ep instanceof EntityPlayerMP
            && !ReikaPlayerAPI.isMiningWith((EntityPlayerMP) ep, is); //type != null;
    }
    ...
    public void onUsingTick(ItemStack is, EntityPlayer ep, int count) {
        ...
        if (ep.worldObj.isRemote) {
            ...
            ChromaSounds.NETWORKOPTCHARGE.playSound(
                ep,
    ...
    onPlayerStoppedUsing(ItemStack is, World world, EntityPlayer ep, int count) {
        ...
        if (count >= CHARGE_TIME
            && ToolChargingSystem.instance.getCharge(is) >= SONAR_COST) {
    ...
    private void fire(ItemStack is, World world, EntityPlayer ep) {
        if (!world.isRemote) {
            ToolChargingSystem.instance.removeCharge(
                is, (int) (SONAR_COST * this.getMaxCharge()), ep
            );
    ...
    public boolean onItemUse(
        ...
        if (ore != null) {
            if (!world.isRemote) {
                ToolChargingSystem.instance.removeCharge(
                    is, (int) (SCAN_COST * this.getMaxCharge()), ep
                );
    ...
    public int getHarvestLevel(ItemStack stack, String toolClass) {
        return toolClass.toLowerCase(Locale.ENGLISH).contains("pick")
            ? 2
    ...
    public float getDigSpeed(ItemStack stack, Block block, int meta) {
        return ForgeHooks.isToolEffective(stack, block, meta)
            ? 4
    ...
    public boolean onBlockDestroyed(
        ItemStack is, World world, Block b, int x, int y, int z, EntityLivingBase elb
    ) {
        if (b.getBlockHardness(world, x, y, z) > 0) {
            is.damageItem(1, elb);
            ToolChargingSystem.instance.addCharge(
                is, this.getMaxCharge() * 4 / this.getMaxDamage()
            ); //so 25% durability for 1 full recharge
        }
        return true;
    }
    ...
    public Result getEnchantValidity(Enchantment e, ItemStack is) {
        return e == Enchantment.unbreaking || e == Enchantment.efficiency
                || e == Enchantment.fortune
                || e.getName().toLowerCase(Locale.ENGLISH).contains("soulbound")
            ? Result.ALLOW
            : Result.DENY;
    }
