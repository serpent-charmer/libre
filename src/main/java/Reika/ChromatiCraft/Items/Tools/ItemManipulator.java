...
public class ItemManipulator extends ItemChromaTool implements IScribeTools {
    ...
    public boolean onItemUse(
        ...
        if (t == ChromaTiles.PYLON && ep.capabilities.isCreativeMode
            ...
            if (ep.isSneaking()) {
                ...
                ChromaAux.changePylonColor(
                    world, cp, CrystalElement.elements[(cp.getColor().ordinal() + 1) % 16]
                );
            }
            return true;
        }

        if (t == ChromaTiles.STRUCTCONTROL && ep.capabilities.isCreativeMode
            && DragonAPICore.debugtest) {
        ...
        if (t == ChromaTiles.STRUCTCONTROL) {
            if (!world.isRemote) {
                ChromatiCraft.logger.debug(
                    "Right clicked struct control. Side="
                    + FMLCommonHandler.instance().getEffectiveSide()
                );
                if (ProgressStage.CTM.playerHasPrerequisites(ep)) {
                    ChromatiCraft.logger.debug(
                        "Player has CTM prereqs. Side="
                        + FMLCommonHandler.instance().getEffectiveSide()
                    );
                    TileEntityStructControl te = (TileEntityStructControl) tile;
                    if (te.isMonument()) {
                        ChromatiCraft.logger.debug(
                            "Tile is a monument. Side="
                            + FMLCommonHandler.instance().getEffectiveSide()
                        );
                        if (world.provider.dimensionId == 0
                            || TuningThresholds.MONUMENT.isSufficientlyTuned(ep)) {
                            ChromatiCraft.logger.debug(
                                "Player is tuned. Side="
                                + FMLCommonHandler.instance().getEffectiveSide()
                            );
                            if (te.triggerMonument(ep)) {
                                ChromatiCraft.logger.debug(
                                    "Monument triggered. Side="
                                    + FMLCommonHandler.instance().getEffectiveSide()
                                );
                                ChromaSounds.USE.playSoundAtBlockNoAttenuation(
                                    te, 1, 1, 128
                                );
                                return true;
                            }
            ...
            if (ep.isSneaking()) {
                if (te.isPlacer(ep)) {
                    ...
                    ReikaSoundHelper.playSoundAtBlock(
                        ...
                        y,
                        z,
                        Block.soundTypeStone.getStepResourcePath(),
                        2,
                        0.5F
                    );
                    te.redirect(s);
        ...
        if (!world.isRemote) {
            if (ModList.THAUMCRAFT.isLoaded() && InterfaceCache.NODE.instanceOf(tile)) {
                if (ProgressStage.CTM.isPlayerAtStage(ep)
                    && ProgressStage.NODE.isPlayerAtStage(ep)
                    && ReikaThaumHelper.isResearchComplete(
                        ep, "NODESTABILIZERADV"
                    )) { //CC and TC progression
                    if (NodeRecharger.instance.isValidNode((INode) tile)) {
                        ReikaSoundHelper.playSoundFromServer(
                            ...
                            y + 0.5,
                            z + 0.5,
                            "thaumcraft:runicShieldEffect",
                            1,
                            1,
                            false
                        );
                        Collection<Aspect> li
                            = new ArrayList(((INode) tile).getAspects().aspects.keySet());
                        for (Aspect asp : li) {
        ...
        if (!world.isRemote && ModList.FORESTRY.isLoaded()
            && InterfaceCache.BEEHOUSE.instanceOf(tile)) {
        ...
        if (!world.isRemote && world.getBlock(x, y, z) instanceof CrystalTypeBlock
            && world.getBlock(x, y - 1, z) == ChromaBlocks.RAYBLEND.getBlockInstance()) {
            ChromaBlocks.RAYBLEND.getBlockInstance().onNeighborBlockChange(
                world, x, y - 1, z, world.getBlock(x, y, z)
            );
        }
        ...
        if (DragonAPICore.debugtest) {
            if (player.isSneaking())
                ...
                PlayerElementBuffer.instance.addToPlayer(
                    player,
                    CrystalElement.elements[count % 16],
                    PlayerElementBuffer.instance.getElementCap(player),
                    false
                );
            }
        }
            ...
            if (mov != null) {
                ...
                if (te instanceof ChargingPoint) {
                    ...
                    ChromaAux.chargePlayerFromPylon(
                        player,
                        cp,
                        cp.getDeliveredColor(player, world, x, y, z),
                        count,
                        true
                    );
                }
    ...
    private void spawnParticles(EntityPlayer player, CrystalElement e) {
        ...
        Minecraft.getMinecraft().effectRenderer.addEffect(
            new EntityRuneFX(player.worldObj, rx, ry, rz, e)
        );
    }
