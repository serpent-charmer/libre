...
public class ItemEnderBucket extends ItemChromaTool {
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        ...
        if (mov != null && mov.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
            if (!world.isRemote)
                this.actAt(
                    world, mov.blockX, mov.blockY, mov.blockZ, mov.sideHit, ep, is
                );
            return is;
        } else {
            if (ep.isSneaking()) {
                if (!world.isRemote) {
                    ...
                    ReikaChatHelper.sendChatToPlayer(
                        ep, "Bucket now in " + this.getMode(is).displayName + " mode."
                    );
                }
            } else {
                ep.openGui(
                    ChromatiCraft.instance,
                    ChromaGuis.ENDERBUCKET.ordinal(),
                    world,
                    0,
                    0,
                    0
    ...
    private boolean
    actAt(World world, int x, int y, int z, int s, EntityPlayer ep, ItemStack is) {
            ...
            if (o.apply(world, x, y, z, dir, ep, is, this.getActiveLink(is, ep))) {
                if (o == Operation.ADDTANK)
                    ...
                    ReikaSoundHelper.playSoundFromServerAtBlock(
                        ...
                        "game.neutral.swim",
                        0.7F,
                        0.7F + 0.3F * world.rand.nextFloat(),
                        true
                    );
                return true;
            }
    ...
    private Operation
    getOperation(World world, int x, int y, int z, EntityPlayer ep, BucketMode mode) {
    ...
    public int getActiveLinkIndex(ItemStack is, EntityPlayer ep) {
        if (is.stackTagCompound == null)
            return -1;
        int idx = is.stackTagCompound.hasKey("selected")
            ? is.stackTagCompound.getInteger("selected")
            : -1;
        return idx;
    }
    ...
    private static enum Operation {
        ...
        private boolean apply(
            ...
            switch (this) {
                case ADDTANK:
                    ((ItemEnderBucket) is.getItem())
                        .addFluidHandler(world.getTileEntity(x, y, z), ep, is);
                    return true;
                case PICKUPWORLD: {
                    if (link == null)
                        return false;
                    ...
                    if (ReikaWorldHelper.softBlocks(world, x, y, z)) {
                        ...
                        if (f != null && f.getBlock() != null) {
                            if (ReikaFluidHelper.lookupFluidForBlock(
                                ...
                                && ReikaWorldHelper.isLiquidSourceBlock(world, x, y, z))
                                return false;
                            link.getCurrentFluidToDrain(true, false);
                            world.setBlock(x, y, z, f.getBlock(), 0, 3);
                            world.markBlockForUpdate(x, y, z);
                            world.getBlock(x, y, z).onNeighborBlockChange(
                                world, x, y, z, f.getBlock()
                            );
                            return true;
                        }
                    }
    ...
    tryDrainFluid(IFluidHandler ifl, Fluid seek, boolean doDrain) {
        for (int i = 0; i < 6; i++) {
            ForgeDirection dir = ForgeDirection.VALID_DIRECTIONS[i];
            FluidStack fs = ifl.drain(dir, FluidContainerRegistry.BUCKET_VOLUME, doDrain);
            if (fs != null && (seek == null || seek == fs.getFluid())
                && fs.amount >= FluidContainerRegistry.BUCKET_VOLUME) {
                return fs;
            }
        }
        return null;
    ...
    public static class TankLink {
        ...
        private TankLink(TileEntity te) {
            this(
                ...
                new BlockKey(
                    te.worldObj.getBlock(te.xCoord, te.yCoord, te.zCoord),
                    te.worldObj.getBlockMetadata(te.xCoord, te.yCoord, te.zCoord)
                )
            );
        ...
        private ItemStack calcItem() {
            ...
            if (b instanceof BlockTileEnum) {
                TileEnum te
                    = ((BlockTileEnum) b)
                          .getMapping(
                              tank.getWorld(), tank.xCoord, tank.yCoord, tank.zCoord
                          );
        ...
        public Fluid getCurrentFluidToDrain(boolean doDrain, boolean render) {
            ...
            for (int i = 0; i < 6; i++) {
                ForgeDirection dir = ForgeDirection.VALID_DIRECTIONS[i];
                FluidStack fs
                    = ifl.drain(dir, FluidContainerRegistry.BUCKET_VOLUME, doDrain);
