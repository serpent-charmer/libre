...
public class ItemKillAuraGun extends ItemProgressGatedTool {
    ...
    private static boolean fire(ItemStack is, EntityPlayer ep, int power) {
        ...
        for (Entity e : li) {
            if (e != ep) {
                ...
                if (dmg > 0) {
                    e.attackEntityFrom(
                        new ReikaEntityHelper.WrappedDamageSource(DamageSource.magic, ep),
                        dmg
                    );
                }
            }
    ...
    private static float getFalloffFactor(ItemStack is) {
        return 1
            - ReikaEnchantmentHelper.getEnchantmentLevel(
                  ChromaEnchants.WEAPONAOE.getEnchantment(), is
              )
            / 10F;
    }
    ...
    private static float getRawDamage(ItemStack is) {
        ...
        return BASE_DAMAGE
            + ReikaEnchantmentHelper.getEnchantmentLevel(Enchantment.power, is) * f;
    }
    ...
    private static void doFireParticles(EntityPlayer ep, int power) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            ) new EntityCCFloatingSeedsFX(ep.worldObj, lx, ly, lz, phi, theta, ico)
                                           .setColor(0xffffff)
                                           .setNoSlowdown()
                                           .setScale(s);
    ...
    getWallAttackDamage(ItemStack is, EntityPlayer ep, Entity e, int power) {
        double d = e.getDistanceToEntity(ep);
        if (d <= RANGE) {
            double[] angs = ReikaPhysicsHelper.cartesianToPolar(
                e.posX - ep.posX, e.posY - ep.posY, e.posZ - ep.posZ
            );
            ...
            //ReikaJavaLibrary.pConsole(ep.rotationYawHead+":"+ep.rotationPitch+":"+Arrays.toString(angs),
            //Side.SERVER);
            ...
            double dmg = getRawDamage(is) - RANGE_FALLOFF * f * Math.max(0, (d - 2))
                - ANGLE_FALLOFF * f * da - (CHARGE_TIME - power) * CHARGE_FALLOFF;
    ...
    public void onUsingTick(ItemStack is, EntityPlayer ep, int count) {
        ...
        if (!ep.worldObj.isRemote)
            ChromaSounds.KILLAURA_CHARGE.playSound(
                ep,
    ...
    private void doChargingParticles(EntityPlayer ep, int power) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            PositionController p
                = new CollectingPositionController(px, py, pz, lx, ly, lz, t);
            EntityFX fx = new EntityCCBlurFX(ep.worldObj, px, py, pz)
