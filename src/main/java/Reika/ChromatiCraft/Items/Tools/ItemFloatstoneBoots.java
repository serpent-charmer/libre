...
public class ItemFloatstoneBoots
    extends ItemArmor implements MultisheetItem, ISpecialArmor {
    ...
    public ItemFloatstoneBoots(int tex, int render) {
        ...
        this.setNoRepair();
        this.setCreativeTab(
            this.isAvailableInCreativeMode() ? this.getCreativePage() : null
        );
    }
    ...
    public void onArmorTick(World world, EntityPlayer ep, ItemStack is) {
        ...
        if (is.stackTagCompound != null) {
            if (is.stackTagCompound.hasKey("special")) {
                ItemStack load = ItemStack.loadItemStackFromNBT(
                    is.stackTagCompound.getCompoundTag("special")
                );
                if (load != null) {
    ...
    public static void addParticles(World world, EntityPlayer ep) {
        if (ep.capabilities.isFlying) {
            ...
            EntityFX fx = new EntityCCBlurFX(
                              world, ep.posX, ep.posY - 1.62, ep.posZ, vx, -0.25, vz
            )
                              .setColor(c)
                              .setLife(15)
                              .setRapidExpand()
    ...
    public final String getItemStackDisplayName(ItemStack is) {
        ...
        String name = ir.hasMultiValuedName() ? ir.getMultiValuedName(is.getItemDamage())
                                              : ir.getBasicName();
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT) {
            //name = ModList.NEI.isLoaded() && DragonAPICore.hasGameLoaded() ?
            //ObfuscatedNameHandler.registerName(name, is) : name;
    ...
    @Override
    @SideOnly(Side.CLIENT)
    public final ModelBiped
    getArmorModel(EntityLivingBase elb, ItemStack is, int armorSlot) {
        return null; //ChromaClient.getArmorRenderer(ChromaItems.getEntry(is));
    }

    @Override
    public final String
    getArmorTexture(ItemStack is, Entity entity, int slot, String type) {
    ...
    public ArmorProperties getProperties(
        ...
        if (is != null && is.getItem() instanceof ISpecialArmor)
            return ((ISpecialArmor) is.getItem())
                .getProperties(player, is, source, damage, slot);
        ItemStack isa = is != null ? is : armor.copy();
        ItemArmor ia = is != null ? (ItemArmor) is.getItem() : this;
        return new ArmorProperties(
            0, ia.damageReduceAmount / 25D, isa.getMaxDamage() + 1 - isa.getItemDamage()
        ); //default
    }
