...
public class ItemWarpCapsule extends ItemChromaTool {
    ...
    private static void teleport(EntityPlayerMP ep, DecimalPosition pos, int dim) {
        if (canTeleport(ep, pos, dim)) {
            ...
            AxisAlignedBB test
                = ReikaAABBHelper.copyAABB(ep.boundingBox).contract(0.25, 0.25, 0.25);
            do {
            ...
            double v[] = ReikaPhysicsHelper.polarToCartesian(
                vel,
                ep.worldObj.rand.nextDouble() * 360,
                ep.worldObj.rand.nextDouble() * 360
            );
            ...
            ep.fallDistance = ep.worldObj.rand.nextFloat() * 5F
                + (float) Math.max(-ep.motionY * 5, 0);

            ep.setCurrentItemOrArmor(0, null);

            ReikaSoundHelper.playSoundFromServer(
                world, dx, dy, dz, "mob.endermen.portal", 2, 1, true
            );
            ReikaSoundHelper.playSoundFromServer(
                world, dx, dy, dz, "mob.endermen.portal", 2, 1, true
            );
    ...
    private static boolean canTeleport(EntityPlayer ep, DecimalPosition pos, int dim) {
        return dim == ep.worldObj.provider.dimensionId
            && new WorldLocation(ep).canSeeTheSky()
            && pos.getDistanceTo(new DecimalPosition(ep)) <= MAXRANGE;
    }
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        ...
        if (pos != null) {
            li.add(
                "Linked to around " + pos.formattedString(1) + " in DIM"
                + getDimension(is)
            );
        }
    }
    ...
    private static DecimalPosition getPosition(ItemStack is) {
        return is.stackTagCompound != null
            ? DecimalPosition.readFromNBT("pos", is.stackTagCompound)
            : null;
    }
