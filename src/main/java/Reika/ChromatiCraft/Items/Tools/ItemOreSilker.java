...
public class ItemOreSilker extends ItemChromaTool implements ToolSprite, EnchantableItem {
    ...
    public int getHarvestLevel(ItemStack stack, String toolClass) {
        return toolClass.toLowerCase(Locale.ENGLISH).contains("pick")
            ? Math.max(3, Items.diamond_pickaxe.getHarvestLevel(stack, toolClass))
    ...
    public boolean onBlockStartBreak(ItemStack is, int x, int y, int z, EntityPlayer ep) {
        ...
        if (id == Blocks.glowstone || ReikaOreHelper.isVanillaOre(id)
            || ModOreList.isModOre(id, meta)) {
    ...
    private void
    dropSilkedOre(World world, int x, int y, int z, Block b, int meta, EntityPlayer ep) {
