        ...
        if (!world.isRemote && e instanceof EntityPlayer && is.stackTagCompound != null
            ...
            for (EntityItem ei : li) {
                if (ei.isDead || ei.delayBeforeCanPickup > 0
                    || ei.getEntityData().getBoolean(TileEntityItemInserter.DROP_TAG))
                    continue;
                ItemStack in = ei.getEntityItem();
                if (in == null || in.stackSize <= 0)
                    continue;
                if (in.getItem() instanceof OwnableItem
                    && !((OwnableItem) in.getItem()).isCollectableBy(ei, ep)) {
                    continue;
                }
