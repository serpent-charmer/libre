...
public class ItemConnector extends ItemChromaTool {
    ...
    public boolean onItemUse(
        ...
        if (te instanceof NBTCopyable) {
            if (is.stackTagCompound != null
                && is.stackTagCompound.hasKey("NBT_transfer")) {
                ((NBTCopyable) te)
                    .readCopyableData(is.stackTagCompound.getCompoundTag("NBT_transfer"));
            ...
            WorldLocation loc
                = WorldLocation.readTag(is.stackTagCompound.getCompoundTag("callback"));
            if (loc != null) {
            ...
            if (is.stackTagCompound != null
                && is.stackTagCompound.getBoolean("noneuclid")) {
                ...
                if (tile instanceof TileEntityTeleport) {
                    ((TileEntityTeleport) tile).facing
                        = ForgeDirection.VALID_DIRECTIONS[face].getOpposite();
                    ((TileEntityTeleport) tile).destination = new BlockVector(
                        x - dx,
                        y + 1 - dy,
                        z - dz,
    ...
    private static boolean connectRift(
        LinkedTile te, World world, int x, int y, int z, ItemStack is, EntityPlayer ep
    ) {
        if (is.stackTagCompound == null) {
            is.stackTagCompound = new NBTTagCompound();
        ...
        if (x1 != Integer.MIN_VALUE && y1 != Integer.MIN_VALUE
            ...
            if (x1 != Integer.MIN_VALUE && y2 != Integer.MIN_VALUE
    ...
    private static boolean connectWindow(
        ...
        if (x1 != Integer.MIN_VALUE && y1 != Integer.MIN_VALUE
            ...
            if (x1 != Integer.MIN_VALUE && y2 != Integer.MIN_VALUE
                && z2 != Integer.MIN_VALUE) {
                TileEntityTransportWindow rf1
                    = (TileEntityTransportWindow) world.getTileEntity(x1, y1, z1);
                TileEntityTransportWindow rf2
                    = (TileEntityTransportWindow) world.getTileEntity(x2, y2, z2);

                //ReikaJavaLibrary.pConsole(rec+"\n"+em);
                if (rf1 == null) {
                    ReikaChatHelper.writeString(
                        "Tile missing at " + x1 + ", " + y1 + ", " + z1
                    );
                    is.stackTagCompound = null;
                    return false;
                }
                ...
                if (rf1.canLinkTo(rf2)) {
                    ...
                    ReikaChatHelper.sendChatToPlayer(
                        ep, "Cannot link windows; states/frames do not match."
                    );
                }
                is.stackTagCompound = null;
            }
    ...
    private static boolean trySimpleConnection(
        SimpleConnection te,
        World world,
        int x,
        int y,
    ...
    private static boolean tryConnection(
        Linkable te, World world, int x, int y, int z, ItemStack is, EntityPlayer ep
    ) {
        if (is.stackTagCompound == null) {
            is.stackTagCompound = new NBTTagCompound();
            new WorldLocation((TileEntity) te).writeToNBT("end1", is.stackTagCompound);
        ...
        if (te.tryConnect(world, te0.xCoord, te0.yCoord, te0.zCoord)
            && bb.tryConnect(world, x, y, z)) {
    ...
    private static <T> boolean tryCopySettings(
        CopyableSettings<T> te,
        World world,
        int x,
        int y,
        int z,
        ...
        if (rx != Integer.MIN_VALUE && ry != Integer.MIN_VALUE
            && rz != Integer.MIN_VALUE) {
            if (ex != Integer.MIN_VALUE && ey != Integer.MIN_VALUE
                && ez != Integer.MIN_VALUE) {
                CopyableSettings<T> em
                    = (CopyableSettings<T>) w1.getTileEntity(ex, ey, ez);
                CopyableSettings<T> rec
                    = (CopyableSettings<T>) w2.getTileEntity(rx, ry, rz);

                //ReikaJavaLibrary.pConsole(rec+"\n"+em);
                if (em == null) {
                    ReikaChatHelper.writeString(
                        "Tile missing at " + ex + ", " + ey + ", " + ez
                    );
                    is.stackTagCompound = null;
                    return false;
                }
                ...
                if (rec.copySettingsFrom((T) em)) {
                    //ReikaJavaLibrary.pConsole("connected", Side.SERVER);
                    ReikaChatHelper.sendChatToPlayer(
                        ep, "Copied settings from " + em + " to " + rec
                    );
                } else {
