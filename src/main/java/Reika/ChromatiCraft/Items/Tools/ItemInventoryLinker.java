...
public class ItemInventoryLinker extends ItemWithItemFilter {
    ...
    public void onUpdate(ItemStack is, World world, Entity e, int slot, boolean held) {
        if (!world.isRemote && e instanceof EntityPlayer && is.stackTagCompound != null
            && this.getMode(is) == Mode.REVERSED && is.stackTagCompound.hasKey("link")) {
                ...
                if (loc.isChunkLoaded()) {
                    ...
                    if (id != Blocks.air) {
                        ...
                        if (te instanceof IInventory) {
                            IInventory ii = (IInventory) te;
                            int look = (int
                            ) (world.getTotalWorldTime() % ii.getSizeInventory());
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        ...
        if (is.stackTagCompound != null) {
            if (is.stackTagCompound.hasKey("link")) {
                WorldLocation loc
                    = WorldLocation.readFromNBT("link", is.stackTagCompound);
                if (loc != null) {
    ...
    public static boolean tryLinkItem(EntityPlayer ep, ItemStack is) {
        //return MinecraftForge.EVENT_BUS.post(new EntityItemPickupEvent(ep, new
        //EntityItem(ep.worldObj, ep.posX, ep.posY, ep.posZ, is)));
        return parseInventoryForLinking(ep, is, ep.inventory.mainInventory, null);
    }

    private static boolean parseInventoryForLinking(
        EntityPlayer ep, ItemStack picked, ItemStack[] inv, ItemStack active
    ) {
        for (int i = 0; i < inv.length; i++) {
            if (active == null
                ...
                if (in != null && in.getItem() == ChromaItems.LINK.getItemInstance()) {
                    if (((ItemInventoryLinker) in.getItem())
                            .matchesItem(ep, in, picked)) {
    ...
    public String getActionName(EntityPlayer ep, ItemStack tool) {
        return this.getMode(tool) == Mode.REVERSED ? "Reversed flow direction"
                                                   : "Sending";
    }
}
