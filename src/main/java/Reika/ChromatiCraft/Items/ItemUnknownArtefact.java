...
public class ItemUnknownArtefact
    extends ItemChromaMulti implements AnimatedSpritesheet, MetadataSpecificTrade {
    //only directly obtainable when silk touched
    //effects: hurt entities when in inv (slowly); too many in chest -> UA bombing
    // (nearby RF storages empty, chest may drop items (except other UAs), maybe low-power
    // explosions
        ...
        if (ei.worldObj.isRemote) {
            ...
            if (ArtefactTypes.list[is.getItemDamage()].emitsLight()) {
                if (ei.worldObj.getBlock(x, y, z).isAir(ei.worldObj, x, y, z)) {
                    ei.worldObj.setBlock(x, y, z, ChromaBlocks.LIGHT.getBlockInstance(),
            Flags.DECAY.getFlag(), 3);
                }
            }
             */
    ...
    public boolean onItemUse(
        ...
        ReikaSoundHelper.playPlaceSound(
            world, x, y, z, ChromaBlocks.ARTEFACT.getBlockInstance()
        );
        return true;
    }
    ...
    public void onUpdate(ItemStack is, World world, Entity e, int slot, boolean held) {
        if (ArtefactTypes.list[is.getItemDamage()].damagesEntities()
            && rand.nextInt(1000) == 0) {
            if (rand.nextBoolean()) {
    ...
    @SideOnly(Side.CLIENT)
    public static void doUA_FX(
        World world, double posX, double posY, double posZ, boolean renderThroughGround
    ) {
        if (rand.nextBoolean()) {
    ...
    public static void onAddedToInventory(IInventory inv, ItemStack is) {
        if (ArtefactTypes.list[is.getItemDamage()].triggersUABombing()) {
            int amt = ReikaInventoryHelper.countItem(
                ChromaItems.ARTEFACT.getItemInstance(),
    ...
    private static void UABombing(Entity e) {
        //UABombingEffects.instance.trigger(e.worldObj, MathHelper.floor_double(e.posX),
    ...
    private static String getRenderText() {
        return ChromaFontRenderer.FontType.OBFUSCATED.id
            + renderTextNext.substring(0, panLocation)
            + renderText.substring(panLocation);
    }
    ...
    private static String generateNewRenderText() {
        ...
        while (sb.length() < RENDER_TEXT_LENGTH) {
            int len = Math.min(
                RENDER_TEXT_LENGTH - sb.length(),
                ReikaRandomHelper.getRandomBetween(
                    RENDER_TEXT_MIN_WORD_LENGTH, RENDER_TEXT_MAX_WORD_LENGTH
                )
            );
