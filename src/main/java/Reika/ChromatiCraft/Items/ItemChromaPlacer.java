...
public class ItemChromaPlacer extends Item implements ISize {
    public ItemChromaPlacer(int tex) {
        ...
        maxStackSize = 64;
        this.setCreativeTab(
            ChromatiCraft.instance.isLocked() ? null : ChromatiCraft.tabChroma
        );
    }
        ...
        if (select && !world.isRemote) {
            if (ChromaTiles.TEList[is.getItemDamage()].isRepeater()) {
                if (world.getTotalWorldTime()%40 == 0) {
                    ...
                    for (CrystalNetworkTile te :
    CrystalNetworker.instance.getNearTilesOfType(world, x, y, z,
    TileEntityCrystalRepeater.class, r)) {
    ...
    public boolean onItemUse(
        ...
        if (m.getBlock() == ChromaBlocks.DECOPLANT.getBlockInstance()) {
            TileEntityMagicPlant te
                = (TileEntityMagicPlant) m.createTEInstanceForRender(0);
        ...
        Block b = m.getBlock();
        if (b == ChromaBlocks.TILECRYSTAL.getBlockInstance()
            || b == ChromaBlocks.TILECRYSTALNONCUBE.getBlockInstance()) {
        ...
        if (m == ChromaTiles.CONSOLE) {
            ((TileEntityCrystalConsole) te).placedDir
                = ReikaEntityHelper.getDirectionFromEntityLook(ep, false).getOpposite();
        }
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        ...
        if (m == ChromaTiles.HEATLILY) {
            MovingObjectPosition mov
                = this.getMovingObjectPositionFromPlayer(world, ep, true);
            if (mov != null
                && mov.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
                int x = mov.blockX;
                int y = mov.blockY;
                int z = mov.blockZ;
                ...
                if (world.getBlock(x, y, z).getMaterial() == Material.water
                    ...
                    if (world.isAirBlock(x, y + 1, z)) {
                        world.setBlock(
                            x, y + 1, z, m.getBlock(), m.getBlockMetadata(), 3
                        );
                        if (!ep.capabilities.isCreativeMode)
                            --is.stackSize;
                    }
        ...
        for (int i = 0; i < ChromaTiles.TEList.length; i++) {
            ...
            if (c.isAvailableInCreativeInventory()) {
                ...
                if (c == ChromaTiles.METEOR) {
                    ...
                    for (int k = 0; k < CrystalTier.values().length; k++) {
                        ItemStack item2 = item.copy();
                        item2.stackTagCompound = new NBTTagCompound();
                        item2.stackTagCompound.setInteger("tier", k);
                        par3List.add(item2);
    ...
    protected boolean
    checkValidBounds(ItemStack is, EntityPlayer ep, World world, int x, int y, int z) {
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        ...
        if (r == ChromaTiles.GUARDIAN) {
            li.add(
                String.format("Protects a radius-%d area", TileEntityGuardianStone.RANGE)
            );
        }
        if (r == ChromaTiles.ADJACENCY) {
            li.add(
                EnumChatFormatting.GOLD
                + "This item is deprecated! Craft it into the new version!"
            );
        } /*
         if (r == ChromaTiles.ASPECTJAR && is.stackTagCompound != null &&
        ...
        if (r == ChromaTiles.FOCUSCRYSTAL) {
            if (ItemStack.areItemStackTagsEqual(
                    is, CrystalTier.TURBOCHARGED.getCraftedItem()
                ))
                li.add("Turbocharged");
        }
        if (r.isRepeater() && is.stackTagCompound != null
        ...
        if (is.stackTagCompound != null && is.stackTagCompound.hasKey("energy")
            && r.isLumenTile()) {
            ElementTagCompound tag = ElementTagCompound.createFromNBT(
                is.stackTagCompound.getCompoundTag("energy")
            );
            ...
            for (int i = 0; i < 16; i++) {
                ...
                String val = String.format(
                    "%d%s",
                    Math.round(ReikaMathLibrary.getThousandBase(amt)),
                    ReikaEngLibrary.getSIPrefix(amt)
                );
                String s
                    = e.getChatColorString() + val + EnumChatFormatting.RESET.toString();
                sb.append(s);
                if (i < 15) {
                    sb.append("/");
                }
    ...
    public String getItemStackDisplayName(ItemStack is) {
        ...
        String s = r.getName();
        if (r == ChromaTiles.FOCUSCRYSTAL) {
            String tier = CrystalTier
                              .tierList
                                  [is.stackTagCompound != null
                                       ? is.stackTagCompound.getInteger("tier")
                                       : 0]
                              .getDisplayName();
            if (!tier.isEmpty())
                s = tier + " " + s;
