    ...
    public boolean onItemUse(
        ...
        if (!ep.canPlayerEdit(x, y, z, 0, is))
            ...
            if (!ep.capabilities.isCreativeMode)
                --is.stackSize;
            world.setBlock(
                x, y, z, ChromaBlocks.ADJACENCY.getBlockInstance(), is.getItemDamage(), 3
            );
        }

        ReikaSoundHelper.playPlaceSound(
            world, x, y, z, ChromaBlocks.ADJACENCY.getBlockInstance()
        );
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        if (is.stackTagCompound != null) {
            ...
            if (is.getItemDamage() == CrystalElement.LIGHTBLUE.ordinal()) {
                ...
                li.add(
                    EnumChatFormatting.GOLD + "Admin Note:" + EnumChatFormatting.WHITE
                    + " Will not cause more than " + max + "ms lag."
                );
            }
                ...
                li.add(String.format(
                    "to %d per tick, with %.2f%% efficiency",
                    TileEntityHeatRelay.getIc2Max(tier),
                    TileEntityHeatRelay.getIc2Eff(tier) * 100
                ));
            }
        }
    }
