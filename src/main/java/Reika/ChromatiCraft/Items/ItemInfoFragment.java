...
public class ItemInfoFragment extends ItemChromaBasic implements SpriteRenderCallback {
            ...
            if (world.getTotalWorldTime() % n == slot % n) {
                ...
                if (r == null) {
                    if (is.stackTagCompound != null
                        && is.stackTagCompound.getBoolean(RANDOM_TAG))
                        this.programShardAndGiveData(is, ep);
                } else {
                    if (r.canPlayerProgressTo(ep)
                        && ChromaResearchManager.instance.getNextResearchesFor(ep)
                               .contains(r))
            ...
            if (getResearch(is) == null) {
                ...
                if (ei.worldObj.getBlock(x, y, z)
                    ...
                    if (ep != null && !ReikaPlayerAPI.isFake(ep)) {
                        ChromaResearch r =
                    ChromaResearchManager.instance.getRandomNextResearchFor(ep); if (r !=
                    null) { setResearch(is, r);
                        }
                    }*/
                }
            }
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        if (isBlank(is)
            && !(
                is.stackTagCompound != null && is.stackTagCompound.getBoolean(RANDOM_TAG)
            )
            && !ChromaResearchManager.instance.getNextResearchesFor(ep).isEmpty())
            ep.openGui(
                ChromatiCraft.instance, ChromaGuis.FRAGSELECT.ordinal(), world, 0, 0, 0
            );
        return is;
    }
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        if (this.isBlank(is)) {
            ...
            if (r == null) {
                li.add(
                    "[ERROR: NO RESEARCH FOR NAME '"
                    + is.stackTagCompound.getString("page") + "'"
                );
                return;
            }
            ...
            String s = read || has
                ? pre + " " + title
                : pre + " " + ChromaFontRenderer.FontType.OBFUSCATED.id + title;
            li.add(s);
    ...
    public boolean hasEffect(ItemStack is, int pass) {
        ...
        ChromaResearch r = this.getResearch(is);
        return r != null
            && ChromaResearchManager.instance.canPlayerStepTo(
                Minecraft.getMinecraft().thePlayer, r
            );
    }
    ...
    public static void programShardAndGiveData(ItemStack is, EntityPlayer ep) {
        ...
        if (r != null && !ep.worldObj.isRemote) {
            ...
            ReikaPacketHelper.sendDataPacket(
                ...
                ChromaPackets.GIVERESEARCH.ordinal(),
                (EntityPlayerMP) ep,
                r.ordinal()
            );
        }
    ...
    public boolean onRender(RenderItem ri, ItemStack is, ItemRenderType type) {
        if (type == ItemRenderType.INVENTORY) {
            ...
            if (r != null) {
                ...
                FragmentCategorization frag
                    = FragmentCategorizationSystem.instance.getCategories(r);
                if (frag != null && !GuiScreen.isCtrlKeyDown()
                    ...
                    for (FragmentCategory e : frag.set()) {
                     ...
                     10).toUpperCase();//String.format("%d", tag.getValue(e)); int color =
                     e.getColor() | 0xff000000; FontRenderer f =
                     Minecraft.getMinecraft().fontRenderer;
                         ReikaGuiAPI.instance.drawCenteredStringNoShadow(f, s,
                     (int)((x+w-0)/sc), (int)((y+w-6)/sc), color); GL11.glTranslated(1, 0,
                     0); ReikaGuiAPI.instance.drawCenteredStringNoShadow(f, s,
                     (int)((x+w-0)/sc), (int)((y+w-6)/sc), color); GL11.glPopMatrix();
                     }
                     else {*/
                        v5.startDrawingQuads();
                        int idx = (int
                                  ) (((System.identityHashCode(e) * 11
                                       + ReikaRenderHelper.getRenderFrame() / 6)
                                      % arr.length)
                                     + arr.length)
                            % arr.length;
                        v5.setColorOpaque_I(
                            ReikaColorAPI.mixColors(arr[idx], 0xffffff, 0.6F)
                        );
