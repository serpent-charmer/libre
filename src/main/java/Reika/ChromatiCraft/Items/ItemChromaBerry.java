...
public class ItemChromaBerry extends ItemCrystalBasic {
    ...
    private static final int BERRY_CHANCE_PER_EAT
        = (100 / ((BERRY_EAT_LIFE / 20) - BERRY_MIN_TRIGGER)) * 4;
            ...
            if (ei.worldObj.getBlockMetadata(x, y, z) == 0) {
                if (this.canCharge(ei)) {
                    ...
                    if (te instanceof TileEntityChroma) {
                        ...
                        int amt = tc.activate(
                            CrystalElement.elements[is.getItemDamage()], is.stackSize
                        );
                        //ReikaJavaLibrary.pConsole(amt+" from "+is.stackSize,
                        //Side.SERVER);
                        if (!ei.worldObj.isRemote)
                            is.stackSize -= amt;
                        //ReikaJavaLibrary.pConsole(ei.age+":"+amt+",
                        //"+df+">"+is.stackSize, Side.SERVER);
                        if (is.stackSize <= 0)
                            ei.setDead();
                        else
    ...
    {
        EntityChromaBerry ei = new EntityChromaBerry(world, location.posX, location.posY,
    location.posZ, itemstack); ei.motionX = location.motionX; ei.motionY =
    location.motionY; ei.motionZ = location.motionZ; ei.delayBeforeCanPickup = 10; return
    ei;
    }*/
    ...
    public ItemStack onEaten(ItemStack is, World world, EntityPlayer ep) {
        ...
        if (ReikaRandomHelper.doWithChance(20)) {
            if (PlayerElementBuffer.instance.hasElement(ep, e)
                && PlayerElementBuffer.instance.getPlayerFraction(ep, e) < 0.25) {
                PlayerElementBuffer.instance.addToPlayer(ep, e, 1, true);
            }
        }
        if (!ep.worldObj.isRemote) {
            int num = this.getOrCreateCounter(ep).addEntry(
                world.getTotalWorldTime(), BERRY_EAT_LIFE
            );
            double d
                = Math.min(1, ((num - BERRY_MIN_TRIGGER) * BERRY_CHANCE_PER_EAT) / 100D);
            ...
            if (num >= BERRY_MIN_TRIGGER && ReikaRandomHelper.doWithChance(d) || true) {
                ...
                int dur = tmax <= tmin ? tmin
                                       : ReikaRandomHelper.getRandomBetween(tmin, tmax);
