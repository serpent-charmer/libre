...
public class ItemT2EnderEye
    extends ItemChromaBasic implements AnimatedSpritesheet, OwnableItem {
    ...
    public boolean onItemUse(
        ...
        if (ret) {
            if (ReikaWorldHelper.checkForAdjBlock(world, x, y, z, Blocks.end_portal)
                != null) {
                is.stackSize = 12;
                is.animationsToGo = 5;
                ReikaSoundHelper.playSoundAtEntity(
                    ep.worldObj, ep, "random.pop", 1, 1.35F
                );
                if (world.isRemote)
                    ReikaSoundHelper.playClientSound(
                        "random.pop", ep.posX, ep.posY, ep.posZ, 1, 1.35F, true
                    );
            }
        }
        return ret;
    ...
    public String getTexture(ItemStack is) {
        return this.useAnimatedRender(is)
            ? "/Reika/ChromatiCraft/Textures/Items/miscanim.png"
            : super.getTexture(is);
    }
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        MovingObjectPosition mov
            = this.getMovingObjectPositionFromPlayer(world, ep, false);

        if (mov != null && mov.typeOfHit == MovingObjectType.BLOCK
            && world.getBlock(mov.blockX, mov.blockY, mov.blockZ)
                == Blocks.end_portal_frame) {
            return is;
        } else {
            if (!world.isRemote) {
                ChunkPosition pos = world.findClosestStructure(
                    "Stronghold", (int) ep.posX, (int) ep.posY, (int) ep.posZ
                );
                ...
                if (pos != null) {
                    ...
                    EntityEnderEyeT2 eye = EntityEnderEyeT2.create(
                        world,
                        ep.posX,
                        ep.posY + 1.62D - ep.yOffset,
                        ep.posZ,
                        is.stackTagCompound
                    );
    ...
    public boolean isUsableBy(ItemStack is, EntityPlayer ep) {
        return is.stackTagCompound == null
            || ep.getUniqueID().toString().equals(is.stackTagCompound.getString("owner"));
    }
