            ...
            switch (this) {
                case GROUND:
                    if (ReikaItemHelper.matchStackWithBlock(is, Blocks.dirt)
                        || ReikaItemHelper.matchStackWithBlock(is, Blocks.grass))
                        return true;
                    if (ReikaItemHelper.matchStackWithBlock(is, Blocks.sand)
                        || ReikaItemHelper.matchStackWithBlock(is, Blocks.gravel))
                        return true;
                case MOBDROPS:
                    if (i == Items.feather || i == Items.porkchop || i == Items.beef
                        || i == Items.chicken || i == Items.leather)
                        return true;
                    if (i == Items.string || i == Items.spider_eye
                        || i == Items.rotten_flesh || i == Items.gunpowder
                        || i == Items.bone || i == Items.arrow)
                        return true;
                    if (i == Items.bow || i == Items.ender_pearl || i == Items.skull
                        || i == Items.blaze_rod || i == Items.ghast_tear
                        || i == Items.magma_cream)
                        return true;
                    if (ReikaItemHelper.matchStacks(is, ReikaItemHelper.inksac))
                        return true;
                    if (ModList.THAUMCRAFT.isLoaded()) {
                        ItemStack brain = ReikaItemHelper.lookupItem(
                            ModList.THAUMCRAFT, "ItemZombieBrain", 0
                        );
                    ...
                    if (ModList.TINKERER.isLoaded()) {
                        ItemStack bone = ReikaItemHelper.lookupItem(
                            ModList.TINKERER, "materials", 8
                        );
