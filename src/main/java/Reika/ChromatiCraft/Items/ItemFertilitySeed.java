...
public class ItemFertilitySeed extends ItemChromaBasic implements BreakerCallback {
    ...
    public boolean onEntityItemUpdate(EntityItem ei) {
        ...
        if (ei.age < INITIAL_DELAY) {
            ...
            if (ei.worldObj.checkChunksExist(
                    x0 - 3, y0 - 3, z0 - 3, x0 + 3, y0 + 3, z0 + 3
                )) {
                ...
                for (int i = 0; i < ticks; i++) {
                    ...
                    if (this.canTickBlock(b, ei.worldObj.getBlockMetadata(x, y, z))) {
                        ...
                        ReikaPacketHelper.sendDataPacketWithRadius(
                            ...
                            ChromaPackets.FERTILITYSEED.ordinal(),
                            ei.worldObj,
                            x,
                            y,
                            z,
    ...
    public static void doFertilizeFX(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < n; i++) {
            double px = x + b.getBlockBoundsMinX()
                + (b.getBlockBoundsMaxX() - b.getBlockBoundsMinX()) * fxRand.nextDouble();
            double py = y + b.getBlockBoundsMinY()
                + (b.getBlockBoundsMaxY() - b.getBlockBoundsMinY()) * fxRand.nextDouble();
            double pz = z + b.getBlockBoundsMinZ()
                + (b.getBlockBoundsMaxZ() - b.getBlockBoundsMinZ()) * fxRand.nextDouble();
            int c = ReikaColorAPI.getModifiedHue(
                0x00ff00, ReikaRandomHelper.getRandomBetween(80, 150)
            );
    ...
    private boolean canTickBlock(Block b, int meta) {
        return b == Blocks.grass || b == Blocks.mycelium || b instanceof BlockSapling
            || ReikaCropHelper.isCrop(b) || ModCropList.isModCrop(b, meta);
    }
    ...
    canBreak(ProgressiveBreaker b, World world, int x, int y, int z, Block id, int meta) {
        return id == Blocks.dirt
            && world.getBlock(x, y + 1, z).getLightOpacity(world, x, y + 1, z) <= 2;
    }
