...
public class ItemLinkedTilePlacer extends Item {
    ...
    public boolean onItemUse(
        ...
        if (!ep.canPlayerEdit(x, y, z, 0, is))
            ...
            if (!ep.capabilities.isCreativeMode)
                --is.stackSize;
            world.setBlock(
                x, y, z, this.getBlock(), this.getMetadata(is.getItemDamage()), 3
            );
        }
    ...
    static void
    linkTile(World world, int x, int y, int z, ItemStack is, EntityPlayer ep) {
    ...
    static WorldLocation getRiftLocation(ItemStack is) {
        return is.stackTagCompound != null
            ? WorldLocation.readFromNBT("loc", is.stackTagCompound)
            : null;
    }
