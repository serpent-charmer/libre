...
public class ItemBlockCrystal extends ItemBlock {
    ...
    public final void
    getSubItems(Item par1, CreativeTabs par2CreativeTabs, List par3List) {
    ...
    public String getItemStackDisplayName(ItemStack is) {
        String name = ChromaBlocks.getEntryByID(field_150939_a)
    ...
    private String obfuscateIf(ItemStack is, String sg) {
        ...
        return ProgressionManager.instance.hasPlayerDiscoveredColor(ep, color)
            ? sg
            : ChromaFontRenderer.FontType.OBFUSCATED.id + sg;
    }
    ...
    public FontRenderer getFontRenderer(ItemStack is) {
        ...
        CrystalElement color = CrystalElement.elements[is.getItemDamage()];
        EntityPlayer ep = Minecraft.getMinecraft().thePlayer;
        return ProgressionManager.instance.hasPlayerDiscoveredColor(ep, color) ? null :
        ChromaFontRenderer.FontType.OBFUSCATED.renderer;
         */
        return null;
    }
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean par4) {
        ...
        PotionEffect eff
            = CrystalPotionController.instance.getEffectFromColor(color, 200, 0, false);
        PotionEffect neff
            = CrystalPotionController.instance.getEffectFromColor(color, 200, 0, true);
        boolean negative = eff != null
            ? ReikaPotionHelper.isBadEffect(Potion.potionTypes[eff.getPotionID()])
            : false;
        boolean nnegative = neff != null
            ? ReikaPotionHelper.isBadEffect(Potion.potionTypes[neff.getPotionID()])
                || neff.getPotionID() == Potion.nightVision.id
            : false;
        if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
            if (block.shouldGiveEffects(color)) {
                li.add(
                    "Effects: "
                    + CrystalPotionController.instance.getEffectName(
                        color, block.getPotionLevel(color) > 0
                    )
                );
                if (negative)
                    li.add("(Mobs only)");
                ...
                li.add(
                    "Nether Effects: "
                    + CrystalPotionController.instance.getNetherEffectName(color)
                );
                li.add(sg);
                li.add("");
