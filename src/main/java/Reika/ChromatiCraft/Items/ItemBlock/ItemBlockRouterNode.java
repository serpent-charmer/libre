...
public class ItemBlockRouterNode extends ItemBlockSidePlaced {
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        ...
        if (is.stackTagCompound == null) {
            li.add(
                "Must be linked to an " + ChromaTiles.ROUTERHUB.getName() + " to work"
            );
        } else {
    ...
    public boolean onItemUse(
        ...
        if (Coordinate.readFromNBT("target", stack.stackTagCompound) == null)
            return false;
        if (!(Coordinate.readFromNBT("target", stack.stackTagCompound)
                  .getTileEntity(world)
                  instanceof TileEntityRouterHub))
            return false;
    ...
    public boolean placeBlockAt(
        ...
        if (super.placeBlockAt(stack, ep, world, x, y, z, side, a, b, c, metadata)) {
            if (stack.stackTagCompound != null) {
                ...
                if (loc != null) {
                    ((BlockRouterNode) world.getBlock(x, y, z))
                        .setConnection(world, x, y, z, loc);
                }
            }
            return true;
        }
