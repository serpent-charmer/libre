...
public final class ElementTagCompound {
    ...
    public ElementTagCompound(int[] data) {
        ...
        if (data.length != 16)
            throw new IllegalArgumentException(
                "You must specify exactly sixteen elements!"
            );
        for (int i = 0; i < 16; i++) {
    ...
    public void addButMinimizeWith(ElementTagCompound tag) {
        ...
        for (CrystalElement e : tag.data.keySet()) {
            ...
            int value = this.getValue(e);
            int val
                = amt > 0 && value > 0 ? Math.min(amt, value) : (amt > 0 ? amt : value);
            this.setTag(e, val);
        }
        this.clearEmptyKeys();
    }
    ...
    public boolean equals(Object o) {
        return o instanceof ElementTagCompound
            && ((ElementTagCompound) o).data.equals(data);
    }
