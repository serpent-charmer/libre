...
public class MonumentCompletionRitual {
    ...
    private final long[] SOUND_TIMINGS
        = new long[] { 0, 28000, 66500, 86000, 104800, 123500 }; //in millis
    ...
    private static class RayNote {
        ...
        private RayNote(MusicKey k, int dur, boolean p, boolean b, boolean first) {
            key = k;
            startBeat = melody.isEmpty() ? 0
                                         : melody.get(melody.size() - 1).startBeat
                    + melody.get(melody.size() - 1).length;
            length = dur;
            percussion = p;
            bell = b;
        ...
        public String toString() {
            return key.toString() + " @ " + startBeat + "-" + length + " [P=" + percussion
                + "/B=" + bell + "/F=" + isFirstInPhrase + "]";
        }
    }
    ...
    public MonumentCompletionRitual(World world, int x, int y, int z, EntityPlayer ep) {
        ...
        packetTarget = new PacketTarget.RadiusTarget(
            world, x + 0.5, y + 0.5, z + 0.5, EFFECT_RANGE
        );
    ...
    private void stepSound() {
        //ReikaJavaLibrary.pConsole("Stepping sound to "+currentSound+" @ "+runTime+"
        //("+(runTime-startTime)+")");
        int idx = currentSound;
        SoundVariant s = ChromaSounds.MONUMENT.getVariant(String.valueOf(idx + 1));
        playingSound = ReikaSoundHelper.playClientSound(
            s, x + 0.5, y + 0.5, z + 0.5, 1, 1F, false
        );
        lastSoundStart = runTime;
        currentSound++;
        nextSoundTime = currentSound >= SOUND_TIMINGS.length
            ? Long.MAX_VALUE
            : runTime + SOUND_TIMINGS[currentSound] - SOUND_TIMINGS[idx];
        if (world.provider.dimensionId == ExtraChromaIDs.DIMID.getValue()) {
            //nextSoundTime += 600;
        }
    ...
    public void tick() {
        if (running) {
            ...
            //ReikaJavaLibrary.pConsole(time+" - "+startTime+" - "+pauseTotal+" =
            //"+runTime+" @ "+FMLCommonHandler.instance().getEffectiveSide());
            tick++;
    ...
    private boolean isReadyToComplete() {
        return runTime - SOUND_TIMINGS[SOUND_TIMINGS.length - 1]
            >= FINAL_SOUND_COMPLETION_DELAY
            - (world.provider.dimensionId == ExtraChromaIDs.DIMID.getValue()
                   ? FINAL_SOUND_COMPLETION_DELAY_EXTRA
                   : 0); //runTime >= SOUND_LENGTH_MILLIS;
    }
    ...
    private void doScriptedFX() {
        ...
        while (flag) {
            ...
            if (e.millis <= runTime) {
                ...
                if (e instanceof RayEvent) {
                    ...
                    this.onEvent(e.type);
                    //ReikaJavaLibrary.pConsole("Removing "+e.type+" @ "+e.millis+" at
                    //"+runTime);
                }
            } else {
        ...
        Set<CrystalElement> set = activeKey == null
            ? null
            : CrystalMusicManager.instance.getColorsWithKeyAnyOctave(activeKey);
        for (int i = 0; i < 16; i++) {
            CrystalElement e = CrystalElement.elements[i];
            colorFade[i] = set != null && set.contains(e)
                ? Math.min(1, colorFade[i] + 0.04F)
                : Math.max(0, colorFade[i] - 0.015F);
            TileEntityDimensionCore te = this.getCore(e);

            shaderPositions[i]
                = Vec3.createVectorHelper(te.xCoord + 0.5, y - 3.5, te.zCoord + 0.5);
            shaderColors[i] = new Vec4(
                e.getRed() / 255F,
                e.getGreen() / 255F,
                e.getBlue() / 255F,
                colorFade[i] * 0.7F
    ...
    private TileEntityDimensionCore getCore(CrystalElement e) {
        return (TileEntityDimensionCore) TileEntityDimensionCore.getLocation(e)
            .offset(x, y, z)
            .getTileEntity(world);
    }
    ...
    private void doRingFX() {
        ...
        for (int i = 0; i < particleRing.size(); i++) {
            ...
            p.particleSizeFactor
                = Math.min(1, p.particleSizeFactor + p.particleSizeFactorVelocity);
            p.particleSizeFactorVelocity
                = Math.max(-0.1F, p.particleSizeFactorVelocity - 0.05F);
            ...
            double r = 32.5
                - 2.5
                    * Math.abs(Math.sin(Math.toRadians(i * 2))
                    ); //29+2*Math.sin(Math.toRadians(tick-i));//32;//+p.particleSizeFactor*0-2.5*Math.abs(Math.sin(Math.toRadians(i*2)));
            ...
            float g = p.particleSizeFactor * p.particleSizeFactor * p.particleSizeFactor
                * (float) ReikaRandomHelper.getRandomPlusMinus(0, 0.125);
    ...
    private void manipulateCamera() {
        ...
        double ang
            = ((world.getTotalWorldTime() + player.getUniqueID().hashCode() % 8000) / 2D);
        double angPrev
            = ((world.getTotalWorldTime() + player.getUniqueID().hashCode() % 8000 - 1)
               / 2D);
        ...
        double cx = x + 0.5 + (R - r) * Math.cos(Math.toRadians(ang))
            + d * Math.cos(Math.toRadians((R - r) / r * ang));
        double cz = z + 0.5 + (R - r) * Math.sin(Math.toRadians(ang))
            - d * Math.sin(Math.toRadians((R - r) / r * ang));
        double cxPrev = x + 0.5 + (R - r) * Math.cos(Math.toRadians(angPrev))
            + d * Math.cos(Math.toRadians((R - r) / r * angPrev));
        double czPrev = z + 0.5 + (R - r) * Math.sin(Math.toRadians(angPrev))
            - d * Math.sin(Math.toRadians((R - r) / r * angPrev));
        double dd = ReikaMathLibrary.py3d(cx - x - 0.5, 0, cz - z - 0.5);
        double ddPrev = ReikaMathLibrary.py3d(cxPrev - x - 0.5, 0, czPrev - z - 0.5);
        double cy
            = y + 15 + 4 + 4 - 0.25 * dd + 4 * Math.sin(Math.toRadians(ang / 4)) - 2;
        double cyPrev = y + 15 + 4 + 4 - 0.25 * ddPrev
            + 4 * Math.sin(Math.toRadians(angPrev / 4)) - 2;
        double[] angs = ReikaPhysicsHelper.cartesianToPolar(
            cx - x - 0.5, cy - y - 0.5, cz - z - 0.5
        );
        double[] angsPrev = ReikaPhysicsHelper.cartesianToPolar(
            cxPrev - x - 0.5, cyPrev - y - 0.5, czPrev - z - 0.5
        );
        double pitch = 90 - angs[1];
        double pitchPrev = 90 - angsPrev[1];
        double yaw = -angs[2];
        double yawPrev = -angsPrev[2];
        ReikaRenderHelper.setCameraPosition(
            player,
            cx,
            cy,
            cz,
    ...
    public boolean doChecks() {
        ...
        for (int i = 0; i < CrystalElement.elements.length; i++) {
            CrystalElement e = CrystalElement.elements[i];
            Coordinate c = TileEntityDimensionCore.getLocation(e).offset(x, y, z);
            ChromaTiles t = ChromaTiles.getTileFromIDandMetadata(
                c.getBlock(world), c.getBlockMetadata(world)
            );
    ...
    private boolean doMineralChecks() {
        ...
        for (Coordinate c : map.keySet()) {
            ...
            if (b != map.get(c)) {
                //ReikaJavaLibrary.pConsole("@ "+c+": "+map.get(c).getLocalizedName()+",
                //"+b.getLocalizedName()); c.setBlock(world, map.get(c));
                return false;
            }
        }
        return true;
        ...
        if (runTime - completionTime < COMPLETION_EXTRA) {
            if (!complete)
                ReikaPacketHelper.sendDataPacket(
                    ...
                    ChromaPackets.MONUMENTCOMPLETE.ordinal(),
                    packetTarget,
                    x,
                    y,
                    z
            ...
            double[] angs = ReikaPhysicsHelper.cartesianToPolar(
                player.posX - x - 0.5, player.posY - y - 0.5, player.posZ - z - 0.5
            );
            double pitch = 90 - angs[1];
            double yaw = -angs[2];
            player.rotationYaw = player.rotationYawHead = player.prevRotationYawHead
            ...
            world.setBlock(
                ...
                y,
                z,
                ChromaTiles.AURAPOINT.getBlock(),
                ChromaTiles.AURAPOINT.getBlockMetadata(),
                3
            );
            ...
            if (!world.isRemote)
                ReikaPacketHelper.sendDataPacket(
                    ChromatiCraft.packetChannel,
                    ChromaPackets.RESETMONUMENT.ordinal(),
                    packetTarget,
                    x,
                    y,
                    z
    ...
    public void endRitual() {
        ...
        ChromaSounds.ERROR.playSoundNoAttenuation(
            world, x, y, z, 1, 0.75F, Integer.MAX_VALUE
        );
        if (world.isRemote)
            this.endSounds();
        else
    ...
    public static void completeMonumentClient(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityFloatingSeedsFX fx = (EntityFloatingSeedsFX
            ) new EntityCCFloatingSeedsFX(world, x + 0.5, y + 0.5, z + 0.5, phi, theta)
                                           .setColor(c)
                                           .setScale(6)
                                           .setLife(l);
        ...
        for (int i = 0; i < CrystalElement.elements.length; i++) {
            CrystalElement e = CrystalElement.elements[i];
            Coordinate c = TileEntityDimensionCore.getLocation(e).offset(x, y, z);
            TileEntityDimensionCore.createBeamLine(
                world, x, y, z, c.xCoord, c.yCoord, c.zCoord, CrystalElement.WHITE, e
            );
        }

        //ReikaSoundHelper.playClientSound(ChromaSounds.PYLONTURBO, x+0.5, y+0.5, z+0.5,
        //1, 2F, false);

        //ReikaSoundHelper.playClientSound(ChromaSounds.MONUMENTCOMPLETE, x+0.5, y+0.5,
        //z+0.5, 2, 0.9439F, false);
    ...
    private void doRays(RayNote n) {
        ...
        //ReikaSoundHelper.playClientSound(ChromaSounds.MONUMENTRAY, x+0.5, y+0.5, z+0.5,
        //1, p, false); ScheduledSoundEventClient se = new
        //ScheduledSoundEventClient(ChromaSounds.MONUMENTRAY, world, x+0.5, y+0.5, z+0.5,
        //1, p); se.attenuate = false; TickScheduler.instance.scheduleEvent(new
        //ScheduledTickEvent(se), 8);
        Collection<CrystalElement> set
            = CrystalMusicManager.instance.getColorsWithKeyAnyOctave(n.key);
        for (CrystalElement e : set) {
            //doRay(e, world, x, y, z, p);
            int time = ReikaRandomHelper.getRandomBetween(
                10, BEAT_LENGTH * n.length / 50 - 15
            );
            TickScheduler.instance.scheduleEvent(
                new ScheduledTickEvent(new ScheduledRayEvent(e, world, x, y, z, p)), time
            );
        }
    }
    ...
    private static class ScheduledRayEvent implements ScheduledEvent {
        ...
        public ScheduledRayEvent(
            CrystalElement e, World w, int x, int y, int z, float p
        ) {
            color = e;
            world = w;
            this.x = x;
    ...
    @SideOnly(Side.CLIENT)
    private static void
    doRay(CrystalElement e, World world, int x, int y, int z, float p) {
        ReikaSoundHelper.playClientSound(
            ChromaSounds.MONUMENTRAY,
            x + 0.5,
            y + 0.5,
            z + 0.5,
            (float) ReikaRandomHelper.getRandomBetween(0.4, 0.8),
            p,
            false
        );
        Coordinate c = TileEntityDimensionCore.getLocation(e).offset(x, y, z);
        DecimalPosition end
            = new DecimalPosition(c.xCoord + 0.5, c.yCoord + 0.5, c.zCoord + 0.5);
        LightningBolt b
            = new LightningBolt(new DecimalPosition(x + 0.5, y + 0.5, z + 0.5), end, 8);
        ...
        for (int i = 0; i < b.nsteps; i++) {
            ...
            for (double r = 0; r <= 1; r += 0.03125) {
                ...
                EntityFX fx = new EntityCCBlurFX(world, dd.xCoord, dd.yCoord, dd.zCoord)
                                  .setScale(s)
                                  .setColor(clr)
                                  .setLife(l)
                                  .setRapidExpand();
                Minecraft.getMinecraft().effectRenderer.addEffect(fx);
                EntityFX fx2 = new EntityCCBlurFX(world, dd.xCoord, dd.yCoord, dd.zCoord)
                                   .setScale(s / 2.5F)
                                   .setColor(0xffffff)
                                   .setLife(l)
                                   .setRapidExpand();
    ...
    public static enum EventType {
        ...
        public void doEventClient(World world, int x, int y, int z) {
            switch (this) {
                case FLARES: {
                    //ReikaSoundHelper.playClientSound(ChromaSounds.USE, x+0.5, y+0.5,
                    //z+0.5, 1, 2F, false);
                ...
                case PARTICLECLOUD: {
                    //ReikaSoundHelper.playClientSound(ChromaSounds.CAST, x+0.5, y+0.5,
                    //z+0.5, 1, 0.5F, false);
                    ...
                    for (int i = 0; i < n; i++) {
                        ...
                        double dx = x + 0.5
                            + r
                                * Math.cos(ang
                                ); //ReikaRandomHelper.getRandomPlusMinus(x+0.5, 20);
                        double dz = z + 0.5
                            + r
                                * Math.sin(ang
                                ); //ReikaRandomHelper.getRandomPlusMinus(z+0.5, 20);
                        int h = rand.nextInt(rand.nextBoolean() ? 6 : 12);
                        double dy = 1
                            + world.getTopSolidOrLiquidBlock(
                                MathHelper.floor_double(dx), MathHelper.floor_double(dz)
                            )
                            + h;
                        int l = ReikaRandomHelper.getRandomBetween(
                            120, 240
                        ); //20+rand.nextInt(40);
                        float s = (float) ReikaRandomHelper.getRandomBetween(
                            4, 12D
                        ); //2+rand.nextFloat()*12;
                        int c = ReikaColorAPI.getModifiedHue(0xff0000, (int) (angd));
                        c = ReikaColorAPI.mixColors(
                            c, 0xffffff, Math.min(1, (float) (fr * fr * 4))
                        );
                        EntityCCFloatingSeedsFX fx
                            = new EntityCCFloatingSeedsFX(world, dx, dy, dz, 0, 90);
                        fx.angleVelocity *= 7;
                        fx.freedom *= 1.5;
                        fx.tolerance *= 0.33;
                ...
                case TWIRL: {
                    ...
                    for (int i = 0; i <= 8; i += 4) {
                        for (double a = 0; a < 360; a += 30 / (1 + i / 4)) {
                            ...
                            SpiralMotionController m = new SpiralMotionController(
                                x + 0.5,
                                z + 0.5,
                                4 - i / 4D,
                                (0.25 * (1 + i / 4D)) / 4D,
                                r,
                                0.0625 * 2 /*+((a/120)%1D)*0.3*/,
                                ang
                            );
                            ColorController clr = new AngleColorController(
                                x + 0.5,
                                z + 0.5,
                                5 - i / 4D,
                                (0.25 * (1 + i / 4D)) / 4D,
                            ...
                            EntityFX fx = new EntityCCBlurFX(world, dx, dy, dz)
                                              ...
                                              .setLife(l)
                                              .setRapidExpand()
                                              .setMotionController(m)
                                              .setPositionController(m)
                                              .setColorController(clr);
                            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
                        }
                    }
                    break;
