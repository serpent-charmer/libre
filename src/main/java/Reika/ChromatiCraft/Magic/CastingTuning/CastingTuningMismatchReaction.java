...
public class CastingTuningMismatchReaction {
    ...
    public CastingTuningMismatchReaction(TileEntityCastingTable te, EntityPlayer ep) {
        ...
        trigger = ep;
        fixedRand.setSeed(
            ChunkCoordIntPair.chunkXZ2Int(te.xCoord, te.zCoord) ^ te.yCoord
        );
    ...
    public boolean tick() {
        ...
        if (fixedRand.nextInt(50) == 0) {
            ...
            List<EntityLivingBase> li
                = tile.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, box);
            EntityLivingBase e = ReikaJavaLibrary.getRandomListEntry(fixedRand, li);
            CrystalElement c = ReikaJavaLibrary.getRandomListEntry(
                fixedRand,
                Arrays.asList(
            ...
            double dy
                = ReikaRandomHelper.getRandomBetween(tile.yCoord + 2.5, tile.yCoord + 9);
        ...
        if (shockwaveCooldown > 0) {
            ...
            ChromaSounds.KILLAURA_CHARGE.playSoundAtBlockNoAttenuation(
                tile, 1, 0.5F + 1.5F * shockwaveCharge * 2, 128
            );
    ...
    private void doParticles() {
        ...
        for (double br = 2.5; br <= 7.5; br += 2.5) {
            for (double a = 0; a < 360; a += 30) {
                ...
                float s = (float) ReikaRandomHelper.getRandomBetween(7.5, 15) * 1.5F
                    * (float) Math.sqrt(br / 7.5);
                int c = ReikaColorAPI.getModifiedHue(
                    0xff0000, ReikaRandomHelper.getRandomBetween(22, 45)
                );
                c = ReikaColorAPI.getColorWithBrightnessMultiplier(
                    c, (float) ReikaRandomHelper.getRandomBetween(0.4, 0.9)
                );
                ...
                SpiralMotionController m = new SpiralMotionController(
                    tile.xCoord + 0.5, tile.zCoord + 0.5, ra, vv, r, 0, ang
                );
    ...
    private void doShockwaveParticles() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            float g = (float) ReikaRandomHelper.getRandomBetween(
                v * 0.5, v
            ); //(float)ReikaRandomHelper.getRandomBetween(0.0625, 0.5);
            EntityCCBlurFX fx = new EntityCCBlurFX(
                tile.worldObj,
                tile.xCoord + 0.5,
                tile.yCoord + 1,
            ...
            MathExpression e = new PeriodicExpression()
                                   .addWave(1, 1, d)
                                   .addWave(0.5, 2, d + 90)
                                   .addWave(0.125, 4, d)
                                   .normalize();
