...
public class RosettaStone {
    ...
    private List<String> getData() {
        /*
        InputStream in1 =
        ChromatiCraft.class.getResourceAsStream("Resources/rosetta.txt"); InputStream in2
        = ChromatiCraft.class.getResourceAsStream("Resources/lore.xml"); File f1 = new
        File(DragonAPICore.getMinecraftDirectory(), "flippedrosetta.txt"); File f2 = new
        File(DragonAPICore.getMinecraftDirectory(), "flippedlore.xml"); File f3 = new
        File(DragonAPICore.getMinecraftDirectory(), "flippedunflipped.xml");
        ...
        if (LoreScripts.instance.hasReroutePath()) {
            return ReikaFileReader.getFileAsLines(
                LoreScripts.instance.getReroutedRosettaFile(), true, Charsets.UTF_8
            );
        } else {
            try {
                ...
                throw new RegistrationException(
                    ChromatiCraft.instance, "Could not load rosetta text", e
                );
            }
        }
    }
    ...
    public void render(ScaledResolution res, double w2, double h2) {
        ...
        int color
            = 0xffffff; //ReikaColorAPI.mixColors(0xb0e0ff, 0x7acfff,
                        //(float)(0.5+0.5*Math.sin(System.currentTimeMillis()/2000D)));
            ...
            for (int i = 0; i < s.length(); i++) {
                ...
                if (c == ' ') {
                    ...
                    if (decodableCharacters.contains(c)
                        && rand.nextInt(20) > 0) { //only render some chars, and still
                                                   //scramble a handful of decodables
                        f.drawString(
                            String.valueOf(c), dx, y, color | (Math.max(4, alpha) << 24)
                        );
    ...
    private void renderSmudge(char c, int x, int y) {
        //Minecraft.getMinecraft().fontRenderer.drawString(EnumChatFormatting.OBFUSCATED.toString()+c,
        //x, y, 0x000000);
        for (int i = 0; i < 4; i++) {
            c = (char) ('a' + rand.nextInt(26));
            int a = Math.max(4, (int) (alpha * (0.5 + 0.5 * rand.nextDouble())));
            Minecraft.getMinecraft().standardGalacticFontRenderer.drawString(
                String.valueOf(c), x, y, 0xffffff | (a << 24)
            );
        }
    }
    ...
    public void test() {
        ...
        if (text.isEmpty())
            throw new RegistrationException(
                ChromatiCraft.instance, "Could not load loretext!"
            );
    }
