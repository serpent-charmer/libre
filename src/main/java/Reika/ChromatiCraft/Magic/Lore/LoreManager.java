...
public class LoreManager implements ProgressAccess {
    ...
    public void triggerLore(EntityPlayer ep, Towers t) {
        ...
        if (ep instanceof EntityPlayerMP) {
            ...
            ReikaPacketHelper.sendDataPacket(
                ...
                ChromaPackets.LORENOTE.ordinal(),
                (EntityPlayerMP) ep,
                t.ordinal()
            );
        }
    ...
    public Towers getNearestTower(World world, double x, double z) {
        ...
        for (int i = 0; i < Towers.towerList.length; i++) {
            ...
            double d2 = p2 != null ? p2.getDistanceTo(x, p2.yCoord, z)
    ...
    public Towers
    getNearestActiveTower(World world, double x, double z, EntityPlayer ep) {
        this.initTowers(world);
        Towers ret = null;
        double mind = Double.POSITIVE_INFINITY;
        for (int i = 0; i < Towers.towerList.length; i++) {
    ...
    public void completeBoard(EntityPlayer ep) {
        ...
        if (ep.worldObj.isRemote) {
            ...
            ReikaPacketHelper.sendDataPacket(
                ChromatiCraft.packetChannel,
                ChromaPackets.LOREPUZZLECOMPLETE.ordinal(),
                PacketTarget.server
            );
        } else {
            ReikaPlayerAPI.syncCustomData((EntityPlayerMP) ep);
    ...
    public void sendTowersToClient(EntityPlayerMP ep) {
        ...
        for (Towers t : Towers.towerList) {
            ...
            tag.setInteger(
                "x", ~p.chunkXPos
            ); //very weak but nontrivial-if-manual encryption
        ...
        ReikaPacketHelper.sendNBTPacket(
            ChromatiCraft.packetChannel,
            ChromaPackets.TOWERLOC.ordinal(),
            data,
