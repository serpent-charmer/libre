...
public class KeyAssemblyPuzzle {
    ...
    private static final int BASE_SIZE
        = new HexGrid(SIZE, CELL_SIZE, true, MapShape.HEXAGON).flower().cellCount();

    public static final int TOWER_COUNT
        = 13; //arranged in a circle http://i.imgur.com/M8cC88i.png
    ...
    //public static final int TOTAL_CELLS = ReikaMathLibrary.roundDownToX(TOWER_COUNT,
    //BASE_SIZE);
    private static final int VOID_COUNT
        = BASE_SIZE - REQUIRED_CELLS; //for 15->169, =13 //for 91, = 11
    ...
    public static long calcSeed(EntityPlayer ep) {
        ...
        long seed2 = ReikaMathLibrary.cantorCombine(
            seed1, seed0.getMostSignificantBits(), seed0.getLeastSignificantBits()
        );
        long seed3
            = seed2; //CANNOT DO ON CLIENT// ^
                     //ReikaWorldHelper.getCurrentWorldID(ep.worldObj).worldCreationTime;
        return seed3;
    }
    ...
    private void generate(
        EntityPlayer ep, long seed
    ) { //fill with valid board; delete some cells, shuffle
        this.seed = seed;
        rand.setSeed(seed);
        activeCells = 0;
        groups.clear();
        ...
        while (!flag) {
            ...
            ChromatiCraft.logger.log(
                "Attempted to generate lore puzzle; attempt #" + attempts + " took " + dur
                + " ms. Success: " + flag
            );
            if (dur > 5000) {
                ChromatiCraft.logger.logError(
                    "Could not generate lore puzzle within 5s, even after " + attempts
                    + " attempts!"
                );
                this.errorGrid();
                return;
    ...
    private void errorGrid() {
        isErrored = true;
        for (HexCell c : cells.values()) {
            c.occupant = new HexTile(
                rand.nextBoolean() ? CrystalElement.BLACK : CrystalElement.MAGENTA
            );
            this.setCellContents(c.location, c.occupant, null);
            c.occupant.update(this, true, null);
        }
            ...
            if (!li.isEmpty()) {
                try {
                    ...
                    ReikaJavaLibrary.pConsole(
                        "Errored on shuffle " + i + ": moving " + h2 + " into " + h
                        + ", dir = " + grid.getOppositeDirection(dir) + ", from " + dir
                        + "; c=" + this.getCell(h)
                    );
                    //e.printStackTrace();
    ...
    private LinkedList<Hex> findHamiltonianPath(Random rand) {
        ...
        while (flag) {
            ...
            long dur = System.currentTimeMillis() - time;
            ChromatiCraft.logger.log(
                "Attempted to path lore puzzle; attempt #" + attempts + "; took " + dur
                + " ms so far. Success: " + !flag
            );
            if (dur > 5000) {
                ChromatiCraft.logger.logError(
                    "Could not path lore puzzle within 5s, even after " + attempts
                    + " attempts!"
                );
                return null;
            }
    ...
    private boolean
    pathSearch(Random rand, Hex h, LinkedList<Hex> path, HashSet<Hex> pathCache) {
        path.add(h);
        pathCache.add(h);
    ...
    private boolean isValidPathCell(Hex h, HashSet<Hex> pathCache) {
        return grid.containsHex(h) && !pathCache.contains(h)
            && this.getCell(h).occupant != null
            && !grid.dividesGrid(
                h, ReikaJavaLibrary.combineCollections(pathCache, emptyHexes)
            );
    }
    ...
    private CrystalElement generateColor(Hex h, Random rand) {
            ...
            if (grid.containsHex(h2)) {
                ...
                if (c.occupant != null) {
                    Collection<CrystalElement> ce
                        = ElementMixer.instance.getRelatedColors(c.occupant.color);
                    if (ce != null)
                        li.addAll(ce);
                }
            }
        ...
        return li.isEmpty()
            ? CrystalElement.elements[rand.nextInt(16)]
            : new ArrayList<CrystalElement>(li).get(rand.nextInt(li.size()));
    }

    private void
    drawTexturedHex(Tessellator v5, Hex h, double sizeX, double sizeY, float f) {
        ...
        if (f < 3) {
            ...
            int clr
                = f >= 2 ? 0xffffff : ReikaColorAPI.mixColors(0xffffff, 0x000000, f - 1);
    ...
    public void render(Tessellator v5, EntityPlayer ep, ScaledResolution res) {
        ...
        if (isErrored) {
            ...
            int c = ReikaColorAPI.mixColors(
                0xffffff,
                0,
                (float) (0.5 + 0.5 * Math.sin(System.currentTimeMillis() / 500D))
            );
        ...
        if (!flag2) {
            ...
            ReikaGuiAPI.instance.drawCenteredStringNoShadow(Minecraft.getMinecraft().fontRenderer,
            String.valueOf(f), x*2+16, y*2+8, 0xffffff); GL11.glPopMatrix();
                 */
                ...
                if (flag && f >= 1) {
					...
					GL11.glPopMatrix();
					 */
                    //this.drawTexturedHex(v5, h, grid.getGridProperties().sizeX,
                    //grid.getGridProperties().sizeY, f);
                } else {
    ...
    public static class HexCell {
        ...
        public void onClick(KeyAssemblyPuzzle p, double dx, double dy, EntityPlayer ep) {
            if (occupant != null) {
                if (this.isPointOnEdge(dx, dy)) {
                    ...
                    //ReikaJavaLibrary.pConsole(a+" > "+dir+" > "+this.getNeighbor(p,
                    //dir).occupant);
            ...
            //ReikaJavaLibrary.pConsole(p.isComplete()+":
            //"+p.activeCells+"/"+p.totalCells);
        }
        ...
        public void
        render(KeyAssemblyPuzzle p, Tessellator v5, boolean inBoard, float f) {
            GL11.glPushMatrix();
            if (inBoard) {
            ...
            if (inBoard) {
                if (isHovered != null || flashTick > 0) {
                    ...
                    if (flashTick > 0) {
                        ...
                        if (occupant != null
                            && this.isPointOnEdge(isHovered.x, isHovered.y)) {
                            double a
                                = -Math.toDegrees(Math.atan2(isHovered.y, isHovered.x));
                            int dir = p.grid.getNeighborDirection(a);
                            Point p1 = null;
                            Point p2 = null;
                            Point p3 = null;
                            ...
                            if (p1 != null) {
                                ...
                                int c = ReikaColorAPI.mixColors(
                                    0x7f7f7f,
                                    0xffffff,
                                    0.5F + 0.5F * MathHelper.sin((frame / 12F) % 360)
                                );
                        ...
                        for (int i = 0; i < 6; i++) {
                            if (Keyboard.isKeyDown(
                                    i == 0 ? Keyboard.KEY_0 : Keyboard.KEY_1 + i - 1
                                )) {
        ...
        public float getSolveFactor() { //0-10 (0-1) fade to black; 10-20 (1-2) fade from
                                        //black to white; 20-30 (2-3) fade from white
            if (solveTick == 0 || this.solveTick() <= 0)
                return 0;
            int tick = this.solveTick();
            return tick / 20F; //tick <= 10 ? tick/10F : tick <= 20 ? 1+(tick-10)/10F :
                               //2+(tick-20)/10F;
        }
        ...
        public void tickSolve() {
            if (solveTick > 0
                && solveTick
                    < /*30*/ 20 - solveOffset /* && GuiScreen.isCtrlKeyDown()*/) {
                solveTick++;
            }
        }
    ...
    private static class HexTile {
                ...
                if (queuedState) {
                    ...
                    if (ep != null) {
                        ...
                        ReikaSoundHelper.playClientSound(
                            ChromaSounds.CAST, ep, 0.5F, 1.5F
                        );
                    }
                } else {
                    p.activeCells--;
                    if (ep != null) {
                        ReikaSoundHelper.playClientSound(
                            ChromaSounds.RIFT, ep, 0.4F, 0.875F
                        );
                        ReikaSoundHelper.playClientSound(
                            ChromaSounds.RIFT, ep, 0.4F, 0.5F
                        );
                    }
                }
                state = queuedState;
            ...
            for (Hex h : location.location.getNeighbors()) {
                ...
                if (c != null && c.occupant != null) {
                    if (this.isRelatedTo(c.occupant)) {
                        //ReikaJavaLibrary.pConsole(color+" & "+c.occupant.color+" @
                        //"+location, color == CrystalElement.WHITE);
                        return true;
                    }
                }
            }
        ...
        render(KeyAssemblyPuzzle p, Tessellator v5, boolean inBoard, float f) {
            ...
            if (!inBoard || location.playerKnows(ep)) {
                ...
                double v
                    = (6 + color.ordinal() / 8 * 64 * 2 + (!inBoard || state ? 64 : 0))
                    / 256D;
        ...
        public boolean move(KeyAssemblyPuzzle p, int direction, EntityPlayer ep) {
            if (!p.grid.getValidMovementDirections(location.location)
                     .contains(direction)) {
                if (ep != null) {
                    location.flash();
            ...
            //ReikaJavaLibrary.pConsole(location.location+" > "+direction+" into
            //"+p.getCell(location.getNeighbor(p, direction).location).occupant.color);
            HexCell target = location.getNeighbor(p, direction);
            if (target == null) {
                throw new RuntimeException(
                    location.location.getNeighbor(direction).toString()
                    + " has null cell yet was moved to?!"
                );
            }
            ...
            ReikaSoundHelper.playClientSound(
                ...
                ep,
                0.5F,
                CrystalMusicManager.instance.getScaledDing(color, 3)
            );
        ...
        private void update(KeyAssemblyPuzzle p, boolean adj, EntityPlayer ep) {
            ...
            //ReikaJavaLibrary.pConsole(location.location+": "+color+", "+state+" >
            //"+flag);
            if (adj) {
                for (Hex h : location.location.getNeighbors()) {
                    HexCell c = p.getCell(h);
    ...
    private static class Move {
        ...
        private Move invert(HexGrid grid) {
            return new Move(
                hex.getNeighbor(direction), grid.getOppositeDirection(direction)
            );
        }
    }
}
