...
public enum Towers {
    ...
    public static void loadPositions(
        World world, double radius
    ) { //radius is in chunks, and is the size of a hex in "chunks as pixels"
        towerChunkCache.clear();
        towerCache.clear();

        double r = radius
            * MathHelper.clamp_double(DragonOptions.WORLDSIZE.getValue() / 6000D, 0.5, 5);
        HexGrid grid = new HexGrid(9, r, true, MapShape.HEXAGON).flower();

        Random rand = new Random(
            world.getSeed() ^ ReikaWorldHelper.getCurrentWorldID(world).worldCreationTime
        );
        rand.nextBoolean();
        rand.nextBoolean();
        ...
        for (int i = 0; i < towerList.length; i++) {
            ...
            //ReikaJavaLibrary.pConsole("Poscalc: Tower "+t+" @ "+t.position+" from seed
            //"+world.getSeed());
        }
