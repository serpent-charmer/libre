...
public class LoreScriptRenderer {
    ...
    public void startRendering(boolean additive, int color) {
        ...
        spaceColor = additive
            ? ((alpha << 24)
               | ReikaColorAPI.getColorWithBrightnessMultiplier(color, 0.375F))
            : ReikaColorAPI.getColorWithAlpha(color, 0.375F * alpha);
        lineColor = additive
            ? ((alpha << 24) | ReikaColorAPI.getColorWithBrightnessMultiplier(color, 0.5F)
            )
            : ReikaColorAPI.getColorWithAlpha(color, 0.5F * alpha);
    ...
    public void
    renderLine(String s, double x, double y, double width, boolean flipVertical) {
        ...
        if (parts.length <= 1) {
            ...
            double sp
                = (width - chars * (CHAR_WIDTH + CHAR_SPACING)) / (parts.length - 1);
            for (int i = 0; i < parts.length; i++) {
                ...
                if (i != parts.length - 1) {
                    ...
                    //ReikaGuiAPI.instance.drawRectFrame((int)dx+CHAR_WIDTH+1, (int)y+2,
                    //(int)sp-1, CHAR_WIDTH-3, 0xff000000);
                    ...
                    for (int dw = 0; dw < sp-2; dw += 4) {
                        ReikaGuiAPI.instance.drawLine(dx2+dw, dy2, dx2+dw, dy2+h,
                    0xff000000);
                    }
                     */
                    ReikaGuiAPI.instance.drawLine(
                        dx2, dy2 - 1, dx2 + sp - 2, dy2 - 1, spaceColor
                    );
                    ReikaGuiAPI.instance.drawLine(
                        dx2,
                        dy2 + h / 2 + 1 + 0.225,
                    ...
                    //ReikaGuiAPI.instance.drawLine_Double(dx2, dy2+h/4+0.5, dx2+sp-2,
                    //dy2+h/4+0.5, c);
                    GL11.glPopAttrib();
                    GL11.glLineWidth(f);
                }
        ...
        GL11.glLineWidth(2);
        ReikaGuiAPI.instance.drawLine(
            x + CHAR_WIDTH, y + 1.5, x - width + CHAR_WIDTH - 0.5, y + 1.5, lineColor
        );
        ReikaGuiAPI.instance.drawLine(
            x + CHAR_WIDTH,
            y - 0.5 + CHAR_WIDTH,
            x - width + CHAR_WIDTH - 0.5,
            y - 0.5 + CHAR_WIDTH,
            lineColor
        );
        GL11.glLineWidth(f);
