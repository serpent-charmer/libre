...
public class ToolChargingSystem {
               ...
               1
                   + 100
                       * ReikaMathLibrary.cosInterpolation(
                           0, this.getItem(is).getMaxCharge(), get
                       )
           ));
    ...
    private void setCharge(ItemStack is, int amt) {
        ...
        if (this.getItem(is).hasChargeStates())
            is.setItemDamage(
                this.getItem(is).getChargeState(this.getCharge(is) / (float) max)
            );
    }
    ...
    public void tryChargeFromPlayer(ItemStack is, EntityPlayer ep) {
        if (is.stackTagCompound == null ||
    !is.stackTagCompound.getBoolean("bufferlinked")) return; if (this.getCharge(is) >=
    this.getItem(is).getMaxCharge()/2) return; float extr =
    this.getItem(is).getPlayerBufferExtractionValue(is); if (extr > 0) { CrystalElement ec
    = this.getItem(is).getColor(is); int get =
    PlayerElementBuffer.instance.getPlayerContent(ep, ec); if (get > 0) { int add =
    this.addCharge(is, (int)(get*extr)); if (add > 0)
                    PlayerElementBuffer.instance.removeFromPlayer(ep, ec, Math.max(1,
    (int)(add/extr)));
            }
        }
    }
        ...
        if (!ei.worldObj.isRemote) {
            if (charge < pi.getMaxCharge()) {
                ...
                TemporaryCrystalReceiver r = new TemporaryCrystalReceiver(
                    loc, 0, CHARGE_RANGE, 0.0625, ResearchLevel.ENDGAME
                );
                CrystalElement e = pi.getColor(is);
                r.addColorRestriction(e);
                int amt = this.getChargeRate(is);
                //CrystalSource s = CrystalNetworker.instance.findSourceWithX(r, e, amt,
                //range, true);
                CrystalSource s = CrystalNetworker.instance.getNearestTileOfType(
                    r, CrystalSource.class, CHARGE_RANGE
                );
                if (s != null && s.isConductingElement(e)) {
                    ...
                    if (rate > 0) {
                        ...
                        ReikaPacketHelper.sendEntitySyncPacket(
                            DragonAPIInit.packetChannel, ei, 32
                        );
                        //ReikaJavaLibrary.pConsole(this.getCharge(is)+"
                        //(+"+this.getChargeRate(is)+",
                        //f="+(this.getCharge(is)/(float)MAX_CHARGE));
                    }
                }
                r.destroy();
            }
    ...
    private int scaleChargeRate(
        int amt, EntityItem ei, ItemStack is, CrystalSource s, WorldLocation loc
    ) {
        ...
        if (loc.getBlock() == ChromaBlocks.CHROMA.getBlockInstance()
            && loc.getBlockMetadata() == 0)
            amt *= 1.25;
        if (TileEntityAuraPoint.isPointWithin(
                loc.getWorld(), loc.xCoord, loc.yCoord, loc.zCoord, 256
            ))
            amt *= 2;
        return amt;
    }
    ...
    public void renderItemAux(RenderItem ri, ItemStack is, ItemRenderType type) {
        if (type == ItemRenderType.INVENTORY
            ...
            int c2 = e == CrystalElement.BLACK
                ? c1
                : ReikaColorAPI.mixColors(c, 0x000000, 0.75F);
    ...
    public static class EntityChargingTool
        extends EntityItem implements ChunkLoadingEntity {
        ...
        public EntityChargingTool(
            World world, double posX, double posY, double posZ, ItemStack is
        ) {
