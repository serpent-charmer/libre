...
public class TunnelNukerSpawner implements TickHandler {
    ...
    public void tick(TickType type, Object... tickData) {
        ...
        if (!world.isRemote && !world.playerEntities.isEmpty()
            ...
            ) world.playerEntities.get(world.rand.nextInt(world.playerEntities.size()));

            ArrayList<Integer> li = ReikaJavaLibrary.makeIntListFromArray(
                ReikaArrayHelper.getLinearArray(Towers.towerList.length)
            );
            ...
            double ey = 7.5
                + world.getTopSolidOrLiquidBlock(
                    MathHelper.floor_double(ex), MathHelper.floor_double(ez)
                );
            ReikaJavaLibrary.pConsole(
                "Spawning tunnel nuker for tower " + t + " @ " + ex + ", " + ez
            );
