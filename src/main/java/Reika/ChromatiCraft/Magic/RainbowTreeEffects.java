...
public class RainbowTreeEffects {
    ...
    private static final NBTIO<PersistentRainbowTreeFX> converter
        = new NBTIO<PersistentRainbowTreeFX>() {
              @Override
    ...
    private File getFile(World world) {
        return new File(
            ...
            "ChromatiCraft_Data/RainbowTreeFX.dat"
        );
    }
    ...
    public void dissolveDecay(BlockTickEvent evt) {
        if (!evt.world.isRemote
            && evt.getBlock() == MystCraftHandler.getInstance().decayID) {
            if (this.isCoordinateClearing(
    ...
    private void loadIfNecessary(World world) {
        ...
        if (!persistent.containsKey(world.provider.dimensionId)) {
            persistent.put(
                world.provider.dimensionId, new PersistentRainbowTreeFX(world)
            );
        }
    }
    ...
    public void addDecayClearing(
        ...
        this.loadIfNecessary(world);
        PersistentRainbowTreeFX fx = persistent.get(world.provider.dimensionId);
        fx.decayCleaning.put(
            new BlockBox(minX, minY, minZ, maxX, maxY, maxZ),
            duration >= 0 ? world.getTotalWorldTime() + duration : -1
        );
        this.save(world);
    }

    public void doRainbowTreeEffects(
        World world,
        int x,
        int y,
        int z,
        ...
        if (!world.isRemote) {
            ...
            if (spreadForest && ChromaOptions.RAINBOWSPREAD.getState()
                && ReikaRandomHelper.doWithChance(chanceFactor * 100 / 50)
                && world.getBiomeGenForCoords(x, z) == ChromatiCraft.rainbowforest)
    ...
    private void fightDecay(World world, int x, int y, int z, double rangeFactor) {
        if (MystCraftHandler.getInstance().decayID != null) {
            ...
            ReikaChunkHelper.removeBlocksFromChunk(
                world, rx, rz, MystCraftHandler.getInstance().decayID, -1
            );
        }
    }
    ...
    private void
    convertPureNodeMagic(World world, int x, int y, int z, double rangeFactor) {
                ...
                for (int k = -r; k <= r; k++) {
                    ...
                    if (id == ThaumIDHandler.Biomes.MAGICFOREST.getID()) {
                        BiomeGenBase natural
                            = ReikaWorldHelper.getNaturalGennedBiomeAt(world, dx, dz);
                    ...
                    if (id == ThaumIDHandler.Biomes.TAINT.getID()) {
                        ...
                        if (natural != null) {
                            if (natural.biomeID == id) {
                                natural
                                    = BiomeGenBase
                                          .forest; //ChromatiCraft.recoveredBiome;//BiomeGenBase.forest;
                            }
                            ReikaWorldHelper.setBiomeForXZ(world, dx, dz, natural, true);
                        }
                    }
    ...
    private void
    convertToRainbowForest(World world, int x, int y, int z, double rangeFactor) {
        int dr = (int) (32 * rangeFactor);
        int rx = ReikaRandomHelper.getRandomPlusMinus(x, dr);
        int rz = ReikaRandomHelper.getRandomPlusMinus(z, dr);
            ...
            for (int i = -r; i <= r; i++) {
                for (int k = -r; k <= r; k++) {
                    ...
                    if (!TileEntityBiomeReverter.stopConversion(world, dx, dz)) {
                        BiomeGenBase biome = world.getBiomeGenForCoords(dx, dz);
                        if (biome != ChromatiCraft.rainbowforest
                            && !BiomeGlowingCliffs.isGlowingCliffs(biome)
                            && !(biome instanceof NonconvertibleBiome)) {
                            ReikaWorldHelper.setBiomeForXZ(
                                world, dx, dz, ChromatiCraft.rainbowforest, true
                            );
                        }
                    }
                }
    ...
    private static class PersistentRainbowTreeFX {
        ...
        private static PersistentRainbowTreeFX readFromNBT(NBTTagCompound NBT) {
            PersistentRainbowTreeFX fx
                = new PersistentRainbowTreeFX(NBT.getInteger("dimID"));
