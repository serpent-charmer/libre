...
public class CrystalPath implements Comparable<CrystalPath> {
    ...
    protected CrystalPath(CrystalNetworker net, boolean real, CrystalElement e, List li) {
        ...
        network = net;
        hasRealTarget = real;
        hasLocus = TileEntityAuraPoint.isPointWithin(
    ...
    protected void initialize() {
        ...
        for (int i = 1; i < nodes.size(); i++) {
            ...
            range += Math.min(
                ((CrystalReceiver) teprev).getReceiveRange(),
                ((CrystalTransmitter) te).getSendRange()
            );
            dist += Math.sqrt(te.getDistanceSqTo(
                teprev.getX() + 0.5, teprev.getY() + 0.5, teprev.getZ() + 0.5
            ));
            if (i < nodes.size() - 1) {
    ...
    public final String toString() {
        return element + ": to " + origin + " from " + transmitter + ": " + nodes.size()
            + "x " + nodes.toString();
    }
        ...
        for (int i = 0; i < nodes.size() - 2; i++) {
            ...
            if (l == null || tgt.location.equals(l.loc1) || tgt.location.equals(l.loc2)) {
                ...
                if (!PylonFinder.lineOfSight(src.location, tgt.location).hasLineOfSight) {
                    ...
                    if (sr.needsLineOfSightToReceiver(rec)
                        || rec.needsLineOfSightFromTransmitter(sr)) {
                        return false;
                    }
                }
            }
    ...
    public final CachedPathValidity stillValid() {
        if (transmitter.isRemoved()
            || (hasRealTarget && PylonFinder.getNetTileAt(origin, false) == null))
        ...
        if (hasRealTarget) {
            ...
            if (!tile.canConduct() || !tile.isConductingElement(element))
                return CachedPathValidity.DORMANT;
            if (!tile.canBeSuppliedBy(transmitter, element)
                || !transmitter.canSupply(tile, element))
                return CachedPathValidity.DORMANT;
        }
    ...
    public final void blink(int ticks, CrystalReceiver r) {
        ...
        src.addSelfTickingTarget(
            locs.location,
            element,
            sx,
            sy,
            sz,
            r.getIncomingBeamRadius(),
            transmitter.getMaximumBeamRadius(),
            ticks
        );
    ...
    public final int compareTo(CrystalPath o) {
        return PylonFinder.getSourcePriority(o.transmitter, element)
            - PylonFinder.getSourcePriority(transmitter, element);
    }
    ...
    public final boolean hasSameEndpoints(CrystalPath p) {
        return PylonFinder.getLocation(transmitter)
                   .equals(PylonFinder.getLocation(p.transmitter))
            && origin.equals(p.origin);
    }

    CrystalPath cleanExtraEndJumps() {
        return new CrystalPath(
            network, hasRealTarget, element, this.getCleanedNodePath()
        );
    }
    ...
    public final CrystalPath optimize(int nsteps) {
        ...
        boolean complete = PylonFinder.optimizeRoute(
            network, li, nsteps, JumpOptimizationCheck.always
        );
    ...
    static final ArrayList<WorldLocation> cleanLocRoute(
        CrystalNetworker net, CrystalElement e, ArrayList<WorldLocation> nodes
    ) {
        return cleanRoute(net, e, createNodeList(nodes));
    }

    /** Reduces hop count; obeys LOS */
    static final ArrayList<WorldLocation>
    cleanRoute(CrystalNetworker net, CrystalElement e, ArrayList<PathNode> nodes) {
