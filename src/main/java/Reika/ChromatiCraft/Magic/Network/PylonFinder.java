...
public class PylonFinder {
    ...
    private final MultiMap<WorldLocation, WorldLocation> duplicates
    ...
    private static final
        HashMap<WorldLocation, EnumMap<CrystalElement, ArrayList<CrystalPath>>> paths
        = new HashMap();
    ...
    PylonFinder(CrystalElement e, CrystalReceiver r, EntityPlayer ep) {
        ...
        user = ep;
        isValidWorld = r.getWorld().provider.dimensionId == 0
            || PylonGenerator.instance.canGenerateIn(r.getWorld());
    }
    ...
    CrystalPath findPylonWith(SourceValidityRule rule) {
        ...
        if (this.isComplete()) {
            ...
            CrystalPath path = new CrystalPath(
                net,
                !(target instanceof WrapperTile),
                element,
                CrystalPath.cleanLocRoute(net, element, li)
            );
    ...
    CrystalFlow findPylon(int amount, int maxthru, SourceValidityRule rule) {
        ...
        if (this.isComplete()) {
            ...
            CrystalFlow flow = new CrystalFlow(
                net,
                target,
                element,
                amount,
    ...
    private void optimizeSkypeaterRoute(ArrayList<WorldLocation> path) {
        this.optimizeRoute(
            net,
            CrystalPath.createNodeList(path),
            Integer.MAX_VALUE,
            new JumpOptimizationCheck() {
                @Override
                public boolean canDirectLink(
                    CrystalNetworkTile t1, CrystalNetworkTile t2
                ) {
                    return t1 instanceof TileEntitySkypeater
                        && t2 instanceof TileEntitySkypeater;
                }
            }
        );
    }

    static boolean optimizeRoute(
        CrystalNetworker net,
        ArrayList<PathNode> path,
        int nsteps,
        JumpOptimizationCheck joc
    ) {
            ...
            for (int i = 0; i < li.size() && !flag; i++) {
                for (int k = i; k < li.size() && !flag; k++) {
                    if (i < k && Math.abs(i - k) > 1) {
                        ...
                        if (te1 instanceof CrystalReceiver
                            && te2 instanceof CrystalTransmitter
                            && joc.canDirectLink(te1, te2)) {
                            double d = Math.min(
                                ((CrystalReceiver) te1).getReceiveRange(),
                                ((CrystalTransmitter) te2).getSendRange()
                            );
                            if (te1.getDistanceSqTo(te2.getX(), te2.getY(), te2.getZ())
                                    <= d * d
                                && net.checkLOS(
                                    (CrystalTransmitter) te2, (CrystalReceiver) te1
                                )) {
                                while (k > i + 1) {
                                    li.remove(k - 1);
                                    k--;
    ...
    private boolean anyConnectedSources() {
        ...
        if (ModularLogger.instance.isEnabled(LOGGER_ID))
            ModularLogger.instance.log(
                LOGGER_ID, "Found " + li.size() + " sources for " + element + ": " + li
            );
    ...
    private boolean isSourceConnected(CrystalSource s) {
        return target instanceof WrapperTile || s instanceof TileEntityCreativeSource
            || !net.getNearbyReceivers(s, element).isEmpty();
    }

    private CrystalPath checkExistingPaths() {
        EnumMap<CrystalElement, ArrayList<CrystalPath>> map
            = paths.get(getLocation(target));
        if (map != null) {
    ...
    private void findFrom(CrystalReceiver r, SourceValidityRule rule) {
        ...
        if (CrystalNetworkLogger.getLogLevel().isAtLeast(LoggingLevel.PATHFIND))
            CrystalNetworkLogger.logPathFind(
                target, element, r, li.toString(), nodes.toString()
            );
        if (ChromaOptions.SHORTPATH.getState() || skypeaterEntry != null) {
            Collections.sort(
                li, new TransmitterDistanceSorter(r)
            ); //basic "start with closest and work outwards" logic; A* too complex and
               //expensive
        }
        ...
        for (CrystalTransmitter te : li) {
            ...
            if (!blacklist.contains(loc2) && !duplicates.containsValue(loc2)) {
                ...
                if (te != target) {
                    if ((te.needsLineOfSightToReceiver(r)
                         || r.needsLineOfSightFromTransmitter(te))
                        && !l.hasLineOfSight()) {
                        continue;
                    }
                    ...
                    if (te instanceof CrystalSource)
                        ReikaJavaLibrary.pConsole("Source: "+(te instanceof
                    CrystalSource)+" && "+this.isConnectableSource((CrystalSource)te,
                    thresh)); else ReikaJavaLibrary.pConsole("Source: false");

                    ReikaJavaLibrary.pConsole("Repeater: "+(te instanceof
                    CrystalRepeater));
                     */

                    if (te instanceof CrystalSource
                        && this.isConnectableSource((CrystalSource) te, rule)) {
                        net.addLink(l, true);
                        nodes.add(loc2);
                        //ReikaJavaLibrary.pConsole("Found source for "+element+" > "+r+",
                        //returning: "+this.isComplete());
                        return;
                        ...
                        if (te instanceof TileEntitySkypeater) {
                            ...
                            if (lastSkypeaterType != null && skypeaterEntry != null
                                && skypeaterEntry != lastSkypeaterType)
                                ; //continue;
                        }
    ...
    private boolean isConnectableSource(CrystalSource te, SourceValidityRule rule) {
        if (te instanceof TileEntityCrystalPylon)
            if (((TileEntityCrystalPylon) te).enhancing)
                return false;
        return te.canSupply(target, element) && target.canBeSuppliedBy(te, element)
            && (user == null || te.playerCanUse(user)) && rule.isValid(te, element);
    }
    ...
    static final CrystalReceiver getReceiverAt(WorldLocation loc, boolean exception) {
        ...
        if (exception)
            throw new IllegalStateException(
                "How did a non-receiver tile (" + te + ") get put in the network here ("
                + loc + ")?"
            );
        else
            return null;
    ...
    static final CrystalTransmitter
    getTransmitterAt(WorldLocation loc, boolean exception) {
        TileEntity te = loc.getTileEntity();
        ...
        if (exception)
            throw new IllegalStateException(
                "How did a non-transmitter tile (" + te
                + ") get put in the network here (" + loc + ")?"
            );
        else
            return null;
    }
    ...
    static final CrystalSource getSourceAt(WorldLocation loc, boolean exception) {
        ...
        if (exception)
            throw new IllegalStateException(
                "How did a non-source tile (" + te + ") get put in the network here ("
                + loc + ")?"
            );
        else
            return null;
    ...
    static final CrystalNetworkTile getNetTileAt(WorldLocation loc, boolean exception) {
        ...
        if (exception)
            throw new IllegalStateException(
                "How did a non-network tile (" + te + ") get put in the network here ("
                + loc + ")?"
            );
        else
            return null;
    ...
    public static CrystalPath
    convertTileListToPath(LinkedList<CrystalNetworkTile> li, CrystalElement e) {
    ...
    static void replacePath(
        CrystalSource src, WorldLocation tgt, CrystalElement color, CrystalPath p
    ) {
        ...
        ChromatiCraft.logger.logError(
            "Tried to replace a " + color + " path from " + src + " to " + tgt
    ...
    public static LOSData
    lineOfSight(World world, int x, int y, int z, Entity e, Block... extra) {
        return lineOfSight(
            world,
            x,
            y,
    ...
    public static LOSData lineOfSight(
        World world, int x1, int y1, int z1, int x2, int y2, int z2, Block... extra
    ) {
    ...
    private static boolean canRainOn(World world, Set<Coordinate> set) {
        for (Coordinate c : set) {
            if (isRainableBiome(world.getBiomeGenForCoords(c.xCoord, c.zCoord))
                && world.getPrecipitationHeight(c.xCoord, c.zCoord) <= c.yCoord)
                return true;
        }
        return false;
    }
    ...
    public static boolean isRainableBiome(BiomeGenBase b) {
        return !BiomeGlowingCliffs.isGlowingCliffs(b)
            && (b instanceof BiomeGenDesert
                || ReikaBiomeHelper.doesBiomeHavePrecipitation(b)
            ); //deserts because sandstorms
    }
    ...
    static {
        ...
        if (ModList.EXTRAUTILS.isLoaded()
            && ExtraUtilsHandler.getInstance().initializedProperly()) {
            ...
            if (b != null) {
                ...
                tracer.addTransparentBlock(
                    b, ExtraUtilsHandler.getInstance().invethereal
                );
                tracer.addTransparentBlock(b, ExtraUtilsHandler.getInstance().ineffable);
                tracer.addTransparentBlock(
                    b, ExtraUtilsHandler.getInstance().invineffable
                );
            }
        }
        if (ModList.TINKERER.isLoaded()
            && TinkerBlockHandler.getInstance().initializedProperly()) {
            tracer.addTransparentBlock(TinkerBlockHandler.getInstance().clearGlassID);
        }
        if (ModList.ENDERIO.isLoaded()) {
            Block b
                = GameRegistry.findBlock(ModList.ENDERIO.modLabel, "blockFusedQuartz");
            if (b != null) {
                tracer.addTransparentBlock(b, 1);
            }
        }
