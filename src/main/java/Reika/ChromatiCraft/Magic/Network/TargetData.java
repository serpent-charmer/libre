...
public class TargetData {
    ...
    public TargetData(CrystalTarget tg) {
        position
            = new DecimalPosition(tg.location).offset(tg.offsetX, tg.offsetY, tg.offsetZ);
        ...
        renderBox = source.asAABB()
                        .addCoord(
                            position.xCoord + 0.5 - source.xCoord,
                            position.yCoord + 0.5 - source.yCoord,
                            position.zCoord + 0.5 - source.zCoord
                        )
                        .expand(1, 1, 1);
    ...
    public boolean isRenderable() {
        //ReikaAABBHelper.renderAABB(renderBox, 0, 0, 0, 0, 0, 0, 160, 255, 255, 255,
        //true);
    ...
    public boolean isMaximumEndpointDistanceWithin(EntityPlayer ep, double dist) {
        return ep.getDistanceSq(position.xCoord, position.yCoord, position.zCoord)
            <= dist * dist
            || ep.getDistanceSq(
                   source.xCoord + 0.5, source.yCoord + 0.5, source.zCoord + 0.5
               )
            <= dist * dist;
    }
}
