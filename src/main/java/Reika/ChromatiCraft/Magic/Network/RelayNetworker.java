...
public final class RelayNetworker {
    public static final RelayNetworker instance
        = new RelayNetworker(getConfigurableRange(), 48);
    ...
    public TileEntityRelaySource findRelaySource(
        World world,
        int x,
        int y,
        int z,
        ...
        if (amt <= 0)
            return null;
        RelayFinder rf = new RelayFinder(
            new Coordinate(x, y, z), Math.min(dist, maxRange), maxDepth, e, amt
        );
        rf.look = dir;
    ...
    private static class RelayPath {
        ...
        private RelayPath(
            ...
            ModularLogger.instance.log(
                LOGGER_ID,
                "Relay pathfinding complete from " + source + " to " + target + " for "
                    + e
            );
        }
        ...
        public void transmit(CrystalElement e) {
            if (!source.worldObj.isRemote) {
                ...
                ReikaPacketHelper.sendNIntPacket(
                    ChromatiCraft.packetChannel,
                    ChromaPackets.RELAYCONNECT.ordinal(),
                    this.getTarget(),
                    dat
                );
            }
    ...
    private static class RelayFinder {
        ...
        private RelayFinder(Coordinate loc, int r, int d, CrystalElement e, int amt) {
            ...
            ModularLogger.instance.log(
                LOGGER_ID, "Relay pathfinding start @ " + loc + " for " + amt + " of " + e
            );
        }
        ...
        private RelayPath findFrom(World world, Coordinate start, int depth) {
            ...
            for (int i = 1; i < maxRange; i++) {
                ...
                if (ChromaTiles.getTileFromIDandMetadata(b, meta)
                    == ChromaTiles.RELAYSOURCE) {
                    path.addLast(c);
                    return new RelayPath(
                        (TileEntityRelaySource) c.getTileEntity(world),
                        target,
                        color,
                        path
                    );
                    ...
                    TileEntityRelayFilter te
                        = (TileEntityRelayFilter) c.getTileEntity(world);
                ...
                if (ChromaTiles.getTileFromIDandMetadata(b, meta) == ChromaTiles.RIFT) {
                    ...
                    if (loc != null) {
                        ...
                        if (world2.provider.dimensionId == world.provider.dimensionId
                            || PylonGenerator.instance.canGenerateIn(world2)) {
                            path.addLast(c);
    ...
    public boolean isRelayPassable(World world, int x, int y, int z) {
        return PylonFinder.isBlockPassable(world, x, y, z)
            || this.isBlockRelayTransparent(world, x, y, z);
    }
    ...
    private boolean isBlockRelayTransparent(World world, int x, int y, int z) {
        ...
        if (b == ChromaBlocks.DECOFLOWER.getBlockInstance()
            && (meta == Flowers.FLOWIVY.ordinal() || meta == Flowers.GLOWDAISY.ordinal()))
            return true;
        ...
        if (b == ChromaBlocks.STRUCTSHIELD.getBlockInstance()
            && meta % 8 == BlockType.GLASS.ordinal())
            return true;
        return false;
    }
}
