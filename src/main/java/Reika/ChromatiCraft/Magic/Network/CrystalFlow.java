...
public final class CrystalFlow extends CrystalPath {
    ...
    CrystalFlow(
        CrystalNetworker net, CrystalPath p, CrystalReceiver r, int amt, int maxthru
    ) {
        this(net, r, p.element, amt, p.nodes, maxthru);
    }

    protected CrystalFlow(
    ...
    private int calcEffectiveThroughput(int maxthru) {
        ...
        for (int i = 1; i < nodes.size() - 1; i++) {
            ...
            if (pn.isRepeater()) {
                insured
                    = Math.max(insured, ((CrystalRepeater) te).getThoughputInsurance());
        ...
        int result = Math.min(
            Math.max(insured, base - this.getThroughputPenalty(base) + bonus),
            Math.min(maxthru, base)
        );
    ...
    private int getThroughputPenalty(int rawthru) {
        ...
        if (att <= 1)
            return 0;
        int amt = (int) Math.pow(
            att / 80, 1.5 + 0.5 * (1 - this.getOptimizationFactor())
        ); //raw, directly determined from attenuation & path optimization
    ...
    void resetTiles() {
        ((CrystalSource) nodes.get(nodes.size() - 1).getTile(true))
            .removeTarget(nodes.get(nodes.size() - 2).location, element);
        for (int i = 1; i < nodes.size() - 1; i++) {
            CrystalNetworkTile te = nodes.get(i).getTile(true);
            if (te instanceof CrystalTransmitter) {
    ...
    public int getDrainThisTick() {
        return Math.min(
            Math.min(Math.min(throttle, maxFlow), transmitter.maxThroughput()),
            remainingAmount
        );
    }
        ...
        if (this.hasReactiveRepeaters()) {
            for (int i = 1; i < nodes.size() - 1; i++) {
                if (ReactiveRepeater.class.isAssignableFrom(nodes.get(i).tileClass)) {
                    CrystalNetworkTile te = nodes.get(i).getTile(true);
                    ((ReactiveRepeater) te)
                        .onTransfer(transmitter, receiver, element, amt);
                }
            }
        }
    }
    ...
    CrystalPath cleanExtraEndJumps() {
        return new CrystalFlow(
            network,
            super.cleanExtraEndJumps(),
            receiver,
            requestedAmount,
            throughputLimit
        );
