...
public class PylonLinkNetwork {
    ...
    public void removeLocation(World world, PylonNode connection) {
        ...
        ChromatiCraft.logger.log(
            "Updating pylon link network: removed a tile @ " + connection
        );
        this.sync(null);
    }
    ...
    getLinkedPylons(World world, UUID uid, CrystalElement color) {
        ...
        if (w == null) {
            //ChromatiCraft.logger.logError("Tried to get link list for unregistered ID
            //"+uid);
            return li;
        }
    ...
    public void sync(EntityPlayerMP ep) {
        ...
        ReikaPacketHelper.sendNBTPacket(
            ChromatiCraft.packetChannel, ChromaPackets.PYLONLINKCACHE.ordinal(), nbt, pt
        );
    }
    ...
    public static class PylonLinkData extends WorldSavedData {
        ...
        private static PylonLinkData initNetworkData(World world) {
            PylonLinkData data
                = (PylonLinkData) world.loadItemData(PylonLinkData.class, IDENTIFIER);
            if (data == null) {
    ...
    public static class PylonNode {
        ...
        public boolean equals(Object o) {
            if (o instanceof PylonNode) {
                ...
                return pn.tile.equals(tile) && pn.pylon.equals(pylon)
                    && pn.getColor() == this.getColor();
            }
            return false;
        }
