...
public class PlayerElementBuffer implements PlayerBufferAPI {
    ...
    public boolean
    addToPlayer(EntityPlayer ep, CrystalElement e, int amt, boolean notify) {
        NBTTagCompound tag = this.getTag(ep);
    ...
    private void warnPlayer(EntityPlayer ep, CrystalElement e, float f1, float f2) {
        if (f2 < f1) {
            ...
            if (s1 != s2) {
                ...
                if (snd != null)
                    snd.playSound(
                        ep, 1, (float) CrystalMusicManager.instance.getDingPitchScale(e)
                    );
            }
        }
    }
    ...
    public boolean setElementCap(EntityPlayer ep, int cap, boolean notify) {
        ...
        if (flag) {
            ...
            if (notify) {
                if (cap % 2 == 0)
                    ChromaSounds.CRAFTDONE.playSound(
                        ep.worldObj, ep.posX, ep.posY, ep.posZ, 0.1F, 0.5F
                    );
        ...
        if (ep instanceof EntityPlayerMP) {
            ReikaPacketHelper.sendDataPacket(
                ...
                ChromaPackets.BUFFERSET.ordinal(),
                (EntityPlayerMP) ep,
                val,
                notify ? 1 : 0
            );
    ...
    private int calcElementCap(EntityPlayer ep) {
        ...
        return Math.max(
            this.getPlayerBuffer(ep).getMaximumValue(),
            MathHelper.clamp_int(
                (int) (Math.min(amt * f, m * Math.pow(amt, p))),
                24,
                this.getPlayerMaximumCap(ep)
            )
        );
    }
    ...
    public int getChargeInefficiency(EntityPlayer ep) {
        return ProgressStage.CTM.isPlayerAtStage(ep)      ? 1
            : ProgressStage.DIMENSION.isPlayerAtStage(ep) ? 2
                                                          : 4;
    }
    ...
    ChromaPackets.BUFFERINC.ordinal(), ep, 0);
    }
     */
    ...
    public boolean isMaxedWithin(EntityPlayer player, CrystalElement e, float frac) {
        return this.getPlayerContent(player, e)
            >= this.getElementCap(player) * (1 - frac);
    }
