...
public class CrystalPotionController implements CrystalPotionAPI {
    ...
    private final EnumMap<CrystalElement, Potion> potions
        = new EnumMap(CrystalElement.class);
    private final EnumMap<CrystalElement, Potion> potionsNether
        = new EnumMap(CrystalElement.class);
    ...
    public boolean isWorldHostile(World world) {
        if (ModList.EXTRAUTILS.isLoaded()
            ...
            && world.provider.dimensionId == ExtraUtilsHandler.getInstance().darkID)
            return true;
        if (ModList.THAUMCRAFT.isLoaded()
            && ThaumIDHandler.getInstance().initializedProperly()
            && world.provider.dimensionId == ThaumIDHandler.getInstance().dimensionID)
            return true;
    ...
    public boolean isPotionAllowed(PotionEffect eff, EntityLivingBase e) {
        ...
        if (!(e instanceof EntityPlayer)) {
            ...
            if (e instanceof EntityCreature) {
                flag = ((EntityCreature) e).getCreatureAttribute()
                    == EnumCreatureAttribute.UNDEAD;
            }
            return this.shouldBeHostile(e, e.worldObj)
                ? ReikaPotionHelper.isBadEffect(pot) == flag
                : true;
        }
        if (this.shouldBeHostile(e, e.worldObj))
            return eff.getPotionID() == Potion.nightVision.id
                || ReikaPotionHelper.isBadEffect(pot);
    ...
    public PotionEffect
    getEffectFromColor(CrystalElement color, int dura, int level, boolean evil) {
    ...
    public void applyEffectFromColor(
        int dura, int level, EntityLivingBase e, CrystalElement color, boolean doFX
    ) {
        ...
        if (forceBad || (!forceGood && this.shouldBeHostile(e, e.worldObj))) {
            ...
            switch (color) {
                ...
                case WHITE:
                    ...
                    ReikaPotionHelper.clearBadPotions(
                        e, level > 0 ? null : this.ignoredBadPotionsForLevelZero()
                    );
                    break;
                case PURPLE:
                    if (e instanceof EntityPlayer && !e.worldObj.isRemote
                        && (level > 0 || rand.nextInt(2) == 0)) {
                        EntityPlayer ep = (EntityPlayer) e;
                        if (doFX)
                            ep.worldObj.playSoundAtEntity(
                                ep, "random.orb", 0.2F, rand.nextFloat() * 2
                            );
                        int amt = 1;

                        int lvl = ItemPendant.getActivePendantLevel(
                            ep, CrystalElement.PURPLE
                        );
                        if (lvl == 1) {
                            amt *= 3;
                        } else if (lvl == 0) {
    ...
    private static void addPotionEffect(EntityLivingBase e, PotionEffect eff) {
        ...
        if (e instanceof EntityPlayer || cur == null
            || cur.getAmplifier() < eff.getAmplifier() || cur.getDuration() < 20
            || eff.getDuration() < 80)
    ...
    @Override
    public PotionEffect
    getEffectFromColor(CrystalElementProxy color, int dura, int level, boolean evil) {
        return this.getEffectFromColor(
            CrystalElement.elements[color.ordinal()], dura, level, evil
        );
    }
}
