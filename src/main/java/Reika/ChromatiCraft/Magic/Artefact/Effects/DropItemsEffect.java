...
public class DropItemsEffect extends UABombingEffect.BlockEffect {
    ...
    public void trigger(IInventory inv, TileEntity te) {
        ArrayList<Integer> slots = ReikaJavaLibrary.makeIntListFromArray(
            ReikaArrayHelper.getLinearArray(inv.getSizeInventory())
        );
        ...
        for (int slot : slots) {
            ...
            if (in != null && in.getItem() != ChromaItems.ARTEFACT.getItemInstance()) {
                ...
                EntityItem ei = ReikaItemHelper.dropItem(
                    te.worldObj, te.xCoord + 0.5, te.yCoord + 0.75, te.zCoord + 0.5, in
                );
                ei.motionX = vx;
                ei.motionY = vy;
                ei.motionZ = vz;
