...
public class ArtefactWithDataCrystalAlloyingEffect implements AlloyingEffect {
    public static final ArtefactWithDataCrystalAlloyingEffect instance
        = new ArtefactWithDataCrystalAlloyingEffect();
    ...
    public void doEffect(EntityItem ei) {
        //ReikaJavaLibrary.pConsole(ei.worldObj.isRemote+" @ "+tick+" for
        //"+ei.getEntityItem());
        if (ei.worldObj.isRemote) {
            ...
            if (tick % 45 == 0)
                ChromaSounds.ARTEALLOY.playSoundNoAttenuation(
                    ei.worldObj, ei.posX, ei.posY, ei.posZ, 2, 1, 96
                );
            if (fireTick == 0 && tick > 10 && rand.nextInt(90) == 0) {
                List<EntityLivingBase> li = ei.worldObj.getEntitiesWithinAABB(
                    EntityLivingBase.class, ReikaAABBHelper.getEntityCenteredAABB(ei, 12)
                );
                for (EntityLivingBase e : li) {
                    TickScheduler.instance.scheduleEvent(
                        new ScheduledTickEvent(
                            new DelayedKnockback(e, new DecimalPosition(ei), 5, 0.5)
                        ),
                        12
                    );
                    TickScheduler.instance.scheduleEvent(
                        new ScheduledTickEvent(new DelayedAttack(e, DamageSource.magic, 2)
                        ),
                        12
                    );
                }
                ReikaPacketHelper.sendDataPacketWithRadius(
                    ...
                    ChromaPackets.ARTEALLOYBURST.ordinal(),
                    ei,
                    128,
                    ei.getEntityId()
    ...
    private void doClientFX(EntityItem ei) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            float g = (float) ReikaRandomHelper.getRandomBetween(0.03125, 0.125)
                * (float) (vy / 0.125) * 1.5F;
            EntityCCBlurFX fx1 = new EntityCCBlurFX(
                Minecraft.getMinecraft().theWorld, ei.posX, ei.posY, ei.posZ, vx, vy, vz
            );
            fx1.setIcon(ChromaIcons.FADE_RAY)
                .setColor(0xffffff)
                .setGravity(g)
                .setScale(s)
                .setLife(40)
    ...
    public void doBurst(EntityItem ei) {
        ...
        fireTick = 50;
        TickScheduler.instance.scheduleEvent(
            new ScheduledTickEvent(new ParticleBurst(new DecimalPosition(ei))), 10
        );
    }
    ...
    private static class ParticleBurst implements ScheduledEvent {
        ...
        private void fireClient() {
            for (int i = 0; i < 32; i++) {
                ...
                EntityCCBlurFX fx1 = new EntityCCBlurFX(
                    Minecraft.getMinecraft().theWorld, px, py, pz, vx, vy, vz
                );
                int c = ReikaColorAPI.getModifiedHue(
                    0xff0000, ReikaRandomHelper.getRandomBetween(20, 45)
                );
                fx1.setIcon(ChromaIcons.FADE_GENTLE)
                    .setColor(c)
                    .setScale(s)
                    .setLife(30)
                    .setRapidExpand();
