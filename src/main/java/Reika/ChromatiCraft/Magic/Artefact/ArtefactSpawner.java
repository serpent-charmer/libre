...
public class ArtefactSpawner implements TickHandler {
    ...
    public void tick(TickType type, Object... tickData) {
        ...
        if (canSpawn && rand.nextInt(2) == 0)
            ReikaPacketHelper.sendDataPacket(
                ChromatiCraft.packetChannel, ChromaPackets.ARTEZONEPARTICLES.ordinal(), ep
            );
    ...
    public void addArtefact(double dx, double dz, EntityPlayer ep, int life) {
        if (artefacts.size() < FORCED_SPAWN_LIMIT) {
            ...
            if (y > 0
                && UnknownArtefactGenerator.canGenerateArtefactAt(ep.worldObj, x, y, z)) {
                double dy = y - 0.5;
    ...
    private int getY(World world, int x, int z) {
        ...
        if (b instanceof BiomeGenOcean) {
            ...
            if (at == Blocks.gravel || at == Blocks.sand || at == Blocks.dirt
                || at == Blocks.clay)
                return y;
            return -1;
        } else {
    ...
    public void checkPlayerBreak(BreakEvent evt) {
        ...
        if (ep instanceof EntityPlayerMP && !ReikaPlayerAPI.isFake(ep)) {
            ...
            //ReikaPacketHelper.sendDataPacket(ChromatiCraft.packetChannel,
            //ChromaPackets.DIGARTEFACT.ordinal(), (EntityPlayerMP)ep, evt.x, evt.y,
            //evt.z);
    ...
    public void checkPlayerBreakClient(World world, int x, int y, int z) {
        if (this.isOnTopOfArtefact(world, x, y, z)) {
            ReikaPacketHelper.sendDataPacket(ChromatiCraft.packetChannel,
    ChromaPackets.ARTEFACTCONFIRM.ordinal(), PacketTarget.server, x, y, z);
        }
    }
     */
    ...
    private void updateShader(World world) {
        if (this.isShaderActive(world)) {
            ...
            ChromaShaders.UAZONE.getShader().setField(
                "skyBright",
                world.provider.getSunBrightness(ReikaRenderHelper.getPartialTickTime())
            );
            ChromaShaders.UAZONE.getShader().setField(
                "headYaw", RenderManager.instance.playerViewX
            );
            ChromaShaders.UAZONE.getShader().setField(
                "headPitch", RenderManager.instance.playerViewY
            );
        } else {
    ...
    public static class SpawnedArtefact {
        ...
        private boolean tick(EntityPlayerMP ep) {
            ...
            age++;
            return age >= maxLife
                || (ep.getUniqueID().equals(spawnedByPlayer)
                    && ep.getDistanceSq(position.xCoord, position.yCoord, position.zCoord)
