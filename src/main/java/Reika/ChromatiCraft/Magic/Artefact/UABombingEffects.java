...
public class UABombingEffects {
    ...
    private final WeightedRandom<UABombingEffect.BlockEffect> blockEffects
        = new WeightedRandom();
    private final WeightedRandom<UABombingEffect.EntityEffect> entityEffects
        = new WeightedRandom();
                    ...
                    for (ICellProvider icp : set) {
                        if (icp instanceof IInventory) {
                            ...
                            for (int i = 0; i < ii.getSizeInventory(); i++) {
                                ...
                                if (cell != null
                                    && cell.getItem() instanceof IStorageCell) {
                                    IStorageCell isc = (IStorageCell) cell.getItem();
                                    if (cell.getItem() instanceof ItemShieldedCell) {
                                    } else {
                                        ICellRegistry icr
                                            = AEApi.instance().registries().cell();
                                        IMEInventoryHandler inv = icr.getCellInventory(
                                            cell, null, StorageChannel.ITEMS
                                        );
                                        ICellInventoryHandler icih
                                            = (ICellInventoryHandler) inv;
                                        ICellInventory cellInv = icih.getCellInv();
                                        IItemList<IAEItemStack> items
                                            = cellInv.getAvailableItems(
                                                StorageChannel.ITEMS.createList()
                                            );
                                        if (items.findPrecise(
                                                MESystemReader.createAEStack(
                                                    ChromaItems.ARTEFACT
                                                        .getStackOfMetadata(
                                                            ArtefactTypes.ARTIFACT
                                                                .ordinal()
                                                        )
                                                )
                                            )
                                            ...
                                            //	TickScheduler.instance.scheduleEvent(new
                                            //ScheduledTickEvent(new MESystemDrain(grid)),
                                            //1);
                                            //}
                                            //ReikaJavaLibrary.pConsole("Detected
                                            //unshielded UA in ME system");
                                        }
                                    }
                                }
                            }
    ...
    private static class MESystemDrain implements ScheduledEvent {
        ...
        public void fire() {
            ...
            if (ig != null) {
                ...
                ie.extractAEPower(
                    Double.MAX_VALUE, Actionable.MODULATE, PowerMultiplier.ONE
                );
            }
        }
