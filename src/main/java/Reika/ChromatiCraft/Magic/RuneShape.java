...
public final class RuneShape {
    ...
    public boolean matchAt(World world, int x, int y, int z) {
        ...
        for (Entry<Coordinate, CrystalElement> e : runes.entrySet()) {
            ...
            if (b == ChromaBlocks.RUNE.getBlockInstance()) {
                ...
                //ReikaJavaLibrary.pConsole("Mismatch, found "+b+" instead of
                //"+e.getValue()+" on "+world+" @ "+c+">"+c2);
                return false;
            }
        }
        return true;
    ...
    public static Coordinate modifyRuneBySeed(Coordinate c, World world) {
        if (TempleCastingRecipe.isRuneRing(c))
            return c;
        WorldIDBase id = world == null || world.isRemote
            ? ReikaWorldHelper.clientWorldID
            : ReikaWorldHelper.getCurrentWorldID(world);
        if (id == null)
            return c;
