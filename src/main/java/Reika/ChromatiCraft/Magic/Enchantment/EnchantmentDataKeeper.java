...
public class EnchantmentDataKeeper extends ChromaticEnchantment {
    ...
    handleBreak(World world, int x, int y, int z, Block b, int meta, EntityPlayer ep) {
        if (handlers.isEmpty())
            initHandlers();
    ...
    private static final class IC2LogDrop extends BlockHandler {
        ...
        @Override
        protected ItemStack
        getItem(World world, int x, int y, int z, Block b, int meta, EntityPlayer ep) {
            if (IC2RubberLogHandler.getInstance().isCrop(b, meta)) {
                return new ItemStack(
                    b, 1, IC2RubberLogHandler.getInstance().getMeta(ForgeDirection.EAST)
                );
            }
            return null;
        }
