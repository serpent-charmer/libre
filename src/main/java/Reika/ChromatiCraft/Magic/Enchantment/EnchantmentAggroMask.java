...
public class EnchantmentAggroMask extends ChromaticEnchantment {
    ...
    public boolean isVisibleToPlayer(EntityPlayer ep, int level) {
        return level == 3 ? ProgressStage.DIMENSION.playerHasPrerequisites(ep)
                && Chromabilities.COMMUNICATE.isAvailableToPlayer(ep)
                          : ProgressStage.KILLMOB.isPlayerAtStage(ep);
    }
}
