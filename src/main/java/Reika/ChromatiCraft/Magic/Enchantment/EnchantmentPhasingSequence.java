...
public class EnchantmentPhasingSequence extends ChromaticEnchantment {
    private static final float[] REDUCTION_FACTORS
        = { 1, 0.9375F, 0.875F, 0.75F, 0.667F };
    ...
    private static float getActualDamage(float baseDmg, int level) {
        return (float
        ) ReikaMathLibrary.roundDownToFraction(baseDmg * REDUCTION_FACTORS[level], 0.125);
    }

    public static float getPenetratingDamage(float baseDmg, int level) {
        return (float) ReikaMathLibrary.roundUpToFraction(
            0.1F * level * getActualDamage(baseDmg, level), 0.25
        );
    }
