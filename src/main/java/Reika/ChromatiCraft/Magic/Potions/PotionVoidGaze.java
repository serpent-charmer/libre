    ...
    public static enum VoidGazeLevels {
        ...
        public boolean canApply(EntityPlayer ep) {
            switch (this) {
                ...
                case FACEFLIP:
                    return ChromaResearchManager.instance.getPlayerResearchLevel(ep)
                               .isAtLeast(ResearchLevel.MULTICRAFT)
                        && ProgressStage.END.isPlayerAtStage(ep)
                        && ProgressStage.CHROMA.isPlayerAtStage(ep)
                        && ProgressionManager.instance
                               .hasPlayerDiscoveredAGeneratedStructure(ep)
        ...
        public boolean isActiveOnPlayer(EntityPlayer ep) {
            if (ItemEtherealPendant.isActive((EntityPlayer) ep))
                return true;
            PotionEffect pot = Minecraft.getMinecraft().thePlayer.getActivePotionEffect(
                ChromatiCraft.voidGaze
            );
            return pot != null && pot.getAmplifier() >= this.ordinal()
                && pot.getDuration() >= 2;
        }
    }
}
