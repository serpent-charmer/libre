...
public class PotionLumarhea extends ChromaPotion {
    ...
    @Override
    public void
    affectEntity(EntityLivingBase src, EntityLivingBase e, int level, double c) {}

    @Override
    public void performEffect(EntityLivingBase e, int level) {
        if (e instanceof EntityPlayer) {
            EntityPlayer ep = (EntityPlayer) e;
            long id = ((e.worldObj.getTotalWorldTime()
                        + ep.getUniqueID().getLeastSignificantBits())
                           % BLOWOUT_TIMER
                       + BLOWOUT_TIMER)
                % BLOWOUT_TIMER;
            if (id == 0) {
                if (e.worldObj.isRemote) {
                    ...
                    PlayerElementBuffer.instance.removeFromPlayer(
                        ep,
                        ElementTagCompound.getUniformTag(
                            10 * this.getRemovedEnergy(ep, level)
                        )
                    );
                ...
                if (e.worldObj.isRemote) {
                    ...
                    if (rand.nextInt(5) == 0)
                        ReikaSoundHelper.playClientSound(
                            ChromaSounds.ERROR, ep, 0.125F, 2F
                        );
                } else
                    PlayerElementBuffer.instance.removeFromPlayer(
                        ep, c, this.getRemovedEnergy(ep, level)
                    );
            }
        }
    }
    ...
    private int getRemovedEnergy(EntityPlayer ep, int level) {
        return Math.max(
            20,
            PlayerElementBuffer.instance.getElementCap(ep)
                / (10 * Math.max(1, 100 - level * 2))
        );
    }
