...
public class PotionBetterSaturation extends ChromaPotion {
    ...
    public void performEffect(EntityLivingBase e, int level) {
        if (!e.worldObj.isRemote && e instanceof EntityPlayer) {
            ...
            if (level > 0
                || ep.getFoodStats().getFoodLevel() < this.getMaxBaseFoodLevel())
                ep.getFoodStats().addStats(level + 1, 1.0F);
        }
    }

    private int getMaxBaseFoodLevel() {
        return ModList.HUNGEROVERHAUL.isLoaded()
            ? HungerOverhaulHandler.getInstance().regenHungerValue
            : 17;
    }

    @Override
    public String getName() {
        return Potion.field_76443_y.getName(
        ); //StatCollector.translateToLocal("chromapotion.sat");
    }
