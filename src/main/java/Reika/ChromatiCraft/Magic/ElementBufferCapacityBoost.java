...
public enum ElementBufferCapacityBoost {
    ...
    private ElementBufferCapacityBoost(
        BiFunction<Integer, Boolean, Integer> func, ProgressAccess req
    ) {
        capFunction = func;
        requirement = req;
    }
    ...
    public boolean isGrantedAutomatically() {
        return ingredient
            == null; //this == ALLCOLORS || this == ABILITY || this == LORECOMPLETE;
    }
        ...
        while (it.hasNext()) {
            ...
            if (tag.func_150285_a_().equals(this.name())) {
                ...
                PlayerElementBuffer.instance.addToPlayer(
                    ep, CrystalElement.BLACK, 0, true
                );
                break;
            }
        }
    ...
    private boolean doGive(EntityPlayer ep) {
        if (!this.isTagPresent(ep)) {
            ...
            //ChromatiCraft.logger.log("Player "+ep.getCommandSenderName()+" just upgraded
            //their element buffer with "+this+"; capacity is now
            //"+PlayerElementBuffer.instance.getPlayerMaximumCap(ep));
    ...
    public boolean isAvailableToPlayer(EntityPlayer ep) {
        boolean flag = requirement.playerHas(ep)
            && (dependency == null || dependency.playerHas(ep));
