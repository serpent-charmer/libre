...
public class WarpNetwork {
    ...
    public WorldLocation getLink(WorldLocation loc, double angle, double tolerance) {
        ...
        if (map.size() == 1) {
            ...
            return Math.abs(ReikaVectorHelper.getAngleDifference(e.getKey(), angle))
                    <= tolerance
                ? e.getValue()
                : null;
        }
        ...
        if (e1 == null) {
            return Math.abs(ReikaVectorHelper.getAngleDifference(e2.getKey(), angle))
                    <= tolerance
                ? e2.getValue()
                : null;
        } else if (e2 == null) {
            //ReikaJavaLibrary.pConsole(e1+" @
            //"+ReikaVectorHelper.getAngleDifference(e1.getKey(), angle));
            return Math.abs(ReikaVectorHelper.getAngleDifference(e1.getKey(), angle))
                    <= tolerance
                ? e1.getValue()
                : null;
        } else {
            double d1
                = Math.abs(ReikaVectorHelper.getAngleDifference(e1.getKey(), angle));
            double d2
                = Math.abs(ReikaVectorHelper.getAngleDifference(e2.getKey(), angle));
            boolean left = d1 < d2;
            return (left ? d1 : d2) <= tolerance ? (left ? e1.getValue() : e2.getValue())
                                                 : null;
        }
    }
        ...
        for (WorldLocation key : data.keySet()) {
            if (key.dimensionID == loc.dimensionID && !loc.equals(key)) {
                ...
                double phi = ReikaPhysicsHelper.cartesianToPolar(
                    loc.xCoord - key.xCoord, 0, loc.zCoord - key.zCoord
                )[2];
                map.put(phi, loc);
    ...
    public static class WarpNetworkData extends WorldSavedData {
        ...
        private static WarpNetworkData initNetworkData(World world) {
            WarpNetworkData data
                = (WarpNetworkData) world.loadItemData(WarpNetworkData.class, IDENTIFIER);
            if (data == null) {
