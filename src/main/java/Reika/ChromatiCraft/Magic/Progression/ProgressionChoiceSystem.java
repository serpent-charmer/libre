...
public class ProgressionChoiceSystem {
    ...
    public ProgressionChoiceSystem(EntityPlayer ep) {
        ...
        rand = new Random(
            ep.getUniqueID().hashCode()
            + DragonAPICore.getLaunchTime() * System.identityHashCode(this)
        );
        rand.nextBoolean();
        rand.nextBoolean();
    }
    ...
    public ArrayList<Selection> pickThreeCategories() {
        ...
        ArrayList<ChromaResearch> next
            = ChromaResearchManager.instance.getNextResearchesFor(player);
        if (next.isEmpty())
        ...
        while (picked.size() < 3 && !next.isEmpty()) {
            ...
            FragmentCategorization c
                = FragmentCategorizationSystem.instance.getCategories(choose);
    ...
    public static class Selection {
        ...
        public static boolean giveToPlayer(EntityPlayer ep, ChromaResearch r) {
            if (r.canPlayerProgressTo(ep)
                && ChromaResearchManager.instance.getNextResearchesFor(ep).contains(r)) {
