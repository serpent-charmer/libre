...
public class FragmentCategorizationSystem {
    public static final FragmentCategorizationSystem instance
        = new FragmentCategorizationSystem();

    private final EnumMap<ChromaResearch, CountMap<FragmentCategory>> mappings
        = new EnumMap(ChromaResearch.class);
    private final EnumMap<ChromaResearch, FragmentCategorization> categories
        = new EnumMap(ChromaResearch.class);
    private final EnumMap<FragmentCategory, FragmentTable> tables
        = new EnumMap(FragmentCategory.class);
    ...
    private FragmentCategorizationSystem() {
        ...
        this.addMapping(
            ChromaStructures.PLAYERINFUSION, FragmentCategory.IMPROVEMENT, 40
        ); /*
...
this.addMapping(ChromaStructures.BIOMEFRAG, FragmentCategory.ARTIFACT, 40);*/
        ...
        this.addMapping(
            ChromaStructures.PYLONBROADCAST, FragmentCategory.IMPROVEMENT, 40
        );
        ...
        this.addMapping(
            ChromaStructures.WIRELESSPEDESTAL, FragmentCategory.IMPROVEMENT, 20
        );
        this.addMapping(ChromaStructures.WIRELESSPEDESTAL2, FragmentCategory.LUMENS, 40);
        this.addMapping(
            ChromaStructures.WIRELESSPEDESTAL2, FragmentCategory.IMPROVEMENT, 40
        );
        ...
        this.addMapping(
            ChromaResearch.STRUCTUREPASSWORDS, FragmentCategory.STRUCTURE, 30
        );
    ...
    private void addMapping(ChromaTiles r, FragmentCategory fc, int amt) {
        ...
        if (f != null && f.getMachine() == r)
            ...
            throw new RegistrationException(
                ...
                "Tried to add a mapping for " + r + "@" + r.getClass()
                    + ", which does not have its own fragment!"
            );
    }
    ...
    private void addMapping(ChromaResearch r, FragmentCategory fc, int amt) {
        if (r == null)
            throw new RegistrationException(
                ChromatiCraft.instance, "Tried to add a mapping for a null fragment!"
            );
        if (r.isDummiedOut())
            ; //throw new RegistrationException(ChromatiCraft.instance, "Tried to add a
              //mapping for a disabled fragment!");
        if (!r.isDummiedOut() && !ChromaResearch.getAllObtainableFragments().contains(r))
            throw new RegistrationException(
                ChromatiCraft.instance,
                "Tried to add a mapping for an unobtainable fragment!"
            );
        ...
        if (map.get(fc) > 0)
            throw new RegistrationException(
                ChromatiCraft.instance, "Duplicate category (" + fc + ") mapping for " + r
            );
    ...
    public void calculate() {
        ...
        if (!empty.isEmpty())
            throw new RegistrationException(
                ChromatiCraft.instance, "Fragments " + empty + " have no categories!"
            );
    }
    ...
    public void dumpData() {
        for (ChromaResearch r : ChromaResearch.getAllObtainableFragments()) {
            ...
            ReikaJavaLibrary.pConsole(
                "Fragment " + r.getTitle() + " (" + r.name() + ") = " + c.weights.size()
                + ":" + c.weights
            );
            if (c.weights.size() == 1)
                ReikaJavaLibrary.pConsole(
                    "Fragment " + r.getTitle() + " has no special categories!"
                );
        }
        for (FragmentCategory c : FragmentCategory.list) {
            FragmentTable t = this.getFragments(c);
            ReikaJavaLibrary.pConsole(
                "Category " + c + " = " + t.weights.size() + ": " + t.weights
            );
        }
    }
    ...
    public static class FragmentTable {
        ...
        public WeightedRandom<ChromaResearch> getSelectionForPlayer(EntityPlayer ep) {
            HashSet<ChromaResearch> set
                = new HashSet(ChromaResearchManager.instance.getNextResearchesFor(ep));
            ...
            Collections.sort(li, new Comparator<FragmentCategory>() {
                ...
                public int compare(FragmentCategory o1, FragmentCategory o2) {
                    return String.CASE_INSENSITIVE_ORDER.compare(
                        ControlledConfig.getUserHash(o1.name()),
                        ControlledConfig.getUserHash(o2.name())
                    );
                }
            });
            for (FragmentCategory c : list) {
