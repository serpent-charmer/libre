...
public class ProgressionCatchupHandling {
    public static final ProgressionCatchupHandling instance
        = new ProgressionCatchupHandling();
    ...
    public void
    attemptSync(TileEntityBase te, double maxDist, ProgressStage p, boolean ignoreTime) {
    ...
    private void triggerProgressCatchup(TileEntity te, double maxDist, ProgressStage p) {
        ...
        if (ProgressionLinking.instance.hasLinkedPlayers(ep)
            && ProgressionManager.instance.canStepPlayerTo(ep, p)) {
            double dist
                = ep.getDistanceSq(te.xCoord + 0.5, te.yCoord + 0.5, te.zCoord + 0.5);
                ...
                if (ReikaRenderHelper.renderFrustrum.isBoundingBoxInFrustum(box)) {
                    ...
                    if (LOS.isClearLineOfSight(te.worldObj)) {
                        ReikaPacketHelper.sendPacketToServer(
                            ...
                            ChromaPackets.PROGSYNC.ordinal(),
                            te,
                            p.ordinal()
                        );
                        progressPacketCooldown++;
