...
public enum ProgressStage implements ProgressElement, ProgressAccess {
    ...
    CRYSTALS(
        Shareability.SELFONLY,
        Reloadability.NEVER,
        ChromaBlocks.CRYSTAL.getStackOfMetadata(CrystalElement.RED.ordinal())
    ), //Found a crystal
    DYETREE(
        Shareability.SELFONLY,
        Reloadability.NEVER,
        ChromaBlocks.DYELEAF.getStackOfMetadata(CrystalElement.YELLOW.ordinal())
    ), //Harvest a dye tree
    MULTIBLOCK(
        Shareability.PROXIMITY, Reloadability.TRIGGER, ChromaTiles.STAND
    ), //Assembled a multiblock
    RUNEUSE(
        Shareability.PROXIMITY,
        Reloadability.TRIGGER,
        ChromaBlocks.RUNE.getStackOfMetadata(CrystalElement.ORANGE.ordinal())
    ), //Placed runes
    PYLON(Shareability.SELFONLY, Reloadability.NEVER, ChromaTiles.PYLON), //Found pylon
    LINK(
        Shareability.PROXIMITY, Reloadability.NEVER, ChromaTiles.COMPOUND
    ), //Made a network connection/high-tier crafting
    CHARGE(
        Shareability.SELFONLY, Reloadability.NEVER, ChromaItems.TOOL.getStackOf()
    ), //charge from a pylon
    ABILITY(
        Shareability.SELFONLY, Reloadability.NEVER, ChromaTiles.RITUAL
    ), //use an ability
    RAINBOWLEAF(
        Shareability.PROXIMITY,
        Reloadability.ALWAYS,
        ChromaBlocks.RAINBOWLEAF.getStackOfMetadata(3)
    ), //harvest a rainbow leaf
    MAKECHROMA(Shareability.PROXIMITY, Reloadability.ALWAYS, ChromaTiles.COLLECTOR),
    SHARDCHARGE(
        Shareability.PROXIMITY, Reloadability.NEVER, ChromaStacks.chargedRedShard
    ),
    ...
    MUDHINT(
        Shareability.PROXIMITY, Reloadability.ALWAYS, ChromaBlocks.MUD.getBlockInstance()
    ), //look at chroma mud
    CHROMA(
        Shareability.SELFONLY, Reloadability.NEVER, ChromaBlocks.CHROMA.getBlockInstance()
    ), //step in liquid chroma
    //STONES(		Shareability.SELFONLY,	ChromaStacks.elementUnit), //craft all elemental
    //stones together
    SHOCK(
        Shareability.SELFONLY,
        Reloadability.NEVER,
        ChromaBlocks.PYLONSTRUCT.getStackOfMetadata(5)
    ), //get hit by a pylon
    HIVE(
        Shareability.ALWAYS,
        Reloadability.ALWAYS,
        new ItemStack(ChromaBlocks.HIVE.getBlockInstance()),
        ModList.FORESTRY.isLoaded()
    ),
    NETHER(Shareability.SELFONLY, Reloadability.NEVER, Blocks.portal), //go to the nether
    END(Shareability.SELFONLY, Reloadability.NEVER, Blocks.end_portal_frame
    ), //go to the end
    TWILIGHT(
        Shareability.SELFONLY,
        Reloadability.NEVER,
        ModList.TWILIGHT.isLoaded() ? ModWoodList.CANOPY.getItem().asItemStack() : null,
        ModList.TWILIGHT.isLoaded()
    ), //Go to the twilight forest
    BEDROCK(Shareability.PROXIMITY, Reloadability.ALWAYS, Blocks.bedrock), //Find bedrock
    CAVERN(
        Shareability.ALWAYS,
        Reloadability.ALWAYS,
        ChromaBlocks.STRUCTSHIELD.getStackOfMetadata(BlockType.CLOAK.metadata)
    ), //Cavern structure
    BURROW(
        Shareability.ALWAYS,
        Reloadability.ALWAYS,
        ChromaBlocks.STRUCTSHIELD.getStackOfMetadata(BlockType.MOSS.metadata)
    ), //Burrow structure
    OCEAN(
        Shareability.ALWAYS,
        Reloadability.ALWAYS,
        ChromaBlocks.STRUCTSHIELD.getStackOfMetadata(BlockType.GLASS.metadata)
    ), //Ocean floor structure
    DESERTSTRUCT(
        Shareability.ALWAYS,
        Reloadability.ALWAYS,
        ChromaBlocks.STRUCTSHIELD.getStackOfMetadata(BlockType.COBBLE.metadata)
    ),
    SNOWSTRUCT(
        Shareability.ALWAYS,
        Reloadability.ALWAYS,
        ChromaBlocks.STRUCTSHIELD.getStackOfMetadata(BlockType.LIGHT.metadata)
    ),
    BIOMESTRUCT(
        Shareability.ALWAYS, Reloadability.ALWAYS, ChromaBlocks.COLORLOCK.getStackOf()
    ),
    DIE(Shareability.SELFONLY, Reloadability.NEVER, Items.skull), //die and lose energy
    ALLCOLORS(
        Shareability.SELFONLY,
        Reloadability.NEVER,
        ChromaItems.ELEMENTAL.getStackOf(CrystalElement.CYAN)
    ), //find all colors
    REPEATER(
        Shareability.ALWAYS, Reloadability.TRIGGER, ChromaTiles.REPEATER
    ), //craft any repeater type
    RAINBOWFOREST(
        Shareability.PROXIMITY,
        Reloadability.ALWAYS,
    ...
    DIMENSION(
        Shareability.SELFONLY, Reloadability.NEVER, ChromaBlocks.PORTAL.getBlockInstance()
    ),
    ...
    POWERTREE(
        Shareability.PROXIMITY,
        Reloadability.TRIGGER,
        ChromaBlocks.POWERTREE.getStackOfMetadata(CrystalElement.YELLOW.ordinal())
    ),
    TURBOCHARGE(Shareability.PROXIMITY, Reloadability.NEVER, ChromaTiles.PYLONTURBO),
    FINDSPAWNER(
        Shareability.PROXIMITY, Reloadability.NEVER, new ItemStack(Blocks.mob_spawner)
    ),
    BREAKSPAWNER(
        Shareability.ALWAYS,
        Reloadability.ALWAYS,
        new ItemStack(
            Items.spawn_egg, 1, (int) EntityList.classToIDMapping.get(EntitySpider.class)
        )
    ),
    KILLDRAGON(
        Shareability.PROXIMITY, Reloadability.ALWAYS, new ItemStack(Blocks.dragon_egg)
    ),
    KILLWITHER(
        Shareability.PROXIMITY, Reloadability.ALWAYS, new ItemStack(Items.nether_star)
    ),
    KILLMOB(Shareability.SELFONLY, Reloadability.NEVER, new ItemStack(Items.skull, 1, 4)),
    ALLCORES(
        Shareability.SELFONLY,
        Reloadability.NEVER,
        ChromaTiles.DIMENSIONCORE
            .getCraftedNBTProduct("color", CrystalElement.RED.ordinal())
    ),
    USEENERGY(Shareability.PROXIMITY, Reloadability.NEVER, ChromaTiles.WEAKREPEATER),
    BLOWREPEATER(
        Shareability.PROXIMITY, Reloadability.ALWAYS, ChromaStacks.crystalPowder
    ),
    STRUCTCOMPLETE(
        Shareability.SELFONLY, Reloadability.NEVER, ChromaBlocks.DIMDATA.getStackOf()
    ),
    NETHERROOF(Shareability.SELFONLY, Reloadability.NEVER, Blocks.netherrack),
    NETHERSTRUCT(
        Shareability.PROXIMITY, Reloadability.ALWAYS, new ItemStack(Blocks.nether_brick)
    ),
    VILLAGECASTING(
        Shareability.PROXIMITY, Reloadability.ALWAYS, new ItemStack(Blocks.cobblestone)
    ),
    FOCUSCRYSTAL(
        Shareability.ALWAYS, Reloadability.TRIGGER, new ItemStack(Items.emerald)
    ),
    ...
    STRUCTCHEAT(
        Shareability.SELFONLY, Reloadability.NEVER, Blocks.tnt
    ), //optional, just to rub it in
    VOIDMONSTER(
        Shareability.PROXIMITY,
        Reloadability.NEVER,
        (ItemStack) null,
        ModList.VOIDMONSTER.isLoaded()
    ),
    VOIDMONSTERDIE(
        Shareability.PROXIMITY,
        Reloadability.ALWAYS,
    ...
    LUMA(
        Shareability.SELFONLY, Reloadability.NEVER, ChromaBlocks.LUMA.getBlockInstance()
    ),
    WARPNODE(
        Shareability.SELFONLY,
        Reloadability.NEVER,
        ChromaBlocks.WARPNODE.getBlockInstance()
    ),
    ...
    RELAYS(
        Shareability.PROXIMITY,
        Reloadability.TRIGGER,
        ChromaTiles.RELAYSOURCE.getCraftedProduct()
    ),
    ENERGYIDEA(Shareability.SELFONLY, Reloadability.NEVER, (ItemStack) null),
    NODE(
        Shareability.PROXIMITY,
        Reloadability.NEVER,
        ModList.THAUMCRAFT.isLoaded() ? ThaumItemHelper.BlockEntry.NODE.getItem() : null,
        ModList.THAUMCRAFT.isLoaded()
    ),
    POTION(
        Shareability.SELFONLY,
        Reloadability.NEVER,
        ReikaPotionHelper.getPotionItem(Potion.regeneration, true, true, false)
    ),
    ...
    MYST(
        Shareability.PROXIMITY,
        Reloadability.NEVER,
        ModList.MYSTCRAFT.isLoaded() ? MystCraftHandler.getInstance().bookID : null,
        ModList.MYSTCRAFT.isLoaded()
    ),
    FARLANDS(
        Shareability.PROXIMITY, Reloadability.NEVER, DimDecoTypes.CLIFFGLASS.getItem()
    ),
    NEVER(
        Shareability.SELFONLY, Reloadability.NEVER, (ItemStack) null, false
    ), //used as a no-trigger placeholder
    ;
    ...
    private ProgressStage(
        Shareability s, Reloadability r, ChromaTiles t, boolean... cond
    ) {
        this(s, r, t.getCraftedProduct(), cond);
    }

    private ProgressStage(
        Shareability s, Reloadability r, ItemStack is, boolean... cond
    ) {
        icon = is;
        boolean flag = true;
    ...
    public boolean isPlayerAtStage(World world, UUID id) {
        ...
        if (ep == null && world instanceof WorldServer) {
            ep = ReikaPlayerAPI.getFakePlayerByNameAndUUID(
                (WorldServer) world, "Progress Backup", id
            );
        }
    ...
    public void renderIcon(RenderItem ri, FontRenderer fr, int x, int y) {
        if (this == ANYSTRUCT) { //item render does not work
            ...
            ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                x, y, ChromaIcons.SPINFLARE.getIcon(), 16, 16
            ); //render directly
            GL11.glPopAttrib();
            ...
            if (this == VOIDMONSTERDIE) {
                ReikaGuiAPI.instance.drawItemStack(
                    ri, fr, new ItemStack(Items.skull), x, y
                );
            }
            GL11.glPopAttrib();
            ...
            ReikaGuiAPI.instance.drawItemStack(
                ri, fr, ChromaTiles.WEAKREPEATER.getCraftedProduct(), x, y
            );
            ReikaTextureHelper.bindTerrainTexture();
            GL11.glPushAttrib(GL11.GL_ALL_ATTRIB_BITS);
            GL11.glColor4f(1, 1, 1, 1);
    ...
    public void renderIconInWorld(Tessellator v5, double s, int x, int y, float f) {
        if (this == ANYSTRUCT) { //item render does not work
            ...
            v5.startDrawingQuads();
            v5.setColorOpaque_I(
                ReikaColorAPI.getColorWithBrightnessMultiplier(0xffffff, f)
            );
            ...
            boolean block = (r != null && r.isPlacer())
                || Block.getBlockFromItem(is.getItem()) != Blocks.air;
    ...
    private void setItemRBTintField(boolean flag) {
        RenderItem ri
            = (RenderItem) RenderManager.instance.entityRenderMap.get(EntityItem.class);
        ...
        public boolean canShareTo(EntityPlayer from, EntityPlayer to) {
            switch (this) {
                case ALWAYS:
                    return true;
                case PROXIMITY:
                    return from.worldObj.provider.dimensionId
                        == to.worldObj.provider.dimensionId
                        && to.getDistanceSqToEntity(from) <= 576;
                case SELFONLY:
                    return false;
            }
