...
public class ProgressionManager implements ProgressRegistry {
    ...
    private final MultiMap<String, ProgressStage> playerMap
        = new MultiMap(CollectionType.HASHSET);

    private final EnumMap<CrystalElement, ColorDiscovery> colorDiscoveries
        = new EnumMap(CrystalElement.class);
    private final EnumMap<CrystalElement, StructureComplete> structureFlags
        = new EnumMap(CrystalElement.class);

    private final EnumMap<ProgressStage, ChromaResearch> auxiliaryReference
        = new EnumMap(ProgressStage.class);

    private final HashMap<ProgressStage, ResearchLevel> nonGatingProgress = new HashMap();

    //private final Comparator<EntityPlayer> playerProgressionComparator = new
    //PlayerProgressionComparator();
    ...
    private void load() {
        ...
        if (ProgressStage.VOIDMONSTER.active) {
            ...
            this.addProgressPrereq(
                ProgressStage.VOIDMONSTERDIE, ProgressStage.VOIDMONSTER
            );
        }
        ...
        for (int i = 0; i < ProgressStage.list.length; i++) {
            ...
            if (p.active && !progressMap.hasElementAsParent(pl)
                && !progressMap.hasElementAsChild(pl)) {
        ...
        this.addChainedProgression(
            ProgressStage.BYPASSWEAK, ProgressStage.BLOWREPEATER, false, false
        );
        this.addChainedProgression(
            ProgressStage.TUNECAST,
            ProgressStage.BYPASSWEAK,
            true,
            false,
            ProgressStage.BLOWREPEATER
        );
        this.addChainedProgression(
            ProgressStage.FOCUSCRYSTAL, ProgressStage.ENERGYIDEA, true, true
        );
        this.addChainedProgression(
            ProgressStage.RELAYS, ProgressStage.ENERGYIDEA, true, false
        );
    }
    ...
    private void addChainedProgression(
        ProgressStage hook,
        ProgressStage chain,
        boolean notify,
        boolean retro,
        ...
        if (!parents.contains(hook))
            progressMap.addParent(
                new ProgressLink(chain), new ProgressLink(hook, LineType.DASHED)
            );
        if (retro) {
            for (ProgressStage par : parents) {
                chains.addValue(
                    par, new ProgressChain(par, hook, chain, notify, false, excl)
                );
            }
        }
    }
    ...
    public Topology getTopology() {
        return progressMap.getTopology(/*this.constructResearchLevelDepthMap(5)*/
        ); //.sort(new AlphabeticalProgressComparator());
    }
    ...
    public HashMap<ProgressLink, ProgressTier> constructResearchLevelDepthMap() {
        ...
        for (ProgressStage p : ProgressStage.list) {
            ResearchLevel rl
                = ChromaResearchManager.instance.getEarliestResearchLevelRequiring(p);
            if (rl != null) {
        ...
        while (it.hasNext()) {
            ...
            try {
                ...
                ChromatiCraft.logger.logError(
                    "Could not load progress stage from NBT String " + val
                    + "; was it removed?"
                );
                it.remove();
            }
        ...
        do {
            ...
            while (it.hasNext()) {
                ...
                if (!this.playerHasPrerequisites(ep, p)) {
                    ...
                    changed = true;
                    ChromatiCraft.logger.logError(
                        "Player " + s + " had progress element " + p
                        + " without its prereqs! Removing!"
                    );
                }
            }
        } while (changed);
    ...
    private NBTTagList getNBTList(EntityPlayer ep) {
        ...
        if (nbt == null) {
            ChromatiCraft.logger.logError(
                "Looking for progression data on player " + ep.getCommandSenderName()
                + ", with no NBT?!"
            );
    ...
    public boolean
    isProgressionEqual(EntityPlayer ep1, EntityPlayer ep2, ProgressStage... ignore) {
    ...
    boolean
    stepPlayerTo(EntityPlayer ep, ProgressStage s, boolean notify, boolean syncToCoop) {
        if (ep == null) {
            ChromatiCraft.logger.logError(
                "Tried to give progress '" + s + "' to null player???"
            );
            return false;
        }
    ...
    private boolean chainProgressTo(
        EntityPlayer ep, ProgressChain chained, boolean notify, boolean syncToCoop
    ) {
        if (this.isPlayerAtStage(ep, chained.prereq)) {
            ...
            this.stepPlayerTo(
                ep, chained.progress, notify && chained.notifyOnTrigger, syncToCoop
            );
            return true;
        }
        return false;
    ...
    public void
    setPlayerStageClient(EntityPlayer ep, ProgressStage s, boolean set, boolean notify) {
        this.setPlayerStage(ep, s, set, true, notify, false);
    }

    public void setPlayerStage(
        EntityPlayer ep, ProgressStage s, boolean set, boolean notify, boolean syncToCoop
    ) {
    ...
    private void setPlayerStage(
        ...
        boolean notify,
        boolean syncToCoop,
        HashSet<UUID> activeList
    ) {
        //ReikaJavaLibrary.pConsole("Giving "+ep.getCommandSenderName()+" progress
        //'"+s+"': "+set+"/"+allowClient+"/"+notify); ReikaJavaLibrary.pConsole("NBT PRE:
        //"); for (String sg :
        //ReikaNBTHelper.parseNBTAsLines(ChromaResearchManager.instance.getRootNBTTag(ep)))
        ...
        if (syncToCoop) {
            ...
            Collection<EntityPlayer> players
                = ProgressionLinking.instance.getShareablePlayers(ep, s);
            for (EntityPlayer e : players) {
                ChromatiCraft.logger.debug(
                    "Sharing progression " + s + " from " + ep.getCommandSenderName()
                    + " to " + e.getCommandSenderName()
                );
        ...
        if (notify && ep instanceof EntityPlayerMP)
            ReikaPacketHelper.sendDataPacket(
                ...
                ChromaPackets.GIVEPROGRESS.ordinal(),
                (EntityPlayerMP) ep,
                s.ordinal(),
                set ? 1 : 0
            );
        ...
        if (set) {
            //ReikaJavaLibrary.pConsole(ReikaStringParser.padToLength(tag.toString(), 24,
            //" ")+" out of "+li+",\t=\t"+li.tagList.contains(tag));
        ...
        if (flag) {
            ...
            if (set) {
                ...
                MinecraftForge.EVENT_BUS.post(
                    new ProgressionEvent(ep, s.name(), ResearchType.PROGRESS)
                );
            } else {
    ...
    public void resetPlayerProgression(EntityPlayer ep, boolean notify) {
        ...
        for (int i = 0; i < CrystalElement.elements.length; i++) {
            ...
            this.markPlayerCompletedStructureColor(
                ep, null, CrystalElement.elements[i], false, notify
            );
        }
    ...
    public void maxPlayerProgression(EntityPlayer ep, boolean notify) {
        ...
        for (int i = 0; i < CrystalElement.elements.length; i++) {
            ...
            this.markPlayerCompletedStructureColor(
                ep, null, CrystalElement.elements[i], true, notify
            );
        }
    ...
    public boolean setPlayerDiscoveredColor(
        EntityPlayer ep, CrystalElement e, boolean disc, boolean notify
    ) {
        ...
        if (had != disc) {
            ...
            if (disc && notify)
                ChromaResearchManager.instance.notifyPlayerOfProgression(
                    ep, colorDiscoveries.get(e)
                );
            if (disc)
                MinecraftForge.EVENT_BUS.post(
                    new ProgressionEvent(ep, e.name(), ResearchType.COLOR)
                );
            ProgressionLoadHandler.instance.updateProgressCache(ep);
            ProgressionLoadHandler.instance.updateBackup(ep);
            return true;
    ...
    public void updateChunks(EntityPlayer ep) {
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT)
            ...
            ReikaPacketHelper.sendDataPacket(
                ...
                PacketIDs.RERENDER.ordinal(),
                new PacketTarget.PlayerTarget((EntityPlayerMP) ep),
                1
            );
    }
    ...
    public boolean hasPlayerDiscoveredColor(EntityPlayer ep, CrystalElement e) {
        NBTTagCompound nbt
            = ChromaResearchManager.instance.getRootNBTTag(ep).getCompoundTag(
                COLOR_NBT_TAG
            );
    ...
    public static class ColorDiscovery implements ProgressElement {
        ...
        public void renderIcon(RenderItem ri, FontRenderer fr, int x, int y) {
            ReikaGuiAPI.instance.drawItemStack(
                ri, fr, ChromaBlocks.RUNE.getStackOfMetadata(color.ordinal()), x, y
            );
        }
    ...
    public static class StructureComplete implements ProgressElement {
        ...
        @Override
        public boolean equals(Object o) {
            return o instanceof StructureComplete
                && ((StructureComplete) o).color == color;
        }
        ...
        @Override
        public boolean giveToPlayer(EntityPlayer ep, boolean notify) {
            return instance.markPlayerCompletedStructureColor(
                ep, null, color, true, notify
            );
        }
    ...
    private static class ProgressChain {
        ...
        @Override
        public boolean equals(Object o) {
            return o instanceof ProgressChain && ((ProgressChain) o).progress == progress;
        }
    ...
    public boolean playerHasResearch(EntityPlayer ep, String key) {
        try {
            ...
            ChromatiCraft.logger.logError(
                "A mod tried to fetch an invalid progress stage '" + key + "'!"
            );
            e.printStackTrace();
            return false;
        }
    ...
    public HashSet<String> getPrerequisites(String key) {
        try {
            ...
            ChromatiCraft.logger.logError(
                "A mod tried to fetch the state of an invalid progress stage '" + key
                + "'!"
            );
            e.printStackTrace();
            return null;
    ...
    public boolean hasPlayerDiscoveredAGeneratedStructure(EntityPlayer ep) {
        return ProgressStage.BURROW.isPlayerAtStage(ep)
            || ProgressStage.CAVERN.isPlayerAtStage(ep)
            || ProgressStage.OCEAN.isPlayerAtStage(ep)
            || ProgressStage.DESERTSTRUCT.isPlayerAtStage(ep);
    }
    ...
    public boolean markPlayerCompletedStructureColor(
        EntityPlayer ep,
        DimensionStructureGenerator gen,
        CrystalElement e,
        boolean set,
        boolean notify
    ) {
        ...
        if (had != set) {
            ...
            if (set && notify)
                ChromaResearchManager.instance.notifyPlayerOfProgression(
                    ep, structureFlags.get(e)
                );
            if (set)
                MinecraftForge.EVENT_BUS.post(
                    new ProgressionEvent(ep, e.name(), ResearchType.DIMSTRUCT)
                );
            ProgressionLoadHandler.instance.updateProgressCache(ep);
            ProgressionLoadHandler.instance.updateBackup(ep);
            return true;
    ...
    private void
    triggerStructurePassword(EntityPlayer ep, DimensionStructureGenerator gen) {
        if (ep instanceof EntityPlayerMP) {
            int hex = gen.getPassword(ep);
            ReikaPacketHelper.sendDataPacket(
                ChromatiCraft.packetChannel,
                ChromaPackets.STRUCTPASSNOTE.ordinal(),
                (EntityPlayerMP) ep,
                hex
            );
        }
