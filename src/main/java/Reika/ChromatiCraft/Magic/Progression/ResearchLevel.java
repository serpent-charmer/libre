...
public enum ResearchLevel implements ProgressElement {
        ...
        switch (this) {
            ...
            case ENERGY:
                return ProgressStage.CHARGE.isPlayerAtStage(ep)
                    && ProgressStage.PYLON.isPlayerAtStage(ep);
            case BASICCRAFT:
    ...
    public String getDisplayName() {
        return StatCollector.translateToLocal(
            "chromaresearch." + this.name().toLowerCase(Locale.ENGLISH)
        );
    }
    ...
    public ResearchLevel post() {
        return this.ordinal() < levelList.length - 1 ? levelList[this.ordinal() + 1]
                                                     : this;
    }
    ...
    public void renderIcon(RenderItem ri, FontRenderer fr, int x, int y) {
        ReikaGuiAPI.instance.drawItemStack(
            ri, fr, ChromaItems.FRAGMENT.getStackOf(), x, y
        );
    }
