...
public class ReservoirHandlers {
    ...
    private static abstract class ChromaReservoirRecipeHandlerBase
        ...
        public final int onTick(TileEntity te, FluidStack stored, EntityPlayer owner) {
            if (stored != null && stored.amount >= 1000
                ...
                float sp = bk != null
                        && bk.matchInWorld(
                            te.worldObj, te.xCoord, te.yCoord - 1, te.zCoord
                        )
                    ? this.getRuneSpeedScale()
                    : 1;
        ...
        doTick(TileEntity te, FluidStack fs, EntityPlayer owner, float speed);
    ...
    public static class ChromaPrepHandler extends ChromaReservoirRecipeHandlerBase {
        ...
        doTick(TileEntity te, FluidStack fs, EntityPlayer owner, float speed) {
            ...
            CrystalElement e
                = dye > 0 ? CrystalElement.elements[fs.tag.getInteger("element")] : null;
            AxisAlignedBB box
                = ReikaAABBHelper.getBlockAABB(te.xCoord, te.yCoord, te.zCoord);
            List<EntityItem> li
                = te.worldObj.getEntitiesWithinAABB(EntityItem.class, box);
            boolean flag = false;
            for (EntityItem ei : li) {
                ItemStack is = ei.getEntityItem();
                if (!te.worldObj.isRemote && rand.nextInt(5) == 0
                    ...
                    if (ether < TileEntityChroma.ETHER_SATURATION && is.stackSize > 0) {
                        ...
                        ReikaPacketHelper.sendDataPacketWithRadius(
                            ...
                            te,
                            24,
                            ReikaParticleHelper.SPELL.ordinal(),
                            8
                        );
                    }
                    if (is.stackSize <= 0)
    ...
    public static class ShardBoostingHandler extends ChromaReservoirRecipeHandlerBase {
        ...
        doTick(TileEntity te, FluidStack fs, EntityPlayer owner, float speed) {
            ...
            for (EntityItem ei : li) {
                ItemStack is = ei.getEntityItem();
                if (e != null && is.getItemDamage() == e.ordinal()
                    && ChromaItems.SHARD.matchWith(is)
                    && dye == TileEntityChroma.BERRY_SATURATION) {
                    if (ItemCrystalShard.canCharge(ei)) {
                        boolean done = false;
                        for (int i = 0; i < ACCEL_FACTOR * speed && !done; i++) {
                            done = ItemCrystalShard.tickShardCharging(
                                ei, e, ether, te.xCoord, te.yCoord, te.zCoord
                            );
                        }
        ...
        protected BlockKey getRune(FluidStack fs) {
            ...
            CrystalElement e
                = dye > 0 ? CrystalElement.elements[fs.tag.getInteger("element")] : null;
            return e != null
                ? new BlockKey(ChromaBlocks.RUNE.getBlockInstance(), e.ordinal())
                : null;
        }
            ...
            if (rand.nextInt(3) == 0) {
                ...
                for (EntityItem ei : li) {
                    if (PoolRecipes.instance.canAlloyItem(ei)) {
                        PoolRecipe pr
                            = PoolRecipes.instance.getPoolRecipe(ei, li, false, owner);
                        if (pr != null) {
                            if (ei.worldObj.isRemote) {
                                ...
                                PoolRecipes.instance.makePoolRecipe(
                                    ei, pr, ether, te.xCoord, te.yCoord, te.zCoord
                                );
                                fs.tag = null;
                                return 1000;
                            }
