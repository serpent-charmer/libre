...
public class AuroraHandler {
    ...
    private AuroraHandler() {
        ...
        if (mod.isLoaded()) {
            try {
                network
                    = Class.forName("org.blockartistry.mod.DynSurround.network.Network");

                data = Class.forName("org.blockartistry.mod.DynSurround.data.AuroraData");

                dimData = Class.forName(
                    "org.blockartistry.mod.DynSurround.data.DimensionEffectData"
                );

                colors
                    = Class.forName("org.blockartistry.mod.DynSurround.data.ColorPair");
                presets
                    = Class.forName("org.blockartistry.mod.DynSurround.data.AuroraPreset"
                    );

                color = Class.forName("org.blockartistry.mod.DynSurround.util.Color");

                dataCtr = data.getDeclaredConstructor(
                ...
                if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT
                    || DragonAPICore.isSinglePlayer()) {
                    aurora = Class.forName(
                        "org.blockartistry.mod.DynSurround.client.aurora.Aurora"
                    );
                    handler = Class.forName(
                        "org.blockartistry.mod.DynSurround.client.AuroraEffectHandler"
                    );
                    renderer = Class.forName(
                        "org.blockartistry.mod.DynSurround.client.aurora.AuroraRenderer"
                    );
                    ...
                    render
                        = renderer.getDeclaredMethod("renderAurora", float.class, aurora);
    ...
    public void sendClear(World world) throws Exception {
        ...
        ReikaPacketHelper.sendDataPacket(
            ...
            ChromaPackets.AURORACLEAR.ordinal(),
            new DimensionTarget(world)
        );
    }
    ...
    public Object addAurora(World world, int x, int z) throws Exception {
        return this.addAurora(
            world, x, z, this.getRandomColorID(), this.getRandomPresetID()
        );
    }

    public Object addAurora(World world, int x, int z, int colors, int preset)
        throws Exception {
    ...
    public Object constructData(World world, int x, int z, int colors, int preset)
        throws Exception {
