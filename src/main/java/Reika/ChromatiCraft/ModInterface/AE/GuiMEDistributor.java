...
public class GuiMEDistributor extends GuiChromaBase {
    ...
    public void initGui() {
        ...
        for (int i = 0; i < texts.length; i++) {
            ...
            int y = k + 16 + (i % 5) * 20;
            texts[i]
                = new GuiTextField(ChromaFontRenderer.FontType.GUI.renderer, x, y, w, h);
        ...
        for (int i = 0; i < med.NSLOTS; i++) {
            ...
            buttonList.add(
                new ImagedGuiButton(i, x, y, 10, 10, u, 96, tex, ChromatiCraft.class)
                    .setTooltip("Match Mode")
            );
        }
    }
    ...
    protected void actionPerformed(GuiButton b) {
        ...
        if (b.id < med.NSLOTS) {
            ...
            ReikaPacketHelper.sendPacketToServer(
                ...
                ChromaPackets.MEDISTRIBFUZZY.ordinal(),
                med,
                b.id
            );
        }
    ...
    private void parseAndSend(int slot) {
        ...
        ReikaPacketHelper.sendPacketToServer(
            ChromatiCraft.packetChannel,
            ChromaPackets.MEDISTRIBTHRESH.ordinal(),
            med,
            slot,
            data
        );
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        for (int i = 0; i < med.NSLOTS; i++) {
            ...
            api.drawItemStack(
                itemRender, fontRendererObj, med.getMapping(i + med.NSLOTS), dx, dy
            );
            String s = null;
            if (api.isMouseInBox(
                    j + dx - 1, j + dx - 1 + 17, k + dy - 1, k + dy + 18 - 1
                )) {
                s = "Query";
            } else if (api.isMouseInBox(
                           j + dx + w - 1, j + dx + w + 16, k + dy - 1, k + dy + 18 - 1
                       )) {
                s = "Transfer";
            } else if (api.isMouseInBox(
                           j + dx - 1 + 18, j + dx + w - 2, k + dy - 1, k + dy + 18 - 1
                       )) {
            ...
            if (s != null)
                api.drawTooltipAt(
                    fontRendererObj,
                    s,
                    api.getMouseRealX() + fontRendererObj.getStringWidth(s) - 80,
