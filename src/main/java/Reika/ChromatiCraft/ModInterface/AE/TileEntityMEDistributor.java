...
public class TileEntityMEDistributor
    extends TileEntityChromaticBase implements IActionHost, SidePlacedTile {
    ...
    public TileEntityMEDistributor() {
        if (ModList.APPENG.isLoaded()) {
            aeGridBlock
                = new DirectionalAEInterface(this, this.getTile().getCraftedProduct());
            ...
            if (network != null) {
                ...
                if (hasWork.hasWork()) {
                    if (checkTimer.checkCap()) {
                        ...
                        output.clear();
                        TileEntity te
                            = this.getAdjacentTileEntity(this.getFacing().getOpposite());
                        ...
                        for (int i = 0; i < NSLOTS; i++) {
                            ...
                            if (f1 != null && f2 != null) {
                                int fit = f2.getMaxStackSize()
                                    - output.addItemsToUnderlyingInventories(
                                        ReikaItemHelper.getSizedItemStack(
                                            f2, f2.getMaxStackSize()
                                        ),
                                        true
                                    );
                                if (fit > 0) {
                                    ...
                                    long has = mode.countItems(
                                        network, f1
                                    ); //this.isFuzzy(i) ? network.getFuzzyItemCount(f1,
                                       //FuzzyMode.IGNORE_ALL, this.useOreDict(i)) :
                                       //network.getItemCount(f1);
                                    int missing = this.getThreshold(i)
                                        - (has > Integer.MAX_VALUE ? Integer.MAX_VALUE
                                                                   : (int) has);
                                    if (missing > 0) {
                                        this.transferItem(
                                            ReikaItemHelper.getSizedItemStack(
    ...
    private void buildCache() {
        if (ModList.APPENG.isLoaded()) {
            ((DirectionalAEInterface) aeGridBlock)
                .disconnectAll()
                .connect(this.getFacing());
    ...
    private void transferItem(ItemStack is, IInventory ii, MatchMode mode) {
        ...
        if (rem != null) {
            ...
            ReikaPacketHelper.sendDataPacketWithRadius(
                ...
                ChromaPackets.METRANSFER.ordinal(),
                this,
                32,
                Item.getIdFromItem(add.getItem()),
                add.getItemDamage()
            );
    ...
    @SideOnly(Side.CLIENT)
    public void
    spawnTransferParticles(World world, int x, int y, int z, int id, int meta) {
        ...
        if (tag == null || tag.isEmpty()) {
            ...
            for (CrystalElement e : tag.elementSet()) {
                ...
                double dy = y - dir.offsetY + rand.nextDouble();
                double dz = z - dir.offsetZ + rand.nextDouble();
                EntityFX fx = new EntityCCBlurFX(e, world, dx, dy, dz, 0, 0, 0)
                                  .setRapidExpand()
                                  .setLife(l)
                                  .setScale(s);
                Minecraft.getMinecraft().effectRenderer.addEffect(fx);
