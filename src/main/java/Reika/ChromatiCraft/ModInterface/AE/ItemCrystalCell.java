...
public class ItemCrystalCell
    extends ItemChromaTool implements SpriteRenderCallback, IStorageCell {
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        if (ModList.APPENG.isLoaded()) {
            ...
            IMEInventoryHandler inv
                = icr.getCellInventory(is, null, StorageChannel.ITEMS);
            ...
            li.add(String.format(
                "%s of %s item bytes used.", usedBytes, cellInv.getTotalBytes()
            ));
            li.add(String.format(
                "%s of %s item types used.",
        ...
        if (ModList.APPENG.isLoaded()) {
            ...
            return in == null
                ...
                : new ItemStack(
                    in.getItem(),
                    1,
                    CrystalElement.WHITE.ordinal() + 16 * (in.getItemDamage() / 16)
                );
        } else {
            return null;
        }
    ...
    public static boolean isItemValid(ItemStack cell, ItemStack is) {
        if (ModList.APPENG.isLoaded()) {
            ...
            if (ChromaItems.PLACER.matchWith(is)
                && is.getItemDamage() == ChromaTiles.DIMENSIONCORE.ordinal())
                return ReikaItemHelper.matchStacks(is, in);
            return in != null
                ? (in.getItem() == is.getItem()
                   && in.getItemDamage() / 16 == is.getItemDamage() / 16)
                : isTypeStorable(is);
        } else {
            return false;
        }
    ...
    public static boolean isTypeStorable(ItemStack is) {
        if (is.getItem() == Items.dye)
            return true;
        if (is.getItem() instanceof ItemCrystalBasic
            || is.getItem() instanceof ItemBlockCrystalColors
            || is.getItem() instanceof ItemBlockCrystal)
            return true;
        if (is.getItem() instanceof CrystalTypesProxy
            && ((CrystalTypesProxy) is.getItem()).isCrystalType(is))
            return true;
        if (ChromaItems.PLACER.matchWith(is)
            && is.getItemDamage() == ChromaTiles.DIMENSIONCORE.ordinal())
            return true;
        return is.getItem() instanceof ItemBlockDyeTypes
            || is.getItem() instanceof ItemColoredModInteract;
    }
        ...
        if (type == ItemRenderType.INVENTORY) {
            if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
                ...
                if (store != null) {
                    ...
                    store = store.copy();
                    store.setItemDamage(
                        ((store.getItemDamage() / 16) * 16)
                        + (int) ((System.currentTimeMillis() / 250) % 16)
                    );
                    //float hue = (float)(((System.currentTimeMillis()/10D)%360)/360F);
                    //int rgb = Color.HSBtoRGB(hue, 1, 1);
                    //GL11.glColor4f(ReikaColorAPI.getRed(rgb)/255F,
                    //ReikaColorAPI.getGreen(rgb)/255F, ReikaColorAPI.getBlue(rgb)/255F,
                    //1);
                    ReikaGuiAPI.instance.drawItemStack(
                        ri, ReikaItemHelper.getSizedItemStack(store, 1), 0, -16
                    );
