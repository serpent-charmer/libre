...
public class ItemShieldedCell extends ItemChromaTool implements IStorageCell {
    ...
    public ItemShieldedCell(int index) {
        super(index);
    }
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        if (ModList.APPENG.isLoaded()) {
            ...
            IMEInventoryHandler inv = icr.getCellInventory(is, null,
            StorageChannel.ITEMS); ICellInventoryHandler icih =
            (ICellInventoryHandler)inv; ICellInventory cellInv = icih.getCellInv(); long
            usedBytes = cellInv.getUsedBytes();

            li.add(String.format("%s of %s item bytes used.", usedBytes,
            cellInv.getTotalBytes())); li.add(String.format("%s of %s item types used.",
            cellInv.getStoredItemTypes(), cellInv.getTotalItemTypes())); if (usedBytes >
            0L) { li.add(String.format("contains %s items.",
            cellInv.getStoredItemCount()));

                IItemList<IAEItemStack> items =
            cellInv.getAvailableItems(StorageChannel.ITEMS.createList());
                li.add(String.format("Stores %s.",
            items.getFirstItem().getItemStack().getDisplayName()));
            }
             */
        } else {
    ...
    public static ItemStack getStoredItem(ItemStack is) {
        if (ModList.APPENG.isLoaded()) {
            ICellRegistry icr = AEApi.instance().registries().cell();
            IMEInventoryHandler inv = icr.getCellInventory(is, null,
    StorageChannel.ITEMS); ICellInventoryHandler icih = (ICellInventoryHandler)inv;
            ICellInventory cellInv = icih.getCellInv();
            IItemList<IAEItemStack> items =
    cellInv.getAvailableItems(StorageChannel.ITEMS.createList()); return items.isEmpty() ?
    null : items.getFirstItem().getItemStack(); //first since one type per cell
        }
        else {
            return null;
        }
    ...
    public boolean onRender(RenderItem ri, ItemStack is, ItemRenderType type) {
        if (type == ItemRenderType.INVENTORY) {
            if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
                ItemStack store = ItemShieldedCell.getStoredItem(is);
                if (store != null) {
                    double s = 0.063;
                    GL11.glScaled(s, -s, s);
                    ReikaGuiAPI.instance.drawItemStack(ri,
    ReikaItemHelper.getSizedItemStack(store, 1), 0, -16); return true;
                }
            }
        }
        return false;
