...
@Strippable(
    value = { "appeng.api.networking.IGridHost",
              "appeng.api.networking.crafting.ICraftingProvider" }
)
public class TileEntityPatternCache
    extends InventoriedChromaticBase implements IGridHost, ICraftingProvider {
    ...
    private void updateAE() {
        ...
        if (grid != null)
            grid.postEvent(
                new MENetworkCraftingPatternChange(this, (IGridNode) aeGridNode)
            );
    }
        ...
        if (igh instanceof IPart) {
            ...
            if (te instanceof IPartHost) {
                ...
                for (int i = 0; i < 6; i++) {
                    ...
                    if (p == ip) {
                        TileEntity tile = te.worldObj.getTileEntity(
    ...
    public void provideCrafting(ICraftingProviderHelper tracker) {
        for (int i = 0; i < SIZE; i++) {
            ...
            if (this.isPattern(is)) {
                tracker.addCraftingOption(
                    this,
                    ((ICraftingPatternItem) is.getItem()).getPatternForItem(is, worldObj)
                );
            }
        }
    }
    ...
    private boolean isPattern(ItemStack is) {
        return is != null && ModList.APPENG.isLoaded()
