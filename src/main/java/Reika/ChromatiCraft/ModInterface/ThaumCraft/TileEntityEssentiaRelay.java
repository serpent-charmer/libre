...
public class TileEntityEssentiaRelay
    extends TileEntityChromaticBase implements IEssentiaTransport, BreakAction, SneakPop {
    ...
    private static HashMap<WorldCoordinates, ArrayList<WorldCoordinates>>
        essentiaHandlerData;
    ...
    static {
        if (ModList.THAUMCRAFT.isLoaded()) {
            try {
                infusionMatrix
                    = Class.forName("thaumcraft.common.tiles.TileInfusionMatrix");
                essentiaHandler
                    = Class.forName("thaumcraft.common.lib.events.EssentiaHandler");
            } catch (Exception e) {
                ReflectiveFailureTracker.instance.logModReflectiveFailure(
                    ModList.THAUMCRAFT, e
                );
                ChromatiCraft.logger.logError(
                    "Could not access TC infusion matrix classes!"
                );
                e.printStackTrace();
            }
        }
            ...
            if (!world.isRemote) {
                ...
                if (te != null && te.getClass() == EssentiaNetwork.getCentrifugeClass()) {
                    ...
                    if (pull != null && ie.getEssentiaAmount(ForgeDirection.UP) > 0) {
                        EssentiaMovement em
                            = network.addEssentia(this, ForgeDirection.DOWN, pull, 1);
            ...
            for (Coordinate c : this.getVisibleOtherNodes()) {
                if (c.hashCode() >= h.hashCode()) {
                    if (this.getTicksExisted() % n == (c.hashCode() ^ h.hashCode()) % n)
                        this.doConnectionParticles(
                            world, x, y, z, c.xCoord, c.yCoord, c.zCoord, true
                        );
                }
            }
            for (Coordinate c : this.getVisibleEndpoints()) {
                if (this.getTicksExisted() % (n2 + n)
                    == (c.hashCode() ^ h.hashCode()) % (n2 + n))
                    this.doConnectionParticles(
                        world, x, y, z, c.xCoord, c.yCoord, c.zCoord, false
                    );
            }
        }
    }
    ...
    @SideOnly(Side.CLIENT)
    private void doConnectionParticles(
        World world, int x, int y, int z, int x2, int y2, int z2, boolean isNode
    ) {
        for (double d = 0; d <= 1; d += 0.03125) {
            ...
            EntityBlurFX fx = new EntityCCBlurFX(world, dx + 0.5, dy + 0.5, dz + 0.5)
    ...
    private void injectFilteredJar(Coordinate loc, Coordinate c) {
        if (essentiaHandlerData == null) {
            try {
                ...
                essentiaHandlerData
                    = (HashMap<WorldCoordinates, ArrayList<WorldCoordinates>>) f.get(null
                    );
            } catch (Exception e) {
                e.printStackTrace();
            }
        ...
        if (essentiaHandlerData != null) {
            WorldCoordinates key = new WorldCoordinates(
                loc.xCoord, loc.yCoord, loc.zCoord, worldObj.provider.dimensionId
            );
            ...
            WorldCoordinates add = new WorldCoordinates(
                c.xCoord, c.yCoord, c.zCoord, worldObj.provider.dimensionId
            );
            if (!li.contains(add)) {
                li.add(add);
            }
    ...
    private int collectEssentiaToTarget(Aspect a, int amt, Coordinate tgt) {
        ...
        EssentiaMovement r
            = network.removeEssentia(this, ForgeDirection.DOWN, a, amt, tgt);
        if (r != null) {
            for (EssentiaPath p : r.paths()) {
                this.addPath(p);
            }
    ...
    public Aspect getEssentiaType(ForgeDirection face) {
        ...
        return te instanceof IEssentiaTransport
            ? ((IEssentiaTransport) te).getSuctionType(face.getOpposite())
            : null;
    }
    ...
    public final void drop() {
        ...
        //this.getTile().getCraftedProduct()); if (!this.shouldDrop()) 	return;
        if (alreadyDropped)
            return;
        alreadyDropped = true;
