...
public class NodeRecharger implements TickHandler {
    ...
    private final HashMap<Integer, HashMap<WorldLocation, NodeReceiverWrapper>> nodes
        = new HashMap();
    ...
    public void updateClient(WorldLocation loc, NBTTagCompound data) {
        ...
        HashMap<WorldLocation, NodeReceiverWrapper> map
            = this.getOrCreateMap(world.provider.dimensionId);
    ...
    public boolean isValidNode(INode tile) { //energized nodes do not implement INode
        return !(tile instanceof CrystalNetworkTile)
            && !tile.getAspectsBase().aspects.isEmpty();
    }
    ...
    private void loadLocation(NBTTagCompound tag) {
        ...
        if (!(te instanceof INode)) {
            ChromatiCraft.logger.logError(
                "Data saved a TC aura node at " + loc
                + ", but the node was deleted between world save and reload?!"
            );
            return;
        }
        INode n = (INode) te;
    ...
    private void register(WorldLocation loc, NodeReceiverWrapper wrap) {
        HashMap<WorldLocation, NodeReceiverWrapper> map
            = this.getOrCreateMap(loc.dimensionID);
        map.put(loc, wrap);
    }
    ...
    public void renderNodeOverlay(EntityPlayer ep, int gsc, INode te) {
        HashMap<WorldLocation, NodeReceiverWrapper> map
            = nodes.get(ep.worldObj.provider.dimensionId);
        if (map == null)
            return;
    ...
    private boolean removeLocation(WorldLocation loc, World world, boolean save) {
        ...
        HashMap<WorldLocation, NodeReceiverWrapper> map
            = nodes.get(world.provider.dimensionId);
        if (map != null) {
    ...
    public NodeReceiverWrapper getWrapper(WorldLocation loc, boolean create) {
        HashMap<WorldLocation, NodeReceiverWrapper> map
            = create ? this.getOrCreateMap(loc.dimensionID) : nodes.get(loc.dimensionID);
        NodeReceiverWrapper wrap = map.get(loc);
        if (wrap == null && create) {
            TileEntity te = loc.getTileEntity();
            if (te instanceof INode) {
        ...
        for (String s : ((Set<String>)tag.func_150296_c())) {
            ...
            for (Object o : li.tagList) {
                ...
                if (te instanceof INode) {
                    ...
                    ChromatiCraft.logger.logError("Data saved a TC aura node at "+loc+",
    but the node was deleted between world save and reload?!");
                }
            }
        }
    ...
    public static class NodeRechargeData extends WorldSavedData {
        ...
        private static NodeRechargeData initNetworkData(World world) {
            NodeRechargeData data = (NodeRechargeData
            ) world.loadItemData(NodeRechargeData.class, IDENTIFIER);
            if (data == null) {
