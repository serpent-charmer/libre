...
public class TileEntityFluxMaker
                    ...
                    if (timer.checkCap()) {
                        ...
                        if (this.spawnBlock(world, x, y, z, f, amt)) {
                            ...
                            if (rand.nextInt(amt == 1000 ? 2 : 8) == 0) {
                                ...
                                Block b = GameRegistry.findBlock(
                                    ModList.THAUMCRAFT.modLabel, "blockFluxGas"
                                );
            ...
            if (f != null) {
                for (int i = 0; i < 2; i++) {
                    ...
                    EntityFluidFX fx = new EntityFluidFX(
                                           world, x + 0.5, y + 0.5, z + 0.5, vx, vy, vz, f
                    )
                                           .setGravity(0.125F)
                                           .setLife(30)
                                           .setScale(0.5F);
            ...
            while (flag2 && has >= 1000) {
                for (int i = 0; i < 6 && flag2 && has >= 1000; i++) {
                    ...
                    if (b.isAir(world, dx, dy, dz)) {
                        ...
                        if (bf.getFluid() == f
                            && bf.getQuantaPercentage(world, dx, dy, dz) < 1) {
                            world.setBlockMetadataWithNotify(
                                dx, dy, dz, world.getBlockMetadata(dx, dy, dz) + 1, 3
                            );
                            added = true;
                        }
                    }
    ...
    private boolean isItemValid(ItemStack is) {
        if (is.getItem(
            ) == GameRegistry.findItem(ModList.THAUMCRAFT.modLabel, "ItemCrystalEssence")
    ...
    protected void readSyncTag(NBTTagCompound NBT) {
        ...
        HashMap<String, Integer> map
            = (HashMap<String, Integer>) ReikaNBTHelper.readMapFromNBT(
                NBT.getCompoundTag("particles")
            );
