...
public final class NodeReceiverWrapper
    implements CrystalReceiver, NotifiedNetworkTile, WrapperTile, LumenRequestingTile {
    ...
    NodeReceiverWrapper(INode n) {
        ...
        ModularLogger.instance.log(
            LOGGER_ID, "Node wrapper created for node " + location
        );
    }
    ...
    private ElementTagCompound getTagValue(Aspect a) {
        return ChromaAspectManager.instance.getElementCost(a, 1).scale(
            ASPECT_EXPANSION_COST_PER_VIS
        );
    }
    ...
    public void tick() {
        ...
        if (auraLocus != null) {
            if (ChromaTiles.getTile(
                    this.getWorld(), auraLocus.xCoord, auraLocus.yCoord, auraLocus.zCoord
                )
                != ChromaTiles.AURAPOINT)
                auraLocus = null;
        } else {
            ...
            if (ep != null) {
                ...
                for (TileEntityAuraPoint te : c) {
                    if (te.worldObj.provider.dimensionId == location.dimensionID
                        && te.getDistanceFrom(
        ...
        if (rand.nextInt(240) == 0) {
            ...
            ReikaPacketHelper.sendDataPacketWithRadius(
                ...
                ChromaPackets.CHARGINGNODE.ordinal(),
                (TileEntity) node,
                32
            );
        }
        ...
        if (status == NodeImprovementStatus.IDLE
            && rand.nextInt(this.modifyChanceByAge(750)) == 0) {
            this.tryImproveNode();
        }
        ...
        if (status == NodeImprovementStatus.IDLE
            && rand.nextInt(this.modifyChanceByAge(200)) == 0) {
        ...
        if (status == NodeImprovementStatus.IDLE
            && rand.nextInt(this.modifyChanceByAge(6000)) == 0) {
            this.startNewAspect();
        }
        ...
        if (status == NodeImprovementStatus.NEWASPECT && activeAspect != null) {
            if (this.tryToAddAspect(activeAspect)) {
                ModularLogger.instance.log(
                    LOGGER_ID,
                    "Node " + location + " gained aspect '" + activeAspect.getName() + "'"
                );
                activeAspect = null;
            }
            if (age % 48 == 0)
                ChromaSounds.CASTHARMONIC.playSound(
                    this.getWorld(),
                    this.getX() + 0.5,
                    this.getY() + 0.5,
                    this.getZ() + 0.5,
    ...
    private void sync() {
        ...
        ReikaPacketHelper.sendNBTPacket(
            ChromatiCraft.packetChannel,
            ChromaPackets.NODERECEIVERSYNC.ordinal(),
            tag,
            new PacketTarget.RadiusTarget(location, 64)
        );
    }
    ...
    private void writeSync(NBTTagCompound tag) {
        if (currentRequestSet != null) {
            tag.setInteger(
                "requestSet",
                ReikaArrayHelper.booleanToBitflags(currentRequestSet.flagSet())
            );
        }
    }
    ...
    private void doAspectRefill(Aspect a, ElementTagCompound tag) {
        ...
        if (node.getAspects().getAmount(a) >= node.getAspectsBase().getAmount(a)) {
            ModularLogger.instance.log(
                LOGGER_ID,
                "Node " + location + " recharge finish " + activeAspect.getName()
            );
            activeAspect = null;
    ...
    private void startNewAspect() {
        ...
        if (!candidateNextAspects.isEmpty()) {
            ...
            ModularLogger.instance.log(
                LOGGER_ID,
                "Node " + location + " attempting to gain aspect '"
                    + activeAspect.getName() + "'"
            );
        }
    }
    ...
    private int modifyChanceByAge(int base) {
        ...
        return (int) Math.max(
            ...
            Math.max(
                base / 10,
                base / (1 + t / 1.2F * ChromaOptions.getNodeGrowthSpeed() / (double) base)
            )
        );
    }
    ...
    private void doAddAspect(Aspect a) {
        ...
        ReikaPacketHelper.sendStringPacketWithRadius(
            ChromatiCraft.packetChannel,
            ChromaPackets.NEWASPECTNODE.ordinal(),
            (TileEntity) node,
            32,
    ...
    private void damageNode(boolean bad) {
        ChromaSounds.ERROR.playSound(
            ((TileEntity) node).worldObj,
            this.getX() + 0.5,
            this.getY() + 0.5,
            this.getZ() + 0.5,
            0.75F,
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ChromatiCraft.packetChannel,
            ChromaPackets.HURTNODE.ordinal(),
            (TileEntity) node,
            32
        );
        ...
        for (Aspect a : al.aspects.keySet()) {
            ...
            if (amt > 1) {
                int rem = Math.min(
                    amt - 1, (int) (rand.nextFloat() * (bad ? amt / 2F : amt / 4F))
                );
                al.remove(a, rem);
            }
        }
    ...
    private void tryImproveNode() {
        ReikaPacketHelper.sendDataPacketWithRadius(
            ChromatiCraft.packetChannel,
            ChromaPackets.HEALNODE.ordinal(),
            (TileEntity) node,
            32
        );
        ...
        if (status == NodeImprovementStatus.IDLE
            && rand.nextInt(this.modifyChanceByAge(360)) == 0) {
        ...
        for (Aspect a : al.aspects.keySet()) {
            ...
            double f = rand.nextDouble()
                * MathHelper.clamp_double(
                    age / 15000D * ChromaOptions.getNodeGrowthSpeed(), 1, 4
                );
            int newamt = Math.min(
                720, Math.max(amt + 1, (int) (amt + f * Math.sqrt(amt)))
            ); //(int)(amt*(1+rand.nextFloat()/5F));
            if (newamt > amt) {
                al.merge(a, newamt);
                ModularLogger.instance.log(
                    LOGGER_ID,
                    "Node " + location + " aspect '" + a.getName() + "' cap increased to "
                        + newamt
                );
                flag = true;
            }
    ...
    private boolean tryImproveNodeType() {
        ...
        if (storedEnergy.containsAtLeast(typeCost)) {
            ...
            ChromaSounds.DASH.playSoundAtBlock(
                location.getWorld(),
                location.xCoord,
                location.yCoord,
                location.zCoord,
    ...
    private boolean tryImproveNodeBrightness() {
        ...
        if (storedEnergy.containsAtLeast(brightnessCost)) {
            ...
            ModularLogger.instance.log(
                LOGGER_ID, "Node " + location + " brightness change"
            );
            this.setStatus(NodeImprovementStatus.IDLE);
            return true;
        } else {
    ...
    private boolean requestEnergy(ElementTagCompound tag) {
        ...
        for (CrystalElement e : tag.elementSet()) {
            int req = MathHelper.ceiling_float_int(
                tag.getValue(e) / this.getEfficiencyFactor()
            );
            int amt = Math.min(req, this.getRemainingSpace(e));
            flag |= CrystalNetworker.instance.makeRequest(
                this, e, amt, this.getReceiveRange()
            );
        }
        return flag;
    }
    ...
    private void requestForNewAspect(Aspect a, ElementTagCompound tag) {
        ...
        for (CrystalElement e : tag.elementSet()) {
            int req = MathHelper.ceiling_float_int(
                NEW_ASPECT_COST / this.getEfficiencyFactor()
            );
            int amt = Math.min(req, this.getRemainingSpace(e));
            boolean flag = CrystalNetworker.instance.makeRequest(
                this, e, amt, this.getReceiveRange()
            );
        }
    }
    ...
    private void destroyNode() {
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ChromatiCraft.packetChannel,
            ChromaPackets.DESTROYNODE.ordinal(),
            (TileEntity) node,
            32
        );
        ...
        EntityLightningBolt elb
            = new EntityLightningBolt(world, x - 0.5, y, z - 0.5); //already has +0.5
    ...
    public static void triggerNewAspectFX(World world, int x, int y, int z, Aspect a) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            ReikaThaumHelper.triggerEffect(
                EffectType.NODEBOLT, world, (float) dx, (float) dy, (float) dz, fx, fy, fz
            );
        }
        ...
        for (int i = 0; i < 64; i++) {
            ...
            EntityBlurFX fx
                = new EntityCCBlurFX(world, x + 0.5, y + 0.5, z + 0.5, vx, 0, vz)
                      .setColor(color)
                      .setGravity(gv)
                      .setScale(2)
                      .setRapidExpand();
        ...
        EntityCenterBlurFX fx = new EntityCenterBlurFX(world, x + 0.5, y + 0.5, z + 0.5)
                                    .setColor(a.getColor())
                                    .setScale(20);
        fx.noClip = true;
        Minecraft.getMinecraft().effectRenderer.addEffect(fx);
    ...
    public static void triggerDestroyFX(World world, int x, int y, int z) {
        NodeReceiverWrapper wrap
            = new NodeReceiverWrapper((INode) world.getTileEntity(x, y, z));
        for (int i = 0; i < 512; i++) {
            Aspect a = ReikaJavaLibrary.getRandomCollectionEntry(
                rand, wrap.node.getAspects().aspects.keySet()
            );
            ...
            int color = a.getColor();
            EntityBlurFX fx = new EntityCCBlurFX(
                                  world,
                                  x + rand.nextDouble(),
                                  y + rand.nextDouble(),
                                  z + rand.nextDouble(),
        ...
        for (int k = 0; k < 16; k++) {
            ...
            if (wrap.getCurrentValue(e) > 0) {
                for (int i = 0; i < 8; i++) {
                    ...
                    double ry = ReikaRandomHelper.getRandomPlusMinus(0, v);
                    double rz = ReikaRandomHelper.getRandomPlusMinus(0, v);
                    EntityRuneFX fx = new EntityRuneFX(
                        world,
                        x + rand.nextDouble(),
                        y + rand.nextDouble(),
                        z + rand.nextDouble(),
    ...
    public static void triggerDamageFX(World world, int x, int y, int z) {
        ReikaThaumHelper.triggerEffect(
            EffectType.NODEBURST, world, x + 0.5, y + 0.5, z + 0.5, 1F
        );
    }
    ...
    public static void triggerChargingFX(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            EntityLaserFX fx
                = new EntityLaserFX(CrystalElement.WHITE, world, px, py, pz, vx, vy, vz)
                      .setColor(a.getColor());
            fx.noClip = true;
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
        }
        for (int i = 0; i < 6; i++) {
            ...
            ReikaThaumHelper.triggerEffect(
                EffectType.NODEBOLT, world, (float) dx, (float) dy, (float) dz, px, py, pz
            );
        }
    }
    ...
    public static void triggerHealFX(World world, int x, int y, int z) {
        ...
        for (double d = -1; d <= 1; d += 0.125) {
            ...
            int color = a.getColor();
            EntityBlurFX fx = new EntityCCBlurFX(world, dx, dy + d, dz)
                                  .setColor(color)
                                  .setScale(2.5F - 2 * (float) Math.abs(d))
                                  .setRapidExpand()
                                  .setLife(20);
            fx.noClip = true;
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
    ...
    public void onPathConnected(CrystalPath p) {
        ChromaSounds.CAST.playSoundAtBlock(
            this.getWorld(), this.getX(), this.getY(), this.getZ(), 1, 0.65F
        );
    }
    ...
    public void load(NBTTagCompound tag, boolean sync) {
        ...
        if (tag.hasKey("requestSet"))
            currentRequestSet = ElementTagCompound.createFromFlags(
                ReikaArrayHelper.booleanFromBitflags(tag.getInteger("requestSet"), 16), 1
            );
        else if (sync)
            currentRequestSet = null;
    ...
    public void recalculateCandidateAspects() {
        ...
        for (Aspect a : ReikaThaumHelper.getAllAspects()) {
            if (!set.contains(a)) {
                ...
                if (prereqs)
                    candidateNextAspects.addEntry(
                        a,
                        1 + ReikaThaumHelper.getMaxAspectTier()
                            - ReikaThaumHelper.getAspectTier(a)
                    );
            }
        }
    }
    ...
    public void renderOverlay(EntityPlayer ep, int gsc) {
        ...
        Tessellator v5 = Tessellator.instance;
        {
            ...
            ReikaTextureHelper.bindTexture(
                ChromatiCraft.class, "Textures/nodeoverlays.png"
            );
            ...
            if (!candidateNextAspects.isEmpty()) {
                ...
                for (Aspect a : li) {
                    if (a != activeAspect) {
                        ...
                        int ap =
            (int)(255*(0.625+0.375*Math.sin(Math.toRadians(offset+t)))); if (ap > 0) {
                            v5.startDrawingQuads();
        ...
        if (activeAspect != null) {
            ...
            if (activeAspect.isPrimal()) {
                ...
                ico = blend == GL11.GL_ONE_MINUS_SRC_ALPHA
                    ? ChromaIcons.ALPHAHOLE.getIcon()
                    : ChromaIcons.WHITEHOLE.getIcon();
    ...
    public ElementTagCompound getRequestedTotal() {
        switch (status) {
            case NEWASPECT:
                return currentRequestSet != null
                    ? currentRequestSet.copy().scale(NEW_ASPECT_COST)
                    : null;
            case BRIGHTENING:
            ...
            case REFILLING:
                int amt = node.getAspectsBase().getAmount(activeAspect)
                    - node.getAspects().getAmount(activeAspect);
                return activeAspect != null ? this.getTagValue(activeAspect).scale(amt)
                                            : null;
            default:
                return null;
        }
    ...
    public void debug(ArrayList<String> li) {
        ...
        li.add(
            "Active Aspect: " + (activeAspect != null ? activeAspect.getTag() : "None")
        );
