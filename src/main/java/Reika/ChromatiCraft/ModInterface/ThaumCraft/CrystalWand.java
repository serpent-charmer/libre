...
public class CrystalWand extends WandRod {
    ...
    private void registerTexture() {
        this.setTexture(DirectResourceManager.getResource(
            "Reika/ChromatiCraft/Textures/Wands/crystalwand.png"
        ));
    ...
    private static class WandUpdater implements IWandRodOnUpdate {
        ...
        public void onUpdate(ItemStack is, EntityPlayer player) {
            if (ReikaPlayerAPI.isFake(player))
                return;
            int n = ReikaItemHelper.matchStacks(is, player.getCurrentEquippedItem())
                ? 100
                : 800;
            if (rand.nextInt(n) == 0) {
                    ...
                    if (al.getAmount(a) > 0) {
                        ...
                        if (sp > 0) {
                            ElementTagCompound cost
                                = ChromaAspectManager.instance.getElementCost(a, 1)
                                      .scale(sp * 4)
                                      .power(1.25);
            ...
            if (data[e.ordinal()] > 0) {
                ...
                for (int i = 0; i < amt; i++) {
                    double px = ReikaRandomHelper.getRandomPlusMinus(
                        player.posX + vec.xCoord, 1
                    );
                    double py = ReikaRandomHelper.getRandomPlusMinus(
                        player.posY + vec.yCoord, 0.5
                    );
                    double pz = ReikaRandomHelper.getRandomPlusMinus(
                        player.posZ + vec.zCoord, 1
                    );
                    EntityFX fx = new EntityRuneFX(player.worldObj, px, py, pz, e)
                                      .setLife(40)
                                      .setScale(2)
                                      .setGravity(0.1F);
