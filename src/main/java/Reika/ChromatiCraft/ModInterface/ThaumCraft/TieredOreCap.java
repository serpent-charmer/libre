...
public class TieredOreCap extends WandCap {
    ...
    private static void registerTexture(TieredOres t, WandCap cap) {
        cap.setTexture(DirectResourceManager.getResource(
            "Reika/ChromatiCraft/Textures/Wands/cap_"
            + t.name().toLowerCase(Locale.ENGLISH) + ".png"
        ));
    }
    ...
    public static void addRecipes() {
        ...
        for (TieredOres t : items.keySet()) {
            ...
            total.add(al);
            ShapedArcaneRecipe ir = ThaumcraftApi.addArcaneCraftingRecipe(
                "", ReikaItemHelper.getSizedItemStack(wt.item, 2), al, recipe
            );
            ...
            li.add(ir);
            //ReikaThaumHelper.addArcaneRecipeBookEntryViaXML(id.toLowerCase(Locale.ENGLISH),
            //desc, "chromaticraft", ir, cost, 2, i, ChromatiCraft.class, ref);
            i++;
        }
        Collections.sort(li, new RecipeComparator());
        ShapedArcaneRecipe[] recipes = li.toArray(new ShapedArcaneRecipe[li.size()]);
        ReikaThaumHelper
            .addResearchForMultipleRecipesViaXML(
            ...
            .setParents("ROD_silverwood", "CAP_thaumium", "SCEPTRE", "CCCONVERT")
            .setConcealed();
        ThaumcraftApi.addWarpToResearch(
            RESEARCH_ID, ChromaOptions.HARDTHAUM.getState() ? 4 : 2
        );
    }
