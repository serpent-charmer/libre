...
public class ItemAbilityFocus extends ItemCustomFocus {
                ...
                for (Aspect a : ChromaAspectManager.instance.getAspects(e, true)) {
                    ...
                    for (Aspect a2 : al2.aspects.keySet()) {
                        al.add(
                            a2,
                            (int
                            ) (tag.getValue(e) * al2.getAmount(a2) * 20
                               * this.getCostScale(focusStack))
                        );
                    }
                }
            }
    ...
    public ItemStack onFocusRightClick(
        ItemStack wand, World world, EntityPlayer ep, MovingObjectPosition mov
    ) {
        ItemStack focus = ReikaThaumHelper.getWandFocusStack(wand);
        if (focus == null || focus.stackTagCompound == null)
            return null;
        Ability a
            = Chromabilities.getAbility(focus.stackTagCompound.getString("ability"));
        if (a != null && Chromabilities.playerHasAbility(ep, a)
            && Chromabilities.canPlayerExecuteAt(ep, a)) {
            int pwr = focus.stackTagCompound.getInteger("power");
            pwr = Math.min(
                pwr,
                MathHelper.ceiling_float_int(
                    a.getMaxPower()
                    * this.getUpgradeLevel(focus, FocusUpgradeType.potency) / 5F
                )
            );
            Chromabilities.triggerAbility(ep, a, pwr, this.getCostScale(focus));
            ThaumcraftApi.internalMethods.consumeVisFromWand(
                wand, ep, this.getVisCost(focus), true, false
            );
        } else {
    ...
    private float getCostScale(ItemStack focus) {
        return Math.max(
            0.5F, 1.5F - 0.25F * this.getUpgradeLevel(focus, FocusUpgradeType.frugal)
        );
    }

    @Override
    public boolean canApplyUpgrade(
        ItemStack focusstack, EntityPlayer player, FocusUpgradeType type, int rank
    ) {
    ...
    public ItemStack onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
        ep.openGui(
            ChromatiCraft.instance, ChromaGuis.ABILITYFOCUS.ordinal(), world, 0, 0, 0
        );
        return is;
    }
    ...
    private Ability getAbility(ItemStack is) {
        return is.stackTagCompound == null
            ? null
            : Chromabilities.getAbility(is.stackTagCompound.getString("ability"));
    }
    ...
    public int getFocusColor(ItemStack focusstack) {
        return ReikaColorAPI.getModifiedHue(
            0x22aaff, (int) (245 + 35 * Math.sin(System.currentTimeMillis() / 250D))
        );
    }

    /**
     * Does the focus have ornamentation like the focus of the nine hells. Ornamentation
     * is a standard icon rendered in a cross around the focus
     */
    @Override
    @SideOnly(Side.CLIENT)
