...
@Strippable(
    value = { "thaumcraft.api.aspects.IAspectSource",
              "thaumcraft.api.aspects.IEssentiaTransport" }
)
public class TileEntityAspectJar extends TileEntityChromaticBase
    implements IAspectSource, IEssentiaTransport, NBTTile, HitAction {
    ...
    public static class JarTilt {
        ...
        private boolean update() {
            float speed = angle > 45 ? 1 + (float) Math.pow(angle - 25, 1.25) / 4F
                                     : 1 + (maxAngle - angle) / 4F;
            if (increasing) {
                angle += speed;
                if (angle >= maxAngle) {
    ...
    public void onHit(World world, int x, int y, int z, EntityPlayer ep) {
        ReikaSoundHelper.playBreakSound(
            world, x, y, z, ThaumItemHelper.BlockEntry.JAR.getBlock()
        );
        ...
        if (hitCount >= HITS_TO_SPILL
            && tick - hits[hits.length - 1] <= COOLDOWN_TO_SPILL) {
    ...
    private void spill(World world, int x, int y, int z) {
        ...
        if (fill) {
            ReikaSoundHelper.playSoundAtBlock(
                ...
                "game.neutral.swim",
                0.6F,
                (float) ReikaRandomHelper.getRandomPlusMinus(1.25F, 0.5F)
            );
            Block b = GameRegistry.findBlock(ModList.THAUMCRAFT.modLabel, "blockFluxGoo");
            Block b2
                = GameRegistry.findBlock(ModList.THAUMCRAFT.modLabel, "blockFluxGas");
    ...
    public boolean takeFromContainer(AspectList ot) {
        return this.takeFromContainer(
            ot.getAspects()[0], ot.getAmount(ot.getAspects()[0])
        );
    }
    ...
    public boolean doesContainerContain(AspectList ot) {
        return this.doesContainerContainAmount(
            ot.getAspects()[0], ot.getAmount(ot.getAspects()[0])
        );
    }
    ...
    public void setDataFromItemStackTag(ItemStack is) {
        ...
        if (is.stackTagCompound != null
            && ReikaItemHelper.matchStacks(is, ChromaTiles.ASPECTJAR.getCraftedProduct())
    ...
    public void addTooltipInfo(List li, boolean shift) {
        for (Aspect a : tank.getAspects()) {
            li.add(
                a.getChatcolor() + a.getLocalizedDescription() + ": " + tank.getLevel(a)
            );
        }
    }
}
