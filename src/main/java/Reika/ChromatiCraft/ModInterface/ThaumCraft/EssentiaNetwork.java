...
public class EssentiaNetwork {
    ...
    static {
        if (ModList.THAUMCRAFT.isLoaded()) {
            try {
                ...
                advancedFurnaceNozzleClass = Class.forName(
                    "thaumcraft.common.tiles.TileAlchemyFurnaceAdvancedNozzle"
                );
            } catch (Exception e) {
    ...
    private static boolean shouldSkipEndpoint(TileEntity te) {
        return te instanceof TileEntityEssentiaRelay
            || (tubeClass != null && tubeClass == te.getClass());
    }
    ...
    public static class EssentiaPath {
        ...
        private void doParticles(World world, int x, int y, int z) {
            for (int i = 0; i < path.size() - 1; i++) {
                ...
                if (i == 0 && sourceOffset != null) {
                    ...
                    ReikaPacketHelper.sendNBTPacket(
                        ChromatiCraft.packetChannel,
                        ChromaPackets.ESSENTIAPARTICLEWITHOFFSET.ordinal(),
                        tag,
                        new PacketTarget.RadiusTarget(world, x, y, z, 32)
                    );
                    ...
                    //this.sendParticle(world, loc1.xCoord, loc2.xCoord, loc1.yCoord,
                    //loc2.yCoord, loc1.zCoord, loc2.zCoord);
                    ReikaPacketHelper.sendStringIntPacket(
                        ChromatiCraft.packetChannel,
                        ChromaPackets.ESSENTIAPARTICLE.ordinal(),
                        new PacketTarget.RadiusTarget(world, x, y, z, 32),
                        aspect.getTag(),
                        loc1.xCoord,
                        loc2.xCoord,
        ...
        public static void sendParticle(
            ...
            for (double d = -r; d <= r; d += dr) {
                ...
                EntityFX fx = new EntityBlurFX(world, px, py, pz, vx, vy,
            vz).setColor(a.getColor()).setLife(l).setScale(s).setRapidExpand().markDestination(x2,
            y2, z2);//.setIcon(ChromaIcons.TRANSFADE).setBasicBlend(); EntityFX fx2 = new
            EntityBlurFX(world, px, py, pz, vx, vy,
            vz).setColor(0xffffff).setLife(l).setScale(s/2).setRapidExpand().markDestination(x2,
            y2, z2);//.setIcon(ChromaIcons.TRANSFADE).setBasicBlend(); fx.noClip = true;
                fx2.noClip = true;
            ...
            for (double d = 0; d <= dd; d += 0.125) {
                ...
                EntityFX fx = new EntityCCBlurFX(world, px, py, pz)
                                  ...
                                  .setScale(s)
                                  .setRapidExpand(
                                  ); //.setIcon(ChromaIcons.TRANSFADE).setBasicBlend();
    ...
    private static class AlembicEndpoint extends ActiveEndpoint {
        ...
        public AspectList getPush(World world) {
            ...
            try {
                ...
                return a != null
                    ? new AspectList().add(a, alembicAmountField.getInt(tile))
                    : null;
            } catch (Exception e) {
                e.printStackTrace();
                return null;
    ...
    private static class LabelledJarEndpoint
        extends ActiveEndpoint implements Comparable<LabelledJarEndpoint> {
        ...
        public boolean isValid(World world) {
            try {
                return super.isValid(world)
                    && filterField.get(this.getTile(world)) == filter;
            } catch (Exception e) {
                e.printStackTrace();
                return false;
            }
        ...
        public int compareTo(LabelledJarEndpoint o) {
            if (o.isVoid == isVoid) {
                ...
                throw new UnreachableCodeException(
                    "Two booleans were neither equal nor was either true. Get a new JVM, because yours is broken."
                );
            }
        }
    }
    ...
    private static class AlvearyEndpoint extends ActiveEndpoint {
        ...
        public AspectList getPull(World world) {
            ...
            if (tile == null || !tile.isAlvearyComplete())
                return null;
            Collection<VisAlvearyEffect> c
                = (Collection<VisAlvearyEffect>)
                      TileEntityLumenAlveary.getEffectSet(VisAlvearyEffect.class);
        ...
        protected DecimalPosition getRayOffset(World world) {
            ...
            if (te != null) {
                ...
                if (c != null) {
                    return new DecimalPosition(
                        c.xCoord - te.xCoord, c.yCoord - te.yCoord, c.zCoord - te.zCoord
                    );
                }
            }
            return null;
    ...
    private static class EnergisticsEndpoint extends NetworkEndpoint {
        ...
        private IGrid getGrid(World world) {
            ...
            if (te == null)
                return null;
            IGridNode node = te instanceof IActionHost
                ? ((IActionHost) te).getActionableNode()
                : ((IGridHost) te).getGridNode(ForgeDirection.UP);
        ...
        public int addAspect(World world, Aspect a, int amount) {
            IGrid grid = this.getGrid(world);
            IEssentiaGrid cache = grid.getCache(IEssentiaGrid.class);
            int fit = cache.injectEssentia(a, amount, Actionable.SIMULATE, source,
        powered);
        }
    ...
    private static class NetworkEndpoint {
        ...
        private NetworkEndpoint(Coordinate loc, IEssentiaTransport te) {
            ...
            for (int i = 0; i < 6; i++) {
                maxsuc = Math.max(
                    maxsuc, te.getSuctionAmount(ForgeDirection.VALID_DIRECTIONS[i])
                );
            }
        ...
        public int addAspect(World world, Aspect a, int amount) {
            ...
            //ReikaJavaLibrary.pConsole("Added "+ret+" of "+a.getTag()+" to "+tile+" @
            //"+point, ret > 0);
            return ret;
        }
        ...
        public int takeAspect(World world, Aspect a, int amount) {
            ...
            //ReikaJavaLibrary.pConsole("Took "+ret+" of "+a.getTag()+" from "+tile+" @
            //"+point, ret > 0);
            return ret;
        }
        ...
        public final boolean equals(Object o) {
            return o != null && o.getClass() == this.getClass()
                && ((NetworkEndpoint) o).point.equals(point);
        }
    ...
    private static class SuctionComparator implements Comparator<NetworkEndpoint> {
        ...
        public int compare(NetworkEndpoint o1, NetworkEndpoint o2) {
            if (o1.getClass() == o2.getClass() && o1 instanceof LabelledJarEndpoint) {
                ...
                AspectList p1
                    = suction ? a1.getPull(referenceWorld) : a1.getPush(referenceWorld);
                AspectList p2
                    = suction ? a2.getPull(referenceWorld) : a2.getPush(referenceWorld);
                if (p1 == null && p2 == null) {
                    ...
                    int amt1 = p1.visSize();
                    int amt2 = p2.visSize();
                    return amt1 == amt2 ? this.compareTiles(a1, a2)
                                        : -Integer.compare(amt1, amt2);
                }
            }
    ...
    public static class NetworkBuilder {
        ...
        private static Collection<Coordinate>
        getNearNodesExcept(World world, Coordinate c, HashSet<Coordinate> set) {
            ...
            for (int i = -r; i <= r; i++) {
                for (int j = -r; j <= r; j++) {
                    for (int k = -r; k <= r; k++) {
                        ...
                        if (set.contains(c2))
                            continue;
                        if (ChromaTiles.getTile(world, dx, dy, dz)
                            == ChromaTiles.ESSENTIARELAY) {
                            EssentiaSubnet net = ((TileEntityEssentiaRelay
        ...
        private static EssentiaSubnet
        buildNetworkWithNodes(World world, HashSet<Coordinate> set) {
            EssentiaSubnet net = new EssentiaSubnet(
                EssentiaNetwork.getNetwork(world), world.provider.dimensionId
            );
            for (Coordinate c : set) {
    ...
    public static class EssentiaSubnet {
        ...
        private final HashMap<ImmutablePair<Coordinate, Coordinate>, EssentiaPathCache>
        ...
        private boolean canConnect(World world, EssentiaNode n, EssentiaNode c) {
            return n.getDistanceTo(c) <= TileEntityEssentiaRelay.SEARCH_RANGE
                && LOS(world, n.position, c.position);
        }
        ...
        public EssentiaMovement removeEssentia(
            TileEntityEssentiaRelay caller,
            ForgeDirection callDir,
            Aspect aspect,
            int amount
        ) {
            return this.removeEssentia(
                caller, callDir, aspect, amount, new Coordinate(caller).offset(callDir, 1)
            );
        }
            ...
            for (NetworkEndpoint p : list) {
                ...
                if (pt != null && !pt.isEmpty()) {
                    ...
                    if (rem > 0) {
                        ...
                        ); //ReikaJavaLibrary.makeListFrom(loc, node, new
                           //Coordinate(caller))
                        if (amount <= 0) {
                            break;
                        }
        ...
        public EssentiaMovement addEssentia(
            TileEntityEssentiaRelay caller,
            ForgeDirection callDir,
            Aspect aspect,
            int amount
        ) {
            return this.addEssentia(
                caller, aspect, amount, new Coordinate(caller).offset(callDir, 1)
            );
        }

        public EssentiaMovement addEssentia(
            TileEntityEssentiaRelay caller, Aspect aspect, int amount, Coordinate src
        ) {
            if (isBeingDestroyed)
                return null;
            ArrayList<EssentiaPath> li = new ArrayList();
            ...
            for (NetworkEndpoint p : list) {
                ...
                if (pt != null && !pt.isEmpty()) {
                    ...
                    if (added > 0) {
                        ...
                        ); //ReikaJavaLibrary.makeListFrom(new Coordinate(caller), node,
                           //loc)
                        if (amount <= 0) {
                            break;
                        }
        ...
        private boolean canTransfer(
            World world, NetworkEndpoint from, NetworkEndpoint to, boolean isTick
        ) {
            if (isBeingDestroyed)
                return false;
            if (from == null || to == null) {
                ChromatiCraft.logger.logError(
                    "Tried to transfer essentia with a null endpoint!"
                );
                return false;
            }
            ...
            if (isJar(from.getTile(world)) && isJar(to.getTile(world)))
                return to instanceof LabelledJarEndpoint
                    && ((LabelledJarEndpoint) to).getPull(world) != null;
            return from.canEmit(world, isTick) && to.canReceive(world, isTick)
                && from.isValid(world) && to.isValid(world);
        }

        private ArrayList<EssentiaPath> transferEssentia(
            World world, NetworkEndpoint from, NetworkEndpoint to, AspectList al
        ) {
            if (isBeingDestroyed)
                return null;
            //ReikaJavaLibrary.pConsole("Attempting transfer of
            //"+ReikaThaumHelper.aspectsToString(al)+" from "+from+" to "+to);
            ArrayList<EssentiaPath> ret = new ArrayList();
            AspectList it = al.copy();
            for (Aspect a : it.aspects.keySet()) {
                int amt = it.getAmount(a);
                EssentiaPath p = this.transferEssentia(
                    world, from, to, a, Math.min(amt, TileEntityEssentiaRelay.THROUGHPUT)
                );
                if (p != null) {
                    ret.add(p);
        ...
        private EssentiaPath transferEssentia(
            World world, NetworkEndpoint from, NetworkEndpoint to, Aspect a, int amount
        ) {
            if (isBeingDestroyed)
                return null;
            ...
            if (put2 <= 0)
                return null;
            //ReikaJavaLibrary.pConsole("Transferred "+put+" & "+put2+" / "+amount+" of
            //"+a.getTag()+" from "+from+" to "+to); ReikaJavaLibrary.pConsole(pt+" of
            //"+from+" , "+to);
        ...
        private EssentiaPathCache
        getPath(World world, NetworkEndpoint from, NetworkEndpoint to) {
            ImmutablePair<Coordinate, Coordinate> key
                = new ImmutablePair(from.point, to.point);
            ...
            if (li != null && !li.isEmpty()) {
                path = new EssentiaPathCache(
                    li,
                    from.needsLOS(),
                    to.needsLOS(),
                    from.getRayOffset(world),
                    to.getRayOffset(world)
                );
        ...
        calculatePath(World world, NetworkEndpoint from, NetworkEndpoint to) {
            if (from.relay.equals(to.relay)) {
                return this.buildPathBetween(from, to, to.relay.position);
            }
            ArrayList<Coordinate> li
                = this.getNodePath(world, from.relay, to.relay, new HashSet());
            return li != null
                ? this.buildPathBetween(from, to, li.toArray(new Coordinate[li.size()]))
                : null;
        }

        private ArrayList<Coordinate> getNodePath(
            World world, EssentiaNode from, EssentiaNode to, HashSet<Coordinate> visited
        ) {
        ...
        private ArrayList<Coordinate>
        buildPathBetween(NetworkEndpoint from, NetworkEndpoint to, Coordinate... hops) {
