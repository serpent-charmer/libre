...
public class GuiAspectFormer extends GuiLetterSearchable<Aspect> {
    ...
    protected void actionPerformed(GuiButton b) {
        ...
        //ReikaJavaLibrary.pConsole(this.getClickCooldown()+" @ "+b.id+" >
        //"+buttonList.size()+":"+buttonList);
        ...
        switch (b.id) {
            ...
            case 2:
                ...
                ReikaPacketHelper.sendStringPacket(
                    ChromatiCraft.packetChannel, ChromaPackets.ASPECT.ordinal(), "", tile
                );
                break;
            case 3:
                mode = mode.next();
                ReikaPacketHelper.sendPacketToServer(
                    ChromatiCraft.packetChannel, ChromaPackets.ASPECTMODE.ordinal(), tile
                );
                index = 0;
                ReikaPacketHelper.sendStringPacket(
                    ChromatiCraft.packetChannel, ChromaPackets.ASPECT.ordinal(), "", tile
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (mode == AspectMode.DEMAND) {
            //api.drawRectFrame(minx, miny, w, w,
            //ReikaColorAPI.mixColors(this.getActive().getColor(), 0x1e1e1e,
            //clickDelay/90F)); Color.HSBtoRGB(hsb[0], Math.min(clickDelay/90F, hsb[1]),
            //Math.min(bright/255F, hsb[2]))
            int color = ReikaColorAPI.mixColors(
                0x010101, 0xffffff, 1 - bright / 255F - clickDelay / 90F
            );
            color = ReikaColorAPI.mixColors(
                color, this.getActive().getColor(), 1 - clickDelay / 90F
            );
            ...
            ElementTagCompound tag
                = TileEntityAspectFormer.getAspectCost(this.getActive());
            int dx = 18;
            int dy = 21;
            boolean text = GuiScreen.isCtrlKeyDown()
                && api.isMouseInBox(
    ...
    protected Collection<Aspect> getAllEntries(EntityPlayer ep) {
        if (!ModList.THAUMCRAFT.isLoaded() && DragonAPICore.isReikasComputer()
