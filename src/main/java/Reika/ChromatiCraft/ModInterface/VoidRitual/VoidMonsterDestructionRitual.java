...
public class VoidMonsterDestructionRitual {
    ...
    private static final Collection<VoidMonsterDestructionRitual> activeRituals
        = new HashSet();
    ...
    private VoidMonsterDestructionRitual(
        WorldLocation loc, EntityPlayer ep, EntityLiving e
    ) {
        startingPlayer = ep;
        center = loc;
        ...
        for (double d = 0.1; d < 1; d += 0.1) {
            double f = 1 - d;
            healthCurve.addPoint(
                ReikaRandomHelper.getRandomPlusMinus(d, 0.045) * targetDuration,
                ReikaRandomHelper.getRandomPlusMinus(f, 0.2)
            );
        }
    ...
    public static void sync() {
        ...
        ReikaPacketHelper.sendNBTPacket(
            ...
            ChromaPackets.VOIDMONSTERRITUALSET.ordinal(),
            NBT,
            PacketTarget.allPlayers.allPlayers
        );
    }
    ...
    void onCompletion() {
        MonsterGenerator.instance.addCooldown(
            this.getEntity(), 20 * 60 * ReikaRandomHelper.getRandomBetween(20, 45)
        );
        this.onEnd();
    }
    ...
    public static enum Effects {
        ...
        public void doEffectServer(VoidMonsterDestructionRitual rit, EntityLiving e) {
            if (e.getHealth() / e.getMaxHealth()
                >= rit.getCurrentTargetHealthFraction()) {
            ...
            ReikaPacketHelper.sendDataPacket(
                ChromatiCraft.packetChannel,
                ChromaPackets.VOIDMONSTERRITUAL.ordinal(),
                new PacketTarget.RadiusTarget(e, 128),
                e.getEntityId(),
                this.ordinal()
            );
        }
        ...
        private void doAttack(EntityLiving e, DamageSource src, int amt) {
            if (damageDelay > 0) {
                TickScheduler.instance.scheduleEvent(
                    new ScheduledTickEvent(new ScheduledDamage(e, src, amt)), damageDelay
                );
            } else {
                runAttack(e, src, amt);
            }
                ...
                case COLLAPSING_SPHERE:
                    for (int i = 0; i < 128; i++) {
                        ...
                        double[] xyz = ReikaPhysicsHelper.polarToCartesian(
                            4 + rand.nextDouble() * 0.25, a1, a2
                        );
                        double px = ex + xyz[0];
                        double py = ey + xyz[1];
                        double pz = ez + xyz[2];
                        EntityBlurFX fx
                            = new EntityCCBlurFX(e.worldObj, px, py, pz).setAlphaFading();
                        int t = ReikaRandomHelper.getRandomBetween(5, 8);
                        fx.setPositionController(
                            new CollectingPositionController(px, py, pz, ex, ey, ez, t)
                        );
                ...
                case RAYS:
                    for (int k = 0; k < 9; k++) {
                        ...
                        for (int i = 0; i < 128; i++) {
                            ...
                            EntityBlurFX fx = new EntityCCBlurFX(e.worldObj, px, py, pz)
                                                  .setAlphaFading();
                            fx.setLife(40).setScale(1.5F + rand.nextFloat() * 1.5F);
                            double d = rand.nextDouble() * 360;
                            MathExpression exp = new PeriodicExpression()
                                                     .addWave(1, 1, d)
                                                     .addWave(0.5, 2, d + 90)
                                                     .addWave(0.125, 4, d)
                                                     .normalize();
                            fx.setColorController(
                                new FlashColorController(exp, 0xffffff, 0x000000)
                            );
                            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
                        }
                    }
