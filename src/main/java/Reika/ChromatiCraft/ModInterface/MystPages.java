...
public class MystPages implements MystcraftPageRegistry {
    ...
    public static enum Pages implements IAgeSymbol {
        ...
        STRUCTURES(
            "Buried Structures", 500, 4, 2, Symbols.STRUCTURE, Symbols.CIVILIZATION
        ),
        PLANTS("Chroma Plants", 20, 3, 0, Symbols.MATERIAL, Symbols.HERBAL),
        ORES("Chroma Ores", 10, 3, 1, Symbols.MATERIAL, Symbols.MINERAL),
        CRYSTALS(
            "Cave Crystals", 80, 2, 0, Symbols.MATERIAL, Symbols.MINERAL, Symbols.ENERGY
        ),
        ...
        LAVARIVER(
            "Sky Rivers", 2, 4, 2, Symbols.STRUCTURE, Symbols.MATERIAL, Symbols.MINERAL
        ),
        VIOLENTPYLONS(
            "Excessive Discharge",
            -100,
        ...
        public boolean existsInWorld(World age) {
            ...
            if (b == null) {
                b = ReikaMystcraftHelper.isMystAge(age)
                    && ReikaMystcraftHelper.isSymbolPresent(age, this.identifier());
        ...
        public void registerLogic(AgeDirector age, long seed) {
            switch (this) {
                case LAVARIVER:
                    BlockDescriptor b1
                        = ModifierUtils.popBlockMatching(age, BlockCategory.STRUCTURE);
                    BlockDescriptor b2
                        = ModifierUtils.popBlockMatching(age, BlockCategory.FLUID);
                    age.registerInterface(new LavaRiverGenerator(
                        seed,
                        b1 != null ? new BlockKey(b1.block, b1.metadata) : null,
                        b2 != null ? new BlockKey(b2.block, b2.metadata) : null
                    ));
                    break;
                default:
                    break;
