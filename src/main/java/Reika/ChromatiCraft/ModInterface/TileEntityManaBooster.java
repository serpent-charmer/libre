...
public class TileEntityManaBooster extends TileEntityWirelessPowered {
        ...
        if (ModList.BOTANIA.isLoaded()) {
            try {
                ...
                ChromatiCraft.logger.logError(
                    "Could not access generating flower mana variable!"
                );
                e.printStackTrace();
            }
        }
                ...
                if (this.getTicksExisted() % 8 == 0) {
                    for (CrystalElement e : required.elementSet()) {
                        if (energy.getValue(e) < this.getMaxStorage(e))
                            this.requestEnergy(
                                e, this.getMaxStorage(e) - energy.getValue(e)
                            );
                    }
                }
            ...
            if (space == -1) {
                ...
                if (transfer > 0) {
                    if (this.doManaTransfer(world, c.location, c2, transfer, accel)) {
                        //ReikaJavaLibrary.pConsole("Spawning burst at
    ...
    private boolean doManaTransfer(
        World world, Coordinate from, Coordinate to, int transfer, boolean accelerated
    ) {
        //for testing
        ...
        if (path != null) {
            EntityChromaManaBurst e
                = new EntityChromaManaBurst(world, transfer, path, accelerated);
    ...
    private ManaPath calculateManaPath(World world, Coordinate from, Coordinate to) {
        ...
        LinkedList<Coordinate> li
            = BreadthFirstSearch
                  .getOpenPathBetween(world, from, mid, 40, OpenPathFinder.defaultRules)
                  .getPath();
        LinkedList<Coordinate> li2
            = BreadthFirstSearch
                  .getOpenPathBetween(world, mid, to, 40, OpenPathFinder.defaultRules)
                  .getPath();
        if (li != null && li2 != null) {
            ...
            for (Coordinate c : li) {
                if (i % 4 == 0 && c != li.getLast()) {
                    if (!c.equals(to) && !c.equals(from) && !c.equals(mid)
                        && OpenPathFinder.isEmptyBlock(
                            world,
                            c.xCoord,
                            c.yCoord,
                            c.zCoord,
                            OpenPathFinder.defaultRules
                        ))
                        set.add(c);
    ...
    public static int receiveMana(World world, Coordinate c, int amt, boolean doRemove) {
        ...
        if (!isGeneratingFlower(te))
            return -1;
        try {
            SubTileGenerating gen
                = (SubTileGenerating) ((ISubTileContainer) te).getSubTile();
                ...
                if (Math.abs(i) + Math.abs(k) <= r) {
                    for (int j = 1; j <= 3; j++) {
                        ...
                        if (flag) {
                            ...
                            if (pools ? this.isValidPool(te)
                                      : this.isGeneratingFlower(te)) {
                                if (pools) {
        ...
        public boolean isValid(World world) {
            for (Coordinate c : coords) {
                if (c.equals(manaSource) || c.equals(manaTarget))
                    continue;
                if (c.getTaxicabDistanceTo(
                        boosterCenter.xCoord, boosterCenter.yCoord, boosterCenter.zCoord
                    )
                    <= 1)
                    continue;
                if (c.getBlock(world) instanceof ISpecialFlower)
                    continue;
                if (!OpenPathFinder.isEmptyBlock(
                        world, c.xCoord, c.yCoord, c.zCoord, OpenPathFinder.defaultRules
                    )) {
                    ChromatiCraft.logger.log(
                        "Invalidating old mana path: "
                        + c.getBlock(world).getLocalizedName() + " @ " + c
                    );
                    return false;
                }
            }
