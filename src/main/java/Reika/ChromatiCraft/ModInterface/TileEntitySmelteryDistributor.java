...
public class TileEntitySmelteryDistributor extends TileEntityChromaticBase {
    ...
    private void
    sendFluid(Fluid f, ForgeDirection dir, Coordinate drain, Coordinate target) {
        int x = drain.xCoord;
        int y = drain.yCoord;
        int z = drain.zCoord;
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            ChromaPackets.SMELTERYFLUIDSEND.ordinal(),
            this,
            48,
            x,
            y,
    ...
    public void sendFluidToClient(
        int x, int y, int z, int x2, int y2, int z2, ForgeDirection drainFace, Fluid f
    ) {
        ParticlePath p = ParticlePath.getPath(
            worldObj,
            new BlockVector(x, y, z, drainFace),
            new BlockVector(x2, y2 + 1, z2, ForgeDirection.UP),
            0.5,
            0.5
        );
        ...
        for (int i = 0; i < path.size(); i++) {
            ...
            EntityFX fx = new EntityCCBlurFX(worldObj, pos.xCoord, pos.yCoord, pos.zCoord)
                              .setIcon(ico)
                              .setLife(l)
                              .setScale(s)
                              .setRapidExpand()
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (world.isRemote) {
            ...
            if (active) {
                ...
                rotationSpeed
                    = Math.min(rotationSpeed + 0.1 * d * d, MAX_ROTATION_SPEED * d);
        ...
        while (it.hasNext()) {
            ...
            if (cb.isValid(world)) {
                ...
                if (capacity > 0 && capacity <= fs.amount) {
                    ...
                    if (space >= capacity) {
                        //ReikaJavaLibrary.pConsole("Can add "+space+" of
                        //"+fs.getLocalizedName()+" to "+te);
                        return cb;
                    }
                }
            } else {
    ...
    private static class CastingBlock implements Comparable<CastingBlock> {
        ...
        public int getCapacity(World world, Fluid f) {
            return this.getTile(world).fill(
                ForgeDirection.UP, new FluidStack(f, Integer.MAX_VALUE), false
            );
        }
        ...
        private void update(World world) {
            ...
            hasNonIngotCast = hasCast
                && !ReikaItemHelper.matchStacks(
                    ii.getStackInSlot(0), TinkerToolHandler.getInstance().getIngotCast()
                );
        }
        ...
        public boolean isValid(World world) {
            TileEntity te = location.getTileEntity(world);
            return te != null
                && (te.getClass() == TinkerSmelteryHandler.basinClass()
                    || te.getClass() == TinkerSmelteryHandler.tableClass());
        }
