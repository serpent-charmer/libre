...
public class CrystalBrewerHandler extends TemplateRecipeHandler {
    private class CrystalRecipe extends CachedRecipe {
        ...
        public PositionedStack getResult() {
            return null; //new PositionedStack(new ItemStack(Items.potionitem.getItem, 1,
                         //this.getPotionDamage()), 131, 24);
        }
        ...
        public ItemStack getInputShard() {
            return ChromaItems.SHARD.getStackOfMetadata(
                boosted ? 16 + color.ordinal() : color.ordinal()
            );
        }

        public Potion getOutputPotion() {
            return Potion.potionTypes[CrystalPotionController.instance
                                          .getEffectFromColor(color, 20, 0, false)
                                          .getPotionID()];
        }
        ...
        public List<PositionedStack> getOtherStacks() {
            ...
            ItemStack out = TileEntityCrystalBrewer.getPotionStackFromColor(
                in.getItemDamage(), color, boosted
            );
    ...
    public void loadCraftingRecipes(ItemStack result) {
        if (result.getItem() == ChromaItems.POTION.getItemInstance()) {
            ...
            for (int i = 0; i < 32; i++) {
                ...
                ItemStack out = TileEntityCrystalBrewer.getPotionStackFromColor(
                    result.getItemDamage(), color, i >= 16
                );
