...
public class PlantDustHandler extends TemplateRecipeHandler {
    private class PlantDustRecipe extends CachedRecipe {
        ...
        public PositionedStack getIngredient() {
            return new PositionedStack(
                source.get(this.getIndex()).asItemStack(), 32, 12 - this.yOffset()
            );
        }
    ...
    public void drawBackground(int recipe) {
        ...
        ReikaGuiAPI.instance.drawTexturedModalRectWithDepth(
            8, 0 - n * 24, 5, 11, 166, 40, ReikaGuiAPI.NEI_DEPTH
        );
    }
    ...
    public void loadTransferRects() {
        transferRects.add(
            new RecipeTransferRect(new Rectangle(57, 10, 51, 20), "ccplants")
        );
    }
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId != null && outputId.equals("ccplants")) {
            ...
            for (ItemStack is : li) {
                Collection<BlockKey> c = PlantDropManager.instance.getPlantForDrops(
                    is, Minecraft.getMinecraft().thePlayer
                );
                if (c != null && !c.isEmpty())
    ...
    public void loadCraftingRecipes(ItemStack result) {
        Collection<BlockKey> plants = PlantDropManager.instance.getPlantForDrops(
            result, Minecraft.getMinecraft().thePlayer
        );
    ...
    public void drawExtras(int recipe) {
        ...
        Minecraft.getMinecraft().fontRenderer.drawString(fr.item.getDisplayName(), 46, 9,
    0xffffff); for (int i = 0; i < 16; i++) { CrystalElement e =
    CrystalElement.elements[i]; int dx = 3+(i/8)*84; int dy = 28+(i%8)*18; IIcon ico =
    e.getGlowRune(); GL11.glColor4f(1, 1, 1, 1); ReikaTextureHelper.bindTerrainTexture();
            ...
            Minecraft.getMinecraft().fontRenderer.drawString(String.valueOf(fr.cost.getValue(e)),
    dx+24, dy+4, c);
        }
