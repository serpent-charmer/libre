...
public class CastingTableHandler extends TemplateRecipeHandler {
    public class CastingCrafting extends CachedRecipe {
        ...
        public ArrayList<PositionedStack> getIngredients() {
            ...
            if (recipe instanceof MultiBlockCastingRecipe) {
                Map<List<Integer>, ItemMatch> map
                    = ((MultiBlockCastingRecipe) recipe).getAuxItems();
    ...
    public void drawBackground(int recipe) {
        ...
        ReikaGuiAPI.instance.drawTexturedModalRectWithDepth(
            0, 0, 5, 3, 214, h, ReikaGuiAPI.NEI_DEPTH
        );
    }
    ...
    public void loadCraftingRecipes(ItemStack result) {
        if (result != null) {
            ArrayList<CastingRecipe> li
                = RecipesCastingTable.instance.getAllRecipesMaking(result);
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        ArrayList<CastingRecipe> li
            = RecipesCastingTable.instance.getAllRecipesUsing(ingredient);
        for (int i = 0; i < li.size(); i++)
            arecipes.add(new CastingCrafting(li.get(i)));
    }
    ...
    public void drawExtras(int recipe) {
        ...
        if (r.recipe instanceof PylonCastingRecipe) {
            ...
            for (CrystalElement e : tag.elementSet()) {
                ...
                ReikaGuiAPI.instance.drawRect(
                    x, y1, x + w, y, e.getJavaColor().darker().darker().getRGB()
                );
            }
        }
        /*
        ...
        f.getStringWidth(s)/2-2*(s.length()/5), 61, 0);
         */
    }
}
