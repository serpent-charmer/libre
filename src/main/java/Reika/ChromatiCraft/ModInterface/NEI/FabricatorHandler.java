...
public class FabricatorHandler extends TemplateRecipeHandler {
    ...
    private class FabricationRecipeDisplay
        extends CachedRecipe implements ElementTagRecipe {
    ...
    public void drawBackground(int recipe) {
        ...
        ReikaGuiAPI.instance.drawTexturedModalRectWithDepth(
            0, 0, 5, 11, 166, 172, ReikaGuiAPI.NEI_DEPTH
        );
    }
    ...
    public void loadTransferRects() {
        if (!(Minecraft.getMinecraft().thePlayer.openContainer
              instanceof ContainerItemFabricator))
            transferRects.add(
                new RecipeTransferRect(new Rectangle(32, -12, 105, 10), "ccfabric")
            );
    }
    ...
    public void loadCraftingRecipes(String outputId, Object... results) {
        if (outputId != null && outputId.equals("ccfabric")) {
            Collection<KeyedItemStack> li
                = FabricationRecipes.recipes().getFabricableItems();
            for (KeyedItemStack is : li) {
                FabricationRecipe f
                    = FabricationRecipes.recipes().getItemRecipe(is.getItemStack());
    ...
    public void drawExtras(int recipe) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            f.drawString(
                String.valueOf(fr.recipe.getCost().getValue(e)), dx + 24, dy + 4, c
            );
        }
        /*
        int dy = -2;
