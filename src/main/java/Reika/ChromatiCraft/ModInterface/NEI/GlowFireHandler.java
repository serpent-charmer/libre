...
public class GlowFireHandler extends TemplateRecipeHandler {
    ...
    public void drawBackground(int recipe) {
        ...
        //ReikaGuiAPI.instance.drawTexturedModalRectWithDepth(0, 0, 5, 11, 166, 172,
        //ReikaGuiAPI.NEI_DEPTH);
    ...
    public void loadTransferRects() {
        transferRects.add(
            new RecipeTransferRect(new Rectangle(32, -12, 105, 10), "ccglowfire")
        );
    }

    private void loadAllRecipes(boolean costs) {
        Collection<KeyedItemStack> li = costs
            ? FabricationRecipes.recipes().getFabricableItems()
            : FabricationRecipes.recipes().getConsumableItems();
        for (KeyedItemStack is : li) {
            ElementTagCompound tag = costs
                ? TileEntityGlowFire.getCost(is.getItemStack())
                : TileEntityGlowFire.getDecompositionValue(is.getItemStack());
            if (tag != null)
    ...
    public void drawExtras(int recipe) {
        ...
        if (fr.cost == null) {
            throw new IllegalStateException(
                this.getRecipeName() + " recipe " + fr.item + " has null cost?!"
            );
        }
        ...
        if (ReikaItemHelper.matchStacks(fr.item, ChromaStacks.glowcavedust)) {
            ...
            for (CrystalElement e : fr.cost.elementSet()) {
                int amt = (int
                   ...
                   + (lim / 2 + 1)
                       * Math.sin(
                           System.identityHashCode(e) * System.identityHashCode(fr)
                           + System.currentTimeMillis() / 2400D
                       ));
                tag.addTag(e, amt);
            }
        }
        //Minecraft.getMinecraft().fontRenderer.drawString(fr.item.getDisplayName(), 46,
        //9, 0xffffff);
        double r = 60 - 6;
        double ox = 81;
        double oy = 70 - 2;
        ...
        Tessellator.instance.setColorOpaque_I(/*!fr.output ? */
        /*0x303030/* : 0xc0c0c0*/ /*);
...
*/
        ...
        GL11.glColor4f(1, 1, 1, 1);
        double er2
            = Math.max(ir, (r * Math.log(tag.getValue(CrystalElement.BLACK) + o) / max));
        Tessellator.instance.startDrawing(GL11.GL_TRIANGLE_FAN);
        ...
        for (int i = 0; i < 16; i++) {
            ...
            //ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(dx, dy, ico, 8, 8);
            //int c = ReikaColorAPI.mixColors(e.getColor(), 0xffffff, 0.75F);
            //Minecraft.getMinecraft().fontRenderer.drawString(String.valueOf(val), dx+10,
            //dy+0, c);
        }
        Tessellator.instance.draw();
        ...
        for (int i = 0; i < 16; i++) {
            ...
            if (tag.getValue(e) > 0) {
                ...
                int tx = dx > ox
                    ? dx + 9
                    : dx - Minecraft.getMinecraft().fontRenderer.getStringWidth(s);
                int ty = dy + 1;
