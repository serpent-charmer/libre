...
public class TileEntityFloatingLandmark extends TileEntityChromaticBase
    implements ITileAreaProvider, BreakAction, AdjacentUpdateWatcher,
    ...
    private void doAxisParticles(World world, int x, int y, int z) {
        ...
        for (int d = 1; d < RANGE; d += 4) {
            ...
            if (dy >= 0 && dy <= 255
                && ep.getDistanceSq(dx + 0.5, dy + 0.5, dz + 0.5) <= 256
                && world.getBlock(dx, dy, dz).isAir(world, dx, dy, dz)) {
                double v = 0.25;
                double vx = dir.offsetX * v;
                double vy = dir.offsetY * v;
                double vz = dir.offsetZ * v;
                EntityCCBlurFX fx
                    = new EntityCCBlurFX(world, dx + 0.5, dy + 0.5, dz + 0.5, vx, vy, vz);
    ...
    private void doBlockParticles(World world, int x, int y, int z) {
        ...
        EntityCCFloatingSeedsFX fx
            = new EntityCCFloatingSeedsFX(world, dx, dy, dz, a1, a2);
        ...
        int c = ReikaColorAPI.mixColors(
            0xffffff, 0x000000, (float) ReikaRandomHelper.getRandomBetween(0.25, 0.75)
        );
        if (isPrimary)
            c = ReikaColorAPI.mixColors(
                0x00ffff, 0x0000ff, (float) ReikaRandomHelper.getRandomBetween(0.25, 0.75)
            );
    ...
    private void addLink(TileEntityFloatingLandmark te) {
        area = new ProportionedBlockBox(
            area.volume.addCoordinate(te.xCoord, te.yCoord, te.zCoord)
        );
    ...
    private void doAreaParticles(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (rand.nextInt(5) < 2) {
                ...
                dx = ReikaRandomHelper.getRandomBetween(
                    edge.root.xCoord,
                    (double) edge.root.xCoord + (edge.length + 0) * edge.axis.offsetX
                );
                dy = ReikaRandomHelper.getRandomBetween(
                    edge.root.yCoord,
                    (double) edge.root.yCoord + (edge.length + 0) * edge.axis.offsetY
                );
                dz = ReikaRandomHelper.getRandomBetween(
                    edge.root.zCoord,
                    (double) edge.root.zCoord + (edge.length + 0) * edge.axis.offsetZ
                );
                if (edge.isPositiveX) {
                    dx++;
                }
                ...
                dx = ReikaRandomHelper.getRandomBetween(
                    area.volume.minX, (double) area.volume.maxX
                );
                dy = ReikaRandomHelper.getRandomBetween(
                    area.volume.minY, (double) area.volume.maxY
                );
                dz = ReikaRandomHelper.getRandomBetween(
                    area.volume.minZ, (double) area.volume.maxZ
                );
            ...
            boolean valid = world.getBlock(ix, iy, iz).isAir(world, ix, iy, iz)
                || ChromaTiles.getTile(world, ix, iy, iz) == this.getTile();
            if (!valid)
                continue;
            ...
            int hue
                = (int) ((this.getTicksExisted() * 2 + dx * 3 + dy * 1 + dz * 2) % 360);
        ...
        for (int d = 1; d < RANGE; d++) {
            for (int i = 0; i < 6; i++) {
                ...
                if (ChromaTiles.getTile(world, dx, dy, dz) == this.getTile()) {
                    TileEntityFloatingLandmark te
                        = (TileEntityFloatingLandmark) world.getTileEntity(dx, dy, dz);
    ...
    public TileEntityFloatingLandmark getPrimary() {
        ...
        return te instanceof TileEntityFloatingLandmark ? (TileEntityFloatingLandmark) te
                                                        : null;
    }
    ...
    protected void writeSyncTag(NBTTagCompound NBT) {
        ...
        ReikaNBTHelper.writeCollectionToNBT(
            connections, NBT, "links", Coordinate.nbtHandler
        );
    }
    ...
    protected void readSyncTag(NBTTagCompound NBT) {
        ...
        ReikaNBTHelper.readCollectionFromNBT(
            connections, NBT, "links", Coordinate.nbtHandler
        );
    }
    ...
    public boolean isValidFromLocation(int x, int y, int z) {
        return this.isAnchored()
            && new Coordinate(x, y, z).getTaxicabDistanceTo(new Coordinate(this)) == 1;
    }
    ...
    public void onAdjacentUpdate(World world, int x, int y, int z, Block b) {
        if (!this.isAnchored()
            && !ReikaWorldHelper.checkForAdjSolidBlock(world, x, y, z)) {
            ChromaSounds.RIFT.playSoundAtBlock(this);
            this.delete();
            ReikaItemHelper.dropItem(
                world, x + 0.5, y + 0.5, z + 0.5, this.getTile().getCraftedProduct()
            );
        }
    }
