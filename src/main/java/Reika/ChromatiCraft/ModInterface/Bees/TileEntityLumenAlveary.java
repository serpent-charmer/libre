...
//@SmartStrip
@Strippable(
    value = { "forestry.api.multiblock.IAlvearyComponent",
              "forestry.api.multiblock.IAlvearyComponent$BeeModifier",
              "forestry.api.multiblock.IAlvearyComponent$BeeListener",
              "forestry.api.apiculture.IBeeModifier",
              "forestry.api.apiculture.IBeeListener",
              "thaumcraft.api.aspects.IEssentiaTransport",
              "thaumcraft.api.aspects.IAspectContainer" }
)
public class TileEntityLumenAlveary extends TileEntityRelayPowered
    implements GuiController, IAlvearyComponent, BeeModifier, BeeListener, IBeeModifier,
               IBeeListener, CopyableSettings<TileEntityLumenAlveary>, IEssentiaTransport,
               IAspectContainer, Shockable {
    ...
    public static final Comparator<AlvearyEffect> effectSorter = new Comparator<
        AlvearyEffect>() {
        @Override
        public int compare(AlvearyEffect o1, AlvearyEffect o2) {
            if (o1 instanceof LumenAlvearyEffect && o2 instanceof LumenAlvearyEffect) {
                ...
                return Integer.compare(
                    100000 * l1.color.ordinal() + l1.requiredEnergy,
                    100000 * l2.color.ordinal() + l2.requiredEnergy
                );
                ...
                if (v1.aspect == v2.aspect) {
                    ...
                    return v1.aspect.getLocalizedDescription().compareToIgnoreCase(
                        v2.aspect.getLocalizedDescription()
                    );
                }
    ...
    static {
        if (ModList.FORESTRY.isLoaded()) {
            try {
                Class c
                    = Class.forName("forestry.apiculture.multiblock.AlvearyController");
    ...
    protected boolean canReceiveFrom(CrystalElement e, ForgeDirection dir) {
        return this.isAlvearyComplete() && this.isSideOpen(dir)
            && this.isAcceptingColor(e);
    }
    ...
    public Coordinate getAlvearyCenter() {
        return relativeLocation != null
            ? relativeLocation.negate().offset(xCoord + 1, yCoord + 1, zCoord + 1)
            : null;
    }
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (ModList.FORESTRY.isLoaded()) {
            if (multipleBoosters) {
                //for (IMultiblockComponent com :
                //this.getMultiblockLogic().getController().getComponents()) {
                //	com.onMachineBroken();
                //}
            }
            ...
            if (this.isAlvearyComplete()) {
                if (!world.isRemote
                    && this.getTicksExisted() % 16
                        == Math.abs(System.identityHashCode(this)) % 16) {
                ...
                if (world.isRemote) {
                    ...
                    for (AlvearyEffect ae : clientSet) {
                        if (selectedEffects.contains(ae.ID) && ae.ticksOnClient()
                            && ae.isActive(this)) {
                            if (ae.tickRate() == 1
                                || this.getTicksExisted() % ae.tickRate() == 0)
                                ae.clientTick(this);
                        }
                    }

                    if (!canWork && cachedQueen != null
                        && this.getTicksExisted() % 32 == 0) {
                                ...
                                for (Aspect a : cost.aspects.keySet()) {
                                    if (aspects.getAmount(a) < VIS_LIMIT) {
                                        if (DragonAPICore.debugtest) {
                                            aspects.add(
                                                a, VIS_LIMIT - aspects.getAmount(a)
                                            );
                                        } else {
                                            int get = VisNetHandler.drainVis(
                                                world, x, y, z, a, cost.getAmount(a)
                                            );
                                            if (get > 0)
                                                aspects.add(a, get);
                                        }
    ...
    private boolean canRunAutomation() {
        return this.isEffectSelectedAndActive(automation)
            && energy.containsAtLeast(automation.color, automation.requiredEnergy);
    }

    /**
     * Replace with one that will never scan multiple times in the same tick, and has
     * more advanced anti-lag behavior
     */
    private void replaceFlowerCacher() {
        ...
        if (worldObj != null && this.isTickingNaturally()) {
            IBeekeepingLogic bkl
                = this.getMultiblockLogic().getController().getBeekeepingLogic();
    ...
    private void doParticles(World world, int x, int y, int z) {
        ...
        if (rand.nextInt(n) == 0) {
            ...
            double dx = side == 0    ? rand.nextDouble() * 3
                : rand.nextBoolean() ? 0 - o
                                     : 3 + o;
            double dy = rand.nextDouble() * 3.5;
            double dz = side == 1    ? rand.nextDouble() * 3
                : rand.nextBoolean() ? 0 - o
                                     : 3 + o;
            ...
            EntityFX fx1 = new EntityCCBlurFX(world, px, py, pz)
                               .setIcon(ChromaIcons.HEXFLARE2)
                               .setLife(l)
                               .setColor(c1)
                               .setScale(s);
            EntityFX fx2 = new EntityCCBlurFX(world, px, py, pz)
                               .setIcon(ChromaIcons.HEXFLARE2)
                               .setScale(s / 2F)
                               .setLife(l)
                               .setColor(c2);
    ...
    public GameProfile getOwner() {
        return placerUUID != null ? new GameProfile(placerUUID, this.getPlacerName())
                                  : null;
    }
            ...
            if (in != null) {
                ...
                if (flag) {
                    ...
                    IAlleleBeeSpecies sp
                        = (IAlleleBeeSpecies) ReikaBeeHelper.getSpecies(in);
                    if (type == EnumBeeType.PRINCESS && ibhi.getQueen() == null
                        && sp.getUID().equals(species)) {
                        ibhi.setQueen(in);
                        inv.setInventorySlotContents(i, null);
                        flag2 = true;
                    } else if (type == EnumBeeType.DRONE && ReikaItemHelper.areStacksCombinable(drone, in, inv.getInventoryStackLimit())) {
                        int amt = Math.min(
                            drone.getMaxStackSize() - drone.stackSize, in.stackSize
                        );
                        drone.stackSize += amt;
                        ibhi.setDrone(drone);
                        in.stackSize -= amt;
    ...
    public boolean hasQueen() {
        return ModList.FORESTRY.isLoaded()
            && this.getQueenItem()
            != null; // && ReikaBeeHelper.getBeeRoot().getType(this.getQueenItem()) ==
                     // EnumBeeType.QUEEN;
    }
    ...
    private boolean calcCanWork() {
        return this.hasQueen() && this.isAlvearyComplete()
            && this.getMultiblockLogic().getController().getBeekeepingLogic().canWork();
    }
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        ...
        if (this.isAlvearyComplete()) {
            ...
            return AxisAlignedBB.getBoundingBox(
                c.xCoord, c.yCoord, c.zCoord, c2.xCoord, c2.yCoord, c2.zCoord
            );
        }
        return box;
    }
    ...
    private void updateRenderItem() {
        renderItem = this.isAlvearyComplete() && this.getQueenItem() != null
            ? new InertItem(worldObj, this.getQueenItem())
            : null;
    }
    ...
    public boolean isJubilant() {
        return cachedQueen != null
            && this.getSpecies().isJubilant(
                cachedQueen.getGenome(), this.getBeeHousing()
            );
    }
    ...
    public static final Collection<? extends AlvearyEffect>
    getEffectSet(Class<? extends AlvearyEffect> c) {
    ...
    public static abstract class AlvearyEffect {
        ...
        //private static final HashMap<AlvearyEffect, String> alvearyEffectText = new
        //HashMap();
    ...
    public static abstract class VisAlvearyEffect extends PoweredAlvearyEffect {
        ...
        protected final void consumeEnergy(TileEntityLumenAlveary te, int amount) {
            ...
            //ReikaJavaLibrary.pConsole(amount+" x "+requiredVis+" = "+amt+", from
            //"+te.aspects.getAmount(aspect));
    ...
    private static abstract class AccelerationEffect extends LumenAlvearyEffect {
        ...
        protected final boolean tick(TileEntityLumenAlveary te) {
            if (te.worldObj.isRemote)
                return true;
            IBeekeepingLogic bkl
                = te.getMultiblockLogic().getController().getBeekeepingLogic();
            //max of 16 ticks per tick, so no massive changes (eg 2 prod cycles), and no
            //ingame time passage, and thus only possible change to canWork is queen
            //death, and even that is called by canWork
    ...
    private static class LightningProductionEffect extends LumenAlvearyEffect {
        ...
        @Override
        protected boolean tick(TileEntityLumenAlveary te) {
            if (te.getBeeHousing().canBlockSeeTheSky()
                && te.getBeeHousing().getBiome().canSpawnLightningBolt()
                && te.worldObj.isThundering()) {
    ...
    private static class HistoryRewriteEffect extends LumenAlvearyEffect {
        ...
        @Override
        protected boolean tick(TileEntityLumenAlveary te) {
            if (te.cachedQueen != null
                && ReikaRandomHelper.doWithChance(mateRewriteChance))
    ...
    private static class GeneticBalancingEffect extends LumenAlvearyEffect {
        ...
        protected boolean tick(TileEntityLumenAlveary te) {
            ...
            if (ReikaRandomHelper.doWithChance(pristineConversionChance)) {
                ...
                if (queen != null
                    && AlleleManager.alleleRegistry.getIndividual(queen)
                            instanceof IBee) {
            ...
            if (ReikaRandomHelper.doWithChance(geneBalancingChance)) {
                ...
                if (queen != null
                    && AlleleManager.alleleRegistry.getIndividual(queen)
                            instanceof IBee) {
                    EnumBeeChromosome gene = EnumBeeChromosome.values(
                    )[rand.nextInt(EnumBeeChromosome.values().length)];
        ...
        @ModDependent(ModList.FORESTRY)
        private void
        balanceGene(ItemStack queen, IBeeGenome ibg, EnumBeeChromosome gene) {
            ReikaBeeHelper.setGene(queen, ibg, gene, ibg.getActiveAllele(gene), true);
        }

        @ModDependent(ModList.FORESTRY)
        private boolean
        canBalance(ItemStack queen, IBeeGenome ibg, EnumBeeChromosome gene) {
    ...
    private static class GeneticImprovementEffect extends LumenAlvearyEffect {
        ...
        @ModDependent(ModList.FORESTRY)
        private void
        improveGene(EnumBeeChromosome gene, IBeeGenome ibg, ItemStack queen) {
            switch (gene) {
                case FERTILITY:
                case FLOWERING:
                case TERRITORY:
                ...
                case TEMPERATURE_TOLERANCE: {
                    if (ibg.getToleranceTemp() == EnumTolerance.NONE)
                        break;
                    EnumTolerance next
                        = ReikaBeeHelper.getOneBetterTolerance(ibg.getToleranceTemp());
                    if (next == null)
                        break;
                    ReikaBeeHelper.setGene(
                        queen, ibg, gene, ReikaBeeHelper.getToleranceGene(next), true
                    );
                    break;
                }
                case HUMIDITY_TOLERANCE: {
                    if (ibg.getToleranceHumid() == EnumTolerance.NONE)
                        break;
                    EnumTolerance next
                        = ReikaBeeHelper.getOneBetterTolerance(ibg.getToleranceHumid());
                    if (next == null)
                        break;
                    ReikaBeeHelper.setGene(
                        queen, ibg, gene, ReikaBeeHelper.getToleranceGene(next), true
        ...
        private boolean canImprove(EnumBeeChromosome gene, IBeeGenome ibg) {
            switch (gene) {
                ...
                case HUMIDITY_TOLERANCE:
                    return ReikaBeeHelper.getToleranceValue(ibg.getToleranceHumid()) > 0
                        && ReikaBeeHelper.getToleranceValue(ibg.getToleranceHumid()) < 2;
                case SPEED:
                    return ibg.getSpeed() < Speeds.FASTEST.getAllele().getValue();
                case TEMPERATURE_TOLERANCE:
                    return ReikaBeeHelper.getToleranceValue(ibg.getToleranceTemp()) > 0
                        && ReikaBeeHelper.getToleranceValue(ibg.getToleranceTemp()) < 2;
                case TERRITORY:
                    return ibg.getTerritory()[0]
                        < Territory.LARGEST.getAllele().getValue()[0];
                case CAVE_DWELLING:
                case EFFECT:
    ...
    private static class TemperatureMatchingEffect extends LumenAlvearyEffect {
        ...
        protected boolean tick(TileEntityLumenAlveary te) {
            ...
            float cur = /*ReikaBeeHelper.getTemperatureRangeCenter(EnumTemperature.getFromBiome(ac.getBiome(),
        ...
        @Override
        protected boolean isHellish(TileEntityLumenAlveary te) {
            return te.getSpecies() != null
                && te.getSpecies().getTemperature() == EnumTemperature.HELLISH;
        }
    ...
    private static class HumidityMatchingEffect extends LumenAlvearyEffect {
        ...
        protected boolean tick(TileEntityLumenAlveary te) {
            ...
            ChunkCoordinates loc = ac.getCoordinates();
            float cur
                = /*ReikaBeeHelper.getHumidityRangeCenter(EnumHumidity.getFromValue(*/ ac
                      .getBiome()
                      .rainfall /*))*/;
    ...
    private static abstract class GeneticRepairEffect extends LumenAlvearyEffect {
        ...
        private GeneticRepairEffect(int tier) {
            super(
                ...
                100 * ReikaMathLibrary.intpow2(24, tier - 1)
            );
            this.tier = tier;
        }
    ...
    private static class EternalEffect extends LumenAlvearyEffect {
        ...
        protected void tick(TileEntityLumenAlveary te) {
            if (te.hasQueen()) {
                ...
                if (ReikaBeeHelper.getBeeRoot().getType(is) == EnumBeeType.QUEEN) {
                    Life l = (Life)ReikaBeeHelper.getGeneEnum(EnumBeeChromosome.LIFESPAN,
        ReikaBeeHelper.getBee(is).getGenome()); if (l == CrystalBees.superLife || true)
                        ReikaBeeHelper.rejuvenateBee(te.getMultiblockLogic().getController(),
        is);
                }
            }
        }*/
        ...
        @Override
        protected float lifespanFactor(TileEntityLumenAlveary te) {
            Life l = (Life) ReikaBeeHelper.getGeneEnum(
                EnumBeeChromosome.LIFESPAN, te.cachedQueen.getGenome()
            );
    ...
    private static class ElementalBoostEffect extends LumenAlvearyEffect {
        ...
        @Override
        protected boolean isActive(TileEntityLumenAlveary te) {
            return super.isActive(te)
                && te.getSpecies() == CrystalBees.getElementalBee(color);
        }
    }
    ...
    private static class EffectIntensificationEffect extends LumenAlvearyEffect {
        ...
        @Override
        protected boolean isActive(TileEntityLumenAlveary te) {
            return super.isActive(te) && te.hasQueen() && te.getBeeGenome().getEffect()
    instanceof CrystalEffect;
        }
    ...
    private static class OmnipresentEffectEffect extends LumenAlvearyEffect {
        ...
        @Override
        protected boolean isActive(TileEntityLumenAlveary te) {
            return super.isActive(te) && te.hasQueen() && te.getBeeGenome() != null
                && te.getBeeGenome().getEffect() instanceof CrystalEffect;
        }
    }
    ...
    public AspectList getAspects() {
        return !aspects.aspects.isEmpty()
            ? aspects.copy()
            : null; //because his code crashes on empty list, but not null one
    }
    ...
    public void onDischarge(int charge, double range) {
        lightningTicks
            = Math.max(lightningTicks, ReikaMathLibrary.logbase2(charge / 6000));
    }
