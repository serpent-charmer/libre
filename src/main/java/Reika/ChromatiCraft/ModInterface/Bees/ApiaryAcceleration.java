...
public class ApiaryAcceleration extends SpecialAcceleration {
    ...
    public void register() {
        ...
        try {
            ...
            ChromatiCraft.logger.logError(
                "Could not find BeeKeepingLogic internal members!"
            );
        }
            ...
            if (type == EnumBeeType.QUEEN) {
                if (lgc.canWork()) {
                    ...
                    if (bee.getHealth() <= 0)
                        return;
                    float f
                        = ((IBeeModifier) beeLogicModifiers.get(lgc))
                              .getLifespanModifier(bee.getGenome(), bee.getMate(), 1.0F);
                    ...
                    if (ReikaRandomHelper.doWithChance(c * fac))
                        ReikaBeeHelper.ageBee(
                            te.worldObj, queen, f * fac
                        ); //not divide, is a lifespan factor, not a how much to age!
    ...
    public void updateBeeHealthBar(TileEntity te, IBeekeepingLogic lgc, IBee bee)
        throws Exception {
