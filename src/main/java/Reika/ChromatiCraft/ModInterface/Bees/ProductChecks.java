...
public class ProductChecks {
    public static abstract class ProductCondition {
        ...
        check(World world, int x, int y, int z, IBeeGenome ibg, IBeeHousing ibh);
    ...
    static class IridescentShardCheck extends ProductCondition {
        ...
        IridescentShardCheck() {
            //new AreaBlockCheck(new BlockKey(ChromaBlocks.CHROMA.getBlockInstance(), 0),
            //1); tileCheck = new AreaBlockCheck(new BlockKey(ChromaTiles.INFUSER), 1, 1);
        }
        ...
        check(World world, int x, int y, int z, IBeeGenome ibg, IBeeHousing ibh) {
            ...
            int tr = (int
            ) (ibg.getTerritory()[0] * 3F * beeModifier.getTerritoryModifier(ibg, 1.0F)
            ); //x, should == z; code from HasFlowersCache
            int r = tr >= 64 ? 128
                             : MathHelper.clamp_int(
                                 16 * ReikaMathLibrary.intpow2(2, (tr - 9) / 2), 16, 96
                             );
        ...
        private TileEntityAuraInfuser
        check(World world, int x, int y, int z, int r, int vr, IBeeHousing ibh) {
            WorldLocation find = TileEntityAuraInfuser.searchForMatch(
                (WorldLocation loc) -> {
                    if (loc.isWithinSquare(world, x, y, z, r, vr, r)
                        || ChromaBeeHelpers.isLumenAlvearyInfSight(ibh)) {
                        if (ChromaTiles.getTile(world, loc.xCoord, loc.yCoord, loc.zCoord)
                            == ChromaTiles.INFUSER) {
            ...
            return find != null ? (TileEntityAuraInfuser) find.getTileEntity(world)
                                : null;
        }
    ...
    static class AuraLocusCheck extends ProductCondition {
        ...
        check(World world, int x, int y, int z, IBeeGenome ibg, IBeeHousing ibh) {
            ...
            if (!TileEntityLocusPoint.hasLoci(TileEntityAuraPoint.class, ep))
                return false;
            TileEntityLumenAlveary tel = ChromaBeeHelpers.getLumenAlvearyController(
                ibh, world, ibh.getCoordinates()
            );
            if (tel != null && tel.hasInfiniteAwareness())
                return true;
            int[] r = ChromaBeeHelpers.getSearchRange(ibg, ibh);
            WorldLocation loc = TileEntityAuraPoint.getMatchFromCache(
                TileEntityAuraPoint.class,
                ep,
                (te
                 -> Math.abs(te.xCoord - x) <= r[0] && Math.abs(te.zCoord - z) <= r[0]
                     && Math.abs(te.yCoord - y) <= r[1])
            );
            return loc != null;
        }
    ...
    static class AreaBlockCheck extends ProductCondition {
        ...
        private boolean
        validate(World world, WorldLocation loc, Coordinate c, int r, int vr) {
        ...
        private Coordinate
        check(World world, int x, int y, int z, WorldLocation loc, int r, int vr) {
            double f = testedCoordinates.contains(loc)
                ? rangeRandom.getRandomEntry()
                : -1; //always run full scan for first scan
            if (f == -1) {
    ...
    static class CrystalPlantCheck extends ProductCondition {
        ...
        CrystalPlantCheck(CrystalElement e) {
            color = e;
            check = new AreaBlockCheck(
                new BlockKey(ChromaBlocks.PLANT.getBlockInstance(), color.ordinal()), 1
            );
        }
            ...
            if (check.check(world, x, y, z, ibg, ibh)) {
                if (world.rand.nextInt(30) == 0) {
                    ...
                    TileEntityCrystalPlant te
                        = (TileEntityCrystalPlant) plant.getTileEntity(world);
    ...
    static class FlowerCheck extends ProductCondition {
        ...
        FlowerCheck(CrystalElement e) {
            color = e;
            check = new AreaBlockCheck(
                new BlockKey(ChromaBlocks.DYEFLOWER.getBlockInstance(), color.ordinal()),
                1
            );
        }
    ...
    static class LeafCheck extends ProductCondition {
        ...
        LeafCheck(CrystalElement e) {
            ...
            //mk.add(new BlockKey(ChromaBlocks.DYELEAF.getBlockInstance(),
            //color.ordinal()));
    ...
    static class ChargedShardCheck extends ProductCondition {
        ...
        ChargedShardCheck(CrystalElement e) {
            ...
            chroma = new AreaBlockCheck(
                new BlockKey(ChromaBlocks.CHROMA.getBlockInstance(), 0), 1
            );
            MultiKey crys = new MultiKey();
            crys.add(
                new BlockKey(ChromaBlocks.CRYSTAL.getBlockInstance(), color.ordinal())
            );
        ...
        check(World world, int x, int y, int z, IBeeGenome ibg, IBeeHousing ibh) {
            return progress.check(world, x, y, z, ibg, ibh)
                && crystal.check(world, x, y, z, ibg, ibh)
                && chroma.check(world, x, y, z, ibg, ibh)
                && leaf.check(world, x, y, z, ibg, ibh);
        }
        ...
        RainbowTreeCheck() {
            ...
            for (LeafMetas lm : LeafMetas.list) {
                if (lm != LeafMetas.PLACED)
                    mk.add(new BlockKey(
                        ChromaBlocks.RAINBOWLEAF.getBlockInstance(), lm.ordinal()
                    ));
            }
