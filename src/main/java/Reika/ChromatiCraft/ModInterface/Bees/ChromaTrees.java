...
public class ChromaTrees {
    ...
    public static void register() {
        ...
        dyeBranch = new TreeBranch(
            "branch.ccdye",
            "Dye",
            "Pigmentum",
            "These leaves shimmer one or more colors, and seem to be associated with crystal energy."
        );

        dyeFlyBranch = new ButterflyBranch(
            "branch.ccdyefly",
            "Dye",
            "Pigmentum",
            "These vibrantly hued butterflies are primarily found in a similarly colored forest."
        );
        ...
        for (int i = 0; i < CrystalElement.elements.length; i++) {
            ...
            //AlleleManager.ersatzSaplings.put(ChromaBlocks.DYELEAF.getStackOf(color),
            //ii);
            AlleleManager.ersatzSpecimen.put(ChromaBlocks.DECAY.getStackOf(color), ii);
            AlleleManager.ersatzSaplings.put(
                ChromaBlocks.DYESAPLING.getStackOf(color), ii
            );
            AlleleManager.ersatzSpecimen.put(
                ChromaBlocks.DYESAPLING.getStackOf(color), ii
            );
            //AlleleManager.ersatzSpecimen.put(ChromaBlocks.DYELEAF.getStackOf(color),
            //ii);
            dyeTrees[i] = tree;
        ...
        AlleleManager.ersatzSaplings.put(
            ChromaBlocks.RAINBOWLEAF.getStackOfMetadata(0), tree
        );
        AlleleManager.ersatzSpecimen.put(
            ChromaBlocks.RAINBOWLEAF.getStackOfMetadata(0), tree
        );
    ...
    private static String[] getHybridParents(CrystalElement color) {
        switch (color) {
            ...
            case RED:
                return new String[] { "extratrees.species.redmaple",
                                      "forestry.treeBaobab" };
            case GREEN:
            ...
            case GRAY:
                return new String[] { "extratrees.species.locust",
                                      "forestry.treeZebrawood" };
            case PINK:
    ...
    private static class RainbowTree extends BasicTreeSpecies {
        ...
        public int getLeafColour(boolean pollinated) {
            return Color.HSBtoRGB(
                (int) ((System.currentTimeMillis() / 10) % 360) / 360F, 0.7F, 1F
            );
        }
        ...
        generate(World world, int x, int y, int z, Random rand, ITreeGenData data) {
            boolean big
                = data.getGirth(world, x, y, z) >= 2 && data.getHeightModifier() >= 1.5;
            if (big) {
                if (RainbowTreeGenerator.getInstance().checkRainbowTreeSpace(
                        world, x, y, z
                    )) {
                    //RainbowTreeGenerator.getInstance().generateLargeRainbowTree(world,
                    //x, y, z, rand);
                    FilledBlockArray arr = RainbowTreeBlueprint.getBlueprint(
                        world, x - 5, y - 2, z - 5, ReikaTreeHelper.OAK
                    );
            ...
            return RainbowTreeGenerator.getInstance().tryGenerateSmallRainbowTree(
                world, x, y, z, rand, data.getHeightModifier()
            );
        }
    }
    ...
    private static class HybridDyeTree extends DyeTreeBase {
        ...
        public HybridDyeTree(CrystalElement e, String... other) {
            super(
                e,
                e.displayName + " Hybrid",
                "chroma.hybrid" + e.name().toLowerCase(Locale.ENGLISH),
                "Pigmentum Hybridus " + e.displayName,
                DyeTreeTypes.list[e.ordinal()].traitsHybrid
            );
            breedParents = other;
        }
        ...
        protected void onRegister() {
            for (String s : breedParents) {
                ...
                if (ia instanceof IAlleleTreeSpecies) {
                    this.addBreeding(
                        (IAlleleTreeSpecies) ia, dyeTrees[color.ordinal()], 5
                    );
                    break;
                }
            }
        ...
        @Override
        protected void placeTree(
            World world, int x, int y, int z, GameProfile owner, ITreeGenData data
        ) {
            int h0 = 2 + rand.nextInt(3);
            int h = h0
                + MathHelper.ceiling_float_int(
                    data.getHeightModifier() * (5 + rand.nextInt(7))
                );
            for (int i = 1; i <= h; i++) {
            ...
            for (int j = 0; j < h; j++) {
                ...
                if (j >= h0) {
                    ...
                    //ReikaJavaLibrary.pConsole("j = "+j+"/"+h+", "+(up ? "up" : "down")+"
                    //to "+w);
                    int r = w;
                    for (int i = -r; i <= r; i++) {
                        for (int k = -r; k <= r; k++) {
                            if ((i != 0 || k != 0)
                                && Math.abs(i) + Math.abs(k) <= w + 1) {
                                int dx = x + i;
                                int dz = z + k;
            ...
            if (world.getBlock(x, y + h + 1, z)
                    .canBeReplacedByLeaves(world, x, y + h + 1, z))
    ...
    private static class DyeTree extends DyeTreeBase {
        protected DyeTree(CrystalElement e, TreeTraits t) {
            super(
                e,
                e.displayName + " Dye",
                "chroma.dye" + e.name().toLowerCase(Locale.ENGLISH),
                "Pigmentum " + e.displayName,
                t
            );
        }
        ...
        protected void placeTree(
            ...
            float h = MathHelper.clamp_float(
                (float) Math.sqrt(data.getHeightModifier()), 0.3F, 2F
            );
            TreeShaper.getInstance().generateRandomWeightedTree(
                world,
                x,
                y,
                z,
                rand,
                ReikaDyeHelper.dyes[color.ordinal()],
                true,
                0,
                0,
                array,
                ...
                if (girth > 2) {
                    ...
                    for (int i = 0; i < girth; i++) {
                     ...
                     for (int i = 1; i < girth; i++) {
                         array = (FilledBlockArray)cache.copy();
                         array.cutTo(c -> c.xCoord == cache.getMinX()+2 && c.zCoord <=
                     cache.getMinZ()+1); array.offset(i, 0, 0); this.place(array, world,
                     owner, data); array = (FilledBlockArray)cache.copy(); array.cutTo(c
                     -> c.xCoord == cache.getMinX()+2 && c.zCoord >= cache.getMaxZ()-1);
                         array.offset(i, 0, 0);
                    ...
                    for (Coordinate c : logs) {
                        for (Coordinate c2 : c.getAdjacentCoordinates()) {
                            if (c2.isEmpty(world)
                                && (leafY.contains(c2.yCoord)
                                    || girth >= 4 && leafY.contains(c2.yCoord + 1)))
                                data.setLeaves(
                                    world, owner, c2.xCoord, c2.yCoord, c2.zCoord
                                );
                        }
                    }
                }
        ...
        private void place(
            ...
            GameProfile owner,
            ITreeGenData data,
            HashSet<Integer> leafY,
            ArrayList<Coordinate> logs
        ) {
            for (Coordinate c : array.keySet()) {
                ...
                if (bk.blockID == ChromaBlocks.DECAY.getBlockInstance()) {
                    if (TreeShaper.canGenerateLeavesAt(
                    ...
                    data.setLogBlock(
                        world, c.xCoord, c.yCoord, c.zCoord, ForgeDirection.UP
                    );
                    if (logs != null)
                        logs.add(c);
                }
    ...
    private static abstract class DyeTreeBase extends TraitsTree {
        ...
        protected DyeTreeBase(
            CrystalElement e, String name, String uid, String latinName, TreeTraits traits
        ) {
        ...
        @Override
        protected final BlockKey getLogBlock(
            ITreeGenome genes,
            World world,
            int x,
            int y,
        ...
        placeTree(World world, int x, int y, int z, GameProfile owner, ITreeGenData data);
    ...
    private static class BerryProvider
        ...
        public ItemStack[] getFruits(
            ...
            if (f >= 1)
                f += 0.05;
            double fnum = Math.min(
                12 * genome.getYield(),
                Math.pow(f, 2)
                    + ReikaRandomHelper.getRandomBetween(
                        0, Math.max(1, 5 * genome.getYield())
                    )
            );
            int num = 0;
            while (fnum > 0) {
            ...
            if (num <= 0)
                return new ItemStack[0];
            return new ItemStack[] { ReikaItemHelper.getSizedItemStack(
                ChromaItems.BERRY.getStackOf(color), num
            ) };
        }
    ...
    private static abstract class ChromaButterflySpecies extends BasicButterflySpecies {
        ...
        public final String getEntityTexture() {
            return DirectResourceManager
                .getResource(
                    "Reika/ChromatiCraft/Textures/Butterflies/"
    ...
    private static class DyeButterfly extends ChromaButterflySpecies {
        ...
        private DyeButterfly(CrystalElement e) {
            super(
                e.displayName + " Dye Leaflet",
                "chroma.dyefly" + e.name().toLowerCase(Locale.ENGLISH),
                "Pigmentum " + e.displayName
            );
            color = e;
        }
        ...
        @Override
        public String getDescription() {
            return "Vibrantly glowing " + ReikaDyeHelper.dyes[color.ordinal()].colorName
                + ", this butterfly seems to have adapted itself to " + color.displayName
                + ".";
        }
