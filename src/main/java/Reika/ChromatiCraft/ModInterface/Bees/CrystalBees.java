...
public class CrystalBees {
    ...
    static final EnumMap<CrystalElement, CrystalEffect> effectMap
        = new EnumMap(CrystalElement.class);
    static final EnumMap<CrystalElement, CrystalAllele> flowerMap
        = new EnumMap(CrystalElement.class);
    static final EnumMap<CrystalElement, ItemHashMap<ProductCondition>> productConditions
        = new EnumMap(CrystalElement.class);
    ...
    private static void loadColorData() {
        ...
        auraColor
            = new ColorBlendList(18F, 0xffff00, 0xffffff, 0x000000, 0x8000ff, 0xff0000);
        ...
        precursorColor = new ColorBlendList(
            25F,
            0xffffff,
            0x404040,
            0x22aaff,
        ...
        towerColor = new ColorBlendList(
            15F, 0x6DE1FF, 0x6DE1FF, 0xffffff, 0x6DE188, 0x6DE188, 0xffffff
        );
    ...
    public static void register() {
        ...
        crystalBranch = new BeeBranch(
            "branch.cccrystal",
            "Crystal",
            "Vitreus",
            "These bees can sense and sometimes field the crystal elements."
        );
        advancedCrystalBranch = new BeeBranch(
            "branch.ccadvcrystal",
            "PolyCrystal",
            "Vitreus Corona",
            "These bees are masters of elemental manipulation and harnessing."
        );
        precursorBranch = new BeeBranch(
            "branch.ccprecursor", "Forerunner", "Praeministri", PrecursorBee.description
        );
        ...
        tower = new PrecursorBee(
            "Communicative",
            "Monumentum Vitreus Notitia",
            Speeds.SLOWER,
            Life.LONG,
            Flowering.SLOWEST,
            Fertility.LOW,
        ...
        chroma = (AdvancedBee) new AdvancedBee(
                     "Iridescent",
                     "Auram Stellans",
                     Speeds.SLOWER,
                     Life.NORMAL,
                     ...
                     .setEffect(chromaEffect);
        lumen = new AdvancedBee(
            "Luminescent",
            "Auram Ardens",
            Speeds.NORMAL,
            Life.SHORTENED,
        ...
        aura = (AdvancedBee) new AdvancedBee(
                   "Radiant",
                   "Auram Pharus",
                   Speeds.SLOW,
                   Life.LONG,
                   ...
                   .setEffect(rechargeEffect);
        multi = (AdvancedBee) new AdvancedBee(
                    "Polychromatic",
                    "Pigmentum Pluralis",
                    Speeds.SLOWEST,
                    Life.ELONGATED,
                    ...
                    .setEffect(multiEffect);

        precursor = (PrecursorBee) new PrecursorBee(
                        "Forerunner",
                        "Populus Antecessoris",
                        Speeds.NORMAL,
                        Life.NORMAL,
                        Flowering.AVERAGE,
                        Fertility.LOW,
                        ...
                        .setEffect(precursorEffect);
        ua = (PrecursorBee) new PrecursorBee(
                 "Monumental",
                 "Peritia Incognita",
                 Speeds.SLOWER,
                 Life.SHORTEST,
        ...
        if (ModList.MAGICBEES.isLoaded()) {
            magical.addBreeding(
                "Imperial", ModList.FORESTRY, "Arcane", ModList.MAGICBEES, 4
            );
        ...
        chroma.addBreeding(
            beeMap.get(CrystalElement.PURPLE), beeMap.get(CrystalElement.WHITE), 5
        );
        lumen.addBreeding(
            beeMap.get(CrystalElement.BLUE), beeMap.get(CrystalElement.BLACK), 5
        );
        ...
        lumen.otherChecks.add(new AreaBlockCheck(
            new BlockKey(ChromaBlocks.POWERTREE.getBlockInstance()), 2, 1
        ));
        ...
        tower.addSpecialty(
            new ItemStack(ChromaBlocks.METAALLOYLAMP.getBlockInstance()), 1
        );
        ...
        for (int i = 0; i < DimDecoTypes.list.length; i++) {
            ...
            if (DimDecoTypes.list[i].canSilkTouch())
                ; //precursor.addSpecialty(ChromaBlocks.DIMGEN.getStackOfMetadata(i),
                  //0.001F);
        }
    ...
    public static ArrayList<String> getBeeDescription(BeeSpecies b) {
        ...
        if (b instanceof ConditionalProductBee) {
            ConditionalProductProvider p
                = ((ConditionalProductBee) b).getProductProvider();
    ...
    addBreeding(String in1, BeeSpecies in2, CrystalElement out) {
        CrystalBee cb = beeMap.get(out);
        cb.addBreeding(in1, in2, 8);
    }

    private static final void
    addBreeding(BeeSpecies in1, BeeSpecies in2, CrystalElement out) {
        CrystalBee cb = beeMap.get(out);
        cb.addBreeding(in1, in2, 8);
    }

    private static final void
    addBreeding(CrystalElement in1, CrystalElement in2, CrystalElement out) {
    ...
    static class PrecursorBee extends AdvancedBee {
        private static final String description
            = "Something feels strange about these bees, almost as if they are a link to the past, or some long-lost knowledge.";

        private PrecursorBee(
            String name,
            String latin,
            Speeds s,
            Life l,
    ...
    static class AdvancedBee extends BasicBee implements ConditionalProductBee {
        ...
        private final CompoundConditionalProductProvider manager
            = new CompoundConditionalProductProvider();

        protected AdvancedBee(
            String name,
            String latin,
            Speeds s,
            Life l,
        ...
        public int getOutlineColor() {
            return colorList != null
                ? colorList.getColor(DragonAPICore.getSystemTimeAsInt() / 30D)
                : 0xffffff;
        }
    ...
    static class BasicBee extends TraitsBee {
        ...
        private BasicBee(
            ...
            super(
                name,
                "bee." + name.toLowerCase(Locale.ENGLISH),
                latin,
                "Reika",
                branch,
                new BeeTraits()
    ...
    static class SparkleBee extends BasicBee {
        ...
        @Override
        public boolean isJubilant(IBeeGenome ibg, IBeeHousing ibh) {
            return super.isJubilant(ibg, ibh)
                && ChromatiCraft.isRainbowForest(ibh.getBiome());
        }
        ...
        @Override
        public int getOutlineColor() {
            return sparkleColor != null
                ? sparkleColor.getColor(DragonAPICore.getSystemTimeAsInt() / 30D)
                : 0xffffff;
        }
    ...
    static final class CrystalBee extends BeeSpecies implements ConditionalProductBee {
        ...
        public CrystalBee(CrystalElement color, BeeTraits traits) {
            super(
                color.displayName + " Crystal",
                "bee.crystal." + color.name().toLowerCase(Locale.ENGLISH),
                "Vitreus " + color.displayName,
                "Reika",
                crystalBranch
            );
            this.color = color;
            ...
            this.addConditionalProduct(
                ChromaItems.BERRY.getStackOf(color), 25, true, new LeafCheck(color)
            );
            this.addConditionalProduct(
                ItemColoredModInteract.ColoredModItems.COMB.getItem(color),
                8,
                true,
                new CrystalPlantCheck(color)
            );
            this.addProduct(ForestryHandler.Combs.HONEY.getItem(), 15);
            this.addConditionalProduct(
                ChromaOptions.isVanillaDyeMoreCommon(color)
                    ? new ItemStack(Items.dye, 1, color.ordinal())
                    : ChromaItems.DYE.getStackOf(color),
                20,
                false,
                new FlowerCheck(color)
            );
            switch (color) {
                case BLACK:
                    this.addConditionalProduct(
                        ...
                        5,
                        true,
                        new ProgressionCheck(TieredPlants.FLOWER.level)
                    );
                    break;
                case RED:
                ...
                case ORANGE:
                    ...
                    this.addConditionalProduct(
                        ...
                        5,
                        true,
                        new ProgressionCheck(TieredOres.FIRESTONE.level)
                    );
                    break;
                case BLUE:
                    this.addConditionalProduct(
                        ...
                        5,
                        true,
                        new ProgressionCheck(TieredPlants.DESERT.level)
                    );
                    break;
                case YELLOW:
                ...
                case WHITE:
                    this.addConditionalProduct(
                        ...
                        5,
                        true,
                        new ProgressionCheck(TieredPlants.CAVE.level)
                    );
                    break;
                case CYAN:
                    this.addConditionalProduct(
                        ...
                        5,
                        true,
                        new ProgressionCheck(TieredOres.WATERY.level)
                    );
                    break;
                case LIME:
                    this.addConditionalProduct(
                        ...
                        5,
                        true,
                        new ProgressionCheck(TieredOres.SPACERIFT.level)
                    );
                    break;
                case GRAY:
                ...
                case LIGHTGRAY:
                    ...
                    break;
                case MAGENTA:
                    ItemStack is = ReikaBeeHelper.getBeeItem(
                        feedstock.getUID(), EnumBeeType.PRINCESS
                    );
        ...
        private void addConditionalProduct(
            ItemStack is, int chance, boolean specialty, ProductCondition c
        ) {
            if (specialty)
        ...
        @Override
        public int getOutlineColor() {
            return crystalColors != null
                ? crystalColors[color.ordinal()].getColor(
                    DragonAPICore.getSystemTimeAsInt() / 5D - color.ordinal() * 32
                )
                : color.getColor(
                ); //because MagicBees is a dumb and calls this from server side
        }
    ...
    public static void showConditionalStatuses(
        World world, int x, int y, int z, EntityPlayer ep, IBeeHousing ibh
    ) {
            ...
            if (ii instanceof IBee) {
                ...
                if (sp instanceof ConditionalProductBee) {
                    ReikaChatHelper.sendChatToPlayer(
                        ep,
                        "Product Conditions for " + EnumChatFormatting.GOLD + sp.getName()
                            + EnumChatFormatting.RESET + " Bee:"
                    );
