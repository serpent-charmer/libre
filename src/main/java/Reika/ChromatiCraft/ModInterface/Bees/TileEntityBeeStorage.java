...
public class TileEntityBeeStorage extends TileEntityMassStorage {
    ...
    private void initClient() {
        render = new InertItem(
            Minecraft.getMinecraft().theWorld, new ItemStack(Blocks.stone)
        );
    }
    ...
    protected void onTick(World world, int x, int y, int z) {
        if (world.isRemote
            && this.getTicksExisted() % 20
                == Math.abs(System.identityHashCode(this)) % 20) {
            Collection<KeyedItemStack> li = this.getItemTypes().keySet();
            KeyedItemStack is = li.isEmpty()
                ? null
                : ReikaJavaLibrary.getRandomCollectionEntry(rand, li);
            render.setEntityItemStack(
                is != null ? is.getItemStack() : new ItemStack(Blocks.stone)
            );
        }
    }
    ...
    protected KeyedItemStack key(ItemStack is) {
        return new KeyedItemStack(this.filterKey(is))
        ...
        if (NBT.hasKey("filtertype")) {
            ...
            try {
                ...
                ChromatiCraft.logger.logError(
                    "Failed to load genetics storage data from disk"
                );
                e.printStackTrace();
            }
        }
    ...
    private static class SpeciesType extends GeneticsType<ISpeciesType> {
        ...
        private static SpeciesType construct(String s) throws Exception {
            ...
            return new SpeciesType((ISpeciesType
            ) Enum.valueOf((Class<Enum>) Class.forName(parts[0]), parts[1]));
        }
    }
    ...
    private static abstract class GeneticsType<T> {
        ...
        public final boolean equals(Object o) {
            return o != null && o.getClass() == this.getClass()
                && this.comparator == ((GeneticsType) o).comparator;
        }
    }
}
