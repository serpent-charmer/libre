...
public class EfficientFlowerCache extends HasFlowersCache {
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        cachedTerritory = tag.hasKey("territory")
            ? BlockBox.readFromNBT(tag.getCompoundTag("territory"))
            : null;
    }
    ...
    private boolean isStillValid(World world, long time) {
        return flowerCoord != null
            && (time % 128 != id
                || (cachedTerritory.isBlockInside(flowerCoord)
                    && checkFlowerAcceptance(
                        cachedFlowerType,
                        world,
                        flowerCoord.xCoord,
                        flowerCoord.yCoord,
    ...
    private void findFlower(World world, boolean runFullScan) {
        if (runFullScan) {
            ...
            //ReikaJavaLibrary.pConsole("Took "+t2+" us to check "+cachedTerritory+" for
            //flower "+cachedFlowerType+"; coord = "+flowerCoord);
        } else {
            flowerCoord = null;
            if (search == null) {
    ...
    checkFlowerAcceptance(String type, World world, int x, int y, int z) {
        ...
        if (FlowerManager.flowerRegistry.isAcceptedFlower(type, world, x, y, z)) {
            if (!blacklistedAcceptanceCaching.contains(type)) {
                blockKey.read(world, x, y, z);
                if (y < 0
                    || blockKey.block.hasTileEntity(blockKey.metadata
                    )) { //since cannot check TE data, skip any bees that have TE-type
                         //flowers
