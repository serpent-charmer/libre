...
public class EntityChromaManaBurst
    extends ParticleEntity implements IEntityAdditionalSpawnData {
    ...
    public EntityChromaManaBurst(World world, int amt, ManaPath path, boolean accel) {
        super(
            world,
            MathHelper.floor_double(path.pathToBooster.get(0).xCoord),
            MathHelper.floor_double(path.pathToBooster.get(0).yCoord),
            MathHelper.floor_double(path.pathToBooster.get(0).zCoord)
        );
                    ...
                    if (hitBooster) {
                        if (amount < targetAmount) {
                            ...
                            this.setAmount(
                                amount
                                + Math.max(
                                    1, (targetAmount - amount) / (isAccelerated ? 12 : 24)
                                )
                            );
                        ...
                        double v = 0;//0.0625;
                        motionX -= dx*v;//+ReikaRandomHelper.getRandomPlusMinus(0,
                        0.03125); motionY -=
                        dy*v;//+ReikaRandomHelper.getRandomPlusMinus(0, 0.03125); motionZ
                        -= dz*v;//+ReikaRandomHelper.getRandomPlusMinus(0, 0.03125);
                             */
                            ...
                            if (boosterEntryTick > 0) {
                                ...
                                if (boosterCollectionTick >= 6) {
                                    ...
                                    p = p.interpolate(
                                        path.boosterEntry, p, boosterCollectionTick / 6D
                                    );
                                    this.setPosition(p);
                                }
                            }
                            ...
                            /*
                        double[] angs =
                        ReikaPhysicsHelper.cartesianToPolar(posX-path.boosterCenter.xCoord,
                        posY-path.boosterCenter.yCoord, posZ-path.boosterCenter.zCoord);
                        double d = 0.75;//angs[0]+(0.75-angs[0])*0.03125;
                        double ang1 =
                        angs[1]+2.2*2;//(ticksExisted*2.2+this.getEntityId())%360; double
                        ang2 =
                        -angs[2]+1.3*2;//(ticksExisted*1.3-this.getEntityId()*1.6)%360;
                        double[] xyz = ReikaPhysicsHelper.polarToCartesian(d, ang1, ang2);
                        DecimalPosition p = new
                        DecimalPosition(path.boosterCenter.offset(xyz[0], xyz[1]-0.375,
                        xyz[2])); this.setPosition(p);
                             */
                            ...
                            if (boosterEntryTick == -1 || boosterExitTick >= 6) {
                                ...
                                int t = pathTick;
                                pathTick
                                    = Math.min(path.pathToPool.size() - 1, pathTick + 1);
    ...
    public float getRenderSize() {
        ...
        float basefrac = (float) Math.pow(base / 16F, 0.25)
            / 1.25F; //scales from 0.4 @ 1 mana to 0.8 @ 16 mana to 1.6 @ 256 mana
        float frac = this.getMana() / base / 3F; //scales from 1 to 2 as it grows
        return frac * basefrac;
    }

    private void extractMana() {
        TileEntityManaBooster.receiveMana(
            worldObj, path.manaSource, startingAmount, true
        );
    ...
    private void dumpMana() {
        ...
        //TileEntityManaBooster.receiveMana(worldObj, path.manaSource, startingAmount,
        //true);
