...
public class TileEntityPageExtractor
    extends InventoriedRelayPowered implements ItemOnRightClick, OperationInterval {
    ...
    private static final AdjacencyCheckHandlerImpl adjacency
        = TileEntityAdjacencyUpgrade.getOrCreateAdjacencyCheckHandler(
            CrystalElement.PURPLE, "Reduce book destruction rate", ChromaTiles.BOOKDECOMP
        );
    ...
    private boolean isBook(ItemStack is) {
        return ModList.MYSTCRAFT.isLoaded()
            ...
            == GameRegistry.findItem(
                ModList.MYSTCRAFT.modLabel, MystObjects.Items.descriptive_book
            );
    }

    private double getConsumptionChance(IAgeSymbol ia) {
        //int rank = ReikaMystcraftHelper.getSymbolRank(ia);
        //ReikaJavaLibrary.pConsole(ia.identifier()+" > "+rank+" >
        //"+(100-98*Math.exp(-rank/1.75)));
    ...
    public ItemStack onRightClickWith(ItemStack item, EntityPlayer ep) {
        if (ep.isSneaking()) {
            ...
            ReikaSoundHelper.playSoundAtBlock(
                worldObj, xCoord, yCoord, zCoord, "random.pop"
            );
            return ret;
    ...
    public OperationState getState() {
        return !symbolMap.isEmpty() && inv[0] != null && inv[1] == null
