...
public class BlockActiveChroma extends BlockLiquidChroma {
    ...
    public IIcon getIcon(IBlockAccess iba, int x, int y, int z, int s) {
        ...
        return te instanceof TileEntityChroma && ((TileEntityChroma)te).isActive() ? ico :
    Blocks.grass.getIcon(1, 0);
    }*/
    ...
    public static int getColor(CrystalElement e, int berries) {
        return ReikaColorAPI.mixColors(
            e.getColor(), 0xffffff, berries / (float) TileEntityChroma.BERRY_SATURATION
        );
    }

    public static float getDoublingChance(int ether) {
        return /*0.75F*/ (float
        ) (Math.pow(ether, 2) / Math.pow(TileEntityChroma.ETHER_SATURATION, 2));
    }
    ...
    public static class TileEntityChroma extends TileEntity {
        ...
        public int getColor() {
            return element != null ? BlockActiveChroma.getColor(element, berryCount)
                                   : 0xffffff;
        }
