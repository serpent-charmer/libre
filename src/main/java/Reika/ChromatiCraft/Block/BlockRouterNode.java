...
public class BlockRouterNode extends Block implements SidedBlock {
    ...
    public boolean canPlaceOn(World world, int x, int y, int z, int side) {
        ...
        ); //.isSideSolid(world, x, y, z, ForgeDirection.VALID_DIRECTIONS[side]);
    }
    ...
    private static void drop(World world, int x, int y, int z) {
        ItemStack is = new ItemStack(
            ChromaBlocks.ROUTERNODE.getBlockInstance(), 1, world.getBlockMetadata(x, y, z)
        );
    ...
    public static abstract class TileEntityRouterNode
        extends TileEntity implements RouterFilter {
