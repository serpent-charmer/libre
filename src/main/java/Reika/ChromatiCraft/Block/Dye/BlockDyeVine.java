...
//commonly hangs down from under the leaves, and rarely on worldgen can have a pod that
//drops the shards
public class BlockDyeVine extends BlockTallGrass {
    ...
    public final int colorMultiplier(IBlockAccess iba, int x, int y, int z) {
        return ReikaColorAPI.mixColors(
            ChromaBlocks.DYELEAF.getBlockInstance().colorMultiplier(iba, x, y, z),
            0x404040,
            0.7F
        );
    }
    ...
    public boolean canBlockStay(World world, int x, int y, int z, int meta) {
        ...
        return (at instanceof BlockDyeLeaf || at instanceof BlockDyeVine)
    ...
    public void render(
        ...
        int c = ReikaColorAPI.mixColors(
            ChromaBlocks.DYELEAF.getBlockInstance().colorMultiplier(world, x, y, z),
            0x404040,
            fc
        );
        ...
        if (isFertile) {
            ico = glowIcon;
            v5.setColorOpaque_I(
                CrystalElement.elements[world.getBlockMetadata(x, y, z)].getColor()
            );
    ...
    public boolean canReplace(World world, int x, int y, int z, int side, ItemStack is) {
        return is != null && world.getBlock(x, y, z).isReplaceable(world, x, y, z)
            && this.canBlockStay(world, x, y, z, is.getItemDamage());
    }
    ...
    getDrops(World world, int x, int y, int z, int meta, int fortune) {
        ...
        if (isFertile) {
            ret.add(ChromaItems.SHARD.getCraftedMetadataProduct(
                1 + world.rand.nextInt(1 + fortune / 2), meta
            ));
        }
        ...
        if (world.rand.nextInt(Math.max(1, 3 - fortune)) == 0)
            ret.add(ChromaItems.BERRY.getCraftedMetadataProduct(
                1 + world.rand.nextInt(1 + fortune), meta
            ));
