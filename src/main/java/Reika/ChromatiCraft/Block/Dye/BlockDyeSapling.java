...
public class BlockDyeSapling extends BlockSapling {
    ...
    public void func_149878_d(World world, int x, int y, int z, int h) {
        ...
        TreeShaper.getInstance().generateRandomWeightedTree(
            world, x, y, z, world.rand, ReikaDyeHelper.dyes[meta], true, 0.25F, 0
        );
    }
    ...
    public boolean onBlockActivated(
        ...
        if (this.canGrowAt(world, x, y, z, true))
            this.func_149878_d(
                world, x, y, z, ep.isSneaking() ? 7 : this.getGrowthHeight()
            );
        else
            world.spawnParticle(
                "happyVillager",
    ...
    canGrowAt(World world, int x, int y, int z, boolean ignoreLight) {
    ...
    @Override
    public void
    func_149879_c(World par1World, int par2, int par3, int par4, Random par5Random) {
