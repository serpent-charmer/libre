...
public class BlockDyeLeaf extends BlockCustomLeaf implements LeafBlockWithExtras {
    ...
    public static int getColor(int meta, boolean damage) {
        ...
        return damage ? ReikaColorAPI.mixColors(
                   0xa0a0a0,
                   ReikaColorAPI.getColorWithBrightnessMultiplier(c.getRGB(), 0.85F),
                   0.4F
               )
                      : c.brighter().getRGB();
    }
    ...
    protected void onRandomUpdate(World world, int x, int y, int z, Random r) {
        if (r.nextInt(10) == 0) {
            ...
            ArrayList<TileEntityCrystalPylon> c = new ArrayList(
                CrystalNetworker.instance.getNearbyPylons(world, x, y, z, e, 16, false)
            );
            if (c == null || c.isEmpty())
                return;
    ...
    getDrops(World world, int x, int y, int z, int meta, int fortune) {
        ...
        if (ReikaRandomHelper.doWithChance(berryChance))
            li.add(ChromaItems.BERRY.getCraftedMetadataProduct(
                berryNum + (int) berryChance, meta
            ));
        return li;
    }
    ...
    onSheared(ItemStack item, IBlockAccess world, int x, int y, int z, int fortune) {
        ...
        ret.add(new ItemStack(
            ChromaBlocks.DYELEAF.getBlockInstance(), 1, world.getBlockMetadata(x, y, z)
        ));
        return ret;
    }
    ...
    public boolean isMatchingLeaf(
        IBlockAccess iba, int x, int y, int z, int lookX, int lookY, int lookZ
    ) {
        Block b = iba.getBlock(lookX, lookY, lookZ);
        return (ModList.FORESTRY.isLoaded()
                && b == ForestryHandler.BlockEntry.LEAF.getBlock())
            || (b == this
                && iba.getBlockMetadata(x, y, z)
                    == iba.getBlockMetadata(lookX, lookY, lookZ));
    }
    ...
    public void onPreWoodcutterBreak(World world, int x, int y, int z) {
        LeafBlockWithExtras.breakHangingVines(
            world, x, y, z, b -> b.blockID instanceof BlockDyeVine
        );
    }
    ...
    getExtraDrops(World world, int x, int y, int z, int fortune) {
        return LeafBlockWithExtras.getExtraDropsForHangingVines(
            world, x, y, z, fortune, b -> b.blockID instanceof BlockDyeVine
        );
    }
}
