    ...
    public boolean canGrowLargeRainbowTreeAt(World world, int x, int y, int z) {
        ...
        for (int dx = 0; dx >= -1; dx--) {
            for (int dz = 0; dz >= -1; dz--) {
                if (id == world.getBlock(x + dx, y, z + dz)
                    && id == world.getBlock(x + dx + 1, y, z + dz)
                    && id == world.getBlock(x + dx, y, z + dz + 1)
                    && id == world.getBlock(x + dx + 1, y, z + dz + 1)) {
                    flag = true;
                    break;
                }
            }
        ...
        return world.getBlockLightValue(x, y, z) >= 9
            && RainbowTreeGenerator.getInstance().checkRainbowTreeSpace(world, x, y, z);
    }
