...
public class BlockDyeGrass extends BlockGrass {
    ...
    public int colorMultiplier(IBlockAccess world, int x, int y, int z) {
        ...
        Color px = RainbowForestGenerator.getColor(x + 16 * rx, y, z)
                       .getJavaColor()
                       .brighter();
        Color mx = RainbowForestGenerator.getColor(x - 16 * rx, y, z)
                       .getJavaColor()
                       .brighter();
        Color pz = RainbowForestGenerator.getColor(x, y, z + 16 * rz)
                       .getJavaColor()
                       .brighter();
        Color mz = RainbowForestGenerator.getColor(x, y, z - 16 * rz)
                       .getJavaColor()
                       .brighter();
        ...
        Color c1 = new Color(
            (int) (color.getRed() * (1 - fx1) + px.getRed() * fx1),
            (int) (color.getGreen() * (1 - fx1) + px.getGreen() * fx1),
            (int) (color.getBlue() * (1 - fx1) + px.getBlue() * fx1)
        );
        Color c2 = new Color(
            (int) (color.getRed() * (1 - fx2) + mx.getRed() * fx2),
            (int) (color.getGreen() * (1 - fx2) + mx.getGreen() * fx2),
            (int) (color.getBlue() * (1 - fx2) + mx.getBlue() * fx2)
        );
        Color c3 = new Color(
            (int) (color.getRed() * (1 - fz1) + pz.getRed() * fz1),
            (int) (color.getGreen() * (1 - fz1) + pz.getGreen() * fz1),
            (int) (color.getBlue() * (1 - fz1) + pz.getBlue() * fz1)
        );
        Color c4 = new Color(
            (int) (color.getRed() * (1 - fz2) + mz.getRed() * fz2),
            (int) (color.getGreen() * (1 - fz2) + mz.getGreen() * fz2),
            (int) (color.getBlue() * (1 - fz2) + mz.getBlue() * fz2)
        );
