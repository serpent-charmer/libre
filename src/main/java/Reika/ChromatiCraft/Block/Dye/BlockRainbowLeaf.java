...
public class BlockRainbowLeaf extends BlockCustomLeaf {
    ...
    public final int colorMultiplier(IBlockAccess iba, int x, int y, int z) {
        ...
        float hue = LeafMetas.list[meta].hasTimeColor()
            ? (System.currentTimeMillis() % 7200) / 7200F
            : (float) (ReikaMathLibrary.py3d(x, y * 3, z + x) % sc) / sc;
    ...
    getDrops(World world, int x, int y, int z, int meta, int fortune) {
        ...
        float rareGoldAppleChance
            = ChromaOptions.getRainbowLeafGoldAppleDropChance(); //0.0025F default
    ...
    getDyes(World world, int x, int y, int z, int fortune, boolean small) {
    ...
    private int getDyeDropCount(int fortune, boolean small) {
        int ret = 1 + rand.nextInt(3 * (1 + fortune)) + fortune
            + rand.nextInt(1 + fortune * fortune);
        if (small)
        ...
        if (rand.nextInt(10) == 0) {
            if (!world.isRemote && ChromaOptions.BALLLIGHTNING.getState())
                world.spawnEntityInWorld(
                    new EntityBallLightning(
                        world, CrystalElement.randomElement(), x + 0.5, y + 0.5, z + 0.5
                    )
                        .setNoDrops()
                );
        }
    ...
    public final void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        world.spawnParticle(
            "reddust", x + rand.nextDouble(), y, z + rand.nextDouble(), r, g, b
        );
        if (BiomeGlowingCliffs.isGlowingCliffs(world.getBiomeGenForCoords(x, z))
            || (ModList.MYSTCRAFT.isLoaded() && ReikaMystcraftHelper.isMystAge(world))) {
            if (world.getBlock(x, y + 1, z) != this) {
                ...
                EntityFloatingSeedsFX fx2 = new EntityCCFloatingSeedsFX(
                    world,
                    x + rand.nextDouble(),
                    y + rand.nextDouble(),
                    z + rand.nextDouble(),
                ...
                fx2.setColor(ReikaColorAPI.mixColors(color, 0xffffff, 0.5F))
                    .setScale(s * 0.66F)
                    .lockTo(fx);
                EntityFloatingSeedsFX fx3 = new EntityCCFloatingSeedsFX(
                    world,
                    x + rand.nextDouble(),
                    y + rand.nextDouble(),
                    z + rand.nextDouble(),
    ...
    getFireSpreadSpeed(IBlockAccess world, int x, int y, int z, ForgeDirection face) {
        if (world instanceof World)
            RainbowTreeEffects.instance.addInstability(
                (World) world, x, y, z
            ); //make burning the tree add the instability back
        return 180;
    }
    ...
    onBlockDestroyedByExplosion(World world, int x, int y, int z, Explosion e) {
        RainbowTreeEffects.instance.addInstability(
            world, x, y, z
        ); //make exploding the tree add the instability back
    }

    @Override
    protected void
    onRandomUpdate(World world, int x, int y, int z, Random r) { /*
if (r.nextInt(20) == 0)
...
for (int i = 0; i < 256; i++) {
  ReikaWorldHelper.temperatureEnvironment(world, rx, i, rz,
ReikaWorldHelper.getBiomeTemp(ChromatiCraft.forest)); world.func_147479_m(rx, i, rz);
  world.markBlockForUpdate(rx, i, rz);
    ...
    public boolean isValidLog(
        ...
        return ((BlockDyeLeaf) ChromaBlocks.DECAY.getBlockInstance())
            .isValidLog(iba, thisX, thisY, thisZ, lookX, lookY, lookZ);
    }
