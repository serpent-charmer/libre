...
public class BlockCrystalTank
    extends Block implements IWailaDataProvider, ConnectedTextureGlass {
    ...
    public void onBlockAdded(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (c == ChromaTiles.TANK) {
                TileEntityCrystalTank tank
                    = (TileEntityCrystalTank) world.getTileEntity(dx, dy, dz);
                te.setTile(tank);
                te.markComplete();
                te.addToTank();
                con = tank;
                ...
                CrystalTankAuxTile tile
                    = (CrystalTankAuxTile) world.getTileEntity(dx, dy, dz);
                if (tile.hasTile()) {
        ...
        if (con != null) {
            ...
            blocks.recursiveAddWithBounds(
                world, x, y, z, this, x - 32, y - 32, z - 32, x + 32, y + 32, z + 32
            );
    ...
    private void confirmHasController(World world, int x, int y, int z) {
        BlockArray blocks = new BlockArray();
        Set<BlockKey> li
            = ReikaJavaLibrary.getSet(new BlockKey(this), new BlockKey(ChromaTiles.TANK));
        blocks.recursiveAddMultipleWithBounds(
            world, x, y, z, li, x - 32, y - 32, z - 32, x + 32, y + 32, z + 32
        );
        TileEntityCrystalTank con = null;
        int count = 0;
        for (int i = 0; i < blocks.getSize(); i++) {
    ...
    public boolean shouldSideBeRendered(IBlockAccess iba, int x, int y, int z, int side) {
        ...
        return iba.getBlock(dx, dy, dz) != this
            && ChromaTiles.getTile(iba, dx, dy, dz) != ChromaTiles.TANK;
    }
    ...
    getEdgesForFace(IBlockAccess world, int x, int y, int z, ForgeDirection face) {
        ...
        if (face.offsetX != 0) { //test YZ
            ...
            if (world.getBlock(x, y, z + 1) == this
                || ChromaTiles.getTile(world, x, y, z + 1) == ChromaTiles.TANK)
                li.remove(2);
            if (world.getBlock(x, y, z - 1) == this
                || ChromaTiles.getTile(world, x, y, z - 1) == ChromaTiles.TANK)
                li.remove(8);
            if (world.getBlock(x, y + 1, z) == this
                || ChromaTiles.getTile(world, x, y + 1, z) == ChromaTiles.TANK)
                li.remove(4);
            if (world.getBlock(x, y - 1, z) == this
                || ChromaTiles.getTile(world, x, y - 1, z) == ChromaTiles.TANK)
                li.remove(6);
            ...
            if (world.getBlock(x, y + 1, z + 1) == this && !li.contains(4)
                && !li.contains(2))
                li.remove(1);
            if (world.getBlock(x, y - 1, z - 1) == this && !li.contains(6)
                && !li.contains(8))
                li.remove(9);
            if (world.getBlock(x, y + 1, z - 1) == this && !li.contains(4)
                && !li.contains(8))
                li.remove(7);
            if (world.getBlock(x, y - 1, z + 1) == this && !li.contains(2)
                && !li.contains(6))
                li.remove(3);
        }
        if (face.offsetY != 0) { //test XZ
            ...
            if (world.getBlock(x + 1, y, z) == this
                || ChromaTiles.getTile(world, x + 1, y, z) == ChromaTiles.TANK)
                li.remove(4);
            if (world.getBlock(x - 1, y, z) == this
                || ChromaTiles.getTile(world, x - 1, y, z) == ChromaTiles.TANK)
                li.remove(6);
            ...
            //Corners; only removed if have adjacent on side AND corner
            if (world.getBlock(x + 1, y, z + 1) == this && !li.contains(4)
                && !li.contains(2))
                li.remove(1);
            if (world.getBlock(x - 1, y, z - 1) == this && !li.contains(6)
                && !li.contains(8))
                li.remove(9);
            if (world.getBlock(x + 1, y, z - 1) == this && !li.contains(4)
                && !li.contains(8))
                li.remove(7);
            if (world.getBlock(x - 1, y, z + 1) == this && !li.contains(2)
                && !li.contains(6))
                li.remove(3);
        }
        if (face.offsetZ != 0) { //test XY
            ...
            //Corners; only removed if have adjacent on side AND corner
            if (world.getBlock(x + 1, y + 1, z) == this && !li.contains(2)
                && !li.contains(4))
                li.remove(1);
            if (world.getBlock(x - 1, y - 1, z) == this && !li.contains(8)
                && !li.contains(6))
                li.remove(9);
            if (world.getBlock(x + 1, y - 1, z) == this && !li.contains(2)
                && !li.contains(6))
                li.remove(3);
            if (world.getBlock(x - 1, y + 1, z) == this && !li.contains(4)
                && !li.contains(8))
                li.remove(7);
        }

        if (!li.contains(2) && !li.contains(4) && !li.contains(6) && !li.contains(8)
            && (x + z) % 2 != 0) {
    ...
    public int isProvidingWeakPower(IBlockAccess iba, int x, int y, int z, int par5) {
        ...
        return te2 != null && te2.getCurrentFluid() == FluidRegistry.getFluid("redstone")
            ? (int) Math.round(te2.getFillPercentage() * 15)
            : 0;
    }
    ...
    public static class CrystalTankAuxTile extends TileEntity implements CrystalTank {
        ...
        public int getLightValue() {
            ...
            return te != null && te.getCurrentFluid() != null
                ? te.getCurrentFluid().getLuminosity()
                : 0;
        }
        ...
        public void markComplete() {
            worldObj.setBlockMetadataWithNotify(
                xCoord, yCoord, zCoord, this.getBaseMetadata() + 1, 3
            );
        }
        ...
        public void reset() {
            controller = null;
            worldObj.setBlockMetadataWithNotify(
                xCoord, yCoord, zCoord, this.getBaseMetadata(), 3
            );
        }
        ...
        public TileEntityCrystalTank getTankController() {
            ...
            return te instanceof TileEntityCrystalTank ? (TileEntityCrystalTank) te
                                                       : null;
        }
        ...
        public CrystalTank getController() {
            TileEntity te
                = controller != null ? controller.getTileEntity(worldObj) : null;
