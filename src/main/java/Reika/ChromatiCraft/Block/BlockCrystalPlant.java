...
public class BlockCrystalPlant extends Block {
    ...
    public boolean canBlockStay(World world, int x, int y, int z) {
        return world.getBlock(x, y - 1, z) == ChromaBlocks.MUD.getBlockInstance()
            || ReikaPlantHelper.SAPLING.canPlantAt(world, x, y, z);
    }
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random r) {
        ...
        if (te.isPure()) {
            ...
            EntityBlurFX fx = new EntityCCBlurFX(world, px, y + r.nextDouble(), pz)
            ...
            if (te.canHarvest()) {
                for (int i = 0; i < 4; i++) {
                    ...
                    fx = new EntityCCBlurFX(
                             ...
                             .setScale(s);
                    fx.setColor(
                        ReikaColorAPI.mixColors(te.getColor().getColor(), 0xffffff, 0.5F)
                    );
            ...
            if (te.is(Modifier.BOOSTED)) {
                px = x + r.nextDouble();
                pz = z + r.nextDouble();
                fx = new EntityCCBlurFX(world, px, y + r.nextDouble(), pz)
                         .setColor(te.getColor().getColor());
                fx.setGravity(-0.03125F);
                Minecraft.getMinecraft().effectRenderer.addEffect(fx);
                float fs = (float) ReikaRandomHelper.getRandomBetween(0.125, 0.75);
                EntityFX fx2 = new EntityCCBlurFX(world, px, fx.posY, pz)
                                   .setColor(0xffffff)
                                   .lockTo(fx)
                                   .setScale(fs);
            ...
            if (te.is(Modifier.PRIMAL)) {
                ...
                fx = new EntityCCBlurFX(world, px, y + 0.9375, pz, vx, 0, vz)
                         .setColor(te.getColor().getColor())
                         .setRapidExpand();
                SpiralMotionController m
                    = new SpiralMotionController(x + 0.5, z + 0.5, 15, 0.015, rad, 0, a);
        ...
        if (this.canBlockStay(world, x, y, z)) {
            if (random.nextInt(12) == 0) {
                TileEntityCrystalPlant te
                    = (TileEntityCrystalPlant) world.getTileEntity(x, y, z);
                te.grow();
            }
        } else {
            this.die(world, x, y, z);
    ...
    public IIcon getIcon(int s, int meta) {
        //return Minecraft.getMinecraft().gameSettings.fancyGraphics ? colorIcon :
        //fastIcon;
        return fastIcon;
    }
    ...
    { ArrayList li = new ArrayList(); ItemStack shard =
    ChromaItems.SHARD.getStackOfMetadata(world.getBlockMetadata(x, y, z)); li.add(shard);
            return li;
        }
        return null;
    }
    ...
    public ItemStack getPickBlock(
        MovingObjectPosition target, World world, int x, int y, int z, EntityPlayer ep
    ) {
