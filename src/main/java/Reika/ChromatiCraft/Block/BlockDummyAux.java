...
public class BlockDummyAux extends BlockContainer implements IWailaDataProvider {
    ...
    public IIcon getIcon(IBlockAccess iba, int x, int y, int z, int s) {
        ...
        return te.getFlag(Flags.RENDER)
            ? ChromaBlocks.STRUCTSHIELD.getBlockInstance().getIcon(
                1, BlockType.STONE.metadata
            )
    ...
    getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        TileEntityDummyAux te = (TileEntityDummyAux) world.getTileEntity(x, y, z);
        return te.getFlag(Flags.HITBOX)
    ...
    public ItemStack getWailaStack(IWailaDataAccessor acc, IWailaConfigHandler cfg) {
        ...
        return b instanceof IWailaDataProvider
            ? ((IWailaDataProvider) b).getWailaStack(acc, cfg)
            : null;
    }
    ...
    public List<String> getWailaHead(
        ...
        if (te == null || te.relay == null)
            return currenttip;
        Block b = te.relay.getBlock(acc.getWorld());
        return b instanceof IWailaDataProvider ? ((IWailaDataProvider)b).getWailaHead(is,
        currenttip, acc, cfg) : currenttip;
         */
        return currenttip;
    }
    ...
    public List<String> getWailaBody(
        ...
        if (te == null || te.relay == null)
            return currenttip;
        Block b = te.relay.getBlock(acc.getWorld());
        return b instanceof IWailaDataProvider ? ((IWailaDataProvider)b).getWailaBody(is,
        currenttip, acc, cfg) : currenttip;
         */
        return currenttip;
    }
    ...
    public List<String> getWailaTail(
        ...
        if (te == null || te.relay == null)
            return currenttip;
        Block b = te.relay.getBlock(acc.getWorld());
        return b instanceof IWailaDataProvider ? ((IWailaDataProvider)b).getWailaTail(is,
        currenttip, acc, cfg) : currenttip;
         */
        return currenttip;
    }
    ...
    public static class TileEntityDummyAux extends TileEntity {
        ...
        private boolean relayClick(EntityPlayer ep, int s, float a, float b, float c) {
            if (relay != null)
                return relay.getBlock(worldObj).onBlockActivated(
                    worldObj, relay.xCoord, relay.yCoord, relay.zCoord, ep, s, a, b, c
                );
            return false;
        }

        public boolean relayManipulatorClick(
            ItemStack is, EntityPlayer ep, int s, float a, float b, float c
        ) {
            if (relay == null)
                return false;
            return ChromaItems.TOOL.getItemInstance().onItemUse(
                is, ep, worldObj, relay.xCoord, relay.yCoord, relay.zCoord, s, a, b, c
            );
        }
