...
public class BlockCastingInjectorFocus extends Block {
    ...
    public IIcon getIcon(int s, int meta) {
        return s == 0
            ? bottom
            : (s == 1 ? topTextures[GuiMachineDescription.runningRender ? 1 : 0]
                      : sideTextures[GuiMachineDescription.runningRender ? 1 : 0]);
    }
    ...
    public IIcon getIcon(IBlockAccess iba, int x, int y, int z, int s) {
        if (s == 0)
            return bottom;
        boolean active
            = GuiMachineDescription.runningRender || this.isValidPosition(iba, x, y, z);
        int idx = active ? 1 : 0;
