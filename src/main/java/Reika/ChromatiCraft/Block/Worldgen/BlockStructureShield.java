...
public class BlockStructureShield
    extends Block implements SemiUnbreakable, Submergeable, CustomSnowAccumulation {
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block b) {
        for (int i = 0; i < 6; i++) {
            ...
            if (!this.isSideSolid(world, x, y, z, dir)) {
                ...
                if (b == b2
                    && this.canBreakLitBlockAt(
                        world, dx, dy, dz, b2, world.getBlockMetadata(dx, dy, dz)
                    )) {
    ...
    private boolean
    canBreakLitBlockAt(World world, int x, int y, int z, Block b, int meta) {
        if (ReikaBlockHelper.isLiquid(b))
            return false;
        if (b == ChromaBlocks.DOOR.getBlockInstance())
            return false;
        if (b instanceof BlockStructureShield || b instanceof BlockDimensionStructure
            || b instanceof BlockDimensionStructureTile)
            return false;
        if (ChromaBlocks.getEntryByID(b) != null
            && ChromaBlocks.getEntryByID(b).isDimensionStructureBlock())
            return false;
    ...
    public IIcon getIcon(IBlockAccess iba, int x, int y, int z, int s) {
        ...
        if (meta % 8 == BlockType.GLASS.ordinal() && s > 1) {
            if (ChromaTiles.getTile(iba, x, y - 2, z) == ChromaTiles.CHROMACRAFTER
                && ((TileEntityChromaCrafter) iba.getTileEntity(x, y - 2, z))
                       .hasStructure()) {
    ...
    shouldSideBeRendered(IBlockAccess world, int dx, int dy, int dz, int s) {
        ...
        int meta = world.getBlockMetadata(
            dx - dir.offsetX, dy - dir.offsetY, dz - dir.offsetZ
        );
        if (b.isOpaqueCube())
            return false;
