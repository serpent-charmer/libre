...
public class BlockEtherealLuma extends BlockFluidClassic {
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < 16; i++) {
            ...
            dimensionIcons[i]
                = ico.registerIcon("chromaticraft:fluid/aether/aether_still_dim_" + i);
        }
    ...
    public IIcon getIcon(IBlockAccess iba, int x, int y, int z, int s) {
        if (iba.getBlockMetadata(x, y, z) != 0
            || this.getFlowVector(iba, x, y, z).lengthVector() > 0)
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random r) {
        if (r.nextInt(2 * (1 + Minecraft.getMinecraft().gameSettings.particleSetting))
            == 0) {
