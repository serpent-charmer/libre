...
public class BlockDecoFlower extends Block implements IShearable, LoadRegistry {
    ...
    getDrops(World world, int x, int y, int z, int meta, int fortune) {
        return world.rand.nextInt(Flowers.list[meta].getDropChance()) == 0
            ? ReikaJavaLibrary.makeListFrom(Flowers.list[meta].getDrop())
            : new ArrayList();
    }
    ...
    public int colorMultiplier(IBlockAccess iba, int x, int y, int z) {
        ...
        return f.isBiomeColored()
            ? iba.getBiomeGenForCoords(x, z).getBiomeGrassColor(x, y, z)
            : 0xffffff;
    }
    ...
    isIvySolid(IBlockAccess world, int x, int y, int z, ForgeDirection dir) {
        Block b = world.getBlock(x, y, z);
        return (b.getMaterial() == Material.rock || ReikaBlockHelper.isFacade(b))
            && b.isSideSolid(world, x, y, z, dir);
    }
        ...
        if (!this.checkAndDropBlock(world, x, y, z)) {
            ...
            if (c != null) {
                ...
                ReikaSoundHelper.playBreakSound(
                    world, c.xCoord, c.yCoord, c.zCoord, this, 1, 1
                );
    ...
    @Override
    public ArrayList<ItemStack>
    onSheared(ItemStack item, IBlockAccess world, int x, int y, int z, int fortune) {
        int meta = world.getBlockMetadata(x, y, z);
        Flowers f = Flowers.list[meta];
    ...
    public static enum Flowers {
        ...
        public int getLightValue(IBlockAccess iba, int x, int y, int z) {
            switch (this) {
                case GLOWDAISY:
                    return iba instanceof World
                            && ReikaWorldHelper.isAdjacentToCrop((World) iba, x, y, z)
                        ? 12
                        : 10;
                case GLOWROOT:
                    return 6;
        ...
        public boolean canGenerateIn(BiomeGenBase b) {
            switch (this) {
                ...
                case LUMALILY:
                    return ReikaBiomeHelper.isSnowBiome(b)
                        && !(b instanceof BiomeGenHills) && b.topBlock == Blocks.grass
                        && !SFAPI.biomeHandler.isPinkForest(b);
                case SANOBLOOM:
                    return BiomeDictionary.isBiomeOfType(b, Type.JUNGLE);
                case VOIDREED:
                    return b.biomeID == ThaumIDHandler.Biomes.EERIE.getID()
                        || b instanceof BiomeGenSwamp
                        || b.getClass().getName().contains("BiomeGenBOPSwamp")
                        || b.getClass().getName().contains("BiomeGenLushSwamp");
                case FLOWIVY:
                    return b instanceof BiomeGenHills
                        || (b.rootHeight >= 1 && b.topBlock == Blocks.grass
                            && !BiomeGlowingCliffs.isGlowingCliffs(b
                            ) /* && ReikaBiomeHelper.getBiomeTemp(world, b) < 40*/);
                case GLOWDAISY:
                case GLOWROOT:
                ...
                case GLOWROOT:
                    if (rand.nextInt(4) == 0) {
                        if (world.checkChunksExist(
                                x - 16, y, z - 16, x + 16, y, z + 16
                            )) {
                            int meta = ((ItemFertilitySeed
                                        ) ChromaItems.FERTILITYSEED.getItemInstance())
                                           .getRandomAmbientDropMeta(rand);
                            ReikaItemHelper.dropItem(
                                world,
                                x + 0.5,
                                y + 0.5,
                                z + 0.5,
                                ChromaItems.FERTILITYSEED.getStackOfMetadata(meta)
                            );
                        }
                        break;
                    }
        ...
        public Coordinate grow(World world, int x, int y, int z, Block b, Random rand) {
            if (ModList.SATISFORESTRY.isLoaded()
                && SFAPI.biomeHandler.isPinkForest(world.getBiomeGenForCoords(x, z)))
                return null;
            ...
            switch (this) {
                ...
                case SANOBLOOM: {
                    ...
                    int ry = y;
                    int oy = y;
                    while (world.getBlock(rx, ry - 1, rz).isAir(world, rx, ry - 1, rz)
                           && ry > 0)
                        ry--;
                    if (Math.abs(oy - ry) <= 12
                        && world.getBlock(rx, ry, rz).isAir(world, rx, ry, rz)
                        && this.canPlantAt(world, rx, ry, rz)) {
                ...
                case GLOWDAISY: {
                    ...
                    if (rand.nextInt(n) == 0) {
                        ForgeDirection dir
                            = ForgeDirection.VALID_DIRECTIONS[2 + rand.nextInt(4)];
                        int dx = x + dir.offsetX;
                        int dz = z + dir.offsetZ;
                        Block idb = world.getBlock(dx, y - 1, dz);
                        if (world.getBlock(dx, y, dz).isAir(world, dx, y, dz)
                            && (idb == Blocks.grass || idb == Blocks.dirt)) {
                            if (this == GLOWDAISY
                                && world.getSavedLightValue(EnumSkyBlock.Sky, dx, y, dz)
                                    > 8)
                                return null;
                            //ReikaJavaLibrary.pConsole("Spreading "+this+" to "+new
                            //Coordinate(dx, y, dz));
                ...
                case FLOWIVY: {
                    if (rand.nextInt(3) == 0) {
                        ForgeDirection dir
                            = ForgeDirection.VALID_DIRECTIONS[rand.nextInt(6)];
                        ...
                        if (world.getBlock(dx, dy, dz).isAir(world, dx, dy, dz)
                            && this.canPlantAt(world, dx, dy, dz)) {
        ...
        public boolean canPlantAt(World world, int x, int y, int z) {
            //ReikaJavaLibrary.pConsole("Testing plantability of "+this+" at "+new
            //Coordinate(x, y, z));
            switch (this) {
                case FLOWIVY: {
                    for (int i = 2; i < 6; i++) {
                    ...
                    //return this.matchAt(world, x, y+1, z) ||
                    //(ReikaPlantHelper.VINES.canPlantAt(world, x, y, z)/* &&
                    //world.getBlock(x, y-1, z).isAir(world, x, y-1,
                    //z)*/);//world.getBlock(x, y+1, z).getMaterial().isSolid();
                    return false;
                }
                case VOIDREED:
                    return this.matchAt(world, x, y - 1, z)
                        || this.isChromaPool(world, x, y, z)
                        || ReikaPlantHelper.SUGARCANE.canPlantAt(world, x, y, z);
                case ENDERFLOWER:
                case LUMALILY:
                case RESOCLOVER:
                case GLOWDAISY: {
                ...
                case SANOBLOOM:
                    return world.getBlock(x, y - 1, z) == Blocks.leaves
                        && world.getBlockMetadata(x, y - 1, z) % 4 == 3;
                case GLOWROOT: {
                    return (this.matchAt(world, x, y + 1, z)
                            || ReikaBlockHelper.isNaturalStone(world, x, y + 1, z)
                            || ReikaBlockHelper.isDirtType(
                                world.getBlock(x, y + 1, z),
                                world.getBlockMetadata(x, y + 1, z)
                            ))
                        && !this.isRootTooLong(world, x, y, z);
                }
            }
            return false;
        }
        ...
        private boolean isChromaPool(World world, int x, int y, int z) {
            ...
            if (idbelow != Blocks.sand
                && !ReikaBlockHelper.isDirtType(idbelow, metabelow))
                return false;
            ForgeDirection liq = ReikaWorldHelper.checkForAdjBlock(
                world, x, y - 1, z, ChromaBlocks.CHROMA.getBlockInstance(), 0
            );
        ...
        private boolean onActiveGrass(World world, int x, int y, int z) {
            ...
            return b instanceof IFluidBlock
                && ((IFluidBlock) b).getFluid() == FluidRegistry.getFluid("ender");
        }

        private boolean isValidGrass(World world, int x, int y, int z) {
            Block b = world.getBlock(x, y, z);
            int meta = world.getBlockMetadata(x, y, z);
            return (b == Blocks.grass && meta == 0)
                || (ReikaTreeHelper.JUNGLE.isTreeLeaf(b, meta)
                    && ReikaTreeHelper.isNaturalLeaf(world, x, y, z));
        }

        private boolean matchAt(World world, int x, int y, int z) {
            return world.getBlock(x, y, z) == ChromaBlocks.DECOFLOWER.getBlockInstance()
                && world.getBlockMetadata(x, y, z) == this.ordinal();
        }
        ...
        public void doParticles(World world, int x, int y, int z, Random r) {
            switch (this) {
                case ENDERFLOWER: {
                    ...
                    EntityBlurFX fx1 = new EntityCCBlurFX(world, dx, dy, dz)
                                           .setLife(l)
                                           .setScale(s * f)
                                           .setColor(c1);
                    EntityBlurFX fx2 = new EntityCCBlurFX(world, dx, dy, dz)
                                           .setLife(l)
                                           .setScale(s)
                                           .setColor(c2);
                ...
                case FLOWIVY: {
                    boolean flag = (x + y + z) % 6 == 0
                        && Minecraft.getMinecraft().theWorld.rand.nextBoolean();
                    if (!flag) {
                    ...
                    if (flag) {
                        ...
                        int c
                            = ReikaColorAPI.mixColors(0x00ff00, 0xa0ffa0, r.nextFloat());
                        EntityFloatingSeedsFX fx
                            = (EntityFloatingSeedsFX) new EntityCCFloatingSeedsFX(
                                  world, dx, dy, dz, 0, -90, ChromaIcons.CENTER
                            )
                                  .setLife(l)
                                  .setScale(s)
                                  .setColor(c);
                ...
                case LUMALILY: {
                    ...
                    int c = ReikaColorAPI.mixColors(
                        ...
                        0xffffff,
                        0.5F
                            + (float) (0.5 * Math.sin(System.currentTimeMillis() / 1000D))
                    );
                    EntityFloatingSeedsFX fx
                        = (EntityFloatingSeedsFX) new EntityCCFloatingSeedsFX(
                              world, dx, dy, dz, 0, 90, ChromaIcons.CENTER
                ...
                case RESOCLOVER: {
                    ...
                    for (int i = 0; i < n; i++) {
                        ...
                        float s = 0.75F;
                        int c
                            = ReikaColorAPI.mixColors(0x0000ff, 0x00ff00, r.nextFloat());
                        EntityBlurFX fx = new EntityCCBlurFX(world, dx, dy, dz)
                                              .setIcon(ChromaIcons.BIGFLARE)
                                              .setLife(l)
                                              .setScale(s)
                                              .setColor(c)
                                              .setRapidExpand();
                ...
                case SANOBLOOM: {
                    ...
                    double[] v = ReikaPhysicsHelper.polarToCartesian(
                        0.0625, r.nextDouble() * 70, r.nextDouble() * 360
                    );
                    EntityBlurFX fx
                        = new EntityCCBlurFX(world, dx, dy, dz, v[0], v[1], v[2])
                              .setLife(l)
                              .setScale(s)
                              .setColor(0xff0000)
                              .setRapidExpand()
                ...
                case VOIDREED: {
                    float g
                        = -(float) ReikaRandomHelper.getRandomPlusMinus(0.0625, 0.03125);
                    ...
                    EntityFX fx1
                        = new EntityCCFloatingSeedsFX(world, dx, dy, dz, 0, 90, ico)
                              .setScale(s)
                              .setLife(l)
                              .setColor(c1)
                              .setGravity(g)
                ...
                case GLOWDAISY: {
                    ...
                    double dy = y + 0.125;
                    int c = ReikaColorAPI.mixColors(
                        0x22aaff, 0x0000ff, world.rand.nextFloat()
                    );
        ...
        public void render(
            ...
            switch (this) {
                ...
                case RESOCLOVER: {
                    ...
                    ReikaRenderHelper.renderFlatInnerTextureOnSide(
                        world, x, y, z, ico, v5, rb, ForgeDirection.DOWN, d, false
                    );
                    break;
                }
                case FLOWIVY: {
                ...
                case ENDERFLOWER:
                    ReikaRenderHelper.renderCropTypeTex(
                        world, x, y, z, ico, v5, rb, 0.1875, 1
                    );
                    //top of flower ReikaRenderHelper.renderFlatInnerTextureOnSide(world,
                    //x, y, z, ico, v5, rb, ForgeDirection.DOWN, 0.75, false);
                    break;
                case GLOWDAISY:
                    ReikaRenderHelper.renderCropTypeTex(
                        world, x, y, z, ico, v5, rb, 0.25, 1
                    );
                    break;
                case GLOWROOT:
                    ReikaRenderHelper.renderCropTypeTex(
