...
public class BlockWarpNode extends BlockContainer {
        ...
        if (e instanceof EntityPlayerMP) {
            ...
            if (world.getTotalWorldTime() - last > 200) {
                ...
                if (vx != 0 || vz != 0) {
                    ...
                    WorldLocation loc = WarpNetwork.instance.getLink(
                        new WorldLocation(world, x, y, z), ang, ANGLE_TOLERANCE
                    );
                    if (loc != null) {
                        this.teleportPlayer(ep, loc);
                        ep.getEntityData().setLong(
                            "lastWarpNode", world.getTotalWorldTime()
                        );
                    }
                }
            }
    ...
    private void teleportPlayer(EntityPlayerMP ep, WorldLocation loc) {
        ...
        if (!ep.capabilities.isCreativeMode && !ProgressStage.LINK.isPlayerAtStage(ep)
            && ep.getRNG().nextInt(
                   ProgressStage.USEENERGY.isPlayerAtStage(ep)       ? 2
                       : ProgressStage.ALLCOLORS.isPlayerAtStage(ep) ? 3
                                                                     : 6
               ) > 0) {
