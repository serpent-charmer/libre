...
public class BlockCaveIndicator extends Block {
    ...
    private void trigger(World world, int x, int y, int z) {
        if (world.isRemote)
            return;
        float f = (float) MusicKey.C5
                      .getInterval(
                          KeySignature.C.getScale().get(world.rand.nextInt(7)).ordinal()
                      )
                      .getRatio(MusicKey.C5);
        ((ChromaSound) ChromaSounds.DING.getUpshiftedPitch())
            .playSoundAtBlock(world, x, y, z, 0.5F, f);
