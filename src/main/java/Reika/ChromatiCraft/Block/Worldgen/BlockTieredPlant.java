...
public final class BlockTieredPlant
    extends BlockChromaTiered implements IPlantable, LoadRegistry {
    ...
    public static enum TieredPlants {
            ...
            switch (this) {
                case BULB:
                    ...
                    for (int i = 0; i < 32; i++) {
                        ...
                        if (world.getBlock(x, dy, z) == Blocks.air
                            && ReikaBlockHelper.isLeaf(b, meta)) {
                ...
                case CAVE:
                    for (int i = 0; i < 64; i++) {
                        ...
                        Block b = world.getBlock(x, dy + 1, z);
                        int meta = world.getBlockMetadata(x, dy + 1, z);
                        boolean flag = b == Blocks.stone
                            || b.isReplaceableOreGen(world, x, dy + 1, z, Blocks.stone)
                ...
                case FLOWER:
                    ...
                    for (int i = -8; i < 8; i++) {
                        int dy = y + i;
                        boolean ground = world.getBlock(x, dy, z) == Blocks.dirt
                            || world.getBlock(x, dy, z) == Blocks.grass;
                ...
                case LILY:
                    ...
                    for (int i = -8; i < 38; i++) {
                        int dy = y + i;
                        boolean water = world.getBlock(x, dy, z) == Blocks.water
                            && world.getBlockMetadata(x, dy, z) == 0;
                        if (water && world.getBlock(x, dy + 1, z) == Blocks.air
                            && world.canBlockSeeTheSky(x, dy + 1, z)) {
                            return new Coordinate(x, dy + 1, z);
                        }
                    }
                    break;
                ...
                case POD: {
                    ...
                    Coordinate c = ReikaWorldHelper.findTreeNear(
                        world, x, world.getTopSolidOrLiquidBlock(x, z), z, 7
                    );
                    if (c != null) {
                        int ymin = c.yCoord;
                        int ymax = c.yCoord;
                        Coordinate c2 = c;
                        while (ReikaBlockHelper.isWood(
                            c2.getBlock(world), c2.getBlockMetadata(world)
                        )) {
                            c2 = c2.offset(0, -1, 0);
                            ymin--;
                        }
        ...
        public ItemStack getDrop() {
            switch (this) {
                case FLOWER:
                    return ChromaStacks.auraDust.copy();
                case CAVE:
                    return ChromaStacks.purityDust.copy();
                case LILY:
                    return ChromaStacks.elementDust.copy();
                case BULB:
                    return ChromaStacks.resonanceDust.copy();
                case DESERT:
                    return ChromaStacks.beaconDust.copy();
                case POD:
                    return ChromaStacks.glowbeans.copy();
                case ROOT:
                    return ChromaStacks.boostroot.copy();
            }
            return null;
        }
    }
    ...
    public final int getLightValue(IBlockAccess iba, int x, int y, int z) {
        ...
        return ModList.COLORLIGHT.isLoaded() ? ReikaColorAPI.getPackedIntForColoredLight(
                   TieredPlants.list[iba.getBlockMetadata(x, y, z)].color, l
               )
                                             : l;
    }
    ...
    @Override
    public Collection<ItemStack> getHarvestResources(
    ...
    getSelectedBoundingBoxFromPool(World world, int x, int y, int z) {
        if (this.isPlayerSufficientTier(
        ...
        if (world.getTotalWorldTime() % 2 == 0
            ...
            switch (meta) {
                case 0: {
                    ...
                    int g = r.nextInt(255);
                    EntityBlurFX fx
                        = new EntityCCBlurFX(world, x + 0.5, y + 0.95, z + 0.5, vx, 0, vz)
                              .setGravity(0.015F)
                              .setColor(0, g, 255)
                              .setScale(2);
                ...
                case 1: {
                    ...
                    EntityBlurFX fx
                        = new EntityCCBlurFX(world, x + 0.5, y + 0.65, z + 0.5, 0, 0, 0)
                              .setGravity(g)
                              .setScale(2);
                    Minecraft.getMinecraft().effectRenderer.addEffect(fx);
                    break;
                ...
                case 2: {
                    ...
                    EntityBlurFX fx
                        = new EntityCCBlurFX(world, x + 0.5, y + 0.375, z + 0.5, 0, 0, 0)
                              .setColor(red, 0, 255)
                              .setGravity(-g)
                              .setScale(4);
                    Minecraft.getMinecraft().effectRenderer.addEffect(fx);
                ...
                case 3: {
                    ...
                    EntityBlurFX fx = new EntityCCBlurFX(world, px, y + 0.75, pz, 0, 0, 0)
                                          .setColor(0, 255, gr)
                                          .setGravity(g)
                                          .setScale(2);
                    Minecraft.getMinecraft().effectRenderer.addEffect(fx);
    ...
    public IIcon getIcon(IBlockAccess world, int x, int y, int z, int s) {
        return this.isPlayerSufficientTier(world, x, y, z,
    Minecraft.getMinecraft().thePlayer) ? this.getIcon(s, world.getBlockMetadata(x, y, z))
    : ChromaIcons.TRANSPARENT.getIcon();
    }*/
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < ARR_LENGTH; i++) {
            front_icons[i]
                = ico.registerIcon("chromaticraft:plant/tierplant_" + i + "_front");
            back_icons[i]
                = ico.registerIcon("chromaticraft:plant/tierplant_" + i + "_back");
        }
    }
    ...
    protected boolean canPlaceBlockOn(World world, int x, int y, int z, TieredPlants p) {
        ...
        switch (p) {
            ...
            case CAVE:
                return b == Blocks.stone || b == Blocks.bedrock
                    || ReikaBlockHelper.isOre(b, meta)
    ...
    private boolean isValidLocation(World world, int x, int y, int z, int meta) {
        ...
        switch (p) {
            ...
            case POD:
                return this.canPlaceBlockOn(world, x, y + 1, z, p)
                    || this.canPlaceBlockOn(world, x + 1, y, z, p)
                    || this.canPlaceBlockOn(world, x - 1, y, z, p)
                    || this.canPlaceBlockOn(world, x, y, z + 1, p)
                    || this.canPlaceBlockOn(world, x, y, z - 1, p);
        }
        return false;
    }
