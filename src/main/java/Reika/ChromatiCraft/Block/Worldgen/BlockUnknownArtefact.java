...
public class BlockUnknownArtefact extends Block {
    ...
    private void onInteracted(World world, int x, int y, int z, EntityPlayer ep) {
        if (world.rand.nextInt(8) == 0 && !world.isRemote) {
            ChromaSounds.MONUMENTRAY.playSoundNoAttenuation(
                world, x + 0.5, y + 0.5, z + 0.5, 1, 0.625F, 64
            );
            //ChromaSounds.MONUMENTRAY.playSoundAtBlockNoAttenuation(world, x+0.5, y+0.5,
            //z+0.5, 1, 0.5F, 64);
            double[] angs = ReikaPhysicsHelper.cartesianToPolar(
                ep.posX - x - 0.5, ep.posY - y - 0.5, ep.posZ - z - 0.5
            );
            double[] v
                = ReikaPhysicsHelper.polarToCartesian(6, angs[1] - 90, -angs[2] - 90);
            ep.motionX += v[0];
            ep.motionZ += v[2];
            ep.motionY = MathHelper.clamp_double(v[1], 1.25, 1.75);
            //ChromaSounds.DISCHARGE.playSoundAtBlockNoAttenuation(world,
            //ep.posX+ep.motionX*4, ep.posY+ep.motionY*4, ep.posZ+ep.motionZ*4, 1, 0.5F,
            //64);
            ep.velocityChanged = true;
            ReikaPacketHelper.sendDataPacketWithRadius(
                ChromatiCraft.packetChannel,
                ChromaPackets.ARTEFACTCLICK.ordinal(),
                world,
                x,
                y,
                z,
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        if (d < 576) {
            ItemUnknownArtefact.doUA_FX(
                world, x + 0.5, y + 0.5 + 1.5 * rand.nextDouble(), z + 0.5, false
            );
        }
    }
            ...
            for (int i = 0; i < n; i++) {
                ReikaItemHelper.dropItem(
                    ...
                    ChromaItems.ARTEFACT.getStackOfMetadata(
                        ArtefactTypes.FRAGMENT.ordinal()
                    )
                );
            }
        }
