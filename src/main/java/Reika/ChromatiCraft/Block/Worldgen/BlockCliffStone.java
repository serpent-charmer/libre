...
public class BlockCliffStone extends Block implements RockProofStone, IWrappableBlock {
    ...
    private static final BreakerCallback cliffStoneTransparifier = new BreakerCallback() {
        public void onPostBreak(
            ProgressiveBreaker b, World world, int x, int y, int z, Block id, int meta
        ) {
        ...
        public boolean canBreak(
            ...
            return id == ChromaBlocks.CLIFFSTONE.getBlockInstance()
                && meta == Variants.STONE.getMeta(false, false);
        }

        @Override
        public void onPreBreak(
            ProgressiveBreaker b, World world, int x, int y, int z, Block id, int meta
        ) {}
    ...
    getPlayerRelativeBlockHardness(EntityPlayer ep, World world, int x, int y, int z) {
        return Variants.getVariant(world.getBlockMetadata(x, y, z))
            .proxy.getPlayerRelativeBlockHardness(ep, world, x, y, z);
    }
    ...
    public float getExplosionResistance(
        ...
        return Variants.getVariant(world.getBlockMetadata(x, y, z))
            .proxy.getExplosionResistance(e, world, x, y, z, ex, ey, ez);
    }

    /*
    @Override
    public ArrayList<ItemStack> getDrops(World world, int x, int y, int z, int meta, int
    fortune) { return Variants.getVariant(meta).proxy.getDrops(world, x, y, z, 0,
    fortune);
    }
     */
    ...
    public static boolean transparify(World world, int x, int y, int z, EntityPlayer ep) {
        ...
        if (id == ChromaBlocks.CLIFFSTONE.getBlockInstance()
            && meta == Variants.STONE.getMeta(false, false)) {
            int depth = 30;
            ProgressiveBreaker br
    ...
    public IIcon getIcon(int s, int meta) {
        ...
        switch (v) {
            ...
            case GRASS:
                return s == 0 ? Variants.DIRT.baseTexture
                    : s == 1  ? grassTop
                              : v.baseTexture;
            case STONE:
                return v.baseTexture;
    ...
    public IIcon getIcon(IBlockAccess iba, int x, int y, int z, int s) {
        ...
        if (v == Variants.DIRT) {
            if (s == 0) {
                ...
                if (iba.getBlockMetadata(x, y - 1, z) != meta
        ...
        if (v == Variants.STONE && s > 1) {
            ...
            if (b == Blocks.stone || ReikaBlockHelper.isOre(b, iba.getBlockMetadata(x,
        y+1, z))) { return stoneBlendUp;
            }
            b = iba.getBlock(x, y-1, z);
            if (b == Blocks.stone || ReikaBlockHelper.isOre(b, iba.getBlockMetadata(x,
        y-1, z))) return stoneBlendDown;
        }
         */
    ...
    public void registerBlockIcons(IIconRegister ico) {
        ...
        stoneBlendUp_Clay
            = ico.registerIcon("chromaticraft:cliffstone/stone_vblend_up_clay");
        stoneBlendUp_Sand
            = ico.registerIcon("chromaticraft:cliffstone/stone_vblend_up_sand");
    ...
    public boolean isReplaceableOreGen(World world, int x, int y, int z, Block b) {
        return b == this
            || b == Variants.getVariant(world.getBlockMetadata(x, y, z)).proxy;
    }
    ...
    @Override
    public boolean
    blockRockGeneration(World world, int x, int y, int z, Block b, int meta) {
        return true;
    }
    ...
    @SideOnly(Side.CLIENT)
    public int
    getColorMultiplier(IBlockAccess iba, int x, int y, int z, Block b, int meta) {
    ...
    @SideOnly(Side.CLIENT)
    public IIcon
    getIcon(IBlockAccess iba, int x, int y, int z, int side, Block b, int meta) {
    ...
    public int getFlammability(
        IBlockAccess iba, int x, int y, int z, ForgeDirection side, Block b, int meta
    ) {
        return 0;
    }
    ...
    @Override
    public boolean
    canEntityDestroy(IBlockAccess iba, int x, int y, int z, Entity e, Block b, int meta) {
    ...
    public static enum Variants {
        ...
        private void registerIcons(IIconRegister ico) {
            ...
            for (int i = 0; i < 9; i++) {
                blend[i] = ico.registerIcon(
                    "chromaticraft:cliffstone/" + s + "_blend_down"
                ); //ico.registerIcon("chromaticraft:cliffstone/"+s+"_blend_"+i);
            }
        }
        ...
        public boolean isGround() {
            return proxy.getMaterial() == Material.ground
                || proxy.getMaterial() == Material.grass || proxy == Blocks.gravel
                || proxy == Blocks.clay || proxy == Blocks.sand;
        }
