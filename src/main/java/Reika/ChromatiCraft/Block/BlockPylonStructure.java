...
public class BlockPylonStructure
    extends BlockProtectedByStructure implements ConnectedTextureGlass, CraftableStone {
    ...
    public static enum StoneTypes {
        ...
        public BlockKey getBlock() {
            return new BlockKey(
                ChromaBlocks.PYLONSTRUCT.getBlockInstance(), this.ordinal()
            );
        }
        ...
        public boolean glows() {
            return this == GLOWCOL || this == GLOWBEAM || this == FOCUS
                || this == RESORING || this == STABILIZER || this == MULTICHROMIC;
        }
    ...
    private int getIconIndex(IBlockAccess iba, int x, int y, int z, int s, int meta) {
        ...
        if (m == StoneTypes.RESORING) {
            if (s <= 1) {
                if (iba.getBlock(x + 1, y, z) == this
                    && iba.getBlockMetadata(x + 1, y, z) == meta)
                    return 0;
                if (iba.getBlock(x - 1, y, z) == this
                    && iba.getBlockMetadata(x - 1, y, z) == meta)
                    return 0;
                if (iba.getBlock(x, y, z + 1) == this
                    && iba.getBlockMetadata(x, y, z + 1) == meta)
                    return 1;
                if (iba.getBlock(x, y, z - 1) == this
                    && iba.getBlockMetadata(x, y, z - 1) == meta)
                    return 1;
            }
            if (iba.getBlock(x, y + 1, z) == this
                && iba.getBlockMetadata(x, y + 1, z) == meta) {
                return 1;
            }
            if (iba.getBlock(x, y - 1, z) == this
                && iba.getBlockMetadata(x, y - 1, z) == meta) {
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (iba.getBlock(x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ)
                    == this) {
                    if (iba.getBlockMetadata(
        ...
        if (m.isBeam() && s <= 1) {
            if (iba.getBlock(x + 1, y, z) == this
                && this.getWrappedMeta(s, iba.getBlockMetadata(x + 1, y, z)) == meta)
                return 2;
            if (iba.getBlock(x - 1, y, z) == this
                && this.getWrappedMeta(s, iba.getBlockMetadata(x - 1, y, z)) == meta)
                return 2;
            ...
            if (iba.getBlock(x, y, z + 1) == this
                && this.getWrappedMeta(s, iba.getBlockMetadata(x, y, z + 1)) == meta)
                return 1;
            if (iba.getBlock(x, y, z - 1) == this
                && this.getWrappedMeta(s, iba.getBlockMetadata(x, y, z - 1)) == meta)
                return 1;
    ...
    public IIcon getBrightOverlay(IBlockAccess iba, int x, int y, int z, int s) {
        ...
        switch (StoneTypes.list[meta]) {
            ...
            case STABILIZER:
                return icons[meta]
                            [variants[meta] / 2
                             + Math.max(0, this.getIconIndex(iba, x, y, z, s, meta))];
            default:
                return null;
        }
    }
    ...
    public IIcon getBrightOverlay(int meta, int s) {
        switch (StoneTypes.list[meta]) {
            ...
            case STABILIZER:
                return icons[meta]
                            [variants[meta] / 2
                             + Math.max(0, this.getIconIndex(s, meta))];
            default:
                return null;
        }
    }
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < 16; i++) {
            for (int k = 0; k < variants[i]; k++) {
                String suff
                    = k > 0 ? String.valueOf(i + "-" + (k + 1)) : String.valueOf(i);
    ...
    @Override
    public void
    onBlockDestroyedByExplosion(World world, int x, int y, int z, Explosion e) {
        int r = 12;
        for (int i = -r; i <= r; i++) {
            for (int j = -r; j <= r; j++) {
                for (int k = -r; k <= r; k++) {
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        if (SpecialDayTracker.instance.loadXmasTextures()
            && StoneTypes.list[world.getBlockMetadata(x, y, z)].isColumn()) {
            double o = 0.0625;
            ...
            EntityCCBlurFX fx
                = new EntityCCBlurFX(world, x + px, y + rand.nextDouble(), z + pz);
            int rgb = ReikaColorAPI.RGBtoHex(
                rand.nextBoolean() ? 255 : 0,
                rand.nextBoolean() ? 255 : 0,
                rand.nextBoolean() ? 255 : 0
            );
    ...
    void triggerBreakCheck(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            blocks.recursiveAddWithBounds(
                world, dx, dy, dz, this, x - 16, y - 32, z - 16, x + 16, y + 32, z + 16
            );
        }
        ...
        for (int h = 9; h < 16; h++) {
            ...
            if (te instanceof TileEntityCrystalPylon) {
                if (!ChromaStructures.PYLON
                         .getArray(
                             world,
                             mx,
                             my + h,
                             mz,
                             ((TileEntityCrystalPylon) te).getColor()
                         )
                         .matchInWorld()) {
                    ((TileEntityCrystalPylon) te).invalidateMultiblock();
                    break;
        ...
        for (int k = 0; k < 6; k++) {
            ...
            for (int i = 1; i <= 5; i++) {
                TileEntity te = world.getTileEntity(
                    x + dir.offsetX * i, y + dir.offsetY * i, z + dir.offsetZ * i
                );
                if (te instanceof TileEntityCrystalRepeater) {
                    ((TileEntityCrystalRepeater) te).validateStructure();
                }
        ...
        TileEntity te = world.getTileEntity(
            blocks.getMidX() - 1, blocks.getMaxY() + 1, blocks.getMidZ()
        );
    ...
    public static void triggerAddCheck(World world, int x, int y, int z) {
        ...
        for (int h = 9; h < 16; h++) {
            TileEntity te = world.getTileEntity(mx, my + h, mz);
            if (te instanceof TileEntityCrystalPylon) {
                FilledBlockArray f = ChromaStructures.PYLON.getArray(
                    world, mx, my + h, mz, ((TileEntityCrystalPylon) te).getColor()
                );
                if (f.matchInWorld()) {
    ...
    getEdgesForFace(IBlockAccess world, int x, int y, int z, ForgeDirection face) {
        ...
        if (face.offsetX != 0) { //test YZ
            ...
            if (this.connectTo(world, x, y + 1, z + 1, meta) && !li.contains(4)
                && !li.contains(2))
                li.remove(1);
            if (this.connectTo(world, x, y - 1, z - 1, meta) && !li.contains(6)
                && !li.contains(8))
                li.remove(9);
            if (this.connectTo(world, x, y + 1, z - 1, meta) && !li.contains(4)
                && !li.contains(8))
                li.remove(7);
            if (this.connectTo(world, x, y - 1, z + 1, meta) && !li.contains(2)
                && !li.contains(6))
                li.remove(3);
        }
        if (face.offsetY != 0) { //test XZ
            ...
            //Corners; only removed if have adjacent on side AND corner
            if (this.connectTo(world, x + 1, y, z + 1, meta) && !li.contains(4)
                && !li.contains(2))
                li.remove(1);
            if (this.connectTo(world, x - 1, y, z - 1, meta) && !li.contains(6)
                && !li.contains(8))
                li.remove(9);
            if (this.connectTo(world, x + 1, y, z - 1, meta) && !li.contains(4)
                && !li.contains(8))
                li.remove(7);
            if (this.connectTo(world, x - 1, y, z + 1, meta) && !li.contains(2)
                && !li.contains(6))
                li.remove(3);
        }
        if (face.offsetZ != 0) { //test XY
            ...
            //Corners; only removed if have adjacent on side AND corner
            if (this.connectTo(world, x + 1, y + 1, z, meta) && !li.contains(2)
                && !li.contains(4))
                li.remove(1);
            if (this.connectTo(world, x - 1, y - 1, z, meta) && !li.contains(8)
                && !li.contains(6))
                li.remove(9);
            if (this.connectTo(world, x + 1, y - 1, z, meta) && !li.contains(2)
                && !li.contains(6))
                li.remove(3);
            if (this.connectTo(world, x - 1, y + 1, z, meta) && !li.contains(4)
                && !li.contains(8))
                li.remove(7);
        }
        return li;
