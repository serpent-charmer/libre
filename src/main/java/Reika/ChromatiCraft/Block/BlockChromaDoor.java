...
public class BlockChromaDoor
    extends BlockContainer implements SemiUnbreakable, IWailaDataProvider {
    ...
    public int getLightValue(IBlockAccess iba, int x, int y, int z) {
        return (iba instanceof World
                && ((World) iba).provider.dimensionId == ExtraChromaIDs.DIMID.getValue())
            ? 0
            : 12;
    }
    ...
    getPlayerRelativeBlockHardness(EntityPlayer ep, World world, int x, int y, int z) {
        ...
        return te.isOwner(ep) ? super.getPlayerRelativeBlockHardness(ep, world, x, y, z)
                              : -1;
    }
    ...
    public static void
    setOpen(World world, int x, int y, int z, boolean open, int delay) {
    ...
    public static int
    getMetadata(boolean open, boolean damage, boolean oneuse, boolean stay) {
        return (open ? 1 : 0) | (damage ? 2 : 0) | (oneuse ? 4 : 0) | (stay ? 8 : 0);
    }

    @Override
    public void onNeighborBlockChange(World world, int x, int y, int z, Block b) {
        //if (this.isOpen(world, x, y, z) && world.isBlockIndirectlyGettingPowered(x, y,
        //z))
    ...
    private boolean
    connectToBlock(IBlockAccess world, int x, int y, int z, ForgeDirection s) {
        Block b = world.getBlock(x, y, z);
        if (b == this)
            return true;
        if (b == ChromaBlocks.PYLON.getBlockInstance()
    ...
    onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase e, ItemStack is) {
        if (e instanceof EntityPlayer)
            ((TileEntityChromaDoor) world.getTileEntity(x, y, z))
                .setPlacer((EntityPlayer) e);
    }
    ...
    getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        this.setBlockBoundsBasedOnState(world, x, y, z);
        return this.isOpen(world, x, y, z)
            ? null
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity e) {
        if (!this.isOpen(world, x, y, z)) {
            ...
            if (this.dealDamage(world, x, y, z)) {
    ...
    public boolean shouldSideBeRendered(IBlockAccess iba, int x, int y, int z, int s) {
        ...
        BlockBounds box2 = BlockBounds.fromBlock(
            this, iba, x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ
        );
    ...
    public static class TileEntityChromaDoor extends TileEntity implements SneakPop {
        ...
        public void updateEntity() {
            boolean flag = false;
            if (!worldObj.isRemote && autoOpen
                && !isOpen(worldObj, xCoord, yCoord, zCoord)) {
                if (worldObj.getBlock(xCoord, yCoord - 1, zCoord) != this.getBlockType()
                    ...
                    if (ep != null) {
                        ...
                        if (Math.abs(ep.posY - yCoord) < 1 && d < 9) {
                            if (d < 2
                                || ReikaEntityHelper.isLookingAt(
                                    ep, xCoord + 0.5, yCoord - 1.5, zCoord + 0.5
                                )
                                || ReikaEntityHelper.isLookingAt(
                                    ep, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5
                                )) {
                                lastAutoOpenDuration
                                    = Math.min(lastAutoOpenDuration + 10, 200);
        ...
        public void open(int delay) {
            ...
            if (delay > 0 && !BlockChromaDoor.stayOpen(worldObj, xCoord, yCoord, zCoord))
                worldObj.scheduleBlockUpdate(
                    xCoord, yCoord, zCoord, this.getBlockType(), delay
                );
        }
        ...
        private void setStates(boolean open) {
            for (Coordinate c : this.getDoorBlocks()) {
                if (matchUIDs(this, (TileEntityChromaDoor) c.getTileEntity(worldObj)))
                    c.setBlockMetadata(
                        worldObj, (c.getBlockMetadata(worldObj) & 0b1110) | (open ? 1 : 0)
                    );
            }
        }
        ...
        matchUIDs(TileEntityChromaDoor te1, TileEntityChromaDoor te2) {
        ...
        public void bindUUID(EntityPlayer ep, UUID id, int flags) {
            for (Coordinate c : this.getDoorBlocks()) {
                TileEntityChromaDoor te
                    = (TileEntityChromaDoor) c.getTileEntity(worldObj);
                if (te == null) {
