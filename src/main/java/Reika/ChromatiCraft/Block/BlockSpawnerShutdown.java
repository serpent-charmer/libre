...
public class BlockSpawnerShutdown extends BlockAttachableMini {
    ...
    protected void createFX(
        ...
        EntityFX fx = new EntityCenterBlurFX(world, dx, dy, dz, 0, 0, 0)
                          .setScale(2 + r.nextFloat() * 2)
                          .setColor(c);
        if (r.nextInt(7) == 0) {
            fx = new EntityLaserFX(CrystalElement.WHITE, world, dx, dy, dz, 0, 0, 0)
                     .setScale(2 + r.nextFloat() * 2)
                     .setColor(c);
        }
    ...
    public static class TileEntitySpawnerShutdown extends TileEntity {
        ...
        public void updateEntity() {
            if (worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord))
                return;
            ForgeDirection dir = ((BlockSpawnerShutdown) this.getBlockType())
