...
public class BlockDistortingEffect extends BlockContainer {
    ...
    public static boolean canReplace(World world, int x, int y, int z) {
        ...
        return b.isOpaqueCube() && b.getRenderType() == 0 && !b.hasTileEntity(meta)
            && !b.isAir(world, x, y, z)
            && !ReikaBlockHelper.isUnbreakable(world, x, y, z, b, meta, null);
    }

    public static void doReplace(World world, int x, int y, int z, boolean global) {
        BlockKey bk = BlockKey.getAt(world, x, y, z);
        //world.setBlock(x, y, z, ChromaBlocks.DISTORTING.getBlockInstance(), global ? 0 :
        //1, 3);
    ...
    public static class TileEntityDistorting extends TileEntity {
        ...
        public final void reset() {
            worldObj.setBlock(
                xCoord, yCoord, zCoord, previous.blockID, previous.metadata, 2
            );
        }
    ...
    private static class DistortedBox {
        ...
        public void render(Tessellator v5) {
            ...
            ReikaRenderHelper.renderBlockPieceNonCuboid(
                block.blockID, block.metadata, v5, renderBox
            );
            if (isGlobal) {
