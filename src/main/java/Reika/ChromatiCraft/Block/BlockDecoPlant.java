...
public class BlockDecoPlant extends BlockChromaTile implements IPlantable {
    ...
    public static boolean isEncased(World world, int x, int y, int z) {
        for (int i = 2; i < 6; i++) {
            ...
            if (world.getBlock(dx, dy, dz) != ChromaTiles.PLANTACCEL.getBlock()
                && !ReikaBlockHelper.isCollideable(world, dx, dy, dz))
                return false;
        }
        return true;
    }
    ...
    public boolean canBlockStay(World world, int x, int y, int z) {
        return ((TileEntityMagicPlant) world.getTileEntity(x, y, z))
            .isPlantable(world, x, y, z);
    }
    ...
    getDrops(World world, int x, int y, int z, int meta, int fortune) {
        ItemStack is
            = ChromaTiles.getTileFromIDandMetadata(this, meta).getCraftedProduct();
    ...
    public IIcon getBacking(IBlockAccess world, int x, int y, int z) {
        int meta = world.getBlockMetadata(x, y, z);
        if (ChromaTiles.getTile(world, x, y, z) == ChromaTiles.PLANTACCEL
            && this.isEncased(Minecraft.getMinecraft().theWorld, x, y, z)) {
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < 16; i++) {
            if (ChromaTiles.getTileFromIDandMetadata(this, i) != null) {
                front_icons[i]
                    = ico.registerIcon("chromaticraft:plant/decoplant_" + i + "_front");
                back_icons[i]
                    = ico.registerIcon("chromaticraft:plant/decoplant_" + i + "_back");
            }
        }
        ...
        switch (meta) {
            ...
            case 1:
                if (e instanceof EntityLivingBase)
                    ((EntityLivingBase) e)
                        .addPotionEffect(
                            new PotionEffect(ChromatiCraft.betterRegen.id, 20, 0)
                        );
                break;
            case 5:
                ...
                if (e instanceof EntityLivingBase)
                    ((EntityLivingBase) e)
                        .addPotionEffect(new PotionEffect(Potion.hunger.id, 20, 1));
                break;
            default:
                break;
        }
