...
public class BlockCrystalTile extends BlockChromaTile {
    ...
    public boolean canHarvest(World world, EntityPlayer player, int x, int y, int z) {
        ...
        return true; //is != null && is.getItem() instanceof ItemPickaxe &&
    ...
    harvestBlock(World world, EntityPlayer ep, int x, int y, int z, int meta) {
        ...
        if (c == ChromaTiles.PYLON) {
            ...
            if (!te.isUnHarvestable()) {
                /*
                if (te instanceof TileEntityWeakRepeater &&
                !((TileEntityWeakRepeater)te).hasRemainingLife()) return;
                 */
                if (this.isFullyHarvestable(world, x, y, z, meta, ep, c, te)) {
                    ...
                    if (c.hasNBTVariants()) {
                        if (is == null) {
                            throw new IllegalStateException(
                                "Block type " + c
                                + " returned null ItemStack when silk touched!"
                            );
                        }
                    ...
                    ReikaItemHelper.dropItems(
                        world, x + 0.5, y + 0.5, z + 0.5, this.getPieces(world, x, y, z)
                    );
                }
            }
        }
    ...
    protected ItemStack
    getHarvestedItemStack(World world, int x, int y, int z, int meta, ChromaTiles c) {
