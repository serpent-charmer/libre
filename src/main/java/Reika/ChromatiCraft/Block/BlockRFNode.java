...
public class BlockRFNode extends BlockAttachableMini {
    ...
    public static class TileEntityRFNode extends TileEntity implements IEnergyReceiver {
        ...
        public void updateEntity() {
            if (worldObj.isRemote) {
                ...
                if (world.getTotalWorldTime() % 8 == 0)
                    RFWeb.getWeb(world).addNode(
                        world,
                        xCoord,
                        yCoord,
                        zCoord,
                        ((BlockRFNode) this.getBlockType())
                            .getSide(world, xCoord, yCoord, zCoord)
                            .getOpposite()
                    );
            }
        }
        ...
        public int receiveEnergy(ForgeDirection from, int maxReceive, boolean simulate) {
            return RFWeb.getWeb(worldObj).tryDistributeEnergy(
                worldObj, xCoord, yCoord, zCoord, maxReceive
            );
        }
