...
public class BlockChromaTrail extends BlockContainer {
    ...
    public boolean onBlockActivated(
        ...
        if (HoldingChecks.MANIPULATOR.isHolding(ep)) {
            for (Coordinate loc :
                 ((TileChromaTrail) world.getTileEntity(x, y, z)).getFullPath()) {
    ...
    public static class TileChromaTrail extends TileEntity {
        ...
        private void doParticles(World world, int x, int y, int z) {
            ...
            if (t < 6) {
                ...
                int l = 13; //20;
                //int l2 = l*64;
                //SplineMotionController s = new SplineMotionController(l2,
                //overallPath).setTick(pathIndex*l2/l);
                ListOfPositionsController s
                    = new ListOfPositionsController(l, overallPath);
                float f = 1.5F * ((6 - t) / 6F);
                EntityBlurFX fx = new EntityCCBlurFX(world, x + 0.5, y + 0.5, z + 0.5)
                                      .setLife(l * 3 / 2)
                                      .setScale(f)
                                      .setColliding();
                fx.setAlphaFading()
                    .setRapidExpand()
                    .setPositionController(s)
                    .setColorController(BlockEtherealLight.colorController);
            ...
            if (HoldingChecks.MANIPULATOR.isClientHolding()) {
                if (world.rand.nextInt(2) == 0) {
                    ...
                    EntityBlurFX fx
                        = new EntityCCBlurFX(world, px, py, pz).setLife(l).setScale(f);
                    int c = ReikaColorAPI.mixColors(
                        0xffffff,
                        BlockEtherealLight.colorController.getColor(fx),
                        world.rand.nextFloat() * 0.375F
                    );
                    fx.setColor(c);
                    Minecraft.getMinecraft().effectRenderer.addEffect(fx);
