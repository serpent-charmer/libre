...
public class BlockLumenRelay extends BlockRelayBase implements IWailaDataProvider {
    ...
    public boolean canPlaceOn(World world, int x, int y, int z, int side) {
        return world.getBlock(x, y, z).isSideSolid(
            world, x, y, z, ForgeDirection.VALID_DIRECTIONS[side]
        );
    }
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block b) {
        ...
        if (!this.canPlaceOn(
                world, x - dir.offsetX, y - dir.offsetY, z - dir.offsetZ, meta
            )) {
            ReikaSoundHelper.playBreakSound(world, x, y, z, this);
            TileEntityLumenRelay te = (TileEntityLumenRelay) world.getTileEntity(x, y, z);
            ItemStack is = ChromaBlocks.RELAY.getStackOfMetadata(
                te.isMulti ? 16 : te.color.ordinal()
            );
    ...
    onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase e, ItemStack is) {
        ...
        te.color = te.isMulti ? CrystalElement.WHITE
                              : CrystalElement.elements[is.getItemDamage()];
        te.setInput(
            ForgeDirection.VALID_DIRECTIONS[world.getBlockMetadata(x, y, z)].getOpposite()
        );
    }
    ...
    public List<String> getWailaHead(
        ItemStack is, List<String> tip, IWailaDataAccessor acc, IWailaConfigHandler cfg
    ) {
        return tip;
    }
