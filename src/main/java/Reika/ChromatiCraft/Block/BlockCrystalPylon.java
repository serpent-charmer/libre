...
public class BlockCrystalPylon extends BlockCrystalTile
    implements ProgressionTrigger, SemiUnbreakable, ILaputaImmobile {
    ...
    public final int getLightValue(IBlockAccess iba, int x, int y, int z) {
        ...
        int color = te instanceof TileEntityCrystalPylon
            ? ((TileEntityCrystalPylon) te).getColor().getColor()
            : 0xffffff;
        color = te instanceof TileEntityDimensionCore
            ? ((TileEntityDimensionCore) te).getColor().getColor()
            : color;
        color = te instanceof TileEntityPersonalCharger
            ? ((TileEntityPersonalCharger) te).getColor().getColor()
            : color;
        int b = te instanceof TileEntityStructControl
            ? ((TileEntityStructControl) te).getBrightness()
            : 15;
        b = te instanceof TileEntitySkypeater ? 0 : b;
        return ModList.COLORLIGHT.isLoaded()
            ? ReikaColorAPI.getPackedIntForColoredLight(color, b)
            : b;
    }
    ...
    @Override
    protected boolean isFullyHarvestable(
        World world,
        int x,
        int y,
        int z,
    ...
    public void registerBlockIcons(IIconRegister ico) {
        ...
        burnedWeakRepeater
            = ico.registerIcon("chromaticraft:basic/weakrepeater_frame_burned");
    }

    @Override
    public IIcon getIcon(IBlockAccess iba, int x, int y, int z, int s) {
        int meta = iba.getBlockMetadata(x, y, z);
        if (ChromaTiles.getTileFromIDandMetadata(this, meta)
            == ChromaTiles.WEAKREPEATER) {
            TileEntityWeakRepeater te
                = (TileEntityWeakRepeater) iba.getTileEntity(x, y, z);
            if (te.isRuptured()) {
                return s == 0 ? weakRepeaterFrame
                    : s == 1  ? ChromaIcons.TRANSPARENT.getIcon()
                              : burnedWeakRepeater;
            }
        }
    ...
    public int colorMultiplier(IBlockAccess iba, int x, int y, int z) {
        ...
        return te instanceof TileEntityCrystalBroadcaster ?
    ((TileEntityCrystalBroadcaster)te).getActiveColor().getColor() : 0xffffff;
    }
     */
    @Override
    public ProgressStage[] getTriggers(
        EntityPlayer ep, World world, int x, int y, int z
    ) {
        ChromaTiles c = ChromaTiles.getTile(world, x, y, z);
        boolean pylon = c == ChromaTiles.PYLON
            && ((TileEntityCrystalPylon) world.getTileEntity(x, y, z)).canConduct();
    ...
    /*
    @Override
    public int getFlammability(IBlockAccess world, int x, int y, int z, ForgeDirection
    face) { return ChromaTiles.getTile(world, x, y, z) == ChromaTiles.WEAKREPEATER ?
    Blocks.planks.getFlammability(world, x, y, z, face) : 0;
    }

    @Override
    public boolean isFlammable(IBlockAccess world, int x, int y, int z, ForgeDirection
    face) { return this.getFlammability(world, x, y, z, face) > 0;
    }

    @Override
    public int getFireSpreadSpeed(IBlockAccess world, int x, int y, int z, ForgeDirection
    face) { return ChromaTiles.getTile(world, x, y, z) == ChromaTiles.WEAKREPEATER ?
    Blocks.planks.getFireSpreadSpeed(world, x, y, z, face) : 0;
    }
     */
    @Override
    public boolean isFireSource(World world, int x, int y, int z, ForgeDirection side) {
        return ChromaTiles.getTile(world, x, y, z) == ChromaTiles.WEAKREPEATER
    ...
    isSideSolid(IBlockAccess world, int x, int y, int z, ForgeDirection dir) {
        ...
        if (c == null) {
            ChromatiCraft.logger.logError(
                "Something requested isSideSolid with invalid positions:"
            );
