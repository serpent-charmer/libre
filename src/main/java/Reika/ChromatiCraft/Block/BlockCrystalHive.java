...
public class BlockCrystalHive extends Block implements MinerBlock {
    ...
    getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        ...
        if (bee != null) {
            ...
            double ch = bee == CrystalBees.getPureBee() ? 15 + 5 * fortune * fortune
                                                        : 10 + 5 * fortune;
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random r) {
        int meta = world.getBlockMetadata(x, y, z);
        ReikaParticleHelper p = meta == 0 ? ReikaParticleHelper.AMBIENTMOBSPELL
                                          : ReikaParticleHelper.ENCHANTMENT;
        int dy = meta == 0 ? y : y + 1;
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < 2; i++) {
            icons[0][i] = ico.registerIcon("chromaticraft:hives/crystal_top"
            ); //make crystal hive translucent?
        }
        for (int i = 2; i < 6; i++) {
