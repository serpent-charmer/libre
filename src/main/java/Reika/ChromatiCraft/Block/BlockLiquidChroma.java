...
public class BlockLiquidChroma extends BlockFluidClassic {
    ...
    public void registerBlockIcons(IIconRegister ico) {
        ...
        theIcon = new IIcon[] {
            ico.registerIcon("chromaticraft:fluid/" + icon),
            ico.registerIcon("chromaticraft:fluid/" + icon + "_flowing")
        };
    }
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random r) {
        Color c = ReikaDyeHelper.getRandomColor()
                      .getJavaColor()
                      .brighter()
                      .brighter()
                      .brighter()
                      .brighter();
        ReikaParticleHelper.spawnColoredParticles(
            world, x, y, z, c.getRed() / 255F, c.getGreen() / 255F, c.getBlue() / 255F, 2
        );
                ...
                if (ct > 0) {
                    ...
                    if (r.nextInt(1 + n) > 0) {
                        for (int i = 0; i < n; i++) {
                            ...
                            double ry
                                = ReikaRandomHelper.getRandomPlusMinus(y + 0.5, 0.5);
                            double rz = ReikaRandomHelper.getRandomPlusMinus(z + 0.5, 1);
                            EntityChromaFluidFX fx
                                = new EntityChromaFluidFX(world, rx, ry, rz).setLife(20);
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity e) {
        ...
        if (e instanceof EntityPlayer) {
            ...
            CrystalPotionController.instance.applyEffectFromColor(
                201, 0, ep, CrystalElement.BLUE, true
            );
            CrystalPotionController.instance.applyEffectFromColor(
                10, 0, ep, CrystalElement.BROWN, true
            );
            CrystalPotionController.instance.applyEffectFromColor(
                10, 0, ep, CrystalElement.MAGENTA, true
            );
            CrystalPotionController.instance.applyEffectFromColor(
                10, 0, ep, CrystalElement.WHITE, true
            );
        } else if (e instanceof EntityItem) {
            ((EntityItem) e).age
                = ((EntityItem) e).age % Math.max(40, ((EntityItem) e).lifespan - 40);
        }
    }
