...
public class BlockFakeSky extends Block {
    ...
    public int getRenderColor(int meta) {
        ...
        if (mc.theWorld != null && mc.thePlayer != null) {
            return ReikaColorAPI.fromVec3(mc.theWorld.getSkyColor(
                mc.thePlayer, ReikaRenderHelper.getPartialTickTime()
            ));
        }
        return 0x55d0ff;
    }
    ...
    public static void doSkyRendering() {
        ...
        if (c != null) {
            ...
            Vec3 clr
                = ep.worldObj.getSkyColor(ep, ReikaRenderHelper.getPartialTickTime());

            Tessellator v5 = Tessellator.instance;
            v5.startDrawingQuads();
            v5.setColorOpaque_F(
                (float) clr.xCoord, (float) clr.yCoord, (float) clr.zCoord
            );
            double o = 0.0075; //*ep.getDistance((arr.getMaxX()+arr.getMinX())/2,
                               //(arr.getMaxY()+arr.getMinY())/2,
                               //(arr.getMaxZ()+arr.getMinZ())/2);
            for (BlockArray arr : c) {
                ...
                v5.addVertex(
                    arr.getMaxX() + 1 + o, arr.getMinY() - o, arr.getMaxZ() + 1 + o
                );
                v5.addVertex(arr.getMinX() - o, arr.getMinY() - o, arr.getMaxZ() + 1 + o);

                v5.addVertex(
                    arr.getMinX() - o, arr.getMaxY() + 1 + o, arr.getMaxZ() + 1 + o
                );
                v5.addVertex(
                    arr.getMaxX() + 1 + o, arr.getMaxY() + 1 + o, arr.getMaxZ() + 1 + o
                );
                v5.addVertex(
                    arr.getMaxX() + 1 + o, arr.getMaxY() + 1 + o, arr.getMinZ() - o
                );
    ...
    private static class SkyCache {
        ...
        private void recalcBlocks() {
            ...
            while (!set.isEmpty()) {
                WorldLocation c
                    = ReikaJavaLibrary.getRandomCollectionEntry(DragonAPICore.rand, set);
                BlockArray arr = new BlockArray();
                arr.recursiveAdd(
                    new CacheAccess(c.dimensionID, set),
                    c.xCoord,
                    c.yCoord,
                    c.zCoord,
                    ChromaBlocks.FAKESKY.getBlockInstance()
                );
        ...
        private class CacheAccess implements IBlockAccess {
            ...
            public Block getBlock(int x, int y, int z) {
                return set.contains(new WorldLocation(dimensionID, x, y, z))
                    ? ChromaBlocks.FAKESKY.getBlockInstance()
                    : Blocks.air;
            }
    ...
    private static class SkyShunt {
        ...
        private void calcMinY(World world) {
            ...
            while (world.getBlock(skyBlock.xCoord, y, skyBlock.zCoord)
                       .getLightOpacity(world, skyBlock.xCoord, y, skyBlock.zCoord)
                   == 0) {
                y--;
            }
            minY = y + 1;
