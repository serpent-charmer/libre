...
public class BlockPath extends Block {
    ...
    public float getExplosionResistance(
        ...
        return world.getBlockMetadata(x, y, z) == PathType.FIRE.ordinal()
            ? 6000
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < icons.length; i++) {
            icons[i][0] = ico.registerIcon(
                "chromaticraft:path/" + PathType.list[i].name.toLowerCase(Locale.ENGLISH)
                + "_back"
            );
            icons[i][1] = ico.registerIcon(
                "chromaticraft:path/" + PathType.list[i].name.toLowerCase(Locale.ENGLISH)
                + "_front"
            );
    ...
    public int colorMultiplier(IBlockAccess world, int x, int y, int z) {
        return world.getBlockMetadata(x, y, z) != PathType.RAINBOW.ordinal() ? 0xffffff
            : ModList.GEOSTRATA.isLoaded() ? this.getOpalColor(world, x, y, z)
                                           : this.getBasicRainbowColor(world, x, y, z);
    }
    ...
    public void onEntityWalking(World world, int x, int y, int z, Entity e) {
        ...
        if (e instanceof EntityLivingBase) {
            ...
            double[] xz
                = ReikaPhysicsHelper.polarToCartesian(1, 0, el.rotationYawHead + 90);
    ...
    private HashSet<ForgeDirection>
    getPathDirectionsAt(World world, int x, int y, int z, int meta) {
        ...
        for (int d = 0; d < 12; d++) {
            for (int i = 2; i < 6; i++) {
                ...
                if (world.getBlock(dx, y, dz) == this
                    && world.getBlockMetadata(dx, y, dz) == meta) {
                    set.add(dir);
                }
            }
            if (set.size() == 4) {
                if (world.getBlock(x - d, y, z - d) == this
                    && world.getBlockMetadata(x - d, y, z - d) == meta) {
                    if (world.getBlock(x + d, y, z - d) == this
                        && world.getBlockMetadata(x + d, y, z - d) == meta) {
                        if (world.getBlock(x - d, y, z + d) == this
                            && world.getBlockMetadata(x - d, y, z + d) == meta) {
