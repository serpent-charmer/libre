...
public class BlockChromaPortal extends Block {
    ...
    public boolean onBlockActivated(
        ...
        if (HoldingChecks.MANIPULATOR.isHolding(ep)) {
            TileEntityCrystalPortal te
                = (TileEntityCrystalPortal) world.getTileEntity(x, y, z);
            if (te.ownedBy(ep)) {
                //world.addWeatherEffect(new EntityLightningBolt(world, x+0.5, y+0.5,
                //z+0.5)); world.createExplosion(ep, x+0.5, y+0.5, z+0.5, 4, false);
                for (int i = -2; i <= 2; i++) {
                    for (int k = -2; k <= 2; k++) {
                ...
                bk.recursiveAddWithBounds(
                    world, x, y, z, this, x - 16, y - 8, z - 16, x + 16, y + 8, z + 16
                );
                for (Coordinate loc : bk.keySet()) {
                    loc.setBlock(world, Blocks.air);
                    ReikaItemHelper.dropItem(
                        world, x + 0.5, y + 0.5, z + 0.5, new ItemStack(this)
                    );
                }
    ...
    public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity e) {
        ...
        for (int i = 2; i < 6; i++) {
            ...
            if (world.getBlock(dx, y, dz) != this) {
                ...
                if (world.getBlock(ddx, y, ddz) == this
                    && !portalCheck.contains(new Coordinate(ddx, y, ddz))) {
        ...
        if (tile instanceof TileEntityCrystalPortal && !world.isRemote) {
            ...
            if (e instanceof EntityPlayer) {
                ...
                if (ReikaItemHelper.matchStacks(is, ChromaStacks.bedrockloot)
                    || ReikaItemHelper.matchStacks(is, ChromaStacks.bedrockloot2)) {
    ...
    public static boolean isPortalFunctional() {
        return ChunkProviderChroma.areGeneratorsReady()
            && ChromatiCraft.instance.isDimensionLoadable();
    }

    public static class TileEntityCrystalPortal
        extends TileEntity implements MultiBlockChromaTile {
        ...
        public void updateEntity() {
            ...
            if (DragonAPICore.debugtest) {
                //ChromaStructures.getPortalStructure(worldObj, xCoord, yCoord, zCoord,
                //false).place(); DragonAPICore.debugtest = false;
            }
        ...
        private void teleportPlayer(EntityPlayer ep) {
            ...
            ReikaEntityHelper.transferEntityToDimension(
                ep, dim, new ChromaTeleporter(dim)
            );
            if (ProgressStage.DIMENSION.stepPlayerTo(ep)) {
                ...
                ReikaSoundHelper.playSound(
                    ChromaSounds.GOTODIM, ep.worldObj, 0, 1024, 0, 1, 1, false
                );
                ReikaPacketHelper.sendDataPacket(
                    ChromatiCraft.packetChannel,
                    ChromaPackets.DIMSOUND.ordinal(),
        ...
        public void addTuningEnergy(ItemStack is) {
            ...
            int amt
                = (int) ((tier2 ? 150 : 1) * Math.pow(is.stackSize, tier2 ? 0.85 : 0.5));
            tuning += amt;
        ...
        public boolean canPlayerUse(EntityPlayer ep) {
            return isPortalFunctional() && charge >= MINCHARGE
                && ProgressionManager.instance.playerHasPrerequisites(
                    ep, ProgressStage.DIMENSION
                );
        }
        ...
        private void idleParticles() {
            ...
            EntityBlurFX fx = new EntityCCBlurFX(worldObj, px, yCoord + 1.25, pz, 0, 0, 0)
                                  .setGravity(g)
                                  .setLife(l)
                                  .setColor(color);
            fx.noClip = true;
        ...
        private void chargingParticles() {
            if (worldObj.rand.nextInt(4) == 0) {
                ...
                if (worldObj.getBlock(x, y, z)
                    ...
                    //EntityFX fx = new EntityBoltFX(worldObj, x+0.5, y+0.5, z+0.5, x+5,
                    //y, z+5);
                    double px = x + worldObj.rand.nextDouble();
                    double py = y + worldObj.rand.nextDouble();
                    double pz = z + worldObj.rand.nextDouble();
                    EntityBallLightningFX fx = new EntityBallLightningFX(
        ...
        private void activeParticles() {
            ...
            EntityBlurFX fx
                = new EntityCCBlurFX(
                      worldObj, xCoord + 0.5, yCoord + 8.25, zCoord + 0.5, vx, 0, vz
                )
                      .setGravity(g);
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
            ...
            EntityFX fx2 = new EntityCenterBlurFX(e, worldObj, x, y, z, vx, 0, vz)
                               .setScale(2)
                               .setNoSlowdown();
            fx2.noClip = true;
            ...
            boolean longAxis
                = (Math.abs(dx) == 7 && vx != 0) || (Math.abs(dz) == 7 && vz != 0);
            int l = !longAxis ? 100 : 60;

            EntityRuneFX fx3 = new EntityRuneFX(worldObj, x, y, z, vx, 0, vz, e)
                                   .setScale(2)
                                   .setLife(l);
            fx3.noClip = true;
        ...
        public void validateStructure(World world, int x, int y, int z) {
            ...
            if (this.getBlockMetadata() == 15) {
                ...
                complete
                    = ChromaStructures.PORTAL.getArray(world, x, y, z).matchInWorld();
        ...
        private boolean getEntities(World world, int x, int y, int z) {
            int[][] pos
                = new int[][] { { -5, 5, -9 }, { -5, 5, 9 }, { 5, 5, -9 }, { 5, 5, 9 },
                                { -9, 5, -5 }, { -9, 5, 5 }, { 9, 5, -5 }, { 9, 5, 5 } };
            for (int i = 0; i < pos.length; i++) {
                int[] loc = pos[i];
                AxisAlignedBB box
                    = ReikaAABBHelper.getBlockAABB(x + loc[0], y + loc[1], z + loc[2]);
                if (world.getEntitiesWithinAABB(EntityEnderCrystal.class, box).size()
                    != 1)
                    return false;
            }
            return true;
        ...
        public int getPortalPosition(IBlockAccess world, int x, int y, int z) {
            if (world.getBlock(x - 1, y, z) != ChromaBlocks.PORTAL.getBlockInstance()) {
                if (world.getBlock(x, y, z - 1)
                    != ChromaBlocks.PORTAL.getBlockInstance()) {
                    return 7;
        ...
        public boolean isFull9x9() {
            for (int i = -1; i <= 1; i++) {
                for (int k = -1; k <= 1; k++) {
                    if (worldObj.getBlock(xCoord + i, yCoord, zCoord + k)
                        != ChromaBlocks.PORTAL.getBlockInstance()) {
                        return false;
                    }
                }
