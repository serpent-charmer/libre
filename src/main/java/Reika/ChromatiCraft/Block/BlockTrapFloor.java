...
public class BlockTrapFloor
    extends Block implements CollisionDelegate, SemiUnbreakable, IWailaDataProvider {
    ...
    public boolean isUnbreakable(World world, int x, int y, int z, int meta) {
        ...
        return b == ChromaBlocks.STRUCTSHIELD.getBlockInstance()
            && ((BlockStructureShield) b)
                   .isUnbreakable(
                       world, x, dy, z, world.getBlockMetadata(x, dy, z)
                   ); //meta >= 8;
    }
}
