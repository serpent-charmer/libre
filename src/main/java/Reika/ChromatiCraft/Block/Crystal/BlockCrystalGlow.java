...
public class BlockCrystalGlow extends CrystalTypeBlock {
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        double v = 0.0125;
        double vx = in ? v * (0.5 - (rx - x))
                       : (rx - x) * v; //ReikaRandomHelper.getRandomPlusMinus(0, 0.03125);
        double vz = in ? v * (0.5 - (rz - z))
                       : (rz - z) * v; //ReikaRandomHelper.getRandomPlusMinus(0, 0.03125);
        EntityFX fx = null;
        int type = rand.nextInt(3);
        switch (type) {
            case 0:
                fx = new EntityCCBlurFX(
                         this.getCrystalElement(world, x, y, z),
                         world,
                         rx,
                         ry,
                         rz,
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block b) {
        ...
        if (!this.canPlaceOn(
            ...
            ItemStack is = ChromaBlocks.GLOW.getStackOfMetadata(
                world.getBlockMetadata(x, y, z) + 16 * te.base.ordinal()
            );
    ...
    public void setBlockBoundsBasedOnState(IBlockAccess iba, int x, int y, int z) {
        ...
        ForgeDirection dir = te instanceof TileEntityCrystalGlow
            ? ((TileEntityCrystalGlow) te).direction
            : ForgeDirection.UNKNOWN;
        ForgeDirection ax = te instanceof TileEntityCrystalGlow
            ? ((TileEntityCrystalGlow) te).longAxis
            : ForgeDirection.UNKNOWN;
        switch (dir) {
            case WEST:
                zmin = 0.5F - w;
