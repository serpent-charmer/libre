...
public class BlockPowerTree extends Block implements IWailaDataProvider {
    ...
    public final int getLightValue(IBlockAccess iba, int x, int y, int z) {
        ...
        return ModList.COLORLIGHT.isLoaded()
            ? ReikaColorAPI.getPackedIntForColoredLight(e.getColor(), 15)
            : 15;
    }
    ...
    removedByPlayer(World world, EntityPlayer ep, int x, int y, int z, boolean harvest) {
        ep.attackEntityFrom(
            ChromatiCraft.pylonDamage[this.getColor(world, x, y, z).ordinal()], 5
        );
    ...
    private void breakEffects(World world, int x, int y, int z, CrystalElement e) {
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            ChromaPackets.LEAFBREAK.ordinal(),
            world,
            x,
            y,
            z,
    ...
    public static void
    breakEffectsClient(World world, int x, int y, int z, CrystalElement e) {
        for (int i = 0; i < 64; i++) {
    ...
    getDrops(World world, int x, int y, int z, int meta, int fortune) {
        ...
        while (n > 0) {
            int rem
                = Math.min(n, ChromaItems.SHARD.getItemInstance().getItemStackLimit());
            ItemStack is = rand.nextInt(3) > 0 ? ChromaStacks.getChargedShard(e)
                                               : ChromaStacks.getShard(e);
    ...
    public List<String> getWailaBody(
        ...
        CrystalElement e = CrystalElement.elements[meta];
        TileEntityPowerTree te = this.getTile(
            acc.getWorld(),
            acc.getPosition().blockX,
            acc.getPosition().blockY,
            acc.getPosition().blockZ
        );
        if (te != null)
            tip.add(String.format(
                "Stored Energy: %d lumens of %s", te.getEnergy(e), e.displayName
            ));
        return tip;
    }
    ...
    public static class TileEntityPowerTreeAux extends TileEntity {
        ...
        public TileEntityPowerTree getCenter() {
            ...
            return c == ChromaTiles.POWERTREE
                ? (TileEntityPowerTree) worldObj.getTileEntity(originX, originY, originZ)
                : null;
        }
