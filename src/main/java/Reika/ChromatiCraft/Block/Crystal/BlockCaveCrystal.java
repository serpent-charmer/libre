...
public class BlockCaveCrystal
    extends CrystalBlock implements ProgressionTrigger, MinerBlock {
    ...
    @Override
    public boolean
    canSilkHarvest(World world, EntityPlayer ep, int x, int y, int z, int meta) {
        return true;
    }
    ...
    getDrops(World world, int x, int y, int z, int meta, int fortune) {
        ...
        if (ep != null)
            num = DimensionTuningManager.instance.getTunedDropCount(
                ep, num, 1, Integer.MAX_VALUE
            );
        for (int i = 0; i < num; i++)
...
Fortune 20 average: 44.3653 = 44<br>
    ...
    public static int getNumberDrops(Random rand, int fortune) {
        return 1 + rand.nextInt(6 + fortune) + (1 + fortune) * rand.nextInt(3)
            + rand.nextInt(1 + fortune);
    }
    ...
    public boolean performEffect(CrystalElement e) {
        return e == CrystalElement.BROWN || e == CrystalElement.BLUE
            ? rand.nextInt(4) == 0
            : true;
    }
    ...
    @Override
    public BlockKey
    getBaseBlock(IBlockAccess iba, int x, int y, int z, ForgeDirection side) {
    ...
    private boolean isDarkStructure(IBlockAccess iba, int x, int y, int z) {
        return iba.getBlock(x, y - 1, z) == ChromaBlocks.RUNE.getBlockInstance()
            && iba.getBlock(x, y + 1, z) == ChromaBlocks.STRUCTSHIELD.getBlockInstance();
    }
