...
public class BlockLightedLeaf
    extends BlockCustomLeaf implements LightedTreeBlock, CustomExcavationStarBehavior {
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random r) {
        if (r.nextInt(6) == 0) {
            ...
            EntityFX fx = new EntityCCFloatingSeedsFX(
                              ...
                              .setColor(0xFFDD87) /*.setRapidExpand()*/.setLife(l)
                              .setGravity(0)
                              .setScale(s);
            fx.noClip = true;
    ...
    public static void setAnimationData(TextureAtlasSprite icon) {
        if (icon.animationMetadata == null) {
            ChromatiCraft.logger.logError(
                "Animation " + icon.getIconName() + " has no data?!"
            );
        } else {
            String s = icon.getIconName();
            int idx = Integer.parseInt(s.substring(s.lastIndexOf('/') + 1));
            icon.animationMetadata = new ShuffledIconControl(
                idx,
                icon.animationMetadata,
                ShuffledIconControl.ANIMATION_SECTIONS_GLOWLEAF,
                ShuffledIconControl.ANIMATION_SECTION_LENGTH_GLOWLEAF
            );
        }
    }
    ...
    public boolean isValidLog(
        ...
        return iba.getBlock(lookX, lookY, lookZ)
            == ChromaBlocks.GLOWLOG.getBlockInstance();
    }
