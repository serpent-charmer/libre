...
public class BlockDimensionDeco extends Block implements MinerBlock {
    ...
    public static enum DimDecoTypes implements DecoType {
        ...
        public ItemStack getItem() {
            return new ItemStack(
                ChromaBlocks.DIMGEN.getBlockInstance(), 1, this.ordinal()
            );
        }

        public boolean hasBlockRender() {
            return this == FLOATSTONE || this == GEMSTONE || this == CRYSTALLEAF
                || this == OCEANSTONE || this == CLIFFGLASS || this == GLOWCAVE;
        }

        public List<IIcon>
        getIcons(IBlockAccess iba, int x, int y, int z, int pass, Random rand) {
        ...
        public List<IIcon> getItemIcons(int pass) {
            if (icons[this.ordinal()] == null) {
                ChromatiCraft.logger.logError(
                    "Dimension Deco Type " + this + " is missing icons!"
                );
                return ReikaJavaLibrary.makeListFrom(
                    Blocks.bedrock.blockIcon,
                    Blocks.emerald_block.blockIcon,
                    Blocks.obsidian.blockIcon,
                    Blocks.mob_spawner.blockIcon
                );
            }
        ...
        public boolean requiresPickaxe() {
            return this == FLOATSTONE || this == GEMSTONE || this == OCEANSTONE
                || this == CLIFFGLASS;
        }
        ...
        private void addDrops(
            ...
            switch (this) {
                ...
                default:
                    //float f = ep == null ? 1 :
                    //DimensionTuningManager.instance.getTunedDropRates(ep);
                    if (ep != null)
                        n = DimensionTuningManager.instance.getTunedDropCount(
                            ep, n, 1, this.getMaxDrops()
                        );
                    for (int i = 0; i < n; i++)
    ...
    public boolean shouldSideBeRendered(IBlockAccess iba, int x, int y, int z, int s) {
        ...
        return iba.getBlock(x, y, z) != this
            ...
            != iba.getBlockMetadata(x - dir.offsetX, y - dir.offsetY, z - dir.offsetZ);
    }
    ...
    public int getRenderColor(int meta) {
        if (meta == DimDecoTypes.GEMSTONE.ordinal()
            ...
            return getGemStoneColor(
                ...
                MathHelper.floor_double(ep.posY),
    ...
    public int colorMultiplier(IBlockAccess iba, int x, int y, int z) {
        ...
        if (meta == DimDecoTypes.GEMSTONE.ordinal()
            || meta == DimDecoTypes.CRYSTALLEAF.ordinal()) {
    ...
    public int getMixedBrightnessForBlock(IBlockAccess iba, int x, int y, int z) {
        int meta = iba.getBlockMetadata(x, y, z);
        if (meta == DimDecoTypes.GEMSTONE.ordinal()
            || meta == DimDecoTypes.OCEANSTONE.ordinal()) {
    ...
    public IIcon getIcon(int s, int meta) {
        if (icons[meta] == null) {
            ChromatiCraft.logger.logError(
                "Dimension Deco Type " + DimDecoTypes.list[meta] + " is missing icons!"
            );
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < DimDecoTypes.list.length; i++) {
            ...
            for (int k = 0; k < n; k++) {
                icons[i].add(ico.registerIcon(
                    "chromaticraft:dimgen/" + deco.name().toLowerCase(Locale.ENGLISH)
                    + "/layer_" + k
                ));
    ...
    public boolean canHarvestBlock(EntityPlayer ep, int meta) {
        return DimDecoTypes.list[meta].requiresPickaxe() ? super.canHarvestBlock(ep, meta)
                                                         : true;
    }
    ...
    getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        return DimDecoTypes.list[meta].hasBlockRender()
    ...
    getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        ...
        DimDecoTypes.list[metadata].addDrops(
            world, x, y, z, metadata, fortune, li, harvesters.get()
        );
        return li;
    }
    ...
    public int onBlockPlaced(
        World world, int x, int y, int z, int s, float a, float b, float c, int meta
    ) {
    ...
    @SideOnly(Side.CLIENT)
    public boolean
    addHitEffects(World world, MovingObjectPosition target, EffectRenderer f) {
    ...
    @SideOnly(Side.CLIENT)
    public boolean
    addDestroyEffects(World world, int x, int y, int z, int meta, EffectRenderer f) {
        if (meta == DimDecoTypes.GEMSTONE.ordinal()) {
            this.doBreakFX(world, x, y, z);
            return true;
        }
    ...
    public boolean isBeaconBase(
        IBlockAccess world, int x, int y, int z, int beaconX, int beaconY, int beaconZ
    ) {
    ...
    public static void setGlowCaveAnimationData(TextureAtlasSprite icon) {
        if (icon.animationMetadata == null) {
            ChromatiCraft.logger.logError("Animation "+icon.getIconName()+" has no
    data?!");
        }
        else {
            String s = icon.getIconName();
            int idx = Integer.parseInt(s.substring(s.lastIndexOf('/')+1));
            icon.animationMetadata = new ShuffledIconControl(idx, icon.animationMetadata,
    ShuffledIconControl.ANIMATION_SECTIONS_GLOWCAVE,
    ShuffledIconControl.ANIMATION_SECTION_LENGTH_GLOWCAVE);
        }
    }*/
            ...
            switch (DimDecoTypes.list[meta]) {
                ...
                case LIFEWATER:
                    if (elb instanceof EntityMob
                        && ((EntityMob) elb).getCreatureAttribute()
                ...
                case MIASMA:
                    ...
                    for (Object o : elb.getActivePotionEffects()) {
                        ...
                        if (ReikaPotionHelper.isBadEffect(p)) {
                            ...
                            PotionEffect repl
                                = new PotionEffect(p.id, time, ef.getAmplifier());
