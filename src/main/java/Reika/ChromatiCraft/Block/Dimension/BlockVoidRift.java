...
public class BlockVoidRift extends Block {
    ...
    public IIcon getIcon(int s, int meta) {
        return s == 1 ? blockIcon
                      : ChromaBlocks.STRUCTSHIELD.getBlockInstance().getIcon(0, 0);
    }
    ...
    public static class TileEntityVoidRift extends TileEntity {
        ...
        private void spawnParticles(World world, int x, int y, int z) {
            int n = Math.max(
                1, (4 - Minecraft.getMinecraft().gameSettings.particleSetting) / 2
            );
            for (int i = 0; i < n; i++) {
                ...
                while (it.hasNext()) {
                    ...
                    if (world.getBlock(dx, y, dz) == this.getBlockType()
                        && world.getBlockMetadata(dx, y, dz) == this.getBlockMetadata())
                        it.remove();
                }
                if (sides.isEmpty())
                    return;
                ForgeDirection dir
                    = ForgeDirection
                          .VALID_DIRECTIONS[sides.get(rand.nextInt(sides.size()))];
                double px = dir.offsetX == 0 ? x + rand.nextDouble()
                                             : x + 0.5 + dir.offsetX * 0.5;
                double pz = dir.offsetZ == 0 ? z + rand.nextDouble()
                                             : z + 0.5 + dir.offsetZ * 0.5;
                ...
                EntityBlurFX fx = new EntityCCBlurFX(world, px, y + 1, pz)
                                      .fadeColors(0xffffff, this.getColor().getColor())
                                      .setScale(s)
                                      .setLife(l)
                                      .setGravity(g)
                                      .setRapidExpand();
        ...
        public AxisAlignedBB getRenderBoundingBox() {
            return AxisAlignedBB.getBoundingBox(
                xCoord, yCoord, zCoord, xCoord + 1, yCoord + 1 + HEIGHT, zCoord + 1
            );
        }
        ...
        public boolean hasAt(int dx, int dz) {
            return this.getAt(dx, dz).blockID
                == this.getBlockType(
                ); // && worldObj.getBlockMetadata(xCoord+dx, yCoord+dy, zCoord+dz) ==
