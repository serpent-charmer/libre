...
public class BlockBedrockCrack extends BlockReplaceOnBreak implements MinerBlock {
    ...
    private ArrayList<ItemStack> getDrops(
        World world, int x, int y, int z, int metadata, int fortune, EntityPlayer ep
    ) {
        ArrayList<ItemStack> li = new ArrayList();
        int n
            = 1 + Math.min(metadata, DragonAPICore.rand.nextInt(1 + metadata + fortune));
        if (ep != null)
            n = DimensionTuningManager.instance.getTunedDropCount(
                ep, n, 1, Integer.MAX_VALUE
            );
        for (int i = 0; i < n; i++)
            li.add(ChromaStacks.bedrockloot.copy());
        if (metadata == 9) {
            n = ep != null
                ? DimensionTuningManager.instance.getTunedDropCount(ep, n, 1, 3)
                : 1;
            for (int i = 0; i < n; i++)
    ...
    @Override
    public Block getBlockReplacedWith(
        EntityPlayer ep, int x, int y, int z, int oldMeta, boolean willHarvest
    ) {
        return willHarvest && oldMeta > 0 ? this : Blocks.bedrock;
    }

    @Override
    public int getMetaReplacedWith(
        EntityPlayer ep, int x, int y, int z, int oldMeta, boolean willHarvest
    ) {
