...
public class BlockDimensionDecoTile extends BlockDimensionDeco {
    ...
    public static enum DimDecoTileTypes implements DecoType {
        ...
        public ItemStack getItem() {
            return new ItemStack(
                ChromaBlocks.DIMGENTILE.getBlockInstance(), 1, this.ordinal()
            );
        }
        ...
        public List<IIcon> getItemIcons(int pass) {
            return pass == 0 ? ReikaJavaLibrary.makeListFrom(icons[1][this.ordinal()])
                             : new ArrayList();
        }
    ...
    getDrops(World world, int x, int y, int z, int metadata, int fortune) {
        ...
        li.add(new ItemStack(
            this.getItemDropped(metadata, world.rand, fortune),
            1,
    ...
    getPlayerRelativeBlockHardness(EntityPlayer ep, World world, int x, int y, int z) {
        return ProgressStage.CTM.isPlayerAtStage(ep)
                && DimDecoTileTypes.list[world.getBlockMetadata(x, y, z)].isMineable()
    ...
    public TileEntity createTileEntity(World world, int meta) {
        return meta == DimDecoTileTypes.GLOWCRACKS.ordinal() ? new TileGlowingCracks()
                                                             : new DimensionDecoTile();
    }
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        if (DimDecoTileTypes.list[world.getBlockMetadata(x, y, z)].needsRandomTick()) {
            ...
            world.scheduleBlockUpdate(
                ...
                z,
                this,
                this.tickRate(world) + rand.nextInt(1 + this.tickRate(world))
            );
        }
    }
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < DimDecoTileTypes.list.length; i++) {
            ...
            DimDecoTileTypes.list[i].itemIcon = ico.registerIcon(
                "chromaticraft:dimgen/" + DimDecoTileTypes.list[i].itemTex
            );
        }
    }
    ...
    getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        if (meta >= DimDecoTileTypes.list.length) {
            ChromatiCraft.logger.logError(
                "Invalid meta for dim deco tile @ " + new Coordinate(x, y, z) + "!"
            );
                ...
                case FIREJET: {
                    if (worldObj.rand.nextBoolean()) {
                        ...
                        if (b instanceof CrystalTypeBlock || b instanceof BlockDyeTypes) {
                            c = CrystalElement
                                    .elements[worldObj.getBlockMetadata(
                        ...
                        EntityCCBlurFX fx = new EntityCCBlurFX(
                            worldObj, xCoord + 0.5, yCoord + 0.9, zCoord + 0.5, 0, vy, 0
                        );
