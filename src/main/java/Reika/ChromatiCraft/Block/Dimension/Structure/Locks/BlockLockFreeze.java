...
public class BlockLockFreeze extends BlockDimensionStructureTile {
    ...
    public void registerBlockIcons(IIconRegister ico) {
        ...
        activeIcon[0]
            = ico.registerIcon("chromaticraft:dimstruct/lockfreeze_button_inactive");
        activeIcon[1]
            = ico.registerIcon("chromaticraft:dimstruct/lockfreeze_button_active");
    }
    ...
    public IIcon getIcon(IBlockAccess iba, int x, int y, int z, int s) {
        ...
        return s <= 1 ? blockIcon
                      : activeIcon
                          [te instanceof TileEntityLockFreeze
                                   && ((TileEntityLockFreeze) te).isActive()
                               ? 1
                               : 0];
    }
    ...
    public static class TileEntityLockFreeze extends StructureBlockTile<LocksGenerator> {
        ...
        private void setTime(int time) {
            ...
            ReikaSoundHelper.playSoundAtBlock(
                worldObj, xCoord, yCoord, zCoord, "random.click", 1, 0.5F
            );
        ...
        public void updateEntity() {
            if (timer > 0) {
                ...
                if (timer == 0) {
                    ReikaSoundHelper.playSoundAtBlock(
                        worldObj, xCoord, yCoord, zCoord, "random.click", 2, 0.5F
                    );
                } else if (timer == 5) {
                    this.ding(1);
                } else if (timer == 10) {
        ...
        private void ding(int n) {
            ChromaSounds.DING.playSoundAtBlock(
                worldObj, xCoord, yCoord, zCoord, 2, n / 2F
            );
        }
