...
public class BlockSpecialShield extends BlockStructureShield {
    ...
    public static final IIcon[][] overlayIcons
        = new IIcon[16][ReikaArrayHelper.getMaxValue(overlays)];
    ...
    public void registerBlockIcons(IIconRegister ico) {
        for (int i = 0; i < BlockType.list.length; i++) {
            ...
            int n = overlays[i];
            for (int k = 0; k < n; k++) {
                overlayIcons[i][k] = ico.registerIcon(
                    "chromaticraft:basic/specialshield_" + i
                    + ReikaStringParser.intToAlphaChar(k)
                );
            }
        }
    ...
    public static int getOverlayIndex(IBlockAccess iba, int x, int y, int z, int meta) {
        return overlays[meta] > 0
            ? ((x + z) % overlays[meta] + overlays[meta]) % overlays[meta]
            : -1;
    }
    ...
    public boolean shouldSideBeRendered(IBlockAccess iba, int dx, int dy, int dz, int s) {
        ...
        return iba.getBlock(dx, dy, dz) != this
            || iba.getBlockMetadata(dx, dy, dz)
            != iba.getBlockMetadata(dx - dir.offsetX, dy - dir.offsetY, dz - dir.offsetZ);
    }
    ...
    public static DimensionStructureGenerator
    getStructure(World world, int x, int y, int z) {
