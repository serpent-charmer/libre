...
public class BlockLaserEffector extends BlockDimensionStructureTile {
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block b) {
        if (DragonAPICore.isReikasComputer()
            ...
            if (world.isBlockIndirectlyGettingPowered(x, y, z)) {
                if (world.getBlockMetadata(x, y, z)
                    == LaserEffectType.EMITTER.ordinal()) {
    ...
    getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        return currentCollisionEntity instanceof EntityPlayer
    ...
    public void addCollisionBoxesToList(
        World world, int x, int y, int z, AxisAlignedBB mask, List li, Entity e
    ) {
    ...
    public void registerBlockIcons(IIconRegister ico) {
        ...
        for (LaserEffectType e : LaserEffectType.list) {
            e.frontTexture = ico.registerIcon(
                "chromaticraft:dimstruct/laser_front/"
                + e.name().toLowerCase(Locale.ENGLISH) + "_back"
            );
            e.frontOverlay = ico.registerIcon(
                "chromaticraft:dimstruct/laser_front/"
                + e.name().toLowerCase(Locale.ENGLISH) + "_front"
            );
        }
    }
    ...
    public static enum LaserEffectType {
        ...
        public boolean affectPulse(World world, int x, int y, int z, EntityLaserPulse e) {
            ...
            if (te instanceof TargetTile) {
                if (e.color.matchColor(te.color)
                    && (e.direction == te.facing || this.isOmniDirectional()))
            ...
            switch (this) {
                ...
                case MIRROR: {
                    if (!ReikaMathLibrary.isValueInsideBoundsIncl(
                            3, 5, Math.abs(e.direction.ordinal() - te.facing.ordinal())
                        ))
                        return true;
                ...
                case SLITMIRROR: {
                    ...
                    //ReikaJavaLibrary.pConsole(e.direction+" & "+te.facing+" > "+dir+"
                    //("+(dir != te.facing)+" & "+(dir != te.facing.getOpposite())+")");
                ...
                case POLARIZER:
                    return e.direction != te.facing
                        && e.direction != te.facing.getOpposite();
                case PRISM: {
                    if (e.direction != te.facing.getOpposite()) {
                        ...
                        RGBColorData dat = new RGBColorData(
                            e.color.red && b1, e.color.green && b2, e.color.blue && b3
                        );
                        //ReikaJavaLibrary.pConsole(dat.red+":"+dat.green+":"+dat.blue+" &
                        //"+b1+","+b2+","+b3);
                        /*
                        if (!dat.isBlack()) {
                            EntityLaserPulse eb = new EntityLaserPulse(world, x, y, z,
                        te.facing.getOpposite(), dat); if (!world.isRemote) {
                    ...
                    if (e.color.red)
                        te.fireParticle(
                            ...
                            new RGBColorData(true, false, false),
                            e.getLevel()
                        );
                    if (e.color.green)
                        te.fireParticle(
                            e.direction,
                            new RGBColorData(false, true, false),
                            e.getLevel()
                        );
                    if (e.color.blue)
                        te.fireParticle(
                            e.direction.getRotation(false, 2),
                            new RGBColorData(false, false, true),
                            e.getLevel()
                        );
                    return true;
                }
                case REFRACTOR:
                    int d = (int) MathHelper.wrapAngleTo180_double(
                        e.direction.angle - te.facing.angle
                    );
                    if (d == 90) {
                        e.refract(false);
                    } else if (d == -45) {
                ...
                case SPLITTER: {
                    if (e.direction == te.facing) {
                        te.fireParticle(
                            e.direction.getRotation(true), e.color, e.getLevel()
                        );
                        te.fireParticle(
                            e.direction.getRotation(false), e.color, e.getLevel()
                        );
                    }
        ...
        public void onInteract(World world, int x, int y, int z, EntityPlayer ep) {
            ...
            if (DragonAPICore.isReikasComputer()
                ...
                if (ChromaItems.SHARD.matchWith(is)) {
                    ...
                    ((TargetTile) te)
                        .trigger(!((TargetTile) te).isTriggered(), true, false);
                    return;
    ...
    public static class TargetTile extends LaserEffectTile {
        ...
        private void doFXClient() {
            for (int i = 0; i < 32; i++) {
                ...
                EntityFX fx = new EntityCCBlurFX(worldObj, x, y, z)
    ...
    public static class PrismTile extends LaserEffectTile {
        ...
        private int timerLength
            = 2; //4 for the complex filter puzzle, as well as maybe complex prisms
    ...
    public static class LaserEffectTile extends StructureBlockTile<LaserPuzzleGenerator> {
        ...
        public boolean isRotateable() {
            ...
            return PARTIAL_ROTATEABILITY ? rotateable
        ...
        public final void fire() {
            if (!worldObj.isRemote) {
                EntityLaserPulse ea = new EntityLaserPulse(
                    worldObj, xCoord, yCoord, zCoord, facing, color, level
                );
                this.affect(ea);
        ...
        fireParticle(CubeDirections dir, RGBColorData clr, String lvl) {
            if (!worldObj.isRemote) {
                EntityLaserPulse ea = new EntityLaserPulse(
                    worldObj, xCoord, yCoord, zCoord, dir, clr, lvl
                );
                this.affect(ea);
                worldObj.spawnEntityInWorld(ea);
            }
