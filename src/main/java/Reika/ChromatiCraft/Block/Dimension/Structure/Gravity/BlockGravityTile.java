...
public class BlockGravityTile extends BlockDimensionStructureTile {
    ...
    public IIcon getIcon(IBlockAccess iba, int x, int y, int z, int s) {
        return s == 1 ? underlay
                      : Blocks.iron_block.getIcon(s, iba.getBlockMetadata(x, y, z));
    }
    ...
    private static boolean canMove(World world, int x, int y, int z, ForgeDirection dir) {
        ...
        return world.getBlock(dx, dy, dz) instanceof BlockStructureShield
            && world.getBlockMetadata(dx, dy, dz) % 8 == BlockType.CLOAK.ordinal();
    }
            ...
            switch (this) {
                ...
                case TINTER:
                    if (world.isRemote)
                        return false;
                    ForgeDirection dir
                        = ReikaDirectionHelper.getImpactedSide(world, x, y, z, e);
                ...
                case RIFT:
                    te.onImpact(
                        ...
                        e,
                        0,
                        e.getDistanceSq(x + 0.5, y + 0.5, z + 0.5)
                    );
                    return true;
                default:
                    return false;
    ...
    public static class RerouteTile extends GravityTile {
        private final EnumMap<CrystalElement, Integer> collection
    ...
    public static class GravityWarp extends GravityTile implements Linkable<Coordinate> {
        ...
        public void readFromNBT(NBTTagCompound tag) {
            ...
            if (otherLocation != null) {
                DecimalPosition p2
                    = new DecimalPosition(
                          otherLocation.offset(new Coordinate(this).negate())
                    )
                          .offset(-0.5, -0.5, -0.5);
        ...
        @Override
        protected void
        onImpact(GravityTiles type, Coordinate c, EntityLumaBurst e, double r, double d) {
            if (otherLocation != null && d <= 0.5 * 0.5 && !this.isSpawnLocked(c, e)) {
                ((GravityWarp) otherLocation.getTileEntity(worldObj))
                    .fireCopy(e.getColor(), e);
            } else {
        ...
        public boolean hasValidConnection() {
            return otherLocation != null
                && otherLocation.getTileEntity(worldObj) instanceof GravityWarp;
        }
    }
        ...
        onImpact(GravityTiles type, Coordinate c, EntityLumaBurst e, double r, double d) {
            ...
            if (d <= 0.625 * 0.625) {
                if (e.getColor() == color) {
                    ...
                    ReikaPhysicsHelper.reflectEntitySpherical(
                        xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, e
                    );
                }
            } else {
                super.onImpact(type, c, e, r, d);
    ...
    public static class GravityTile extends StructureBlockTile<GravityPuzzleGenerator> {
        ...
        public void updateEntity() {
            ...
            if (r > 0) {
                ...
                AxisAlignedBB box = ReikaAABBHelper.getBlockAABB(xCoord, yCoord, zCoord)
                                        .expand(r, r, r);
                List<EntityLumaBurst> li
                    = worldObj.getEntitiesWithinAABB(EntityLumaBurst.class, box);
        ...
        protected EntityLumaBurst fire(CrystalElement c) {
            ...
            if (this.getTileType() == GravityTiles.SPLITTER
                && worldObj.rand.nextBoolean())
                dir = dir.getOpposite();
            EntityLumaBurst e
                = new EntityLumaBurst(worldObj, xCoord, yCoord, zCoord, dir, c);
            if (!worldObj.isRemote) {
        ...
        protected EntityLumaBurst fireFree(CrystalElement c, double ang) {
            EntityLumaBurst e
                = new EntityLumaBurst(worldObj, xCoord, yCoord, zCoord, ang, c);
            if (!worldObj.isRemote) {
                worldObj.spawnEntityInWorld(e);
            }
            return e;
        ...
        protected EntityLumaBurst fireCopy(CrystalElement c, EntityLumaBurst e2) {
            EntityLumaBurst e
                = new EntityLumaBurst(worldObj, xCoord, yCoord, zCoord, 0, c);
            e.copyFrom(e2);
            if (!worldObj.isRemote) {
                worldObj.spawnEntityInWorld(e);
            }
