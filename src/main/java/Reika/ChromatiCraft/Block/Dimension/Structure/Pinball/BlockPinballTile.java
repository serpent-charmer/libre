        ...
        if (DragonAPICore.isReikasComputer()
            ...
            if (world.isBlockIndirectlyGettingPowered(x, y, z)) {
                if (world.getBlockMetadata(x, y, z)
                    == PinballRerouteType.EMITTER.ordinal()) {
    ...
    public static enum PinballRerouteType {
        ...
        public void onInteract(World world, int x, int y, int z, EntityPlayer ep) {
            ...
            if (DragonAPICore.isReikasComputer()
                ...
                if (!this.isOmniDirectional() && is != null
                    && is.getItem() == Items.diamond) {
                    te.rotateable = !te.rotateable;
                    world.markBlockForUpdate(x, y, z);
                    return;
                }
				else if (te instanceof PinballTargetTile && is != null && is.getItem() == Items.glowstone_dust) {
                    ((PinballTargetTile) te)
                        .trigger(!((PinballTargetTile) te).isTriggered(), true, false);
                    return;
                }
            ...
            if (doFX) {
                ...
                if (set) {
                    ...
                    for (int i = 0; i < 32; i++) {
                        ...
                        EntityFX fx = new EntityBlurFX(worldObj, x, y,
                z).setColor(this.getRenderColor()).setLife(l);
    ...
    public static class PinballTile extends StructureBlockTile<PinballGenerator> {
        ...
        public void fire() {
            if (!worldObj.isRemote) {
                EntityTNTPinball ea = new EntityTNTPinball(
                    worldObj, xCoord, yCoord, zCoord, facing, 0.125
                );
