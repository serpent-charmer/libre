...
public class BlockStructureDataStorage extends BlockDimensionStructureTile {
    ...
    public static abstract class StructureInterfaceTile extends TileEntity {
        ...
        public final void
        loadData(DimensionStructureGenerator gen, HashMap<String, Object> map) {
            if (gen == null)
                return;
            structureUID = gen.id;
        ...
        onDataLoad(DimensionStructureGenerator gen, HashMap<String, Object> map);
    ...
    public static class TileEntityStructurePassword
        extends StructureInterfaceTile implements InertIInv {
        ...
        protected void onRightClick(EntityPlayer ep, int s) {
            if (!this.checkPassword(ep
                ) /* && ChromaOptions.canPlayerUseStructureBypass(ep)*/) {
                ep.openGui(
                    ChromatiCraft.instance,
                    ChromaGuis.STRUCTUREPASS.ordinal(),
                    worldObj,
                    xCoord,
                    yCoord,
                    zCoord
        ...
        public String getInventoryName() {
            ...
            if (gen != null) {
                return gen.getType().getDisplayText() + " "
                    + ReikaStringParser.parseRomanRumeral(gen.getGenerationIndex())
                    + " Bypass";
        ...
        private boolean isValidItem(Item i) {
            return i instanceof ItemCrystalBasic
                || (i instanceof ItemBlock
                    && ((ItemBlock) i).field_150939_a instanceof CrystalTypeBlock);
        }
        ...
        public boolean checkPassword(EntityPlayer ep) {
            ...
            //ReikaJavaLibrary.pConsole(pass+" , "+given+" >
            //"+Arrays.toString(ReikaJavaLibrary.splitIntToHexChars(pass)));
    ...
    public static class TileEntityStructureDataStorage extends StructureInterfaceTile {
        ...
        public void readFromNBT(NBTTagCompound NBT) {
             ...
             */
            extraData = dat != null && !dat.hasNoTags()
                ? (HashMap<String, Object>) ReikaNBTHelper.getValue(dat)
                : null;
