...
public class BlockTeleport extends BlockDimensionStructure implements IWailaDataProvider {
    private static boolean debug
        = ReikaObfuscationHelper.isDeObfEnvironment() && DragonAPICore.isReikasComputer();
    ...
    private TileEntityTeleport getTileEntity(World world, int x, int y, int z) {
        ...
        if (meta == 0) {
            ...
            arr.recursiveAddWithBounds(
                world, x, y, z, this, x - 4, y - 4, z - 4, x + 4, y + 4, z + 4
            );
            int mx = arr.getMidX();
            int my = arr.getMinY();
            int mz = arr.getMidZ();
    ...
    public static class TileEntityTeleport
        ...
        private void teleport(Entity e) {
            ...
            if (destination == null) {
                if (debug)
                    ChromatiCraft.logger.logError(
                        "Could not teleport " + e + "; null destination!"
                    );
                return;
            }
            ...
            if (g != null) {
                Collection<TriggerCriteria> criteria
                    = g.getCriteria(xCoord, yCoord, zCoord);
                if (criteria != null) {
            ...
            if (e instanceof EntityPlayer) {
                ...
                byte a =
            (byte)(MathHelper.floor_float(((EntityPlayer)e).rotationYawHead*256.0F/360.0F));
                ((EntityPlayerMP)e).playerNetServerHandler.sendPacket(new
            S19PacketEntityHeadLook(e, a)); onTeleport((EntityPlayer)e, this);
            }
            else {
                e.setLocationAndAngles(nx, ny, nz, e.rotationYaw+yaw, e.rotationPitch);
            }
             */
            if (!(destination.xCoord == 0 && destination.yCoord == 0
                  && destination.zCoord == 0)) //self pos
        ...
        private float getYaw(Entity e) {
            int rel = ReikaDirectionHelper.getRelativeAngle(facing,
        destination.direction); if (rel > 180) rel = rel-360; return rel;
        }
         */
        ...
        private void onPlayerTeleported(EntityPlayer ep, TileEntityTeleport te) {
            HashMap<Coordinate, TeleportTriggerAction> actions
                = this.getGenerator().getActions(xCoord, yCoord, zCoord);
            //ReikaJavaLibrary.pConsole(new WorldLocation(this)+" from "+new
            //WorldLocation(te)+" from "+actions);
            if (actions != null && !actions.isEmpty()) {
                Coordinate rel = new Coordinate(te).offset(-xCoord, -yCoord, -zCoord);
                TeleportTriggerAction a = actions.get(rel);
                //ReikaJavaLibrary.pConsole(new Coordinate(this)+" from "+new
                //Coordinate(te)+" by "+rel+" to "+a+" from "+actions);
                if (a != null) {
                    a.trigger(rel, this);
                    //ReikaJavaLibrary.pConsole(new Coordinate(this)+" from "+new
                    //Coordinate(te)+", triggered "+a+", remaining "+actions);
                }
            }
        }
    ...
    private static void onTeleport(EntityPlayer ep, TileEntityTeleport te) {
        ...
        if (g == null) {
            ChromatiCraft.logger.logError(
                "Teleport block @ " + te.xCoord + ", " + te.yCoord + ", " + te.zCoord
                + " has no strucure with ID=" + te.uid
            );
            ChromatiCraft.logger.log(
                "Available Structures: " + DimensionStructureType.NONEUCLID.getUUIDs()
            );
            return;
        }
    ...
    public static class SameFacing implements TriggerCriteria {
        ...
        public boolean isValid(Entity e, TileEntityTeleport te) {
            if (e instanceof EntityLivingBase) {
                return ReikaEntityHelper.getDirectionFromEntityLook(
