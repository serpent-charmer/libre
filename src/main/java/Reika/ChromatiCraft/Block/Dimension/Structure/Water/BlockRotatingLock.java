...
public class BlockRotatingLock extends Block {
    ...
    public boolean onBlockActivated(
        ...
        if (!world.isRemote && this.hasTileEntity(world.getBlockMetadata(x, y, z)))
            ((TileEntityRotatingLock) world.getTileEntity(x, y, z))
                .startRotating(ep.isSneaking());
        return true;
    }
    ...
    public static class TileEntityRotatingLock
        ...
        private void updateHasFluidState() {
            ...
            if (gen != null) {
                ...
                if (f != null) {
                    boolean fluid = worldObj.getBlock(xCoord, yCoord + 1, zCoord)
                        == ChromaBlocks.EVERFLUID.getBlockInstance();
                    /*
                    if (fluid) {
                        TileEntityEverFluid te =
                    (TileEntityEverFluid)worldObj.getTileEntity(xCoord, yCoord+1, zCoord);
        ...
        private void startRotating(boolean reverse) {
            ...
            for (int i = -2; i <= 2; i++) {
                for (int k = -2; k <= 2; k++) {
                    if (worldObj.getBlock(xCoord + i, yCoord + 1, zCoord + k)
                        == ChromaBlocks.EVERFLUID.getBlockInstance()) {
            ...
            for (int i = -2; i <= 2; i++) {
                for (int k = -2; k <= 2; k++) {
                    if (i != 0 || k != 0) {
                        ...
                        if (b == Blocks.air
                            || b == ChromaBlocks.EVERFLUID.getBlockInstance()) {
                            worldObj.setBlock(
                                xCoord + i,
                                yCoord + 1,
                                zCoord + k,
        ...
        private void finishRotating(boolean reverse) {
            if (worldObj.isRemote)
                return;
            direction = reverse ? ReikaDirectionHelper.getLeftBy90(direction)
                                : ReikaDirectionHelper.getRightBy90(direction);
            worldObj.markBlockForUpdate(xCoord, yCoord, zCoord);
            ReikaSoundHelper.playBreakSound(
                worldObj, xCoord, yCoord, zCoord, Blocks.stone, 2, 0
            );
            WaterPuzzleGenerator gen = this.getGenerator();
            if (gen != null) {
                WaterFloor f = gen.getLevel(level);
            ...
            lockState
                = (FilledBlockArray) lockState.rotate90Degrees(xCoord, zCoord, reverse);
            lockState.place();
            //}
            ReikaWorldHelper.causeAdjacentUpdates(worldObj, xCoord, yCoord, zCoord);
        }
        ...
        public Collection<ForgeDirection> getOpenEndsForRender() {
            ...
            //ReikaJavaLibrary.pConsole(openEndsAtZero+" > "+ret, xCoord == -5035 &&
            //zCoord == -2992 && level == 0);
            return ret;
        }
