...
public class BlockEverFluid extends BlockContainer {
    ...
    public void updateTick(World world, int x, int y, int z, Random rand) {
        ...
        if (te != null && te.sourceLocation != null) {
            ...
            //b.recursiveMultiAddWithBounds(world, x, y, z, x-100, y-100, z-100, x+100,
            //y+100, z+100, this, ChromaBlocks.WATERLOCK.getBlockInstance());
    ...
    private void
    trySpread(World world, int x, int y, int z, TileEntityEverFluid te, Random rand) {
        if (this.flowIntoBlock(world, x, y, z, te, ForgeDirection.DOWN)) {
            //do not flow outwards
        } else if (world.getBlock(x, y - 1, z) != this) {
            for (int i = 2; i < 6; i++) {
                this.flowIntoBlock(
                    world, x, y, z, te, ForgeDirection.VALID_DIRECTIONS[i]
                );
            }
        }
    }
    ...
    private boolean flowIntoBlock(
        World world, int x, int y, int z, TileEntityEverFluid src, ForgeDirection dir
    ) {
        ...
        if (dy > 0 && y > 0 && dir == ForgeDirection.DOWN
            && world.getBlock(x, y - 1, z) == ChromaBlocks.WATERLOCK.getBlockInstance()) {
            dy--;
            placeSource = true;
        }
        if (this.canFlowInto(world, dx, dy, dz, src)) {
            ...
            TileEntityEverFluid te
                = (TileEntityEverFluid) world.getTileEntity(dx, dy, dz);
            if (!placeSource) {
    ...
    private boolean isBlockingBlock(World world, int x, int y, int z) {
        ...
        if (b instanceof BlockStructureShield
            || b == ChromaBlocks.WATERLOCK.getBlockInstance())
            return false;
        return true;
    }

    private boolean
    canFlowInto(IBlockAccess world, int x, int y, int z, TileEntityEverFluid te) {
        if (y < 0)
            return false;
    ...
    public void onBlockAdded(World world, int x, int y, int z) {
        ...
        world.playSoundEffect(
            ...
            z,
            "liquid.water",
            world.rand.nextFloat() * 0.25F + 0.75F,
    ...
    public static void placeSource(World world, int x, int y, int z) {
        ...
        ((TileEntityEverFluid) world.getTileEntity(x, y, z)).sourceLocation
