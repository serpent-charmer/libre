...
public class BlockRayblendFloor extends BlockDimensionStructureTile {
    ...
    public IIcon getIcon(int s, int meta) {
        return ChromaBlocks.SPECIALSHIELD.getBlockInstance().getIcon(s,
    ...
    @Override
    protected boolean onRightClicked(
        World world,
        int x,
        int y,
        int z,
    ...
    public static class TileEntityRayblendFloor
        ...
        public boolean allowsCrystalAt(int x, int z, CrystalElement e) {
            ...
            return g != null && !isBlocked
                && g.allowsCrystalAt(worldObj, puzzleID, x, z, e);
        }
        ...
        private void blockUpdate(World world, int x, int y, int z) {
            ...
            CrystalElement e
                = b instanceof CrystalTypeBlock ? CrystalElement.elements[meta] : null;
            if (e != null) {
                /*
                if (!g.allowsCrystalAt(puzzleID, x, z, e)) {
                    ReikaItemHelper.dropItem(world, x+0.5, y+1.5, z+0.5, new ItemStack(b,
                1, meta)); world.setBlockToAir(x, y+1, z);
        ...
        public void
        populate(UUID uid, UUID grid, int x, int z, boolean blocked, CrystalElement e) {
            puzzleID = uid;
            gridID = grid;
            tileX = x;
            tileZ = z;
