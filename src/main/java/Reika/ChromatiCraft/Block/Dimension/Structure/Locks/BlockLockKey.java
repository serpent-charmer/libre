...
public class BlockLockKey extends Block {
    ...
    public BlockLockKey(Material mat) {
        ...
        this.setCreativeTab(
            DragonAPICore.isReikasComputer() ? ChromatiCraft.tabChromaGen : null
        );
    ...
    getPlayerRelativeBlockHardness(EntityPlayer ep, World world, int x, int y, int z) {
        ...
        if (te.delegate != null) {
            ...
            if (te2 instanceof TileEntityStructControl) {
                if (((TileEntityStructControl) te2)
                        .isInaccessible(world, x, y, z, te, ep))
                    return -1;
            }
        }
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random r) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityFX fx = new EntitySparkleFX(
                              world,
                              x + r.nextDouble(),
                              y + r.nextDouble(),
                              z + r.nextDouble(),
    ...
    public void breakBlock(World world, int x, int y, int z, Block b2, int meta2) {
        if (!world.isRemote) {
            ...
            if (te.delegate != null) {
                TileEntity te2 = te.delegate.getTileEntity(world);
                if (te2 instanceof TileEntityStructControl) {
                    ((TileEntityStructControl) te2)
                        .onDelegatedTileRemoved(world, x, y, z, te);
                }
            }
        }
    ...
    private static LocksGenerator getGenerator(World world, int x, int y, int z) {
        return (LocksGenerator) DimensionStructureType.LOCKS.getGenerator(
            ((TileEntityLockKey) world.getTileEntity(x, y, z)).uid
        );
    }
    ...
    public static class TileEntityLockKey
        extends StructureBlockTile<LocksGenerator> implements InteractionDelegateTile {
