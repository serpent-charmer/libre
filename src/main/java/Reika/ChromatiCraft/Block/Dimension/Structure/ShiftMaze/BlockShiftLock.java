...
public class BlockShiftLock
    extends BlockDimensionStructure implements IWailaDataProvider {
    ...
    public static enum Passability {
        ...
        public static Passability
        getDirectionalPassability(ForgeDirection dir, boolean open) {
            switch (dir) {
                case EAST:
    ...
    public float getBlockHardness(World world, int x, int y, int z) {
        ...
        return meta == Passability.BREAKABLE.ordinal()
            ? 1
    ...
    public void addCollisionBoxesToList(
        ...
        if (e.boundingBox.intersectsWith(box
            )) //entity inside block space, you can go through
            return;
    ...
    public boolean shouldSideBeRendered(IBlockAccess iba, int dx, int dy, int dz, int s) {
        return super.shouldSideBeRendered(iba, dx, dy, dz, s)
            && iba.getBlock(dx, dy, dz) != this;
    }
    ...
    public ItemStack getWailaStack(IWailaDataAccessor acc, IWailaConfigHandler config) {
        ...
        if (mov != null) {
            ...
            if (Passability.list[acc.getMetadata()].isDisguised(mov.sideHit))
                return ChromaBlocks.STRUCTSHIELD.getStackOfMetadata(
