...
public class BlockGOLTile extends BlockDimensionStructure {
    ...
    private void toggle(World world, int x, int y, int z) {
        ...
        if (te.getGenerator() == null) {
            ChromatiCraft.logger.logError(
                "No structure for tile at " + x + ", " + y + ", " + z + ", UID=" + te.uid
                + "!?!"
            );
            return;
        }
        ...
        if (te.isActive()) {
            ReikaSoundHelper.playSoundAtBlock(
                world, x, y, z, "random.click", 0.75F, 0.65F
            );
            ...
            if (flag)
                ReikaSoundHelper.playSoundAtBlock(
                    world, x, y, z, "random.click", 0.75F, 0.75F
                );
            else
                ChromaSounds.ERROR.playSoundAtBlock(world, x, y, z);
        }
    ...
    public static class GOLTile extends StructureBlockTile<GOLGenerator> {
        ...
        private void scheduleRecalculation(int ticks) {
            worldObj.scheduleBlockUpdate(xCoord, yCoord, zCoord, this.getBlockType(),
        ticks);
        }
         */
        ...
        private void setActive(boolean ac) {
            ...
            if (ac) {
                worldObj.setBlockMetadataWithNotify(
                    xCoord, yCoord + GOLGenerator.ROOM_HEIGHT, zCoord, 3, 3
                );
            }
