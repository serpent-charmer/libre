...
public class BlockAntKey extends BlockDimensionStructureTile {
    ...
    public boolean onRightClicked(
        ...
        ItemStack is = ((AntKeyTile) world.getTileEntity(x, y, z))
                           .onRightClickWith(ep.getCurrentEquippedItem(), ep);
        ep.setCurrentItemOrArmor(0, is);
        ReikaSoundHelper.playSoundAtBlock(world, x, y, z, "random.pop", 1, 1);
        return true;
    }

    public static class AntKeyTile extends StructureBlockTile<AntFarmGenerator>
        implements InertIInv, ItemOnRightClick {
        private int invSize;
        ...
        public ItemStack onRightClickWith(ItemStack item, EntityPlayer ep) {
            ...
            for (int i = incr ? 0 : invSize - 1; incr ? i < invSize : i >= 0;
                 i += incr ? 1 : -1) {
                boolean flag = false;
        ...
        public void markDirty() {
            ...
            for (int i = 0; i < invSize; i++) {
                items[i] = inv[i] != null ? new InertItem(
                               worldObj, ReikaItemHelper.getSizedItemStack(inv[i], 1)
                           )
                                          : null;
            }
        }
