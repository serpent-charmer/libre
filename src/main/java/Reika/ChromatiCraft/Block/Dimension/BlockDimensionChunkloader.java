...
public class BlockDimensionChunkloader extends Block implements MinerBlock {
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ReikaParticleHelper p = rand.nextInt(3) == 0
            ? ReikaParticleHelper.WITCH
            : ReikaParticleHelper.AMBIENTMOBSPELL;
    ...
    onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase e, ItemStack is) {
        if (world.provider.dimensionId == ExtraChromaIDs.DIMID.getValue()) {
            ...
            if (te instanceof TileEntityDimensionChunkloader) {
                ...
                String s = "Missing TileEntity for CC dimension chunkloader @ DIM"
    ...
    public static class TileEntityDimensionChunkloader
        extends TileEntity implements ChunkLoadingTile {
        ...
        public void updateEntity() {
            ...
            if (ep != null) {
                ...
                ChromatiCraft.logger.log(
                    "Activating chunkloader crystal " + new WorldLocation(this)
                    + " placed by " + placer
                );
                deletionTimer = TIMER;
            }
        ...
        private void delete(String reason) {
            ChromatiCraft.logger.log(
                "Deleting chunkloader crystal placed by " + placer + ": " + reason
            );
