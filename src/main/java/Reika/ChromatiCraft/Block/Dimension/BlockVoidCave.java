        ...
        for (int i = 2; i < 6; i++) {
            if ((meta & (1 << (i - 2))) != 0) {
                ...
                for (int n = 0; n < 16; n++) {
                    ...
                    int base = ReikaColorAPI.mixColors(
                        0xffffff, 0x22aaff, rand.nextFloat() * 0.5F
                    );
                    base = ReikaColorAPI.getModifiedHue(
                        base,
                        ReikaRandomHelper.getRandomPlusMinus(
                            ReikaColorAPI.getHue(base), 30
                        )
                    );
                    int c = ReikaColorAPI.getColorWithBrightnessMultiplier(
                        base, (float) ReikaRandomHelper.getRandomBetween(0.5, 1)
                    );
