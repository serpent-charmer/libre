...
public class BlockLiquidLumen extends BlockFluidClassic {
    private static final NoiseGeneratorBase hueNoise
        = new SimplexNoiseGenerator(System.currentTimeMillis()).setFrequency(1 / 32D);
    private static final NoiseGeneratorBase saturationNoise
        = new SimplexNoiseGenerator(-System.currentTimeMillis()).setFrequency(1 / 20D);
    ...
    public void registerBlockIcons(IIconRegister ico) {
        theIcon = new IIcon[] { ico.registerIcon("chromaticraft:fluid/lumen"),
                                ico.registerIcon("chromaticraft:fluid/flowinglumen") };
    }
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random r) {
        if (r.nextInt(15) == 0) {
            ...
            EntityCCBlurFX fx
                = new EntityCCBlurFX(world, px, y + 0.9375 - s / 16, pz, vx, vy, vz);
            fx.setColor(this.getColor(px, pz)).setGravity(g).setScale(s).setLife(l);
            fx.setIcon(ChromaIcons.FADE_GENTLE.getIcon())
                .setAlphaFading()
                .setColorController(particleColor);
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
        }
        if (r.nextInt(150) == 0) {
            double px = x + r.nextDouble();
            double pz = z + r.nextDouble();
            EntityBlurFX fx = new EntityCCBlurFX(world, px, y + 0.9375, pz, 0, 0.75, 0)
                                  .setIcon(ChromaIcons.SPARKLEPARTICLE)
                                  .setColor(this.getColor(px, pz))
                                  .setScale(5F)
                                  .setLife(120)
                                  .setBasicBlend();
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
    ...
    public static int getColor(double x, double z) {
        int hue
            = (int) ReikaMathLibrary.normalizeToBounds(hueNoise.getValue(x, z), 195, 310);
        float sat = Math.min(
            0.75F,
            (float) ReikaMathLibrary.normalizeToBounds(
                saturationNoise.getValue(x, z), 0.25, 1.5
            )
        );
        return ReikaColorAPI.getModifiedSat(
            ReikaColorAPI.getModifiedHue(0xff0000, hue), sat
        );
    }
