...
public class BlockColoredAltar extends CrystalTypeBlock {
    ...
    public static class TileEntityColoredAltar
        extends TileEntity implements RenderFetcher {
        private int ticks;
        ...
        public CrystalElement getColor() {
            return worldObj != null ? CrystalElement.elements[this.getBlockMetadata()]
                                    : renderColor;
        }
        ...
        private void doAoEFX() {
            ...
            if (!otherAltars.isEmpty() && f > 0.0125F / 25 && rand.nextFloat() < f) {
                ...
                if (rand.nextFloat() < f) {
                    EntityPlayer ep
                        = worldObj.getClosestPlayer(xCoord, yCoord, zCoord, 12);
                    if (ep != null) {
                        //ChromaAux.dischargeIntoPlayer(xCoord+0.5, yCoord+0.5,
                        //zCoord+0.5, rand, ep, e, 1, 1);
                        ReikaPacketHelper.sendDataPacket(
                            ChromatiCraft.packetChannel,
                            ChromaPackets.ALTARSHOCK.ordinal(),
                            PacketTarget.server,
                            xCoord,
                            yCoord,
                            zCoord,
                ...
                if (f > 0.0006F && rand.nextFloat() < f2) {
                    ...
                    //PacketIDs.SETAIR.ordinal(), PacketTarget.server, xCoord, yCoord,
                    //zCoord);
                    ReikaPacketHelper.sendDataPacket(
                        ChromatiCraft.packetChannel,
                        ChromaPackets.ALTARBREAK.ordinal(),
                        PacketTarget.server,
                        xCoord,
                        yCoord,
                        zCoord
                    ...
                    for (double a1 = 0; a1 < 360; a1 += da) {
                        for (double a2 = -90 + da; a2 < 90 - da; a2 += da) {
                            double v
                                = ReikaRandomHelper.getRandomPlusMinus(0.125, 0.0625);
                        ...
                        if (i != 0 || j != 0 || k != 0) {
                            ...
                            if (worldObj.getBlock(dx, dy, dz) == b) {
                                ...
                                float f = MathHelper.clamp_float(
                                    4
                                        - c.getTaxicabDistanceTo(xCoord, yCoord, zCoord)
                                            / 6,
                                    1,
                                    4
                                );
        ...
        //Have particles (blur) spill over the rim, use some more to make a pulsing glow
        //effect in the middle, and then make some laser ones lazily float up
        @SideOnly(Side.CLIENT)
        private void renderParticles(CrystalElement e) {
            ...
            double d = Math.max(
                1,
                Minecraft.getMinecraft().thePlayer.getDistanceSq(
                    xCoord + 0.5, yCoord + 0.25, zCoord + 0.5
                ) / 256D
            );
            n /= d;
            ...
            for (int i = 0; i < num; i++) {
                ...
                EntityFX fx = new EntityCCBlurFX(e, worldObj, dx, dy, dz, 0, 0, 0)
                                  .setGravity(g)
                                  .setLife(l)
                                  .setScale(s)
                                  .setColliding(dang);
            ...
            if (rand.nextInt(2) == 0) {
                ...
                if (e == CrystalElement.WHITE || e == CrystalElement.BLACK
                    || e == CrystalElement.GRAY || e == CrystalElement.LIGHTGRAY) {
                    int avg = (r + g + b) / 3;
                    r = g = b = avg;
                }

                EntityFX fx = new EntityCCBlurFX(worldObj, dx, dy, dz, 0, 0, 0)
                                  .setGravity(0)
                                  .setLife(l)
                                  .setScale(s)
                                  .setColor(r, g, b);
            ...
            if (rand.nextInt(8) == 0) {
                ...
                float s = (float) ReikaRandomHelper.getRandomPlusMinus(1.5F, 1F);
                float g = -(float) ReikaRandomHelper.getRandomPlusMinus(0.0625, 0.03125);
                EntityFX fx = new EntityLaserFX(e, worldObj, dx, dy, dz, 0, 0, 0)
                                  .setGravity(g)
                                  .setScale(s);
                Minecraft.getMinecraft().effectRenderer.addEffect(fx);
            }
        ...
        public AxisAlignedBB getRenderBoundingBox() {
            return AxisAlignedBB.getBoundingBox(
                xCoord - 0.5, yCoord, zCoord - 0.5, xCoord + 1.5, yCoord + 3, zCoord + 1.5
            );
        }
    ...
    addDestroyEffects(World world, int x, int y, int z, int meta, EffectRenderer eff) {
        return ReikaRenderHelper.addModelledBlockParticles(
            "/Reika/ChromatiCraft/Textures/TileEntity/",
            world,
            x,
            y,
            z,
            this,
            eff,
            ReikaJavaLibrary.makeListFrom(new double[] { 0, 0, 1, 1 }),
            ChromatiCraft.class
        );
    }
    ...
    @SideOnly(Side.CLIENT)
    public final boolean
    addHitEffects(World world, MovingObjectPosition tg, EffectRenderer eff) {
        return ReikaRenderHelper.addModelledBlockParticles(
            "/Reika/ChromatiCraft/Textures/TileEntity/",
            world,
            tg,
