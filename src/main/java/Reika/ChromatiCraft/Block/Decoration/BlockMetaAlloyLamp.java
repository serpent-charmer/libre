...
public class BlockMetaAlloyLamp extends Block implements SidedBlock {
    ...
    public int getLightValue(IBlockAccess world, int x, int y, int z) {
        return this.hasPod(world, x, y, z) ? ModList.COLORLIGHT.isLoaded()
                ? ReikaColorAPI.getPackedIntForColoredLight(0x000000, 15)
                : 15
                                           : 0;
    }
    ...
    private void setPod(World world, int x, int y, int z, boolean pod) {
        world.setBlockMetadataWithNotify(
            x, y, z, (world.getBlockMetadata(x, y, z) & 7) | (pod ? 0 : 8), 3
        );
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        if (this.hasPod(world, x, y, z) && rand.nextBoolean()) {
            ...
            int c = ReikaColorAPI.getColorWithBrightnessMultiplier(
                ReikaColorAPI.mixColors(0x006020, 0x000060, rand.nextFloat()),
                0.5F + rand.nextFloat() * 0.5F
            );
            EntityFX fx = new EntityCCBlurFX(world, px, py, pz)
                              .setIcon(ChromaIcons.FADE_CLOUD)
                              .setColor(c)
                              .setLife(l)
                              .setScale((float) s)
                              .setAlphaFading();
    ...
    public void setSide(World world, int x, int y, int z, int side) {
        world.setBlockMetadataWithNotify(
            x, y, z, (world.getBlockMetadata(x, y, z) & 8) | side, 3
        );
        world.markBlockForUpdate(x, y, z);
    }
    ...
    public boolean canPlaceOn(World world, int x, int y, int z, int side) {
        ...
        return b.isSideSolid(world, x, y, z, ForgeDirection.VALID_DIRECTIONS[side])
            && (b.getMaterial() == Material.rock || b.getMaterial() == Material.ground
