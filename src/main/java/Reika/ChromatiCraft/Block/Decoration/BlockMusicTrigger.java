...
public class BlockMusicTrigger extends Block implements SemiUnbreakable {
    ...
    private void pingCallback(
        World world, int x, int y, int z, CrystalElement e, int idx, EntityPlayer ep
    ) {
        ...
        for (int i = 2; i < 6 && te == null; i++) {
            for (int d = 4; d <= 5; d++) {
                ...
                //ReikaJavaLibrary.pConsole(BlockKey.getAt(world, dx, dy, dz)+" @
                //"+dir+"x"+d+" @ "+new Coordinate(dx, dy, dz));
        ...
        if (te != null) {
            te.onMusicTrigger(
                world, x, y, z, e, CrystalMusicManager.instance.getKeys(e).get(idx), ep
            );
        }
    }
    ...
    public static void
    createParticle(World world, int x, int y, int z, CrystalElement e) {
    ...
    private int getIndex(int s, float a, float b, float c) { //0-3 or -1 for none
        ...
        if (s == 4 || s == 5) { //a == 0 or 1
            if (ReikaMathLibrary.isValueInsideBoundsIncl(m1a, m1b, c)
                && ReikaMathLibrary.isValueInsideBoundsIncl(m2a, m2b, b))
                return 0;
            if (ReikaMathLibrary.isValueInsideBoundsIncl(m2a, m2b, c)
                && ReikaMathLibrary.isValueInsideBoundsIncl(m2a, m2b, b))
                return 1;
            if (ReikaMathLibrary.isValueInsideBoundsIncl(m2a, m2b, c)
                && ReikaMathLibrary.isValueInsideBoundsIncl(m1a, m1b, b))
                return 2;
            if (ReikaMathLibrary.isValueInsideBoundsIncl(m1a, m1b, c)
                && ReikaMathLibrary.isValueInsideBoundsIncl(m1a, m1b, b))
                return 3;
        } else if (s == 2 || s == 3) { //c == 0 or 1
            if (ReikaMathLibrary.isValueInsideBoundsIncl(m1a, m1b, a)
                && ReikaMathLibrary.isValueInsideBoundsIncl(m2a, m2b, b))
                return 0;
            if (ReikaMathLibrary.isValueInsideBoundsIncl(m2a, m2b, a)
                && ReikaMathLibrary.isValueInsideBoundsIncl(m2a, m2b, b))
                return 1;
            if (ReikaMathLibrary.isValueInsideBoundsIncl(m2a, m2b, a)
                && ReikaMathLibrary.isValueInsideBoundsIncl(m1a, m1b, b))
