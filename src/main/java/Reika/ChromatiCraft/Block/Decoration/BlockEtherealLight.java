...
public class BlockEtherealLight extends Block {
    ...
    public void updateTick(World world, int x, int y, int z, Random r) {
        if ((Flags.SLOWDECAY.isPresent(world, x, y, z) && r.nextInt(5) == 0)
            || Flags.FASTDECAY.isPresent(world, x, y, z)) {
    ...
    public void onBlockAdded(World world, int x, int y, int z) {
        if (Flags.FASTDECAY.isPresent(world, x, y, z)) {
            world.scheduleBlockUpdate(
                x, y, z, this, ReikaRandomHelper.getRandomBetween(15, 30)
            );
        }
    }
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block n) {
        if (Flags.SLOWDECAY.isPresent(world, x, y, z)) {
            world.scheduleBlockUpdate(
                x, y, z, this, ReikaRandomHelper.getRandomBetween(40, 200)
            );
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        if (Flags.PARTICLES.isPresent(world, x, y, z)) {
            if (rand.nextInt(2) == 0) {
                ...
                EntityBlurFX fx = new EntityCCBlurFX(world, rx, ry, rz, vx, 0, vz)
                                      .setGravity(gv)
                                      .setLife(l)
                                      .setScale(1.5F)
                                      .setColor(r, g, b)
    ...
    public static int getParticleColor(World world, int y) {
        return ReikaColorAPI.getModifiedHue(
            0xff0000, (int) (360D * y / world.provider.getAverageGroundLevel())
        );
    }
