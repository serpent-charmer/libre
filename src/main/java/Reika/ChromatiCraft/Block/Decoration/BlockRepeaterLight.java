...
public class BlockRepeaterLight extends Block {
    public static final ChromaTiles[] MODELS = { ChromaTiles.REPEATER,
                                                 ChromaTiles.COMPOUND,
                                                 ChromaTiles.BROADCAST,
    ...
    public void randomDisplayTick(World world, int x, int y, int z, Random rand) {
        ...
        if (MODELS[world.getBlockMetadata(x, y, z)] == ChromaTiles.COMPOUND
            && rand.nextInt(8) == 0) {
            EntityRuneFX fx
                = new EntityRuneFX(
                      world, x + 0.5, y + 0.5, z + 0.5, CrystalElement.randomElement()
                )
                      .setScale(5)
                      .setFading()
                      .setGravity(0);
