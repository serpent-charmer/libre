...
public class ChromaHelpData {
    ...
    private ChromaHelpData() {
        ...
        for (int i = 0; i < TieredOres.list.length; i++) {
            this.addKey(
                ChromaBlocks.TIEREDORE,
                i,
                "ore_" + TieredOres.list[i].name().toLowerCase(Locale.ENGLISH)
            );
        }

        for (int i = 0; i < TieredPlants.list.length; i++) {
            this.addKey(
                ChromaBlocks.TIEREDPLANT,
                i,
                "plant_" + TieredPlants.list[i].name().toLowerCase(Locale.ENGLISH)
            );
        }

        for (int i = 0; i < Flowers.list.length; i++) {
            this.addKey(
                ChromaBlocks.DECOFLOWER,
                i,
                "flower_" + Flowers.list[i].name().toLowerCase(Locale.ENGLISH)
            );
        }
    ...
    public String getText(World world, int x, int y, int z) {
        HelpKey hk
            = this.getKey(world.getBlock(x, y, z), world.getBlockMetadata(x, y, z));
