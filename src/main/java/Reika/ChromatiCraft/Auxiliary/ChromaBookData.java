...
public class ChromaBookData {
        ...
        if (h.isCrafting()) {
            ...
            if (!li.isEmpty()) {
                ...
                if (h.getTitle().isEmpty()) {
                    fr.drawString(
                        c.getOutputForDisplay().getDisplayName(), posX + 6, posY - 2, 0
                    );
                }
            }
        }
    ...
    public static void drawCastingRecipe(
        FontRenderer fr,
        RenderItem ri,
        CastingRecipe c,
        int subpage,
        ...
        ItemStack ctr = c instanceof MultiBlockCastingRecipe
            ? ((MultiBlockCastingRecipe) c).getMainInput()
            : c.getArrayForDisplay()[4];
        GL11.glPushAttrib(GL11.GL_ALL_ATTRIB_BITS);
        if (ctr != null && isout.stackTagCompound != null)
            ReikaNBTHelper.combineNBT(
                isout.stackTagCompound,
                c.getOutputTag(Minecraft.getMinecraft().thePlayer, ctr.stackTagCompound)
            );
            ...
            for (int i = 0; i < 9; i++) {
                ...
                if (in != null) {
                    if (i == 4 && c instanceof MultiBlockCastingRecipe)
                        in = ReikaItemHelper.getSizedItemStack(
                            in,
                            ((MultiBlockCastingRecipe) c).getRequiredCentralItemCount()
                        );
                    ...
                    //FontRenderer fr2 =
                    //in.getDisplayName().contains(FontType.OBFUSCATED.id) ?
                    //FontType.OBFUSCATED.renderer : fr;
        ...
        if (subpage == 1) {
            RuneShapeRenderer.instance.render(
                ((TempleCastingRecipe) c).getRunes(), posX + 128, posY + 110
            );
        }
        if (subpage == 2) {
            Map<List<Integer>, ItemMatch> items
                = ((MultiBlockCastingRecipe) c).getAuxItems();
            for (List<Integer> key : items.keySet()) {
                ...
                if (out.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
                    List<ItemStack> dmg
                        = ReikaItemHelper.getAllMetadataPermutations(out.getItem());
                    if (System.currentTimeMillis() % 1000 == 0) {
                        for (int f = 0; f < permuOffset.length; f++) {
                            for (int g = 0; g < permuOffset[f].length; g++) {
                                permuOffset[f][g]
                                    = ReikaRandomHelper.getRandomPlusMinus(0, 16);
                            }
                        }
                    }
                    out = dmg.get((int
                    ) ((System.currentTimeMillis() / 1000
                        + permuOffset[i / 2 + 2][k / 2 + 2])
                       % dmg.size()));
                }
        ...
        if (subpage == 3) {
            ...
            for (CrystalElement e : tag.elementSet()) {
                ...
                int y1 = posY + 26;
                //int y = posY+26+h2;
                //gui.drawRect(x, y1, x+w, y,
                //e.getJavaColor().darker().darker().getRGB()); gui.drawRect(x, y1+dy,
                //x+w, y, e.getColor());
            ...
            for (CrystalElement e : p.getElements()) {
                ...
                ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                    ix - si / 2, iy - si / 2, e.getOutlineRune(), si, si
                );
                ba += ang;
            }
            ...
            f.drawSplitString(
                String.valueOf(tot), px + 3 + 40, py - 5 + 3 + h, 80, 0xffffff
            );
            ...
            int frac = (int
               ...
               * Math.pow(
                   (double) tot
                       / RecipesCastingTable.instance.getMaxRecipeTotalEnergyCost(),
                   0.5
               ));
            ...
            for (int i = 0; i < 4; i++)
                ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                    px + 11, py + 108 - frac + 3, ChromaIcons.RIFT.getIcon(), 16, frac + 6
                );
            ...
            ReikaGuiAPI.instance.drawTexturedModalRect(
                px + 1, py + 108 - frac, 0, 118, 36, 7
            );
        }
        if (subpage == 0 && isBook
            && ChromaResearch.DOUBLECRAFT.playerHas(Minecraft.getMinecraft().thePlayer)) {
            ReikaTextureHelper.bindFinalTexture(
                ChromatiCraft.class, "Textures/GUIs/Handbook/misc.png"
            );
            gui.drawTexturedModalRect(
                posX + 10, posY + 31, 224, c.canGiveDoubleOutput() ? 32 : 64, 32, 32
            );
            if (gui.isMouseInBox(posX + 10, posX + 42, posY + 31, posY + 63)) {
                ReikaTextureHelper.bindFinalTexture(
                    ChromatiCraft.class, "Textures/Ability/doublecraft.png"
                );
                GL11.glPushMatrix();
                double s = 0.1;
                GL11.glScaled(s, s, s);
                gui.drawTexturedModalRect(
                    (int) ((gui.getMouseRealX() + 10) / s),
                    (int) ((gui.getMouseRealY() + 10) / s),
                    0,
                    0,
                    256,
                    256
    ...
    public static void drawRecipeMissingProgress(
        CastingRecipe r,
        EntityPlayer player,
        ...
        li.removeIf(
            e -> e instanceof ProgressAccess && ((ProgressAccess) e).playerHas(player)
        );
        if (showMissingTuning && r instanceof TempleCastingRecipe
            && ((TempleCastingRecipe) r).requiresTuningKey()) {
        ...
        if (!li.isEmpty()) {
            ...
            int dx = x0 - w / 2;
            ReikaGuiAPI.instance.drawRect(
                dx - gap, y0 - gap, w + gap, h + gap, 0x000000, false
            );
            ReikaGuiAPI.instance.drawRectFrame(
                dx - gap, y0 - gap, w + gap, h + gap, 0x22aaff
            );
    ...
    public static void drawCompressedCastingRecipe(
        FontRenderer fr, RenderItem ri, CastingRecipe c, int posX, int posY
    ) {
        GL11.glDisable(GL11.GL_LIGHTING);
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (in != null) {
                if (i == 4 && c instanceof MultiBlockCastingRecipe)
                    in = ReikaItemHelper.getSizedItemStack(
                        in, ((MultiBlockCastingRecipe) c).getRequiredCentralItemCount()
                    );
                int x = 47;
                int y = 96;
                int dx = x + posX + i % 3 * 18;
                int dy = y + posY + i / 3 * 18;
                //FontRenderer fr2 = in.getDisplayName().contains(FontType.OBFUSCATED.id)
                //? FontType.OBFUSCATED.renderer : fr;
                gui.drawItemStackWithTooltip(ri, fr, in, dx, dy);
            }
        }
        ...
        if (c instanceof PylonCastingRecipe) {
            ...
            for (CrystalElement e : tag.elementSet()) {
                ...
                //int y1 = posY+26+h2;
                //gui.drawRect(x, y, x+h2, y+w,
                //e.getJavaColor().darker().darker().getRGB()); gui.drawRect(x, y, x+ht,
                //y+w, e.getColor());
    ...
    public static void drawPoolRecipe(
        FontRenderer fr, RenderItem ri, PoolRecipe r, int subpage, int posX, int posY
    ) {
        rand.setSeed(System.currentTimeMillis() / 1000);
        rand.nextBoolean();
        ...
        gui.drawTexturedModelRectFromIcon(
            posX + 196, posY + 81, ChromatiCraft.chroma.getIcon(), 36, 36
        );
