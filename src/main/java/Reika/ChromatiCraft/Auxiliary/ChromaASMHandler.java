...
public class ChromaASMHandler implements IFMLLoadingPlugin {
    ...
    public static class ASMExecutor implements IClassTransformer {
        ...
        private static enum ClassPatch implements ASMEnum {
            ...
            //CHARWIDTH("Reika.ChromatiCraft.Auxiliary.ChromaFontRenderer"), //Thank you,
            //Optifine T_T CHUNKPOPLN("net.minecraft.world.gen.ChunkProviderServer",
            //"ms"), WORLDLIGHT("net.minecraft.world.World", "ahb"),
            ...
            TEXTURELOAD(
                "net.minecraft.client.renderer.texture.TextureAtlasSprite", "bqd"
            ),
                ...
                switch (this) {
                    case ENDPROVIDER: {
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "func_73187_a", "initializeNoiseField", "([DIIIIII)[D"
                        );
                        String func = FMLForgePlugin.RUNTIME_DEOBF ? "func_76129_c"
                                                                   : "sqrt_float";
                        AbstractInsnNode loc = ReikaASMHelper.getFirstMethodCall(
                            cn, m, "net/minecraft/util/MathHelper", func, "(F)F"
                        );
                        ...
                        call.add(new FieldInsnNode(
                            Opcodes.GETFIELD,
                            ReikaASMHelper.convertClassName(cn, false),
                            FMLForgePlugin.RUNTIME_DEOBF ? "field_73200_m" : "endWorld",
                            "Lnet/minecraft/world/World;"
                        ));
                        call.add(new VarInsnNode(Opcodes.FLOAD, 23)
                        ); //+1 to all the arguments because of double_2nd somewhere lower
                           //on stack
                        ...
                        call.add(new MethodInsnNode(
                            ...
                            "Reika/ChromatiCraft/Auxiliary/ChromaAux",
                            "getIslandBias",
                            "(Lnet/minecraft/world/World;FFFII)F",
                            false
                        ));
                        call.add(new VarInsnNode(Opcodes.FSTORE, 23));
                        m.instructions.insert(loc, call);
                        ReikaASMHelper.log(
                            "Successfully applied " + this + " ASM handler!"
                        );
                        break;
                    }
                    case REACHDIST: {
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "func_78757_d", "getBlockReachDistance", "()F"
                        );
                        AbstractInsnNode ret = ReikaASMHelper.getLastOpcode(
                            m.instructions, Opcodes.FRETURN
                        );
                        m.instructions.insertBefore(
                            ret, new VarInsnNode(Opcodes.ALOAD, 0)
                        );
                        m.instructions.insertBefore(
                            ret,
                            new MethodInsnNode(
                                Opcodes.INVOKESTATIC,
                                "Reika/ChromatiCraft/Auxiliary/ChromaAux",
                                "getReachDistanceClient",
                                "(FLnet/minecraft/client/multiplayer/PlayerControllerMP;)F",
                                false
                            )
                        );
                        break;
                 ...
                 Class.forName("optifine.OptiFineClassTransformer");
                         ReikaASMHelper.log("Optifine loaded. Editing FontRenderer
                 class.");
                     }
                     catch (Exception e) {
                         ReikaASMHelper.log("Optifine loaded. Not editing FontRenderer
                 class."); break;
                     }
                     /*
                     //FieldNode fn = ReikaASMHelper.getFieldByName(cn, "charWidth");
                     //cn.fields.remove(fn);
                     String field = FMLForgePlugin.RUNTIME_DEOBF ? "field_78286_d" :
                 "charWidth"; int count = 0; boolean primed = false; for (MethodNode m :
                 cn.methods) { for (int i = 0; i < m.instructions.size(); i++) {
                             ...
                             if (ain instanceof FieldInsnNode) {
                                 ...
                                 if (fin.name.equals(field) && fin.desc.equals("[I")) {
                                     fin.desc = "[F";
                                     count++;
                                     ReikaASMHelper.log("Successfully applied "+this+" ASM
                 handler x"+count+"!"); primed = true;
                                 }
                                 /*
                                 if (FMLForgePlugin.RUNTIME_DEOBF &&
                 fin.name.equals("charWidth")) { fin.name = "field_78286_d"; fin.owner =
                 "net/minecraft/client/gui/FontRenderer";
                                 }*/
                    /*
             }
             else if (primed && (ain.getOpcode() == Opcodes.IALOAD || ain.getOpcode() ==
     Opcodes.IASTORE)) { if (ain.getOpcode() == Opcodes.IALOAD)
  ...
  break;
       ...
       case WORLDLIGHT:
       case WORLDLIGHT2: {
           MethodNode m = ReikaASMHelper.getMethodByName(cn, "func_72802_i",
       "getLightBrightnessForSkyBlocks", "(IIII)I"); m.instructions.clear();
       ...
       "Reika/ChromatiCraft/Auxiliary/ChromaAux", "overrideLightValue",
       "(Lnet/minecraft/world/IBlockAccess;IIII)I", false)); m.instructions.add(new
       InsnNode(Opcodes.IRETURN)); ReikaASMHelper.log("Successfully applied "+this+" ASM
       handler!"); break;
       }
    */
                    /*
                    case ENTITYCOLLISION: {
                        String func = FMLForgePlugin.RUNTIME_DEOBF ? "func_72945_a" :
                    "getCollidingBoundingBoxes"; String sig =
                    "(Lnet/minecraft/entity/Entity;Lnet/minecraft/util/AxisAlignedBB;)Ljava/util/List;";
                        MethodNode m = ReikaASMHelper.getMethodByName(cn, "func_70091_d",
                    "moveEntity", "(DDD)V"); for (int i = 0; i < m.instructions.size();
                    i++) { AbstractInsnNode ain = m.instructions.get(i); if
                    (ain.getOpcode() == Opcodes.INVOKEVIRTUAL) { MethodInsnNode min =
                    (MethodInsnNode)ain; if (min.owner.contains("World") &&
                    min.name.equals(func) && min.desc.equals(sig)) {

                                    min.owner = "Reika/ChromatiCraft/Auxiliary/ChromaAux";
                                    min.name = "interceptEntityCollision";
                                    //min.desc = sig;
                                    ReikaASMHelper.changeOpcode(min,
                    Opcodes.INVOKESTATIC);

                                    //Remove ALOAD 0 and GETFIELD worldObj
                                    AbstractInsnNode world =
                    ReikaASMHelper.getLastInsnBefore(m.instructions,
                    m.instructions.indexOf(min), Opcodes.GETFIELD,
                    "net/minecraft/entity/Entity", "worldObj",
                    "Lnet/minecraft/world/World;"); AbstractInsnNode end = world;
                                    AbstractInsnNode start = end.getPrevious();
                                    /*
                                    ArrayList<AbstractInsnNode> li = new ArrayList();
                                    for (int k = m.instructions.indexOf(start); k <=
                    m.instructions.indexOf(end); k++) { li.add(m.instructions.get(k));
                                    }
                                    li.add(min);
                    ...
                    case COLLISIONBOXES: {
                        String sig
                            = "(Lnet/minecraft/entity/Entity;Lnet/minecraft/util/AxisAlignedBB;)Ljava/util/List;";
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "func_72945_a", "getCollidingBoundingBoxes", sig
                        );
                        if (m.attrs != null)
                            m.attrs.clear();
                        ...
                        m.instructions.add(new MethodInsnNode(
                            Opcodes.INVOKESTATIC,
                            "Reika/ChromatiCraft/Auxiliary/ChromaAux",
                            "interceptEntityCollision",
                            "(Lnet/minecraft/world/World;Lnet/minecraft/entity/Entity;Lnet/minecraft/util/AxisAlignedBB;)Ljava/util/List;",
                            false
                        ));
                    ...
                    case ENTITYPUSHOUT: {
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "func_145771_j", "func_145771_j", "(DDD)Z"
                        );
                        if (ReikaASMHelper.checkForClass(
                                "api.player.forge.PlayerAPITransformer"
                            )) {
                            m = ReikaASMHelper.getMethodByName(
                                cn, "localPushOutOfBlocks", "(DDD)Z"
                            ); //Try his method instead
                        }
                        AbstractInsnNode ain = ReikaASMHelper.getFirstOpcode(
                            m.instructions, Opcodes.GETFIELD
                        );
                        if (ain == null)
                            ReikaASMHelper.throwConflict(
                                this.toString(), cn, m, "Could not find field lookup"
                            );
                     ...
                     case RAYTRACEHOOK2:
                     case RAYTRACEHOOK3: {
                         MethodNode m = ReikaASMHelper.getMethodByName(cn,
                     "func_70071_h_", "onUpdate", "()V");

                         String func1 = FMLForgePlugin.RUNTIME_DEOBF ? "func_149668_a" :
                     "getCollisionBoundingBoxFromPool"; String func2 =
                     FMLForgePlugin.RUNTIME_DEOBF ? "func_72933_a" : "rayTraceBlocks";
                         String func3 = FMLForgePlugin.RUNTIME_DEOBF ? "func_147447_a" :
                     "func_147447_a";

                         String world = FMLForgePlugin.RUNTIME_DEOBF ? "field_70170_p" :
                     "worldObj";
                         ...
                         for (int i = 0; i < m.instructions.size(); i++) {
                             ...
                             if (ain.getOpcode() == Opcodes.INVOKEVIRTUAL) {
                                 ...
                                 if (min.name.equals(func1)) {
                                     VarInsnNode pre =
                     (VarInsnNode)ReikaASMHelper.getLastNonZeroALOADBefore(m.instructions,
                     i);
                                     //m.instructions.remove(pre);
                                     pre.var = 0;
                                     min.owner =
                     "Reika/ChromatiCraft/Auxiliary/ChromaAux";
                                     ReikaASMHelper.addLeadingArgument(min,
                     "Lnet/minecraft/entity/Entity;"); min.name =
                     "getInterceptedCollisionBox"; ReikaASMHelper.changeOpcode(min,
                     Opcodes.INVOKESTATIC);
                                 }
                                 else if (min.name.equals(func2)) {
                                     AbstractInsnNode pre =
                     ReikaASMHelper.getLastFieldRefBefore(m.instructions, i, world);
                                     m.instructions.remove(pre);
                                     min.owner =
                     "Reika/ChromatiCraft/Auxiliary/ChromaAux";
                                     ReikaASMHelper.addLeadingArgument(min,
                     "Lnet/minecraft/entity/Entity;"); min.name =
                     "getInterceptedRaytrace"; ReikaASMHelper.changeOpcode(min,
                     Opcodes.INVOKESTATIC);
                                 }
                    ...
                    case STOPSLOWFALL: {
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "func_70612_e", "moveEntityWithHeading", "(FF)V"
                        );

                        String func = FMLForgePlugin.RUNTIME_DEOBF ? "func_72899_e"
                                                                   : "blockExists";
                        MethodInsnNode blockExistsNode
                            = ReikaASMHelper.getFirstMethodCall(
                                cn, m, "net/minecraft/world/World", func, "(III)Z"
                            );
                        JumpInsnNode ifEqJump
                            = (JumpInsnNode) ReikaASMHelper.getLastOpcodeBefore(
                                m.instructions,
                                m.instructions.indexOf(blockExistsNode),
                                Opcodes.IFEQ
                            );
                        ...
                        ins.add(new MethodInsnNode(
                            Opcodes.INVOKESTATIC,
                            "Reika/ChromatiCraft/World/Dimension/SkyRiverManagerClient",
                            "stopSlowFall",
                            "()Z",
                            false
                        ));
                    ...
                    case TRANSPARENCY4: {
                        ...
                        li.add(new MethodInsnNode(
                            ...
                            "Reika/ChromatiCraft/Auxiliary/ChromaAux",
                            "groundOpacity",
                            "(Lnet/minecraft/world/IBlockAccess;IIILnet/minecraft/block/Block;)I",
                            false
                        ));
                    ...
                    case FLOWERCACHE: {
                        ...
                        String sig =
                    "(Lforestry/api/apiculture/IBee;Lforestry/api/apiculture/IBeeHousing;)Z";
                        MethodNode m = ReikaASMHelper.getMethodByName(cn, "hasFlowers",
                    sig); m.instructions.clear();

                        m.instructions.add(new VarInsnNode(Opcodes.ALOAD, 0));
                        m.instructions.add(new VarInsnNode(Opcodes.ALOAD, 1));
                        m.instructions.add(new VarInsnNode(Opcodes.ALOAD, 2));
                        m.instructions.add(new MethodInsnNode(Opcodes.INVOKESPECIAL,
                    cn.superName, "hasFlowers", sig, false)); m.instructions.add(new
                    InsnNode(Opcodes.IRETURN));

                        sig = "(Lforestry/api/core/INBTTagable;)V";
                        m = ReikaASMHelper.getMethodByName(cn, "<init>", sig);
                        //m.instructions.insert(new MethodInsnNode(Opcodes.INVOKESPECIAL,
                    cn.superName, "<init>", sig, false));
                        //m.instructions.insert(new VarInsnNode(Opcodes.ALOAD, 0));
                        MethodInsnNode min =
                    (MethodInsnNode)ReikaASMHelper.getFirstOpcode(m.instructions,
                    Opcodes.INVOKESPECIAL); min.owner = cn.superName; break;
                    }*/
                    case TEXTURELOAD: {
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn,
                            "func_147964_a",
                            "loadSprite",
                            "([Ljava/awt/image/BufferedImage;Lnet/minecraft/client/resources/data/AnimationMetadataSection;Z)V"
                        );
                    ...
                    case LOREHANDLER: {
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "getDataStream", "()Ljava/io/InputStream;"
                        );
                        m.instructions.clear();
                        m.instructions.add(
                            new LdcInsnNode("Reika.ChromatiCraft.ChromatiCraft")
                        );
                        m.instructions.add(new MethodInsnNode(
                            Opcodes.INVOKESTATIC,
                            "java/lang/Class",
                        ...
                        m.instructions.add(new MethodInsnNode(
                            Opcodes.INVOKESTATIC,
                            "Reika/DragonAPI/Libraries/IO/ImageToStringConverter",
                            "decodeToString",
                        ...
                        m.instructions.add(
                            new TypeInsnNode(Opcodes.NEW, "java/io/ByteArrayInputStream")
                        );
                    ...
                    case ROSETTAHANDLER: {
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "loadInternalRosettaFile", "()Ljava/util/List;"
                        );
                        m.instructions.clear();
                        m.instructions.add(
                            new LdcInsnNode("Reika.ChromatiCraft.ChromatiCraft")
                        ...
                        m.instructions.add(new MethodInsnNode(
                            Opcodes.INVOKESTATIC,
                            "Reika/DragonAPI/IO/ReikaFileReader",
                            "getFileAsLines",
                    ...
                    case STOPLIGHTUPDATES: {
                        MethodNode m = ReikaASMHelper.getMethodByName(
                            cn, "func_72835_b", "tick", "()V"
                        );
                        String func = FMLForgePlugin.RUNTIME_DEOBF
                            ? "func_73156_b"
                            : "unloadQueuedChunks"; //contrary to name all this does is
                                                    //update light
                        MethodInsnNode min
                            = ReikaASMHelper.getFirstMethodCallByName(cn, m, func);
                        min.owner = "Reika/ChromatiCraft/Auxiliary/ChromaAux";
                        min.name = "interceptClientChunkUpdates";
                        min.desc
                            = "(Lnet/minecraft/client/multiplayer/ChunkProviderClient;)Z";
