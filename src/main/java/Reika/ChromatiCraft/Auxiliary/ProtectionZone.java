...
public final class ProtectionZone {
    ...
    public boolean hasTile(World world) {
        return ChromaTiles.getTile(world, originX, originY, originZ)
            == ChromaTiles.GUARDIAN;
    }
    ...
    public boolean isBlockInZone(int x, int y, int z) {
        ...
        //return dd <= range+0.5;
        return Math.abs(x - originX) <= range && Math.abs(z - originZ) <= range
            && (ChromaOptions.GUARDCHUNK.getState() || Math.abs(y - originY) <= range);
    }

    @Override
    public String toString() {
        return "Zone by " + creator.toString() + " in world " + dimensionID + " at "
            + originX + ", " + originY + ", " + originZ + " (Radius " + range + ")";
    }
