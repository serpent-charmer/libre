...
public class LegacyTileAcceleratorRecipe implements IRecipe {
    static {
        RecipeSorter.register(
            "chromaticraft:tileaccel",
            LegacyTileAcceleratorRecipe.class,
            Category.SHAPELESS,
        ...
        for (int i = 0; i < ics.getSizeInventory(); i++) {
            ...
            if (in != null) {
                if (ReikaItemHelper.matchStacks(
                        in, ChromaTiles.ADJACENCY.getCraftedProduct()
                    ))
                    c++;
                else
                    return false;
        ...
        for (int i = 0; i < ics.getSizeInventory(); i++) {
            ...
            if (in != null) {
                if (ReikaItemHelper.matchStacks(
                    ...
                    tier = in.stackTagCompound == null
                        ? tier
                        : in.stackTagCompound.getInteger("tier");
                }
            }
        }
