...
public class RecursiveCastingAutomationSystem extends CastingAutomationSystem {
    ...
    private void recoverCachedIngredients() {
        Iterator<Entry<ItemMatch, Ingredient>> it
            = cachedIngredients.data.entrySet().iterator();
        while (it.hasNext()) {
    ...
    public void setRecipe(CastingRecipe c, int amt, EntityPlayer ep) {
        this.recoverCachedIngredients();
        prereqs = null;
        if (c != null && recursionEnabled && tile.canRecursivelyRequest(c)
            && cachedIngredients.isEmpty()) {
            prereqs = new RecipeChain(tile.getAvailableRecipes());
            RecipePrereq pre = prereqs.createPrereq(
                null, new ItemMatch(c.getOutput()), c, amt * c.getOutput().stackSize
            );
    ...
    private void intakeNecessaryItems() {
        for (ItemMatch im : prereqs.getAllUsedItems()) { //second verification pass
            ...
            if (has < amt) {
                //ReikaJavaLibrary.pConsole("Missing items on second check, cannot
                //craft!");
                prereqs = null;
                return;
            }
        ...
        //ReikaJavaLibrary.pConsole("Took in intermediate ingredients:
        //"+cachedIngredients.toString());
    }
    ...
    private class RecipeChain {
        ...
        public RecipePrereq
        createPrereq(RecipePrereq p, ItemMatch im, CastingRecipe sel, int needed) {
    ...
    private static class RecipePrereq {
        ...
        private void calculate() {
            craftsRemaining = MathHelper.ceiling_float_int(
                totalItemsNeeded / (float) recipe.getOutput().stackSize
            );
        }
        ...
        @Override
        public String toString() {
            return "(" + recipe.toString() + ") x" + totalItemsNeeded + " / "
                + craftsRemaining;
        }
    }
