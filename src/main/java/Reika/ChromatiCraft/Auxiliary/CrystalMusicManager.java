...
public class CrystalMusicManager {
    ...
    private final EnumMap<CrystalElement, MusicKey> baseKeys
        = new EnumMap(CrystalElement.class);
    private final EnumMap<CrystalElement, ArrayList<MusicKey>> allKeys
        = new EnumMap(CrystalElement.class);
    private final MultiMap<MusicKey, CrystalElement> sourceElements
    ...
    private void addTonic(CrystalElement e, MusicKey m) {
        ...
        sourceElements.addValue(
            this.isMinorKey(e) ? m.getMinorThird() : m.getMajorThird(), e
        );
        sourceElements.addValue(m.getFifth(), e);
        sourceElements.addValue(m.getOctave(), e);
        //ReikaJavaLibrary.pConsole("Loaded Notes "+m+"/"+(this.isMinorKey(e) ?
        //m.getMinorThird() : m.getMajorThird())+"/"+m.getFifth()+"/"+m.getOctave()+" for
        //color "+e);
    }
    ...
    public double getThird(CrystalElement e) {
        ...
        return this.isMinorKey(e) ? base * key.getMinorThird().getRatio(key)
                                  : base * key.getMajorThird().getRatio(key);
    }
    ...
    public float getScaledDing(CrystalElement e, int n) {
        ...
        switch (n) {
            case 0:; //base *= 1;
                break;
            case 1:
                base *= this.isMinorKey(e) ? key.getMinorThird().getRatio(key)
                                           : key.getMajorThird().getRatio(key);
                break;
            case 2:
    ...
    public boolean isMinorKey(CrystalElement e) {
        if (e == CrystalElement.CYAN || e == CrystalElement.ORANGE
            || e == CrystalElement.GREEN || e == CrystalElement.LIGHTGRAY
            || e == CrystalElement.GRAY)
            return true;
    ...
    public Set<CrystalElement> getColorsWithKeyAnyOctave(MusicKey key) {
        ...
        int offset = -12;
        while (set.isEmpty() && offset >= -60) {
            set = this.getColorsWithKey(key.getInterval(offset));
            offset -= 12;
        }
        offset = 12;
        while (set.isEmpty() && offset <= 72) {
            set = this.getColorsWithKey(key.getInterval(offset));
            offset += 12;
        }
        return set;
