...
public class CrystalNetworkLogger {
    ...
    public static void
    logRequest(CrystalReceiver r, CrystalElement e, int amount, CrystalFlow p) {
        if (level.isAtLeast(LoggingLevel.CORE)) {
            ...
            if (p != null)
                s = s + "; max throughput of " + p.maxFlow + ", total cost " + p.totalCost
                    + " (loss = " + p.getSignalLoss() + ")";
            ChromatiCraft.logger.log(s);
            dumpStack();
        }
    }

    public static void logPathFind(
        CrystalReceiver target,
        CrystalElement e,
        CrystalReceiver r,
        String transmitters,
        String steps
    ...
    public static void logFlowBreak(CrystalFlow p, FlowFail f) {
        if (level.isAtLeast(LoggingLevel.CORE)) {
            String s = p.element + " flow from " + p.transmitter + " to " + p.receiver
                + " broken due to: " + f.text + ". " + p.getRemainingLumens()
    ...
    public static void logFlowSatisfy(CrystalFlow p) {
        if (level.isAtLeast(LoggingLevel.CORE)) {
            String s = p.element + " flow from " + p.transmitter + " to " + p.receiver
                + " satisfied and terminated. " + p.totalCost + " lumens transferred.";
            ChromatiCraft.logger.log(s);
            dumpStack();
        }
    }
    ...
    public static void logFlowTick(CrystalFlow p, int amt) {
        if (level.isAtLeast(LoggingLevel.ALL)) {
            String s = "Ticking " + p.element + " flow from " + p.transmitter + " to "
                + p.receiver + "; " + amt + " lumens transferred this tick; "
                + p.getRemainingLumens() + " remaining.";
            ChromatiCraft.logger.log(s);
            dumpStack();
        }
    }
    ...
    public static void logTileAdd(CrystalNetworkTile te) {
        if (level.isAtLeast(LoggingLevel.STATE)) {
            String s = "Added tile " + te
                + " to network; UUID = " + (te != null ? te.getUniqueID() : "[]");
            ChromatiCraft.logger.log(s);
            dumpStack();
        }
    }
    ...
    public static void logTileRemove(CrystalNetworkTile te) {
        if (level.isAtLeast(LoggingLevel.STATE)) {
            String s = "Removed tile " + te
                + " from network; UUID = " + (te != null ? te.getUniqueID() : "[]");
            ChromatiCraft.logger.log(s);
            dumpStack();
        }
    }
    ...
    public static class NetworkLoggerCommand extends DragonCommandBase {
        ...
        public void processCommand(ICommandSender ics, String[] args) {
            ...
            ReikaChatHelper.sendChatToAllOnServer(
                "Crystal Network Logger Status: " + level
            );
        }
