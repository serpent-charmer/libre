    ...
    public void processCommand(ICommandSender ics, String[] args) {
        if (DragonAPICore.isReikasComputer()
            ...
            ChromaDimensionBiomeType b
                = Biomes.valueOf(args[0].toUpperCase(Locale.ENGLISH));
            int r = 1;
            ...
            ReikaChatHelper.sendChatToPlayer(
                ep,
                "Running generators on " + (r * 2 + 1) + "x" + (r * 2 + 1) + " chunks..."
            );
            ...
            BiomeTerrainProvider terrain
                = new ForcedBiomeTerrainProvider(b, ep.worldObj.getSeed());
            for (int i = -r; i <= r; i++) {
                for (int k = -r; k <= r; k++) {
            ...
            for (int i = -r; i <= r; i++) {
                for (int k = -r; k <= r; k++) {
                    ...
                    for (ChromaWorldGenerator g : gen) {
                        float f
                            = g.getGenerationChance(ep.worldObj, dx, dz, b.getBiome());
                        int n = (int) f;
