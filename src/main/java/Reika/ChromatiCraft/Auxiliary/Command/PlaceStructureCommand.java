...
public class PlaceStructureCommand extends DragonCommandBase {
    ...
    public void processCommand(ICommandSender ics, String[] args) {
        ...
        if (s == ChromaStructures.NETHERTRAP)
            ((VoidMonsterNetherStructure) s.getStructure())
                .setTNT(args.length == 1 || Boolean.parseBoolean(args[1]));
        if (s.getStructure() instanceof GeneratedStructureBase) {
            ((GeneratedStructureBase) s.getStructure()).markForWorldgen();
        }
        CrystalElement e = s.requiresColor
            ? CrystalElement.valueOf(args[1].toUpperCase(Locale.ENGLISH))
            : null;
        ...
        if (s.getStructure() instanceof FragmentStructureBase) {
            ...
            TileEntityStructControl te
                = (TileEntityStructControl) c.getTileEntity(ep.worldObj);
            ...
            DungeonGenerator.instance.modifyBlocks(
                s, arr, ep.getRNG(), Modify.MOSSIFY, Modify.GRASSDIRT
            );
            if (s == ChromaStructures.BURROW) {
                if (args.length > 2 && args[2].equals("true")) {
                    FilledBlockArray arr2
                        = ((BurrowStructure) fs).getFurnaceRoom(ep.worldObj, x, y, z);
                    arr2.place();
                    DungeonGenerator.instance.modifyBlocks(
                        s, arr2, ep.getRNG(), Modify.MOSSIFY, Modify.GRASSDIRT
                    );
        ...
        if (s.getStructure() instanceof GeneratedStructureBase) {
            ((GeneratedStructureBase) s.getStructure())
                .runCallbacks(ep.worldObj, ep.getRNG());
        }
        s.getStructure().resetToDefaults();
    }
