        ...
        if (DragonAPICore.isReikasComputer()
            ...
            if (args.length > 0) {
                if (args[0].equalsIgnoreCase("monument")) {
                    ...
                    ((TileEntityStructControl) gen.getController().getTileEntity(
                         ep.worldObj
                     ))
                        .setPlacer(ep);
                    for (int i = 0; i < 16; i++) {
                        ...
                        Coordinate c2 = rel.offset(
                            gen.getPosX(), gen.getPosY() + cy, gen.getPosZ()
                        );
                        c2.setBlock(
                            ep.worldObj,
                        ...
                        TileEntityDimensionCore te
                            = (TileEntityDimensionCore) c2.getTileEntity(ep.worldObj);
                        te.setColor(e);
                        te.setPlacer(ep);
                        te.prime(true);
                    }
                    ...
                    ReikaChatHelper.sendChatToPlayer(
                        ep,
                        "Placing " + args.length + " structures (" + Arrays.toString(args)
                            + ")..."
                    );
                    ...
                    for (StructurePair p : ChunkProviderChroma.getStructures()) {
                        ...
                        if (ReikaArrayHelper.arrayContains(args, type.name(), true)) {
                            ReikaChatHelper.sendChatToPlayer(
                                ep,
                                "Generating " + type + " @ " + p.generator.getEntryPosX()
                                    + ", " + p.generator.getEntryPosZ() + "..."
                            );
                            p.generator.generateAll(ep.worldObj);
                            ReikaChatHelper.sendChatToPlayer(
                                ep, "Generating " + type + " complete."
                            );
                        } else {
                            ReikaChatHelper.sendChatToPlayer(
                                ep,
                                "Not generating " + type + ". " + Arrays.toString(args)
