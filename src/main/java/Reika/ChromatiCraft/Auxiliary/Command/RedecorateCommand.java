...
public class RedecorateCommand extends DragonCommandBase {
    private static final MultiMap<String, ChunkCoordIntPair> generatedChunks
    ...
    public void processCommand(ICommandSender ics, String[] args) {
        if (args.length != 2) {
            this.sendChatToSender(
                ics,
                EnumChatFormatting.RED + "Incorrect formatting. Use [radius] [generator]"
            );
            return;
        }
        int r = Integer.parseInt(args[0]);
        ChromaDecorator d
            = ChromatiCraft.getDecorator(args[1].toLowerCase(Locale.ENGLISH));
        if (d == null) {
            this.sendChatToSender(
                ics, EnumChatFormatting.RED + "Unrecognized decorator."
            );
            return;
        }
        ...
        for (int i = -rc; i <= rc; i++) {
            for (int k = -rc; k <= rc; k++) {
                if (ReikaWorldHelper.isChunkGeneratedChunkCoords(
                        (WorldServer) world, x + i, z + k
                    )) {
        ...
        this.sendChatToSender(
            ics, EnumChatFormatting.GREEN + "Decorator run on " + n + " chunks."
        );
    }

    private boolean shouldDecorateChunk(Chunk ch, ChromaDecorator d) {
        HashSet<ChunkCoordIntPair> c
            = (HashSet<ChunkCoordIntPair>) generatedChunks.get(d.getCommandID());
    ...
    private File getFile(World world, ChromaDecorator d) {
        ...
        if (f != null) {
            return new File(
                f,
                "DIM" + world.provider.dimensionId + "/redecorate_" + d.getCommandID()
                    + ".dat"
            );
        }
        return null;
