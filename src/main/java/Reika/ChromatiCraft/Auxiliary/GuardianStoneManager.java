...
public class GuardianStoneManager {
    ...
    private final MultiMap<BlockKey, Action> blockExceptions
        = new MultiMap(CollectionType.HASHSET);
    private final MultiMap<KeyedItemStack, Action> itemExceptions
        = new MultiMap(CollectionType.HASHSET);
        ...
        for (String s : ChromatiCraft.config.getGuardianExceptions()) {
            if (this.parseString(s)) {
                ...
                ChromatiCraft.logger.logError(
                    "Could not parse Guardian Stone exception, due to a malformed line (or missing item): "
                    + s
                );
            }
        }
    ...
    private boolean parseString(String s) {
        ...
        if (parts.length != 2)
            return false;
        boolean empty = parts[0].equals("none") || parts[0].equals("null")
            || parts[0].equals("empty");
    ...
    public boolean canPlayerOverrideProtections(EntityPlayer ep) {
        if (ReikaPlayerAPI.isFake(ep))
            return false;
        return ReikaPlayerAPI.isReika(ep)
            || (ep instanceof EntityPlayerMP
                && ReikaPlayerAPI.isAdmin((EntityPlayerMP) ep));
    }

    public ProtectionZone
    addZone(World world, int x, int y, int z, EntityPlayer ep, int r) {
        if (ep == null) {
            int dim = world.provider.dimensionId;
            ChromatiCraft.logger.logError(
                "Tried to generate a protection zone for a null player at " + x + ", " + y
                + ", " + z + " in " + dim
            );
            return null;
        }
    ...
    private ArrayList<ProtectionZone>
    getProtectionZonesForArea(World world, int x, int y, int z) {
    ...
    public void removeAreasForStone(TileEntityGuardianStone te) {
        ...
        while (it.hasNext()) {
            ...
            if (zone.originX == x && zone.originY == y && zone.originZ == z
                && zone.dimensionID == id)
                it.remove();
        }
    }
    ...
    public boolean
    doesPlayerHavePermissions(World world, int x, int y, int z, EntityPlayer ep) {
    ...
    public void guardArea(PlayerInteractEvent event) {
        ...
        if (!world.isRemote) {
            if (!this.doesPlayerHavePermissions(world, x, y, z, ep)
                && !this.isWhitelistedAction(
                    event.action, world, x, y, z, ep.getCurrentEquippedItem()
                ))
                event.setCanceled(true);
        }
    }
    ...
    private boolean
    isWhitelistedAction(Action a, World world, int x, int y, int z, ItemStack is) {
        return blockExceptions.get(BlockKey.getAt(world, x, y, z)).contains(a)
            || itemExceptions
                   .get(is != null ? new KeyedItemStack(is).setSimpleHash(true) : null)
                   .contains(a);
    }
    ...
    public void preventEndermen(EnderTeleportEvent event) {
        if (event.entityLiving instanceof EntityEnderman) {
            ...
            if (!this.getProtectionZonesForArea(event.entityLiving.worldObj, x, y, z)
                     .isEmpty()) {
                event.setCanceled(true);
            }
        }
