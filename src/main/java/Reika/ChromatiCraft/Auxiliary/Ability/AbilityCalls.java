...
public class AbilityCalls {
    public static void convertBufferToLP(EntityPlayer ep, int data) {
        ...
        PlayerElementBuffer.instance.removeFromPlayer(
            ep, TileEntityLifeEmitter.getLumensPerHundredLP()
        );
    }
    ...
    public static void doNukerFX(World world, int x, int y, int z, EntityPlayer ep) {
        ...
        ElementTagCompound tag = ItemElementCalculator.instance.getValueForItem(
            BlockKey.getAt(world, x, y, z).asItemStack()
        );
        int c = tag == null || tag.isEmpty()
            ? 0x22aaff
            : tag.asWeightedRandom().getRandomEntry().getColor();
        for (double d = 0.125; d <= 1; d += 0.03125 / 2) {
            double dx = ep.posX + d * lx;
            double dy = ep.posY + d * ly;
            double dz = ep.posZ + d * lz;
            EntityBlurFX fx = new EntityCCBlurFX(world, dx, dy, dz)
                                  .setLife(5)
                                  .setAlphaFading()
                                  .setScale(0.5F)
                                  .setColor(c);
    ...
    public static void analyzeBees(EntityPlayer ep) {
        int slot
            = (int) (ep.worldObj.getTotalWorldTime() % ep.inventory.mainInventory.length);
    ...
    public static void setNoclipState(EntityPlayer ep, boolean set) {
        ...
        //ep.noClip = set;// && ((ep.capabilities.allowFlying && ep.capabilities.isFlying)
        //|| ep.isSneaking() || KeyWatcher.instance.isKeyDown(ep, Key.JUMP));
        /*if (ep.noClip) {
            ep.moveEntity(-ep.motionX, -ep.motionY, -ep.motionZ);
            List<AxisAlignedBB> li = ep.worldObj.getCollidingBoundingBoxes(ep,
        ep.boundingBox.addCoord(ep.motionX, ep.motionY,
        ep.motionZ));//AbilityHelper.instance.getNoclipBlockBoxes(ep);
            ...
            AxisAlignedBB epbox = ep.boundingBox;//.addCoord(ep.motionX, ep.motionY,
        ep.motionZ);

            //ReikaJavaLibrary.pConsole("S: "+epbox+"+"+li+"="+(li.isEmpty() ? false :
        li.get(0).intersectsWith(epbox))+" & "+ep.motionY, Side.SERVER);
            ...
            //ReikaJavaLibrary.pConsole("E: "+epbox+"+"+li+"="+(li.isEmpty() ? false :
        li.get(0).intersectsWith(epbox))+" & "+ep.motionY, Side.SERVER);
        }*/
    }
            ...
            if (mov != null) {
                ...
                if (world.canBlockSeeTheSky(x, y + 1, z)
                    && ReikaPlayerAPI.playerCanBreakAt(
                        (WorldServer) ep.worldObj, x, y, z, (EntityPlayerMP) ep
                    )) {
                    world.addWeatherEffect(
                        new EntityLightningBolt(world, x + 0.5, y + 0.5, z + 0.5)
                    );
                    int r = 2 + power * 4;
                    if (power == 2) {
                        new FlyingBlocksExplosion(world, x + 0.5, y - 2.5, z + 0.5, 6)
                            .setTumbling(new LightningTumble(world, x, y, z, r))
                            .doExplosion();
                    } else if (power == 1) {
                        world.newExplosion(
                            null, x + 0.5, y - 0.5, z + 0.5, 4, true, true
                        );
                    }
                    for (int i = -r; i <= r; i++) {
                        for (int j = -r; j <= r; j++) {
    ...
    public static void teleportPlayerMenu(EntityPlayer ep) {
        ep.openGui(
            ChromatiCraft.instance, ChromaGuis.TELEPORT.ordinal(), ep.worldObj, 0, 0, 0
        );
    }
    ...
    public static void causeShockwave(EntityPlayer ep) {
        if (ep.worldObj.isRemote) {
            ...
            AxisAlignedBB box
                = AxisAlignedBB
                      .getBoundingBox(
                          ep.posX, ep.posY, ep.posZ, ep.posX, ep.posY, ep.posZ
                      )
                      .expand(16, 4, 16);
            List<EntityLivingBase> li
                = ep.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, box);
            for (EntityLivingBase e : li) {
                if (e != ep
                    && ReikaMathLibrary.py3d(e.posX - ep.posX, 0, e.posZ - ep.posZ)
                        <= 16) {
    ...
    private static void spawnShockwaveParticles(EntityPlayer ep) {
        for (int i = 0; i < 360; i++) {
            ...
            EntityCenterBlurFX fx
                = new EntityCenterBlurFX(
                      ep.worldObj, ep.posX, ep.posY - 1.62 + 0.1, ep.posZ, vx, 0, vz
                )
                      .setColor(0x0080ff)
                      .setScale(2);
            fx.noClip = false;
            ...
            if (i % 30 == 0) {
                for (double d = 0.25; d <= 16; d += 0.5) {
                    EntityCenterBlurFX fx2 = new EntityCenterBlurFX(
                                                 ep.worldObj,
                                                 ep.posX + dx * d,
                                                 ep.posY - 1.62 + 0.1,
                                                 ep.posZ + dz * d,
    ...
    public static void addInvPage(EntityPlayer ep) {
        ...
        PlayerElementBuffer.instance.removeFromPlayer(
            ep, AbilityHelper.instance.getElementsFor(Chromabilities.HOTBAR)
        );
    }
    ...
    public static void setPlayerMaxHealth(EntityPlayer ep, int value) {
        ...
        ep.getEntityAttribute(SharedMonsterAttributes.maxHealth)
            .removeModifier(
                new AttributeModifier(Chromabilities.HEALTH_UUID, "Chroma", 0, 2)
            );
        if (value > 0) {
            ep.getEntityAttribute(SharedMonsterAttributes.maxHealth)
                .applyModifier(
                    new AttributeModifier(Chromabilities.HEALTH_UUID, "Chroma", factor, 2)
                );
            //if (added > 0)
            //	ep.heal(added);
        }
    ...
    public static void attractItemsAndXP(EntityPlayer ep, int range, boolean nc) {
        ...
        for (EntityItem ent : inbox) {
            ...
            if (!ent.getEntityData().hasKey("cc_magnetized"))
                ent.getEntityData().setString(
                    "cc_magnetized", ep.getUniqueID().toString()
                );
        }
        ...
        if (!ep.worldObj.isRemote) {
                        ...
                        if (ReikaMathLibrary.py3d(i, j, k) <= r + 0.5) {
                            ...
                            if (b != Blocks.air && b.isOpaqueCube()
                                ...
                                if (b instanceof SemiUnbreakable
                                    && ((SemiUnbreakable) b)
                                           .isUnbreakable(
                                               ep.worldObj, dx, dy, dz, meta
                                           )) {
                                    continue;
                                }
                                if (ep.worldObj.provider.dimensionId
                                ...
                                if (ReikaPlayerAPI.playerCanBreakAt(
                                    ...
                                    if (power
                                        > b.getExplosionResistance(
                                              ...
                                              ep.posY,
                                              ep.posZ
                                          ) / AbilityHelper.SONIC_EXPLO_FACTOR) {
                                        ArrayList<ItemStack> li = b.getDrops(
                                            ep.worldObj, dx, dy, dz, meta, 0
                                        );
                                        if (b instanceof BlockTieredResource) {
                                            BlockTieredResource bt
                                                = (BlockTieredResource) b;
                                            li.clear();
                                            if (bt.isPlayerSufficientTier(
                                                    ep.worldObj, dx, dy, dz, ep
                                                ))
                                                li.addAll(bt.getHarvestResources(
                                                    ep.worldObj, dx, dy, dz, 0, ep
                                                ));
                                            else
                                                li.addAll(bt.getNoHarvestResources(
                                                    ep.worldObj, dx, dy, dz, 0, ep
                                                ));
                                        }
                                        ...
                                        b.removedByPlayer(
                                            ep.worldObj, ep, dx, dy, dz, true
                                        );
                                        ReikaSoundHelper.playBreakSound(
                                            ep.worldObj, dx, dy, dz, b, 0.1F, 1F
                                        );
            ...
            for (ItemStack is : drops.keySet()) {
                ...
                while (amt > 0) {
                    ...
                    amt -= drop;
                    DecimalPosition pos = ReikaRandomHelper.getRandomSphericalPosition(
                        x + 0.5, y + 0.5, z + 0.5, r
                    );
                    ...
                    ReikaItemHelper.dropItem(
                        ...
                        pos.yCoord,
                        pos.zCoord,
                        ReikaItemHelper.getSizedItemStack(is, drop)
                    );
                }
            }
            AxisAlignedBB box = AxisAlignedBB.getBoundingBox(x, y, z, x + 1, y + 1, z + 1)
                                    .expand(r, r, r);
            List<EntityXPOrb> li
                = ep.worldObj.getEntitiesWithinAABB(EntityXPOrb.class, box);
            int amt = 0;
    ...
    static boolean shiftArea(
        WorldServer world, BlockBox box, ForgeDirection dir, int dist, EntityPlayerMP ep
    ) {
            ...
            for (int j = 0; j <= box.getSizeY(); j++) {
                for (int k = 0; k <= box.getSizeZ(); k++) {
                    ...
                    if (!ep.capabilities.isCreativeMode) {
                        if (b instanceof SemiUnbreakable
                            && ((SemiUnbreakable) b).isUnbreakable(world, x, y, z, meta))
                            continue;
                    }
                    if (ep.capabilities.isCreativeMode
                        || !ReikaBlockHelper.isUnbreakable(world, x, y, z, b, meta, ep)) {
                        if (!b.hasTileEntity(meta)
                            || ChromaOptions.SHIFTTILES.getState()) {
        ...
        ElementTagCompound cost
            = AbilityHelper.instance.getUsageElementsFor(Chromabilities.SHIFT, ep)
                  .scale(factor);
        boolean nrg = PlayerElementBuffer.instance.playerHas(ep, cost);
        boolean flag = false;
        if (nrg && ReikaPlayerAPI.playerCanBreakAt(world, toDel, ep)
            && ReikaPlayerAPI.playerCanBreakAt(world, moved, ep)) {
    ...
    public static void launchFireball(EntityPlayer ep, int charge) {
        ...
        EntityAbilityFireball ef = new EntityAbilityFireball(
            ep.worldObj, ep, look.xCoord, look.yCoord + 1, look.zCoord
        );
    ...
    public static void stopArrows(EntityPlayer ep) {
        if (!ep.worldObj.isRemote) {
            ...
            List<EntityArrow> li
                = ep.worldObj.getEntitiesWithinAABB(EntityArrow.class, box);
            for (EntityArrow e : li) {
                if (e.shootingEntity != ep
                    && (!(e.shootingEntity instanceof EntityPlayer)
        ...
        for (EntityMob e : li) {
            if (!(e instanceof EntityEnderman || e instanceof EntityPigZombie)) {
                if (e.getEntityToAttack() == ep || e.getEntityToAttack() == null) {
                    ...
                    //e.attackEntityFrom(DamageSource.causeMobDamage(ReikaEntityHelper.getDummyMob(ep.worldObj,
                    //e.posX, e.posY, e.posZ)), 0);
                }
            ...
            for (int k = 0; k < tasks.size(); k++) {
                ...
                if (a instanceof EntityAINearestAttackableTarget) {
                    EntityAINearestAttackableTarget nat =
            (EntityAINearestAttackableTarget)a; nat.targetEntitySelector = new
            AbilityHelper.PlayerExemptAITarget(nat.targetEntitySelector);
                }
            }*/
        }
    }
        ...
        if (!ep.worldObj.isRemote) {
            for (int i = 0; i < 6; i++) {
                ...
                double phi = ReikaRandomHelper.getRandomPlusMinus(
                    ep.rotationYawHead + 90, ANGLE
                );
                double theta
                    = ReikaRandomHelper.getRandomPlusMinus(-ep.rotationPitch, ANGLE);
                ...
                for (double d = 0; d <= 8; d += 0.125) {
                    ...
                    if (!b.isAir(ep.worldObj, x, y, z) && !ReikaBlockHelper.isLiquid(b)
                        && b != Blocks.mob_spawner
                        && !ReikaBlockHelper.isUnbreakable(
                            ep.worldObj,
                            x,
                            y,
                            z,
                            b,
                            ep.worldObj.getBlockMetadata(x, y, z),
                            ep
                        )) {
                        if (ep.worldObj
                                .getEntitiesWithinAABB(
                                    EntityNukerBall.class,
                                    ReikaAABBHelper.getBlockAABB(x, y, z)
                                )
                                .isEmpty()) {
                ...
                if (c != null) {
                    ...
                    ReikaPacketHelper.sendDataPacket(
                        ...
                        ChromaPackets.NUKERLOC.ordinal(),
                        new PacketTarget.RadiusTarget(ep, 64),
                        c.xCoord,
                        c.yCoord,
                        c.zCoord,
                        ep.getEntityId()
    ...
    public static void tickFireRain(EntityPlayer ep) {
        ...
        if (world.isRemote) {
                    ...
                    for (int j = -r; j <= r; j++) {
                        for (int k = -r; k <= r; k++) {
                            ...
                            if (wood == ModWoodList.DARKWOOD) {
                                //ReikaJavaLibrary.pConsole(new Coordinate(ddx, ddy,
                                //ddz));
                                world.setBlock(ddx, ddy, ddz, Blocks.fire);
                                if (world.rand.nextInt(60) == 0)
                                    world.newExplosion(
                                        ep, ddx + 0.5, ddy + 0.5, ddz + 0.5, 8, true, true
                                    );
                            }
                        }
                    }
            ...
            if (world.rand.nextInt(20) == 0) {
                 ...
                 */
                ChromaSounds.FIRE.playSoundAtBlock(
                    world, dx, dy, dz, 1.5F, 1 + world.rand.nextFloat() * 0.5F
                );
            }
    ...
    private static void doFireRainParticles(EntityPlayer ep) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int l = 200;
            EntityFX fx = new EntityFireFX(ep.worldObj, rx, ry, rz)
                              .setGravity(g)
                              .setScale(8)
                              .setLife(l);
    ...
    public static boolean doLightCast(EntityPlayer ep) {
        ...
        ProgressiveBreaker b
            = ProgressiveRecursiveBreaker.instance.addCoordinateWithReturn(
                ep.worldObj, c.xCoord, c.yCoord, c.zCoord, 200
            );
    ...
    public static boolean doLaserPulse(EntityPlayer ep) {
        ...
        if (world.isRemote) {
            ...
            AxisAlignedBB box
                = AxisAlignedBB.getBoundingBox(px, py, pz, px, py, pz).expand(64, 32, 64);
            List<EntityLiving> li = world.getEntitiesWithinAABB(EntityLiving.class, box);
            for (EntityLiving e : li) {
                e.attackEntityFrom(
                    new ReikaEntityHelper.WrappedDamageSource(
                        ChromatiCraft.pylonDamage[CrystalElement.BLUE.ordinal()], ep
                    ),
                    Integer.MAX_VALUE
                );
            }
                    ...
                    for (int j = -(int) Math.ceil(h); j <= Math.ceil(h); j++) {
                        if (ReikaMathLibrary.isPointInsideEllipse(i, j, k, r, h, r)) {
                            ...
                            if (b == Blocks.bedrock && dpy <= 4)
                                continue;
                            if (b == ChromaBlocks.HOVER.getBlockInstance()
                                && world.provider.dimensionId
                                    == ExtraChromaIDs.DIMID.getValue())
                                continue;
                            if (b instanceof SemiUnbreakable
                                && ((SemiUnbreakable) b)
                                       .isUnbreakable(world, dpx, dpy, dpz, meta))
                                continue;
                            if (ReikaPlayerAPI.playerCanBreakAt(
                                    (WorldServer) world,
                                    dpx,
                                ...
                                boolean flag = false;
                                if ((0.5 + 0.5 * world.rand.nextDouble()) * d
                                    < r * (0.5 + world.rand.nextDouble() * 0.5)) {
                                    if (ReikaBlockHelper.isOre(b, meta)) {
                                        ItemStack is = ReikaBlockHelper.getSilkTouch(
                                            world, dpx, dpy, dpz, b, meta, ep, false
                                        );
                                        ItemStack out
                                        ...
                                        if (out != null) {
                                            out = out.copy();
                                            out.stackSize *= 2;
                                            EntityItem ei = ReikaItemHelper.dropItem(
                                                world, dx, dy, dz, out
                                            );
            ...
            for (float f = 0.1F; f <= 2; f *= 2) {
                ReikaSoundHelper.playSoundFromServer(
                    world, px, py, pz, "random.explode", 2, f, true
                );
                ReikaSoundHelper.playSoundFromServer(
                    world, ep.posX, ep.posY, ep.posZ, "random.explode", 1, f, true
                );
            }
    ...
    @SideOnly(Side.CLIENT)
    private static void
    doLaserPunchParticles(EntityPlayer ep, double px, double py, double pz) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            double ry = ReikaRandomHelper.getRandomPlusMinus(py + 1.5, 1)
                + (ep.worldObj.getTopSolidOrLiquidBlock(
                       MathHelper.floor_double(rx), MathHelper.floor_double(rz)
                   )
                   - ep.worldObj.getTopSolidOrLiquidBlock(
                       MathHelper.floor_double(px), MathHelper.floor_double(pz)
                   ));
            ...
            int c = f < 0.5 ? ReikaColorAPI.mixColors(0xffffff, 0x00a0ff, 1 - (f * 2))
                            : ReikaColorAPI.mixColors(0x0000ff, 0x00a0ff, (f - 0.5F) * 2);
            EntityFX fx = new EntityCCBlurFX(ep.worldObj, rx, ry, rz, vx, vy, vz)
                              .setColor(c)
                              .setScale(s)
                              .setLife(l)
                              .setRapidExpand()
        ...
        for (double dy = py; dy < 1024; dy += 1) {
            EntityFX fx = new EntityCCBlurFX(ep.worldObj, px, dy, pz)
                              .setColor(0xffffff)
                              .setScale(16)
                              .setLife(120)
                              .setRapidExpand();
    ...
    private static void doGrowthAuraParticles(EntityPlayer ep, int x, int y, int z) {
        for (int i = 0; i < 4; i++)
            ChromaFX.doGrowthWandParticles(
                ep.worldObj,
                ReikaRandomHelper.getRandomPlusMinus(x, 4),
                y - 1,
                ReikaRandomHelper.getRandomPlusMinus(z, 4)
            );
        for (int i = 0; i < 6; i++) {
    ...
    private static void
    ping(int px, int pz, CrystalElement color, EntityPlayer ep, Random rand) {
        int x = MathHelper.floor_double(ep.posX);
        int z = MathHelper.floor_double(ep.posZ);
        double dx = px - x;
        double dz = pz - z;
        ...
        //ReikaJavaLibrary.pConsole(s.color+": DD="+dist+", ang="+ang+", pow="+pow+",
        //fac="+fac+", factor="+factor+", delay="+delay);
        DimensionPingEvent evt
            = new DimensionPingEvent(ChromaSounds.ORB, 1, ep, dist, ang);
        if (color != null)
    ...
    private static boolean isValidWaterBlocks(Block id, Block idbelow) {
        return (idbelow instanceof BlockLiquid || idbelow instanceof BlockFluidBase)
            && !((id instanceof BlockLiquid || id instanceof BlockFluidBase));
    }
    ...
    public static void waterRun(EntityPlayer ep) {
        ...
        if (isValidWaterBlocks(id, idbelow)
            && ReikaMathLibrary.py3d(ep.motionX, 0, ep.motionZ) >= 0.15) {
            ep.fallDistance = 0;
            if (ep instanceof EntityPlayerMP) {
                ((EntityPlayerMP) ep).playerNetServerHandler.floatingTickCount = 0;
            }
            for (int i = 0; i < 8; i++)
                ReikaParticleHelper.RAIN.spawnAt(
                    ep.worldObj,
                    ReikaRandomHelper.getRandomPlusMinus(ep.posX, 0.25),
                    ep.posY - 1,
                    ReikaRandomHelper.getRandomPlusMinus(ep.posZ, 0.25)
                );
            if (ep.ticksExisted % 2 == 0)
                ep.playSound(
                    "random.splash",
                    0.0625F + ep.worldObj.rand.nextFloat() * 0.25F,
                    0.25F + ep.worldObj.rand.nextFloat()
                );
    ...
    public static void superbuild(
        ...
        double maxd = ReikaMathLibrary.py3d(
            ep.posX - x - 0.5, ep.posY - y - 0.5, ep.posZ - z - 0.5
        );

        boolean reachedX
            = dir.offsetX != 0 && (dir.offsetX < 0 ? x <= ep.posX : x >= ep.posX);
        boolean reachedY = dir.offsetY != 0
            && (dir.offsetY < 0 ? y <= ep.posY - 1 : y >= ep.posY - 1
            ); //on this direction, stop at foot level
        boolean reachedZ
            = dir.offsetZ != 0 && (dir.offsetZ < 0 ? z <= ep.posZ : z >= ep.posZ);
        reached = reachedX || reachedY || reachedZ;

        ElementTagCompound tag
            = AbilityHelper.instance.getUsageElementsFor(Chromabilities.SUPERBUILD, ep)
                  .scale(0.05F);

        while (!reached && hasBlock && !hitBlock
               && PlayerElementBuffer.instance.playerHas(ep, tag)) {
            x += dir.offsetX;
            y += dir.offsetY;
            z += dir.offsetZ;
            ...
            if (!hitBlock) {
                ...
                //ReikaPacketHelper.sendDataPacketWithRadius(DragonAPIInit.packetChannel,
                //PacketIDs.BREAKPARTICLES.ordinal(), world, x, y, z, 32,
                //Block.getIdFromBlock(b), meta);
                if (!ep.capabilities.isCreativeMode)
                    is.stackSize--;
                ...
                TickScheduler.instance.scheduleEvent(
                    new ScheduledTickEvent(
                        new ScheduledBlockPlace(world, x, y, z, b, meta)
                    ),
                    delay
                );
                double d = ReikaMathLibrary.py3d(
                    ep.posX - x - 0.5, ep.posY - y - 0.5, ep.posZ - z - 0.5
                );
                float v = (float) (0.5 * (1 - d / maxd));
                TickScheduler.instance.scheduleEvent(
                    new ScheduledTickEvent(new ScheduledSoundEvent(
                        ChromaSounds.RIFT, world, ep.posX, ep.posY, ep.posZ, v, 2
                    )),
                    delay
                );
                TickScheduler.instance.scheduleEvent(
                    new ScheduledTickEvent(new ScheduledPacket(
                        ChromatiCraft.packetChannel,
                        ChromaPackets.SUPERBUILD.ordinal(),
                        world,
                        x,
                        y,
                        z,
    ...
    @SideOnly(Side.CLIENT)
    public static void
    doSuperbuildFX(World world, int x, int y, int z, ForgeDirection dir) {
        EntityBlurFX fx;
        double o = 0.0625;
    ...
    @SideOnly(Side.CLIENT)
    private static void
    createSuperbuildParticle(World world, double px, double py, double pz) {
        EntityBlurFX fx = new EntityCCBlurFX(world, px, py, pz)
                              .setLife(30)
                              .setScale(0.625F)
                              .setRapidExpand()
                              .setAlphaFading();
        fx.setColorController(new BlendListColorController(
            new ColorBlendList(10, 0xffffff, 0x22aaff, 0x0000ff, 0x000000)
        ));
        Minecraft.getMinecraft().effectRenderer.addEffect(fx);
    }
    ...
    public static void doChestCollection(EntityPlayerMP ep) {
        ...
        if (ModList.FORESTRY.isLoaded())
            ReikaBeeHelper.collectButterflies(
                ep.worldObj, ReikaAABBHelper.getEntityCenteredAABB(ep, 4), ep
            );
            ...
            if (te instanceof IInventory) {
                ...
                for (int i = 0; i <= s; i++) {
                    ...
                    if (i == s && ii.getStackInSlot(s) == null)
                        breakChest(
                            ep, x, y, z, b
                        ); //made it to last slot and successfully emptied it, or it was
                           //empty
                }
            }
        }
    ...
    private static void emptySlot(
        EntityPlayer ep, int x, int y, int z, Block b, IInventory ii, int slot, int size
    ) {
        ii.setInventorySlotContents(slot, null);
        if (slot == size) { //chest is empty, since only makes it to last slot if all
                            //slots before are empty
    ...
    private static void breakChest(EntityPlayer ep, int x, int y, int z, Block b) {
        ...
        ItemStack is
            = ReikaBlockHelper.getSilkTouch(ep.worldObj, x, y, z, b, meta, ep, false);
        if (is != null) {
            if (!ItemInventoryLinker.tryLinkItem(ep, is)
                && !ReikaInventoryHelper.addToIInv(is, ep.inventory))
                return;
        }
    ...
    public static void doChestCollectionFX(EntityPlayer ep) {
        Collection<LightningBolt> c =
    AbilityHelper.instance.getCollectionBeamsForPlayer(ep); for (LightningBolt b : c) {
            ChromaFX.renderBolt(b, ReikaRenderHelper.getPartialTickTime(), 192, 0.1875,
    6); b.update();
        }
    }*/
    ...
    public static boolean doMobBait(EntityPlayer ep) {
        ...
        if (ep.worldObj.provider.dimensionId != -1 || ep.posY < 127.5) {
            ...
            Entity at = ep.worldObj.findNearestEntityWithinAABB(
                EntityMonsterBait.class, box, ep
            );
            if (at != null)
                return false;
        }
