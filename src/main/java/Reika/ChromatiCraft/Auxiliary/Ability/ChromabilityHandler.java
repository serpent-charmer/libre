...
public class ChromabilityHandler implements TickHandler {
    ...
    public void tick(TickType type, Object... tickData) {
        ...
        for (Ability c : li) {
            if ((!ep.worldObj.isRemote || c.actOnClient())
                && Chromabilities.playerHasAbility(ep, c)
                && Chromabilities.enabledOn(ep, c)) {
        ...
        if (ep.worldObj != null && ModList.MYSTCRAFT.isLoaded()
            && MystPages.Pages.BUFFERDRAIN.existsInWorld(ep.worldObj)) {
            PlayerElementBuffer.instance.removeFromPlayer(
                ep, CrystalElement.randomElement(), 1
            );
        }
