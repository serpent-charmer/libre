...
public class AbilityHotkeys {
    ...
    public static void
    tick(EntityPlayer ep, Ability sel, int data) { //null 'sel' to make it a general tick
    ...
    public static void cacheAbility(EntityPlayer ep, Ability a, int data, int slot) {
        ...
        ReikaChatHelper.sendChatToPlayer(
            ep,
            "Saved ability " + a.getDisplayName() + " to ability hotkey slot " + slot
                + " with power level " + data
        );
    }
    ...
    public static class CachedAbilitySelection {
        ...
        public static CachedAbilitySelection readFromNBT(NBTTagCompound tag) {
            ...
            return a != null ? new CachedAbilitySelection(a, tag.getInteger("data"))
                             : null;
        }
    }
}
