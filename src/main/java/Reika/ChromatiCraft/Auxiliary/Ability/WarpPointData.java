...
public class WarpPointData {
    ...
    private static Collection<WarpPoint> readMapWriter() {
        ...
        File f = new File(
            DragonAPICore.getMinecraftDirectory(), "saves/mapwriter_mp_worlds"
        ); //just read all files for now
    ...
    private static Collection<WarpPoint> readJourneyMap() {
        HashSet<WarpPoint> map = new HashSet();
        File f = new File(
            DragonAPICore.getMinecraftDirectory(), "journeymap/data/mp"
        ); //just read all files for now
        if (f.exists() && f.isDirectory()) {
    ...
    private static Collection<WarpPoint> readVoxelMap() {
        HashSet<WarpPoint> map = new HashSet();
        File f = new File(
            DragonAPICore.getMinecraftDirectory(), "mods/VoxelMods/voxelMap"
        ); //just read all files for now
        if (f.exists() && f.isDirectory()) {
    ...
    private static void readJourneyMapFile(File f, HashSet<WarpPoint> map) {
        ...
        if (e instanceof JsonObject) {
            ...
            if (j.getAsJsonPrimitive("enable").getAsBoolean()) {
                ...
                try {
                    WarpPoint p = new WarpPoint(
                        id, new WorldLocation(dims.get(0).getAsInt(), x, y, z)
                    );
                    map.add(p);
                } catch (Exception ex) {
                    ChromatiCraft.logger.logError(
                        "Could not parse waypoint entry: " + e.toString()
                    );
                    ex.printStackTrace();
                }
            }
            ...
            if (s.startsWith("name")) {
                ...
                try {
                    WarpPoint p = new WarpPoint(
                        "[" + idx + "] " + label,
                        new WorldLocation(
                            Integer.parseInt(dat.get("dimensions")),
                            Integer.parseInt(dat.get("x")),
                            Integer.parseInt(dat.get("y")),
                            Integer.parseInt(dat.get("z"))
                        )
                    );
                    map.add(p);
                } catch (Exception e) {
