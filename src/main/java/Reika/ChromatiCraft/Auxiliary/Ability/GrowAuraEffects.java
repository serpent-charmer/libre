...
public class GrowAuraEffects {
    public static final GrowAuraEffect rainbowLeafFX = new GrowAuraEffect() {
        ...
        public void performEffect(EntityPlayer ep, int x, int y, int z, int power) {
            RainbowTreeEffects.instance.doRainbowTreeEffects(
                ep.worldObj, x, y, z, 4, 0.25, DragonAPICore.rand, false
            );
        }
        @Override
    ...
    public static final GrowAuraEffect cleanRadiation = new GrowAuraEffect() {
        ...
        public void performEffect(EntityPlayer ep, int x, int y, int z, int power) {
            ...
            for (EntityRadiation e :
                 ((List<EntityRadiation>)
                      ep.worldObj.getEntitiesWithinAABB(EntityRadiation.class, box))) {
                e.clean();
            }
        }
        @Override
    ...
    public static final GrowAuraEffect butterflyAttract = new GrowAuraEffect() {
        ...
        public void performEffect(EntityPlayer ep, int x, int y, int z, int power) {
            ReikaBeeHelper.attractButterflies(
                ep.worldObj, ep.posX, ep.posY, ep.posZ, 32, null
            );
        }
        @Override
        public boolean isEffectViable() {
    ...
    public static final BlockBasedGrowAuraEffect fertilize = new BlockBasedGrowAuraEffect(
        ...
        public void performEffect(
            EntityPlayer ep, int x, int y, int z, int power, Block b
        ) {
                ...
                if (world.rand.nextInt(8) == 0) {
                    if (world.rand.nextInt(4) == 0) {
                        ...
                        if (m != NodeModifier.BRIGHT)
                            n.setNodeModifier(
                                m == NodeModifier.FADING ? NodeModifier.PALE
                                                         : NodeModifier.BRIGHT
                            );
                    } else {
                        ...
                        if (t != NodeType.PURE && t != NodeType.NORMAL) {
                            n.setNodeType(
                                t == NodeType.HUNGRY || t == NodeType.TAINTED
                                    ? NodeType.DARK
                                    : t == NodeType.DARK ? NodeType.UNSTABLE
                                                         : NodeType.NORMAL
                            );
                        }
                    }
    ...
    public static final BlockBasedGrowAuraEffect bonemeal = new BlockBasedGrowAuraEffect(
        ...
        public void performEffect(
            ...
            if (ep.worldObj.rand.nextInt(b == Blocks.grass ? 18 : 6) == 0) {
                EntityPlayer fake = ReikaPlayerAPI.getFakePlayerByNameAndUUID(
                    (WorldServer) ep.worldObj, "Random", Chromabilities.FAKE_UUID
                );
                fake.setCurrentItemOrArmor(0, ReikaItemHelper.bonemeal.copy());
                ItemDye.applyBonemeal(
                    fake.getCurrentEquippedItem().copy(), ep.worldObj, x, y, z, fake
                );
            }
        }
        @Override
