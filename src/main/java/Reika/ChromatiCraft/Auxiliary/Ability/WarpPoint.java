...
public class WarpPoint implements Comparable<WarpPoint> {
    ...
    void teleportPlayerTo(EntityPlayerMP ep) {
        ChromatiCraft.logger.log(
            "Warping player " + ep.getCommandSenderName() + " to warppoint " + this
        );
        if (ep.worldObj.provider.dimensionId != location.dimensionID) {
            DimensionManager.getWorld(location.dimensionID)
                .getBlock(location.xCoord, location.yCoord, location.zCoord); //force load
            ReikaEntityHelper.transferEntityToDimension(ep, location.dimensionID);
        }
        //ep.setPositionAndUpdate(location.xCoord+0.5, location.yCoord+0.25,
        //location.zCoord+0.5);
        ep.playerNetServerHandler.setPlayerLocation(
            location.xCoord + 0.5,
            location.yCoord + 0.5,
            location.zCoord + 0.5,
            ep.rotationYaw,
    ...
    public boolean canTeleportPlayer(EntityPlayer ep) {
        ...
        if (location.dimensionID == ExtraChromaIDs.DIMID.getValue()
            || dim == ExtraChromaIDs.DIMID.getValue())
    ...
    public int compareTo(WarpPoint o) {
        if (label.startsWith("[") && o.label.startsWith("[") && label.length() > 1
            && o.label.length() > 1) { //minimap ordering
            ...
            if (Character.isDigit(s1.charAt(0)) && Character.isDigit(s2.charAt(0))) {
                ...
                if (ReikaJavaLibrary.isValidInteger(s1)
                    && ReikaJavaLibrary.isValidInteger(s2)) {
