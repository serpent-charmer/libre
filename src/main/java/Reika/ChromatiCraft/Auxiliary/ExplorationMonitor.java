...
public class ExplorationMonitor implements TickHandler {
    ...
    public void tick(TickType type, Object... tickData) {
        ...
        if (!world.isRemote) {
            ...
            //ProgressionManager.instance.setPlayerDiscoveredColor(ep, CrystalElement.RED,
            //true);
            ...
            if (mov != null) {
                ...
                if (ChromaTiles.getTile(world, x, y, z) == ChromaTiles.PYLON) {
                    TileEntityCrystalPylon te
                        = (TileEntityCrystalPylon) world.getTileEntity(x, y, z);
                    if (te.hasStructure()
                        && te.getEnergy(te.getColor())
                            >= te.getMaxStorage(te.getColor()) / 10) {
                        ProgressionManager.instance.setPlayerDiscoveredColor(
                            ep, te.getColor(), true, true
                        );
                        if (ModList.THAUMCRAFT.isLoaded()
                            && ReikaItemHelper.matchStacks(
                                ep.getCurrentEquippedItem(),
                ...
                if (b instanceof ProgressionTrigger) {
                    ProgressStage[] ps
                        = ((ProgressionTrigger) b).getTriggers(ep, world, x, y, z);
                    if (ps != null) {
            ...
            if (world.provider.dimensionId == 0 && ep.posY < 18
                && world.getSavedLightValue(EnumSkyBlock.Sky, x0, y0, z0) == 0) {
                ProgressStage.DEEPCAVE.stepPlayerTo(ep);
            }

            if (world.provider.dimensionId == 0
                && ChromatiCraft.isRainbowForest(world.getBiomeGenForCoords(x0, z0))) {
                ProgressStage.RAINBOWFOREST.stepPlayerTo(ep);
            }

            if (world.provider.dimensionId == 0
                && BiomeGlowingCliffs.isGlowingCliffs(world.getBiomeGenForCoords(x0, z0)
                )) {
