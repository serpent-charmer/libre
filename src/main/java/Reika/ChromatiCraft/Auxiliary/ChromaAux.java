...
public class ChromaAux {
    public static final Color[] sideColors
        = { Color.CYAN,   Color.BLUE, Color.YELLOW, Color.BLACK, new Color(255, 120, 0),
            Color.MAGENTA };
    public static final String[] sideColorNames
        = { "CYAN", "BLUE", "YELLOW", "BLACK", "ORANGE", "MAGENTA" };

    //private static final GenerationInterceptWorld cliffRelayWorld = new
    //GenerationInterceptWorld(); private static final GenerationInterceptWorld
    //rainbowRelayWorld = new GenerationInterceptWorld();

    private static HashMap<String, ArrayList<CrystalElement>> fluidRunes = new HashMap();
    private static final MultiMap<TargetData, CrystalElement> beamColorMixes
        = new MultiMap();
    ...
    public static final IWGWatcher slimeIslandBlocker = new IWGWatcher() {
        ...
        public boolean canIWGRun(
            ...
            if (ReikaChunkHelper
                    .chunkContainsBiomeType(world, cx, cz, BiomeGlowingCliffs.class)) {
    ...
    public static final InterceptionException dimensionException
        = new InterceptionException() {
              @Override
              public boolean doesExceptionApply(
                  World world, int x, int y, int z, Block set, int meta
              ) {
                ...
                if (ChromatiCraft.isRainbowForest(b)) {
                    ...
                    if (dat.newBlock == ThaumItemHelper.BlockEntry.TOTEM.getBlock())
                        ...
                        TileEntityMobSpawner tm
                            = (TileEntityMobSpawner) dat.getTileEntity(world);
                        MobSpawnerBaseLogic lgc = tm.func_145881_a();
                        lgc.activatingRangeFromPlayer
                            = lgc.getEntityNameToSpawn().equals("CaveSpider")
                                && lgc.getSpawnerY() < 55
                            ? 12
                            : 6;
                        lgc.minSpawnDelay *= 2;
                    ...
                    if (ModList.CHISEL.isLoaded()
                        && dat.oldBlock == ChromaBlocks.CLIFFSTONE.getBlockInstance()
                        && ChiselBlockHandler.isWorldgenBlock(
                            dat.newBlock, dat.newMetadata
                        )) {
                        dat.revert(world);
                    } else if (dat.newBlock == Blocks.mob_spawner) {
                        TileEntityMobSpawner tm
                        ...
                        if (tm == null)
                            continue;
                        if (ReikaSpawnerHelper.getMobSpawnerMobName(tm)
                                .toLowerCase(Locale.ENGLISH)
                                .contains("wisp")) {
                            ReikaSpawnerHelper.setMobSpawnerMob(
                                tm, ChromaEntities.GLOWCLOUD.entityName
                            );
                        }
                    }
    ...
    hasGui(World world, int x, int y, int z, EntityPlayer ep) {
        ...
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT) {
            Object GUI
                = ChromaGuiHandler.instance.getClientGuiElement(0, ep, world, x, y, z);
            if (GUI != null)
                return true;
        }
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER) {
            Object GUI
                = ChromaGuiHandler.instance.getServerGuiElement(0, ep, world, x, y, z);
            if (GUI != null)
                return true;
        }
        return false;
    ...
    public static void
    spawnInteractionBallLightning(World world, int x, int y, int z, CrystalElement e) {
        if (!world.isRemote && ChromaOptions.BALLLIGHTNING.getState()) {
            ...
            double dy = world.getTopSolidOrLiquidBlock(dx, dz)
                + ReikaRandomHelper.getSafeRandomInt(8
                ); //ReikaRandomHelper.getRandomPlusMinus(y+0.5, 16);
            world.spawnEntityInWorld(
                new EntityBallLightning(world, e, dx + 0.5, dy + 0.5, dz + 0.5)
                    .setNoDrops()
            );
        }
    }
    ...
    public static void
    doPylonAttack(CrystalElement color, EntityLivingBase e, float amt) {
        doPylonAttack(color, e, amt, false, 0, false);
    }

    public static void doPylonAttack(
        ...
        if (e instanceof EntityPlayer) {
            ...
            //DO NOT UNCOMMENT, AS ALLOWS DISCOVERY OF ALL COLORS BEFORE
            //PREREQ//ProgressionManager.instance.setPlayerDiscoveredColor(ep, color,
            //true);
            ...
            if (taperNew) {
                if (e.ticksExisted < 600) {
                    ...
                    amt = 1
                        + (e.ticksExisted - 600)
                            / 100; //increase by 1/2 heart every 5 seconds, up to 2.5
                                   //hearts at 50 seconds
                }
            }
        }
        ...
        e.attackEntityFrom(
            ChromatiCraft.pylonDamage[color == null ? 16 : color.ordinal()], amt
        );
        ...
        if (e.getHealth() > Math.max(0, last - originalAmt)) {
            if (originalAmt >= last) { //kill
                e.setHealth(0.1F);
                e.attackEntityFrom(
                    ChromatiCraft.pylonDamage[color == null ? 16 : color.ordinal()],
                    Float.MAX_VALUE
                );
            } else
    ...
    public static float getIslandBias(
        World world, float originalBias, float dx, float dz, int chunkX, int chunkZ
    ) { //chunkX and Z are actually /8
        /*
        ...
        //ReikaJavaLibrary.pConsole(distsq+" @ "+chunkX*8+","+chunkZ*8+" >
        //"+dx*16+","+dz*16);
        if (distsq < EndOverhaulManager.MIN_DIST_SQ_CH) {
            //ReikaJavaLibrary.pConsole(" > "+originalBias);
            return originalBias
                - MathHelper.sqrt_float(Math.max(0, distsq - 100)) * 4.0F; //vanilla
        } else if (distsq < EndOverhaulManager.MAX_DIST_SQ_CH) {
            double ang = Math.toDegrees(Math.atan2(dz, dx))
                + EndOverhaulManager.getAngleOffset(world);
            ang = (ang % 360 + 360) % 360;
            double da = Math.min(
                ang % EndOverhaulManager.DEGREE_SEPARATION,
                EndOverhaulManager.DEGREE_SEPARATION
                    - (ang % EndOverhaulManager.DEGREE_SEPARATION)
            ); //0-6
            double diff
                = (EndOverhaulManager.MAX_DIST_SQ_CH - EndOverhaulManager.MIN_DIST_SQ_CH)
                * 0.5;
            double df = (distsq <= EndOverhaulManager.MIN_DIST_SQ_CH + diff
                             ? distsq - EndOverhaulManager.MIN_DIST_SQ_CH
                             : EndOverhaulManager.MAX_DIST_SQ_CH - distsq)
                / diff;
    ...
    public static MultiMap<TargetData, CrystalElement>
    getBeamColorMixes(Collection<CrystalTarget> c) {
    ...
    public static void
    changePylonColor(World world, TileEntityCrystalPylon te, CrystalElement e) {
        try {
            ...
            te = (TileEntityCrystalPylon
            ) world.getTileEntity(te.xCoord, te.yCoord, te.zCoord);
            CrystalNetworker.instance.removeTile(old);
            PylonGenerator.instance.removeCachedPylon(old);
            te.setColor(e);
            BlockArray runes
                = te.getRuneLocations(te.worldObj, te.xCoord, te.yCoord, te.zCoord);
            for (int i = 0; i < runes.getSize(); i++) {
                ...
                if (c.getBlock(te.worldObj) == ChromaBlocks.RUNE.getBlockInstance())
                    te.worldObj.setBlockMetadataWithNotify(
                        c.xCoord, c.yCoord, c.zCoord, te.getColor().ordinal(), 3
                    );
            }
    ...
    public static void notifyServerPlayersExcept(EntityPlayer ep, ProgressElement p) {
        String sg = EnumChatFormatting.GOLD + ep.getCommandSenderName()
            + EnumChatFormatting.RESET
            + " has learned something new: " + p.getFormatting() + p.getTitle();
            ...
            for (EntityPlayer ep2 : ((List<EntityPlayer>) ws[i].playerEntities)) {
                if (ep2 != ep) {
                    ...
                    ReikaChatHelper.sendChatToPlayer(
                        ep2,
                        "You have learned something new: " + p.getFormatting()
                            + p.getTitle()
                    );
                }
            }
    ...
    { world = Minecraft.getMinecraft().theWorld; int base =
    defaultLightBrightness((World)world, x, y, z, val); double dist =
    EntityFlyingLight.getClosestLight((World)world, x, y, z);
        //dist = Math.min(dist, Minecraft.getMinecraft().thePlayer.getDistance(x+0.5,
    y+0.5, z+0.5)); int l = 0; int m = 5; if (dist < m) { l = 15;
        }
        else if (dist < 15+m) {
            l = 15+m-(int)dist;
        }
        //ReikaJavaLibrary.pConsole(Integer.toHexString(base)+"; "+l, dist <
    Double.POSITIVE_INFINITY); return ReikaMathLibrary.bitRound(base, 20) | (l << 4);
    }
    ...
    private static int defaultLightBrightness(World world, int x, int y, int z, int val) {
        ...
        int block = Math.max(val, world.getSkyBlockTypeBrightness(EnumSkyBlock.Block, x,
    y, z)); return sky << 20 | block << 4;
    }
     */

    @Deprecated
    public static void
    permuteRunes(HashMap<Coordinate, CrystalElement> map, World world, EntityPlayer ep) {}
    ...
    public static boolean chargePlayerFromPylon(
        EntityPlayer player, ChargingPoint te, CrystalElement e, int tick, boolean doFX
    ) {
        if (te.canConduct() && te.allowCharging(player, e)
            && allowPlayerChargingAt(player, te, e)) {
            int add = Math.max(
                1,
                (int
                ) (PlayerElementBuffer.instance.getChargeSpeed(player)
            ...
            if (add > 0 && PlayerElementBuffer.instance.canPlayerAccept(player, e, add)) {
                ...
                //player.getEntityData().setLong("lastSelfCharge",
                ...
                if (te instanceof TileEntityCrystalPylon)
                    ProgressionManager.instance.setPlayerDiscoveredColor(
                        player, ((TileEntityCrystalPylon) te).getColor(), true, true
                    );
    ...
    public static void
    chargePlayerTools(EntityPlayer player, ChargingPoint te, CrystalElement e) {
        ...
        if (at != null) {
            ...
            if (at.getItem() instanceof ActivatedInventoryItem) {
                ...
                if (((ActivatedInventoryItem) at.getItem()).isSlotActive(at, slot)) {
                    ItemStack in
                        = ((ActivatedInventoryItem) at.getItem()).getItem(at, slot);
    ...
    private static void tryChargePoweredItem(
        ItemStack in, EntityPlayer player, ChargingPoint te, CrystalElement e
    ) {
        if (in.stackTagCompound == null
            || !in.stackTagCompound.getBoolean("bufferlinked"))
            return;
        ...
        if (pi.getColor(in) == e) {
            ...
            if (sc > 0) {
                int rate = Math.max(
                    0,
                    (int
                    ) (te.getHeldToolChargingPower(player, e, in) * sc
                       * ToolChargingSystem.instance.getChargeRate(in))
                );
                if (rate > 0)
    ...
    allowPlayerChargingAt(EntityPlayer player, ChargingPoint te, CrystalElement e) {
        return true;
    }

    public static List<AxisAlignedBB>
    interceptEntityCollision(World world, Entity e, AxisAlignedBB box) {
        if (e instanceof EntityPlayer
            && world.provider.dimensionId == ExtraChromaIDs.DIMID.getValue()) {}
        if (e instanceof EntityPlayer
            && Chromabilities.ORECLIP.enabledOn((EntityPlayer) e)) {
            return AbilityHelper.instance.getNoclipBlockBoxes((EntityPlayer) e, box);
        } else {
            return getSurrogateCollidingAABBs(
                world, e, box
            ); //e.worldObj.getCollidingBoundingBoxes(e, box);
        }
    }

    public static List<AxisAlignedBB>
    getSurrogateCollidingAABBs(World world, Entity ep, AxisAlignedBB box) {
            ...
            for (int z = i1; z < j1; ++z) {
                if (world.blockExists(x, 64, z)) {
                    for (int y = k - 1; y < l; ++y) {
                        ...
                        if (x >= -30000000 && x < 30000000 && z >= -30000000
                            && z < 30000000) {
                        ...
                        if (world.provider.dimensionId == ExtraChromaIDs.DIMID.getValue()
                            && ChromaDimensionManager.isBlockedAir(
                                world, x, y, z, block, ep
                            )) {
                            Blocks.stone.addCollisionBoxesToList(
                                world, x, y, z, box, li, ep
                            );
                            ChromaDimensionManager.onPlayerBlockedFromBiome(
                                world, x, y, z, ep
                            );
                            flag = false;
                        } else if (world.isRemote && isParticle(ep)) {
                            if (ChromaFX.cancelParticleBlockCollision(
                                    world, x, y, z, block, ep
                                ))
                                flag = false;
                        } else if (ep instanceof EntityGlowCloud) {
                            if (EntityGlowCloud.isBlockNonColliding(
                                    world, x, y, z, block
                                )) {
                                flag = false;
                            }
                        } else if (ep instanceof EntityItem) {
                            if (((EntityItem) ep).getEntityItem().getItem()
                                == ChromaItems.FERTILITYSEED.getItemInstance()) {
                                if (block == ChromaBlocks.DYELEAF.getBlockInstance()
                                    || block == ChromaBlocks.DECAY.getBlockInstance()
                                    || block == ChromaBlocks.GLOWLEAF.getBlockInstance())
                                    flag = false;
                        ...
                        if (flag) {
                            block.addCollisionBoxesToList(
                                world, x, y, z, box, li, ep
                            ); //necessary for vanilla and mods like TreeClimbing
                        }
                    }
                }
            }
        ...
        double d0 = 0.25D;
        List<Entity> list
            = world.getEntitiesWithinAABBExcludingEntity(ep, box.expand(d0, d0, d0));
    ...
    public static AxisAlignedBB getInterceptedCollisionBox(
        Entity e, World world, int x, int y, int z, AxisAlignedBB def
    ) {
        Block b = world.getBlock(x, y, z);
        if (b == ChromaBlocks.SELECTIVEGLASS.getBlockInstance()
            && (e instanceof IProjectile || e instanceof EntityFireball
                || e instanceof CustomProjectile)) {
        ...
        if (e instanceof EntityLaserPulse
            && b == ChromaBlocks.LASEREFFECT.getBlockInstance()) {
            return null;
        }
        return def;
    }
    ...
    /*
    public static MovingObjectPosition getInterceptedRaytrace(Entity e, Vec3 vec1, Vec3
    vec2) { return getInterceptedRaytrace(e, vec1, vec2, false, false, false);
    }
     */
    public static MovingObjectPosition getInterceptedRaytrace(
        Entity e,
        Vec3 vec1,
        Vec3 vec2,
        boolean b1,
    ...
    public static TileEntityLootChest generateLootChest(
        World world,
        int x,
        int y,
        int z,
        int m,
        Random rand,
        ChestGenLootLocation loc,
        int bonus
    ) {
        if (y < 0 || y > 256) {
            ChromatiCraft.logger.logError(
                "Tried to generate a loot chest outside the map!"
            );
            return null;
        }
    ...
    public static int groundOpacity(IBlockAccess iba, int x, int y, int z, Block b) {
        if (iba instanceof World) {
            World w = (World) iba;
            if (!(ModList.MYSTCRAFT.isLoaded() && ReikaMystcraftHelper.isMystAge(w))
                && w.getWorldInfo().getTerrainType() == WorldType.FLAT
        ...
        return b2 == ChromaBlocks.CLIFFSTONE.getBlockInstance()
                && BlockCliffStone.isTransparent(iba, x, y, z)
            ? 0
            : b.getLightOpacity();
    }
    ...
    public static void dischargeIntoPlayer(
        double x,
        double y,
        double z,
        Random rand,
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            ChromaPackets.FIREDUMPSHOCK.ordinal(),
            e.worldObj,
            (int) x,
            (int) y,
            (int) z,
    ...
    public static boolean interceptClientChunkUpdates(ChunkProviderClient p) {
        return MonumentCompletionRitual.areRitualsRunning()
            ? false
            : p.unloadQueuedChunks(
            ); //contrary to name all this does is update lighting/sky height
    }

    public static void
    logTileCacheError(World world, WorldLocation loc, TileEntity te, ChromaTiles tile) {
        ChromatiCraft.logger.logError(
            "Incorrect tile (" + te + ") @ " + loc + " (with " + loc.getBlockKey(world)
            + ") in " + tile.getName() + " cache!?"
        );
        if (loc.getBlock(world) == tile.getBlock()
            && loc.getBlockMetadata(world) == tile.getBlockMetadata()) {
    ...
    ExtraChromaIDs.DIMID.getValue()) { DimensionStructureGenerator struct =
    ChromaDimensionManager.getStructurePlayerIsIn(Minecraft.getMinecraft().thePlayer); if
    (struct != null && struct.getType() == DimensionStructureType.NONEUCLID) val =
    Minecraft.getMinecraft().theWorld.rand.nextDouble()*10000;
        }
        return val instanceof Number ? ((Number)val).doubleValue() : val;
    }*/

    public static boolean verifyCustomRecipeOutputItem(ItemStack is, boolean throwExc)
        throws Exception {
        Exception ex = null;
        if (is.getItem() instanceof ItemChromaBasic
            || is.getItem().getClass().getName().startsWith("Reika.ChromatiCraft"))
            ex = new IllegalArgumentException(
                "This item is not allowed as an output, as it is a native ChromatiCraft item with its own recipe."
            );
        else if (SensitiveItemRegistry.instance.contains(is))
            ex = new IllegalArgumentException(
                "This item is not allowed as an output, as it is a progression-sensitive item."
            );
        if (ex != null) {
            if (throwExc)
                throw ex;
