...
public class ProtectionBeaconStructure extends ChromaStructureBase {
    ...
    public FilledBlockArray getArray(World world, int x, int y, int z) {
        ...
        for (int i = -2; i <= 2; i++) {
            for (int k = -2; k <= 2; k++) {
                array.setBlock(
                    x + i, y - 1, z + k, crystalstone, StoneTypes.SMOOTH.ordinal()
                );
            }
        }
        ...
        for (int i = -2; i <= 2; i++) {
            array.setBlock(
                x + 3, y - 1, z + i, crystalstone, StoneTypes.GROOVE2.ordinal()
            );
            array.setBlock(
                x - 3, y - 1, z + i, crystalstone, StoneTypes.GROOVE2.ordinal()
            );
            array.setBlock(
                x + i, y - 1, z + 3, crystalstone, StoneTypes.GROOVE1.ordinal()
            );
            array.setBlock(
                x + i, y - 1, z - 3, crystalstone, StoneTypes.GROOVE1.ordinal()
            );
