...
public class BurrowStructure extends FragmentStructureBase {
    ...
    public static void buildLootCache() {
        ...
        for (int i = 0; i < 16; i++) {
            lootItems.addEntry(
                new ItemDrop(ChromaItems.SHARD.getStackOfMetadata(i), 2, 8), 5
            );
            lootItems.addEntry(
                new ItemDrop(ChromaBlocks.CRYSTAL.getStackOfMetadata(i), 2, 8), 1
            );
        }
    ...
    private final TileCallback furnaceCall = new TileCallback() {
        ...
        public void onTilePlaced(World world, int x, int y, int z, TileEntity te) {
            world.setBlock(
                ...
                y + 1,
                z,
                ChromaBlocks.HEATLAMP.getBlockInstance(),
                ForgeDirection.UP.ordinal(),
                2
            );
            ((TileEntityHeatLamp) world.getTileEntity(x, y + 1, z)).temperature
                = ReikaRandomHelper.getRandomBetween(50, 160);
            ...
            ItemStack is = ReikaJavaLibrary.getRandomCollectionEntry(
                world.rand, ore.getAllOreBlocks()
            );
            is = is.copy();
    ...
    private final TileCallback keyChestCall = new TileCallback() {
        ...
        public void onTilePlaced(World world, int x, int y, int z, TileEntity te) {
            if (doorID != null) {
                ...
                ((TileEntityChest) te)
                    .setInventorySlotContents(world.rand.nextInt(27), this.getDoorKey());
            } else {
                throw new IllegalStateException("Burrow has no chest ID!");
            }
        }
    ...
    private final TileCallback lootCall = new TileCallback() {
        ...
        public void onTilePlaced(World world, int x, int y, int z, TileEntity tile) {
            ...
            int filled = ReikaRandomHelper.getRandomBetween(
                13, 20
            ); //was 9 & 27, then 18 & 27, then 8 & 16, then 12 & 18
            ...
            if (ReikaBiomeHelper.isSnowBiome(biome) && world.rand.nextInt(4) == 0) {
                add.add(new ItemStack(
                    Blocks.ice, ReikaRandomHelper.getRandomBetween(12, 32), 0
                ));
            }
                ...
                if (tree != null) {
                    if (tree.getSaplingID() != null) {
                        ItemStack sapling = new ItemStack(
                            tree.getSaplingID(),
                            ReikaRandomHelper.getRandomBetween(1, 6),
                            tree.getSaplingMeta()
                        );
                        add.add(sapling);
                    }
    ...
    public FilledBlockArray getArray(World world, int x, int y, int z) {
        ...
        //ReikaJavaLibrary.pConsole("Running burrow lamp "+this.getCurrentColor()+" on
    ...
    public FilledBlockArray getLootRoom(World world, int x, int y, int z) {
        ...
        array.setBlock(
            x + 3, y + 4, z + 6, shield, BlockType.CRACK.metadata
        ); //crack to expose chest
    ...
    private void placeDoor(FilledBlockArray array, int x, int y, int z) {
        array.setBlock(
            ...
            BlockChromaDoor.getMetadata(false, false, true, true)
        );
        if (this.isWorldgen())
    ...
    @Override
    public int
    getChestYield(Coordinate c, TileEntityLootChest te, FilledBlockArray arr, Random r) {
        return c.yCoord <= arr.getMinY() + 4 ? 0 : -1;
    }

    @Override
    public String getChestLootTable(
        Coordinate c, TileEntityLootChest te, FilledBlockArray arr, Random r
    ) {
        return ChestGenHooks.BONUS_CHEST;
    }

    @Override
    public int modifyLootCount(
        TileEntityLootChest tileEntityLootChest, String s, int bonus, Random r, int count
    ) {
    ...
    @Override
    public float
    getFragmentChance(TileEntityLootChest te, String s, int bonus, Random r) {
