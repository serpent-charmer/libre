...
public class NetworkOptimizerStructure extends ChromaStructureBase {
    ...
    public FilledBlockArray getArray(World world, int x, int y, int z) {
        ...
        for (int j = 3; j <= 7; j++) {
            ...
            for (int i = -r; i <= r; i++) {
                for (int k = -r; k <= r; k++) {
                    if (j == 3) {
                        array.setBlock(
                            x + i, y - j, z + k, ChromaBlocks.LUMA.getBlockInstance(), 0
                        );
                    } else {
                        StoneTypes s = j == 5 && Math.abs(i) == r && Math.abs(k) == r
                            ? StoneTypes.BRICKS
                            : StoneTypes.SMOOTH;
        ...
        for (int i = -r; i <= r; i++) {
            array.setBlock(
                x + i, y - 7, z - r, crystalstone, StoneTypes.SMOOTH.ordinal()
            );
            array.setBlock(
                x + i, y - 7, z + r, crystalstone, StoneTypes.SMOOTH.ordinal()
            );
            array.setBlock(
                x - r, y - 7, z + i, crystalstone, StoneTypes.SMOOTH.ordinal()
            );
            array.setBlock(
                x + r, y - 7, z + i, crystalstone, StoneTypes.SMOOTH.ordinal()
            );
