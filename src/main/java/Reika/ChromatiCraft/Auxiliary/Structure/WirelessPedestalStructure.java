...
public class WirelessPedestalStructure extends ChromaStructureBase {
    ...
    public FilledBlockArray getArray(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            array.setEmpty(
                x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ, false, false
            );
        }
            ...
            for (int j = -1; j <= 1; j += 2) {
                for (int k = -1; k <= 1; k += 2) {
                    ...
                    if (ModList.MYSTCRAFT.isLoaded())
                        array.addBlock(x+i, y+j, z+k,
        MystCraftHandler.getInstance().crystalID);
                }
            }
        }
         */
        ...
        for (int i = -3; i <= 3; i++) {
            for (int k = -3; k <= 3; k++) {
                ...
                if (m != StoneTypes.SMOOTH.ordinal()) {
                    int m2 = m == StoneTypes.BRICKS.ordinal()
                ...
                if (Math.abs(i) <= 2 && Math.abs(k) <= 2) {
                    if (Math.abs(i) > 1 || Math.abs(k) > 1) {
                        array.setBlock(
                            x + i, y - 5, z + k, crystalstone, StoneTypes.SMOOTH.ordinal()
                        );
                        array.setBlock(
                            x + i, y - 4, z + k, ChromaBlocks.LUMA.getBlockInstance()
                        );
                    }
                }
            }
