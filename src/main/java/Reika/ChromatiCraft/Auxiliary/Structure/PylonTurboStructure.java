...
public class PylonTurboStructure extends PylonStructure {
    ...
    public FilledBlockArray getArray(World world, int x, int y, int z) {
        ...
        BlockKey[] col = new BlockKey[] {
            ...
            new BlockKey(
                ChromaTiles.PYLONTURBO.getBlock(),
                ChromaTiles.PYLONTURBO.getBlockMetadata()
            ),
        };
        ...
        for (int l = 0; l < Location.list.length; l++) {
            ...
            for (int i = 0; i < col.length; i++) {
                array.setBlock(
                    x + c.xCoord, y + 1 + i, z + c.zCoord, col[i].blockID, col[i].metadata
                );
            }
        }
        ...
        for (Coordinate c : TileEntityCrystalPylon.getPowerCrystalLocations()) {
            this.setTile(
                array, x + c.xCoord, y + 9 + c.yCoord, z + c.zCoord, ChromaTiles.CRYSTAL
            );
        }
