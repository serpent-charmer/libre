...
public class MusicTempleStructure extends ChromaStructureBase {
    private static final Block fluid
        = Blocks.water; //ChromaBlocks.LIFEWATER.getBlockInstance();
    ...
    static {
        for (int idx = 0; idx < 8; idx++) {
            ...
            for (int i = 0; i <= 6; i++) {
                StoneTypes s = i == 0
                    ? StoneTypes.EMBOSSED
                    : (i == 6 ? StoneTypes.ENGRAVED : StoneTypes.COLUMN);
                pillars[idx].put(
                    root.offset(0, i, 0), new BlockKey(crystalstone, s.ordinal())
                );
            }
            pillars[idx].put(
                root.offset(1, 6, 0),
                new BlockKey(crystalstone, StoneTypes.BEAM.ordinal())
            );
            pillars[idx].put(
                root.offset(-1, 6, 0),
                new BlockKey(crystalstone, StoneTypes.BEAM.ordinal())
    ...
    public FilledBlockArray getArray(World w, int x, int y, int z) {
        ...
        Block ringB = ChromaBlocks.STRUCTSHIELD.getBlockInstance(
        ); //Blocks.lapis_block;//ChromaBlocks.DIMGEN.getBlockInstance();
