...
public final class ChromaDescriptions {
    ...
    private static final HashMap<ChromaResearch, String> data
        = new HashMap<ChromaResearch, String>();
    private static final PluralMap<String> notes = new PluralMap(2);

    private static final HashMap<ChromaTiles, Object[]> machineData
        = new HashMap<ChromaTiles, Object[]>();
    private static final HashMap<ChromaTiles, Object[]> machineNotes
        = new HashMap<ChromaTiles, Object[]>();
    private static final HashMap<ChromaBlocks, Object[]> blockData
        = new HashMap<ChromaBlocks, Object[]>();
    private static final HashMap<ChromaItems, Object[]> itemData
        = new HashMap<ChromaItems, Object[]>();
    private static final HashMap<ChromaItems, Object[]> itemNotes
        = new HashMap<ChromaItems, Object[]>();
    private static final HashMap<ChromaResearch, Object[]> miscData
        = new HashMap<ChromaResearch, Object[]>();
    private static final EnumMap<Chromabilities, Object[]> abilityData
        = new EnumMap(Chromabilities.class);
    private static final HashMap<String, Object[]> hoverData
        = new HashMap<String, Object[]>();
    private static final HashMap<CrystalElement, Object[]> elementData
        = new HashMap<CrystalElement, Object[]>();

    private static final HashMap<String, String> hoverText
        = new HashMap<String, String>();
    private static final EnumMap<ProgressStage, ProgressNote> progressText
        = new EnumMap(ProgressStage.class);
    private static final EnumMap<Chromabilities, String> abilityText
        = new EnumMap(Chromabilities.class);
    private static final EnumMap<CrystalElement, String> elementText
    ...
    private static boolean hasLocalizedFor(Language language) {
        String lang = language.getLanguageCode();
        try (
            InputStream o = ChromatiCraft.class.getResourceAsStream(
    ...
    private static void loadRosetta() {
        if (Minecraft.getMinecraft().thePlayer != null)
            LoreManager.instance.getOrCreateRosetta(Minecraft.getMinecraft().thePlayer)
                .loadText();
    }
    ...
    public static void loadData() {
        ...
        for (ChromaResearch h : machinetabs) {
            ...
            if (m == ChromaTiles.ADJACENCY) {
                ...
                int i = 0;
                for (String s : pages) {
                    String text = machines.getValueAtNode(
                        "machines:" + m.name().toLowerCase(Locale.ENGLISH) + s
                    );
                ...
                for (AlvearyEffect e : TileEntityLumenAlveary.getEffectSet()) {
                    String text = machines.getValueAtNode(
                        "machines:" + m.name().toLowerCase(Locale.ENGLISH) + ":"
                ...
                String desc = machines.getValueAtNode(
                    "machines:" + m.name().toLowerCase(Locale.ENGLISH) + DESC_SUFFIX
                );
                String aux = machines.getValueAtNode(
                ...
                if (m.isCrystalNetworkTile()) {
                    CrystalNetworkTile te
                        = (CrystalNetworkTile) m.createTEInstanceForRender(0);
                    if (te instanceof CrystalReceiver) {
                        aux += String.format(
                            "Lumen Reception Range: %d m\n",
                            ((CrystalReceiver) te).getReceiveRange()
                        );
                    }
                    if (te instanceof CrystalTransmitter) {
                        aux += String.format(
                            "Lumen Transmission Range: %d m\n",
                ...
                if (m.hasPrerequisite()) {
                    ...
                    aux += "\nDependencies: "
                        + ReikaStringParser.capitalizeWords(
                            ReikaStringParser.splitCamelCase(sg).replaceAll(
                                " Craft", "Craft"
                            )
                        );
                }
        ...
        for (ChromaResearch h : blocktabs) {
            String desc
                = blocks.getValueAtNode("blocks:" + h.name().toLowerCase(Locale.ENGLISH));
            if (h == ChromaResearch.HEATLAMP) {
                desc = blocks.getValueAtNode(
                    "blocks:" + h.name().toLowerCase(Locale.ENGLISH) + ":desc"
                );
                notes.put(
                    blocks.getValueAtNode(
                        "blocks:" + h.name().toLowerCase(Locale.ENGLISH) + ":heat"
                    ),
                    h,
                    0
                );
                notes.put(
                    blocks.getValueAtNode(
                        "blocks:" + h.name().toLowerCase(Locale.ENGLISH) + ":cold"
                    ),
                    h,
                    1
                );
        ...
        for (ChromaResearch h : tooltabs) {
            ...
            if (h.getItem().getItemInstance() instanceof DynamicallyGeneratedSubpage
                && ((DynamicallyGeneratedSubpage) h.getItem().getItemInstance())
                       .replaceOriginal()) {
                ...
                notes.put(
                    String.format(
                        tools.getValueAtNode(key + ":notes"), itemNotes.get(h.getItem())
                    ),
                    h,
                    0
                );
        ...
        for (ChromaResearch h : structuretabs) {
            String desc = structures.getValueAtNode(
                "structure:" + h.name().toLowerCase(Locale.ENGLISH)
            );
            addEntry(h, desc);
        }
        ...
        for (ChromaResearch h : abilitytabs) {
            ...
            String desc = abilities.getValueAtNode(
                "ability:" + a.name().toLowerCase(Locale.ENGLISH)
            );
        ...
        for (CrystalElement e : CrystalElement.elements) {
            String desc = elements.getValueAtNode(
                "elements:" + e.name().toLowerCase(Locale.ENGLISH)
            );
        ...
        for (int i = 0; i < ProgressStage.list.length; i++) {
            ...
            String title = progress.getValueAtNode(
                "progression:" + p.name().toLowerCase(Locale.ENGLISH) + ":title"
            );
            String hint = progress.getValueAtNode(
                "progression:" + p.name().toLowerCase(Locale.ENGLISH) + ":hint"
            );
            String reveal = progress.getValueAtNode(
                "progression:" + p.name().toLowerCase(Locale.ENGLISH) + ":reveal"
            );
            String desc = progress.getValueAtNode(
                "progression:" + p.name().toLowerCase(Locale.ENGLISH) + ":desc"
            );
        ...
        for (int i = 0; i < ChromaEnchants.enchantmentList.length; i++) {
            ...
            String desc = enchants.getValueAtNode(
                "enchants:" + e.name().toLowerCase(Locale.ENGLISH)
            );
            notes.put(desc, ChromaResearch.ENCHANTS, i + 1);
        }
        String desc = enchants.getValueAtNode("enchants:boostedlevel");
        notes.put(
            desc, ChromaResearch.ENCHANTS, ChromaEnchants.enchantmentList.length + 1
        );
    ...
    private static void loadNumericalData() {
        ...
        //addNotes(ChromaTiles.TELEPUMP,
        //TileEntityTeleportationPump.getRequiredEnergy().toDisplay());
        //addNotes(ChromaTiles.MINER, TileEntityMiner.getRequiredEnergy().toDisplay());

        //addNotes(ChromaTiles.REPROGRAMMER,
        //TileEntitySpawnerReprogrammer.getRequiredEnergy().toDisplay());
        addNotes(
            ChromaTiles.TANK,
            TileEntityCrystalTank.FACTOR / 1000,
            TileEntityCrystalTank.MAXCAPACITY / 1000
        );
        addNotes(ChromaTiles.CHARGER, TileEntityCrystalCharger.CAPACITY);
        //addNotes(ChromaTiles.TICKER,
        //TileEntityInventoryTicker.getRequiredEnergy().toDisplay());
        ...
        addNotes(
            ChromaTiles.FABRICATOR,
            FabricationRecipes.FACTOR,
            FabricationRecipes.INITFACTOR,
            FabricationRecipes.POWER
        );
        addNotes(
            ChromaTiles.BEACON,
            TileEntityCrystalBeacon.RATIO,
            TileEntityCrystalBeacon.POWER,
            TileEntityCrystalBeacon.MAXRANGE
        );
        ...
        addNotes(
            ChromaTiles.POWERTREE,
            TileEntityPowerTree.BASE,
            TileEntityPowerTree.RATIO,
            TileEntityPowerTree.POWER
        );
        addNotes(
            ChromaTiles.LAMPCONTROL,
            TileEntityLampController.MAXRANGE,
            TileEntityLampController.MAXCHANNEL
        );
        addNotes(
            ChromaTiles.ASPECTJAR,
            TileEntityAspectJar.CAPACITY_PRIMAL,
            TileEntityAspectJar.CAPACITY
        );
        addNotes(
            ChromaTiles.WIRELESS,
            TileEntityWirelessSource.TRANSMIT_RANGE,
            ChromaStructures.WIRELESSPEDESTAL.getDisplayName(),
            ChromaTiles.WIRELESS.getName()
        );
        addNotes(
            ChromaTiles.TOOLSTORAGE, TileEntityToolStorage.ToolType.getTypesAsString()
        );
        ...
        addNotes(
            ChromaTiles.MANABOOSTER,
            TileEntityManaBooster.FLOWER_RANGE,
            TileEntityManaBooster.POOL_RANGE
        );
        ...
        addData(
            ChromaItems.SHARE, ChromaTiles.TABLE.getName(), ChromaTiles.RITUAL.getName()
        );
        ...
        addData(
            Chromabilities.LIFEPOINT, new Object[] { CrystalElement.MAGENTA.displayName }
        );

        miscData.put(
            ChromaResearch.ENCHANTS, new Object[] { ChromaTiles.ENCHANTER.getName() }
        );
    }
    ...
    public static ProgressNote getProgressText(ProgressStage p) {
        return progressText.containsKey(p)
            ? progressText.get(p)
            : new ProgressNote("#NULL", "#NULL", "#NULL", "#NULL");
    }
