        ...
        public ItemStack doAction(
            ...
            if (te instanceof TileEntityItemStand) {
                ((TileEntityItemStand) te)
                    .onRightClickWith(
                        null, ReikaDispenserHelper.getDispenserPlayer(ref, is)
                    );
            }
            if (te instanceof TileEntityAuraInfuser) {
                ((TileEntityAuraInfuser) te)
                    .onRightClickWith(
                        null, ReikaDispenserHelper.getDispenserPlayer(ref, is)
                    );
            }
    ...
    public static class ProjectileToolDispenserAction extends DispenserAction {
        ...
        public ItemStack doAction(
            ...
            if (i instanceof ProjectileFiringTool) {
                ...
                ep.setLocationAndAngles(
                    ref.getX(),
                    ref.getY() - 1.62,
                    ref.getZ(),
                    -ReikaDirectionHelper.getRelativeAngle(dir, ForgeDirection.SOUTH),
                    0
                );
                ((ProjectileFiringTool) i).fire(is, world, ep, false);
            } else {
                ChromatiCraft.logger.logError(
                    "Invalid item " + is.getDisplayName()
                    + " for projectile-tool dispenser action!"
                );
            }
            return is;
        }
