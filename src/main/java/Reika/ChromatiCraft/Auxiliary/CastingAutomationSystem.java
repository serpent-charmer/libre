...
public class CastingAutomationSystem {
    ...
    public CastingAutomationSystem(CastingAutomationBlock te) {
        ...
        if (ModList.APPENG.isLoaded()) {
            aeGridBlock = new BasicAEInterface(
                (TileEntity) te,
                ((TileEntityChromaticBase) te).getTile().getCraftedProduct()
            );
    ...
    protected final boolean matches(Object object, ItemStack is) {
        if (object instanceof ItemStack) {
            return ReikaItemHelper.matchStacks(is, (ItemStack) object)
                && ItemStack.areItemStackTagsEqual(is, (ItemStack) object);
        } else if (object instanceof List) {
            return ReikaItemHelper.listContainsItemStack(
                (Collection<ItemStack>) object, is, true
            );
    ...
    protected final int countItem(Object item) {
        ...
        if (ModList.APPENG.isLoaded()) {
            ChromatiCraft.logger.debug(
                "Delegate " + tile + " counting " + li + " from " + ingredients + " / "
                + network
            );
        } else {
            ChromatiCraft.logger.debug(
        ...
        for (ItemStack is : li) {
            if (ModList.APPENG.isLoaded() && network != null) {
                if (is.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
                    ret += network.getFuzzyItemCount(
                        is, FuzzyMode.IGNORE_ALL, false, is.stackTagCompound != null
                    );
                } else {
                    ret += network.getItemCount(is, is.stackTagCompound != null);
                }
                ChromatiCraft.logger.debug(
                    tile + " failed to find any " + is + " in its ME System."
                );
            }
    ...
    protected final Collection<ItemStack>
    findItems(Object item, int amt, boolean simulate) {
    ...
    getExtraItems(Object item, int amt, boolean simulate, boolean allowMultiple) {
        return null;
    }

    private final ArrayList<ItemStack>
    getItems(Object item, int amt, boolean simulate, boolean allowMultiple) {
        List<ItemStack> li = new ArrayList();
        if (item instanceof ItemStack)
            li.add((ItemStack) item);
        if (item instanceof List)
        ...
        if (DragonAPICore.debugtest)
            return ReikaJavaLibrary.makeListFrom(
                ReikaItemHelper.getSizedItemStack(li.get(0), amt)
            );

        if (ModList.APPENG.isLoaded()) {
            ChromatiCraft.logger.debug(
                "Delegate " + tile + " requesting " + li + " from " + ingredients + " / "
                + network
            );
        } else {
            ChromatiCraft.logger.debug(
        ...
        for (ItemStack is : li) {
            if (ModList.APPENG.isLoaded() && network != null) {
                if (is.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
                    ExtractedItemGroup rem = network.removeItemFuzzy(
                        ReikaItemHelper.getSizedItemStack(is, wanted),
                        simulate,
                    ...
                    if (rem != null) {
                        for (ExtractedItem ei : rem.getItems()) {
                            ItemStack is2 = ReikaItemHelper.getSizedItemStack(
                                ei.getItem(), (int) ei.amount
                            );
                            is2.setItemDamage(0);
                            ret.add(is2);
                    ...
                    int rem = (int) network.removeItem(
                        ReikaItemHelper.getSizedItemStack(is, wanted),
                        simulate,
                        is.stackTagCompound != null
                    );
                    if (rem > 0) {
                        ...
                        //network.triggerCrafting(worldObj, is, amt, null, null); GOD DAMN
                        //IT AE
                    }
                }
                ChromatiCraft.logger.debug(
                    tile + " failed to find " + is + " in its ME System."
                );
                if (wanted <= 0)
                    break;
            }
        ...
        if (te != null && recipe != null && recipesToGo > 0 && tile.isAbleToRun(te)) {
            ...
            if (tile.canCraft(world, x, y, z, te)) {
                if (this.isRecipeReady(world, x, y, z, te)) {
                    ...
                    int amt = Math.min(
                        recipesToGo,
                        recipe.getOutput().getMaxStackSize()
                            / recipe.getOutput().stackSize
                    );
                    if (recipe instanceof MultiBlockCastingRecipe) {
                        MultiBlockCastingRecipe mr = (MultiBlockCastingRecipe) recipe;
                        amt = Math.min(
                            amt,
                            mr.getMainInput().getMaxStackSize()
                                / mr.getRequiredCentralItemCount()
                        );
                    }
                    ...
                    if (c != null) {
                        recipeCycles = recipeCycles > 0
                            ? Math.min(recipeCycles, c.getItem().stackSize)
                            : c.getItem().stackSize;
                        ChromaSounds.CAST.playSoundAtBlock(
                            world, c.loc.xCoord, c.loc.yCoord, c.loc.zCoord
                        );
                        int[] dat
                            = new int[] { c.loc.xCoord,
                                          c.loc.yCoord,
                                          c.loc.zCoord,
                                          0,
                                          Item.getIdFromItem(c.getItem().getItem()),
                                          c.getItem().getItemDamage(),
                                          c.getItem().stackTagCompound != null ? 1 : 0 };
                        ReikaPacketHelper.sendDataPacketWithRadius(
                            ChromatiCraft.packetChannel,
                            ChromaPackets.CASTAUTOUPDATE.ordinal(),
                            (TileEntity) tile,
                            48,
                            dat
                        );
    ...
    private UpdateStep prepareRecipeStep(
        World world, int x, int y, int z, TileEntityCastingTable te, int amt
    ) {
        stepDelay.update();
            ...
            if (recipe instanceof MultiBlockCastingRecipe) {
                ...
                for (List<Integer> key : map.keySet()) {
                    ...
                    if (stand != null) {
                        ...
                        if ((item == null && in != null)
                            || (item != null && !item.match(in))) {
                            if (in != null) {
                                ...
                                //ReikaJavaLibrary.pConsole("Looking for "+item+", got
                                //"+ret);
                                if (ret != null) {
                ...
                if (tile.canPlaceCentralItemForMultiRecipes()) {
                    ...
                    for (int i = 0; i < 9; i++) {
                        ...
                        if (i == 4) {
                            if (in != null) {
                                if (ReikaItemHelper.matchStacks(in, ctr)
                                    && in.stackSize >= mr.getRequiredCentralItemCount()
                                    && (ctr.stackTagCompound == null
                                        || ItemStack.areItemStackTagsEqual(in, ctr))) {
                                    //matches
                                } else {
                                    if (this.recoverItem(in)) {
                                ...
                                ItemStack ret = this.findItem(
                                    ctr, amt * mr.getRequiredCentralItemCount(), false
                                );
                                //ReikaJavaLibrary.pConsole("Looking for center item
                                //"+ctr+", got "+ret);
                                if (ret != null) {
    ...
    private boolean
    triggerCrafting(World world, int x, int y, int z, TileEntityCastingTable te) {
        return te.triggerCrafting(currentPlayer);
    }

    private boolean
    isRecipeReady(World world, int x, int y, int z, TileEntityCastingTable te) {
    ...
    public final void receiveUpdatePacket(World world, int[] data) {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            switch (rand.nextInt(3)) {
                case 0:
                    fx = new EntityCenterBlurFX(world, px, py, pz)
                             .setColor(color)
                             .setGravity(g);
                    break;
                case 1:
                    fx = new EntityLaserFX(CrystalElement.WHITE, world, px, py, pz)
                             .setColor(color)
                             .setGravity(g);
                    break;
                case 2:
