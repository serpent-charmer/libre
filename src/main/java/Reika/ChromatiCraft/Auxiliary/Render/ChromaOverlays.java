...
public class ChromaOverlays {
    ...
    private final RemoteSourcedAsset crosshair
        = ChromaClient.dynamicAssets.createAsset("Textures/crosshair.png");
    ...
    private static final ColorBlendList chromaColor
        = new ColorBlendList(8F, ChromaFX.getChromaColorTiles());
    ...
    public void renderInWorld(EntityRenderingLoopEvent evt) {
        ...
        -RenderManager.renderPosZ); GL11.glDisable(GL11.GL_LIGHTING);
        GL11.glEnable(GL11.GL_BLEND);
        GL11.glShadeModel(GL11.GL_SMOOTH);
        GL11.glDisable(GL11.GL_TEXTURE_2D);
        GL11.glDisable(GL11.GL_CULL_FACE);

        //ReikaRenderHelper.renderTube(-279, 8, 724, -283, 8, 724, 0xff0000ff, 0xff77aaff,
        0.5, 0.5);
    ...
    //Not highest because of Dualhotbar
    public void renderHUD(RenderGameOverlayEvent.Pre evt) {
        ...
        boolean fullScreenRendersActive
            = FullScreenOverlayRenderer.instance.isRenderingHexGroups();
        ...
        if (evt.type == ElementType.HELMET) {
            ...
            if (PylonGenerator.instance.canGenerateIn(ep.worldObj)
                || FullScreenOverlayRenderer.instance.isPylonOverlayForced())
    ...
    private boolean isEnergyDisplayTool(ItemStack is) {
        return is != null
            && (ChromaItems.TOOL.matchWith(is) || is.getItem() instanceof ItemWandBase);
    }
    ...
    private void renderStructureText(EntityPlayer ep, int gsc) {
        ...
        if (structureText != null) {
            ...
            float frac = (ep.worldObj.getTotalWorldTime() - structureTextTick
                          + ReikaRenderHelper.getPartialTickTime())
                / 50F;
            ChromaFontRenderer.FontType.GUI.renderer.drawTitleScroll(
                structureText, x, y, frac, 0xff888888, 0xff3090ff, 0xff000000, 0xffffffff
            );
            if (frac >= 2) {
                structureText = null;
            }
    ...
    private void renderFlareMessages(int gsc) {
        ...
        int y = (Minecraft.getMinecraft().displayHeight
                 - ChromaFontRenderer.FontType.GUI.renderer.FONT_HEIGHT - 16)
                / gsc
            - dy * flareMessages.size();
    ...
    private void renderPingOverlays(EntityPlayer ep, int gsc) {
        ...
        for (Entry<Ping, Integer> e : pings.entrySet()) {
            ...
            if (tick != null) {
                Ping p = e.getKey();
                float alpha = tick >= (PING_LENGTH - FADEIN)
                    ? (PING_LENGTH - tick) / (float) FADEIN
                    : tick < FADEOUT ? tick / (float) FADEOUT
                                     : 1;
                ...
                if (!renderCircle) {
                    ...
                    ReikaTextureHelper.bindTexture(
                        ChromatiCraft.class, "Textures/dimping.png"
                    );
                ...
                double dr
                    = MathHelper.clamp_double(r * Math.pow(dist, 1.5) / 1000000D, 2, r);
                ...
                if (p.color != null
                    && ProgressionManager.instance.hasPlayerCompletedStructureColor(
                        ep, p.color
                    )) {
                    ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                        dx + 8, dy + 10, ChromaIcons.CHECK.getIcon(), s / 2, s / 2
                    );
                }
    ...
    private void renderTransitionHUD(EntityPlayer ep, ScaledResolution sr, ItemStack is) {
        ...
        ChromaFontRenderer.FontType.HUD.renderer.drawString(
            mode.desc, x + 20, y + 4, 0xffffff, true
        );
        GL11.glDisable(GL11.GL_LIGHTING);
    }
    ...
    private void renderOreHUD(EntityPlayer ep, ScaledResolution sr, ItemStack is) {
        ...
        if (ico == null) {
            ChromatiCraft.logger.logError(
                "Ore Ping Delegate " + otype + " for " + otype.getPrimary()
                + " has a null icon!"
            );
    ...
    private void renderKillAuraCrosshair(RenderGameOverlayEvent.Pre evt, int gsc) {
        ...
        for (int i = -1; i <= p; i++) {
            ...
            if (i == -1)
                i2 = "0b";
            ReikaTextureHelper.bindTexture(
                ChromatiCraft.class, "Textures/killlaura_" + i2 + ".png"
            );
            int alpha = 255;
            float f = (float) (t - m);
        ...
        BlendMode.DEFAULT.apply();
        //GL11.glDisable(GL11.GL_DEPTH_TEST); //turn off depth testing to avoid this
        //occluding other elements
    ...
    private void renderBoostedHealthBar(RenderGameOverlayEvent.Pre evt, EntityPlayer ep) {
        ...
        if (ep.isPotionActive(Potion.regeneration)
            || ep.isPotionActive(ChromatiCraft.betterRegen)) {
        ...
        boolean highlight
            = ep.hurtResistantTime >= 10 && ep.hurtResistantTime / 3 % 2 == 1;
    ...
    private void renderAbilityStatus(EntityPlayer ep, int gsc) {
        ...
        for (Ability c : li) {
            ...
            ReikaTextureHelper.bindTexture(
                c.getTextureReferenceClass(), c.getTexturePath(!flag)
            );
            Tessellator v5 = Tessellator.instance;
            v5.startDrawingQuads();
            v5.setColorOpaque_I(0xffffff);
            int x = Minecraft.getMinecraft().displayWidth / gsc - 20;
            int y = Minecraft.getMinecraft().displayHeight / gsc / 2 - 8
                - (int) (li.size() / 2F * 20) + i * 20;
    ...
    private int getPieX(int r, int space, int gsc) {
        return ChromaOptions.PIELOC.getValue() < 2
            ? r + space
            : Minecraft.getMinecraft().displayWidth / gsc - r - space;
    }

    private int getPieY(int r, int space, int gsc) {
        return ChromaOptions.PIELOC.getValue() % 2 == 0
            ? r + space
            : Minecraft.getMinecraft().displayHeight / gsc - r - space - 16;
    }
    ...
    private void renderElementWarnings(EntityPlayer ep, int gsc, Collection<Ability> c) {
        ...
        if (i > 0) {
            ...
            ReikaTextureHelper.bindFinalTexture(
                DragonAPICore.class, "Resources/warning.png"
            );
            GL11.glEnable(GL11.GL_BLEND);
            v5.startDrawingQuads();
    ...
    private void renderElementPie(EntityPlayer ep, int gsc) {
        ...
        ReikaGuiAPI.instance.drawCenteredString(
            f, s, ox, oy + r + f.FONT_HEIGHT - 4, 0xffffff
        );
    }
    ...
    public void addBottleneckWarning(
        int dim, int x, int y, int z, int level, boolean isThroughput, CrystalElement e
    ) {
        PylonFinderOverlay.instance.addBottleneckWarning(
            new WorldLocation(dim, x, y, z), e, WarningLevels.list[level], isThroughput
        );
    }
        ...
        private void render(int gsc, int x, int y, int color) {
            ...
            if (charSplit >= 0) {
                if (charSplit >= text.length()) {
                    ...
                    int x2
                        = x + ChromaFontRenderer.FontType.GUI.renderer.getStringWidth(s1);
                    ChromaFontRenderer.FontType.OBFUSCATED.renderer.drawString(
                        s2, x2, y, c
                    );
                }
            } else {
