    ...
    public void onBlockFailure(World world, int x, int y, int z, BlockCheck seek) {
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT) {
            ...
            ReikaPacketHelper.sendDataPacketWithRadius(
                ...
                ChromaPackets.STRUCTUREERROR.ordinal(),
                world,
                x,
                y,
                z,
                32,
                Block.getIdFromBlock(bk.blockID),
                bk.metadata
            );
        }
    }
    ...
    private static class ErrorOverlay {
        ...
        public void renderArea(Tessellator v5) {
            ...
            if (age % 5 == 0) {
                ...
                String off = ReikaRandomHelper.doWithChance(0.033)
                    ? ReikaJavaLibrary
                          .getRandomCollectionEntry(DragonAPICore.rand, box.getVertices())
                          .ID
                    : null;
                if (off != null) {
        ...
        public void jitter(String bigOffset, double r) {
            for (CubeVertex cv : renderBox.getVertices()) {
                ...
                cv.offset(
                    ReikaRandomHelper.getRandomPlusMinus(0, dr),
                    ReikaRandomHelper.getRandomPlusMinus(0, dr),
                    ReikaRandomHelper.getRandomPlusMinus(0, dr)
                );
            }
