...
public class WorldRenderIntercept implements WorldShaderSystem {
    ...
    private final THashMap<Integer, WorldRenderer> listMap = new THashMap(
        16 * DEFAULT_CAPACITY_COMPONENT * DEFAULT_CAPACITY_COMPONENT, 0.98F
    );
    ...
    private void runIntercept(IntBuffer lists) {
        ...
        if (VoidMonsterDestructionRitual.ritualsActive()) {
            ...
            for (EffectVisual e :
                 VoidMonsterRitualClientEffects.EffectVisual.getTerrainShaders()) {
        ...
        while (lists.remaining() > 0) {
            ...
            if (f > 0) {
                ...
                //ReikaJavaLibrary.pConsole("Running GL list # "+id+" which maps to chunk
                //"+c);
                if (c != null) {
                ...
                s.getShader().setTextureUnit(
                    "bgl_LightMapTexture", OpenGlHelper.lightmapTexUnit
                );
