...
public class RuneShapeRenderer {
    ...
    public void render(RuneViewer s, int midx, int midy) {
        int y = s.isEmpty()
            ? 0
            : s.getMinY() + (int) ((System.currentTimeMillis() / 5000) % s.getSizeY());
    ...
    public void render(RuneViewer s, int midx, int midy, int y) {
        ...
        ItemStack(ChromaBlocks.PYLONSTRUCT.getBlockInstance(), 1, 0), px, py);
            }
        }*//*
        ...
        for (int x = -5; x <= 5; x++) {
            for (int z = -5; z <= 5; z++) {
                ...
                if (x == 0 && z == 0) {
                    ico = ChromaTiles.TABLE.getBlock().getIcon(
