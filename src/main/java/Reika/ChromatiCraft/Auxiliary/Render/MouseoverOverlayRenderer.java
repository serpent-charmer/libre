...
public class MouseoverOverlayRenderer {
    public static final MouseoverOverlayRenderer instance
        = new MouseoverOverlayRenderer();
    ...
    void renderTileOverlays(EntityPlayer ep, int gsc) {
        ...
        if (pos != null) {
            ...
            if (te instanceof FocusAcceleratable
                && ChromaResearch.FOCUSCRYSTALS.playerHas(ep)) {
                GL11.glPushAttrib(GL11.GL_ALL_ATTRIB_BITS);
                GL11.glPushMatrix();
    ...
    private void renderStatusOverlay(EntityPlayer ep, int gsc, OperationInterval te) {
        ...
        if (state == OperationState.RUNNING) {
            ...
            for (double a = 0; a < ma; a += da) {
                ...
                //ReikaJavaLibrary.pConsole(a+">"+x+","+y+" @ "+du+","+dv+" from
                //"+u+","+v);
    ...
    private void
    renderAccelerationOverlay(EntityPlayer ep, int gsc, FocusAcceleratable te) {
        int ar = 30;
        int ox = Minecraft.getMinecraft().displayWidth / (gsc * 2) + ar + 8;
        int oy = Minecraft.getMinecraft().displayHeight / (gsc * 2) - ar - 8;
        ...
        v5.addVertexWithUV(ox + r / 2, oy + r / 2, 0, u + 0.0625, v + 0.0625);
        float f
            = (te.getAccelerationFactor() - 1) / (te.getMaximumAcceleratability() - 1);
        double ma = 360 * f;
        double da = 0.25;
        for (double a = 0; a < ma; a += da) {
            double dx = Math.sin(Math.toRadians(a + 90));
    ...
    public void renderStorageOverlay(EntityPlayer ep, int gsc, LumenTile lt) {
        ...
        /*
                CrystalElement e =
           CrystalElement.elements[(int)(System.currentTimeMillis()/500%16)]; int amt =
           tag.getValue(e); String s = String.format("%.0f%s",
           ReikaMathLibrary.getThousandBase(amt), ReikaEngLibrary.getSIPrefix(amt));
                Minecraft.getMinecraft().fontRenderer.drawStringWithShadow(s, ox, oy+r/2,
           ReikaColorAPI.mixColors(e.getColor(), 0xffffff, 0.5F));
         */
    }

    private void
    renderItemContents(EntityPlayer ep, int gsc, TileEntityChromaCrafter te) {
        ...
        for (ItemStack is : c.keySet()) {
            ...
            ReikaGuiAPI.instance.drawItemStack(
                ...
                ReikaItemHelper.getSizedItemStack(is, c.get(is)),
                x,
                y
            );
            i++;
        ...
        if (berries > 0) {
            ...
            int clr = ReikaColorAPI.mixColors(
                0xffff0000,
                0xffffff,
                Math.min(1, berries / (float) TileEntityChroma.ETHER_SATURATION)
            );
            ReikaGuiAPI.instance.drawRectFrame(
                ox - r, my - r + 1, mx - ox + 18 + r * 2, oy - my + 16 + r * 2, clr
            );
            String s = String.valueOf(berries) + " "
                + ChromaStacks.etherBerries.getDisplayName();
            int ds = 1;
            FontType.GUI.renderer.drawString(
                s,
                mx + 19 - FontType.GUI.renderer.getStringWidth(s) + ds,
                oy + 21 + ds,
                ReikaColorAPI.getColorWithBrightnessMultiplier(clr, 0.65F)
            );
            FontType.GUI.renderer.drawString(
                s,
                mx + 19 - FontType.GUI.renderer.getStringWidth(s),
                oy + 21,
                ReikaColorAPI.mixColors(clr, 0xffffffff, 0.45F)
            );
        }
        ...
        if (r != null) {
            ...
            ReikaGuiAPI.instance.drawCenteredString(
                FontType.GUI.renderer, s, x - 3, y, 0xffffff
            );
            ReikaGuiAPI.instance.drawItemStack(
                itemRender,
                FontType.GUI.renderer,
                out,
                x + FontType.GUI.renderer.getStringWidth(s) / 2 + 3,
                y - 4
            );
        }
    }
    ...
    private void
    renderInfuserStatuses(EntityPlayer ep, int gsc, TileEntityPlayerInfuser te) {
        int ar = 16;
        int ox = Minecraft.getMinecraft().displayWidth / (gsc * 2) + ar - 8;
        int oy = Minecraft.getMinecraft().displayHeight / (gsc * 2) - ar - 8;
        ReikaRenderHelper.disableLighting();
        ...
        if (r != null) {
            ...
            if (out != null) {
                ReikaGuiAPI.instance.drawItemStack(itemRender, FontType.GUI.renderer, out,
            x+3, y-4);
            }
             */
            ...
            if (r.isAvailableToPlayer(ep) && !r.playerHas(ep)) {
                ...
                ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                    x + 1, y - 6, ChromaIcons.NOENTER.getIcon(), 16, 16
                );
            }
        } else {
            int dx = x;
            ...
            for (ElementBufferCapacityBoost b : ElementBufferCapacityBoost.list) {
                if (b.playerHas(ep) || b.isAvailableToPlayer(ep)) {
                    ...
                    if (b.playerHas(ep)) {
                        ...
                        ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                            dx + 3 + 8, y + 4, ChromaIcons.CHECK.getIcon(), 8, 8
                        );
                    } else {
                        ItemStack out = b.getIngredient();
                        if (out != null) {
                            ReikaGuiAPI.instance.drawItemStack(
                                itemRender, FontType.GUI.renderer, out, dx + 3 + 6, y + 4
                            );
                            dx += 18;
                        }
                    }
            ...
            if (!flag) {
                ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                    x + 1, y - 6, ChromaIcons.X.getIcon(), 16, 16
                );
            }
        }
    }
    ...
    private void
    renderConditionalBeeProductOverlay(EntityPlayer ep, int gsc, IBeeHousing te) {
        ...
        if (queen != null) {
            ...
            if (bee != null) {
                IAlleleBeeSpecies type
                    = (IAlleleBeeSpecies) bee.getGenome().getActiveAllele(
                ...
                if (type instanceof ConditionalProductBee) {
                    ...
                    ConditionalProductProvider p
                        = ((ConditionalProductBee) type).getProductProvider();
