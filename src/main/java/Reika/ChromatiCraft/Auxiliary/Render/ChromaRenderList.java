...
public class ChromaRenderList {
    private static HashMap<ChromaTiles, ChromaRenderBase> renders
        = new HashMap<ChromaTiles, ChromaRenderBase>();
    private static HashMap<ChromaTiles, ChromaTiles> overrides
        = new HashMap<ChromaTiles, ChromaTiles>();
    ...
    public static TileEntitySpecialRenderer instantiateRenderer(ChromaTiles m) {
        ...
        try {
            ChromaRenderBase r
                = (ChromaRenderBase) Class.forName(m.getRenderer()).newInstance();
            if (addRender(m, r))
                return r;
            else
                return renders.get(overrides.get(m));
