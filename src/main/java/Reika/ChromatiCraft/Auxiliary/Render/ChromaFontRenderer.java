...
public class ChromaFontRenderer extends BasicFontRenderer {
    ...
    protected float renderCharFraction(int charIndex, boolean italic, float fraction) {
        if (type == FontType.OBFUSCATED) {
            //charIndex =
            //(charWidth.length+charIndex/*+(int)posX/16%16-(int)posY/16%16*/+offsets[charIndex])%charWidth.length;
            ...
            charIndex = min
                + (charWidth.length + charIndex % charWidth.length) % (max - min + 1);
        }
    ...
    protected void readFontTexture() {
        if (System.currentTimeMillis() - lastReload < 250)
            return;
        BufferedImage buf
            = ReikaImageLoader.readImage(ChromatiCraft.class, type.texture, null);
        int w = buf.getWidth();
        int h = buf.getHeight();
        int[] rgb = new int[w * h];
    ...
    public void drawTitleScroll(
        String sg, int x, int y, float fraction, int c1, int c2, int cflash1, int cflash2
    ) {
        ...
        for (int d = 2; d <= 6; d++) {
            ...
            this.drawTitleText(
                sg, sx + d / s, sy + d / s, fraction, over, c1o, c2o, cflash1o, cflash2o
            );
        }
    ...
    private void drawTitleText(
        ...
        String pre =
        EnumChatFormatting.BOLD.toString()+EnumChatFormatting.UNDERLINE.toString(); int
        idx = (int)(fraction*2*len); if (idx >= len) { String sg2 = pre+sg; String clip =
        sg2.substring(0, sg2.length()-1); this.drawString(clip, sx, sy, c1);
            this.drawString(sg2.substring(clip.length()),
        sx+(int)(fr.getStringWidth(clip)/s), sy, cflash1); idx -= len;
        }
        else {
            c2 = c1;
        }
        ...
        for (int i = 0; i < pos1; i++) {
            ...
            int ret
                = fr.drawStringFloatPos(String.valueOf(sg.charAt(i)), px, sy, c, false);
        }
