...
public class FullScreenOverlayRenderer {
    ...
    //private final RemoteSourcedAsset pylonAura =
    //ChromaClient.dynamicAssets.createAsset("Textures/aura-bar-half-grid.png");

    private final EnumMap<CrystalElement, Float> factors
    ...
    void renderWashout(RenderGameOverlayEvent.Pre evt, int tick) {
        ...
        int a = (int
        ) (washout > WASHOUT_LENGTH - FLASH_FADE
               ? (255F * (WASHOUT_LENGTH - washout) / FLASH_FADE)
               : 255F * Math.min(1, washout / 255F));
        ...
        //ReikaJavaLibrary.pConsole(washout+" > A="+a+", cfrac = "+(a/255F)+",
    ...
    void renderPylonAura(EntityPlayer ep, int gsc) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            PylonEntry c = containsColor ? null
                                         : PylonGenerator.instance.getNearestPylonSpawn(
                                             ep.worldObj, ep.posX, ep.posY, ep.posZ, e
                                         );
            double dd = containsColor ? 0
                : c != null           ? c.location.getDistanceTo(
                      ep.posX, (ep.posY + c.location.yCoord + 0.5) * 0.5, ep.posZ
                  )
            ...
            double dv = v+1D/imgh;
                ...
                if (res > 0) {
                    if (res > maxIntensity) {
                        ...
                        if (dd < 6) {
                            glowIntensity
                                = Math.min(1, glowIntensity + (float) (6 - dd) / 12F);
                        }
                        glowCenter = c != null ? new Coordinate(c.location) : null;
                    }
                    float mult = Math.min(1, res);
                    /*
                //int color = ReikaColorAPI.getColorWithBrightnessMultiplier(e.getColor(),
                mult); v5.startDrawingQuads(); v5.setBrightness(240);
        ...
        occluding other elements GL11.glPopAttrib();
         */
    ...
    boolean renderLoreHexes(RenderGameOverlayEvent.Pre evt, int tick) {
        if (!renderingGroups.isEmpty()) {
            ...
            //ReikaJavaLibrary.pConsole(Integer.toHexString(c1)+" &
            //"+Integer.toHexString(c2));
            ReikaGuiAPI.instance.drawGradientRect(
                0,
                0,
                evt.resolution.getScaledWidth(),
                evt.resolution.getScaledHeight(),
                c1,
                c2
            );
            GL11.glTranslated(0, 0, -dz);
            ...
            GL11.glPushMatrix();
            //ReikaGuiAPI.instance.drawLine(0, evt.resolution.getScaledHeight()/2,
            //evt.resolution.getScaledWidth(), evt.resolution.getScaledHeight()/2,
            //0xffffffff);
            //ReikaGuiAPI.instance.drawLine(evt.resolution.getScaledWidth()/2, 0,
            //evt.resolution.getScaledWidth()/2, evt.resolution.getScaledHeight(),
            //0xffffffff);
            int i = -renderingGroups.size() / 2;
            double s = 2;
            GL11.glTranslated(
                evt.resolution.getScaledWidth_double() / 2,
                evt.resolution.getScaledHeight_double() / 2,
                800
            );
            GL11.glTranslated(
                -KeyAssemblyPuzzle.CELL_SIZE / 2D * s - 0.5,
                -KeyAssemblyPuzzle.CELL_SIZE / 2D * s + 2,
                0
            );
            GL11.glScaled(s, s, s);
    ...
    private static class TileGroupRender extends FullElementRender {
        ...
        protected void render() {
            KeyAssemblyPuzzle p
                = LoreManager.instance.getPuzzle(Minecraft.getMinecraft().thePlayer);
