...
public class MobSonarRenderer {
    ...
    @SideOnly(Side.CLIENT)
    private void
    renderPoint(EntityEntry loc, int ticks, EntityPlayer ep, Tessellator v5) {
        ...
        GL11.glPushMatrix();
        RenderManager rm = RenderManager.instance;
        GL11.glTranslated(loc.location.xCoord-rm.renderPosX,
        loc.location.yCoord-rm.renderPosY, loc.location.zCoord-rm.renderPosZ);
        ...
        float f = Math.min(1, f1+0.375F);
        double s = 1.5*Math.sqrt(f);
        int c1 = EntityAnimal.class.isAssignableFrom(loc.entityType) ? 0x22ff22 :
        0xffffff; int c = ReikaColorAPI.getColorWithBrightnessMultiplier(c1, f1);
        v5.setColorOpaque_I(c);
    ...
    private static class EntityEntry {
        ...
        public boolean equals(Object o) {
            return o instanceof EntityEntry
                && ((EntityEntry) o).entityID
                == entityID; //.location.equals(location) && ((EntityEntry)o).entityType
                             //== entityType;
        }
    }
