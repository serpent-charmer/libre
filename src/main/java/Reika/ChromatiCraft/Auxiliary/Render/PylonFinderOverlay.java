...
public class PylonFinderOverlay {
    ...
    private static final ColorBlendList powerCrystalDiamondColor
        = new ColorBlendList(200).addAll(ChromaFX.getChromaColorTiles());
    ...
    public void renderPylonFinderHUD(RenderGameOverlayEvent evt) {
        if (evt.type == ElementType.HELMET) {
            ...
            if (PylonGenerator.instance.canGenerateIn(ep.worldObj)) {
                boolean compass
                    = ChromaItems.FINDER.matchWith(ep.getCurrentEquippedItem())
                    || (ep.getEntityData().hasKey(ItemPylonFinder.NBT_KEY)
                        && ep.getEntityData().getLong(ItemPylonFinder.NBT_KEY)
                            >= ep.worldObj.getTotalWorldTime() - 20);
                if (compass || !renders.isEmpty()) {
                    ...
                    if (compass) {
                        for (int i = 0; i < CrystalElement.elements.length; i++) {
                            ...
                            PylonEntry c = PylonGenerator.instance.getNearestPylonSpawn(
                                ep.worldObj,
                                ep.posX,
                                ep.posY + ep.getEyeHeight(),
                                ep.posZ,
                                e
                            );
                            if (c != null) {
                                RenderPosition pos = new RenderPosition(
                                    c.location, ep, yaw, pitch, fov, h
                                );
                                new PylonDisplay(c).render(
                                    v5, ep, pos, fov, h, evt.resolution
                                );
                            }
                        }
                    }
                    ...
                    while (it.hasNext()) {
                        ...
                        RenderPosition pos
                            = new RenderPosition(e.location, ep, yaw, pitch, fov, h);
                ...
                for (CrystalElement e : right) {
                    ...
                    int y = (int)(h+(i-left.size()/2D)*f.FONT_HEIGHT);
                    f.drawString(s,
                evt.resolution.getScaledWidth()-24-f.getStringWidth(s), y, e.getColor());
                    i++;
                }*/
    ...
    public void addBottleneckWarning(
        WorldLocation loc, CrystalElement e, WarningLevels w, boolean isThroughput
    ) {
    ...
    private static class PylonDisplay extends RenderEntry {
        ...
        protected void render(
            ...
            if (pos.phi >= 180 && 360 - fov > pos.phi) {
                ...
                boolean link
                    = loc.playerLink != null && loc.playerLink.equals(ep.getUniqueID());
                ...
                if (link) {
                    ...
                    ChromaFontRenderer.FontType.HUD.drawString(
                        s, (int) cx + 1 - sw / 2, (int) pos.cy + 16, 0xffffff
                    );
                }
                ...
                for (Coordinate c2 : loc.getCrystals()) {
                    ...
                    v5.startDrawingQuads();
                    v5.setColorOpaque_I(powerCrystalDiamondColor.getColor(
                        System.currentTimeMillis() + c2.hashCode() / 2
                    ));
                    v5.setBrightness(240);
                ...
                int base = ReikaMathLibrary.intpow2(
                    10, (int) ReikaMathLibrary.logbase(pos.dl, 10)
                );
                int dist
                    = ReikaMathLibrary.roundToNearestX(base, (int) Math.round(pos.dl));
                String unit = ReikaEngLibrary.getSIPrefix(dist);
                String s = String.format(
                    "%.0f%sm", ReikaMathLibrary.getThousandBase(dist), unit
                );
                ...
                fr.drawString(
                    ...
                    (int) cx + 11 + d,
                    (int) pos.cy + 11 - d,
                    ReikaColorAPI.mixColors(0, e.getColor(), 0.67F)
                );
                fr.drawString(
                    s,
                    (int) cx + 10 + d,
                    (int) pos.cy + 10 - d,
                    ReikaColorAPI.mixColors(0xffffff, e.getColor(), 0.8F)
                );
    ...
    private static class BottleneckWarning extends RenderEntry {
        ...
        private BottleneckWarning(
            WorldLocation loc, CrystalElement e, WarningLevels w, boolean thru
        ) {
            super(loc, 6000);
            level = w;
            isThroughput = thru;
        ...
        protected void render(
            ...
            if (pos.phi >= 180 && 360 - fov > pos.phi) {
                ...
                ReikaTextureHelper.bindTexture(
                    ChromatiCraft.class, "Textures/pylonbottleneck.png"
                );
                GL11.glEnable(GL11.GL_TEXTURE_2D);
                BlendMode.DEFAULT.apply();
                GL11.glEnable(GL11.GL_BLEND);
                ...
                v5.setColorRGBA_I(
                    ReikaColorAPI.mixColors(color.getColor(), 0xffffff, 0.675F), a
                );
                v5.setBrightness(240);
    ...
    private static class RenderPosition {
        ...
        private RenderPosition(
            WorldLocation loc, EntityPlayer ep, float yaw, float pitch, int fov, int h
        ) {
