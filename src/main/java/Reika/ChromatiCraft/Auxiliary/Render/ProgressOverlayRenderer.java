...
public class ProgressOverlayRenderer {
    ...
    private static final int PROGRESS_DURATION
        = Math.max(100, ChromaOptions.PROGRESSDURATION.getValue());

    private final TreeMap<ProgressElement, Integer> progressFlags
        = new TreeMap(new ProgressComparator());
    ...
    void renderProgressOverlays(EntityPlayer ep, int gsc) {
        ...
        for (ProgressElement p : progressFlags.keySet()) {
            ...
            int sh = 24
                + (fr.listFormattedStringToWidth(p.getShortDesc(), sw * 2).size() - 1)
                    * 4; //24;
            int w = sw + 28; //144;
            int h = tick > PROGRESS_DURATION - sh ? PROGRESS_DURATION - tick
                : tick < sh                       ? tick
                                                  : sh;
            ...
            if (h == sh) {
                ...
                fr.drawSplitString(
                    p.getShortDesc(), x + w - 4 - sw, dy + 8 + 4, sw * 2, 0xffffff
                );
                GL11.glPopMatrix();
    ...
    void addProgressionNote(ProgressElement p) {
        ...
        //ReikaJavaLibrary.pConsole("Adding "+p+" to map
        //("+progressFlags.keySet().contains(p)+"), set is "+progressFlags.keySet());
    }
    ...
    private static final class ProgressComparator implements Comparator<ProgressElement> {
        ...
        private int getIndex(ProgressElement e) {
            ...
            if (e instanceof StructureComplete) {
                ...
                return 3000000 + 1000 * ((ChromaResearch) e).level.ordinal()
                    + ((ChromaResearch) e).ordinal();
            } else if (e instanceof CastingRecipe) {
                return 3000000
                    + RecipesCastingTable.instance.getIDForRecipe((CastingRecipe) e);
            } else
                return -1;
        }
    }
