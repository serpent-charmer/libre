...
public class ProbeInfoOverlayRenderer {
    public static final ProbeInfoOverlayRenderer instance
        = new ProbeInfoOverlayRenderer();

    private static final int CONNECTIVITY_DURATION = 480;

    private final EnumMap<CrystalElement, PylonConnection> connectivityFlags
    ...
    void renderConnectivityOverlays(EntityPlayer ep, int gsc) {
        if (!connectivityFlags.isEmpty()) {
            Iterator<Entry<CrystalElement, PylonConnection>> it
                = connectivityFlags.entrySet().iterator();
    ...
    @SideOnly(Side.CLIENT)
    private void renderConnectivityStatus(
        Tessellator v5, EntityPlayer ep, int gsc, PylonConnection pc
    ) {
        int ar = 12;
        IIcon ico = pc.valid
            ? (pc.connected ? ChromaIcons.CHECK.getIcon() : ChromaIcons.X.getIcon())
            : ChromaIcons.NOENTER.getIcon();
    ...
    public void markConnectivity(
        EntityPlayer ep, CrystalElement e, boolean connected, boolean valid
    ) {
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT) {
            ...
            ReikaPacketHelper.sendDataPacket(
                ...
                ChromaPackets.CONNECTIVITY.ordinal(),
                (EntityPlayerMP) ep,
                e.ordinal(),
                connected ? 1 : 0,
                valid ? 1 : 0
