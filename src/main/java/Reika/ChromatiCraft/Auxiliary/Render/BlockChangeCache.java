...
public class BlockChangeCache {
    ...
    public void updateAndVerifyCache(
        World world, int x, int y, int z, BlockKey bk, EntityPlayer ep, ItemStack is
    ) {
        Coordinate loc = new Coordinate(x, y, z);
        boolean sp = ep.isSneaking();

        if (!loc.equals(overlayOrigin) || !bk.equals(overlayBlock) || (isSneak != sp)
            || (is.getItem() != overlayItem)) {
            overlay = null;
        }
        overlayBlock = bk;
        overlayOrigin = loc;
    ...
    public BlockArray getCachedOverlay(
        World world,
        int x,
        int y,
        int z,
