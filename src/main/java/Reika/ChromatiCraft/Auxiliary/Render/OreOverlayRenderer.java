...
public class OreOverlayRenderer {
    ...
    public void loadOres() {
        ...
        blocks.put(
            new BlockKey(Blocks.lit_redstone_ore),
            blocks.get(new BlockKey(Blocks.redstone_ore))
        );
        ...
        ChromatiCraft.logger.log(
            "Initialized ore map with " + blocks.size() + " entries: " + blocks.keySet()
        );
    }
    ...
    public OrePingDelegate getOreTypeByData(ItemStack is) {
        if (is.stackTagCompound == null || !is.stackTagCompound.hasKey("oreType"))
            return null;
        ItemStack ore
            = ItemStack.loadItemStackFromNBT(is.stackTagCompound.getCompoundTag("oreType")
            );
        if (ore == null)
            return null;
    ...
    public void
    addCoordinate(World world, int x, int y, int z, Block id, int meta, boolean ping) {
        if (world.isRemote) {
            ...
            ReikaPacketHelper.sendDataPacket(
                ...
                ChromaPackets.OREPINGLOC.ordinal(),
                world,
                x,
                y,
                z,
                new PacketTarget.RadiusTarget(world, x, y, z, 90),
    ...
    private void renderPoint(WorldLocation loc, OreRender val, EntityPlayer ep) {
        ...
        GL11.glTranslated(
            loc.xCoord + 0.5 - rm.renderPosX,
            loc.yCoord + 0.5 - rm.renderPosY,
            loc.zCoord + 0.5 - rm.renderPosZ
        );
    ...
    public void
    startScan(World world, int x, int y, int z, EntityPlayer ep, OrePingDelegate ore) {
    ...
    private void
    playSoundWithCooldown(SoundEnum s, EntityPlayer ep, float vol, float pitch) {
    ...
    private abstract static class OreOverlayCallback implements BreakerCallback {
        ...
        onAdd(World world, int x, int y, int z, ProgressiveBreaker b, Block id, int meta);
    }
    ...
    private static class OreScanCallback extends OreOverlayCallback {
        ...
        protected void onAdd(
            World world, int x, int y, int z, ProgressiveBreaker b, Block id, int meta
        ) {
    ...
    private static class OrePingCallback extends OreOverlayCallback {
        ...
        protected void onAdd(
            ...
            for (Coordinate c : arr.keySet()) {
                b.exclude(c);
                instance.addCoordinate(
                    world, c.xCoord, c.yCoord, c.zCoord, id, meta, true
                );
            }
        }
    }
        ...
        private OreRender(Block b, int meta, boolean ping) {
            ...
            if (del == null) {
                ChromatiCraft.logger.logError(
                    "Block " + bk + " has null oreping delegate?!"
                );
            }
