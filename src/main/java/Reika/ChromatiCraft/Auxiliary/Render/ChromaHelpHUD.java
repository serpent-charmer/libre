...
public class ChromaHelpHUD {
    ...
    public void renderHUD(RenderGameOverlayEvent evt) {
        if (evt.type == ElementType.HELMET) {
            ...
            boolean recalc
                = ep.ticksExisted % 8 == 0; //Only update 1/8 ticks, to help FPS
            if (recalc)
                this.getData(ep);
            if (/*this.isDifferent(look)*/ text == null || text.isEmpty()
                || text != last_text) {
                this.closePanel();
            } else {
                this.openPanel();
    ...
    private void getData(EntityPlayer ep) {
        ...
        if (look != null) {
            ...
            tierView = block instanceof BlockTieredResource
                ? ((BlockTieredResource) block)
                      .isPlayerSufficientTier(
                          Minecraft.getMinecraft().theWorld, posX, posY, posZ, ep
                      )
                : false;
        } else {
            text = null;
    ...
    private void renderText(String s, int gsc, boolean know) {
        FontRenderer f = know ? ChromaFontRenderer.FontType.HUD.renderer
    ...
    private boolean isDifferent(MovingObjectPosition look) {
        ...
        if (look == null || last_look == null)
            return true;
        //return look.blockX != last_look.blockX || look.blockY != last_look.blockY ||
    look.blockZ != last_look.blockZ; World world = Minecraft.getMinecraft().theWorld;
    }
     */
}
