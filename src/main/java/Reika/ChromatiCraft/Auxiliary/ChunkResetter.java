...
public class ChunkResetter implements TickHandler {
    ...
    private static class ChunkReset {
        ...
        private void step(WorldServer world) {
            //ReikaJavaLibrary.pConsole("Ticking reset of
            //"+regen.getChunkCoordIntPair()+"; "+new Coordinate(stepX, stepY, stepZ));
        ...
        private void onFinish(WorldServer world) {
            world.getChunkFromChunkCoords(regen.xPosition, regen.zPosition)
                .isTerrainPopulated
                = false;
            generator.populate(generator, regen.xPosition, regen.zPosition);
            GameRegistry.generateWorld(
                regen.xPosition, regen.zPosition, world, generator, generator
            );
            ReikaJavaLibrary.pConsole(
                "Finished regenning " + regen.getChunkCoordIntPair()
            );
        }
        ...
        private void resetCoordinate(WorldServer world) {
            ...
            //ReikaJavaLibrary.pConsole("Replacing "+prev+" with "+b+" @ "+wx+",
            //"+stepY+", "+wz);
        ...
        private void updateStepPosition() {
            ...
            if (flag1) {
                stepZ++;
                //ReikaJavaLibrary.pConsole(stepY+" > "+stepZ+":"+range+" >
