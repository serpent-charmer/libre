...
public class BiomeStructurePuzzle implements FragmentStructureData {
    ...
    public void generate(Random rand) {
        ...
        ArrayList<CrystalElement> li
            = ReikaJavaLibrary.makeListFromArray(CrystalElement.elements);
        for (int i = 0; i < 8; i++) {
            doorColors.add(li.remove(rand.nextInt(li.size())));
        }

        ArrayList<Integer> idx
            = ReikaJavaLibrary.makeIntListFromArray(ReikaArrayHelper.getLinearArray(8));
        ...
        while (melody.isEmpty()) {
            int attempts = 0;
            MelodyPrefab pre
                = MusicPuzzleGenerator.getRandomPrefab(rand, Integer.MAX_VALUE, excl);
            ...
            if (attempts >= 10) {
                ...
                ChromatiCraft.logger.log(
                    "Failed to perform " + pre + " with only eight colors"
                );
            }
        }
    ...
    public void addToArray(FilledBlockArray array, int x0, int y0, int z0) {
        ...
        for (int i = 0; i < 4; i++) {
            for (Entry<Coordinate, ForgeDirection> e :
                 this.getSwitchDoorLocations(root, i).entrySet()) {
                Coordinate c = e.getKey();
                array.setBlock(
                    c.xCoord,
                    c.yCoord,
                    c.zCoord,
                    ChromaBlocks.SHIFTLOCK.getBlockInstance(),
                    Passability.CLOSED.ordinal()
                );
            }
        }
        ...
        for (Coordinate c : this.getSwitchLocations(root)) {
            array.setBlock(
                c.xCoord, c.yCoord, c.zCoord, ChromaBlocks.PANELSWITCH.getBlockInstance()
            );
            ...
            array.setBlock(
                ...
                red.yCoord,
                red.zCoord,
                ChromaBlocks.LIGHTPANEL.getBlockInstance(),
                3
            );
            array.setBlock(
                green.xCoord,
        ...
        for (Coordinate c : this.getBarrierLocations(root)) {
            array.setBlock(
                c.xCoord, c.yCoord, c.zCoord, ChromaBlocks.DOOR.getBlockInstance()
            );
        }

        for (int i = 0; i < 4; i++) {
            for (Entry<Coordinate, CrystalElement> e :
                 this.getCrystalLocations(root).entrySet()) {
                Coordinate c = e.getKey();
                array.setBlock(
                    c.xCoord,
                    c.yCoord,
                    c.zCoord,
                    ChromaBlocks.LAMP.getBlockInstance(),
                    e.getValue().ordinal()
                );
            }
        }
    ...
    public void placeData(World world, TileEntityStructControl root) {
        ...
        for (Entry<Coordinate, CrystalElement> e : runes.entrySet()) {
            ...
            c.setBlock(
                world, ChromaBlocks.RUNE.getBlockInstance(), e.getValue().ordinal()
            );
        }
        ...
        for (int i = 0; i < 4; i++) {
            for (Entry<Coordinate, ForgeDirection> e :
                 this.getSwitchDoorLocations(ref, i).entrySet()) {
                SwitchGroup gr = doorKeys[i];
                Passability p
                    = Passability.getDirectionalPassability(e.getValue(), false);
                p = Passability.CLOSED;
                e.getKey().setBlock(
                    world, ChromaBlocks.SHIFTLOCK.getBlockInstance(), p.ordinal()
                );
            }
        }
        ...
        world.setBlock(
            ...
            root.yCoord + 10,
            root.zCoord,
            ChromaBlocks.DIMDATA.getBlockInstance(),
            0,
            3
        );
        TileEntityStructureDataStorage tile = (TileEntityStructureDataStorage
        ) world.getTileEntity(root.xCoord, root.yCoord + 10, root.zCoord);
        ...
        for (int i = 2; i < 4 /*6*/; i++) {
            ...
            world.setBlock(
                dx, dy, dz, ChromaBlocks.LOCKKEY.getBlockInstance(), keyIndex, 3
            );
        ...
        for (int i = 0; i < 4; i++) {
            for (Entry<Coordinate, CrystalElement> e :
                 this.getCrystalLocations(ref).entrySet()) {
                e.getKey().setBlock(
                    world, ChromaBlocks.LAMP.getBlockInstance(), e.getValue().ordinal()
                );
            }
        }
    ...
    private Block getLiquid(World world, Coordinate root) {
        BiomeGenBase b
            = ReikaWorldHelper.getNaturalGennedBiomeAt(world, root.xCoord, root.zCoord);
    ...
    private CrystalElement findMostEffective(HashSet<MusicKey> needed) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            if (crystalColors.contains(e))
                continue;
            ArrayList<MusicKey> li
                = new ArrayList(CrystalMusicManager.instance.getKeys(e));
            li.retainAll(needed);
            if (li.size() > bestAmt) {
                best = e;
                bestAmt = li.size();
    ...
    private static class SwitchGroup {
        ...
        public boolean isSameCorner() {
            boolean c1 = areaNS == ForgeDirection.NORTH && areaEW == ForgeDirection.WEST
                && NW && !SW && !NE && !SE;
            boolean c2 = areaNS == ForgeDirection.SOUTH && areaEW == ForgeDirection.WEST
                && !NW && SW && !NE && !SE;
            boolean c3 = areaNS == ForgeDirection.NORTH && areaEW == ForgeDirection.EAST
                && !NW && !SW && NE && !SE;
            boolean c4 = areaNS == ForgeDirection.SOUTH && areaEW == ForgeDirection.EAST
                && !NW && !SW && !NE && SE;
            return c1 || c2 || c3 || c4;
        }

        private static SwitchGroup
        random(ForgeDirection ns, ForgeDirection ew, Random rand) {
        ...
        @Override
        public int hashCode() {
            return ((NW ? 1 : 0) << 0) | ((SW ? 1 : 0) << 1) | ((NE ? 1 : 0) << 2)
                | ((SE ? 1 : 0) << 3);
        }
        ...
        public boolean validate(
            ...
            boolean nw = NW
                == BlockLightSwitch.isSwitchUp(
                    world, switchNW.xCoord, switchNW.yCoord, switchNW.zCoord
                );
            boolean sw = SW
                == BlockLightSwitch.isSwitchUp(
                    world, switchSW.xCoord, switchSW.yCoord, switchSW.zCoord
                );
            boolean ne = NE
                == BlockLightSwitch.isSwitchUp(
                    world, switchNE.xCoord, switchNE.yCoord, switchNE.zCoord
                );
            boolean se = SE
                == BlockLightSwitch.isSwitchUp(
                    world, switchSE.xCoord, switchSE.yCoord, switchSE.zCoord
                );
            return nw && sw && ne && se;
        }
    }
    ...
    @Override
    public void handleTileInteract(
        World world,
        int x,
        int y,
        int z,
        ...
        if (te instanceof LightSwitchTile) {
            ...
            } else if (!ProgressionManager.instance.playerHasPrerequisites(
                           ep, ProgressStage.BIOMESTRUCT
                       )) {
    ...
    public boolean isInaccessible(
        ...
        return !ProgressionManager.instance.playerHasPrerequisites(
            ep, ProgressStage.BIOMESTRUCT
        );
    }

    @Override
    public void handleMusicTrigger(
        World world,
        int x,
        int y,
        int z,
    ...
    private void updateSwitchDoors(World world, Coordinate root) {
        for (int i = 0; i < 4; i++) {
            boolean flag = doorKeys[i].validate(
                world,
                this.switchLoc(root, ForgeDirection.NORTH, ForgeDirection.WEST),
                this.switchLoc(root, ForgeDirection.SOUTH, ForgeDirection.WEST),
                this.switchLoc(root, ForgeDirection.NORTH, ForgeDirection.EAST),
                this.switchLoc(root, ForgeDirection.SOUTH, ForgeDirection.EAST)
            );
    ...
    private HashMap<Coordinate, ForgeDirection>
    getSwitchDoorLocations(Coordinate root, int i) {
    ...
    private HashMap<Coordinate, CrystalElement>
    getCrystalLocations(Coordinate root /*, int i*/) {
            ...
            for (int a = 3; a <= 5; a++) {
                for (int b = 4; b <= 5; b++) {
                    if (b == 5 && a == 4)
                        continue;
                    ret.add(root.offset(
                        dir.offsetX * b + left.offsetX * a,
                        1,
                        left.offsetZ * a + dir.offsetZ * b
                    ));
                }
            }
        }
    ...
    private Collection<Coordinate> getLowerChestLocations(Coordinate root) {
        ...
        for (int i = 2; i < 6; i++) {
            ...
            ForgeDirection left = ReikaDirectionHelper.getLeftBy90(dir);
            ret.add(root.offset(
                dir.offsetX * 5 + left.offsetX * 4, 2, left.offsetZ * 4 + dir.offsetZ * 5
            ));
        }
        return ret;
    }
    ...
    public void onTick(TileEntityStructControl te) {
        ...
        EntityPlayer ep = te.worldObj.getClosestPlayer(
            te.xCoord + 0.5, te.yCoord + 0.5, te.zCoord + 0.5, 20
        );
        if (ep == null)
            return;
        AxisAlignedBB box
            = ReikaAABBHelper.getBlockAABB(te).expand(1, 0, 1).offset(0, 6, 0);
        List<EntityPlayer> li
            = te.worldObj.getEntitiesWithinAABB(EntityPlayer.class, box);
        for (EntityPlayer ep2 : li) {
            if (!ProgressionManager.instance.playerHasPrerequisites(
                    ep2, ProgressStage.BIOMESTRUCT
                ))
    ...
    private boolean isPlayingMelody(EntityPlayer ep) {
        return melodyIndex
            >= 0; //ChromaItems.PROBE.matchWith(ep.getCurrentEquippedItem());//isPlayingMelody;
    }
    ...
    private void playNextNote(TileEntityStructControl te) {
        ...
        if (key != null) { //skip rest
            ChromaSounds.DING.playSoundAtBlock(
                te, 1, (float) CrystalMusicManager.instance.getPitchFactor(key)
            );

            ArrayList<CrystalElement> c
                = new ArrayList(CrystalMusicManager.instance.getColorsWithKey(key));
            for (CrystalElement e : c) {
                ...
                ReikaPacketHelper.sendPositionPacket(
                    ...
                    ChromaPackets.RUNEPARTICLE.ordinal(),
                    te.worldObj,
                    dx,
                    dy,
                    dz,
    ...
    public void render() {
        ...
        if (!ProgressionManager.instance.playerHasPrerequisites(
            ...
            double r = 12;
            int color = ReikaColorAPI.mixColors(
                0xff0000,
                0x700000,
                (float) (0.5 + 0.5 * Math.sin(System.currentTimeMillis() / 100D))
            );
            var5.startDrawingQuads();
            var5.setBrightness(240);
            ...
            for (double k = -r; k <= r - dk; k += dk) {
                ...
                double r2
                    = Math.abs(k) >= r ? 0 : Math.sqrt(Math.max(0, dr * dr - k * k));
                double r3 = Math.abs(k) >= r
                    ? 0
                    : Math.sqrt(Math.max(0, dr2 * dr2 - (k + dk) * (k + dk)));
