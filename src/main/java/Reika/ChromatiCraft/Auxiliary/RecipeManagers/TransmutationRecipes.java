...
public class TransmutationRecipes {
    ...
    private TransmutationRecipes() {
        ...
        /*
        this.addRecipe(ChromaStacks.beaconDust, new ElementTagCompound(), Items.iron_axe,
        Items.arrow, Blocks.diamond_block, ChromaStacks.icyDust);
        this.addRecipe(ChromaStacks.beaconDust, new ElementTagCompound(), Items.iron_axe,
        Items.arrow, Blocks.gold_block); this.addRecipe(ChromaStacks.beaconDust, new
        ElementTagCompound(), Items.iron_axe, Items.slime_ball);
        this.addRecipe(ChromaStacks.elementDust, new ElementTagCompound(),
        Items.gold_ingot, Items.boat, ChromaStacks.brownShard, ChromaStacks.aqua);
        this.addRecipe(ChromaStacks.auraDust, new ElementTagCompound(), Items.stone_axe,
        Blocks.stained_glass, Blocks.bedrock, ChromaStacks.bindingCrystal);
        this.addRecipe(ChromaStacks.elementDust, new ElementTagCompound(),
        Items.gold_ingot, Items.baked_potato, ChromaStacks.brownShard, ChromaStacks.aqua);
    ...
    private ItemMatch parseItem(Object o) {
        if (o instanceof String) {
            ...
            throw new RegistrationException(
                ...
                "Illegal input item " + o + " for transmutation recipe"
            );
        }
    }
    ...
    public ElementTagCompound getEnergyValue(ItemStack is) {
        if (ChromaItems.BERRY.matchWith(is)) {
            ...
            return new ElementTagCompound(
                CrystalElement.elements[is.getItemDamage() % 16],
                is.getItemDamage() >= 16 ? 36 : 6
            );
        } else if (ChromaItems.SEED.matchWith(is)) {
            return new ElementTagCompound(
                CrystalElement.elements[is.getItemDamage() % 16], 2
            );
        } else {
            return null;
        }
