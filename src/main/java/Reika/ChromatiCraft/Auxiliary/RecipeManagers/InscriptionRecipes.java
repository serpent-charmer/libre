...
public class InscriptionRecipes {
    ...
    private InscriptionRecipe addRecipe(BlockKey in, ChromaTiles out, int time) {
        return this.addRecipe(
            in, new BlockKey(out.getBlock(), out.getBlockMetadata()), time
        );
    }
    ...
    public void loadCustomInscriptionRecipes() {
        CustomRecipeList crl
            = new CustomRecipeList(ChromatiCraft.instance, "inscription");
        if (crl.load()) {
            for (LuaBlock lb : crl.getEntries()) {
                ...
                if (flag) {
                    ChromatiCraft.logger.log(
                        "Loaded custom inscription recipe '" + lb.getString("type") + "'"
                    );
                } else {
                    ChromatiCraft.logger.logError(
                        "Could not load custom inscription recipe '"
                        + lb.getString("type") + "'"
                    );
                    if (e != null)
                        e.printStackTrace();
    ...
    private boolean addCustomRecipe(LuaBlock lb, CustomRecipeList crl) throws Exception {
        ...
        try {
            InscriptionRecipe r = this.addCustomRecipe(
                BlockKey.fromItem(in), BlockKey.fromItem(out), time
            );
        } catch (Exception e) {
    ...
    public static class InscriptionRecipe {
        ...
        public ElementTagCompound getInputElements() {
            ElementTagCompound tag
                = ItemElementCalculator.instance.getValueForItem(input.asItemStack());
            if (duration > 200)
        ...
        public void doFX(World world, int x, int y, int z) {
            ...
            for (int i = 0; i < 32; i++) {
                ...
                EntityFX fx = new EntityCCBlurFX(world, px, py, pz)
                                  .setColor(c)
                                  .setScale(1 + world.rand.nextFloat())
                                  .setLife(l);
                Minecraft.getMinecraft().effectRenderer.addEffect(fx);
            }
            ReikaSoundHelper.playClientSound(
                ChromaSounds.CAST, x + 0.5, y + 0.5, z + 0.5, 1, 1
            );
        }
