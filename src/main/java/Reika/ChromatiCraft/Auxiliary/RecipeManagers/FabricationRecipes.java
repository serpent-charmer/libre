...
public class FabricationRecipes {
    ...
    private FabricationRecipes() {
        ...
        for (int i = 0; i < ChromaItems.TIERED.getNumberMetadatas(); i++) {
            ...
            if (tag != null && !tag.isEmpty()) {
                tag = this.processTag(tag);
                ProgressStage level
                    = ((ItemTieredResource) is.getItem()).getDiscoveryTier(is);
                float scale = 5;
        ...
        if (ModList.THAUMCRAFT.isLoaded()) {
            ItemStack item
                = ReikaItemHelper.lookupItem(ModList.THAUMCRAFT, "ItemEldritchObject", 3);
            if (item != null) {
                tag = new ElementTagCompound();
                HashSet<CrystalElement> set = new HashSet();
                set.addAll(
                    ChromaAspectManager.instance.getColorsForAspect(Aspect.AIR, true)
                );
                set.addAll(
                    ChromaAspectManager.instance.getColorsForAspect(Aspect.FIRE, true)
                );
                set.addAll(
                    ChromaAspectManager.instance.getColorsForAspect(Aspect.WATER, true)
                );
                set.addAll(
                    ChromaAspectManager.instance.getColorsForAspect(Aspect.ENTROPY, true)
                );
                set.addAll(
                    ChromaAspectManager.instance.getColorsForAspect(Aspect.EARTH, true)
                );
                set.addAll(
                    ChromaAspectManager.instance.getColorsForAspect(Aspect.ORDER, true)
                );
        ...
        if (ModList.BOTANIA.isLoaded()) {
            ItemStack item
                = ReikaItemHelper.lookupItem(ModList.BOTANIA, "manaResource", 5);
            if (item != null) {
                tag = new ElementTagCompound();
        ...
        if (ModList.BLOODMAGIC.isLoaded()) {
            ...
            if (item != null) {
                ...
                this.addRecipe(
                    new ItemStack(item, 1, BloodMagicHandler.BLUE_SHARD_META), false, tag
                );
                this.addRecipe(
                    new ItemStack(item, 1, BloodMagicHandler.RED_SHARD_META), false, tag
                );
            }
        }
        ...
        if (ModList.THAUMICTINKER.isLoaded()) {
            ...
            this.addRecipe(
                ReikaItemHelper.lookupItem(ModList.THAUMICTINKER, "kamiResource", 6),
                false,
                nether
            );
            this.addRecipe(
                ReikaItemHelper.lookupItem(ModList.THAUMICTINKER, "kamiResource", 7),
                false,
                end
            );
        }
        ...
        if (ModList.APPENG.isLoaded()) {
            Block item
                = GameRegistry.findBlock(ModList.APPENG.modLabel, "tile.BlockSkyStone");
            if (item != null) {
                tag = new ElementTagCompound();
        ...
        if (ModList.TINKERER.isLoaded()) {
            ...
            ItemStack yellowheart
                = ReikaItemHelper.lookupItem("TConstruct:heartCanister:3");
            if (redheart != null) {
                tag = new ElementTagCompound();
        ...
        if (ModList.TWILIGHT.isLoaded()) {
            ItemStack item
                = ReikaItemHelper.lookupItem("TwilightForest:item.charmOfLife1");
            if (item != null) {
                tag = new ElementTagCompound();
            ...
            item = ReikaItemHelper.lookupItem(
                /*"TwilightForest:item.carminite"*/ "TwilightForest:item.borerEssence"
            );
            if (item != null) {
                tag = new ElementTagCompound();
                tag.addValueToColor(CrystalElement.PINK, 500);
    ...
    private FabricationRecipe addRecipe(
        ItemStack is, boolean useNBT, ElementTagCompound tag, ProgressStage... progress
    ) {
    ...
    private FabricationRecipe addFluidRecipe(
        ItemStack is, Fluid fluid, ElementTagCompound tag, ProgressStage... progress
    ) {
        FabricationRecipe f = new FabricationRecipe(is, tag);
        for (ProgressStage p : progress)
            f.addProgress(p);
    ...
    public static class FabricationRecipe {
        ...
        public Collection<ProgressStage> getProgress() {
            return progress != null ? Collections.unmodifiableCollection(progress)
                                    : new ArrayList();
        }
    ...
    public FabricationRecipe getOrCreateFluidRecipe(ItemStack is, Fluid f) {
        ...
        if (ftag == null)
            return null;
        ElementTagCompound tag = FabricationRecipes.recipes().processTag(ftag).scale(
            1 / FabricationRecipes.SCALE
        );
