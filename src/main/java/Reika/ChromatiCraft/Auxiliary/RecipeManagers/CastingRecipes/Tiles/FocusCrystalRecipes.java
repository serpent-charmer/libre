...
public class FocusCrystalRecipes {
    ...
    private static final ItemStack exquisiteCrystal
        = CrystalTier.EXQUISITE.getCraftedItem();
    private static final ItemStack turboCrystal
        = CrystalTier.TURBOCHARGED.getCraftedItem();

    private static final ItemStack defaultSlab = ReikaItemHelper.stoneSlab.asItemStack();

    private static final ItemStack refinedSlabInner
        = ChromaBlocks.PYLONSTRUCT.getStackOfMetadata(StoneTypes.SMOOTH.ordinal());
    private static final ItemStack refinedSlabOuter
        = ReikaItemHelper.chiseledQuartz.asItemStack();

    private static final ItemStack exquisiteSlabInner = ChromaStacks.auraIngot;
    private static final ItemStack exquisiteSlabOuter
    ...
    public static IIcon getBaseRenderIcon(CrystalTier tier) {
        ...
        return is != null
            ? Block.getBlockFromItem(is.getItem()).getIcon(1, is.getItemDamage())
            : null;
    }
    ...
    public static class RefinedFocusCrystalRecipe extends MultiBlockCastingRecipe {
        public RefinedFocusCrystalRecipe(TempleCastingRecipe r) {
            super(
                refinedCrystal,
                exponentialCosts ? ChromaStacks.bindingCrystal : defaultCrystal
            );
    ...
    public static class ExquisiteFocusCrystalRecipe extends PylonCastingRecipe {
        public ExquisiteFocusCrystalRecipe(TempleCastingRecipe r) {
            super(
                exquisiteCrystal,
                exponentialCosts ? ChromaStacks.crystalLens : refinedCrystal
            );
            this.addRunes(r.getRunes());
    ...
    private static IRecipe getDefaultRecipe() {
        return ReikaRecipeHelper.getShapedRecipeFor(
            defaultCrystal,
            " A ",
            "BCB",
            "SAS",
            ...
            defaultSlab,
            'A',
            exponentialCosts ? basicCrystal : defaultDust,
            'B',
            defaultDust,
            'C',
            exponentialCosts ? ChromaStacks.bindingCrystal : basicCrystal
        );
    }
}
