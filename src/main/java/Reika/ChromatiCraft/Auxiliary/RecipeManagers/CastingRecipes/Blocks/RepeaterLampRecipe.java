...
public class RepeaterLampRecipe extends CastingRecipe {
    public RepeaterLampRecipe(ChromaTiles c) {
        super(
            calcOutput(c),
            ReikaRecipeHelper.getShapedRecipeFor(calcOutput(c), getRecipe(c))
        );
    }

    private static Object[] getRecipe(ChromaTiles c) {
        return new Object[] {
            "ggg", "grg", "ggg", 'r', c.getCraftedProduct(), 'g', Items.glowstone_dust
        };
    }

    private static ItemStack calcOutput(ChromaTiles c) {
        return ChromaBlocks.REPEATERLAMP.getStackOfMetadata(
            BlockRepeaterLight.getMetadataFor(c)
        );
    }
