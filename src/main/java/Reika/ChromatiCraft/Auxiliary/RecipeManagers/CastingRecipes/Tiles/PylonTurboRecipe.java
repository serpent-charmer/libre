...
public class PylonTurboRecipe extends PylonCastingRecipe {
    public PylonTurboRecipe(
        ItemStack out, ItemStack main, RecipeCrystalRepeater repeater
    ) {
        super(out, main);
            ...
            for (int i = 0; i < EffectType.list.length; i++) {
                ...
                if (e.getChance(te.getRandom(), tick)) {
                    ...
                    ReikaPacketHelper.sendDataPacketWithRadius(
                        ...
                        ChromaPackets.PYLONTURBORECIPE.ordinal(),
                        te,
                        64,
                        i
                    );
    ...
    private static void genParticleRing(TileEntityCastingTable te) {
        ...
        for (double a = 0; a < 360; a += 2) {
            ...
            ) new EntityCCFloatingSeedsFX(te.worldObj, x, y, z, a, 0)
                                           .setScale(s)
                                           .setNoSlowdown();
    ...
    private static void glowArea(TileEntityCastingTable te) {
        for (int i = 2; i < 6; i++) {
            for (int k = 1; k < 8; k += 1) {
                Coordinate c
                    = new Coordinate(te).offset(ForgeDirection.VALID_DIRECTIONS[i], k);
                if (c.getBlock(te.worldObj)
                        .isAir(te.worldObj, c.xCoord, c.yCoord, c.zCoord))
                    c.setBlock(
                        te.worldObj,
                        ChromaBlocks.LIGHT.getBlockInstance(),
                        Flags.FASTDECAY.getFlag()
                    );
            }
        }
    }
    ...
    private static void glowAreaClient(TileEntityCastingTable te) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int c = ReikaColorAPI.mixColors(
                CrystalElement.BLUE.getColor(), 0xffffff, rand.nextFloat()
            );
            ...
            EntityFX fx = new EntityCCBlurFX(te.worldObj, px, py, pz, 0, v, 0)
    ...
    private static void generateSkyParticle(TileEntityCastingTable te) {
        ...
        for (double d = -r; d <= r; d += 0.03125) {
            ...
            double s2 = s * 0.5;
            double s3 = s * 0.25;
            EntityFX fx = new EntityCCBlurFX(te.worldObj, x, dy, z, 0, v, 0)
                              .setIcon(ChromaIcons.FADE_STAR)
                              .setNoSlowdown()
                              .setRapidExpand()
                              .setScale((float) s)
                              .setColor(c1)
                              .setLife(l);
            EntityFX fx2 = new EntityCCBlurFX(te.worldObj, x, dy, z, 0, v, 0)
                               .setIcon(ChromaIcons.FADE_STAR)
                               .setNoSlowdown()
                               .setRapidExpand()
                               .setScale((float) s2)
                               ...
                               .setLife(l)
                               .lockTo(fx);
            EntityFX fx3 = new EntityCCBlurFX(te.worldObj, x, dy, z, 0, v, 0)
                               .setIcon(ChromaIcons.FADE_STAR)
                               .setNoSlowdown()
                               .setRapidExpand()
                               .setScale((float) s3)
        ...
        EntityFX fx = new EntityCCBlurFX(te.worldObj, x, y, z)
                          .setIcon(ChromaIcons.RINGFLARE)
                          .setRapidExpand()
                          .setScale(24)
                          .setColor(c1)
                          .setLife(l);
    ...
    public float getAutomationCostFactor(
        ...
        return is == null                                             ? 8
            : ReikaItemHelper.matchStacks(is, ChromaStacks.lumenCore) ? 4
                                                                      : 2;
    }
