...
public class PathRecipe extends TempleCastingRecipe {
    public PathRecipe(ItemStack main, int meta, ItemStack outer) {
        super(
            ChromaBlocks.PATH.getStackOfMetadata(meta),
            getRecipe(ChromaBlocks.PATH.getStackOfMetadata(meta), main, outer)
        );
    ...
    private static IRecipe getRecipe(ItemStack out, ItemStack main, ItemStack outer) {
        return ReikaRecipeHelper.getShapedRecipeFor(
            out, "OSO", "SMS", "OSO", 'O', outer, 'M', main, 'S', Blocks.stonebrick
        );
    }
