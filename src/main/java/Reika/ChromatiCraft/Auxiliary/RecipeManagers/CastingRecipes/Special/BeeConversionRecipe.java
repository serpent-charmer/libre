...
public class BeeConversionRecipe extends PylonCastingRecipe {
    ...
    public BeeConversionRecipe() {
        super(
            new ItemStack(ForestryHandler.ItemEntry.PRINCESS.getItem()),
            new ItemStack(ForestryHandler.ItemEntry.DRONE.getItem())
        );
    ...
    public ItemStack getOutputForDisplay(ItemStack center) {
        ...
        return ctr.stackTagCompound != null ? this.calcOutput(ctr.stackTagCompound)
                                            : this.getOutput();
    }
    ...
    public ItemStack[] getArrayForDisplay() {
        ...
        if (displayDrone == null
            || lastRedisplayTime < time - (GuiScreen.isShiftKeyDown() ? 100 : 1000)) {
        ...
        while (is == null) {
            ...
            try {
                ...
                ChromatiCraft.logger.logError(
                    "Could not generate bee item for bee type " + species + ": "
                    + e.toString()
                );
            }
        }
    ...
    /*
    @Override
    public NBTTagCompound handleNBTResult(TileEntityCastingTable te, EntityPlayer ep,
    NBTTagCompound originalCenter, NBTTagCompound tag) { return
    this.calcOutput(originalCenter).stackTagCompound;
    }
     */
