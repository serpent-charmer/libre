...
public final class EnchantmentRecipe extends MultiBlockCastingRecipe {
    ...
    public EnchantmentRecipe(
        ...
        if (parent == null)
            throw new RegistrationException(
                ...
                "No parent for enchantment recipe " + ctr.getDisplayName() + " {"
                    + e.getName() + " " + lvl + "}"
            );
    ...
    public NBTTagCompound getOutputTag(EntityPlayer ep, NBTTagCompound input) {
        NBTTagCompound tag
            = input != null ? (NBTTagCompound) input.copy() : new NBTTagCompound();
    ...
    protected boolean isValidCentralNBT(ItemStack is) {
        return super.isValidCentralNBT(is)
            && ReikaEnchantmentHelper.getEnchantmentLevel(enchantment, is) < level;
    }
