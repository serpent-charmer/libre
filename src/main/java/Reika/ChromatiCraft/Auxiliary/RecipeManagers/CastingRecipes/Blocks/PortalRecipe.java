...
public class PortalRecipe extends PylonCastingRecipe implements CoreRecipe {
            ...
            for (int i = 0; i < EffectType.list.length; i++) {
                ...
                if (e.getChance(te.getRandom(), tick)) {
                    ...
                    ReikaPacketHelper.sendDataPacketWithRadius(
                        ...
                        ChromaPackets.PORTALRECIPE.ordinal(),
                        te,
                        64,
                        i
                    );
    ...
    private static void genExplosion(TileEntityCastingTable te) {
        ...
        ReikaSoundHelper.playSoundAtBlock(
            te.worldObj, te.xCoord, te.yCoord, te.zCoord, "random.explode"
        );
    }

    private static void genLightning(TileEntityCastingTable te) {
        te.worldObj.addWeatherEffect(
            new EntityLightningBolt(te.worldObj, te.xCoord, te.yCoord, te.zCoord)
        );
    ...
    private static void generateBurstParticles(TileEntityCastingTable te) {
        ...
        ReikaSoundHelper.playClientSound(
            ChromaSounds.INFUSE, x + 0.5, y + 0.5, z + 0.5, 1, 1
        );
        ...
        for (int i = 0; i < 32; i++) {
            ...
            EntityFX fx
                = new EntitySparkleFX(te.worldObj, x + 0.5, y + 0.5, z + 0.5, vx, vy, vz)
                      .setColor(c)
                      .setScale(1);
            fx.noClip = true;
