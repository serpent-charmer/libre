...
public abstract class RepeaterRecipe
    extends MultiBlockCastingRecipe implements EnergyLinkingRecipe {
    ...
    public float[] getHarmonics() {
        return new float[] { (float) MusicKey.A5.getRatio(MusicKey.D5),
                             (float) MusicKey.Fs5.getRatio(MusicKey.D5) };
    }

    @Override
    public final boolean canRunRecipe(TileEntity te, EntityPlayer ep) {
        return super.canRunRecipe(te, ep)
            && ChromaResearchManager.instance.getPlayerResearchLevel(ep).ordinal()
            >= ResearchLevel.NETWORKING.ordinal()
            && ProgressStage.BLOWREPEATER.isPlayerAtStage(ep);
    }

    @Override
    public final NBTTagCompound handleNBTResult(
    ...
    @Override
    public final float getAutomationCostFactor(
