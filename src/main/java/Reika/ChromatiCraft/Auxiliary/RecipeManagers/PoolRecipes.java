...
public class PoolRecipes {
    ...
    private final ItemHashMap<Collection<PoolRecipe>> recipes
        = new ItemHashMap().setOneWay();
    ...
    private PoolRecipes() {
        this.addRecipe(
            ...
            ReikaItemHelper.getSizedItemStack(ChromaStacks.chromaDust, 16)
        );
        this.addRecipe(
            ChromaStacks.fieryIngot,
            new ItemStack(Items.gold_ingot),
            ReikaItemHelper.getSizedItemStack(ChromaStacks.firaxite, 16),
            new ItemStack(Items.blaze_powder, 8),
            new ItemStack(Items.coal, 2, 0)
        );
        this.addRecipe(
            ChromaStacks.enderIngot,
            new ItemStack(Items.iron_ingot),
            ReikaItemHelper.getSizedItemStack(ChromaStacks.enderDust, 16),
            new ItemStack(Items.ender_pearl, 4, 0)
        );
        this.addRecipe(
            ChromaStacks.waterIngot,
            new ItemStack(Items.iron_ingot),
            ReikaItemHelper.getSizedItemStack(ChromaStacks.waterDust, 16),
            new ItemStack(Items.gold_ingot, 2, 0)
        );
        this.addRecipe(
            ChromaStacks.conductiveIngot,
            new ItemStack(Items.gold_ingot),
            new ItemStack(Items.redstone, 8, 0),
            ReikaItemHelper.getSizedItemStack(ChromaStacks.beaconDust, 16)
        );
        this.addRecipe(
            ChromaStacks.auraIngot,
            new ItemStack(Items.iron_ingot),
            ReikaItemHelper.getSizedItemStack(ChromaStacks.auraDust, 8),
            new ItemStack(Items.glowstone_dust, 8, 0),
            new ItemStack(Items.redstone, 16, 0),
            new ItemStack(Items.quartz, 4, 0)
        );
        this.addRecipe(
            ChromaStacks.spaceIngot,
            new ItemStack(Items.iron_ingot),
            ReikaItemHelper.getSizedItemStack(ChromaStacks.spaceDust, 16),
            new ItemStack(Items.glowstone_dust, 32, 0),
            new ItemStack(Items.redstone, 64, 0),
            new ItemStack(Items.quartz, 16, 0),
            new ItemStack(Items.diamond, 4, 0)
        );

        PoolRecipe pr = this.addRecipe(
            ChromaStacks.experienceGem,
            ChromaStacks.iridChunk,
            new ItemStack(Blocks.obsidian, 4, 0),
            new ItemStack(Items.emerald, 8, 0)
        );
        //pr.allowDoubling = false;

        pr = this.addRecipe(
            ChromaStacks.complexIngot,
            ChromaStacks.experienceGem,
            ChromaStacks.chromaIngot,
            ChromaStacks.enderIngot,
            ChromaStacks.waterIngot,
        ...
        pr = this.addRecipe(
                     ...
                     ChromaItems.DATACRYSTAL.getStackOf(),
                     ReikaItemHelper.getSizedItemStack(ChromaStacks.crystalPowder, 18)
        )
                 .addProgress(ProgressStage.TOWER);

        pr = this.addRecipe(
                     ChromaItems.STRUCTMAP.getStackOf(),
                     new ItemStack(Items.map),
                     new ItemStack(Items.compass),
                     ChromaItems.DATACRYSTAL.getStackOf(),
                     ChromaItems.ARTEFACT.getStackOfMetadata(
                         ArtefactTypes.ARTIFACT.ordinal()
                     ),
                     ChromaBlocks.METAALLOYLAMP.getStackOf(),
                     ChromaItems.ENDEREYE.getCraftedProduct(12),
                     ReikaItemHelper.getSizedItemStack(ChromaStacks.auraDust, 64),
                     ReikaItemHelper.getSizedItemStack(ChromaStacks.spaceDust, 48),
                     new ItemStack(Items.glowstone_dust, 32)
        )
                 .addProgress(ProgressStage.TOWER)
                 .addProgress(ProgressStage.ARTEFACT);
    ...
    private PoolRecipe
    addRecipe(ItemStack out, ItemStack main, ItemStack... ingredients) {
        ...
        String id
            = "#" + recipes.size() + "=" + main + ">" + out + "+" + ingredients.length;
    ...
    public PoolRecipe getPoolRecipe(
        EntityItem ei, Collection<EntityItem> li, boolean block, EntityPlayer ep
    ) {
        ...
        if (prs != null) {
            ...
            if (!block
                || (ei.worldObj.getBlock(x, y, z)
                        == ChromaBlocks.CHROMA.getBlockInstance()
                    && ei.worldObj.getBlockMetadata(x, y, z) == 0)) {
    ...
    public void
    makePoolRecipe(EntityItem ei, PoolRecipe pr, int ether, int x, int y, int z) {
        AxisAlignedBB box = ReikaAABBHelper.getBlockAABB(x, y, z);
        Collection<EntityItem> li
            = ei.worldObj.getEntitiesWithinAABB(EntityItem.class, box);
        boolean flag
            = ei.worldObj.getBlock(x, y, z) == ChromaBlocks.CHROMA.getBlockInstance();
        pr.makeFrom(li);
        ReikaEntityHelper.decrEntityItemStack(ei, 1);
        int n = pr.allowDoubling()
                && ReikaRandomHelper.doWithChance(
                    BlockActiveChroma.getDoublingChance(ether)
                )
            ? 2
            : 1;
        ...
        for (int i = 0; i < n2; i++) {
            EntityItem newitem = ReikaItemHelper.dropItem(
                ei, ReikaItemHelper.getSizedItemStack(pr.getOutput(), 1)
            );
            ...
            for (LuaBlock lb : crl.getEntries()) {
                ...
                if (flag) {
                    ChromatiCraft.logger.log(
                        "Loaded custom alloying recipe '" + lb.getString("type") + "'"
                    );
                } else {
                    ChromatiCraft.logger.logError(
                        "Could not load custom alloying recipe '" + lb.getString("type")
                        + "'"
                    );
                    if (e != null)
                        e.printStackTrace();
    ...
    private boolean addCustomRecipe(LuaBlock lb, CustomRecipeList crl) throws Exception {
        ...
        PoolRecipe r
            = this.addCustomRecipe(out, main, li.toArray(new ItemStack[li.size()]));
        r.allowDoubling
            = lb.containsKey("allow_doubling") ? lb.getBoolean("allow_doubling") : true;
        return true;
    }

    public PoolRecipe
    addCustomRecipe(ItemStack out, ItemStack main, ItemStack... ingredients) {
    ...
    public static class PoolRecipe {
            ...
            for (int i = 0; i < input.length; i++) {
                if (inputs.containsKey(input[i]))
                    throw new RegistrationException(
                        ...
                        "You cannot have two stacks of " + input[i] + "!"
                    );
                int amt = input[i].stackSize;
                if (amt > input[i].getMaxStackSize())
                    throw new RegistrationException(
                        ChromatiCraft.instance, "Stack size of " + input[i] + " too big!"
                    );
        ...
        public ItemStack programToAEPattern(boolean berries) {
            ...
            return AEPatternHandling.getEncodedPattern(
                out, false, false, in.toArray(new ItemStack[in.size()])
            );
        }
    }
    ...
    public Collection<PoolRecipe> getAllPoolRecipes() {
        return Collections.unmodifiableCollection(
            ReikaJavaLibrary.getCompoundCollection(recipes.values())
        );
    }
