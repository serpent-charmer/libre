        ...
        switch (gui) {
            ...
            case TILE:
                ...
                if (te instanceof TileEntityAutoEnchanter)
                    return new ContainerAutoEnchanter(
                        player, (TileEntityAutoEnchanter) te
                    );
                if (te instanceof TileEntityCollector)
                    return null; //return new ContainerCollector(player,
                                 //(TileEntityCollector)te);
                if (te instanceof TileEntitySpawnerReprogrammer)
                    return new ContainerSpawnerProgrammer(
                        player, (TileEntitySpawnerReprogrammer) te
                    );
                if (te instanceof TileEntityCrystalBrewer)
                    return new ContainerCrystalBrewer(
                        player, (TileEntityCrystalBrewer) te
                    );
                ...
                if (te instanceof TileEntityCrystalCharger)
                    return new ContainerCrystalCharger(
                        player, (TileEntityCrystalCharger) te
                    );
                if (te instanceof TileEntityCrystalFurnace)
                    return new ContainerCrystalFurnace(
                        player, (TileEntityCrystalFurnace) te
                    );
                if (te instanceof TileEntityItemCollector)
                    return new ContainerItemCollector(
                        player, (TileEntityItemCollector) te
                    );
                if (te instanceof TileEntityItemFabricator)
                    return new ContainerItemFabricator(
                        player, (TileEntityItemFabricator) te
                    );
                if (te instanceof TileEntityTeleportationPump)
                    return new ContainerTelePump(
                        player, (TileEntityTeleportationPump) te
                    );
                ...
                if (te instanceof TileEntityInventoryTicker)
                    return new ContainerInventoryTicker(
                        player, (TileEntityInventoryTicker) te
                    );
                ...
                if (te instanceof TileEntityMEDistributor)
                    return new ContainerMEDistributor(
                        player, (TileEntityMEDistributor) te
                    );
                ...
                if (te instanceof TileEntityEnchantDecomposer)
                    return new ContainerEnchantDecomposer(
                        player, (TileEntityEnchantDecomposer) te
                    );
                ...
                if (te instanceof TileEntityNetworkItemTransporter)
                    return new ContainerNetworkItemTransporter(
                        player, (TileEntityNetworkItemTransporter) te
                    );
            ...
            case STRUCTUREPASS:
                return new ContainerStructurePassword(
                    player, (TileEntityStructurePassword) world.getTileEntity(x, y, z)
                );
            case FRAGSELECT:
    ...
    getClientGuiElement(int id, EntityPlayer player, World world, int x, int y, int z) {
        ...
        switch (gui) {
            ...
            case TILE:
                ...
                if (te instanceof TileEntityCollector)
                    return null; //return new GuiCollector(player,
                                 //(TileEntityCollector)te);
                if (te instanceof TileEntitySpawnerReprogrammer)
                    return new GuiSpawnerProgrammer(
                        player, (TileEntitySpawnerReprogrammer) te
                    );
                if (te instanceof TileEntityCrystalBrewer)
                ...
                if (te instanceof TileEntityEnchantDecomposer)
                    return new GuiEnchantDecomposer(
                        player, (TileEntityEnchantDecomposer) te
                    );
                ...
                if (te instanceof TileEntityNetworkItemTransporter)
                    return new GuiNetworkItemTransporter(
                        player, (TileEntityNetworkItemTransporter) te
                    );
            ...
            case MACHINEDESC: {
                ...
                return r == ChromaResearch.ACCEL ? new GuiAdjacencyDescription(player)
                                                 : new GuiMachineDescription(player, r);
            }
            case TOOLDESC:
            ...
            case CRAFTING:
                return new GuiCraftingRecipe(
                    player, ChromaResearch.researchList[x].getVanillaRecipes(), y
                );
            case RECIPE:
                return new GuiCastingRecipe(
                    player, ChromaResearch.researchList[x].getCraftingRecipes(), y, z > 0
                );
            case ALLOYING:
            ...
            case INFO: {
                ChromaResearch r = ChromaResearch.researchList[x];
                return r == ChromaResearch.PACKCHANGES ? new GuiPackChanges(player)
                                                       : new GuiBasicInfo(player, r);
            }
            case STRUCTURE:
            ...
            case STRUCTUREPASS:
                return new GuiStructurePassword(
                    player, (TileEntityStructurePassword) world.getTileEntity(x, y, z)
                );
            case ENDERBUCKET:
