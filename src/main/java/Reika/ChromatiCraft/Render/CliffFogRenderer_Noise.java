...
public class CliffFogRenderer_Noise {
    ...
    private final NoiseGeneratorBase cloudNoiseA
        = new SimplexNoiseGenerator(System.currentTimeMillis())
              .addOctave(2, 0.5)
              .addOctave(4, 0.125)
              .addOctave(8, 0.125)
              .setFrequency(1 / 24D * RESOLUTION);
    private final NoiseGeneratorBase cloudNoiseB
    ...
    private final NoiseGeneratorBase cloudHeight
        = new SimplexNoiseGenerator(-System.currentTimeMillis())
              .addOctave(2, 0.25)
              .setFrequency(1 / 80D * RESOLUTION);
    private final NoiseGeneratorBase cloudThickness
        = new SimplexNoiseGenerator(~System.currentTimeMillis())
              .setFrequency(1 / 12D * RESOLUTION);
    ...
    public void initialize() {
        ...
        for (int i = 0; i < size; i++) {
            for (int k = 0; k < size; k++) {
                float f = (float
                ) (Math.max(0, cloudNoiseA.getValue(i, k))
                   * Math.max(0, cloudNoiseB.getValue(i, k)));
                ...
                thickData[i][k] = f > 0
                    ? (float
                       ...
                       * ReikaMathLibrary.normalizeToBounds(
                           cloudThickness.getValue(i, k), MINHEIGHT, MAXHEIGHT
                       ))
                    : 0;
            }
        }
    ...
    private void renderLayer(int layer, Tessellator v5) {
        double a
            = System.currentTimeMillis() * (1 + layer / 192D) / 90000D % (2 * Math.PI)
            + layer / 80D;
    ...
    private void
    renderVertex(Tessellator v5, double x, double y, double z, int i, int k) {
        ...
        v5.setColorOpaque_I(
            c7.value > 0
                ? ReikaColorAPI.getColorWithBrightnessMultiplier(0xffffff, c7.value)
                : 0
        );
        v5.addVertex(
            x + this.offset(i),
            y + c7.heightOffset - c7.value * c7.thickness,
            z + this.offset(k)
        );

        v5.setColorOpaque_I(
            c9.value > 0
                ? ReikaColorAPI.getColorWithBrightnessMultiplier(0xffffff, c9.value)
                : 0
        );
        v5.addVertex(
            x + this.offset(pi == 0 ? clouds.length : pi),
            y + c9.heightOffset - c9.value * c9.thickness,
            z + this.offset(k)
        );

        v5.setColorOpaque_I(
            c3.value > 0
                ? ReikaColorAPI.getColorWithBrightnessMultiplier(0xffffff, c3.value)
                : 0
        );
        v5.addVertex(
            x + this.offset(pi == 0 ? clouds.length : pi),
            y + c3.heightOffset - c3.value * c3.thickness,
            z + this.offset(pk == 0 ? clouds.length : pk)
        );

        v5.setColorOpaque_I(
            c1.value > 0
                ? ReikaColorAPI.getColorWithBrightnessMultiplier(0xffffff, c1.value)
                : 0
        );
        v5.addVertex(
            x + this.offset(i),
            y + c1.heightOffset - c1.value * c1.thickness,
            z + this.offset(pk == 0 ? clouds.length : pk)
        );
