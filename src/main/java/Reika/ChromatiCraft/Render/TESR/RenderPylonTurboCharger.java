...
public class RenderPylonTurboCharger extends ChromaRenderBase {
    ...
    private static final LightningBolt[][] bolts
        = new LightningBolt[3][Location.list.length + 1];
        ...
        for (int i = 0; i < bolts.length; i++) {
            for (int k = 0; k < bolts[i].length; k++) {
                ...
                LightningBolt b = new LightningBolt(
                    new DecimalPosition(0.5, 0.5, 0.5), end, 4 + i * 4
                );
                b.setVariance(0.375);
                ...
                if (te.getSkyTick() > 0) {
                    ...
                    for (int i = 0; i < 512; i += h) {
                        ...
                        TileEntityCrystalPylon tp
                            = te.getPylon(te.worldObj, te.xCoord, te.yCoord, te.zCoord);
                        if (tp != null) {
                            ChromaFX.drawEnergyTransferBeam(
                                p1, p2, tp.getColor().getColor(), 0.35, 0.35, 6, t, true
                            );
                        }
                        GL11.glPopMatrix();
                    }
    ...
    private void renderConnection(
        TileEntityPylonTurboCharger te, double par2, double par4, double par6, float par8
    ) {
        ...
        if (loc != null) {
            ...
            if (ChromaTiles.getTileFromIDandMetadata(
                    test.getBlock(te.worldObj), test.getBlockMetadata(te.worldObj)
                )
