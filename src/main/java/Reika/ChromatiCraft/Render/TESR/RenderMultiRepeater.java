...
public class RenderMultiRepeater extends RenderCrystalRepeater {
    ...
    protected void doAuxRendering(TileEntityCrystalRepeater te, float par8) {
        ...
        if (tc.connectedToPylon()) {
            ...
            v5.setColorOpaque_I(ReikaColorAPI.getColorWithBrightnessMultiplier(
                this.getOffsetColor(tc, 16), 0.75F
            ));
            ...
            s = s * 1.5;
            v5.setColorOpaque_I(ReikaColorAPI.getColorWithBrightnessMultiplier(
                this.getOffsetColor(tc, 8), 0.25F
            ));
            v5.addVertexWithUV(-s, -s, 0, u, v);
            v5.addVertexWithUV(s, -s, 0, du, v);
            v5.addVertexWithUV(s, s, 0, du, dv);
            ...
            s = s * 1.5;
            v5.setColorOpaque_I(ReikaColorAPI.getColorWithBrightnessMultiplier(
                this.getOffsetColor(tc, 0), 0.125F
            ));
            v5.addVertexWithUV(-s, -s, 0, u, v);
            v5.addVertexWithUV(s, -s, 0, du, v);
            v5.addVertexWithUV(s, s, 0, du, dv);
    ...
    protected int getHaloRenderColor(TileEntityCrystalRepeater te) {
        //CrystalElement e =
        //((TileEntityCompoundRepeater)te).getRenderColorWithOffset(-8); CrystalElement
        //next = CrystalElement.elements[(e.ordinal()+1)%16]; float f =
        //te.getTicksExisted()%32/32F; ReikaJavaLibrary.pConsole(f+" from "+e+" to
        //"+next); return CrystalElement.getBlendedColor(te.getTicksExisted(),
        //32);//ReikaColorAPI.mixColors(next.getColor(), e.getColor(), f);
