...
public class RenderFloatingLandmark extends ChromaRenderBase {
    ...
    private void drawInner(TileEntityFloatingLandmark te) {
        ...
        double s = te.isInWorld() ? 0.4375 : 0.4;
        if (te.hasWorldObj()) {
        ...
        IIcon ico
            = ChromaIcons.CAUSTICS.getIcon(); //ChromaIcons.GUARDIANMIDDLE.getIcon();
        float u = ico.getMinU();
        float v = ico.getMinV();
        ...
        double s3 = 1.75;
        ico = ChromaIcons.PORTALRING.getIcon(); //ChromaIcons.GUARDIANMIDDLE.getIcon();
        u = ico.getMinU();
        v = ico.getMinV();
        du = ico.getMaxU();
        dv = ico.getMaxV();
        v5.startDrawingQuads();
        v5.setColorOpaque_I(0xffffff);
        v5.addVertexWithUV(-s3, -s3, 0, u, v);
        v5.addVertexWithUV(s3, -s3, 0, du, v);
        v5.addVertexWithUV(s3, s3, 0, du, dv);
        v5.addVertexWithUV(-s3, s3, 0, u, dv);
        v5.draw();
