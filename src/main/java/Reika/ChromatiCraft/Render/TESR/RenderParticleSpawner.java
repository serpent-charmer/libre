...
public class RenderParticleSpawner extends ChromaRenderBase {
    ...
    public void renderTileEntityAt(
        ...
        if (te.isInWorld()) {
            ...
            if (HoldingChecks.MANIPULATOR.isClientHolding()) {
                ...
                MovingObjectPosition mov
                    = ReikaPlayerAPI.getLookedAtBlockClient(4.5, false);
                if (mov != null && mov.blockX == te.xCoord && mov.blockY == te.yCoord
                    && mov.blockZ == te.zCoord) {
                    flag = true;
                }
            }
            if (flag) {
        ...
        IIcon[] icons
            = { ChromaIcons.CAUSTICS_GENTLE.getIcon(), ChromaIcons.GLOWFRAME.getIcon() };
        ...
        for (double d1 = -0.5; d1 <= 0.5; d1 += dd) {
            for (double d2 = -0.5; d2 <= 0.5; d2 += dd) {
                ...
                double h11 =
        0.5+0.25*Math.sin(te.xCoord+te.zCoord+4*d11+2*Math.toRadians(te.getTicksExisted()+par8));
                double h12 =
        0.5+0.25*Math.sin(te.xCoord+te.zCoord+4*d12+2*Math.toRadians(te.getTicksExisted()+par8));
                double h21 =
        0.5+0.25*Math.sin(te.xCoord+te.zCoord+4*d21+2*Math.toRadians(te.getTicksExisted()+par8));
                double h22 =
        0.5+0.25*Math.sin(te.xCoord+te.zCoord+4*d22+2*Math.toRadians(te.getTicksExisted()+par8));
