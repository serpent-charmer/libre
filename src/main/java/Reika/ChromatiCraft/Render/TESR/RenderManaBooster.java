...
public class RenderManaBooster extends ChromaRenderBase {
    ...
    private void drawInner(TileEntityManaBooster te, float par8) {
        ...
        IIcon[] icons = { ChromaIcons.SUNFLARE.getIcon(),
                          ChromaIcons.TURBO.getIcon(),
                          ChromaIcons.CONCENTRIC2REV.getIcon() };
        double[] sz = { te.isInWorld() ? 1.5 : 1.25,
                        te.isInWorld() ? 0.75 : 1.25,
                        te.isInWorld() ? 0.875 : 0.9375 };
        float[] br = { 1, 1, 0.75F };
        ...
        int c = ReikaColorAPI.getModifiedHue(
            0xff0000, (int) (System.currentTimeMillis() / 15D % 360)
        );
