...
public class RenderCrystalPortal extends ChromaRenderBase {
    ...
    public void renderTileEntityAt(
        ...
        int p = te.getPortalPosition(
            StructureRenderer.isRenderingTiles() ? StructureRenderer.getRenderAccess()
                                                 : te.worldObj,
            te.xCoord,
            te.yCoord,
            te.zCoord
        ...
        if (MinecraftForgeClient.getRenderPass() == 1
            || StructureRenderer.isRenderingTiles()) {
            if (StructureRenderer.isRenderingTiles() || te.isFull9x9()) {
                if (p == 5) {
                    ...
                    int rgb = ReikaColorAPI.getModifiedHue(
                        0x0000ff, (int) (240 + 40 * Math.sin((te.getTicks() + par8) / 8F))
                    );
                    ...
                    if (te.hasWorldObj() && !StructureRenderer.isRenderingTiles()) {
                        ...
                        if (ep.getDistanceSq(
                            ...
                            <= 16384) {
                            ChromaShaders[] sh = { ChromaShaders.PORTAL_INCOMPLETE,
                                                   ChromaShaders.PORTAL };
                            for (ChromaShaders shd : sh) {
                                shd.rampDownAmount = 0.0125F;
                                shd.rampDownFactor = 0.98F;
                                shd.lingerTime = 8;
                                shd.getShader().setField(
                                    "distance",
                                    Math.max(
                                        1,
                                        ep.getDistanceSq(
            ...
            double v = te.getTicks() % 256 / 256D
                + Math.cos(Math.toRadians((te.getTicks() / 2D) % 360));
            double s = 0.25;
            double du = u + s;
            double dv = v + s;
            double o = 0.001;
            ...
            if (te.worldObj.getBlock(te.xCoord, te.yCoord, te.zCoord + 1)
            ...
            if (te.worldObj.getBlock(te.xCoord + 1, te.yCoord, te.zCoord)
                != ChromaBlocks.PORTAL.getBlockInstance()) {
    ...
    private void
    renderPortalTuning(TileEntityCrystalPortal te, Tessellator v5, float par8) {
        float f = HoldingChecks.MANIPULATOR.getFade();
        ItemStack is = Minecraft.getMinecraft().thePlayer.getCurrentEquippedItem();
        if (ReikaItemHelper.matchStacks(ChromaStacks.bedrockloot, is)
            || ReikaItemHelper.matchStacks(ChromaStacks.bedrockloot2, is)) {
            f = 1;
        }
        if (f <= 0)
            return;
        ...
        double p = 0.875
            + 0.125
                * Math.sin(
                    System.currentTimeMillis() / 600D
                ); //0.675;//+0.125*Math.sin(t/720D);
    ...
    private void
    renderCompletedFX(TileEntityCrystalPortal te, Tessellator v5, float par8) {
        ...
        v5.setColorRGBA_I(
            ReikaColorAPI.getColorWithBrightnessMultiplier(
                CrystalElement.getBlendedColor(te.getTicks(), 20), 0.0625F
            ),
            255
        );
        v5.addVertexWithUV(-1, 1, 2, u, dv);
        ...
        boolean half
            = StructureRenderer.isRenderingTiles() || te.getCharge() >= te.MINCHARGE / 2;
        ...
        for (double i = 0; i <= n; i += dh) {
            ...
            double a = 255 - 64 * i;
            v5.setColorRGBA_I(
                ReikaColorAPI.GStoHex((int
                ) (a * (0.75 + 0.25 * Math.sin((te.getTicks() + par8 + i * 96) / 4F)))),
                255
            );
        ...
        if (half) {
            ...
            for (double a = 0; a < 180; a += 60) {
                ...
                ReikaTextureHelper.bindTexture(
                    ChromatiCraft.class, "Textures/arches2.png"
                );
                for (double ux = 0; ux <= 0.5; ux += 0.5) {
                    //v5.setColorRGBA_I(CrystalElement.getBlendedColor(te.getTicks()+(int)(8*ux),
                    //20), 255);
    ...
    private void renderStaticTextureLayer(
        double par2, double par4, double par6, float ptick, int rgb
    ) {
        ...
        GL11.glTexGen(
            GL11.GL_S, GL11.GL_OBJECT_PLANE, this.genBuffer(1.0F, 0.0F, 0.0F, 0.0F)
        );
        GL11.glTexGen(
            GL11.GL_T, GL11.GL_OBJECT_PLANE, this.genBuffer(0.0F, 0.0F, 1.0F, 0.0F)
        );
        GL11.glTexGen(
            GL11.GL_R, GL11.GL_OBJECT_PLANE, this.genBuffer(0.0F, 0.0F, 0.0F, 1.0F)
        );
        GL11.glTexGen(
            GL11.GL_Q, GL11.GL_EYE_PLANE, this.genBuffer(0.0F, 1.0F, 0.0F, 0.0F)
        );
    ...
    private void renderStaticTexture(double par2, double par4, double par6, float ptick) {
        ...
        for (int i = 0; i < 16; ++i) {
            ...
            if (i == 0) {
                ...
                ReikaTextureHelper.bindTexture(
                    ChromatiCraft.class, "Textures/Dimension Bump Maps/cyan.png"
                );
                f7 = 0.1F;
                f5 = 65.0F;
                f6 = 0.125F;
                ...
                ReikaTextureHelper.bindTexture(
                    ChromatiCraft.class, "Textures/Dimension Bump Maps/gray.png"
                );
            }
            ...
            GL11.glLoadIdentity();
            GL11.glTranslatef(
                0.0F, Minecraft.getSystemTime() % 700000L / 700000.0F, 0.0F
            );
            GL11.glScalef(f6, f6, f6);
            GL11.glTranslatef(0.5F, 0.5F, 0.0F);
            GL11.glRotatef((i * i * 4321 + i * 9) * 2.0F, 0.0F, 0.0F, 1.0F);
            ...
            f7 = 0.5F
                + 0.5F * (float) Math.sin(System.currentTimeMillis() / 200D + i / 2D);
