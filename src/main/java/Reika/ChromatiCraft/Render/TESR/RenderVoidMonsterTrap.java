...
public class RenderVoidMonsterTrap extends ChromaRenderBase {
    ...
    public void renderTileEntityAt(
        ...
        if (te.isInWorld() && te.hasStructure()) {
            ...
            if (LOS.isClearLineOfSight(te)) {
                double dist
                    = ep.getDistance(te.xCoord + 0.5, te.yCoord + 0.5, te.zCoord + 0.5);
                float f = 0;
                if (dist <= 16) {
                    f = 1;
                } else if (dist <= 32) {
                ...
                ChromaShaders.VOIDTRAP.getShader().setField(
                    "rotation", te.getShaderRotation()
                );
            }
        }
    ...
    private void renderTethers(TileEntityVoidMonsterTrap te, float par8) {
        ...
        for (VoidMonsterTether v : c) {
            ...
            for (VariableEndpointSpline s : v.getSplines()) {
                ...
                //ChromaFX.renderBeam(0.5, 0.5, 0.5, e.posX-te.xCoord, te.yCoord-e.posY,
                //-(e.posZ-te.zCoord), par8, 255, d, 0xffffff);
                for (int i = 0; i < li.size() - 1; i++) {
                    ...
                    ChromaFX.renderBeam(
                        pos1.xCoord - te.xCoord,
                        te.yCoord + 1 - pos1.yCoord,
                        te.zCoord + 1 - pos1.zCoord,
                        pos2.xCoord - te.xCoord,
                        te.yCoord + 1 - pos2.yCoord,
                        te.zCoord + 1 - pos2.zCoord,
                        par8,
                        255,
                        d,
                        clr
    ...
    private void drawInner(TileEntityVoidMonsterTrap te, float par8) {
        ...
        //float f = te.isInWorld() ? ((float)(Math.sin(t/5.4D)+Math.cos(t/3.9D))*15-14) :
        //0; if (f > 1) 	f = 1;
        float f = te.getFlashBrightness();

        for (int i = 0; i < icons.length; i++) {
            int c = ReikaColorAPI.getModifiedHue(
                0xff0000, 265 + (int) (15 * Math.sin(t / 30D + i))
            );
            c = ReikaColorAPI.getModifiedSat(
                c, 0.9F + 0.1F * (float) Math.sin(t / 20D + i / 4D)
            );
