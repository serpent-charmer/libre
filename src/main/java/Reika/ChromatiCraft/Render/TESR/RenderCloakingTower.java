...
public class RenderCloakingTower extends ChromaRenderBase {
    private static final ColorBlendList blends = new ColorBlendList(10)
                                                     .addColor(0xffffff)
                                                     .addColor(0xff0000)
                                                     .addColor(0xffffff)
                                                     .addColor(0x00ff00)
    ...
    private static final ChromaIcons[] icons
        = { ChromaIcons.CAUSTICS, ChromaIcons.CAUSTICS_GENTLE };
    ...
    private void
    renderGlowTower(TileEntityCloakingTower tile, double par2, double par4, double par6) {
        ReikaTextureHelper.bindTerrainTexture();
        ...
        //ReikaRenderHelper.renderTube(0, -h, 0, 0, h, 0, 0xffffffff, 0xffffffff, 0.375,
        //0.375);
        ReikaTextureHelper.bindTexture(
            ChromatiCraft.class, "/Reika/ChromatiCraft/Textures/beam.png"
        );
        double t = (((System.currentTimeMillis() / 600D) % 360 + 360) % 360) / 30D
            + Math.abs(tile.hashCode());
        int c = flag ? ReikaColorAPI.GStoHex((int) (222 + 32 * Math.sin(t * 8)))
                     : ReikaColorAPI.GStoHex((int) (128 + 64 * Math.sin(t * 32)));
        ...
        ChromaFX.drawEnergyTransferBeam(
            p1, p2.offset(0, -2 * h, 0), c, 0.35, 0.35, (byte) 6, t, true
        );
        GL11.glPopAttrib();
        ...
        for (double i = -h; i < h; i += d) {
            for (int f = f1; f < 2; f++) {
                ...
                double r = 0.03125;
                ReikaRenderHelper.renderTube(
                    -wan, i, wan, -wap, i + d, wap, c1, c2, r, r, 6
                );
                ReikaRenderHelper.renderTube(
                    wan, i, wan, wap, i + d, wap, c1, c2, r, r, 6
                );
