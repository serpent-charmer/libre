...
public class RenderCrystalRepeater extends CrystalTransmitterRender {
    private final RemoteSourcedAsset rangeTexture
        = ChromaClient.dynamicAssets.createAsset("Textures/repeaterrange3.png");
    ...
    public void renderTileEntityAt(
        ...
        if (tile.hasWorldObj()
            && ((MinecraftForgeClient.getRenderPass() == 1 && te.canConduct())
            ...
            RenderBlocks.getInstance().renderBlockAsItem(
                c.getBlock(), c.getBlockMetadata(), 1
            );
            GL11.glPopMatrix();
    ...
    private void
    renderCasterHalo(TileEntityCrystalRepeater te, float fade, UUID uid, float par8) {
        int clr = this.getHaloRenderColor(te);
        clr = ReikaColorAPI.mixColors(
            clr, 0xffffff, 0.875F + 0.125F * (float) Math.sin(te.getTicksExisted() / 90D)
        );
        clr = ReikaColorAPI.getColorWithBrightnessMultiplier(clr, fade);
        double s = 1.75 + 0.0625 * Math.sin(te.getTicksExisted() / 6D);
        if (te.isTurbocharged())
            s *= 1.25;
        CastingTuningManager.instance.getTuningKey(te.worldObj, uid)
            .drawIcon(Tessellator.instance, s, clr);
    }

    private void renderIconHalo(
        TileEntityCrystalRepeater te, IIcon ico, double sizeScale, float par8
    ) {
        ...
        if (a != 0) {
            ...
            if (c != null) {
                ...
                for (WorldLocation loc : c) {
                    if (loc.xCoord+loc.yCoord+loc.zCoord > te.xCoord+te.yCoord+te.zCoord)
    { //hack to ensure 1-way rendering v5.addVertex(0, 0, 0);
                        v5.addVertex(loc.xCoord-te.xCoord, loc.yCoord-te.yCoord,
    loc.zCoord-te.zCoord);
                    }
                }
        ...
        if (a > 0) {
            ...
            this.renderSphere(
                ...
                ReikaColorAPI.getColorWithBrightnessMultiplier(
                    this.getHaloRenderColor(te), a / 512F
                ),
                r
            );
        }
    ...
    protected void
    renderSphere(TileEntityCrystalRepeater te, float par8, int color, double r) {
        GL11.glPushAttrib(GL11.GL_ALL_ATTRIB_BITS);
    ...
    private void renderPlayerConnectivityLine(TileEntityCrystalRepeater te, float par8) {
        ...
        if (a > 0) {
            ...
            if (mov != null && mov.typeOfHit == MovingObjectType.BLOCK) {
                ForgeDirection dir = mov.sideHit >= 0
                    ? ForgeDirection.VALID_DIRECTIONS[mov.sideHit]
                    : ForgeDirection.UNKNOWN;
                int x = mov.blockX + dir.offsetX;
                int y = mov.blockY + dir.offsetY;
                int z = mov.blockZ + dir.offsetZ;
                if (ReikaMathLibrary.py3d(x - te.xCoord, y - te.yCoord, z - te.zCoord)
                    <= te.getSendRange()) {
                    if (PylonFinder
                            .lineOfSight(
    ...
    private void renderHalo(TileEntityCrystalRepeater te, float par8) {
        ...
        for (int i = 0; i < a; i += step) {
            ...
            ReikaTextureHelper.bindTexture(
                ChromatiCraft.class, "Textures/Turbo/sections.png"
            );
            double s = te.worldObj != null
                ? 1.5 + 0.5 * Math.sin(Math.toRadians(4 * d + i * 2))
                : 1.75;
            v5.startDrawingQuads();
            ...
            if (te.worldObj != null) {
                ReikaTextureHelper.bindTexture(
                    ChromatiCraft.class, "Textures/Turbo/radiate.png"
                );
