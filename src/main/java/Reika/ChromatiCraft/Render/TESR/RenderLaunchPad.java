...
public class RenderLaunchPad extends ChromaRenderBase {
    ...
    private void renderCharge(TileEntityLaunchPad tile, float par8) {
        ...
        if (f > 0) {
            ReikaTextureHelper.bindTexture(
                ChromatiCraft.class, "Textures/launchcharge_additive.png"
            );
            //double x0 = 0.5-s;
            //double x1 = 0.5+s;
            //double z0 = 0.5-s;
    ...
    private void
    renderFan(TileEntityLaunchPad tile, Tessellator v5, float par8, float f, int color) {
        double s = 1.5;
        double lim = f * 360;
        double da = 5;
        double h = 1.001;
        ...
        for (double a = 0; a <= lim; a += da) {
            ...
            double r = s
                * Math.min(Math.abs(1D / Math.cos(ang)), Math.abs(1D / Math.sin(ang)));
