...
public class RenderMultiBuilder extends ChromaRenderBase {
    ...
    private static void addColumn(int x, int y, int z, Block b) {
        columns.put(
            new DecimalPosition(x / 16D + 0.03125, y / 16D - 0.03125, z / 16D + 0.03125),
            getColorFromBlock(b)
        );
    }
    ...
    private void renderArea(TileEntityMultiBuilder te) {
        ...
        ReikaTextureHelper.bindTexture(
            ChromatiCraft.class, "Textures/blockbox2.png"
        ); //use diff tex, more geometric
        ...
        for (Point p : c) {
            ...
            double u = 0;
            double du = u + 1;
            double v = ((System.identityHashCode(te) + System.currentTimeMillis() / 64D
                         + (p.x + p.y) * 24)
                        % 64)
                / 64D;
            double dv = v + 1 / 64D;
