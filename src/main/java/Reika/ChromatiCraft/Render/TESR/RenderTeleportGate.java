...
public class RenderTeleportGate
    extends ChromaRenderBase implements MultipointChecker<TileEntityTeleportGate> {
    ...
    public boolean
    isClearLineOfSight(TileEntityTeleportGate te, RayTracer trace, World world) {
    ...
    public void renderTileEntityAt(
        ...
        if (!te.isInWorld()
            || (!te.hasStructure() && MinecraftForgeClient.getRenderPass() == 1)) {
            if (te.isInWorld()) {
            ...
            int c = ReikaColorAPI.mixColors(
                te.PRIVATE_COLOR,
                te.PUBLIC_COLOR,
                0.5F + 0.5F * (float) Math.sin(System.currentTimeMillis() / 400D)
            );
        ...
        if ((MinecraftForgeClient.getRenderPass() == 1 && te.hasStructure())
            ...
            double t = !StructureRenderer.isRenderingTiles()
                ? (te.getTicksExisted() + par8) / 4D
                : (System.currentTimeMillis() % 200000) / 100D;
            ...
            if (te.isInWorld() || StructureRenderer.isRenderingTiles()) {
                if (StructureRenderer.isRenderingTiles()
                    ...
                    s = 10;
                    v5.setColorOpaque_I(
                        ReikaColorAPI.getColorWithBrightnessMultiplier(c, 0.625F)
                    );
                    v5.addVertexWithUV(-s, s, 0, u, dv);
                    v5.addVertexWithUV(+s, s, 0, du, dv);
                    v5.addVertexWithUV(+s, -s, 0, du, v);
                ...
                if (ta > 0) {
                    ...
                    float f2 = ta > te.ACTIVATION_DURATION - 20
                        ? (te.ACTIVATION_DURATION - ta) / 20F
                        : 1;
                    ...
                    v5.startDrawingQuads();
                    v5.setColorOpaque_I(
                        ReikaColorAPI.getColorWithBrightnessMultiplier(c, f2)
                    );
                    v5.setBrightness(240);
                    u = ico.getMinU();
                    v = ico.getMinV();
                    ...
                    if (f3 > 0) {
                        GL11.glPushMatrix();
                        double ang = ReikaPhysicsHelper.cartesianToPolar(
                            te.xCoord + 0.5 - RenderManager.renderPosX,
                            te.yCoord + 0.5 - RenderManager.renderPosY,
                            te.zCoord + 0.5 - RenderManager.renderPosZ
                        )[2];
                        ...
                        v5.setColorOpaque_I(
                            ReikaColorAPI.getColorWithBrightnessMultiplier(c, f4)
                        );
                        int mi = (int) (f3 * 24);
                        for (i = 0; i < mi; i++) {
                            ico = i == mi - 1 ? ChromaIcons.LASEREND.getIcon()
                                              : ChromaIcons.LASER.getIcon();
                            u = ico.getMinU();
                            v = ico.getMinV();
                            du = ico.getMaxU();
                            dv = ico.getMaxV();
