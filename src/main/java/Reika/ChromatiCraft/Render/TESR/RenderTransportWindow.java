...
public class RenderTransportWindow extends ChromaRenderBase {
    ...
    private final ColorBlendList colorList = new ColorBlendList(2000)
                                                 .addColor(0x00ff00)
                                                 .addColor(0xa000ff)
                                                 .addColor(0xffffff)
                                                 .multiplySaturation(0.5F);
    ...
    public void renderTileEntityAt(
        ...
        if (te.isInWorld() && te.doRender()) {
            ...
            if ((ReikaMathLibrary.approxr(phi, phit, 60)
                 || ReikaMathLibrary.approxr(phi, phit + 360, 60))
                && ReikaMathLibrary.approxr(theta, 80, 80))
    ...
    private void doConnectionRender(
        TileEntityTransportWindow te, double par2, double par4, double par6, float par8
    ) {
        ...
        if (src != null) {
            this.renderConnection(
                src, loc, -loc.xCoord, -loc.yCoord, -loc.zCoord, 0x0000ff, 0xffffff
            );
        }
        if (tgt != null) {
            this.renderConnection(
                loc, tgt, -loc.xCoord, -loc.yCoord + 1, -loc.zCoord, 0xff00000, 0x00ff000
            );
        }
    }
    ...
    private void renderConnection(
        ...
        v5.addVertex(
            loc1.xCoord + 0.5 + dx, loc1.yCoord + 0.5 + dy, loc1.zCoord + 0.5 + dz
        );
        v5.setColorOpaque_I(c2);
        v5.addVertex(
            loc2.xCoord + 0.5 + dx, loc2.yCoord + 0.5 + dy, loc2.zCoord + 0.5 + dz
        );
        /*
    ...
    private void renderTexture(
        ...
        //int c = ReikaColorAPI.mixColors(0x00ff00, 0xa000ff,
        //f);//ReikaColorAPI.getModifiedHue(0xff0000,
        //(int)(System.currentTimeMillis()/32D%360)); c = ReikaColorAPI.getModifiedSat(c,
        //0.5F);
    ...
    public String getImageFileName(RenderFetcher te) {
        return ((TileEntityTransportWindow) te).renderTexture() ? "window.png"
                                                                : "window_notex.png";
    }
}
