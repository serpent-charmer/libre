...
public class RenderProgressionLinker
    extends ChromaRenderBase implements MultipointChecker<TileEntityProgressionLinker> {
    ...
    public void renderTileEntityAt(
        ...
        if (te.hasStructure()) {
            if (te.hasPlayer()) {
                ...
                //RenderDataNode.renderPrism(System.currentTimeMillis()/20D,
                //Tessellator.instance, 1, h, 0.5+dy);
                ...
                ReikaTextureHelper.bindTexture(
                    ChromatiCraft.class, "Textures/lightcone-foggy.png"
                );
                ...
                double h = 2.5;
                double f = 0.5 + 0.5 * Math.sin(tick / 8D)
                    + 0.125 * Math.sin((tick + 300) / 3D)
                    + 0.25 * Math.cos((tick + 20) / 7D);
                int c = ReikaColorAPI.mixColors(
                    0x22aaff, 0x5588ff, (float) MathHelper.clamp_double(f, 0, 1)
                );
                ...
                if (te.failure != null) {
                    GL11.glPushAttrib(GL11.GL_ALL_ATTRIB_BITS);
                    //v5.setColorOpaque_I(ReikaColorAPI.getColorWithBrightnessMultiplier(0xffffff,
                    //te.getFailFade()));
                    GL11.glTranslated(
                        0, 2.25 + 0.125 * Math.sin(te.getTicksExisted() / 15D), 0
                    );
                    GL11.glPushMatrix();
    ...
    public boolean
    isClearLineOfSight(TileEntityProgressionLinker te, RayTracer trace, World world) {
        EntityPlayer ep = Minecraft.getMinecraft().thePlayer;
        double r = 0.5;
        for (double i = -r; i <= r; i += r) {
            for (double k = -r; k <= r; k += r) {
                trace.setOrigins(
                    te.xCoord + i, te.yCoord, te.zCoord, ep.posX, ep.posY, ep.posZ
                );
