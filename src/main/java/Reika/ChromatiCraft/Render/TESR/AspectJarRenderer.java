...
public class AspectJarRenderer extends ChromaRenderBase {
    ...
    public AspectJarRenderer() {
        ...
        try {
            ...
            throw new RegistrationException(
                ...
                "Could not create ThaumCraft jar model instance to use for ChromatiCraft jar!"
            );
        }
    ...
    static {
        if (ModList.THAUMCRAFT.isLoaded()) {
            try {
                jarModelClass
                    = Class.forName("thaumcraft.client.renderers.models.ModelJar");
                ...
                throw new RegistrationException(
                    ChromatiCraft.instance,
                    "Could not read ThaumCraft jar renderer to use for ChromatiCraft jar!"
                );
            }
        }
    }
    ...
    private void renderUpgradeFlare(TileEntityAspectJar te, float par8) {
        ...
        for (double s : ss) {
            ...
            v5.setColorOpaque_I(
                s == s1 ? 0xffffff
                        : primalAspectColors.getColor(te.getTicksExisted() + par8)
            );
    ...
    private void renderAspect(TileEntityAspectJar te, Aspect a, int amt, int i)
        throws Exception {
        if (amt <= 0)
            return;
        GL11.glPushMatrix();
        ...
        double level = 0.625
            * Math.sqrt(
                amt / cap
            ); //ReikaMathLibrary.logbase2(te.getAmount())/ReikaMathLibrary.logbase2(te.CAPACITY);
        ...
        if (te.worldObj != null) {
            bright = Math.max(
                200,
                jar.getMixedBrightnessForBlock(
