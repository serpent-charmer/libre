...
public class RenderWirelessSource extends ChromaRenderBase {
    ...
    private void drawInner(TileEntityWirelessSource te, float par8) {
        ...
        double s = StructureRenderer.isRenderingTiles() ? 0.625
            : te.isInWorld()                            ? 0.4375
                                                        : 0.33;
        if (te.hasWorldObj()) {
    ...
    private void drawFilaments(TileEntityWirelessSource te, int c) {
        //beziers with start point in center of tile, one at random point fixed outer
        //radius, 2 more randomly placed along axis between with some deg offset
    }
}
