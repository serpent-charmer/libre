...
public class RenderCrystalFence extends ChromaRenderBase {
    ...
    public void renderTileEntityAt(
        ...
        if (te.isInWorld() && te.isValid() && MinecraftForgeClient.getRenderPass() == 1) {
            ...
            for (int i = 0; i < p.segmentCount(); i++) {
                int ap = i == p.segmentCount() - 1 ? te.getSegmentAlpha(0)
                                                   : te.getSegmentAlpha(i + 1);
                int a = te.getSegmentAlpha(i);
                int an = i == 0 ? te.getSegmentAlpha(p.segmentCount() - 1)
                                : te.getSegmentAlpha(i - 1);
                ...
                //v5.addVertex(c1.xCoord+0.5-te.xCoord, 000-te.yCoord,
                //c1.zCoord+0.5-te.zCoord); v5.addVertex(c2.xCoord+0.5-te.xCoord,
                //255-te.yCoord, c2.zCoord+0.5-te.zCoord);
                //v5.addVertex(c2.xCoord+0.5-te.xCoord, 255-te.yCoord,
                //c2.zCoord+0.5-te.zCoord); v5.addVertex(c2.xCoord+0.5-te.xCoord,
                //000-te.yCoord, c2.zCoord+0.5-te.zCoord); v5.draw();

                if (a > MIN_ALPHA || ap > MIN_ALPHA || an > MIN_ALPHA) {
                    if (a <= MIN_ALPHA)
                        a = MIN_ALPHA;
                    ReikaTextureHelper.bindTexture(
                        ChromatiCraft.class, "Textures/hex-5.png"
                    );
                } else {
                    a = MIN_ALPHA;
                    ReikaTextureHelper.bindTexture(
                        ChromatiCraft.class, "Textures/hex-7.png"
                    );
                }
                ...
                for (int l = 0; l < 2; l++) {
                    for (double h = my; h < py; h += 0.5) {
                        ...
                        int color1 = colors.getColor(
                            te.getTicksExisted() + par8
                            + Math.abs(((x1 - te.xCoord) + z1 - (te.zCoord) * 2) * 12)
                        );
                        int color2 = colors.getColor(
                            te.getTicksExisted() + par8
                            + Math.abs(((x2 - te.xCoord) + z2 - (te.zCoord) * 2) * 12)
                        );

                        if (a > MIN_ALPHA && te.colorFade(i)) {
                            color1 = ReikaColorAPI.mixColors(
                                0xffffff, color1, (a - MIN_ALPHA) / 223F
                            );
                            color2 = ReikaColorAPI.mixColors(
                                0xffffff, color2, (a - MIN_ALPHA) / 223F
                            );
                        }
                ...
                v5.draw();
                //v5.addVertex(loc.xCoord-te.xCoord+0.5, loc.yCoord-te.yCoord+0.5,
                //loc.zCoord-te.zCoord+0.5); ReikaAABBHelper.renderAABB(box, par2, par4,
                //par6, te.xCoord, te.yCoord, te.zCoord, a, 64, 192, 255, true);
            }
    ...
    private void
    getAlphas(int a, int an, int ap, int l, int len, double h, int my, int py) {
        double mid = len / 2D;
        double pedge = l + 1;
