...
public class TankRender extends ChromaRenderBase {
    ...
    private void renderRunes(
        TileEntityCrystalTank te, double par2, double par4, double par6, float par8
    ) {
        ...
        if (et != null && te.getTicksExisted() != last && te.getTicksExisted() % d == 0
            || (et != null && Keyboard.isKeyDown(Keyboard.KEY_NUMPAD6))) {
    ...
    private void renderGlint(
        ...
        float hue
            = (290 + 20 * (float) Math.sin(Math.toRadians(te.getTicksExisted() + par8)))
            / 360F;
        ...
        double o = 0.001;
        double u = (System.currentTimeMillis() % 5000) / 200D
            + Math.sin(System.currentTimeMillis() / 500D);
    ...
    private void renderLiquid(
        ...
        for (int i = 0; i < blocks.getSize(); i++) {
            ...
            boolean disp = flip ? (h < 1 || te.getFillLevelForY(y + 1) == 1)
                                : (h > 0 || te.getFillLevelForY(y - 1) == 1);

            if (disp) {
                int br = f.getLuminosity() > 10
                    ? 240
                    : (Keyboard.isKeyDown(Keyboard.KEY_NUMPAD7)
                           ? 240
                           : te.lighting.getCachedBrightness(te.worldObj, c)
                    ); /*te.getBlockType().getMixedBrightnessForBlock(te.worldObj, x, y,
                          z));*/
                /*
                int[] brs = new int[6];
                for (int k = 0; k < 6; k++) {
                    ForgeDirection dir = ForgeDirection.VALID_DIRECTIONS[k];
                    brs[k] = b.getMixedBrightnessForBlock(te.worldObj, x+dir.offsetX,
                ...
                if (x == te.xCoord && y == te.yCoord && z == te.zCoord) {
                    if (!canRenderInTank)
                        continue;
                    br = te.lighting.getCachedBrightness(
                        te.worldObj, c.offset(0, 1, 0)
                    ); //te.worldObj.getBlock(x, y+1,
                       //z).getMixedBrightnessForBlock(te.worldObj, x, y+1, z);
                }
                ...
                if (h < 1
                    || b.shouldSideBeRendered(
                        te.worldObj, x, y, z, ForgeDirection.UP.ordinal()
                    )) {
                    ...
                    v5.addVertexWithUV(
                        ew,
                        max + te.getHeightOffsetAtCorner(x, y, z, -1, 1, h, par8),
                        es,
                        u,
                        dv
                    );
                ...
                if (b.shouldSideBeRendered(
                        te.worldObj, x, y, z, ForgeDirection.DOWN.ordinal()
                    )) {
                    this.setFaceBrightness(
                        v5, ForgeDirection.DOWN, f.getLuminosity(), clr
                    );
                    double ee = blocks.hasBlock(x + 1, y, z) ? 1 : 1 - o;
                    double ew = blocks.hasBlock(x - 1, y, z) ? 0 : 0 + o;
                    double es = blocks.hasBlock(x, y, z + 1) ? 1 : 1 - o;
                ...
                if (b.shouldSideBeRendered(
                        te.worldObj, x, y, z, ForgeDirection.NORTH.ordinal()
                    )) {
                    this.setFaceBrightness(
                        v5, ForgeDirection.NORTH, f.getLuminosity(), clr
                    );
                    double ee = blocks.hasBlock(x + 1, y, z) ? 1 : 1 - o;
                    double ew = blocks.hasBlock(x - 1, y, z) ? 0 : 0 + o;
                    ...
                    v5.addVertexWithUV(
                        ew,
                        eu + te.getHeightOffsetAtCorner(x, y, z, -1, -1, h, par8),
                        0 + o,
                        u,
                        dv
                    );
                ...
                if (b.shouldSideBeRendered(
                        te.worldObj, x, y, z, ForgeDirection.SOUTH.ordinal()
                    )) {
                    this.setFaceBrightness(
                        v5, ForgeDirection.SOUTH, f.getLuminosity(), clr
                    );
                    double ee = blocks.hasBlock(x + 1, y, z) ? 1 : 1 - o;
                    double ew = blocks.hasBlock(x - 1, y, z) ? 0 : 0 + o;
                    double eu = blocks.hasBlock(x, y + 1, z) ? h : max;
                ...
                if (b.shouldSideBeRendered(
                        te.worldObj, x, y, z, ForgeDirection.WEST.ordinal()
                    )) {
                    this.setFaceBrightness(
                        v5, ForgeDirection.WEST, f.getLuminosity(), clr
                    );
                    double es = blocks.hasBlock(x, y, z + 1) ? 1 : 1 - o;
                    double en = blocks.hasBlock(x, y, z - 1) ? 0 : 0 + o;
                    double eu = blocks.hasBlock(x, y + 1, z) ? h : max;
                ...
                if (b.shouldSideBeRendered(
                        te.worldObj, x, y, z, ForgeDirection.EAST.ordinal()
                    )) {
                    this.setFaceBrightness(
                        v5, ForgeDirection.EAST, f.getLuminosity(), clr
                    );
                    double es = blocks.hasBlock(x, y, z + 1) ? 1 : 1 - o;
                    double en = blocks.hasBlock(x, y, z - 1) ? 0 : 0 + o;
                    double eu = blocks.hasBlock(x, y + 1, z) ? h : max;
    ...
    private void
    setFaceBrightness(Tessellator v5, ForgeDirection dir, int brightness, int color) {
        ...
        f -= sub * 0.75F;
        v5.setColorOpaque(
            (int) (f * ReikaColorAPI.getRed(color)),
            (int) (f * ReikaColorAPI.getGreen(color)),
            (int) (f * ReikaColorAPI.getBlue(color))
        );
    }
}
