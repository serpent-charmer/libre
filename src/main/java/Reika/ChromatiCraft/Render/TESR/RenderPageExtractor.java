...
public class RenderPageExtractor extends ChromaRenderBase {
    private final ResourceLocation bookTex
    ...
    public RenderPageExtractor() {
        for (int i = 0; i < 12; i++) {
            ...
            BasicVariablePoint point
                = new BasicVariablePoint(new DecimalPosition(0, 0, 0), 1, v);
        ...
        if (!te.isInWorld() || MinecraftForgeClient.getRenderPass() == 1) {
            ...
            for (BasicVariablePoint point : points) {
                double r = 0.75
                    + 0.25
                        * Math.sin(
                            t / 50D + Math.toRadians(point.hashCode() / 32D % 360D)
                        );
