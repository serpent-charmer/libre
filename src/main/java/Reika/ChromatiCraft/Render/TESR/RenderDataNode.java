...
public class RenderDataNode extends ChromaRenderBase {
    private final ColorBlendList beamColors
        = new ColorBlendList(30, 0xafafaf, 0x73DCFF, 0xb0e0ff, 0x4C79EE, 0x2C02FF);
    ...
    public void renderTileEntityAt(
        ...
        if (MinecraftForgeClient.getRenderPass() == 0
            || StructureRenderer.isRenderingTiles() || !te.isInWorld()) {
        ...
        if (MinecraftForgeClient.getRenderPass() == 1
            ...
            if (te.isInWorld()) {
                GL11.glPushMatrix();
                GL11.glTranslated(
                    0, te.getExtension0() + te.getExtension1() + te.getExtension2(), 0
                );
            ...
            if ((StructureRenderer.isRenderingTiles() || te.getTower() != null)
                && te.isInWorld() && ScriptLocations.TOWER.isEnabled()
    ...
    private void renderTower(TileEntityDataNode te, Tessellator v5) {
        int l = te.isInWorld() ? te.getBlockType().getMixedBrightnessForBlock(
                    te.worldObj, te.xCoord, te.yCoord, te.zCoord
                )
                               : 240;
        ...
        double dy2
            = te.isInWorld() ? 0.5 + te.getExtension1() + te.getExtension2() : -0.5;
    ...
    private void renderBase(TileEntityDataNode te, Tessellator v5, int l, double t) {
        ...
        ReikaTextureHelper.bindTerrainTexture();
        IIcon ico = ChromaBlocks.STRUCTSHIELD.getBlockInstance().getIcon(
            1, BlockType.MOSS.metadata
        );
        float u = ico.getMinU();
        float du = ico.getMaxU();
        float v = ico.getMinV();
    ...
    private void renderSymbol(TileEntityDataNode te, Tessellator v5) {
        Towers t = StructureRenderer.isRenderingTiles() ? Towers.towerList[(int
                   ) ((System.currentTimeMillis() / 1000) % Towers.towerList.length)]
                                                        : te.getTower();
        if (t != null) {
            ...
            if (d > 0) {
                ...
                double du = u + 1 / 8D;
                double dv = v + 1 / 8D;
                double tt = StructureRenderer.isRenderingTiles()
                    ? System.currentTimeMillis() / 25D
                    : te.getTicksExisted();
                double tk = (-tt / 1.5D) % 360D;
                ReikaTextureHelper.bindTexture(
                    ChromatiCraft.class, "Textures/towersymbols.png"
                );
                GL11.glPushMatrix();
                GL11.glDisable(GL11.GL_CULL_FACE);
    ...
    public static void renderFlare(Tessellator v5, float colorFactor, boolean inWorld) {
        ...
        v5.setColorOpaque_I(
            ReikaColorAPI.getColorWithBrightnessMultiplier(0xb0e0ff, colorFactor)
        );
    ...
    private void renderPrism(TileEntityDataNode te, Tessellator v5) {
        ...
        double t = StructureRenderer.isRenderingTiles() ? System.currentTimeMillis() / 50D
                                                        : te.getTicksExisted();
        double dy = te.isInWorld() ? 0.5 + te.getExtension1() + te.getExtension2()
                + 0.0625 * Math.sin((t / 8D) % (2 * Math.PI))
                                   : -0.5;
        this.renderPrism(
            te.isInWorld() ? t * 2 : System.currentTimeMillis() / 20D, v5, f, h, dy, false
        );
    }

    public static void renderPrism(
        double tick,
        Tessellator v5,
        float colorFactor,
        double h,
        ...
        ReikaTextureHelper.bindTexture(
            ChromatiCraft.class,
            orangeglow ? "Textures/datanode_orange.png" : "Textures/datanode.png"
        );

        double s = 0.125 * 0.875;

        int c1
            = 0xa0e0ff; //orangeglow ? ReikaColorAPI.getModifiedHue(0xff0000,
                        //(int)(33+12*Math.sin(tick/24D))) : 0xa0e0ff; //orange 0xffb070
        int c2 = 0xffffff;
        if (orangeglow) {
            c1 = 0xffffff;
        }
