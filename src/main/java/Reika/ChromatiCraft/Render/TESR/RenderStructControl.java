...
public class RenderStructControl extends ChromaRenderBase {
                ...
                if (flag) {
                    ...
                    if (LOS.isClearLineOfSight(te)) {
                        ...
                        if (f > 0) {
                            ...
                            ChromaShaders.STRUCTCONTROL.getShader().setField(
                                "distance", dd
                            );
                        }
                    }
                ...
                if (te.isMonument()) {
                    ...
                    if (br > 0) {
                        ...
                        ReikaTextureHelper.bindTexture(
                            ChromatiCraft.class, "Textures/monument_lines_big.png"
                        );
                        //v5.startDrawingQuads();
                        v5.startDrawing(GL11.GL_TRIANGLE_FAN);
                        v5.setBrightness(240);
                        v5.setColorOpaque_I(
                            ReikaColorAPI.getColorWithBrightnessMultiplier(0xffffff, br)
                        );
                        ...
                        for (int i = 0; i <= 16; i++) {
                            ...
                            for (double a = min; a <= max; a += d) {
                                ...
                                double u = 0.5 + dt * cs;
                                double v = 0.5 + dt * ss;
                                //v5.setColorOpaque_I(CrystalElement.getBlendedColor((int)((a+180-12.25)*2),
                                //45)); v5.setColorOpaque_I(mix.isEmpty() ? 0 :
                                //this.getColorMix(mix));
                                CrystalElement e = CrystalElement.elements[i % 16];
                                int c = e.getColor();
                                c = ReikaColorAPI.getColorWithBrightnessMultiplier(
                                    c, MonumentCompletionRitual.getIntensity(e)
                                );
    ...
    private float getMonumentBrightness() {
        ...
        for (int i = 0; i < 16; i++) {
            max = Math.max(
                max, MonumentCompletionRitual.getIntensity(CrystalElement.elements[i])
            );
        }
        return max;
    }
    ...
    private int getColorMix(Collection<CrystalElement> mix) {
        ...
        return n == 0
            ? 0
            : ReikaColorAPI.RGBtoHex((int) (r / n), (int) (g / n), (int) (b / n));
    }
