        ...
        if (te.isInWorld() && MinecraftForgeClient.getRenderPass() == 1) {
            ...
            if (!li.isEmpty()) {
                for (Coordinate c : li) {
                    ...
                    if (a > 0) {
                        DecimalPosition d = new DecimalPosition(c).offset(-0.5, -0.5,
            -0.5); d =
            d.offset(((TileEntityRouterNode)c.getTileEntity(te.worldObj)).getSide(),
            -0.5).offset(-te.xCoord, -te.yCoord, -te.zCoord); ChromaFX.renderBeam(0.5,
            0.75, 0.5, d.xCoord+0.5, d.yCoord+0.5, d.zCoord+0.5, par8, a, 0.1875);
                    }
                }
            }
            ...
            if (!li.isEmpty()) {
                for (Coordinate c : li) {
                    int a =
            (int)(255*Math.sin((te.getTicksExisted()+c.xCoord*2+c.zCoord)/16D)); if (a >
            0) { DecimalPosition d = new DecimalPosition(c).offset(-0.5, -0.5, -0.5); d =
            d.offset(((TileEntityRouterNode)c.getTileEntity(te.worldObj)).getSide(),
            -0.5).offset(-te.xCoord, -te.yCoord, -te.zCoord); ChromaFX.renderBeam(0.5,
            0.75, 0.5, d.xCoord+0.5, d.yCoord+0.5, d.zCoord+0.5, par8, a, 0.1875);
                    }
            ...
            while (it.hasNext()) {
                ...
                if (node != null) {
                    int a = (int
                    ) (255 * ReikaMathLibrary.cosInterpolation(0, h.lifespan, h.age));
                    if (a > 0) {
                        DecimalPosition d
                            = new DecimalPosition(c).offset(-0.5, -0.5, -0.5);
                        d = d.offset(((TileEntityRouterNode) node).getSide(), -0.5)
                                .offset(-te.xCoord, -te.yCoord, -te.zCoord);
                        ChromaFX.renderBeam(
                            ox,
                            oy,
                            oz,
