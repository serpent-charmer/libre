...
public class RenderAlveary extends ChromaRenderBase {
    ...
    public void renderTileEntityAt(
        ...
        if (te.isInWorld() && MinecraftForgeClient.getRenderPass() == 1) {
            ...
            if (te.isAlvearyComplete()) {
                double t = te.getTicksExisted() + par8 + te.xCoord + te.zCoord * 0.5D
                    + te.yCoord * 1.5D;
    ...
    private void renderBee(TileEntityLumenAlveary te, double t, float par8) {
        ...
        if (ei != null) {
            ...
            Coordinate c = te.getRelativeLocation();
            Vec3 rel = Vec3.createVectorHelper(
                te.xCoord - c.xCoord + 1.5 - ep.posX,
                te.yCoord - c.yCoord + 2.5 - ep.posY,
                te.zCoord - c.zCoord + 1.5 - ep.posZ
            );
            ...
            double a = 180
                + ReikaPhysicsHelper.cartesianToPolar(
                    RenderManager.renderPosX - te.xCoord,
                    RenderManager.renderPosY - te.yCoord,
                    RenderManager.renderPosZ - te.zCoord
                )[2];
            ...
            if (text) {
                ...
                FontRenderer f = sg.startsWith(EnumChatFormatting.OBFUSCATED.toString())
                    ? ChromaFontRenderer.FontType.OBFUSCATED.renderer
                    : ChromaFontRenderer.FontType.GUI.renderer;

                GL11.glTranslated(
                    -f.getStringWidth(sg) / 32D / s2,
                    0.1875D / s2 - ei.hoverStart * 0.375 / s2 - 0.125 / s2,
                    0
                );
    ...
    private void renderOverlay(TileEntityLumenAlveary te, double t, float par8) {
        ...
        for (int i = 0; i < clr.length; i++) {
            for (int j = 0; j < clr[i].length; j++) {
                for (int k = 0; k < clr[i][j].length; k++) {
                    float f
                        = 0.5F + 0.5F * (float) Math.sin(i * 1.5 + j * 3.5 + k + t / 6D);
        ...
        if (te.hasQueen() && te.getSpecies() == CrystalBees.getPrecursorBee()) {
            if (te.isInWorld() && ScriptLocations.ALVEARY.isEnabled()
                ...
                GL11.glPushAttrib(GL11.GL_ALL_ATTRIB_BITS);
                InWorldScriptRenderer.renderAlvearyScript(
                    te, par8, v5, 0.03125 / 1.45, 2304
                );
                GL11.glPopAttrib();
            }
        }
