...
public class RenderSkypeater extends CrystalTransmitterRender {
    private final RemoteSourcedAsset texture
        = ChromaClient.dynamicAssets.createAsset("Textures/airpeater_small.png");
    ...
    public void renderTileEntityAt(
        ...
        if (tile.hasWorldObj()
            ...
            double d = Math.max(
                0,
                Minecraft.getMinecraft().thePlayer.getDistance(
                    te.xCoord + 0.5, te.yCoord + 0.5 + 1.62, te.zCoord + 0.5
                ) - 8
            );
            ...
            //if
            //(ChromaItems.TOOL.matchWith(Minecraft.getMinecraft().thePlayer.getCurrentEquippedItem()))
            //{ 	f += 0.25;
            //}
            ...
            int c = ReikaColorAPI.mixColors(
                NodeClass.SHORE.color,
                NodeClass.WATER.color,
                0.5F + 0.5F * MathHelper.sin(tick)
            );
            v5.setColorOpaque_I(c);
            double s = 1.33;
