...
public class RenderNetworkOptimizer extends ChromaRenderBase {
    ...
    private void drawInner(TileEntityNetworkOptimizer te, float par8) {
        ...
        boolean valid = te.hasStructure() || !te.isInWorld()
            || StructureRenderer.isRenderingTiles();
        ...
        IIcon ico
            = valid ? ChromaIcons.COLORWHIRL.getIcon() : ChromaIcons.BIGFLARE.getIcon();
        float u = ico.getMinU();
        float v = ico.getMinV();
        ...
        if (valid) {
            int c = 0xffffff; //ReikaColorAPI.getModifiedHue(0xff0000,
                              //(int)(((System.currentTimeMillis()/5)%360)));
            br = 0.75F;
