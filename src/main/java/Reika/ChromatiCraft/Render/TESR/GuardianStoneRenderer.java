...
public class GuardianStoneRenderer extends ChromaRenderBase {
    ...
    private static void
    addParticle(double r, double e, double theta, double i, double raan, double arg) {
        orbits.add(new ParticleOrbit(
            new Orbit(r, e, theta, i, raan, arg),
            ReikaColorAPI.RGBtoHex(rand.nextInt(255), 120, 255)
        ));
    }
    ...
    private void drawLineRing(
        double r, double h, double y, float speed, int red, int green, int blue
    ) {
        Tessellator v5 = Tessellator.instance;
    ...
    private void
    drawLine(double r, double h, double y, float speed, int red, int green, int blue) {
        Tessellator v5 = Tessellator.instance;
        GL11.glColor4f(1, 1, 1, 1);
        GL11.glDisable(GL11.GL_TEXTURE_2D);
        double ang = (System.currentTimeMillis() / 20D * speed) % 360;
