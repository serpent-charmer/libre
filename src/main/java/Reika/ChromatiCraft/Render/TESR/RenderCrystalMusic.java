...
public class RenderCrystalMusic extends ChromaRenderBase {
    ...
    public void renderTileEntityAt(
        ...
        if (!tile.hasWorldObj() && !StructureRenderer.isRenderingTiles()) {
            ...
            for (double s = min; s <= max; s += ds) {
                ...
                long tick = StructureRenderer.isRenderingTiles()
                    ? (System.currentTimeMillis() / 40) % 1200
                    : te.getTicksExisted();
    ...
    private void drawOuter(TileEntityCrystalMusic te, double size, float par8, int da) {
        ...
        float dv = ico.getMaxV();
        double a = 0.625
            + 0.375 * Math.sin(24 * Math.toRadians(da * 8 + te.getTicksExisted() + par8));
        this.drawBox((float) a, size, 0, u, v, du, dv);
    }

    private void
    drawBox(float a, double size, double s, float u, float v, float du, float dv) {
        Tessellator v5 = Tessellator.instance;
