...
public class RenderChromaCrafter extends ChromaRenderBase {
        ...
        if (!te.isInWorld() || MinecraftForgeClient.getRenderPass() == 0) {
            ...
            if (te.isInWorld()) {
                ...
                Tessellator.instance.setBrightness(
                    te.getBlockType().getMixedBrightnessForBlock(
                ...
                if (te.hasStructure()) {
                    ...
                    RenderBlocks.getInstance().renderFaceXNeg(
                        b, 0, 0, 0, BlockStructureShield.lowerConnectedIcon
                    );
                    RenderBlocks.getInstance().renderFaceXPos(
                        b, 0, 0, 0, BlockStructureShield.lowerConnectedIcon
                    );
                    RenderBlocks.getInstance().renderFaceXNeg(
                        b, 0, 1, 0, BlockStructureShield.centerConnectedIcon
                    );
                    RenderBlocks.getInstance().renderFaceXPos(
                        b, 0, 1, 0, BlockStructureShield.centerConnectedIcon
                    );
                    RenderBlocks.getInstance().renderFaceXNeg(
                        b, 0, 2, 0, BlockStructureShield.upperConnectedIcon
                    );
                    RenderBlocks.getInstance().renderFaceXPos(
                        b, 0, 2, 0, BlockStructureShield.upperConnectedIcon
                    );
                    Tessellator.instance.setColorOpaque_F(0.875F, 0.875F, 0.875F);
                    RenderBlocks.getInstance().renderFaceZNeg(
                        b, 0, 0, 0, BlockStructureShield.lowerConnectedIcon
                    );
                    RenderBlocks.getInstance().renderFaceZPos(
                        b, 0, 0, 0, BlockStructureShield.lowerConnectedIcon
                    );
                    RenderBlocks.getInstance().renderFaceZNeg(
                        b, 0, 1, 0, BlockStructureShield.centerConnectedIcon
                    ...
                    RenderBlocks.getInstance().renderFaceYPos(
                        b, 0, 0, 0, b.getIcon(1, BlockType.GLASS.ordinal())
                    );
                }
    ...
    private void renderFluid(TileEntityChromaCrafter te, float par8) {
        ...
        while (amt > 0) {
            ...
            double o2 = o;
            double o3 = o;
            if (te.worldObj.getBlock(te.xCoord, (int) (te.yCoord + y + 1), te.zCoord)
                == ChromaBlocks.STRUCTSHIELD.getBlockInstance())
                o2 = 0;
            Block bb
                = te.worldObj.getBlock(te.xCoord, (int) (te.yCoord + y - 1), te.zCoord);
            if (bb == ChromaBlocks.STRUCTSHIELD.getBlockInstance()
                || bb == te.getBlockType())
                o3 = 0;
        ...
        if (r != null) {
            ...
            for (ItemStack is : li) {
                ...
                RenderManager.instance
                    .renderEntityWithPosYaw(ei, 0.5 + dx, dy, 0.5 + dz, 0, 0 /*tick*/);
