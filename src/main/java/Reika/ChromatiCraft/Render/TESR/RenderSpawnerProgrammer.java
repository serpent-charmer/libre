...
public class RenderSpawnerProgrammer extends ChromaRenderBase {
    ...
    private void renderSpawner() {
        ...
        float r = 4000;
        float dt
            = (float) Math.sin(Math.toRadians((System.currentTimeMillis() / 50) % 360)
            ); //(System.currentTimeMillis()%(int)r)/r;
        u = dt;
        du = 1 + dt;
        v = dt;
        dv = 1 + dt;
        ...
        dt = (float
        ) Math.sin(Math.toRadians((System.currentTimeMillis() / 80 + 120) % 360));
        u = dt;
        du = 1 + dt;
        v = dt;
        dv = 1 + dt;
