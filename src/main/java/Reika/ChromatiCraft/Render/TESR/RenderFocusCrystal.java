...
public class RenderFocusCrystal extends ChromaRenderBase {
    ...
    public void renderTileEntityAt(
        ...
        float t = world ? te.getTicksExisted()
        ...
        if (MinecraftForgeClient.getRenderPass() == 0
            ...
            String tex = "Textures/TileEntity/focuscrystal"
                + te.getTier().getTextureSuffix() + ".png";
            ...
            if (te.getTier().useOrganizedModel()) {
                ordered.renderAll(
                    te, ReikaJavaLibrary.makeListFrom(t, te.getTier().getRenderColor(t))
                );
            } else {
                unordered.renderAll(
                    te, ReikaJavaLibrary.makeListFrom(t, te.getTier().getRenderColor(t))
        ...
        if (StructureRenderer.isRenderingTiles()
            || (te.isInWorld() && MinecraftForgeClient.getRenderPass() == 1)) {
            GL11.glPushAttrib(GL11.GL_ALL_ATTRIB_BITS);
    ...
    private void renderBase(TileEntityFocusCrystal te, float tick) {
        ...
        if (ico != null) {
            ...
            v5.setBrightness(
                te.isInWorld() ? te.getBlockType().getMixedBrightnessForBlock(
    ...
    private void renderFlare(TileEntityFocusCrystal te, float tick, double n, double dy) {
        ...
        int c = ReikaColorAPI.getColorWithBrightnessMultiplier(
            te.getTier().getRenderColor(tick), 0.75F
        );
            ...
            ChromaFX.renderBeam(
                ...
                0.375F
                    + (float
                       ...
                       * Math.sin(
                           System.identityHashCode(te) + System.currentTimeMillis() / 800D
                       ))
            );
        }
    }
    ...
    private void
    renderBobber(TileEntityFocusCrystal te, float tick, double n, double dy) {
        GL11.glPushAttrib(GL11.GL_ALL_ATTRIB_BITS);
        GL11.glDisable(GL11.GL_TEXTURE_2D);
        GL11.glPushMatrix();
        GL11.glTranslated(0.5, 0.5 + dy, 0.5);
