...
public class RenderCastingTable extends ChromaRenderBase {
    ...
    private void renderRunes(
        TileEntityCastingTable te, double par2, double par4, double par6, float par8
    ) {
        ...
        if (te.getTicksExisted() != last && te.getTicksExisted() % d == 0) {
            ...
            while (c.getBlock(te.worldObj) != ChromaBlocks.PYLONSTRUCT.getBlockInstance()
                   || c.getBlockMetadata(te.worldObj) > 2) {
