...
public class RenderWaterLock extends ChromaRenderBase {
    ...
    public void renderTileEntityAt(
        ...
        if (MinecraftForgeClient.getRenderPass() == 0) {
            ...
            //GL11.glScaled(s, s, s);
            Collection<ForgeDirection> li
                = ReikaJavaLibrary.makeSetFromArray(ForgeDirection.VALID_DIRECTIONS);
            ...
            for (ForgeDirection dir : li) {
                GL11.glPushMatrix();
                GL11.glRotated(
                    ReikaDirectionHelper.getHeading(dir) + te.getRotationProgress(),
                    0,
                    1,
                    0
                );
            ...
            for (int i = 2; i < 6; i++) {
                GL11.glPushMatrix();
                GL11.glRotated(
                    ReikaDirectionHelper.getHeading(ForgeDirection.VALID_DIRECTIONS[i])
                        + te.getRotationProgress(),
                    0,
                    1,
                    0
                );
        ...
        if ((te.isCheckpoint() || te.isEndpoint())
            ...
            long tick
                = Minecraft.getMinecraft().theWorld.getTotalWorldTime() + te.hashCode();
            int idx = (int) (tick % 48);
            double u = (idx % 12) / 12D;
            double v = (idx / 12) / 4D;
            double du = u + 1 / 12D;
            ...
            double f = 0.5 + 0.5 * Math.sin(tick / 8D)
                + 0.125 * Math.sin((tick + 300) / 3D) + 0.25 * Math.cos((tick + 20) / 7D);
            int c1;
            int c2;
            if (te.isEndpoint()) {
                c1 = 0xffff00;
                ...
                boolean fluid = te.worldObj.getBlock(te.xCoord, te.yCoord + 1, te.zCoord)
            ...
            int c = ReikaColorAPI.mixColors(
                c1, c2, (float) MathHelper.clamp_double(f, 0, 1)
            );
