...
public class RenderGravityTile extends ChromaRenderBase {
    //private final RandomVariance randomX = new RandomVariance(0.0625, 0.0625,
    //0.03125/32D); private final RandomVariance randomY = new RandomVariance(0.03125,
    //0.0625, 0.03125/32D); private final RandomVariance randomZ = new
    //RandomVariance(0.0625, 0.0625, 0.03125/32D);
    ...
    public void renderTileEntityAt(
        ...
        if (te instanceof GravityTarget) {
            ...
            if (f >= 1) {
                ...
                if (f > 1.0625) {
                    ...
                    if (shake > 0) {
                        double shakex
                            = shake * Math.sin(System.currentTimeMillis() / 10D);
                        double shakez
                            = shake * Math.cos(System.currentTimeMillis() / 20D);
                        GL11.glTranslated(shakex, shakez, 0);
                    }
                }
                ChromaIcons[] ico = new ChromaIcons[] { ChromaIcons.CONCENTRIC2REV,
                ...
                float cm = (float
                ) (0.8125 + 0.1875 * Math.sin(System.currentTimeMillis() / 500D));
        ...
        if (flag) {
            ...
            /*
            double f = Math.min(1,
            (double)te.timer.getTotalTick()/te.timer.getTotalDuration()); if (f >= 0.75)
            ...
            if (shake > 0) {
                ...
                randomY.update();
                randomZ.update();
                GL11.glTranslated(shake*randomX.getValue(), shake*randomY.getValue(),
            shake*randomZ.getValue());
            }
             */
        }
        ...
        if (ico != null) {
            ...
            v5.setColorOpaque_I(ReikaColorAPI.getColorWithBrightnessMultiplier(
                te.getColor().getColor(), te.renderAlpha / 255F
            ));
            v5.addVertexWithUV(-1, 1, 0, u, dv);
            v5.addVertexWithUV(1, 1, 0, du, dv);
            v5.addVertexWithUV(1, -1, 0, du, v);
        ...
        if (te.getTileType() == GravityTiles.TINTER) {
            ...
            v5.setColorOpaque_I(ReikaColorAPI.getColorWithBrightnessMultiplier(
                te.getColor().getColor(), te.renderAlpha / 255F * f
            ));
            v5.addVertexWithUV(-1, 1, 0, u, dv);
            v5.addVertexWithUV(1, 1, 0, du, dv);
            v5.addVertexWithUV(1, -1, 0, du, v);
