...
public class RenderPistonDisplay extends ChromaRenderBase {
    ...
    private void renderDisplay(TilePistonDisplay te) {
        ...
        for (int door = 0; door < li.size(); door++) {
            ...
            double w
                = (tw * 2 / d.colorCount) * (1 - sw) - pad / (d.colorCount * 2) - gap;
            for (int i = 0; i < d.colorCount; i++) {
                double ox = pad + i * w - tw + gap * d.colorCount / 2D;
                double t = System.identityHashCode(d) * 2.8734 + i * 3.47820
                    + System.currentTimeMillis() / 300D;
            ...
            GL11.glTranslated(
                -fr.getStringWidth(sg) / 2D + (1 - sw) * w / fs + sw * 1.35 / fs * ds
                    - ox,
                fr.FONT_HEIGHT / 2D * ds * 1.35,
                0
            );
