...
public class PowerTreeRender extends ChromaRenderBase {
    ...
    public void renderTileEntityAt(
        ...
        if (tile.hasWorldObj() && MinecraftForgeClient.getRenderPass() == 0) {
            ChromaFX.drawEnergyTransferBeams(
                new WorldLocation(te), te.getOutgoingBeamRadius(), te.getTargets()
            );
        }
        GL11.glPopAttrib();
        GL11.glPopMatrix();
    ...
    private void renderHalo(
        TileEntityPowerTree te, double par2, double par4, double par6, float par8
    ) {
        ...
        int d = 1;
        double dsq = GuiScreen.isCtrlKeyDown()
            ? 0
            : Minecraft.getMinecraft().thePlayer.getDistanceSq(
                te.xCoord + 0.5, te.yCoord + 0.5 - 7, te.zCoord + 0.5
            );
        double da = dsq >= 4096 ? 60 : dsq >= 1024 ? 45 : dsq >= 256 ? 30 : 20;
        double i = 0;
        int c = 0xff000000
            | ReikaColorAPI.getModifiedHue(0x5f0000, te.getTicksExisted() % 360);
                    ...
                    if (r2 > 0) {
                        ...
                        if (h == 2) {
                            ReikaRenderHelper.renderTube(
                                x1b, y2, z1b, x2b, y2, z2b, c1, c1, dr, dr, 4
                            );
                        }
                        GL11.glEnable(GL11.GL_TEXTURE_2D);
                        Tessellator.instance.startDrawingQuads();
                        int c2 = ReikaColorAPI.getModifiedHue(
                            0xff7070, te.getTicksExisted() % 360
                        );
                        if (h == -11) {
