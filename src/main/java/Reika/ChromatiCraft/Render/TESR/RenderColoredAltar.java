...
// have translucent shimmering colored "spiky" base in a metal cup, have twirling cubes
// flying around and rising particles
public class RenderColoredAltar extends ChromaRenderBase {
    private static final Random rand = new Random();
    private static final EnumMap<CrystalElement, ColorBlendList[]> colors
        = new EnumMap(CrystalElement.class);
    private static final EnumMap<CrystalElement, ArrayList<Orbit>> orbits
        = new EnumMap(CrystalElement.class);
    ...
    private void renderFloatingCubes(
        TileEntityColoredAltar te,
        CrystalElement e,
        double par2,
        double par4,
        double par6,
        ...
        float pdist = (float) Minecraft.getMinecraft().thePlayer.getDistance(
        ...
        for (int i = 0; i < Math.min(n, li.size()); i++) {
            ...
            double dn
                = 4D * (1 + Minecraft.getMinecraft().gameSettings.particleSetting / 2D);
            if (pdist > 8) {
            ...
            cb.rvX = v * (1 + 0.5 * Math.sin(Math.toRadians(t)))
                * (1 + 0.5 * Math.cos(Math.toRadians(90 + t)));
            cb.rvY = v * (1 + 0.25 * Math.sin(Math.toRadians(2 * t)))
                * (1 + 0.75 * Math.cos(Math.toRadians(t)));
            cb.rvZ = v * (1 + 0.75 * Math.sin(Math.toRadians(90 + t)))
                * (1 + 0.25 * Math.cos(Math.toRadians(2 * t)));
    ...
    private void renderCubeAtPos(
        ...
        /*
        float p = GL11.glGetFloat(GL11.GL_LINE_WIDTH);
        float w = Math.max(0.125F,
        2F-0.03125F*(float)Minecraft.getMinecraft().thePlayer.getDistance(te.xCoord+0.5,
        te.yCoord+0.5, te.zCoord+0.5)); GL11.glLineWidth(w);
