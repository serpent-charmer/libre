...
public class RenderBiomePainter extends ChromaRenderBase {
    ...
    public void renderTileEntityAt(
        ...
        this.renderModel(
            te, outerFrame, this.getTextureFolder() + "biomepainterouter.png"
        );
    ...
    private void renderInnerFrame(TileEntityBiomePainter te, float par8) {
        ...
        if (te.isInWorld()) {
            ...
            te.rotation.rvX = v * (1 + 0.5 * Math.sin(Math.toRadians(t)))
                * (1 + 0.5 * Math.cos(Math.toRadians(90 + t)));
            te.rotation.rvY = v * (1 + 0.25 * Math.sin(Math.toRadians(2 * (t))))
                * (1 + 0.5 * Math.cos(Math.toRadians(t)));
            te.rotation.rvZ = v * (1 + 0.5 * Math.sin(Math.toRadians(90 + t)))
        ...
        this.renderModel(
            te, innerFrame, this.getTextureFolder() + "biomepainterinner.png"
        );
        GL11.glPopMatrix();
    }
    ...
    private void renderGlow(TileEntityBiomePainter te, float par8, int axis) {
        ...
        int c = te.isInWorld() ? ReikaColorAPI.getModifiedHue(
                    0xff0000, (te.getTicksExisted() * 2 + axis * 10) % 360
                )
                               : 0xffffff;
        ...
        double dy = te.isInWorld()
            ? 0.5 * Math.sin((te.getTicksExisted() + par8) / 12D + axis / 4D)
            : 0;
        if (axis >= 3) {
            dy = -dy;
        }
