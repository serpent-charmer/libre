...
public class ChromaItemRenderer implements IItemRenderer {
    ...
    public void renderItem(ItemRenderType type, ItemStack item, Object... data) {
        ...
        if (ChromaItems.RIFT.matchWith(item))
            machine = ChromaTiles.getTileFromIDandMetadata(
                ChromaBlocks.RIFT.getBlockInstance(), item.getItemDamage()
            );
        if (ChromaItems.ADJACENCY.matchWith(item))
            machine = ChromaTiles.ADJACENCY;
        boolean entity = type == ItemRenderType.ENTITY || type == ItemRenderType.EQUIPPED
        ...
        if (!entity
            && (GuiScreen.isShiftKeyDown()
                || RenderItemInSlotEvent.isRenderingStackHovered(item))
            && ProgressStage.USEENERGY.isPlayerAtStage(ep) && machine.isLumenTile()
            && (item.stackTagCompound == null
                || !item.stackTagCompound.getBoolean("tooltip"))) {
            int idx = -1;
