...
public class RenderSplashGunShot extends Render {
    ...
    doRender(Entity e, double par2, double par4, double par6, float par8, float ptick) {
        ...
        double f2
            = 0.625 + 0.375 * Math.sin((System.currentTimeMillis() / mod * 1.8) % 360);
        double f3
            = 0.625 + 0.375 * Math.sin((System.currentTimeMillis() / mod * 2.3) % 360);
        double f4
            = 0.625 + 0.375 * Math.sin((System.currentTimeMillis() / mod * 0.7) % 360);
        v5.setColorOpaque_I(
            ReikaColorAPI.getColorWithBrightnessMultiplier(0xffffff, (float) f1)
        );
        v5.addVertexWithUV(-s2, -s2, -0.05, u2, v2);
        v5.setColorOpaque_I(
            ReikaColorAPI.getColorWithBrightnessMultiplier(0xffffff, (float) f2)
        );
        v5.addVertexWithUV(s2, -s2, -0.05, du2, v2);
        v5.setColorOpaque_I(
            ReikaColorAPI.getColorWithBrightnessMultiplier(0xffffff, (float) f3)
        );
        v5.addVertexWithUV(s2, s2, -0.05, du2, dv2);
        v5.setColorOpaque_I(
            ReikaColorAPI.getColorWithBrightnessMultiplier(0xffffff, (float) f4)
        );
