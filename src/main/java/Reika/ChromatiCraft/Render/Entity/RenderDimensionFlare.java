...
public class RenderDimensionFlare extends Render {
    private final RemoteSourcedAsset texture
        = ChromaClient.dynamicAssets.createAsset("Textures/dimflare2.png");
    ...
    doRender(Entity e, double par2, double par4, double par6, float par8, float ptick) {
        ...
        if (false) {
            ReikaTextureHelper.bindFinalTexture(
                ChromatiCraft.class, "Textures/dimflare_trail_anim.png"
            );
            int frame
                = (int) ((System.currentTimeMillis() / 70) % 16); //(e.ticksExisted/4)%16;
            int w = 4;
            u = (frame % w) / (double) w;
            v = (frame / w) / (double) w;
            du = u + 1D / w;
            ...
            double ang = Math.toDegrees(
                Math.acos(dot / (vel.lengthVector() * pos.lengthVector()))
            );
