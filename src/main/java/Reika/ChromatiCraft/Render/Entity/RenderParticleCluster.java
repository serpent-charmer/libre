...
import Reika.ChromatiCraft.Entity.EntityParticleCluster.Particle;
...
public class RenderParticleCluster extends Render {
    @Override
    public void
    doRender(Entity e, double par2, double par4, double par6, float par8, float ptick) {
        ReikaTextureHelper.bindTerrainTexture();
        EntityParticleCluster eb = (EntityParticleCluster) e;
        Tessellator v5 = Tessellator.instance;
        IIcon icon = ChromaIcons.FADE_RAY.getIcon();
        ...
        if (!e.isDead) {
            for (Particle p : eb.getParticles()) {
                GL11.glPushMatrix();
                double x = p.getRenderPosX(ptick);
                double z = p.getRenderPosZ(ptick);
                double y = eb.posY;
                GL11.glTranslated(x - eb.posX, y - eb.posY, z - eb.posZ);
