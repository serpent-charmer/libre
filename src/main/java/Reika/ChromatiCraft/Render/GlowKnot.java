...
public class GlowKnot {
    ...
    public void render(double x, double y, double z, int color, boolean inworld) {
        ...
        int a = 160;
        v5.setColorRGBA_I(
            ReikaColorAPI.getColorWithBrightnessMultiplier(color, a / 255F), a
        );
    ...
    private static class KnotPoint implements SplineAnchor {
            ...
            if (Math.abs(dr) >= 0.05)
                radius += 0.025
                    * Math.signum(dr
                       ...
                       //Math.abs(dr)*0.03125*0.03125*0.03125)*Math.signum(dr);
            if (Math.abs(dt) >= 1)
                theta += 0.25
                    * Math.signum(dt
                    ); //Math.max(0.125,
                       //Math.abs(dt)*0.03125*0.03125*0.03125)*Math.signum(dt);
            if (Math.abs(dp) >= 1)
                phi += 0.25
                    * Math.signum(dp
                    ); //Math.max(0.125,
                       //Math.abs(dp)*0.03125*0.03125*0.03125)*Math.signum(dp);
        }
        ...
        private void pickNewTarget() {
            targetRadius = ReikaRandomHelper.getRandomPlusMinus(
                maxSize, maxSize / 16D
            ); //rand.nextDouble()*maxSize;
