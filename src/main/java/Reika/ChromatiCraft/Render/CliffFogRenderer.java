...
public class CliffFogRenderer {
    ...
    private final RemoteSourcedAsset texture
        = ChromaClient.dynamicAssets.createAsset("Textures/biomefog_alpha.png");
    ...
    public void render() {
        ...
        if (BiomeGlowingCliffs.isGlowingCliffs(b) && clouds.size() < CLOUD_COUNT
            && rand.nextInt(4) == 0) {
        ...
        double v = 0.125;
        double a
            = ((4 * ep.worldObj.getTotalWorldTime()) % 24000) / 24000D * (Math.PI * 2);
    ...
    private Cloud createCloud(World world, double x, double y, double z) {
        ...
        if (!BiomeGlowingCliffs.isGlowingCliffs(world.getBiomeGenForCoords(
        ...
        Cloud c = new Cloud(rand.nextFloat(), rand.nextFloat(), sx, sy, s, l, dx, dy, dz)
                      .setTextureSpeed(tx, ty)
                      .setSizeSpeed(fx, fy);
    ...
    private static class Cloud {
        ...
        private float getAlpha() {
            return (float) Math.sin(Math.toRadians(180D * age / lifespan)
            ); //age > 0.5F*lifespan ? 2F*(1-age/lifespan) : 2F*age/lifespan;
        }
        ...
        private void render(Tessellator v5, double x, double y, double z) {
            ...
            /*
            v5.addVertexWithUV(-scale*sizeX,	-scale*sizeY,	0, textureX,
            textureY);
            v5.addVertexWithUV(scale*sizeX,		-scale*sizeY,	0, textureX+sizeX,
            textureY);
            v5.addVertexWithUV(scale*sizeX,		scale*sizeY,	0, textureX+sizeX,
            textureY+sizeY); v5.addVertexWithUV(-scale*sizeX,	scale*sizeY,	0,
            textureX,		textureY+sizeY);
             */
