...
public class EntityBallLightningFX extends EntityFX implements CustomRenderFX {
    private static final RenderMode renderMode
        = new RenderMode()
              .setFlag(RenderModeFlags.FOG, false)
              .setFlag(RenderModeFlags.ADDITIVE, true)
              .setFlag(RenderModeFlags.DEPTH, true)
    ...
    public EntityBallLightningFX(
        World world, double x, double y, double z, CrystalElement e
    ) {
        super(world, x, y, z);
        ...
        double[] vals = ReikaPhysicsHelper.polarToCartesian(
            0.0625, rand.nextInt(360), rand.nextInt(360)
        );
        jitterVX = vals[0];
        jitterVY = vals[1];
        jitterVZ = vals[2];
    ...
    public void onUpdate() {
        ...
        if (rand.nextInt(3) == 0) {
            ...
            Minecraft.getMinecraft().effectRenderer.addEffect(
                new EntityCCBlurFX(worldObj, px, py, pz)
                    .setIcon(ChromaIcons.CENTER)
                    .setScale(s)
                    .setColor(
                        (int) (particleRed * 255),
                        (int) (particleGreen * 255),
                        (int) (particleBlue * 255)
                    )
            );
        }
        ...
        if (particleAge == particleMaxAge) {
            for (int i = 0; i < 18; i++) {
                double[] vals = ReikaPhysicsHelper.polarToCartesian(
                    0.125, rand.nextInt(360), rand.nextInt(360)
                );
                Minecraft.getMinecraft().effectRenderer.addEffect(new EntitySparkleFX(
                    worldObj, posX, posY, posZ, vals[0], vals[1], vals[2]
                ));
            }
        }
    }
