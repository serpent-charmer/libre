...
public class EntityCenterBlurFX extends EntityFX implements CustomRenderFX {
    private static final TextureMode texture
        = new CustomTextureMode(ChromatiCraft.class, "Textures/Particle/64x.png");
    private static final RenderMode render
        = new RenderMode()
              .setFlag(RenderModeFlags.FOG, false)
              .setFlag(RenderModeFlags.ADDITIVE, true)
              .setFlag(RenderModeFlags.ALPHACLIP, false)
    ...
    public void onUpdate() {
        ...
        //particleScale =
        //scale*(float)Math.sin(Math.toRadians(180D*particleAge/particleMaxAge)); if
        //(particleAge < 10) 	particleScale = scale*(particleAge+1)/10F; else if
        //(particleAge > 50) 	particleScale = scale*(61-particleAge)/10F; else
        //	particleScale = scale;
        ...
        if (cyclescale > 0) {
            CrystalElement e
                = CrystalElement.elements[(int) ((particleAge * cyclescale) % 16)];
            particleRed = e.getRed() / 255F;
            particleGreen = e.getGreen() / 255F;
            particleBlue = e.getBlue() / 255F;
        }
    ...
    {
        v5.draw();
        ReikaTextureHelper.bindFinalTexture(ChromatiCraft.class,
    "Textures/Particle/64x.png"); BlendMode.ADDITIVEDARK.apply(); GL11.glColor4f(1, 1, 1,
    1); v5.startDrawingQuads(); v5.setBrightness(this.getBrightnessForRender(0));
