...
public class EngravedRuneFX extends EntityFX {
    ...
    public EngravedRuneFX(
        World world, double x, double y, double z, CrystalElement e, ForgeDirection dir
    ) {
        super(world, x, y, z);
        particleMaxAge = 30;
        noClip = true;
    ...
    public void renderParticle(
        ...
        v5.addVertexWithUV(
            dx + 1, dy, dz, particleIcon.getMaxU(), particleIcon.getMinV()
        );
        v5.addVertexWithUV(
            dx + 1, dy, dz + 1, particleIcon.getMaxU(), particleIcon.getMaxV()
        );
        v5.addVertexWithUV(
            dx, dy, dz + 1, particleIcon.getMinU(), particleIcon.getMaxV()
        );
        v5.draw();
        v5.startDrawingQuads();
    }
