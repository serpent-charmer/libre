...
public class CliffFogRenderer_Noise2 {
    ...
    private final NoiseGeneratorBase cloudNoise
        = new SimplexNoiseGenerator(System.currentTimeMillis())
              .addOctave(2, 0.5)
              .addOctave(4, 0.125)
              .addOctave(8, 0.125)
              .setFrequency(1 / 24D);
    private final NoiseGeneratorBase cloudFreqNoise
        = new SimplexNoiseGenerator(-System.currentTimeMillis()).setFrequency(1 / 36D);
    ...
    private final float getCloudHeight(double x, double z, double t) {
        ...
        double f = Math.sin(x + t) + Math.sin(1.7 * z + 1.3 * t)
            + 0.36 * Math.sin(0.98 * x + 1.23 * t + 11.4)
            + Math.sin(0.41 * z + 2.73 * t + 5.2);
        return (float) (v * f);
    }
}
