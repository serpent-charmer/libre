...
public class DimensionDecoRenderer extends ISBRH {
        ...
        for (int i = 0; i < latticeRotations.length; i++) {
            for (int j = 0; j < latticeRotations[i].length; j++) {
                for (int k = 0; k < latticeRotations[i][j].length; k++) {
                    latticeRotations[i][j][k]
                        = ReikaRandomHelper.getRandomBetween(0D, 360D);
                }
            }
        }
    }
    ...
    @Override
    public void
    renderInventoryBlock(Block block, int metadata, int modelId, RenderBlocks rb) {
        DecoType type = block instanceof BlockDimensionDecoTile
            ? DimDecoTileTypes.list[metadata]
            : DimDecoTypes.list[metadata];
    ...
    public boolean renderWorldBlock(
        ...
        DecoType type = b instanceof BlockDimensionDecoTile ? DimDecoTileTypes.list[meta]
                                                            : DimDecoTypes.list[meta];
        if (renderPass == 0) {
            if (type.hasBlockRender()) {
                ...
                for (IIcon ico : li) {
                    ...
                    Tessellator.instance.setColorOpaque_I(
                        idx == 1 ? b.colorMultiplier(world, x, y, z) : 0xffffff
                    );
            ...
            if (type.hasBlockRender()) {
                ...
                for (IIcon ico : li) {
                    Tessellator.instance.setBrightness(240);
                    Tessellator.instance.setColorOpaque_I(
                        idx == 0 ? b.colorMultiplier(world, x, y, z) : 0xffffff
                    );
                    if (b.shouldSideBeRendered(
                            world, x, y - 1, z, ForgeDirection.DOWN.ordinal()
                        ))
    ...
    private void render(
        ...
        switch (type) {
            ...
            case CLIFFGLASS:
                break;
            case GLOWCAVE:
                //rb.renderStandardBlockWithAmbientOcclusion(this.getGlowCaveUnderlay(world,
                //x, y, z), x, y, z, 1, 1, 1);
                break;
        }
    }
        ...
        switch (type) {
            case MIASMA: {
                ...
                int c = ReikaColorAPI.getModifiedHue(
                    ...
                    220
                        + (int
                        ) (80 * Math.sin((x * x * 2 + y * y + z * z * 8) / (100000D * 20))
                        )
                );
            ...
            case AQUA: {
                ...
                for (int n = 0; n <= 2; n++) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        ...
                        h[i] = 0.5 + 0.5 * Math.sin(Math.toRadians(idx))
                            + 0.05 * Math.sin(Math.toRadians(idx2));
                    }
            ...
            case LATTICE: {
                ...
                for (int n = 0; n < 6; n++) {
                    ...
                    int c = ReikaColorAPI.getModifiedHue(
                        0xff4040, (int) (n * 7.5) + ((i + j + k) % 12) * 30
                    );
                    ...
                    vt5.rotateNonOrthogonal(
                        latticeRotations[0][i][n],
                        latticeRotations[1][j][n],
                        latticeRotations[2][k][n]
                    );
                    vt5.render();
                }
                break;
            ...
            case GLOWCAVE: { /*
                 ...
                 Tessellator.instance.setBrightness(240);
                 Tessellator.instance.setColorOpaque_I(0xffffff);
                 if (b.shouldSideBeRendered(world, x, y-1, z,
                 ForgeDirection.DOWN.ordinal())) rb.renderFaceYNeg(b, x, y, z, ico); if
                 (b.shouldSideBeRendered(world, x, y+1, z, ForgeDirection.UP.ordinal()))
                     rb.renderFaceYPos(b, x, y, z, ico);
                 if (b.shouldSideBeRendered(world, x, y, z-1,
                 ForgeDirection.NORTH.ordinal())) rb.renderFaceZNeg(b, x, y, z, ico); if
                 (b.shouldSideBeRendered(world, x, y, z+1,
                 ForgeDirection.SOUTH.ordinal())) rb.renderFaceZPos(b, x, y, z, ico); if
                 (b.shouldSideBeRendered(world, x-1, y, z, ForgeDirection.WEST.ordinal()))
                     rb.renderFaceXNeg(b, x, y, z, ico);
                 if (b.shouldSideBeRendered(world, x+1, y, z,
                 ForgeDirection.EAST.ordinal())) rb.renderFaceXPos(b, x, y, z, ico);*/
                break;
            }
        }
    }
