...
public class TieredOreRenderer extends ISBRH {
    ...
    private final double[][] stoneOffsets
        = new double[16][(numSections + 1) * (numSections + 1)];
    ...
    @Override
    public void
    renderInventoryBlock(Block b, int metadata, int modelId, RenderBlocks rb) {
        ...
        boolean tier = bt.getProgressStage(metadata).isPlayerAtStage(
            Minecraft.getMinecraft().thePlayer
        );
        IIcon ico
            = tier ? bt.getBacking(metadata) : bt.getDisguise(metadata).getIcon(0, 0);
    ...
    public boolean renderWorldBlock(
        ...
        if (t.isPlayerSufficientTier(
            ...
            rb.renderStandardBlockWithAmbientOcclusion(
                t.getDisguise(meta), x, y, z, 1, 1, 1
            );
        }
        return true;
    }
    ...
    private void renderSimpleGeode(
        ...
        IIcon icos
            = ((BlockTieredOre) b).getGeodeStoneIcon(Math.abs(x + y * x * z + z) % 16);
    ...
    private void renderGeode(
        ...
        if (val != null)
            ReikaJavaLibrary.pConsole("Rerendering "+TieredOres.list[meta]+" geode at
        "+x+", "+y+", "+z+"; "+(System.currentTimeMillis()-val.longValue())+" ms since
        last render here."); else ReikaJavaLibrary.pConsole("Rerendering
        "+TieredOres.list[meta]+" geode at "+x+", "+y+", "+z+"; Has not rendered
        before."); renderMap.put(c, System.currentTimeMillis());
         */
        ...
        for (int i = 0; i < numSections; i++) {
            ...
            for (int k = 0; k < numSections; k++) {
                double d2 = k * s;
                boolean center
                    = false; //ReikaMathLibrary.isValueInsideBoundsIncl(2, numSections-3,
                             //i) && ReikaMathLibrary.isValueInsideBoundsIncl(2,
                             //numSections-3, k);
                //center |= ReikaMathLibrary.isValueInsideBoundsIncl(1, numSections-2, i)
                //&& ReikaMathLibrary.isValueInsideBoundsIncl(2, numSections-3, k); center
                //|= ReikaMathLibrary.isValueInsideBoundsIncl(2, numSections-3, i) &&
                //ReikaMathLibrary.isValueInsideBoundsIncl(1, numSections-2, k);
                float us1 = center ? (float) (ub + (dub - ub) * d)
                                   : (float) (us + (dus - us) * d);
                float vs1 = center ? (float) (vb + (dvb - vb) * d2)
                                   : (float) (vs + (dvs - vs) * d2);
                float us2 = center ? (float) (ub + (dub - ub) * (d + s))
                                   : (float) (us + (dus - us) * (d + s));
                float vs2 = center ? (float) (vb + (dvb - vb) * (d2 + s))
                                   : (float) (vs + (dvs - vs) * (d2 + s));

                /*
                v5.addVertexWithUV(d, stoneOffsets[da][(i)*numSections+(k+1)], d2+s, us1,
                vs1); v5.addVertexWithUV(d+s, stoneOffsets[da][(i+1)*numSections+(k+1)],
                d2+s, us2, vs1); v5.addVertexWithUV(d+s,
                stoneOffsets[da][(i+1)*numSections+(k)], d2, us2, vs2);
                v5.addVertexWithUV(d, stoneOffsets[da][(i)*numSections+(k)], d2, us1,
                vs2);
                 */

                v5.addVertexWithUV(
                    d, stoneOffsets[da][(i) *numSections + (k + 1)], d2 + s, us1, vs2
                );
                v5.addVertexWithUV(
                    d + s,
                    stoneOffsets[da][(i + 1) * numSections + (k + 1)],
                    d2 + s,
                    us1,
                    vs1
                );
                v5.addVertexWithUV(
                    d + s, stoneOffsets[da][(i + 1) * numSections + (k)], d2, us2, vs1
                );
                v5.addVertexWithUV(
                    d, stoneOffsets[da][(i) *numSections + (k)], d2, us2, vs2
                );
            }
        }
        ...
        if (world != null) {
            ...
            for (int i = 0; i < 6; i++) {
                ...
                ReikaJavaLibrary.pConsole(dir+" > "+b.shouldSideBeRendered(world,
        x+dir.offsetX, y+dir.offsetY, z+dir.offsetZ, dir.ordinal()));
            }
            ReikaJavaLibrary.pConsole("---------------------------------------");
        }
         */
        if (world == null
            || b.shouldSideBeRendered(world, x, y + 1, z, ForgeDirection.UP.ordinal())) {
            if (world != null) {
                Tessellator.instance.setBrightness(
                    b.getMixedBrightnessForBlock(world, x, y + 1, z)
                );
        ...
        if (world == null
            ...
            if (world != null) {
                Tessellator.instance.setBrightness(
                    b.getMixedBrightnessForBlock(world, x - 1, y, z)
                );
        ...
        if (world == null
            ...
            if (world != null) {
                Tessellator.instance.setBrightness(
                    b.getMixedBrightnessForBlock(world, x, y, z + 1)
                );
        ...
        for (int i = 0; i < numSections; i++) {
            ...
            for (int k = 0; k < numSections; k++) {
                ...
                v5.addVertexWithUV(
                    d, oreOffsets[da][(i) *numSections + (k + 1)], d2b, uo1, vo1
                );
                v5.addVertexWithUV(
                    db, oreOffsets[da][(i + 1) * numSections + (k + 1)], d2b, uo2, vo1
                );
                v5.addVertexWithUV(
                    db, oreOffsets[da][(i + 1) * numSections + (k)], d2, uo2, vo2
                );
                v5.addVertexWithUV(
                    d, oreOffsets[da][(i) *numSections + (k)], d2, uo1, vo2
                );
            }
        }
        if (world == null
