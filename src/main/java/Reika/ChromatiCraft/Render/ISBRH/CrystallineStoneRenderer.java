...
public class CrystallineStoneRenderer extends ISBRH {
    public CrystallineStoneRenderer(int id) {
        super(id);
    }

    @Override
    public void
    renderInventoryBlock(Block bk, int metadata, int modelId, RenderBlocks rb) {
    ...
    public boolean renderWorldBlock(
        IBlockAccess world, int x, int y, int z, Block bk, int modelId, RenderBlocks rb
    ) {
        BlockPylonStructure b = (BlockPylonStructure) bk;
        ...
        v5.setBrightness(rb.renderMinY > 0.0D ? l : b.getMixedBrightnessForBlock(world, x,
        y - 1, z)); v5.setColorOpaque_F(f1, f1, f1); rb.renderFaceYNeg(b, x, y, z, ico);

        v5.setBrightness(rb.renderMaxY < 1.0D ? l : b.getMixedBrightnessForBlock(world, x,
        y + 1, z)); v5.setColorOpaque_F(f2, f2, f2); rb.renderFaceYPos(b, x, y, z, ico);

        v5.setBrightness(rb.renderMinZ > 0.0D ? l : b.getMixedBrightnessForBlock(world, x,
        y, z - 1)); v5.setColorOpaque_F(f3, f3, f3); rb.renderFaceZNeg(b, x, y, z, ico);

        v5.setBrightness(rb.renderMaxZ < 1.0D ? l : b.getMixedBrightnessForBlock(world, x,
        y, z + 1)); v5.setColorOpaque_F(f4, f4, f4); rb.renderFaceZPos(b, x, y, z, ico);

        v5.setBrightness(rb.renderMinX > 0.0D ? l : b.getMixedBrightnessForBlock(world, x
        - 1, y, z)); v5.setColorOpaque_F(f5, f5, f5); rb.renderFaceXNeg(b, x, y, z, ico);

        v5.setBrightness(rb.renderMaxX < 1.0D ? l : b.getMixedBrightnessForBlock(world, x
        + 1, y, z)); v5.setColorOpaque_F(f6, f6, f6); rb.renderFaceXPos(b, x, y, z, ico);
         */
        ...
        if (renderPass == s.getBrightRenderPass()
            ...
            for (int i = 0; i < 6; i++) {
                ...
                if (b.shouldSideBeRendered(
                        world, x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ, i
                    )) {
    ...
    private void renderConnectedTextures(
        IBlockAccess world,
        int x,
        int y,
        int z,
        StoneTypes s,
        BlockPylonStructure b,
        int meta,
        RenderBlocks rb,
        Tessellator v5
    ) {
