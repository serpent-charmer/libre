...
public class RelayRenderer extends ISBRH {
    ...
    public boolean renderWorldBlock(
        ...
        if (renderPass == 1) {
            ...
            this.renderConnectivity(
                v5, ((TileRelayBase) world.getTileEntity(x, y, z)).getInput(), dir
            );
        }
    ...
    private void
    renderConnectivity(Tessellator v5, ForgeDirection in, ForgeDirection face) {
        double w = 0.1875;
        double h = 0.25;
        ...
        for (int i = 0; i < 6; i++) {
            ...
            switch (dir) {
                case EAST:
                    v5.addVertexWithUV(
                        h * face.offsetX + 0.5 + w,
                        h * face.offsetY + 0.5 - w,
                        h * face.offsetZ + 0.5 - w,
                        u,
                        v
                    );
                    v5.addVertexWithUV(
    ...
    private void renderDir(
        ...
        IIcon ico = renderPass == 1
            ? ChromaBlocks.CRYSTAL.getBlockInstance().getIcon(0, 0)
            : ChromaBlocks.PYLONSTRUCT.getBlockInstance().getIcon(0, 0);
        float u = ico.getMinU();
        float v = ico.getMinV();
        float du = ico.getMaxU();
        float dv = ico.getMaxV();
        ...
        /*
        v5.addVertexWithUV(minx, 									miny, minz,
        u, v); v5.addVertexWithUV(minx+w*2*dir.offsetX, 					miny,
        minz+w*2*dir.offsetZ, 					du, v);
        v5.addVertexWithUV(minx+w*2*dir.offsetX+w*2*dir.offsetZ, 	miny,
        minz+w*2*dir.offsetX+w*2*dir.offsetZ, 	du, dv);
        v5.addVertexWithUV(minx+w*2*dir.offsetZ, 					miny,
        minz+w*2*dir.offsetX, 					u, dv);
         */
        TessellatorVertexList v5 = new TessellatorVertexList(
            0.5 - Math.abs(dir.offsetX) * 0.5,
            0.5 - Math.abs(dir.offsetY) * 0.5,
            0.5 - Math.abs(dir.offsetZ) * 0.5
        );
