...
public class MetaAlloyRenderer extends ISBRH {
    ...
    private void
    renderLowPetals(IBlockAccess world, int x, int y, int z, Tessellator v5) {
        //float f = 0.5F+rand.nextFloat()*0.5F;
        //int c = ReikaColorAPI.mixColors(Blocks.grass.colorMultiplier(world, x, y, z),
        //0x22aaff, f); ReikaJavaLibrary.pConsole(this.calcSeed(x, y, z)+" @ "+x+","+z+" >
        //"+f+" > "+Integer.toHexString(c)); Tessellator.instance.setColorOpaque_I(c);
    ...
    private void renderRaisedPetals(
        IBlockAccess world, int x, int y, int z, double ang1, double ang2, double ang3
    ) {
