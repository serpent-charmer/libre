...
public class RayBlendFloorRenderer extends ISBRH {
    private final RayBlendGridRenderer edge
        = (RayBlendGridRenderer) new RayBlendGridRenderer().setIcons(
            BlockSpecialShield.edgeIcons
        );
    ...
    public boolean renderWorldBlock(
        ...
        v5.setColorOpaque_I(ReikaColorAPI.getColorWithBrightnessMultiplier(
            b.colorMultiplier(world, x, y, z), 1
        ));
        ...
        if (b.shouldSideBeRendered(world, x, y + 1, z, 1)) {
            ...
            TileEntityRayblendFloor te
                = (TileEntityRayblendFloor) world.getTileEntity(x, y, z);
            if (te.isBlocked()) {
                ...
                if (clr instanceof CrystalElement) {
                    rb.renderFaceYPos(
                        b, x, y + 0.01, z, ((CrystalElement) clr).getEngravingRune()
                    );
                }
            }
        }
    ...
    private static class RayBlendGridRenderer extends EdgeDetectionRenderer {
        ...
        match(IBlockAccess world, int x0, int y0, int z0, int x, int y, int z) {
            return super.match(world, x0, y0, z0, x, y, z)
                && this.matchGrids(world, x0, y0, z0, x, y, z);
        }

        private boolean
        matchGrids(IBlockAccess world, int x1, int y1, int z1, int x2, int y2, int z2) {
            TileEntityRayblendFloor te1
                = (TileEntityRayblendFloor) world.getTileEntity(x1, y1, z1);
            TileEntityRayblendFloor te2
                = (TileEntityRayblendFloor) world.getTileEntity(x2, y2, z2);
