...
public class SparklingBlockRender extends ISBRH {
    ...
    renderInventoryBlock(Block block, int metadata, int modelId, RenderBlocks rb) {
        rb.renderBlockAsItem(
            BlockSparkle.BlockTypes.list[metadata].getBlockProxy(), 0, 1
        );
        ...
        //rb.renderBlockAsItem(ChromaBlocks.SPECIALSHIELD.getBlockInstance(),
        //BlockType.GLASS.metadata, 1);
        /*
        GL11.glTranslated(-0.5, -0.5, -0.5);
        ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(0, 0,
        ChromaIcons.SPARKLE.getIcon(), 1, 1); GL11.glTranslated(0, 0, 1);
        ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(0, 0,
        ChromaIcons.SPARKLE.getIcon(), 1, 1); GL11.glRotated(90, 1, 0, 0);
        GL11.glTranslated(0, -1, -1);
        ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(0, 0,
        ChromaIcons.SPARKLE.getIcon(), 1, 1); GL11.glTranslated(0, 0, 1);
        ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(0, 0,
        ChromaIcons.SPARKLE.getIcon(), 1, 1); GL11.glRotated(90, 0, 1, 0);
        ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(0, 0,
        ChromaIcons.SPARKLE.getIcon(), 1, 1); GL11.glTranslated(0, 0, 1);
        ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(0, 0,
        ChromaIcons.SPARKLE.getIcon(), 1, 1);
         */
        GL11.glTranslated(-0.5, -0.5, -0.5);
    ...
    public boolean renderWorldBlock(
        ...
        for (int s = 0; s < 6; s++) {
            ...
            if (block.shouldSideBeRendered(
                    world, x - dir.offsetX, y - dir.offsetY, z - dir.offsetZ, s
                )) {
                int n = 1 + rand.nextInt(6);
                boolean[][] draw = new boolean[16][16];
                for (int i = 0; i < n; i++) {
                    int c = ReikaColorAPI.RGBtoHex(
                        192 + rand.nextInt(64),
                        192 + rand.nextInt(64),
                        192 + rand.nextInt(64)
                    );
                    int c2 = ReikaColorAPI.getColorWithBrightnessMultiplier(c, 0.5F);
                    IIcon ico = rand.nextInt(3) == 0
                        ? ChromaIcons.BASICFADE_FAST.getIcon()
                        : ChromaIcons.BASICFADE.getIcon();
