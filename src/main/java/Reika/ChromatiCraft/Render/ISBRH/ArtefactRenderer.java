...
public class ArtefactRenderer extends ISBRH {
    ...
    private void
    renderPedestal(IBlockAccess world, int x, int y, int z, RenderBlocks rb) {
        ReikaRenderHelper.renderBlockSubCube(
            x, y, z, 1, 1, 1, 14, 2, 14, Tessellator.instance, rb, Blocks.stonebrick, 0
        );
        ReikaRenderHelper.renderBlockSubCube(
            x, y, z, 2, 12, 2, 12, 2, 12, Tessellator.instance, rb, Blocks.stonebrick, 0
        );
        ReikaRenderHelper.renderBlockSubCube(
            x, y, z, 4, 2, 4, 8, 11, 8, Tessellator.instance, rb, Blocks.stonebrick, 0
        );
        ReikaRenderHelper.renderBlockSubCube(
            x, y, z, 3, 2, 3, 10, 5, 10, Tessellator.instance, rb, Blocks.stonebrick, 0
        );
    }
    ...
    private void renderArtefactSimple(IBlockAccess world, int x, int y, int z, Block b) {
        ...
        double ang1
            = natural ? 90 - 60 + rand.nextDouble() * 120 : 85 + rand.nextDouble() * 10;
        double ang2
            = natural ? -45 + rand.nextDouble() * 90 : -5 + rand.nextDouble() * 10;
    ...
    private void
    renderDirtChunks(IBlockAccess world, int x, int y, int z, RenderBlocks rb) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            dz += i * 0.001;
            ReikaRenderHelper.renderBlockSubCube(
                x, y, z, dx, dy, dz, s, Tessellator.instance, rb, Blocks.dirt, 0
            );
        }
    }
