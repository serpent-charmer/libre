...
public class CrystalEncrustingRenderer extends ISBRH {
    ...
    private final GridDistortion[] distortions
        = new GridDistortion[MAX_SEGMENTS - MIN_SEGMENTS + 1];
    ...
    renderInventoryBlock(Block bk, int metadata, int modelId, RenderBlocks rb) {
        ...
        int c = ReikaColorAPI.mixColors(
            CrystalElement.elements[metadata].getColor(), 0xffffff, 0.85F
        );

        rand.setSeed(
            Minecraft.getMinecraft().thePlayer.getUniqueID().hashCode() ^ metadata
        );
        rand.nextBoolean();
        int pieces = 12;
        int n = 6;
    ...
    public boolean renderWorldBlock(
        ...
        for (CrystalGrowth g : c) {
            this.renderCrystalFace(
                world, x, y, z, b, te, g, rb, CrystalElement.elements[meta].getColor()
            );
        }
        return true;
    }
    ...
    private void renderCrystalFace(
        IBlockAccess world,
        int x,
        int y,
        int z,
        Block bk,
        TileCrystalEncrusted te,
        CrystalGrowth g,
        RenderBlocks rb,
        int color
    ) {
        ...
        for (int i = 0; i < pieces; i++) {
            ...
            double h = rh / 96D;
            this.renderCrystalPiece(
                world, x, y, z, bk, te, g, rb, color, a, b, w, h, grid
            );
        }
    }

    private void renderCrystalPiece(
        IBlockAccess world,
        int x,
        int y,
        int z,
        ...
        //red = MathHelper.clamp_float((float)ReikaRandomHelper.getRandomPlusMinus(red,
        //0.2F), 0, 1); green =
        //MathHelper.clamp_float((float)ReikaRandomHelper.getRandomPlusMinus(green, 0.2F),
        //0, 1); blue =
        //MathHelper.clamp_float((float)ReikaRandomHelper.getRandomPlusMinus(blue, 0.2F),
        //0, 1);
        ...
        renderBlock.clamp();
        ReikaRenderHelper.renderBlockPieceNonCuboid(
            world, x, y, z, bk, Tessellator.instance, renderBlock
        );

        Tessellator.instance.setBrightness(240);
        Tessellator.instance.setColorRGBA_I(
            ReikaColorAPI.mixColors(color, 0xffffff, 0.9F), te.isSpecial() ? 240 : 192
        );
        renderBlock.renderIconOnSides(
            world, x, y, z, ChromaIcons.GLOWFRAME_TRANS.getIcon(), Tessellator.instance
        );
        ...
        if (te.isSpecial()) {
            ...
            renderBlock.renderIconOnSides(
                world, x, y, z, BlockEncrustedCrystal.specialIcon, Tessellator.instance
            );
        }
    }
