...
public class EverFluidRenderer extends ISBRH {
    ...
    public float getFluidHeightForRender(
        IBlockAccess world, int x, int y, int z, Block block, float base
    ) {
        Block b = world.getBlock(x, y, z);
        if (b == block) {
            int meta = world.getBlockMetadata(x, y, z);
            Block verticalOrigin = world.getBlock(x, y + 1, z);
            if (verticalOrigin.getMaterial().isLiquid()
                || verticalOrigin instanceof IFluidBlock) {
                return 1;
            }
    ...
    public boolean renderWorldBlock(
        ...
        boolean[] renderSides = { block.shouldSideBeRendered(world, x, y, z - 1, 2),
        ...
        if (!renderTop && !renderBottom && !renderSides[0] && !renderSides[1]
            ...
            if (flow11 != 1) {
                float flow00
                    = this.getFluidHeightForRender(world, x - 1, y, z - 1, block, flow11);
                float flow01
                    = this.getFluidHeightForRender(world, x - 1, y, z, block, flow11);
                float flow02
                    = this.getFluidHeightForRender(world, x - 1, y, z + 1, block, flow11);
                float flow10
                    = this.getFluidHeightForRender(world, x, y, z - 1, block, flow11);
                float flow12
                    = this.getFluidHeightForRender(world, x, y, z + 1, block, flow11);
                float flow20
                    = this.getFluidHeightForRender(world, x + 1, y, z - 1, block, flow11);
            ...
            if (renderer.renderAllFaces || renderTop) {
                ...
                float flowDir
                    = -1000; //(float) BlockFluidBase.getFlowDirection(world, x, y, z);
                ...
                tessellator.setColorOpaque_F(
                    LIGHT_Y_POS * red, LIGHT_Y_POS * green, LIGHT_Y_POS * blue
                );
            ...
            if (renderer.renderAllFaces || renderBottom) {
                ...
                tessellator.setColorOpaque_F(
                    LIGHT_Y_NEG * red, LIGHT_Y_NEG * green, LIGHT_Y_NEG * blue
                );
                renderer.renderFaceYNeg(
                    block, x, y + RENDER_OFFSET, z, this.getIcon(block.getIcon(0, bMeta))
                );
            }
            ...
            for (int side = 0; side < 4; ++side) {
                ...
                if (renderer.renderAllFaces || renderSides[side]) {
                    ...
                    tessellator.setBrightness(
                        block.getMixedBrightnessForBlock(world, x2, y, z2)
                    );
