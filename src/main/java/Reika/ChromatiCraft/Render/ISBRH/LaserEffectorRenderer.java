...
public class LaserEffectorRenderer extends ISBRH {
    ...
    public boolean renderWorldBlock(
        ...
        double slab = 0.33;
        double h = slab
            + (type == LaserEffectType.TARGET || type == LaserEffectType.TARGET_THRU
                   ? 0.55
                   : 0.33);
        double w = 0.0625;
        double out = 0.1875;
    ...
    private void renderFullBlock(
        ...
        Block block,
        int meta,
        LaserEffectType type,
        LaserEffectTile te,
        RenderBlocks rb,
        Tessellator v5
    ) {
        ForgeDirection face
            = te.getFacing().isCardinal() ? te.getFacing().getCardinal() : null;
        if (type == LaserEffectType.TARGET)
            face = face.getOpposite();
        for (int i = 0; i < 6; i++) {
            boolean side = face != null && face.ordinal() == i;
            boolean color = renderPass == 1 && side;
            IIcon ico = side ? (renderPass == 1 ? type.frontOverlay : type.frontTexture)
                             : BlockLaserEffector.baseTexture;
            float f = 1F;
    ...
    private void renderCentralItem(
        IBlockAccess world,
        int x,
        int y,
        int z,
        ...
        LaserEffectType type = LaserEffectType.list[meta];
        IIcon ico = ChromaBlocks.CRYSTAL.getBlockInstance().getIcon(
            0, CrystalElement.WHITE.ordinal()
        );
        float u = ico.getMinU();
        float v = ico.getMinV();
        float du = ico.getMaxU();
