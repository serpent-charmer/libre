...
public class CrystalRenderer extends ISBRH {
    ...
    public void renderInventoryBlock(Block b, int meta, int modelID, RenderBlocks rb) {
        ...
        int color = ReikaColorAPI.getColorWithBrightnessMultiplier(
            ((CrystalRenderedBlock) b).getTintColor(meta), 0.8F
        );
        if (staticColor >= 0)
            color = staticColor;
        int alpha = 255;
    ...
    public boolean renderWorldBlock(
        ...
        //boolean discover =
        //ProgressionManager.instance.hasPlayerDiscoveredColor(Minecraft.getMinecraft().thePlayer);
        int color = /*discover ? */ ((CrystalRenderedBlock) b)
                        .getTintColor(meta); // : 0xbbbbbb;
        ...
        boolean above
            = world.getBlock(x, y + 1, z) == ChromaBlocks.CRYSTAL.getBlockInstance();
        boolean below = !((CrystalRenderedBlock) b).renderBase()
            && world.getBlock(x, y - 1, z) instanceof CrystalBlock;

        //v5.setBrightness(rb.renderMaxY < 1.0D ? l : b.getMixedBrightnessForBlock(world,
        //x, y+1, z));
        v5.setBrightness(240);
        ...
        boolean flip = !world.getBlock(x, y - 1, z)
        ...
        if (renderPass == 1) {
            ...
            ); //((x*3+y+z*7)%16+16)%16; //16 combos -> binary selector
            if (val > 15 || ((CrystalRenderedBlock) b).renderAllArms())
                val = 15;
            if ((val & 8) == 8)
                this.renderXAngledSpike(
                    u, v, xu, xv, 0.1875, w, below, flip
    ...
    private void renderSpike(
        double u, double v, double xu, double xv, int w, boolean above, boolean flip
    ) {
        double core = 0.15;
    ...
    private void renderXAngledSpike(
        double u,
        double v,
        double xu,
        double xv,
        ...
        v5.addVertexWithUV(
            0.5 + core * dir + out + tout, dy + vl + dvl + htip, 0.5, xu, xv
        ); //tip
        v5.addVertexWithUV(
            0.5 + core * dir + out + tout, dy + vl + dvl + htip, 0.5, xu, xv
        );
        ...
        v5.addVertexWithUV(
            0.5 + core * dir + out + tout, dy + vl + dvl + htip, 0.5, u, xv
        );
        v5.addVertexWithUV(
            0.5 + core * dir + out + tout, dy + vl + dvl + htip, 0.5, xu, xv
        ); //tip
        ...
        if (!below) {
            //base
            v5.addVertexWithUV(
                0.5 - core * dir * ds + out * 2.56, dy + dvl, 0.5 - core, u, xv
            );
            v5.addVertexWithUV(
                0.5 - core * dir * ds + out * 2.56, dy + dvl, 0.5 + core, xu, xv
            );
    ...
    private void renderZAngledSpike(
        double u,
        double v,
        double xu,
        double xv,
        ...
        v5.addVertexWithUV(
            0.5, dy + vl + dvl + htip, 0.5 + core * dir + out + tout, xu, xv
        ); //tip
        v5.addVertexWithUV(
            0.5, dy + vl + dvl + htip, 0.5 + core * dir + out + tout, u, xv
        );
        ...
        if (!below) {
            //base
            v5.addVertexWithUV(
                0.5 - core, dy + dvl + 0.0025, 0.5 - core * dir * ds + out * 2.56, u, xv
            );
            v5.addVertexWithUV(
                0.5 + core, dy + dvl + 0.0025, 0.5 - core * dir * ds + out * 2.56, xu, xv
            );
