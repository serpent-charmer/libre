...
public class VoidRiftRenderer extends ISBRH {
    ...
    /*
    private boolean renderAura(Block block, IBlockAccess world, int x, int y, int z,
    CrystalElement e) { boolean flag = false; for (int i = 2; i < 6; i++) { ForgeDirection
    dir = ForgeDirection.VALID_DIRECTIONS[i]; ForgeDirection left =
    ReikaDirectionHelper.getLeftBy90(dir); int dx = x+dir.offsetX; int dy = y+dir.offsetY;
