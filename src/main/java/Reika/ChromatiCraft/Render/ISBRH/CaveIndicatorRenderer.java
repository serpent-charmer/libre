...
public class CaveIndicatorRenderer extends ISBRH {
    public CaveIndicatorRenderer(int id) {
        super(id);
    }
    ...
    renderInventoryBlock(Block b, int metadata, int modelId, RenderBlocks rb) {
        ...
        IIcon ico = BlockCaveIndicator.innerTexture_Active;
        tessellator.startDrawingQuads();
        tessellator.addTranslation(0, -0.1F, 0);
        tessellator.setNormal(0.0F, 1.0F, 0.0F);
    ...
    public boolean renderWorldBlock(
        ...
        if (world.getBlockMetadata(x, y, z) == 1) {
            ico = BlockCaveIndicator.innerTexture_Active;
            v5.setBrightness(240);
        } else {
            ico = BlockCaveIndicator.innerTexture;
