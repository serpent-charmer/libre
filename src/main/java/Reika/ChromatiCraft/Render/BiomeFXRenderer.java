...
public class BiomeFXRenderer {
    ...
    static {
        colorMapA.put(
            ExtraChromaIDs.RAINBOWFOREST.getValue(), new SolidColorReference(0x22aaff)
        );
        colorMapB.put(
            ExtraChromaIDs.RAINBOWFOREST.getValue(), new SolidColorReference(0x0060ff)
        );

        colorMapA.put(
            ExtraChromaIDs.ENDERFOREST.getValue(), new SolidColorReference(0xa060ff)
        );
        colorMapB.put(
            ExtraChromaIDs.ENDERFOREST.getValue(), new SolidColorReference(0xffaaff)
        );

        colorMapA.put(
            ExtraChromaIDs.LUMINOUSCLIFFS.getValue(),
            new ColorBlendReference(new ColorBlendList(5, ChromaFX.getChromaColorTiles()))
        );
        colorMapB.put(
            ExtraChromaIDs.LUMINOUSCLIFFS.getValue(),
            colorMapA.get(ExtraChromaIDs.LUMINOUSCLIFFS.getValue())
        );
    }
    ...
    public void render() {
        ...
        if (c1 != null
            && ep.worldObj.getSavedLightValue(EnumSkyBlock.Sky, px, py, pz) > 5) {
            ...
            int c = 0xff000000
                | (c1 == c2
                       ...
                       : ReikaColorAPI.mixColors(
                           ...
                           c2.getColor(tick),
                           (float
                           ) (0.5 + 0.5 * Math.sin(System.currentTimeMillis() / 1500D))
                       ));
        ...
        for (RainPoint p : points) {
            ...
            double u
                = (System.currentTimeMillis() / (int) (50 * p.animationSpeed)) % 64 / 64D;
            double du = u + 1 / 64D;
            ...
            my = Math.max(
                my,
                ep.worldObj.getPrecipitationHeight(
    ...
    private void generateNewPoint(EntityPlayer ep) {
        ...
        RainPoint p = new RainPoint(
            dx,
            dz,
            40 + r.nextInt(440),
            0.0625 + 0.0625 * r.nextDouble(),
            ReikaRandomHelper.getRandomPlusMinus(9, 3),
            0.5F + r.nextFloat() * 1.5F
        );
        points.add(p);
    }
