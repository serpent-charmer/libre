...
public abstract class GeneratedStructureBase extends ColoredStructureBase {
    ...
    private final MultiMap<Block, Coordinate> cache
    ...
    public final void markForWorldgen() {
        if (this.isDisplay())
            throw new IllegalStateException(
                "The structure cannot be both display and worldgen!"
            );
    ...
    public final void runCallbacks(World world, Random rand) {
        ...
        for (Entry<Coordinate, TileCallback> e : callbacks.entrySet()) {
            ...
            try {
                e.getValue().onTilePlaced(
                    world, c.xCoord, c.yCoord, c.zCoord, c.getTileEntity(world)
                );
            } catch (Exception ex) {
                failures.add(
                    new Exception("Threw exception running tile callback @ " + e, ex)
                );
            }
        }
    }
