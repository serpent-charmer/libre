...
public abstract class ChromaDimensionBiomeTerrainShaper {
    ...
    public ChromaDimensionBiomeTerrainShaper(
        long seed, Collection<ChromaDimensionBiomeType> c
    ) {
        dimensionSeed = seed;
        biomeSet.addAll(c);
    }

    public abstract void generateColumn(
        World world,
        int chunkX,
        int chunkZ,
        int i,
    ...
    protected final boolean cutBlock(World world, int x, int y, int z) {
        ...
        if (b != Blocks.air
            && (b == Blocks.bedrock || WorldGenFissure.canCutInto(world, x, y, z))
            && !(b instanceof BlockDimensionDeco)) {
