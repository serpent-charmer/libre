...
public abstract class FragmentStructureBase extends GeneratedStructureBase {
    ...
    addLootChest(FilledBlockArray array, int x, int y, int z, ForgeDirection dir) {
        this.addLootChest(array, x, y, z, getChestMeta(dir));
    }

    protected final void
    addLootChest(FilledBlockArray array, int x, int y, int z, int meta) {
    ...
    public abstract String getChestLootTable(
        Coordinate c, TileEntityLootChest te, FilledBlockArray arr, Random r
    );

    public int
    modifyLootCount(TileEntityLootChest te, String s, int bonus, Random r, int count) {
        return count;
    }
