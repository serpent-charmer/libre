...
public abstract class ChromaDimensionBiome
    extends BiomeGenBase implements CustomMapColorBiome {
    ...
    public final int getMapColor(World world, int x, int z) {
        int color = biomeType != null ? 0xff000000
                | Color.HSBtoRGB(
                    biomeType.ordinal() / (float) Biomes.biomeList.length, 1, 1
                )
                                      : 0xffffffff;
    ...
    public int getBiomeGrassColor(int x, int y, int z) {
        ...
        double rz = z / 8D;
        float f = (float
        ) ReikaMathLibrary.normalizeToBounds(grassColor.getValue(rx, rz), 1, 1.5);
