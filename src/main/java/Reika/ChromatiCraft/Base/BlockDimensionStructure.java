...
public abstract class BlockDimensionStructure extends Block {
    ...
    public final boolean onBlockActivated(
        ...
        if (world.provider.dimensionId != ExtraChromaIDs.DIMID.getValue()
            || TuningThresholds.STRUCTURES.isSufficientlyTuned(ep)) {
    ...
    @Override
    public final void
    onEntityCollidedWithBlock(World world, int x, int y, int z, Entity e) {
        if (world.provider.dimensionId == ExtraChromaIDs.DIMID.getValue()
            && e instanceof EntityPlayer
            && !TuningThresholds.STRUCTURES.isSufficientlyTuned((EntityPlayer) e)) {
        } else {
