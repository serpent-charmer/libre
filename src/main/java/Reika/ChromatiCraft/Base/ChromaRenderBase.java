...
public abstract class ChromaRenderBase
    ...
    renderModel(TileEntity tile, ChromaModelBase model, Object... args) {
        this.preRenderModel();
        if (args.length > 0) {
    ...
    protected final void
    renderModel(TileEntityChromaticBase tile, ChromaModelBase model, Object... args) {
        this.renderModel(
            tile, model, this.getTextureFolder() + this.getImageFileName(tile), args
        );
    }

    protected final void renderModel(
        TileEntityChromaticBase tile, ChromaModelBase model, String tex, Object... args
    ) {
        if (!tile.renderModelsInPass1()
