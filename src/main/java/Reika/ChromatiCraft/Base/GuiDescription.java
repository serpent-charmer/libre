...
public abstract class GuiDescription extends GuiBookSection {
    ...
    protected GuiDescription(
        ChromaGuis g, EntityPlayer ep, ChromaResearch r, int x, int y
    ) {
    ...
    public void drawScreen(int x, int y, float f) {
        ...
        if (flag) {
                ...
                if (li != null) {
                    for (int i = textOffset; i < li.size(); i++) {
                        fontRendererObj.drawString(
                            ...
                            px,
                            posY + descY
                                + (fontRendererObj.FONT_HEIGHT + 2) * (i - textOffset),
                            c
                        );
                        if (i - textOffset > 9)
                            break;
            ...
            if (disable) {
                fontRendererObj.drawSplitString(
                    "This item has been disabled by your server admin or modpack creator.",
                    px,
                    posY + descY,
                    242,
                    0xffffff
    ...
    public final String getPageTitle() {
        if (page == ChromaResearch.ACCEL && subpage > 1)
            ...
            return "Alveary Effect - "
                + TileEntityLumenAlveary.getSortedEffectList()
                      .get(subpage - 2)
                      .getDescription();
        return page.isConfigDisabled() ? page.getTitle() + " (Disabled)"
                                       : super.getPageTitle();
    }
    ...
    protected final void drawEffectDescriptions(
        int posX, int posY, Collection<? extends ItemSpecificEffectDescription> li
    ) {
        int oy = posY + 107;
        int dy = 0;
                    ...
                    if (dx2 >= 0 && dy2 >= oy && dy2 <= oy + 90) {
                        ...
                        if (api.isMouseInBox(dx2, dx2 + 16, dy2, dy2 + 16)) {
                            ...
                            api.drawTooltipAt(
                                fontRendererObj,
                                sg,
                                api.getMouseRealX() + fontRendererObj.getStringWidth(sg)
                                    + 22,
                                api.getMouseRealY() + 15
                            );
                        }
                    }
                    dx += 18;
