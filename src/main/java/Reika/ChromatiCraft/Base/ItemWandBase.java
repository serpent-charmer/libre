...
public abstract class ItemWandBase
    extends ItemChromaTool implements DynamicallyGeneratedSubpage {
    ...
    public final boolean sufficientEnergy(EntityPlayer ep, float f) {
        return ep.capabilities.isCreativeMode
            || PlayerElementBuffer.instance.playerHas(ep, energy.copy().scale(f));
    }
    ...
    public String generateUsageData(boolean color) {
        ...
        for (CrystalElement e : energy.elementSet()) {
            sb.append(
                (color ? e.getChatColorString() : "") + " "
                + e.displayName /*+": "+energy.getValue(e)*/ + "\n"
            );
        }
        return sb.toString();
    }
