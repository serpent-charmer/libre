...
public abstract class ChromaBookGui extends GuiScreen implements CustomSoundGui {
    ...
    protected final boolean shouldDisplayFragment(ChromaResearch r) {
        return ChromaResearchManager.instance.playerHasFragment(player, r)
            && this.isFragmentPresent(r);
    }
    ...
    public void initGui() {
        ...
        //buttonList.add(new GuiButton(12, j+xSize-27, k-4, 20, 20, "X"));	//Close gui
        //button
    }

    protected final void addAuxButton(GuiButton b, String name) {
        buttonList.add(b);
        auxButtons.addRegionByWH(
            b.xPosition - 1, b.yPosition - 1, b.width, b.height, new AuxButton(b, name)
        );
    }
    ...
    public void drawScreen(int x, int y, float f) {
        ...
        if (ab != null) { //api.isMouseInBox(posX+xSize-1, posX+xSize-1+22, posY+13,
                          //posY+13+39)
            //String sg = "Save & Exit";
            int bw = ab.button.width;
            int sp = 4;
            String sg = ab.text;
            int sw = fontRendererObj.getStringWidth(sg
            ); //20+fontRendererObj.getStringWidth(sg)/2;
            ...
            if (api.getMouseRealX() > posX + xSize - 2) {
                ...
                dx = posX + xSize + sw + 20 + bw
                    + sp; //Math.max(posX+xSize+22*0, api.getMouseRealX())+sw+32;
            }
    ...
    goTo(ChromaGuis next, ChromaResearch to, EntityPlayer player) {
