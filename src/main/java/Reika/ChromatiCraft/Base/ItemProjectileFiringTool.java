...
public abstract class ItemProjectileFiringTool
    extends ItemChromaTool implements ProjectileFiringTool {
    ...
    public final void
    fire(ItemStack is, World world, EntityPlayer ep, boolean randomVec) {
        Entity e = this.createProjectile(is, world, ep, randomVec);
        Vec3 vec = ep.getLookVec();
        e.setLocationAndAngles(
            ep.posX + vec.xCoord, ep.posY + vec.yCoord + 1.5, ep.posZ + vec.zCoord, 0, 0
        );
    ...
    createProjectile(ItemStack is, World world, EntityPlayer ep, boolean randomVec);

    public static abstract class ProgressGatedProjectileFiringTool
        extends ItemProgressGatedTool implements ProjectileFiringTool {
        ...
        onItemRightClick(ItemStack is, World world, EntityPlayer ep) {
            if (!world.isRemote) {
