...
public abstract class BlockChromaTiered
    extends BlockTieredResource implements IWailaDataProvider {
    ...
    getWailaStack(IWailaDataAccessor acc, IWailaConfigHandler cfg) {
        ...
        return this.isClientSufficient(acc.getWorld(), mov.blockX, mov.blockY, mov.blockZ)
            ? null
            : this.getWailaDisguise(acc.getMetadata());
    }

    @SideOnly(Side.CLIENT)
    private boolean isClientSufficient(World world, int x, int y, int z) {
        return this.isPlayerSufficientTier(
