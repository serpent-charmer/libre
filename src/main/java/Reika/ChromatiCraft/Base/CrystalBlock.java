...
public abstract class CrystalBlock extends CrystalTypeBlock
    implements CrystalRenderedBlock, IInfusionStabiliser, SemiUnbreakable, Submergeable {
    ...
    public final float getEnchantPowerBonus(World world, int x, int y, int z) {
        return this != ChromaBlocks.LAMP.getBlockInstance()
                && world.getBlockMetadata(x, y, z) == CrystalElement.PURPLE.ordinal()
            ? (this == ChromaBlocks.SUPER.getBlockInstance() ? 1.5F : 1)
            : 0;
    }
    ...
    getPlayerRelativeBlockHardness(EntityPlayer ep, World world, int x, int y, int z) {
        return this.isUnbreakable(world, x, y, z, world.getBlockMetadata(x, y, z))
            ? -1
            : super.getPlayerRelativeBlockHardness(ep, world, x, y, z);
    }
    ...
    public final void onNeighborBlockChange(World world, int x, int y, int z, Block b) {
        if (world.isBlockIndirectlyGettingPowered(x, y, z)) {
            ...
            ding(
                ...
                z,
                e,
                (float) getDingPitchFromRedstone(e, world.getBlockPowerInput(x, y, z))
            );
        }
    }
        ...
        if (this.shouldGiveEffects(e) && this.performEffect(e)) {
            if (rand.nextInt(3) == 0)
                ReikaPacketHelper.sendUpdatePacket(
                    ...
                    ChromaPackets.CRYSTALEFFECT.ordinal(),
                    x,
                    y,
                    z,
                    PacketTarget.server
    ...
    @SideOnly(Side.CLIENT)
    private void
    doParticles(World world, int x, int y, int z, CrystalElement e, Random rand) {
        EntityFX fx;
        if (rand.nextInt(20) > 0) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                fx = new EntityCCBlurFX(e, world, rrx, rry, rrz, 0, 0, 0)
                         .setIcon(ChromaIcons.SPARKLEPARTICLE)
                         .setLife(l)
                         .setScale(s2)
                         .enableAlphaTest();
                if (rand.nextBoolean())
            ...
            fx = new EntityCCFloatingSeedsFX(
                     world, rx, y + 0.5, rz, 0, 90, ChromaIcons.CENTER
            )
                     .setColor(e.getColor())
                     .setScale(4)
                     .setLife(120)
    ...
    public final void updateEffects(World world, int x, int y, int z) {
        if (!world.isRemote) {
            CrystalElement color
                = CrystalElement.elements[world.getBlockMetadata(x, y, z)];
    ...
    public final void applyEffect(
        CrystalElement color, List<EntityLivingBase> li, int x, int y, int z, int r
    ) {
        ...
        for (EntityLivingBase e : li) {
            ...
            if (ReikaMathLibrary.py3d(
                    e.posX - x - 0.5,
                    e.posY + e.getEyeHeight() / 2F - y - 0.5,
                    e.posZ - z - 0.5
                )
                <= r) {
                int dura2 = dura;
    ...
    public final int getTintColor(int meta) {
        int c0 = ReikaColorAPI.getModifiedSat(
            CrystalElement.elements[meta].getColor(), 0.65F
        );
