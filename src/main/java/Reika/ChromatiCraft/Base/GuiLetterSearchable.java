...
public abstract class GuiLetterSearchable<E>
    extends GuiChromaBase implements KeyConsumingGui {
    ...
    public void initGui() {
        ...
        buttonList.add(new CustomSoundImagedGuiButton(
                           -600,
                           j + this.getSearchButtonX(),
                           k + this.getSearchButtonY(),
                           10,
                           10,
                           70,
                           66,
    ...
    protected final int decrIndex(Function<E, Boolean> isValidToStopAt) {
        ...
        if (index > 0) {
            do {
                ...
                index--;
                at = this.getActive();
            } while (index > 0 && at != null && isValidToStopAt != null
                     && !isValidToStopAt.apply(at));
        }
        return amt;
    }
    ...
    protected final int incrIndex(Function<E, Boolean> isValidToStopAt) {
        ...
        if (index < this.filteredList.size() - 1) {
            do {
                ...
                index++;
                at = this.getActive();
                //ReikaJavaLibrary.pConsole("Stepped to "+at+" idx ="+index+":
                //"+(isValidToStopAt != null && !isValidToStopAt.apply(at)));
            } while (index < filteredList.size() - 1 && at != null
                     && isValidToStopAt != null && !isValidToStopAt.apply(at));
        }
        return amt;
    }
    ...
    private boolean handleKey(char c, int idx) {
        ...
        if (!this.searchActive) {
            ...
            this.filterString
                = this.filterString.substring(0, this.filterString.length() - 1);
    ...
    private void updateFilter() {
        ...
        if (!Strings.isNullOrEmpty(filterString)) {
            ...
            start.removeIf(
                e
                -> !this.getString(e).toLowerCase(Locale.ENGLISH).startsWith(filterString)
            );
            contains.removeIf(
                e -> !this.getString(e).toLowerCase(Locale.ENGLISH).contains(filterString)
            );
    ...
    protected void drawSearch() {
            ...
            for (int i = 0; i < li.size(); i++) {
                ...
                if (i == li.size() - 1)
                    c = ReikaColorAPI.mixColors(
                        c,
                        0x00ff00,
                        0.5F + 0.5F * MathHelper.sin(player.ticksExisted / 5F)
                    );
                this.drawString(
                    fontRendererObj,
                    sg,
                    (xSize - fontRendererObj.getStringWidth(sg) / 2 - 6) * 2,
                    (4 + (fontRendererObj.FONT_HEIGHT + 1) * i) * 2,
                    c
                );
            }
            GL11.glPopMatrix();
        ...
        if (this.isSearchActive() && this.searchActive) {
            ...
            ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                ...
                (int) (xSize / (2 * s)),
                (int) (ySize / (2 * s)),
                this.noResultsFound ? 0xffa0a0 : 0xa0ffa0
            );
            GL11.glPopMatrix();
        } else {
