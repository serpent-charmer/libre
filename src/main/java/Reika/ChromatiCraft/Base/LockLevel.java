...
public abstract class LockLevel
    extends StructurePiece<LocksGenerator> implements Comparable<LockLevel> {
    ...
    protected final void generateWhiteRune(int x, int y, int z) {
        currentGenerator.setBlock(
            x, y, z, ChromaBlocks.RUNE.getBlockInstance(), CrystalElement.WHITE.ordinal()
        );
    }

    protected final void generateRune(int x, int y, int z, LockColor color) {
        currentGenerator.setBlock(
            x, y, z, ChromaBlocks.RUNE.getBlockInstance(), shuffleMap.get(color).ordinal()
        );
    }

    protected final void generateKey(int x, int y, int z) {
        currentGenerator.setTileEntity(
            x,
            y,
            z,
            ChromaBlocks.LOCKKEY.getBlockInstance(),
            level.ordinal(),
            new UUIDPlace(parent.id)
        );
    }
    ...
    protected final void generateTimer(int x, int y, int z) {
        currentGenerator.setTileEntity(
            ...
            y,
            z,
            ChromaBlocks.LOCKFREEZE.getBlockInstance(),
            level.ordinal(),
            new UUIDPlace(parent.id)
        );
    }
    ...
    protected final void generateLock(int x, int y, int z, LockColor... colors) {
        ...
        currentGenerator.setTileEntity(
            ...
            0,
            new LockColorSet(parent, level.ordinal(), parent.id, elements)
        );
    }
    ...
    protected final void generateGate(int x, int y, int z) {
        currentGenerator.setTileEntity(
            ...
            ChromaBlocks.COLORLOCK.getBlockInstance(),
            1,
            new LockColorSet(parent, level.ordinal(), parent.id)
        );
    }
    ...
    public final void permute(Random rand) {
        shuffleMap.clear();
        ArrayList<CrystalElement> set
            = ReikaJavaLibrary.makeListFrom(CrystalElement.elements);
