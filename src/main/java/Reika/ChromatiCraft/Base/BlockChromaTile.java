...
public class BlockChromaTile extends BlockTileEnum<TileEntityChromaticBase, ChromaTiles>
    ...
    @Override
    public boolean
    isSideSolid(IBlockAccess world, int x, int y, int z, ForgeDirection dir) {
        ...
        if (c == null) {
            ...
            if (get == null || time - get.longValue() > 2000) {
                ChromatiCraft.logger.logError(
                    "No tile at location " + loc + " [" + loc.getBlock(world) + ":"
                    + loc.getBlockMetadata(world) + "] !?"
                );
    ...
    public float getExplosionResistance(
        ...
        if (t == ChromaTiles.TABLE || t == ChromaTiles.DATANODE
            || t == ChromaTiles.EXPLOSIONSHIELD)
    ...
    public final TileEntityChromaticBase createTileEntity(World world, int meta) {
        return (TileEntityChromaticBase
        ) ChromaTiles.createTEFromIDAndMetadata(this, meta);
    }
    ...
    public IIcon getIcon(int s, int meta) {
        ...
        if (GuiMachineDescription.runningRender
            && ChromaTiles.getTileFromIDandMetadata(this, meta) == ChromaTiles.INJECTOR)
            idx = 1;
    ...
    public IIcon getIcon(IBlockAccess iba, int x, int y, int z, int s) {
        ...
        return icons[meta][s]
                    [te instanceof VariableTexture
                         ? ((VariableTexture) te).getIconState(s)
                         : 0];
    }
    ...
    public int getLightValue(IBlockAccess world, int x, int y, int z) {
        ...
        if (c == ChromaTiles.TANK) {
            TileEntityCrystalTank te
                = (TileEntityCrystalTank) world.getTileEntity(x, y, z);
            return te.getCurrentFluid() != null ? te.getCurrentFluid().getLuminosity()
                                                : 0;
        }
        ...
        for (int i = 0; i < tiles.size(); i++) {
            ...
            if (c.hasBlockRender()) {
                for (int k = 0; k < 6; k++) {
                    ...
                    icons[c.getBlockMetadata()][k][0]
                        = ico.registerIcon("chromaticraft:tile/" + path);
                    if (c.hasTextureVariants()) {
                        icons[c.getBlockMetadata()][k][1]
                            = ico.registerIcon("chromaticraft:tile/" + path + "_variant");
                    }
                }
            }
        }
    ...
    public void onBlockExploded(World world, int x, int y, int z, Explosion e) {
        ...
        if (t == ChromaTiles.LANDMARK) {
            ReikaItemHelper.dropItem(
                world,
                x + par5Random.nextDouble(),
                y + par5Random.nextDouble(),
                z + par5Random.nextDouble(),
                t.getCraftedProduct()
            );
        }
    ...
    public int isProvidingStrongPower(IBlockAccess world, int x, int y, int z, int s) {
        ChromaTiles c = ChromaTiles.getTile(world, x, y, z);
        TileEntityChromaticBase te
            = (TileEntityChromaticBase) world.getTileEntity(x, y, z);
        if (c.suppliesRedstone()) {
            return ((RedstoneTile) te)
                .getStrongPower(
                    world, x, y, z, ForgeDirection.VALID_DIRECTIONS[s].getOpposite()
                );
        }
        return 0;
    }
    ...
    public final boolean onBlockActivated(
        ...
        if (ReikaRandomHelper.doWithChance(2))
            ChromaAux.spawnInteractionBallLightning(
                world, x, y, z, CrystalElement.randomElement()
            );
        ...
        if (is != null && ChromaItems.isRegistered(is)
            && ChromaItems.getEntry(is).overridesRightClick(is)) {
            return false;
        }
        if (is != null
            && ReikaItemHelper.matchStackWithBlock(
                is, ChromaBlocks.ROUTERNODE.getBlockInstance()
            ))
            return false;
        if (is != null && ModList.THAUMCRAFT.isLoaded()
            && is.getItem() == ThaumItemHelper.ItemEntry.WAND.getItem().getItem()
            && ReikaThaumHelper.getWandFocus(is)
                == ChromaItems.MANIPFOCUS.getItemInstance())
            return false;
        ...
        if (m == ChromaTiles.TABLE && is != null && ModList.BOTANIA.isLoaded()
            && is.getItem() == BotaniaHandler.getInstance().wandID) {
            if (world.isRemote) {
                int[] colors = BotaniaHandler.getInstance().getWandColors(is);
                ReikaDyeHelper dye1
                    = ReikaDyeHelper
                          .dyes[15 - colors[0]]; //compensate for reversed color order
        ...
        if (ChromaItems.BUCKET.matchWith(is) && is.getItemDamage() == 0
            && is.stackSize == 1 && te instanceof ChromaPowered) {
        ...
        if (is != null && is.getItem() == Items.bucket && is.stackSize == 1
            && te instanceof ChromaExtractable) {
        ...
        if (ModList.THAUMCRAFT.isLoaded() && te instanceof TileEntityAspectJar
            && is != null) {
            ...
            if (is.getItem() instanceof IEssentiaContainerItem && is.stackSize == 1) {
                ...
                if (al != null && al.size() > 0) {
                    ...
                    ReikaSoundHelper.playSoundAtBlock(
                        ...
                        "game.neutral.swim",
                        0.6F,
                        (float) ReikaRandomHelper.getRandomPlusMinus(1, 1F)
                    );
                    if (!ep.capabilities.isCreativeMode) {
                        al.remove(a, added);
        ...
        if (m == ChromaTiles.MUSIC && is != null
            && is.getItem() instanceof MusicDataItem) {
        ...
        if (ChromaItems.SHARD.matchWith(is) && is.getItemDamage() >= 16
            && m == ChromaTiles.LAMP) {
            if (((TileEntityChromaLamp) te)
                    .addColor(CrystalElement.elements[is.getItemDamage() % 16])) {
                if (!ep.capabilities.isCreativeMode)
                    is.stackSize--;
            }
        }
        ...
        if (m == ChromaTiles.PARTICLES && is != null) {
            ...
            if (is.getItem() == Items.book) {
                if (is.stackTagCompound != null
                    ...
                    tp.readCopyableData(
                        is.stackTagCompound.getCompoundTag("particleprogram")
                    );
                    world.markBlockForUpdate(x, y, z);
        ...
        /*
        if (ChromaItems.SHARD.matchWith(is) && (m == ChromaTiles.WEAKREPEATER || m ==
        ChromaTiles.SKYPEATER)) { CrystalRepeater tw = (CrystalRepeater)te; if
        (!world.isRemote) { CrystalElement e =
        CrystalElement.elements[is.getItemDamage()%16]; if
        (CrystalNetworker.instance.checkConnectivity(e, tw)) {
                    ChromaSounds.CAST.playSoundAtBlock(world, x, y, z);
                    int rd = e.getRed();
                    int gn = e.getGreen();
                    int bl = e.getBlue();
                    ReikaPacketHelper.sendDataPacketWithRadius(DragonAPIInit.packetChannel,
        PacketIDs.COLOREDPARTICLE.ordinal(), te, 64, rd, gn, bl, 32, 8);
                    //shows -1?
        ReikaPacketHelper.sendDataPacketWithRadius(DragonAPIInit.packetChannel,
        PacketIDs.NUMBERPARTICLE.ordinal(), te, 64, tw.getSignalDepth(e));
                }
                else {
        ...
        /*
        if (m == ChromaTiles.TURRET && ChromaItems.SHARD.matchWith(is) &&
        is.getItemDamage() < 16) { TileEntityLumenTurret tile = (TileEntityLumenTurret)te;
            boolean flag = false;
            if (!tile.hasPassiveUpgrade && is.getItemDamage() ==
        CrystalElement.GREEN.ordinal()) { tile.hasPassiveUpgrade = true; flag = true;
            }
            if (!tile.hasPlayerUpgrade && is.getItemDamage() ==
        CrystalElement.LIGHTGRAY.ordinal()) { tile.hasPlayerUpgrade = true; flag = true;
            }
            if (flag) {
        ...
        if (ChromaItems.LENS.matchWith(is) && is.stackSize == 1
            && te instanceof TileEntityCrystalLaser) {
        ...
        if (m != ChromaTiles.TELEPORT) {
            if (te != null && ChromaAux.hasGui(world, x, y, z, ep)
                && ((TileEntityBase) te).isPlayerAccessible(ep)) {
                ep.openGui(
                    ChromatiCraft.instance, ChromaGuis.TILE.ordinal(), world, x, y, z
                );
                return true;
            }
        }
...
if (core != null && m.isEnchantable()) {
  ...
  ReikaEnchantmentHelper.applyEnchantments(core, ench);
    ...
    canSilkHarvest(World world, EntityPlayer player, int x, int y, int z, int metadata) {
        return false;
    }
    ...
    @Override
    public void
    harvestBlock(World world, EntityPlayer ep, int x, int y, int z, int meta) {
        ...
        if (m != null) {
            ...
            ((EnchantableMachine)te).getEnchantments();
            ...
            if (is != null && m.hasNBTVariants()) {
                ...
                is.stackTagCompound
                    = (NBTTagCompound) (!nbt.hasNoTags() ? nbt.copy() : null);
            }
            ...
            if (m == ChromaTiles.PYLONTURBO
                && ((TileEntityChromaticBase) te)
                       .getOwners(false)
                       .isEmpty()) //fix for ones in accidental worldgen
                is = null;
            if (is != null) {
                li = ReikaJavaLibrary.makeListFrom(is);
            }
            if (m == ChromaTiles.ASPECTJAR
                && ((TileEntityAspectJar) te).hasDirectDrainUpgrade())
    ...
    public final void breakBlock(World world, int x, int y, int z, Block par5, int par6) {
        ...
        if (te instanceof TileEntityItemCollector) {
            ...
            ReikaWorldHelper.splitAndSpawnXP(
                ...
                y + 0.5,
                z + 0.5,
                ((TileEntityItemCollector) te).getExperience()
            );
        }
    ...
    public float getEnchantPowerBonus(World world, int x, int y, int z) {
        ...
        if (c == ChromaTiles.FUNCTIONRELAY) {
            return ((TileEntityFunctionRelay) world.getTileEntity(x, y, z))
                .getEnchantPowerInRange(world, x, y, z);
        }
    ...
    getWailaStack(IWailaDataAccessor accessor, IWailaConfigHandler config) {
        return null; //ChromaTiles.getTileFromIDandMetadata(this,
                     //accessor.getMetadata()).getCraftedProduct();
    }
    ...
    public List<String> getWailaHead(
        ...
        if (mov != null) {
            ...
            int y = mov.blockY;
            int z = mov.blockZ;
            currenttip.add(EnumChatFormatting.WHITE+this.getPickBlock(mov, world, x, y,
        z).getDisplayName());
        }*/
        return currenttip;
    }
    ...
    public List<String> getWailaBody(
        ...
        if (te.getTile()
            != ChromaTiles.MUSIC) //to prevent lag, and has nothing to sync anywyas
        ...
        if (te instanceof TileEntityAccelerator) {
            currenttip.add(String.format(
                "Time Acceleration Factor: %dx",
                1 + ((TileEntityAccelerator) te).getAccel()
            ));
        }
        ...
        if (te instanceof TileEntityCrystalTank) {
            ...
            if (amt > 0 && f != null) {
                currenttip.add(String.format(
                    "Tank: %dmB/%dmB of %s", amt, capacity, f.getLocalizedName()
                ));
            } else {
            ...
            String input = in != null
                ? String.format(
                    "%d/%d mB of %s", amtin, liq.getCapacity(), in.getLocalizedName()
                )
                : "Empty";
            String output = out != null
                ? String.format(
                    "%d/%d mB of %s", amtout, liq.getCapacity(), out.getLocalizedName()
                )
                : "Empty";
            ...
            String input = in != null
                ? String.format(
                    "%d/%d mB of %s", amt, liq.getCapacity(), in.getLocalizedName()
                )
                : "Empty";
            ...
            if (tanks != null) {
                for (int i = 0; i < tanks.length; i++) {
                    ...
                    String input = fs != null ? String.format(
                                       "%d/%d mB of %s",
                                       fs.amount,
                                       info.capacity,
                                       fs.getFluid().getLocalizedName(fs)
                                   )
                                              : "Empty";
        ...
        if (te instanceof LumenTile) {
            ...
            for (CrystalElement e : tag.elementSet()) {
                ...
                currenttip.add(String.format("   %s%s: %d/%d", e.getChatColorString(),
        e.displayName, amt, max));
                //sb.append(String.format("%s%.0f%s%s%s", e.getChatColorString(),
        ReikaMathLibrary.getThousandBase(amt), ReikaEngLibrary.getSIPrefix(amt),
        ...
        if (te.getMachine().isEnchantable()) {
            if (((EnchantableMachine)te).hasEnchantments()) {
                ...
                ArrayList<Enchantment> li =
        ((EnchantableMachine)te).getValidEnchantments(); for (int i = 0; i < li.size();
        i++) { Enchantment e = li.get(i); int level =
        ((EnchantableMachine)te).getEnchantment(e); if (level > 0) currenttip.add("
        "+EnumChatFormatting.LIGHT_PURPLE.toString()+e.getTranslatedName(level));
                }
            }
        }*/
