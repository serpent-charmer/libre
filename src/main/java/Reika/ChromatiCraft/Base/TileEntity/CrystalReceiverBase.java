...
public abstract class CrystalReceiverBase extends TileEntityCrystalBase
    implements CrystalReceiver, LumenConsumer, LumenRequestingTile {
    ...
    protected final boolean requestEnergy(CrystalElement e, int amount) {
        ...
        if (amt > 0) {
            flag = CrystalNetworker.instance.makeRequest(
                this, e, amount, this.getReceiveRange()
            );
        }
        if (flag) {
    ...
    requestEnergyDifference(ElementTagCompound tag, boolean requireAll) {
        tag.subtract(energy);
