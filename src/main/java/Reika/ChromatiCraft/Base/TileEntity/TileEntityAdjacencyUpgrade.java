...
public abstract class TileEntityAdjacencyUpgrade
    extends TileEntityWirelessPowered implements NBTTile, SneakPop {
    public static final int MAX_TIER = 8;

    private static final NestedMap<CrystalElement, String, AdjacencyEffectDescription>
        effectMap = new NestedMap();
    private static final EnumMap<CrystalElement, AdjacencyCheckHandlerImpl>
        adjacencyChecks = new EnumMap(CrystalElement.class);
    ...
    public final int getConsumedEnergy() {
        return ChromaOptions.POWEREDACCEL.getState()
            ? Math.max(1, ReikaMathLibrary.intpow2(tier + 1, 2) / 4)
            : 0;
    }
        ...
        if (ChromaOptions.POWEREDACCEL.getState()) {
            ...
            if (this.getTicksExisted() % 8 == 0) {
                if (energy.getValue(e) < this.getMaxStorage(e))
                    this.requestEnergy(
                        this.getColor(), this.getMaxStorage(e) - energy.getValue(e)
                    );
            }
    ...
    private void
    spawnActionParticles(World world, int x, int y, int z, ForgeDirection dir) {
        double o = 0.0625;
        ...
        fx.setRapidExpand()
            .setAlphaFading()
            .setLife(ReikaRandomHelper.getRandomBetween(8, 40))
    ...
    private void spawnParticles(World world, int x, int y, int z) {
        ...
        if (rand.nextInt(1 + p2) == 0) {
            int p = this.getTier() > 0 ? (this.getTier() + 1) / 2
                : rand.nextBoolean()   ? 1
                                       : 0;
            for (int i = 0; i < p; i++) {
                ...
                double vy = v * (dy - 0.5);
                double vz = v * (dz - 0.5);
                //ReikaParticleHelper.FLAME.spawnAt(world, x-0.5+dx*2, y-0.5+dy*2,
                //z-0.5+dz*2, v*(-1+dx*2), v*(-1+dy*2), v*(-1+dz*2));
                //Minecraft.getMinecraft().effectRenderer.addEffect(new
                //EntitySparkleFX(world, x+0.5, y+0.5, z+0.5, vx, vy, vz));
                //Minecraft.getMinecraft().effectRenderer.addEffect(new
                //EntitySparkleFX(world, x+0.5, y+0.5, z+0.5, vx, vy, vz));
                ...
                Minecraft.getMinecraft().effectRenderer.addEffect(
                    new EntitySparkleFX(world, dx + 0.5, dy + 0.5, dz + 0.5, vx, vy, vz)
                );
            }
        }
    }

    protected abstract EffectResult
    tickDirection(World world, int x, int y, int z, ForgeDirection dir, long startTime);
    ...
    getAdjacentUpgrade(World world, int x, int y, int z, CrystalElement color) {
        Integer ret = getAdjacentUpgrades(world, x, y, z).get(color);
        return ret != null ? ret.intValue() : 0;
    }

    protected static HashMap<CrystalElement, Integer>
    getAdjacentUpgrades(TileEntityBase core) {
        HashMap<CrystalElement, Integer> set = new HashMap();
        for (int i = 0; i < 6; i++) {
            TileEntity te
                = core.getAdjacentTileEntity(ForgeDirection.VALID_DIRECTIONS[i]);
            ...
            if (te instanceof TileEntityAdjacencyUpgrade) {
                ...
                if (ta.canRun(ta.worldObj, ta.xCoord, ta.yCoord, ta.zCoord)
                    && (!ChromaOptions.POWEREDACCEL.getState()
                        || ta.energy.containsAtLeast(ta.getColor(), 100))) {
    ...
    protected static HashMap<CrystalElement, Integer>
    getAdjacentUpgrades(World world, int x, int y, int z) {
        HashMap<CrystalElement, Integer> set = new HashMap();
        for (int i = 0; i < 6; i++) {
    ...
    public static AdjacencyCheckHandler createAdjacencyCheckHandler(
        CrystalElementProxy color, String desc, ItemStack... items
    ) {
        return getOrCreateAdjacencyCheckHandler(
            CrystalElement.elements[color.ordinal()], desc, items
        );
    }

    public static AdjacencyCheckHandlerImpl
    getOrCreateAdjacencyCheckHandler(CrystalElement color, String desc, TileEnum te) {
        return getOrCreateAdjacencyCheckHandler(color, desc, te.getCraftedProduct());
    }

    public static AdjacencyCheckHandlerImpl getOrCreateAdjacencyCheckHandler(
        CrystalElement color, String desc, ItemStack... items
    ) {
    ...
    public static AdjacencyEffectDescription
    registerEffectDescription(CrystalElement color, String desc) {
        return registerEffectDescription(color, desc, 0);
    }

    public static AdjacencyEffectDescription
    registerEffectDescription(CrystalElement color, String desc, int index) {
        AdjacencyEffectDescription ef = effectMap.get(color, desc);
        if (ef == null) {
            ef = (AdjacencyEffectDescription) new AdjacencyEffectDescription(color, desc)
                     .setOrderIndex(index);
    ...
    public static final class AdjacencyCheckHandlerImpl implements AdjacencyCheckHandler {
        ...
        public int getAdjacentUpgradeTier(World world, int x, int y, int z) {
            Integer get = TileEntityAdjacencyUpgrade.getAdjacentUpgrades(world, x, y, z)
                              .get(color);
        ...
        public double getFactorSimple(World world, int x, int y, int z) {
            ...
            return tier > 0 ? ChromatiAPI.getAPI().adjacency().getFactor(
                       CrystalElementAccessor.getByIndex(color.ordinal()), tier
                   )
                            : 1;
        }
    ...
    public static final class AdjacencyEffectDescription
        extends ItemListEffectDescription {
        public final CrystalElement color;
    ...
    public static Collection<AdjacencyEffectDescription>
    getSpecificEffects(CrystalElement e) {
