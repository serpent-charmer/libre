...
@Strippable(
    value = { "appeng.api.networking.security.IActionHost",
              "appeng.api.storage.ICellContainer",
              "appeng.api.storage.IMEInventoryHandler" }
)
public abstract class TileEntityMassStorage extends TileEntityChromaticBase
    implements IInventory, IActionHost /*, ICellContainer, IMEInventoryHandler*/,
               BreakAction, ConditionBreakDropsInventory {
    ...
    public final ItemStack getStackInSlot(int slot) {
        ...
        if (slot > li.size()) {
            ChromatiCraft.logger.logError(
                "Something tried pulling from an off-list slot #" + slot + "!"
            );
            Thread.dumpStack();
            return null;
        }
    ...
    public final void setInventorySlotContents(int slot, ItemStack is) {
        if (slot == 0) {
            ...
            throw new IllegalArgumentException(
                "Something tried to insert into an invalid slot!"
            );
        }
    }
        ...
        return slot == 0
            && (pendingInput == null
                || ReikaItemHelper.areStacksCombinable(
                    pendingInput, is, is.getMaxStackSize()
                ))
            && this.isItemValid(is) && this.getItems().size() < this.maxItemCount();
    }
    ...
    public final void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (ModList.APPENG.isLoaded()) {
            ...
            if (!world.isRemote && network != null && !MEStacks.isEmpty()
                && pendingInput == null) {
                hasWork.tick();
                if (hasWork.hasWork()) {
        ...
        if (ModList.APPENG.isLoaded()) {
            ...
            if (network != null && this.canPullFromME()) {
                ...
                Collection<ItemStack> li
                    = new ArrayList(); //network.getRawMESystemContents();

                IStorageGrid isg = (IStorageGrid) this.getActionableNode()
                                       .getGrid()
                ...
                if (set != null) {
                    for (ICellProvider icp : set) {
                        if (this.isStorageBusToSelf(icp))
                            continue;
                        List<IMEInventoryHandler> invs
                            = icp.getCellArray(StorageChannel.ITEMS);
    ...
    private final boolean isStorageBusToSelf(ICellProvider icp) {
        try {
            if (icp.getClass() == storageBusClass) {
                ...
                TileEntity tile = block.getTile().worldObj.getTileEntity(
                    loc.x + dir.offsetX, loc.y + dir.offsetY, loc.z + dir.offsetZ
                );
                return tile == this;
            }
            return false;
    ...
    protected void readSyncTag(NBTTagCompound NBT) {
        ...
        if (NBT.hasKey("types")) {
            types.readFromNBT(
                NBT.getCompoundTag("types"), KeyedItemStackConverter.instance
            );
        }
    }
    ...
    public final boolean canAccept(IAEStack input) {
        return input.isMeaningful() && input.isItem() &&
    this.isItemValid(((IAEItemStack)input).getItemStack());
    }
    ...
    @Override
    @ModDependent(ModList.APPENG)
    public final IAEStack injectItems(IAEStack input, Actionable type, BaseActionSource
    src) { FIX ME this disobeys maxItemCount if (!input.isItem()) return input; ItemStack
    ref = ((IAEItemStack)input).getItemStack(); if (ref == null || ref.getItem() == null)
            return input;
        ...
        long added = 0;
        while (amt > 0) {
            ItemStack put = ReikaItemHelper.getSizedItemStack(ref,
    Math.min((int)Math.min(Integer.MAX_VALUE, amt), ref.getMaxStackSize()));
            this.addItem(put);
            amt -= put.stackSize;
            added += put.stackSize;
        }
    ...
    @Override
    @ModDependent(ModList.APPENG)
    public final IAEStack extractItems(IAEStack request, Actionable mode, BaseActionSource
    src) { if (!request.isItem()) return null; ItemStack seek =
    ((IAEItemStack)request).getItemStack(); int amt = this.countExactItem(seek); if (amt
    == 0) return null; long get = Math.min(request.getStackSize(), amt); if (mode ==
    Actionable.MODULATE) { long take = get; Iterator<ItemStack> it =
    this.getItems().iterator(); while (it.hasNext()) { ItemStack at = it.next(); if
    (ReikaItemHelper.matchStacks(at, seek) && ItemStack.areItemStackTagsEqual(seek, at)) {
    ...
    private int countExactItem(ItemStack seek) {
        int amt = 0;
        for (ItemStack is : this.getItems()) {
            if (ReikaItemHelper.matchStacks(is, seek) &&
    ItemStack.areItemStackTagsEqual(is, seek)) { amt += is.stackSize;
            }
        }
        return amt;
    }
    ...
    public final void breakBlock() {
        ReikaItemHelper.dropItems(
            worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, this.getItems()
        );
    ...
    public static class WorldToolCrateData extends WorldSavedData {
        ...
        private static WorldToolCrateData initItemData(World world) {
            WorldToolCrateData data = (WorldToolCrateData
            ) world.loadItemData(WorldToolCrateData.class, IDENTIFIER);
            if (data == null) {
