...
public abstract class ChargedCrystalPowered
    extends InventoriedChromaticBase implements LumenConsumer {
    ...
    protected final int getStoredEnergy(boolean onlyUsedColors) {
        if (ChromaItems.STORAGE.matchWith(inv[0])) {
            ...
            ElementTagCompound tag
                = ((ItemStorageCrystal) is.getItem()).getStoredTags(is);
            if (onlyUsedColors) {
                for (int i = 0; i < 16; i++) {
    ...
    public final boolean canExtractItem(int slot, ItemStack is, int side) {
        return slot == 0 ? this.getStoredEnergy(true) == 0
                         : this.canExtractOtherItem(slot, is, side);
    }

    public final boolean isItemValidForSlot(int slot, ItemStack is) {
        return slot == 0 ? ChromaItems.STORAGE.matchWith(is)
                         : this.isItemValidForOtherSlot(slot, is);
    }
