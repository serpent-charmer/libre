...
public abstract class TileEntityRelayPowered
    ...
    private final boolean requestEnergy(CrystalElement e, int amt, ForgeDirection dir) {
        TileEntityRelaySource te = RelayNetworker.instance.findRelaySource(
            worldObj, xCoord, yCoord, zCoord, dir, e, amt, 128
        );
        if (te != null) {
            int has = te.getEnergy(e);
            int trans = Math.min(
                Math.min(amt, has), this.getMaxStorage(e) - energy.getValue(e)
            );
