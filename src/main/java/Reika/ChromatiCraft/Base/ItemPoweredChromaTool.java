...
public abstract class ItemPoweredChromaTool
    extends ItemChromaTool implements PoweredItem {
    ...
    doTick(ItemStack is, World world, EntityPlayer e, boolean held);
    ...
    public void addInformation(ItemStack is, EntityPlayer ep, List li, boolean vb) {
        li.add(String.format(
            "Energy: %.2f%s",
            100F * ToolChargingSystem.instance.getCharge(is) / this.getMaxCharge(),
            "%"
        ));
    }
    ...
    @Override
    public final void
    onUpdate(ItemStack is, World world, Entity e, int slot, boolean held) {
        if (e instanceof EntityPlayer) {
            ...
            if (this.isActivated(ep, is, held)) {
                if (ToolChargingSystem.instance.getCharge(is) > 0) {
                    if (this.doTick(is, world, ep, held)
                        && !(ep).capabilities.isCreativeMode) {
                        ToolChargingSystem.instance.removeCharge(
                            is, this.getChargeConsumptionRate(ep, world, is), ep
                        );
                    }
                }
            }
    ...
    public static void doStandardChargeFX(EntityItem ei, int charge) {
        ...
        if (charge == pi.getMaxCharge()) {
            ...
            for (int i = 0; i < 360; i += 60) {
                ...
                EntityBlurFX fx
                    = new EntityCCBlurFX(
                          ei.worldObj, ei.posX, (int) ei.posY + 0.25, ei.posZ, vx, 0, vz
                    )
                          .setLife(40)
                          .setRapidExpand()
                          .setScale(1.5F);
            ...
            double[] vel = ReikaPhysicsHelper.polarToCartesian(
                v, itemRand.nextDouble() * 360, itemRand.nextDouble() * 360
            );
    ...
    public String getNotes(int subpage) {
        return "Requires charging with " + this.getColor(new ItemStack(this)).displayName
            + " ambient energy to be used";
    }
