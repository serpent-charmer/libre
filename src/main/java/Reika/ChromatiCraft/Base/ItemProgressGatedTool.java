...
public abstract class ItemProgressGatedTool extends ItemChromaTool {
    ...
    public final UseResult canPlayerUse(EntityPlayer ep) {
        ...
        if (!ChromaResearchManager.instance.playerHasFragment(
                ep, ChromaResearch.getPageFor(ep.getCurrentEquippedItem())
            ))
            return consequence;
        /*}
        else { //no need to check if the fragment is checked, since cannot have fragment
        w/o progress for (ProgressStage p : progress) if (!p.isPlayerAtStage(ep)) return
        consequence;
        }*/
