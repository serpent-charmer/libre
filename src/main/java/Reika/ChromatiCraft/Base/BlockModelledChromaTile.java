...
public class BlockModelledChromaTile extends BlockChromaTile {
    ...
    getSelectedBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        box = box != null
            ? box
            : ReikaAABBHelper.getBlockAABB(x, y, z).contract(0.375, 0.375, 0.375);
        this.setBounds(box, x, y, z);
        return box;
    }
    ...
    getCollisionBoundingBoxFromPool(World world, int x, int y, int z) {
        ...
        //if (te.isFlipped) {
        //	box = AxisAlignedBB.getBoundingBox(x+m.getMinX(te), y+(1-m.getMaxY(te)),
        //z+m.getMinZ(te), x+m.getMaxX(te), y+(1-m.getMinY(te)), z+m.getMaxZ(te));
        //}
        this.setBounds(box, x, y, z);
        return box;
    }
    ...
    isSideSolid(IBlockAccess world, int x, int y, int z, ForgeDirection dir) {
        ...
        switch (c) {
            ...
            case AVOLASER:
                return dir
                    == ((TileEntityAvoLaser) world.getTileEntity(x, y, z))
                           .getFacing()
                           .getOpposite();
            default:
