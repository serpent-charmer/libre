...
public abstract class BlockAttachableMini extends Block implements SidedBlock {
    ...
    public final int getLightValue(IBlockAccess iba, int x, int y, int z) {
        return ModList.COLORLIGHT.isLoaded()
            ? ReikaColorAPI.getPackedIntForColoredLight(this.getColor(iba, x, y, z), 7)
            : 7;
    }
    ...
    public final void setSide(World world, int x, int y, int z, int side) {
        world.setBlockMetadataWithNotify(
            x, y, z, (world.getBlockMetadata(x, y, z) & (~7)) | side, 3
        );
    }
    ...
    public final void drop(World world, int x, int y, int z) {
        ItemStack is
            = new ItemStack(this, 1, this.damageDropped(world.getBlockMetadata(x, y, z)));
    ...
    createFX(World world, int x, int y, int z, double dx, double dy, double dz, Random r);
