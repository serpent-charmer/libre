...
public abstract class GuiProgressStages extends GuiScrollingPage {
    ...
    protected final void initMap() {
        ...
        for (ProgressLink p : this.getProgress()) {
            ...
            //if (map.getByDepth(depth).size() == 1 && (map.getParents(p) == null ||
            //map.getParents(p).isEmpty())) 	dx = (int)(1.5*(elementWidth+spacingX));
            int dy = depth * (elementHeight + spacingY);
            offsets.put(depth, d + 1);
            renderPositions.put(p.parent, new Point(dx, dy));
            elementWidth
                = 20; //Math.max(elementWidth,
                      //Minecraft.getMinecraft().fontRenderer.getStringWidth(p.getTitleString())+20);
            maxX = Math.max(maxX, dx + elementWidth);
            maxY = Math.max(maxY, dy + elementHeight);
            //ReikaJavaLibrary.pConsole(maxX+", "+maxY+" # "+p+" @
            //"+dx+"+"+elementWidth+", "+dy+"+"+elementHeight);
        }
    ...
    public final void initGui() {
        ...
        this.addAuxButton(
            new CustomSoundImagedGuiButton(
                10, j + xSize, k, 22, 39, 42, 126, file, ChromatiCraft.class, this
            ),
            "Return"
        );
        ...
        this.addAuxButton(
            new CustomSoundImagedGuiButton(
                20, j - 13, k - 7, 13, 35, 1, 185, file, ChromatiCraft.class, this
            ),
            "Tree"
        );
        this.addAuxButton(
            new CustomSoundImagedGuiButton(
                21, j - 13, k + 27, 13, 35, 1, 221, file, ChromatiCraft.class, this
            ),
            "Levels"
        );
    }
    ...
    private boolean elementOnScreen(ProgressLink p, int posX, int posY, int x, int y) {
        return x >= posX + 8 && x <= posX + xSize - elementWidth - 8 && y >= posY + 24 + 1
            && y - posY + elementHeight < ySize / 2 + 8;
    }
    ...
    private void renderElement(ProgressLink pl, int x, int y) {
        ...
        border = ReikaColorAPI.mixColors(
            border, 0xffffff, 0.5F + 0.25F * (float) Math.sin(Math.toRadians(t))
        );
        api.drawRectFrame(x, y, elementWidth, elementHeight, border); //temp
        if (!has) {
            Collection<ChromaResearch> li
                = ChromaResearchManager.instance.getMissingResearch(player, p);
            if (!li.isEmpty()) {
        ...
        if (see || near) {
            String s = see ? p.getTitleString()
                           : EnumChatFormatting.OBFUSCATED.toString()
                    + p.getTitleString(); //p.name();
            if (!see)
                color = 0xb5b5b5;
        ...
        if (see) {
            if (has || p.alwaysRenderFullBright()) {
                ...
                int d = -1;
                double t =
                Math.toRadians(((System.currentTimeMillis()/12D)+p.hashCode())%360); int c
                = ReikaColorAPI.mixColors(0x00ff00, 0x000000,
                0.5F+0.49F*(float)(Math.sin(t))); api.drawRectFrame(x-d, y-d,
                elementWidth+d*2, elementHeight+d*2, c); //temp
                 */
                if (hover) {
            ...
            api.drawTexturedModelRectFromIcon(
                x + 2, y + 2, ChromaIcons.QUESTION.getIcon(), 16, 16
            );
            GL11.glPopAttrib();
        }
    ...
    private void renderHoverOverlay(int x, int y, boolean has) {
        ...
        int d = 2;
        api.drawRectFrame(
            x + d, y + d, elementWidth - d * 2, elementHeight - d * 2, 0xffffff
        );
        GL11.glPushAttrib(GL11.GL_ALL_ATTRIB_BITS);
        GL11.glEnable(GL11.GL_BLEND);
        ...
        api.drawTexturedModelRectFromIcon(
            x + elementWidth / 2 - 2 + d,
            y + elementHeight / 2 - 2 + d,
            ico,
            (elementWidth - d * 2) / 2,
            (elementHeight - d * 2) / 2
        );
        ...
        if (active == null) {
            ...
            if (p.isOneStepAway(player) || p.playerHasPrerequisites(player)) {
                fontRendererObj.drawSplitString(
                    p.getTitleString(), px, posY + descY + 36, 242, 0xffffff
                );
            } else {
            ...
            if (this.renderClearText(p, player)) {
                fontRendererObj.drawSplitString(
                    p.getHintString(), px, posY + descY + 36 + 20, 242, 0xffffff
                );
            } else {
                ChromaFontRenderer.FontType.OBFUSCATED.renderer.drawSplitString(
                    p.getHintString(), px, posY + descY + 36 + 20, 242, 0xffffff
            ...
            if (p.isPlayerAtStage(player)) {
                ...
                dy -= fontRendererObj.FONT_HEIGHT * fontRendererObj.getStringWidth(s)
                    / 242;
                fontRendererObj.drawSplitString(s, px, dy, 242, 0xffffff);
            } else {
                ChromaFontRenderer.FontType.OBFUSCATED.renderer.drawSplitString(
                    this.getIncompleteText(), px, dy, 242, 0xffffff
                );
            }
            GL11.glEnable(GL11.GL_LIGHTING);
    ...
    private void randomizeString() {
        ...
        clearLength = Math.min(
            Math.max(4, rand.nextInt(s.length())),
            Math.min(rand.nextInt(3) == 0 ? 12 : 6, s.length() - randomIndex)
        );
    }
}
