...
public class GuiFluidRelay extends GuiLetterSearchable<Fluid> {
    ...
    public void initGui() {
        ...
        int iny = 45;
        int dy = 5;
        buttonList.add(new CustomSoundImagedGuiButtonSneakIcon(
            0,
            j + in,
            k + iny - dy,
            10,
        ...
        buttonList.add(new CustomSoundImagedGuiButton(
            4, j + in - dx, k + iny, 10, 10, 90, 56, tex, ChromatiCraft.class, this
        ));
        buttonList.add(new CustomSoundImagedGuiButton(
                           5,
                           j + xSize - 10 - in + dx,
    ...
    protected void actionPerformed(GuiButton b) {
        ...
        switch (b.id) {
            ...
            case 4:
                ReikaPacketHelper.sendPacketToServer(
                    ChromatiCraft.packetChannel, ChromaPackets.RELAYCLEAR.ordinal(), relay
                );
                break;
            case 5:
                ReikaPacketHelper.sendPacketToServer(
                    ChromatiCraft.packetChannel, ChromaPackets.RELAYCOPY.ordinal(), relay
                );
                break;
            case 6:
                relay.autoFilter = !relay.autoFilter;
                ReikaPacketHelper.sendPacketToServer(
                    ChromatiCraft.packetChannel, ChromaPackets.RELAYAUTO.ordinal(), relay
                );
                break;
        }
        if (delta1 != 0) {
            ReikaPacketHelper.sendPacketToServer(
                ...
                ChromaPackets.RELAYPRESSUREBASE.ordinal(),
                relay,
                delta1
            );
        }
        if (delta2 != 0) {
            ReikaPacketHelper.sendPacketToServer(
                ChromatiCraft.packetChannel,
                ChromaPackets.RELAYPRESSUREVAR.ordinal(),
                relay,
                delta2
            );
        }
    ...
    protected void drawGuiContainerBackgroundLayer(float f0, int a, int b) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (i == mansel) {
                int c = ReikaColorAPI.mixColors(
                    ...
                    0.5F
                        + 0.5F
                            * MathHelper.sin(
                                (System.currentTimeMillis() % 1000000) / 250F
                            )
                );
            ...
            if (f != null) {
                api.drawTexturedModelRectFromIcon(
                    x, y, ReikaLiquidRenderer.getFluidIconSafe(f), 16, 16
                );
        ...
        String s = String.format(
            "Pressure: %d + %d/B", relay.getBasePressure(), relay.getFunctionPressure()
        );
        api.drawCenteredStringNoShadow(
            fontRendererObj, s, j + xSize / 2, k + 45, 0xffffff
        );
    ...
    protected boolean isSearchActive() {
        return super.isSearchActive()
            && ((ContainerFluidRelay) inventorySlots).getManualSelectSlot() >= 0;
    }
