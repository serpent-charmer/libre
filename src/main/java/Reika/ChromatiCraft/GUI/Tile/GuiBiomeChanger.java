...
public class GuiBiomeChanger extends GuiChromaBase {
    //private static final ColorDistributor biomeColors = new ColorDistributor(); //static
    //to make consistent across GUI openings
    ...
    static { /*
        ...
        for (int i = 0; i < BiomeGenBase.biomeList.length; i++) {
            BiomeGenBase b = BiomeGenBase.biomeList[i];
            if (b != null) {
                int rgb = BiomeMapCommand.getBiomeColor(
                    0, 0, b
                ); //biomeColors.getColor(color);
    ...
    public void setWorldAndResolution(Minecraft mc, int x, int y) {
        ...
        painter = new BiomePainter(
            dx + 10,
            dy + ySize / 2 - TileEntityBiomePainter.RANGE,
            TileEntityBiomePainter.RANGE * 2 + 1,
            TileEntityBiomePainter.RANGE * 2 + 1,
            1,
            tile
        );
    ...
    public void initGui() {
        ...
        if (page == GuiPages.BIOME) {
            int w = 160;
            search
                = new GuiTextField(fontRendererObj, j + xSize / 2 - w / 2, k + 20, w, 16);
        ...
        int w = 10;
        buttonList.add(
            new CustomSoundGuiButton(0, j + w, k + ySize - 20 - w, 20, 20, "<", this)
        );
        buttonList.add(new CustomSoundGuiButton(
            1, j + xSize - 20 - w, k + ySize - 20 - w, 20, 20, ">", this
        ));

        if (page == GuiPages.PAINT) {
            buttonList.add(new CustomSoundGuiButton(
                2, j + w, k + 16, 50, 20, erase ? "Paint" : "Erase", this
            ));
            buttonList.add(
                new CustomSoundGuiButton(3, j + w + 50, k + 16, 20, 20, "", this) {
                    @Override
                    public void drawButton(Minecraft mc, int x, int y) {
                        super.drawButton(mc, x, y);
                        ReikaTextureHelper.bindTexture(
                            ChromatiCraft.class, GuiBiomeChanger.this.getFullTexturePath()
                        );
                        api.drawTexturedModalRect(
                            xPosition + 4, yPosition + 4, 0, 212, 12, 12
                        );
                    }
                }
            );
            buttonList.add(
                new CustomSoundGuiButton(4, j + w + 70, k + 16, 20, 20, "", this) {
                    @Override
                    public void drawButton(Minecraft mc, int x, int y) {
                        super.drawButton(mc, x, y);
                        ReikaTextureHelper.bindTexture(
                        ...
                        api.drawTexturedModalRect(
                            xPosition + 2, yPosition + 2, 12, 212, 16, 16
                        );
                    }
                }
            );
        ...
        if (page == GuiPages.BRUSH) {
            for (int i = 0; i < Brush.brushList.length; i++) {
                ...
                buttonList.add(
                    b.getButton(10 + i, j + 20 + 96 * (i % 2), k + 20 + 40 * (i / 2))
                );
            }
        }
    }
    ...
    protected void mouseClicked(int x, int y, int b) {
        ...
        if (page == GuiPages.BIOME) {
            ...
            for (Object o : buttonList) {
                ...
                if (x >= gb.xPosition && y >= gb.yPosition && x < gb.xPosition + gb.width
                    && y < gb.yPosition + gb.height) {
                    return;
                }
            }
            ...
            painter.activeElement = erase ? fallback
                : biome != null           ? biomeEntries.get(biome)
                                          : null;
    ...
    private void recalcSelectedBiomes() {
        ...
        for (BiomeGenBase out : c) {
            if (code == null || code.isEmpty()
                || (out.biomeName != null
                    && out.biomeName.toLowerCase(Locale.ENGLISH)
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (page == GuiPages.BIOME) {
            ...
            for (BiomeGenBase b : visibleBiomes) {
                String name = b.biomeName != null
                    ? b.biomeName
                    : b.getClass().getSimpleName() + "@" + b.biomeID;
                int w = fontRendererObj.getStringWidth(name);
                fontRendererObj.drawString(name, dx, dy, 0xffffff);
                biomeRegions.addRegionByWH(
                    dx - 1, dy - 1, w + 2, fontRendererObj.FONT_HEIGHT + 2, b
                );
                if (api.isMouseInBox(
                        j + dx - 2,
                        j + dx + w + 2,
                        k + dy - 1,
                        k + dy + fontRendererObj.FONT_HEIGHT + 1
                    )) {
                    api.drawRectFrame(
                        dx - 2, dy - 2, w + 3, fontRendererObj.FONT_HEIGHT + 2, 0xffffff
                    );
                }
                mw = Math.max(mw, 6 + w);
                dy += h;
    ...
    protected void drawGuiContainerBackgroundLayer(float p, int a, int b) {
        ...
        if (page == GuiPages.PAINT) {
            ...
            painter.drawLegend(
                fontRendererObj,
                j + 10 + TileEntityBiomePainter.RANGE * 2 + 3,
                k + ySize / 2 - TileEntityBiomePainter.RANGE
            );
            GL11.glPushMatrix();
            GL11.glTranslated(0, 0, 100);
            if (showRangeMarkers) {
                for (int dx = -TileEntityBiomePainter.RANGE;
                     dx <= TileEntityBiomePainter.RANGE;
                     dx += 16) {
                    for (int dz = -TileEntityBiomePainter.RANGE;
                         dz <= TileEntityBiomePainter.RANGE;
                         dz += 16) {
                        ...
                        api.drawRect(
                            ...
                            x + 1,
                            y + 1,
                            dx == 0 && dz == 0 ? 0xffffffff : 0xffff0000
                        );
                    }
                }
            } else if (showTerrain) {
        ...
        if (!erase) {
            String s = String.format(
                "Active Biome: %s",
                painter.activeElement != null ? painter.activeElement.getName() : "None"
            );
            api.drawCenteredStringNoShadow(
                fontRendererObj, s, j + xSize / 2, k + 180, 0xffffff
            );
        }
        api.drawCenteredStringNoShadow(
            fontRendererObj,
            "Brush: " + painter.brush.name,
            j + xSize / 2,
            k + 190,
            0xffffff
        );
    ...
    private static final PaintElement fallback = new PaintElement() {
        ...
        public void onPaintedTo(int x, int y) {
            GuiBiomeChanger gui
                = (GuiBiomeChanger) Minecraft.getMinecraft().currentScreen;
            if (gui == null)
                return;
    ...
    private static class BiomePaint implements PaintElement {
        ...
        public void draw(int i, int k, int x, int y, int s, boolean legend) {
            ...
            if (!legend && showTerrain) {
                int old = c;
                int dy = ReikaWorldHelper.getTopNonAirBlock(
                    Minecraft.getMinecraft().theWorld, dx, dz, true
                );
                if (ReikaFluidHelper.lookupFluidForBlock(
                        Minecraft.getMinecraft().theWorld.getBlock(dx, dy, dz)
                    )
                    == FluidRegistry.WATER) {
                    c = 0xff3050ff;
                }
                c = Minecraft.getMinecraft()
                        .theWorld.getBlock(dx, dy, dz)
                        .getMapColor(0)
                        .colorValue;
            ...
            if (!staticTileRef.canChangeBiomeAt(dx, dz, biome)) {
                int n = 10;
                c = ((dx + dz) % n + n) % n >= n / 2
                    ? ReikaColorAPI.getColorWithBrightnessMultiplier(color, 0.5F)
                    : ReikaColorAPI.getColorWithBrightnessMultiplier(color, 0.25F);
                c = 0xff000000 | c;
            }
        ...
        public String getName() {
            return biome.biomeName != null
                ? biome.biomeName
                : biome.getClass().getSimpleName() + "@" + biome.biomeID;
        }

        @Override
        public boolean isPaintable(PaintElement original) {
            return original == fallback
                || TileEntityBiomePainter.getValidBiomesFor(((BiomePaint) original).biome)
                       .contains(biome);
        }
    ...
    private class BiomePainter extends GuiPainter {
        ...
        public BiomePainter(
            int x, int y, int w, int h, int s, TileEntityBiomePainter te
        ) {
            super(x, y, w, h, s);
            tile = te;
            this.init();
