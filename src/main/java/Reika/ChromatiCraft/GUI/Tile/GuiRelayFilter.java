...
public class GuiRelayFilter extends GuiContainer {
    ...
    public void initGui() {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            int y = k + 20 + (i % 4) * 20;
            buttonList.add(
                new ImagedGuiButton(i, x, y, 20, 20, 180, 0, tex, ChromatiCraft.class)
            );
        }
    }
    ...
    private void sendData(int color) {
        ReikaPacketHelper.sendPacketToServer(
            ...
            ChromaPackets.RELAYFILTER.ordinal(),
            tile,
            color,
            allow[color] ? 1 : 0
        );
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            fontRendererObj,
            StatCollector.translateToLocal("chroma.relayfilter"),
            xSize / 2,
            5,
            0xffffff
        );
        ...
        for (int i = 0; i < 16; i++) {
            ...
            IIcon ico2
                = allow[i] ? ChromaIcons.CHECK.getIcon() : ChromaIcons.NOENTER.getIcon();
            ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                x + 2 + 8, y + 2 + 8, ico2, 8, 8
            );
        }
    }
