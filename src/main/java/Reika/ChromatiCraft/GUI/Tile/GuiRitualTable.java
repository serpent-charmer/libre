...
public class GuiRitualTable extends GuiChromability {
    ...
    public void initGui() {
        ...
        if (!abilities.isEmpty())
            buttonList.add(new CustomSoundImagedGuiButton(
                2, j + 8, k + 8, 50, 50, 0, 193, tex, ChromatiCraft.class, this
            ));
    }
    ...
    protected void actionPerformed(GuiButton b) {
        if (b.id == 2) {
            ...
            ChromaResearch r = a instanceof Chromabilities
                ? ChromaResearch.getPageFor((Chromabilities) a)
                : null;
            if (r == null
                || ChromaResearchManager.instance.playerHasFragment(player, r)) {
                ReikaPacketHelper.sendPacketToServer(
                    ChromatiCraft.packetChannel,
                    ChromaPackets.ABILITYCHOOSE.ordinal(),
                    tile,
                    Chromabilities.getAbilityInt(a)
                );
                player.closeScreen();
            } else {
    ...
    protected String getBackTexture(Ability a) {
        return Chromabilities.playerHasAbility(player, a) ? "Textures/GUIs/ability3.png"
                                                          : "Textures/GUIs/ability4.png";
    }
