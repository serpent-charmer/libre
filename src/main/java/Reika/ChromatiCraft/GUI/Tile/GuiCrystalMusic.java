...
public class GuiCrystalMusic extends GuiChromaBase implements PianoGui {
    ...
    public void initGui() {
        ...
        int r = 72;
        wheel = new PianoWheel(
            this, MusicKey.C3, 4, r, j + xSize / 2, k + ySize / 2 + 4, false
        );
        ...
        if (page == Pages.KEYS) {
            buttonList.add(new CustomSoundImagedGuiButton(
                0, j + 4, k + 4 + 20, 19, 20, 228, 136, file, ChromatiCraft.class, this
            ));

            buttonList.add(new CustomSoundImagedGuiButton(
                1, j + 23, k + 4 + 20, 19, 20, 228, 156, file, ChromatiCraft.class, this
            ));
            buttonList.add(new CustomSoundImagedGuiButton(
                6, j + 42, k + 4 + 20, 19, 20, 209, 156, file, ChromatiCraft.class, this
            ));

            buttonList.add(new CustomSoundImagedGuiButton(
                2, j + 4, k + 4 + 40, 28, 20, 228, 196, file, ChromatiCraft.class, this
            ));
            buttonList.add(new CustomSoundImagedGuiButton(
                3, j + 32, k + 4 + 40, 21, 20, 228, 176, file, ChromatiCraft.class, this
            ));
            ...
            buttonList.add(new CustomSoundImagedGuiButton(
                ...
                26,
                20,
                music.isBasslineEnabled() ? 221 : 195,
                57,
                file,
                ChromatiCraft.class,
                this
            ...
            buttonList.add(new CustomSoundImagedGuiButton(
                8, j + 23, k + 4 + 140, 19, 19, 228, 97, file, ChromatiCraft.class, this
            ));
        } else {
            buttonList.add(new CustomSoundImagedGuiButton(
                5, j + 4, k + 4 + 20, 19, 20, 209, 136, file, ChromatiCraft.class, this
            ));
        }
        ...
        if (page == Pages.MIDI) {
            ...
            ticksPer = new GuiTextField(
                fontRendererObj,
                j + xSize / 2 + 7,
                k + 126,
                fontRendererObj.getStringWidth("60") * 2,
                16
            );
    ...
    protected void actionPerformed(GuiButton b) {
        if (b.id == 0) {
            ReikaPacketHelper.sendPacketToServer(
                ChromatiCraft.packetChannel, ChromaPackets.MUSICCLEAR.ordinal(), music
            );
        } else if (b.id == 1) {
            ReikaPacketHelper.sendPacketToServer(
                ChromatiCraft.packetChannel,
                ChromaPackets.MUSICCLEARCHANNEL.ordinal(),
                music,
                channel
            );
        } else if (b.id == 2) {
            ...
            music.dispatchDemo(
            ); //in case serverside does not have functioning MIDI environment
            ReikaPacketHelper.sendPacketToServer(
                ChromatiCraft.packetChannel, ChromaPackets.MUSICDEMO.ordinal(), music
            );
        } else if (b.id == 4) {
            page = page.otherPage();
        } else if (b.id == 5) {
            ...
            ReikaPacketHelper.sendPacketToServer(
                ChromatiCraft.packetChannel,
                ChromaPackets.MUSICBKSP.ordinal(),
                music,
                channel
            );
        } else if (b.id == 7) {
            music.togglePause();
            ReikaPacketHelper.sendPacketToServer(
                ChromatiCraft.packetChannel, ChromaPackets.MUSICPAUSE.ordinal(), music
            );
        } else if (b.id == 8) {
            ReikaPacketHelper.sendPacketToServer(
                ChromatiCraft.packetChannel, ChromaPackets.MUSICSTOP.ordinal(), music
            );
        } else if (b.id == 9) {
            music.toggleBass();
            ReikaPacketHelper.sendPacketToServer(
                ChromatiCraft.packetChannel, ChromaPackets.MUSICBASS.ordinal(), music
            );
    ...
    protected void drawGuiContainerBackgroundLayer(float p, int a, int b) {
        ...
        if (page == Pages.KEYS) {
            ...
            ReikaTextureHelper.bindTexture(
                ChromatiCraft.class, "Textures/GUIs/musicoverlay.png"
            );
            this.drawTexturedModalRect(
                wheel.originX - wheel.radius - 16,
                wheel.originY - wheel.radius - 16,
                0,
                0,
                176,
                176
            ...
            if (!input.isFocused()) {
                ...
                //fontRendererObj.drawStringWithShadow(file.substring(d,
                //Math.min(file.length(), 37+d)), posX+10, posY+ySize-15, 0xaaaaaa);
            }
            ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                fontRendererObj,
                "Select a MIDI file. Be sure to include the drive",
                j + xSize / 2 + 1,
                k + 60,
                0xffffff
            );
            ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                fontRendererObj,
                "letter and file extension and use \"/\", not \"\\\".",
                j + xSize / 2 + 1,
                k + 70,
                0xffffff
            );
            ...
            fontRendererObj.drawString(
                "(" + midiBPM + " bpm)", j + xSize / 2 + 1 + 37, k + 130, 0xffffff
            );
        }
    }
    ...
    public void onKeyPressed(MusicKey key) {
        ...
        for (CrystalElement e : CrystalMusicManager.instance.getColorsWithKey(key)) {
            music.playCrystal(
                music.worldObj, music.xCoord, music.yCoord, music.zCoord, e, length, key
            );
        }
        ReikaPacketHelper.sendPacketToServer(
            ChromatiCraft.packetChannel,
            ChromaPackets.MUSICNOTE.ordinal(),
            music,
            channel,
            key.ordinal(),
            length,
