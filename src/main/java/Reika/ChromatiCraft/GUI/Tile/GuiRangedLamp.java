...
public class GuiRangedLamp extends GuiContainer {
    ...
    protected void actionPerformed(GuiButton b) {
        ...
        if (b.id == 0) {
            ReikaPacketHelper.sendPacketToServer(
                ChromatiCraft.packetChannel, ChromaPackets.LAMPINVERT.ordinal(), lamp
            );
    ...
    public void updateScreen() {
        ...
        if (!(input.getText().matches("^[0-9 ]+$"))) {
            ...
            ReikaPacketHelper.sendPacketToServer(
                ...
                ChromaPackets.LAMPCHANNEL.ordinal(),
                lamp,
                channel
            );
            return;
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            fontRendererObj,
            StatCollector.translateToLocal("chroma.lampblock"),
            xSize / 2,
            5,
            0xffffff
        );
        ...
        if (!input.isFocused()) {
            fontRendererObj.drawString(
                String.format("%d", lamp.getChannel()), xSize / 2 + 6 - 20, 25, 0xffffff
            );
        }
    }
