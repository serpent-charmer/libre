...
public class GuiParticleSpawner extends GuiChromaBase {
    ...
    private static final ScrollingButtonList iconButtons
        = new ScrollingButtonList(MAX_ICON_ROWS, MAX_ICON_COLS);
    ...
    public void initGui() {
        ...
        for (int i = 0; i < GuiPage.list.length; i++) {
            ...
            buttonList.add(new CustomSoundImagedGuiButton(
                i, x, y, 20, 20, u, v, file, ChromatiCraft.class, this
            ));
        }
        ...
        switch (page) {
            ...
            case VELOCITY:
                for (int i = 0; i < 12; i++) {
                    ...
                    int u = i % 2 == 1 ? 126 : 106;
                    int v = 173;
                    GuiButton b = new CustomSoundImagedGuiButton(
                        i + 100, x, y, 20, 20, u, v, file, ChromatiCraft.class, this
                    );
                    buttonList.add(b);
                }
                break;
            case COLOR:
                for (int i = 0; i < 16; i++) {
                    ...
                    int u = 106;
                    int v = 213;
                    ImagedGuiButton b = new CustomSoundImagedGuiButton(
                        i + 100, x, y, 20, 20, u, v, file, ChromatiCraft.class, this
                    );
                ...
                for (int i = 0; i < 8; i++) {
                    ...
                    int v = i >= 6 ? 193 : 173;
                    GuiButton b = new CustomSoundImagedGuiButton(
                        i + 200, x, y, 20, 20, u, v, file, ChromatiCraft.class, this
                    );
                    buttonList.add(b);
                }
                int in = 45;
                buttonList.add(new GuiButton(
                    300, j + 29 + in, k + 142, 143 - in * 2, 20, !RGBMode ? "HSV" : "RGB"
                ));
                break;
            case ICON: {
                ...
                for (IconEnum ico : permittedIcons) {
                    if (i >= iconButtons.getBaseOffset()
                        && i <= iconButtons.getHighestVisible()) {
            ...
            case SAVELOAD:
                ...
                if (filename != null)
                    s = filename.getText();
                filename
                    = new GuiTextField(fontRendererObj, j + 32, k + 106, xSize - 40, 16);
        ...
        if (b.id == -1) {
            ...
            if (GuiScreen.isShiftKeyDown()) {
                ...
                if (page == GuiPage.COLOR
                    || (page == GuiPage.TIMING
                        && !ReikaMathLibrary.isValueInsideBoundsIncl(2, 3, b.id - 100)))
                    n = 5;
    ...
    private File getFolder() {
        return new File(
            new File(DragonAPICore.getMinecraftDirectory(), "ChromatiCraft_Data"),
            "ParticlePrograms"
        );
    }
    ...
    protected void keyTyped(char c, int i) {
        if (i != mc.gameSettings.keyBindInventory.getKeyCode()
            && (filename == null || !filename.isFocused()))
            super.keyTyped(c, i);
            ...
            if (pos != null) {
                ...
                api.drawRectFrame(
                    ...
                    ReikaColorAPI.mixColors(
                        0xff00ff,
                        0x22aaff,
                        0.5F + 0.5F * (float) Math.sin(System.currentTimeMillis() / 300D)
                    )
                );
            }
        }
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int x, int y) {
        ...
        if (page == GuiPage.SAVELOAD) {
            api.drawCenteredStringNoShadow(
                fontRendererObj, "Particle File:", j + xSize / 2, k + 20, 0x00ff00
            );

            String path
                = this.getFile().getParentFile().getAbsolutePath().replace("\\.\\", "\\")
                + "\\";
            ...
            for (int i = 0; i < li.size(); i++) {
                ...
                fontRendererObj.drawString(
                    s, j + 35, k + 31 + i * (fontRendererObj.FONT_HEIGHT + 1), 0xffffff
                );
            }
            filename.drawTextBox();
            if (!filename.isFocused()) {
                //	fontRendererObj.drawString(input.getText(), input.xPosition+4,
                //input.yPosition+4, 0xa0a0a0);
            }
            int dx = fontRendererObj.getStringWidth(filename.getText());
            //fontRendererObj.drawString(".par", filename.xPosition+4+dx,
            //filename.yPosition+4, 0xffffff);
        }
        ...
        for (Option o : page.options) {
            ...
            ReikaTextureHelper.bindTexture(
                ChromatiCraft.class, this.getFullTexturePath()
            );
            ...
            if (o instanceof VariableOption) {
                ...
                GL11.glTranslated(0, 0, 350);
                api.drawCenteredStringNoShadow(
                    fontRendererObj, s, dx + 51, dy + 6, 0xffffff
                );
                GL11.glTranslated(0, 0, -350);
                i++;
            }
    ...
    private static enum GuiPage {
        ...
        private void initOptions(TileEntityParticleSpawner p, boolean RGB) {
            ...
            switch (this) {
                case POSITION:
                    options.add(
                        new VariableOption("X Position", p.particles.particlePositionX)
                    );
                    options.add(
                        new VariableOption("Y Position", p.particles.particlePositionY)
                    );
                    options.add(
                        new VariableOption("Z Position", p.particles.particlePositionZ)
                    );
                    break;
                case VELOCITY:
                    options.add(
                        new VariableOption("X Velocity", p.particles.particleVelocityX)
                    );
                    options.add(
                        new VariableOption("Y Velocity", p.particles.particleVelocityY)
                    );
                    options.add(
                        new VariableOption("Z Velocity", p.particles.particleVelocityZ)
                    );
                    break;
                case COLOR:
                    ...
                    if (RGB) {
                        ...
                        options.add(new SliderOption(
                            "Saturation", new BoundedValue(0F, 1, sat).setStep(0.03125F)
                        ));
                        options.add(new SliderOption(
                            "Luminosity", new BoundedValue(0F, 1, lum).setStep(0.03125F)
                        ));
                    }
                    options.add(
                        new ToggleOption("Cycling Color", p.particles.cyclingColor)
                    );
                    break;
                case ICON:
                    break;
                ...
                case MODIFIER:
                    ...
                    options.add(
                        new ToggleOption("Collision", p.particles.particleCollision)
                    );
