...
public class GuiLumenAlveary extends GuiChromaBase {
    ...
    private final AlvearyEffectControlSet controls
        = new AlvearyEffectControlSet(AlvearyEffect.class);
    ...
    private AlvearyEffectControlSet getCurrentControlSet() {
        return currentCategory == null ? controls
                                       : this.getControlSet(currentCategory.classRef);
    }
        ...
        if (set.isTopLevel()) {
            ...
            if (e != null) {
                ...
                ReikaPacketHelper.sendStringIntPacket(
                    ...
                    ChromaPackets.ALVEARYEFFECT.ordinal(),
                    tile,
                    e.effect.ID,
                    e.isActive ? 1 : 0
                );
    ...
    private static class AlvearyEffectControl
        implements ColorCallback, Comparable<AlvearyEffectControl> {
        ...
        public int getColor(Object key) {
            return ReikaColorAPI.getColorWithBrightnessMultiplier(
                0xffffff, this.getBrightnessModifier()
            );
        }
        ...
        protected final float getBrightnessModifier() {
            ...
            return (float
               ...
               + 0.0625F * Math.sin(System.currentTimeMillis() / mod + effect.hashCode())
            );
        }
    }
    ...
    private static class LumenAlvearyEffectControl extends AlvearyEffectControl {
        ...
        public int getColor(Object key) {
            ...
            c = ReikaColorAPI.getColorWithBrightnessMultiplier(
                c, this.getBrightnessModifier()
            );
            if (isHovered) {
    ...
    private static class AlvearyEffectControlSet<K, V extends AlvearyEffectControl, E
                                                     extends AlvearyEffect>
        implements Comparable<AlvearyEffectControlSet> {
        private final MultiMap<K, V> controls
            = new MultiMap(CollectionType.LIST, this.getDeterminator());
        private final HashMap<Class, AlvearyEffectControlSet> children = new HashMap();

        private final Proportionality<K> buttonsGlobal
            = new Proportionality(this.getDeterminator());
        private final Proportionality<AlvearyEffectControlSet> childButtons
            = new Proportionality(new SortedDeterminator());
        ...
        protected final void addControl(K k, E e) {
            ...
            buttonsGlobal.addColorRenderer(
                k, new IntColorCallback(this.getColorForKey(k))
            );
    ...
    private static class LumenAlvearyEffectControlSet extends AlvearyEffectControlSet<
        CrystalElement,
        LumenAlvearyEffectControl,
        LumenAlvearyEffect> {
        ...
        @Override
        protected LumenAlvearyEffectControl
        constructControl(CrystalElement k, LumenAlvearyEffect e, int idx) {
    ...
    private static class VisAlvearyEffectControlSet extends AlvearyEffectControlSet<
        Aspect,
        VisAlvearyEffectControl,
        VisAlvearyEffect> {
        ...
        @Override
        protected VisAlvearyEffectControl
        constructControl(Aspect k, VisAlvearyEffect e, int idx) {
