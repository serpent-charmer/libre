    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (f != null) {
            if (api.isMouseInBox(j + 24, j + 151, k + 16, k + 115)) {
                String s = String.format(
                    "%s: %d/%d",
                    f.getLocalizedName(),
                    Math.round(tank.getCurrentFluidLevel() / 1000F),
                    tank.getCapacity() / 1000
                );
                api.drawTooltipAt(
                    fontRendererObj, s, api.getMouseRealX() - j, api.getMouseRealY() - k
                );
            }
        }
        ...
        for (float d = s; d < 1; d += s) {
            String sg = String.format(
                "%.1f%sB",
                ReikaMathLibrary.getThousandBase(cap * d),
                ReikaEngLibrary.getSIPrefix(cap * d)
            );
            String sg2 = String.format("- %d%s -", Math.round(d * 100), "%");
            fontRendererObj.drawString(
                sg,
                146 - fontRendererObj.getStringWidth(sg),
                fontRendererObj.FONT_HEIGHT / 2 + 109 - (int) (d * 101),
                c
            );
            fontRendererObj.drawString(
                sg2, 24, fontRendererObj.FONT_HEIGHT / 2 + 109 - (int) (d * 101), c
            );
        }
        GL11.glPopMatrix();
        ...
        String form2 = String.format(
            "Capacity~4n^2 = %.1f%sB",
            ReikaMathLibrary.getThousandBase(cap),
            ReikaEngLibrary.getSIPrefix(cap)
        );
        fontRendererObj.drawString(form, 25, 125, 0xffffff);
        fontRendererObj.drawString(
            form2, 25, 126 + fontRendererObj.FONT_HEIGHT + 3, 0xffffff
        );
    }
            ...
            for (int i = 0; i < 8; i++) {
                for (int m = 0; m < num; m++) {
                    this.drawTexturedModelRectFromIcon(
                        j + 24 + i * 16,
                        k + 16 + 100 - (m + 1) * 16,
                        ReikaLiquidRenderer.getFluidIconSafe(f),
                        16,
                        16
                    );
                }
