...
public class GuiLampController extends GuiChromaBase {
    ...
    public void initGui() {
        ...
        if (control == Control.MANUAL) {
            buttonList.add(
                new CustomSoundGuiButton(1, j + 85, k + 60, 60, 20, "Toggle", this)
            );
        }
    ...
    protected void actionPerformed(GuiButton b) {
        ...
        if (b.id == 0 && lockout == 0) {
            ReikaPacketHelper.sendPacketToServer(
                ...
                ChromaPackets.LAMPCONTROL.ordinal(),
                lamp,
                1,
                0
            );
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (!input.isFocused()) {
            fontRendererObj.drawString(
                String.format("%d", lamp.getChannel()), xSize / 2 + 6, 25, 0xffffffff
            );
        }
        ...
        switch (c) {
            ...
            case REDSTONE:
                ...
                break;
            case RFSTORAGE:
                is = GameRegistry.findItemStack(
                    ModList.THERMALEXPANSION.modLabel, "powerCoilElectrum", 1
                );
                break;
            case SHAFTPOWER:
                is = GameRegistry.findItemStack(
                    ModList.ROTARYCRAFT.modLabel, "rotarycraft_item_gearcraft", 1
                );
                if (is != null)
                    is.setItemDamage(0);
                break;
