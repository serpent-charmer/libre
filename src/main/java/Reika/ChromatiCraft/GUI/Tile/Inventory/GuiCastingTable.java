...
public class GuiCastingTable extends GuiChromaBase {
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
                ...
                if (is != null) {
                    ...
                    int y2 = b + dy + 17; /*
                     if (api.isMouseInBox(x1, x2, y1, y2)) {
                         ...
                         api.drawTooltipAt(fontRendererObj, s,
                     mx-fontRendererObj.getStringWidth(s)/2, my);
                     }*/
                }
            }
        }
        ...
        boolean[] struct = new boolean[] {
            true, tile.hasStructure, tile.hasStructure2, tile.hasPylonConnections
        };
        for (int i = 0; i <= tier.ordinal(); i++) {
            ...
            this.drawTexturedModelRectFromIcon(
                x, y, ChromaIcons.DIAMOND.getIcon(), 16, 16
            );
            if (!struct[i]) {
                BlendMode.DEFAULT.apply();
                GL11.glColor4f(1, 1, 1, 1);
                this.drawTexturedModelRectFromIcon(
                    x + 2, y + 2, ChromaIcons.NOENTER.getIcon(), 12, 12
                );
            }
        }
        ...
        if (r != null) {
            ...
            if (res != null
                && !ChromaResearchManager.instance.playerHasFragment(player, res)) {
                ReikaTextureHelper.bindTerrainTexture();
                GL11.glEnable(GL11.GL_BLEND);
                ...
                if (ctr != null && out.stackTagCompound != null) {
                    ReikaNBTHelper.combineNBT(
                        out.stackTagCompound, r.getOutputTag(player, ctr.stackTagCompound)
                    );
                }
            ...
            if (!r.canRunRecipe(tile, player)) {
                ...
                if (api.isMouseInBox(189 + a, 189 + 16 + a, 12 + b, 28 + b)) {
                    ...
                    ChromaBookData.drawRecipeMissingProgress(
                        r, player, itemRender, fontRendererObj, x0, y0, !tile.isTuned()
                    );
                }
            }
            zLevel = 0;
            ...
            if (r instanceof PylonCastingRecipe) {
                ...
                for (CrystalElement e : tag.elementSet()) {
                    ...
                    //this.drawRect(x, y1, x+w, y,
                    //e.getJavaColor().darker().darker().getRGB()); this.drawRect(x,
                    //y1+dy, x+w, y, e.getColor());

                    ChromaFX.drawFillBar(
                        e, x, y1, w, h, energy / (float) tag.getValue(e)
                    );
                }
            }
        }
    ...
    public String getGuiTexture() {
        return tile.isAtLeast(RecipeType.PYLON) ? "table5"
            : this.isMultiForm()                ? "table4"
                                                : "table2";
    }
}
