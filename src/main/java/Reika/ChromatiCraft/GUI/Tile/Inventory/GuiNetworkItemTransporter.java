...
public class GuiNetworkItemTransporter extends GuiChromaBase {
    ...
    public GuiNetworkItemTransporter(
        EntityPlayer p5ep, TileEntityNetworkItemTransporter te
    ) {
    ...
    protected final void func_146977_a(Slot slot) {
        if (slot.slotNumber >= 12 && slot.inventory == net) {
            if (this.isMouseOverSlot(slot, api.getMouseRealX(), api.getMouseRealY())) {
                String s = "Hold " + EnumChatFormatting.GREEN + "CTRL"
                    + EnumChatFormatting.RESET + " to see/set filters";
                int dx = 25;
                if (this.isFilterMode()) {
                    s = "Release " + EnumChatFormatting.GREEN + "CTRL"
                        + EnumChatFormatting.RESET + " to see received items";
                    dx = 61;
                }
                api.drawTooltip(
                    fontRendererObj, s, -fontRendererObj.getStringWidth(s) + dx, -28
                );
            }
            ...
            if (this.isFilterMode()) {
                api.drawItemStack(
                    itemRender,
                    fontRendererObj,
                    net.getFilter(slot.slotNumber - 12),
