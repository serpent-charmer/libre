...
public class GuiItemInserter extends GuiChromaBase {
            ...
            if (tile.getStackInSlot(i) != null) {
                for (int f = 0; f < TileEntityItemInserter.TARGETS; f++) {
                    ...
                    if (c != null) {
                        ...
                        //ReikaJavaLibrary.pConsole(i+","+f+">"+tile.isLinkEnabled(i, f)+"
                        //@ "+dx+","+dy);
                        buttonList.add(
                            new ImagedGuiButton(
                                id,
                                ...
                                .setTooltip("Slot " + i + " > " + tile.getLink(f))
                        );
                    }
                }
            }
    ...
    protected void actionPerformed(GuiButton b) {
        ...
        if (b.id < TileEntityItemInserter.TARGETS) {
            ReikaPacketHelper.sendPacketToServer(
                ...
                ChromaPackets.INSERTERMODE.ordinal(),
                tile,
                b.id
            );
            ...
            ReikaPacketHelper.sendPacketToServer(
                ChromatiCraft.packetChannel,
                ChromaPackets.INSERTERCONNECTION.ordinal(),
                tile,
                i,
                k
            );
    ...
    protected void mouseClicked(int x, int y, int b) {
        ...
        if (Keyboard.isKeyDown(Keyboard.KEY_LSHIFT)) {
            ...
            if (slot >= 0) {
                ...
                ReikaPacketHelper.sendPacketToServer(
                    ChromatiCraft.packetChannel,
                    ChromaPackets.INSERTERCLEAR.ordinal(),
                    tile,
                    slot
                );
            }
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int a, int b) {
        ...
        for (int i = 0; i < 6; i++) {
            if (ReikaRedstoneHelper.isPoweredOnSide(
                    tile.worldObj,
                    tile.xCoord,
                    tile.yCoord,
                    tile.zCoord,
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        for (int i = 0; i < TileEntityItemInserter.TARGETS; i++) {
            ...
            if (c != null) {
                ...
                if (b != Blocks.air) {
                    ItemStack is = ReikaRenderHelper.getBlockItem(
                        b, c.getBlockMetadata(tile.worldObj)
                    );
