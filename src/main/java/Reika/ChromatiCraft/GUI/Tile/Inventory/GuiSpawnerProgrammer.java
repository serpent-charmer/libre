...
public class GuiSpawnerProgrammer extends GuiLetterSearchable<String> {
    ...
    static {
        for (Entry<String, Class> e :
             ((Map<String, Class>) EntityList.stringToClassMapping).entrySet()) {
    ...
    public GuiSpawnerProgrammer(EntityPlayer ep, TileEntitySpawnerReprogrammer tile) {
        ...
        ReikaPacketHelper.sendStringPacket(
            ...
            ChromaPackets.SPAWNERPROGRAM.ordinal(),
            this.getActive(),
            prog
        );
    }
    ...
    public void initGui() {
        ...
        switch (page) {
            case MOBTYPE:
                ...
                break;
            case TIMER:
                minDelay.bar = new FractionalBar(
                    j + xSize / 2 - 64 + 34,
                    k + 55,
                    100,
                    6,
                    TileEntitySpawnerReprogrammer.MIN_MINDELAY,
                    18000,
                    minDelay.currentValue
                ); //10s to 15 min
                maxDelay.bar = new FractionalBar(
                    j + xSize / 2 - 64 + 34,
                    k + 85,
                    100,
                    6,
                    TileEntitySpawnerReprogrammer.MIN_MINDELAY,
                    18000,
                    maxDelay.currentValue
                );
                break;
            case COUNTS:
                maxNearMobs.bar = new FractionalBar(
                    j + xSize / 2 - 64 + 34,
                    k + 55,
                    100,
                    6,
                    0,
                    16,
                    maxNearMobs.currentValue
                );
                spawnCount.bar = new FractionalBar(
                    j + xSize / 2 - 64 + 34,
                    k + 85,
                    100,
                    6,
                    1,
                    16,
                    spawnCount.currentValue
                );
                break;
            case RANGES:
                spawnRange.bar = new FractionalBar(
                    j + xSize / 2 - 64 + 34,
                    k + 55,
                    100,
                    6,
                    1,
                    32,
                    spawnRange.currentValue
                );
                activationRange.bar = new FractionalBar(
                    j + xSize / 2 - 64 + 34,
                    k + 85,
                    100,
                    6,
    ...
    protected void mouseClicked(int x, int y, int b) {
        ...
        if (flag) {
            ...
            ReikaSoundHelper.playClientSound(
                ChromaSounds.GUICLICK, Minecraft.getMinecraft().thePlayer, 1, 0.8F
            );
        }
    }
    ...
    private void sendData() {
        ReikaPacketHelper.sendPacketToServer(
            ChromatiCraft.packetChannel,
            ChromaPackets.SPAWNERDATA.ordinal(),
            prog,
            minDelay.currentValue,
            maxDelay.currentValue,
            maxNearMobs.currentValue,
    ...
    private String getMobDisplayName() {
        ...
        if (EntityEnderman.class.isAssignableFrom(c)
            || EntityPigZombie.class.isAssignableFrom(c)) {
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (page == Pages.MOBTYPE) {
            ...
            ReikaGuiAPI.instance.drawCenteredString(
                fontRendererObj, display, xSize / 2, 47, debug ? 0xffff00 : c
            );
        }
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int a, int b) {
        ...
        switch (page) {
            ...
            case RANGES:
                ...
                activationRange.bar.drawTitle(
                    fontRendererObj, "Activation Range", 0xffffff
                );
                break;
        }
    }
