...
public class GuiMiner extends GuiChromaBase {
    ...
    private static final int COLS_PER_CAT
        = Math.max(1, COLS / ((MineralCategory.values().length - 1) * 2 - 1));
    ...
    protected void actionPerformed(GuiButton b) {
        ...
        if (b.id == 0) {
            ...
            ReikaPacketHelper.sendPacketToServer(
                ...
                ChromaPackets.MINERCATEGORY.ordinal(),
                tile,
                MineralCategory.ANY.ordinal()
            );
            this.initGui();
        }
    }
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        for (MineralCategory mc : c.keySet()) {
            ...
            int ox = (int
               ...
               * (12 + mc.ordinal() * ITEM_SIZE * s * s
                  + (mc.ordinal() * CAT_SPACE * ITEM_SIZE * s * s)));
            int oy = (int) ((1 / s) * (28));
            ...
            if (api.isMouseInBox(j + ox - 4, j - 4 + ox + 10, k, k + ySize)) {
                ...
                fontRendererObj.drawString(
                    mc.displayName + " (x" + cn + ")",
                    10,
                    27 - fontRendererObj.FONT_HEIGHT,
                    sel ? 0xff00ff00 : 0xffffffff
                );
            ...
            for (ItemDisplay id : c.get(mc)) {
                if (i >= 0) {
                    int dx
                        = ox + (int) ((1 / s) * ((i % COLS_PER_CAT) * ITEM_SIZE * s * s));
                    int dy
                        = oy + (int) ((1 / s) * ((i / COLS_PER_CAT) * ITEM_SIZE * s * s));
                    if (api.isMouseInBox(
                            j + dx - 4, j + dx + 10 - 4, k + dy - 9, k + dy + 12 - 9
                        )) {
                        String sg = id.getDisplayItem().getDisplayName() + ": "
                            + id.getAmount();
                        api.drawTooltipAt(
                            fontRendererObj,
                            sg,
                            dx + fontRendererObj.getStringWidth(sg) + 36,
                            dy
                        );
                        selectedItem = id;
        ...
        dx-j+fontRendererObj.getStringWidth(sg)+36, dy-k);
            }
        }
         */

        String sg
            = String.format("Scan: %.2f%s done", 100 * tile.getDigCompletion(), "%");
        fontRendererObj.drawString(sg, 4, 111, 0xffffff);

        if (tile.isReady()) {
            int g = 160
                + (int
                ) (95 * Math.sin(Math.toRadians(System.currentTimeMillis() / 3 % 360)));
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int a, int b) {
        ...
        for (MineralCategory mc : c.keySet()) {
            int ox = (int
               ...
               * (j + 12 + mc.ordinal() * ITEM_SIZE * s
                  + (mc.ordinal() * CAT_SPACE * ITEM_SIZE * s)));
            int oy = (int) ((1 / s) * (k + 28));
            //fontRendererObj.drawString(mc.displayName, ox,
            //oy-fontRendererObj.FONT_HEIGHT-2, 0xffffffff);
            int i = -offset;
            for (ItemDisplay id : c.get(mc)) {
                if (i >= 0) {
