...
public class GuiAutoEnchanter extends GuiLetterSearchable<Enchantment> {
    ...
    public void initGui() {
        ...
        int w = 4;
        buttonList.add(
            new CustomSoundGuiButton(0, j + dx - w, k + dy - 13, 20, 20, "-", this)
        );
        if (ench.isCompatible(this.getActive()) == EnchantValidity.VALID)
            buttonList.add(new CustomSoundGuiButton(
                1, j + dx + 20 + w, k + dy - 13, 20, 20, "+", this
            ));
        else
            buttonList.add(new CustomSoundGuiButton(
                               1, j + dx + 20 + w, k + dy - 13, 20, 20, "x", this
            )
                               .disable());

        buttonList.add(new CustomSoundGuiButton(
            10, j + dx - w, k + dy + 20 + 5 - 13, 48, 20, "Reset", this
        ));
        ...
        switch (b.id) {
            case 0:
                ReikaPacketHelper.sendPacketToServer(
                    ...
                    ChromaPackets.ENCHANTER.ordinal(),
                    ench,
                    this.getID(),
                    0,
            ...
            case 10:
                ReikaPacketHelper.sendPacketToServer(
                    ChromatiCraft.packetChannel,
                    ChromaPackets.ENCHANTERRESET.ordinal(),
                    ench
                );
                index = 0;
                break;
            case 2:
                this.decrIndex(
                    this.getFilter(GuiScreen.isShiftKeyDown(), GuiScreen.isCtrlKeyDown())
                );
                break;
            case 3:
                this.incrIndex(
    ...
    private Function<Enchantment, Boolean>
    getFilter(boolean newType, boolean skipInvalid) {
        Enchantment pre = this.getActive();
        return (r)
                   -> (newType && this.getActive().type != pre.type)
            || (skipInvalid && !this.isValidForItem(this.getActive()));
    }
    ...
    private String getEnchantDisplayString() {
        ...
        return level > 0 ? e.getTranslatedName(level)
                         : StatCollector.translateToLocal(e.getName()) + " 0";
    }
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        fontRendererObj.drawString(
            "Cost:", 32 - fontRendererObj.getStringWidth("Cost:"), 28, 0xffffff
        );
        fontRendererObj.drawString(
            sg,
            32 - fontRendererObj.getStringWidth(sg),
            38,
            amt > ench.getCapacity()          ? 0xff0000
                : amt > ench.getChromaLevel() ? 0xffff00
                                              : 0x00ff00
        );

        String display = this.getEnchantDisplayString();
        EnchantValidity e = ench.isCompatible(this.getActive());
        if (e != EnchantValidity.VALID
            && api.isMouseInBox(xSize / 2 - 65 + j, xSize / 2 + 65 + j, 70 + k, 85 + k)) {
