...
public class GuiTelePump extends GuiChromaBase {
    ...
    public void initGui() {
        ...
        buttonList.add(new CustomSoundImagedGuiButton(
            1000, j + 7, k + 15, 140, 8, 7, 15, tex, ChromatiCraft.class, this
        ));
        buttonList.add(new CustomSoundImagedGuiButton(
            1001, j + 7, k + 63, 140, 8, 7, 63, tex, ChromatiCraft.class, this
        ));

        //for (int i = 0; i < list.size(); i++) {
        //	Fluid f = list.get(i);
        buttonList.add(new CustomSoundImagedGuiButton(
            0, j + 9, k + 31, 136, 24, 0, 168, tex, ChromatiCraft.class, this
        ));
        //}
    }
        ...
        if (b.id == 1000) {
            ...
            if (pump.isFluidDiscovered(sel, player)) {
                ReikaPacketHelper.sendPacketToServer(
                    ...
                    ChromaPackets.TELEPUMP.ordinal(),
                    pump,
                    sel.getID()
                );
                player.closeScreen();
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        for (int i = min; i <= max; i++) {
            ...
            if (i == active) {
                double ct = disc ? pump.getFluidCount(f)
                        / (double) FluidContainerRegistry.BUCKET_VOLUME
                                 : 0;
                String sg = String.format(
                    "%s (%.3f%sB)",
                    f.getLocalizedName(),
                    ReikaMathLibrary.getThousandBase(ct),
                    ReikaEngLibrary.getSIPrefix(ct)
                );
                if (!disc)
                    sg = "[Unknown] ("
                        + ReikaStringParser.padToLength(
                            String.valueOf(ReikaRandomHelper.getRandomBetween(0, 99999)),
                            5,
                            " "
                        )
                        + " B)";
    ...
    private static class FluidListSorter implements Comparator<Fluid> {
        ...
        public int compare(Fluid o1, Fluid o2) {
            return -1000000
                * Integer.compare(tile.getFluidCount(o1), tile.getFluidCount(o2))
                + ReikaFluidHelper.fluidComparator.compare(
                    o1, o2
                ); //less fluid -> end of list
        }
    }
}
