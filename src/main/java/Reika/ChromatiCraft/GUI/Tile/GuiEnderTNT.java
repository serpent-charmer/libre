...
public class GuiEnderTNT extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            fontRendererObj,
            StatCollector.translateToLocal("chroma.endertnt"),
            xSize / 2,
            5,
            0xffffff
        );
        ...
        for (int i = 0; i < 4; i++)
            fontRendererObj.drawString(
                s[i],
                xSize / 2 - 48 - fontRendererObj.getStringWidth(s[i]),
                25 + 20 * i,
                0xffffff
            );
        if (!input.isFocused()) {
            fontRendererObj.drawString(
                String.format("%d", dim), xSize / 2 - 34, 25, 0xffffffff
            );
        }
        if (!input2.isFocused()) {
            fontRendererObj.drawString(
                String.format("%d", tx), xSize / 2 - 34, 45, 0xffffffff
            );
        }
        if (!input3.isFocused()) {
            fontRendererObj.drawString(
                String.format("%d", ty), xSize / 2 - 34, 65, 0xffffffff
            );
        }
        if (!input4.isFocused()) {
            fontRendererObj.drawString(
                String.format("%d", tz), xSize / 2 - 34, 85, 0xffffffff
            );
        }
        //World world = DimensionManager.getWorld(dim); //cannot get clientside
        String sn = "Dimension "
            + dim; //world != null ? world.provider.getDimensionName() : "NO SUCH WORLD";
        int c = 0xffffff; //world != null ? 0xffffff : 0xff0000;
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            fontRendererObj, String.format("[%s]", sn), xSize / 2 + 46, 25, c
        );
    }
