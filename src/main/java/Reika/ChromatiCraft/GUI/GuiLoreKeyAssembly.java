...
public class GuiLoreKeyAssembly extends GuiScreen implements CustomSoundGui {
    ...
    public final void drawScreen(int x, int y, float f) {
        ...
        if (h != null) {
            ...
            //ReikaGuiAPI.instance.drawLine(x, y,
            //sr.getScaledWidth()/2+puzzle.CELL_SIZE/2+p.x,
            //sr.getScaledHeight()/2+puzzle.CELL_SIZE/2+p.y, 0xffffffff);
        }
    ...
    protected void mouseClicked(int x, int y, int button) {
        ...
        if (button == 0 && !LoreManager.instance.hasPlayerCompletedBoard(player)) {
            ...
            Hex h = puzzle.getHexAt(
                x - sr.getScaledWidth() / 2 - puzzle.CELL_SIZE / 2,
                y - sr.getScaledHeight() / 2 - puzzle.CELL_SIZE / 2
            );
            if (h != null) {
                if (!LoreManager.instance.hasScannedAllTowers(player)) {
                    ...
                    Point p = puzzle.getHexLocation(h);
                    puzzle.getCell(h).onClick(
                        puzzle,
                        x - (sr.getScaledWidth() / 2 + puzzle.CELL_SIZE / 2 + p.x),
                        y - (sr.getScaledHeight() / 2 + puzzle.CELL_SIZE / 2 + p.y)
                            + 0 * 16,
                        player
                    );
