...
public class GuiTeleportAbility extends GuiChromaTool {
    ...
    public void initGui() {
        ...
        if (screen == Screen.SET) {
            buttonList.add(new CustomSoundGuiButton(
                ...
                120,
                20,
                ChromaFontRenderer.FontType.GUI.id + "   Add Point",
                this
            ));
            //buttonList.add(new CustomSoundGuiButton(3, j+88, k+ySize-28, 60, 20,
            //ChromaFontRenderer.FontType.GUI.id+"   Minimap", this));
        } else if (screen == Screen.SELECT || screen == Screen.MINIMAP) {
            buttonList.add(new CustomSoundGuiButton(
                1,
                j + 28,
                k + ySize - 28,
                screen == Screen.SELECT ? 60 : 120,
                20,
                ChromaFontRenderer.FontType.GUI.id + "   Go To",
                this
            ));
            if (screen == Screen.SELECT)
                buttonList.add(new CustomSoundGuiButton(
                    ...
                    60,
                    20,
                    ChromaFontRenderer.FontType.GUI.id + "   Remove",
                    this
                ));
            buttonList.add(new CustomSoundGuiButton(
                4, j + 4, k + 4, 10, 10, ChromaFontRenderer.FontType.GUI.id + "   -", this
            ));
            buttonList.add(new CustomSoundGuiButton(
                5,
                j + 14,
                ...
                10,
                10,
                ChromaFontRenderer.FontType.GUI.id + "   +",
                this
            ));
        }
        ...
        if (screen == Screen.SET) {
            newLabel = new GuiTextField(
                ChromaFontRenderer.FontType.GUI.renderer, j + 8, k + 30, 160, 16
            );
    ...
    protected void actionPerformed(GuiButton b) {
        if (b.id >= 10 && pageAge > 6) {
            ...
            ReikaPacketHelper.sendStringPacket(
                ...
                ChromaPackets.NEWTELEPORT.ordinal(),
                newLabel.getText()
            );
            ...
            if (screen == Screen.MINIMAP)
                ...
                AbilityHelper.instance.gotoWarpPoint(this.getCurrentSelected().label,
            player);
             */
            if (screen == Screen.MINIMAP) {
                ...
                ReikaPacketHelper.sendDataPacket(
                    ChromatiCraft.packetChannel,
                    ChromaPackets.MAPTELEPORT.ordinal(),
                    PacketTarget.server,
                ...
                ReikaPacketHelper.sendStringPacket(
                    ChromatiCraft.packetChannel,
                    ChromaPackets.TELEPORT.ordinal(),
                    this.getCurrentSelected().label
                );
            player.closeScreen();
        } else if (b.id == 2 && this.getCurrentSelected() != null) {
            AbilityHelper.instance.removeWarpPoint(
                this.getCurrentSelected().label, player
            );
            ReikaPacketHelper.sendStringPacket(
                ChromatiCraft.packetChannel,
                ChromaPackets.DELTELEPORT.ordinal(),
                this.getCurrentSelected().label
            );
        } /*
         else if (b.id == 3) {
            ...
            listOffset = Math.min(
                this.maxListOffset(), listOffset + (GuiScreen.isShiftKeyDown() ? 10 : 1)
            );
            return;
        }
        this.initGui();
    ...
    private boolean isValidPoint() {
        return !newLabel.getText().isEmpty()
            && !AbilityHelper.instance.playerCanWarpTo(player, new WorldLocation(player));
    }
    ...
    public void drawScreen(int x, int y, float ptick) {
        ...
        ReikaTextureHelper.bindTexture(
            ChromatiCraft.class, "Textures/GUIs/teleports.png"
        );
        ...
        if (screen == Screen.SELECT || screen == Screen.MINIMAP) {
            ...
            for (int i = 0; i < sz; i++) {
                ...
                fr.drawString(
                    ...
                    dx,
                    dy,
                    (i + listOffset) == selection ? 0x00ff00 : 0xffffff
                );
            }
        }
        ...
        if (screen == Screen.SET) {
            ...
            ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                fr, newLabel.getText(), tx, k + 60, 0xffffff
            );
            ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                fr,
                "[" + new WorldLocation(player).toString() + "]",
                tx,
                k + 60 + fr.FONT_HEIGHT + 2,
                0xffffff
            );
        }
