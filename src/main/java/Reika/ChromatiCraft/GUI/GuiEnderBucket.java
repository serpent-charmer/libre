...
public class GuiEnderBucket extends GuiChromaTool {
    ...
    public GuiEnderBucket(EntityPlayer ep) {
        super(ep);
        links = new SubviewableList(
            this.getItem().getLinks(player.getCurrentEquippedItem(), player), PER_PAGE
        );
    }
    ...
    public void initGui() {
        ...
        buttonList.add(new CustomSoundImagedGuiButton(
            1000, j + 8, k + 16 + h, 180, 8, 0, 192, tex, ChromatiCraft.class, this
        ));
        buttonList.add(new CustomSoundImagedGuiButton(
            1001, j + 8, k + 150, 180, 8, 0, 200, tex, ChromatiCraft.class, this
        ));
        ...
        buttonList.add(new CustomSoundGuiButton(
            ...
            80,
            20,
            this.getCurrentMode().displayName + " Mode",
            this
        ));
        buttonList.add(
            new CustomSoundGuiButton(1003, j + 103, k + 18, 80, 20, "Remove", this)
        );
    }
    ...
    protected void actionPerformed(GuiButton b) {
        ...
        if (b.id < 1000) {
            ReikaPacketHelper.sendDataPacket(
                ...
                ChromaPackets.ENDERBUCKETLINK.ordinal(),
                PacketTarget.server,
                links.getAbsoluteIndex(b.id)
            );
            //player.closeScreen();
        }
        int id = b.id - 1000;
        switch (id) {
            ...
            case 2:
                ...
                ReikaPacketHelper.sendDataPacket(
                    ChromatiCraft.packetChannel,
                    ChromaPackets.ENDERBUCKETMODE.ordinal(),
                    PacketTarget.server
                );
                break;
            case 3:
                ReikaPacketHelper.sendDataPacket(
                    ChromatiCraft.packetChannel,
                    ChromaPackets.ENDERBUCKETREMOVE.ordinal(),
                    PacketTarget.server,
                    this.getItem().getActiveLinkIndex(
    ...
    public void drawScreen(int x, int y, float ptick) {
        ...
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            ChromaFontRenderer.FontType.GUI.renderer, "Ender Bucket", tx, ty, 0xffffff
        );
        ...
        if (!li.isEmpty()) {
            int active = this.getItem().getActiveLinkIndex(
                player.getCurrentEquippedItem(), player
            );
            for (int i = 0; i < li.size(); i++) {
                ...
                fontRendererObj.drawString(
                    tl.getDisplayName(), j + 35, dy, sel ? 0xa0ffa0 : 0xffffff
                );
                ItemStack is = tl.getIcon();
                if (is != null) {
                    ReikaGuiAPI.instance.drawItemStack(
                        itemRender, fontRendererObj, is, j + 12, dy - 4
                    );
                }
                GL11.glDisable(GL11.GL_LIGHTING);
                ...
                if (ReikaGuiAPI.instance.isMouseInBox(j + 12, j + 186, dy - 6, dy + 14)) {
                    ReikaGuiAPI.instance.drawTooltip(
                        fontRendererObj,
                        (f != null ? f.getLocalizedName() : "Fill-Only") + " @ "
                            + tl.tank.toString()
                    );
                }
            }
        ...
        ReikaTextureHelper.bindTexture(
            ChromatiCraft.class, "Textures/GUIs/enderbucket2.png"
        );
