...
public class GuiFlightWand extends GuiChromaTool {
    ...
    public void initGui() {
        ...
        for (int i = 0; i < l; i++) {
            ...
            GuiButton b = new CustomSoundImagedGuiButton(
                i, j + 8, k + 14 + i * 40, 18, 18, u, 0, tex, ChromatiCraft.class, this
            );
    ...
    protected void actionPerformed(GuiButton b) {
        ...
        ReikaPacketHelper.sendDataPacket(
            ...
            ChromaPackets.HOVERWAND.ordinal(),
            PacketTarget.server,
            b.id
        );
        this.initGui();
    ...
    public void drawScreen(int x, int y, float ptick) {
        ...
        ReikaTextureHelper.bindTexture(
            ChromatiCraft.class, "Textures/GUIs/hoverwand.png"
        );
        ReikaGuiAPI.instance.drawTexturedModalRect(j, k, 0, 0, xSize, ySize);
        int tx = j + xSize / 2;
        int ty = k + 4;
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            ChromaFontRenderer.FontType.GUI.renderer, "Hover Selection", tx, ty, 0xffffff
        );

        int l = HoverType.list.length;
        HoverType md = this.getCurrentMode();
        for (int i = 0; i < l; i++) {
            int color = HoverType.list[i]
                            .renderColor; //md.ordinal() == i ? 0x00ff00 : 0xffffff;
            if (md.ordinal() == i) {
                color = 0xffffff;
            }
            tx = j + 30;
            ty = k + 19 + i * 40;
            ChromaFontRenderer.FontType.GUI.renderer.drawString(
                HoverType.list[i].desc, tx, ty, color
            );
        }
        GL11.glPopMatrix();
    }
