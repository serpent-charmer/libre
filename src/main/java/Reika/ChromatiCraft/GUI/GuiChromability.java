...
public class GuiChromability extends GuiScreen implements CustomSoundGui {
    ...
    public void initGui() {
        ...
        if (!abilities.isEmpty()) {
            ...
            buttonList.add(new CustomSoundImagedGuiButton(
                1, midx + out, midy - h / 2, w, h, 232, 0, tex, ChromatiCraft.class, this
            ));
        }
    ...
    public final void drawScreen(int x, int y, float f) {
        ...
        if (abilities.isEmpty()) {
            ReikaTextureHelper.bindTexture(
                ChromatiCraft.class, "Textures/GUIs/ability2.png"
            );
            this.drawTexturedModalRect(j, k, 0, 0, xSize, ySize);

            ReikaTextureHelper.bindTexture(
                ChromatiCraft.class, "Textures/Ability/unknown.png"
            );
            GL11.glPushMatrix();
            double d = 0.1953125;
            GL11.glScaled(d, d, d);
            ...
            int sp = Math.max(
                1,
                (int
                ) (360D / Math.max(1, ReikaRenderHelper.getFPS())
                   * Math.max(1, 6 * Math.abs(-(px * px) + 2 * px)))
            );
            if (dx > 0) {
                dx += sp;
            }
            ...
            if (dx != 0) { //performance boost
                if (index > 0) {
                    ...
                    fontRendererObj.drawString(
                        c.getDisplayName(), j + 63 + dx - width, k + 9, 0xffffff
                    );
                }
            ...
            fontRendererObj.drawSplitString(
                desc, j + dx + fx, k + fy, xSize - fx * 2, 0xffffff
            );
            ...
            if (dx != 0) { //performance boost
                if (index > 0) {
                    ...
                    if (!this.hasFragment(c))
                        desc = FontType.OBFUSCATED.id + desc;
                    fontRendererObj.drawSplitString(
                        desc, j + dx + fx - width, k + fy, xSize - fx * 2, 0xffffff
                    );
                }
    ...
    private boolean hasFragment(Ability c) {
        return !(c instanceof Chromabilities)
            || ChromaResearchManager.instance.playerHasFragment(
                player, ChromaResearch.getPageFor((Chromabilities) c)
            );
    }

    protected String getBackTexture(Ability a) {
        return Chromabilities.playerHasAbility(player, a) ? "Textures/GUIs/ability.png"
                                                          : "Textures/GUIs/ability2.png";
    }
    ...
    private void drawPreview(Ability c, int j, int k) {
        ...
        if (dx != 0) { //performance boost
            if (index > 0) {
                c = abilities.get(index - 1);
                ReikaTextureHelper.bindTexture(
                    c.getTextureReferenceClass(), this.getTextureName(c)
                );
                x = j + 8 + dx - width;
                this.drawTexturedModalRect((int) (x / d), (int) (y / d), 0, 0, 256, 256);
            }
    ...
    private String getTextureName(Ability c) {
        return this.hasFragment(c)
            ? c.getTexturePath(!Chromabilities.playerHasAbility(player, c))
            : "Textures/Ability/unknown.png";
    }
