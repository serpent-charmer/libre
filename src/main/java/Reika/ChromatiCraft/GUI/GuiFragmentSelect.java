...
public class GuiFragmentSelect extends GuiContainer implements CustomSoundGui {
    ...
    public void initGui() {
        ...
        for (int i = 0; i < 3; i++)
            buttonList.add(
                new SelectionButton(i, j + 33 + i * (9 + SelectionButton.SIZE), k + 37)
            );
    }
    ...
    public final void drawScreen(int x, int y, float f) {
        ...
        if (selectionIndex >= 0) {
            ...
            float frac = MathHelper.clamp_float(
                1F - (selectionCountdown - time) / (float) DURATION, 0, 1
            );
            if (frac > 0) {
                //int dy = (int)(b.yPosition+(1-frac)*b.SIZE);
                //ReikaGuiAPI.instance.drawLine(b.xPosition, dy, b.xPosition+b.SIZE, dy,
                //0xffffffff);
                ...
                v5.addVertexWithUV(
                    x0, y0, 35, ico.getInterpolatedU(8), ico.getInterpolatedV(8)
                );
                double r = b.SIZE * 0.5;
                for (int i = 0; i <= 360 * frac; i += 2) {
                    ...
                    v5.addVertexWithUV(
                        ...
                        y0 + dy * r,
                        35,
                        ico.getInterpolatedU(8 + 8 * dx),
                        ico.getInterpolatedV(8 + 8 * dy)
                    );
                }
                b.rollout = 1 - frac;
                v5.draw();
    ...
    protected void drawGuiContainerBackgroundLayer(float ptick, int mx, int my) {
        ...
        double a = 0.75 + 0.04 * Math.sin(System.currentTimeMillis() / 437D)
            + 0.06 * Math.cos(System.currentTimeMillis() / 639D);
    ...
    private class SelectionButton extends CustomSoundImagedGuiButton {
        ...
        public SelectionButton(int id, int x, int y) {
            super(
                ...
                TEX_SIZE - SIZE,
                TEX_SIZE - SIZE,
                "Textures/fragmentcategories.png",
                ChromatiCraft.class,
                GuiFragmentSelect.this
            );
        ...
        protected void renderButton() {
            ...
            int d1 = 6;
            v5.addVertexWithUV(
                xPosition - d1, yPosition - d1 + height + d1 * 2, zLevel, 0, 1
            );
            v5.addVertexWithUV(
                xPosition - d1 + width + d1 * 2,
                yPosition - d1 + height + d1 * 2,
                zLevel,
                1,
                1
            );
            v5.addVertexWithUV(
                xPosition - d1 + width + d1 * 2, yPosition - d1, zLevel, 1, 0
            );
            ...
            if (s != null && s.fragment != null) {
                s.fragment.drawTabIcon(
                    itemRender, (int) (xPosition / sc), (int) (yPosition / sc)
                );
            }
            GL11.glDepthMask(true);
            GL11.glPopMatrix();
            ...
            //GL11.glColor4f(ReikaColorAPI.getRed(c)/255F, ReikaColorAPI.getGreen(c)/255F,
            //ReikaColorAPI.getBlue(c)/255F, 1);
            double dd = 100;
            ...
            v5.addVertexWithUV(
                xPosition + 0, yPosition + height, zLevel, (u + 0) * f, (v + height) * f
            );
            ...
            v5.addVertexWithUV(
                xPosition + width, yPosition + 0, zLevel, (u + width) * f, (v + 0) * f
            );
            v5.setColorOpaque_I(c4);
            v5.addVertexWithUV(
                xPosition + 0, yPosition + 0, zLevel, (u + 0) * f, (v + 0) * f
            );
            v5.draw();
            zLevel = z;
            GL11.glEnable(GL11.GL_BLEND);
            ...
            int gs = (int
               ...
               * (0.5
                  + 0.5
                      * MathHelper.sin(
                          ((t * 4) % 50000) + System.identityHashCode(this) % 50000
                      )));
