...
public class GuiProgressByLevel extends GuiProgressStages {
    private final Map<ProgressLink, ProgressTier> levels
        = ProgressionManager.instance.constructResearchLevelDepthMap();
    ...
    public void drawScreen(int x, int y, float f) {
        ...
        for (ProgressTier pt : tiers.keySet()) {
            ...
            ResearchLevel at
                = ChromaResearchManager.instance.getPlayerResearchLevel(player);
            int clr = pt.level.isAtLeast(at) ? 0xff0000 : 0x00ff00;
            if (at == pt.level)
                clr = ReikaColorAPI.mixColors(
                    0x22aaff,
                    0xffffff,
                    (float) (0.5 + 0.5 * Math.sin(System.currentTimeMillis() / 120D))
                );
