...
public class GuiPoolRecipe extends GuiBookSection {
    private static final Comparator<PoolRecipe> recipeSorter
        = new Comparator<PoolRecipe>() {
              @Override
    ...
    public void initGui() {
        ...
        if (ModList.APPENG.isLoaded()
            && ChromaResearchManager.instance.playerHasFragment(
                player, ChromaResearch.CHROMACRAFTER
            )) {
            if (this.getActiveRecipe().allowDoubling())
                buttonList.add(new CustomSoundGuiButton(
                    4, j + xSize - 27 - 20 - 48, k - 2, 20, 20, " ", this
                ));
            buttonList.add(new CustomSoundGuiButton(
                5, j + xSize - 27 - 20 - 28, k - 2, 20, 20, " ", this
            ));
        }
        if (ReikaObfuscationHelper.isDeObfEnvironment())
            buttonList.add(
                new CustomSoundGuiButton(6, j + xSize - 27, k + 18, 20, 20, " ", this)
            );
            ...
            if (button.id == 0 && index > 0) {
                ...
                if (pr != null)
                    ReikaPacketHelper.sendStringIntPacket(
                        ...
                        ChromaPackets.ALLOYPATTERN.ordinal(),
                        PacketTarget.server,
                        pr.ID,
                        button.id == 5 && this.getActiveRecipe().allowDoubling() ? 1 : 0
                    );
            } else if (button.id == 6) {
                PoolRecipe pr = this.getActiveRecipe();
                if (pr != null)
                    ReikaPacketHelper.sendStringPacket(
                        ChromatiCraft.packetChannel,
                        ChromaPackets.ALLOYITEMS.ordinal(),
                        pr.ID,
                        PacketTarget.server
                    );
            }
    ...
    protected void drawAuxData(int posX, int posY) {
        ChromaBookData.drawPoolRecipe(
            fontRendererObj, ri, this.getActiveRecipe(), subpage, posX, posY
        );
    }
        ...
        if (ModList.APPENG.isLoaded()
            ...
            if (this.getActiveRecipe().allowDoubling())
                api.drawItemStack(
                    itemRender,
                    fontRendererObj,
                    new ItemStack(AppEngHandler.getInstance().getEncodedPattern()),
                    posX + xSize - 27 - 16 - 48,
                    posY + 8
                );
            api.drawItemStack(
