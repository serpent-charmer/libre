...
public class GuiFragmentRecovery extends ChromaBookGui {
    ...
    protected void mouseClicked(int x, int y, int b) {
        ...
        if (r != null) {
            ...
            if (cr || ink >= 0) {
                int paper
                    = cr ? -1 : ReikaInventoryHelper.locateInInventory(Items.paper, inv);
                if (cr || paper >= 0) {
                    this.giveResearch(r);
                }
            }
    ...
    public void drawScreen(int x, int y, float f) {
        ...
        ReikaTextureHelper.bindTexture(
            ChromatiCraft.class, "Textures/GUIs/Handbook/navbcg.png"
        );
        ...
        for (int i = 0; i < missingVisible.size() && i < 35; i++) {
            ...
            if (api.isMouseInBox(dx * 2, dx * 2 + 32, dy * 2, dy * 2 + 32)) {
                api.drawRectFrame(
                    ...
                    32,
                    32,
                    CrystalElement.getBlendedColor(this.getGuiTick(), 300)
                );
                fontRendererObj.drawString(
                    r.getTitle() + " (" + r.level.getDisplayName() + ")",
                    j + 8,
                    k + 196,
                    0xffffff
                );
    ...
    private void giveResearch(ChromaResearch r) {
        ReikaPacketHelper.sendPacketToServer(
            ChromatiCraft.packetChannel, ChromaPackets.RERESEARCH.ordinal(), r.ordinal()
        );
        missing.remove(r);
        this.recalculateMissingVisible();
    }
    ...
    private void recalculateMissingVisible() {
        ...
        Collections.sort(
            missingVisible, ChromaResearchManager.instance.researchComparator
        );
    }
    ...
    private boolean isVisible(ChromaResearch r) {
        ...
        if (r.level != ResearchLevel.ENTRY) {
            Collection<ChromaResearch> lvl
                = ChromaResearchManager.instance.getResearchForLevelAndBelow(r.level.pre()
                );
