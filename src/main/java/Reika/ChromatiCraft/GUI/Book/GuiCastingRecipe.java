...
public class GuiCastingRecipe extends GuiBookSection {
    ...
    public GuiCastingRecipe(
        EntityPlayer ep, ArrayList<CastingRecipe> out, int offset, boolean nei
    ) {
    ...
    public void initGui() {
        ...
        if (ChromaResearchManager.instance.playerHasFragment(player, ChromaResearch.AUTO))
            buttonList.add(new CustomSoundGuiButton(
                4, j + xSize - 27 - 20 - 28, k - 2, 20, 20, " ", this
            ));

        if (recipes.size() > 1) {
            buttonList.add(new CustomSoundImagedGuiButton(
                0, j + 205, k - 3, 10, 12, 183, 6, file, ChromatiCraft.class, this
            ));
            buttonList.add(new CustomSoundImagedGuiButton(
                1, j + 215, k - 3, 10, 12, 193, 6, file, ChromatiCraft.class, this
            ));
        }

        if (subpage == 0
            && this.getActiveRecipe().getItemCountsForDisplay().size() > 10) {
            buttonList.add(new CustomSoundImagedGuiButton(
                2, j + 205, k + 50, 12, 10, 100, 6, file, ChromatiCraft.class, this
            ));
            buttonList.add(new CustomSoundImagedGuiButton(
                3, j + 205, k + 60, 12, 10, 112, 6, file, ChromatiCraft.class, this
            ));
        }

        if (subpage == 1
            && !((RuneTempleRecipe) this.getActiveRecipe())
                    .getRunePositions()
                    .isEmpty()) {
            CustomSoundImagedGuiButton b = new CustomSoundImagedGuiButton(
                SAVE_AND_EXIT + 1,
                j + xSize,
                k + 45,
                22,
                ...
                protected void renderButton() {
                    ...
                    IIcon ico
                        = CrystalElement
                              .elements[(int) ((System.currentTimeMillis() / 1000L) % 16)]
                              .getGlowRune();
                    ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                        xPosition + 1, yPosition + 11, ico, 16, 16
                    );
            ...
            if (x >= j && y >= k && x < j + xSize && y < k + ySize) {
                ...
                if (is != null) {
                    ReikaSoundHelper.playClientSound(
                        ChromaSounds.GUICLICK, player, 0.33F, 1
                    );
    ...
    protected void actionPerformed(GuiButton button) {
        if (System.currentTimeMillis() - buttoncooldown >= 50) {
            if (button.id == 0 && index > 0) {
                ...
                index--;
                while (
                    GuiScreen.isShiftKeyDown() && index > 0
                    && ReikaItemHelper.matchStacks(is, this.getActiveRecipe().getOutput())
                ) {
                    index--;
                }
                recipeTextOffset = 0;
                ...
                index++;
                while (GuiScreen.isShiftKeyDown() && index < recipes.size() - 1
                       && cr.shouldGroupAsRecipe(is1, this.getActiveRecipe().getOutput())
                ) {
                    index++;
                }
                recipeTextOffset = 0;
    ...
    protected void drawAuxData(int posX, int posY) {
        ...
        ChromaBookData.drawCastingRecipe(
            fontRendererObj, ri, this.getActiveRecipe(), subpage, posX, posY, true
        );
    }
    ...
    protected void drawAuxGraphics(int posX, int posY) {
        if (ChromaResearchManager.instance.playerHasFragment(player, ChromaResearch.AUTO))
            api.drawItemStack(
                itemRender,
                fontRendererObj,
                ChromaTiles.AUTOMATOR.getCraftedProduct(),
                posX + xSize - 27 - 16 - 28,
                posY + 7
            );
        ...
        if (subpage == 0) {
            ...
            for (int i = recipeTextOffset; i < li.size(); i++) {
                ...
                FontRenderer fr = s0.contains(FontType.OBFUSCATED.id)
                    ? FontType.OBFUSCATED.renderer
                    : fontRendererObj;
        ...
        if (subpage == 3) {
            ...
            ((PylonRecipe)this.getActiveRecipe()).getRequiredAura(); int i = 0;
            ...
            for (CrystalElement e : set) {
                ...
                String s2 = String.format("%d %s", tag.getValue(e), wrap ? "L" :
            "Lumens"); int color = ReikaColorAPI.mixColors(e.getColor(), 0xffffff, 0.5F);
        ...
        if (!this.getActiveRecipe().canRunRecipe(null, player)) {
            ...
            api.drawTexturedModelRectFromIcon(
                posX + 9, posY + 13, ChromaIcons.NOENTER.getIcon(), 16, 16
            );
            GL11.glPopAttrib();
            if (api.isMouseInBox(posX + 9, posX + 25, posY + 13, posY + 29)) {
                ChromaBookData.drawRecipeMissingProgress(
    ...
    public String getPageTitle() {
        String s = this.getActiveRecipe().canRunRecipe(null, player)
            ? ""
            : ChromaFontRenderer.FontType.OBFUSCATED.id;
        return s
            + this.getActiveRecipe().getOutputForDisplay().getDisplayName(
            ); //+" Casting";
    }
