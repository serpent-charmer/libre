...
public class GuiNotes extends ChromaBookGui {
    ...
    public GuiNotes(EntityPlayer ep) {
        ...
        if (held.stackTagCompound != null && held.stackTagCompound.hasKey("notes")) {
            NBTTagList tag
                = held.stackTagCompound.getTagList("notes", NBTTypes.STRING.ID);
    ...
    public void onGuiClosed() {
        ReikaPacketHelper.sendDataPacket(
            ...
            ChromaPackets.BOOKNOTESRESET.ordinal(),
            PacketTarget.server
        );
        for (String s : data) {
            ReikaPacketHelper.sendStringPacket(
                ChromatiCraft.packetChannel, ChromaPackets.BOOKNOTE.ordinal(), s
            );
    ...
    public void initGui() {
        ...
        for (int i = 0; i < LINES; i++) {
            input[i] = new NoteTextField(
                ChromaFontRenderer.FontType.LEXICON.renderer,
                j + 8,
                k + 3 + i * 20,
                240,
                19
    ...
    private static class NoteTextField extends GuiTextField {
        ...
        public void drawTextBox() {
            ...
            String s = renderer.trimStringToWidth(
                text.substring(lineScrollOffset), this.getWidth()
            );
            ...
            boolean flag2 = this.getCursorPosition() < text.length()
                || text.length() >= this.getMaxStringLength();
            int k1 = j1;
            ...
            if (flag1) {
                if (flag2)
                    Gui.drawRect(
                        k1, i1 - 1, k1 + 1, i1 + 1 + renderer.FONT_HEIGHT, -3092272
                    );
                else
            ...
            if (k != j) {
                ...
                this.drawCursorVertical(
                    k1, i1 - 1, l1 - 1, i1 + 1 + renderer.FONT_HEIGHT
                );
            }
        }
    }
