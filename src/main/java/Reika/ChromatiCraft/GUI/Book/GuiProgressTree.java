...
public class GuiProgressTree extends GuiProgressStages {
    ...
    { Map<ProgressLink, Integer> ret = new HashMap(); for (Entry<ProgressLink, Integer> e
    : map.entrySet()) { ProgressLink p = e.getKey(); int l = e.getValue(); ResearchLevel
    rl = ChromaResearchManager.instance.getEarliestResearchLevelRequiring(p.parent);
            //if (rl != null)
            //l += rl.ordinal()*2;
            ret.put(p, l);
        }
    ...
    private void renderLine(int posX, int posY, ProgressLink p) {
        ...
        for (ProgressLink par : c) {
            ...
            ImmutablePair<java.awt.Point, java.awt.Point> ps = ReikaVectorHelper.clipLine(
                x1,
                x2,
                y1,
                y2,
            ...
            if (ps != null) {
                int clr = p.parent == this.getActive() || par.parent == this.getActive()
                    ? (par.parent.isPlayerAtStage(player) ? 0x00ff00 : 0xff4040)
                    : 0xffffff;
