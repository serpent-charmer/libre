...
public class GuiTransitionWand extends GuiChromaTool {
    ...
    protected void actionPerformed(GuiButton b) {
        ...
        if (item == null)
            return;
        this.getItem().setMode(
            player.getCurrentEquippedItem(), TransitionMode.list[b.id]
        );
        ReikaPacketHelper.sendDataPacket(
            ChromatiCraft.packetChannel,
            ChromaPackets.TRANSITIONWAND.ordinal(),
            PacketTarget.server,
            b.id
        );
        this.initGui();
    ...
    public void drawScreen(int x, int y, float ptick) {
        ...
        ReikaTextureHelper.bindTexture(
            ChromatiCraft.class, "Textures/GUIs/transition2.png"
        );
        ...
        ReikaGuiAPI.instance.drawCenteredStringNoShadow(
            ...
            "Transition Wand Mode Selection",
            tx,
            ty,
            0xffffff
        );
        ...
        for (int i = 0; i < l; i++) {
            ...
            tx = j + 8 + i * w;
            ty = k + 33;
            ChromaFontRenderer.FontType.GUI.renderer.drawSplitString(
                TransitionMode.list[i].desc, tx, ty, w, color
            );
        }
        GL11.glPopMatrix();
    }
