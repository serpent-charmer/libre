...
public class GuiAbilitySelect extends GuiScreen {
    ...
    public GuiAbilitySelect(EntityPlayer ep) {
        ...
        while (it.hasNext()) {
            ...
            if (a instanceof Chromabilities) {
                ...
                if (ChromaOptions.ABILITYSHOWONLY.getState()
                    && !Chromabilities.playerHasAbility(ep, a)) {
                    it.remove();
                    break;
                }
            }
    ...
    protected final void keyTyped(char c, int key) {
        ...
        if (key == Keyboard.KEY_ESCAPE
            || (ChromaOptions.KEYBINDABILITY.getState()
                && key == ChromaClient.key_ability.getKeyCode())) {
            player.closeScreen();
            ...
            if (ability != null && data < Chromabilities.maxPower(player, ability)
                && Chromabilities.playerHasAbility(player, ability))
                data++;
        }
    ...
    public final void drawScreen(int x, int y, float f) {
        ...
        ReikaTextureHelper.bindTexture(
            ChromatiCraft.class, "Textures/GUIs/abilityselect.png"
        );
        int tx = width / 2 - 128;
        ...
        if (ability != null && ability.getMaxPower() > 0) {
            ...
            this.drawTexturedModalRect(
                width / 2 - 12, height / 2 + 5 - sc, 116, 76 - sc, 24, sc
            );
            ReikaGuiAPI.instance.drawCenteredStringNoShadow(
                FontType.GUI.renderer,
                String.format("%d/%d", data, ability.getMaxPower()),
                width / 2,
                height / 2 - 72,
                0xffffff
            );
        ...
        boolean ctrl
            = GuiScreen.isCtrlKeyDown() || Keyboard.isKeyDown(Keyboard.KEY_LSHIFT);
        if (move > 0) {
            if (ability != null && ctrl) {
                if (data < Chromabilities.maxPower(player, ability)
                    && Chromabilities.playerHasAbility(player, ability))
                    data++;
            } else {
                this.scrollRight(step);
        ...
        double sp = Math.min(10, 270D / Math.max(1, ReikaRenderHelper.getFPS()))
            * Math.max(1, Math.abs(offset - dx) / 4) / 5D;
        if (offset > dx) {
            offset -= sp;
        }
        if (offset < dx) {
        ...
        for (int i = 0; i < AbilityHotkeys.SLOTS; i++) {
            ...
            if (kb != null) {
                ...
                fontRendererObj.drawString(
                    "Press " + disp + " to save this ability to hotkey slot " + i,
                    8,
                    8 + i * (fontRendererObj.FONT_HEIGHT + 1),
                    0xffffff
                );
                ...
                if (a != null) {
                    ...
                    ReikaTextureHelper.bindTexture(
                        ChromatiCraft.class, "Textures/NoAbilityHotKey.png"
                    );
                }
                this.drawTexturedModalRect(ix, iy, 0, 0, 256, 256);
                GL11.glPopMatrix();
                fontRendererObj.drawString(
                    String.valueOf(i), (int) (ix * sz) - 6, (int) (iy * sz), 0xffffff
                );
                fontRendererObj.drawString(
                    disp,
                    (int) (ix * sz) + 13 - fontRendererObj.getStringWidth(disp) / 2,
                    (int) (iy * sz) + 27,
                    0xffffff
                );
            }
    ...
    protected void mouseClicked(int x, int y, int button) {
        ...
        if (button == 0
            && ReikaGuiAPI.instance.isMouseInBox(
                width / 2 - 37, width / 2 + 37, height / 2 - 37, height / 2 + 37
            )) {
            this.selectAbility();
        }
    }
