...
public class TileEntityItemFabricator
    extends InventoriedCrystalReceiver implements OperationInterval {
    ...
    private static class Recipe {
        ...
        private static int duration(ElementTagCompound energy) {
            return ReikaMathLibrary.roundUpToX(
                20, (int) Math.sqrt(energy.getTotalEnergy())
            );
        }
    }
        ...
        if (out == null) {
            ...
            if (rec != null) {
                if (rec.hasProgress(this.getPlacer())) {
                    recipe = f != null ? new FluidRecipe(rec.getCost(), out, f)
                                       : new Recipe(rec.getCost(), out);
                    this.onRecipeChanged(last);
                }
            }
        }
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        //ReikaJavaLibrary.pConsole(recipe.energy+" <"+craftingTick+"> "+energy,
        //Side.SERVER);
    ...
    private void checkAndRequest() {
        ...
        for (CrystalElement e : recipe.energy.elementSet()) {
            int total = (int
            ) Math.min(this.getMaxStorage(e), 24L * recipe.energy.getValue(e) * n);
    ...
    private boolean canCraft() {
        ...
        if (inv[1] == null)
            return true;
        return ReikaItemHelper.matchStacks(inv[1], recipe.output)
            && inv[1].stackSize + recipe.output.stackSize <= inv[1].getMaxStackSize();
    }
    ...
    private boolean canExportFluid(Fluid f) {
        ...
        return te instanceof IFluidHandler
            && ((IFluidHandler) te)
                   .fill(ForgeDirection.UP, new FluidStack(f, 1000), false)
            == 1000;
    }
    ...
    private void craft(World world, int x, int y, int z) {
        if (recipe instanceof FluidRecipe) {
            ...
            ((IFluidHandler) te)
                .fill(
                    ForgeDirection.UP,
                    new FluidStack(((FluidRecipe) recipe).fluid, 1000),
                    true
                );
        } else {
            int n = 1;
            ...
            if (ep != null && !ReikaPlayerAPI.isFake(ep)
                ...
                n = 2;
            ReikaInventoryHelper.addOrSetStack(
                ReikaItemHelper.getSizedItemStack(recipe.output, n), inv, 1
            );
        }
    ...
    public OperationState getState() {
        return recipe == null
            ...
            : (energy.containsAtLeast(recipe.energy) ? OperationState.RUNNING
