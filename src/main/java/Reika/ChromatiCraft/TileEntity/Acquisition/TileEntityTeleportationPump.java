...
public class TileEntityTeleportationPump
    extends ChargedCrystalPowered implements IFluidHandler, OwnedTile, ChunkLoadingTile {
    ...
    private static final Comparator<FluidSource> comparator
        = new Comparator<FluidSource>() {
              @Override
    ...
    public int getFluidCount(Fluid f) {
        return counts.containsKey(f)
            ? counts.get(f)
            : 0; //fluids.containsKey(f) ? fluids.get(f).size() : 0;
    }
                ...
                for (int i = -r; i <= r; i++) {
                    for (int k = -r; k <= r; k++) {
                        ...
                        if (fs != null) {
                            ...
                            //"+dx+","+dy+","+dz, Side.SERVER,
                            //f.getName().startsWith("o"));
                        }
                    }
                }
                scanY++;
            ...
            if (selected != null) {
                ...
                if (li != null && !li.isEmpty()) {
                    ...
                    if (src.matchInWorld(world)) {
                        if (this.canAddFluid(src.fluid.amount, selected)
                            && this.hasEnergy(required)) {
                    ...
                    if (!tank.isEmpty()) {
                        for (int k = 0; k < 6; k++) {
                            ...
                            if (te instanceof IFluidHandler) {
                                ...
                                int amt
                                    = ifl.fill(dir.getOpposite(), tank.getFluid(), true);
                                if (amt > 0) {
    ...
    private void fillBucket(Fluid f) {
        if (inv[1] != null) {
            ...
            ItemStack is = FluidContainerRegistry.fillFluidContainer(
                new FluidStack(f, amt), inv[1]
            );
            if (is != null) {
                inv[1] = is;
                tank.removeLiquid(amt);
    ...
    private void doScanFX(World world, int x, int y, int z) {
        ...
        if (Minecraft.getMinecraft().gameSettings.particleSetting == 2) {
            ...
            fx.setIcon(ChromaIcons.FADE_RAY)
                .setGravity((float) ReikaRandomHelper.getRandomBetween(0.03125F, 0.0625F))
                .setColor(0xC227FF);
    ...
    private void addFluidBlock(int x, int y, int z, FluidStack fs) {
        ...
        //ReikaJavaLibrary.pConsole("Found "+fs.amount+" of "+f.getName()+" @ "+x+",
        //"+y+", "+z+", now have "+counts.get(f)+" mB");
    }
    ...
    private static class FluidSource {
        ...
        public boolean matchInWorld(World world) {
            return ReikaFluidHelper.lookupFluidForBlock(location.getBlock(world))
                == fluid.getFluid();
        }
    }
}
