...
public class TileEntityCollector
    extends FluidIOInventoryBase implements OwnedTile, ChromaExtractable {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!input.isEmpty() && !this.hasRedstoneSignal()) {
            ...
            if (fs != null) {
                ...
                int produce
                    = Math.min(speed, input.getLevel() / fs.amount / XP_PER_CHROMA);
                if (produce > 0) {
    ...
    private void internalizeXP() {
        if (inv[0] != null && inv[0].getItem() == Items.experience_bottle
    ...
    private void intakeXPFromPlayer(EntityPlayer ep, boolean doAABB) {
        if (ep != null
            && !(
                doAABB ? ReikaPlayerAPI.isFakeOrNotInteractable(
                    ep, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, 2
                )
                       : ReikaPlayerAPI.isFake(ep)
            )) {
    ...
    public boolean isValidFluid(Fluid f) {
        ...
        if (f.equals(FluidRegistry.getFluid("xpjuice")))
            return true;
        if (ReikaXPFluidHelper.fluidsExist()
            && f.equals(ReikaXPFluidHelper.getFluid().getFluid()))
            return true;
        return false;
    }
