...
public class TileEntityCrystalRepeater extends CrystalTransmitterBase
    implements LinkWatchingRepeater, RegionalSensitiveRepeater, CrystalFuse, NBTTile,
               SneakPop, OwnedTile, MultiBlockChromaTile {
    ...
    public void onAdjacentBlockUpdate() {
        redstoneCache = worldObj.isBlockIndirectlyGettingPowered(
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote)
            this.setState(
                StateFlags.RAINLOSS,
                this.hasState(StateFlags.RAINABLE) && world.isRaining()
            );
        ...
        if (world.isRemote && this.canConduct()) {
            ...
            ProgressionCatchupHandling.instance.attemptSync(
                this, 12, ProgressStage.REPEATER
            );
        }
    ...
    private void doRainParticles(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int c = ReikaColorAPI.mixColors(
                c1, 0xffffff, (float) ReikaRandomHelper.getRandomPlusMinus(0.5, 0.2)
            );
    ...
    private void doEnhancedStructureParticles(World world, int x, int y, int z) {
        ...
        if (d < 1024 && (d < 256 || ReikaRandomHelper.doWithChance(100 * 256 / d))) {
            ...
            EntityFX fx = new EntityCCBlurFX(world, dx, dy, dz, vx, vy, vz)
                              .setColor(c)
                              .setNoSlowdown()
                              .setScale(s);
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
            EntityFX fx2 = new EntityCCBlurFX(world, dx, dy, dz, vx, vy, vz)
                               .setColor(0xffffff)
                               .setNoSlowdown()
                               .setScale(s / 2.5F)
                               .lockTo(fx);
    ...
    public final void validateStructure() {
        ...
        this.setState(
            StateFlags.ENHANCEDSTRUCT,
            this.hasStructure() && this.isTurbocharged() && this.checkEnhancedStructure()
        );
    ...
    protected boolean checkForStructure() {
        ...
        for (int i = 2; i < 4; i++) {
            ...
            int m2
                = (i == 3 && this.isTurbocharged()) ? StoneTypes.RESORING.ordinal() : 0;
            if (id != ChromaBlocks.PYLONSTRUCT.getBlockInstance()
                || (meta != 0 && meta != m2))
                return false;
        }
        return true;
    ...
    public int maxThroughput() {
        ...
        if (this.isTableGrouped())
            ...
            ret *= 1
                + Math.min(
                    0.5, clusterCount * 0.5 / 6D
                ); //25% bonus with >= 4 repeaters in a group
        return ret;
    }
    ...
    public int getSignalDegradation(boolean point) {
        ...
        if (this.isTableGrouped())
            ...
            ret *= Math.max(
                0.5, 1 - clusterCount * 0.5 / 3D
            ); //50% reduction with >= 4 repeaters in a group
        if (this.isEnhancedStructure())
            ret /= 2;
        return ret;
    }
    ...
    public CrystalElement getActiveColor() {
        ...
        return this.canConduct()
            ? CrystalElement.elements[worldObj.getBlockMetadata(dx, dy, dz)]
            : null;
    }
    ...
    public CrystalSource getEnergySource() {
        CrystalElement e = this.getActiveColor();
        CrystalPath p
            = e != null ? CrystalNetworker.instance.getConnectivity(e, this) : null;
    ...
    !Chromabilities.PYLON.enabledOn(player) && rand.nextInt(60) == 0)
                p.attackEntityByProxy(player, this);
            CrystalNetworker.instance.makeRequest(this, p.getColor(), 100,
    this.getReceiveRange(), 1);
        }
    }
     */
    ...
    public void setDataFromItemStackTag(ItemStack is) {
        ...
        this.setState(
            StateFlags.TURBO,
            ReikaItemHelper.matchStacks(is, this.getTile().getCraftedProduct())
                && is.stackTagCompound != null
                && is.stackTagCompound.getBoolean("boosted")
        );
        casterID = ReikaItemHelper.matchStacks(is, this.getTile().getCraftedProduct())
                && is.stackTagCompound != null && is.stackTagCompound.hasKey("caster")
            ? UUID.fromString(is.stackTagCompound.getString("caster"))
            : null;
    }
    ...
    public final void drop() {
        //ReikaItemHelper.dropItem(worldObj, xCoord+0.5, yCoord+0.5, zCoord+0.5,
        //this.getTile().getCraftedProduct());
    ...
    public final void refreshConnectionRender() {
        if (worldObj.isRemote) {
            ...
            ReikaPacketHelper.sendDataPacketWithRadius(
                ...
                ChromaPackets.REPEATERCONN.ordinal(),
                this,
                128
            );
        }
    ...
    public int updateAndGetConnectionRenderAlpha() {
        ...
        if (HoldingChecks.REPEATER.isHolding(ep)
            && ep.getDistanceSq(xCoord + 0.5, yCoord + 0.5, zCoord + 0.5)
                < 4 + Math.min(this.getSendRange(), this.getReceiveRange())) {
            this.refreshConnectionRender();
        }
        return connectionRenderTick > 0
            ? (connectionRenderTick > 10 ? 255 : 25 * connectionRenderTick)
            : 0;
    }
        ...
        for (int i = -r; i <= r; i++) {
            for (int j = -r; j <= r; j++) {
                for (int k = -r; k <= r; k++) {
                    boolean flag = false;
                    TileEntity te
                        = worldObj.getTileEntity(xCoord + i, yCoord + j, zCoord + k);
                    if (te instanceof CrystalReceiver
                        && ((CrystalNetworkTile) te).canConduct()
                        && this.canTransmitTo((CrystalReceiver) te)) {
                        flag = true;
                    }
                    if (te instanceof CrystalTransmitter
                        && ((CrystalNetworkTile) te).canConduct()
                        && ((CrystalTransmitter) te).canTransmitTo(this)) {
                        flag = true;
                    }
                    if (flag) {
                        if (this.getDistanceSqTo(te.xCoord, te.yCoord, te.zCoord)
                            <= r * r) {
                            if (!(te instanceof CrystalReceiver
                                      ? this.needsLineOfSightToReceiver((CrystalReceiver
                                      ) te)
                                      : ((CrystalTransmitter) te)
                                            .needsLineOfSightToReceiver(this))
                                || (PylonFinder
                                        .lineOfSight(
                                            worldObj,
                                            xCoord,
    ...
    private void doSurge() {
        if (!worldObj.isRemote)
            ReikaPacketHelper.sendDataPacketWithRadius(
                ChromatiCraft.packetChannel,
                ChromaPackets.REPEATERSURGE.ordinal(),
                this,
                64,
                surgeColor.ordinal()
            );
        ...
        while (y > 0 && b == ChromaBlocks.PYLONSTRUCT.getBlockInstance()
               || b == ChromaBlocks.RUNE.getBlockInstance()) {
            ReikaWorldHelper.dropAndDestroyBlockAt(
                worldObj, xCoord, y, zCoord, null, true, true
            );
            y--;
    ...
    private void doSurgingParticles(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityBlurFX fx = new EntityCCBlurFX(
                surgeColor, worldObj, dx, dy, dz, vxyz[0], vxyz[1], vxyz[2]
            );
    ...
    overloadClient(World world, int x, int y, int z, CrystalElement e) {
        for (int i = 0; i < 256; i++) {
            double dx = x + rand.nextDouble();
            double dy = y + rand.nextDouble();
            double dz = z + rand.nextDouble();
    ...
    public final void onLinkRecalculated(CrystalLink l) {
        this.setState(
            StateFlags.RAINABLE,
            this.canBeRainAffected(l) && l.isRainable()
                && PylonFinder.isRainableBiome(
    ...
    protected boolean canBeRainAffected(CrystalLink l) {
        ...
        if (loc instanceof NaturalCrystalSource)
            return false;
        if (loc instanceof TileEntityCrystalRepeater
            && ((TileEntityCrystalRepeater) loc).isTurbocharged())
            return false;
        return true;
    }
    ...
    //I actually had an idea for the 'lumen road bonus' - have each beam's attenuation
    //come from a sort of 'punch through' cost like how trucks have to push air aside then
    //have each punch 'weaken' the air in a 5x5 centered on itself - meaning that the more
    //you have together, the more they 'share' the punch-through cost
    @Override
    public void onRegionUpdated() {
        clusterCount
            = CrystalNetworker.instance
                  .getNearTilesOfType(
                      this, TileEntityCrystalRepeater.class, this.getSensitivityRadius()
                  )
                  .size()
            - 1;
    }
