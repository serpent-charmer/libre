...
public class TileEntityCompoundRepeater
    extends TileEntityCrystalRepeater implements ConnectivityAction {
    private final EnumMap<CrystalElement, Integer> depth
    ...
    protected void onFirstTick(World world, int x, int y, int z) {
        ...
        CrystalNetworker.instance.getAllNearbyPylons(this, this.getReceiveRange()); for
        (TileEntityCrystalPylon te : c) { if (te.canConduct() &&
        PylonFinder.lineOfSight(world, x, y, z, te.xCoord, te.yCoord, te.zCoord)) {
    ...
    private void particles(World world, int x, int y, int z) {
        if (this.getColorCycleTick() % 32 == 5) {
            ...
            Minecraft.getMinecraft().effectRenderer.addEffect(
                new EntityRuneFX(world, px, py, pz, 0, 0, 0, e).setScale(5).setFading()
            );
        }
    }
    ...
    public int getColorCycleTick() {
        return (int
        ) ((worldObj.getTotalWorldTime() + xCoord / 8D % 16 + zCoord / 8D % 16) % 512D
    ...
    public boolean isConductingElement(CrystalElement e) {
        return e != null
            && this.canConduct(
            ); // && (e != CrystalElement.BLUE && e != CrystalElement.RED && e !=
               // CrystalElement.WHITE && e != CrystalElement.YELLOW && e !=
               // CrystalElement.MAGENTA && e != CrystalElement.LIME && e !=
               // CrystalElement.ORANGE);
    }

    @Override
    public int maxThroughput() { //no clustering bonuses
        return this.isTurbocharged() ? (this.isEnhancedStructure() ? 20000 : 12000)
                                     : 1000;
    }
    ...
    public int getSignalDegradation(boolean point) {
        return this.isTurbocharged()
            ? (this.isEnhancedStructure() ? (point ? 0 : 10) : 20)
            : 100;
    }
    ...
    protected boolean checkForStructure() {
        for (int i = 1; i <= 5; i++) {
            ...
            int m2b = m2 == this.getColumnBeam() && this.isTurbocharged()
                ? StoneTypes.list[m2].getGlowingVariant().ordinal()
                : m2;
    ...
    protected boolean checkEnhancedStructure() {
        for (int i = 2; i <= 4; i += 2) {
            ...
            if (b != ChromaBlocks.PYLONSTRUCT.getBlockInstance()
                || meta
                    != StoneTypes.list[this.getColumnBeam()].getGlowingVariant().ordinal(
                    )) {
                return false;
            }
        }
    ...
    public boolean checkConnectivity() {
        for (int i = 0; i < CrystalElement.elements.length; i++) {
            ...
            if (CrystalNetworker.instance.checkConnectivity(e, worldObj, xCoord, yCoord,
    zCoord, this.getReceiveRange())) return true;
        }
        return false;
    }
     */
    ...
    public CrystalElement getActiveColor() {
        return CrystalElement
            .elements[(this.getRenderColorWithOffset(7).ordinal() + 2) % 16];
    }

    /*
    @Override
    public CrystalSource getEnergySource(CrystalElement e) {
        return e != null ? CrystalNetworker.instance.getConnectivity(e, worldObj, xCoord,
    yCoord, zCoord, this.getReceiveRange()) : null;
    }*/
    ...
    !Chromabilities.PYLON.enabledOn(player) && rand.nextInt(20) == 0)
                p.attackEntityByProxy(player, this);
            CrystalNetworker.instance.makeRequest(this, p.getColor(), 15000,
