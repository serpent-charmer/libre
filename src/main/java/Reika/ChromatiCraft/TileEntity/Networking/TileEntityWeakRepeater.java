...
public class TileEntityWeakRepeater
    extends TileEntityCrystalRepeater implements DynamicRepeater, ReactiveRepeater {
    ...
    public TileEntityWeakRepeater() {
        //originalUse = ReikaRandomHelper.getRandomBetween(MAX_LUMENS_MIN,
        //MAX_LUMENS_MAX); remainingUse = originalUse;
    }
    ...
    private void doDestroyingFXClient(World world, int x, int y, int z) {
        ...
        EntityFX fx = new EntityCCBlurFX(world, dx, dy, dz)
                          .setIcon(ChromaIcons.TURBO)
                          .setRapidExpand()
                          .setColor(e.getColor())
                          .setScale(s)
                          .setLife(l);
        EntityFX fxb = new EntityCCBlurFX(world, dx, dy, dz)
                           .setIcon(ChromaIcons.TURBO)
                           .setRapidExpand()
                           .setColor(e.getColor())
                           .setScale(s / 1.125F)
                           .setLife(l);
        EntityFX fx2 = new EntityCCBlurFX(world, dx, dy, dz)
    ...
    private void doDestroyFXClient(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityFX fx = new EntityCCBlurFX(world, dx, dy, dz, vel[0], vel[1], vel[2])
                              .setRapidExpand()
                              .setColor(c1)
                              .setScale(s);
            EntityFX fx1 = new EntityCCBlurFX(world, dx, dy, dz, vel[0], vel[1], vel[2])
                               .setRapidExpand()
                               .setColor(c2)
                               .setScale(s * 0.5F)
                               .lockTo(fx);
            EntityFX fx2 = new EntityCCBlurFX(world, dx, dy, dz, vel[0], vel[1], vel[2])
                               .setIcon(ChromaIcons.TRANSFADE)
                               .setRapidExpand()
                               .setColor(c3)
                               .setScale(s * 0.25F)
        ...
        if (eolTicks > 0) {
            ...
            if (eolTicks == 0 && ReikaRandomHelper.doWithChance(f)) {
                EntityBallLightningFX fx
                    = new EntityBallLightningFX(world, x + 0.5, y + 0.5, z + 0.5, e);
            ...
            if (frac < 0.5 && ReikaRandomHelper.doWithChance(0.125 * Math.pow(f, 2))) {
                ...
                LightningBolt b = new LightningBolt(
                    new DecimalPosition(x + 0.5, y + 0.5, z + 0.5),
                    new DecimalPosition(ex, ey, ez),
                    4
                );
                b.setVariance(0.375);
                b.maximize();
                ...
                ReikaSoundHelper.playClientSound(
                    ChromaSounds.DISCHARGE,
                    x + 0.5,
                    y + 0.5,
                    z + 0.5,
                    0.125F,
                    CrystalMusicManager.instance.getRandomScaledDing(e),
                    true
                );
            }
        }
    ...
    @Override
    public void
    onTransfer(CrystalSource src, CrystalReceiver r, CrystalElement e, int amt) {
        /*
        if (remainingUse > 0) {
            remainingUse -= amt;
            if (remainingUse <= 0) {
    ...
    private boolean canSafelySupply(CrystalReceiver r) {
        return r instanceof TileEntityRelaySource || r instanceof TileEntityRitualTable
            || r instanceof TileEntityPersonalCharger;
    }
    ...
    protected boolean checkForStructure() {
        ...
        return tree != null
            || (mod != null && this.isValidWood(mod)
                && (mod.canBePlacedSideways() || dir.offsetY != 0));
    }
    ...
    public void setDataFromItemStackTag(ItemStack is) {
        ...
        //remainingUse = is.stackTagCompound != null &&
        //is.stackTagCompound.hasKey("remain") ? is.stackTagCompound.getInteger("remain")
        //: remainingUse; originalUse = is.stackTagCompound != null &&
        //is.stackTagCompound.hasKey("total") ? is.stackTagCompound.getInteger("total") :
        //originalUse;
    }
    ...
    protected void getSneakPopDrops(ArrayList<ItemStack> li) {
        if (eolTicks == 0 && !this.isRuptured()) //this.hasRemainingLife();
            ...
            li.addAll(((BlockCrystalPylon) this.getBlockType())
                          .getPieces(worldObj, xCoord, yCoord, zCoord));
    }
    ...
    public boolean canTransmitTo(CrystalReceiver r) {
        ...
        ); // && r.getResearchTier().ordinal() <= this.getResearchTier().ordinal();
    }
    ...
    private static enum RepeaterFailures {
        ...
        private void
        doEffect(World world, int x, int y, int z, TileEntityWeakRepeater te) {
            switch (this) {
                case BURN:
                    te.delete();
                    for (int i = 0; i < 6; i++) {
                        ReikaWorldHelper.ignite(
                            world,
                            x + te.dirs[i].offsetX,
                            y + te.dirs[i].offsetY,
                            z + te.dirs[i].offsetZ
                    ...
                    ReikaSoundHelper.playSoundAtBlock(
                        world, x, y, z, "random.fizz", 2, 0.5F
                    );
                    break;
                case EXPLOSION:
                    te.delete();
                ...
                case RUPTURE:
                    ...
                    for (int m = 0; m < 40; m++) {
                        ReikaParticleHelper.LAVA.spawnAt(
                            world,
                            x + rand.nextDouble(),
                            y + rand.nextDouble() * 1.5,
                            z + rand.nextDouble()
                        );
                    }
