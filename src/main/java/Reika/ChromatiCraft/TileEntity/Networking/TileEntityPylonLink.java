...
public class TileEntityPylonLink extends TileEntityChromaticBase
    ...
    private void doFX(World world, int x, int y, int z) {
        ...
        for (int i = 1; i <= 4; i++) {
            ...
            double py = y + 1 + 4.5
                + 4.5 * Math.sin(this.getTicksExisted() / 64D * (1 + i / 8D) + i * 5);
            EntityFX fx = new EntityCCBlurFX(world, px, py, pz)
                              .setColor(c)
                              .setScale(1.25F)
                              .setRapidExpand()
    ...
    public Collection<WorldLocation> getLinkedPylons() {
        ...
        return PylonLinkNetwork.instance.getLinkedPylons(
            worldObj, placerUUID, connection.getColor()
        );
    }
    ...
    public TileEntityCrystalPylon getPylon() {
        ...
        for (int i = 1; i < d; i++) {
            ...
            if (!world.getBlock(x, dy, z).isAir(world, x, dy, z)
                && !(
                    i == 1
                    && ChromaTiles.getTile(world, x, dy, z) == ChromaTiles.PYLONTURBO
                ))
                return null;
        }
