...
public class TileEntityCrystalPylon extends CrystalTransmitterBase
    implements NaturalCrystalSource, ChargingPoint, ChunkLoadingTile, INode, IWandable {
    ...
    public int getRenderColor() {
        return ReikaColorAPI.mixColors(
            color.getColor(), 0x888888, (float) energy / this.getCapacity()
        );
    }
        ...
        if (linkTile != null) {
            ...
            if (te == null)
                world.setBlock(
                    ...
                    linkTile.yCoord,
                    linkTile.zCoord,
                    ChromaTiles.PYLONLINK.getBlock(),
                    ChromaTiles.PYLONLINK.getBlockMetadata(),
                    2
                );
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (DragonAPICore.debugtest) {
            if (!hasMultiblock) {
                ...
                b.place();
                //world.setBlock(x, y+9, z, this.getTile().getBlock(),
                //this.getTile().getBlockMetadata(), 3); TileEntityCrystalPylon te =
                //(TileEntityCrystalPylon)world.getTileEntity(x, y+9, z);
                color = e;
                hasMultiblock = true;
                this.syncAllData(true);
            }
        ...
        if (!world.getBlock(x, y - 1, z).isAir(world, x, y - 1, z)
            && ReikaWorldHelper.isBlockEncased(
                world, x, y, z, null
            )) { //Someone attempting to jar
            this.doJarRejection(world, x, y, z);
            //ReikaPacketHelper.sendDataPacketWithRadius(ChromatiCraft.packetChannel,
            //ChromaPackets.PYLONJAR.ordinal(), this, 128);
        }
        ...
        long diff = world.getTotalWorldTime()
            - lastWorldTick; //1; //was commented out, not sure why
        lastWorldTick = world.getTotalWorldTime();

        if (hasMultiblock) {
            //ReikaJavaLibrary.pConsole(energy, Side.SERVER, color ==
            //CrystalElement.BLUE);
                            ...
                            if (!loc.equals(world, x, y, z)) {
                                ...
                                if (tile2 instanceof TileEntityCrystalPylon) {
                                    TileEntityCrystalPylon tp
                                        = (TileEntityCrystalPylon) tile2;
                                    if (tp.color == color) {
                                        int amt = Math.min(
                                            this.getDonatedRecharge(),
                                            tp.getCapacity() - tp.energy
                                        );
                                        tp.energy += amt;
                                        energy -= amt;
                                    }
            ...
            if (world.isRemote) {
                ...
                if (HoldingChecks.POWERCRYS.isClientHolding())
                    ChromaFX.doPlacementHintParticles(
                        world,
                        x,
                        y,
                        z,
            ...
            if (!world.isRemote && hasMultiblock && structure != null) {
                ...
                if (bk.blockID == ChromaBlocks.PYLONSTRUCT.getBlockInstance()
                    || bk.blockID == ChromaBlocks.RUNE.getBlockInstance()) {
                    c = c.offset(0, 1, 0);
            ...
            if (!world.isRemote && ModList.MYSTCRAFT.isLoaded()
                && ReikaMystcraftHelper.isMystAge(world)
                && MystPages.Pages.UNSTABLEPYLONS.existsInWorld(world)
                && rand.nextInt(2000) == 0)
                this.destabilize();
            ...
            if (!world.isRemote) {
                ...
                if (rand.nextInt(rate) == 0) {
                    boolean canAttack = world.getTotalWorldTime() - lastAttackTime
                        >= minTicksBetweenAttack;
                    if (canAttack) {
                        int r = this.getAttackRange();
                        AxisAlignedBB box
                            = ReikaAABBHelper.getBlockAABB(x, y, z).expand(r, r, r);
                ...
                for (EntityPlayer ep : ((List<EntityPlayer>) world.playerEntities)) {
                    if (ep.capabilities.isCreativeMode
                        || Chromabilities.PYLON.enabledOn(ep))
                        continue;
                    int d = (int
                    ) MathHelper.clamp_double(Math.abs(ep.posY - y - 0.5) / 6D, 1, 10);
                    if (ep.hurtResistantTime <= rand.nextInt(11 - d)
                        && ep.boundingBox.intersectsWith(box)
                        && PylonFinder.lineOfSight(world, x, y, z, ep).hasLineOfSight) {
                        ...
                        if (ep instanceof EntityPlayerMP)
                            ReikaPacketHelper.sendDataPacket(
                                ChromatiCraft.packetChannel,
                                ChromaPackets.PYLONATTACKRECEIVE.ordinal(),
                                this,
                                (EntityPlayerMP) ep,
                                this.getColor().ordinal()
                            );
                    }
                }

                //ReikaJavaLibrary.pConsole(minTicksBetweenAttack,
                //worldObj.getClosestPlayer(x, y, z, 12) != null);
                minTicksBetweenAttack
                    = Math.min(minTicksBetweenAttack + 1, MAX_ATTACK_DELAY);
            }
            ...
            if (!world.isRemote && ChromaOptions.BALLLIGHTNING.getState()
                && energy >= this.getCapacity() / 2 && rand.nextInt(1000) == 0
                && ReikaWorldHelper.isRadiusLoaded(world, x, z, 2)
                && EntityBallLightning.canSpawnHere(world, x + 0.5, y + 0.5, z + 0.5)) {
            ...
            //	for (int i = 0; i < 500; i++)
            if (!world.isRemote && hasMultiblock && energy == this.getCapacity()
                && !this.isEnhanced() && !this.isEnhancing() && !this.isUnstable()
                && this.getBoosterCrystals(world, max, y, z, false).isEmpty()) {
                ...
                //for (int i = 0; i < 6; i++) {
                Coordinate c = c1.offset(
                    ReikaRandomHelper.getRandomPlusMinus(0, r),
                    ReikaRandomHelper.getRandomPlusMinus(0, 2),
                    ReikaRandomHelper.getRandomPlusMinus(0, r)
                );
                if (c.getTaxicabDistanceTo(c1) >= 4 && !structure.hasBlock(c)
                    && c.getBlock(world) == ChromaBlocks.PYLONSTRUCT.getBlockInstance()) {
                    Coordinate c2 = c.offset(
                        ReikaDirectionHelper.getRandomDirection(true, rand), 1
                    );
    ...
    private void
    attackEntitiesInBox(World world, int x, int y, int z, AxisAlignedBB box) {
    ...
    private void tryGrowEncrusted(World world, int x, int y, int z) {
        ...
        this.tryGrowEncrustedAt(
            world, c, c2, c.getBlock(world) != ChromaBlocks.RUNE.getBlockInstance()
        );
        //}
    }

    private void
    tryGrowEncrustedAt(World world, Coordinate from, Coordinate c, boolean addToCount) {
        if (!structure.hasBlock(c)) {
            Block b = c.getBlock(world);
            if (isEncrustedGrowable(world, c, b)
                && (!addToCount || encrustedBlocks.size() < 6))
                this.growEncrustedAt(
                    world, from, c.xCoord, c.yCoord, c.zCoord, true, addToCount
                );
            else if (b == ChromaBlocks.ENCRUSTED.getBlockInstance() && c.getBlockMetadata(world) == this.getColor().ordinal())
                this.growEncrustedAt(
                    world, from, c.xCoord, c.yCoord, c.zCoord, false, addToCount
                );
        }
    }
    ...
    private void growEncrustedAt(
        ...
        if (place) {
            world.setBlock(
                ...
                y,
                z,
                ChromaBlocks.ENCRUSTED.getBlockInstance(),
                this.getColor().ordinal(),
                3
            );
    ...
    private TileEntityPylonLink getLinkTile() {
        if (linkTile != null) {
            ...
            return tile instanceof TileEntityPylonLink ? (TileEntityPylonLink) tile
                                                       : null;
        }
        return null;
    }
    ...
    private int getAttackRate(World world) {
        ...
        if (ModList.MYSTCRAFT.isLoaded() && ReikaMystcraftHelper.isMystAge(world)
            && MystPages.Pages.VIOLENTPYLONS.existsInWorld(world))
            base /= 4;
        return base;
    }
    ...
    private void doDestabilizedTick(World world, int x, int y, int z) {
        ...
        if (rand.nextInt(1000) == 0) {
            ...
            world.newExplosion(
                null,
                x + 0.5 - 1 + rand.nextInt(3),
                y + 0.5 - 1,
                z + 0.5 - 1 + rand.nextInt(3),
                4,
                true,
                true
            );
    ...
    public void sendRandomShock(World world, boolean canJumpColors, int dmg) {
        LinkedList<CrystalNetworkTile> li
            = CrystalNetworker.instance.findPathToRandomReceiverFromSource(
                this, canJumpColors && rand.nextInt(2) == 0 ? null : color, false
            );
        if (li.size() > 1) {
            ...
            p.blink(l, r);
            world.spawnEntityInWorld(
                new EntityOverloadingPylonShock(world, this, li2, sp, dmg)
            );
        }
    }

    private void
    shortCircuitWith(World world, int x, int y, int z, TileEntityCrystalPylon te) {
        double sp = EntityOverloadingPylonShock.getRandomSpeed();
        int l = (int) sp;
        this.addSelfTickingTarget(
            new WorldLocation(te),
            color,
            0,
            0,
            0,
            this.getOutgoingBeamRadius() * 2.5,
            Double.POSITIVE_INFINITY,
            l
        );
        ArrayList<Coordinate> li
            = ReikaJavaLibrary.makeListFrom(new Coordinate(this), new Coordinate(te));
    ...
    private int getAttackRange() {
        return (this.isEnhancing()      ? 16
                    : this.isEnhanced() ? 12
                                        : 8)
            + rand.nextInt(this.isEnhancing() ? 16 : 8);
    }

    private float getAttackDensity() {
        return 1F
            - (minTicksBetweenAttack - MIN_ATTACK_DELAY)
            / (float) (MAX_ATTACK_DELAY - MIN_ATTACK_DELAY);
    }

    private boolean isEnhancing() {
        if (ChromaTiles.getTile(worldObj, xCoord, yCoord - 8, zCoord)
            == ChromaTiles.PYLONTURBO) {
            TileEntityPylonTurboCharger te = (TileEntityPylonTurboCharger
            ) worldObj.getTileEntity(xCoord, yCoord - 8, zCoord);
            return te.getTick() > 0;
        }
        return false;
    }
                ...
                for (int k = -1; k <= 1; k++) {
                    if (i != 0 || j != 0 || k != 0) {
                        ...
                        ReikaSoundHelper.playBreakSound(
                            world, dx, dy, dz, world.getBlock(dx, dy, dz)
                        );
    ...
    private void doJarRejectionParticles(World world, int x, int y, int z) {
        ReikaParticleHelper.EXPLODE.spawnAroundBlockWithOutset(
            world, x, y, z, 0, 0, 0, 16, 0.25
        );
        for (int i = 0; i < 256; i++) {
            EntityFloatingSeedsFX fx = new EntityCCFloatingSeedsFX(
                world,
                x + 0.5,
                y + 0.5,
                z + 0.5,
            ...
            fx.setColor(color.getColor())
                .setScale(2 + rand.nextFloat() * 8)
                .setLife(40 + rand.nextInt(120));
    ...
    private void spawnLightning(World world, int x, int y, int z) {
        EntityBallLightningFX e
            = new EntityBallLightningFX(world, x + 0.5, y + 0.5, z + 0.5, color);
    ...
    public void destroyPowerCrystals(int n) {
        ArrayList<TileEntityChromaCrystal> crys
            = this.getBoosterCrystals(worldObj, xCoord, yCoord, zCoord, false);
    ...
    private void charge(World world, int x, int y, int z, int max, int ticks) {
        ...
        int a = ticks;
        if (energy < max) {
            ArrayList<TileEntityChromaCrystal> blocks
                = this.getBoosterCrystals(world, x, y, z, true);
    ...
    @SideOnly(Side.CLIENT)
    private void spawnRechargeParticles(
        World world, int x, int y, int z, ArrayList<TileEntityChromaCrystal> blocks
    ) {
        int i = 0;
        for (TileEntityChromaCrystal te : blocks) {
            ...
            double py = dy + 0.125;
            double pz = dz + 0.5;
            //EntityRuneFX fx = new EntityRuneFX(world, dx+0.5, dy+0.5, dz+0.5, vx, vy,
            //vz, color);
            float sc = (float
            ) (2F
               + Math.sin(
                   4 * Math.toRadians(this.getTicksExisted() + i * 90 / blocks.size())
               ));
            EntityBlurFX fx = new EntityCCBlurFX(color, world, px, py, pz, vx, vy, vz)
                                  .setScale(sc)
                                  .setLife(38)
                                  .setNoSlowdown();
            //EntityLaserFX fx = new EntityLaserFX(color, world, px, py, pz, vx, vy,
            //vz).setScale(3);
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
            i++;
        }
    ...
    public ArrayList<TileEntityChromaCrystal>
    getBoosterCrystals(World world, int x, int y, int z, boolean matchOwner) {
        ...
        for (Coordinate c : crystalPositions) {
            if (world.checkChunksExist(
                ...
                if (ChromaTiles.getTile(world, x + c.xCoord, y + c.yCoord, z + c.zCoord)
                    == ChromaTiles.CRYSTAL) {
                    TileEntityChromaCrystal te = (TileEntityChromaCrystal
                    ) world.getTileEntity(x + c.xCoord, y + c.yCoord, z + c.zCoord);
                    {
                        EntityPlayer ep = te.getPlacer();
                        if (!matchOwner
                            || (ep != null && (owner == null || ep == owner))) {
                            if (owner == null)
                                owner = ep;
                            li.add(te);
                        }
    ...
    public boolean isValidPowerCrystal(TileEntityChromaCrystal te) {
        return crystalPositions.contains(
            new Coordinate(te).offset(-xCoord, -yCoord, -zCoord)
        );
    }
    ...
    public void onPowerCrystalBreak(TileEntityChromaCrystal te) {
        ...
        worldObj.addWeatherEffect(new EntityLightningBolt(
            worldObj, te.xCoord + 0.5, te.yCoord + 0.5, te.zCoord + 0.5
        ));
        AxisAlignedBB box
            = ReikaAABBHelper.getBlockAABB(xCoord, yCoord, zCoord).expand(24, 16, 24);
        List<EntityLivingBase> li
            = worldObj.getEntitiesWithinAABB(EntityLivingBase.class, box);
        for (EntityLivingBase e : li) {
            if (e instanceof EntityPlayer) {
                if (((EntityPlayer) e).capabilities.isCreativeMode) {
                    ...
                    float amt = Math.max(
                        5, Math.min(e.getHealth() - 4, e.getMaxHealth() * 0.75F)
                    );
                    ChromaAux.doPylonAttack(color, e, amt);
                    ChromaSounds.DISCHARGE.playSound(
                        e.worldObj, e.posX, e.posY, e.posZ, 1, 1
                    );
                }
            } else {
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ChromatiCraft.packetChannel,
            ChromaPackets.PYLONCRYSTALBREAK.ordinal(),
            this,
            64
        );
        this.syncAllData(true);
    ...
    public void doPowerCrystalBreakFX(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityFX fx = new EntityCCFloatingSeedsFX(
                              world,
                              x + 0.5,
                              y + 0.5,
                              z + 0.5,
        ...
        ReikaSoundHelper.playClientSound(
            ChromaSounds.DISCHARGE, x + 0.5, y + 0.5, z + 0.5, 1, 1, false
        );
    }
    ...
    void attackEntity(EntityLivingBase e, boolean sound) {
        ...
        ChromaAux.doPylonAttack(
            color,
            e,
            Math.max(this.isEnhanced() ? 10 : 5, e.getHealth() / 4F),
            true,
            0,
            true
        );
        ...
        PotionEffect eff
            = CrystalPotionController.instance.getEffectFromColor(color, 200, 2, false);
        if (eff != null) {
            e.addPotionEffect(eff);
        }

        if (e instanceof EntityPlayer) {
            minTicksBetweenAttack = Math.max(
                minTicksBetweenAttack - ReikaRandomHelper.getRandomBetween(18, 60),
                MIN_ATTACK_DELAY
            );
    ...
    private void sendClientAttack(CrystalTransmitter te, EntityLivingBase e) {
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ChromatiCraft.packetChannel,
            ChromaPackets.PYLONATTACK.ordinal(),
            te.getWorld(),
            tx,
            ty,
            tz,
    ...
    public void validateMultiblock(FilledBlockArray struct) {
        ...
        broadcast = !worldObj.isRemote
            && ChromaStructures.PYLONBROADCAST
                   .getArray(worldObj, xCoord, yCoord, zCoord, color)
                   .matchInWorld();
    ...
    private void spawnParticle(World world, int x, int y, int z) {
        ...
        if (rand.nextInt(1 + p / 2) == 0) {
            ...
            while (n > 0) {
                ...
                EntityFlareFX fx
                    = new EntityFlareFX(color, world, rx, ry, rz, 0.6F + dt * 0.15F);
        ...
        if (this.isEnhanced()) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                fx.fadeColors(
                      ReikaColorAPI.mixColors(color.getColor(), 0xffffff, 0.375F),
                      color.getColor()
                )
                    .setScale(s)
                    .setLife(l)
    ...
    public boolean canConduct() {
        return hasMultiblock
            && (energy >= 5000 || (energy > 10 && !this.getTargets().isEmpty()));
    }
    ...
    public int maxThroughput() {
        ...
        return energy >= thresh ? this.getLinkedThroughput(base)
                                : this.getReducedThroughput(thresh, base);
    }
    ...
    public void forceCrystalColorMatch() {
        for (Coordinate c : encrustedBlocks) {
            if (c.getBlock(worldObj) == ChromaBlocks.ENCRUSTED.getBlockInstance()) {
                BlockEncrustedCrystal.setColor(
                    worldObj, c.xCoord, c.yCoord, c.zCoord, color
                );
            }
        }
    }
    ...
    public boolean drain(CrystalElement e, int amt) {
        if (e == color && energy >= amt && amt > 0) {
            if (ModList.MYSTCRAFT.isLoaded()
                && MystPages.Pages.LOSSY.existsInWorld(worldObj))
                amt = amt * 3 / 2;
            energy -= amt;
    ...
    public AspectList getAspects() {
        ...
        Collection<Aspect> li
            = ChromaAspectManager.instance.getAspects(this.getColor(), true);
        for (Aspect a : li) {
            as.add(a, n);
        }
        return as;
    ...
    @Override
    public String getId() { //Normally based on world coords, but uses just color to make
                            //each pylon color scannable once
        String s = "Pylon_"
            + color.toString(
            ); //"Pylon_"+worldObj.provider.dimensionId+":"+xCoord+":"+yCoord+":"+zCoord;
        if (this.isEnhanced())
            s = s + "_E";
        return s;
    }
    ...
    @ModDependent(ModList.THAUMCRAFT)
    public ItemStack
    onWandRightClick(World world, ItemStack wandstack, EntityPlayer player) {
        ...
        if (!worldObj.isRemote && this.canConduct() && player.ticksExisted % 5 == 0) {
            if (this.canPlayerWandPylon(wandstack, player)) {
                ...
                for (Aspect a : al.aspects.keySet()) {
                    ...
                    if (ReikaThaumHelper.isResearchComplete(
                            player, "ROD_silverwood_staff"
                        )) {
                        amt *= 1.5;
                        eff *= 0.8;
                    }
    ...
    private boolean canPlayerWandPylon(ItemStack wandstack, EntityPlayer player) {
        ...
        if (ChromaOptions.HARDTHAUM.getState()) {
            ...
            if (cap.getBaseCostModifier() > gold.getBaseCostModifier()
                || rod.getCapacity() < great.getCapacity())
                return false;
        }
        return (ChromaOptions.HARDTHAUM.getState() ? ProgressStage.ALLCOLORS
                                                   : ProgressStage.PYLON)
            .isPlayerAtStage(player);
    }

    @Override
    @ModDependent(ModList.THAUMCRAFT)
    public void
    onWandStoppedUsing(ItemStack wandstack, World world, EntityPlayer player, int count) {
    ...
    public Collection<ChunkCoordIntPair> getChunksToLoad() {
        return ChunkManager.getChunkSquare(
            xCoord, zCoord, 1
        ); //load a 3x3 to ensure power crystals
    }

    @Override
    protected final void
    onInvalidateOrUnload(World world, int x, int y, int z, boolean invalid) {
        if (!world.isRemote) {
            if (invalid) {
                this.unload();
            }
    ...
    getDeliveredColor(EntityPlayer ep, World world, int clickX, int clickY, int clickZ) {
        return color;
    }
    ...
    @Override
    public float
    getHeldToolChargingPower(EntityPlayer ep, CrystalElement e, ItemStack is) {
