...
public class TileEntityWirelessSource extends CrystalReceiverBase
    implements WirelessSource, LocationCached, MultiBlockChromaTile {
    ...
    private static final ThreadSafeTileCache cache
        = new ThreadSafeTileCache().setTileClass(TileEntityWirelessSource.class);
    ...
    public void validateStructure() {
        ...
        broadcasting = !worldObj.isRemote
            && ChromaStructures.WIRELESSPEDESTAL
                   .getArray(worldObj, xCoord, yCoord, zCoord)
                   .matchInWorld();
        enhancedBroadcasting = broadcasting
    ...
    private void broadcastEnergy(World world) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            for (WorldLocation loc : li) {
                //if (loc.dimensionID == world.provider.dimensionId) { //world specific
                //means tile accelerators in other worlds cannot work
                TileEntity te = loc.getTileEntity();
                if (te instanceof TileEntityWirelessSource) {
                    if (this.longRangeTransfer(
                            (TileEntityWirelessSource) te, CrystalElement.randomElement()
                        ))
                        break;
                }
                //}
    ...
    private boolean longRangeTransfer(TileEntityWirelessSource te, CrystalElement e) {
        ...
        if (amt > 0) {
            ...
            boolean inter
                = te.worldObj.provider.dimensionId != worldObj.provider.dimensionId;
            float loss = inter ? 4F : 2F; //50% loss + another 50% if cross-dimension
            if (enhancedBroadcasting)
                loss = TileEntityAuraPoint.hasAuraPoints(placerUUID)
                    ? 1
                    : (inter ? 2 : 1.25F);
    ...
    private void checkAndRequest() {
        for (int i = 0; i < CrystalElement.elements.length; i++) {
            ...
            if (space
                > (this.isBeacon() ? 0 : this.getEnergy(e)
                )) { //50% full, unless beacon, in which case all
    ...
    private void doParticles(World world, int x, int y, int z) {
        if (rand.nextInt(1 + Minecraft.getMinecraft().gameSettings.particleSetting)
            == 0) {
            int c = CrystalElement.getBlendedColor(this.getTicksExisted() + 15, 25);
            double v = ReikaRandomHelper.getRandomBetween(0.03125, 0.125);
            double[] vel = ReikaPhysicsHelper.polarToCartesian(
                v, rand.nextDouble() * 360, rand.nextDouble() * 360
            );
            float g = rand.nextBoolean() ? 0.0625F : -0.0625F;
            EntityBlurFX fx = new EntityCCBlurFX(
                                  world, x + 0.5, y + 0.5, z + 0.5, vel[0], vel[1], vel[2]
            )
                                  .setColor(c)
                                  .setRapidExpand()
                                  .setGravity(g);
        ...
        if (broadcasting) {
            ...
            double px = x - 2.5;
            double pz = z + d;
            AttractiveMotionController am
                = new AttractiveMotionController(this, 0.03125 / 24, 0.1875, 0.9875);
            EntityBlurFX fx = new EntityCCBlurFX(world, px, y - 3, pz)
                                  .setColor(c)
                                  .setAlphaFading()
                                  .setRapidExpand()
                                  .setGravity(-0.0625F)
                                  .setMotionController(am)
                                  .setLife(80);
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);

            px = x + 3.5;
            pz = z + 1 - d;
            fx = new EntityCCBlurFX(world, px, y - 3, pz)
                     .setColor(c)
                     .setAlphaFading()
                     .setRapidExpand()
                     .setGravity(-0.0625F)
    ...
    public boolean canTransmitTo(TileEntityWirelessPowered te) {
        return te.getDistanceFrom(xCoord + 0.5, yCoord + 0.5, zCoord + 0.5)
    ...
    public int request(CrystalElement e, int amt, int x, int y, int z) {
        ...
        if (!TileEntityAuraPoint.hasAuraPoints(placerUUID)) {
            ...
            LOSData los
                = PylonFinder.lineOfSight(worldObj, xCoord, yCoord, zCoord, x, y, z);
    ...
    public int getRenderColor(float ptick) {
        int t = this.isInWorld() && !StructureRenderer.isRenderingTiles()
            ? this.getTicksExisted()
            : Math.abs((int) (System.currentTimeMillis() / 50));
