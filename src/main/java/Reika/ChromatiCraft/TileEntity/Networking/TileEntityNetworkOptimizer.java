...
public class TileEntityNetworkOptimizer
    extends CrystalReceiverBase implements MultiBlockChromaTile {
    ...
    public void validateStructure() {
        ...
        hasStructure
            = ChromaStructures.OPTIMIZER.getArray(worldObj, xCoord, yCoord, zCoord)
                  .matchInWorld();
        ...
        if (ret) {
            for (int i = 0; i < 24; i++) {
                ReikaPacketHelper.sendDataPacketWithRadius(
                    ...
                    ChromaPackets.OPTIMIZE.ordinal(),
                    this,
                    48,
                    clr
                );
    ...
    protected void onFirstTick(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 16; i++) {
            data[i] = new PathData();
            data[i].pathData.addAll(
                CrystalNetworker.instance.getAllCachedPaths(CrystalElement.elements[i])
            );
        }
    }
