...
public class TileEntityRelaySource extends InventoriedCrystalReceiver
    implements InertIInv, ItemOnRightClick, MultiBlockChromaTile {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (inv[0] != null && ChromaItems.STORAGE.matchWith(inv[0])) {
            for (CrystalElement e :
                 ItemStorageCrystal.getStoredTags(inv[0]).elementSet()) {
                int amt = ItemStorageCrystal.getStoredEnergy(inv[0], e);
                int add = Math.min(
                    amt,
                    Math.min(
                        this.getMaxStorage(e) - energy.getValue(e),
                        this.maxThroughput() * 4
                    )
                );
                if (add > 0) {
    ...
    protected void onFirstTick(World world, int x, int y, int z) {
        ...
        if (ep != null && !ReikaPlayerAPI.isFake(ep) && !world.isRemote) {
            ...
            enhanced = ChromaResearchManager.instance.playerHasFragment(
                           ep, ChromaResearch.RELAYSTRUCT
                       )
    ...
    public void validateStructure() {
        ...
        hasEnhancedStructure
            = ChromaStructures.RELAY.getArray(worldObj, xCoord, yCoord, zCoord)
                  .matchInWorld();
    }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack is) {
        return ChromaItems.STORAGE.matchWith(is)
            && ItemStorageCrystal.getTotalEnergy(is) > 0;
    }
    ...
    public void onDrain(CrystalElement e, int amt) {
        ...
        if (worldObj.isRemote) {
            ProgressionCatchupHandling.instance.attemptSync(
                this, 8, ProgressStage.RELAYS, true
            );
        }
    }
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        return enhanced ? ReikaAABBHelper.getBlockAABB(this).expand(3, 2, 3)
                        : ReikaAABBHelper.getBlockAABB(this);
    }
