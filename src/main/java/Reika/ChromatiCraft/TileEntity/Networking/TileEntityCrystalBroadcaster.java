...
public class TileEntityCrystalBroadcaster
    extends TileEntityCrystalRepeater implements NotifiedNetworkTile, ConnectivityAction,
                                                 MultiBlockChromaTile, BreakAction {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        //if (this.getTicksExisted()%200 == 0 && Keyboard.isKeyDown(Keyboard.KEY_INSERT)
        //&& world.isRemote) 	this.onConnected(CrystalElement.randomElement());
    }
    ...
    protected boolean checkForStructure() {
        ...
        glassPotential
            = ((BroadcasterStructure) ChromaStructures.BROADCAST.getStructure())
                  .getGlassCount();
    ...
    private boolean isAir(World world, int x, int y, int z) {
        ...
        if (b.isAir(world, x, y, z))
            return true;
        if (b == ChromaBlocks.DIMGEN.getBlockInstance()
            && world.getBlockMetadata(x, y, z) == DimDecoTypes.CLIFFGLASS.ordinal()) {
            if (Math.abs(x - xCoord) <= GLASS_RANGE_H
                && Math.abs(y - yCoord) <= GLASS_RANGE_V
                && Math.abs(z - zCoord) <= GLASS_RANGE_H) {
                glassBlocks++;
                return true;
            }
        }
    ...
    private void checkInterfere() {
        TileEntityCrystalBroadcaster te = CrystalNetworker.instance.getNearestTileOfType(
            this, this.getClass(), INTERFERENCE_RANGE
        );
        if (te != null) {
    ...
    public static void updateAirCaches(World world, int x, int y, int z) {
        if (world.provider.dimensionId == ExtraChromaIDs.DIMID.getValue())
            return;
        Collection<TileEntityCrystalBroadcaster> c
            = CrystalNetworker.instance.getNearTilesOfType(
                world, x, y, z, TileEntityCrystalBroadcaster.class, AIR_SEARCH
            );
        for (TileEntityCrystalBroadcaster te : c) {
            if (Math.abs(te.yCoord - y) <= AIR_SEARCH_Y
                && Math.abs(te.xCoord - x) <= AIR_SEARCH
                && Math.abs(te.zCoord - z) <= AIR_SEARCH) {
    ...
    public void breakBlock() {
        if (interference != null) {
            ...
            if (te instanceof TileEntityCrystalBroadcaster) {
                ...
                if (tb.interference != null
                    && tb.interference.equals(worldObj, xCoord, yCoord, zCoord)) {
    ...
    private void onConnected(CrystalElement e) {
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            ChromaPackets.BROADCASTLINK.ordinal(),
            this,
            512,
            e.ordinal()
        );
    ...
    public void onConnectedParticles(CrystalElement e) {
        ...
        double dd = Minecraft.getMinecraft().thePlayer.getDistanceSq(
            xCoord + 0.5, Minecraft.getMinecraft().thePlayer.posY, zCoord + 0.5
        );
        if (dd <= soundDist * soundDist) {
            double d = Math.sqrt(dd);
            float f = d < soundDist * 0.5 ? 1F : (float) (2F * (1 - d / soundDist));
            ReikaSoundHelper.playClientSound(
                ChromaSounds.REPEATERRING,
                xCoord + 0.5,
                yCoord + 0.5,
                zCoord + 0.5,
                f,
        ...
        for (int i = 0;
             i < 128 - 32 * Minecraft.getMinecraft().gameSettings.particleSetting;
             i++) {
            ...
            int c = ReikaColorAPI.mixColors(
                e.getColor(), 0xffffff, 0.5F + rand.nextFloat() * 0.5F
            );
