...
public class TileEntityAuraInfuser_Old1
    extends FluidReceiverInventoryBase implements ItemOnRightClick {
        ...
        if (this.getTicksExisted()%1 == 0) {
            ...
            for (int n = 0; n < 6; n++) {
                for (int i = 0; i < 360; i += 60) {
                    ...
                    double py = y+0.125+n*0.0625+ReikaRandomHelper.getRandomPlusMinus(0,
        0.05); double pz = z+0.5+r*Math.cos(Math.toRadians(a)); int red = n < 2 ? 255 : 0;
                    int green = n < 4 ? 255 : 0;
                    int blue = 255;
                    EntityBlurFX fx = new EntityBlurFX(world, px, py, pz, 0, 0,
        0).setColor(red, green, blue).setScale(0.5F);
    ...
    public int getColor() {
        return color != null ? ReikaColorAPI.mixColors(
                   color.getColor(), 0xffffff, berries / (float) BERRY_UNIT
               )
                             : 0xffffff;
    }
    ...
    private boolean canProcess() {
        ...
        if (inv[0].getItemDamage() >= 16)
            return false;
        boolean hasFluid = tank.isFull()
            && tank.getActualFluid().equals(FluidRegistry.getFluid("chroma"));
        if (!hasFluid && progressTimer == 0)
            return false;
        //hasfluid &= tank.getNBTInt("color") == inv[0].getItemDamage()%16;
        boolean hasenergy = berries >= BERRY_UNIT
            && color == CrystalElement.elements[inv[0].getItemDamage()]; //energy.getValue(e)
                                                                         //>= BERRY_UNIT;
        return hasenergy;
    }
    ...
    public boolean canExtractItem(int i, ItemStack is, int j) {
        return ChromaItems.SHARD.matchWith(is) && is.getItemDamage() >= 16
            && is.getItemDamage() < 32;
    }
    ...
    public boolean isItemValidForSlot(int i, ItemStack is) {
        ...
        boolean berry = ChromaItems.BERRY.matchWith(is)
            && (color == null || is.getItemDamage() == color.ordinal())
            && berries < BERRY_UNIT;
        return shard || berry;
    }
    ...
    public boolean canReceiveFrom(ForgeDirection from) {
        return from.offsetY == 0 && progressTimer == 0
            && (!(ChromaItems.SHARD.matchWith(inv[0]) && inv[0].getItemDamage() >= 16));
    }
    ...
    public ItemStack onRightClickWith(ItemStack item, EntityPlayer ep) {
        ...
        //ReikaItemHelper.getSizedItemStack(item, item.stackSize-1));
        return ret;
    }
}
