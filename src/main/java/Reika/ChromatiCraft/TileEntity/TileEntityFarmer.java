...
public class TileEntityFarmer extends TileEntityRelayPowered implements ComplexAOE {
    private static final WeightedRandom<Coordinate>[] coordinateRand
        = new WeightedRandom[4];
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote && !this.hasRedstoneSignal()
            && this.getEnergy(CrystalElement.GREEN) >= 200) {
    ...
    private boolean operateAt(World world, Coordinate c, boolean allowRelays) {
        ...
        if (allowRelays && o instanceof TileEntityFunctionRelay) {
            return this.operateAt(
                world, ((TileEntityFunctionRelay) o).getRandomCoordinate(), false
            );
            ...
            if (crop.isRipe(world, c.xCoord, c.yCoord, c.zCoord)) {
                ...
                ArrayList<ItemStack> li
                    = crop.getDrops(world, c.xCoord, c.yCoord, c.zCoord, fortune);
                if (fortune < 3) {
                    CropMethods.removeOneSeed(crop, li);
                }
                ReikaItemHelper.dropItems(
                    world, c.xCoord + 0.5, c.yCoord + 0.5, c.zCoord + 0.5, li
                );
                crop.setHarvested(world, c.xCoord, c.yCoord, c.zCoord);
                ReikaSoundHelper.playBreakSound(
                    world, c.xCoord, c.yCoord, c.zCoord, c.getBlock(world)
                );
            ...
            int fortune = this.getFortune();
            ArrayList<ItemStack> li
                = b.getDrops(world, c.xCoord, c.yCoord, c.zCoord, fortune, meta);
            ReikaItemHelper.dropItems(
                world, c.xCoord + 0.5, c.yCoord + 0.5, c.zCoord + 0.5, li
            );
    ...
    private void sendParticles(Coordinate c) {
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            ChromaPackets.FARMERHARVEST.ordinal(),
            this,
            48,
            c.xCoord,
            c.yCoord,
    ...
    private Object getCropOrRelayAt(World world, Coordinate c) {
        ...
        if ((b == Blocks.cactus || b == Blocks.reeds)
            && c.offset(0, -1, 0).getBlock(world) == b)
            return b;
        int meta = c.getBlockMetadata(world);
    ...
    private Coordinate
    getRandomPosition(World world, int x, int y, int z) { /*
...
int dy = ReikaWorldHelper.findTopBlockBelowY(world, dx, y, dz);//Math.min(y,
world.getTopSolidOrLiquidBlock(x, z)); return new Coordinate(dx, dy, dz);*/
        Coordinate pos
            = coordinateRand[this.getFacing().ordinal() - 2].getRandomEntry().offset(
                x, y, z
            );
        int dy = ReikaWorldHelper.findTopBlockBelowY(
            world, pos.xCoord, y, pos.zCoord
        ); //Math.min(y, world.getTopSolidOrLiquidBlock(x, z));
        Coordinate ret = pos.setY(dy);
        if (ReikaWorldHelper.isSubmerged(world, x, y, z)
            && ret.getTaxicabDistanceTo(x, y, z) >= 5)
            ret = null;
        return ret;
    }
    ...
    public ElementTagCompound getRequiredEnergy() {
        ...
        tag.addTag(
            ...
            this.getMaxStorage(CrystalElement.GREEN)
                - energy.getValue(CrystalElement.GREEN)
        );
        tag.addTag(
            CrystalElement.PURPLE,
            this.getMaxStorage(CrystalElement.PURPLE)
