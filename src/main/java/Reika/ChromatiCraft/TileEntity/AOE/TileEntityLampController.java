...
public class TileEntityLampController extends TileEntityChromaticBase
    implements LocationCached, GuiController, SimpleShaftPowerReceiver {
    private static final MultiMap<Integer, LightSource> map
        = new MultiMap(CollectionType.CONCURRENTSET, new ConcurrencyDeterminator());
    private static final MultiMap<Integer, WorldLocation> lights
        = new MultiMap(CollectionType.CONCURRENTSET, new ConcurrencyDeterminator());
    ...
    private static class LightSource {
        ...
        public boolean equals(Object o) {
            return o instanceof LightSource
                && ((LightSource) o).location.equals(location);
        }
    }
    ...
    private boolean hasAdjacentRF() {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (te instanceof IEnergyHandler
                && ((IEnergyHandler) te).getEnergyStored(dir.getOpposite()) > 0)
                return true;
        }
        return false;
    }
            ...
            for (int j = -this.getRange(); j <= this.getRange(); j++) {
                for (int k = -this.getRange(); k <= this.getRange(); k++) {
                    ...
                    if (world.getBlock(dx, dy, dz)
                        == ChromaBlocks.LAMPBLOCK.getBlockInstance()) {
                        this.addLight((TileEntityRangedLamp
    ...
    private static void onLightsChanged(int channel) {
        ThreadSafeSet<WorldLocation> c
            = (ThreadSafeSet<WorldLocation>) lights.get(channel);
    ...
    public static boolean activeSourceInRange(TileEntityRangedLamp te) {
        ThreadSafeSet<LightSource> c
            = (ThreadSafeSet<LightSource>) map.get(te.getChannel());
