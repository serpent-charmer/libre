...
public class TileEntityMeteorTower
    extends CrystalReceiverBase implements NBTTile, MultiBlockChromaTile {
    ...
    private static final Comparator<EntityLiving> mobPriority
        = new MobPriorityComparator();

    static {
        for (int i = 0; i < 3; i++) {
            required[i] = new ElementTagCompound();
            required[i].addValueToColor(
                CrystalElement.PINK, 1000 * ReikaMathLibrary.intpow2(i + 1, 2)
            );
    ...
    private void doChargingFX(World world, int x, int y, int z) {
        ...
        double posY = y - 6.5 + 0.5
            + 4
                * Math.sin(
                    0.03125 * this.getTicksExisted()
                    / (1 + 1D / attacks[tier].chargeTime - 0.5625 * Math.pow(f, 1.5))
                );
    ...
    private void doFireFX(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            ) new EntityCCFloatingSeedsFX(world, px, py, pz, a1, a2, ChromaIcons.NODE2)
                                           .setColor(COLORS[tier])
                                           .setScale(s)
                                           .setLife(40)
                                           .setRapidExpand();
    ...
    private boolean searchAndFire(World world, int x, int y, int z) {
        ...
        List<EntityLiving> li = world.getEntitiesWithinAABB(
            EntityLiving.class, ReikaAABBHelper.getBlockAABB(x, y, z).expand(r, 200, r)
        );
        if (!li.isEmpty()) {
    ...
    private void doAttack(EntityLiving e) {
        ...
        AxisAlignedBB box
            = ReikaAABBHelper.getBlockAABB(xCoord, yCoord, zCoord).expand(6, 6, 6);
        ...
        for (EntityLivingBase in : li) {
            ReikaEntityHelper.knockbackEntityFromPos(
                xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, in, 2
            );
        }
    ...
    public void validateStructure() {
        ...
        hasStructure = !worldObj.isRemote
            && struct.getArray(worldObj, xCoord, yCoord, zCoord).matchInWorld();
