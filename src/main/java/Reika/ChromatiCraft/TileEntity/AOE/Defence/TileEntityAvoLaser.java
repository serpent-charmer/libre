...
public class TileEntityAvoLaser extends TileEntityRelayPowered implements SidePlacedTile {
    ...
    private static final AdjacencyCheckHandlerImpl adjacency
        = TileEntityAdjacencyUpgrade.getOrCreateAdjacencyCheckHandler(
            CrystalElement.PINK, "Increase damage", ChromaTiles.AVOLASER
        );
    ...
    private void attack(EntityLivingBase e) {
        if (!attackCooldowns.containsKey(e.getUniqueID())) {
            ...
            float base = this.hasPinkRune()
                ? 12
                : 8; //4 hearts, since original does 40/100 damage
    ...
    private DamageSource getDamageSource() {
        if (damageSource == null) {
            damageSource = new CustomStringDamageSource(
                "has sacrificed themselves to the Sovereign Temple of "
                + this.getPlacerName()
            );
    ...
    private boolean hasPinkRune() {
        ...
        return worldObj.getBlock(dx, dy, dz) == ChromaBlocks.RUNE.getBlockInstance()
            && worldObj.getBlockMetadata(dx, dy, dz) == CrystalElement.PINK.ordinal();
    }
    ...
    private void spawnParticles(World world, int x, int y, int z, AxisAlignedBB box) {
        ...
        box = box.expand(0.125, 0.125, 0.125)
                  .offset(dir.offsetX * 0.5, dir.offsetY * 0.5, dir.offsetZ * 0.5);
        for (int i = 0; i < 2; i++) {
    ...
    public AxisAlignedBB getScanBox(World world, int x, int y, int z) {
        ...
        if (endDist > 0) {
            AxisAlignedBB box
                = ReikaAABBHelper.getBeamBox(x, y, z, dir, startDist, endDist);
            return box;
        } else {
            return null;
        }
    ...
    public boolean checkLocationValidity() {
        ...
        return worldObj.getBlock(dx, dy, dz)
            .isSideSolid(worldObj, dx, dy, dz, dir.getOpposite());
    }
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        ForgeDirection dir = this.getFacing();
        return ReikaAABBHelper.getBlockAABB(this).addCoord(
            MAXDIST * dir.offsetX, MAXDIST * dir.offsetY, MAXDIST * dir.offsetZ
        );
    }
}
