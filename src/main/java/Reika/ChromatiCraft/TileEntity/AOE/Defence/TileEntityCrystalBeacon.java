...
public class TileEntityCrystalBeacon extends CrystalReceiverBase
    implements LocationCached, RangeUpgradeable, MultiBlockChromaTile, OwnedTile {
    private static final ThreadSafeTileCache cache
        = new ThreadSafeTileCache().setTileClass(TileEntityCrystalBeacon.class);
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote && hasStructure && this.getCooldown() == 0
            && checkTimer.checkCap()) {
    ...
    public void validateStructure() {
        ...
        hasStructure = ChromaStructures.PROTECT.getArray(worldObj, xCoord, yCoord, zCoord)
                           .matchInWorld();
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        return this.hasStructure()
            ? ReikaAABBHelper.getBlockAABB(xCoord, yCoord, zCoord).expand(6, 4, 6)
    ...
    private void spawnParticles(World world, int x, int y, int z) {
        ...
        double py = ReikaRandomHelper.getRandomPlusMinus(
            y + 1.5 + 0.5 * (1 + Math.sin(Math.toRadians(angle))), d
        );
        CrystalElement c = CrystalElement.randomElement(
        ); //CrystalElement.elements[(this.getTicksExisted()/16)%16];
        EntityBlurFX fx = new EntityCCBlurFX(c, world, px, py, pz, 0, 0, 0)
        ...
        if (energy.contains(CrystalElement.RED) && hasStructure) {
            ...
            fx = new EntityCCBlurFX(c, world, px, py, pz, 0, v, 0)
                     .setIcon(ChromaIcons.FADE_GENTLE)
                     .setScale(1F)
                     .setLife(30)
                     .setColor(CrystalElement.RED.getColor())
                     .setRapidExpand()
                     .setGravity(g);
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
            ...
            for (int t = 0; t < 80; t += 30) {
                ...
                int dy = 1;
                d = 1
                    - (((this.getTicksExisted() + t + i * 20) % 80) / 80D
                    ); //rand.nextDouble();
                px = x + 0.5 + dx * d;
                pz = z + 0.5 + dz * d;
                py = y + 0.5 + dy * d;
                ...
                double vy = 0;
                double vz = 0;
                fx = new EntityCCBlurFX(c, world, px, py, pz, vx, vy, vz)
                         .setIcon(ChromaIcons.NODE2)
                         .setScale(1.5F)
                         .setLife(10)
                         .setColor(CrystalElement.RED.getColor());
                EntityFX fx2 = new EntityCCBlurFX(c, world, px, py, pz, vx, vy, vz)
                                   .setIcon(ChromaIcons.NODE2)
                                   .setScale(0.75F)
                                   .setLife(10)
                                   .setColor(0xffffff);
            ...
            for (EntityPlayer ep : this.getOwners(false)) {
                long dur = ep.worldObj.getTotalWorldTime()
                    - ep.getEntityData().getCompoundTag(NBT_TAG).getLong("time");
                        ...
                        for (int n = 0; n < 8; n++) {
                            ...
                            for (double dt2 = 0; dt2 <= 3; dt2 += 1.5) {
                                ...
                                fx = new EntityCCBlurFX(world, px, py, pz)
                                         ...
                                         .setScale(0.7F)
                                         .setLife(8);
                                int clr = ReikaColorAPI.getColorWithBrightnessMultiplier(
                                    CrystalElement.RED.getColor(), ap
                                );
                                fx.setColor(clr);
                                Minecraft.getMinecraft().effectRenderer.addEffect(fx);
                            }
    ...
    public static boolean isPlayerInvincible(EntityPlayer ep, float dmg) {
        if (updateDamageNBT(ep, dmg))
            return true;
        if (cache.lookForMatch(
                ep.worldObj,
                true,
                (WorldLocation loc, TileEntity tile)
                    -> {
    ...
    isEntityInvincible(EntityLiving ep, String owner, String uuid, float dmg) {
        if (updateDamageNBT(ep, dmg))
            return true;
        if (cache.lookForMatch(
                ep.worldObj,
