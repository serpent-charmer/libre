...
public class TileEntityCrystalFence
    extends TileEntityRelayPowered implements OwnedTile, BreakAction {
    ...
    private static final AdjacencyCheckHandlerImpl adjacency
        = TileEntityAdjacencyUpgrade.getOrCreateAdjacencyCheckHandler(
            CrystalElement.PINK, "Increase damage", ChromaTiles.FENCE
        );
        ...
        if (!world.isRemote && !fence.isEmpty()) {
            if (chargingTick > 0) {
                ...
                if (this.getTicksExisted() % t == 0) {
                    ...
                    int offset = (i / 7) * 12;
                    MusicKey key = MusicKey.getByIndex(
                        MusicKey.C5.ordinal() + n.ordinal() + offset % 12
                    );
                    if (key == null)
                        key = MusicKey.A1;
                    ChromaSounds.DING.playSoundAtBlock(
                        this, 1, (float) key.getRatio(MusicKey.C5)
                    );
                }
            } else {
    ...
    private void onFenceBreak(World world, int x, int y, int z, boolean notify) {
        if (!world.isRemote && notify) {
            ...
            ReikaPacketHelper.sendDataPacketWithRadius(
                ...
                ChromaPackets.FENCEBREAK.ordinal(),
                world,
                x,
                y,
                z,
    ...
    private void affectEntities() {
        ...
        for (int i = 0; i < li.size(); i++) {
            ...
            List<EntityLivingBase> ents
                = worldObj.getEntitiesWithinAABB(EntityLivingBase.class, aabb);
    ...
    public void triggerSegment(int i, boolean fadeColor) {
        if (worldObj.isRemote) {
            ...
            ReikaPacketHelper.sendDataPacketWithRadius(
                ChromatiCraft.packetChannel,
                ChromaPackets.FENCETRIGGER.ordinal(),
                this,
                64,
                i,
                fadeColor ? 1 : 0
    ...
    public int getSegmentAlpha(int i) {
        ...
        if (get.intValue() > FADE_START) {
            return FADE_START
                - ((get.intValue() - FADE_START) * (FADE_START / FADE_LENGTH));
        } else {
            return get.intValue();
        }
    }
