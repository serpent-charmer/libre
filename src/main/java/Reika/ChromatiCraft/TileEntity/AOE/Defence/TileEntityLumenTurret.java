...
public class TileEntityLumenTurret extends TileEntityChromaticBase {
    ...
    private void doParticles(World world, int x, int y, int z) {
        ...
        if (u.check(this)) {
            ...
            EntityFX fx = new EntityCCBlurFX(world, x + 0.5, y + 0.625, z + 0.5)
                              .setColor(u.color.getColor())
                              .setGravity(g)
                              .setLife(l)
                              .setScale(s);
    ...
    private void attackEntities(World world, int x, int y, int z) {
        List<EntityLivingBase> li = world.getEntitiesWithinAABB(
            EntityLivingBase.class, this.getBox(world, x, y, z)
        );
    ...
    private boolean shouldAttack(EntityLivingBase e) {
        ...
        if (e instanceof EntityPlayer)
            return !this.isOwnedByPlayer((EntityPlayer) e)
                && this.isHostile((EntityPlayer) e) && TurretUpgrades.PLAYERS.check(this);
    ...
    private boolean canSeeEntity(World world, int x, int y, int z, Entity e) {
        ...
        for (int i = 0; i <= n; i++) {
            tracer.setOrigins(
                x + 0.5, y + 0.5625, z + 0.5, e.posX, e.posY + e.height * i / n, e.posZ
            );
    ...
    private void doAttackEntity(World world, int x, int y, int z, EntityLivingBase e) {
        ...
        e.attackEntityFrom(
            new LumenTurretDamage(
                this,
                TurretUpgrades.NONPLAYER.check(this)
                    || Chromabilities.COMMUNICATE.enabledOn(this.getPlacer())
            ),
            this.getAttackDamage(e)
        );
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            ChromaPackets.TURRETATTACK.ordinal(),
            this,
            32,
            e.getEntityId()
        );
    ...
    public void doAttackParticles(int entityID) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityFX fx = new EntityFlareFX(
                              CrystalElement.PINK, worldObj, px, py, pz, dvx, dvy, dvz
            )
                              .setNoGravity()
                              .setLife(l)
                              .setScale(2);
    ...
    public static enum TurretUpgrades {
        ...
        public boolean check(TileEntityLumenTurret te) {
            ...
            if (b == ChromaBlocks.PYLONSTRUCT.getBlockInstance()
                && te.worldObj.getBlockMetadata(te.xCoord, y, te.zCoord)
                    == StoneTypes.MULTICHROMIC.ordinal())
                return true;
            while (y > 0 && y >= te.yCoord - MAX_UPGRADES
                   && b == ChromaBlocks.RUNE.getBlockInstance()) {
                if (te.worldObj.getBlockMetadata(te.xCoord, y, te.zCoord)
                    == color.ordinal())
                    return true;
                y--;
            }
