...
public class TileEntityCloakingTower
    extends TileEntityChromaticBase implements LocationCached, MultiBlockChromaTile {
    private static final ThreadSafeTileCache cache
        = new ThreadSafeTileCache().setTileClass(TileEntityCloakingTower.class);
    ...
    public void validateStructure() {
        ...
        active = ChromaStructures.CLOAKTOWER.getArray(worldObj, xCoord, yCoord, zCoord)
                     .matchInWorld();
    }
    ...
    private void rechargeTools(EntityPlayer ep) {
        int slot = rand.nextBoolean()
            ? rand.nextInt(9)
            : rand.nextInt(ep.inventory.getSizeInventory()); //bias to hotbar
    ...
    private boolean canCharge(ItemStack is) {
        if (InterfaceCache.RFENERGYITEM.instanceOf(is.getItem())
            || InterfaceCache.IELECTRICITEM.instanceOf(is.getItem())
            || InterfaceCache.MUSEELECTRICITEM.instanceOf(is.getItem())) {
    ...
    private boolean charge(ItemStack is) {
        if (InterfaceCache.RFENERGYITEM.instanceOf(is.getItem())) {
            ...
            int amt = Math.min(
                Math.max(1, ie.getMaxEnergyStored(is) / 100),
                ie.getMaxEnergyStored(is) - ie.getEnergyStored(is)
            );
            if (amt > 0) {
            ...
            double amt = Math.min(
                Math.max(1, max / 100), max - ElectricItem.manager.getCharge(is)
            );
            if (amt > 0) {
                double add = ElectricItem.manager.charge(
                    is, amt, Integer.MAX_VALUE, true, false
                );
                return add > 0;
            }
            return false;
            ...
            int amt = Math.min(
                Math.max(1, ms.getMaxCharge(is) / 100),
                ms.getMaxCharge(is) - (int) ms.getCurrentEnergy(is)
            );
            if (amt > 0) {
    ...
    private void createParticles(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            double h = dy - yCoord;
            double r
                = ReikaRandomHelper.getRandomPlusMinus(this.getWidth(h) + 0.5, 0.125);
            ...
            if (rand.nextBoolean())
                g = -g;
            EntityFX fx
                = new EntityCenterBlurFX(world, dx, dy, dz).setGravity(g).setLife(l);
        ...
        for (int i = 0; i < nd; i += di) {
            ...
            int l = 60;
            EntityFX fx
                = new EntityCenterBlurFX(world, dx, dy, dz).setGravity(0).setLife(l);
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
        }
    }
    ...
    public double getWidth(double i) {
        ...
        if (!this.isActive()) {
            return 0.75 + 0.03125 * Math.sin(-t / 8D + i)
                + 0.03125 * Math.sin(t / 16D + i);
        }
        return 0.75 + 0.125 * Math.sin(i - t / 16D) + 0.0625 * Math.cos(t / 8D)
            + 0.375 * Math.sin(t / 64D);
    }
    ...
    public static boolean isPlayerCloaked(EntityPlayer ep) {
        return cache.lookForMatch(
            ...
            (WorldLocation loc, TileEntity tile)
                ...
                return te.isOwnedByPlayer(ep) && te.isActive()
                    && loc.getCylinderDistanceTo(ep) <= MAXRANGE;
            },
