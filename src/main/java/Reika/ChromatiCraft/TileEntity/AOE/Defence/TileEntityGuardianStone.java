...
public class TileEntityGuardianStone extends TileEntityChromaticBase {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        if (placer != null) {
            if (world.isRemote) {
                ...
                zone = GuardianStoneManager.instance.addZone(
                    world, x, y, z, this.getPlacer(), RANGE
                );
    ...
    private void doAreaParticles(World world, int x, int y, int z) {
        ...
        double minY0
            = ChromaOptions.GUARDCHUNK.getState() ? 0 : zone.originY - zone.range;
        double minZ0 = zone.originZ - zone.range;
        double maxX0 = zone.originX + 1 + zone.range;
        double maxY0
            = ChromaOptions.GUARDCHUNK.getState() ? 256 : zone.originY + 1 + zone.range;
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (line) {
                ...
                ForgeDirection face
                    = ReikaJavaLibrary.getRandomCollectionEntry(rand, set);
                switch (face) {
                    case DOWN:
                        py = minY;
                        break;
            ...
            if (line)
                ...
                l *= 4;
            EntityBlurFX fx
                = new EntityCCBlurFX(world, px, py, pz).setScale(s).setLife(l);
