...
public class TileEntityExplosionShield
    extends CrystalReceiverBase implements LocationCached, RangeUpgradeable {
    private static final ThreadSafeTileCache cache
        = new ThreadSafeTileCache().setTileClass(TileEntityExplosionShield.class);
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote && this.getCooldown() == 0 && checkTimer.checkCap()) {
            ...
            energy.subtract(
                CrystalElement.RED,
                Math.min(1000, Math.max(10, energy.getValue(CrystalElement.RED) / 5000))
            );
        }
    }
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        return this.hasStructure() ? ReikaAABBHelper.getBlockAABB(xCoord, yCoord,
    zCoord).expand(6, 4, 6) : super.getRenderBoundingBox();
    }
     */
    @Override
    ...
    private void spawnParticles(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (b.isOpaqueCube() && b.renderAsNormalBlock()
                && b.getRenderType() == 0) { //skip ones inside the ground/walls/etc
                continue;
            }
            EntityBlurFX fx = new EntityCCBlurFX(world, px, py, pz);
            fx.setLife(ReikaRandomHelper.getRandomBetween(5, 100))
                .setScale(1 + rand.nextFloat())
        ...
        for (int i = 0; i < n2; i++) {
            ...
            if (Math.abs(px - ep.posX) > 12 || Math.abs(py - ep.posY) > 12
                || Math.abs(pz - ep.posZ) > 12) {
                continue;
            }
    ...
    public static void dampenExplosion(World world, Explosion e) {
        cache.applyToMatches(
            ...
            (WorldLocation loc, TileEntity tile)
                ...
                //if (ReikaMathLibrary.py3d(e.explosionX-loc.xCoord-0.5, 0,
                //e.explosionZ-loc.zCoord-0.5) <= te.range) { if (te.canDampen(e)) {
                Iterator<ChunkPosition> it = e.affectedBlockPositions.iterator();
                while (it.hasNext()) {
                    ChunkPosition p = it.next();
                    if (te.isLocationProtectedBy(
                            world, p.chunkPosX, p.chunkPosY, p.chunkPosZ, e.explosionSize
                        )) {
                        it.remove();
                    }
                }
    ...
    isLocationProtected(World world, int x, int y, int z, double power) {
        return cache.lookForMatch(
            world,
            true,
            (WorldLocation loc, TileEntity te)
                -> {
                return ((TileEntityExplosionShield) te)
                    .isLocationProtectedBy(world, x, y, z, power);
            },
            (WorldLocation loc, TileEntity te) -> {
                ChromaAux.logTileCacheError(world, loc, te, ChromaTiles.EXPLOSIONSHIELD);
            }
    ...
    public boolean isLocationProtectedBy(World world, int x, int y, int z, double power) {
        ...
        if (energy.containsAtLeast(CrystalElement.RED, cost)) {
            int r = this.getRange();
            int ry = this.getYRange();
            if (Math.abs(x - xCoord) <= r && Math.abs(z - zCoord) <= r
                && Math.abs(y - yCoord) <= ry) {
                return true;
            }
        }
        return false;
