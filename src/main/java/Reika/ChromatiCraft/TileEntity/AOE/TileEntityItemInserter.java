...
public class TileEntityItemInserter
    extends InventoriedChromaticBase implements LinkerCallback {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote && inv[slot] != null && (omniMode || inv[slot].stackSize > 1)
            && !ReikaRedstoneHelper.isPoweredOnSide(world, x, y, z, dirs[slot])) {
            Coordinate c = this.sendItem(slot);
            if (c != null) {
                ReikaPacketHelper.sendDataPacketWithRadius(
                    ChromatiCraft.packetChannel,
                    ChromaPackets.INSERTERACTION.ordinal(),
                    this,
                    64,
                    Item.getIdFromItem(inv[slot].getItem()),
                    inv[slot].getItemDamage(),
                    c.xCoord,
                    c.yCoord,
                    c.zCoord
                );
    ...
    private void doJammedFX(World world, int x, int y, int z) {
        ...
        fx.setIcon(ChromaIcons.FADE_RAY)
            ...
            .setLife(l)
            .setScale(0.9F)
            .setGravity(-(float) ReikaRandomHelper.getRandomBetween(0.125, 0.375));
    ...
    public void sendItemClientside(int id, int meta, int x, int y, int z) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityFloatingSeedsFX fx = new EntityCCFloatingSeedsFX(
                worldObj, px, py, pz, -angs[2] + 270, angs[1] - 90, ChromaIcons.CENTER
            );
    ...
    private void rebuildCache() {
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            DragonAPIInit.packetChannel, PacketIDs.GUIRELOAD.ordinal(), this, 12
        );
    }
    ...
    public boolean isItemValidForSlot(int slot, ItemStack is) {
        ...
        if (ReikaFluidHelper.getFluidForItem(is) != null)
            if (is.stackSize > 1 || inv[slot] != null)
                return false;
        return omniMode ? targets[slot] != null
                        : ReikaItemHelper.matchStacks(inv[slot], is);
    }
    ...
    private Coordinate sendItem(int slot) {
        ...
        if (left == null || left.stackSize < put.stackSize
            || ReikaItemHelper.matchStacks(left, is.getItem().getContainerItem(is))) {
            if (left != null) {
                if (ReikaInventoryHelper.addOrSetStack(left, inv, TARGETS)) {
                    inv[slot] = left.copy();
                } else {
                    ReikaItemHelper.dropItem(
                        worldObj, xCoord + 0.5, yCoord + 1.25, zCoord + 0.5, left
                    );
                }
            }
            return c;
        ...
        private ItemStack send(World world, Coordinate c, ItemStack is, EntityPlayer ep) {
            switch (this) {
                ...
                case RIGHTCLICK: {
                    ...
                    boolean flag = false;
                    if (is.getItem() instanceof ItemBucket
                        && c.offset(0, 1, 0).isEmpty(world) && is.stackSize == 1) {
                        flag = ((ItemBucket) is.getItem())
                                   .tryPlaceContainedLiquid(
                                       world, c.xCoord, c.yCoord + 1, c.zCoord
                                   );
                        if (flag) {
                        ...
                        flag = is.getItem().onItemUse(
                            is, ep, world, c.xCoord, c.yCoord + 1, c.zCoord, 1, 0, 0, 0
                        );
                    } else {
                        flag = c.getBlock(world).onBlockActivated(
                            world, c.xCoord, c.yCoord, c.zCoord, ep, 1, 0, 0, 0
                        );
                    }
                ...
                case LEFTCLICK: {
                    ...
                    ItemStack orig = is.copy();
                    ep.setCurrentItemOrArmor(0, is);
                    c.getBlock(world).onBlockClicked(
                        world, c.xCoord, c.yCoord, c.zCoord, ep
                    );
                    ItemStack ret = ep.getCurrentEquippedItem();
                    ep.setCurrentItemOrArmor(0, hold);
                    return ret;
                ...
                case ENTITY:
                    ...
                    EntityItem ei = ReikaItemHelper.dropItem(
                        world,
                        c.xCoord + 0.5,
                        c.yCoord + c.getBlock(world).getBlockBoundsMaxY() + 0.125,
                        c.zCoord + 0.5,
                        is
                    );
