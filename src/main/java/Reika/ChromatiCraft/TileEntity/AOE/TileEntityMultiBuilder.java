...
public class TileEntityMultiBuilder
    extends TileEntityChromaticBase implements LocationCached {
    private static final ThreadSafeTileCache cache
        = new ThreadSafeTileCache().setTileClass(TileEntityMultiBuilder.class);
    ...
    protected void onFirstTick(World world, int x, int y, int z) {
        if (bounds == null)
            bounds = BlockBox.block(this)
                         .expand(ForgeDirection.UP, 8)
                         .expand(ForgeDirection.DOWN, 1);
    ...
    public void expand(ForgeDirection dir) {
        ...
        bounds = bounds.expand(
            Math.abs(dir.offsetX), 0, Math.abs(dir.offsetZ)
        ); //bounds.expand(dir, 1);
    ...
    protected void readSyncTag(NBTTagCompound NBT) {
        ...
        bounds = NBT.hasKey("bounds") ? BlockBox.readFromNBT(NBT.getCompoundTag("bounds"))
                                      : null;
    ...
    private void checkAndBuild(
        World world, int x, int y, int z, Block b, int meta, EntityPlayer ep, ItemStack is
    ) {
        if (world.isRemote)
            return;
        if (world.provider.dimensionId == worldObj.provider.dimensionId
            && bounds.isBlockInsideExclusive(x, y, z)) {
    ...
    private void
    checkAndBreak(World world, int x, int y, int z, Block b, int meta, EntityPlayer ep) {
        if (world.isRemote)
            return;
        if (world.provider.dimensionId == worldObj.provider.dimensionId
            && bounds.isBlockInsideExclusive(x, y, z)) {
            for (Point p : region.getRegions(xCoord, zCoord, bounds)) {
                if (p.x != 0 || p.y != 0) {
                    ...
                    int dz = z + bounds.getSizeZ() * p.y;
                    this.mineBlock(
                        (WorldServer) world, dx, dy, dz, b, meta, (EntityPlayerMP) ep
                    );
                }
            }
        }
    ...
    private boolean buildBlock(
        ...
        if (ReikaWorldHelper.softBlocks(world, x, y, z)
            && ReikaPlayerAPI.playerCanBreakAt(world, x, y, z, ep)) {
    ...
    private void mineBlock(
        WorldServer world, int x, int y, int z, Block b, int meta, EntityPlayerMP ep
    ) {
        if (new BlockKey(b, meta).matchInWorld(world, x, y, z)
            && ReikaPlayerAPI.playerCanBreakAt(world, x, y, z, ep)) {
            if (ep.capabilities.isCreativeMode) {
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        return bounds != null ? ReikaAABBHelper.scaleAABB(
                   bounds.asAABB(), 1 + region.radiusX, 1, 1 + region.radiusZ
               )
    ...
    public void renderBeams(float ptick, HashMap<DecimalPosition, Integer> columns) {
        ...
        if (rand.nextInt(10) == 0) {
            ...
            renderBeams.add(
                new RenderBeam(pos1, pos2, columns.get(pos1), columns.get(pos2), rand)
            );
        }
    }
    ...
    public static void placeBlock(
        ...
        if (ep == null)
            return;
        cache.applyToMatches(
            world,
            true,
            (WorldLocation loc, TileEntity te)
                -> {
                ((TileEntityMultiBuilder) te)
                    .checkAndBuild(world, x, y, z, b, meta, ep, is);
            },
            (WorldLocation loc, TileEntity te) -> {
    ...
    public static void
    breakBlock(World world, int x, int y, int z, Block b, int meta, EntityPlayer ep) {
        if (ep == null)
            return;
        cache.applyToMatches(
            world,
            ...
            (WorldLocation loc, TileEntity te)
                ...
                if (x != te.xCoord || y != te.yCoord || z != te.zCoord)
                    ((TileEntityMultiBuilder) te)
                        .checkAndBreak(world, x, y, z, b, meta, ep);
            },
            (WorldLocation loc, TileEntity te) -> {
                ChromaAux.logTileCacheError(world, loc, te, ChromaTiles.MULTIBUILDER);
            }
            ...
            switch (this) {
                ...
                case ELLIPSE:
                    return ReikaMathLibrary.isPointInsideEllipse(
                        i, 0, k, radiusX, 0, radiusZ
                    );
                case DIAMOND:
                    return Math.abs(i) + Math.abs(k) < Math.max(radiusX, radiusZ);
                case HEXAGON:
                    return ReikaMathLibrary.isPointInsidePolygon(
                        i, k, 6, radiusX, radiusZ
                    );
                case OCTAGON:
                    return ReikaMathLibrary.isPointInsidePolygon(
                        i, k, 8, radiusX, radiusZ
                    );
                    //case TRIANGLE1:
                    //	break;
                    //case TRIANGLE2:
    ...
    private static class RenderBeam {
        ...
        private RenderBeam(
            DecimalPosition p1, DecimalPosition p2, int c1, int c2, Random rand
        ) {
            pos1 = p1;
            pos2 = p2;
