...
public class TileEntityCrystalLaser extends InventoriedRelayPowered {
    private static final EnumMap<CrystalElement, LensEffect> effects
    ...
    static {
        addLensEffect(CrystalElement.BLUE, "Creates light", (te, dx, dy, dz, d) -> {
            if (te.worldObj.getBlock(dx, dy, dz).isAir(te.worldObj, dx, dy, dz)) {
                te.worldObj.setBlock(
                    dx, dy, dz, ChromaBlocks.LIGHT.getBlockInstance(), 0, 3
                );
            }
        });
        addLensEffect(
            ...
            (te, dx, dy, dz, d)
                -> BlockTickEvent.fire(
                    te.worldObj, dx, dy, dz, rand, UpdateFlags.getForcedUnstoppableTick()
                )
        );
        addLensEffect(
            ...
            "Applies heat (400C)",
            (te, dx, dy, dz, d
            ) -> ReikaWorldHelper.temperatureEnvironment(te.worldObj, dx, dy, dz, 400)
        );
        ...
        addLensEffect(CrystalElement.RED, "Adds resistance", (te, box, li) -> {
            for (Entity e : li) {
                if (e instanceof EntityLivingBase)
                    ((EntityLivingBase) e)
                        .addPotionEffect(new PotionEffect(Potion.resistance.id, 100, 1));
            }
        });
        ...
        addLensEffect(CrystalElement.MAGENTA, "Adds regeneration", (te, box, li) -> {
            for (Entity e : li) {
                if (e instanceof EntityLivingBase)
                    ((EntityLivingBase) e)
                        .addPotionEffect(new PotionEffect(Potion.regeneration.id, 100, 1)
                        );
            }
        });
    ...
    private static void addLensEffect(CrystalElement e, String desc, LensEffectAABB loc) {
        effects.put(e, new AABBLensEffect(e, desc) {
            ...
            public void doEffect(
                TileEntityCrystalLaser te, AxisAlignedBB box, List<Entity> li
            ) {
    ...
    private static void
    addLensEffect(CrystalElement e, String desc, LensEffectLocal loc) {
        effects.put(e, new LocalLensEffect(e, desc) {
            @Override
            public void doEffect(
                TileEntityCrystalLaser te, int dx, int dy, int dz, int dist
            ) {
    ...
    public CrystalElement getColor() {
        return inv[0] != null && ChromaItems.LENS.matchWith(inv[0])
            ? CrystalElement.elements[inv[0].getItemDamage()]
            : null;
    }
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        ...
        int fy = f.offsetY;
        int fz = f.offsetZ;
        return r > 0 ? ReikaAABBHelper.getBlockAABB(xCoord, yCoord, zCoord)
                           .expand(fx * r, fy * r, fz * r)
    ...
    public static abstract class AABBLensEffect
        extends LensEffect implements LensEffectAABB {
    ...
    public static abstract class LocalLensEffect
        extends LensEffect implements LensEffectLocal {
