...
public class TileEntityAreaBreaker
    extends ChargedCrystalPowered implements BreakAction, ItemOnRightClick {
    ...
    private final HashMap<Coordinate, ImmutablePair<Integer, Integer>> breakLocs
        = new HashMap();
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        if (this.isRunning()) {
            ...
            if (newloctick == 0) {
                if (breakLocs.size() < Math.max(
                        1, Math.min(MAX_ACTIVE_BREAKERS, area[activeIndex].getSize() / 16)
                    )) {
                    Coordinate c = area[activeIndex].getNextAndMoveOn();
                    Block b = c != null ? c.getBlock(world) : Blocks.air;
                    while (!area[activeIndex].isEmpty()
                           && !this.isCoordValid(c, b, world, x, y, z)) {
            ...
            for (Coordinate c : breakLocs.keySet()) {
                if (this.hasEnergy(required)) {
                    ...
                    if (get.left > 1) {
                        ...
                        this.useEnergy(
                            required.copy().scale(this.getEnergyCostScale() * 0.25F)
                        );
    ...
    private boolean
    isCoordValid(Coordinate c, Block b, World world, int x, int y, int z) {
        return c != null && this.shouldTryBreaking(world, c.xCoord, c.yCoord, c.zCoord, b)
            && !c.equals(x, y, z) && c.getTileEntity(world) == null
            && (areaOverride != null ? areaOverride.isBlockInside(c)
                                     : shape.isBlockInShape(
                                         c.xCoord - x, c.yCoord - y, c.zCoord - z, range
                                     ));
    }
    ...
    private void createBolts(Coordinate c) {
        for (int i = 0; i < 3; i++) {
            LightningBolt b
                = new LightningBolt(new DecimalPosition(this), new DecimalPosition(c), 8);
            b.setVariance(0.25);
    ...
    private void initArea() {
        for (int i = 0; i < area.length; i++) {
            area[i] = new BlockSpiral(xCoord, yCoord + i, zCoord, range)
                          .setRightHanded()
                          .setGridSize(1)
                          .calculate();
        }
    ...
    private void
    dropItems(World world, int x, int y, int z, Block b, int meta, EntityPlayer ep) {
        ...
        if (b instanceof BlockTieredResource) {
            ...
            items = harvest ? bt.getHarvestResources(world, x, y, z, 0, ep)
                            : bt.getNoHarvestResources(world, x, y, z, 0, ep);
        } else if (b instanceof BlockTileEnum) {
            items = ReikaJavaLibrary.makeListFrom(
                ((BlockTileEnum) b).getMapping(world, x, y, z).getCraftedProduct()
            );
        } else {
            items = ReikaJavaLibrary.makeListFrom(ReikaBlockHelper.getSilkTouch(
                world, x, y, z, b, meta, this.getPlacer(), false
            ));
        }
        if (items != null) {
    ...
    public void incRange() {
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            PacketIDs.NUMBERPARTICLE.ordinal(),
            this,
            512,
            range
        );
    ...
    private void readIAP(TileEntity te) {
        ...
        areaOverride = BlockBox.getFromIAP(iap)
                           .contract(ForgeDirection.UP, 1)
                           .contract(ForgeDirection.EAST, 1)
                           .contract(ForgeDirection.SOUTH, 1);
            ...
            switch (this) {
                ...
                case PYRAMID:
                    return new Coordinate(dx, dy, dz)
                               .getTaxicabDistanceTo(new Coordinate(0, 0, 0))
                        <= r;
                case DIAMONDPRISM:
                    return new Coordinate(dx, 0, dz).getTaxicabDistanceTo(
