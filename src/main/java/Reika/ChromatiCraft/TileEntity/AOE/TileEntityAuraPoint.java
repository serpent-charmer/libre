...
public class TileEntityAuraPoint extends TileEntityLocusPoint {
    ...
    private void regenPylons(World world, int x, int y, int z) {
        //CrystalNetworker.instance.getNearbyPylons(world, x, y, z,
    CrystalElement.randomElement(), 512, false); TemporaryCrystalReceiver tr = new
    TemporaryCrystalReceiver(this, Integer.MAX_VALUE, 48, 0.375, ResearchLevel.CTM);
        CrystalPath path =
    CrystalNetworker.instance.getConnectivity(CrystalElement.randomElement(), tr); if
    (path != null) { if (path.transmitter instanceof TileEntityCrystalPylon) {
    ...
    private void killEntities(World world, int x, int y, int z) {
        ...
        AxisAlignedBB box
            = AxisAlignedBB.getBoundingBox(x - r, 0, z - r, x + 1 + r, 256, z + 1 + r);
    ...
    private int getLootingLevel() {
        return (int
        ) Math.min(6, this.getTileEntityAge() / 288000); //1 per 4h, max 6 @ 24h
    }
    ...
    private void attack(World world, int x, int y, int z, EntityLivingBase e) {
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            ChromaPackets.AURATTACK.ordinal(),
            this,
            192,
            e.getEntityId()
        );
    ...
    public void doAttackFX(Entity e) {
        if (e != null) {
            ...
            for (double d = 0; d <= dd; d += 0.5) {
                ...
                EntityFX fx = new EntityCCBlurFX(e.worldObj, px, py, pz)
    ...
    private boolean shouldAttack(EntityLivingBase e) {
        ...
        if (ReikaEntityHelper.isHostile(e) || e instanceof EntityEnderman
    ...
    ChromaPackets.AURAGROW.ordinal(), world, c.xCoord, c.yCoord, c.zCoord, 64);
                    }
                }
            }
        }
    ...
    true)); Coordinate c = new Coordinate(dx, dy, dz); if (cache.containsBlock(c)) { k--;
                continue;
            }
            else {
                CropType type = this.getCropAt(world, c);
    ...
    public static void doGrowFX(World world, int x, int y, int z) {
        ReikaParticleHelper.BONEMEAL.spawnAroundBlockWithOutset(
            world, x, y, z, 4, 0.0625
        );
    }
    ...
    private void heal(EntityLivingBase e) {
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ChromatiCraft.packetChannel,
            ChromaPackets.AURAHEAL.ordinal(),
            this,
            64,
            e.getEntityId()
        );
    ...
    public static boolean isPointWithin(World world, int x, int y, int z, int r) {
        return TileEntityLocusPoint.isPointWithin(
            TileEntityAuraPoint.class, world, x, y, z, r
        );
    }
