...
public class TileEntityItemCollector extends InventoriedRelayPowered
    implements NBTTile, LocationCached, IFluidHandler, RangeUpgradeable {
    private int experience = 0;
    public boolean canIntake = false;

    public static final int MAXRANGE = 24;

    private final ConfigurableRangeTracker range
        = new ConfigurableRangeTracker(MAXRANGE, 24, 1);
    ...
    private static final ThreadSafeTileCache cache
        = new ThreadSafeTileCache().setTileClass(TileEntityItemCollector.class);

    private static final AdjacencyCheckHandlerImpl adjacency
        = TileEntityAdjacencyUpgrade.getOrCreateAdjacencyCheckHandler(
            CrystalElement.PURPLE, "Increase filter count", ChromaTiles.ITEMCOLLECTOR
        );
                ...
                for (int s = 0; s < 6; s++) {
                    ...
                    if (te instanceof IInventory && !te.isInvalid()) {
                        for (int i = 0; i < inv.length; i++) {
                            ItemStack is = inv[i];
                            if (is != null
                                && ReikaInventoryHelper.addToIInv(is, (IInventory) te)) {
                                inv[i] = null;
                            }
                        }
                    }
    ...
    private void doScan(World world, int x, int y, int z) {
        ...
        List<Entity> li = world.selectEntitiesWithinAABB(
            Entity.class, box, ReikaEntityHelper.itemOrXPSelector
        );
        for (Entity e : li) {
    ...
    public boolean checkAbsorb(Entity e) {
        ...
        if (e instanceof EntityItem || e instanceof EntityXPOrb) {
            if (Math.abs(e.posX - x - 0.5) <= this.getRange()
                && Math.abs(e.posY - y - 0.5) <= this.getYRange()
                && Math.abs(e.posZ - z - 0.5) <= this.getRange()) {
    ...
    private boolean match(ItemStack is, ItemStack filter) {
        if (ChromaItems.SEED.matchWith(is))
            return ChromaItems.SEED.matchWith(filter)
                && is.getItemDamage() % 16 == filter.getItemDamage() % 16;
        return ReikaItemHelper.matchStacks(is, filter)
            && ItemStack.areItemStackTagsEqual(is, filter);
    }
