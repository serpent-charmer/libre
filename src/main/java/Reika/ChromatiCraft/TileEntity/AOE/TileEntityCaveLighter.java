...
public class TileEntityCaveLighter
    extends TileEntityChromaticBase implements RangeUpgradeable {
    public static final int BASE_RANGE
        = Math.min(64, ChromaOptions.CAVELIGHTERRANGE.getValue());
    public static final int MAXY = 80;
    private static final int ZONE_SIZE
        = MathHelper.clamp_int(4, 16, ChromaOptions.CAVELIGHTERSIZE.getValue());
                ...
                if (spiral[i].getSize() > 0) {
                    ...
                    if (this.placeBlockAt(world, cx, cy, cz)) {
                        ...
                        ReikaPacketHelper.sendDataPacketWithRadius(
                            ...
                            ChromaPackets.LIGHTERACT.ordinal(),
                            this,
                            32,
                            cx,
                            cy,
                        ...
                        //ReikaJavaLibrary.pConsole("Lighting "+new Coordinate(cx, cy,
                        //cz));
                    }
                    //else {
                    //	Coordinate cp = new Coordinate(cx, cy, cz);
                    //	ReikaJavaLibrary.pConsole("Not lighting "+cp+",
                    //"+cp.getBlock(world).getLocalizedName());
                    //}
                    flag = true;
                    if (spiral[i].getSize() == 0) {
                        ReikaPacketHelper.sendDataPacketWithRadius(
                            ChromatiCraft.packetChannel,
                            ChromaPackets.LIGHTERDELTAY.ordinal(),
                            this,
                            32,
                            (i + 1) * ZONE_SIZE
                        );
                        if (i == spiral.length - 1) {
                            ReikaPacketHelper.sendDataPacketWithRadius(
                                ChromatiCraft.packetChannel,
                                ChromaPackets.LIGHTEREND.ordinal(),
                                this,
                                32
                            );
                            //complete = true;
    ...
    public void doDeltaYParticles(int newY) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityFX fx = new EntityCCBlurFX(worldObj, rx, ry, rz)
                              .setColor(c)
                              .setGravity(-0.03125F)
                              .setLife(30)
                              .setScale(1.25F);
    ...
    public void doCompletionParticles() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityFX fx = new EntityCCBlurFX(worldObj, rx, ry, rz, dx * v, dy * v, dz * v)
                              .setColor(0xffffff)
                              .setGravity(0)
                              .setLife(60)
                              .setScale(1.75F)
        ...
        ReikaSoundHelper.playClientSound(
            ChromaSounds.CAST, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, 1, 1
        );
    }
    ...
    public void doLightedParticles(int x, int y, int z) {
        ...
        double dz = z - zCoord;
        double ang
            = Math.toRadians(-90 - ReikaPhysicsHelper.cartesianToPolar(dx, 0, dz)[2]);
        double mr = 3;
        for (double r = 0; r < mr; r += 0.0625) {
            ...
            EntityFX fx = new EntityCCBlurFX(worldObj, px, yCoord + 0.5, pz)
    ...
    private void initSpirals(World world, int x, int y, int z) {
        int r = this.getRange();
        for (int i = 0; i < spiral.length; i++) {
            spiral[i] = new BlockSpiral(x, i * ZONE_SIZE, z, r / ZONE_SIZE)
                            .setRightHanded()
                            .setGridSize(ZONE_SIZE)
                            .calculate();
        }
    }
    ...
    private boolean placeBlockAt(World world, int x, int y, int z) {
        return world.getBlock(x, y, z).isAir(world, x, y, z)
            && y < this.getMaxY(world, x, z) && this.isDark(world, x, y, z)
            && this.isLightable(world, x, y, z);
    }
    ...
    public static boolean isLightable(World world, int x, int y, int z) {
        if (ModList.SATISFORESTRY.isLoaded()
            ...
            && SFAPI.caveHandler.isInCave(world, x + 0.5, y + 0.5, z + 0.5)) {
            return world.getBlockLightValue(x, y, z) == 0 && (x + y + z) % 48 == 0
                && SFAPI.caveHandler.getDistanceToCaveCenter(world, x, y, z) >= 64;
        }
        return true;
    }
    ...
    private boolean isDark(World world, int x, int y, int z) {
        return !world.canBlockSeeTheSky(x, y + 1, z)
            ...
            && world.getSavedLightValue(EnumSkyBlock.Sky, x, y, z) < 4;
    }
