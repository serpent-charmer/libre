...
public class TileEntityHeatRelay
    extends TileEntityAdjacencyUpgrade implements IHeatSource {
    ...
    private static final double[] factors = {
        0.03125, 0.0625, 0.125, 0.25, 0.375, 0.5, 0.75, 1,
    };

    private static final int[] ic2HeatMax = {
        2, 5, 10, 20, 50, 100, 250, 1000,
    };

    private static final double[] ic2HeatEff = {
        0.1, 0.25, 0.5, 1, 1.5, 2.5, 5, 10,
    };
    ...
    private static void initHandlers() {
        ...
        if (ModList.IC2.isLoaded()) {
            ic2HUEffect = TileEntityAdjacencyUpgrade.registerEffectDescription(
                CrystalElement.ORANGE, "Transfers HU"
            );
    ...
    private static void setBlockTemp(Block b, int temp) {
        ...
        TileEntityAdjacencyUpgrade
            .registerEffectDescription(
                CrystalElement.ORANGE, "Transfers block temperature"
            )
            .addItems(new ItemStack(b))
            .setOrderIndex(-1000);
    }
    ...
    private static void addHeatEffect(TileEnum c, HeatTier tier) {
        ...
        String desc = "Balances heat between "
            + tier.getDescription().toLowerCase(Locale.ENGLISH) + " machines";
        TileEntityAdjacencyUpgrade.registerEffectDescription(CrystalElement.ORANGE, desc)
            .addItems(c.getCraftedProduct())
            .setOrderIndex(tier.ordinal());
    }
    ...
    protected void doCollectiveTick(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 6; i++) {
            BlockKey bk = BlockKey.getAt(
                world, x + dirs[i].offsetX, y + dirs[i].offsetY, z + dirs[i].offsetZ
            );
        ...
        for (ThermalTile te : set.keySet()) {
            ...
            if (hr <= tierThisCycle) {
                ...
                int t2 = (int
                ) (this.getEqualizationFactor() * Tavg
                   + (1 - this.getEqualizationFactor()) * t1);
    ...
    public int maxrequestHeatTick(ForgeDirection dir) {
        return !this.hasRedstoneSignal() && this.hasSufficientEnergy()
            ? ic2HeatMax[this.getTier()]
            : 0;
    }
    ...
    private int collectHeat(ForgeDirection skip, int seek) {
        if (seek <= 0)
            return 0;
        //ReikaJavaLibrary.pConsole(this.getWorldObj().getTotalWorldTime()+": "+seek+"
        //from "+skip+" = "+this.getAdjacentTileEntity(skip));
        int has = 0;
        for (int i = 0; i < 6; i++) {
        ...
        //ReikaJavaLibrary.pConsole(this.getWorldObj().getTotalWorldTime()+": got
        //"+(int)(has*ic2HeatEff[this.getTier()]));
