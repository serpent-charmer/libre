...
public class TileEntityAccelerator extends TileEntityAdjacencyUpgrade {
    ...
    private static HashMap<Class<? extends TileEntity>, Acceleration> actions
        = new HashMap();
    ...
    private static void initHandlers() {
        if (blacklist != null)
            return;
        defaultEffect = TileEntityAdjacencyUpgrade.registerEffectDescription(
            CrystalElement.LIGHTBLUE, "Accelerates operations"
        );
        defaultEffect.setOrderIndex(Integer.MIN_VALUE)
            .addDisplays(new DefaultEffectDisplay());
        blacklist = TileEntityAdjacencyUpgrade.registerEffectDescription(
            CrystalElement.LIGHTBLUE, "Does nothing"
        );

        blacklistTile(
            "icbm.sentry.turret.Blocks.TileTurret", ModList.ICBM, "", BlacklistReason.BUGS
        ); //by request
        blacklistTile(
            "bluedart.tile.decor.TileForceTorch",
            ModList.DARTCRAFT,
            "",
            BlacklistReason.CRASH
        ); //StackOverflow
        blacklistTile(
            "com.sci.torcherino.tile.TileTorcherino",
            null,
            "Torcherino:tile.torcherino",
            BlacklistReason.CRASH
        ); //StackOverflow
        blacklistTile(
            "com.sci.torcherino.tile.TileTorcherino",
            null,
            "Torcherino:tile.inverse_torcherino",
            BlacklistReason.CRASH
        ); //StackOverflow
        blacklistTile(
            "com.sci.torcherino.tile.TileTorcherino",
            null,
            "Torcherino:tile.compressed_torcherino",
            BlacklistReason.CRASH
        ); //StackOverflow
        blacklistTile(
            "com.sci.torcherino.tile.TileTorcherino",
        ...
        blacklistTile(
            "com.sci.torcherino.tile.TileTorcherino",
            null,
            "Torcherino:tile.double_compressed_inverse_torcherino",
            BlacklistReason.CRASH
        ); //StackOverflow
        ...
        blacklistTile(
            ...
            BlacklistReason.BUGS
        );
        blacklistTile(
            "mrtjp.projectred.integration.Sequencer",
            ModList.PROJRED,
            "ProjRed|Integration:projectred.integration.gate:18",
            BlacklistReason.BUGS
        );
        blacklistTile(
            "mrtjp.projectred.integration.Repeater",
            ModList.PROJRED,
            "ProjRed|Integration:projectred.integration.gate:10",
            BlacklistReason.BUGS
        );
        blacklistTile(
            "mrtjp.projectred.integration.PulseFormer",
            ModList.PROJRED,
            "ProjRed|Integration:projectred.integration.gate:9",
            BlacklistReason.BUGS
        );
        blacklistTile(
            "mrtjp.projectred.integration.StateCell",
            ModList.PROJRED,
            "ProjRed|Integration:projectred.integration.gate:20",
            BlacklistReason.BUGS
        );
        blacklistTile(
            "mrtjp.projectred.integration.Synchronizer",
            ModList.PROJRED,
            "ProjRed|Integration:projectred.integration.gate:21",
            BlacklistReason.BUGS
        );
    ...
    private static class BlacklistDisplay extends GuiStackListDisplay {
        ...
        public void draw(FontRenderer fr, int x, int y) {
            ...
            ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                x, y, ChromaIcons.X.getIcon(), 16, 16
            );
            GL11.glPopAttrib();
        }
    }
    ...
    private static void
    blacklistTile(String name, ModList mod, String s, BlacklistReason r) {
        initHandlers();
        Class cl;
        ...
        try {
            ...
            if (cl == TileEntity.class)
                throw new IllegalArgumentException(
                    "You cannot blacklist the core TE class!"
                );
            ChromatiCraft.logger.log(
                "TileEntity \"" + name
                + "\" has been blacklisted from the TileEntity Accelerator, because "
                + r.message
            );
            actions.put(cl, blacklistKey);
            ...
            ChromatiCraft.logger.log(
                "Could not add " + name
                + " to the Accelerator blacklist: Class not found!"
            );
        }
    }
    ...
    @Override
    protected EffectResult
    tickDirection(World world, int x, int y, int z, ForgeDirection dir, long time) {
        if (lagTimer[dir.ordinal()] > 0) {
            ReikaParticleHelper.CLOUD.spawnAroundBlock(
                world, x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ, 4
            );
    ...
    private void logLagWarning(long time, ForgeDirection dir, TileEntity te) {
        ...
        if (ticksSinceLagPausedWarning >= 20) {
            ...
            String s = "Tile Accelerator "+this+" is self-throttling due to lag ("+dur+"
        ns), accelerating "+te; ChromatiCraft.logger.log(s); ticksSinceLagPausedWarning =
        0;
        }*/
    ...
    private static Acceleration getAccelerate(Class<? extends TileEntity> c) {
        ...
        if (a != null) {
            if (a == blacklistKey)
                ChromatiCraft.logger.log(
                    "Calculated acceleration blacklist " + a + " for tile class " + c
                );
            else
                ChromatiCraft.logger.log(
                    "Calculated acceleration mode " + a + " for tile class " + c
                );
        }
        actions.put(c, a);
        return a;
    ...
    private static Acceleration calculateAccelerate(Class<? extends TileEntity> c) {
        ...
        if (s.contains("conduit") || (s.contains("wire") && !s.contains("wireless"))
            || s.contains("cable")) { //almost always part of a network object
            return blacklistKey;
        }
        if (s.contains("solar")) { //power exploit
            return blacklistKey;
        }
        if (s.contains("windturbine") || s.contains("wind turbine")
            || s.contains("windmill") || s.contains("wind mill")) { //power exploit
            return blacklistKey;
        }
    ...
    private static abstract class Acceleration {
        ...
        protected abstract void tick(TileEntity te, int factor, TileEntity accelerator)
            throws Exception;
    ...
    public static abstract class SpecialAcceleration extends Acceleration {
        protected SpecialAcceleration() {
            TileEntityAdjacencyUpgrade
                .registerEffectDescription(
                    CrystalElement.LIGHTBLUE, this.getDescription()
                )
                .addDisplays(this.getRelevantItems());
        }
    ...
    private static final class APIAcceleration extends Acceleration {
        ...
        private APIAcceleration(CustomAcceleration acc) {
            accel = acc;
            TileEntityAdjacencyUpgrade
                .registerEffectDescription(CrystalElement.LIGHTBLUE, acc.getDescription())
                .addItems(acc.getItems());
        }
