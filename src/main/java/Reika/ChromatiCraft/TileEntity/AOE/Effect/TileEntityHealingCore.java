...
public class TileEntityHealingCore extends TileEntityAdjacencyUpgrade {
    private static final HashMap<BlockKey, BaseRepairInterface> blockInteractions
        = new HashMap();
    ...
    tickDirection(World world, int x, int y, int z, ForgeDirection dir, long startTime) {
        ...
        if (bi instanceof BlockRepairInterface && bi != NoInterface.instance
            && (bi.runOnClient() || !world.isRemote)) {
            try {
                ((BlockRepairInterface) bi).tick(world, dx, dy, dz, tier);
            } catch (Exception ex) {
                ChromatiCraft.logger.logError(
                    "Could not tick repair interface " + bi + " for "
                    + BlockKey.getAt(world, dx, dy, dz) + " @ " + this
                );
                this.writeError(ex);
            }
        ...
        if (te != null) {
            ...
            if (s instanceof TileRepairInterface && s != NoInterface.instance
                && (s.runOnClient() || !world.isRemote)) {
                try {
                    ...
                    ChromatiCraft.logger.logError(
                        "Could not tick repair interface " + s + " for " + te + " @ "
                        + this
                    );
                    this.writeError(ex);
                }
    ...
    private static abstract class BaseRepairInterface {
        protected BaseRepairInterface(
            String desc, Consumer<AdjacencyEffectDescription> addItems
        ) {
            if (this.isActive()) {
                addItems.accept(TileEntityAdjacencyUpgrade.registerEffectDescription(
                    CrystalElement.MAGENTA, desc
                ));
            }
        }
    ...
    private static abstract class BlockRepairInterface extends BaseRepairInterface {
        protected BlockRepairInterface(
            String desc, Consumer<AdjacencyEffectDescription> addItems
        ) {
            super(desc, addItems);
        }

        protected abstract void tick(World world, int x, int y, int z, int tier)
            throws Exception;
    }
    ...
    private static class AnvilRepairInterface extends BlockRepairInterface {
        protected AnvilRepairInterface() {
            super(
                "Repairs damage",
                adj -> adj.addDisplays(new GuiStackDisplay(Blocks.anvil))
            );
        }
    ...
    private static abstract class FieldSetRepairInterface extends TileRepairInterface {
        protected FieldSetRepairInterface(
            String desc, Consumer<AdjacencyEffectDescription> addItems
        ) {
            super(desc, addItems);
        }
        ...
        getReplacedValue(TileEntity te, int tier, Number original) throws Exception;
    ...
    private static abstract class ItemSlotRepairInterface extends TileRepairInterface {
        protected ItemSlotRepairInterface(
            String desc, Consumer<AdjacencyEffectDescription> addItems
        ) {
            super(desc, addItems);
        }
    ...
    private static class RailTurbineInterface extends ItemSlotRepairInterface {
        private Field
            inventory; //ranges from 0 at 0.00% to 100k at 100% -> each 1000 is 1%

        private RailTurbineInterface() {
            super(
                "Repairs turbine rotors",
                adj -> adj.addDisplays(new GuiStackDisplay("Railcraft:machine.alpha:1"))
            );
        }
        ...
        public boolean shouldRepairItem(ItemStack is) {
            return is.getItemDamage() > 0
                ...
                == GameRegistry.findItem(
                    ModList.RAILCRAFT.modLabel, "part.turbine.rotor"
                );
        }
    ...
    private static class DecalcificationInterface extends FieldSetRepairInterface {
        private Field
            calcification; //ranges from 0 at 0.00% to 100k at 100% -> each 1000 is 1%

        private DecalcificationInterface() {
            super(
                "Decalcification",
                adj -> adj.addDisplays(new GuiStackDisplay("IC2:blockMachine3"))
            );
        }
        ...
        @Override
        protected double getReplacedValue(TileEntity te, int tier, Number original)
            throws Exception {
    ...
    private static final class APIHealingBlock extends BlockRepairInterface {
        ...
        private APIHealingBlock(CustomBlockHealing acc) {
            super(acc.getDescription(), adj -> adj.addItems(acc.getItems()));
            effect = acc;
        }
