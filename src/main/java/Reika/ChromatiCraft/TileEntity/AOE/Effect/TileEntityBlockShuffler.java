...
public class TileEntityBlockShuffler extends TileEntityAdjacencyUpgrade {
    /** Tick delays */
    private static final int[] timing = { 600, 100, 40, 20, 8, 4, 2, 1 };
    ...
    private void doShuffle(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            Coordinate c
                = new Coordinate(x + dir.offsetX, y + dir.offsetY, z + dir.offsetZ);
            BlockKey bk = c.getBlockKey(world);
            if (this.canMove(
                    world, c.xCoord, c.yCoord, c.zCoord, bk.blockID, bk.metadata
                )) {
                blocks.put(c, bk);
                li.add(c);
            }
        ...
        if (li.size() > 1) {
            ...
            while (!li.isEmpty() && !keys.isEmpty()) {
                ...
                ReikaSoundHelper.playBreakSound(
                    world, dest.xCoord, dest.yCoord, dest.zCoord, bk.blockID
                );
