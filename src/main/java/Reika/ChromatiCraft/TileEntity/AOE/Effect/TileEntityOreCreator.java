...
public class TileEntityOreCreator extends TileEntityAdjacencyUpgrade {
    private static final ArrayList<OreType>[][] oreLists
        = new ArrayList[OreRarity.values().length][3];

    private static final OreRarity[] rarities = {
        OreRarity.EVERYWHERE, OreRarity.EVERYWHERE, OreRarity.COMMON, OreRarity.AVERAGE,
        OreRarity.SCATTERED,  OreRarity.SCARCE,     OreRarity.SCARCE, OreRarity.RARE,
    };
    ...
    private static boolean isItemStackGenerationPermitted(ItemStack is) {
        if (ModList.TINKERER.isLoaded()
            && ReikaItemHelper.matchStackWithBlock(
                is, TinkerBlockHandler.getInstance().gravelOreID
            ))
            return false;
        ...
        if (ModList.RAILCRAFT.isLoaded()
            && RailcraftHandler.getInstance().isDarkOre(b, is.getItemDamage()))
            return false;
        if (ModList.APPENG.isLoaded()
            && ReikaItemHelper.matchStackWithBlock(
                is, AppEngHandler.getInstance().chargedCertusOre
            ))
            return false;
    ...
    private void doCreateOre(World world, int x, int y, int z, OreType ore) {
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            ChromaPackets.ORECREATE.ordinal(),
            world,
            x,
            y,
            z,
    ...
    public static void
    doOreCreationFX(World world, int x, int y, int z, int id, int meta) {
