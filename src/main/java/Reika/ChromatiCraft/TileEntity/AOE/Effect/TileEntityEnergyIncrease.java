...
public class TileEntityEnergyIncrease
    extends TileEntityAdjacencyUpgrade implements BreakAction {
    private static final HashMap<Class, EnergyInterface> interactions = new HashMap();

    public static final AdjacencyEffectDescription basicEnergy
        = TileEntityAdjacencyUpgrade.registerEffectDescription(
            CrystalElement.YELLOW, "Increases energy generation rate"
        );

    private static double[] factors = {
        0.125, 0.25, 0.5, 1, 3, 7, 15, 31,
    };
            ...
            if (e != NoInterface.instance) {
                ...
                try {
                    ...
                    ChromatiCraft.logger.logError(
                        "Could not tick energy interface " + e + " for " + te + " @ "
                        + this
                    );
                    this.writeError(ex);
                }
    ...
    private static abstract class EnergyInterface {
        protected EnergyInterface() {
            if (this.getMod() == null || this.getMod().isLoaded()) {
                try {
                    ...
                    for (int i = 0; i < cs.length; i++) {
                        try {
                            ...
                            ChromatiCraft.logger.logError(
                                "Could not find class " + cs[i] + " for " + this.getMod()
                            );
                        }
                    }
                    this.init();
                    ...
                    ChromatiCraft.logger.logError(
                        "Could not load " + this + " for " + this.getMod() + ":"
                    );
                    e.printStackTrace();
                ...
                ChromatiCraft.logger.log(
                    "Not loading " + this + " for " + this.getMod() + "; Mod not present."
                );
            }
        }

        protected abstract void
        tick(TileEntityEnergyIncrease booster, TileEntity te, double boost)
            throws Exception;
        ...
        protected final void scaleNumberInField(TileEntity te, Field f, double boost)
            throws Exception {
        ...
        setNumberToField(TileEntity te, Field f, Number prev, double val)
            throws Exception {
            Object set = null;
    ...
    private static abstract class EnergyInjectionInterface extends BasicEnergyInterface {
        ...
        tick(TileEntityEnergyIncrease booster, TileEntity te, double boost)
            throws Exception {
            double gen = this.getBaseGeneration(te)
                * boost; //not 1+ since tile is still doing its own 1x
            this.inject(te, gen);
        }
    ...
    private static abstract class EnergyDumpInterface extends BasicEnergyInterface {
        ...
        tick(TileEntityEnergyIncrease booster, TileEntity te, double boost)
            ...
            TileEntity rec = te.worldObj.getTileEntity(
                te.xCoord + dir.offsetX, te.yCoord + dir.offsetY, te.zCoord + dir.offsetZ
            );
            if (rec != null)
        ...
        protected abstract void dumpEnergy(TileEntity receiver, double gen)
            throws Exception;
    }
    ...
    private static class NoInterface extends EnergyInterface { //Used for null
        ...
        @Override
        protected void tick(TileEntityEnergyIncrease booster, TileEntity te, double boost)
            throws Exception {}
    ...
    private static class IC2GeneratorInterface extends EnergyInjectionInterface {
        ...
        protected void inject(TileEntity te, double gen) throws Exception {
            storage.set(
                te, Math.min(storage.getDouble(te) + gen, maxStorage.getShort(te))
            );
        }
    ...
    private static class IC2ReactorInterface extends EnergyInterface {
        ...
        protected void init() throws Exception {
            ...
            Class c2 =
            Class.forName("ic2.core.block.reactor.tileentity.TileEntityReactorChamberElectric");
            ...
            TileEntityAdjacencyUpgrade
                .registerEffectDescription(
                    CrystalElement.YELLOW, "Increases reactor output"
                )
                .addDisplays(new GuiStackDisplay("IC2:blockGenerator:5"));
        }
    ...
    private static class TEDynamoInterface extends EnergyInjectionInterface {
        ...
        protected void init() throws Exception {
            ...
            Class c2
                = Class.forName("cofh.thermalexpansion.block.TileTEBase$EnergyConfig");
    ...
    private static class EiOGeneratorInterface extends EnergyInterface {
        ...
        protected String[] getClasses() {
            return new
    String[]{"crazypants.enderio.machine.generator.AbstractGeneratorEntity"};
        }
    ...
    private static class RailCraftEngineInterface extends EnergyInjectionInterface {
        ...
        protected void init() throws Exception {
            Class c = Class.forName(
                "mods.railcraft.common.blocks.machine.beta.TileEngineSteam"
            );
            getMaxOutputRF = c.getDeclaredMethod("getMaxOutputRF");
            getMaxOutputRF.setAccessible(true);
            Class c2
                = Class.forName("mods.railcraft.common.blocks.machine.beta.TileEngine");
        ...
        protected void inject(TileEntity te, double gen) throws Exception {
            energy.setInt(
                te, Math.min(energy.getInt(te) + (int) gen, (int) maxEnergy.invoke(te))
            );
        }
    ...
    private static class RailCraftTurbineInterface extends EnergyInjectionInterface {
        ...
        protected void init() throws Exception {
            Class c = Class.forName(
                "mods.railcraft.common.blocks.machine.alpha.TileSteamTurbine"
            );
            output = c.getDeclaredField("output");
            output.setAccessible(true);
            ...
            energy.setAccessible(true);
            Class c2
                = Class.forName("mods.railcraft.common.blocks.machine.TileMultiBlock");
    ...
    private static class ExUGeneratorInterface extends EnergyInjectionInterface {
        ...
        protected void init() throws Exception {
            Class c = Class.forName(
                "com.rwtema.extrautils.tileentity.generators.TileEntityGenerator"
            );
    ...
    private static class HexGeneratorInterface extends EnergyInterface {
        ...
        @Override
        protected String[] getClasses() {
            return new String[]{"com.celestek.hexcraft.tileentity.TileHexoriumGenerator",
    "com.celestek.hexcraft.tileentity.HexEnergySource"};
        }

        @Override
        protected final void tick(TileEntityEnergyIncrease booster, TileEntity te, double
    boost) throws Exception { te = this.getActingTileEntity(te); if (booster.cachedValue
    != null) booster.cachedValue = new ImmutableTriple(); this.scaleNumberInField(te,
    perTick, boost); this.scaleNumberInField(te, producibleAmount, boost);
