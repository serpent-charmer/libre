...
public class TileEntityHoverPad extends TileEntityChromaticBase {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (this.hasRedstoneSignal()) {
            ...
            List<EntityPlayer> li
                = hoverBox.getEntitiesWithinAABB(EntityPlayer.class, world);
    ...
    public void clearBox() {
        ...
        if (!worldObj.isRemote) {
            ReikaPacketHelper.sendDataPacketWithRadius(
                ...
                ChromaPackets.CLEARHOVERBOX.ordinal(),
                this,
                192
            );
        }
    ...
    private CompoundAABB calculateBox(World world, int x, int y, int z) {
        ...
        b.recursiveAddMultipleWithBounds(
            world, x, y, z, ids, x - 32, y, z - 32, x + 32, y, z + 32
        );
        ...
        for (Coordinate c : set) {
            ...
            for (int i = 1; i < 24; i++) {
                ...
                if (bk.getCollisionBoundingBoxFromPool(
    ...
    private static class ParticleField {
        ...
        @SideOnly(Side.CLIENT)
        private void
        doParticles(World world, int x, int y, int z, TileEntityHoverPad te) {
            int n = mode.getParticleRate(te);
            for (int i = 0; i < n; i++) {
                if ((te.getTicksExisted() + i * mode.particleModulo / n)
                        % mode.particleModulo
                    == 0) {
                    AxisAlignedBB box = te.hoverBox.getRandomComponentBox(true);
                    particleX[i] = ReikaRandomHelper.getRandomBetween(box.minX, box.maxX);
                ...
                EntityCCBlurFX fx = new EntityCCBlurFX(
                    world, particleX[i], particleY[i], particleZ[i], vx, v, vz
                );
