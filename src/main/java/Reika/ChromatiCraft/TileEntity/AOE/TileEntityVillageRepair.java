    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (village != null && world.isRemote) {
            for (int i = 0; i < 16; i++) {
                ...
                double px =
        village.getCenter().posX+0.5+village.getVillageRadius()*Math.cos(a); double pz =
        village.getCenter().posZ+0.5+village.getVillageRadius()*Math.sin(a); EntityFX fx =
        new EntityBlurFX(world, px, y, pz).setScale(2).setGravity(-0.03125F);
