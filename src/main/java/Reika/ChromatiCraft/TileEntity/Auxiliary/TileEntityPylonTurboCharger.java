...
public class TileEntityPylonTurboCharger extends TileEntityPylonEnhancer {
            ...
            if (location == null && ritualTick > 0) {
                if (revTick > 0) {
                    ...
                    if (revTick < 16) {
                        ...
                        TileEntityPylonTurboCharger te
                            = (TileEntityPylonTurboCharger) c.getTileEntity(world);
    ...
    private void doRevEffects(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < Location.list.length; i++) {
            ...
            EntityFX fx
                = new EntityCenterBlurFX(te.getColor(), world, px, py, pz, vx, vy, vz)
                      .setScale(s);
    ...
    private void findLocation(World world, int x, int y, int z) {
        for (int i = 0; i < Location.list.length; i++) {
            Location loc = Location.list[i];
            Coordinate c = loc.getRelativePylonLocation().offset(
                x, y, z
            ); //loc.position.negate().offset(x, y, z);
            TileEntity te = c.getTileEntity(world);
            if (te instanceof TileEntityCrystalPylon
                && ((TileEntityCrystalPylon) te).canConduct()) {
                /*
            if (te instanceof TileEntityPylonTurboCharger) {
                if (((TileEntityPylonTurboCharger)te).getPylon(world, te.xCoord,
            te.yCoord, te.zCoord) != null) {*/
                location = loc;
                return;
                //}
            }
    ...
    private void doRitualTick(World world, int x, int y, int z) {
        ...
        if (te == null) {
            ...
            if ((RITUAL_LENGTH - ritualTick) % 200 == 0) {
                ChromaSounds.PYLONBOOSTRITUAL.playSoundAtBlockNoAttenuation(
                    this, 0.75F, 1, EFFECT_RANGE
                );
            }
    ...
    private boolean triggerEffect(int min, double factor, int sub) {
        if (ritualTick < min || (RITUAL_LENGTH - ritualTick) < min)
            return false;
        return ritualTick % ((int) (2 * Math.max(min, min * factor))) == 0
            || rand.nextInt(Math.max(min, (int) (ritualTick * factor) - sub)) == 0;
    }
    ...
    private void failRitual(World world, int x, int y, int z) {
        ...
        worldObj.addWeatherEffect(
            new EntityLightningBolt(world, x + 0.5, y + 8.5, z + 0.5)
        );
    ...
    private void doCompleteParticles() {
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            ChromaPackets.PYLONTURBOCOMPLETE.ordinal(),
            this,
            EFFECT_RANGE
        );
    }
    ...
    private void doEvent(World world, int x, int y, int z, EventType type) {
        ...
        switch (type) {
            case FLASH:
                ...
                if (rand.nextBoolean())
                    CrystalNetworker.instance.overloadColorConnectedTo(
                        te, te.getColor(), 1 + rand.nextInt(1), true
                    );
                else
            ...
            case JETS:
                if (jetTick % 4 == 0)
                    ChromaSounds.INFUSE.playSoundAtBlockNoAttenuation(
                        this.getPylon(world, x, y, z), 0.5F, 0.5F, EFFECT_RANGE
                    );
                break;
            case PARTICLEBURST:
                //this.rockScreen(6);
                //ChromaSounds.CAST.playSoundAtBlockNoAttenuation(this.getPylon(world, x,
                //y, z), 1, 1);
                break;
            case SKY:
                dat = skyTick;
            ...
            case POTIONS:
                ...
                AxisAlignedBB box = AxisAlignedBB.getBoundingBox(
                    x - 12, y - 2, z - 12, x + 1 + 12, y + 1 + 24, z + 1 + 12
                );
                List<EntityLivingBase> li
                    = world.getEntitiesWithinAABB(EntityLivingBase.class, box);
                for (EntityLivingBase e : li) {
                    PotionEffect eff
                        = CrystalPotionController.instance.getEffectFromColor(
                            te.getColor(), 200, 4, false
                        );
                    if (eff != null) {
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ChromatiCraft.packetChannel,
            ChromaPackets.PYLONTURBOEVENT.ordinal(),
            this,
            EFFECT_RANGE,
            type.ordinal(),
            dat
    ...
    private void doFailParticles(boolean hasPylon) {
        ReikaPacketHelper.sendDataPacketWithRadius(
            ChromatiCraft.packetChannel,
            ChromaPackets.PYLONTURBOFAIL.ordinal(),
            this,
            EFFECT_RANGE,
            hasPylon ? 1 : 0
        );
    ...
    @SideOnly(Side.CLIENT)
    public void
    doFailParticlesClient(World world, int x, int y, int z, boolean hasPylon) {
        ...
        if (hasPylon) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                EntityFX fx = new EntityCCBlurFX(world, px, py, pz, vx, vy, vz)
                                  .fadeColors(0xffffff, te.getColor().getColor())
                                  .setScale(s)
                                  .setLife(l)
                                  .setRapidExpand();
                Minecraft.getMinecraft().effectRenderer.addEffect(fx);
    ...
    public void doCompleteParticlesClient(World world, int x, int y, int z) {
        ...
        for (double a1 = 0; a1 < 360; a1 += da) {
            for (double a2 = -90 + da; a2 < 90 - da; a2 += da) {
                ...
                EntityCCBlurFX fx = new EntityCCBlurFX(
                    world, x + 0.5, y + 0.5 + 8, z + 0.5, xyz[0], xyz[1], xyz[2]
                );
                fx.setRapidExpand().setScale(s).setLife(l).setGravity(0).fadeColors(
                    0xffffff, c1
                );
                IIcon ico = null;
        ...
        int c1 = this.getPylon(world, x, y, z).getColor().getColor();
        int l = 30 + rand.nextInt(20);
        EntityFX fx = new EntityCCBlurFX(world, x + 0.5, y + 0.5 + 8, z + 0.5, 0, v, 0)
                          .setRapidExpand()
                          .setScale(s)
                          .setLife(l)
                          .setGravity(0)
                          .fadeColors(0xffffff, c1);
        Minecraft.getMinecraft().effectRenderer.addEffect(fx);

        s = (float) ReikaRandomHelper.getRandomBetween(2D, 3D);
        l = 30 + rand.nextInt(20);
        v = ReikaRandomHelper.getRandomPlusMinus(0.125, 0.0625);
        fx = new EntityCCBlurFX(world, x + 0.5, y + 0.5 + 8, z + 0.5, 0, -v, 0)
                 .setRapidExpand()
                 .setScale(s)
                 .setLife(l)
                 .setGravity(0)
        ...
        switch (evt) {
            case FLASH: {
                ...
                ReikaSoundHelper.playClientSound(
                    ChromaSounds.PYLONFLASH, x + 0.5, y + 0.5, z + 0.5, 1, 1, false
                ); //play sound here to ensure sync
            ...
            case PARTICLEBURST: {
                ...
                ReikaSoundHelper.playClientSound(
                    ChromaSounds.CAST, x + 0.5, y + 0.5, z + 0.5, 1, 1, false
                ); //play sound here to ensure sync
                for (int a = 0; a < 360; a += 2) {
                    ...
                    for (int i = 0; i < n; i++) {
                        ...
                        int l = 40 + rand.nextInt(120);
                        float g = -(float
                        ) ReikaRandomHelper.getRandomPlusMinus(0.0625, 0.03125);
                        EntityFX fx
            ...
            case BEAM: {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    EntityFX fx
                        = new EntityLaserFX(te.getColor(), world, px, py, pz, vx, vy, vz)
                              .setScale(s);
                    Minecraft.getMinecraft().effectRenderer.addEffect(fx);

                    fx = new EntityLaserFX(
                             te.getColor(), world, px, py, pz, -vx, -vy, -vz
                    )
                             .setScale(s);
                    Minecraft.getMinecraft().effectRenderer.addEffect(fx);
                }
            ...
            case POTIONS:
                ...
                ReikaSoundHelper.playClientSound(
                    ChromaSounds.POWERDOWN, x + 0.5, y + 0.5, z + 0.5, 1, 1, false
                ); //play sound here to ensure sync
                int r = 16;
                int n = 16 + rand.nextInt(32);
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    float g
                        = -(float) ReikaRandomHelper.getRandomPlusMinus(0.125, 0.0625);
                    EntityFX fx
                        = new EntityRuneFX(
                              world, dx, y - 1 - s / 4F, dz, 0, 0.125, 0, te.getColor()
                        )
                              .setGravity(g)
                              .setLife(l)
                              .setScale(s);
    ...
    //For some reason screen rock only works if the player was recently damaged; that is
    //the real reason for triggering this effect
    private void triggerStartFX(World world, int x, int y, int z) {
        for (int i = 0; i < Location.list.length; i++) {
            Location loc = Location.list[i];
            Coordinate c = loc.position.offset(x, y, z);
            world.addWeatherEffect(
                new EntityLightningBolt(world, c.xCoord, c.yCoord, c.zCoord)
            );
        }

        AxisAlignedBB box = AxisAlignedBB.getBoundingBox(
            x - 64, y - 64, z - 64, x + 1 + 64, y + 1 + 64, z + 1 + 64
        );
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ChromatiCraft.packetChannel,
            ChromaPackets.PYLONTURBOSTART.ordinal(),
            this,
            EFFECT_RANGE
        );
    }
    ...
    public void doStartFXClient(World world, int x, int y, int z) {
        ReikaSoundHelper.playClientSound(
            ChromaSounds.PYLONBOOSTSTART, x + 0.5, y + 0.5, z + 0.5, 2, 1, false
        ); //play sound here to ensure sync
        ReikaSoundHelper.playClientSound(
            ChromaSounds.PYLONBOOSTSTART, x + 0.5, y + 0.5, z + 0.5, 2, 1, false
        ); //play sound here to ensure sync
    ...
    public TileEntityCrystalPylon getPylon(World world, int x, int y, int z) {
        ...
        return tile instanceof TileEntityCrystalPylon ? (TileEntityCrystalPylon) tile
                                                      : null;
    }
        ...
        if (this.checkPylon(world, x, y, z)) {
            if (this.canPlayerTurbocharge(world, x, y, z, ep)) {
                ...
                if (hasAuxiliaries) {
                    ...
                    ChromaSounds.PYLONBOOSTSTART.playSoundAtBlockNoAttenuation(
                        this, 1, 1, EFFECT_RANGE
                    );
                    return true;
                }
            }
    ...
    private boolean
    canPlayerTurbocharge(World world, int x, int y, int z, EntityPlayer ep) {
        TileEntityCrystalPylon te = this.getPylon(world, x, y, z);
        return ProgressionManager.instance.hasPlayerCompletedStructureColor(
            ep, te.getColor()
        ); //ProgressStage.CTM.isPlayerAtStage(ep);
    }

    private boolean checkPylon(World world, int x, int y, int z) {
        TileEntityCrystalPylon te = this.getPylon(world, x, y, z);
        if (te != null) {
            return !te.isEnhanced()
                && te.getEnergy(te.getColor())
                >= (TileEntityCrystalPylon.MAX_ENERGY * 3 / 4)
                && te.canConduct()
                && te.getBoosterCrystals(world, x, y + 8, z, true).size() == 8
                && te.worldObj.canBlockSeeTheSky(te.xCoord, te.yCoord + 1, te.zCoord);
        }
        return false;
    }
    ...
    private boolean canFunction(World world, int x, int y, int z) {
        if (world.getBlock(x, y - 1, z) == ChromaBlocks.PYLONSTRUCT.getBlockInstance()
            ...
            if (world.getBlock(x, y - 2, z) == ChromaBlocks.PYLONSTRUCT.getBlockInstance()
                && world.getBlockMetadata(x, y - 2, z) == 2) {
                if (world.canBlockSeeTheSky(x, y + 1, z)) {
                    ...
                    Coordinate end
                        = l.position.offset(location.position.negate()).offset(x, y, z);
                    return end.getTileEntity(world) instanceof TileEntityPylonTurboCharger
                        && PylonFinder
                               .lineOfSight(
                                   world,
                                   x,
            ...
            if (location != null) {
                ...
                if (te instanceof TileEntityPylonTurboCharger) {
                    ...
                    TileEntityCrystalPylon tp
                        = tc.getPylon(tc.worldObj, tc.xCoord, tc.yCoord, tc.zCoord);
