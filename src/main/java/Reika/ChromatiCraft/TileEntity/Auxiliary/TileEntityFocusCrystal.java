...
public class TileEntityFocusCrystal
    extends TileEntityChromaticBase implements NBTTile, BreakAction {
    ...
    public static enum CrystalTier {
        ...
        public int getEffectiveOrdinal() {
            return this == TURBOCHARGED ? EXQUISITE.getEffectiveOrdinal()
                                        : this.ordinal();
        }
    }
    ...
    public static float
    getSummedFocusFactorDirect(FocusAcceleratable f, Collection<Coordinate> locations) {
        TileEntity te = (TileEntity) f;
        return getSummedFocusFactorDirect(
            f, te.worldObj, te.xCoord, te.yCoord, te.zCoord, locations
        );
    }

    public static float
    getSummedFocusFactor(FocusAcceleratable f, Collection<FocusLocation> locations) {
        TileEntity te = (TileEntity) f;
        return getSummedFocusFactor(
            f, te.worldObj, te.xCoord, te.yCoord, te.zCoord, locations
        );
    }

    public static float getSummedFocusFactorDirect(
        FocusAcceleratable acc,
        World world,
        int x,
        int y,
        int z,
        Collection<Coordinate> locations
    ) {
        float sum = 1;
        for (Coordinate c : locations) {
            c = c.offset(x, y, z);
            if (ChromaTiles.getTile(world, c.xCoord, c.yCoord, c.zCoord)
                == ChromaTiles.FOCUSCRYSTAL) {
                TileEntityFocusCrystal te
                    = (TileEntityFocusCrystal) c.getTileEntity(world);
    ...
    public static float getSummedFocusFactor(
        FocusAcceleratable acc,
        World world,
        int x,
        int y,
        int z,
        Collection<FocusLocation> locations
    ) {
        float sum = 1;
    ...
    public void addConnection(FocusAcceleratable src, boolean sync) {
        connection
            = new FocusConnection(new SimpleFocusLocation(src, this), (TileEntity) src);
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (connection != null && this.getTicksExisted() % 8 == 0) {
            if (this.getConnection().target.getTileEntity(world)
                    instanceof FocusAcceleratable) {
            } else {
                connection = null;
            }
        }
    ...
    private void doParticles(World world, int x, int y, int z) {
        if (rand.nextInt(2 + Minecraft.getMinecraft().gameSettings.particleSetting)
            == 0) {
            double hr = 0.825;
            double vr = 0.375;
            ...
            int c = this.getTier().getRenderColor(
                this.getTicksExisted() + ReikaRenderHelper.getPartialTickTime()
