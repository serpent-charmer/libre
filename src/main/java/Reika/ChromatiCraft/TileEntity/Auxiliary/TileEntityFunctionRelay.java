...
public class TileEntityFunctionRelay extends TileEntityChromaticBase {
    private static final TreeMap<String, RelayedEffectDescription> specialEffects
        = new TreeMap();
    ...
    public static void setDefaultEffects() {
        setSpecialEffect(
            "Expands bookshelf search", new ItemStack(Blocks.enchanting_table)
        );
        ...
        for (double d = a; d < 360 + a; d += 15) {
            for (double d2 = -90 + a2; d2 <= 90 + a2; d2 += 15) {
                double r = 1.125
                    + 0.125
                        * Math.sin(
                            this.getTicksExisted() / 8D + d / (3 * Math.PI - 0.01)
                            + d2 / (4 * Math.PI - 0.01)
                        );
                double[] xyz
                    = ReikaPhysicsHelper.polarToCartesian(r, theta + d2, phi + d);
                double px = x + 0.5 + xyz[0];
                double py = y + 0.5 + xyz[1];
                double pz = z + 0.5 + xyz[2];
                EntityBlurFX fx
                    = new EntityCCBlurFX(world, px, py, pz).setLife(l).setAlphaFading();
                ColorBlendList cbl = new ColorBlendList(
                    l * 4 /*/4F*/, 0xff00ff, 0xffff00, 0xff00ff, 0x00ffff
                );
                fx.setColor(
                    cbl.getColor(this.getTicksExisted() + Math.abs((int) (d * d2 / 512D)))
                );
                //fx.setPositionController(new PulsingSpherePositionController(l*4, x+0.5,
                //y+0.5, z+0.5, 1, 1.25, theta, phi)).setColorController(new
                //BlendListColorController(cbl));
    ...
    public Coordinate getRandomCoordinate() {
        return activeCoords.isEmpty()
            ? null
            : activeCoords.get(rand.nextInt(activeCoords.size()));
    }
    ...
    public float getEnchantPowerInRange(World world, int x, int y, int z) {
        ...
        for (j = -1; j <= 1; ++j) {
            for (int k = -1; k <= 1; ++k) {
                if ((j != 0 || k != 0) && world.isAirBlock(x+k, y, z+j) &&
        world.isAirBlock(x+k, y+1, z+j)) { power += ForgeHooks.getEnchantPower(world,
        x+k*2, y,    z+j*2); power += ForgeHooks.getEnchantPower(world, x+k*2, y+1,
        z+j*2);
