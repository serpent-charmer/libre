...
public class TileEntityLumenWire extends TileEntityChromaticBase
    implements BreakAction, SidePlacedTile, CustomHitbox, RedstoneTile {
    ...
    private void toggle(World world, int x, int y, int z, boolean on) {
        ...
        if (change)
            ReikaSoundHelper.playSoundAtBlock(
                world, x, y, z, "random.click", 1F, on ? 0.75F : 0.675F
            );
        if (on)
        ...
        if (change) {
            ...
            TileEntityLumenWire te
                = (TileEntityLumenWire) connection.getTileEntity(world);
            if (te != null)
                te.causeUpdates(
                    world, connection.xCoord, connection.yCoord, connection.zCoord
                );
            ...
            MinecraftForge.EVENT_BUS.post(
                new LumenWireToggleEvent(world, x, y, z, connectionUID, on)
            );
    ...
    private void causeUpdates(World world, int x, int y, int z) {
        ...
        ReikaWorldHelper.causeAdjacentUpdates(
            world,
            x - this.getFacing().offsetX,
            y - this.getFacing().offsetY,
            z - this.getFacing().offsetZ
        );
    }
    ...
    private void sendOCActivation(World world, int x, int y, int z, boolean on) {
        if (!world.isRemote) {
            ...
            Node n = this.node();
            Network.newPacket(
                n != null ? n.address() : "NULL",
                null,
                500,
                new Object[] { tag, connectionUID.toString(), on }
            );
            if (n != null)
    ...
    private void doConnectionParticles(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            /*
            EntityBlurFX fx = new EntityBlurFX(world, dx, dy,
            dz).setLife(l).setColor(check.renderColor).setScale(s); EntityBlurFX fx2 = new
            EntityBlurFX(world, dx, dy, dz).setLife(l).setColor(0xffffff).setScale(s/2);
            if (rand.nextBoolean()) {
                fx.setRapidExpand();
                fx2.setRapidExpand();
            }
            ...
            particles.addEffect(
                dx - x, dy - y, dz - z, ico.getIcon(), l, s, check.renderColor
            );
            particles.addEffect(
                dx - x, dy - y, dz - z, ico.getIcon(), l, s / 2F, 0xffffff
            );
        }
    }
    ...
    private void doActiveParticles(World world, int x, int y, int z) {
        ...
        int p = rand.nextInt(4);
        int hue = ReikaRandomHelper.getRandomPlusMinus(
            ReikaColorAPI.getHue(check.renderColor), 60
        );
        int c = ReikaColorAPI.getModifiedHue(check.renderColor, hue);
        EntityBlurFX fx = new EntityCCBlurFX(world, dx, dy, dz)
                              .setIcon(ChromaIcons.ROSES_WHITE)
                              .setLife(l)
                              .setColor(c)
                              .setScale(s);
        Minecraft.getMinecraft().effectRenderer.addEffect(fx);
    ...
    private void checkConnection(World world, int x, int y, int z) {
        ...
        for (int i = 1; i <= MAX_LENGTH; i++) {
            ...
            if (t == this.getTile()) {
                TileEntityLumenWire te
                    = (TileEntityLumenWire) world.getTileEntity(dx, dy, dz);
    ...
    private void disconnect(boolean callOther) {
        if (connection != null && callOther) {
            TileEntityLumenWire te
                = (TileEntityLumenWire) connection.getTileEntity(worldObj);
            if (te != null)
                te.disconnect(false);
        }
        connection = null;
        ...
        private boolean check(TileEntityLumenWire te, World world, AxisAlignedBB box) {
            switch (this) {
                ...
                case ITEM:
                    return !world
                                .getEntitiesWithinAABBExcludingEntity(
                                    null, box, ReikaEntityHelper.itemOrXPSelector
                                )
                                .isEmpty();
                case MOB:
                ...
                case OWNER:
                    ...
                    return false;
                case PASSIVE:
                    return !world.getEntitiesWithinAABB(EntityAnimal.class, box)
                                .isEmpty();
                case PLAYER:
    ...
    public boolean checkLocationValidity() {
        ...
        return loc.getBlock().isSideSolid(
            worldObj, loc.xCoord, loc.yCoord, loc.zCoord, this.getFacing()
        );
    }
    ...
    @Override
    public int
    getStrongPower(IBlockAccess world, int x, int y, int z, ForgeDirection side) {
        return activeTick > 1 ? 15 : 0;
    }

    @Override
    public int
    getWeakPower(IBlockAccess world, int x, int y, int z, ForgeDirection side) {
