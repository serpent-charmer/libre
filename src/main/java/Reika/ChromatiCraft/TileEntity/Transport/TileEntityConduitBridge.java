...
public class TileEntityConduitBridge
    extends TileEntityChromaticBase implements IFluidHandler, IEnergyHandler {
    ...
    public FluidTankInfo[] getTankInfo(ForgeDirection from) {
        return new FluidTankInfo[] { new FluidTankInfo(
            (FluidStack) this.amount(Interaction.FLUID, from),
            this.capacity(Interaction.FLUID, from)
        ) };
    }
    ...
    private int insert(Interaction a, ForgeDirection dir, Object amt, boolean simulate) {
        switch (a) {
            case FLUID:
                ...
                return tank.isEmpty() || fs.getFluid() == tank.getActualFluid()
                    ? tank.fill(fs, !simulate)
                    : 0;
            case RF:
                int val = (int) amt;
    ...
    private Object
    remove(Interaction a, ForgeDirection dir, Object amt, boolean simulate) {
        switch (a) {
            case FLUID:
                if (amt instanceof FluidStack) {
                    FluidStack fs = (FluidStack) amt;
                    return fs.getFluid() == tank.getActualFluid()
                        ? tank.drain(fs.amount, !simulate)
                        : null;
