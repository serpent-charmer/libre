...
public class TileEntityTransportWindow
    extends TileEntityChromaticBase implements BreakAction {
    ...
    public boolean canLinkTo(TileEntityTransportWindow te) {
        return this.matchRenderStates(te) && hasStructure != null
            && hasStructure.equals(te.hasStructure);
    }
        ...
        if (!world.isRemote) {
            if (hasStructure != null) {
                ...
                if (te != null) {
                    ...
                    AxisAlignedBB box = ReikaAABBHelper.getBlockAABB(x, y, z).offset(
    ...
    private void teleport(EntityPlayer ep, TileEntityTransportWindow te) {
        ...
        ep.setPositionAndUpdate(te.xCoord+0.5+dir.offsetX*0.9,
        ep.posY+te.yCoord-yCoord+0.01, te.zCoord+0.5+dir.offsetZ*0.9);
         */
        ReikaEntityHelper.seamlessTeleport(
            ep,
            xCoord,
            yCoord,
            zCoord,
            ...
            te.getFacing().getOpposite()
        );
    }

    private TileEntityTransportWindow getTarget() {
        TileEntity te = target != null ? target.getTileEntity() : null;
        return te instanceof TileEntityTransportWindow ? (TileEntityTransportWindow) te
                                                       : null;
    }
    ...
    private boolean canTeleportPosition(ForgeDirection dir, EntityPlayer ep) {
        ...
        boolean dist = dx < (dir.offsetX == 0 ? mw : md) && dy < 0.15
            && dz < (dir.offsetZ == 0 ? mw : md);
        if (!dist)
            return false;
        ...
        return (
            ReikaMathLibrary.approxr(phi, phit, 5)
            || ReikaMathLibrary.approxr(phi, phit + 360, 5)
        );
    }
    ...
    public boolean shouldRefresh(
        ...
        return ChromaTiles.getTileFromIDandMetadata(oldBlock, oldMeta)
            != ChromaTiles.getTileFromIDandMetadata(newBlock, newMeta);
    }
    ...
    static {
        ...
        acceptedFrames.add(new BlockKey(
            ChromaBlocks.STRUCTSHIELD.getBlockInstance(), BlockType.CLOAK.ordinal()
        ));
        acceptedFrames.add(new BlockKey(
            ChromaBlocks.STRUCTSHIELD.getBlockInstance(), BlockType.CLOAK.metadata
        ));
        acceptedFrames.add(new BlockKey(
            ChromaBlocks.STRUCTSHIELD.getBlockInstance(), BlockType.STONE.ordinal()
        ));
        acceptedFrames.add(new BlockKey(
            ChromaBlocks.STRUCTSHIELD.getBlockInstance(), BlockType.STONE.metadata
        ));
        acceptedFrames.add(new BlockKey(
            ChromaBlocks.STRUCTSHIELD.getBlockInstance(), BlockType.LIGHT.ordinal()
        ));
        acceptedFrames.add(new BlockKey(
            ChromaBlocks.STRUCTSHIELD.getBlockInstance(), BlockType.LIGHT.metadata
        ));
        acceptedFrames.add(new BlockKey(
            ChromaBlocks.STRUCTSHIELD.getBlockInstance(), BlockType.MOSS.ordinal()
        ));
        acceptedFrames.add(new BlockKey(
            ChromaBlocks.STRUCTSHIELD.getBlockInstance(), BlockType.MOSS.metadata
        ));
        acceptedFrames.add(new BlockKey(
            ChromaBlocks.STRUCTSHIELD.getBlockInstance(), BlockType.COBBLE.ordinal()
        ));
        acceptedFrames.add(new BlockKey(
            ChromaBlocks.STRUCTSHIELD.getBlockInstance(), BlockType.COBBLE.metadata
        ));
    ...
    private class WindowTimer extends PlayerTimer {
        ...
        protected boolean shouldTickPlayer(EntityPlayer ep) {
            return TileEntityTransportWindow.this.canTeleportPosition(
                TileEntityTransportWindow.this.getFacing(), ep
            );
        }
    }
}
