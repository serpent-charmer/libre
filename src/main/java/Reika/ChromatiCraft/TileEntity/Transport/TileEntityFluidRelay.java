...
public class TileEntityFluidRelay
    extends TileEntityChromaticBase implements BreakAction, SidePlacedTile, CustomHitbox {
                ...
                for (int j = -r; j <= r; j++) {
                    if (i != 0 || k != 0 || j != 0) {
                        ...
                        if (ChromaTiles.getTile(world, dx, dy, dz) == this.getTile()
                            && ReikaMathLibrary.py3d(i, j, k) <= r) {
                            connections.add(new Coordinate(dx, dy, dz));
                            TileEntityFluidRelay te
                                = (TileEntityFluidRelay) world.getTileEntity(dx, dy, dz);
    ...
    private void renderFluid(Fluid f) {
        if (!worldObj.isRemote) {
            ReikaPacketHelper.sendDataPacketWithRadius(
                ...
                ChromaPackets.RELAYFLUID.ordinal(),
                this,
                24,
                f.getID()
            );
    ...
    public void pushFluids() {
        ...
        for (Fluid f : fluidTypes) {
            FluidStack max = ifl.drain(
                this.getFacing().getOpposite(),
                new FluidStack(f, Integer.MAX_VALUE),
                false
            );
            if (max != null) {
                int ret = network.push(
                    worldObj, f, Math.min(max.amount, this.getThroughput())
                );
                if (ret > 0) {
                    ifl.drain(
                        this.getFacing().getOpposite(), new FluidStack(f, ret), true
                    );
                    this.renderFluid(f);
                }
            }
    ...
    public int pushFluid(Fluid f, int amt) {
        ...
        if (ifl != null) {
            int ret
                = ifl.fill(this.getFacing().getOpposite(), new FluidStack(f, amt), true);
            if (ret > 0)
                this.renderFluid(f);
            return ret;
        } else {
    ...
    public int suckFluid(Fluid f, int amt) {
        ...
        if (ifl != null) {
            FluidStack ret
                = ifl.drain(this.getFacing().getOpposite(), new FluidStack(f, amt), true);
            if (ret != null)
                this.renderFluid(f);
    ...
    public AxisAlignedBB getHitbox() {
        ForgeDirection dir = this.getFacing();
        double d = 0.3125;
        return ReikaAABBHelper.getBlockAABB(this).contract(d, d, d).offset(
            dir.offsetX * d, dir.offsetY * d, dir.offsetZ * d
        );
    }
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        return ReikaAABBHelper.getBlockAABB(this).expand(
            RELAY_RANGE, RELAY_RANGE, RELAY_RANGE
        );
    }
}
