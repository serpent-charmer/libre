...
public class TileEntityItemRift
    extends TileEntityRelayPowered implements SidePlacedTile, Linkable<Coordinate> {
    private static final ElementTagCompound required
        = ElementTagCompound.of(CrystalElement.LIME);
    ...
    private boolean[] filterSet
        = ReikaArrayHelper.getTrueArray(ItemCategory.values().length);
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote && isEmitting && this.hasValidConnection()) {
            ...
            if (src != null) {
                ChromaTiles c = ChromaTiles.getTile(
                    world, otherEnd.xCoord, otherEnd.yCoord, otherEnd.zCoord
                );
                if (c == this.getTile()) {
                    TileEntityItemRift tile
                        = (TileEntityItemRift) otherEnd.getTileEntity(world);
                    IInventory tgt = tile.getAttachment();
                    if (tgt != null) {
                        ItemStack moved = this.transferItems(
                            src,
                            tgt,
                            this.getFacing().getOpposite(),
                            Math.min(
                                tgt.getInventoryStackLimit(),
                                this.getMaxTransferRate(
                                    energy.getValue(CrystalElement.LIME)
                                )
                            )
                        );
                        if (moved != null) {
                            energy.subtract(
                                CrystalElement.LIME,
                                this.getConsumedEnergy(moved.stackSize)
                            );
                            ElementTagCompound tag
                                = ItemElementCalculator.instance.getValueForItem(moved);
                            ...
                            ReikaPacketHelper.sendDataPacketWithRadius(
                                ...
                                ChromaPackets.ITEMRIFTMOVE.ordinal(),
                                this,
                                64,
                                flags,
                                tag.getTotalEnergy(),
            ...
            if (isFunctioning != wasFunctioning) {
                ...
                if (this.hasValidConnection()) {
                    TileEntityItemRift te
                        = (TileEntityItemRift) this.getConnection().getTileEntity(world);
    ...
    public void doMoveParticles(int flags, int total, ForgeDirection dir, int dist) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityCCBlurFX fx = new EntityCCBlurFX(
                worldObj, px, py, pz, dir.offsetX * v, dir.offsetY * v, dir.offsetZ * v
            );
            int l = dist * 18;
            ...
            if (otherEnd != null) {
                ...
                box = box.addCoord(
                    otherEnd.xCoord - xCoord,
                    otherEnd.yCoord - yCoord,
                    otherEnd.zCoord - zCoord
                );
    ...
    private void spawnParticles(World world, int x, int y, int z) {
        ...
        int[] c = { CrystalElement.LIME.getColor(),
                    0x22aaff,
                    CrystalElement.YELLOW.getColor() };
        for (int f = 0; f < c.length; f++) {
            for (int i = 0; i < n; i++) {
                double d
                    = (this.getTicksExisted() / 2D + i * len * 9D / n) % (len * 9) / 9D
                    - 0.5 + f * 1.25;
                if (d >= len - 0.5)
                    d -= len;
                double ang = Math.toRadians(
                    (this.getTicksExisted() * 12D + f * 360D / c.length) % 360
                );
        ...
        for (int i = 0; i <= 1; i++) {
            ...
            //EntitySparkleFX fx = new EntitySparkleFX(world, px, py, pz, 0, 0,
            //0).setScale(1).setLife(15);
            EntityCCBlurFX fx = new EntityCCBlurFX(world, px, py, pz, 0, 0, 0);
    ...
    transferItems(IInventory src, IInventory tgt, ForgeDirection move, int rate) {
        //ReikaJavaLibrary.pConsole(src+" >> "+tgt);
        ItemStack total = null;
        int[] from = src instanceof ISidedInventory
            ? ((ISidedInventory) src).getAccessibleSlotsFromSide(move.ordinal())
            : ReikaArrayHelper.getLinearArray(src.getSizeInventory());
        for (int i = 0; i < from.length; i++) {
            int slotfrom = from[i];
            ItemStack in = src.getStackInSlot(slotfrom);
            if (in != null
                && (total == null
                    || (ReikaItemHelper.matchStacks(total, in, true)
                        && ItemStack.areItemStackTagsEqual(total, in)))) {
                boolean extract = src instanceof ISidedInventory
                    ? ((ISidedInventory) src).canExtractItem(slotfrom, in, move.ordinal())
                    : true;
                if (extract) {
                    ItemStack in2 = ReikaItemHelper.getSizedItemStack(
                        in, Math.min(in.stackSize, rate)
                    );
                    int added = ReikaInventoryHelper.addStackAndReturnCount(
                        in2, tgt, move.getOpposite()
                    );
                    if (added > 0) {
                        in.stackSize -= added;
                        if (total == null)
    ...
    public final void resetOther() {
        if (otherEnd == null)
            return;
        ChromaTiles m = ChromaTiles.getTile(
            worldObj, otherEnd.xCoord, otherEnd.yCoord, otherEnd.zCoord
        );
        if (m == this.getTile()) {
    ...
    private final boolean canConnectTo(World world, int x, int y, int z) {
        ...
        //ReikaJavaLibrary.pConsole(isEmitting ? Arrays.toString(source) :
    ...
    private boolean isPassableBlock(World world, int x, int y, int z) {
        ...
        //RayTracer.getTransparentBlocks().contains(BlockKey.getAt(world, x, y, z)) ||
        //(ModList.BCTRANSPORT.isLoaded() && world.getBlock(x, y, z) ==
        //BCPipeHandler.getInstance().pipeID);
        return RelayNetworker.instance.isRelayPassable(world, x, y, z)
            || (ModList.BCTRANSPORT.isLoaded()
                && world.getBlock(x, y, z) == BCPipeHandler.getInstance().pipeID);
    }
    ...
    public boolean checkLocationValidity() {
        ...
        return b instanceof BlockChest
            || b.isSideSolid(worldObj, dx, dy, dz, dir.getOpposite())
            || ReikaBlockHelper.getBlockEdgeGap(
                   b, worldObj, dx, dy, dz, dir.getOpposite()
               )
            <= 0.0625;
    }
    ...
    private static enum ItemCategory {
        ...
        private static ItemCategory getCategory(ItemStack is) {
            ...
            if (ReikaBlockHelper.isOre(is)
                || Block.getBlockFromItem(i) instanceof BlockOre)
                return RAW;
            if (i instanceof ItemFood)
                return FOOD;
            if (i instanceof ItemArmor)
                return ARMOR;
            if (i instanceof ItemTool || i instanceof ItemPotion
                || i instanceof ItemNameTag || i instanceof ItemFlintAndSteel
                || i instanceof ItemEnchantedBook || i instanceof ItemWritableBook
                || i.isItemTool(is) || i.isMap())
                return TOOLS;
            return MISC;
        }
    }
