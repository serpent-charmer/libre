...
public class TileEntityFluidDistributor
    extends TileEntityAreaDistributor implements IFluidHandler {
    ...
    private void sendFluid(FluidStack fs, WorldLocation loc) {
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            ChromaPackets.FLUIDSEND.ordinal(),
            this,
            48,
            x,
            y,
    ...
    private void sendFluidToClient(WorldLocation loc, Fluid f, int amt) {
        ...
        double[] angs = ReikaPhysicsHelper.cartesianToPolar(
            loc.xCoord - xCoord, loc.yCoord - yCoord, loc.zCoord - zCoord
        );
        ...
        EntityFX fx = new EntityFluidFX(worldObj, px, py, pz, vel[0], vel[1], vel[2], f)
                          .setLife(l)
                          .setScale(s)
                          .setPositionController(p); //.markDestination(x, y, z);
        fx.noClip = true;
    ...
    private Spline getOrCreateSpline(Fluid f, WorldLocation loc) {
        ...
        if (s == null) {
            ...
            for (double d = 0; d <= 1; d += 0.125) {
                BasicVariablePoint p = new BasicVariablePoint(
                    DecimalPosition.interpolate(pos1, pos2, d), 0.5, 0.03125 / 4
                );
                p.tolerance *= 0.03125;
                s.addPoint(p);
            }
            BasicVariablePoint p
                = new BasicVariablePoint(new DecimalPosition(loc), 0.5, 0.03125 / 4);
            p.tolerance *= 0.03125;
            s.addPoint(p);
        }
        return s;
    ...
    protected boolean isValidTarget(TileEntity te) {
        ...
        if (te instanceof IFluidHandler) {
            ...
            if (s.contains("conduit") || ReikaStringParser.containsWord(s, "duct")
                || s.contains("cable") || s.contains("pipe")) {
                blacklist.add(c);
                return false;
            }
