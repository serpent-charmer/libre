...
public class TileEntityTeleportGate extends CrystalReceiverBase
    implements LocationCached, OwnedTile, GuiController, ChunkLoadingTile,
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote && this.hasStructure() && this.getCooldown() == 0
            && checkTimer.checkCap()) {
    ...
    private void tryCreateSnapshot() {
        File f = new File(
            new File(this.getPreviewFolder(), "screenshots"),
            this.getPreviewFilename(new WorldLocation(this))
        );
        if (f.exists())
            return;
        this.takeSnapshot();
    ...
    public void takeSnapshot() {
        ...
        TickScheduler.instance.scheduleEvent(
            new ScheduledTickEvent(new TakeImage(new WorldLocation(this), flag)), 2
        );
    }
    ...
    private static class TakeImage implements ScheduledEvent {
        ...
        public void fire() {
            ...
            ScreenShotHelper.saveScreenshot(
                dir, prev, mc.displayWidth, mc.displayHeight, mc.getFramebuffer()
            );
    ...
    private static File getPreviewFolder() {
        return new File(
            DragonAPICore.getMinecraftDirectory(), "mods/Reika/ChromatiCraft/GateShots"
        );
    }
    ...
    public static BufferedImage getPreview(WorldLocation loc) {
        ...
        if (img == null) {
            File f = new File(
                new File(getPreviewFolder(), "screenshots"), getPreviewFilename(loc)
            );
            if (!f.exists())
                return null;
            try {
    ...
    private void consumeNametags(World world, int x, int y, int z) {
        if (this.getTicksExisted() % 8 == 0) {
            AxisAlignedBB box = AxisAlignedBB.getBoundingBox(
                x - 0.75, y, z - 0.75, x + 1.75, y + 1.5, z + 1.75
            );
    ...
    private void doGuiChecks(World world, int x, int y, int z) {
        ...
        if (this.getTicksExisted() % 8 == 0) {
            AxisAlignedBB gui
                = ReikaAABBHelper.getBlockAABB(this).contract(0.25, 0, 0.25);
    ...
    private void openGui(World world, int x, int y, int z, EntityPlayer ep) {
        if (world.isRemote) {
            ...
            ReikaPacketHelper.sendNBTPacket(
                ...
                ChromaPackets.GATECACHE.ordinal(),
                getCacheAsNBT(),
                new PlayerTarget((EntityPlayerMP) ep)
            );
    ...
    private void onActivatingTick(World world, int x, int y, int z) {
        float f = (float
        ) (0.5 + 1.5 * (ACTIVATION_DURATION - activationTick) / ACTIVATION_DURATION);
    ...
    private void doActivatingParticles(World world, int x, int y, int z) {
        ...
        int i = 0;
        double f = Math.min(
            1, (0.5 + 1D * (ACTIVATION_DURATION - activationTick) / ACTIVATION_DURATION)
        );
        ...
        for (Orbit o : activationOrbits) {
            ...
            for (double t = dt; t <= dt + 0.75 * 8; t += 0.5) {
                ...
                EntityFX fx = new EntityCCBlurFX(world, p.xCoord, p.yCoord, p.zCoord)
                                  .setColor(c)
                                  .setRapidExpand()
                                  .setScale(s)
                                  .setLife(l)
            ...
            for (double t = dt; t <= dt + 0.75 * 8; t += 0.5) {
                ...
                DecimalPosition p
                    = o.getPosition(x + 0.5, y + 0.5 + 1.5, z + 0.5, -t + 200);
                EntityFX fx = new EntityCCBlurFX(world, p.xCoord, p.yCoord, p.zCoord)
                                  .setColor(c)
                                  .setRapidExpand()
                                  .setScale(s)
    ...
    public void validateStructure() {
        ...
        hasStructure = !worldObj.isRemote
            && ChromaStructures.TELEGATE.getArray(worldObj, xCoord, yCoord, zCoord)
                   .matchInWorld();
        if (!hasStructure)
            ChunkManager.instance.unloadChunks(this);
    }
    ...
    canTeleport(WorldLocation loc1, WorldLocation loc2, EntityPlayer caller) {
        TileEntity te1 = loc1.getTileEntity();
        TileEntity te2 = loc2.getTileEntity();
        ModularLogger.instance.log(
            LOGGER_ID,
            "Attempting teleport from " + loc1 + " to " + loc2 + " of "
                + caller.getCommandSenderName()
        );
        if (te1 instanceof TileEntityTeleportGate
            && te2 instanceof TileEntityTeleportGate) {
                ...
                if (tg1.direction.canSend && tg2.direction.canReceive) {
                    ...
                    if (tg1.canPlayerUse(caller) && tg2.canPlayerUse(caller)) {
                        ...
                        ModularLogger.instance.log(
                            LOGGER_ID,
                            "Cost is " + cost
                                + ", has1: " + tg1.energy.containsAtLeast(cost)
                                + ", has2: " + tg2.energy.containsAtLeast(cost)
                        );
                        return tg1.energy.containsAtLeast(cost)
                            || tg2.energy.containsAtLeast(cost);
                    }
                }
            }
        }
    ...
    public static void
    startTriggerTeleport(WorldLocation loc1, WorldLocation loc2, EntityPlayerMP ep) {
        if (canTeleport(loc1, loc2, ep)) {
            ...
            ReikaPacketHelper.sendDataPacket(
                ChromatiCraft.packetChannel,
                ChromaPackets.TELEPORTCONFIRM.ordinal(),
                ep,
                1
            );
        } else
    ...
    private static void
    triggerTeleport(TileEntityTeleportGate te1, TileEntityTeleportGate te2) {
        if (!te1.worldObj.isRemote) {
            AxisAlignedBB box
                = ReikaAABBHelper.getBlockAABB(te1).expand(1.5, 1.5, 1.5).offset(0, 1, 0);
    ...
    public static void
    teleportFrom(World world, int x, int y, int z, TileEntityTeleportGate te2, Entity e) {
        ...
        if (world.provider.dimensionId != te2.worldObj.provider.dimensionId) {
            ReikaEntityHelper.transferEntityToDimension(
                e, te2.worldObj.provider.dimensionId, new GateTeleporter(te2, dx, dy, dz)
            );
        } else {
            if (e instanceof EntityLivingBase) {
                ...
                if (e instanceof EntityPlayer) {
                    onTeleportPlayer(
                        (EntityPlayer) e, dx, dy, dz, te2.worldObj, dx2, dy2, dz2
                    );
                } else {
    ...
    private static void onTeleportPlayer(
        EntityPlayer ep,
        double dx,
        double dy,
        double dz,
        ...
        boolean flag = ReikaInventoryHelper.checkForItemStack(
        ...
        if (flag) {
            double[] xyz = ReikaPhysicsHelper.polarToCartesian(
                2 + rand.nextDouble() * 6, 35, rand.nextDouble() * 360
            );
            ep.motionX = xyz[0];
            ...
            ny = Math.max(
                ny,
                w2.getTopSolidOrLiquidBlock(
                    MathHelper.floor_double(nx), MathHelper.floor_double(nz)
                ) + 2
            );
        }
    ...
    private static ElementTagCompound
    getCost(TileEntityTeleportGate te1, TileEntityTeleportGate te2) {
    ...
    private void doTelegateFX() {
        if (!worldObj.isRemote) {
            ReikaPacketHelper.sendDataPacketWithRadius(
                ChromatiCraft.packetChannel, ChromaPackets.ACTIVEGATE.ordinal(), this, 96
            );
        }
    ...
    public void activateClientside(World world, int x, int y, int z) {
        for (int i = 0; i < 128; i++) {
            ...
            double vx = dx * v / dd;
            double vz = dz * v / dd;
            EntityFX fx = new EntityCCBlurFX(world, px, py, pz, vx, 0, vz)
    ...
    private void doParticles(World world, int x, int y, int z) {
        float pf = 1;
        double dd
            = Minecraft.getMinecraft().thePlayer.getDistanceSq(x + 0.5, y + 0.5, z + 0.5);
        if (dd >= 16384) {
            pf = 0.125F;
        } else if (dd >= 4096) {
            pf = 0.25F;
        ...
        double dt = StructureRenderer.isRenderingTiles()
            ? System.currentTimeMillis() / 50D
            : this.getTicksExisted();
        double r = 3.25;
        int l = 30;
        int c = this.getRenderColor();
        ...
        for (double t = dt; t <= dt + 0.75; t += 0.25) {
            if (ReikaRandomHelper.doWithChance(pf)) {
                ...
                if (GuiScreen.isCtrlKeyDown())
                    ...
                    particles.addEffect(
                        px - x, py - y, pz - z, ChromaIcons.FADE.getIcon(), l, s, c, true
                    );
                ...
                fx = new EntityCCBlurFX(world, px, py, pz)
                         .setColor(c)
                         .setRapidExpand()
                         .setScale(s)
                         .setLife(l)
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityFX fx = new EntityCCBlurFX(world, px, py, pz, 0, vy, 0)
                              .setColor(c)
                              .setRapidExpand()
                              .forceIgnoreLimits()
                              .setScale(s)
        ...
        CollectingPositionController p
            = new CollectingPositionController(px, py, pz, x + 0.5, ty, z + 0.5, l);
        EntityFX fx = new EntityCCBlurFX(world, px, py, pz)
                          .setPositionController(p)
                          .setColor(c)
                          .setLife(l)
                          .forceIgnoreLimits();
        Minecraft.getMinecraft().effectRenderer.addEffect(fx);
        ...
        if (rand.nextInt(4) == 0) {
            ...
            if (rand.nextInt(4) == 0)
                f = f * 2 + 0.125F;
            fx = new EntityShaderFX(world, px, py, pz, f, ChromaShaders.LENSPARTICLE)
                     .setLife(l)
                     .setScale(s)
                     .setGravity(g)
                     .forceIgnoreLimits()
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        return this.hasStructure() ? ReikaAABBHelper.getBlockAABB(this).expand(6, 6, 6)
    ...
    public static WorldLocation getRandomGate(World world, EntityPlayer ep) {
        ...
        return c.isEmpty() ? null
                           : ReikaJavaLibrary.getRandomCollectionEntry(rand, c).location;
    }
    ...
    public static WorldLocation
    getNearestGate(World world, int x, int y, int z, EntityPlayer ep) {
        GateData ret = null;
    ...
    private static class GateTeleporter extends Teleporter {
        ...
        public void placeInPortal(Entity e, double x, double y, double z, float facing) {
            e.setLocationAndAngles(
                location.xCoord + 0.5 + relativeX,
                location.yCoord + 0.5 + relativeY,
                location.zCoord + 0.5 + relativeZ,
                e.rotationYaw,
                e.rotationPitch
            );
    ...
    public static class GateData {
        ...
        private GateData(TileEntityTeleportGate te) {
            this(
                new WorldLocation(te), getFlags(te), te.owners, te.publicMode, te.gateName
            );
        }
        ...
        private GateData(
            WorldLocation loc, int flags, Collection<UUID> c, boolean pb, String name
        ) {
            location = loc;
            statusFlags = flags;
            owners.addAll(c);
