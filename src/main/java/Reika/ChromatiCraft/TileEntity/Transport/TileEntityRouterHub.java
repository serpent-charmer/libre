...
public class TileEntityRouterHub extends TileEntityChromaticBase
    implements IActionHost, RouterFilter, AdjacentUpdateWatcher {
            ...
            for (int k = -r; k <= r; k++) {
                for (int j = -r / 2; j <= r / 2; j++) {
                    ...
                    if (world.getBlock(dx, dy, dz)
                        == ChromaBlocks.ROUTERNODE.getBlockInstance()) {
                        TileEntityRouterNode te
                            = (TileEntityRouterNode) world.getTileEntity(dx, dy, dz);
            ...
            if (highlights.size() < 2) {
                if (rand.nextBoolean()) {
                    if (!extractions.isEmpty()) {
                        Coordinate c =
        ReikaJavaLibrary.getRandomCollectionEntry(extractions.keySet());
                        this.addHighlight(c);
                    }
                }
                else {
                    if (!insertions.isEmpty()) {
                        Coordinate c =
        ReikaJavaLibrary.getRandomCollectionEntry(insertions.keySet());
                        this.addHighlight(c);
                    }
                }
            }
    ...
    public void addHighlight(Coordinate c) {
        if (worldObj.isRemote) {
            ...
            ReikaPacketHelper.sendDataPacketWithRadius(
                ...
                ChromaPackets.ROUTERLINK.ordinal(),
                this,
                48,
                c.xCoord,
                c.yCoord,
    ...
    public void addExtractor(TileEntityRouterExtraction te) {
        Coordinate c = new Coordinate(te);
        extractions.put(
            c, new Extraction(c, te.getSide(), te.isBlacklist, te.getFilter())
        );
        this.syncAllData(true);
    }
    ...
    private ItemStack findItem(ItemStack is, MatchMode mode, boolean simulate) {
        ...
        if (ModList.APPENG.isLoaded()) {
            ChromatiCraft.logger.debug(
                "Router " + this + " requesting " + is + " from " + ingredients + " / "
                + network
            );
        } else {
            ChromatiCraft.logger.debug(
        ...
        if (ModList.APPENG.isLoaded()) {
            ...
            ChromatiCraft.logger.debug(
                this + " failed to find " + is + " in its ME System."
            );
        }
    ...
    private boolean
    canExtractItemFrom(int slot, ItemStack in, IInventory ii, ForgeDirection side) {
        if (ModList.FORESTRY.isLoaded() && InterfaceCache.BEEHOUSE.instanceOf(ii)) {
            if (ii instanceof IAlvearyComponent
                && ii.getClass().getName().endsWith("Sieve")) {
                return slot < 4;
            } else {
                ...
                if (bhi.getQueen() == null
                    && (type == EnumBeeType.PRINCESS || type == EnumBeeType.QUEEN)) {
                    return false;
                }
        ...
        return !(ii instanceof ISidedInventory)
            || ((ISidedInventory) ii).canExtractItem(slot, in, side.ordinal());
    }
    ...
    private static abstract class Connection {
        ...
        protected final IInventory getTarget(World world) {
            ...
            return ReikaInventoryHelper.isAutomatableInventory(te) ? (IInventory) te
                                                                   : null;
        }
        ...
        public static Connection readFromNBT(NBTTagCompound NBT) {
            ...
            try {
                ...
                Constructor<Connection> con = cl.getDeclaredConstructor(
                    Coordinate.class,
    ...
    private static class Insertion extends Connection {
        ...
        protected boolean performOperation(TileEntityRouterHub te) {
            ...
            //ReikaJavaLibrary.pConsole("Running "+c.size()+" insertions from "+te+":
            //"+c);
            for (ItemRule i : c) {
                ItemStack found = te.findItem(
                    i.item, i.mode, true
                ); //i.mode.removeItems(te.network, i.item, true);
                //ReikaJavaLibrary.pConsole("Found: "+found);
                if (found != null) {
                    ItemStack move
                        = ReikaItemHelper.getSizedItemStack(i.item, found.stackSize);
                    //ReikaJavaLibrary.pConsole("Attempting to move "+move);
                    if (ReikaInventoryHelper.addToIInv(move, ii)) {
                        te.findItem(
                            move, i.mode, false
                        ); //i.mode.removeItems(te.network, i.item, false);
    ...
    private static class Extraction extends Connection {
        ...
        protected boolean performOperation(TileEntityRouterHub te) {
            ...
            for (int i = 0; i < ii.getSizeInventory(); i++) {
                ...
                if (in != null) {
                    if (te.canExtractItemFrom(i, in, ii, direction)
                        && this.allowsItem(in) != blacklist) {
