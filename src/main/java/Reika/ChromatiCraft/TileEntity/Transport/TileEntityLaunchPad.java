...
public class TileEntityLaunchPad
    extends TileEntityChromaticBase implements MultiBlockChromaTile {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        if (!world.isRemote && this.hasStructure()) {
            AxisAlignedBB box = AxisAlignedBB.getBoundingBox(
                x - 0.75, y + 0.9375, z - 0.75, x + 1.75, y + 2.5, z + 1.75
            );
            ...
            box = AxisAlignedBB.getBoundingBox(
                x - 1, y + 0.9375, z - 1, x + 2, y + 1.0625, z + 2
            );
            ...
            if (!li.isEmpty()) {
                if (charge == CHARGE_DURATION) {
                    ...
                    needsBigUpdate = true;
                    ChromaSounds.KILLAURA.playSoundAtBlockNoAttenuation(
                        this, 1, 0.5F, 90
                    );
                    ReikaPacketHelper.sendDataPacketWithRadius(
                        ChromatiCraft.packetChannel,
                        ChromaPackets.LAUNCHFIRE.ordinal(),
                        this,
                        90
                    );
                } else {
                    charge++;
                    ChromaSounds.KILLAURA_CHARGE.playSoundAtBlock(
                        this, 1, 0.5F + this.getChargeFraction() * 0.5F
                    );
                }
            } else if (charge > 0) {
    ...
    private boolean checkForEnhanced(World world, int x, int y, int z) {
        for (int i = -2; i <= 2; i++) {
            for (int k = -2; k <= 2; k++) {
                ...
                Fluid f = world.getBlockMetadata(dx, y, dz) == 0
                    ? ReikaFluidHelper.lookupFluidForBlock(b)
                    : null;
