...
@Strippable(
    value = { "buildcraft.api.transport.IPipeConnection",
              "cofh.api.energy.IEnergyHandler",
              "thaumcraft.api.aspects.IEssentiaTransport",
              "thaumcraft.api.aspects.IAspectContainer",
              "vazkii.botania.api.mana.IManaCollisionGhost",
              "vazkii.botania.api.mana.IManaReceiver",
              "mrtjp.projectred.api.IBundledTile" }
)
@Injectable(
    value = { "dan200.computercraft.api.peripheral.IPeripheral",
              "li.cil.oc.api.network.Environment",
              "li.cil.oc.api.network.ManagedPeripheral",
              "li.cil.oc.api.network.SidedEnvironment" }
)
public class TileEntityRift extends LinkedTileBase
    implements WorldRift, IPipeConnection, IFluidHandler, IEnergyHandler,
               IEssentiaTransport, IAspectContainer, ISidedInventory, ChunkLoadingTile,
               IManaCollisionGhost, IManaReceiver, IBundledTile {
    ...
    private void initOCNodes() {
        for (int i = 0; i < 6; i++) {
            sidedOCNode[i]
                = Network.newNode((Environment) this, Visibility.Network).create();
        }
    }
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (!world.isRemote && this.isLinked() && this.getOther() != null) {
            AxisAlignedBB box
                = ReikaAABBHelper.getBlockAABB(x, y, z); //.expand(0.15, 0.15, 0.15);
    ...
    private void teleport(Entity e) {
        ...
        if (ModList.BOTANIA.isLoaded() && e instanceof IManaBurst) {
            ...
            /*
            int c = b.getColor();
            float[] hsb = Color.RGBtoHSB(ReikaColorAPI.getRed(c),
            ReikaColorAPI.getGreen(c), ReikaColorAPI.getBlue(c), null); hsb[1] = 1; hsb[0]
            += 0.1667F; //60 deg purpleshift if (hsb[0] > 1) hsb[0] -= 1; c =
            Color.HSBtoRGB(hsb[0], hsb[1], hsb[2]); b.setColor(c);
             */
            b.setColor(color);
            ...
            ReikaSoundHelper.playSoundAtBlock(
                ...
                0.5F,
                (float) ReikaRandomHelper.getRandomPlusMinus(1, 0.25)
            );
        }
        e.setLocationAndAngles(
            te.xCoord + rx, te.yCoord + ry, te.zCoord + rz, e.rotationYaw, e.rotationPitch
        );
        ...
        if (this.isLinked()) {
            ...
            if (te != null) {
                for (int i = 0; i < 6; i++) {
                    ...
                    int pwr = worldObj.getIndirectPowerLevelTo(
                        xCoord + dir.offsetX,
                        yCoord + dir.offsetY,
                        zCoord + dir.offsetZ,
                        opp.ordinal()
        ...
        if (ModList.OPENCOMPUTERS.isLoaded() && !world.isRemote) {
            for (int i = 0; i < 6; i++) {
                if (sidedOCNode[i] != null && te.sidedOCNode[i] != null)
                    ((Node) te.sidedOCNode[dirs[i].getOpposite().ordinal()])
                        .connect((Node) sidedOCNode[i]);
            }
        }
    }
    ...
    public int getColor() {
        return this.isInWorld()
            ? color
            : Color.HSBtoRGB((System.currentTimeMillis() % 15000) / 15000F, 1, 1);
    }
    ...
    public TileEntityRift getOther() {
        return this.isLinked() ? ((TileEntityRift) this.getLinkTarget().getTileEntity())
                               : null;
    }
    ...
    private TileEntity getAdjacentTargetTile(ForgeDirection dir) {
        TileEntity te = this.isLinked() && this.getOther() != null
            ? this.getOther().getAdjacentTileEntity(dir)
            : null;
        return te != null && !te.isInvalid() && te.worldObj != null ? te : null;
    }

    private TileEntity getSingleDirTile() {
        return singleDirection != null && this.isLinked()
            ? this.getOther().getAdjacentTileEntity(singleDirection.getOpposite())
            : null;
    }
    ...
    public boolean isConnectable(ForgeDirection face) {
        if (this.getOther() != null
            && this.getAdjacentTargetTile(face.getOpposite())
                    instanceof IEssentiaTransport) {
            return ((IEssentiaTransport) this.getAdjacentTargetTile(face.getOpposite()))
                .isConnectable(face);
        }
        return false;
    }
    ...
    public void setSuction(Aspect aspect, int amount) {
        if (this.getOther() != null
            && this.getSingleDirTile() instanceof IEssentiaTransport) {
    ...
    public int takeEssentia(Aspect aspect, int amount, ForgeDirection face) {
        if (this.getOther() != null
            && this.getAdjacentTargetTile(face.getOpposite())
                    instanceof IEssentiaTransport) {
            return ((IEssentiaTransport) this.getAdjacentTargetTile(face.getOpposite()))
                .takeEssentia(aspect, amount, face);
        }
        return 0;
    }
    ...
    public int addEssentia(Aspect aspect, int amount, ForgeDirection face) {
        if (this.getOther() != null
            && this.getAdjacentTargetTile(face.getOpposite())
                    instanceof IEssentiaTransport) {
            return ((IEssentiaTransport) this.getAdjacentTargetTile(face.getOpposite()))
                .addEssentia(aspect, amount, face);
        }
        return 0;
    }
    ...
    @ModDependent(ModList.BCTRANSPORT)
    private int sendEnergyToBCPipe(
        TileEntity other, ForgeDirection from, int maxReceive, boolean simulate
    ) {
        if (simulate)
            return maxReceive;
        ...
        if (pipe.getPipeType() == PipeType.POWER) {
            ...
            if (p instanceof Pipe && ((Pipe) p).transport instanceof PipeTransportPower) {
                return (int) ((PipeTransportPower) ((Pipe) p).transport)
                    .receiveEnergy(
                        from, maxReceive
                    ); //I know it says to never call this, but nothing else works...
            }
        }
        return 0;
    }
    ...
    @ModDependent(ModList.BCTRANSPORT)
    private int takeEnergyFromBCPipe(
        TileEntity other, ForgeDirection from, int maxExtract, boolean simulate
    ) {
        if (simulate)
            return maxExtract;
        IPipeTile pipe = (IPipeTile) other;
        if (pipe.getPipeType() == PipeType.POWER) {
            IPipe p = pipe.getPipe();
            if (p instanceof Pipe && ((Pipe) p).transport instanceof PipeTransportPower) {
                ((PipeTransportPower) ((Pipe) p).transport)
                    .requestEnergy(from, maxExtract);
                return 0;
            }
        }
        return 0;
    ...
    public int receiveEnergy(ForgeDirection from, int maxReceive, boolean simulate) {
        TileEntity other = this.getOther() != null
            ? this.getAdjacentTargetTile(from.getOpposite())
            : null;
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        if (this.getOther() != null
            && this.getAdjacentTargetTile(from.getOpposite()) instanceof IFluidHandler) {
            return ((IFluidHandler) this.getAdjacentTargetTile(from.getOpposite()))
    ...
    public boolean isItemValidForSlot(int i, ItemStack itemstack) {
        if (this.getOther() != null && this.getSingleDirTile() instanceof IInventory) {
            return ((IInventory) this.getSingleDirTile())
    ...
    public int[] getAccessibleSlotsFromSide(int var1) {
        if (this.getOther() != null) {
            if (this.getSingleDirTile() instanceof ISidedInventory) {
                return ((ISidedInventory) this.getSingleDirTile())
    ...
    public boolean canExtractItem(int i, ItemStack itemstack, int j) {
        if (this.getOther() != null
            && this.getSingleDirTile() instanceof ISidedInventory) {
            return ((ISidedInventory) this.getSingleDirTile())
                .canExtractItem(i, itemstack, j);
        }
        return false;
    }
    ...
    public AspectList getAspects() {
        if (this.getOther() != null
            && this.getSingleDirTile() instanceof IAspectContainer) {
    ...
    public int addToContainer(Aspect tag, int amount) {
        if (this.getOther() != null
            && this.getSingleDirTile() instanceof IAspectContainer) {
            return ((IAspectContainer) this.getSingleDirTile())
                .addToContainer(tag, amount);
        }
        return 0;
    }
    ...
    public boolean doesContainerContainAmount(Aspect tag, int amount) {
        if (this.getOther() != null
            && this.getSingleDirTile() instanceof IAspectContainer) {
            return ((IAspectContainer) this.getSingleDirTile())
                .doesContainerContainAmount(tag, amount);
        }
        return false;
    }
    ...
    public Object[] callMethod(
        ...
        if (this.getOther() != null && this.getSingleDirTile() instanceof IPeripheral) {
            return ((IPeripheral) this.getSingleDirTile())
                .callMethod(computer, context, method, arguments);
        }
        return null;
    }
    ...
    public String[] methods() {
        if (this.getOther() != null && this.getSingleDirTile() instanceof
    ManagedPeripheral) { return ((ManagedPeripheral)this.getSingleDirTile()).methods();
        }
        return new String[0];
    }
    ...
    @Override
    @ModDependent(ModList.OPENCOMPUTERS)
    public Object[] invoke(String method, Context context, Arguments args) throws
    Exception { if (this.getOther() != null && this.getSingleDirTile() instanceof
    ManagedPeripheral) { return
    ((ManagedPeripheral)this.getSingleDirTile()).invoke(method, context, args);
        }
        return null;
    }
    ...
    public final void onConnect(Node node) {
        ...
        if (te != null) {
            for (int i = 0; i < 6; i++) {
                if (sidedOCNode[i] != null && sidedOCNode[i] == node
                    && this.getOtherNode(te, i) != null) {
    ...
    public byte[] getBundledSignal(int dir) {
        ...
        byte[] ret = ProjectRedAPI.transmissionAPI.getBundledInput(
            other.getWorld(),
            other.xCoord,
            other.yCoord,
            other.zCoord,
            ForgeDirection.VALID_DIRECTIONS[dir].getOpposite().ordinal()
        );
        if (ret == null)
            return ret;
