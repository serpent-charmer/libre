...
public class TileEntityChromaCrafter extends InventoriedRelayPowered
    implements IFluidHandler, OwnedTile, IPipeConnection, OperationInterval, BreakAction,
               FocusAcceleratable {
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (inv[0] != null) {
            ...
            if (ReikaItemHelper.matchStacks(inv[0], ChromaStacks.etherBerries)) {
                ...
                inv[0] = null;
                //ReikaJavaLibrary.pConsole(hasEtherBerries+" /
                //"+recipeItems.size()+":"+recipeItems+" > "+recipe);
            ...
            //ReikaJavaLibrary.pConsole(hasEtherBerries+" ["+(inv[0] != null ?
            //inv[0].getDisplayName() : "null")+"] /
            //"+recipeItems.size()+":"+recipeItems+" > "+recipe);
        }
    ...
    private void onRecipeTick(World world, int x, int y, int z) {
        if (energy.containsAtLeast(required) && tank.getLevel() == 3000
            && this.hasStructure())
            recipeTick--;
        if (recipeTick == 0) {
            if (!world.isRemote)
    ...
    public boolean hasStructure() {
        for (int i = 1; i <= 2; i++) {
            if (worldObj.getBlock(xCoord, yCoord + i, zCoord)
                    != ChromaBlocks.STRUCTSHIELD.getBlockInstance()
                || worldObj.getBlockMetadata(xCoord, yCoord + i, zCoord) % 8
                    != BlockType.GLASS.ordinal())
                return false;
        }
        return true;
    }
    ...
    private boolean canAcceptMoreOf(ItemStack is) {
        return ReikaItemHelper.matchStacks(is, ChromaStacks.etherBerries)
            ? this.getBerryCount() + is.stackSize <= 512
            : this.getItemCount(is) + is.stackSize <= 64;
    }
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        FluidStack fs = new FluidStack(
            resource.getFluid(), Math.min(25, resource.amount)
        ); //25mB/t limit
    ...
    public ConnectOverride overridePipeConnection(PipeType type, ForgeDirection with) {
        if (type == PipeType.FLUID)
            return with.offsetY == 0 ? ConnectOverride.CONNECT
                                     : ConnectOverride.DISCONNECT;
        if (type == PipeType.ITEM)
            return with != ForgeDirection.UP ? ConnectOverride.CONNECT
                                             : ConnectOverride.DISCONNECT;
    ...
    public OperationState getState() {
        ...
        return energy.containsAtLeast(required) ? OperationState.RUNNING
    ...
    public void breakBlock() {
        ReikaItemHelper.dropItems(
            worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, recipeItems
        );
    ...
    public float getAccelerationFactor() {
        return TileEntityFocusCrystal.getSummedFocusFactorDirect(
                   this, this.getRelativeFocusCrystalLocations()
               )
            / 2; //caps at 6x for turbo, 4x for exq
    }
