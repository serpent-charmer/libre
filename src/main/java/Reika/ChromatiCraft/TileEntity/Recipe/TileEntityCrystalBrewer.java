...
public class TileEntityCrystalBrewer
    extends InventoriedChromaticBase implements OperationInterval {
    ...
    private boolean canBrew() {
        ...
        if (inv[0].getItem() != ChromaItems.SHARD.getItemInstance())
            return false;
        if (!this.isSlotModifiable(1) && !this.isSlotModifiable(2)
            && !this.isSlotModifiable(3))
            return false;
    ...
    private boolean isSlotModifiable(int i) {
        ...
        if (is.getItem() == Items.potionitem) {
            if (CrystalPotionController.instance.isPotionModifier(
                    CrystalElement.elements[zd % 16]
                ))
                return ReikaPotionHelper.isActualPotion(is.getItemDamage());
            else
                return !ReikaPotionHelper.isActualPotion(is.getItemDamage());
        } else if (is.getItem() == ChromaItems.POTION.getItemInstance()) {
            return CrystalPotionController.instance.isPotionModifier(
                CrystalElement.elements[zd % 16]
            );
        }
        return false;
    ...
    private void brew() {
        ...
        for (int i = 1; i < 4; i++) {
            if (inv[i] != null) {
                inv[i]
                    = this.getPotionStackFromColor(inv[i].getItemDamage(), color, boost);
            }
        }
    }

    public static ItemStack
    getPotionStackFromColor(int dmg, CrystalElement color, boolean boost) {
        ...
        if (custom) {
            ...
            if (CrystalPotionController.instance.isCorruptedPotion(color)) {
                ...
                int cmeta = PotionHelper.applyIngredient(
                    is.getItemDamage(), PotionHelper.fermentedSpiderEyeEffect
                );
        ...
        if (boost) {
            is.setItemDamage(
                is.getItemDamage() | ReikaPotionHelper.BOOST_BIT
                | ReikaPotionHelper.EXTENDED_BIT
            );
        }
        return is;
    }
    ...
    public static List<ItemStack> getPotionsForColor(CrystalElement color) {
        if (CrystalPotionController.isPotionModifier(color)) {
            ...
            return
    ReikaJavaLibrary.makeListFrom(getPotionStackFromColor(ReikaPotionHelper.AWKWARD_META,
    color));
        }
    }*/
    ...
    public boolean isUseableByPlayer(EntityPlayer ep) {
        return ReikaMathLibrary.py3d(
                   ep.posX - xCoord - 0.5, ep.posY - yCoord - 0.5, ep.posZ - zCoord - 0.5
               )
            <= 8;
    }
