...
public class TileEntityPlayerInfuser extends TileEntityAuraInfuser {
    ...
    protected void onFirstTick(World world, int x, int y, int z) {
        ...
        double d = 0.0625;
        targetBox = AxisAlignedBB.getBoundingBox(
            x - d, y + 0.375 - d, z - d, x + 1 + d, y + 0.75 + d, z + 1 + d
        );
    }
    ...
    protected void collectFocusCrystalLocations(FilledBlockArray arr) {
        for (Coordinate c : arr.keySet()) {
            if (arr.getBlockAt(c.xCoord, c.yCoord, c.zCoord)
                ...
                if (arr.getMetaAt(c.xCoord, c.yCoord, c.zCoord)
    ...
    protected boolean isReady() {
        if (inv[0] == null || inv[0].stackSize < 8)
            return false;
        if (targetBox == null
            || !targetBox.intersectsWith(this.getCraftingPlayer().boundingBox))
            return false;
        ElementBufferCapacityBoost e = this.getSelectedEffect();
        return e != null
            && ElementBufferCapacityBoost.getAvailableBoosts(this.getCraftingPlayer())
                   .contains(e);
    }
    ...
    protected void spawnParticles(World world, int x, int y, int z) {
        int n = Math.max(
            1, rand.nextInt(4) - Minecraft.getMinecraft().gameSettings.particleSetting
        );
        for (int i = 0; i < n; i++) {
            Coordinate p = ReikaJavaLibrary.getRandomCollectionEntry(
                rand, this.getChromaLocations()
            );
            double r = 1.85;
            ...
            EntityChromaFluidFX fx
                = new EntityChromaFluidFX(
                      CrystalElement.WHITE, world, px, p.yCoord + 0.5, pz, vx, vy, vz
                )
                      .setScale(s)
                      .setGravity(g);
    ...
    protected void doAmbientParticles(World world, int x, int y, int z) {
        Coordinate p
            = ReikaJavaLibrary.getRandomCollectionEntry(rand, this.getChromaLocations());
        double px = p.xCoord + rand.nextDouble();
        double pz = p.zCoord + rand.nextDouble();
        float s = 1.75F;
        ...
        EntityChromaFluidFX fx
            = new EntityChromaFluidFX(
                  CrystalElement.WHITE, world, px, p.yCoord + 0.5, pz, 0, vy, 0
            )
                  .setScale(s)
                  .setGravity(g);
        Minecraft.getMinecraft().effectRenderer.addEffect(fx);
