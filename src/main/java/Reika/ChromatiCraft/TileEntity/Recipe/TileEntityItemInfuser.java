...
public class TileEntityItemInfuser extends TileEntityAuraInfuser {
    ...
    protected void collectFocusCrystalLocations(FilledBlockArray arr) {
        for (Coordinate c : arr.keySet()) {
            if (c.yCoord == yCoord - 1
                && c.getTaxicabDistanceTo(new Coordinate(this)) > 2) {
            ...
            if (is.stackTagCompound != null) {
                ...
                if (extra > 0) {
                    ...
                    ReikaItemHelper.dropItem(
                        ei, ReikaItemHelper.getSizedItemStack(is, extra)
                    );
    ...
    protected void spawnParticles(World world, int x, int y, int z) {
        ...
        for (int i = 0; i < 360; i += 60) {
            ...
            double r = 1.85;
            double v = tall ? 0.0425 * (1 + fac)
                            : ReikaRandomHelper.getRandomPlusMinus(0.0425, 0.005);
            ...
            EntityChromaFluidFX fx
                = new EntityChromaFluidFX(
                      CrystalElement.WHITE, world, px, py, pz, vx, vy, vz
                )
                      .setScale(s)
                      .setGravity(g);
