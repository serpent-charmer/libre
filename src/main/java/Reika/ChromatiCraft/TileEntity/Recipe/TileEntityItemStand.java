...
public class TileEntityItemStand extends InventoriedChromaticBase
    implements ItemOnRightClick, OwnedTile /*, HitAction*/, InertIInv, IPipeConnection,
    ...
    private static final MultiMap<UUID, WorldLocation> spreadSet
        ...
        if (world.isRemote) {
            if (tile != null) {
                ...
                if (te instanceof TileEntityCastingTable
                    && ((TileEntityCastingTable) te).getCraftingTick() > 0) {
    ...
    private void spawnItemParticles(World world, int x, int y, int z) {
        if (rand.nextInt(2) == 0) {
            ...
            ElementTagCompound tag
                = ItemElementCalculator.instance.getValueForItem(inv[0]);
            ...
            EntityFX fx = new EntityCCBlurFX(world, rx, ry, rz, 0, 0, 0)
                              .setColor(r, g, b)
                              .setGravity(gv)
                              .setLife(l);
    ...
    public ItemStack onRightClickWith(ItemStack item, EntityPlayer ep) {
        ...
        boolean all = this.recentClicked() && ReikaItemHelper.matchStacks(item, inv[0]) &&
        item != null && sum <= item.getMaxStackSize(); if (!all) this.dropSlot();
        /*
        if (!all && inv[0] != null) {
        ...
        if (!spreadSet.isEmpty() && !worldObj.isRemote) {
            ItemStack ret = item == null
                    && spreadSet.containsValueForKey(uid, new WorldLocation(this))
                ? null
                : spreadItems(ep, item);
        ...
        if (inv[0] == null) {
            ...
            if (ReikaItemHelper.matchStacks(inv[0], item)) {
                int add = Math.min(
                    item.stackSize, inv[0].getMaxStackSize() - inv[0].stackSize
                );
                inv[0].stackSize += add;
                item.stackSize -= add;
            } else if (item == null) {
    ...
    private void updateItem() {
        item = inv[0] != null
            ? new InertItem(worldObj, ReikaItemHelper.getSizedItemStack(inv[0], 1))
            : null;
        if (worldObj != null) {
    ...
    public void dropSlot() {
        if (inv[0] != null) {
            ReikaItemHelper.dropItem(
                worldObj, xCoord + 0.5, yCoord + 1, zCoord + 0.5, inv[0]
            );
            inv[0] = null;
        }
    }
