...
public class TileEntityRitualTable extends InventoriedCrystalReceiver
    implements /*GuiController, */ BreakAction, TriggerableAction, OwnedTile,
    ...
    private void onRitualTick(World world, int x, int y, int z) {
        ...
        tickNoPlayer = 0;
        AxisAlignedBB box
            = ReikaAABBHelper.getBlockAABB(x, y, z).offset(0, 2, 0).expand(0, 1, 0);
        ...
        //ReikaJavaLibrary.pConsole(abilityTick+":"+inbox+" from "+ep.boundingBox+" &
        //"+box, Side.SERVER); ReikaJavaLibrary.pConsole(energy+"/"+tag, Side.SERVER);
        //ReikaJavaLibrary.pConsole(box.maxX > ep.boundingBox.minX && box.minX <
        //ep.boundingBox.maxX ? (box.maxY > ep.boundingBox.minY && box.minY <
        //ep.boundingBox.maxY ? box.maxZ > ep.boundingBox.minZ && box.minZ <
        //ep.boundingBox.maxZ : false) : false);
    ...
    private void spawnParticles(
        World world, int x, int y, int z, boolean canTick, ElementTagCompound tag
    ) {
        ...
        for (int i = 0; i < 360; i += 120) {
            ...
            EntityGlobeFX fx
                = new EntityGlobeFX(world, rx, y, rz, vx, 0.1875, vz).setScale(1.5F);
        ...
        if (n > 0) {
            ...
            for (CrystalElement e : tag.elementSet()) {
                ...
                double vx = v * Math.cos(ang);
                double vz = v * Math.sin(ang);
                EntityGlobeFX fx = new EntityGlobeFX(e, world, rx, y, rz, vx, 0.1875, vz)
                                       .setScale(1.5F);
                Minecraft.getMinecraft().effectRenderer.addEffect(fx);
                i++;
            }
        ...
        if (canTick) {
            ...
            if (mc.thePlayer.getCommandSenderName().equals(
                    ritualPlayer.getCommandSenderName()
                )) {
    ...
    private void giveAbility(EntityPlayer ep) {
        if (ep == null || ReikaPlayerAPI.isFake(ep)) {
            ChromatiCraft.logger.logError(
                "Tried to give ability to null or fake player???"
            );
            return;
        }
        boolean flag = false;
    ...
    private static void doCompletionParticles(EntityPlayer ep, Ability a) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            double[] v = ReikaPhysicsHelper.polarToCartesian(
                v0, rand.nextDouble() * 360, rand.nextDouble() * 360
            );
            ...
            EntityBlurFX fx = new EntityCCBlurFX(
                ep.worldObj, ep.posX, ep.posY - 1.62 + 0.8, ep.posZ, v[0], v[1], v[2]
            );
    ...
    public void validateStructure() {
        ...
        ((RitualStructure) ChromaStructures.RITUAL.getStructure())
            .initializeEnhance(isEnhanced, false);
        FilledBlockArray array = ChromaStructures.RITUAL.getArray(world, x, y, z);
        hasStructure = array.matchInWorld();
        //ReikaJavaLibrary.pConsole(hasStructure+" / "+isEnhanced+": "+this.getSide()+" @
        //"+array);
        hasEnhancedStructure = isEnhanced && hasStructure
            && ChromaStructures.RITUAL2.getArray(world, x, y, z).matchInWorld();
    ...
    public boolean triggerRitual(EntityPlayer ep) {
        ...
        if (hasStructure && abilityTick == 0 && ability != null
            && AbilityRituals.instance.hasRitual(ability) && this.isOwnedByPlayer(ep)) {
            ritualPlayer = ep;
            if (worldObj.isRemote)
                return true;
            ElementTagCompound tag = AbilityRituals.instance.getAura(ability);
    ...
    protected void readSyncTag(NBTTagCompound NBT) {
        ...
        ability = a != null && !a.isEmpty() && !a.equals("null")
            ? Chromabilities.getAbility(a)
            : null;
    ...
    public ElementTagCompound getRequestedTotal() {
        return ability != null && abilityTick > 0
            ? AbilityHelper.instance.getElementsFor(ability)
            : null;
    }
    ...
    public float getOperationFraction() {
        return ability == null
            ? 0
            : 1 - abilityTick / (float) AbilityRituals.instance.getDuration(ability);
    }
