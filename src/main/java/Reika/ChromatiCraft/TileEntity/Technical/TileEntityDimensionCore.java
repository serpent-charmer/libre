...
public class TileEntityDimensionCore
    extends TileEntityLocusPoint implements PlayerBreakHook {
    ...
    private static final EnumMap<CrystalElement, Coordinate> locations
        = new EnumMap(CrystalElement.class);
    private static final EnumMap<CrystalElement, HashSet<CrystalElement>> beams
        = new EnumMap(CrystalElement.class);
    private static final
        ArrayList<ArrayList<ImmutablePair<CrystalElement, Integer>>>[] melody
        = new ArrayList[2];
    ...
    private static void addMelodyNote(int track, MusicKey key) {
        ...
        if (c.isEmpty())
            throw new RegistrationException(
                ChromatiCraft.instance, "No such color for note " + key
            );
        ...
        for (CrystalElement e : c) {
            ...
            if (idx == -1) {
                throw new RegistrationException(
                    ...
                    "No such index for note " + key + " for color " + e
                );
            }
    ...
    private static void addColor(CrystalElement e, int x, int y, int z) {
        locations.put(
            e, new Coordinate(x - 21, y - 5, z - 21)
        ); //base offset of controller is (21, 5, 21)
    }
    ...
    private void doScanForEntry(World world, int x, int y, int z) {
        ...
        for (EntityPlayerMP ep : ((List<EntityPlayerMP>) world.getEntitiesWithinAABB(
                 EntityPlayerMP.class, box
             ))) {
    ...
    private AxisAlignedBB getStructureEntryBox() {
        ...
        return AxisAlignedBB.getBoundingBox(
            ...
            z - r,
            x + r + 1,
            ReikaWorldHelper.getTopNonAirBlock(worldObj, x, z, true) + 9,
            z + r + 1
        );
    }
        ...
        switch (structure) {
            ...
            case TDMAZE:
                AxisAlignedBB box = AxisAlignedBB.getBoundingBox(
                    x - 1, y + 2, z - 1, x + 2, y + 4, z + 2
                );
                List<EntityPlayer> li
                    = world.getEntitiesWithinAABB(EntityPlayer.class, box);
                if (!li.isEmpty()) {
                    EntityPlayer ep = li.get(0);
                    triggered = true;
                    boolean w = rand.nextBoolean();
                    int dx = w               ? rand.nextBoolean() ? -2 : 2
                        : rand.nextBoolean() ? 1
                                             : -1;
                    int dz = !w              ? rand.nextBoolean() ? -2 : 2
                        : rand.nextBoolean() ? 1
                                             : -1;
                    int dx2 = Math.abs(dx) == 1
                        ? dx
                        : (int) Math.signum(dx) * (Math.abs(dx) + 1);
                    int dz2 = Math.abs(dz) == 1
                        ? dz
                        : (int) Math.signum(dz) * (Math.abs(dz) + 1);
                    world.setBlockMetadataWithNotify(
                        x + dx, y + 2, z + dz, BlockType.CRACKS.metadata, 3
                    );
                    world.setBlockMetadataWithNotify(
                        x + dx, y + 3, z + dz, BlockType.CRACKS.metadata, 3
                    );
                    world.setBlockMetadataWithNotify(
                        x - dx2, y + 1, z - dz2, BlockType.CRACKS.metadata, 3
                    );
                    world.setBlockMetadataWithNotify(
                        x + dx, y, z + dz, BlockType.CRACKS.metadata, 3
                    );
                    world.setBlockMetadataWithNotify(
                        x + dx, y - 1, z + dz, BlockType.CRACKS.metadata, 3
                    );
    ...
    private void spawnConnectFX(World world, int x, int y, int z) {
        ...
        if (tick % sp == 0) {
            ArrayList<ArrayList<ImmutablePair<CrystalElement, Integer>>> song = melody
                [(ChunkProviderChroma.getMonumentGenerator().hashCode()
                  ^ Minecraft.getMinecraft().hashCode())
                 % melody.length];
            ArrayList<ImmutablePair<CrystalElement, Integer>> li
                = song.get((int) ((tick / sp) % song.size()));
                        ...
                        if (ts.isMonument()) {
                            ...
                            for (CrystalElement e : m) {
                                ...
                                if (te instanceof TileEntityDimensionCore
                                    && ((TileEntityDimensionCore) te).getColor() == e) {
                            ...
                            ReikaSoundHelper.playClientSound(
                                ChromaSounds.ORB, x, y, z, 1F / li.size(), mult, false
                            );
                            ReikaSoundHelper.playClientSound(
                                ChromaSounds.DING, x, y, z, 0.3F / li.size(), mult
                            );
                            ...
                            for (int i = 0; i < n; i++) {
                                ...
                                int l = 40;
                                float g = (float) ReikaRandomHelper.getRandomPlusMinus(
                                    0.03125, 0.0150625
                                );
                                float s = 2
                                    * (float
                                    ) ReikaRandomHelper.getRandomPlusMinus(1.25, 0.5);
                                EntityFX fx
                                    = new EntityLaserFX(color, world, px, py, pz, 0, 0, 0)
                                          .setGravity(g)
                                          .setScale(s);
                            ...
                            for (int i = 0; i < n; i++) {
                                ...
                                fx = (EntityFloatingSeedsFX) fx.setGravity(g)
                                         .setScale(s)
                                         .setLife(l)
    ...
    @SideOnly(Side.CLIENT)
    private void
    createBeamLine(World world, int x, int y, int z, Coordinate c, CrystalElement e) {
        createBeamLine(world, x, y, z, c.xCoord, c.yCoord, c.zCoord, color, e);
    }

    @SideOnly(Side.CLIENT)
    public static void createBeamLine(
        World world,
        int x1,
        int y1,
        int z1,
        ...
        for (double p = 0; p <= dd; p += pd) {
            ...
            float s = 1
                + 1.5F
                    * (float) Math.sin(
                        f * Math.PI
                    ); //+MathHelper.sin((this.getTicksExisted()+color.ordinal()*12)/32F);
            ...
            int clr
                = ReikaColorAPI.mixColors(e1.getColor(), e2.getColor(), 1 - (float) f);
    ...
    private Coordinate getOtherColor(CrystalElement e) {
        ...
        return new Coordinate(
            xCoord - c.xCoord + c2.xCoord,
            yCoord - c.yCoord + c2.yCoord,
            zCoord - c.zCoord + c2.zCoord
        );
    }
    ...
    protected void onFirstTick(World world, int x, int y, int z) {
        ...
        if (this.getPlacer() == null && !this.hasStructure() && !world.isRemote) {
            ChromatiCraft.logger.logError(
                this + " was never given a structure!? Color = " + color + ", UID=" + uid
            );
        }
    }
    ...
    public void setDataFromItemStackTag(ItemStack is) {
        ...
        color = is.stackTagCompound != null
            ? CrystalElement.elements[is.stackTagCompound.getInteger("color")]
    ...
    public boolean isBreakable(EntityPlayer ep) {
        ...
        if (ReikaPlayerAPI.isFake(ep))
            return false;
        if (!worldObj.isRemote && !ep.capabilities.isCreativeMode
            && this.hasStructure()) {
    ...
    public boolean breakByPlayer(EntityPlayer ep) {
        ...
        if (this.hasStructure()) {
            ...
            if (!ep.capabilities.isCreativeMode
                && !this.getStructure().shouldAllowCoreMining(worldObj, ep)) {
                return false;
            }
    ...
    private void openStructure() {
        ...
        for (Coordinate c2 : set) {
            //Coordinate c2 = c.offset(-gen.getPosX(), -gen.getPosY(),
            //-gen.getPosZ()).offset(xCoord, yCoord, zCoord);
            Block b = c2.getBlock(worldObj);
            BlockKey b2 = b instanceof BlockStructureShield ? new BlockKey(
                              ChromaBlocks.STRUCTSHIELD.getBlockInstance(),
                              BlockType.CRACKS.metadata % 8
                          )
                                                            : new BlockKey(Blocks.air);
            c2.setBlock(worldObj, b2.blockID, b2.metadata);
            //ReikaJavaLibrary.pConsole(new
            //Coordinate(this)+":"+c+">"+c2+":"+c2.getBlockKey(worldObj));
            //ReikaJavaLibrary.pConsole(new Coordinate(this)+" > "+c2+" %
            //"+c2.getBlockKey(worldObj));
        }
