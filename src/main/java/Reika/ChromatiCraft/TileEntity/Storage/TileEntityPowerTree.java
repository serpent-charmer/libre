...
public class TileEntityPowerTree extends CrystalReceiverBase
    implements CrystalBattery, OwnedTile, WirelessSource, MultiBlockChromaTile {
    private static final EnumMap<CrystalElement, BlockVector> origins
        = new EnumMap(CrystalElement.class);
    private static final EnumMap<CrystalElement, Integer> yOffsets
        = new EnumMap(CrystalElement.class);
    private static final EnumMap<CrystalElement, ArrayList<Coordinate>> locations
        = new EnumMap(CrystalElement.class);
    private static final ArrayList<Integer> directions = new ArrayList();

    private final EnumMap<CrystalElement, Integer> growth
        = new EnumMap(CrystalElement.class);
    private final EnumMap<CrystalElement, Integer> steps
        = new EnumMap(CrystalElement.class);
    private final EnumMap<CrystalElement, Integer> boost
        = new EnumMap(CrystalElement.class);
    ...
    static {
        ...
        addOrigin(
            CrystalElement.LIGHTGRAY, new BlockVector(ForgeDirection.NORTH, 1, -5, -2)
        );
        ...
        addOrigin(
            CrystalElement.LIGHTBLUE, new BlockVector(ForgeDirection.WEST, -1, -3, -1)
        );
        addOrigin(CrystalElement.MAGENTA, new BlockVector(ForgeDirection.EAST, 2, -7, 0));
        addOrigin(
            CrystalElement.ORANGE, new BlockVector(ForgeDirection.WEST, -1, -9, -1)
        );
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (world.isRemote) {
            if (!targets.isEmpty()) {
                ...
                ChromaFX.drawLeyLineParticles(
                    world, x, y, z, this.getOutgoingBeamRadius(), targets
                );
            }
        ...
        if (this.canConduct()) {
            if (!world.isRemote) {
                ...
                ProgressionCatchupHandling.instance.attemptSync(
                    this, 18, ProgressStage.POWERTREE
                );
            }

            for (EntityPlayer ep : this.getOwners(false)) {
                if (Chromabilities.RECHARGE.enabledOn(ep)) {
                    if (TileEntityAuraPoint.hasAuraPoints(ep)
                        || ep.getDistanceSq(x + 0.5, y + 0.5, z + 0.5) <= 2048) {
                        CrystalElement e
                            = CrystalElement.elements[(this.getTicksExisted() / 16) % 16];
                        if (this.getEnergy(e) > 0) {
                            int cap = PlayerElementBuffer.instance.getElementCap(ep);
                            int space = cap
                                - PlayerElementBuffer.instance.getPlayerContent(ep, e);
                            //long last = ep.getEntityData().getLong("lastSelfCharge");
                            //int gap = ep.worldObj.getTotalWorldTime()-last > 50 ? 20 :
                            //0;
                            if (space > 0) {
                                //int amt = Math.min(this.getEnergy(e), Math.min(space,
                                //Math.max(cap/24/8, 1+rand.nextInt(Math.max(1,
                                //space/4*4))))); ReikaJavaLibrary.pConsole(e+":"+amt);
                                ChromaAux.chargePlayerFromPylon(
                                    ep,
                                    this,
                                    e,
                                    this.getTicksExisted(),
                                    ep.getDistanceSq(x + 0.5, y + 0.5, z + 0.5) <= 4096
                                );
                            }
                        }
                    }
    ...
    private void doParticles(World world, int x, int y, int z) {
        if (canSendEnergy) {
            ...
            fx2.setIcon(ChromaIcons.FADE_GENTLE)
                .setColor(ReikaColorAPI.getColorWithBrightnessMultiplier(c, 0.7F))
                .setGravity(g)
                .setScale(s)
                .lockTo(fx);
    ...
    private boolean findBoost(CrystalElement e) {
        AxisAlignedBB box
            = AxisAlignedBB
                  .getBoundingBox(
                      xCoord, yCoord, zCoord - 1, xCoord + 2, yCoord + 1, zCoord + 1
                  )
                  .expand(7, 7, 7)
                  .offset(0, -6, 0);
            ...
            if (!ei.isDead && ei.delayBeforeCanPickup == 0) {
                ...
                if (is.stackSize > 0
                    && ReikaItemHelper.matchStacks(
                        is, ChromaItems.ELEMENTAL.getStackOf(e)
                    )) {
                    is.stackSize--;
                    if (is.stackSize > 0)
    ...
    private void spawnBeamParticles(World world, int x, int y, int z) {
        ...
        if (rand.nextInt(1+p*2) == 0) {
            for (CrystalTarget tg : targets) {
                ...
                EntityLaserFX fx = new EntityLaserFX(tg.color, world, px, py,
    pz).setScale(15); Minecraft.getMinecraft().effectRenderer.addEffect(fx);
            }
        }
    }
     */
    ...
    public void validateStructure() {
        ...
        FilledBlockArray f
            = ChromaStructures.TREE.getArray(worldObj, xCoord, yCoord, zCoord);
        ...
        enhanced = hasMultiblock && hadEnhancedProgress
            && ChromaStructures.TREE_BOOSTED.getArray(worldObj, xCoord, yCoord, zCoord)
                   .matchInWorld();
        this.checkHasSendFocus();
        for (int i = 0; i < 16; i++)
    ...
    private void checkHasSendFocus() {
        ...
        canSendEnergy = hasMultiblock
            && ChromaStructures.TREE_SENDER.getArray(worldObj, xCoord, yCoord, zCoord)
                   .matchInWorld();
    }
    ...
    private void onBreakMultiblock() {
        for (int i = 0; i < 16; i++) {
            ...
            worldObj.setBlock(
                xCoord + c.xCoord, yCoord + c.yCoord, zCoord + c.zCoord, Blocks.air
            );
        }
        energy.clear();
    }
    ...
    private void grow(CrystalElement e) {
        ...
        ArrayList<Coordinate> li = locations.get(e);
        //ReikaJavaLibrary.pConsole(e+" : "+this.getEnergy(e)+" >
        //"+(this.getEnergy(e)*100L)+" >
        //"+(this.getEnergy(e)*100L/this.getMaxStorage(e)));
        if (this.getEnergy(e) * 100L / this.getMaxStorage(e) >= 95) {
            ...
            if (stage < li.size()) {
                ...
                if (b == ChromaBlocks.POWERTREE.getBlockInstance()) {
                    TileEntityPowerTreeAux te
                        = (TileEntityPowerTreeAux) c.getTileEntity(worldObj);
                    if (te.grow()) {
                ...
                } else if (ReikaWorldHelper.softBlocks(
                               worldObj, c.xCoord, c.yCoord, c.zCoord
                           )) {
                    c.setBlock(
                        worldObj, ChromaBlocks.POWERTREE.getBlockInstance(), e.ordinal()
                    );
                    TileEntityPowerTreeAux te
                        = (TileEntityPowerTreeAux) c.getTileEntity(worldObj);
        ...
        if (set.size() >= 12 && set.contains(CrystalElement.BLACK)
            && set.contains(CrystalElement.YELLOW) && set.contains(CrystalElement.PURPLE)
            && set.contains(CrystalElement.LIGHTBLUE)) {
    ...
    private int getStorageForStep(CrystalElement e, int step) {
        return BASE
            + ReikaMathLibrary.intpow2(step, (this.isEnhanced() ? POWER_TURBO : POWER))
            * RATIO;
    }
    ...
    @Override
    public final void addSelfTickingTarget(
        WorldLocation loc,
        CrystalElement e,
        double dx,
        double dy,
        ...
        TickingCrystalTarget tg
            = new TickingCrystalTarget(this, loc, e, dx, dy, dz, w, maxW, duration);
        if (!worldObj.isRemote) {
            if (!targets.contains(tg)) {
                targets.add(tg);
    ...
    public void onBreakLeaf(World world, int x, int y, int z, CrystalElement e) {
        ...
        if (index >= 0) {
            ...
            for (int i = index + 1; i < li.size(); i++) {
                ...
                ArrayList<ItemStack> items = c2.getBlock(world).getDrops(
                    world, c2.xCoord, c2.yCoord, c2.zCoord, c2.getBlockMetadata(world), 0
                );
                ReikaItemHelper.dropItems(
                    world,
                    c2.xCoord + rand.nextDouble(),
                    c2.yCoord + rand.nextDouble(),
                    c2.zCoord + rand.nextDouble(),
                    items
                );
                c2.setBlock(world, Blocks.air);
                ReikaSoundHelper.playBreakSound(
                    world, c2.xCoord, c2.yCoord, c2.zCoord, Blocks.glass
                );
            }
        } else {
        }
    ...
    public void onUsedBy(EntityPlayer ep, CrystalElement e) {
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            PacketIDs.COLOREDPARTICLE.ordinal(),
            world,
            x,
            y,
            z,
        ...
        if (mod == 0) {
            ...
            ChromaSounds.DING.playSoundAtBlock(
                world, x, y, z, 1, (float) CrystalMusicManager.instance.getThird(e)
            );
        } else if (mod == 8) {
            ChromaSounds.DING.playSoundAtBlock(
                world, x, y, z, 1, (float) CrystalMusicManager.instance.getFifth(e)
            );
        } else if (mod == 12) {
            ChromaSounds.DING.playSoundAtBlock(
                world, x, y, z, 1, (float) CrystalMusicManager.instance.getOctave(e)
            );
        }
    }
    ...
    public Coordinate getChargeParticleOrigin(EntityPlayer ep, CrystalElement e) {
        ...
        return step == 0 ? new Coordinate(this)
                         : li.get(Math.min(step - 1, rand.nextInt(li.size())))
    ...
    public boolean canTransmitTo(TileEntityWirelessPowered te) {
        return this.getDistanceSqTo(te.xCoord, te.yCoord, te.zCoord)
            <= 1024; // && PylonFinder.lineOfSight(worldObj, xCoord, yCoord, zCoord,
