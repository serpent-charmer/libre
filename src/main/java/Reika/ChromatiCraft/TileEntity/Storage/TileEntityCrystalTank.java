...
public class TileEntityCrystalTank extends TileEntityChromaticBase
    implements CrystalTank, TankWatcher, BreakAction, AdjacentUpdateWatcher {
    ...
    private static final AdjacencyCheckHandlerImpl adjacency
        = TileEntityAdjacencyUpgrade.getOrCreateAdjacencyCheckHandler(
            CrystalElement.CYAN, "Increase capacity", ChromaTiles.TANK
        );
    ...
    private void initCoords(World world, int x, int y, int z) {
        if (blocks.isEmpty()) {
            ...
            Set<BlockKey> li = ReikaJavaLibrary.getSet(
                new BlockKey(ChromaBlocks.TANK.getBlockInstance())
            );
            toadd.recursiveAddMultipleWithBounds(
    ...
    public int fill(ForgeDirection from, FluidStack resource, boolean doFill) {
        int add
            = Math.min(this.getCapacity() - this.getCurrentFluidLevel(), resource.amount);
        if (add > 0) {
    ...
    private void updateBoostFactor() {
        ...
        capacityIncreaseFactor
            = lvl <= 0 ? 0 : TileEntityTankCapacityUpgrade.getCapacityFactor(lvl - 1);
    }
    ...
    @SideOnly(Side.CLIENT)
    public double
    getHeightOffsetAtCorner(int x, int y, int z, int dx, int dz, double h, float ptick) {
        ...
        double idx = 4D * (this.getTicksExisted() + ptick)
            + 48 * ((x + z + (dx + dz) * 0.5) % 16);
        double idx2 = 4D * (this.getTicksExisted() + ptick)
            + 128 * ((x + z + (dx + dz) * 0.5) % 32);
        ...
        idx *= fac;
        idx2 *= fac;
        double offset = 0.075 + 0.05 * Math.sin(Math.toRadians(idx))
