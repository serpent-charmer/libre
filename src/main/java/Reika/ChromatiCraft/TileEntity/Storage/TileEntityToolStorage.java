...
public class TileEntityToolStorage extends TileEntityMassStorage {
    ...
    protected KeyedItemStack key(ItemStack is) {
        return new KeyedItemStack(is)
            .setIgnoreMetadata(!filter.isDamageImportant())
            .setIgnoreNBT(!filter.isNBTImportant())
            .setSized(false)
            .setSimpleHash(true);
    }
    ...
    public static enum ToolType {
        ...
        TINKER(
            ModList.TINKERER.isLoaded()
                ? TinkerToolHandler.Tools.HAMMER.getToolOfMaterials(1, 1, 1, 1)
                : (ItemStack) null
        ),
        ...
        private static ToolType getTypeForTool(ItemStack is) {
            ...
            if (is.getItem() instanceof ItemEnchantedBook)
                return BOOK;
            if (is.getItem() == Items.iron_horse_armor
                || is.getItem() == Items.golden_horse_armor
                || is.getItem() == Items.diamond_horse_armor)
                return HORSEARMOR;
            if (is.getItem() instanceof ItemPotion)
                return POTION;
            if (ModList.BOTANIA.isLoaded()
                && InterfaceCache.BREWITEM.instanceOf(is.getItem()))
                return POTION;
        ...
        private static boolean isValidMiscToolItem(ItemStack is) {
            if (ModList.APPENG.isLoaded()) {
                Item i = is.getItem();
                if (i == AppEngHandler.getInstance().get1KCell()
                    || i == AppEngHandler.getInstance().get4KCell())
                    return false;
                if (i == AppEngHandler.getInstance().get16KCell()
                    || i == AppEngHandler.getInstance().get64KCell())
                    return false;
            }
