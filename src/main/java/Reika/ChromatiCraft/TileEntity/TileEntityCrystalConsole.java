...
public class TileEntityCrystalConsole
    extends InventoriedChromaticBase implements BreakAction, IBundledTile {
    ...
    public void toggle(int slot, boolean on) {
        ...
        ReikaSoundHelper.playSoundAtBlock(
            worldObj, xCoord, yCoord, zCoord, "random.click", 0.8F, 0.5F
        );
        states[slot] = on;
    ...
    private void validateMultiblock(World world, int x, int y, int z) {
        ...
        Sorter<ForgeDirection> s = new Sorter(
            ForgeDirection.EAST,
            ForgeDirection.WEST,
        ...
        if (flag) {
            for (Coordinate c : arr.keySet()) {
                TileEntityCrystalConsole te
                    = (TileEntityCrystalConsole) c.getTileEntity(world);
    ...
    private boolean isValidShape(BlockArray arr) {
        return arr.getVolume() == arr.getSize()
            && (arr.getSizeX() == 1 || arr.getSizeZ() == 1);
    }
    ...
    protected void readSyncTag(NBTTagCompound NBT) {
        ...
        states
            = ReikaArrayHelper.booleanFromBitflags(NBT.getInteger("states"), slotCount);
    ...
    public void breakBlock() {
        for (int i = 0; i < 6; i++) {
            ...
            if (te instanceof TileEntityCrystalConsole) {
                ((TileEntityCrystalConsole) te)
                    .validateMultiblock(te.worldObj, te.xCoord, te.yCoord, te.zCoord);
            }
        }
    }
    ...
    public static enum ControlMode {
        ...
        private void toggle(
            boolean on, TileEntityCrystalConsole te, WorldLocation loc, Object... data
        ) {
            switch (this) {
                case BUNDLED:
