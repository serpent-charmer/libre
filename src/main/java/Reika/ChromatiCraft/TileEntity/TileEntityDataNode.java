...
public class TileEntityDataNode extends TileEntityChromaticBase
    implements OperationInterval, StructureRenderingParticleSpawner {
    ...
    private final HashSet<String> scannedPlayers
        = new HashSet(); //not uuid since written to NBT
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (ep != null
            || (ReikaObfuscationHelper.isDeObfEnvironment()
                && KeyWatcher.instance.isKeyDown(
                    world.getPlayerEntityByName("Reika"), Key.LCTRL
                ))) {
        ...
        if (extension0 > 0 && !world.isRemote) {
            ...
            if (extension0 + extension1 > lastLowerHeight
                || extension2 > lastUpperHeight) {
            ...
            if (extension2 == EXTENSION_LIMIT_2) {
                if (this.getTicksExisted() % 50 == 0
                    || lastUpperHeight < EXTENSION_LIMIT_2)
            ...
            for (ChromaSounds s : snd) {
                ...
                List<EntityPlayer> li = world.getEntitiesWithinAABB(
                    EntityPlayer.class,
                    ReikaAABBHelper.getBlockAABB(this).expand(30, 18, 30)
                );
                for (EntityPlayer ep2 : li) {
                    float f = (float
                    ) Math.max(0.4, 2 - ep2.getDistance(x + 0.5, y + 2.5, z + 0.5) / 15D);
                    s.playSound(ep2, f, 1);
                }
            }
        }
        ...
        if (tower != null && !world.isRemote) {
            if (world.getBlock(x, y + 1, z) != ChromaBlocks.DUMMYAUX.getBlockInstance()
                || world.getBlock(x, y - 1, z)
                    != ChromaBlocks.STRUCTSHIELD.getBlockInstance()) {
                FilledBlockArray arr
                    = ChromaStructures.DATANODE.getArray(world, x, y - 1, z);
                arr.remove(x, y, z);
                arr.place();
            }
            if (rand.nextInt(plantRand) == 0) {
                if (this.spawnMetaAlloy(world, x, y, z)) {
                    ...
                    for (Coordinate c :
                         metaAlloyPlants.getLocations()) { //verify all flowers
                        if (c.getBlock(world)
            ...
            if (rand.nextInt(300) == 0) {
                ...
                List<EntityTunnelNuker> li
                    = world.getEntitiesWithinAABB(EntityTunnelNuker.class, box);
                if (li.size() < 8) {
                    if (ReikaWorldHelper.isRadiusLoaded(world, x, z, 6)) {
                        ...
                        int dy = world.getTopSolidOrLiquidBlock(dx, dz) + 3
                            + rand.nextInt(12);
    ...
    private boolean spawnMetaAlloy(World world, int x, int y, int z) {
        ...
        while (dy >= 0
                   && (b.isAir(world, dx, dy, dz)
                       || ReikaBlockHelper.isLeaf(world, dx, dy, dz))
               || ReikaBlockHelper.isWood(world, dx, dy, dz)) {
            dy--;
            b = world.getBlock(dx, dy, dz);
        }
        if (b == Blocks.grass
            && world.getBlock(dx, dy + 1, dz).isAir(world, dx, dy, dz)) {
            if (metaAlloyPlants.add(new Coordinate(dx, dy + 1, dz))) {
                world.setBlock(
                    dx, dy + 1, dz, ChromaBlocks.METAALLOYLAMP.getBlockInstance()
                );
    ...
    private void doParticles(World world, int x, int y, int z) {
        ...
        if (this.canBeAccessed()
            && rand.nextInt(5)
                == 0 /* && !this.hasBeenScanned(Minecraft.getMinecraft().thePlayer)*/) {
            if (tower != null) {
                if (tower == Towers.ALPHA) {
                    int idx = 1
                        + ((this.getTicksExisted() / 120) % (Towers.towerList.length - 1)
                        );
    ...
    private void sendParticlesToTower(World world, int x, int y, int z, Towers t) {
        ...
        EntityFloatingSeedsFX fx
            = new EntityCCFloatingSeedsFX(world, px, py, pz, a, 0, ChromaIcons.FADE);
        fx.freedom *= 0.5;
    ...
    public void scan(EntityPlayer ep) {
        if (DragonAPICore.isReikasComputer()
            && ReikaObfuscationHelper.isDeObfEnvironment()
            && KeyWatcher.instance.isKeyDown(ep, Key.LCTRL)) {
    ...
    private void doScan(EntityPlayer ep) {
        ...
        ReikaPacketHelper.sendDataPacketWithRadius(
            ChromatiCraft.packetChannel, ChromaPackets.DATASCAN.ordinal(), this, 128
        );
        ...
        EntityItem ei = ReikaItemHelper.dropItem(
            worldObj, xCoord + 0.5, yCoord + 5, zCoord + 0.5, is, 3
        );
    ...
    public static void doScanFX(World world, int x, int y, int z) {
        //ReikaSoundHelper.playClientSound(ChromaSounds.BOUNCE, x+0.5, y+0.5, z+0.5, 2, 2,
        //false);
        ReikaSoundHelper.playClientSound(
            ChromaSounds.BOUNCE, x + 0.5, y + 0.5, z + 0.5, 2, 1, false
        );
        ReikaSoundHelper.playClientSound(
            ChromaSounds.BOUNCE, x + 0.5, y + 0.5, z + 0.5, 2, 1, false
        );
        ReikaSoundHelper.playClientSound(
            ChromaSounds.MONUMENTRAY, x + 0.5, y + 0.5, z + 0.5, 2, 0.8F, false
        );
        ReikaSoundHelper.playClientSound(
            ChromaSounds.MONUMENTRAY, x + 0.5, y + 0.5, z + 0.5, 2, 1.6F, false
        );
        ReikaSoundHelper.playClientSound(
            ChromaSounds.KILLAURA, x + 0.5, y + 0.5, z + 0.5, 2, 2F, false
        );
        ReikaSoundHelper.playClientSound(
            ChromaSounds.KILLAURA, x + 0.5, y + 0.5, z + 0.5, 2, 1F, false
        );
        ...
        for (double a = 0; a < 360; a += 1) {
            EntityFloatingSeedsFX fx
                = new EntityCCFloatingSeedsFX(world, x + 0.5, y + 4.5, z + 0.5, a, 0);
        ...
        for (int i = -8; i <= 8; i++) {
            ...
            EntityFX fx = new EntityBlurFX(world, x+0.5, y+4.5+i*0.25, z+0.5, 0, 0.5,
        0).setColor(0xa0e0ff).setLife(120).setRapidExpand().setScale(s);
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
        }
         */

        for (double i = 0; i <= 64; i += 0.25) {
            EntityFX fx = new EntityCCBlurFX(world, x + 0.5, y + 4.5 + i, z + 0.5)
                              .setColor(0xa0e0ff)
                              .setLife(120)
                              .setRapidExpand()
                              .setAlphaFading()
            ...
            fx = new EntityCCBlurFX(world, x + 0.5, y + 4.5 + i, z + 0.5)
                     .setColor(0xffffff)
                     .setLife(120)
                     .setAlphaFading()
                     .setScale(1.5F);
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        return AxisAlignedBB
            .getBoundingBox(
                ...
                zCoord - 0.5,
                xCoord + 0.5,
                yCoord + EXTENSION_LIMIT_0 + EXTENSION_LIMIT_1 + EXTENSION_LIMIT_2 + 2,
                zCoord + 0.5
            )
            .expand(6, 6, 6);
    }
    ...
    public OperationState getState() {
        return !this.canBeAccessed() || scanCooldown > 0 ? OperationState.INVALID
            : scanTick > 0                               ? OperationState.RUNNING
