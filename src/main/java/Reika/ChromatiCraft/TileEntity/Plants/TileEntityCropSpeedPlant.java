...
public class TileEntityCropSpeedPlant extends TileEntityMagicPlant implements ComplexAOE {
    private static double[][] growthDistrib = {
        { 0, 0, 4, 0, 0 }, { 0, 1, 8, 1, 0 }, { 4, 8, 0, 8, 4 },
        { 0, 1, 8, 1, 0 }, { 0, 0, 4, 0, 0 },
    };

    private static double[][] hydrateDistrib = {
        { 0, 1, 4, 1, 0 }, { 1, 2, 6, 2, 1 }, { 4, 6, 4, 6, 4 },
        { 1, 2, 6, 2, 1 }, { 0, 1, 4, 1, 0 },
    };

    private static final WeightedRandom<Coordinate> growthRand
        = WeightedRandom.fromArray(growthDistrib);
    private static final WeightedRandom<Coordinate> hydrateRand
        = WeightedRandom.fromArray(hydrateDistrib);
    ...
    private void growCrop(World world, int x, int y, int z) {
        ...
        boolean flag = b instanceof BlockSapling
            || b == ChromaBlocks.DECOFLOWER.getBlockInstance() || b instanceof BlockReed
            || b instanceof BlockCactus || b == Blocks.vine;
    ...
    public boolean isPlantable(World world, int x, int y, int z) {
        return world.getBlock(x, y - 1, z) == Blocks.farmland
            ...
            || (world.getBlock(x, y - 1, z) == ChromaBlocks.CLIFFSTONE.getBlockInstance()
                && Variants.getVariant(world.getBlockMetadata(x, y - 1, z))
                    == Variants.FARMLAND);
    }
