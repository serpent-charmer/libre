...
public class TileEntityHarvesterPlant extends TileEntityMagicPlant implements ComplexAOE {
    private static double[][] randomDistrib = {
        { 3, 2, 1, 1, 2, 3, 2, 1, 1, 2, 3 }, { 1, 6, 4, 3, 4, 5, 4, 3, 4, 6, 2 },
        { 1, 4, 7, 5, 4, 6, 4, 5, 7, 4, 1 }, { 1, 3, 5, 8, 6, 7, 6, 8, 5, 3, 1 },
        { 2, 4, 4, 6, 9, 0, 9, 6, 4, 4, 2 }, { 3, 5, 6, 7, 0, 0, 0, 7, 6, 5, 3 },
        { 2, 4, 4, 6, 9, 0, 9, 6, 4, 4, 2 }, { 1, 3, 5, 8, 6, 7, 6, 8, 5, 3, 1 },
        { 1, 4, 7, 5, 4, 6, 4, 5, 7, 4, 1 }, { 2, 6, 4, 3, 3, 5, 3, 3, 4, 6, 2 },
        { 3, 2, 1, 1, 2, 3, 2, 1, 1, 2, 3 },
    };

    private static double[] heightDistrib = { 10, 8, 5, 2 };

    private static final WeightedRandom<Coordinate> coordinateRand
        = WeightedRandom.fromArray(randomDistrib);
        ...
        if (!world.isRemote) {
            ...
            if (this.isActive(world, x, y, z)) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if (ChromaTiles.getTileFromIDandMetadata(b, bmeta)
                        == ChromaTiles.FUNCTIONRELAY) {
                        c = ((TileEntityFunctionRelay) c.getTileEntity(world))
                                .getRandomCoordinate();
                        b = c.getBlock(world);
                        bmeta = c.getBlockMetadata(world);
                    }
                    if (c.getTaxicabDistanceTo(new Coordinate(this)) > 1
                        && this.canHarvest(
                            b, bmeta, world, c.xCoord, c.yCoord, c.zCoord
                        )) {
    ...
    private void loadCache(World world, int x, int y, int z) {
        ...
        for (int i = 2; i < 6; i++) {
            ...
            if (bk.blockID.getMaterial() == Material.plants
                || bk.blockID.getMaterial() == Material.leaves) {
                flowerCache.add(bk);
            }
        }
    }
    ...
    private boolean canHarvest(Block b, int meta, World world, int x, int y, int z) {
        if (ModList.BOTANIA.isLoaded() && BotaniaHandler.getInstance().flowerID == b)
            return true;
        return flowerCache.contains(new BlockKey(b, meta))
            && ReikaWorldHelper.checkForAdjBlock(world, x, y, z, b, meta) != null
    ...
    public boolean isPlantable(World world, int x, int y, int z) {
        return ReikaPlantHelper.FLOWER.canPlantAt(world, x, y, z)
            || ReikaBlockHelper.isLeaf(world, x, y - 1, z)
