...
public class TileEntityCobbleGen
    extends TileEntityMagicPlant implements OperationInterval {
    private final MultiMap<String, Coordinate> fluidLocations
            ...
            if (!world.isRemote) {
                for (int i = 0; i < FluidMix.list.length; i++) {
                    ...
                    Coordinate secondary
                        = this.getFluid(world, x, y, z, f.secondaryFluid);
    ...
    private void doRecipeTick(World world, int x, int y, int z) {
        ...
        if (recipeTick == 0) {
            ...
            if (world.isRemote) {
                ...
                FluidStack f1 = this.getFluidAtBlock(
                    world,
                    primaryLocation.xCoord,
                    primaryLocation.yCoord,
                    primaryLocation.zCoord
                );
                FluidStack f2 = this.getFluidAtBlock(
                    world,
                    secondaryLocation.xCoord,
                    secondaryLocation.yCoord,
                    secondaryLocation.zCoord
                );
                if (f1 != null
                    && f1.getFluid().getName().equals(activeRecipe.primaryFluid)
                    && f1.amount >= activeRecipe.requiredPrimaryAmount && f2 != null
                    && f2.getFluid().getName().equals(activeRecipe.secondaryFluid)
                    && f2.amount >= activeRecipe.requiredSecondaryAmount) {
                } else {
            ...
            if (b1 instanceof FluidBlockSurrogate) {
                ...
                if (fb.supportsQuantization(
                    ...
                    fb.drain(
                        ...
                        primaryLocation.zCoord,
                        f,
                        activeRecipe.requiredPrimaryAmount,
                        true
                    );
            ...
            if (b2 instanceof FluidBlockSurrogate) {
                ...
                if (fb.supportsQuantization(
                    ...
                    fb.drain(
                        ...
                        secondaryLocation.zCoord,
                        f,
                        activeRecipe.requiredSecondaryAmount,
                        true
                    );
                } else if (ReikaRandomHelper.doWithChance(
                               activeRecipe.consumeSecondaryFluid
                           )) {
                    fb.drain(
                        world,
                        secondaryLocation.xCoord,
    ...
    private void terminateCrafting(World world, int x, int y, int z, boolean success) {
        ReikaPacketHelper.sendDataPacketWithRadius(
            ...
            ChromaPackets.COBBLEGENEND.ordinal(),
            this,
            64,
            activeRecipe.ordinal(),
            success ? 1 : 0
        );
        activeRecipe = null;
        recipeTick = 0;
    ...
    @SideOnly(Side.CLIENT)
    public void
    endCraftingFX(World world, int x, int y, int z, int recipe, boolean success) {
        ...
        for (int i = a; i < 360; i += n) {
            ...
            if (success) {
                fx = new EntityCCBlurFX(
                         world, x + 0.5, y + 0.125, z + 0.5, vel[0], -0.125, vel[2]
                )
                         .setGravity(-0.125F);
            } else {
                fx = new EntityCCBlurFX(
                         world, x + 0.5, y + 0.125, z + 0.5, vel[0], 0, vel[2]
                )
                         .setIcon(ChromaIcons.SPARKLE)
                         .setNoSlowdown();
            }
            int c = tag == null || tag.isEmpty()
                ? 0x22aaff
                : ReikaJavaLibrary.getRandomCollectionEntry(rand, tag.elementSet())
                      .getColor();
    ...
    private void doRecipeParticles(World world, int x, int y, int z) {
        if (activeRecipe == null || primaryLocation == null || secondaryLocation == null)
            return;
        double px
            = ReikaRandomHelper.getRandomPlusMinus(primaryLocation.xCoord + 0.5, 0.35);
        double pz
            = ReikaRandomHelper.getRandomPlusMinus(primaryLocation.zCoord + 0.5, 0.35);
        EntityFluidFX fx1 = new EntityFluidFX(
            world,
            px,
            primaryLocation.yCoord + 0.85,
            pz,
            FluidRegistry.getFluid(activeRecipe.primaryFluid)
        );
        ...
        EntityFluidFX fx2 = new EntityFluidFX(
            world,
            px,
            secondaryLocation.yCoord + 0.85,
            pz,
            FluidRegistry.getFluid(activeRecipe.secondaryFluid)
        );

        fx1.setMotionController(new AttractiveMotionController(
            xCoord + 0.5,
            yCoord - 0.375,
            zCoord + 0.5,
            0.0625 / 24D,
            ReikaRandomHelper.getRandomPlusMinus(0.155, 0.005),
            ReikaRandomHelper.getRandomPlusMinus(0.98, 0.005)
        ));
        fx2.setMotionController(new AttractiveMotionController(
            xCoord + 0.5,
            yCoord - 0.375,
            zCoord + 0.5,
            0.0625 / 24D,
        ...
        if (rand.nextBoolean()) {
            CrystalElement e = ChromaAux.getRune(FluidRegistry.getFluid(
                rand.nextBoolean() ? activeRecipe.primaryFluid
                                   : activeRecipe.secondaryFluid
            ));
            fx = new EntityRuneFX(
                     world,
                     x + rand.nextDouble(),
            ...
            ElementTagCompound tag
                = ItemElementCalculator.instance.getValueForItem(activeRecipe.output);
            CrystalElement e = tag != null && !tag.isEmpty()
                ? ReikaJavaLibrary.getRandomCollectionEntry(rand, tag.elementSet())
                : null;
            if (e != null)
                fx = new EntityRuneFX(
                         world, x + rand.nextDouble(), dy + 1, z + rand.nextDouble(), e
                )
                         .setGravity(g)
                         .setScale(s);
        }
    ...
    private void scanPosition(World world, int dx, int y, int dz, boolean all) {
        ...
        if (dy != -1) {
            if (ChromaTiles.getTileFromIDandMetadata(
                    world.getBlock(dx, dy, dz), world.getBlockMetadata(dx, dy, dz)
                )
                == ChromaTiles.FUNCTIONRELAY) {
                TileEntityFunctionRelay te
                    = (TileEntityFunctionRelay) world.getTileEntity(dx, dy, dz);
                if (all) {
                    for (Coordinate c : te.getCoordinates()) {
                        FluidStack f
                            = this.getFluidAtBlock(world, c.xCoord, c.yCoord, c.zCoord);
                        if (f != null) {
            ...
            if (f != null) {
                fluidLocations.addValue(
                    f.getFluid().getName(), new Coordinate(dx, dy, dz)
                );
            }
        }
    }
    ...
    private FluidStack getFluidAtBlock(World world, int dx, int dy, int dz) {
        ...
        if (ReikaWorldHelper.isLiquidSourceBlock(world, dx, dy, dz)) {
            ...
            return f != null
                ? new FluidStack(f, fb.drain(world, dx, dy, dz, f, 1000, false))
                : null;
        }
        return null;
    }
    ...
    private int getYPosition(World world, int x, int y, int z) {
        ...
        if (!b.isAir(world, x, y, z)
            && ChromaTiles.getTileFromIDandMetadata(b, world.getBlockMetadata(x, y, z))
                != this.getTile())
            return -1;
        int d = 0;
        while (y > 0
               && (b.isAir(world, x, y, z)
                   || ChromaTiles.getTileFromIDandMetadata(
                          b, world.getBlockMetadata(x, y, z)
                      ) == this.getTile())) {
            y--;
            d++;
            ...
            for (LuaBlock lb : crl.getEntries()) {
                ...
                if (flag) {
                    ChromatiCraft.logger.log(
                        "Loaded custom fluidmix recipe '" + lb.getString("type") + "'"
                    );
                } else {
                    ChromatiCraft.logger.logError(
                        "Could not load custom fluidmix recipe '" + lb.getString("type")
                        + "'"
                    );
                    if (e != null)
                        e.printStackTrace();
    ...
    private static boolean addCustomRecipe(LuaBlock lb, CustomRecipeList crl)
        throws Exception {
        ...
        addRecipe(
            lb.getString("type").toUpperCase(Locale.ENGLISH),
            f1,
            f2,
            out,
            time,
    ...
    public static void addRecipe(
        ...
        ItemStack out,
        int time,
        float chanceConsumePrimary,
        float chanceConsumeSecondary
    ) {
        Class[] types = new Class[] { Fluid.class, Fluid.class, ItemStack.class,
                                      int.class,   float.class, float.class };
        Object[] args = new Object[] {
            primary, secondary, out, time, chanceConsumePrimary, chanceConsumeSecondary
        };
    ...
    private static enum FluidMix {
        ...
        //ALLOY1("iron.molten", "lava",
        //ChromaBlocks.PYLONSTRUCT.getStackOfMetadata(StoneTypes.SMOOTH.ordinal()), 10,
        //10F, 20F),
        CLIFFSTONE(
            "luma",
            "lava",
            ChromaBlocks.CLIFFSTONE
                .getStackOfMetadata(Variants.STONE.getMeta(false, false)),
            120,
            0F,
            10F
        ),
        ...
        private FluidMix(String f1, String f2, ItemStack is, int t, float c1, float c2) {
            ...
            requiredPrimaryAmount
                = Math.max(1, (int) (1000 * consumePrimaryFluid / 100D));
            requiredSecondaryAmount
                = Math.max(1, (int) (1000 * consumeSecondaryFluid / 100D));
        }
    }
    ...
    private static enum OutputModifier {
        ...
        @SideOnly(Side.CLIENT)
        private void
        doParticles(World world, int x, int y, int z, TileEntityCobbleGen te) {
            switch (this) {
                case COBBLESMELT:
                    ...
                    for (int i = 0; i < 360; i += sp) {
                        ...
                        EntityFX fx
                            = new EntityCCBlurFX(
                                  CrystalElement.ORANGE, world, dx, dy, dz, 0, 0.1875, 0
                            )
                                  .setIcon(ChromaIcons.TRIDOT)
                                  .setScale(2.5F)
                                  .setNoSlowdown()
                                  .setLife(25);
                        Minecraft.getMinecraft().effectRenderer.addEffect(fx);
    ...
    public boolean isPlantable(World world, int x, int y, int z) {
        return (world.getBlock(x, y + 1, z)
            ...
            || ChromaTiles.getTile(world, x, y + 1, z) == ChromaTiles.PLANTACCEL;
    }
