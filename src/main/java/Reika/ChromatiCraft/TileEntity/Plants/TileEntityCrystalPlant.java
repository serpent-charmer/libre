...
public class TileEntityCrystalPlant extends TileEntity {
    ...
    public void grow() {
        if (growthTick > 0) {
            ...
            if (this.isPure()) {
                for (int i = 2; i < 6; i++) {
                    double chance = this.is(Modifier.PRIMAL) ? 80
                        : this.is(Modifier.BOOSTED)          ? 30
                                                             : 5;
                    if (worldObj.getBlock(xCoord, yCoord - 1, zCoord)
                        == ChromaBlocks.MUD.getBlockInstance())
                        chance *= 2;
                    if (ReikaRandomHelper.doWithChance(Math.min(1, chance))) {
                        ...
                        if (id == ChromaBlocks.PLANT.getBlockInstance()
                            && meta == this.getColor().ordinal()) {
                            TileEntityCrystalPlant te = (TileEntityCrystalPlant
                            ) worldObj.getTileEntity(dx, dy, dz);
                            te.grow();
                        }
                    }
                }
        ...
        for (int i = 2; i < 6; i++) {
            ...
            if (id == ChromaBlocks.PLANT.getBlockInstance()
                ...
                TileEntityCrystalPlant te
                    = (TileEntityCrystalPlant) worldObj.getTileEntity(dx, dy, dz);
    ...
    public void harvest(boolean drops) {
        ...
        if (drops) {
            ...
            ReikaItemHelper.dropItems(
                worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, li
            );
        }
        this.updateLight();
    }
    ...
    public ArrayList<ItemStack> getDrops() {
        ...
        if (this.isPure() || random.nextInt(4) == 0) {
            ...
            if (ChromaOptions.CRYSTALFARM.getState()) {
                if (time - lastShardTick
                    >= (this.is(Modifier.PRIMAL)        ? 60
                            : this.is(Modifier.BOOSTED) ? 150
                                                        : 600)) {
                    if (ReikaRandomHelper.doWithChance(
                            this.is(Modifier.PRIMAL)        ? 10
                                : this.is(Modifier.BOOSTED) ? 5
                                                            : 2
                        )) {
    ...
    public void setStates(ItemStack item) {
        ...
        sterile = item.stackTagCompound != null
            && item.stackTagCompound.getBoolean("sterile");
    }
