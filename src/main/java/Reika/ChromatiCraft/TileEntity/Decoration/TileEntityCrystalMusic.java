...
public class TileEntityCrystalMusic extends TileEntityChromaticBase
    implements SpecialStructureTile, GuiController, TriggerableAction, BreakAction {
    ...
    static {
        MusicScore mus = null;
        try {
            mus = new MIDIInterface(ChromatiCraft.class, "Resources/music-demo.mid")
                      .fillToScore(false)
                      .scaleSpeed(11, false);
    ...
    private int getTime(int channel) {
        ...
        return pos
            + (n != null && !n.isEmpty() ? n.iterator().next().length : 0
            ); //only used for manual entry, which does not allow chords
    }
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (world.isRemote) {
            for (Entry<CrystalElement, Coordinate> e : colorPositions.entrySet()) {
                ...
                if (c2.getBlock(world) instanceof CrystalBlock
                    && c2.getBlockMetadata(world) == cc.ordinal())
                    continue;
                ChromaFX.doPlacementHintParticles(
                    world, x, y, z, Arrays.asList(c), cc.getColor()
                );
            }
        }
        ...
        if (DragonAPICore.debugtest) {
            for (CrystalElement e : colorPositions.keySet()) {
                colorPositions.get(e).offset(x, y, z).setBlock(
                    world, ChromaBlocks.LAMP.getBlockInstance(), e.ordinal()
                );
            }
        }
    }
    ...
    private void calcPaths(World world, int x, int y, int z) {
        if (!world.isRemote) {
            if (this.playRepeaters(world, x, y, z)) {
                ...
                for (int i = 0; i < 16; i++) {
                    networkConnections[i] = CrystalNetworker.instance.getConnectivity(
                        CrystalElement.elements[i], receiver
                    );
                }
            } else {
                receiver = null;
    ...
    private boolean playRepeaters(World world, int x, int y, int z) {
        ...
        return b == Blocks.quartz_block
            || b == ChromaBlocks.PYLONSTRUCT.getBlockInstance() || this.hasTemple();
    }
    ...
    private boolean playNote(World world, int x, int y, int z, Note n, int track) {
        ...
        if (!set.isEmpty()) {
            ...
            if (canPlay)
                ; //this.generateParticles(world, x, y, z,
                  //ReikaJavaLibrary.getRandomCollectionEntry(set));
        } else {
            canPlay = true;
            ...
            //CrystalElement e = CrystalElement.randomElement();
            //ChromaFX.doBoltFX(worldObj, xCoord, yCoord, zCoord, new
            //DecimalPosition(colorPositions.get(e).offset(x, y, z)), e.getColor(),
            //1, 1.5, false);
        }
        ...
        if (canPlay) {
            ...
            /*
            if (s == ChromaSounds.FLUTE || (s instanceof SoundVariant &&
            ((SoundVariant)s).root == ChromaSounds.FLUTE)) { f = 1F; v = 0.5F;
            }*/
            ...
            ReikaPacketHelper.sendDataPacketWithRadius(
                ...
                ChromaPackets.CRYSTALMUSERROR.ordinal(),
                this,
                BROADCAST_RANGE
            );
            return false;
    ...
    private boolean isBassline(MusicKey n) {
        return enableBassline
            && n.pitch < CrystalMusicManager.instance.getBasePitch(CrystalElement.BLACK);
    }
    ...
    private boolean attentuate(World world, int x, int y, int z) {
        return !temple.isComplete()
            ...
            != Blocks.quartz_block; //world.getBlock(x, y-1, z) !=
                                    //ChromaBlocks.RUNE.getBlockInstance() ||
                                    //world.getBlockMetadata(x, y-1, z) !=
                                    //CrystalElement.YELLOW.ordinal();
    }

    public boolean playCrystal(
        World world, int x, int y, int z, CrystalElement e, int length, MusicKey note
    ) {
        if (temple.isComplete() && temple.isPlayingMelody()) {
            return true;
        }
        ...
        if (b instanceof CrystalBlock && c.getBlockMetadata(world) == e.ordinal()) {
            if (this.isBassline(note))
                ChromaFX.doBoltFX(
                    ...
                    yCoord,
                    zCoord,
                    new DecimalPosition(colorPositions.get(e).offset(x, y, z)),
                    e.getColor(),
                    1,
                    0.75,
                    false
            ...
            if (networkConnections[e.ordinal()] != null
                && networkConnections[e.ordinal()].stillValid().canConduct()) {
                //ReikaJavaLibrary.pConsole(networkConnections[e.ordinal()]);
                networkConnections[e.ordinal()].blink(
                    MathHelper.clamp_int(length / 2, 1, 40), receiver
                );
            }
            return true;
        }
    ...
    private void generateParticles(
        ...
        if (world.isRemote) {
            ...
            ReikaPacketHelper.sendDataPacketWithRadius(
                ChromatiCraft.packetChannel,
                ChromaPackets.CRYSTALMUS.ordinal(),
                this,
                BROADCAST_RANGE,
                x,
                y,
    ...
    public void doParticles(
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityFX fx = new EntityCCFloatingSeedsFX(world, px, py, pz, 0, 90)
                              .setLife(l)
                              .setColor(e.getColor())
                              .setGravity(g);
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
    ...
    public boolean loadLocalMIDI(String file, int quarterTicks) {
        ...
        if (!f.exists() || f.isDirectory()) {
            ChromatiCraft.logger.logError(
                "Could not load local MIDI: file is not a MIDI file!"
            );
            return false;
        }
        if (f.length() > 1024 * ChromaOptions.MIDISIZE.getValue()
            && !DragonAPICore.isSinglePlayerFromClient()) {
            ChromatiCraft.logger.logError(
                "Could not load local MIDI: file is too large (" + f.length()
                + " bytes) and cannot be safely used!"
            );
            return false;
        }
        try {
            float speed
                = 99F / quarterTicks; //9 ticks per quarter note at 11, 8 at 12.375
            MusicScore mus
                = new MIDIInterface(f).fillToScore(false).scaleSpeed(speed, true);
            mus.normalizeToRange(MusicKey.C4, MusicKey.C6);
            this.dispatchTrack(mus);
            ChromatiCraft.logger.log(
                "Loaded MIDI with " + mus.noteCount() + " notes in "
                + mus.getActiveTracks().size() + " tracks with a speed of " + quarterTicks
                + " ticks/beat"
            );
            return true;
        } catch (Exception e) {
    ...
    private void dispatchTrack(MusicScore mus) {
        ReikaPacketHelper.sendPacketToServer(
            ChromatiCraft.packetChannel, ChromaPackets.MUSICCLEAR.ordinal(), this
        );
            ...
            for (Entry<Integer, NoteData> e : track.entryView()) {
                ...
                for (MusicScore.Note n : c.notes()) {
                    if (n != null && n.key != null)
                        ReikaPacketHelper.sendPacketToServer(
                            ChromatiCraft.packetChannel,
                            ChromaPackets.FIXEDMUSICNOTE.ordinal(),
                            this,
                            i,
                            n.key.ordinal(),
                            n.length,
    ...
    public AxisAlignedBB getRenderBoundingBox() {
        return temple.isRendering() ? ReikaAABBHelper.getBlockAABB(this).expand(9, 6, 9)
