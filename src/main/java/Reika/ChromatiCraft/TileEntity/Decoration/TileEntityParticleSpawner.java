...
public class TileEntityParticleSpawner
    extends TileEntityChromaticBase implements GuiController, NBTCopyable {
    ...
    public static class ParticleDefinition {
        ...
        public VariableValue<Double> particlePositionX
            = new VariableValue(-16D, 16D).setStep(0.03125);
        public VariableValue<Double> particlePositionY
            = new VariableValue(-16D, 16D).setStep(0.03125);
        public VariableValue<Double> particlePositionZ
            = new VariableValue(-16D, 16D).setStep(0.03125);

        public VariableValue<Double> particleVelocityX
            = new VariableValue(-2D, 2D).setStep(0.03125);
        public VariableValue<Double> particleVelocityY
            = new VariableValue(-2D, 2D).setStep(0.03125);
        public VariableValue<Double> particleVelocityZ
            = new VariableValue(-2D, 2D).setStep(0.03125);

        public VariableValue<Float> particleSize
            = new VariableValue(0.125F, 50F, 1F).setStep(0.125F);

        public VariableValue<Float> particleGravity
            = new VariableValue(-2F, 2F).setStep(0.03125F / 2F);

        public VariableValue<Integer> particleLife
            = new VariableValue(5, 1200, 20).setStep(1);
        ...
        public BoundedValue<Integer> particleRate
            = new BoundedValue(1, 300, 10).setStep(1);
        ...
        public EntityFX getFX(int tick) {
            if (tick % (int) particleRate.getValue() == 0) {
                ...
                double px = location.xCoord + 0.5
                    + ReikaRandomHelper.getRandomPlusMinus(
                        particlePositionX.getValue(), particlePositionX.getVariation()
                    );
                double py = location.yCoord + 0.5
                    + ReikaRandomHelper.getRandomPlusMinus(
                        particlePositionY.getValue(), particlePositionY.getVariation()
                    );
                double pz = location.zCoord + 0.5
                    + ReikaRandomHelper.getRandomPlusMinus(
                        particlePositionZ.getValue(), particlePositionZ.getVariation()
                    );
                double vx = ReikaRandomHelper.getRandomPlusMinus(
                    particleVelocityX.getValue(), particleVelocityX.getVariation()
                );
                double vy = ReikaRandomHelper.getRandomPlusMinus(
                    particleVelocityY.getValue(), particleVelocityY.getVariation()
                );
                double vz = ReikaRandomHelper.getRandomPlusMinus(
                    particleVelocityZ.getValue(), particleVelocityZ.getVariation()
                );
                float g = (float) ReikaRandomHelper.getRandomPlusMinus(
                    particleGravity.getValue(), particleGravity.getVariation()
                );
                float s = (float) ReikaRandomHelper.getRandomPlusMinus(
                    particleSize.getValue(), particleSize.getVariation()
                );
                int l = ReikaRandomHelper.getRandomPlusMinus(
                    (int) particleLife.getValue(), (int) particleLife.getVariation()
                );
                if (particleIcon instanceof CrystalElement) {
                    EntityRuneFX fx
                        = new EntityRuneFX(
                              world, px, py, pz, vx, vy, vz, (CrystalElement) particleIcon
                        )
                              .setGravity(g)
                              .setLife(l)
                              .setScale(s);
                    ...
                    if (f == ChromatiCraft.chroma) {
                        EntityChromaFluidFX fx
                            = new EntityChromaFluidFX(world, px, py, pz, vx, vy, vz)
                                  .setGravity(g)
                                  .setLife(l)
                                  .setScale(s);
                        if (noSlowdown)
                        ...
                        EntityFluidFX fx
                            = new EntityFluidFX(world, px, py, pz, vx, vy, vz, f)
                                  .setGravity(g)
                                  .setLife(l)
                                  .setScale(s);
                        fx.noClip = true;
        ...
        public void sendData() {
            ...
            ReikaPacketHelper.sendNBTPacket(
                ...
                ChromaPackets.PARTICLESPAWNER.ordinal(),
                nbt,
                PacketTarget.server
            );
        }
        ...
        public void readFromNBT(NBTTagCompound NBT) {
            ...
            if (particleIcon == null) {
                ChromatiCraft.logger.logError(
                    "Tried to load invalid particle type '" + NBT.getString("icon")
                    + "' from NBT."
                );
            }
        ...
        public void readLuaBlock(LuaBlock lb) {
            ...
            if (particleIcon == null) {
                ChromatiCraft.logger.logError(
                    "Tried to load invalid particle type '" + lb.getString("icon")
                    + "' from NBT."
                );
            }
    ...
    public static class VariableValue<N extends Number> {
        ...
        private void onValueChange() {
            variation = new BoundedValue(0D, this.getMaxVariance(), variation.getValue())
                            .setStep(bounds.getStep());
        }

        private double getMaxVariance() {
            return Math.min(
                Math.abs(Math.abs(bounds.getMinValue()) - Math.abs(bounds.getValue())),
                Math.abs(Math.abs(bounds.getMaxValue()) - Math.abs(bounds.getValue()))
            );
        }
