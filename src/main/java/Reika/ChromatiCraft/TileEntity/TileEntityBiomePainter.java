...
public class TileEntityBiomePainter
    extends TileEntityChromaticBase implements GuiController {
    private static final Collection<BiomeConnection> blacklist = new ArrayList();
    private static final MultiMap<BiomeGenBase, BiomeGenBase> availableBiomes
        = new MultiMap();
    ...
    public BiomeGenBase getNaturalBiomeAt(int dx, int dz) {
        return ReikaWorldHelper.getNaturalGennedBiomeAt(
            worldObj, xCoord + dx - RANGE, zCoord + dz - RANGE
        );
    }
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        //ReikaJavaLibrary.pConsole(world.getBiomeGenForCoords(x,
        //z)+"/"+ReikaWorldHelper.getNaturalGennedBiomeAt(world, x, z), Side.SERVER);
        if (world.isRemote) {
    ...
    private void doParticles(World world, int x, int y, int z) {
        ...
        EntityFX fx = new EntityCenterBlurFX(world, px, py, pz, vx, vy, vz)
                          .setGravity(g)
                          .setColor(rand.nextInt(0xffffff));
    ...
    public void changeBiomeAt(int dx, int dz, BiomeGenBase biome) {
        if (this.canChangeBiomeAt(dx, dz, worldObj.getBiomeGenForCoords(dx, dz))) {
            if (!worldObj.isRemote) {
                ...
                //ReikaJavaLibrary.pConsole("Setting biome "+biome+" with ID "+id+" @
                //"+dx+", "+dz);
                ReikaPacketHelper.sendPacketToServer(
                    ChromatiCraft.packetChannel,
                    ChromaPackets.BIOMEPAINT.ordinal(),
                    this,
                    dx,
                    dz,
                    id
    ...
    private static class ChromaBiomeBlacklist implements BiomeConnection {
        ...
        public boolean isLegalTransition(BiomeGenBase in, BiomeGenBase out) {
            return (DragonAPICore.isReikasComputer()
                ...
                || (this.isAccessibleBiome(in) && this.isAccessibleBiome(out));
        }
