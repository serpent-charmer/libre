...
public class TileEntityAutoEnchanter extends FluidReceiverInventoryBase
    implements ChromaPowered, ChromaExtractable, OperationInterval, VariableTexture {
    ...
    static {
        ...
        allowedPairs.add(
            new ImmutablePair(Enchantment.sharpness, Enchantment.baneOfArthropods)
        );
    ...
    public EnchantValidity isCompatible(Enchantment e) {
        ...
        if (inv[0] != null) {
            if (!this.isCompatible(
                    ReikaEnchantmentHelper.getEnchantments(inv[0]).keySet(), e
                ))
    ...
    private boolean isCompatible(Enchantment e, Enchantment e2) {
        return e == e2 || allowedPairs.contains(new ImmutablePair(e, e2))
            || allowedPairs.contains(new ImmutablePair(e2, e))
            || ReikaEnchantmentHelper.areEnchantsCompatible(e, e2);
    }

    public boolean
    isEnchantValid(Enchantment e, ItemStack is, Item i, boolean checkLevels) {
        ...
        if (checkLevels
            && ReikaEnchantmentHelper.getEnchantmentLevel(e, is) >= selected.get(e))
            return false;
        ...
        if (i instanceof ItemShears)
            return e.type == EnumEnchantmentType.digger
                || e.type == EnumEnchantmentType.breakable
                || e.type == EnumEnchantmentType.all;

        if (ModList.TINKERER.isLoaded() && TinkerToolHandler.getInstance().isTool(is)) {
            return e instanceof ChromaticEnchantment
                && (e.type == EnumEnchantmentType.all
                    || e.type == EnumEnchantmentType.digger);
        } else if (ModList.TINKERER.isLoaded() && TinkerToolHandler.getInstance().isWeapon(is)) {
            return e instanceof ChromaticEnchantment
                && (e.type == EnumEnchantmentType.all
                    || e.type == EnumEnchantmentType.weapon);
        }
    ...
    private boolean isSoulboundable(ItemStack is, Item i) {
        ...
        if (Loader.isModLoaded("Backpack")
            && i.getClass().getName().toLowerCase(Locale.ENGLISH).contains("backpack"))
            return true;
        if (Loader.isModLoaded("EnderStorage")
            && i.getClass().getName().toLowerCase(Locale.ENGLISH).contains("enderpouch"))
            return true;
        if (ModList.THAUMCRAFT.isLoaded()
            && i.getClass().getName().toLowerCase(Locale.ENGLISH).contains("focuspouch"))
            return true;
        if (ModList.THAUMCRAFT.isLoaded()
            && i == ThaumItemHelper.ItemEntry.WAND.getItem().getItem())
            return true;
        if (ModList.THAUMICTINKER.isLoaded()
            && i.getClass().getName().toLowerCase(Locale.ENGLISH).contains("ichorpouch"))
            return true;
        if (i instanceof ItemChromaTool || i instanceof ItemTool || i instanceof ItemSword
            || i instanceof ItemShears || i instanceof ItemArmor)
            return true;
        if (ModList.TINKERER.isLoaded()
            && (TinkerToolHandler.getInstance().isTool(is)
                || TinkerToolHandler.getInstance().isWeapon(is)))
            return true;
        return false;
    }
    ...
    private void applyEnchants() {
        ...
        if (inv[1] != null) {
            ...
            for (Entry<Enchantment, Integer> e : selected.entrySet())
                dmg += e.getValue() * ReikaRandomHelper.getRandomBetween(4D, 15D)
                    * this.getCostFactor(e.getKey());
    ...
    public int getConsumedChroma() {
        ...
        for (Enchantment e : selected.keySet()) {
            ...
            if (inv[1] != null)
                level = Math.max(
                    0.25F,
                    level - 0.8F * ReikaEnchantmentHelper.getEnchantmentLevel(e, inv[1])
                );
    ...
    public OperationState getState() {
        return this.isValid(inv[0]) && this.enchanting()
            ? (this.hasSufficientChroma() ? OperationState.RUNNING
    ...
    public static boolean isBlacklisted(Enchantment e) {
        if (e.getName()
                ...
                .startsWith("enchantment.molecule")) //Minechem
            return true;
        if (ModList.THAUMCRAFT.isLoaded()
            && InfusionEnchantmentHandler.instance.isInfusionEnchantment(e))
            return true;
    ...
    public static void blacklistEnchantment(Enchantment e) {
        if (ReikaEnchantmentHelper.isVanillaEnchant(e)) {
            ...
            ChromatiCraft.logger.logError(
                "You cannot blacklist ChromatiCraft enchantments!"
            );
        } else {
            blacklist.add(e);
            ChromatiCraft.logger.log(
                "Received request to blacklist enchantment " + e.getName() + " from "
                + ReikaRegistryHelper.getActiveLoadingMod()
            );
        }
    }
    ...
    public static int getAvailableEnchantmentLevel(Enchantment e, EntityPlayer ep) {
        return e instanceof ChromaticEnchantment
            ? getMaxEnchantment(ep, (ChromaticEnchantment) e)
            : getMaxEnchantmentLevel(e);
    }
    ...
    public boolean isAssisted() {
        return worldObj.getBlock(xCoord, yCoord + 1, zCoord)
                   instanceof BlockEnchantmentTable;
    }
    ...
    private float getAssistPower() {
        ...
        for (int dz = -1; dz <= 1; dz++) {
            for (int dx = -1; dx <= 1; dx++) {
                if ((dz != 0 || dx != 0)
                    && worldObj.isAirBlock(xCoord + dx, y, zCoord + dz)
                    && worldObj.isAirBlock(xCoord + dx, y + 1, zCoord + dz)) {
                    ret += ForgeHooks.getEnchantPower(
                        worldObj, xCoord + dx * 2, y, zCoord + dz * 2
                    );
                    ret += ForgeHooks.getEnchantPower(
                        worldObj, xCoord + dx * 2, y + 1, zCoord + dz * 2
                    );
                    max += 2;

                    if (dx != 0 && dz != 0) {
                        ret += ForgeHooks.getEnchantPower(
                            worldObj, xCoord + dx * 2, y, zCoord + dz
                        );
                        ret += ForgeHooks.getEnchantPower(
                            worldObj, xCoord + dx * 2, y + 1, zCoord + dz
                        );
                        ret += ForgeHooks.getEnchantPower(
                            worldObj, xCoord + dx, y, zCoord + dz * 2
                        );
                        ret += ForgeHooks.getEnchantPower(
                            worldObj, xCoord + dx, y + 1, zCoord + dz * 2
                        );
                        max += 4;
                    }
                }
