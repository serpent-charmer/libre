...
public class TileEntityEnchantDecomposer extends InventoriedRelayPowered
    ...
    public int getProcessTime() {
        ...
        HashMap<Enchantment, Integer> map
            = ReikaEnchantmentHelper.getEnchantments(inv[0]);
        int t = 0;
    ...
    public static int getChromaValue(ItemStack is) {
        ...
        for (Enchantment e : map.keySet()) {
            val += ReikaMathLibrary.roundDownToX(
                100, (int) (100 * Math.pow(getEnchantPowerValue(e), map.get(e)))
            );
        }
        return val;
    }
    ...
    public float getOperationFraction() {
        return !this.canProcess()
            ? 0
            : processTimer / (float) (Math.max(1, this.getProcessTime()));
    }

    @Override
    public OperationState getState() {
        return inv[0] != null && isItemValid(inv[0])
