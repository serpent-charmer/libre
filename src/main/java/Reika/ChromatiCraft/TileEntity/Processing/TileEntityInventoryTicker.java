    ...
    private void updateItems() {
        ...
        if (ep != null) {
            for (int i = 0; i < this.getSizeInventory(); i++) {
                ItemStack is = inv[i];
                if (is != null) {
                    for (int k = 0; k < ticks /* && energy.containsAtLeast(required)*/;
                         k++) {
                        try {
                            is.getItem().onUpdate(
                                is, worldObj, ep, hotbar ? 0 : 9, hotbar
                            );
                        } catch (Exception e) {
                            ChromatiCraft.logger.logError(
                                "An item (" + is.getDisplayName()
                                + ") has thrown an exception when being ticked in " + this
                                + ": " + e
                            );
