...
public class TileEntityGlowFire extends InventoriedChromaticBase
    implements LumenTile, InertIInv, ItemOnRightClick, BreakAction {
    ...
    public float isSmothered() {
        ...
        if (f <= 1 || f == Double.NaN || f == Double.POSITIVE_INFINITY
    ...
    public boolean craft() {
        ...
        if (ep != null && !ReikaPlayerAPI.isFake(ep)
            && Chromabilities.DOUBLECRAFT.enabledOn(ep))
            out.stackSize *= 2;
        inv[0] = null;
        boolean flag = true;
        boolean flag2 = false;
    ...
    private double getValueRatio() {
        if (averageOutputValue.getAverage() == 0)
            return 1;
        return (averageOutputValue.getAverage() - temperatureBoost)
            / averageIngredientValue.getAverage();
    }
    ...
    private boolean craftAndDrop(ItemStack in) {
        ...
        for (int i = 0; i < 16; i++) {
            if (!flags[i]) {
                ...
                if (e.isPrimary()) {
                    //ReikaJavaLibrary.pConsole("Insufficient "+e+", have
                    //"+energy.getValue(e)+", need "+cost.getValue(e));
                    return false;
                }
    ...
    public static ElementTagCompound getCompositionCost(CrystalElement e, int amt) {
        ...
        while (!tag.isPrimaryOnly()) {
            ...
            while (it.hasNext()) {
                ...
                if (in.isPrimary()) {
                    ...
                    Collection<CrystalElement> parents2
                        = ElementMixer.instance.getMixParents(in);
        ...
        for (CrystalElement in : tag.elementSet()) {
            tag.setTag(
                in, MathHelper.ceiling_double_int(Math.pow(amt, -0.75 + tag.getValue(in)))
            );
        }
        return tag;
    }
            ...
            if (this.isConsumableItem(ei)) {
                ...
                if (tag != null) {
                    ...
                    ReikaPacketHelper.sendDataPacketWithRadius(
                        ...
                        ChromaPackets.FIRECONSUMEITEM.ordinal(),
                        this,
                        32,
                        tag.keySetAsBits()
                    );
    ...
    private boolean isConsumableItem(EntityItem ei) {
        if (ei.getEntityData().getBoolean(DROP_TAG)
            || ei.getEntityData().hasKey(ReikaItemHelper.PLAYER_DEATH_DROP_KEY))
            return false;
    ...
    public static ElementTagCompound getCost(ItemStack is) {
        ...
        ElementTagCompound tag
            = ItemElementCalculator.instance.getValueForItem(is).copy();
        //if (tag == null)
        //	tag =
        //FabricationRecipes.recipes().getItemRecipe(is).getCost().copy().scale(1F/FabricationRecipes.FACTOR).power(1F/FabricationRecipes.POWER).scale(1F/FabricationRecipes.INITFACTOR);
        if (tag != null)
    ...
    private static ElementTagCompound scaleCostTag(ItemStack is, ElementTagCompound tag) {
        ...
        if (is.getItem() == Items.dye || ChromaItems.DYE.matchWith(is)
            || ReikaItemHelper.isOreNugget(is))
            tag.scale(0.25F);
        if (sc)
            tag.scale(1.2F);
    ...
    scaleDecompositionTag(ItemStack is, ElementTagCompound tag) {
        if (is.getItem() != Items.dye && !ChromaItems.DYE.matchWith(is)
            && !ReikaItemHelper.isOreNugget(is))
            tag.scale(BASE_ITEM_FACTOR);
        ...
        for (int i = 0; i < 16; i++) {
            ...
            if ((colors & (1 << e.ordinal())) != 0) {
                ...
                for (int k = 0; k < n; k++) {
                    ...
                    EntityFX fx = new EntityCCBlurFX(world, rx, ry, rz)
                                      .setColor(e.getColor())
                                      .setLife(l)
                                      .setScale(s)
                                      .setRapidExpand();
    ...
    public void empty() {
        if (!worldObj.isRemote) {
            ...
            ReikaPacketHelper.sendDataPacketWithRadius(
                ChromatiCraft.packetChannel,
                ChromaPackets.FIREDUMP.ordinal(),
                this,
                64,
                energy.keySetAsBits()
            );
            ...
            for (EntityPlayer ep : li) {
                ...
                for (CrystalElement e : energy.elementSet()) {
                    TickScheduler.instance.scheduleEvent(
                        new ScheduledTickEvent(new GlowFireDischarge(
                            this,
                            ep,
                            e,
                            energy.getValue(e) / (float) this.getMaxStorage(e)
                        )),
                        tick
                    );
    ...
    public static void emptyClientFX(World world, int x, int y, int z, int colors) {
        ...
        for (int i = 0; i < n; i++) {
            EntityFX fx = new EntityCCFloatingSeedsFX(
                              ...
                              z + 0.5,
                              rand.nextDouble() * 360,
                              ReikaRandomHelper.getRandomPlusMinus(0, 10D)
            )
                              .setColor(li.get(rand.nextInt(li.size())))
                              .setColliding();
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
        }
    }
    ...
    private void doParticles(World world, int x, int y, int z) {
        ...
        if (rand.nextInt(200) == 0) {
            primaryAngleThetaVel = ReikaRandomHelper.getRandomPlusMinus(
                0, 3
            ); //ReikaRandomHelper.getRandomPlusMinus(0, 90);
            primaryAnglePhiVel
                = ReikaRandomHelper.getRandomPlusMinus(0, 3); //rand.nextDouble()*360;
        }
    ...
    private void addParticle(
        ...
        int c = ReikaColorAPI.getModifiedHue(
            0x1070ff, (int) (215 + 70 * Math.sin(timer / 40D))
        );
        float f = this.isSmothered();
        if (f > 0) {
            int c2 = ReikaColorAPI.getModifiedHue(c, rand.nextInt(60));
            c2 = ReikaColorAPI.getModifiedSat(c2, 0.5F);
            c2 = ReikaColorAPI.getColorWithBrightnessMultiplier(
                c2, 0.5F + 0.25F * rand.nextFloat()
            );
            c = ReikaColorAPI.mixColors(c2, c, f);
        }
        EntityBlurFX fx
            = new EntityCCBlurFX(world, x + 0.5, y + 0.5, z + 0.5, v[0], v[1], v[2])
                  .setColor(c)
                  .setScale(s)
                  .setLife(l);
        if (primary) {
        ...
        if (rand.nextBoolean()) {
            ...
            if (GuiScreen.isCtrlKeyDown())
                ...
                particles.addEffectWithVelocity(
                    0.5,
                    0.5,
                    0.5,
                    v[0],
    ...
    private void retargetSecondary(int i) {
        secondaryAngleThetaVel[i] = ReikaRandomHelper.getRandomPlusMinus(
            0, 2
        ); //ReikaRandomHelper.getRandomPlusMinus(0, 90);
        secondaryAnglePhiVel[i]
            = ReikaRandomHelper.getRandomPlusMinus(0, 2); //rand.nextDouble()*360;
    }
    ...
    private void dropItem(ItemStack is) {
        EntityItem ei = ReikaItemHelper.dropItem(
            worldObj, xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, is
        );
        ei.getEntityData().setBoolean(DROP_TAG, true);
        worldObj.playSound(
            xCoord + 0.5, yCoord + 0.5, zCoord + 0.5, "random.pop", 1, 0.8F, true
        );
    }
    ...
    private static void dischargeIntoPlayer(
        TileEntityGlowFire tile, EntityPlayer player, CrystalElement color, float power
    ) {
        ChromaAux.dischargeIntoPlayer(
            tile.xCoord + 0.5,
            /*tile.yCoord+0.125*/ player.posY,
            tile.zCoord + 0.5,
            rand,
            player,
            color,
    ...
    private static class GlowFireDischarge implements DelayableSchedulableEvent {
        ...
        public GlowFireDischarge(
            TileEntityGlowFire te, EntityPlayer ep, CrystalElement e, float amt
        ) {
        ...
        public boolean canTick() {
            return player != null && !ReikaPlayerAPI.isFake(player)
                && player.worldObj != null
                && ReikaPlayerAPI.getPlayerByNameAnyWorld(player.getCommandSenderName())
                != null;
        }
    }
}
