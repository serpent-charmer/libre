...
public class TileEntityCrystalFurnace extends InventoriedRelayPowered
    implements IFluidHandler, IPipeConnection, OperationInterval, VariableTexture {
    ...
    private boolean canSmelt() {
        ...
        if (!ReikaItemHelper.areStacksCombinable(
                out, inv[1], this.getInventoryStackLimit()
            ))
            return false;
        return true;
    }
    ...
    public static int getMultiplyRate(ItemStack in, ItemStack out) {
        if (in.getItem() instanceof CrystalFurnaceMultiplier) {
            int ret = ((CrystalFurnaceMultiplier) in.getItem())
                          .getMultiplyRateAsInput(in, out);
            if (ret > 0)
                return ret;
        }
        if (out.getItem() instanceof CrystalFurnaceMultiplier) {
            int ret = ((CrystalFurnaceMultiplier) out.getItem())
                          .getMultiplyRateAsOutput(out, in);
            if (ret > 0)
                return ret;
        }
    ...
    private static int calcMultiplyRate(ItemStack in, ItemStack out) {
        if (ChromaBlocks.PYLONSTRUCT.match(out)) {
            ...
            //ReikaItemHelper.matchStacks(out,
            //RockTypes.getTypeFromID(Block.getBlockFromItem(in.getItem())).getItem(RockShapes.SMOOTH)))
            return 1;
        }
    ...
    public float getOperationFraction() {
        return !this.canSmelt() ? 0
                                : smeltTimer / (float) (Math.max(1, this.getSmeltTime()));
    }

    @Override
    public OperationState getState() {
        return inv[0] != null
                && FurnaceRecipes.smelting().getSmeltingResult(inv[0]) != null
            ? (energy.containsAtLeast(smelt) ? OperationState.RUNNING
