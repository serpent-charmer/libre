...
public class TileEntitySpawnerReprogrammer
    extends InventoriedRelayPowered implements OperationInterval {
    ...
    private static void addDisallowedMob(String name) {
        //addDisallowedMob((Class<? extends
        //EntityLiving>)EntityList.stringToClassMapping.get(name));
        disallowedMobNames.add(name);
    }

    public static boolean isMobAllowed(Class mob) {
        return !mob.isInterface() && (mob.getModifiers() & Modifier.ABSTRACT) == 0
            && !TameHostile.class.isAssignableFrom(mob)
            && EntityLiving.class.isAssignableFrom(mob) && !isBlacklisted(mob);
    }
    ...
    private static boolean isBlacklisted(Class<? extends EntityLiving> mob) {
        ...
        if (allowance == null) {
            ...
            ChromatiCraft.logger.log(
                "Computed spawner controller rule for " + mob + ": " + allowance
            );
    ...
    public int[] getData() {
        return new int[] { minDelay,   maxDelay,   maxNear,
                           spawnCount, spawnRange, activeRange };
    }
    ...
    private boolean canConvert() {
        return energy.containsAtLeast(required) && this.isValidSpawner(inv[0])
            && inv[1] == null;
    }
    ...
    private void programSpawner() {
        ...
        if (ReikaItemHelper.matchStackWithBlock(is, Blocks.mob_spawner)) {
            ...
            ReikaSpawnerHelper.setSpawnerItemNBT(
                is, minDelay, maxDelay, maxNear, spawnCount, spawnRange, activeRange, true
            );
            ReikaSpawnerHelper.setSpawnerItemNBT(is, selectedMob, true);
        } else if (is.getItem() instanceof ProgrammableSpawner) {
            ((ProgrammableSpawner) is.getItem())
                .setSpawnerType(is, this.getMobClass(selectedMob));
            ((ProgrammableSpawner) is.getItem())
                .setSpawnerData(
                    is, minDelay, maxDelay, maxNear, spawnCount, spawnRange, activeRange
                );
        }
        inv[1] = is;
    ...
    private boolean isValidSpawner(ItemStack is) {
        if (is == null)
            return false;
        return ReikaItemHelper.matchStackWithBlock(is, Blocks.mob_spawner)
            || is.getItem() instanceof ItemSpawner
            || is.getItem() instanceof ProgrammableSpawner;
    }
    ...
    public OperationState getState() {
        return this.isValidSpawner(inv[0])
            ? (energy.containsAtLeast(required) ? OperationState.RUNNING
