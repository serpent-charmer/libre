...
public class TileEntityProgressionLinker
    extends TileEntityChromaticBase implements OwnedTile, MultiBlockChromaTile {
    ...
    public void validateStructure() {
        ...
        hasStructure
            = ChromaStructures.PROGRESSLINK.getArray(worldObj, xCoord, yCoord, zCoord)
                  .matchInWorld();
    }
    ...
    public void updateEntity(World world, int x, int y, int z, int meta) {
        ...
        if (this.hasStructure() && this.hasPlayer()) {
            ...
            if (ep == null
                || !ep.boundingBox.intersectsWith(
                    ReikaAABBHelper.getBlockAABB(this).addCoord(0, 4, 0)
                )) {
                linkProgress = 0;
                targetPlayer = null;
            } else {
    ...
    private void linkPlayers(EntityPlayer ep, EntityPlayer target) {
        ...
        if (lf == null) {
            for (int i = 0; i <= 100; i += 5) {
                //ChromaSounds.LOREHEX.playSoundAtBlock(this, 1, 0.5F);
                ScheduledSoundEvent evt
                    = new ScheduledSoundEvent(ChromaSounds.LOREHEX, ep, 1, 2F);
            ...
            ReikaPacketHelper.sendNBTPacket(
                ...
                ChromaPackets.LINKFAIL.ordinal(),
                tag,
