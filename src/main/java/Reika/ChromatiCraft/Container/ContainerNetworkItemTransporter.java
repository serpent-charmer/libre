...
public class ContainerNetworkItemTransporter extends CoreContainer {
    ...
    public ContainerNetworkItemTransporter(
        EntityPlayer player, TileEntityNetworkItemTransporter te
    ) {
        super(player, te);
        tile = te;
        ...
        if (show != showingFilters || inventorySlots.isEmpty()) {
            ...
            for (int j = 0; j < 9; j++) {
                ...
                for (int i = 0; i < 3; i++) {
                    int x = dx + 26 + j * 18;
                    int y = dy + i * 18;
                    Slot s = show ? new SlotNoClick(tile, idx, x, y, false, false)
                                  : new Slot(tile, idx, x, y);
            ...
            if (ep.worldObj.isRemote)
                ReikaPacketHelper.sendDataPacket(
                    ...
                    ChromaPackets.NETWORKITEMMODE.ordinal(),
                    PacketTarget.server,
                    showingFilters ? 1 : 0
                );
        }
