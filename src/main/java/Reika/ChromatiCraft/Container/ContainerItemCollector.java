    ...
    public ContainerItemCollector(EntityPlayer player, TileEntityItemCollector te) {
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 9; j++) {
                this.addSlotToContainer(
                    new Slot(te, i * 9 + j, dx + 26 + j * 18, dy + i * 18)
                );
            }
        }
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 9; j++) {
                ...
                //dy+i*18));
                this.addSlotToContainer(
                    new GhostSlot(te, 27 + i * 9 + j, dx + 26 + j * 18, dy + i * 18 + 9)
                );
            }
        }
    ...
    public void stepOffset(EntityPlayer ep, int d) {
        ...
        if (tile.worldObj.isRemote) {
            ReikaPacketHelper.sendDataPacket(
                ...
                ChromaPackets.COLLECTORROW.ordinal(),
                PacketTarget.server,
                rowOffset
            );
        } else {
    ...
    public ItemStack slotClick(int slot, int button, int action, EntityPlayer ep) {
        ...
        if (slot >= tile.getSizeInventory() && slot < tile.getSizeInventory() + 2 * 9) {
            ...
            if (button == 1) { //right click
                ...
                tile.setMapping(
                    slot + off - tile.getSizeInventory(),
                    ReikaItemHelper.getSizedItemStack(held, 1)
                );
            }
