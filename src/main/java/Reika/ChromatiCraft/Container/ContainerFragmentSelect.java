        ...
        for (int i = 0; i < 3; ++i)
            for (int k = 0; k < 9; ++k)
                this.addSlotToContainer(
                    new Slot(player.inventory, k + i * 9 + 9, 9 + k * 18, 77 + i * 18)
                );
        for (int i = 0; i < 9; ++i)
    ...
    public void detectAndSendChanges() {
        ...
        if (!player.worldObj.isRemote) {
            ...
            for (int i = 0; i < crafters.size(); ++i) {
                ...
                for (int k = 0; k < 3; k++) {
                    //icrafting.sendProgressBarUpdate(this, k*2+1, options[k] != null ?
                    //options[k].category.ordinal() : -1);
                    //icrafting.sendProgressBarUpdate(this, k*2+2, options[k] != null ?
                    //options[k].fragment.ordinal() : -1);
                    int bits = options[k] != null ? options[k].fragment.ordinal()
                            | (options[k].category.ordinal() << 11)
                                                  : -1;
                    //if (options[k] != null)
                    //	ReikaJavaLibrary.pConsole(options[k].fragment+"@"+options[k].fragment.ordinal()+"
                    //> "+bits+"="+Integer.toBinaryString(bits));
    ...
    public void updateProgressBar(int id, int val) {
        if (id == 0) {
            ...
            if (val < 0) {
                ...
                int r = val & 2047;
                int f = (val >> 11) & 31;
                options[slot] = new Selection(
                    FragmentCategory.list[f], ChromaResearch.researchList[r]
                );
            }
        } else if (id == 20) {
    ...
    public boolean selectSlot(int slot) {
        if (player.worldObj.isRemote) {
            ReikaPacketHelper.sendPacketToServer(
                ChromatiCraft.packetChannel, ChromaPackets.FRAGSELECT.ordinal(), slot
            );
            return true;
        } else {
            ItemStack in = inventory.getStackInSlot(0);
            if (ChromaItems.FRAGMENT.matchWith(in)
                && options[slot].giveToPlayer(player)) {
    ...
    private static class SlotFragmentSelect extends Slot {
        ...
        public boolean isItemValid(ItemStack is) {
            return ChromaItems.FRAGMENT.matchWith(is)
                && ItemInfoFragment.getResearch(is) == null;
        }
