...
public class ContainerAuraPouch extends Container {
    ...
    public ContainerAuraPouch(EntityPlayer ep) {
        ...
        for (int i = 0; i < h; ++i)
            for (int k = 0; k < w; ++k)
                this.addSlotToContainer(
                    new Slot(inventory, k + i * w, 8 + k * 18, 17 + i * 18)
                );

        for (int i = 0; i < 3; ++i)
            for (int k = 0; k < 9; ++k)
                this.addSlotToContainer(
                    new Slot(player.inventory, k + i * 9 + 9, 8 + k * 18, 84 + i * 18)
                );
        for (int i = 0; i < 9; ++i)
        ...
        if (!ChromaItems.AURAPOUCH.matchWith(tool)) {
            ChromatiCraft.logger.logError(
                "Opened an Aura Pouch GUI without holding an aura pouch!?"
            );
            return;
        }
    ...
    public ItemStack slotClick(int slot, int button, int par3, EntityPlayer ep) {
        ...
        if (inGUI) {
            ...
            if (button == 1) {
                ...
                if (player instanceof EntityPlayerMP)
                    ReikaPacketHelper.sendDataPacket(
                        ...
                        ChromaPackets.AURAPOUCH.ordinal(),
            ...
            if (ChromaItems.AURAPOUCH.matchWith(in
                ) /* && !ChromaOptions.RECURSIVEPOUCH.getState()*/) { //introduces ability
                                                                      //to move item and
                                                                      //close gui
                return ep.inventory.getItemStack();
            }
    ...
    public void onContainerClosed(EntityPlayer ep) {
        ...
        if (is == null || !(is.getItem() instanceof ItemAuraPouch)) {
            ChromatiCraft.logger.logError(
                "Tried to access aura pouch inventory without a pouch!"
            );
            return;
        }
        ItemAuraPouch iil = (ItemAuraPouch) is.getItem();
