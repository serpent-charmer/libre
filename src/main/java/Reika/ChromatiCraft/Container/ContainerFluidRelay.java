...
public class ContainerFluidRelay extends CoreContainer {
    ...
    public void onCharTyped(char c) {
        if (manualSelectionSlot >= 0) {
            if (manualSelectionChar == c) {
                manualSelectionIndex = manualOptions.isEmpty() ? 0 :
    (manualSelectionIndex+1)%manualOptions.size();
            }
            else {
                manualSelectionChar = c;
                manualOptions.clear();
                manualOptions.addAll(allFluids);
                manualOptions.removeIf(f ->
    f.getName().toLowerCase(Locale.ENGLISH).charAt(0) != manualSelectionChar);
                manualSelectionIndex = 0;
            }
            Fluid f = manualOptions.isEmpty() ? null :
    manualOptions.get(manualSelectionIndex); if (relay.worldObj.isRemote)
                ReikaPacketHelper.sendDataPacket(ChromatiCraft.packetChannel,
    ChromaPackets.RELAYFLUIDKEY.ordinal(), relay.worldObj, relay.xCoord, relay.yCoord,
    relay.zCoord, PacketTarget.server, manualSelectionSlot, f == null ? -1 : f.getID());
            else
    ...
    public void setFluid(Fluid f) {
        if (relay.worldObj.isRemote)
            ReikaPacketHelper.sendDataPacket(
                ...
                ChromaPackets.RELAYFLUIDKEY.ordinal(),
                relay.worldObj,
                relay.xCoord,
                relay.yCoord,
                relay.zCoord,
    ...
    public ItemStack slotClick(int slot, int mouse, int action, EntityPlayer ep) {
        ...
        if (slot >= 0 && slot < 7) {
            ...
            if (mouse == 0) {
                FluidStack fs
                    = held != null ? ReikaFluidHelper.getFluidForItem(held) : null;
