...
public class ContainerBookPages extends Container {
    ...
    public static final int MAX_SCROLL
        = 1 + ChromaResearch.getAllObtainableFragments().size() / width - height;
    ...
    public void scroll(boolean up) {
        ...
        //mouseY = Mouse.getY();
        //player.closeScreen();
        //player.openGui(ChromatiCraft.instance, ChromaGuis.BOOKPAGES.ordinal(), null,
        //scroll, 0, 0);
        this.populate();
    }
    ...
    private static class BookInventory extends BasicInventory {
        ...
        private boolean containsFragment(ChromaResearch r) {
            for (int i = 0; i < inv.length; i++) {
                ItemStack is = inv[i];
                if (ChromaItems.FRAGMENT.matchWith(is)
                    && ItemInfoFragment.getResearch(is) == r)
                    return true;
            }
            return false;
        }
