...
public class BiomeRainbowForest extends BiomeGenBase implements CustomMapColorBiome {
    ...
    @Override
    public int
    getBiomeGrassColor(int x, int y, int z) { //use dyeGrass block, with metadata
        int original = BiomeGenBase.forest.getBiomeGrassColor(x, y, z);
        Color c = ReikaDyeHelper.dyes[(Math.abs(x / 16) + y + Math.abs(z / 16)) % 16]
                      .getJavaColor()
                      .brighter();
    ...
    public void plantFlower(World world, Random rand, int x, int y, int z) {
        if (rand.nextInt(8) > 0
            && ChromaBlocks.DYEFLOWER.getBlockInstance().canBlockStay(world, x, y, z)) {
    ...
    static {
        classes.add(new ModSpawnEntry(
            ModList.TWILIGHT, "twilightforest.entity.passive.EntityTFBighorn", 3, 1, 3
        ));
        classes.add(new ModSpawnEntry(
            ModList.TWILIGHT, "twilightforest.entity.passive.EntityTFBunny", 4, 1, 2
        ));
        classes.add(new ModSpawnEntry(
            ModList.TWILIGHT, "twilightforest.entity.passive.EntityTFDeer", 1, 1, 8
        ));
        classes.add(new ModSpawnEntry(
            ModList.TWILIGHT, "twilightforest.entity.passive.EntityTFRaven", 3, 1, 1
        ));
        classes.add(new ModSpawnEntry(
            ModList.TWILIGHT, "twilightforest.entity.passive.EntityTFSquirrel", 5, 1, 2
        ));
        classes.add(new ModSpawnEntry(
            ModList.TWILIGHT, "twilightforest.entity.passive.EntityTFTinyBird", 10, 1, 4
        ));

        caveClasses.add(new ModSpawnEntry(
            ModList.TWILIGHT,
        ...
        monsterClasses.add(new ModSpawnEntry(
            ModList.THAUMCRAFT, "thaumcraft.common.entities.monster.EntityWisp", 5, 1, 1
        ));
    }
}
