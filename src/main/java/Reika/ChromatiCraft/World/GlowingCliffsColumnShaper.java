...
public class GlowingCliffsColumnShaper extends TerrainShaper {
    ...
    private static final Interpolation EDGE_BLENDING = new Interpolation(false)
                                                           .addPoint(0, 1)
                                                           .addPoint(0.05, 0.925)
                                                           .addPoint(0.1, 0.85)
                                                           .addPoint(0.3, 0.8)
    ...
    private static final BlockKey BIOME_STONE
        = STONE; //new BlockKey(ChromaBlocks.CLIFFSTONE.getBlockInstance(),
                 //Variants.STONE.getMeta(false, false));
    private static final BlockKey BIOME_DIRT
        = DIRT; //new BlockKey(ChromaBlocks.CLIFFSTONE.getBlockInstance(),
                //Variants.DIRT.getMeta(false, false));
    private static final BlockKey BIOME_GRASS
        = GRASS; //new BlockKey(ChromaBlocks.CLIFFSTONE.getBlockInstance(),
                 //Variants.GRASS.getMeta(false, false));

    public GlowingCliffsColumnShaper(long seed) {
        seed += ChromaOptions.LUMCLIFFSEEDSHIFT.getValue();
        landmassControl = new SimplexNoiseGenerator(seed)
                              .setFrequency(1D /*/128D*/ /*/192D*/ /*/160D*/ / 144D)
                              .addOctave(2.5, 0.5)
                              .addOctave(6, 0.125);
        ...
        oceanDepth = new SimplexNoiseGenerator(ReikaMathLibrary.cycleBitsRight(seed, 16))
                         .setFrequency(1 / 32D);
        caveCeilNoise
            = new SimplexNoiseGenerator(ReikaMathLibrary.cycleBitsLeft(seed, 16))
                  .setFrequency(1 / 16D);

        upperPlateauTop
            = new SimplexNoiseGenerator(seed * 4 + 8192).setFrequency(1 / 24D);
        upperPlateauBottom
            = new SimplexNoiseGenerator(seed * 8 + 4096).setFrequency(1 / 24D);
        upperPlateauEdge
            = new SimplexNoiseGenerator(seed * 2 + 65536).setFrequency(1 / 24D);
        middlePlateauCaveDepth
            = new SimplexNoiseGenerator(seed * 16 + 32).setFrequency(1 / 24D);

        middlePlateauTop
            = new SimplexNoiseGenerator(-seed * 4 + 4096).setFrequency(1 / 32D);
        middlePlateauBottom
            = new SimplexNoiseGenerator(-seed * 8 + 65536).setFrequency(1 / 32D);
        middlePlateauEdge
            = new SimplexNoiseGenerator(-seed * 2 + 16384).setFrequency(1 / 32D);
        lowerPlateauCaveDepth
            = new SimplexNoiseGenerator(-seed * 16 + 32).setFrequency(1 / 32D);
    ...
    generateColumn(World world, int x, int z, Random rand, BiomeGenBase biome) {
        ...
        int dirtt = (int
        ) ReikaMathLibrary.normalizeToBounds(dirtThickness.getValue(x, z), 1, 4);
        double hval = this.calcHval(world, x, z, biome);
        GlowCliffRegion r = this.getRegion(world, x, z, biome);
        double middlethresh
            = r == GlowCliffRegion.WATER ? 0 : this.calcMiddleThresh(x, z);
        ...
        switch (r) {
            case WATER:
                ...
                break;
            case SHORES:
                this.generateLowPlateau(
                    world, x, z, biome, rand, middlethresh, hval, dirtt
                );
                break;
            case PLATEAU: {
                double cave = ReikaMathLibrary.normalizeToBounds(
                    lowerPlateauCaveDepth.getValue(x, z),
                    LOWER_CAVE_MIN_THRESHOLD,
                    LOWER_CAVE_MAX_THRESHOLD
                );
                this.generateMidPlateau(
                    world, x, z, biome, rand, hval < cave, middlethresh, cave, hval, dirtt
                );
                break;
            }
            case HIGH_PLATEAU: {
                double cave = ReikaMathLibrary.normalizeToBounds(
                    middlePlateauCaveDepth.getValue(x, z),
                    MIDDLE_CAVE_MIN_THRESHOLD,
                    MIDDLE_CAVE_MAX_THRESHOLD
                );
                this.generateUpperPlateau(
                    world, x, z, biome, rand, hval < cave, topthresh, cave, hval, dirtt
                );
                break;
            }
        }
     ...
     */
    public void
    blendEdge(World world, int x, int z, Block[] blockArray, byte[] metaArray) {
    ...
    private int calcTop(Block[] blockArray, int x, int z) {
        ...
        for (int y = 255; y >= 0; y--) {
            int idx = posIndex + y;
            if (blockArray[idx] != null && blockArray[idx] != Blocks.air
                && blockArray[idx].getMaterial() != Material.air
                && blockArray[idx] != Blocks.water
                && blockArray[idx].getMaterial() != Material.water)
                return y;
        }
        return 0;
    }
    ...
    private double calcTopThresh(int x, int z) {
        return ReikaMathLibrary.normalizeToBounds(
            upperPlateauEdge.getValue(x, z), UPPER_MIN_THRESHOLD, UPPER_MAX_THRESHOLD
        );
    }

    private double calcMiddleThresh(int x, int z) {
        return ReikaMathLibrary.normalizeToBounds(
            middlePlateauEdge.getValue(x, z), MIDDLE_MIN_THRESHOLD, MIDDLE_MAX_THRESHOLD
        );
    }

    private double calcHval(World world, int x, int z, BiomeGenBase b) {
        double hval = ReikaMathLibrary.normalizeToBounds(
            landmassControl.getValue(x, z), 0, this.getHvalLimit(world, x, z, b)
        );
        ...
        if (f != null) {
            //double dh =
            //SHORELINE_THRESHOLD+0.125*(MIDDLE_MIN_THRESHOLD-SHORELINE_THRESHOLD); hval =
            //(hval-dh)*f+dh;
            double dm = f.biome == ChromatiCraft.glowingcliffsEdge
                ? /*HVAL_LIMIT_EDGE*/ this
                      .calcHval(world, f.xCoord, f.zCoord, f.biome /*, hval*/)
                : this.getBlendedBiomeHeight(f);
            double dh = hval - dm;
            //double ds = 0.5;
        ...
        if (b == ChromatiCraft.glowingcliffsEdge) { //blending up from edge to main biome
            ...
            if (fe != null) {
                ...
                hval = f2*this.calcHval(world, c.xCoord, c.zCoord,
        ChromatiCraft.glowingcliffs, referenceValue)+(1-f2)*hval;
            }
        }
         */
    ...
    private double getBlendedBiomeHeight(BlendPoint f) {
        if (ChromaOptions.BIOMEBLEND.getState()) {
            if (this.isBiomeOceanic(f.biome))
                ...
                return MIDDLE_MAX_THRESHOLD
                    + 0.1; //this.calcMiddleThresh(f.xCoord, f.zCoord);
    ...
    private int
    getBlendedHeight(World world, int x, int y, int z, double f, Coordinate loc) {
        double f2 = EDGE_BLENDING.getValue(f);
        return (int
        ) (f2 * this.calcIntendedHeight(world, loc.xCoord, loc.zCoord) + (1 - f2) * y);
    }
    ...
    private Object[] getInvertedDistanceFactor(World world, int x, int z, int search) {
        ...
        return mind == Integer.MAX_VALUE
            ? null
            : new Object[] { mind / (double) search, result };
    }

    private BlendPoint
    getDistanceFactor(World world, int x, int z, int search, BiomeGenBase biome) {
        BlendPoint ret = null;
        int look = search;
    ...
    private void generateLowPlateau(
        World world,
        int x,
        int z,
        BiomeGenBase biome,
        ...
        this.generateLandColumn(
            world, x, z, biome, rand, dirt, top, Integer.MAX_VALUE, Integer.MIN_VALUE
        );
    }

    private int getLowPlateauHeight(double hval, double max, int x, int z) {
        return (int) ReikaMathLibrary.linterpolate(
            hval, SHORELINE_THRESHOLD, max, MIN_SHORE_Y, MAX_SHORE_Y
        );
    }

    private void generateMidPlateau(
        World world,
        int x,
        int z,
        BiomeGenBase biome,
        ...
        if (cave) {
            caveFloor = (int) ReikaMathLibrary.linterpolate(
                hval, SHORELINE_THRESHOLD, caveThresh, MIN_SHORE_Y, MAX_SHORE_Y
            );
            caveCeil = (int) ReikaMathLibrary.normalizeToBounds(
                middlePlateauBottom.getValue(x, z),
                MIN_MIDDLE_BOTTOM_Y,
                MAX_MIDDLE_BOTTOM_Y
            );
            ...
            double d =
            ReikaMathLibrary.normalizeToBounds(Math.abs(hval-LOWER_CAVE_MAX_THRESHOLD), 0,
            1, 0, LOWER_CAVE_MAX_THRESHOLD-LOWER_CAVE_MIN_THRESHOLD); caveCeil =
            caveFloor+(int)(d*dh);
             */

            double dh = caveCeil - caveFloor;
            dh *= ReikaMathLibrary.normalizeToBounds(
                caveCeilNoise.getValue(x, z), 0.8, 1.2
            );
            //double d = Math.abs(hval-caveThresh);
            //d = ReikaMathLibrary.normalizeToBounds(d, 0.125, 1, 0, caveThresh-thresh);
            //d = ReikaMathLibrary.ellipticalInterpolation(d, 0, caveThresh-thresh, 0.125,
            //1);
            caveCeil = caveFloor + 4
                + (int) ReikaMathLibrary.ellipticalInterpolation(
                    Math.abs(hval - thresh), 0, caveThresh - thresh, 0, dh
                );
            //ReikaJavaLibrary.pConsole("["+caveFloor+", "+baseCeil+"] ("+dh+") @ "+hval+"
            //of ["+thresh+", "+caveThresh+"] {"+Math.abs(hval-thresh)+",
            //"+(caveThresh-thresh)+"} > "+caveCeil); caveCeil = caveFloor+(int)(d*dh);
            ...
            top = baseCeil + 2
                + (int) ReikaMathLibrary.ellipticalInterpolation(
                    hval - caveThresh, 0, caveThresh - thresh, 0, dh
                );
    ...
    private int getMidPlateauHeight(int x, int z) {
        return (int) ReikaMathLibrary.normalizeToBounds(
            middlePlateauTop.getValue(x, z), MIN_MIDDLE_TOP_Y, MAX_MIDDLE_TOP_Y
        );
    }

    private void generateUpperPlateau(
        World world,
        int x,
        int z,
        BiomeGenBase biome,
        ...
        int caveCeil = Integer.MIN_VALUE;
        if (cave) {
            caveFloor = (int) ReikaMathLibrary.normalizeToBounds(
                middlePlateauTop.getValue(x, z), MIN_MIDDLE_TOP_Y, MAX_MIDDLE_TOP_Y
            );
            caveCeil = (int) ReikaMathLibrary.normalizeToBounds(
                upperPlateauBottom.getValue(x, z), MIN_UPPER_BOTTOM_Y, MAX_UPPER_BOTTOM_Y
            );
            ...
            //d = ReikaMathLibrary.normalizeToBounds(d, 0.125, 1, 0,
            ///*MIDDLE_CAVE_MAX_THRESHOLD*/caveThresh-/*MIDDLE_CAVE_MIN_THRESHOLD*/thresh);
            caveCeil = caveFloor + 4
                + (int) ReikaMathLibrary.ellipticalInterpolation(
                    Math.abs(hval - thresh), 0, caveThresh - thresh, 0, dh
                );
            //double d = ReikaMathLibrary.linterpolate(hval, MIDDLE_CAVE_MIN_THRESHOLD,
            //MIDDLE_CAVE_MAX_THRESHOLD, 0, 1); caveCeil = caveFloor+(int)(d*dh);

            //double d = MIDDLE_CAVE_MAX_THRESHOLD-hval;
            //caveCeil = (int)ReikaMathLibrary.linterpolate(d, MIDDLE_CAVE_MIN_THRESHOLD,
            //MIDDLE_CAVE_MAX_THRESHOLD, caveFloor, caveCeil);
    ...
    private int getTopPlateauHeight(int x, int z) {
        return (int) ReikaMathLibrary.normalizeToBounds(
            upperPlateauTop.getValue(x, z), MIN_UPPER_TOP_Y, MAX_UPPER_TOP_Y
        );
    }

    private void generateWater(
        World world, int x, int z, BiomeGenBase biome, Random rand, double hval, int dirtt
    ) {
        ...
        for (int i = 0; i < floor; i++) {
            this.setBlock(
                x, i, z, stone.blockID, stone.hasMetadata() ? stone.metadata : 0
            );
        }
        ...
        for (int h = 1; h <= dirtt; h++) {
            this.setBlock(
                x, floor - h, z, dirt.blockID, dirt.hasMetadata() ? dirt.metadata : 0
            );
        }
    }
    ...
    private int getOceanFloor(int x, int z, double hval) {
        int min = (int) ReikaMathLibrary.normalizeToBounds(
            oceanDepth.getValue(x, z), MIN_OCEAN_FLOOR_Y, MAX_OCEAN_FLOOR_Y
        );
        int floor = (int
        ) ReikaMathLibrary.linterpolate(hval, 0, SHORELINE_THRESHOLD, min, SEA_LEVEL + 1);
        return floor;
    }
        ...
        if (caveFloor > 0 && caveFloor < 256) {
            ...
            if (ReikaRandomHelper.doWithChance(0.003)) {
                this.setBlock(
                    ...
                    caveCeil - 1,
                    z,
                    ChromaBlocks.TIEREDPLANT.getBlockInstance(),
                    TieredPlants.CAVE.ordinal()
                );
            } else if (ReikaRandomHelper.doWithChance(0.008)) {
                int nt = Math.min(
                    ReikaRandomHelper.getRandomBetween(1, 4), caveCeil - caveFloor - 3
                );
                for (int i = 1; i <= nt; i++)
                    this.setBlock(
                        x,
    ...
    protected boolean isPlant(Block b) {
        return b == ChromaBlocks.DECOFLOWER.getBlockInstance()
            || b == ChromaBlocks.TIEREDPLANT.getBlockInstance() || b == Blocks.sapling;
    }
