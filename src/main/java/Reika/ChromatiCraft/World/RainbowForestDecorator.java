...
public class RainbowForestDecorator extends StackableBiomeDecorator {
            ...
            for (int i = 0; i < 16; i++) {
                for (int j = 0; j < 16; j++) {
                    ...
                    if (id == Blocks.grass.blockID) {
                        //currentWorld.setBlock(x, y-1, z, ChromaBlocks.GRASS.getBlock(),
        gen.getColor(x, y, z).ordinal(), 3);
                    }
                }
            }
        }*/
    ...
    private void generateUndergroundLight() {
        if (randomGenerator.nextInt(2) == 0) {
            ...
            BlockTypes type
                = BlockTypes
                      .list[randomGenerator.nextInt(BlockSparkle.BlockTypes.list.length)];
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (this.canGenerateFlower(x, y, z)) {
                if (y < 128 && randomGenerator.nextInt(1 + (128 - y) / 16) > 0)
                    if (ChromaBlocks.DYEFLOWER.getBlockInstance().canBlockStay(
                            currentWorld, x, y, z
                        ))
                        currentWorld.setBlock(
                            x,
    ...
    private boolean canGenerateFlower(int x, int y, int z) {
        ...
        if (b instanceof BlockLiquid || b instanceof BlockFluidBase)
            return false;
        return ReikaPlantHelper.FLOWER.canPlantAt(currentWorld, x, y, z)
            && ReikaWorldHelper.softBlocks(currentWorld, x, y, z);
    }
    ...
    private void auxDeco(BiomeGenBase biome) {
        ...
        int k;
        boolean doGen
            = TerrainGen.decorate(currentWorld, randomGenerator, chunk_X, chunk_Z, SAND);
        for (i = 0; doGen && i < sandPerChunk2; ++i) {
            ...
            sandGen.generate(
                ...
                randomGenerator,
                j,
                currentWorld.getTopSolidOrLiquidBlock(j, k),
                k
            );
        }

        doGen
            = TerrainGen.decorate(currentWorld, randomGenerator, chunk_X, chunk_Z, CLAY);
        ...
        doGen = TerrainGen.decorate(
            currentWorld, randomGenerator, chunk_X, chunk_Z, SAND_PASS2
        );
        ...
        int l;
        doGen = TerrainGen.decorate(
            currentWorld, randomGenerator, chunk_X, chunk_Z, FLOWERS
        );
        ...
        doGen
            = TerrainGen.decorate(currentWorld, randomGenerator, chunk_X, chunk_Z, GRASS);
        for (j = 0; doGen && j < grassPerChunk; ++j) {
            k = chunk_X + randomGenerator.nextInt(16) + 8;
            l = randomGenerator.nextInt(128);
            i1 = chunk_Z + randomGenerator.nextInt(16) + 8;
            WorldGenerator worldgenerator1
                = biome.getRandomWorldGenForGrass(randomGenerator);
            worldgenerator1.generate(currentWorld, randomGenerator, k, l, i1);
        }

        doGen
            = TerrainGen.decorate(currentWorld, randomGenerator, chunk_X, chunk_Z, REED);
        ...
        doGen
            = TerrainGen.decorate(currentWorld, randomGenerator, chunk_X, chunk_Z, LAKE);
        if (doGen && generateLakes) {
            for (j = 0; j < 50; ++j) {
                ...
                (new WorldGenLiquids(Blocks.flowing_water))
                    .generate(currentWorld, randomGenerator, k, l, i1);
            }

            if (this.generateLavaLakes()) {
                for (j = 0; j < 20; ++j) {
                    k = chunk_X + randomGenerator.nextInt(16) + 8;
                    l = randomGenerator.nextInt(
                        randomGenerator.nextInt(randomGenerator.nextInt(112) + 8) + 8
                    );
                    i1 = chunk_Z + randomGenerator.nextInt(16) + 8;
