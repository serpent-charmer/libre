...
public class WorldGenGlowCave extends ChromaWorldGenerator {
    ...
    public WorldGenGlowCave(DimensionGenerators g, Random rand, long seed) {
        super(g, rand, seed);
        wallSelectionNoise
            = new SimplexNoiseGenerator(seed).setFrequency(1 / 3D).addOctave(2, 0.5);
    }

    @Override
    public float
    getGenerationChance(World world, int cx, int cz, ChromaDimensionBiome biome) {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (bk == Blocks.grass || bk == Blocks.sand) {
            ...
            for (Coordinate c : set1) {
                if (c.getBlock(world).getMaterial() == Material.water
                    || ReikaWorldHelper.checkForAdjMaterial(
                           world, c.xCoord, c.yCoord, c.zCoord, Material.water
                       ) != null
                    || ReikaWorldHelper.checkForAdjBlock(
                           world,
                           c.xCoord,
                           c.yCoord,
                           c.zCoord,
                ...
                for (int k = -r; k <= r; k++) {
                    ...
                    if (d <= r) {
                        ...
                        if (rand.nextDouble() < f) {
                            int dy = ReikaWorldHelper.findTopBlockBelowY(
                                world, dx, oy + 20, dz
                            );
                            ...
                            if (b == Blocks.grass || b == Blocks.sand)
                                world.setBlock(
                                    ...
                                    dz,
                                    ChromaBlocks.STRUCTSHIELD.getBlockInstance(),
                                    rand.nextBoolean() ? BlockType.MOSS.metadata
    ...
    private void growFrom(
        ...
        rVar.addPoint(
            li.size() - 1, ReikaRandomHelper.getRandomBetween(MIN_RADIUS, maxr)
        );
        for (int i = 0; i < li.size() - 1; i++) {
            ...
            if (this.getLine(
                ...
                && forkDepth > 0 && i > 10) //prevent overtangling
                break;
    ...
    private void fork(
        ...
        this.growFrom(
            world,
            rand,
            p1.xCoord + (p2.xCoord - p1.xCoord) * f,
            p1.yCoord + (p2.yCoord - p1.yCoord) * f,
            p1.zCoord + (p2.zCoord - p1.zCoord) * f,
            rVar.getValue(idx + f),
            set,
            ret,
            depth + 1,
            rand.nextInt(3) == 0 ? 0 : ReikaRandomHelper.getRandomBetween(6, 20)
        );
        set.addAll(ret);
    }
    ...
    private void generate(World world, Random rand, HashSet<Coordinate> set, int upper) {
        ...
        for (Coordinate c : set) {
            ...
            boolean edge
                = c.yCoord <= upper && !set.containsAll(c.getAdjacentCoordinates());
            Block b = Blocks.air;
            int meta = 0;
            if (!top) {
                if (edge) {
                    if (this.isBedrockWall(world, rand, c)) {
                        ...
                        if (rand.nextInt(20) == 0) {
                            ...
                            meta = ReikaRandomHelper.doWithChance(20)
            ...
            if (b == Blocks.air
                && rand.nextInt(c.yCoord >= 16 ? 160 : 10 + 10 * c.yCoord) == 0) {
                b = ChromaBlocks.LIGHT.getBlockInstance();
                meta = c.yCoord > 12 ? Flags.PARTICLES.getFlag() : 0;
            }
            if (b == Blocks.air && rand.nextInt(60) == 0 && c.yCoord < upper - 10
                && !set.contains(c.offset(0, 2, 0))) {
            ...
            if (b == Blocks.stone) {
                if (rand.nextInt(3) == 0) {
                    OreType ore =
            GlowingCliffsAuxGenerator.instance.oreRand.getRandomEntry(); ItemStack is =
            ore.getFirstOreBlock(); b = Block.getBlockFromItem(is.getItem()); meta =
            is.getItemDamage();
                }
    ...
    private boolean isBedrockWall(World world, Random rand, Coordinate c) {
        ...
        if (c.yCoord <= FULL_BEDROCK_Y)
            return true;
        return c.yCoord <= ReikaMathLibrary.normalizeToBounds(
                   wallSelectionNoise.getValue(c.xCoord, c.zCoord),
                   FULL_BEDROCK_Y,
                   MAX_BEDROCK_Y
               );
    }
