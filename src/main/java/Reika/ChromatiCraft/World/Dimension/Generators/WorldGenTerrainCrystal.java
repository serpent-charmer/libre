...
public class WorldGenTerrainCrystal extends ChromaWorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            t.hasResource
                = !t.hasTreasure && !t.hasCore && !t.hasPool && rand.nextInt(3) == 0;

            if (rand.nextInt(4) == 0)
                t.heightExponent = ReikaRandomHelper.getRandomBetween(
                    0.25, rand.nextBoolean() ? 1.5 : 2D
                ); //0.5 is a hemispherical bottom; 2 is pointed

            if (rand.nextInt(8) == 0) {
                t.stone = new BlockKey(
            ...
            if (b.isAtLeastXPercent(world, 90, Blocks.air)) {
                b.place();
                if (t.hasTreasure) {
                    TileEntityLootChest te = (TileEntityLootChest
                    ) world.getTileEntity(dx, dy - t.size / 2, dz);
    ...
    private static class TerrainCrystal {
        ...
        public FilledBlockArray generate(World world, int x, int y, int z) {
            ...
            if (hasTreasure) {
                arr.setBlock(
                    x, y - size / 2, z, ChromaBlocks.LOOTCHEST.getBlockInstance()
                );
            }
