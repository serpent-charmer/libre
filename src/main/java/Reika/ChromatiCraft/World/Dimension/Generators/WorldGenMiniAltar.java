...
public class WorldGenMiniAltar extends ChromaWorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (this.canGenerate(world, x, y, z)) {
            ...
            for (int i = -r; i <= r; i++) {
                for (int k = -r; k <= r; k++) {
                    int m = (i == 0 && k == 0) ? BlockType.LIGHT.metadata % 8
                                               : BlockType.STONE.metadata % 8;
    ...
    private void generateArches(World world, int x, int y, int z, Random rand, Block bk) {
        ...
        world.setBlock(
            x, y + t + 1, z, ChromaBlocks.LAMP.getBlockInstance(), rand.nextInt(16), 3
        );
    }
            ...
            for (int a = -2; a <= 2; a++) {
                for (int b = -2; b <= 2; b++) {
                    ...
                    world.setBlock(
                        ...
                        z + b,
                        wall ? bk : Blocks.air,
                        wall ? BlockType.STONE.metadata % 8 : 0,
                        3
                    );
                }
            }
    ...
    private void generateChest(World world, int x, int y, int z, int yc, Random rand) {
        world.setBlock(
            x, y - yc, z, ChromaBlocks.LOOTCHEST.getBlockInstance(), rand.nextInt(4), 3
        );
    ...
    private boolean canGenerate(World world, int x, int y, int z) {
        ...
        for (int i = -r; i <= 3; i++) {
            for (int k = -r; k <= 3; k++) {
                if (world.getBlock(x + i, y, z + k) != Blocks.grass
                    || world.getBlock(x + i, y + 1, z + k) != Blocks.air) {
                    //ReikaJavaLibrary.pConsole(world.getBlock(x+i, y,
                    //z+k)+":"+world.getBlock(x+i, y+1, z+k));
                    return false;
                }
            }
        }
