...
public class WorldGenMiasma extends ChromaWorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        boolean isVoid
            = BiomeDistributor.getBiome(x, z).getExactType() == SubBiomes.VOIDLANDS;

        int ymin = 0;
        int ymax = 0;
        if (!isVoid) {
            for (int i = TerrainGenSkylandCanyons.LOW_FLOOR - 2;
                 i <= TerrainGenSkylandCanyons.HIGH_FLOOR + 2;
                 i++) {
        ...
        for (int i = TerrainGenSkylandCanyons.LOW_CEIL - 2;
             i <= TerrainGenSkylandCanyons.HIGH_CEIL + 2;
             i++) {
            if (!world.getBlock(x, i, z).isAir(world, x, i, z)) {
                ymax = i - 1;
                break;
            ...
            for (int k = -r; k <= r; k++) {
                for (int j = -r2; j <= r2; j++) {
                    if (ReikaMathLibrary.isPointInsideEllipse(i, j, k, r, r2, r)) {
                        double c = (1 - Math.abs((double) i) / r)
                            * (1 - Math.abs((double) k) / r)
                            * (1 - Math.abs((double) j) / r2);
                        c *= 0.25;
