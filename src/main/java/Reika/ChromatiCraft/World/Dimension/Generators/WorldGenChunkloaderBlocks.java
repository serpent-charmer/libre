...
public class WorldGenChunkloaderBlocks extends ChromaWorldGenerator {
    private final double[][] crossSection = {
        { 0, 1, 3, 4, 3, 1, 0 },  { 1, 2, 5, 6, 5, 2, 1 }, { 3, 5, 7, 8, 7, 5, 3 },
        { 4, 6, 8, 10, 8, 6, 4 }, { 3, 5, 7, 8, 7, 5, 3 }, { 1, 2, 5, 6, 5, 2, 1 },
        { 0, 1, 3, 4, 3, 1, 0 },
    };
                ...
                if (flag) {
                    for (int j = 0; j < 6; j++) {
                        int dx = MathHelper.floor_double(
                            x + vx * j + (i - crossSection.length / 2) / (1 + j / 1.25D)
                        );
                        int dz = MathHelper.floor_double(
                            z + vz * j + (k - crossSection.length / 2) / (1 + j / 1.25D)
                        );
                        int dy = y + j - 1;
                        world.setBlock(
                            dx, dy, dz, ChromaBlocks.CHUNKLOADER.getBlockInstance(), 0, 3
                        );
                        if (j == 0 || j == 1) {
                            int ddy = j == 0 ? dy : dy - j;
                            for (int a = -1; a <= 1; a++) {
                                for (int b = -1; b <= 1; b++) {
                                    int ddx = dx + a;
                                    int ddz = dz + b;
                                    if (world.getBlock(ddx, ddy, ddz)
                                        != ChromaBlocks.CHUNKLOADER.getBlockInstance())
                                        world.setBlock(
                                            ddx,
                                            ddy,
                                            ddz,
