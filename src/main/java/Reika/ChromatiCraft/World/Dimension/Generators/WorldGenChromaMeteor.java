...
public class WorldGenChromaMeteor extends ChromaWorldGenerator {
    ...
    private void
    generateMeteor(World world, int x, int y, int z, Random rand, int r, int ra) {
        for (int i = -r; i <= r; i++) {
            for (int k = -r; k <= r; k++) {
                for (int h = -ra; h <= ra; h++) {
    ...
    private BlockKey getMeteorBlock() {
        return ModList.METEORCRAFT.isLoaded()
            ? MeteorGenerator.instance.getBlock(MeteorType.STONE, true)
            : basicOres.getRandomEntry();
    }

    private void generateTrack(
        World world, int x, int y, int z, Random rand, int r, int ra, double ang
    ) {
                ...
                for (int k = -r; k <= r; k++) {
                    for (int h = -ra; h <= ra; h++) {
                        if (ReikaMathLibrary.isPointInsideEllipse(i, h, k, rc, ra, rc)) {
                            ...
                            Block b2 = b == Blocks.stone ? rand.nextInt(40) == 0
                                    ? Blocks.obsidian
                                    : Blocks.cobblestone
                                : b == Blocks.dirt || b == Blocks.grass ? Blocks.dirt
                                                                        : Blocks.gravel;
    ...
    private void generateCrater(World world, int x, int y, int z, Random rand, int r) {
            ...
            for (int k = -rc; k <= rc; k++) {
                for (int h = ra; h >= -ra; h--) {
                    ...
                    if (ReikaMathLibrary.isPointInsideEllipse(i, h, k, rc, ra, rc)) {
                        ...
                        Block b = world.getBlock(dx, dy - 1, dz);
                        Block b2 = b == Blocks.stone
                            ? rand.nextInt(40) == 0 ? Blocks.obsidian : Blocks.cobblestone
                            : b == Blocks.dirt || b == Blocks.grass ? Blocks.dirt
                                                                    : Blocks.gravel;
                        //world.setBlock(dx, dy-1, dz, b2);
                    }
        ...
        for (double a = 0; a < 360; a += 1) {
            ...
            for (int hy = -3; hy <= t; hy++) {
                int cx = MathHelper.floor_double(
                    x + (rc + hy) * Math.cos(Math.toRadians(a))
                );
                int cz = MathHelper.floor_double(
                    z + (rc + hy) * Math.sin(Math.toRadians(a))
                );
                int cy = y + hy - 3;
                for (int h = 0; h < 16; h++) {
                    ...
                    if (!b.isAir(world, cx, cy + h, cz))
                        world.setBlock(
                            ...
                            cz,
                            h == 0
                                ? (rand.nextInt(18) == 0 ? Blocks.quartz_block
                                                         : Blocks.stone)
                                : Blocks.air
                        );
                }
