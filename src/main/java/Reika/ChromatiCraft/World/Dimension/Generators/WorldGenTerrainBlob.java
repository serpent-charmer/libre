...
public class WorldGenTerrainBlob extends ChromaWorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        Blob b = new Blob(
            rand.nextDouble() * 360, 8 + rand.nextDouble() * 24, 2 + rand.nextDouble() * 6
        );
        ...
        switch (rand.nextInt(20)) {
            ...
            case 4:
                if (ModOreList.PLATINUM.existsInGame())
                    b.centerBlock
                        = BlockKey.fromItem(ModOreList.PLATINUM.getRandomOreBlock());
                break;
            case 5:
                if (ModOreList.IRIDIUM.existsInGame())
                    b.centerBlock
                        = BlockKey.fromItem(ModOreList.IRIDIUM.getRandomOreBlock());
                break;
            case 10:
                b.centerBlock = new BlockKey(
                    ChromaBlocks.DIMGEN.getBlockInstance(),
                    DimDecoTypes.OCEANSTONE.ordinal()
                );
                break;
            case 11:
                break;
    ...
    private static class Blob {
        ...
        private void calculate(int x, int y, int z, Random rand) {
            ...
            for (double d = 0; d <= 1; d += 0.5D / length) {
                ...
                double r = maxWidth
                    * Math.pow(
                        0.25 + 3 * (0.5 - Math.abs(d - 0.5) / 2D), /*curvePower*/ 1
                    );
                //ReikaJavaLibrary.pConsole("Placing sphere R="+r+"/ "+d+" @
                //"+x+","+y+","+z);
            ...
            for (double i = -r; i <= r; i += 0.5) {
                for (double j = -r; j <= r; j += 0.5) {
                    for (double k = -r; k <= r; k += 0.5) {
                        if (Math.pow(Math.abs(i), pow) + Math.pow(Math.abs(j), pow)
                                + Math.pow(Math.abs(k), pow)
                            <= Math.pow(Math.abs(r), pow)) {
        ...
        private void generate(World world, Random rand) {
            for (Coordinate c : coords) {
                if (!hasDirtTop || coords.contains(c.offset(0, 1, 0))) {
                    ...
                    c.setBlock(
                        world,
                        b.isAir(world, c.xCoord, c.yCoord + 1, c.zCoord) ? Blocks.grass
                                                                         : Blocks.dirt
                    );
                }
            }
