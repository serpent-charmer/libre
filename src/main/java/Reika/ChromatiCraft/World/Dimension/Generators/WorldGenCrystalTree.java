...
public class WorldGenCrystalTree extends ChromaWorldGenerator {
    ...
    public static final BlockKey CRYSTAL_TRUNK = new BlockKey(
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (this.checkSpace(world, x, y, z, tree)) {
            ...
            for (int i = 0; i < h; i++) {
                int n = c.trunkWidth;
                world.setBlock(
                    x, y + i, z, CRYSTAL_TRUNK.blockID, CRYSTAL_TRUNK.metadata, 3
                );
                for (int a = 0; a < n; a++) {
                    for (int b = 0; b < n; b++) {
                        world.setBlock(
                            ...
                            y + i,
                            z - b,
                            CRYSTAL_TRUNK.blockID,
                            CRYSTAL_TRUNK.metadata,
                            3
                        );
                    }
                }
            ...
            //((TileEntitySign)world.getTileEntity(x, y+h+tree.getSizeY(), z)).signText =
            //new String[]{c.name(), String.valueOf(x), y+", "+h, String.valueOf(z)};
            return true;
        }
        return false;
    }
    ...
    private boolean checkSpace(World world, int x, int y, int z, FilledBlockArray tree) {
        for (Coordinate c : tree.keySet()) {
            ...
            boolean flag = needsAir
                ? at.isAir(world, c.xCoord, c.yCoord, c.zCoord)
                : at.canBeReplacedByLeaves(world, c.xCoord, c.yCoord, c.zCoord);
            if (!flag)
                return false;
        }
        return true;
    ...
    private static enum CrystalTree {
        ...
        private void populate(FilledBlockArray f) {
            ...
            BlockKey l = new BlockKey(
                ChromaBlocks.DIMGEN.getBlockInstance(), DimDecoTypes.CRYSTALLEAF.ordinal()
            );
            switch (this) {
                case VANILLA:
                case CLIPPED:
                ...
                case FIR: {
                    ...
                    for (int i = 2; i < 6; i++) {
                        ...
                        f.setBlock(
                            3 * dir.offsetX + dir2.offsetX,
                            4,
                            3 * dir.offsetZ + dir2.offsetZ,
                            t
                        );
                        f.setBlock(
                            3 * dir.offsetX - dir2.offsetX,
