...
public enum ThreadedGenerators {
    ...
    private static final EnumMap<ThreadedGenerators, ThreadedGenerator> runMap
        = new EnumMap(ThreadedGenerators.class);
    ...
    public void run(long seed) {
        try {
            ...
            throw new RuntimeException(
                "Could not start threaded generator " + this.getName(), e
            );
        }
    }
        ...
        public void run() {
            if (Thread.currentThread()
                ...
                FMLCommonHandler.instance().raiseException(
                    new IllegalStateException(msg), msg, true
                );
            }
            if (generator.isRunning) {
                String msg
                    = "You cannot run two threaded generator instances simultaneously!";
                FMLCommonHandler.instance().raiseException(
                    new IllegalStateException(msg), msg, true
                );
            }
            ...
            ChromatiCraft.logger.log(
                "Initializing Dimension " + generator.getName() + " generation thread..."
            );
            try {
                ...
                ChromatiCraft.logger.log(String.format(
                    "Dimension " + generator.getName()
                        + " generation thread complete; Elapsed time: %.3fs. %s",
                    el,
                    sg
                ));
            } catch (Throwable e) {
                e.printStackTrace();
                String msg = "Dimension " + generator.getName()
                    + " generation thread failed with " + e.getClass().getName() + ".";
    ...
    public static void reset() {
        runMap.clear();
        ChromatiCraft.logger.log(
            "All " + generators.length + " dimension generators complete."
        );
    }
}
