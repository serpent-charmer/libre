...
public class BiomeDistributor extends ThreadedGenerator {
    ...
    private final MultiMap<ChromaDimensionBiomeType, Point> blobLocations
        = new MultiMap();
    ...
    private static ChromaDimensionBiomeType[] buildBiomeList() {
        ChromaDimensionBiomeType[] arr = new ChromaDimensionBiomeType
            [Biomes.biomeList.length + SubBiomes.biomeList.length + 1];
    ...
    public static ChromaDimensionBiome getBiome(int x, int z) {
        double d
            = ChunkProviderChroma.getDistanceToNearestStructureBlockCoordsWithinRange(
                x, z, MAX_STRUCTURE_RADIUS
            );
        if (d <= MAX_STRUCTURE_RADIUS) {
            ...
            if (map.isPointInsideCurve(dx, dz)) {
                return map == monumentBlob ? Biomes.MONUMENT.getBiome()
                                           : Biomes.STRUCTURE.getBiome();
            }
        }
    ...
    private void addStructureBiomes() throws InterruptedException {
        ...
        while (!ChunkProviderChroma.isGeneratorReady(ThreadedGenerators.STRUCTURE)) {
            ...
            if (!printed)
                ChromatiCraft.logger.log(
                    "Waiting for structure generator to finish to place relevant biomes..."
                );
            printed = true;
        }
        for (StructurePair p : ChunkProviderChroma.getStructures()) {
            ...
            LobulatedCurve c
                = LobulatedCurve
                      .fromMinMaxRadii(MIN_STRUCTURE_RADIUS, MAX_STRUCTURE_RADIUS, 12)
                      .generate(rand);
        ...
        for (int i = 0; i < ChromaDimensionManager.Biomes.biomeList.length; i++) {
            ...
            for (int k = 0; k < b.spawnWeight; k++) {
                ...
                for (int m = 0; m < SPREADERS_PER_DOT; m++) {
                    ...
                    spreaders.add(new Spreader(b, ForgeDirection.VALID_DIRECTIONS[2+m%4],
                sx, sz));
                }
                 */
    ...
    private void placeBlob(
        ChromaDimensionBiomeType b,
        int dx,
        int dz,
        double f,
        ChromaDimensionBiomeType over
    ) {
        f *= SIZE / 4096F;
    ...
    private void createImage(String phase) throws IOException {
        ...
        BufferedImage img = new BufferedImage(
            biomes.length, biomes.length, BufferedImage.TYPE_INT_ARGB
        );
        for (int i = 0; i < biomes.length; i++) {
            for (int k = 0; k < biomes[i].length; k++) {
                ...
                int color = b != null ? 0xff000000
                        | Color.HSBtoRGB(
                            ((Biomes) b).ordinal() / (float) Biomes.biomeList.length, 1, 1
                        )
                                      : 0xffffffff;
    ...
    private static void paint(
        int x, int z, ChromaDimensionBiomeType b, int r, ChromaDimensionBiomeType over
    ) {
        for (int i = -r; i <= r; i++) {
            for (int k = -r; k <= r; k++) {
                if (i * i + k * k <= r * r) {
    ...
    private class Spreader {
        ...
        private ArrayList<ForgeDirection> getDirections() {
            ...
            for (int i = 2; i < 6; i++) {
                ForgeDirection dir = ForgeDirection.VALID_DIRECTIONS[i];
                ChromaDimensionBiomeType b = getBiome(BiomeDistributor.this.getAdj(
                    posX, posZ, dir.offsetX * stepSize, dir.offsetZ * stepSize
                ));
                if (b == null)
                    li.add(dir);
            }
    ...
    public String getStateMessage() {
        return "Biome array populated, " + SIZE + "x" + SIZE + " with "
            + blobLocations.totalSize() + " biome patches of "
            + blobLocations.keySet().size() + " types.";
    }
}
