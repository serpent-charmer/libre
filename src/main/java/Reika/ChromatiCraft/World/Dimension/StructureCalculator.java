...
public class StructureCalculator extends ThreadedGenerator {
    ...
    private StructureCalculator(long seed, int max) {
        ...
        //Would base off world seed, but is loaded "outside" a MC world and as such cannot
        //reference it; make it file-specific instead seededRand = new Random(new
        //File("c:").getTotalSpace() ^ System.getProperty("os.name").hashCode());
    ...
    public static void sendSeed(EntityPlayer ep) {
        ...
        ReikaPacketHelper.sendDataPacket(
            ...
            ChromaPackets.STRUCTSEED.ordinal(),
            (EntityPlayerMP) ep,
            spl[0],
            spl[1]
        );
    ...
    private static long generateOrGetGenSeed() {
        ...
        File f = new File(
            DragonAPICore.getMinecraftDirectory(), "ChromatiCraft_Data/DimensionGen.dat"
        );
        try {
            if (f.exists()) {
                try {
                    List<String> li
                        = ReikaFileReader.getFileAsLines(f, true, Charsets.UTF_8);
                    if (!li.isEmpty()) {
                        String s = li.get(0);
            ...
            ReikaFileReader.writeLinesToFile(
                f,
                ReikaJavaLibrary.makeListFrom("Seed:" + String.valueOf(seed)),
                true,
                Charsets.UTF_8
            );
        } catch (IOException e) {
        ...
        return new File("c:").getTotalSpace()
            ^ System.getProperty("os.name").hashCode(); //fallback
    }
    ...
    public void run() throws Throwable {
        ...
        if ((DragonAPICore.isReikasComputer()
            ...
            ChromatiCraft.logger.log(
                "Generated Structures: " + ChunkProviderChroma.structures
            );
        }
    }
    ...
    private void generate() throws OutOfMemoryError {
        this.initSeed();
        /*
        ArrayList<CrystalElement> colors =
        ReikaJavaLibrary.makeListFromArray(CrystalElement.elements); for (int i = 0; i <
        DimensionStructureType.types.length; i++) { int index =
        rand.nextInt(colors.size()); CrystalElement e = colors.get(index);
            colors.remove(index);
            ChunkProviderChroma.structures.add(new
        StructurePair(DimensionStructureType.types[i], e));
        }
         */
        ...
        for (int i = 0; i < CrystalElement.elements.length; i++) {
            ...
            ChunkProviderChroma.structures.add(
                new StructurePair(e.createGenerator(idx), CrystalElement.elements[i])
            );
        ...
        structureOriginX
            = ReikaRandomHelper.getRandomPlusMinus(0, STRUCTURE_CENTER_VARIATION);
        structureOriginZ
            = ReikaRandomHelper.getRandomPlusMinus(0, STRUCTURE_CENTER_VARIATION);
        ...
        for (StructurePair s :
             new ArrayList<StructurePair>(ChunkProviderChroma.structures)) {
    ...
    public static EnumMap<CrystalElement, StructureTypeData> getStructureColorTypes() {
        EnumMap<CrystalElement, StructureTypeData> map
        ...
        for (int i = 0; i < CrystalElement.elements.length; i++) {
            ...
            structs.remove(index);
            map.put(
                CrystalElement.elements[i],
                new StructureTypeData(CrystalElement.elements[i], e, idx)
            );

            if (structs.isEmpty()) {
                structs = throwaway.getUsableStructures(
                ); //reuse structure types as necessary
                idx++;
            }
        }
    ...
    private void generateMonument() {
        ChunkProviderChroma.monument.startCalculate(
            structureOriginX, structureOriginZ, rand
        );
        if ((DragonAPICore.isReikasComputer()
             && ReikaObfuscationHelper.isDeObfEnvironment())
            || DragonAPICore.debugtest) {
            String sg = "CHROMATICRAFT: Generated the monument at " + structureOriginX
                + ", " + structureOriginZ + ".";
    ...
    private ArrayList<DimensionStructureType> getUsableStructures() {
        ArrayList<DimensionStructureType> li
            = ReikaJavaLibrary.makeListFromArray(DimensionStructureType.types);
        if (allowUnfinishedStructures && DragonAPICore.isReikasComputer()
            && ReikaObfuscationHelper.isDeObfEnvironment()) {
            return li;
        }
    ...
    private void tryGenerate(StructurePair p, int attempt) {
        try {
            ...
            if (redo)
                ...
                sb.append(
                    "Already failed too many (" + maxAttempts + ") times. Giving up."
                );
    ...
    private void doGenerate(StructurePair s) {
        ...
        if ((DragonAPICore.isReikasComputer()
             && ReikaObfuscationHelper.isDeObfEnvironment())
            || DragonAPICore.debugtest) {
            String sg = "CHROMATICRAFT: Generated a " + s.color + " " + s.generator
                + " at " + s.generator.getEntryPosX() + ", " + s.generator.getEntryPosZ()
                + " with password "
                + new ElementEncodedNumber(s.generator.getPassword(null)) + " in " + dt
                + " s";
            ReikaJavaLibrary.pConsole(sg);
        }
    }
