...
public class ChromaCloudRenderer extends IRenderHandler {
    ...
    private final FogStarBrightness[][] fogStarBrightness
        = new FogStarBrightness[1 + FOG_STAR_SECTIONS][1 + FOG_STAR_SECTIONS];
    ...
    private ChromaCloudRenderer() {
        ...
        for (int i = 0; i < 4; i++) {
            skyTex[i] = ChromaClient.dynamicAssets.createAsset(
                "Textures/clouds/dimsky_" + i + ".png"
            );
        }
    }
    ...
    public void render(float ptick, WorldClient world, Minecraft mc) {
        ...
        float playerY = (float
           ...
           + (mc.renderViewEntity.posY - mc.renderViewEntity.lastTickPosY) * ptick);
        float y = 70 * 0 + 28 + 8
            - playerY * 0.0625F; //world.provider.getCloudHeight() - playerY + 0.33F;

        double x = (mc.renderViewEntity.prevPosX
                    + (mc.renderViewEntity.posX - mc.renderViewEntity.prevPosX) * ptick)
            / f2;
        double z = (mc.renderViewEntity.prevPosZ
                    + (mc.renderViewEntity.posZ - mc.renderViewEntity.prevPosZ) * ptick)
                / f2
            + 0.33D;
                ...
                for (int i = 0; i < n; i++) {
                    if (flag)
                        ReikaTextureHelper.bindTexture(
                            ChromatiCraft.class, "Textures/clouds/dimsky_april.png"
                        );
                    else
                    ...
                    double slide
                        = (((world.getTotalWorldTime() % 24000 + ptick) / 24000D) * w * s)
                        % w;
                    ...
                    double mult = flag ? 1
                            ...
                            + 0.375
                                * Math.sin(
                                    System.currentTimeMillis() / 2000D + i * Math.PI / 2
                                );
                    ...
                    this.drawCloudLayer2(
                        v5, a, b, c1, w * 1, 180 + d - playerY * 0.4375, i
                    );
    ...
    public static void drawVoidFog(EntityPlayer ep) {
        ...
        if (ep.posY < 20 && ChromaDimensionManager.getStructurePlayerIsIn(ep) == null) {
            ...
            ChromaShaders.DIMFLOOR.setIntensity(
                Math.min(ChromaShaders.DIMFLOOR.getIntensity(), f)
            );
        ...
        int color = ReikaColorAPI.getModifiedHue(
            0xff0000, 190 + (int) (10 * Math.sin((ep.ticksExisted + f) / 83F))
        );
        color = ReikaColorAPI.getModifiedSat(
            color, 0.25F + (float) (0.125 * Math.cos((ep.ticksExisted + f) / 57F))
        );
        ...
        while (a > 0.125) {
            h += h1 + h / 3D;
            //v5.setColorOpaque_I(ReikaColorAPI.getColorWithBrightnessMultiplier(color,
            //a));
        ...
        ReikaTextureHelper.bindTexture(
            ChromatiCraft.class, "Textures/clouds/dimfog-stars.png"
        );
        v5.startDrawingQuads();
    ...
    private double getCloudOffset(int i, int k, int index) {
        ...
        double d = 2
            + 2
                * Math.sin(
                    (i * i + k * k) / 32D * ((time % 250000 + 250000) % 250000) / 250000D
                );
        return d * (Math.sin(i + time / 5000D) + Math.sin(k / 2D + time / 2000D));
    }

    private void drawCloudLayer2(
        Tessellator v5, int a, int b, int color, double w, double h, int index
    ) {
        v5.startDrawingQuads();
    ...
    private void
    drawCloudLayer(Tessellator v5, int color, double w, double h, int index) {
        v5.startDrawingQuads();
        v5.setColorRGBA_I(0xffffff, color);
        ...
        for (int i = 0; i < s; i++) {
            ...
            for (int k = 0; k < s; k++) {
                ...
                double hmm = h
                    + (off[i + 1][k + 1] + off[i - 1 + 1][k - 1 + 1]
                       + off[i - 1 + 1][k + 1] + off[i + 1][k - 1 + 1])
                        / 5D;
                double hpm = h
                    + (off[i + 1][k + 1] + off[i + 1 + 1][k - 1 + 1]
                       + off[i + 1 + 1][k + 1] + off[i + 1][k - 1 + 1])
    ...
    private void renderCloudsVanilla(WorldClient world, Minecraft mc, float ptick) {
        ...
        double posX = (mc.renderViewEntity.prevPosX
                       + (mc.renderViewEntity.posX - mc.renderViewEntity.prevPosX) * ptick
                       + slide * 0.03D)
            / f2;
        double posZ
        ...
        ReikaTextureHelper.bindTexture(
            ChromatiCraft.class, "Textures/clouds/dimension.png"
        );
        GL11.glEnable(GL11.GL_BLEND);
                ...
                for (int dz = -b1 + 1; dz <= b1; ++dz) {
                    ...
                    if (f4 > -f3 - 1.0F) {
                        tessellator.setColorRGBA_F(
                            red * 0.7F, green * 0.7F, blue * 0.7F, 0.8F
                        );
                    ...
                    tessellator.setColorRGBA_F(
                        red * 0.9F, green * 0.9F, blue * 0.9F, 0.8F
                    );
                    int j1;
                    ...
                    tessellator.setColorRGBA_F(
                        red * 0.8F, green * 0.8F, blue * 0.8F, 0.8F
                    );
