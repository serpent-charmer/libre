...
public class Aurora {
    private static final RemoteSourcedAsset texture
        = ChromaClient.dynamicAssets.createAsset("Textures/aurora4.png");
    ...
    public Aurora(
        ...
        int c2,
        double speed,
        /*double variance, double segSize, */ double x1,
        double y1,
        double z1,
        double x2,
        double y2,
    ...
    public void render() {
        ...
        for (int i = 0; i < li.size() - 1; i++) {
            ...
            double y1 = point1.yCoord
                + (point2.yCoord - point1.yCoord) * (i / (double) li.size());
            double y2 = point1.yCoord
                + (point2.yCoord - point1.yCoord) * ((i + 1) / (double) li.size());
            ...
            float fb = (i + 1) < n        ? (i + 1) / n
                : li.size() - (i + 1) < n ? (li.size() - (i + 1) - 1) / n
                                          : 1;
    ...
    private static class AuroraSpline {
        ...
        private AuroraSpline(
            DecimalPosition p1, DecimalPosition p2, int n, double vel, double var
        ) {
            ...
            double ang
                = Math.toRadians(-ReikaPhysicsHelper.cartesianToPolar(dx, dy, dz)[2]);
            ...
            for (int i = 0; i <= n; i++) {
                ...
                spline.addPoint(new AuroraPoint(
                    mid, end ? 0 : var, end ? 0 : vel, p1, dx, dy, dz, f, fx, fz
                ));
            }
        }
