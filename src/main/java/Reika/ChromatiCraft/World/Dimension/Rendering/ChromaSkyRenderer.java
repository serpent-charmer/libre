...
public class ChromaSkyRenderer extends IRenderHandler {
    ...
    private final int[] starColors = { 0xffffff, 0xC6FFFA, 0xC6E5FF, 0x9BB6FF, 0x7C70FF,
                                       0xFF6868, 0xFFADAD, 0xFFDAAD, 0xFFF0AD };
    ...
    public void render(float partialTicks, WorldClient world, Minecraft mc) {
        ...
        float f = mc.thePlayer.posY <= 18 ? 0
            : mc.thePlayer.posY >= 30     ? 1
                                          : (float) ((mc.thePlayer.posY - 18) / 12F);
        if (ReikaEntityHelper.canEntitySeeTheSky(mc.thePlayer))
            f = 1;
        //if (mc.thePlayer.posY > 0 && mc.thePlayer.rotationPitch < -30 &&
        //ReikaEntityHelper.canEntitySeeTheSky(mc.thePlayer)) 	f = Math.min(1,
        //f-mc.thePlayer.rotationPitch/90F);
    ...
    private static class Supernova extends Star {
        ...
        public static Supernova random(Random rand) {
            ...
            Supernova ret = new Supernova(
                c,
                ReikaRandomHelper.getRandomBetween(18D, 24D, rand),
                s.d0,
                s.d1,
                s.d2,
                s.d3,
    ...
    private static class Star extends TexturedQuad {
        ...
        private static Star
        createRandomized(int c, int tex, double tw, double ta, Random rand) {
            ...
            if (d4 < 1.0D && d4 > 0.01D) {
                ...
                return new Star(
                    c, tex, size * 12, tw, ta, d0, d1, d2, d3, d9, d10, d12, d13, d16
                );
            }
            return null;
        }
        ...
        protected void render(double d, float brightness, double time) {
            ...
            int c2 = ReikaColorAPI.getColorWithBrightnessMultiplier(
                color,
                (float
                ) (tb
                   + twinkleAmplitude * Math.sin(twinkleOffset + time * twinkleSpeed / td)
                )
            );
            int c = brightness >= 24
                ? c2
                : ReikaColorAPI.getColorWithBrightnessMultiplier(c2, brightness / 24F);
    ...
    private static class TexturedQuad {
        ...
        constructRandomized(int tex, int row, double s, Random rand) {
            double d0 = rand.nextFloat() * 2.0F - 1.0F;
            double d1 = rand.nextFloat() * 2.0F - 1.0F;
            double d2 = rand.nextFloat() * 2.0F - 1.0F;
            double d3 = 0.15F + rand.nextFloat() * 0.1F;
            ...
            if (d4 < 1.0D && d4 > 0.01D) {
                ...
                return new TexturedQuad(
                    tex, row, s, d0, d1, d2, d3, d9, d10, d12, d13, d16
                );
            }
            return null;
        }
        ...
        protected void render(double d, float brightness, double time) {
            ...
            for (int j = 0; j < 4; j++) {
                ...
                Tessellator.instance.addVertexWithUV(
                    d5 + d24, d6 + d22, d7 + d25, texU[j], texV[j]
                );
            }
        }
    }
