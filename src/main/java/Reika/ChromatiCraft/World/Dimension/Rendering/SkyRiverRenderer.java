...
public class SkyRiverRenderer {
    ...
    public void render() {
        ...
        Collection<RiverPoint> c = SkyRiverGenerator.getPointsWithin(
            Minecraft.getMinecraft().thePlayer, 512, false
        );
        ReikaTextureHelper.bindTexture(
            ChromatiCraft.class, "/Reika/ChromatiCraft/Textures/SkyRiver2.png"
        );
        ...
        for (RiverPoint p : c) {
            ...
            double r1 = SkyRiverGenerator.RIVER_TUNNEL_RADIUS
                + 4 * Math.sin(t - p.positionID * STEP_PER_POINT);
            double r2 = SkyRiverGenerator.RIVER_TUNNEL_RADIUS
                + 4 * Math.sin(t - p.nextRiverPoint.positionID * STEP_PER_POINT);
            int clr = ReikaColorAPI.getModifiedHue(
                0xff0000,
                (int) ((p.positionID * 4 - System.currentTimeMillis() / 500D) % 360D)
            );
            int c1 = clr;
            int c2 = ReikaColorAPI.getModifiedHue(
                0xff0000,
                (int
                ) (((p.positionID + 1) * 4 - System.currentTimeMillis() / 500D) % 360D)
            );
            ...
            ChromaFX.drawEnergyTransferBeam(
                p.position, p.next, c1, c2, r1, r2, 36, t / 4500D, true
            );
                ...
                if (p.positionID == 1) {
                    ...
                    for (int i = 0; i < n; i++) {
                        ...
                        PositionController pc
                            = new CollectingPositionController(px, py, pz, tx, ty, tz, l);
                        EntityFX fx = new EntityCCBlurFX(
                                          Minecraft.getMinecraft().theWorld, px, py, pz
                        )
                                          .setIcon(ChromaIcons.FADE_GENTLE)
                                          .setLife(l)
                                          .setScale(14)
                                          .setPositionController(pc)
                                          .setColor(clr);
