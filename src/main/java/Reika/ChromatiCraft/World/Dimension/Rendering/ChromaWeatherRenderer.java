        ...
        for (int z = ez - radius; z <= ez + radius; z += d) {
            for (int x = ex - radius; x <= ex + radius; x += d) {
                ...
                if (!(biome instanceof ChromaDimensionBiome)) {
                    //ChromatiCraft.logger.logError("Biome at "+x+", "+z+" in dim
                    //"+world.provider.dimensionId+" is not a dimension biome, but is
                    ...
                    if (minY != maxY) {
                        ...
                        if (b1 != 0) {
                            ...
                            ReikaTextureHelper.bindTexture(
                                ChromatiCraft.class,
                                "Textures/DimWeather/"
                                    + b.getExactType().name().toLowerCase(Locale.ENGLISH)
                                    + ".png"
                            );
                            ReikaTextureHelper.bindTexture(
                                ChromatiCraft.class, "Textures/DimWeather/deepocean.png"
                            );
                            v5.startDrawingQuads();
                        }

                        float f = (float
                        ) (96 + 32 * Math.sin(System.currentTimeMillis() / 2000D));
                        frame = (tick + (x % 16) * 6 + (z % 16) * 8)
                            / f; //((rendererUpdateCount+x*x*3121+x*45238971+z*z*418711+z*13761
                                 //& 31)+ptick)/32.0F*(3.0F+rand.nextFloat());
                        double dx = x + 0.5F - e.posX;
                        double dz = z + 0.5F - e.posZ;
                        float dd = MathHelper.sqrt_double(dx * dx + dz * dz) / radius;
                        float colorFactor = 1F;
                        v5.setBrightness(
                            240 /*world.getLightBrightnessForSkyBlocks(x, y, z, 0)*/
                        );
                        float alpha = 1;
                        float frameSizeX = 1F;
                        float frameSizeY = 1F;
                        ...
                        v5.addVertexWithUV(
                            ...
                            z - staggerY + 0.5,
                            u + 0F * frameSizeX,
                            minY * frameSizeY / 4F + frame * frameSizeY
                        );
                        v5.addVertexWithUV(
                            x + staggerX + 0.5,
                            minY,
                        ...
                        v5.addVertexWithUV(
                            ...
                            z + staggerY + 0.5,
                            u + 1F * frameSizeX,
                            maxY * frameSizeY / 4F + frame * frameSizeY
                        );
                        v5.addVertexWithUV(
                            x - staggerX + 0.5,
                            maxY,
