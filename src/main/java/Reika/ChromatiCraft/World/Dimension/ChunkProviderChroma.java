...
public class ChunkProviderChroma implements IChunkProvider {
    ...
    static {
        for (int i = 0; i < 16; i++) {
            bumpMaps[i] = new BumpMap(ChromatiCraft.class, "Textures/Dimension Bump
    Maps/"+CrystalElement.elements[i].name().toLowerCase()+".png");
        }
    }
     */
    ...
    private static synchronized void regenerateGenerators(boolean doGen, int invflags) {
        ...
        if (doGen)
            generationFlags
                = ReikaMathLibrary.getNBitflags(ThreadedGenerators.generators.length)
                & ~invflags;
    ...
    public ChunkProviderChroma(World world) {
        ...
        offsetNoise = (SimplexNoiseGenerator) new SimplexNoiseGenerator(randomSeed)
                          .setFrequency(0.015);
        chunkManager = new ChromaChunkManager(world);
        worldType
            = WorldType
                  .DEFAULT; //world.getWorldInfo().getTerrainType(); //not that it matters
    ...
    static double
    getDistanceToNearestStructureBlockCoordsWithinRange(int x, int z, double r) {
    ...
    public void generateColumnData(int chunkX, int chunkZ, Block[] columnData) {
        byte b0 = 63; //32;//63;
        //biomesForGeneration = chunkManager.getBiomesForGeneration(biomesForGeneration,
    ...
    private void applyRawBumpMapping(int cx, int cz, Block[] data) {
        for (int dx = 0; dx < 16; dx++) {
            for (int dz = 0; dz < 16; dz++) {
                ...
                int x = cx * 16 + dx;
                int z = cz * 16 + dz;
                //int bump = this.calculateBlendedBump(x, z,
                //bumpMaps[CrystalElement.CYAN.ordinal()]);
                int bump = -(int
                ) (0.25
                   * Math.abs(Math.tan((x % 256) * (x % 256) + (z % 256) * (z % 256)))
                ); //(int)(3*Math.sin(x/64D)+4*Math.sin(z/512D)+1*Math.cos(x/16D+z/32D+x*z/128D));
                    ...
                    if (sy >= 0) {
                        if (bump > 0) {
                            ...
                            for (int i = 0; i < bump; i++) {
                                ...
                                //ReikaJavaLibrary.pConsole("Setting "+x+", "+(sy-i)+",
                                //"+z);
                            }
                        }
                    }
    ...
    private int calculateBlendedBump(int x, int z, BumpMap bp) {
        ...
        return 127
            + (bp.getBump(mx, mz) + bp.getBump(mxp, mz) + bp.getBump(mxm, mz)
               + bp.getBump(mx, mzp) + bp.getBump(mx, mzm))
            / 5 / 16;
    }
    ...
    public void replaceBlocksForBiome(
        ...
        Block[] columnData,
        byte[] metaData,
        BiomeGenBase[] biomeData,
        int[] dy0
    ) {
        double d0 = 0.03125D;
        stoneNoise = noiseGen4.func_151599_a(
            stoneNoise, chunkX * 16, chunkZ * 16, 16, 16, d0 * 2.0D, d0 * 2.0D, 1.0D
        );
        ...
        for (int dx = 0; dx < 16; dx++) {
            for (int dz = 0; dz < 16; dz++) {
                ...
                this.generateSurfaceGrass(
                    x, z, posIndex, columnData, metaData, dy, biome
                );
            }
        }
    }

    private void generateSandBeaches(
        int x,
        int z,
        int posIndex,
        Block[] columnData,
    ...
    private void generateSurfaceGrass(
        int x,
        int z,
        int posIndex,
        Block[] columnData,
    ...
    private void generateBedrockLayer(
        ...
        double sz
            = d2 * Math.cos(Math.PI * 1.5 + Math.toRadians((z % 128 / 128D) * 360D));
        double sxz1 = d3 * Math.sin(Math.toRadians(x * z / 8D))
            * Math.cos(Math.toRadians(x * z / 16D));
        double sxz2 = d4 * Math.cos(Math.toRadians(x * z * z / 128D))
            * Math.sin(Math.toRadians(x * x * z / 256D));
     ...
     * generates all the blocks for the specified chunk from the map seed and chunk seed
     */
    public Chunk provideChunk(int chunkX, int chunkZ) {
        rand.setSeed(chunkX * 341873128712L + chunkZ * 132897987541L);
        Block[] ablock = new Block[65536];
        byte[] abyte = new byte[65536];
        biomesForGeneration = new BiomeGenBase[256]; //chunkManager.loadBlockGeneratorData(biomesForGeneration,
        ...
        this.replaceBlocksForBiome(
            chunkX, chunkZ, ablock, abyte, biomesForGeneration, offset
        );
    ...
    private Block[] shiftTerrainGen(
        int chunkX, int chunkZ, Block[] ablock, int[] offset
    ) {
    ...
    private void applyNoiseLayers(int chunkX, int chunkZ) {
        ...
        double d2 = 512.0D;
        double d3 = 512.0D;
        noiseData6 = noiseGen6.generateNoiseOctaves(
            noiseData6, chunkX, chunkZ, 5, 5, 200.0D, 200.0D, 0.5D
        );
        noiseData3 = noiseGen3.generateNoiseOctaves(
            noiseData3,
            chunkX,
            0,
            chunkZ,
        ...
        noiseData1 = noiseGen1.generateNoiseOctaves(
            noiseData1, chunkX, 0, chunkZ, 5, 33, 5, 684.412D, 684.412D, 684.412D
        );
        noiseData2 = noiseGen2.generateNoiseOctaves(
            noiseData2, chunkX, 0, chunkZ, 5, 33, 5, 684.412D, 684.412D, 684.412D
        );
        boolean flag1 = false;
        boolean flag = false;
        int idx = 0;
            ...
            for (int k1 = 0; k1 < 5; k1++) {
                ...
                for (int l1 = -b0; l1 <= b0; l1++) {
                    for (int i2 = -b0; i2 <= b0; i2++) {
                        ...
                        //These two gives a map that is half plains, half mountains -
                        desirable f4 *= 8; f3 = 0.125F;
                        ...
                        //float f3 =
                        //-0.25F+0.75F*MathHelper.sin(chunkX/32F)*MathHelper.cos(chunkZ/48F);
                        //float f4 =
                        //0.0625F*2*(2+1F*MathHelper.cos(chunkX/96F)+1F*MathHelper.sin(chunkZ/64F));

                        //float f3 =
                        //-0.25F+(float)Math.sqrt(Math.sin(chunkX/8D)*Math.sin(chunkX/8D)+Math.cos(chunkZ/8D)*Math.cos(chunkZ/8D));
                        //float f4 =
                        //0.0625F*(float)Math.sqrt((chunkX*chunkX+chunkZ*chunkZ)/128D);

                        //FINAL GENERATION
                        float f0 = (float
                        ) Math.sqrt((chunkX * chunkX + chunkZ * chunkZ) / (65536D * 32));

                        //New from BiomeDistributor
                        f0 *= 0.03125;

                        double dd = this.getDistanceToNearestStructureChunkCoords(
                            chunkX, chunkZ
                        );
                        if (dd <= 8) {
                            f0 *= dd / 8D;
                        ...
                        for (int i = -r; i <= r; i++) {
                            for (int k = -r; k <= r; k++) {
                                ...
                                ChromaDimensionBiome biome = BiomeDistributor.getBiome(dx,
                        dz); int base = biome.getExactType().getBaseHeightDelta();
                                avg.addValue(base, 1D/(1D+ReikaMathLibrary.py3d(i, 0,
                        k)));
                            }
                        }
                        ...
                        //8*Math.sin(chunkX/256D)+32*Math.cos(chunkZ/512D); - large
                        //mountains, interesting terrain
    ...
    public void populate(IChunkProvider loader, int chunkX, int chunkZ) {
        ...
        //{ SpawnerAnimals.performWorldGenSpawning(worldObj, biomegenbase, k + 8, l + 8,
        //16, 16, rand);
        //}
    ...
    private StructurePair getGenStructure() {
        CrystalElement e = unusedColors.isEmpty() ? null :
    ReikaJavaLibrary.getRandomCollectionEntry(unusedColors); unusedColors.remove(e);

        DimensionStructureType gen = unusedTypes.isEmpty() ? null :
    ReikaJavaLibrary.getRandomCollectionEntry(unusedTypes); unusedTypes.remove(gen);
    ...
    public void
    onPopulationHook(IChunkProvider gen, IChunkProvider loader, int x, int z) {
        ...
        for (StructurePair s : structures) {
            //ReikaJavaLibrary.pConsole("Generating chunk "+x+", "+z+" for a "+s.color+"
            //"+s.generator);
    ...
    private void generateExtraChromaOre(
        World world, IChunkProvider gen, IChunkProvider loader, int x, int z
    ) {
        for (int i = 0; i < 6; i++) {
            if (i < 3)
    ...
    /**
     * Save extra data not associated with any Chunk.  Not saved during autosave, only
     * during world unload.  Currently unimplemented.
     */
    public void saveExtraData() {}

    /**
     * Unloads chunks that are marked to be unloaded. This is not guaranteed to unload
     * every such chunk.
     */
