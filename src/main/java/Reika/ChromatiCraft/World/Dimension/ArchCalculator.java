...
public class ArchCalculator extends ThreadedGenerator {
    ...
    private static final ChunkSplicedGenerationCache arches
        = new ChunkSplicedGenerationCache().setWrapping(AREA, AREA);
    ...
    public static class Arch {
        ...
        private Arch(
            int x, int z, double ca, double ia, double da, double r, double rv, double d
        ) {
        ...
        public void calculate(Random rand) {
            ...
            do {
                this.placeBlob(rand);
                double[] d = ReikaPhysicsHelper.polarToCartesian(
                    stepDistance, angle, compassAngle
                );
                posX += d[0];
                posY += d[1];
                posZ += d[2];
        ...
        private void placeBlob(Random rand) {
            ...
            double r
                = radius - radiusVariation / 2D + rand.nextDouble() * radiusVariation;
        ...
        public void generate(World world, Random rand) {
            for (Coordinate c : coords) {
                if (coords.contains(c.offset(0, 1, 0))
                    || coords.contains(c.offset(0, -1, 0))) {
                    if (coords.contains(c.offset(0, 1, 0))) {
