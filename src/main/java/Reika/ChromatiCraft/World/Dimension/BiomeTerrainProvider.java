...
public class BiomeTerrainProvider {
    ...
    public void generateChunk(World world, int chunkX, int chunkZ, Random rand) {
        //ReikaJavaLibrary.pConsole("Providing chunk "+chunkX+", "+chunkZ+"
        //["+(chunkX*16)+"-"+(chunkX*16+15)+","+(chunkZ*16)+"-"+(chunkZ*16+15)+"]");
        chunkX = chunkX << 4;
        chunkZ = chunkZ << 4;
        for (int i = 0; i < 16; i++) {
            for (int k = 0; k < 16; k++) {
                ...
                for (ChromaDimensionBiomeTerrainShaper s : terrain) {
                    if (s.canGenerateIn(b)) {
                        ...
                        s.generateColumn(
                            ...
                            top,
                            rand,
                            this.getEdgeFactor(world, dx, dz, s)
                        );
                    }
                }
            }
    ...
    getEdgeFactor(World world, int x, int z, ChromaDimensionBiomeTerrainShaper t) {
