...
public enum DimensionGenerators {
    RIFT(
        WorldGenFissure.class,
        GeneratorType.TERRAIN,
        GeneratorTheme.ENERGY,
        Integer.MIN_VALUE
    ),
    METEOR(
        WorldGenChromaMeteor.class,
        GeneratorType.FEATURE,
        GeneratorTheme.GEOHISTORICAL,
        Integer.MIN_VALUE
    ),
    ...
    FLOATSTONE(
        WorldGenFloatstone.class, GeneratorType.TERRAIN, GeneratorTheme.SKYFEATURE, 100
    ),
    ...
    FORESTS(
        WorldGenTreeCluster.class, GeneratorType.FEATURE, GeneratorTheme.FOLIAGE, -50
    ),
    MOONPOOL(WorldGenMoonPool.class, GeneratorType.STRUCTURE, GeneratorTheme.OCEANIC, 0),
    TERRAINCRYSTAL(
        WorldGenTerrainCrystal.class,
        GeneratorType.TERRAIN,
        GeneratorTheme.SKYFEATURE,
        Integer.MAX_VALUE
    ),
    ALTAR(
        WorldGenMiniAltar.class, GeneratorType.STRUCTURE, GeneratorTheme.PRECURSORS, -500
    ),
    //MOUNTAIN(WorldGenCrystalMountain.class,			GeneratorType.TERRAIN,
    //GeneratorTheme.CRYSTAL,			Integer.MIN_VALUE),
    CRYSTALTREE(
        WorldGenCrystalTree.class, GeneratorType.FEATURE, GeneratorTheme.CRYSTAL, -100
    ),
    GLOWBUSH(
        WorldGenLightedShrub.class, GeneratorType.FEATURE, GeneratorTheme.FOLIAGE, 500
    ),
    CRYSBUSH(
        WorldGenCrystalShrub.class, GeneratorType.FEATURE, GeneratorTheme.FOLIAGE, 500
    ),
    AURORA(
        WorldGenAurorae.class,
        GeneratorType.FEATURE,
        GeneratorTheme.SKYFEATURE,
        Integer.MAX_VALUE
    ),
    //CANYON(WorldGenSkylandCanyons.class,			GeneratorType.TERRAIN,
    //GeneratorTheme.GEOHISTORICAL,	Integer.MIN_VALUE);
    ARCH(
        WorldGenIslandArch.class,
        GeneratorType.TERRAIN,
        GeneratorTheme.OCEANIC,
        Integer.MIN_VALUE
    ),
    BLOBS(
        WorldGenTerrainBlob.class,
        GeneratorType.TERRAIN,
        GeneratorTheme.OCEANIC,
        Integer.MIN_VALUE
    ),
    GLASSCLIFFS(
        WorldGenGlassCliffs.class,
        GeneratorType.FEATURE,
        GeneratorTheme.GEOHISTORICAL,
        Integer.MIN_VALUE
    ),
    CRACKS(
        WorldGenGlowingCracks.class,
        GeneratorType.FEATURE,
        GeneratorTheme.ENERGY,
        Integer.MAX_VALUE
    ),
    CHUNKLOADER(
        WorldGenChunkloaderBlocks.class,
        GeneratorType.FEATURE,
        GeneratorTheme.PRECURSORS,
        Integer.MAX_VALUE
    ),
    GLOWCAVE(
        WorldGenGlowCave.class,
        GeneratorType.TERRAIN,
        GeneratorTheme.GEOHISTORICAL,
        Integer.MIN_VALUE
    ),
    ...
    private DimensionGenerators(
        Class<? extends ChromaWorldGenerator> c, GeneratorType t, GeneratorTheme h, int p
    ) {
        genClass = c;
        type = t;
        theme = h;
    ...
    public ChromaWorldGenerator getGenerator(Random rand, long seed) {
        if (generator == null) {
            try {
                Constructor<ChromaWorldGenerator> c = genClass.getConstructor(
                    DimensionGenerators.class, Random.class, long.class
                );
                generator = c.newInstance(this, rand, seed);
            } catch (Exception e) {
                throw new RegistrationException(
                    ChromatiCraft.instance,
                    "Could not create generator for dimension generator " + this,
                    e
                );
            }
        }
    ...
    public boolean generateIn(ChromaDimensionBiome b) {
        ...
        if (b.biomeType == Biomes.STRUCTURE || b.biomeType == Biomes.MONUMENT) {
            return ReikaRandomHelper.doWithChance(25)
                && this.generateIn(Biomes.CENTER.getBiome())
                && this.canGenerateInStructureBiome();
        }
        ...
        if (this == TREES || this == GLOWBUSH)
            return b.biomeType == Biomes.FOREST || b.biomeType == Biomes.PLAINS
                || b.getExactType() == Biomes.ISLANDS || b.biomeType == Biomes.SPARKLE
                || b.biomeType == Biomes.GLOWCRACKS;
        switch (this) {
            case ALTAR:
            ...
            case JETS:
                return true;
            case METEOR:
                return !b.getExactType().isWaterBiome()
                    && b.getExactType().isReasonablyFlat()
                    && b.biomeType != Biomes.GLOWCRACKS;
            case MIASMA:
            ...
            case CRACKS:
                return b.getExactType().isReasonablyFlat()
                    && !b.getExactType().isWaterBiome();
            case GLOWCAVE:
                return b.biomeType == Biomes.CENTER || b.biomeType == Biomes.FOREST
                    || b.getExactType() == Biomes.PLAINS;
            default:
                return true;
        }
    }
