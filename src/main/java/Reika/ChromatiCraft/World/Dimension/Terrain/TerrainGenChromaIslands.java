...
public class TerrainGenChromaIslands extends ChromaDimensionBiomeTerrainShaper {
    ...
    //private static final int MAX_ARCH_HEIGHT =
    //64+ChunkProviderChroma.VERTICAL_OFFSET+64;
    ...
    public TerrainGenChromaIslands(long seed) {
        ...
        //archHeightNoise = new SimplexNoiseGenerator(ReikaMathLibrary.cycleBitsLeft(seed,
        //16)); archLocationNoise = new
        //SimplexNoiseGenerator(ReikaMathLibrary.cycleBitsLeft(seed, 32)); archThreshNoise
        //= new SimplexNoiseGenerator(ReikaMathLibrary.cycleBitsLeft(seed, 48));
    }
    ...
    public void generateColumn(
        ...
        //3d noise map, have "blobs" of ground on surface and suspended midwater, put
        //stuff in
        ...
        if (!ocean && false) {
            double thresh =
        ReikaMathLibrary.normalizeToBounds(archThreshNoise.getValue(rx, rz), 0,
        ARCH_VAL_LIMIT); double aval = Math.abs(archLocationNoise.getValue(rx, rz));
            //ReikaJavaLibrary.pConsole(aval+" & "+thresh+":
        "+ReikaMathLibrary.approxp(aval, thresh, 0.05)); if
        (ReikaMathLibrary.approxr(aval, thresh, 0.1)) { double ay =
        ReikaMathLibrary.normalizeToBounds(archHeightNoise.getValue(rx, rz),
        MIN_ARCH_HEIGHT, MAX_ARCH_HEIGHT); ay1 = ay-2; ay2 = ay+2;
            }
        }
         */
        double min = ocean ? MIN_FLOOR_OCEAN : MIN_FLOOR_SHALLOWS;
        double max = ocean ? MAX_FLOOR_OCEAN : MAX_FLOOR_SHALLOWS;
        double f = ReikaMathLibrary.normalizeToBounds(
            floorNoise.getValue(rx * floorScale, rz * floorScale), min, max
        );
        double d = 0.25;
        if (!ocean) {
            double df = 1
                - Math.abs(bubbleNoise.getValue(rx * floorScale * d, rz * floorScale * d)
                );
            df = Math.pow(df, 6);
        ...
        for (int y = 0; y <= maxy; y++) {
            ...
            if (b == Blocks.air) {
                ...
                if (at == ChromaBlocks.GLOWLEAF.getBlockInstance()
                    || at == ChromaBlocks.GLOWLOG.getBlockInstance())
                    continue;
