...
public class TerrainGenGlowingCracks extends ChromaDimensionBiomeTerrainShaper {
    ...
    public TerrainGenGlowingCracks(long seed) {
        ...
        try (
            InputStream in
            = ChromatiCraft.class.getResourceAsStream("Textures/cracks.png")
        ) {
    ...
    public void generateColumn(
        ...
        //if (Math.abs(crackShapeA.getValue(rx, rz)) <= 0.05 ||
        //Math.abs(crackShapeB.getValue(rx, rz)) <= 0.05 ||
        //Math.abs(crackShapeC.getValue(rx, rz)) <= 0.05) {
        if (crackShape.contains(
                new Point((dx % 1500 + 1500) % 1500, (dz % 1500 + 1500) % 1500)
            )) {
            int space = 30;
            int y = 64 + ChunkProviderChroma.VERTICAL_OFFSET + space;
            int m = CRACK_DEPTH + space;
            int st = (int) ReikaMathLibrary.normalizeToBounds(
                shieldingThickness.getValue(rx, rz),
                MIN_SHIELD_THICKNESS,
                Math.min(m - 2, MAX_SHIELD_THICKNESS)
            );
            int shield = m - st;
            int dark = m
                - (int) ReikaMathLibrary.normalizeToBounds(
                    shieldingDarkThickness.getValue(rx, rz), 1, st
                );
            for (int j = 0; j <= m; j++) {
                int dy = y - j;
                this.cutBlock(world, dx, dy, dz);
                if (j >= shield
                    || (j > 1 && j < shield && rand.nextInt(Math.max(2, shield - j)) <= 1
                    )) {
                    for (int d = 2; d < 6; d++) {
                        ...
                        if (at == Blocks.stone
                            || at == ChromaBlocks.TIEREDORE.getBlockInstance()) {
                            world.setBlock(
                                ddx,
                                dy,
                                ddz,
                                ChromaBlocks.STRUCTSHIELD.getBlockInstance(),
                                j >= dark ? BlockType.CLOAK.ordinal()
                                          : BlockType.STONE.ordinal(),
                                2
                            );
                        }
                    }
            ...
            for (int d = 0; d < 6; d++)
                world.setBlock(
                    dx,
                    y - space - CRACK_DEPTH - 1 - d,
                    dz,
                    ChromatiCraft.lumen.getBlock()
                );
        } else {
            for (int y = 64 + ChunkProviderChroma.VERTICAL_OFFSET - 10;
                 y <= 64 + ChunkProviderChroma.VERTICAL_OFFSET + 10;
                 y++) {
                if (world.getBlock(dx, y, dz) == Blocks.water
                    || world.getBlock(dx, y, dz) == Blocks.flowing_water) {
