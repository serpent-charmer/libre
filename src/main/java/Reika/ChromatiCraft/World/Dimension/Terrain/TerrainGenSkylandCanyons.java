...
public class TerrainGenSkylandCanyons extends ChromaDimensionBiomeTerrainShaper {
    ...
    public void generateColumn(
        ...
        int dz = chunkZ + k;
        boolean isVoid
            = BiomeDistributor.getBiome(dx, dz).getExactType() == SubBiomes.VOIDLANDS;
        ...
        double df = ReikaMathLibrary.normalizeToBounds(
            canyonDepth.getValue(rx, rz), MIN_DEPTH, MAX_DEPTH
        );
        ...
        world.setBlock(
            dx, 64 + ChunkProviderChroma.VERTICAL_OFFSET - h - 1, dz, Blocks.grass, 0, 2
        );
        for (int di = 0; di < dt; di++) {
            world.setBlock(
                dx,
                64 + ChunkProviderChroma.VERTICAL_OFFSET - h - 2 - di,
                dz,
                Blocks.dirt,
                0,
                2
        ...
        double floor = ReikaMathLibrary.normalizeToBounds(
            bottomLowerEdge.getValue(rx, rz), LOW_FLOOR, HIGH_FLOOR
        );
        double h2 = ReikaMathLibrary.normalizeToBounds(
            bottomEdge.getValue(rx, rz), LOW_CEIL, HIGH_CEIL
        );
        if (edgeFactor < 1) {
        ...
        if (isVoid) {
            double densityControl = ReikaMathLibrary.normalizeToBounds(
                islandIntensity.getValue(rx, rz), -20, -3
            ); //more negative for less
            double islandHeight = Math.max(
                0,
                ReikaMathLibrary.normalizeToBounds(
                    bottomLowerEdge.getValue(rx, rz), densityControl, HIGH_FLOOR / 2D
                )
            );
            if (islandHeight > 0) {
