...
public class TerrainGenCrystalMountain extends ChromaDimensionBiomeTerrainShaper {
    ...
    public void generateColumn(
        ...
        int dt = (int) ReikaMathLibrary.normalizeToBounds(
            dirtNoise.getValue(rx * 8, rz * 8), 0, 4
        ); //1+rand.nextInt(4);
        ...
        if (dat.isCliff) {
            ...
            double gt = 0.75;
            double gw = ReikaMathLibrary.normalizeToBounds(
                gemNoise.getValue(rx * 2, rz * 2), 0, dc * gt
            ); //Math.max(, gemNoiseLow.getValue(rx, rz));
            double base
                = this.isFlatWorld(world) ? 3 : 64 + ChunkProviderChroma.VERTICAL_OFFSET;
            double mid = base + dat.shearThreshold
                + dc / 2; //dat.shearThreshold+dat.shearHeight/2;

            //ReikaJavaLibrary.pConsole(mid+" @ "+dx+", "+dz);
            g1 = mid - gw / 2;
            g2 = mid + gw / 2; //ReikaMathLibrary.normalizeToBounds(gemNoise2.getValue(rx,
                               //rz), shearThresh+1, val-dt-1);
            //ReikaJavaLibrary.pConsole(g1+":"+g2+" by "+mid+" @ "+dx+", "+dz+" in
            //"+(dat.shearThreshold+1)+" & "+(dat.maxHeight-dt-1));
        }
    ...
    private HeightData calcHeight(double rx, double rz) {
        ...
        double val = Math.max(
            0,
            (voff + mountainHeight.getValue(rx, rz)) * (MAX_AMPLITUDE - MAX_SHEAR)
                / (1 + voff)
        );
        double shear = ReikaMathLibrary.normalizeToBounds(
            shearNoise.getValue(rx, rz), MIN_SHEAR, MAX_SHEAR
        );
        double shearThresh = ReikaMathLibrary.normalizeToBounds(
            threshNoise.getValue(rx, rz), MIN_THRESH, MAX_THRESH
        );
        if (val >= shearThresh)
            val += shear;
    ...
    private boolean isCliff(
        ...
        return Math.abs(h - hpx) > 4 || Math.abs(h - hpz) > 4 || Math.abs(h - hnx) > 4
            || Math.abs(h - hnz) > 4;
    }
