...
public class TerrainGenSparklingSands extends ChromaDimensionBiomeTerrainShaper {
    ...
    private static final int SHORE_Y
        = 63 + ChunkProviderChroma.VERTICAL_OFFSET; // less than this == water
    private static final int LAYER_THICKNESS = 3;

    private static final ArrayList<BlockKey>[] HEIGHT_LEVELS = new ArrayList[] {
        ReikaJavaLibrary.makeListFrom(new BlockKey(
            ChromaBlocks.SPARKLE.getBlockInstance(), BlockTypes.CLAY.ordinal()
        )),
        ReikaJavaLibrary.makeListFrom(
            new BlockKey(
                ChromaBlocks.SPARKLE.getBlockInstance(), BlockTypes.SAND.ordinal()
            ),
            new BlockKey(
                ChromaBlocks.SPARKLE.getBlockInstance(), BlockTypes.GRAVEL.ordinal()
            )
        ),
        ReikaJavaLibrary.makeListFrom(
            new BlockKey(
                ChromaBlocks.SPARKLE.getBlockInstance(), BlockTypes.DIRT.ordinal()
            ),
            new BlockKey(
                ChromaBlocks.SPARKLE.getBlockInstance(), BlockTypes.STONE.ordinal()
            )
        ),
    };
    ...
    private static final int MIN_Y
        = SHORE_Y - 1 - LAYER_THICKNESS * UNDERWATER_LAYERS - 1;
    ...
    public void generateColumn(
        ...
        int h = (int
               ...
               * ReikaMathLibrary.normalizeToBounds(
                   heightNoise.getValue(rx, rz), MIN_Y, MAX_Y
               )
           + (1 - edgeFactor) * (64 + ChunkProviderChroma.VERTICAL_OFFSET));
        ...
        if (dune) {
            h += dn
                * ReikaMathLibrary.normalizeToBounds(
                    duneHeightNoise.getValue(rx, rz), MIN_DUNE_HEIGHT, MAX_DUNE_HEIGHT
                );
        }
    ...
    private void generateLandColumn(
        ...
        if (dune) {
            ...
            b = new BlockKey(
                ...
                val < 0 ? BlockTypes.SAND.ordinal() : BlockTypes.STONE.ordinal()
            );
        } else {
            if (topBlocks.size() == 1) {
                b = topBlocks.get(0);
                ...
                val = ReikaMathLibrary.normalizeToBounds(
                    val, 0, topBlocks.size() - 0.001
                );
                val = MathHelper.clamp_double(
                    val + beachTypeOffsetNoise.getValue(rx, rz),
                    0,
                    topBlocks.size() - 0.001
                );
