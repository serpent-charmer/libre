...
public class WaterFloor extends StructurePiece<WaterPuzzleGenerator> {
    ...
    public WaterFloor(
        WaterPuzzleGenerator s, int lvl, int r, WaterPath path, Random rand
    ) {
        super(s);
        level = lvl;
        gridSize = r * 2 + 1;
        ...
        for (Point p : path.solution) {
            ...
            c++;
            if (c > 2 && rand.nextInt(Math.max(2, 6 - c)) == 0 && !p.equals(path.startLoc)
                && !p.equals(path.endLoc)) {
                c = 0;
                checkpoints.add(l);
            }
        }
    ...
    public int getWidth() {
        int r = (gridSize - 1) / 2;
        return r * (2 + Lock.SIZE) + (r - 1) * 2 + 6
            + 4; //locks + gaps + center space + outer wall space
    }
    ...
    public void generate(ChunkSplicedGenerationCache world, int x, int y, int z) {
        ...
        for (int i = 0; i < flowGrid.length; i++) {
            for (int k = 0; k < flowGrid[i].length; k++) {
                flowGrid[i][k].centerLocation = new Coordinate(x, y, z).offset(
                    (i - r0) * (Lock.SIZE * 2 + 1 + 1),
                    1,
                    (k - r0) * (Lock.SIZE * 2 + 1 + 1)
                );
            }
        }
        int r = this.getWidth();
        for (int i = -r; i <= r; i++) {
            for (int k = -r; k <= r; k++) {
                for (int h = -3; h <= HEIGHT; h++) {
                    if ((Math.abs(i) >= r - 4 || Math.abs(k) >= r - 4) && Math.abs(k) < r
                        && Math.abs(i) < r
                        && (h < HEIGHT || (Math.abs(i) > r - 4 && Math.abs(k) > r - 4))) {
                            ...
                            if (h == 2)
                                if (Math.abs(i) > r - 4 && Math.abs(k) > r - 4) {
                                    world.setBlock(
                                        ...
                                        y + h,
                                        z + k,
                                        ChromaBlocks.DOOR.getBlockInstance()
                                    );
                                    doorCoordinates.add(
                                        new Coordinate(x + i, y + h, z + k)
                                    );
                                } else
                                    world.setBlock(
                                        x + i,
                                        ...
                                        z + k,
                                        ChromaBlocks.STRUCTSHIELD.getBlockInstance(),
                                        bottom ? BlockType.STONE.metadata
                        ...
                        if (Math.abs(i) % (2 * Lock.SIZE + 2) == Lock.SIZE + 1
                            || Math.abs(k) % (2 * Lock.SIZE + 2) == Lock.SIZE + 1)
                            world.setBlock(
                                x + i,
                                y + h,
                                z + k,
                            ...
                            world.setBlock(
                                ...
                                y + h,
                                z + k,
                                ChromaBlocks.WATERLOCK.getBlockInstance(),
                                1
                            ); //this is the "pads"
                ...
                for (int i = -2; i <= 2; i++) {
                    for (int k = -2; k <= 2; k++) {
                        if (Math.abs(i) == 2 || Math.abs(k) == 2 || h == HEIGHT + 4) {
                            ...
                            world.setBlock(
                                x - r + 2 + i, y + h, z - r + 2 + k, Blocks.air
                            );
                            world.setBlock(
                                x - r + 2 + i, y + h, z + r - 2 + k, Blocks.air
                            );
        ...
        for (int i = 0; i < flowGrid.length; i++) {
            for (int k = 0; k < flowGrid[i].length; k++) {
                Lock l = flowGrid[i][k];
                world.setTileEntity(
                    l.centerLocation.xCoord,
                    l.centerLocation.yCoord,
                    l.centerLocation.zCoord,
                    ChromaBlocks.WATERLOCK.getBlockInstance(),
                    0,
                    new LockCallback(
                        parent.id,
                        ...
                        k - r0,
                        l.facing,
                        checkpoints.contains(l),
                        path.endsAt(l.location),
                        l.openEnds
                    )
                );
                for (int n = 2; n < 6; n++) {
                    ...
                    world.setBlock(
                        l.centerLocation.xCoord + (Lock.SIZE + 1) * dir.offsetX,
                        y + 2,
                        l.centerLocation.zCoord + (Lock.SIZE + 1) * dir.offsetZ,
                        Blocks.air
                    );
                }
            }
        ...
        for (Point p : path.getSolution()) {
            world.setBlock(x+p.x*(Lock.SIZE*2+1+1), y+4, z+p.y*(Lock.SIZE*2+1+1),
        Blocks.emerald_block);
        }
        world.setBlock(x+path.startLoc.x*(Lock.SIZE*2+1+1), y+4,
        z+path.startLoc.y*(Lock.SIZE*2+1+1), Blocks.redstone_block);
        world.setBlock(x+path.endLoc.x*(Lock.SIZE*2+1+1), y+4,
        z+path.endLoc.y*(Lock.SIZE*2+1+1), Blocks.gold_block);
         */
        ...
        //ReikaJavaLibrary.pConsole("Level "+level+": Path from "+path.startLoc+" to
        //"+path.endLoc+", end at "+lx+","+lz);
                ...
                for (int d = 2; d < 6; d++) {
                    ...
                    if (!flowGrid[i][k].isDirectionOpen(dir)) {
                        for (int l = 1; l <= Lock.SIZE; l++) {
                            blockedChannels.add(flowGrid[i][k].centerLocation.offset(dir,
    l).offset(0, 1, 0));
                        }
                    }
                }
    ...
    public void updateFluid(World world, int i, int k, boolean fluid) {
        ...
        if (flag != flag2) {
            if (flag2)
                ChromaSounds.CASTHARMONIC.playSoundNoAttenuation(
                    world, origin.xCoord, origin.yCoord, origin.zCoord, 0.4F, 2F, 512
                );
    ...
    private static class LockCallback extends DimensionStructureTileCallback {
        ...
        public void onTilePlaced(World world, int x, int y, int z, TileEntity te) {
            if (te instanceof TileEntityRotatingLock) {
                ((TileEntityRotatingLock) te)
                    .setData(
                        direction, level, lockX, lockY, isCheckpoint, isEndpoint, ends
                    );
                ((TileEntityRotatingLock) te).uid = uid;
                //while (world.rand.nextInt(4) > 0) cannot do this since gens chunk by
                //chunk
