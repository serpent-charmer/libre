...
public class LaserPuzzleGenerator extends DimensionStructureGenerator {
    ...
    private static class LaserPuzzleStatus {
        ...
        private void checkCompletion(World world) {
            ...
            //ReikaJavaLibrary.pConsole(puzzle.getName()+":  "+puzzle.isSolved+" from
            //"+activeTargets.size()+":"+activeTargets+" & "+targets.size()+":"+targets);
            puzzle.applyDoorState(world);
            if (puzzle.isSolved) {
                AxisAlignedBB box = puzzle.getBounds().asAABB();
                List<EntityLaserPulse> li
                    = world.getEntitiesWithinAABB(EntityLaserPulse.class, box);
    ...
    public void
    completeTrigger(String level, World world, Coordinate c, boolean complete) {
