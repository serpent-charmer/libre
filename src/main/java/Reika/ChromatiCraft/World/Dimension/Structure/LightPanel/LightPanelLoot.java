    ...
    public void generate(ChunkSplicedGenerationCache world, int x, int y, int z) {
        ...
        for (int i = -WIDTH; i <= WIDTH; i++) {
            for (int k = -WIDTH; k <= WIDTH; k++) {
                ...
                int h = Math.max(
                    0,
                    WIDTH - (Math.abs(i) + Math.abs(k)) / 2
                        - (Math.abs(i) <= 1 || Math.abs(k) <= 1 ? 6 : 5)
                );
                for (int j = 0; j <= maxh; j++) {
                    ...
                    if (!wall && j > h && j < h + HEIGHT) {
                        ...
                        int m = j > h && j < maxh && !wall ? BlockType.CLOAK.metadata
                                                           : BlockType.STONE.metadata;
                        world.setBlock(
                            dx, dy, dz, ChromaBlocks.STRUCTSHIELD.getBlockInstance(), m
                        );
                        if (j == 0) {
