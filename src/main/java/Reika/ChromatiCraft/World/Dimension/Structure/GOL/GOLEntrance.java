...
public class GOLEntrance extends DynamicStructurePiece<GOLGenerator> {
    public GOLEntrance(GOLGenerator s) {
        super(s);
    }
        ...
        for (int i = -r2; i <= r2; i++) {
            for (int k = -r2; k <= r2; k++) {
                if (Math.abs(i) > 2 || Math.abs(k) > 2) {
                    ...
                    int dy = top - 1;
                    int dz = z + k;
                    int m = Math.abs(i) == r2 || Math.abs(k) == r2
    ...
    private void
    generatePrefab(World world, int x, int y, int z, Block b, int ms, int mg, int ml) {
        ...
        world.setBlock(x + 4, y + 6, z + 10, Blocks.air);
        ...
        world.setBlock(x + 5, y + 6, z + 10, Blocks.air);
