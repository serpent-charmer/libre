...
public class MazeGrid implements SegmentRestriction {
    ...
    public MazeSegment getSegment(int x, int z) {
        ...
        if (!segmentMap.containsKey(p))
            throw new ArrayIndexOutOfBoundsException(
                "Point out of bounds: x=" + x + ", z=" + z + " - Bounds: xSize=" + xSize
                + ", zSize=" + zSize
            );
    ...
    public MazeGrid subGrid(int offsetX, int offsetZ, int sizeX, int sizeZ) {
        if (!this.isInBounds(offsetX, offsetZ)
            || !this.isInBounds(offsetX + sizeX, offsetZ + sizeZ))
    ...
    public ShiftMazeDoor getDoor(Point p1, Point p2) {
        ...
        for (ShiftMazeDoor door : splittingDoors) {
            if ((door.pointDirFrom.equals(p1) && door.pointDirTo.equals(p2))
                && door.pointDirFrom.equals(p2) && door.pointDirTo.equals(p1)) {
                return door;
            }
        }
        return null;
    ...
    public static class SubGrid extends MazeGrid {
        ...
        private SubGrid(
            MazeGrid parent, int offsetSubX, int offsetSubZ, int sizeX, int sizeZ
        ) {
            super(sizeX, sizeZ);
            this.parent = parent;
            offsetX = offsetSubX;
        ...
        @Override
        public MazeGrid subGrid(int offsetX, int offsetZ, int sizeX, int sizeZ) {
            return super.subGrid(
                this.offsetX + offsetX, this.offsetZ + offsetZ, sizeX, sizeZ
            );
        }
    }
