...
public class RayBlendGenerator extends DimensionStructureGenerator {
    ...
    protected void calculate(int chunkX, int chunkZ, Random rand) {
        ...
        int extra
            = 5 + RayBlendPuzzle.PADDING_LOWER * 2 + RayBlendPuzzle.PADDING_UPPER * 2;
        ...
        for (int i = 0; i < sz.length; i++) {
            ...
            gr.addGap(
                rb.getGenerationBounds().minX + 1, rb.getGenerationBounds().maxX - 1
            );
                        ...
                        for (int dh = -1; dh <= h; dh++) {
                            if (Math.abs(i) == w || dh == -1 || dh == h) {
                                ...
                                if (doors.containsKey(odx)) {
                                    ...
                                    puzzles.get(id).addDoor(
                                        new Coordinate(odx, y + 3 + dh, z + i)
                                    );
                                } else {
    ...
    private RayBlendPuzzle createPuzzle(Random rand, PuzzleProfile p, int idx) {
        int attempts = 1;
        RayBlendPuzzle ret
            = new RayBlendPuzzle(this, idx, p.gridSize, p.initialFill, rand);
        while (!ret.prepare(p, rand)) {
            //ChromatiCraft.logger.log("Puzzle population failed; generating a new
            //rayblend puzzle");
            ret = new RayBlendPuzzle(this, idx, p.gridSize, p.initialFill, rand);
            attempts++;
            if (attempts > 40) {
                ChromatiCraft.logger.logError(
                    "Failed to generate a " + p + " puzzle in a reasonable time!"
                );
                return null;
            }
        }
        //ChromatiCraft.logger.log("Successfully generated a "+p+" puzzle in "+attempts+"
        //attempts.");
        return ret;
    }
    ...
    private PuzzleProfile[] getProfileList() {
        if (DEBUG)
            return new PuzzleProfile[] {
                new PuzzleProfile(2, 0), new PuzzleProfile(2, 0), new PuzzleProfile(2, 0),
                new PuzzleProfile(2, 0), new PuzzleProfile(2, 0), new PuzzleProfile(2, 0),
                new PuzzleProfile(2, 0), new PuzzleProfile(2, 0), new PuzzleProfile(2, 0),
                new PuzzleProfile(2, 0)
            };
        switch (ChromaOptions.getStructureDifficulty()) {
            case 1:
                return new PuzzleProfile[] { new PuzzleProfile(2, 0.5F),
                                             new PuzzleProfile(2, 0.4F, true, false),
                                             new PuzzleProfile(3, 0.4F),
                                             new PuzzleProfile(3, 0.3F, true, true),
                                             new PuzzleProfile(4, 0.3F, true, false) };
            case 2:
                return new PuzzleProfile[] { new PuzzleProfile(2, 0.5F),
                                             new PuzzleProfile(2, 0.4F, true, false),
                                             new PuzzleProfile(2, 0.25F, true, true),
                                             new PuzzleProfile(3, 0.25F),
                                             new PuzzleProfile(3, 0.2F, true, false),
                                             new PuzzleProfile(4, 0.2F, true, true) };
            case 3:
            default:
                return new PuzzleProfile[] { new PuzzleProfile(2, 0.5F),
                                             new PuzzleProfile(2, 0.4F, true, false),
                                             new PuzzleProfile(2, 0.3F, true, true),
                                             new PuzzleProfile(2, 0.2F, true, true),
                                             new PuzzleProfile(3, 0.2F),
                                             new PuzzleProfile(3, 0.15F, true, false),
                                             new PuzzleProfile(3, 0.1F, true, true),
                                             new PuzzleProfile(4, 0.2F, true, false),
                                             new PuzzleProfile(4, 0.1F, true, true) };
        }
    }
    ...
    public void onBlockUpdate(World world, int x, int y, int z, Block b) {
        ...
        if (p != null) {
            if (b instanceof CrystalTypeBlock && world.getBlockMetadata(x, y-1, z) <= 1) {
                p.addCrystal(world, CrystalElement.elements[world.getBlockMetadata(x, y+1,
    z)], x, z);
            }
            else {
