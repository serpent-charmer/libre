...
public class LightPanelGenerator extends DimensionStructureGenerator {
    private static final int[] ROW_COUNTS = {
        3, 4, 6, 6, 8, 8, 10, 10, 12, 12, 14, 16,
    };

    private static final int[] SWITCH_COUNTS = {
        3, 3, 4, 4, 5, 6, 6, 8, 8, 8, 10, 10,
    };
        ...
        for (int i = 0; i < usablePatterns.length; i++) {
            ...
            try {
                ...
                FMLCommonHandler.instance().raiseException(
                    e, "Error initializing Light Panel Puzzle", true
                );
            }
        }
    }
    ...
    private static void loadData(int tier) throws Exception {
        ...
        /*
        InputStream in =
        ChromatiCraft.class.getResourceAsStream(PATH+"/tier_"+tier+".lpuzzle"); String sg
        = ReikaFileReader.getFileAsLines(in, true).get(0); BufferedImage img =
        ImageIO.read(new ByteArrayInputStream(Base64.decodeBase64(sg)));//new
        BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB);
         */

        try (
            InputStream in
            = ChromatiCraft.class.getResourceAsStream(PATH + "/tier" + tier + ".png")
        ) {
                ...
                for (int k = 0; k < nsw; k++) {
                    for (int i = 0; i < nrw; i++) {
                        for (int l = 0; l < LightType.list.length; l++) {
                            ...
                            int value = ReikaMathLibrary.clipLeadingHexBits(
                                rgb & type.renderColor
                            );
                            if (value == 0xff) {
                                p.connect(k, i, type);
                            }
        ...
        if (usablePatterns[tier].isEmpty())
            throw new IllegalStateException(
                "This puzzle is unsolvable, as there are no valid patterns for tier "
                + tier + "!"
            );
    }
    ...
    protected void calculate(int chunkX, int chunkZ, Random rand) {
        ...
        for (int i = 0; i < levels.length; i++) {
            ...
            LightPanelRoom lpr
                = new FixedLightPanelRoom(this, r, s, i, rand, x, posY, z, p);
            lpr.generatePuzzle();
            levels[i] = lpr;
    ...
    public void toggleSwitch(
        World world, int x, int y, int z, int level, int channel, boolean active
    ) {
