...
public class DoorSection extends StructurePiece<PistonTapeGenerator> {
    ...
    public void generate(ChunkSplicedGenerationCache world, int x, int y, int z) {
        ...
        for (int d = 0; d <= len; d++) {
            for (int i = 0; i <= WIDTH; i++) {
                for (int h = 0; h < HEIGHT; h++) {
                    if (isClosed && d == len && i > 4)
                        world.setBlock(
                            x + i * left.offsetX + d * tunnelDir.offsetX,
                            y + h,
                            z + i * left.offsetZ + d * tunnelDir.offsetZ,
                            b,
                            ms
                        );
                    else
            ...
            for (int h = 1; h < HEIGHT; h++) {
                world.setBlock(
                    x + left.offsetX * WIDTH + d * tunnelDir.offsetX,
                    y + h,
                    z + left.offsetZ * WIDTH + d * tunnelDir.offsetZ,
                    b,
                    ms
                );
            }
        ...
        for (int d = 0; d <= len; d++) {
            ...
            for (int h = 1; h < HEIGHT; h++) {
                world.setBlock(
                    x + d * tunnelDir.offsetX, y + h, z + d * tunnelDir.offsetZ, b, ms
                );
                world.setBlock(
                    x + d * tunnelDir.offsetX + 1 * left.offsetX,
                    y + h,
                    z + d * tunnelDir.offsetZ + 1 * left.offsetZ,
                    b,
                    ms
                );
                world.setBlock(
                    x + d * tunnelDir.offsetX + 5 * left.offsetX,
                    y + h,
                    z + d * tunnelDir.offsetZ + 5 * left.offsetZ,
                    b,
                    ms
                );
                //world.setBlock(x+d*tunnelDir.offsetX+WIDTH*left.offsetX, y+h,
                //z+d*tunnelDir.offsetZ+WIDTH*left.offsetZ, b, ms);
            }
        }
        for (int hw = 2; hw <= 4; hw++) {
            for (int i = 1; i < HEIGHT; i++) {
                this.placeDoorBlock(
                    world, x + left.offsetX * hw, y + i, z + left.offsetZ * hw
                );
            }
        }
        ...
        for (int i = 1; i < HEIGHT; i++) {
            ...
            world.setBlock(
                x + left.offsetX * 6 + (a + 1) * tunnelDir.offsetX,
                y + i,
                z + left.offsetZ * 6 + (a + 1) * tunnelDir.offsetZ,
                b,
                m
            );
            world.setBlock(
                x + left.offsetX * 6 + (a + 3) * tunnelDir.offsetX,
                y + i,
                z + left.offsetZ * 6 + (a + 3) * tunnelDir.offsetZ,
                b,
                m
            );

            world.setBlock(
                x + left.offsetX * 5 + (a + 2) * tunnelDir.offsetX,
                y + i,
                z + left.offsetZ * 5 + (a + 2) * tunnelDir.offsetZ,
                escd,
                esc
            );
        }
        ...
        world.setBlock(
            x + left.offsetX + 2 * tunnelDir.offsetX,
            y,
            z + left.offsetZ + 2 * tunnelDir.offsetZ,
            b,
        ...
        world.setBlock(
            ...
            z + left.offsetZ + 2 * tunnelDir.offsetZ,
            b,
            BlockType.CRACK.metadata
        );

        //world.setBlock(x+left.offsetX*WIDTH+2*tunnelDir.offsetX, y+2,
        //z+left.offsetZ*WIDTH+2*tunnelDir.offsetZ, b, BlockType.LIGHT.metadata);

        Coordinate door
            = new Coordinate(x + left.offsetX * 3, y + 1, z + left.offsetZ * 3);
        for (int i = 0; i < level.bitsPerDoor; i++) {
            this.placeTarget(
                world,
                x + left.offsetX * 5 + (i - level.bitsPerDoor / 2) * tunnelDir.offsetX,
                y + 2,
                z + left.offsetZ * 5 + (i - level.bitsPerDoor / 2) * tunnelDir.offsetZ,
                i,
                door
            );
        }
    ...
    private void placeTarget(
        ChunkSplicedGenerationCache world, int x, int y, int z, int idx, Coordinate door
    ) {
        ...
        world.setTileEntity(
            ...
            y,
            z,
            ChromaBlocks.PISTONTARGET.getBlockInstance(),
            1,
            new DoorTargetCallback(
                doorData.getValue(idx),
                level,
                index,
                ReikaDirectionHelper.getRightBy90(tunnelDir),
                door,
                parent.id
            )
        );
    ...
    private void placeDoorBlock(ChunkSplicedGenerationCache world, int x, int y, int z) {
        ...
        //world.setTileEntity(x, y, z, ChromaBlocks.COLORLOCK.getBlockInstance(), 0, new
        //DoorKeySet(parent, level.ordinal(), parent.id, elements));
    ...
    private static class DoorTargetCallback extends DimensionStructureTileCallback {
        ...
        private DoorTargetCallback(
            DoorValue d, TapeStage s, int step, ForgeDirection dir, Coordinate c, UUID uid
        ) {
            id = uid;
            data = d;
            door = c;
        ...
        public void onTilePlaced(World world, int x, int y, int z, TileEntity te) {
            ...
            ((PistonDoorTile) te)
                .setData(
                    direction, stage, doorIndex, data.index, data.getParent().colorCount
                );
