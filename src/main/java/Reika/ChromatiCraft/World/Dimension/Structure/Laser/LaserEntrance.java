...
public class LaserEntrance extends DynamicStructurePiece<LaserPuzzleGenerator> {
    ...
    public void generate(World world, int x, int z) {
        ...
        for (int i = -r; i <= r; i++) {
            for (int k = -r; k <= r; k++) {
                ...
                for (int j = 2; j <= 7; j++) {
                    ...
                    boolean wall
                        = Math.abs(i) == r || Math.abs(k) == r || j == 2 || j == 7;
                    boolean wall2 = Math.abs(i) == Math.abs(k) && Math.abs(i) == r / 2;
                    wall = wall || wall2;
                    wall = wall
                        && (i != r || j > 6 || j == 2 || Math.abs(k) > (j == 6 ? 1 : 2));
                    if (wall) {
                        if (wall2 && j == 4)
                            world.setBlock(
                                dx,
        ...
        for (int r2 = 3; r2 <= 6; r2++) {
            ...
            for (int i = -r2; i <= r2; i++) {
                world.setBlock(
                    x + i, dy, z - r2, ChromaBlocks.STRUCTSHIELD.getBlockInstance(), m, 3
                );
                world.setBlock(
                    x + i, dy, z + r2, ChromaBlocks.STRUCTSHIELD.getBlockInstance(), m, 3
                );
                world.setBlock(
                    x - r2, dy, z + i, ChromaBlocks.STRUCTSHIELD.getBlockInstance(), m, 3
                );
                world.setBlock(
                    x + r2, dy, z + i, ChromaBlocks.STRUCTSHIELD.getBlockInstance(), m, 3
                );
