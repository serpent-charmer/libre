...
public class MazeSegmentGraph {
    ...
    public MazeSegmentNode createOrGetNode(int id) {
        if (segmentNodes.containsKey(id))
    ...
    public void connect(MazeSegmentNode segNode, MazeSegmentNode otherNode) {
        if (!segNode.connectionDoors.containsKey(otherNode))
            segNode.connectionDoors.put(
                otherNode, new LinkedList<MazeGrid.ShiftMazeDoor>()
            );
        if (!otherNode.connectionDoors.containsKey(segNode))
            otherNode.connectionDoors.put(
                segNode, new LinkedList<MazeGrid.ShiftMazeDoor>()
            );
    }

    //Needs to be done in case they COULD connect, but can't/didn't connect for some
    //reason.
    ...
    public void pushNewDoor(MazeCalculator.BufferedDoorState emptyDoor) {
        MazeGrid.ShiftMazeDoor door
            = new MazeGrid.ShiftMazeDoor(emptyDoor.from, emptyDoor.to, emptyDoor.dir);
