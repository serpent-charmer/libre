...
public class LocksEntrance extends DynamicStructurePiece<LocksGenerator> {
        ...
        for (int i = -radius; i <= radius; i++) {
            ...
            for (int k = -radius; k <= radius; k++) {
                ...
                for (int j = 0; j <= h; j++) {
                    ...
                    boolean enter
                        = step * sign == -radius && j > 0 && j < h - 1 && d < radius - 1;
                    boolean wall = !enter
                        && (Math.abs(i) == radius || Math.abs(k) == radius || j == 0
                            || j == h);
                    boolean window = j == h
                        && (radius == 6
                                ? d == 1
                                : h == this.getHeight(0) - 1 && h == hm && h == hp)
                        && d2 > 0 && d2 < radius - 1;
                    world.setBlock(
                        dx,
                        dy,
                        ...
                        wall ? bk : Blocks.air,
                        wall
                            ? window ? BlockType.GLASS.metadata : BlockType.STONE.metadata
                            : 0,
                        3
                    );
                }
    ...
    private void generateTunnel(World world, int x, int y, int z) {
        ...
        int d = y - posY;
        LockRoomConnector tunnel = new LockRoomConnector(parent, 0, 0, 0, 0)
                                       .setWindowed()
                                       .setLength(facing.getOpposite(), length)
                                       .setOpenFloor(d - 5);
        tunnel.generate(
            new ChunkSplicedGenerationCache.RelayCache(world), dx, posY + d, dz
        );
    }
}
