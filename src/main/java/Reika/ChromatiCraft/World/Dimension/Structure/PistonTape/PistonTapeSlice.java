...
public class PistonTapeSlice extends StructurePiece<PistonTapeGenerator> {
    ...
    protected PistonTapeSlice(
        PistonTapeGenerator g,
        ForgeDirection dir,
        int idx,
        PistonTapeLoop p,
        LoopDimensions size
    ...
    private void genRandomColor(int i, Random rand) {
        ...
        } else if (bitColors[i2] == null) { //no wrap, but this position was set before as
                                            //i2 from an earlier bit
        ...
        //ReikaJavaLibrary.pConsole("At position "+i+" with offset value @ "+i2+", made
        //"+netColors[i]+" from "+bitColors[i]+" and "+bitColors[i2]);
    }
    ...
    public void generate(ChunkSplicedGenerationCache world, int x, int y, int z) {
        for (int d = -1; d <= dimensions.totalDepth + 2; d++) { //was from -2
            for (int h = -2; h <= dimensions.totalHeight + 2; h++) {
                ...
                if (b == ChromaBlocks.STRUCTSHIELD.getBlockInstance()
                    && d == dimensions.totalDepth / 2) {
                    m = BlockType.LIGHT.metadata;
                }
                world.setBlock(
                    x - d * facing.offsetX, y + h, z - d * facing.offsetZ, b, m
                );
            }
        }
        ...
        this.placePiston(
            world,
            x + facing.offsetX,
            y + dimensions.totalHeight,
            z + facing.offsetZ,
            facing.getOpposite()
        );
        this.placePiston(
            world,
            x - facing.offsetX * dimensions.totalDepth,
            y + dimensions.totalHeight + 1,
            z - facing.offsetZ * dimensions.totalDepth,
            ForgeDirection.DOWN
        );
        this.placePiston(
            world,
            x - facing.offsetX * (dimensions.totalDepth + 1),
            y,
            z - facing.offsetZ * (dimensions.totalDepth + 1),
            facing
        );
    ...
    private void generateTape(ChunkSplicedGenerationCache world, int x, int y, int z) {
        ...
        dimensions.calculatePositions(
            zeroBit.xCoord, zeroBit.yCoord, zeroBit.zCoord, 1, facing, true
        );
        ...
        if (parent.DEBUG) {
            for (int i = 0; i < dimensions.bitLength; i++) {
                ...
                world.setBlock(
                    ...
                    c.yCoord,
                    c.zCoord,
                    ChromaBlocks.PISTONBIT.getBlockInstance(),
                    m
                );
                if (bitColors[i] != null && !bitColors[i].isWhite()) {
                    ReikaJavaLibrary.pConsole(
                        "Slice " + busIndex + " @ " + i + ": " + bitColors[i] + " > " + m
                    );
                }
            }
        } else {
            for (int d = 0; d <= dimensions.totalDepth; d++) {
                for (int h = 0; h <= dimensions.totalHeight; h++) {
                    if (d == 0 || d == dimensions.totalDepth || h == 0
                        || h == dimensions.totalHeight) {
                        if (d == 0 && h == dimensions.totalHeight)
                            continue;
                        if (d == dimensions.totalDepth && h == 0)
                            continue;
                        this.placeBit(
                            world, x - d * facing.offsetX, y + h, z - d * facing.offsetZ
                        );
                    }
                }
            }
    ...
    private void placePiston(
        ChunkSplicedGenerationCache world, int x, int y, int z, ForgeDirection dir
    ) {
    ...
    private void placeEmitter(ChunkSplicedGenerationCache world, int x, int y, int z) {
        ...
        world.setTileEntity(
            ...
            y,
            z,
            ChromaBlocks.LASEREFFECT.getBlockInstance(),
            LaserEffectType.EMITTER.ordinal(),
            new PistonEmitterCallback(this)
        );
    }
    ...
    private void placeTarget(ChunkSplicedGenerationCache world, int x, int y, int z) {
        ...
        world.setTileEntity(
            ...
            0,
            new PistonTargetCallback(this, loop.level)
        );
    }
    ...
    static class LoopSliceDimensions extends LoopDimensions {
        ...
        public void calculatePositions(
            ...
            boolean skipFirstAndThirdCorners
        ) {
