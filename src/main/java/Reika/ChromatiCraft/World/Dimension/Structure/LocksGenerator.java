...
public class LocksGenerator extends DimensionStructureGenerator {
    ...
    protected void calculate(int x, int z, Random rand) {
        ...
        ForgeDirection dir
            = ForgeDirection
                  .SOUTH; //ReikaDirectionHelper.getRandomDirection(false, rand);
        ...
        new LockRoomConnector(this, 0, 0, 0, 0)
            .setLength(dir, len2)
            .setOpenCeiling()
            .generate(world, x, posY, z);
        ...
        new LocksLoot(this, flip)
            .generate(world, c.xCoord + dx, c.yCoord, c.zCoord + dz); //6 was 7
    }
    ...
    private Coordinate genRooms(int x, int y, int z, ForgeDirection dir, Random rand) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            dx += l.getEnterExitDL() * dir2.offsetX
                + l.getEnterExitDT() * Math.abs(turn.offsetX);
            dz += l.getEnterExitDL() * dir2.offsetZ
                + l.getEnterExitDT() * Math.abs(turn.offsetZ);
    ...
    private LockLevel genNewLockLevel(int i, Random rand) {
        try {
            ...
            throw new RegistrationException(
                ChromatiCraft.instance, "Could not instantiate lock level " + i + "!"
            );
        }
    }
        ...
        for (Coordinate loc : lockCache) {
            ...
            if (te == null) {
                ChromatiCraft.logger.logError(
                    "Colored lock @ " + loc + " in DIM" + world.provider.dimensionId
                    + " has no TileEntity!!"
                );
                Block b = loc.getBlock(world);
                ReikaJavaLibrary.pConsole(
                    "Present block ID: " + Block.getIdFromBlock(b) + " = " + b.getClass()
                );
