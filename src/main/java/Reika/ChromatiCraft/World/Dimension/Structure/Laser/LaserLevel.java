...
public class LaserLevel extends StructurePiece<LaserPuzzleGenerator>
    implements PlacementCallback, NBTCallback {
    ...
    public LaserLevel(LaserPuzzleGenerator s, String name) {
        ...
        data = new StructureExport(name, PATH, ChromatiCraft.class)
                   .addIgnoredBlock(new BlockKey(Blocks.stone))
                   .addIgnoredBlock(new BlockKey(Blocks.air));
        this.name = name;
        try {
            ...
            throw new RuntimeException(
                "Could not load structure data for laser level '" + name + "'", e
            );
        }
    }
    ...
    public void generate(ChunkSplicedGenerationCache world, int x, int y, int z) {
        ...
        for (int i = 0; i < r.length; i++) {
            ...
            int rb = i >= 4 ? r2 : r1;
            BlockBox box
                = data.getBounds().expand(r1, 0, rb).offset(new Coordinate(0, i - 1, 0));
            for (int dx = def.minX - w; dx <= def.maxX + w; dx++) {
                for (int dz = def.minZ - w2; dz <= def.maxZ + w2; dz++) {
                    int dy = y + i - 1;
                    boolean wall
                        = i == 0 || i == r.length - 1 || !box.isBlockInside(dx, dy, dz);
                    if (wall) {
                        world.setBlock(
                            dx,
                            dy,
        ...
        for (int dx = def.minX-w; dx <= def.maxX+w; dx++) {
            for (int dz = def.minZ-w; dz <= def.maxZ+w; dz++) {
                ...
                if (bk != null && bk.blockID ==
        ChromaBlocks.STRUCTSHIELD.getBlockInstance()) { BlockKey bk2 = world.getBlock(dx,
        dy+1, dz); if (bk2 == null) { boolean flag = true; for (int i = 2; i <= 6; i++) {
        ...
        for (int dx = def.minX - w + 1; dx <= def.maxX + w - 1; dx++) {
            for (int dz = def.minZ - w2 + 2; dz <= def.maxZ + w2 - 2; dz++) {
                if (dx != def.minX - w && dx != def.maxX + w && dz != def.minZ - w2
                    && dz != def.maxZ + w2) {
                    boolean flag = false;
                    ...
                    if (Math.abs(dz - z) <= 1
                        && (dx == def.minX - w + 1 || dx == def.maxX + w - 1)) {
                        flag = true;
                    }
                    if (flag) {
    ...
    public NBTBase getOverriddenValue(
        Coordinate c, BlockKey bk, String key, NBTBase original, NBTTagCompound data
    ) {
        if (key.equals("dir")) {
            if (!data.getBoolean("fixed")
                && !LaserEffectType.list[bk.metadata].isOmniDirectional()
                && (BlockLaserEffector.LaserEffectTile.PARTIAL_ROTATEABILITY
                        ? data.getBoolean("free")
                        : bk.metadata != LaserEffectType.EMITTER.ordinal()
                            && bk.metadata != LaserEffectType.TARGET.ordinal()
                            && bk.metadata != LaserEffectType.TARGET_THRU.ordinal())) {
    ...
    public void applyDoorState(World world) {
        TileEntityChromaDoor te
            = (TileEntityChromaDoor) doorLocation.getTileEntity(world);
        if (te == null) {
            ChromatiCraft.logger.log(
                "No door for laser puzzle room " + name + " @ " + doorLocation + "?"
            );
            return;
        }
        if (isSolved) {
