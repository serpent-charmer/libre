    ...
    public void calculateMazeSegment(List<Point> pointsInSegment) {
        ...
        for (int xx = 0; xx < grid.getXSize(); xx++) {
            for (int zz = 0; zz < grid.getZSize(); zz++) {
                ...
                if (!pointsInSegment.contains(at))
                    continue;
                ...
                if (!associated.containsKey(at))
                    associated.put(at, new LinkedList<Point>());

                if (pointsInSegment.contains(southAt)
                    && grid.isInBounds(southAt.x, southAt.y)
                    && grid.isFree(southAt.x, southAt.y)
                    && !grid.getSegment(southAt.x, southAt.y).specialLock) {
                    cons.add(new GridConnection(at, southAt, ForgeDirection.SOUTH));
                    if (!associated.containsKey(southAt))
                        associated.put(southAt, new LinkedList<Point>());
                }
                if (pointsInSegment.contains(eastAt)
                    && grid.isInBounds(southAt.x, southAt.y)
                    && grid.isFree(eastAt.x, eastAt.y)
                    && !grid.getSegment(eastAt.x, eastAt.y).specialLock) {
                    cons.add(new GridConnection(at, eastAt, ForgeDirection.EAST));
                    if (!associated.containsKey(eastAt))
                        associated.put(eastAt, new LinkedList<Point>());
                }
            }
        }
        ...
        for (GridConnection connection : cons) {
            ...
            if (associated.get(from).contains(to))
                continue;
            ...
            grid.getSegment(to.x, to.y)
                .addConnection(connection.fromToDirection.getOpposite());
        }
    }
