...
public class PistonTapeStaircase extends StructurePiece<PistonTapeGenerator> {
    ...
    public PistonTapeStaircase(
        PistonTapeGenerator gen, TapeStage s1, TapeStage s2, Random r
    ) {
        super(gen);
        ...
        for (Coordinate c : set) {
            ...
            for (Coordinate c2 : c.getAdjacentCoordinates()) {
                if (!set.contains(c2)) {
                    ...
                    m = rand.nextInt(12) == 0 && set.contains(c.offset(0, 1, 0))
                            && set.contains(c.offset(0, -1, 0))
