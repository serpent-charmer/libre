...
public class MazeRoom extends StructurePiece {
    ...
    public void generate(
        ChunkSplicedGenerationCache world, int x, int y, int z
    ) { //XYZ is of origin cell

        MultiMap<Coordinate, ForgeDirection> openCells
                        ...
                        if (j == 0) {
                            ...
                            if (bk != null && bk.blockID == Blocks.air) {
                                world.setBlock(
                                    ...
                                    dy,
                                    dz,
                                    ChromaBlocks.HOVER.getBlockInstance(),
                                    HoverType.DAMPER.getPermanentMeta()
                                );
        ...
        if (rand.nextInt(Math.max(1, 3 - cellRadius)) == 0) {
            String s = rand.nextBoolean() ? ChestGenHooks.DUNGEON_CHEST
                                          : ChestGenHooks.PYRAMID_DESERT_CHEST;
            ItemStack is = rand.nextBoolean()
                ? ChromaStacks.getChargedShard(CrystalElement.randomElement())
                : ChromaStacks.rawCrystal.copy();
            int m = 2;
            ...
            parent.generateLootChest(
                ...
                y + size / 2,
                z + size / 2,
                ReikaDirectionHelper.getRandomDirection(false, rand),
                s,
                rand.nextInt(1 + cellRadius),
                is,
                100
            );
        }
        ...
        if (cellRadius == 2) {
            if (rand.nextBoolean()
                && !openCells.get(new Coordinate(0, 0, 0))
                        .contains(ForgeDirection.DOWN)) { //blocks center down
                for (int i = -2; i <= size + 2; i++) {
                    for (int k = -2; k <= size + 2; k++) {
                        ...
                        if (i == -2 || i == size + 2 || k == -2 || k == size + 2) {
                            ...
                            world.setBlock(
                                dx, dy, dz, ChromaBlocks.CHROMA.getBlockInstance()
                            );
