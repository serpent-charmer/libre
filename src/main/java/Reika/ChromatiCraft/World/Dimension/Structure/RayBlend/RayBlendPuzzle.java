...
public class RayBlendPuzzle extends StructurePiece<RayBlendGenerator> {
    ...
    private boolean isNotUniform(Random rand) {
        ...
        for (Subgrid g : grids.values()) {
            //ReikaJavaLibrary.pConsole("Comparing "+g.slots.keySet()+" and
            //"+g1.slots.keySet()+": "+g1.isCongruent(g));
            if (!g1.isCongruent(g)) {
                return true;
            }
        }
    ...
    private boolean populate(Subgrid sg, Random rand) {
        ...
        while (attempts0 < 80) {
            ...
            boolean valid = unGriddedPoints.size() == totalCellCount
                || this.canExpandInto(sg, start);
            if (valid) {
        ...
        while (attempts < 80) {
            ...
            if (sg.size() == cellsPerSubgrid
                && sg.getStringiness() <= this.getMaxAllowedStringiness()) {
                return true;
            }
        }
        return false;
    ...
    private boolean canExpandInto(
        Subgrid sg, Point p
    ) { //determines whether the cell splits the remaining cells into 2+ isolated regions
        ...
        if (groups.size() == 1) {
            ...
            //ReikaJavaLibrary.pConsole("Split detected; group list of size
            //"+groups.size()+": "+groups);
    ...
    private void randomize(PuzzleProfile p, Random rand) {
        while (!unfinished.isEmpty()) {
            ...
            Subgrid g = ReikaJavaLibrary.getRandomCollectionEntry(
                rand, unfinished
            ); //g.slots[rand.nextInt(gridSize)][rand.nextInt(gridSize)];
            if (g.unpopulated.isEmpty()) {
                unfinished.remove(g);
            } else {
                GridSlot slot = ReikaJavaLibrary.getRandomCollectionEntry(
                    rand, g.unpopulated
                ); //g.slots[rand.nextInt(gridSize)][rand.nextInt(gridSize)];
        ...
        if (p.allowCaging) {
            while (!uncaged.isEmpty()) {
                ...
                while (!dirs.isEmpty()) {
                    ForgeDirection dir
                        = ReikaJavaLibrary.getRandomCollectionEntry(rand, dirs);
                    dirs.remove(dir);
            ...
            if (!cages.isEmpty()) {
                GridCage cage
                    = ReikaJavaLibrary.getRandomCollectionEntry(rand, cages.values());
                GridSlot gs = ReikaJavaLibrary.getRandomCollectionEntry(rand, cage.slots);
                gs.appearsAtStart = true; //force at least one cage to have at least one
                                          //crystal per puzzle
            }
        }
        ...
        while (n > 0 && !candidateStarts.isEmpty()) {
            GridSlot gs
                = ReikaJavaLibrary.getRandomCollectionEntry(rand, candidateStarts);
    ...
    private CrystalMix getValidMix(Point p, Random rand) {
        ...
        GridSlot g2 = this.getRandomSlotExcluding(
            rand, p, g1.positionKey()
        ); //this.getAt(i, p.y);
        int n = 0;
    ...
    private CrystalMix
    getValidMix(Point p, CrystalElement base, GridSlot g1, GridSlot g2) {
    ...
    private boolean canCage(GridSlot slot, GridSlot slot2) {
        return slot.color != null && slot2.color != null
            && ElementMixer.instance.isMixable(slot.color, slot2.color);
    }

    private boolean pickRandomColorForSlot(Random rand, GridSlot gs) {
        HashSet<CrystalElement> set
            = ReikaJavaLibrary.makeSetFromArray(CrystalElement.elements);
        ...
        if (set.isEmpty()) {
            ...
            gs.isGoal
                = false; //rand.nextInt(20) == 0 && gs.color != CrystalElement.BROWN;
    ...
    private void colorExclusionPing(World world, CrystalElement e, GridSlot gs) {
        spawnPingParticle(
            world, e, gs.getWorldX(), generatorOrigin.yCoord + 2, gs.getWorldZ()
        );
    }

    public static void
    spawnPingParticle(World world, CrystalElement e, int x, int y, int z) {
        if (world.isRemote) {
            doPingParticle(world, e, x, y, z);
        } else {
            ReikaPacketHelper.sendDataPacket(
                ChromatiCraft.packetChannel,
                ChromaPackets.RAYBLENDPING.ordinal(),
                world,
                x,
                y,
                z,
                new RadiusTarget(world, x, y, z, 32),
                e.ordinal()
            );
        }
    }
    ...
    private static void
    doPingParticle(World world, CrystalElement e, int x, int y, int z) {
    ...
    public void tick(World world) {
        for (CrystalMix mix : mixPoints.values()) {
            mix.sendParticles(
                world,
                generatorOrigin.xCoord,
                generatorOrigin.yCoord + 2,
                generatorOrigin.zCoord
            );
        }
    }
    ...
    public boolean allowsCrystalAt(World world, int x, int z, CrystalElement e) {
        ...
        return gs.parent.countColorPresence(world, e) == 0
            && !this.rowOrColHas(dx, dz, e);
    }
    ...
    public void generate(ChunkSplicedGenerationCache world, int x, int y, int z) {
        ...
        if (!RayBlendGenerator.DEBUG) {
            ...
            for (int i = min1; i < max1; i++) {
                for (int k = min1; k < max1; k++) {
                    ...
                    world.setBlock(
                        dx, y + 1, dz, ChromaBlocks.STRUCTSHIELD.getBlockInstance(), m
                    );
                }
            ...
            for (int i = min2; i < max2; i++) {
                for (int k = min2; k < max2; k++) {
                    ...
                    if (i <= min1 || i >= max1 || k <= min1 || k >= max1) {
                        ...
                        boolean edgeI
                            = i == min1 || i == max1 || i == max2 - 1 || i == min2 + 1;
                        boolean edgeK
                            = k == min1 || k == max1 || k == max2 - 1 || k == min2 + 1;
                        if ((edgeI && edgeK)
                            || (dx == MathHelper.floor_float(midX) && edgeK)
                            || (dz == MathHelper.floor_float(midZ) && edgeI)
                                && m == BlockType.STONE.metadata)
            ...
            for (int k = 0; k < edgeLength; k++) {
                ...
                if (RayBlendGenerator.DEBUG) {
                    world.setTileEntity(
                        ...
                        y + 1,
                        dz,
                        ChromaBlocks.RAYBLEND.getBlockInstance(),
                        0,
                        new RayblendFloorCallback(
                            parent.id,
                            ID,
                            gs.parent.ID,
                            gs.xPos,
                    ...
                    if (gs.isBlocked) {
                        ...
                        //ChromaBlocks.STRUCTSHIELD.getBlockInstance(),
                        //BlockType.LIGHT.metadata); world.setBlock(dx, y+1, dz,
                        //ChromaBlocks.STRUCTSHIELD.getBlockInstance(),
                        //BlockType.GLASS.metadata); world.setBlock(dx, y+2, dz,
                        //ChromaBlocks.STRUCTSHIELD.getBlockInstance(),
                        //BlockType.GLASS.metadata);
                        world.setBlock(
                            dx,
                            y,
                            dz,
                            ChromaBlocks.DIMGEN.getBlockInstance(),
                            DimDecoTypes.LIFEWATER.ordinal()
                        );
                        world.setTileEntity(
                            dx,
                            y + 1,
        ...
        for (GridCage g : cages.values()) {
            for (GridSlot gs : g.slots) {
                world.setBlock(gs.getWorldX(), y+1, gs.getWorldZ(),
        ChromaBlocks.GLASS.getBlockInstance(), g.blendedColor.ordinal());
            }
        }*/
    ...
    public void generateBypass(ChunkSplicedGenerationCache world) {
        if (bypassCoord != null) {
            for (int i = 1; i <= 4; i++) {
                world.setBlock(
                    bypassCoord.xCoord,
                    bypassCoord.yCoord + i,
                    bypassCoord.zCoord - 1,
                    ChromaBlocks.STRUCTSHIELD.getBlockInstance(),
                    BlockType.STONE.metadata
                );
                world.setBlock(
                ...
                if (bc == ChromaBlocks.DIMDATA.getBlockInstance())
                    parent.generatePasswordTile(
                        bypassCoord.xCoord, bypassCoord.yCoord + i, bypassCoord.zCoord
                    );
                else
                    world.setBlock(
                        bypassCoord.xCoord,
            ...
            for (int i = 1; i <= 4; i++) {
                Block b
                    = i == 4 ? ChromaBlocks.STRUCTSHIELD.getBlockInstance() : Blocks.air;
                int m = i == 4 ? BlockType.STONE.metadata : 0;
                for (int d = -3; d <= 3; d++) {
                    world.setBlock(
                        bypassCoord.xCoord - 1,
                        bypassCoord.yCoord + i,
                        bypassCoord.zCoord + d,
                        b,
                        m
                    );
                }
    ...
    public boolean containsCrystalPosition(int x, int y, int z) {
         ...
         */
        return y == generatorOrigin.yCoord + 2
            && this.getAt(x - generatorOrigin.xCoord, z - generatorOrigin.zCoord) != null;
    }
    ...
    private static class Subgrid implements CloneCallback<Point> {
        ...
        public boolean isCongruent(Subgrid g) {
            ...
            if (g.slots.size() != slots.size())
                return false;
            Collection<Point> set1 = new ArrayList(
                ReikaJavaLibrary.cloneCollectionObjects(slots.keySet(), this)
            );
            Collection<Point> set2 = new ArrayList(
                ReikaJavaLibrary.cloneCollectionObjects(g.slots.keySet(), this)
            );
        ...
        private void rotatePoints(Collection<Point> set) {
            for (Point p : set) {
                Vec3 r = ReikaVectorHelper.rotateVector(
                    Vec3.createVectorHelper(p.x, 0, p.y), 0, 90, 0
                );
    ...
    private static class GridCage {
        ...
        public boolean canAbsorb(GridSlot gs) {
            return gs.color != null && !gs.isBlocked
                && ElementMixer.instance.isMixable(blendedColor, gs.color);
        }
    ...
    private static class GridSlot {
        ...
        @Override
        public String toString() {
            return "[" + xPos + ", " + zPos + "]; " + color + "/" + currentCrystal + "; "
                + isBlocked + "/" + isGoal + "/" + appearsAtStart;
        }
    }
        ...
        public void sendParticles(World world, int x0, int y0, int z0) {
            if (world.rand.nextInt(5) == 0)
                ReikaPacketHelper.sendPositionPacket(
                    ChromatiCraft.packetChannel,
                    ChromaPackets.RAYBLENDMIX.ordinal(),
                    world,
                    position.x + x0 + 0.5,
                    y0 + 0.5,
                    position.y + z0 + 0.5,
                    new PacketTarget.RadiusTarget(world, x0, y0, z0, 32),
                    color.ordinal(),
                    1
                );
            if (world.rand.nextInt(5) == 0)
            ...
            //this.doParticle(world, position.x+x0+0.5, y0+0.5, position.y+z0+0.5, color,
            //true); this.doParticle(world, pos1.xPos+x0+0.5, y0+0.5, pos1.zPos+z0+0.5,
            //pos1.color, false); this.doParticle(world, pos2.xPos+x0+0.5, y0+0.5,
            //pos2.zPos+z0+0.5, pos2.color, false);
        }

        @SideOnly(Side.CLIENT)
        public static void doParticle(
            World world, double x, double y, double z, CrystalElement e, boolean mix
        ) {
            if (mix) {
    ...
    private static class RayblendFloorCallback extends DimensionStructureTileCallback {
        ...
        public RayblendFloorCallback(
            UUID p, UUID id, UUID grid, int x, int z, boolean blocked, GridCage gc
        ) {
            parent = p;
            uid = id;
            this.grid = grid;
        ...
        public void onTilePlaced(World world, int x, int y, int z, TileEntity te) {
            ...
            ((TileEntityRayblendFloor) te)
                .populate(uid, grid, xPos, zPos, isBlocked, color);
        }
    }
}