...
public class AntFarmTunnel extends StructurePiece<AntFarmGenerator> {
    ...
    public AntFarmTunnel(
        AntFarmGenerator a, Coordinate c1, Coordinate c2, int curve, int r
    ) {
        super(a);
    ...
    private void initialize() {
        ...
        for (DecimalPosition p : li) {
            this.generateTunnelSection(
                MathHelper.floor_double(p.xCoord),
                MathHelper.floor_double(p.yCoord),
                MathHelper.floor_double(p.zCoord),
                tunnelRadius
            );
        }
    }
            ...
            for (int j = -r; j <= r; j++) {
                for (int k = -r; k <= r; k++) {
                    ...
                    if (dd <= r + 0.5) {
                        ...
                        blocks.put(
                            c, (pre != null && pre.booleanValue()) || dd <= r - 0.5
                        );
                    }
                }
            }
    ...
    public boolean
    intersectsWith(HashSet<Coordinate> tunnelSpaces, HashSet<Coordinate> levelSpaces) {
        for (Coordinate c : blocks.keySet()) {
