...
public class ThreeDMazeGenerator extends DimensionStructureGenerator {
    ...
    private final MultiMap<Coordinate, ForgeDirection> locationCache
        ...
        for (int i = 0; i < MAX_SIZE_X; i++) {
            for (int j = 0; j < MAX_SIZE_Y; j++) {
                for (int k = 0; k < MAX_SIZE_Z; k++) {
                    ...
                    Collection<ForgeDirection> dirs
                        = locationCache.get(new Coordinate(i, j, k));
                    ...
                    if (i % ndl == 0 && j % (ndl / 2) == 0 && k % ndl == 0)
                        p.setLighted();
                    if (i != 0 && j != 0 && k != 0 && i != MAX_SIZE_X - 1
                        && j != MAX_SIZE_Y - 1 && k != MAX_SIZE_Z - 1
                        && rand.nextInt(10) == 0) {
    ...
    private void cutExits(Random rand) {
        locationCache.addValue(
            new Coordinate(MAX_SIZE_X / 2, MAX_SIZE_Y - 1, MAX_SIZE_Z / 2),
            ForgeDirection.UP
        );
        locationCache.addValue(
            new Coordinate(MAX_SIZE_X / 2, 0, MAX_SIZE_Z / 2), ForgeDirection.DOWN
        );
    }

    /**
     * Turns it from a simply connected "perfect" maze to a multiply connected "braid"
     * maze; adds significant difficulty
     */
    private void cutExtras(Random rand) {
        ...
        for (int i = 0; i < n; i++) {
            Coordinate c = new Coordinate(
                rand.nextInt(MAX_SIZE_X),
                rand.nextInt(MAX_SIZE_Y),
                rand.nextInt(MAX_SIZE_Z)
            );
    ...
    private void addRooms(Random rand) {
        ...
        for (int a = 0; a < n; a++) {
            ...
            int d = 4 + s;
            int tries = 0;
            while ((c.getTaxicabDistanceTo(end) < d || c.getTaxicabDistanceTo(start) < d)
                   && tries < 50) {
    ...
    private void generatePathFrom(int x, int y, int z, Random rand) {
        ...
        //this.stepPath(x, y, z, rand, this.getMovementDirection(x, y, z,
        //ForgeDirection.DOWN, rand));
    ...
    private void stepPath(int x, int y, int z, Random rand, ForgeDirection dir) {
        ...
        if (this.isFull()) {
            //ReikaJavaLibrary.pConsole("Have "+coordCache.size()+" points; is full;
            //returning from "+dir+" from "+x+", "+y+", "+z);
            pathLocs.removeLast();
            return;
        } else if (this.hasUnvisitedNeighbors(x, y, z)) {
            dir = this.getMovementDirection(x, y, z, rand);
            //ReikaJavaLibrary.pConsole("Have "+coordCache.size()+" points; stepping
            //forward "+dir+" from "+x+", "+y+", "+z);
            locationCache.addValue(c, dir);
            ...
            //z-dir.offsetZ)); ReikaJavaLibrary.pConsole("Current has:
            //"+coordCache.contains(new Coordinate(x, y, z)));
            //ReikaJavaLibrary.pConsole("Have "+coordCache.size()+" points; stepping
            //backward, opposite of "+dir+", from "+x+", "+y+", "+z);
            //this.stepPath(x-dir.offsetX, y-dir.offsetY, z-dir.offsetZ, rand, dir);
            step = new Coordinate(x - dir.offsetX, y - dir.offsetY, z - dir.offsetZ);
    ...
    private Collection<ForgeDirection>
    getMovementDirections(int x, int y, int z, ForgeDirection last, Random rand) {
        if (rand.nextInt(3) == 0 && this.canMove(x, y, z, last)
        ...
        for (ForgeDirection dir : ForgeDirection.VALID_DIRECTIONS) {
            if (li.size() < n) {
                if (dir != last && this.canMove(x, y, z, dir)
                    && !this.hasCellFrom(x, y, z, dir)) {
                    li.add(dir);
    ...
    private ForgeDirection getMovementDirection(int x, int y, int z, Random rand) {
        //if (rand.nextInt(3) == 0 && this.canMove(x, y, z, last) && !this.hasCellFrom(x,
        //y, z, last)) //bias towards continuing direction 	return last;
        ...
        while (li.get(idx) == last || !this.canMove(x, y, z, li.get(idx))
            ...
            if (li.isEmpty()) {
                for (int i = 0; i < 6; i++) {
                    ...
                    //ReikaJavaLibrary.pConsole(dir+": LAST="+(dir == last)+"|
                    //CANMOVE="+this.canMove(x, y, z, dir)+"| HAS="+this.hasCellFrom(x, y,
                    //z, dir));
                }
                //ReikaJavaLibrary.pConsole("UNVISIT="+this.hasUnvisitedNeighbors(x, y,
                //z)); ReikaJavaLibrary.pConsole("Had no paths yet has an unvisited
                //neighbor?!!");
            }
    ...
    private boolean canMove(int x, int y, int z, ForgeDirection dir) {
        ...
        return dx >= 0 && dx < MAX_SIZE_X && dy >= 0 && dy < MAX_SIZE_Y && dz >= 0
            && dz < MAX_SIZE_Z;
    }
    ...
    public boolean isLocationInMaze(double x, double y, double z) {
        ...
        return px >= 0 && px < MAX_SIZE_X && py >= 0 && py < MAX_SIZE_Y && pz >= 0
            && pz < MAX_SIZE_Z;
    }
