...
public class BridgeEntryPlatform extends BridgeNode {
    ...
    public void generate(ChunkSplicedGenerationCache world, int x, int y, int z) {
            ...
            for (int k = -1; k <= 1; k++) {
                if (i != 0 || k != 0) {
                    world.setBlock(
                        ...
                        z + k,
                        Blocks.planks,
                        ReikaItemHelper.spruceWood.metadata
                    );
                }
            }
        }
        ...
        for (int i = -3; i <= 3; i++) {
            world.setBlock(
                x + i, y, z + 3, Blocks.planks, ReikaItemHelper.spruceWood.metadata
            );
            world.setBlock(
                x + i, y, z - 3, Blocks.planks, ReikaItemHelper.spruceWood.metadata
            );
            world.setBlock(
                x + 3, y, z + i, Blocks.planks, ReikaItemHelper.spruceWood.metadata
            );
            world.setBlock(
                x - 3, y, z + i, Blocks.planks, ReikaItemHelper.spruceWood.metadata
            );
