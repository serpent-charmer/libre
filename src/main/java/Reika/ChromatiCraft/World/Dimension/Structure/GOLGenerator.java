...
public class GOLGenerator extends DimensionStructureGenerator {
    ...
    protected void calculate(int x, int z, Random rand) {
        ...
        for (int k = -1; k <= ROOM_HEIGHT + 1; k++) {
            for (int i = -SIZE - 1; i <= SIZE + 1; i++) {
                int m = (Math.abs(k % 8) == Math.abs(i % 8)) ? BlockType.LIGHT.metadata
                                                             : BlockType.CLOAK.metadata;
                world.setBlock(
                    x + i,
                    floorY + k,
        ...
        world.setTileEntity(
            ...
            floorY + 1,
            posZ,
            ChromaBlocks.GOLCONTROL.getBlockInstance(),
            0,
            new GOLTileCallback(this, false)
        );
    ...
    protected final void placeTile(int x, int z, boolean startOn) {
        world.setTileEntity(
            ...
            floorY,
            z,
            ChromaBlocks.GOL.getBlockInstance(),
            0,
            new GOLTileCallback(this, startOn)
        );
        world.setBlock(
            x, floorY + ROOM_HEIGHT, z, ChromaBlocks.GOL.getBlockInstance(), 2
        );
    }
    ...
    public void checkConditions(World world) {
        ...
        if (num >= min) {
            ...
            ChromaSounds.CAST.playSoundAtBlock(
                world, door.xCoord - SIZE * 2 - 3, door.yCoord, door.zCoord, 2, 1
            );
            solved = true;
        } else {
