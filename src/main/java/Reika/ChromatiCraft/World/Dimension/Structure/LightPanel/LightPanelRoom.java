...
public class LightPanelRoom extends StructurePiece<LightPanelGenerator> {
    ...
    public LightPanelRoom(
        LightPanelGenerator s,
        int r,
        int sw,
        int lvl,
        Random rand,
    ...
    protected void doGenerateConnections() {
        ...
        int nred = Math.max(
            2,
            Math.min(
                rowCount - 1, rowCount / 2 + ReikaRandomHelper.getRandomPlusMinus(0, 2)
            )
        );
    ...
    public final void
    toggleSwitch(World world, int x, int y, int z, int sw, boolean active) {
        ...
        if (active)
            ChromaSounds.DING.playSoundAtBlock(
                ...
                z,
                2,
                (float) CrystalMusicManager.instance.getPitchFactor(pitches[sw])
            );
    }
        ...
        for (int i = 0; i < rowCount; i++) {
            for (int k = 0; k < LightType.list.length; k++) {
                ...
                for (int n = 0; n < switchCount; n++) {
                    if (switches[n].active
                        && switches[n].group.containsLight(i, LightType.list[k])) {
                        flag = true;
                        break;
                    }
                }
                lights[i][k].state = flag;
                BlockLightPanel.activate(
                    world,
                    lights[i][k].location.xCoord,
                    lights[i][k].location.yCoord,
                    lights[i][k].location.zCoord,
                    flag
                );
            }
        }
    ...
    public final void generate(ChunkSplicedGenerationCache world, int x, int y, int z) {
        ...
        for (int i = -this.getWidth(); i <= this.getWidth(); i++) {
            ...
            if (Math.abs(i) < Math.abs(LightType.list.length * 2 - 1)) {
                int mb = Math.abs(i) == Math.abs(LightType.list.length * 2 - 1) - 1
                    ? BlockType.STONE.metadata
                    : BlockType.CLOAK.metadata;
                world.setBlock(
                    dx, y + 1 + 4, z + i, ChromaBlocks.STRUCTSHIELD.getBlockInstance(), mb
                );
                world.setBlock(
                    dx,
                    y + 1 + 4 + rowCount + 1,
        ...
        for (int i = -2; i <= 2; i++) {
            for (int k = 1; k <= 3; k++) {
                world.setBlock(
                    ...
                    BlockChromaDoor.getMetadata(false, false, false, true)
                );
            }
        }
        ...
        Object[] items = { ChromaStacks.lumaDust, 70, ChromaStacks.chargedBlueShard, 90,
                           ChromaStacks.lumenGem, 50, ChromaStacks.glowChunk,        10 };
    ...
    private void
    placeSwitch(ChunkSplicedGenerationCache world, int x, int y, int z, int sw) {
        world.setTileEntity(
            x,
            y,
            z,
            ChromaBlocks.PANELSWITCH.getBlockInstance(),
            0,
            new SwitchCallback(parent.id, level, sw)
        );
    }

    private void placeLight(
        ChunkSplicedGenerationCache world, int x, int y, int z, int row, LightType type
    ) {
        Coordinate c = new Coordinate(x, y, z);
        lights[row][type.ordinal()].location = c;
        world.setBlock(
            x, y, z, ChromaBlocks.LIGHTPANEL.getBlockInstance(), type.ordinal() * 2
        );
    }
    ...
    public final boolean isComplete() {
        for (int i = 0; i < rowCount; i++) {
            if (!lights[i][LightType.TARGET.ordinal()].state)
                return false;
            if (lights[i][LightType.BLOCK.ordinal()].state
                && !lights[i][LightType.CANCEL.ordinal()].state)
                return false;
        }
        return true;
    }
    ...
    private static class SwitchCallback extends DimensionStructureTileCallback {
