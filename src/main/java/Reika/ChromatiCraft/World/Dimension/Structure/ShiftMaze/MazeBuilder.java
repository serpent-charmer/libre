...
public class MazeBuilder {
    ...
    public void build(
        ShiftMazeGenerator gen, MazeGrid finishedGrid /* ,
                                                       * Map<Integer
                                                       * ,
                                                       * List<Point
                                                       * >>
        ...
        for (int xx = 0; xx < finishedGrid.getXSize(); xx++) {
            for (int zz = 0; zz < finishedGrid.getZSize(); zz++) {
                ...
                MazeBaseBuilder.build(
                    gen, world, translatedX, posY, translatedZ, segment
                );
    ...
    private void setDoor(
        ShiftMazeGenerator gen,
        ChunkSplicedGenerationCache world,
        int x,
        int y,
        int z,
        List<ShiftMazeState> states
    ) {
        world.setBlock(
            x,
            y,
            ...
            states.contains(gen.getActiveState()) ? Passability.OPEN.ordinal()
    ...
    public void buildExit(
        ShiftMazeGenerator gen, int endX, int posY, int endZ, List<UUID> createdKeys
    ) {
        int xCursor = endX;
