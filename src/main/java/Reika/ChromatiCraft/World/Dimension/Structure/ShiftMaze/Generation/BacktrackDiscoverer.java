...
public class BacktrackDiscoverer {
    ...
    public boolean doStep(
        ...
        int sX,
        int sZ,
        boolean markPassedNodes,
        boolean lockNodes,
        GridSearchCondition condition
    ) {
        ...
        for (ForgeDirection dir : dirs) {
            ...
            if ((restriction == null ? grid.isInBounds(tX, tZ)
                                     : restriction.isInBounds(tX, tZ))
                && grid.isFree(tX, tZ)) {
                isFlatDeadEnd = false;
                ...
                if (doStep(
                        grid, tX, tZ, markPassedNodes, lockNodes, condition.copyNext()
                    ))
                    return true;
            }
        }
        ...
        public boolean isMet(MazeGrid grid, int sX, int sZ) {
            if (steps <= 0) {
                for (ForgeDirection dir : MazeGrid.MazeSegment.VALID_CONNECTIONS) {
                    if (MazeCalculator.canGenerateDoor(grid, sX, sZ, dir, dstBorders))
                        return true;
                }
            }
            return false;
