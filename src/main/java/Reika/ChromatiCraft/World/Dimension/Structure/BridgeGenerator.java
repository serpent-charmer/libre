...
public class BridgeGenerator extends DimensionStructureGenerator { //TODO cancelled
    ...
    protected void calculate(int chunkX, int chunkZ, Random rand) {
        ...
        for (int i = -SIZE; i <= SIZE; i++) {
            for (int j = -SIZE; j <= SIZE; j++) {
                for (int k = -SIZE; k <= SIZE; k++) {
                    if (ReikaMathLibrary.isPointInsidePowerEllipse(
                            i, j, k, SIZE, SIZE / 4D, SIZE / 2D, 1.5
                        )) {
                        if (ReikaMathLibrary.isPointInsidePowerEllipse(
                                i, j, k, SIZE - 1.5, SIZE / 4D - 1.5, SIZE / 2D - 1.5, 1.5
                            )) {
                            world.setBlock(
                                x + i, y + j, z + k, j < -1 ? Blocks.water : Blocks.air
                            );
                        } else {
                            world.setBlock(
                                x + i,
        ...
        new BridgeEntryPlatform(this)
            .connect(ForgeDirection.EAST)
