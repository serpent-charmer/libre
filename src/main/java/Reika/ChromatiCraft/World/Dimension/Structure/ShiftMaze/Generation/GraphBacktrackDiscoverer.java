...
public class GraphBacktrackDiscoverer {
    public static ArrayDeque<MazeSegmentNode>
    getNodePath(MazeSegmentNode fromNode, MazeSegmentNode toNode, int steps) {
    ...
    private static ArrayDeque<MazeSegmentNode> recDiscoverGraph(
        ...
        for (MazeSegmentNode next : fromNode.connectionDoors.keySet()) {
            if (nodePath.contains(next))
                continue; //We don't go back/in circles
