...
public class MazeSegmentNode {
    ...
    public final Map<MazeSegmentNode, List<MazeGrid.ShiftMazeDoor>> connectionDoors
        = new HashMap<>();
    ...
    public void
    addStateForConnection(MazeSegmentNode other, Random rand, ShiftMazeState state) {
        if (state == null)
        ...
        if (doors == null || doors.isEmpty()) {
            throw new IllegalArgumentException(
                "Trying to add door state to connection without doors. Nodes not connected in graph."
            );
        }
