...
public class AntFarmGenerator
    ...
    protected void calculate(int chunkX, int chunkZ, Random rand) {
        ...
        for (int y = mh; y <= mh + h; y += dhy) {
            ...
            levels.addLast(l);
            if (pre != null) {
                AntFarmTunnel t = this.createTunnel(
                    pre.posX, pre.posY, pre.posZ, x, y, z, 3, 2, rand
                );
        ...
        for (AntFarmLevel l : levels) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                int dz = l.posZ + z;
                AntFarmTunnel t
                    = this.createTunnel(l.posX, l.posY, l.posZ, dx, dy, dz, 3, 3, rand);
                //TODO revisit
                //while (t.intersectsWith(tunnelSpaces, levelSpaces)) {
    ...
    private AntFarmTunnel
    createTunnel(int x, int y, int z, int x2, int y2, int z2, int w, int c, Random rand) {
        return new AntFarmTunnel(
            this, new Coordinate(x, y, z), new Coordinate(x2, y2, z2), c, w
        );
    }
