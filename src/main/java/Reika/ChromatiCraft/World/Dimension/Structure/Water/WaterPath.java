...
public class WaterPath {
    ...
    public void genPath(Random rand) {
        ...
        while (flag) {
            flag = false;
            ArrayList<Integer> indicesToTry = ReikaJavaLibrary.makeIntListFromArray(
                ReikaArrayHelper.getLinearArray(li.size() - 1)
            ); //-1 since cannot do from last point
    ...
    private boolean
    tryExtendPath(Point p1, Point p2, int idx, ArrayList<Point> li, ForgeDirection dir) {
        if (this.canStepTo(p1, dir, 1, false) && this.canStepTo(p2, dir, 1, false)) {
            ...
            while (flag2) {
                flag2 = false;
                dist++;
                if (this.canStepTo(p1, dir, dist, false)
                    && this.canStepTo(p2, dir, dist, false)) {
                    flag2 = true;
                }
            }
    ...
    private boolean
    canStepTo(Point from, ForgeDirection dir, int dist, boolean allowRevisit) {
        Point to = new Point(from.x + dir.offsetX * dist, from.y + dir.offsetZ * dist);
        return Math.abs(to.x) <= gridSize && Math.abs(to.y) <= gridSize
            && (allowRevisit || !visitedCells.contains(to));
    }
