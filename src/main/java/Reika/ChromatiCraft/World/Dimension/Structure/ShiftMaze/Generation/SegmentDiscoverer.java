...
public class SegmentDiscoverer {
    ...
    //Note: If doors are not ignored, door segments themselves don't belong to any segment
    //unless they're open.
    public void runCalc(
        Collection<Point> segmentStartPoints,
        ShiftMazeState mazeState,
        boolean ignoreDoorStates
    ) {
    ...
    private void
    startCalculation(Point start, ShiftMazeState mazeState, boolean ignoreDoorStates) {
        int count = 0;
        ...
        while (!todo.isEmpty()) {
            ...
            List<Point> surrounded
                = getConnectingSegments(next, mazeState, ignoreDoorStates);
    ...
    private List<Point> getConnectingSegments(
        Point center, /*int segmentId, */ ShiftMazeState state, boolean ignoreStates
    ) {
        ...
        for (ForgeDirection dir : MazeGrid.MazeSegment.VALID_CONNECTIONS) {
            ...
            if (grid.isInBounds(tX, tZ)) {
                ...
                if (other.getConnections().contains(dir.getOpposite())
                    || at.getConnections().contains(dir
                    )) { //If either of them connects to the other.
