...
public class MazeCalculator {
    ...
    public void calcGraphPaths() {
        ...
        for (Point genGoal : generationGoals.keySet()) {
            ...
            //System.out.println("Trying to find path from " + start + " to " +
            //toConnectTo);
            ArrayDeque<MazeSegmentNode> nodePath
                = GraphBacktrackDiscoverer.getNodePath(start, toConnectTo, 2);
            //System.out.println("Connecting " + start + " to " + toConnectTo + " with 2
            //Steps> " + nodePath.toString());
    ...
    public void calcSegmentGraph() {
        ...
        for (Integer segId : segmentPoints.keySet()) {
            ...
            for (Point p : segmentContents) {
                if (grid.getSegment(p.x, p.y).specialLock)
                    continue;

                for (ForgeDirection dir : MazeGrid.MazeSegment.VALID_CONNECTIONS) {
                    Point other = new Point(p.x + dir.offsetX, p.y + dir.offsetZ);
                    if (!grid.isInBounds(other.x, other.y))
                        continue;
                    if (segmentContents.contains(other))
                        continue;
                    if (grid.getSegment(other.x, other.y).specialLock)
                        continue;
                    ...
                    BufferedDoorState state
                        = new BufferedDoorState(p, other, dir, node, otherNode);
                    if (!prePotentialDoorPoints.contains(state))
    ...
    private int getContainingSegmentId(Point p) {
        for (Integer segId : segmentPoints.keySet()) {
            if (segmentPoints.get(segId).contains(p))
                return segId;
        }
    ...
    public void calcSegmentInternalMazes() {
        ...
        for (int segId : segmentIds.keySet()) {
            ...
            backtrackStepper.doStep(
                grid,
                goal.x,
                goal.y,
                true,
                false,
                new BacktrackDiscoverer.MatchCoordCondition(-1, -1)
            ); //Fill.
        ...
        lblDe:
            for (Point de : deadEndPoints) {
                ...
                for (int i = rIndex; i < (rIndex + 4); i++) {
                    ForgeDirection dir
                        = MazeGrid.MazeSegment.VALID_CONNECTIONS.get(i % 4);
    ...
    public void calcMazeSegments() {
            ...
            do {
                ...
                for (int xx = 0; xx < divPoint.x; xx++) {
                    ...
                    for (List<Point> pList : segmentPoints.values()) {
                        if (pList.contains(at))
                            contains = true; //Some segment already contains this pos.
                    }
                    if (!contains) {
                        if (grid.getSegment(xx, step).specialLock)
                            continue;
                        containing.add(at);
                    }
                }
        ...
        for (int xx = 0; xx < grid.getXSize(); xx++) {
            for (int zz = 0; zz < grid.getZSize(); zz++) {
                ...
                if (grid.getSegment(xx, zz).specialLock)
                    continue;
                boolean contains = false;
                for (List<Point> pList : segmentPoints.values()) {
                    if (pList.contains(at))
    ...
    public void genFixedAnchors(ShiftMazeGenerator gen, int amtChests) {
        ...
        if (reached != null) {
            ...
            for (int t = rIndex; t < (rIndex + 4); t++) {
                ...
                if (canGenerateDoor(grid, reached.x, reached.y, dir, 0)) {
                    genGoalEndDoor
                        = this.genDoorUnsafe(reached.x, reached.y, dir, stateEndDoor);
                    gennedDoor = true;
        ...
        for (int i = 0; i < amtChests; i++) {
            ...
            backtrackStepper.doStep(
                ...
                true,
                true,
                new BacktrackDiscoverer.GenerateDoorAfterStepCondition(
                    1 + seededRand.nextInt(2), 2
                )
            );
            ...
            if (reached != null) {
                ...
                List<ForgeDirection> dirs
                    = this.sortDirections(reached, new Point(reached.x, center.y));
                for (ForgeDirection dir : dirs) {
                    if (canGenerateDoor(grid, reached.x, reached.y, dir, 2)) {
                        Point genGoal
                            = this.genDoorUnsafe(reached.x, reached.y, dir, chestState);
    ...
    canGenerateDoor(MazeGrid grid, int sX, int sZ, ForgeDirection dir, int dstToBorders) {
        Point doorE = new Point(sX + dir.offsetX, sZ + dir.offsetZ);
        return grid.isFree(doorE.x, doorE.y)
            && !violatesAnchors(doorE, grid, null, null, dstToBorders);
    }

    private Point
    genDoorUnsafe(int sX, int sZ, ForgeDirection dir, ShiftMazeState... doorStates) {
    ...
    private Point getRandomPos(int xLayer) {
        ...
        for (int zz = 0; zz < grid.getZSize(); zz++) {
            ...
            if (grid.isFree(xLayer, pZ)
                && !violatesAnchors(
                    p, grid, this.getGenerationGoals(), this.getPlayerGoals(), 2
                ))
                return p;
        }
        throw new IllegalStateException(
            "Cannot get random position that's far enough away from all current anchors. Maze too small?"
        );
        /*Point p;
        do {
            int rX = seededRand.nextInt(grid.getXSize());
            ...
            tries--;
            if(tries < 0)
                throw new IllegalStateException("Cannot get random position that's far
        enough away from all current anchors. Maze too small?"); } while
        (!grid.isFree(p.x, p.y) || violatesAnchors(p, grid, getGenerationGoals(),
        getPlayerGoals())); return p;*/
    }

    private static boolean violatesAnchors(
        Point p,
        MazeGrid grid,
        Collection<Point> genGoals,
        Collection<Point> plGoals,
        int dstToBorders
    ) {
        ...
        public boolean equals(Object o) {
            ...
            if ((from.equals(that.from) && to.equals(that.to)
                ...
                || from.equals(that.to) && to.equals(that.from)
                    && fromNode.equals(that.toNode) && toNode.equals(that.fromNode))
                return true;
