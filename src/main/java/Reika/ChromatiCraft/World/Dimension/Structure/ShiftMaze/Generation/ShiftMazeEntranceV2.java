...
public class ShiftMazeEntranceV2 extends DynamicStructurePiece<ShiftMazeGenerator> {
    public ShiftMazeEntranceV2(ShiftMazeGenerator s) {
        super(s);
    }
    ...
    private int generateEntrance(
        World world, int x, int y, int z, int top
    ...
    private void generateShaft(World world, int x, int y, int z, int h, boolean top) {
        for (int i = -2; i <= 2; i++) {
            ...
            for (int k = -2; k <= 2; k++) {
                ...
                for (int n = -3; n < h; n++) {
                    int dy = y + n;
                    boolean wall = Math.abs(i) == 2 || Math.abs(k) == 2 || n == -3
                        || (n == h - 1 && !top);
                    if (wall) {
                        int m = i == 0 && k == 0
                            ? BlockStructureShield.BlockType.LIGHT.metadata
                            : BlockStructureShield.BlockType.STONE.metadata;
                        world.setBlock(
                            dx, dy, dz, ChromaBlocks.STRUCTSHIELD.getBlockInstance(), m, 3
                        );
                    } else if (n == -2) {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int k = -1; k < 2; k++) {
                int dz = z + i;
                int dy = y + k;
                world.setBlock(x + 1, dy, dz, Blocks.air);
            }
        }
    }
    ...
    private void generatePrefab(World world, int x, int y, int z) {
        ...
        world.setBlock(
            x + 5, y + 3, z + 3, b, BlockStructureShield.BlockType.LIGHT.metadata, 3
        );
        world.setBlock(
            x + 5, y + 3, z + 7, b, BlockStructureShield.BlockType.LIGHT.metadata, 3
        );
        ...
        world.setBlock(x + 4, y + 6, z + 7, Blocks.air);
        world.setBlock(x + 4, y + 6, z + 8, Blocks.air);
        world.setBlock(x + 4, y + 6, z + 9, Blocks.air);
