...
public class AntFarmTunnel_Old extends StructurePiece<AntFarmGenerator> {
    ...
    private void initialize(int x, int y, int z, HashSet<Coordinate> airSpaces) {
        ...
        for (int d = 0; d < length3; d++) {
            ...
            this.generateTunnelSection(
                dx, y, dz, Math.max(2, Math.round(tunnelRadius - 0.125F * d)), airSpaces
            );
        }
    ...
    private void
    generateTunnelSection(int dx, int dy, int dz, int r, HashSet<Coordinate> airSpaces) {
        for (int i = -r; i <= r; i++) {
            for (int j = -r; j <= r; j++) {
                for (int k = -r; k <= r; k++) {
