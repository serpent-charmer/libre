...
public class RegionMapper extends ThreadedGenerator {
    ...
    public void run() throws Throwable {
        //angleMap.clear();
        StructureCalculator calc = (StructureCalculator
        ) ThreadedGenerators.STRUCTURE.getCurrentlyActiveGenerator();
        boolean printed = false;
        while (!calc.arePositionsDetermined()) {
            Thread.sleep(50);
            if (!printed)
                ChromatiCraft.logger.log(
                    "Waiting for structure generator to finish to calculate locations..."
                );
            printed = true;
        }
        ...
        /*LobulatedCurve c*/ region
            = LobulatedCurve.fromMinMaxRadii(r1, r2, 20).generate(rand);
        /*
    ...
    public String getStateMessage() {
        return "Central region created; " + region.minRadius + " +/- "
            + (region.amplitudeVariation * 20);
    }
}
