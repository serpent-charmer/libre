...
public class CheatingPreventionSystem {
    public static final CheatingPreventionSystem instance
        = new CheatingPreventionSystem();
    ...
    private CheatingPreventionSystem() {
        if (ModList.ENDERIO.isLoaded()) {
            Block b
                = GameRegistry.findBlock(ModList.ENDERIO.modLabel, "blockTravelAnchor");
    ...
    private void banBlock(Block b, BanReaction r) {
        ...
        if (i != null)
            bannedItems.put(
                new KeyedItemStack(i)
                    .setIgnoreMetadata(!bk.hasMetadata())
                    .setSimpleHash(true),
                r
            );
    }
    ...
    public void handleRightClicks(PlayerInteractEvent evt) {
        if (evt.action == Action.RIGHT_CLICK_AIR
            || evt.action == Action.RIGHT_CLICK_BLOCK) {
    ...
    private BanReaction getReaction(ItemStack is) {
        return is == null || is.getItem() == null
            ? null
            : bannedItems.get(new KeyedItemStack(is).setSimpleHash(true));
    }
    ...
    public void punishCheatingPlayer(EntityPlayer ep) {
        ...
        ep.attackEntityFrom(
            DamageSource.generic, ReikaRandomHelper.getRandomBetween(5, 10)
        );
        Vec3 v = ep.getLookVec();
        ReikaEntityHelper.knockbackEntityFromPos(
            ep.posX + v.xCoord, ep.posY + v.yCoord - 1.5, ep.posZ + v.zCoord, ep, 2.5
        );
