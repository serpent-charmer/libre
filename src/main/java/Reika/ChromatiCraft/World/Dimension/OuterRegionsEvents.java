...
public class OuterRegionsEvents {
    ...
    public void tickPlayerInOuterRegion(EntityPlayer ep) {
        ...
        if (ep.worldObj.rand.nextInt(400) == 0) {
            ...
            if (ep.worldObj.getEntitiesWithinAABB(EntityDimensionFlare.class, box).size()
                < 8)
    ...
    public boolean doRejectAttack(EntityDimensionFlare e, EntityPlayer ep) {
        ...
        if (ep.worldObj.isRemote) {
            ...
            ReikaPacketHelper.sendDataPacket(
                ...
                ChromaPackets.FLAREATTACK.ordinal(),
                (EntityPlayerMP) ep,
                e.getEntityId()
            );
            if (ep.getHealth() <= dmg) {
    ...
    private void doRejectAttackFX(EntityDimensionFlare e, EntityPlayer ep) {
        ReikaSoundHelper.playClientSound(
            ChromaSounds.FLAREATTACK, ep, 1, e.getIdentity().soundPitch
        );
        int n = 4 + ep.worldObj.rand.nextInt(4);
        LightningBolt b = new LightningBolt(
            new DecimalPosition(e), new DecimalPosition(ep).offset(0, -0.8, 0), n
        );
        b.scaleVariance(2);
        b.maximize();
        ...
        for (int i = 0; i < b.nsteps; i++) {
            ...
            for (double r = 0; r <= 1; r += 0.03125) {
                ...
                EntityFX fx
                    = new EntityCCBlurFX(ep.worldObj, dd.xCoord, dd.yCoord, dd.zCoord)
                          .setScale(s)
                          .setColor(clr)
                          .setLife(l)
                          .setRapidExpand()
    ...
    public void doFlareAggro(EntityPlayer ep) {
        ...
        List<EntityDimensionFlare> li
            = ep.worldObj.getEntitiesWithinAABB(EntityDimensionFlare.class, box);
