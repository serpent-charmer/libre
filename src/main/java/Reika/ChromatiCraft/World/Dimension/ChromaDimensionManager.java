...
public class ChromaDimensionManager {
    ...
    public static enum Biomes implements ChromaDimensionBiomeType {
        PLAINS(
            BiomeGenCrystalPlains.class,
            "Crystal Plains",
            8,
            0,
            ExtraChromaIDs.PLAINS,
            SubBiomes.MOUNTAINS,
            Type.MAGICAL,
            Type.PLAINS
        ),
        ISLANDS(
            BiomeGenIslands.class,
            "Iridescent Archipelago",
            6,
            -5,
            ExtraChromaIDs.ISLANDS,
            SubBiomes.DEEPOCEAN,
            Type.MAGICAL,
            Type.BEACH,
            Type.WET
        ...
        SKYLANDS(
            BiomeGenSkylands.class,
            "Lumen Skylands",
            2,
            0,
            ExtraChromaIDs.SKYLANDS,
            SubBiomes.VOIDLANDS,
            Type.MAGICAL,
            Type.COLD
        ),
        FOREST(
            BiomeGenGlowingForest.class,
            "Glowing Forest",
            10,
            10,
            ExtraChromaIDs.FOREST,
            SubBiomes.CRYSFOREST,
            Type.MAGICAL,
            Type.FOREST
        ),
        SPARKLE(
            BiomeGenSparkle.class,
            "Sparkling Sands",
            4,
            0,
            ExtraChromaIDs.SPARKLE,
            Type.MAGICAL,
            Type.BEACH,
            Type.SANDY
        ),
        GLOWCRACKS(
            BiomeGenGlowCracks.class,
            "Radiant Fissures",
            3,
            0,
            ExtraChromaIDs.GLOWCRACKS,
            Type.MAGICAL,
            Type.HOT
        ),
        STRUCTURE(
            StructureBiome.class,
            "Structure Field",
            0,
            0,
            ExtraChromaIDs.STRUCTURE,
            Type.MAGICAL,
            Type.PLAINS
        ),
        CENTER(
            BiomeGenCentral.class,
            "Luminescent Sanctuary",
            0,
            0,
            ExtraChromaIDs.CENTRAL,
            Type.MAGICAL,
            Type.FOREST,
            Type.DENSE
        ),
        ...
        private Biomes(
            Class<? extends ChromaDimensionBiome> c,
            String n,
            int w,
            int h,
            ExtraChromaIDs id,
        ...
        private void create() {
            ...
            IDCollisionTracker.instance.addBiomeID(
                ChromatiCraft.instance, id, biomeClass
            );
            IDMap.put(id, this);
            ...
            try {
                Constructor c
                    = biomeClass.getConstructor(int.class, String.class, Biomes.class);
                instance = (ChromaDimensionBiome) c.newInstance(id, biomeName, this);
            } catch (Exception e) {
                throw new RegistrationException(
                    ChromatiCraft.instance,
                    "Could not create biome instance " + this + ": "
    ...
    public static enum SubBiomes implements ChromaDimensionBiomeType {
        MOUNTAINS(
            BiomeGenChromaMountains.class,
            "Crystal Mountains",
            0.75,
            0,
            ExtraChromaIDs.MOUNTAIN,
            Type.MAGICAL,
            Type.MOUNTAIN
        ),
        DEEPOCEAN(
            BiomeGenChromaOcean.class,
            "Aura Ocean",
            0.4,
            -30,
            ExtraChromaIDs.OCEAN,
            Type.MAGICAL,
            Type.OCEAN
        ),
        CRYSFOREST(
            BiomeGenCrystalForest.class,
            "Crystal Forest",
            0.2,
            15,
            ExtraChromaIDs.CRYSFOREST,
            Type.MAGICAL,
            Type.FOREST
        ),
        VOIDLANDS(
            BiomeGenVoidlands.class,
            "Voidland",
            0.1,
            8,
            ExtraChromaIDs.VOID,
        ...
        private SubBiomes(
            Class<? extends ChromaDimensionSubBiome> c,
            String n,
            double w,
            int h,
            ExtraChromaIDs id,
        ...
        private void create(Biomes b) {
            ...
            try {
                Constructor c
                    = biomeClass.getConstructor(int.class, String.class, SubBiomes.class);
    ...
    public static void checkChromaDimensionUnload() {
        ...
        if (world != null) {
            ChromatiCraft.logger.log(
                "Checking dimension for unload - player entities: " + world.playerEntities
            );
    ...
    public static void resetDimension(World world) {
        ...
        if (dimensionAge <= 1200 && !serverStopping) {
            ChromatiCraft.logger.log(
                "Dimension is only " + dimensionAge + " ticks old; not resetting"
            );
            return;
        }
        if (dimensionClearing) {
            ChromatiCraft.logger.log(
                "Dimension already resetting; not attempting to reset during a reset"
            );
            return;
        }
        dimensionClearing = true;
        ChromatiCraft.logger.log(
            "Resetting dimension of age " + dimensionAge + "; Server shutdown? "
            + serverStopping
        );
        dimensionSeed = -1;
        dimensionAge = 0;
        ...
        if (dim.exists() && dim.isDirectory()) {
            ...
            if (!del) {
                ChromatiCraft.logger.logError(
                    "Could not delete dimension chunk data; you must delete it manually or the dimension will be invalid."
                );
            }
        }
        ReikaPacketHelper.sendDataPacketToEntireServer(
            ChromatiCraft.packetChannel, ChromaPackets.LEAVEDIM.ordinal()
        );
    ...
    public static void resetDimensionClient() {
        ...
        System.gc();
        File f = new File(
            DragonAPICore.getMinecraftDirectory(), "mods/VoxelMods/voxelMap/cache"
        );
    ...
    addPlayerToStructure(EntityPlayerMP ep, DimensionStructureGenerator structure) {
        if (!DimensionTuningManager.TuningThresholds.STRUCTURES.isSufficientlyTuned(ep))
            return false;
        playersInStructures.put(ep, structure);
        /*
        if (ProgressionManager.instance.hasPlayerCompletedStructureColor(ep,
        structure.getCoreColor(ep.worldObj))) { structure.forceOpen(ep.worldObj, ep);
        }
         */
        ReikaPacketHelper.sendDataPacket(
            ChromatiCraft.packetChannel,
            ChromaPackets.STRUCTUREENTRY.ordinal(),
            ep,
            structure.getType().ordinal()
        );
        return true;
    }
    ...
    public static void
    addPlayerToStructureClient(EntityPlayer ep, DimensionStructureType structure) {
    ...
    public static void renderAurorae() {
        ...
        for (EntityAurora e : aurorae) {
            ...
            if (a != null) {
                ...
                //GL11.glTranslated(RenderManager.renderPosX-e.posX,
                //RenderManager.renderPosY-e.posY, RenderManager.renderPosZ-e.posZ);
                a.render();
                GL11.glPopMatrix();
            }
        }
    ...
    public static void
    onPlayerBlockedFromBiome(World world, int x, int y, int z, Entity ep) {
        if (world.isRemote) {
    ...
    @SideOnly(Side.CLIENT)
    public static void
    onPlayerBlockedFromBiomeClient(World world, int x, int y, int z, Entity ep) {
        Random rand = world.rand;
        //EntityBlurFX fx = new EntityBlurFX(world, x+rand.nextDouble(),
        //y+rand.nextDouble(), z+rand.nextDouble());
                ...
                for (int k = -r; k <= r; k++) {
                    ...
                    for (int d = 2; d < 6; d++) {
                        ...
                        if (ReikaMathLibrary.py3d(
                                sx - ep.posX, sy - ep.posY, sz - ep.posZ
                            )
                            > dr1)
                            continue;
                        ...
                        fx.setIcon(ChromaIcons.FADE_GENTLE)
                            .setAlphaFading()
                            .setScale(sc)
                            .setColor(ReikaColorAPI.getColorWithBrightnessMultiplier(
                                0x22aaff, br
                            ))
                            .setRapidExpand()
                            .setLife(l);
    ...
    public static boolean
    isBlockedAir(World world, int x, int y, int z, Block b, Entity ep) {
        return !b.isOpaqueCube() && ep instanceof EntityPlayer
            && isStructureBiome(world.getBiomeGenForCoords(x, z))
            && !TuningThresholds.STRUCTUREBIOMES.isSufficientlyTuned((EntityPlayer) ep);
    }
    ...
    public static boolean isDisallowedEntity(Entity e) {
        if (ModList.THAUMCRAFT.isLoaded() && isThaumEvilMob(e))
            return true;
        if (ModList.ARSMAGICA.isLoaded()
            && e.getClass().getName().endsWith("EntityDryad"))
            return true;
        return false;
    }
    ...
    private static boolean isThaumEvilMob(Entity e) {
        ...
        if (e instanceof IMob
            && e.getClass().getName().startsWith("thaumcraft.common.entities.monster"))
            return true;
        return false;
    }
}
