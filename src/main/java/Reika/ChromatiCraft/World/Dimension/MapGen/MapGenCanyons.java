...
public class MapGenCanyons extends ChromaFeatureBase {
    ...
    protected void generate(
        ...
        if (rand.nextInt(8) == 0)
            this.func_151540_a(rand.nextLong(), chunkX, chunkZ, data, ravineX, ravineY,
        ravineZ, f2, f, f1, 0, 0, 3.0D);
         */
    ...
    /*
    protected void func_151540_a(long seed, int cx, int cz, Block[] data, double rx,
    double ry, double rz, float f2, float f, float f1, int i1, int i2, double d1)
    {
        ...
        for (; i1 < i2; ++i1)
        {
            ...
            if (flag1 || random.nextInt(4) != 0)
            {
                ...
                if (rx >= lx - 16.0D - d12 * 2.0D && rz >= lz - 16.0D - d12 * 2.0D && rx
    <= lx + 16.0D + d12 * 2.0D && rz <= lz + 16.0D + d12 * 2.0D)
                {
                            ...
                            for (j3 = k4; j3 < j2; ++j3)
                            {
                                ...
                                if (d13 * d13 + d14 * d14 < 1.0D)
                                {
                                    for (int l3 = n2 - 1; l3 >= j4; --l3)
                                    {
                                        ...
                                        if ((d13 * d13 + d14 * d14) * field_75046_d[l3] +
    d11 * d11 / 6.0D < 1.0D)
                                        {
                                            Block block1 = data[k3];

                                            this.digBlock(data, k3, k2, l3, j3, cx, cz,
    flag);
                                        }
    ...
    private void digBlock(Block[] data, int index, int x, int y, int z, int chunkX, int
    chunkZ, boolean foundTop) { data[index] = null;
    }
     */
}
