...
public class SkyRiverManagerClient {
    ...
    public static void handleSkyRiverMovementClient() {
        ...
        SkyRiverGenerator.RiverPoint closest
            = SkyRiverGenerator.getClosestPoint(pl, 48, false);
    ...
    public static void handleRayUpdatePacket(NBTTagCompound cmp) {
        ...
        for (int i = 1; i < n - 1; i++) {
            ...
            ChunkCoordIntPair ch = new ChunkCoordIntPair(
                MathHelper.floor_double(p1.xCoord) / 16,
                MathHelper.floor_double(p1.zCoord) / 16
            );
            SkyRiverGenerator.RiverPoint p
                = new SkyRiverGenerator.RiverPoint(i, n, ch, p1, pa, pb);
            if (prev != null) {
        ...
        SkyRiverManager.debugMessage(
            "Client> Received " + points.size() + " SkyRiver Ray-Points from Server."
        );
    }
