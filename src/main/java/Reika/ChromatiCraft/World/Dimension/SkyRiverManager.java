...
/* This class was originally written by HellFirePvP, but has since been heavily modified
 * by Reika. The core logic remains his. */
public class SkyRiverManager {
    public static final PlayerChunkTracker.TrackingCondition skyRiverCondition
        = new TrackingConditionSkyRiver();
    private static final List<DelayedSkyRiverPacketEvent> delayedPackets
        = new LinkedList();
    private static final PlayerMap<RiverStatus> statusData
        = new PlayerMap(); // Allows the server to keep up after leaving the SkyRiver
    private static final int SKYRIVER_ENTER_DELAY
        = 60; // ticks - allows the server to catch up after untrack.
            ...
            for (Object objPl : w.playerEntities) {
                ...
                if (rs.isMovable()) {
                    SkyRiverGenerator.RiverPoint closest
                        = SkyRiverGenerator.getClosestPoint(pl, 16, true);
                    if (closest != null) {
                        if (SkyRiverGenerator.isWithinSkyRiver(pl, closest)) {
                            if (!PlayerChunkTracker.shouldStopChunkloadingFor(pl)) {
                                PlayerChunkTracker.startTrackingPlayer(
                                    pl, skyRiverCondition
                                );
                                sendSkyriverEnterStatePacket(pl, true);
                                debugMessage(
                                    "Player " + pl.getCommandSenderName()
                                    + " has entered a SkyRiver."
                                );
                            }
    ...
    static void
    movePlayer(EntityPlayer player, SkyRiverGenerator.RiverPoint rp, boolean doesMove) {
    ...
    private static void movePlayer(
        EntityPlayer player,
        RiverStatus rs,
        SkyRiverGenerator.RiverPoint rp,
        boolean doesMove
    ) {
        ...
        dst = ReikaVectorHelper.getDistFromPointToLine(
            rp.next.xCoord,
            rp.next.yCoord,
            rp.next.zCoord,
            rp.position.xCoord,
        ...
        if (dst < SkyRiverGenerator.RIVER_TUNNEL_RADIUS) {
            pullVector = Vec3.createVectorHelper(
                rp.next.xCoord - rp.position.xCoord,
                rp.next.yCoord - rp.position.yCoord,
                rp.next.zCoord - rp.position.zCoord
            );
            nodeVector = Vec3.createVectorHelper(
                rp.next.xCoord - plPos.xCoord,
                rp.next.yCoord - plPos.yCoord,
                rp.next.zCoord - plPos.zCoord
            );
        } else {
            ...
            if (dst < SkyRiverGenerator.RIVER_TUNNEL_RADIUS) {
                pullVector = Vec3.createVectorHelper(
                    rp.position.xCoord - rp.prev.xCoord,
                    rp.position.yCoord - rp.prev.yCoord,
                    rp.position.zCoord - rp.prev.zCoord
                );
                nodeVector = Vec3.createVectorHelper(
                    rp.position.xCoord - plPos.xCoord,
                    rp.position.yCoord - plPos.yCoord,
                    rp.position.zCoord - plPos.zCoord
                );
            } else {
        ...
        ); // Normalized vector parallel in the direction of the SkyRiver
        nodeVector = nodeVector.normalize(
        ); // Normalized vector pointing from the player towards the next node.
        moveVector = Vec3.createVectorHelper(
            pullVector.xCoord * 0.6 + nodeVector.xCoord * 0.4,
            pullVector.yCoord * 0.6 + nodeVector.yCoord * 0.4,
            pullVector.zCoord * 0.6 + nodeVector.zCoord * 0.4
        );
        if (doesMove) {
            Vec3 playerMove
                = Vec3.createVectorHelper(player.motionX, player.motionY, player.motionZ)
                      .normalize();
            moveVector = Vec3.createVectorHelper(
                moveVector.xCoord * 0.1 + playerMove.xCoord * 0.9,
                moveVector.yCoord,
                moveVector.zCoord * 0.1 + playerMove.zCoord * 0.9
            );
        }
        ...
        if (f <= 0) {
            ...
            rand.setSeed(
                player.getUniqueID().hashCode() ^ player.worldObj.getTotalWorldTime()
            );
            rand.nextBoolean();
            rand.nextBoolean();
    ...
    private static void sendRiverPoints(EntityPlayer player, SkyRiverGenerator.Ray r) {
        ...
        ReikaPacketHelper.sendNBTPacket(
            ...
            ChromaPackets.SKYRIVER_SYNC.ordinal(),
            cmp,
            new PacketTarget.PlayerTarget((net.minecraft.entity.player.EntityPlayerMP
            ) player)
        );
    }
    ...
    public static void clearClientRiver(EntityPlayer player) {
        ...
        ) { // Remove packets in queue. That way we don't send wrong data.
        ...
        ReikaPacketHelper.sendDataPacket(
            ChromatiCraft.packetChannel,
            ChromaPackets.SKYRIVER_STATE.ordinal(),
    ...
    private static void
    sendSkyriverEnterStatePacket(EntityPlayer player, boolean allowEntering) {
        debugMessage(
            "Sending SkyRiver State-Change to " + player.getCommandSenderName()
            + " - new State: " + (allowEntering ? "Allow" : "Deny")
        );
        ReikaPacketHelper.sendDataPacket(
            ChromatiCraft.packetChannel,
            ChromaPackets.SKYRIVER_STATE.ordinal(),
    ...
    // Fired, once a player leaves a SkyRiver. If this happens, we can expect LAG from
    // ChunkLoading. Don't allow new SkyRiver entries from anyone that isn't inside one
    // atm. Players already inside don't matter, they don't cause ChunkLoading anyway
    // since they're already tracked. New people do matter since if they'd move around,
    // they accumulate non-tracked movement packets which cause >200 chunks to load. RIP
    // server if we don't handle that. Thus, everyone gets a refresh-state DENY packet and
    // set onto the enter-delay.
    ...
    // If the player is in the chroma dim, start sending them immediately rather than 10
    // sec after login. He might need the rays.
    protected static void startSendingRiverPackets(EntityPlayer player) {
        ...
        if (player.worldObj.provider.dimensionId == ExtraChromaIDs.DIMID.getValue()) {
            ...
            SkyRiverGenerator.RiverPoint rp
                = SkyRiverGenerator.getClosestPoint(player, 128, true);
            if (rp != null) {
        ...
        for (int i = 0; i < SkyRiverGenerator.rays.size(); i++) {
            ...
            schedulePacketSending(
                player, toSend, ticksDelay + (i * 10)
            ); // 10 Ticks as delay between packets to split the load.
        }
    }
    ...
    private static void
    schedulePacketSending(EntityPlayer player, SkyRiverGenerator.Ray toSend, int delay) {
        DelayedSkyRiverPacketEvent pktEvent
            = new DelayedSkyRiverPacketEvent(player, toSend);
    ...
    private static class DelayedSkyRiverPacketEvent
        ...
        public DelayedSkyRiverPacketEvent(
            EntityPlayer player, SkyRiverGenerator.Ray toSend
        ) {
            recipient = player;
            this.toSend = toSend;
        }
    ...
    public static class TrackingConditionSkyRiver
        ...
        public void onUntrack(EntityPlayer player) {
            debugMessage(
                "Player " + player.getCommandSenderName() + " has left a SkyRiver"
            );
            handleUntrack();
        }
    }
