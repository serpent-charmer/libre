...
public class SkyRiverGenerator extends ThreadedGenerator {
    ...
    protected static final MultiMap<ChunkCoordIntPair, RiverPoint> clientPoints
        = new MultiMap().setNullEmpty();
    protected static final MultiMap<ChunkCoordIntPair, RiverPoint> serverPoints
        = new MultiMap().setNullEmpty();

    public static final double RIVER_TUNNEL_RADIUS
        = 12.0D; // The tunnel radius of the actual SkyRiver
    ...
    private static final double OUTER_RADIUS_MIN
        = StructureCalculator.getMaximumPossibleDistance() + RegionMapper.MAX_BUFFER
        + 512;
    ...
    public void run() throws Throwable {
        ...
        for (double d = 0; d < 360; d += FULL_RAY_ANGLE) {
            double r1 = INNER_RADIUS_MIN
                + rand.nextDouble() * (INNER_RADIUS_MAX - INNER_RADIUS_MIN);
            double r2 = OUTER_RADIUS_MIN
                + rand.nextDouble() * (OUTER_RADIUS_MAX - OUTER_RADIUS_MIN);
            this.generateRay(d, r1, r2);
        }

        for (double d = 0; d < 360; d += LAYER2_RAY_ANGLE) {
            if (d % FULL_RAY_ANGLE != 0) {
                double r1 = LAYER2_RADIUS_MIN
                    + rand.nextDouble() * (LAYER2_RADIUS_MAX - LAYER2_RADIUS_MIN);
                double r2 = OUTER_RADIUS_MIN
                    + rand.nextDouble() * (OUTER_RADIUS_MAX - OUTER_RADIUS_MIN);
                this.generateRay(d, r1, r2);
            }
        ...
        for (Ray r : rays) {
            ...
            for (int i = 1; i < n - 1; i++) {
                ...
                ChunkCoordIntPair ch = new ChunkCoordIntPair(
                    MathHelper.floor_double(pos.xCoord) / 16,
                    MathHelper.floor_double(pos.zCoord) / 16
                );
    ...
    private void exportAsImage() throws IOException {
        File f = new File(
            ...
            "DimensionRiver/" + seed + "L/" + System.nanoTime() + ".png"
        );
        if (f.exists())
            f.delete();
        ...
        for (RiverPoint p : serverPoints.allValues(false)) {
            ...
            n = Math.max(
                n, Math.max(1 + (int) Math.abs(c.xCoord), 1 + (int) Math.abs(c.zCoord))
            );
        }
        n /= 5;
        BufferedImage img
            = new BufferedImage(n * 2 + 1, n * 2 + 1, BufferedImage.TYPE_INT_ARGB);
        for (RiverPoint p : serverPoints.allValues(false)) {
    ...
    public static Collection<RiverPoint>
    getPointsForChunk(int x, int z, boolean isServerSide) {
        ChunkCoordIntPair pos = new ChunkCoordIntPair(x, z);
        MultiMap<ChunkCoordIntPair, RiverPoint> points
            = isServerSide ? serverPoints : clientPoints;
        Collection<RiverPoint> c = points.get(pos);
        return c != null ? Collections.unmodifiableCollection(c) : null;
    }

    public static Collection<RiverPoint>
    getPointsWithin(EntityPlayer ep, double range, boolean isServerSide) {
        ...
        if (chRange == 0) {
            ...
            for (int xDiff = -chRange - 1; xDiff <= chRange; xDiff++) {
                for (int zDiff = -chRange - 1; zDiff <= chRange; zDiff++) {
                    Collection<RiverPoint> c
                        = getPointsForChunk(x + xDiff, z + zDiff, isServerSide);
                    if (c != null)
                        c2.addAll(c);
                }
            }
    ...
    public static boolean isWithinSkyRiver(EntityPlayer player, RiverPoint closestPoint) {
        ...
        return isBetween(closestPoint.prev, closestPoint.position, player)
            || isBetween(closestPoint.position, closestPoint.next, player);
    }

    public static boolean
    isBetween(DecimalPosition pos1, DecimalPosition pos2, Entity toCheck) {
        double dst = ReikaVectorHelper.getDistFromPointToLine(
            pos1.xCoord,
            pos1.yCoord,
            pos1.zCoord,
            pos2.xCoord,
    ...
    public static RiverPoint
    getClosestPoint(EntityPlayer ep, double range, boolean isServerSide) {
        Collection<RiverPoint> c = getPointsWithin(ep, range, isServerSide);
        Double d = Double.POSITIVE_INFINITY;
        RiverPoint cl = null;
        for (RiverPoint p : c) {
            double dist = ep.getDistanceSq(
                p.position.xCoord, p.position.yCoord, p.position.zCoord
            );
        ...
        for (double d = r1; d <= r2; d += Math.max(NODE_LENGTH, 2 * Math.sqrt(d))) {
            ...
            if (d < LAYER2_RADIUS_MIN) {
                if (d < LAYER2_RADIUS_MIN - ANGLE_VARIATION_FADE_RANGE) {
                    ...
                    var = ReikaMathLibrary.linterpolate(
                        d,
                        LAYER2_RADIUS_MIN - ANGLE_VARIATION_FADE_RANGE,
                        LAYER2_RADIUS_MIN,
                        INNER_ANGLE_VARIATION,
                        ANGLE_VARIATION
                    );
            ...
            double y = VERTICAL_POSITION_MIN
                + rand.nextDouble() * (VERTICAL_POSITION_MAX - VERTICAL_POSITION_MIN);
            r.addPoint(x, y, z);
            // ReikaJavaLibrary.pConsole("For ray "+ang+" @ "+d+", a="+Math.toDegrees(a)+"
            // out of "+var);
        }
        if (r.points.size() <= 2)
    ...
    public String getStateMessage() {
        return "Generated sky rivers, with " + rays.size() + " rays and "
            + serverPoints.totalSize() + " points.";
    }
            ...
            for (int i = 0; i < points.size() - 1; i++) {
                ...
                if (dst < maxDst) {
                    ...
                    Vec3 vec = Vec3.createVectorHelper(
                        (next.xCoord - pos.xCoord) / pointsToAdd,
                        (next.yCoord - pos.yCoord) / pointsToAdd,
                        (next.zCoord - pos.zCoord) / pointsToAdd
                    );
                    for (int j = 0; j < Math.round(pointsToAdd); j++) {
                        newPoints.add(new DecimalPosition(
                            pos.xCoord + (vec.xCoord * j),
                            pos.yCoord + (vec.yCoord * j),
                            pos.zCoord + (vec.zCoord * j)
                        ));
                    }
                }
            }
    ...
    public static class RiverPoint {
        ...
        public RiverPoint(
            int id,
            int len,
            ChunkCoordIntPair ch,
            DecimalPosition pos,
            DecimalPosition prev,
