...
public class EnderPoolGenerator extends WorldGenerator {
    ...
    private boolean
    generateRandomShapeAt(World world, Random r, int x, int y, int z, int max) {
    ...
    private boolean generateCircularPool(
        World world, int x, int y, int z, Random rand, Block id, BlockArray blocks
    ) {
            ...
            for (int j = -r[i] - 1; j <= r[i] + 1; j++) {
                for (int k = -r[i] - 1; k <= r[i] + 1; k++) {
                    ...
                    if (dd <= r[i] + 1.5) {
                        ...
                        if (world.getBlock(x + j, y + i + 1, z + k)
    ...
    private boolean generateEllipticalPool(
        World world, int x, int y, int z, Random rand, Block id, BlockArray blocks
    ) {
        int d = 2 + rand.nextInt(3);
        int[] r = { d, d - 1, d - 3 };
