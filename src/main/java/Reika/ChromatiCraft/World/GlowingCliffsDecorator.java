...
public class GlowingCliffsDecorator extends BiomeDecorator {
    ...
    GlowingCliffsDecorator() {
        ...
        gravelGen
            = new NoFloatingGravelVein(/*((WorldGenMinable)gravelGen).numberOfBlocks*/ 32
            );
    }
    ...
    @Override
    public void
    decorateChunk(World world, Random rand, BiomeGenBase biome, int chunkX, int chunkZ) {
    ...
    protected void genDecorations(BiomeGenBase biome) {
        ...
        for (i = 0; i < sandPerChunk2; i++) {
            ...
            sandGen.generate(
                decoWorld, randomGenerator, j, decoWorld.getTopSolidOrLiquidBlock(j, k), k
            );
        }
        ...
        for (j = 0; j < waterlilyPerChunk; j++) {
            ...
            for (i1 = this.nextInt(decoWorld.getHeightValue(k, l) * 2);
                 i1 > 0 && decoWorld.isAirBlock(k, i1 - 1, l);
                 --i1) {
                ;
            }
        ...
        /*
        doGen = TerrainGen.decorate(decoWorld, randomGenerator, chunk_X, chunk_Z,
        PUMPKIN); if (randomGenerator.nextInt(32) == 0) { j = chunk_X +
        randomGenerator.nextInt(16) + 8; k = chunk_Z + randomGenerator.nextInt(16) + 8; l
        = this.nextInt(decoWorld.getHeightValue(j, k) * 2); (new
        WorldGenPumpkin()).generate(decoWorld, randomGenerator, j, l, k);
        }
         */
        ...
        for (int a = 0; a < flowsPerChunk; a++) {
            for (j = 0; j < 50; j++) {
                ...
                (new WorldGenLiquids(Blocks.flowing_water))
                    .generate(decoWorld, randomGenerator, k, l, i1);
            }

            for (j = 0; j < 20; j++) {
                k = chunk_X + randomGenerator.nextInt(16) + 8;
                l = randomGenerator.nextInt(
                    randomGenerator.nextInt(randomGenerator.nextInt(240) + 8) + 8
                );
                i1 = chunk_Z + randomGenerator.nextInt(16) + 8;
    ...
    public void genIslandDecorations(World world, BiomeGenBase biome, Island is) {
        ...
        for (i = 0; i < sandPerChunk2; i++) {
            ...
            sandGen.generate(
                islandWorld, islandRandom, j, is.getTopY(islandWorld, j, k), k
            );
        }
        ...
        for (int a = 0; a < flowsPerChunk; a++) {
            for (j = 0; j < 50; j++) {
                ...
                (new WorldGenLiquids(Blocks.flowing_water))
                    .generate(islandWorld, islandRandom, k, l, i1);
            }
