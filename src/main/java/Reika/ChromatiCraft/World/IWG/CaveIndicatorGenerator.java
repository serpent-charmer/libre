...
public class CaveIndicatorGenerator implements RetroactiveGenerator, ChromaDecorator {
            ...
            for (int i = 0; i < 16; i++) {
                for (int k = 0; k < 16; k++) {
                    ...
                    if (BiomeGlowingCliffs.isGlowingCliffs(
                            world.getBiomeGenForCoords(posX, posZ)
                        )) {
                        int maxy = 64;
                        for (int n = 0; n < 2; n++) {
                            ...
                            if (this.canGenerateAt(world, posX, posY, posZ)) {
                                world.setBlock(
                                    ...
                                    posY,
                                    posZ,
                                    ChromaBlocks.CAVEINDICATOR.getBlockInstance()
                                );
                            } else {
                            }
                        }
    ...
    public static boolean canGenerateAt(World world, int x, int y, int z) {
        ...
        if (ida != Blocks.air && !ReikaWorldHelper.softBlocks(world, x, y + 1, z))
            return false;
        return canGenerateIn(world, x, y, z) && !world.canBlockSeeTheSky(x, y + 1, z)
            && world.getBlockLightValue(x, y, z)
            < 8; // && ReikaWorldHelper.checkForAdjBlock(world, x, y, z,
                 // ChromaBlocks.CAVEINDICATOR.getBlockInstance()) == null;
    }
