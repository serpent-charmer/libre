...
public class LumaGenerator implements RetroactiveGenerator, ChromaDecorator {
                            ...
                            for (int j = -ry; j <= ry; j++) {
                                if (ReikaMathLibrary.isPointInsideEllipse(
                                    ...
                                    if (ReikaRandomHelper.doWithChance(c)
                                        && this.isValidLocation(world, dx, dy, dz)) {
                                        world.setBlock(
                                            dx,
                                            dy,
                                            dz,
    ...
    private boolean isValidLocation(World world, int x, int y, int z) {
        if (!world.getBlock(x, y, z).isReplaceableOreGen(world, x, y, z, Blocks.stone)
            && !world.getBlock(x, y, z).isReplaceableOreGen(world, x, y, z, Blocks.dirt))
            return false;
        ...
        for (int i = 0; i < 6; i++) {
            ...
            if (dir != ForgeDirection.UP) {
                ...
                if (b != ChromaBlocks.LUMA.getBlockInstance()
    ...
    private boolean generateIn(World world) {
        return Math.abs(world.provider.dimensionId) != 1
