...
public class UnknownArtefactGenerator implements RetroactiveGenerator {
    public static final UnknownArtefactGenerator instance
        = new UnknownArtefactGenerator();
        ...
        for (int i = 0; i < Towers.towerList.length; i++) {
            ...
            for (double a = 0; a < 360; a += 2) {
                ...
                for (int r = INNER_RADIUS; r <= OUTER_RADIUS; r += 8) {
                    ...
                    ChunkCoordIntPair p2
                        = new ChunkCoordIntPair((dx >> 4) << 4, (dz >> 4) << 4);
                    UAChunks.add(p2);
                    //ReikaJavaLibrary.pConsole("Artefact: Tower "+t+" @ "+p+" from seed
                    //"+world.getSeed()+" > "+p2, t == Towers.ALPHA);
                }
            }
        }
    }
        ...
        if (this.canGenerateIn(world)) {
            ...
            if (this.isGennableChunk(world, chunkX * 16, chunkZ * 16, random)) {
                if (this.generate(world, chunkX * 16, chunkZ * 16, random)) {
                    //ChromatiCraft.logger.log("Successful generation of "+s.name()+" at
                    //"+chunkX*16+", "+chunkZ*16);
                }
            }
        }
    ...
    public boolean isUAChunk(World world, int cx, int cz) {
        ...
        if (UAChunks.isEmpty() || cachedSeed != world.getSeed())
            this.calculateUAChunks(world);
        return this.canGenerateIn(world)
            && UAChunks.contains(new ChunkCoordIntPair(cx, cz));
    }

    public static boolean canGenerateArtefactAt(
        World world, int x, int y, int z
    ) { //the artefact would be at x, y-1, z
        //ReikaJavaLibrary.pConsole("Attempting @ "+x+", "+y+", "+z);
        Block b = world.getBlock(x, y, z);
        BiomeGenBase biome = world.getBiomeGenForCoords(x, z);
        //if (biome instanceof BiomeGenOcean || BiomeDictionary.isBiomeOfType(biome,
        //Type.WATER)) 	return false;
        ...
        if (!(biome instanceof BiomeGenOcean)
            && world.getBlock(x, y - 1, z) != biome.fillerBlock) {
            //ReikaJavaLibrary.pConsole("Not dirt");
            return false;
        }
        if (ReikaWorldHelper.checkForAdjMaterial(world, x, y - 1, z, Material.air)
            != null) {
    ...
    private static boolean isValidSurfaceBlock(World world, int x, int y, int z) {
        ...
        if (b instanceof BlockBush)
            return true;
        if (b == Blocks.water
            && world.getBiomeGenForCoords(x, z) instanceof BiomeGenOcean)
            return true;
        return false;
    }
