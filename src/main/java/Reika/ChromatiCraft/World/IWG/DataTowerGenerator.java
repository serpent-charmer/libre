...
public class DataTowerGenerator implements RetroactiveGenerator {
    ...
    private boolean generate(World world, int cx, int cz, Random rand) {
        ...
        if (c == null) {
            ...
            for (int i = -r; i <= r; i++) {
                for (int k = -r; k <= r; k++) {
                    int dx = x + i;
                    int dz = z + k;
                    while (ReikaWorldHelper.softBlocks(world, dx, y - 1, dz)
                           || ReikaBlockHelper.isLeaf(world, dx, y - 1, dz)
                           || ReikaBlockHelper.isWood(world, dx, y - 1, dz)) {
                        y--;
                    }
                }
            }
        ...
        world.setBlock(
            dx, y - 1, dz, ChromaBlocks.LOOTCHEST.getBlockInstance(), rand.nextInt(4), 3
        );
    ...
    private int prepareForTower(World world, int x, int z) {
        ...
        while (b.isAir(world, x, y, z) || ReikaBlockHelper.isLeaf(world, x, y, z)
    ...
    private void generateLoot(TileEntityLootChest te, Random rand) {
        te.populateChest(
            ChestGenHooks.STRONGHOLD_LIBRARY, ChromaStructures.CAVERN, 2, rand
        );
