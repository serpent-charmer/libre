...
public class DungeonGenerator implements RetroactiveGenerator {
    ...
    private EnumMap<ChromaStructures, VoronoiNoiseGenerator> structs
        = new EnumMap(ChromaStructures.class);
    private EnumMap<ChromaStructures, HashMap<WorldChunk, StructureGenStatus>> statusMap
        = new EnumMap(ChromaStructures.class);
    private EnumMap<ChromaStructures, TileEntityCache<StructureGenData>> structureMap
        = new EnumMap(ChromaStructures.class);

    private DungeonGenerator() {
        ChromaStructures[] li
            = { ChromaStructures.CAVERN,     ChromaStructures.BURROW,
                ChromaStructures.OCEAN,      ChromaStructures.DESERT,
                ChromaStructures.SNOWSTRUCT, ChromaStructures.BIOMEFRAG };
    ...
    private void updateNoisemaps(World world) {
        for (Entry<ChromaStructures, VoronoiNoiseGenerator> e : structs.entrySet()) {
            ...
            if (v == null || v.seed != sd) {
                v = (VoronoiNoiseGenerator) new VoronoiNoiseGenerator(
                        sd
                        ^ (ReikaFileReader
                               .getRealPath(world.getSaveHandler().getWorldDirectory())
                               .hashCode()
                           + s.ordinal())
                )
                        .setFrequency(0.75D / this.getNoiseScale(s));
                v.randomFactor = 0.55;
                e.setValue(v);
            }
        }
    ...
    private Collection<WorldLocation>
    getNearbyZones(ChromaStructures s, WorldServer world, double x, double z, double r) {
        this.updateNoisemaps(world);
        Collection<DecimalPosition> li = structs.get(s).getCellsWithin2D(x, z, r);
        //ReikaJavaLibrary.pConsole("Found all potential zones within "+r+" of "+x+",
        //"+z+": "+li);
    ...
    getNearestZone(ChromaStructures s, WorldServer world, double x, double z, double r) {
        return this.getNearestZone(s, world, x, z, r, null);
    }

    private WorldLocation getNearestZone(
        ChromaStructures s,
        WorldServer world,
        double x,
        double z,
        double r,
        WorldLocation exclude
    ) {
        Collection<WorldLocation> c = this.getNearbyZones(
            s, world, MathHelper.floor_double(x), MathHelper.floor_double(z), r
        );
    ...
    /** Block coords! */
    public StructureSeekData getNearestRealStructure(
        ChromaStructures s,
        WorldServer world,
        double x,
        double z,
        double r,
        boolean requireGenned
    ) {
        this.updateNoisemaps(world);
        ...
        if (!requireGenned && li.isEmpty()) { //no generated valid, consult noise
            ...
            while (it2.hasNext()) {
                ...
                StructureGenStatus stat = this.getGenStatus(
                    s,
                    world,
                    MathHelper.floor_double(loc.xCoord),
                    MathHelper.floor_double(loc.zCoord)
                );
                if (!stat.hasStructure() && stat.isFinalized())
                    it2.remove();
                else if (requireGenned && !stat.isGenerated())
    ...
    /** In BLOCK coords */
    public StructureGenStatus
    getGenStatus(ChromaStructures s, WorldServer world, int x, int z) {
    ...
    /** In CHUNK coords */
    private void markChunkStatus(
        World world, int x, int z, ChromaStructures s, StructureGenStatus stat
    ) {
    ...
    public void recacheStructureTile(TileEntityStructControl te) {
        this.markChunkStatus(
            te.worldObj,
            te.xCoord >> 4,
            te.zCoord >> 4,
            te.getStructureType(),
            StructureGenStatus.SUCCESS
        );
    }
    ...
    private HashMap<WorldChunk, StructureGenStatus>
    loadMapFromNBT(ChromaStructures s, NBTTagCompound tag) {
    ...
    private void
    writeMapToNBT(NBTTagCompound tag, HashMap<WorldChunk, StructureGenStatus> map) {
    ...
    public void updateStatusCacheFile() {
        ...
        if (f == null)
            throw new IllegalStateException(
                "Could not find a world to define structure status cache!"
            );
    ...
    private int getNoiseScale(ChromaStructures s) {
        switch (s) {
            ...
            case OCEAN:
                return 640; //was 1024, but this causes too-rare due to cave req in new
                            //voronoi system
            case CAVERN:
                return 144;
            case BURROW:
    ...
    public void onGenerateStructure(
        ChromaStructures s,
        FragmentStructureBase s2,
        World world,
        TileEntityStructControl te
    ) {
        s2.onPlace(world, te);
    }
    ...
    private boolean
    checkChunk(World world, int chunkX, int chunkZ, Random random, ChromaStructures s) {
        if (!this.isVoidWorld(world, chunkX * 16, chunkZ * 16)
            && this.isGennableChunk(world, chunkX * 16, chunkZ * 16, s)
                == StructureGenStatus.PLANNED) {
            //ReikaWorldHelper.forceGenAndPopulate(world, chunkX*16, chunkZ*16, s ==
            //Structures.OCEAN ? 2 : 1); causes extra structures
            ChunkCoordIntPair pos = this.tryGenerateInChunk(
                world,
                chunkX * 16,
                chunkZ * 16,
                random,
                s,
                ChromaOptions.getStructureTriesPerChunk()
            );
            if (pos == null)
                pos = this.tryGenerateInChunksAround(
                    world,
                    chunkX * 16,
                    chunkZ * 16,
                    random,
            ...
            if (pos != null) {
                this.markChunkStatus(
                    world, chunkX, chunkZ, s, StructureGenStatus.INERT_GEN
                );
                this.markChunkStatus(
                    world, pos.chunkXPos, pos.chunkZPos, s, StructureGenStatus.SUCCESS
                );
                //ChromatiCraft.logger.log("CC STRUCTURE STATUS: Successful generation of
                //"+s.name()+" at "+chunkX*16+", "+chunkZ*16);
                return true;
            } else {
                //ChromatiCraft.logger.log("CC STRUCTURE STATUS: Failed to generate a
                //"+s.name()+" at "+chunkX*16+", "+chunkZ*16+"; this grid cell is dead");
                this.markChunkStatus(
                    world, chunkX, chunkZ, s, StructureGenStatus.FAILURE
                );
            }
        } else {
            //ChromatiCraft.logger.log("CC STRUCTURE STATUS: Not generating a "+s.name()+"
            //at "+chunkX*16+", "+chunkZ*16+"; not planned"); this.markChunkStatus(world,
            //chunkX << 4, chunkZ << 4, s, StructureStatus.INERT);
        }
        return false;
    }

    private ChunkCoordIntPair tryGenerateInChunksAround(
        World world, int cx, int cz, Random rnd, ChromaStructures s, int tries
    ) {
        int r = 16;
            ...
            for (int i = -r; i <= r; i += 16) {
                for (int k = -r; k <= r; k += 16) {
                    ...
                    ChunkCoordIntPair ret
                        = this.tryGenerateInChunk(world, dx, dz, rnd, s, tries);
                    if (ret != null)
                        return ret;
                }
            }
    ...
    private ChunkCoordIntPair tryGenerateInChunk(
        World world, int cx, int cz, Random r, ChromaStructures s, int tries
    ) {
        ...
        while (!flag && n < tries) {
            ...
            switch (s) {
                ...
                case CAVERN: {
                    ...
                    FilledBlockArray struct
                        = ChromaStructures.CAVERN.getArray(world, x, y, z, r);
                    if (this.isValidCavernLocation(world, x, y, z, struct)) {
                        ...
                        for (int i = 7; i < 18; i++) {
                            ...
                            if (b.isAir(world, dx, y, z)
                                && b2.isAir(world, dx, y - 1, z)) {
                                break;
                            } else {
                        ...
                        //ChromatiCraft.logger.log("Successful generation of "+s.name()+"
                        //at "+x+","+y+","+z);
                        try {
                            world.setBlock(
                                x,
                                y,
                                z,
                                ChromaTiles.STRUCTCONTROL.getBlock(),
                                ChromaTiles.STRUCTCONTROL.getBlockMetadata(),
                                3
                            );
                            ...
                            ((GeneratedStructureBase) s.getStructure())
                                .addError(new Exception(
                                    "Failed to place controller @ " + x + ", " + y + ", "
                                        + z,
                                    e
                                ));
                        }
                ...
                case BURROW: {
                    ...
                    FilledBlockArray arr
                        = ChromaStructures.BURROW.getArray(world, x, y, z, r, e);
                    if (this.isValidBurrowLocation(world, x, y, z, arr)) {
                        ((GeneratedStructureBase) s.getStructure()).markForWorldgen();
                        arr.place(2);
                        TileEntityStructControl te = null;
                        //world.setBlockMetadataWithNotify(x-7, y-5, z-2, 5, 3); //that
                        //chest that never points right
                        //ChromatiCraft.logger.log("Successful generation of "+s.name()+"
                        //at "+x+","+y+","+z);
                        ...
                        if (r.nextInt(2) == 0) {
                            furn = this.tryGenerateBurrowFurnaceRoom(
                                world, x, y, z, s, arr, r
                            );
                            if (furn != null) {
                                if (r.nextInt(2) == 0) {
                                    loot = this.tryGenerateBurrowLootRoom(
                                        world, x, y, z, s, arr, r
                                    );
                                }
                            }
                        ...
                        try {
                            ...
                            te = (TileEntityStructControl
                            ) world.getTileEntity(x - 5, y - 8, z - 2);
                            te.generate(s, e);
                            ...
                            ((GeneratedStructureBase) s.getStructure())
                                .addError(new Exception(
                                    "Failed to place controller @ " + (x - 5) + ", "
                                        + (y - 8) + ", " + (z - 2),
                                    ex
                                ));
                        }
                        ...
                        if (furn != null)
                            this.modifyBlocks(
                                s, furn, r, Modify.MOSSIFY, Modify.GRASSDIRT
                            );
                        if (loot != null)
                            this.modifyBlocks(
                                s, loot, r, Modify.MOSSIFY, Modify.GRASSDIRT
                            );
                        flag = true;
                        this.logErrors(world, x, y, z, s, te);
                    }
                ...
                case OCEAN: {
                    ...
                    if (b == Blocks.water || b == Blocks.flowing_water) {
                        ...
                        //{ 	y--; 	b = world.getBlock(x, y, z);
                        //}
                        FilledBlockArray struct
                            = ChromaStructures.OCEAN.getArray(world, x, y, z, r);
                ...
                case DESERT: {
                    ...
                    if (this.isValidBiomeForDesertStruct(biome)) {
                        ...
                        FilledBlockArray struct
                            = ChromaStructures.DESERT.getArray(world, x, y, z, r);
                        ...
                        if (this.isValidDesertLocation(world, x, y, z, struct)) {
                            ...
                            try {
                                ...
                                te = (TileEntityStructControl
                                ) world.getTileEntity(x + 7, y + 3, z + 7);
                                te.generate(s, CrystalElement.WHITE);
                                rx = te.xCoord;
                                rz = te.zCoord;
                                this.onGenerateStructure(s, s2, world, te);
                                ...
                                ((GeneratedStructureBase) s.getStructure())
                                    .addError(new Exception(
                                        "Failed to place controller @ " + (x + 7) + ", "
                                            + (y + 3) + ", " + (z + 7),
                                        e
                                    ));
                            }
                            ...
                            for (int k = 0; k < struct.getSize(); k++) {
                                ...
                                if (c.yCoord > struct.getMaxY() - 2) {
                                    ...
                                    if (b1 == Blocks.air && b2 == Blocks.sand
                                        && ReikaRandomHelper.doWithChance(
                                            20
                                            + 30D
                                                * Math.abs(
                                                    Math.abs(c.xCoord - struct.getMidX())
                                                    + Math.abs(
                                                        c.zCoord - struct.getMidZ()
                                                    )
                                                )
                                                / 7D
                                        )) {
                            ...
                            for (int k1 = struct.getMinX(); k1 <= struct.getMaxX();
                                 k1++) {
                                for (int k2 = struct.getMinZ(); k2 <= struct.getMaxZ();
                                     k2++) {
                                    Coordinate c = new Coordinate(
                                        k1, this.getTop(world, k1, k2), k2
                                    );
                                    if (c.getBlock(world) == Blocks.air
                                        && c.offset(0, -1, 0).getBlock(world)
                                            == Blocks.sand
                                        && ReikaRandomHelper.doWithChance(2)) {
                                        if (c.offset(1, 0, 0).getBlock(world)
                                                == Blocks.air
                                            && c.offset(-1, 0, 0).getBlock(world)
                                                == Blocks.air) {
                                            if (c.offset(0, 0, 1).getBlock(world)
                                                    == Blocks.air
                                                && c.offset(0, 0, -1).getBlock(world)
                                                    == Blocks.air) {
                                                c.setBlock(world, Blocks.cactus);
                                                if (ReikaRandomHelper.doWithChance(40)) {
                                                    c.offset(0, 1, 0).setBlock(
                                                        world, Blocks.cactus
                                                    );
                                                    if (ReikaRandomHelper.doWithChance(40
                                                        ))
                                                        c.offset(0, 2, 0).setBlock(
                                                            world, Blocks.cactus
                                                        );
                                                }
                                            }
                ...
                case SNOWSTRUCT: {
                    int y = this.getTop(world, x, z) - 1;
                    FilledBlockArray arr
                        = ChromaStructures.SNOWSTRUCT.getArray(world, x, y, z, r);
                    if (this.isValidSnowStructLocation(world, x, y, z, arr)) {
                        ...
                        try {
                            ...
                            te = (TileEntityStructControl
                            ) world.getTileEntity(x + 8, y - 3, z + 6);
                            te.generate(s, CrystalElement.WHITE);
                            rx = te.xCoord;
                            rz = te.zCoord;
                            this.onGenerateStructure(s, s2, world, te);
                            ...
                            ((GeneratedStructureBase) s.getStructure())
                                .addError(new Exception(
                                    "Failed to place controller @ " + (x + 8) + ", "
                                        + (y - 3) + ", " + (z + 6),
                                    e
                                ));
                        }
                ...
                case BIOMEFRAG: {
                    ...
                    FilledBlockArray arr
                        = ChromaStructures.BIOMEFRAG.getArray(world, x, y, z, r);
                    arr.sink(
                        world,
                        Material.wood,
                        Material.leaves,
            ...
            if (flag) {
                if (rx != cx || rz != cz)
                    this.markChunkStatus(
                        world, cx >> 4, cz >> 4, s, StructureGenStatus.INERT_GEN
                    );
    ...
    private int getTop(World world, int x, int z) {
        ...
        Block b = world.getBlock(x, y, z);
        while (y >= 0
               && (b.isAir(world, x, y, z) || b.isWood(world, x, y, z)
    ...
    private void logErrors(
        World world, int x, int y, int z, ChromaStructures s, TileEntityStructControl te
    ) {
        Collection<Exception> errors
            = ((GeneratedStructureBase) s.getStructure()).getErrors();
        if (!errors.isEmpty()) { /*
            ...
            ChromatiCraft.logger.logError(
                "Structure " + s + " encountered " + errors.size()
                + " errors during generation @ " + x + ", " + y + ", " + z
                + "; Error ID: " + id
            );
            int top = this.getTop(world, x, z) - 1;
            ReikaJavaLibrary.pConsole(
                "Regional metadata: Biome - " + world.getBiomeGenForCoords(x, z)
                + "; top block: " + BlockKey.getAt(world, x, top, z) + " @ y=" + top
    ...
    private boolean isValidBiomeForDesertStruct(BiomeGenBase biome) {
        return biome.topBlock == Blocks.sand
    ...
    private boolean
    isValidDesertLocation(World world, int x, int y, int z, FilledBlockArray struct) {
        if (!this.isValidBiomeForDesertStruct(
                world.getBiomeGenForCoords(struct.getMinX(), struct.getMinZ())
            ))
            return false;
        if (!this.isValidBiomeForDesertStruct(
                world.getBiomeGenForCoords(struct.getMaxX(), struct.getMinZ())
            ))
            return false;
        if (!this.isValidBiomeForDesertStruct(
                world.getBiomeGenForCoords(struct.getMinX(), struct.getMaxZ())
            ))
            return false;
        if (!this.isValidBiomeForDesertStruct(
                world.getBiomeGenForCoords(struct.getMaxX(), struct.getMaxZ())
            ))
            return false;
        if (world.getBlock(struct.getMinX(), struct.getMinY() + 3, struct.getMinZ())
            == Blocks.air)
            return false;
        if (world.getBlock(struct.getMaxX(), struct.getMinY() + 3, struct.getMinZ())
            == Blocks.air)
            return false;
        if (world.getBlock(struct.getMinX(), struct.getMinY() + 3, struct.getMaxZ())
            == Blocks.air)
            return false;
        if (world.getBlock(struct.getMaxX(), struct.getMinY() + 3, struct.getMaxZ())
            == Blocks.air)
            return false;
        return true;
    }
    ...
    private boolean isValidBiome(ChromaStructures s, BiomeGenBase b) {
        ...
        switch (s) {
            ...
            case CAVERN:
                return true;
            case BURROW:
                return !ocean && b.topBlock == Blocks.grass
                    && !this.isValidBiome(ChromaStructures.SNOWSTRUCT, b)
                    && !ChromatiCraft.isCCBiome(b);
            case DESERT:
                return !ocean && this.isValidBiomeForDesertStruct(b);
            case SNOWSTRUCT:
                return b != BiomeGenBase.biomeList[BiomeGenBase.icePlains.biomeID + 128]
                    && !ocean && b.topBlock == Blocks.grass && b.getEnableSnow()
                    && ReikaBiomeHelper.getBiomeDecorator(b).treesPerChunk < 1
                    && !b.biomeName.toLowerCase(Locale.ENGLISH).contains("forest");
            case BIOMEFRAG:
    ...
    private FilledBlockArray tryGenerateBurrowFurnaceRoom(
        World world,
        int x,
        int y,
        int z,
        ...
        FilledBlockArray arr2
            = ((BurrowStructure) s.getStructure()).getFurnaceRoom(world, x, y, z);
    ...
    private FilledBlockArray tryGenerateBurrowLootRoom(
        World world,
        int x,
        int y,
        int z,
        ...
        FilledBlockArray arr2
            = ((BurrowStructure) s.getStructure()).getLootRoom(world, x, y, z);
        for (Coordinate c : arr2.keySet()) {
            if (c.isEmpty(world))
                return null;
        }
    ...
    public static enum Modify {
                ...
                case ADJTREES2:
                    for (int x = arr.getMinX() - 1; x <= arr.getMaxX() + 1; x++) {
                        for (int z = arr.getMinZ() - 1; z <= arr.getMaxZ() + 1; z++) {
                            boolean walls = x >= arr.getMinX() && x <= arr.getMaxX()
                                && z >= arr.getMinZ() && z <= arr.getMaxZ();
                            for (int y = arr.getMinY(); y <= arr.getMaxY() + 2; y++) {
                                if (ReikaBlockHelper.isWood(arr.world, x, y, z)
                                    || (walls
                                        && ReikaBlockHelper.isLeaf(arr.world, x, y, z))) {
                ...
                case CLEANENTRANCE:
                    if (s == ChromaStructures.BIOMEFRAG) {
                        for (int dx = arr.getMinX() - 2; dx <= arr.getMaxX() + 2; dx++) {
                            for (int dz = arr.getMinZ() - 2; dz <= arr.getMaxZ() + 2;
                                 dz++) {
                                if (dx < arr.getMinX() || dx > arr.getMaxX()
                                    || dz < arr.getMinZ() || dz > arr.getMaxZ()) {
                                    int ymin = arr.getMaxY() - 3;
                                    if (dx >= arr.getMinX() - 1 && dx <= arr.getMaxX() + 1
                                        && dz >= arr.getMinZ() - 1
                                        && dz <= arr.getMaxZ() + 1) {
                                        ymin -= 2;
                                    }
                                    ...
                                    if (b.isWood(arr.world, dx, y0 + 1, dz)
                                        || b.isLeaves(arr.world, dx, y0 + 1, dz)
                                        || b0.isWood(arr.world, dx, y0, dz)
                                        || b0.isLeaves(arr.world, dx, y0, dz)) {
                                        continue;
                                    }
                                    for (int dy = y0; dy >= ymin; dy--) {
                                        b = arr.world.getBlock(dx, dy, dz);
                                        if (ReikaWorldHelper.softBlocks(
                                                arr.world, dx, dy, dz
                                            )
                                            ...
                                            if (!ReikaWorldHelper.softBlocks(
                                                    arr.world, dx, dy - 1, dz
                                                ))
                                                arr.world.setBlock(
                                                    dx, dy - 1, dz, Blocks.grass
                                                );
                                        } else
                                            break;
                                    }
        ...
        private boolean
        applyEach(ChromaStructures s, FilledBlockArray arr, Coordinate c, Random r) {
            switch (this) {
                case ADJTREES: {
                    for (int i = 0; i < 6; i++) {
                        ...
                        if (b != Blocks.air
                            ...
                            if (tree != null) {
                                ...
                                barr.recursiveMultiAddWithBounds(
                                    arr.world,
                                    c2.xCoord,
                                    c2.yCoord,
                                    c2.zCoord,
                ...
                case CLEANENTRANCE: {
                    if (s == ChromaStructures.SNOWSTRUCT) {
                        ...
                        if (b == ChromaBlocks.STRUCTSHIELD.getBlockInstance()
                            && c.getBlockMetadata(arr.world) == BlockType.MOSS.metadata) {
                                ...
                                for (int i = 1; i < 5; i++) {
                                    for (int dx = -i; dx <= i; dx++) {
                                        for (int dz = -i; dz <= i; dz++) {
                                            ...
                                            if (b2 == Blocks.grass
                                                || b2.getMaterial() == Material.ground
                                                || b2.getMaterial() == Material.plants
                                                || b2 == Blocks.stone
                                                || b2 == Blocks.snow_layer) {
                ...
                case GRASSDIRT: {
                    Block b = c.getBlock(arr.world);
                    if (b == Blocks.dirt) {
                        if (arr.world.getBlockLightValue(c.xCoord, c.yCoord + 1, c.zCoord)
                            > 8) {
                ...
                case MOSSIFY: {
                    ...
                    if (b == ChromaBlocks.STRUCTSHIELD.getBlockInstance()) {
                        ...
                        if (meta == BlockType.STONE.metadata) {
                            ...
                            if (r.nextInt(ct) == 0) {
                                arr.world.setBlockMetadataWithNotify(
                                    c.xCoord,
                                    c.yCoord,
                                    c.zCoord,
                ...
                case ENCRUSTED: {
                    ...
                    if (r.nextInt(n) == 0) {
                        ...
                        if (b == ChromaBlocks.STRUCTSHIELD.getBlockInstance()) {
                            for (int i = 0; i < 6; i++) {
                                Coordinate c2
                                    = c.offset(ForgeDirection.VALID_DIRECTIONS[i], 1);
                                Block b2 = c2.getBlock(arr.world);
                                if (b2 == Blocks.air) {}
                            }
                        }
    ...
    public void
    modifyBlocks(ChromaStructures s, FilledBlockArray arr, Random r, Modify... flags) {
        for (Modify m : flags) {
            try {
                m.apply(s, arr, r);
            } catch (Exception e) {
                ((GeneratedStructureBase) s.getStructure())
                    .addError(new Exception("Failed to run pass '" + m + "'", e));
            }
        }
        for (int k = 0; k < arr.getSize(); k++) {
            Coordinate c = arr.getNthBlock(k);
            for (Modify m : flags) {
                try {
                    ...
                    ((GeneratedStructureBase) s.getStructure())
                        .addError(new Exception(
                            "Failed to run localized pass '" + m + "' @ " + c, e
                        ));
                }
            }
        }
    ...
    public void programSpawners(ChromaStructures s, FilledBlockArray arr) {
        switch (s) {
            case OCEAN:
                for (Coordinate c : ((GeneratedStructureBase) s.getStructure())
                                        .getCachedBlocks(Blocks.mob_spawner)) {
                    Block b = c.getBlock(arr.world);
                    if (b == Blocks.mob_spawner) {
                        TileEntityMobSpawner te = (TileEntityMobSpawner
                        ) arr.world.getTileEntity(c.xCoord, c.yCoord, c.zCoord);
                        ...
                        ReikaSpawnerHelper.setMobSpawnerMob(
                            te,
                            (String
                            ) EntityList.classToStringMapping.get(EntityCreeper.class)
                        );
                    }
                }
                break;
            case DESERT:
                for (Coordinate c : ((GeneratedStructureBase) s.getStructure())
                    ...
                    if (b == Blocks.mob_spawner) {
                        ...
                        Class mob = c.yCoord <= arr.getMinY() + 4 ? EntityBlaze.class
                            : Math.abs(c.xCoord - arr.getMinX())
                                == Math.abs(c.zCoord - arr.getMinZ())
                            ? EntitySpider.class
    ...
    public void populateChests(ChromaStructures struct, FilledBlockArray arr, Random r) {
        try {
            for (Coordinate c :
                 ((GeneratedStructureBase) struct.getStructure())
                     .getCachedBlocks(GeneratedStructureBase.getChestGen())) {
                Block b = c.getBlock(arr.world);
                if (b == ChromaStructureBase.getChestGen()) {
                    TileEntityLootChest te
                        = (TileEntityLootChest) c.getTileEntity(arr.world);
                    if (te.isUntouchedWorldgen()) {
                        int bonus = ((FragmentStructureBase) struct.getStructure())
                                        .getChestYield(c, te, arr, r);
                        this.populateChest(c, te, struct, arr, bonus, r);
                    } else {
                        ChromatiCraft.logger.logError(
                            "Structure " + struct + " gen in chest " + te + " @ " + c
                            + " failed because is not untouched worldgen."
                        );
                    }
                }
            }
            ...
            ((GeneratedStructureBase) struct.getStructure())
                .addError(new Exception("Failed to populate chests", e));
        }
    }
    ...
    public void populateChest(
        Coordinate c,
        TileEntityLootChest te,
        ChromaStructures struct,
        FilledBlockArray arr,
        int bonus,
        Random r
    ) {
        String s = ((FragmentStructureBase) struct.getStructure())
                       .getChestLootTable(c, te, arr, r);
        if (s == null) {
            return;
        }
    ...
    private void addSnowCover(FilledBlockArray arr, int r) {
        ...
        for (int k = 0; k < arr.getSize(); k++) {
            Coordinate c = arr.getNthBlock(k);
            if (c.isEmpty(arr.world) && Blocks.snow_layer.canPlaceBlockAt(arr.world,
        c.xCoord, c.yCoord, c.zCoord)) { if (arr.world.getPrecipitationHeight(c.xCoord,
        c.zCoord) <= c.yCoord) c.setBlock(arr.world, Blocks.snow_layer);
            }
        }
         */
        for (int x = arr.getMinX() - r; x <= arr.getMaxX() + r; x++) {
            for (int z = arr.getMinZ() - r; z <= arr.getMaxZ() + r; z++) {
                ...
                if (arr.world.getBlock(x, top, z) != Blocks.snow_layer
                    && arr.world.getBlock(x, top + 1, z) == Blocks.air) {
    ...
    private boolean
    isValidCavernLocation(World world, int x, int y, int z, FilledBlockArray arr) {
        boolean flag = false;
        for (int i = 6; i < 18; i++) {
            int dx = x + i;
            Block b = world.getBlock(dx, y, z);
    ...
    private boolean
    isValidBurrowLocation(World world, int x, int y, int z, FilledBlockArray arr) {
        ...
        for (int k = 0; k < arr.getSize(); k++) {
            ...
            if (this.getTop(world, dx, dz) < y - 2)
                return false;
            if (arr.hasBlockAt(dx, dy, dz, Blocks.stone)
                || arr.hasBlockAt(
                    dx, dy, dz, ChromaBlocks.STRUCTSHIELD.getBlockInstance()
                )) {
                if (b.isAir(world, dx, dy, dz)
                    || ReikaWorldHelper.checkForAdjMaterial(
                           world, dx, dy, dz, Material.air
                       ) != null) {
                    return false;
                }
            }
    ...
    private boolean
    isValidOceanLocation(World world, int x, int y, int z, FilledBlockArray struct) {
        //needs to be at least 8 blocks deep
        if (world.getBlock(x, y + 8, z) != Blocks.water
            && world.getBlock(x, y + 8, z) != Blocks.flowing_water) {
            ChromatiCraft.logger.debug(
                "Ocean Temple generation @ " + x + ", " + y + ", " + z
        ...
        boolean flag1 = true;
        boolean flag2 = true;
        FilledBlockArray cap
            = (FilledBlockArray) this.getEndcap1(world, x, y, z).offset(1, 0, 0);
        ...
        for (int k = 0; k < struct.getSize(); k++) {
            ...
            if (b == ChromaBlocks.STRUCTSHIELD.getBlockInstance()) {
                ChromatiCraft.logger.debug(
                    "Ocean Temple generation @ " + x + ", " + y + ", " + z
                    + " failed: Intersects other structure"
                );
                return false;
            }
        ...
        ChromatiCraft.logger.debug(
            "Ocean Temple generation @ " + x + ", " + y + ", " + z + " failed: No cave."
        );
        return false;
    }

    private boolean
    isValidSnowStructLocation(World world, int x, int y, int z, FilledBlockArray arr) {
        ...
        for (int d = 1; d <= 5; d++) { //starts at the top non-air
            ...
            if (id1 == Blocks.air || id2 == Blocks.air || id3 == Blocks.air
                || id4 == Blocks.air)
                return false;
            if (ReikaBlockHelper.isLiquid(id1) || ReikaBlockHelper.isLiquid(id2)
                || ReikaBlockHelper.isLiquid(id3) || ReikaBlockHelper.isLiquid(id4))
                return false;
        }
        return true;
    }
    ...
    private boolean
    isValidBiomeStructLocation(World world, int x, int y, int z, FilledBlockArray arr) {
        if (BiomeGlowingCliffs.isGlowingCliffs(world.getBiomeGenForCoords(x, z))
            && y > GlowingCliffsColumnShaper.MAX_MIDDLE_TOP_Y + 2)
            return false;
        int h1 = this.getTop(world, arr.getMinX(), arr.getMinZ());
        int h2 = this.getTop(world, arr.getMaxX(), arr.getMinZ());
        int h3 = this.getTop(world, arr.getMinX(), arr.getMaxZ());
    ...
    /** Block coords */
    public StructureGenStatus
    isGennableChunk(World world, int x, int z, ChromaStructures s) {
        this.updateNoisemaps(world);
        ArrayList<WorldLocation> c = new ArrayList(
            this.getNearbyZones(s, (WorldServer) world, x, z, this.getExclusionZone(s))
        );
        if (c.isEmpty())
    ...
    private boolean canGenerateIn(World world) {
        ...
        if (world.getWorldInfo().getTerrainType() == WorldType.FLAT) {
            return ChromaOptions.FLATGEN.getState()
                && ReikaWorldHelper.getSuperflatHeight(world) > 15;
        }
        ...
        if (PlanetDimensionHandler.isOtherWorld(world))
            return false;
        return !ChromatiCraft.config.isDimensionBlacklistedForStructures(
    ...
    private static class StructureGenData {
        ...
        public static StructureGenData readFromNBT(NBTTagCompound tag) {
            ...
            StructureGenData ret = new StructureGenData(
                WorldChunk.fromSerialString(tag.getString("chunk")), stat
            );
