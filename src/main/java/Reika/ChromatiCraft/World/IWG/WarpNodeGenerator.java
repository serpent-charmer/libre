...
public class WarpNodeGenerator implements RetroactiveGenerator {
    ...
    private ShuffledGrid getGrid(World world) {
        ...
        if (arr == null) {
            arr = new ShuffledGrid(
                GRIDSIZE, 20, 55
            ); //avg spacing every 880 blocks, with offsets up to 320m
            ...
            if (this.isGennableChunk(world, chunkX, chunkZ)) {
                ...
                if (ReikaRandomHelper.doWithChance(
                    ...
                    if (this.canGenerateAt(world, posX, posY, posZ)) {
                        world.setBlock(
                            posX, posY, posZ, ChromaBlocks.WARPNODE.getBlockInstance()
                        );
                        MinecraftForge.EVENT_BUS.post(
                            new NexusGenEvent(world, posX, posY, posZ, random)
                        );
                    }
                }
            }
    ...
    private int getMinY(World world, int x, int z, BiomeGenBase b) {
        ...
        if (BiomeGlowingCliffs.isGlowingCliffs(b)) {
            ...
            min = SFAPI.biomeHandler.getTrueTopAt(world, x, z)
                + 40; //SFAPI.biomeHandler.getBaseTerrainHeight(b);
        }
        return min;
    }
    ...
    private int getMaxY(World world, int x, int z, BiomeGenBase b) {
        ...
        if (BiomeGlowingCliffs.isGlowingCliffs(b)
            || (ModList.SATISFORESTRY.isLoaded() && SFAPI.biomeHandler.isPinkForest(b))) {
            max = 255;
        }
