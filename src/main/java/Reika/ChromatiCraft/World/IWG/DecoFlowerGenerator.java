            ...
            for (int i = 0; i < Flowers.list.length; i++) {
                ...
                if (random.nextInt(p.getGenerationChance()) == 0) {
                    ...
                    int n = random.nextInt(5) == 0
                        ? 1 + random.nextInt(4) + random.nextInt(6)
                        : 1;
                    int tries = 0;
                    while (done < n && tries < 40) {
                        ...
                        if (p.canGenerateIn(b)) {
                            int posY = ReikaRandomHelper.getRandomPlusMinus(
                                world.getTopSolidOrLiquidBlock(posX, posZ), 25
                            );
                            if (p != Flowers.FLOWIVY && p != Flowers.GLOWROOT) {
                                while (world.getBlock(posX, posY - 1, posZ)
                                           .isAir(world, posX, posY - 1, posZ)
                                       && posY > 0)
                                    posY--;
                                if (posY <= 0)
                                    continue;
                            ...
                            while (!world.getBlock(posX, posY, posZ)
                                        .isAir(world, posX, posY, posZ)
                                   && posY < 255)
                                posY++;
                            if (p == Flowers.GLOWROOT) {
                                while (posY < 255
                                       && !Flowers.GLOWROOT.canPlantAt(
                            ...
                            if (world.getBlock(posX, posY, posZ)
                                    .isAir(world, posX, posY, posZ)
                                && p.canPlantAt(world, posX, posY, posZ)) {
                                if (p == Flowers.FLOWIVY) {
                                    while (world.getBlock(posX, posY + 1, posZ)
                                               .isAir(world, posX, posY + 1, posZ)
                                           && p.canPlantAt(world, posX, posY + 1, posZ)) {
                                        posY++;
                                    }
                                }
                                if (p == Flowers.FLOWIVY
                                ...
                                if (p == Flowers.FLOWIVY) {
                                    ...
                                    for (int h = 0; h < g; h++) {
                                        if (world.getBlock(posX, posY - h, posZ)
                                                .isAir(world, posX, posY - h, posZ)
                                            && p.canPlantAt(
                                                world, posX, posY - h, posZ
                                            )) {
                                            world.setBlock(
                                                posX,
                                                posY - h,
