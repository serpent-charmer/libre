        ...
        while (!attempts.isEmpty()) {
            ...
            if (BiomeGlowingCliffs.isGlowingCliffs(b)) {
                ...
                if (region == GlowCliffRegion.WATER) {
                    ...
                    //	return;
                    int y = -16 + GlowingCliffsColumnShaper.MAX_MIDDLE_TOP_Y
                        + rand.nextInt(8);
                    if (this.generateAt(world, dx, y, dz, NodeClass.WATER))
                        break;
                } else if (region == GlowCliffRegion.SHORES) {
                    int y = -4 + GlowingCliffsColumnShaper.MAX_MIDDLE_TOP_Y
                        + rand.nextInt(8);
    ...
    private boolean generateAt(World world, int x, int y, int z, NodeClass c) {
        if (world.getBlock(x, y, z).isAir(world, x, y, z)
            ...
            && world.getBlock(x, world.getTopSolidOrLiquidBlock(x, z) + 1, z)
                == Blocks.water) {
            if (CrystalNetworker.instance.getNearestTileOfType(
                    world, x, y, z, TileEntitySkypeater.class, 32
                )
                != null)
                return false;
            world.setBlock(
                ...
                y,
                z,
                ChromaTiles.SKYPEATER.getBlock(),
                ChromaTiles.SKYPEATER.getBlockMetadata(),
                3
            );
