...
public class GlowingCliffsAuxGenerator implements RetroactiveGenerator {
    public static final GlowingCliffsAuxGenerator instance
        = new GlowingCliffsAuxGenerator();

    private static final int MAX_CONTOUR_HEIGHT = 6;
    private static final int SCALE_RANGE = 2;

    private static final BlockKey STONE = new BlockKey(
        ChromaBlocks.CLIFFSTONE.getBlockInstance(), Variants.STONE.getMeta(false, true)
    ); //Loader.isModLoaded("HarderUnderground") ? new BlockKey(Blocks.stone, 1) : new
       //BlockKey(Blocks.stone);
    private static final BlockKey DIRT = new BlockKey(Blocks.dirt
    ); //new BlockKey(ChromaBlocks.CLIFFSTONE.getBlockInstance(),
       //Variants.DIRT.getMeta(false, true));
    private static final BlockKey GRASS = new BlockKey(Blocks.grass
    ); //new BlockKey(ChromaBlocks.CLIFFSTONE.getBlockInstance(),
       //Variants.GRASS.getMeta(false, true));
    ...
    public static final HashMap<Coordinate, ImmutablePair<Integer, Integer>>
        TEMP_ISLAND_CACHE = new HashMap();
    ...
    public void generate(
        ...
        if (contours == null || noiseSeed != seed) {
            contours
                = new SimplexNoiseGenerator(seed >> 1).setFrequency(1 / 16D).addOctave(
                    2, 0.5, 1000
                );
        ...
        for (int i = 0; i < 16; i++) {
            for (int k = 0; k < 16; k++) {
                ...
                if (BiomeGlowingCliffs.isGlowingCliffs(biome)) {
                    for (int y = GlowingCliffsColumnShaper.SEA_LEVEL;
                         y <= GlowingCliffsColumnShaper.MAX_MIDDLE_TOP_Y + 1;
                         y++) {
                        ...
                        while (b == Blocks.sapling && n <= 8) {
                            ((BlockSapling)Blocks.sapling).func_149878_d(world, x, y, z,
                        rand); b = world.getBlock(x, y, z);
                        }
                        if (b == Blocks.sapling) {
                        ...
                        if (b == Blocks.sapling) {
                            ...
                            WorldGenAbstractTree tree
                                = ChromatiCraft.glowingcliffs.getUndergroundTreeGen(
                                    rand, true, 40
                                ); //biome.func_150567_a(rand);
                            ((GlowingTreeGen) tree)
                                .setGlowChance(10); // no, because if triggers a chunk to
                                                    // gen, will get this on surface
        ...
        for (int i = 0; i < 16; i++) {
            for (int k = 0; k < 16; k++) {
                ...
                if (BiomeGlowingCliffs.isGlowingCliffs(world.getBiomeGenForCoords(x, z)
                    ...
                    for (int y = GlowingCliffsColumnShaper.SEA_LEVEL - 4;
                         y <= GlowingCliffsColumnShaper.MAX_UPPER_TOP_Y;
                         y++) {
                        Block b = world.getBlock(x, y, z);
                        ...
                        if (((b instanceof BlockTieredPlant
                              && meta == TieredPlants.FLOWER.ordinal())
                             || (b instanceof BlockDecoFlower
                                 && meta == Flowers.GLOWDAISY.ordinal()))
                            && !world.getBlock(x, y - 1, z).getMaterial().isSolid()) {
                            b2 = Blocks.air;
                        }
                        if (b2 != b) {
    ...
    private void
    generateMaterialVeins(World world, int chunkX, int chunkZ, Random random) {
        if (ReikaChunkHelper.chunkContainsBiomeTypeBlockCoords(
                world, chunkX, chunkZ, BiomeGlowingCliffs.class
            )) {
                ...
                if (b != null) {
                    for (int x = chunkX; x < chunkX+16; x++) {
                        for (int z = chunkZ; z < chunkZ+16; z++) {
                            for (int y = GlowingCliffsColumnShaper.SEA_LEVEL-5; y <
            GlowingCliffsColumnShaper.MAX_UPPER_TOP_Y; y++) { double val =
            crystalNoise.getValue(x, y, z); if (Math.abs(val) <= 0.125) { Block b2 =
            world.getBlock(x, y, z); if (b2.isReplaceableOreGen(world, x, y, z,
            Blocks.stone)) { if (ReikaWorldHelper.checkForAdjMaterial(world, x, y, z,
            Material.air) != null) { world.setBlock(x, y, z, b);
                                        }
                                    }
                                }
                            }
    ...
    private void generateIslands(World world, int chunkX, int chunkZ, Random random) {
        double s = ReikaMathLibrary.normalizeToBounds(
            islandFrequency.getValue(chunkX, chunkZ), 1D / SCALE_RANGE, SCALE_RANGE
        );
        if (ReikaRandomHelper.doWithChance(33 / s)) {
            ...
            if (BiomeGlowingCliffs.isGlowingCliffs(world.getBiomeGenForCoords(x, z))) {
                boolean water
                    = world.getBlock(x, world.getTopSolidOrLiquidBlock(x, z) + 1, z)
                    == Blocks.water;
                int c = water ? 3 : 6;
    ...
    private Island initializeIsland(
        World world, int x, int z, Random rand, double sizeScale, boolean overWater
    ) {
        ...
        is.originY = Math.min(
            250,
            Math.max(
                world.getTopSolidOrLiquidBlock(x, z) + 10 + (int) is.maxThickness, 90
            ) + rand.nextInt(80)
        );
    ...
    private boolean generateIsland(World world, int x, int z, Random rand, Island is) {
        ...
        if (is.canGenerate(world)) {
            ...
            ((GlowingCliffsDecorator) b.theBiomeDecorator)
                .genIslandDecorations(world, b, is);
            ...
            for (int i = 0; i < n; i++) {
                ...
                new WorldGenMinable(bk.blockID, bk.metadata, size, Blocks.stone)
                    .generate(world, rand, dx, dy, dz);
            }
            return true;
        }
        return false;
    ...
    public static class Island {
        ...
        private void calculate(World world, int x, int z, Random rand) {
            double da = 1;
            LobulatedCurve lb
                = LobulatedCurve.fromMinMaxRadii(innerRadius, outerRadius, degree);
            lb.generate(rand);
            for (double d = 0; d < 360; d += da) {
                ...
                for (double dr = 0; dr <= r; dr += 0.5) {
                    ...
                    double t = ReikaMathLibrary.linterpolate(
                        r - dr, 0, outerRadius, minThickness, maxThickness
                    );
                    double cont = Math.abs(instance.contours.getValue(dx, dz));
                    int oy = (int) (cont * MAX_CONTOUR_HEIGHT);
                    double lr = r * lakeScale;
                    if (hasLake && dr <= lr) {
                        double ld = /*Math.min(t-4, */ 2 * lakeDepth
                            * ((lr - dr) / r / lakeScale) /*)*/;
                    ...
                    for (int i = 0; i <= t; i++) {
                        ...
                        BlockKey bk = STONE;
                        double dt
                            = ReikaMathLibrary.linterpolate(dr, 0, outerRadius, 0.5, 3);
                        if (i == 0) {
                            bk = GRASS;
                        } else if (i < dt) {
                            bk = DIRT;
        ...
        private void generate(World world) {
            for (Coordinate c : topMap.keySet()) {
                TEMP_ISLAND_CACHE.put(
                    c,
                    new ImmutablePair(
                        world.getTopSolidOrLiquidBlock(c.xCoord, c.zCoord) + 1,
                        topMap.get(c)
                    )
                );
            }
            ...
            HashMap<Coordinate, BlockKey> toplace
                = this.placeBlocks(world, blocks, placedBlocks);
         ...
         */
        private HashMap<Coordinate, BlockKey> placeBlocks(
            World world,
            HashMap<Coordinate, BlockKey> blocks,
            HashSet<Coordinate> placedBlocks
        ) {
            ...
            for (Coordinate c : blocks.keySet()) {
                ...
                if (water) {
                    ...
                    if (dir != null) {
                        ...
                        if (e1 == null || e2 == null
                            || (e1.blockID == Blocks.flowing_water
                                && this.getEdgeSide(c2) != null)
                            || (e2.blockID == Blocks.flowing_water
                                && this.getEdgeSide(c3) != null)) {
                ...
                if ((bk.blockID == DIRT.blockID || bk.blockID == GRASS.blockID)
                    && !this.blocks.containsKey(cb))
                    bk = STONE;
                if (bk.blockID == GRASS.blockID && this.blocks.containsKey(ca))
                    bk = DIRT;

                if ((bk.blockID == DIRT.blockID || bk.blockID == GRASS.blockID)
                    && this.blocks.containsKey(cb) && !placedBlocks.contains(cb)) {
                    deferredBlocks.put(c, bk);
                } else {
                    placedBlocks.add(c);
                    bk.place(
                        world, c.xCoord, c.yCoord, c.zCoord, 10
                    ); //+8=10 == no light updates
                    if (water)
    ...
    private static class CrystalOreVein extends ExposedOreVein {
        ...
        public boolean canPlaceBlockHere(World world, int x, int y, int z) {
            return super.canPlaceBlockHere(world, x, y, z)
                && world.getBlock(x, y, z) != ChromaBlocks.CLIFFSTONE.getBlockInstance();
        }
    }
}
