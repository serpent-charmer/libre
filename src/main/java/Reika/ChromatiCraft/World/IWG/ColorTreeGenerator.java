...
public class ColorTreeGenerator implements RetroactiveGenerator, ChromaDecorator {
                ...
                for (int i = 0; i < trees; i++) {
                    if (r.nextInt(CHANCE) == 0) {
                        ...
                        if (r.nextInt(this.getRainbowChance(world)) == 0
                            && RainbowTreeGenerator.getInstance().checkRainbowTreeSpace(
                                world, x, y, z
                            )) {
                            ...
                            TreeShaper.getInstance().generateRandomWeightedTree(
                                ...
                                z,
                                r,
                                ReikaDyeHelper.dyes[r.nextInt(16)],
                                false,
                                1,
                                1
                            );
    ...
    private int getRainbowChance(World world) {
        return world.provider.dimensionId == ReikaTwilightHelper.getDimensionID() ? 16
                                                                                  : 32;
    }

    public static int getTreeCount(World world, BiomeGenBase biome) {
        if (ModList.THAUMCRAFT.isLoaded()
            && biome.biomeID == ThaumIDHandler.Biomes.TAINT.getID())
            return 0;
    ...
    public static boolean canGenerateTree(World world, int x, int z) {
        ...
        if (ModList.MYSTCRAFT.isLoaded() && ReikaMystcraftHelper.isMystAge(world)
            && !ChromatiCraft.isRainbowForest(biome)) {
        ...
        if (biome == BiomeGenBase.mushroomIsland
            ...
            return false;
        int y = world.getTopSolidOrLiquidBlock(x, z);
        //ReikaJavaLibrary.pConsole(world.getBlock(x, y, z)+","+world.getBlock(x, y-1,
        //z)+","+world.getBlock(x, y-2, z)+":"+BlockDyeSapling.canGrowAt(world, x, y, z));
