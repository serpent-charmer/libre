...
public class EnderOakGenerator extends WorldGenAbstractTree {
    ...
    public EnderOakGenerator(
        int h0, int h1, int f0, int f1, int fr0, int fr1, float b, int bl, float c
    ) {
        super(false);
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        if (!ReikaPlantHelper.SAPLING.canPlantAt(world, x, y, z))
            return false;
        int trunk = ReikaRandomHelper.getRandomBetween(
            minTrunkBaseHeight, maxTrunkBaseHeight, rand
        );
        int leaf = ReikaRandomHelper.getRandomBetween(
            minFoliageHeight, maxFoliageHeight, rand
        );
    ...
    private class Tree {
            ...
            if (branchChancePerLevel > 0) {
                ...
                if (!levels.isEmpty()) {
                    ...
                    if (n > 0) {
                        ...
                        for (int i = 0; i < n; i++) {
                            int yl
                                = ReikaJavaLibrary.getRandomCollectionEntry(rand, levels);
                            int dh = y + totalHeight - yl;
                            Coordinate c = new Coordinate(x, yl, z);
                            float phi = phi0 + a0 * i;
                            phi = (float
                            ) ReikaRandomHelper.getRandomPlusMinus(phi, 24F, rand);
                            float theta = (float
                            ) ReikaRandomHelper.getRandomPlusMinus(0F, 30F, rand);
            ...
            if (columnChancePerLeaf > 0) {
                ...
                for (Coordinate c : li) {
                    ...
                    for (int i = 2; i < 6; i++) {
                        ...
                        if (li.contains(new Coordinate(
                                c.xCoord + dir.offsetX, c.yCoord, c.zCoord + dir.offsetZ
                            )))
                            n++;
                    }
        ...
        private void generateLayer(
            ...
            for (int i = -r; i <= r; i++) {
                for (int k = -r; k <= r; k++) {
                    ...
                    //ReikaJavaLibrary.pConsole("dr of "+dr+" at h = "+h+" of
                    //"+totalHeight);
                    if (d <= dr) {
        ...
        private void permuteRadius(Random rand, int y) {
            ...
            if (rand.nextInt(2) == 0) {
                ...
                currentRadiusExponent
                    += sign * ReikaRandomHelper.getRandomBetween(0.025, 0.1, rand);
            }
            ...
            currentRadiusExponent
                = MathHelper.clamp_float(currentRadiusExponent, 0.35F, 0.65F);
        }
        ...
        private boolean canReplace(World world, int x, int y, int z) {
            ...
            if (b == Blocks.grass || b == Blocks.dirt)
                return false;
            return b.canBeReplacedByLeaves(world, x, y, z)
                || EnderOakGenerator.super.isReplaceable(world, x, y, z);
        }
        ...
        private void place(World world) {
            for (Coordinate c : logs) {
                c.setBlock(
                    ...
                    ReikaTreeHelper.OAK.getBaseLogMeta(),
                    2
                );
            }
            for (Coordinate c : leaves.allValues(false)) {
                ...
                c.setBlock(
                    world,
                    ReikaTreeHelper.OAK.getLeafID(),
                    ReikaTreeHelper.OAK.getBaseLeafMeta(),
                    2
                );
            }
        }
        ...
        private class Branch {
            ...
            private void calculate(World world, Random rand) {
                int l = Math.min(
                    (int) (leafHeight * 0.67),
                    ReikaRandomHelper.getRandomBetween(2, maxBranchLength)
                );
