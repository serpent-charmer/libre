...
public class VillagersFailChromatiCraft {
    ...
    static {
        entries.add(new PerVillageStructureEntry(
            BrokenChromaStructure.class,
            1,
            "FailChC",
            15,
            6,
            16,
            ChromaOptions.getVillageStructureRarity(7, 3)
        ));
        entries.add(new PerVillageStructureEntry(
            WoodenChromaStructure.class,
            4,
            "ChCHouse",
            15,
            8,
            15,
            ChromaOptions.getVillageStructureRarity(4, 1)
        ));
    }
    ...
    private static abstract class ChromaVillagePiece extends VillagePiece {
        ...
        protected final TileEntityLootChest generateLootChest(
            World world, int i, int j, int k, Location s, int bonus, int dir, Random rand
        ) {
            return ChromaAux.generateLootChest(
                world,
                this.getXWithOffset(i, k),
                this.getYWithOffset(j),
                this.getZWithOffset(i, k),
                dir,
                rand,
                s,
                bonus
        ...
        protected final TileEntityLootChest generateLootChestFixed(
            World world, int i, int j, int k, Location s, int bonus, int dir, Random rand
        ) {
            return ChromaAux.generateLootChest(
                world,
    ...
    public static class WoodenChromaStructure extends ChromaVillagePiece {
        ...
        protected boolean generate(World world, Random rand) {
            ...
            genBiome = world.getBiomeGenForCoords(
            ...
            TileEntitySign ts = (TileEntitySign) this.placeTileEntityAtFixedPosition(
                world, 7, 2, 7, Blocks.standing_sign, 9
            );
            if (ts != null)
                ts.signText = new String[] {
                    "Your thingy was", "weird. It's my", "house now", "--Villager 19"
                };

            TileEntityLootChest te = this.generateLootChestFixed(
                world, 7, 0, 7, Location.VILLAGE, 1, 0, rand
            );
            if (te != null)
        ...
        private Block getStair(World world) {
            ...
            if (genBiome == BiomeGenBase.taiga || genBiome == BiomeGenBase.taigaHills
                || genBiome == BiomeGenBase.coldTaiga
                || genBiome == BiomeGenBase.coldTaigaHills)
                return Blocks.spruce_stairs;
            if (genBiome == BiomeGenBase.jungle || genBiome == BiomeGenBase.jungleEdge
                || genBiome == BiomeGenBase.jungleHills)
                return Blocks.jungle_stairs;
            if (genBiome == BiomeGenBase.birchForest
                || genBiome == BiomeGenBase.birchForestHills)
        ...
        private Block getBase(World world) {
            return BiomeDictionary.isBiomeOfType(genBiome, Type.SANDY)
                ? Blocks.sandstone
                : Blocks.cobblestone;
        }
    ...
    public static class BrokenChromaStructure extends ChromaVillagePiece {
        ...
        protected boolean generate(World world, Random rand) {
            ...
            TileEntityLootChest te
                = this.generateLootChest(world, 7, 0, 10, Location.VILLAGE, 1, 0, rand);
            if (te != null)
                te.addProgress(ProgressStage.VILLAGECASTING);

            TileEntityChest tc = (TileEntityChest
            ) this.generateTileEntity(world, 7, 1, 11, Blocks.chest, 4);
            if (tc != null) {
                int[] slots = { 2, 4, 6, 11, 12, 13, 15, 20, 22, 24 };
                for (int i = 0; i < slots.length; i++) {
                    tc.setInventorySlotContents(
                        slots[i], itemRand.getRandomEntry().copy()
                    );
                }
            }

            TileEntitySign ts = (TileEntitySign
            ) this.generateTileEntity(world, 0, 6, 11, Blocks.wall_sign, 4);
            if (ts != null)
                ts.signText = new String[] {
                    "guys i think my", "lapiz is broken", "", "--Villager 26"
                };
            ts = (TileEntitySign
            ) this.generateTileEntity(world, 1, 3, 3, Blocks.wall_sign, 2);
            if (ts != null)
                ts.signText = new String[] { "Couldn't reach.", "", "Good enough?", "" };
            ts = (TileEntitySign
            ) this.generateTileEntity(world, 6, 1, 11, Blocks.wall_sign, 4);
            if (ts != null)
                ts.signText
                    = new String[] { "Propertee", "of Villager", "#73", "~angryface" };
