...
public class EndOverhaulManager extends IRenderHandler implements RetroactiveGenerator {
        ...
        for (Tendril t : tendrils) {
            ...
            if (map != null) {
                for (Entry<Coordinate, BlockKey> e : map.entrySet()) {
                    ...
                    if (!c.isEmpty(world)
                        && (bk.equals(coatingBlock)
                            || c.getBlock(world) != Blocks.end_stone))
                        continue;
    ...
    public void updateRenderer(AbstractClientPlayer ep) {
        ...
        if (renderFactor > 0.2 && !Minecraft.getMinecraft().isGamePaused()
            && ReikaRandomHelper.doWithChance(renderFactor * 0.02)) {
            ThrottleableEffectRenderer.getRegisteredInstance().registerDelegateRenderer(
                EntityAnglerLight.class, CCParticleEngine.instance
            );
            double[] xyz = ReikaPhysicsHelper.polarToCartesian(
                80,
                ReikaRandomHelper.getRandomPlusMinus(0D, 30D),
                DragonAPICore.rand.nextDouble() * 360
            );
            xyz[0] += ep.posX;
            xyz[1] += ep.posY;
            xyz[2] += ep.posZ;
    ...
    private class EntityAnglerLight extends EntityCCBlurFX {
        ...
        private EntityAnglerLight(EntityPlayer ep, double[] xyz) {
            ...
            this.setIcon(ChromaIcons.ANGLER_BASIC)
                .setBasicBlend()
                .setAlphaFading()
                .setRapidExpand();
            this.setLife(900).setScale(180).setColor(
                DragonAPICore.rand.nextInt(5) == 0 ? 0xE57042 : 0xffffff
            );
        }
    ...
    public void render(float partialTicks, WorldClient world, Minecraft mc) {
        ...
        GL11.glFogf(
            GL11.GL_FOG_DENSITY, MathHelper.clamp_float(renderFactor * 0.000002F, 0, 1)
        );
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            float s = 200; //MathHelper.clamp_double(ReikaMathLibrary.linterpolate(renderFactor,
                           //0.75, 0.25, 100, 20), 0, 100);
            GL11.glFogf(
                GL11.GL_FOG_START,
                (float) ReikaMathLibrary.linterpolate(renderFactor, 0, 1, 150, 0)
            );
    ...
    private class Island {
        ...
        private void runGen1(World world, Random rand, HashSet<Coordinate> set) {
            ...
            it.limit = BlockBox.block(x, y, z).expand(
                radius + 2, (int) (radius * thickness) + 2, radius + 2
            );
            it.run(world, new PropagationCondition() {
                ...
                public boolean isValidLocation(
                    ...
                    double dr = ReikaMathLibrary.normalizeToBounds(
                        shape.getValue(x, y, z), 0.75, 1.25
                    );
                    return ReikaMathLibrary.isPointInsideEllipse(
                        dx, dy, dz, dr * radius, dr * radius * thickness, dr * radius
                    );
                }
            });
    ...
    private class Tendril {
        ...
        private Tendril(long seed, double ang) {
            ...
            length = Math.sqrt(ReikaRandomHelper.getRandomBetween(
                         MIN_DIST_SQ_CH + (MAX_DIST_SQ_CH - MIN_DIST_SQ_CH) * 0.67,
                         MAX_DIST_SQ_CH,
                         rand
                     ))
                * 16D;
            origin = new DecimalPosition(
                ReikaRandomHelper.getRandomPlusMinus(0.5, 32, rand),
                45,
                ReikaRandomHelper.getRandomPlusMinus(0.5, 32, rand)
            );
            endpoint = new DecimalPosition(
                length * Math.cos(Math.toRadians(ang)),
                origin.yCoord,
                length * Math.sin(Math.toRadians(ang))
            );
        }
        ...
        private Tendril calculate(World world) {
            ...
            LightningBolt lb = new LightningBolt(origin, endpoint, (int) (length / 27))
                                   .setVariance(24, 18, 24, f1, f2, f1)
                                   .setRandom(rand)
                                   .maximize();
                        ...
                        for (int c = -3; c <= 3; c++) {
                            ...
                            if (dist <= 4) {
                                ...
                                WorldChunk wc = new WorldChunk(
                                    world, loc.xCoord >> 4, loc.zCoord >> 4
                                );
            ...
            for (int i = 1; i < li.size() - 1; i++) {
                ...
                //double ang1 = Math.toDegrees(Math.atan2(pos.zCoord-prev.zCoord,
                //pos.xCoord-prev.xCoord)); double ang2 =
                //Math.toDegrees(Math.atan2(next.zCoord-pos.zCoord,
                //next.xCoord-pos.xCoord)); double ang = (ang2+ang1)/2;
                ...
                if (rand.nextInt(18) == 0
                    && (lastThorn < 0 || pos.getDistanceTo(li.get(lastThorn)) > 9)) {
                    lastThorn = i;
                    double len = ReikaRandomHelper.getRandomBetween(6D, 18D, rand);
                    double ang = rand.nextDouble() * 360;
                    Vec3 point = ReikaVectorHelper.rotatePointAroundAxisByAngle(
                        0,
                        len,
                        0,
                        0,
                        0,
                        0,
                        next.xCoord - prev.xCoord,
                        next.yCoord - prev.yCoord,
                        next.zCoord - prev.zCoord,
                        ang
                    );
                    for (double d = 0; d <= 1; d += 0.03125) {
                        ...
                        double r = ReikaMathLibrary.linterpolate(
                            d, 0, 1, 1.9 + len * 0.02, 0.9
                        );
            ...
            Island il = new Island(
                ...
                ReikaRandomHelper.getRandomBetween(24, 60, rand),
                rand.nextLong()
            );
                ...
                for (int b = -2; b <= 2; b++) {
                    for (int c = -2; c <= 2; c++) {
                        if (ReikaMathLibrary.py3d(a, b, c) <= 1.6) {
                            //ReikaJavaLibrary.pConsole(new DecimalPosition(a, b+4, c)+" >
                            //"+point);
                            Coordinate loc = new Coordinate(
                                point.xCoord + pos.xCoord + a,
                                point.yCoord + pos.yCoord + b,
                                point.zCoord + pos.zCoord + c
                            );
