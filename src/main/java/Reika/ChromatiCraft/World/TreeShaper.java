...
public class TreeShaper {
    ...
    public boolean isLogTypeEverAllowed(ModWoodList wood) {
        return wood != ModWoodList.BAMBOO && wood != ModWoodList.LIGHTED
            && wood != ModWoodList.SLIME && wood != ModWoodList.TAINTED;
    }
    ...
    private TreeShaper() {
        for (int i = 0; i < ReikaTreeHelper.treeList.length; i++) {
            ...
            if (ChromatiCraft.config.shouldGenerateLogType(tree)) {
                ...
                ChromatiCraft.logger.log(
                    "Dye Tree Generation with Log Type " + tree.getName()
                    + " logs: Enabled"
                );
            } else {
                ChromatiCraft.logger.log(
            ...
            if (this.isLogTypeEverAllowed(tree)) {
                if (tree.exists()) {
                    if (ChromatiCraft.config.shouldGenerateLogType(tree)) {
                        ...
                        ChromatiCraft.logger.log(
                            "Dye Tree Generation with Log Type " + tree.getBasicInfo()
                            + ": Enabled"
                        );
                    } else {
                        ChromatiCraft.logger.log(
        ...
        if (validLogs.isEmpty()) {
            throw new InstallationException(
                ChromatiCraft.instance, "You must enable at least one log type!"
            );
        }
    }
    ...
    public void generateRandomWeightedTree(
        World world,
        int x,
        int y,
        int z,
        Random rand,
        ReikaDyeHelper color,
        boolean forceGen,
        float vineChance,
        float vineFertilityFactor
    ) {
        this.generateRandomWeightedTree(
            world,
            x,
            y,
            z,
    ...
    private void generateVine(
        float fertileFactor, World world, int x, int y, int z, int meta, Random rand
    ) {
        ...
        while (n < max && air) {
            this.setBlock(
                world, x, y, z, ChromaBlocks.DYEVINE.getBlockInstance(), meta, 2
            );
            y--;
        ...
        if (fertileFactor > 0
            && ReikaRandomHelper.doWithChance(BASE_FERTILE_CHANCE * fertileFactor))
            world.setBlock(
                x, y + 1, z, ChromaBlocks.FERTILEDYEVINE.getBlockInstance(), meta, 2
            );
    }

    private int generateNormalTree(
        World world,
        int x,
        int y,
        int z,
                    ...
                    if (this.canGenerateLeavesAt(world, x + i, y + h - 3, z + j)) {
                        ...
                        if (vineChance > 0
                            && ReikaRandomHelper.doWithChance(
                                BASE_VINE_CHANCE * vineChance, rand
                            )) {
                            this.generateVine(
                                vineFertility, world, x + i, y + h - 4, z + j, meta, rand
                            );
                        }
                    }
                }
                ...
                for (int j = -1; j <= 1; j++) {
                    if (i * j == 0)
                        if (this.canGenerateLeavesAt(world, x + i, y + h - 8, z + j)) {
                            this.setBlock(
                                world, x + i, y + h - 8, z + j, leafID, meta, 3
                            );
                            if (vineChance > 0
                                && ReikaRandomHelper.doWithChance(
                                    BASE_VINE_CHANCE * vineChance, rand
                ...
                for (int j = -1; j <= 1; j++) {
                    if (this.canGenerateLeavesAt(world, x + i, y + h - 7, z + j)) {
                        ...
                        if (vineChance > 0
                            ...
                            this.generateVine(
                                vineFertility, world, x + i, y + h - 8, z + j, meta, rand
                            );
                        }
                    }
                }
                ...
                for (int j = -w; j <= w; j++) {
                    if (i * j != w * w && i * j != -w * w)
                        if (this.canGenerateLeavesAt(world, x + i, y + h - 6, z + j)) {
                            this.setBlock(
                                world, x + i, y + h - 6, z + j, leafID, meta, 3
                            );
                            if (vineChance > 0
                                && ReikaRandomHelper.doWithChance(
                                    BASE_VINE_CHANCE * vineChance, rand
                ...
                for (int j = -w; j <= w; j++) {
                    if (i * j != w * w && i * j != -w * w)
                        if (this.canGenerateLeavesAt(world, x + i, y + h - 5, z + j)) {
                            this.setBlock(
                                world, x + i, y + h - 5, z + j, leafID, meta, 3
                            );
                        }
                }
            }
            ...
            for (int i = -w; i <= w; i++) {
                for (int j = -w; j <= w; j++) {
                    if (i * j != w * w && i * j != -w * w)
                        if (this.canGenerateLeavesAt(world, x + i, y + h - 3, z + j)) {
                            this.setBlock(
                                world, x + i, y + h - 3, z + j, leafID, meta, 3
                            );
                        }
                }
            }
            for (int i = -w; i <= w; i++) {
                for (int j = -w; j <= w; j++) {
                    if (i * j != w * w && i * j != -w * w)
                        if (this.canGenerateLeavesAt(world, x + i, y + h - 2, z + j)) {
                            this.setBlock(
                                world, x + i, y + h - 2, z + j, leafID, meta, 3
                            );
                        }
                }
            }
            ...
            for (int i = 1; i < 2; i++) {
                ...
                for (int j = -1; j <= 1; j++) {
                    for (int k = -1; k <= 1; k++) {
                        for (int m = -1; m <= 1; m++) {
                            if (j * k * m == 0
                                && this.canGenerateLeavesAt(
                                    world, dx + j, dy + k, dz + m
                                )) {
                                this.setBlock(
                                    world, dx + j, dy + k, dz + m, leafID, meta, 3
                                );
                            }
                        }
                    }
    ...
    public static boolean canGenerateLeavesAt(World world, int x, int y, int z) {
        ...
        boolean leaves = world.getBlock(x, y, z) == Blocks.leaves
            || world.getBlock(x, y, z) == Blocks.leaves2;
        //ReikaJavaLibrary.pConsole(x+", "+y+", "+z, !soft && !leaves && world.getBlock(x,
        //y, z) != Blocks.log.blockID);
        return soft || leaves
            || world.getBlock(x, y, z).canBeReplacedByLeaves(world, x, y, z);
    }
}
