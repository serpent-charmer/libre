...
public class LavaRiverGenerator implements IPopulate {
            ...
            for (int k = 0; k < 16; k++) {
                ...
                if (val <= RIVER_THRESH) {
                    double h = ReikaMathLibrary.normalizeToBounds(
                        heightNoise.getValue(rx / 8, rz / 8), MIN_HEIGHT, MAX_HEIGHT
                    );
                    int y = (int) h;
                    if (val < RIVER_CENTER_THRESH) {
                        for (int j = 0; j < 1; j++) {
                            int dy = y + j;
                            world.setBlock(
                                dx, dy, dz, structBlock.blockID, structBlock.metadata, 2
                            );
                        }
                        for (int j = 1; j <= 1; j++) {
                            int dy = y + j;
    ...
    private Block getLiquid(double rx, double rz) {
        ...
        double t = ReikaMathLibrary.normalizeToBounds(
            blockNoise.getValue(rx, rz), 0, blockTypes.lastValue()
        );
