...
public class NetherStructureGenerator implements RetroactiveGenerator {
    public static final NetherStructureGenerator instance
        = new NetherStructureGenerator();
    ...
    public void generate(
        ...
        if (world.provider.dimensionId != -1)
            return;
        int n = 8;
        if (rand.nextInt(1) == 0 && Math.abs(chunkX % /*(n-(world.getSeed()%n))*/ 4) == 0
            && Math.abs(chunkZ % /*(n-(world.getSeed()%n))*/ 4) == /*n/2*/ 2) {
    ...
    private void runGlowstoneGen(World world, int chunkX, int chunkZ, Random rand) {
        if (rand.nextInt(2) == 0) {
            ...
            new WorldGenMinable(Blocks.glowstone, n, Blocks.air)
    ...
    private boolean generateBedrockBypass(World world, int x, int z, Random rand) {
        ...
        for (int i = -r; i <= r; i++) {
            for (int k = -r; k <= r; k++) {
                ...
                if (ReikaMathLibrary.py3d(i, 0, k) >= r - 0.5
                    && ReikaMathLibrary.py3d(i, 0, k) <= r + 1.5) {
                    for (int d = 0; d < 6; d++) {
