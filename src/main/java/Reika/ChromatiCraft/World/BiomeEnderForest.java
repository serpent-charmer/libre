...
public class BiomeEnderForest extends BiomeGenForest implements CustomMapColorBiome {
    ...
    private final NoiseGeneratorBase extraX
        = new Simplex3DGenerator(3571 ^ root).setFrequency(1 / 4D);
    private final NoiseGeneratorBase extraY
        = new Simplex3DGenerator(-8247 ^ root).setFrequency(1 / 4D);
    private final NoiseGeneratorBase extraZ
        = new Simplex3DGenerator(5648723 ^ root).setFrequency(1 / 4D);
    private final VoronoiNoiseGenerator colorNoise
        = (VoronoiNoiseGenerator) new VoronoiNoiseGenerator(23657 ^ root)
              .setFrequency(1 / 12D)
              .setDisplacement(extraX, extraY, extraZ, 6);

    private final WorldGenAbstractTree enderOakLarge
        = new EnderOakGenerator(3, 7, 5, 12, 3, 5, 0.15F, 6, 0.15F);
    private final WorldGenAbstractTree enderOakSmall
        = new EnderOakGenerator(2, 4, 3, 5, 2, 3, 0, 0, 0.1F);
    private final WorldGenAbstractTree enderOakNarrow
        = new EnderOakGenerator(6, 12, 6, 15, 1, 2, 0.35F, 4, 0F);
    ...
    public int getBiomeGrassColor(int x, int y, int z) {
        if (ChromaOptions.ENDERCOLORING.getState()) {
            ...
            return ReikaColorAPI.mixColors(
                BiomeGenBase.forest.getBiomeGrassColor(x, y, z),
                BiomeGenBase.icePlains.getBiomeGrassColor(x, y, z),
                this.getMix(x, y, z)
            );
        }
    }
    ...
    public int getBiomeFoliageColor(int x, int y, int z) {
        if (ChromaOptions.ENDERCOLORING.getState()) {
            ...
            if (c == null) {
                c = ReikaColorAPI.mixColors(BiomeGenBase.forest.getBiomeFoliageColor(x, y,
            z), BiomeGenBase.icePlains.getBiomeFoliageColor(x, y, z), this.getMix(x, y,
            z)); colorMap.put(loc, c);
            }
            return c;
             */
            return ReikaColorAPI.mixColors(
                BiomeGenBase.forest.getBiomeFoliageColor(x, y, z),
                BiomeGenBase.icePlains.getBiomeFoliageColor(x, y, z),
                this.getMix(x, y, z)
            );
        }
    }
    ...
    private float getMix(int x, int y, int z) {
        colorRand.setSeed(new Coordinate(
                              x / 6 + extraX.getValue(x, z) * 8,
                              y / 4 + extraY.getValue(x, z) * 3,
                              z / 6 + extraZ.getValue(x, z) * 8
        )
                              .hashCode());
