...
public class BiomeGlowingCliffs
    extends BiomeGenBase implements WinterBiomeStrengthControl, CustomMapColorBiome {
    private static final NoiseGeneratorBase hueShift
        = new SimplexNoiseGenerator(System.currentTimeMillis()).setFrequency(1 / 8D);
    private static final NoiseGeneratorBase lumShift
        = new SimplexNoiseGenerator(-System.currentTimeMillis()).setFrequency(1 / 6D);
    private static final NoiseGeneratorBase waterColorMix
        = new SimplexNoiseGenerator(~System.currentTimeMillis()).setFrequency(1 / 10D);
    private static final NoiseGeneratorBase skyColorMix
        = new SimplexNoiseGenerator(~System.currentTimeMillis() * 2)
              .setFrequency(1 / 20D);
    //private final SimplexNoiseGenerator fogDensityXZ = new
    //SimplexNoiseGenerator(2*System.currentTimeMillis()).setFrequency(1/24D); private
    //final SimplexNoiseGenerator fogDensityXY = new
    //SimplexNoiseGenerator(-4*System.currentTimeMillis()).setFrequency(1/18D); private
    //final SimplexNoiseGenerator fogDensityYZ = new
    //SimplexNoiseGenerator(5*System.currentTimeMillis()).setFrequency(1/18D);
    ...
    private final WorldGenAbstractTree basicTreeGen
        = new SlightlyFloweringSmallTree(false); //defaults to oak
    private final WorldGenAbstractTree bigTreeGen
        = new SlightlyFloweringBigTree(false); //defaults to oak
    ...
    public BiomeGlowingCliffs(int id, boolean register) {
        ...
        if (ChromaOptions.BIOMEBLEND.getState())
            this.setHeight(new Height(
                BiomeGenBase.extremeHillsPlus.rootHeight,
                BiomeGenBase.extremeHillsPlus.heightVariation
            ));
        else
        ...
        spawnableMonsterList.add(
            new BiomeGenBase.SpawnListEntry(EntitySpider.class, 10, 4, 4)
        );
        spawnableMonsterList.add(
            new BiomeGenBase.SpawnListEntry(EntityZombie.class, 10, 4, 4)
        );
        spawnableMonsterList.add(
            new BiomeGenBase.SpawnListEntry(EntitySkeleton.class, 5, 4, 4)
        ); //0.5x
        //this.spawnableMonsterList.add(new
        //BiomeGenBase.SpawnListEntry(EntityCreeper.class, 100, 4, 4)); //0x
        spawnableMonsterList.add(
            new BiomeGenBase.SpawnListEntry(EntitySlime.class, 10, 4, 4)
        );
        spawnableMonsterList.add(
            new BiomeGenBase.SpawnListEntry(EntityEnderman.class, 2, 1, 4)
        ); //2x
        //spawnableMonsterList.add(new BiomeGenBase.SpawnListEntry(EntityWitch.class, 5,
        //1, 1)); //0x

        spawnableCaveCreatureList.add(
            new BiomeGenBase.SpawnListEntry(EntityBat.class, 10, 8, 8)
        );

        glowCloudList = ReikaJavaLibrary.makeListFrom(
            new BiomeGenBase.SpawnListEntry(EntityGlowCloud.class, 30, 1, 1)
        );

        //if (ModList.THAUMCRAFT.isLoaded())
        //	spawnableMonsterList.add(new ModSpawnEntry(ModList.THAUMCRAFT,
        //"thaumcraft.common.entities.monster.EntityWisp", 5, 1, 1).getEntry());
    }
    ...
    public static void updateRenderFactor(AbstractClientPlayer ep) {
        ...
        } else if (BiomeGlowingCliffs.isGlowingCliffs(ep.worldObj.getBiomeGenForCoords(
    ...
    public int getSkyColorByTemp(float temp) {
        ...
        return ReikaColorAPI.getShiftedHue(
            0xd0a0ff, (float) (15 * skyColorMix.getValue(ep.posX, ep.posZ))
        );
    }
    ...
    public int getWaterColor(IBlockAccess world, int x, int y, int z, int l) {
        if (isClearWater(world, x, y, z))
            return 0xffffff;
        float f = (float
        ) ReikaMathLibrary.normalizeToBounds(waterColorMix.getValue(x, z), 0, 1);
    ...
    public static boolean isClearWater(IBlockAccess world, int x, int y, int z) {
        ...
        return (y > 65 && ReikaWorldHelper.getWaterDepth(world, x, y, z) <= 3)
            || world.getBlockMetadata(x, y, z) > 0
            || w.getSavedLightValue(EnumSkyBlock.Sky, x, y, z) < 2
            || ReikaWorldHelper.countAdjacentBlocks(world, x, y, z, Blocks.air, false)
            >= 2;
    }

    private int shiftBrightness(int base, int x, int y, int z) {
        float f = (float
        ) ReikaMathLibrary.normalizeToBounds(lumShift.getValue(x, z), 0.05F, 0.6F);
    ...
    private int shiftHue(int base, int x, int z) {
        float h
            = (float) ReikaMathLibrary.normalizeToBounds(hueShift.getValue(x, z), 0, 50);
    ...
    public List getSpawnableList(EnumCreatureType type) {
        return type == ChromatiCraft.glowCloudType ? glowCloudList
                                                   : super.getSpawnableList(type);
    }

    @Override
    public void genTerrainBlocks(
        World world, Random rand, Block[] arr, byte[] m, int x, int z, double stoneNoise
    ) {
    ...
    public static void blendTerrainEdgesAndGenCliffs(
        World world, int chunkX, int chunkZ, Block[] blockArray, byte[] metaArray
    ) {
        initTerrain(world);
                ...
                if (!BiomeGlowingCliffs.isGlowingCliffs(b)) {
                    if (ChromaOptions.BIOMEBLEND.getState()) {
                        if (WorldgenProfiler.profilingEnabled())
                            WorldgenProfiler.startGenerator(
                                world,
                                "Luminous cliff adjacency biome blending",
                                chunkX,
                                chunkZ
                            );
                        terrain.blendEdge(world, x, z, blockArray, metaArray);
                        if (WorldgenProfiler.profilingEnabled())
                            WorldgenProfiler.onRunGenerator(
                                world,
                                "Luminous cliff adjacency biome blending",
                                chunkX,
                                chunkZ
                    ...
                    if (WorldgenProfiler.profilingEnabled())
                        WorldgenProfiler.startGenerator(
                            world, "Luminous cliff terrain shaping", chunkX, chunkZ
                        );
                    terrain.generateColumn(
                        world, x, z, chunkX, chunkZ, blockArray, metaArray, b
                    );
                    if (WorldgenProfiler.profilingEnabled())
                        WorldgenProfiler.onRunGenerator(
                            world, "Luminous cliff terrain shaping", chunkX, chunkZ
    ...
    public WorldGenAbstractTree func_150567_a(Random rand) {
        if (rand.nextInt(20) == 0) {
            ...
            //return super.func_150567_a(rand);//rand.nextInt(10) == 0 ?
            //worldGeneratorBigTree : worldGeneratorTrees;
            return rand.nextInt(10) == 0 ? bigTreeGen : basicTreeGen;
        }
    }

    public WorldGenAbstractTree
    getUndergroundTreeGen(Random rand, boolean construct, int bigChance) {
        return rand.nextInt(bigChance) == 0
            ? construct ? new GlowingTreeGenerator() : glowTree
            : construct ? new SmallGlowingTreeGenerator()
                        : smallGlowTrees;
    }
        ...
        return b instanceof BiomeGlowingCliffs
            || (ModList.MYSTCRAFT.isLoaded()
                && ReikaMystcraftHelper.getMystParentBiome(b)
                        instanceof BiomeGlowingCliffs);
    }

    /* No longer callable, nor was it a good idea
    public static void runIWGs(int cx, int cz, World world, IChunkProvider gen,
    IChunkProvider loader) { List<IWorldGenerator> iwgs =
    ReikaWorldHelper.getModdedGenerators(); ResettableRandom rand =
    ReikaWorldHelper.getModdedGeneratorChunkRand(cx, cz, world);
    ...
    private static class GlowingTreeGenerator
        extends ModifiableBigTree implements GlowingTreeGen {
        ...
        public BlockKey getLeafBlock(int x, int y, int z) {
            return BiomeGlowingCliffs.selectGlowingTreeLeaf(
                rand, glowChance, super.getLeafBlock(x, y, z), true
            );
        }
    ...
    private static class SmallGlowingTreeGenerator
        extends ModifiableSmallTrees implements GlowingTreeGen {
        private static final int DEFAULT_GLOW_CHANCE = 16;
        private int glowChance = DEFAULT_GLOW_CHANCE;
        ...
        public BlockKey getLeafBlock(int x, int y, int z) {
            return BiomeGlowingCliffs.selectGlowingTreeLeaf(
                rand, glowChance, super.getLeafBlock(x, y, z), false
            );
        }
    ...
    private static class SlightlyFloweringSmallTree extends ModifiableSmallTrees {
        ...
        @Override
        public BlockKey getLeafBlock(int x, int y, int z) {
            return BiomeGlowingCliffs.selectBasicTreeLeaf(
                rand, super.getLeafBlock(x, y, z), false
            );
        }
    }
    ...
    private static class SlightlyFloweringBigTree extends ModifiableBigTree {
        ...
        public BlockKey getLeafBlock(int x, int y, int z) {
            return BiomeGlowingCliffs.selectBasicTreeLeaf(
                rand, super.getLeafBlock(x, y, z), true
            );
        }
    }
    ...
    public static BlockKey selectGlowingTreeLeaf(
        Random rand, int glowChance, BlockKey leafDefault, boolean isBig
    ) {
        if (rand.nextInt(glowChance) == 0)
            return new BlockKey(ChromaBlocks.GLOWLEAF.getBlockInstance());
        return ModList.BOP.isLoaded() && rand.nextInt(isBig ? 20 : 25) == 0
            ? BoPBlockHandler.LeafTypes.flowering.getBlock()
            : leafDefault;
    }

    public static BlockKey
    selectBasicTreeLeaf(Random rand, BlockKey leafDefault, boolean isBig) {
        return ModList.BOP.isLoaded() && rand.nextInt(isBig ? 15 : 10) == 0
            ? BoPBlockHandler.LeafTypes.flowering.getBlock()
            : leafDefault;
    }
}
