...
public class EntityPistonSpline extends EntitySplineProjectile {
    ...
    private void spawnParticle() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            float g = rand.nextInt(3) > 0
                ? 0
                : (float) ReikaRandomHelper.getRandomBetween(0, 0.125);
            int c = color.getRenderColor(
            ); //ReikaColorAPI.mixColors(color.getRenderColor(), 0xffffff, 0.75F);
