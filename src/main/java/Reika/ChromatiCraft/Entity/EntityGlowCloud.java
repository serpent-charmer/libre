...
public class EntityGlowCloud
    extends EntityLiving implements EtherealEntity, IMob, DestroyOnUnload {
    ...
    public void onUpdate() {
        ...
        if (worldObj.isRemote) {
            ...
            //worldObj.setBlock(MathHelper.floor_double(posX),
            //MathHelper.floor_double(posY), MathHelper.floor_double(posZ),
            //ChromaBlocks.LIGHT.getBlockInstance(), Flags.DECAY.getFlag(), 3);
        }
    ...
    private void doAmbientEffects() {
        ...
        if (c.getBlock(worldObj) == ChromaBlocks.TANK.getBlockInstance()) {
            ...
            if (te != null) {
                ...
                if (te2 != null
                    && (te2.isEmpty() || te2.getCurrentFluid() == ChromatiCraft.luma)) {
        ...
        if (te instanceof IEnergyHandler) {
            ...
            if (((IEnergySink) te)
                    .injectEnergy(
                        ForgeDirection.VALID_DIRECTIONS[rand.nextInt(6)], amtToSpawn, 32
                    )
                < 50) {
                if (cachedTile == null)
                    cachedTile = new Coordinate(te);
    ...
    private void updateLight() {
        ...
        if (ticksExisted % r == 0) {
            if (d <= 144
                || ReikaRenderHelper.renderFrustrum.isBoundingBoxInFrustum(
                    this.getLightingBox()
                )) {
                Coordinate c = new Coordinate(this);
    ...
    private void deleteOldLight() {
        if (worldObj.isRemote) {
            if (light != null
                && light.getBlock(worldObj) == ChromaBlocks.LIGHT.getBlockInstance()) {
                light.setBlock(worldObj, Blocks.air);
            }
            if (oldLight != null
                && oldLight.getBlock(worldObj) == ChromaBlocks.LIGHT.getBlockInstance()) {
    ...
    private void lifeParticles() {
        ...
        EntityFX fx = new EntityCCBlurFX(worldObj, px, py, pz)
                          .setColor(c)
                          .setLife(l)
                          .setScale(s)
                          .setAlphaFading()
                          .setRapidExpand(); //.setColliding();
        ...
        fx = new EntityCCBlurFX(worldObj, px, py, pz)
                 .setColor(c)
                 .setLife(l / 2)
                 .setScale(s * 3)
                 .setAlphaFading()
        ...
        if (rand.nextInt(3) == 0)
            this.drop(
                ...
                ReikaItemHelper.getSizedItemStack(
                    ChromaStacks.energyPowder, 1 + rand.nextInt(4)
                )
            );
    }
    ...
    private void sendDeathParticles() {
        ReikaPacketHelper.sendDataPacket(
            ...
            ChromaPackets.CLOUDDIE.ordinal(),
        ...
        if (!worldObj.isRemote) {
            ...
            if (isAngry) {
                ...
                if (ep != null) {
                    ...
                    if (velocity != null) {
                        velocity.aimFrom(
                            posX, posY, posZ, ep.posX, ep.posY + 1.62, ep.posZ
                        );
                ...
                if (ep != null && ep.getDistanceSqToEntity(this) > 144) {
                    ...
                    if (LOS.isClearLineOfSight(worldObj)) {
                        if (velocity != null) {
                            velocity.aimFrom(
                                posX, posY, posZ, ep.posX, ep.posY + 1.3, ep.posZ
                            );
    ...
    private void attack() {
        if (worldObj.isRemote) {
            ...
            ReikaPacketHelper.sendDataPacket(
                ChromatiCraft.packetChannel,
                ChromaPackets.CLOUDATTACK.ordinal(),
                new PacketTarget.RadiusTarget(this, 32),
                this.getEntityId()
            );
        }
    ...
    public void doAttackFX() {
        ReikaSoundHelper.playClientSound(
            ChromaSounds.FLAREATTACK,
            this,
            2,
            2 * CrystalMusicManager.instance.getRandomScaledDing(CrystalElement.BLACK)
        );
        ...
        for (int i = 0; i < 180; i++) {
            ...
            EntityBlurFX fx = new EntityCCBlurFX(
                worldObj, px, py, pz, xyz[0] * v, xyz[1] * v, xyz[2] * v
            );
    ...
    public boolean getCanSpawnHere() {
        ...
        return rand.nextInt(n) == 0 /* && spawnedEntities < SPAWN_LIMIT*/
            && !ReikaEntityHelper.existsAnotherValidEntityWithin(
                this, dim ? 20 : 32, naturalSpawnedSelector
            ); // && worldObj.getClosestPlayer(posX, posY, posZ, 64) != null;
    }
    ...
    func_145780_a(int par1, int par2, int par3, Block par4) //play step sound
    {}
    ...
    public void doDeathParticles() {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            int t2 = (int) (t * (0.5 + rand.nextDouble()));
            float s = 1 + 2 * rand.nextFloat();
            fx.setPositionController(
                  new CollectingPositionController(px, py, pz, posX, posY, posZ, t)
            )
                .setColor(color)
                .setAlphaFading()
                .setRapidExpand()
    ...
    private float getColorFraction() {
        return colorTransitionTick > 0
            ? (float) colorTransitionTick / COLOR_TRANSITION_LENGTH
            : 0;
    }
    ...
    public boolean attackEntityFrom(DamageSource src, float dmg) {
        ...
        if (src.getClass().getName().equals("tconstruct.smeltery.SmelteryDamageSource")) {
            ...
            if (!ReikaPlayerAPI.isFake((EntityPlayer) e)) {
                ...
                List<EntityGlowCloud> li = worldObj.getEntitiesWithinAABBExcludingEntity(
                    this,
                    box,
                    new ReikaEntityHelper.ClassEntitySelector(this.getClass(), true)
                );
    ...
    isBlockNonColliding(World world, int x, int y, int z, Block block) {
        ...
        if (block.getMaterial() == Material.glass
            && block.getLightOpacity(world, x, y, z) == 0)
            return true;
        if (block.getMaterial() == Material.leaves || block instanceof BlockLeavesBase
