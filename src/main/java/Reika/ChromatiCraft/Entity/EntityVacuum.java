...
public class EntityVacuum
    extends Entity implements IEntityAdditionalSpawnData, CustomProjectile {
    ...
    private void destroy() {
        if (!worldObj.isRemote) {
            ...
            ReikaPacketHelper.sendDataPacket(
                ...
                ChromaPackets.VACUUMGUNEND.ordinal(),
                pt,
                this.getEntityId()
            );
        }
    ...
    private void travelParticles() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityFX fx = new EntitySparkleFX(worldObj, rx, ry, rz, vx, 0, vz)
                              .setGravity(g)
                              .setScale(s)
                              .setLife(l);
            fx.noClip = true;
    ...
    public static void doDestroyParticles(int entityID) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityFX fx
                = new EntityCCBlurFX(c, e.worldObj, rx, ry, rz, vp[0], vp[1], vp[2])
                      .setNoSlowdown()
                      .setScale(s)
                      .setRapidExpand()
                      .setLife(l);
    ...
    private void suckInEntities() {
        AxisAlignedBB box
            = AxisAlignedBB.getBoundingBox(posX, posY, posZ, posX, posY, posZ)
                  .expand(entityRange, entityRange, entityRange);
                    ...
                    if (i * i + j * j + k * k <= blockRange * blockRange + 0.5) {
                        ...
                        if (y >= 0 && y <= 255) {
                            ...
                            if (this.canMove(b, meta, worldObj, dx, dy, dz)) {
                                EntityFallingBlock e = new EntityTumblingBlock(
                                    worldObj,
                                    dx,
                                    dy,
                                    dz,
                                    b,
                                    worldObj.getBlockMetadata(dx, dy, dz)
                                );
    ...
    private boolean canMove(Block b, int meta, World world, int x, int y, int z) {
        ...
        if (b instanceof SemiUnbreakable)
            return !((SemiUnbreakable) b)
                        .isUnbreakable(world, x, y, z, world.getBlockMetadata(x, y, z));
        if (ReikaBlockHelper.isLiquid(b) && world.getBlockMetadata(x, y, z) != 0)
            return false;
        if (b instanceof SelectiveMovable
            && !((SelectiveMovable) b).canMove(world, x, y, z))
            return false;
        return world.isRemote
            || ReikaPlayerAPI.playerCanBreakAt(
                (WorldServer) world, x, y, z, (EntityPlayerMP) firingPlayer
            );
    }
