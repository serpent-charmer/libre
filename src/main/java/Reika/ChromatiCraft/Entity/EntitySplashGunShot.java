...
public class EntitySplashGunShot extends EntityFireball {
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote) {
            if (rand.nextInt(12) == 0) {
                AxisAlignedBB box
                    = ReikaAABBHelper.getEntityCenteredAABB(this, 5).expand(3, 0, 3);
    ...
    private void attack(EntityLivingBase e) {
        float dmg = firingPlayer.isPotionActive(Potion.damageBoost)
                || Chromabilities.RANGEDBOOST.enabledOn(firingPlayer)
            ? 20
            : 10;
    ...
    private void destroy() {
        ...
        ReikaPacketHelper.sendDataPacket(
            ChromatiCraft.packetChannel,
            ChromaPackets.SPLASHGUNEND.ordinal(),
            pt,
            this.getEntityId()
        );
    }
    ...
    public static void doDestroyParticles(int entityID) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            double[] vp = ReikaPhysicsHelper.polarToCartesian(
                v, e.worldObj.rand.nextDouble() * 360, e.worldObj.rand.nextDouble() * 360
            );
            float s = 5F + e.worldObj.rand.nextFloat() * 10;
            int l = 20 + e.worldObj.rand.nextInt(20);
            CrystalElement c = e.worldObj.rand.nextBoolean() ? CrystalElement.WHITE
                                                             : CrystalElement.PINK;
            int clr = ReikaColorAPI.getModifiedSat(
                ReikaColorAPI.getColorWithBrightnessMultiplier(c.getColor(), 0.4F), 2
            );
            EntityCCBlurFX fx
                = new EntityCCBlurFX(e.worldObj, rx, ry, rz, vp[0], vp[1], vp[2]);
            fx.setRapidExpand()
                .setAlphaFading()
                .setScale(s)
                .setLife(l)
        ...
        ReikaSoundHelper.playClientSound(
            ChromaSounds.SHOCKWAVE, Minecraft.getMinecraft().thePlayer, 0.5F, 1
        );
        ReikaSoundHelper.playClientSound(
            ChromaSounds.POWERDOWN, Minecraft.getMinecraft().thePlayer, 2, 0.75F
        );
    }
    ...
    public static void doAttackParticles(int sourceID, int targetID) {
        ...
        if (src != null && tgt != null) {
            CrystalElement e
                = world.rand.nextBoolean() ? CrystalElement.WHITE : CrystalElement.PINK;
            LightningBolt b = new LightningBolt(
                new DecimalPosition(src), new DecimalPosition(tgt), 6
            );
            b.setVariance(0.5);
            b.maximize();
        ...
        ReikaSoundHelper.playClientSound(
            ChromaSounds.DISCHARGE, Minecraft.getMinecraft().thePlayer, 0.25F, 1.5F
        );
    }
}
