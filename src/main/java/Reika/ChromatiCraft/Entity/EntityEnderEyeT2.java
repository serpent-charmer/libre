...
public final class EntityEnderEyeT2
    extends EntityEnderEye implements IEntityAdditionalSpawnData {
    ...
    static {
        ...
        double dm
            = DEVIATION_CHANCE_BASE + (DEVIATION_CHANCE_MAX - DEVIATION_CHANCE_BASE) / 2D;
    ...
    public void moveTowards(double x, int y, double z) {
        ...
        if (totalDistance > FUZZ) {
            int offset = new Random(
                             System.identityHashCode(worldObj)
                             + worldObj.getTotalWorldTime() / 24000
            )
                             .nextInt(16);
            colorData = new EncodedPosition(
                offset, MathHelper.floor_double(finalX), MathHelper.floor_double(finalZ)
            );
        }
        ...
        deviateTime = c != null
                && ReikaRandomHelper.doWithChance(
                    deviationCurve.getValue(c.getDistanceTo(this))
                )
            ? 10 + rand.nextInt(80 + ADDITIONAL_LIFE - 40 - 10)
            : -1;
    }
    ...
    private Coordinate getNearestTower() {
        Towers t = LoreManager.instance.getNearestActiveTower(
            worldObj, posX, posZ, this.getOwner(worldObj)
        );
        if (t != null) {
            int x;
            int y;
    ...
    private void deviate() {
        ...
        if (deviationTick > 30) {
            ...
            EntityEnderEyeT2 repl
                = EntityEnderEyeT2.create(worldObj, posX, posY, posZ, data);
    ...
    private void doParticles() {
        if (ticksExisted >= 6) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                int c = ReikaColorAPI.getModifiedHue(
                    0xff0000, ReikaRandomHelper.getRandomBetween(75, 180)
                );
                if (rand.nextInt(6) > 0)
                    c = (int) distanceColor.getValue(totalDistance);
                EntityCCBlurFX fx
                    = (EntityCCBlurFX) new EntityCCBlurFX(worldObj, dx, dy, dz)
                          .setLife(l)
                          .setScale(s)
                          .setColor(c);
        ...
        if (colorData != null && spiralColor != null
            && (deviateTime < 0 || deviateTime > ticksExisted)) {
            int t = ticksExisted;
            ...
            double r = 3;
            double l = t / lf;
            Vec3 vec = ReikaVectorHelper.getPointAroundVector(
                Vec3.createVectorHelper(motionX, motionY, motionZ), r, ang
            );
            EntityCCBlurFX fx = new EntityCCBlurFX(
                worldObj, posX + vec.xCoord, posY + vec.yCoord, posZ + vec.zCoord
            );
            float s = 7.5F;
            fx.setLife(180).setScale(s).setColor(c);
            fx.setIcon(ChromaIcons.FADE_BASICBLEND)
                .setRapidExpand()
                .setAlphaFading()
                .setBasicBlend();
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
    ...
    public void readSpawnData(ByteBuf buf) {
        ...
        if (amt != -1) {
            ...
            for (int i = 0; i < colorData.totalLength(); i++) {
                ...
                int t2 = lf * (i + 1) / amt;
                int cr = 3; //2;
                int c = colorData.isVariableChange(i) ? 0x22aaff
                    : colorData.isPartOfNegative(i)   ? 0x000000
                                                      : 0xffffff;
    ...
    public void sync() {
        ReikaPacketHelper.sendDataPacket(
            ...
            ChromaPackets.ENDEREYESYNC.ordinal(),
            new PacketTarget.RadiusTarget(this, 200),
            this.getEntityId(),
            ticksExisted
        );
    }
    ...
    public static EntityEnderEyeT2
    create(World world, double x, double y, double z, NBTTagCompound tag) {
