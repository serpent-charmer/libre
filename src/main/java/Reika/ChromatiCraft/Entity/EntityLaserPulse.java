...
public class EntityLaserPulse
    ...
    public EntityLaserPulse(
        World world, int x, int y, int z, CubeDirections dir, RGBColorData c, String l
    ) {
    ...
    protected boolean onEnterBlock(World world, int x, int y, int z) {
        ...
        if (b.isAir(world, x, y, z))
            return false;
        if (b == ChromaBlocks.SPECIALSHIELD.getBlockInstance()
            && world.getBlockMetadata(x, y, z) % 8 == BlockType.GLASS.metadata % 8)
            return false;
    ...
    public void reflect(CubeDirections d) {
        ...
        if (dir == null) {
            ChromatiCraft.logger.logError(
                "Tried to reflect from " + direction + " off of " + d + ", vec=" + dx
                + "," + dz
            );
            this.setDead();
            return;
