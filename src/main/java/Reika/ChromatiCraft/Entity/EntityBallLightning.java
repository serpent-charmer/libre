...
public class EntityBallLightning extends EntityLiving
    implements IEntityAdditionalSpawnData, DestroyOnUnload, EtherealEntity {
    ...
    public EntityBallLightning(
        World world, CrystalElement color, double x, double y, double z
    ) {
        super(world);
        this.color = color;
    ...
    private void lifeParticles() {
        ...
        if (rand.nextInt(1 + p * 2) == 0) {
            ...
            if (rand.nextInt(4) == 0)
                g = -g;
            EntityLaserFX fx = new EntityLaserFX(color, worldObj, px, py, pz)
                                   .setColor(this.getRenderColor())
                                   .setGravity(g);
            EntityLaserFX fx2 = new EntityLaserFX(color, worldObj, px, py, pz)
                                    .setColor(0xffffff)
                                    .setScale(0.42F)
                                    .setGravity(g);
    ...
    private void doBolt(EntityBallLightning other) {
        Vec3 vec = ReikaVectorHelper.getVec2Pt(
            posX, posY, posZ, other.posX, other.posY, other.posZ
        );
        ...
        for (double i = 0; i < len; i += 0.0625) {
            ...
            if (ChromaOptions.HOSTILEFOREST.getState()) {
                AxisAlignedBB box = AxisAlignedBB.getBoundingBox(dx, dy, dz, dx, dy, dz)
                                        .expand(0.5, 0.5, 0.5);
                List<EntityLivingBase> elb
    ...
    private void gluonParticles(
        ...
        int c = ReikaColorAPI.mixColors(
            other.getRenderColor(), this.getRenderColor(), frac
        );
    ...
    public AxisAlignedBB getCollisionBox(Entity entity) {
        if (entity instanceof EntityLivingBase && !(entity instanceof EntityBallLightning)
            && !worldObj.isRemote) {
            boolean flag
    ...
    public void onLivingUpdate() {
        ...
        if (!worldObj.isRemote && colorTransitionFraction == 0
            && rand.nextInt(400) == 0) {
            EntityBallLightning e
                = ReikaEntityHelper.getNearestEntityOfSameType(this, 24);
        ...
        if (!worldObj.isRemote) {
            ...
            //}
                ...
                AxisAlignedBB box
                    = AxisAlignedBB.getBoundingBox(posX, 0, posZ, posX, 1024, posZ)
                          .expand(24, 0, 24);
                List<EntityBallLightning> li
                    = worldObj.getEntitiesWithinAABB(this.getClass(), box);
    ...
    public static boolean canSpawnHere(World world, double x, double y, double z) {
        AxisAlignedBB box
            = AxisAlignedBB.getBoundingBox(x - 24, 0, z - 24, x + 24, 1024, z + 24);
        List<EntityBallLightning> li
            = world.getEntitiesWithinAABB(EntityBallLightning.class, box);
    ...
    public void onDeath(DamageSource src) {
        ...
        if (!worldObj.isRemote) {
            ...
            if (e instanceof EntityPlayer) {
                ...
                if (doDrops
                    && !ReikaPlayerAPI.isFakeOrNotInteractable(ep, posX, posY, posZ, 8)) {
                    ...
                    if (looting > 1) {
                        if (color.isPrimary())
                            ...
                            else if (TieredPlants.FLOWER.level.isPlayerAtStage(ep))
    ...
    @SideOnly(Side.CLIENT)
    public static void
    receiveDeathParticles(World world, double dx, double dy, double dz, int color) {
        doDeathParticles(world, dx, dy, dz, color);
    }

    @SideOnly(Side.CLIENT)
    private static void
    doDeathParticles(World world, double dx, double dy, double dz, int color) {
        int n = 32;
        for (int i = 0; i < n; i++) {
            EntityCenterBlurFX fx = new EntityCenterBlurFX(world, dx, dy, dz)
                                        .setColor(color)
                                        .setScale(1.5F);
            ...
            EntityCenterBlurFX fx2 = new EntityCenterBlurFX(world, dx, dy, dz)
                                         .setColor(0xffffff)
                                         .setScale(0.5F);
    ...
    public void readSpawnData(ByteBuf buf) {
        int c = buf.readInt();
        color = c >= 0 && c < 16 ? CrystalElement.elements[c]
                                 : CrystalElement.randomElement();
    }

    /*
    public int getRenderColor() {
        return targetColor != null ? ReikaColorAPI.mixColors(targetColor.getColor(),
    color.getColor(), colorTransitionFraction) : color.getColor();
    }
     */

    private int calcRenderColor() {
        return targetColor != null ? ReikaColorAPI.mixColors(
                   targetColor.getColor(), color.getColor(), colorTransitionFraction
               )
                                   : color.getColor();
    }
    ...
    public int getRenderColor() {
        ...
        return tg != null ? ReikaColorAPI.mixColors(tg.getColor(), c.getColor(), frac) :
    c.getColor();
    }
     */
