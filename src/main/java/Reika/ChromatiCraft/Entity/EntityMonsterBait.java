...
public class EntityMonsterBait
    extends InertEntity implements IEntityAdditionalSpawnData, VoidMonsterBait {
    ...
    public EntityMonsterBait(World world, EntityPlayer ep) {
        ...
        this.setLocationAndAngles(
            ep.posX, ep.posY + ep.getEyeHeight() / 4F, ep.posZ, 0, 0
        );
    ...
    private void doParticles() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            double[] xyz
                = ReikaPhysicsHelper.polarToCartesian(r, 0, rand.nextDouble() * 360);
            int c = ReikaColorAPI.mixColors(
                0xa0a0a0, 0xffffff, Math.min(rand.nextFloat(), this.getBrightness())
            );
            ...
            EntityCCBlurFX fx = new EntityCCBlurFX(
                worldObj, posX + xyz[0], posY + xyz[1], posZ + xyz[2]
            );
            ...
            if (currentEntity != null) {
                MotionController m = new EntityLockMotionController(
                    currentEntity, 0.03125 / 8, 0.125 * 4, 0.875
                );
                fx.setMotionController(m);
                c = ReikaColorAPI.mixColorBiDirectional(
                    ReikaEntityHelper.mobToColor(currentEntity),
                    0x000000,
                    0xffffff,
                    (float) ReikaRandomHelper.getRandomPlusMinus(0.5, 0.25)
                );
            }
            fx.setColor(c);
    ...
    public float getBrightness() {
        return life <= 0       ? 0
            : life >= MIN_LIFE ? 1
                               : (float) Math.sqrt(life / (float) MIN_LIFE);
    }

    public int getRenderColor(double layer) {
        if (currentEntity != null) {
            float f = (float) (0.5 + 0.25 * Math.sin(ticksExisted / 12D + layer * 60D));
            return ReikaColorAPI.mixColorBiDirectional(
                ReikaEntityHelper.mobToColor(currentEntity), 0x000000, 0xffffff, f
            );
        } else {
            int c0 = ReikaColorAPI.getColorWithBrightnessMultiplier(
                0xffffff, this.getBrightness()
            );
            return ReikaColorAPI.getColorWithBrightnessMultiplier(
                c0, 0.65F + (float) (0.35F * Math.sin(ticksExisted / 10D + layer * 40D))
            );
        }
    }
}
