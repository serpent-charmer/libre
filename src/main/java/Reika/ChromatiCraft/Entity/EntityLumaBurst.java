...
public class EntityLumaBurst extends ParticleEntity {
    ...
    public EntityLumaBurst(
        World world, int x, int y, int z, CubeDirections dir, CrystalElement e
    ) {
    ...
    public EntityLumaBurst(
        World world, int x, int y, int z, double ang, CrystalElement e
    ) {
        super(world, x, y, z);
        this.setColor(e);
        this.setAngle(ang);
    ...
    public void setRandomDirection(boolean fullFreedom) {
        if (fullFreedom) {
            ...
            double[] vel
                = ReikaPhysicsHelper.polarToCartesian(this.getSpeed(), slope, ang);
            motionX = vel[0];
            motionY = vel[1];
            motionZ = vel[2];
            ...
            this.setDirection(
                CubeDirections.list[rand.nextInt(CubeDirections.list.length)], false
            );
        }
    }
}
