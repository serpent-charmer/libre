...
public class EntityChainGunShot extends EntityFireball {
    private int impacts = Math.max(
        2, Math.min(12, rand.nextInt(6) + rand.nextInt(6))
    ...
    private void spawnTraceParticle() {
        ...
        for (double d = 0; d < 1; d += dd) {
            ...
            EntityFX fx = new EntityCCBlurFX(worldObj, px, py, pz, 0, 0, 0)
                              .setRapidExpand()
                              .setLife(l)
                              .setScale(s)
                              .setColor(0, 192, 255);
    ...
    protected void onImpact(MovingObjectPosition mov) {
        ...
        if (!worldObj.isRemote) {
            if (mov != null && mov.typeOfHit == MovingObjectType.ENTITY) {
                ...
                if (e != firingPlayer && e != lastHit && e instanceof EntityLivingBase
                    ...
                    ReikaPacketHelper.sendDataPacket(
                        ...
                        ChromaPackets.CHAINGUNHURT.ordinal(),
                        pt,
                        e.getEntityId()
                    );
                    int dmg = firingPlayer.isPotionActive(Potion.damageBoost)
    ...
    private void findAndRedirect(Entity start) {
        List<Entity> li = worldObj.getEntitiesWithinAABBExcludingEntity(
            start, boundingBox.expand(16, 6, 16)
        );
        if (!li.isEmpty()) {
    ...
    private void destroy() {
        ...
        ReikaPacketHelper.sendDataPacket(
            ChromatiCraft.packetChannel,
            ChromaPackets.CHAINGUNEND.ordinal(),
            pt,
            this.getEntityId()
        );
    }
    ...
    public static void doDamagingParticles(int entityID) {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            CrystalElement c = e.worldObj.rand.nextBoolean() ? CrystalElement.BLACK
                                                             : CrystalElement.PINK;
            EntityFX fx = new EntityCCBlurFX(c, e.worldObj, rx, ry, rz, vx, vy, vz)
                              .setNoSlowdown()
                              .setScale(s)
                              .setRapidExpand()
                              .setLife(l);
    ...
    public static void doDestructionParticles(int entityID) {
        EntityChainGunShot ec
            = (EntityChainGunShot) Minecraft.getMinecraft().theWorld.getEntityByID(
                entityID
            );
        for (int i = 0; i < 128; i++) {
            ...
            int color = ReikaColorAPI.getModifiedHue(
                0x0000ff, ReikaRandomHelper.getRandomPlusMinus(240, 60)
            );
            EntityFX fx = new EntityCCBlurFX(ec.worldObj, rx, ry, rz, vx, vy, vz)
                              .setNoSlowdown()
                              .setScale(s)
                              .setRapidExpand()
                              .setLife(l)
