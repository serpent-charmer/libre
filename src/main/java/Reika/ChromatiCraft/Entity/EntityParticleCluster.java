...
public class EntityParticleCluster
    ...
    private void move() {
        ...
        ticksUntilMove
            = ReikaRandomHelper.getRandomBetween(MIN_MOVEMENT_DELAY, MAX_MOVEMENT_DELAY);
    }
    ...
    public static class Particle {
        ...
        private static final double[] MOTION
            = { 0, 0.03125, 0.125, 0.25, 0.5, 0.75, 0.875, 0.9375, 1 };
        ...
        private void startMoving(EntityParticleCluster parent, double dx, double dz) {
            ticksUntilMove = ReikaRandomHelper.getRandomBetween(
        ...
        public double getRenderPosX(float ptick) {
            if (moveProgress == -1)
                return posX;
            double avg = ReikaMathLibrary.getUnequalAverage(
                MOTION[moveProgress], MOTION[moveProgress + 1], ptick
            );
