...
public class EntityTunnelNuker extends EntityLiving implements DestroyOnUnload {
    ...
    public void onLivingUpdate() {
        ...
        if (SFAPI.biomeHandler.isPinkForest(
                worldObj.isRemote
                    ? worldObj.getBiomeGenForCoords(x, z)
                    : ReikaWorldHelper.getNaturalGennedBiomeAt(worldObj, x, z)
            ))
    ...
    public static void doEntityTick(EntityLivingBase e) {
        if (e.worldObj.isRemote) {
            ...
            if (e.ticksExisted % 8 == 0) {
                ChromaSounds.TUNNELNUKERAMBIENT.playSound(
                    e, 0.2F + e.getRNG().nextFloat() * 0.2F, 1
                );
            }
            if (e.getRNG().nextInt(160) == 0) {
                ChromaSounds.TUNNELNUKERCALL.playSound(
                    e, 0.25F, 0.75F + e.getRNG().nextFloat() * 0.75F
                );
            }
    ...
    public static void doTunnelNukerFX(EntityLivingBase e) {
        ...
        fx.setRapidExpand().setAlphaFading().setScale(s).setLife(l).setColor(c
    ...
    public boolean getCanSpawnHere() {
        ...
        int x = MathHelper.floor_double(posX);
        int z = MathHelper.floor_double(posZ);
        return worldObj.canBlockSeeTheSky(x, y + 1, z)
            && !ReikaEntityHelper.existsAnotherValidEntityWithin(
                this, 32, new ReikaEntityHelper.ClassEntitySelector(this.getClass(), true)
            );
    }
