...
public class EntityDimensionFlare extends Entity /*Living*/ {
    ...
    private void spawnParticles() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            EntityBlurFX fx
                = new EntityCCBlurFX(worldObj, posX, posY, posZ, v[0], v[1], v[2])
                      .setColor(dir.color)
                      .setScale(1.5F + rand.nextFloat() * 1.5F);
        ...
        for (int i = 0; i < 1; i++) {
            ...
            double[] xyz = ReikaPhysicsHelper.polarToCartesian(
                1.25, rand.nextDouble() * 360, rand.nextDouble() * 360
            );
            EntityBlurFX fx = new EntityCCBlurFX(
                                  worldObj, posX + xyz[0], posY + xyz[1], posZ + xyz[2]
            )
                                  .setColor(dir.color)
                                  .setScale(0.75F + rand.nextFloat() * 0.5F);
            double d = rand.nextDouble() * 360;
            fx.setMotionController(
                new EntityLockMotionController(this, 0.03125 / 4, 0.125 * 8, 0.875)
            );
            Minecraft.getMinecraft().effectRenderer.addEffect(fx);
        }
        ...
        for (int i = 0; i < 1; i++) {
            ...
            ColorDirection dir = trailColors.get(idx);
            double[] xyz = ReikaPhysicsHelper.polarToCartesian(
                0.625, rand.nextDouble() * 360, rand.nextDouble() * 360
            );
            EntityBlurFX fx = new EntityCCBlurFX(
                                  worldObj,
                                  posX + xyz[0] - motionX,
    ...
    private void sendMessageToPlayer() {
        ...
        ReikaPacketHelper.sendStringPacket(
            ...
            ChromaPackets.FLAREMSG.ordinal(),
            s,
            new PacketTarget.PlayerTarget((EntityPlayerMP) target)
        );
    }
    ...
    public static enum RelationState {
        ...
        WELCOME(
            Integer.MAX_VALUE, "Friendliness", "Welcoming", "Admiration", "Gratitude"
        );
