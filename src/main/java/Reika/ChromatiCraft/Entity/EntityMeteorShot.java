...
public class EntityMeteorShot extends Entity implements IEntityAdditionalSpawnData {
    ...
    public static void registerAdjacency() {
        adjacency = TileEntityAdjacencyUpgrade.getOrCreateAdjacencyCheckHandler(
            CrystalElement.PINK, "Increase damage", ChromaTiles.METEOR
        );
    }
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote && !isDead && !this.impacted() && ticksExisted % 5 == 0)
            ReikaPacketHelper.sendSoundPacket(
                ChromaSounds.METEOR, worldObj, posX, posY, posZ, 0.75F, 1, false, 64
            );
    }
    ...
    private void spawnParticles() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            float s = ReikaRandomHelper.getRandomBetween(
                Math.max(1, level * 2), 2 + level * 3
            );
            EntityFX fx = new EntityFireSmokeFX(worldObj, px, py, pz, c)
                              .setScale(s)
                              .setRapidExpand();
    ...
    private void impact() {
        ...
        ReikaPacketHelper.sendDataPacket(
            ...
            ChromaPackets.METEORIMPACT.ordinal(),
            new PacketTarget.RadiusTarget(this, 96),
            this.getEntityId()
        );
        motionX = motionY = motionZ = 0;
    ...
    public static void doClientImpact(World world, int id) {
        ...
        if (e != null) {
            ...
            for (int i = 0; i < n; i++) {
                ...
                EntityFX fx
                    = new EntityCCBlurFX(e.worldObj, px, py, pz).setScale(s).setColor(c);
                Minecraft.getMinecraft().effectRenderer.addEffect(fx);
            }
            ...
            while (v > 0) {
                ...
                ReikaSoundHelper.playClientSound(
                    ChromaSounds.IMPACT, e.posX, e.posY, e.posZ, m, 2, false
                );
                v -= m;
            }
        }
    ...
    private double getDamage(EntityLiving e) {
        return this.getBaseDamage()
            * (1
               - 0.5 * e.getDistanceToEntity(this)
                   / TileEntityMeteorTower.attacks[level].splashRange);
    }
