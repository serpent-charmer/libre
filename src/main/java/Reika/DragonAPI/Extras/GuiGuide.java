...
public class GuiGuide extends GuiScreen {
    ...
    private static final XMLInterface guide = new XMLInterface(
        DragonAPICore.class,
        PARENT + "guide.xml",
    ...
    private static final String[] tabLabels
        = { "Info", "Getting Started", "Useful Notes", "Tips and Tricks" };
    ...
    private static void loadData() {
        for (int i = 0; i < mods.size(); i++) {
            ...
            for (int j = 0; j < tabTags.length; j++) {
                String tag = "dragonapi:" + mod.name().toLowerCase(Locale.ENGLISH) + ":"
                    + tabTags[j];
    ...
    public void initGui() {
        ...
        if (screen > 0) {
            for (int i = 0; i < tabLabels.length; i++) {
                ...
                buttonList.add(new GuiButton(i, j - 19 - 65 + 1, k + 20 * i, 85, 20, s)
                ); //Prev Page
            }
        }
    }
        ...
        if (screen >= 1) {
            ...
            if (mods.get(screen - 1).isLoaded()) {
                fontRendererObj.drawString(
                    "(Installed)", posX + 10 + w, posY + 8, 0x007700
                );
            } else {
                fontRendererObj.drawString(
                    "(Not Installed)", posX + 10 + w, posY + 8, 0x770000
                );
            }
        }
    }
    ...
    public void drawScreen(int x, int y, float f) {
        ...
        int yo = 0;
        /*
        fontRendererObj.drawString(HandbookRegistry.getEntry(screen, page).getTitle(),
        posX+xo+6, posY+yo+6, 0x000000); HandbookRegistry h =
        HandbookRegistry.getEntry(screen, page);
         */
        String s = "This book contains basic information about each of Reika's mods.";
        if (screen > 0) {
            String tag
                = "dragonapi:" + mods.get(screen - 1).name().toLowerCase(Locale.ENGLISH)
                + ":" + tabTags[page];
            s = data.get(tag);
        }
        fontRendererObj.drawSplitString(
            String.format("%s", s), posX + 9, posY + 88, 241, 0xffffff
        );
