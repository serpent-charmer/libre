...
public class IDHelper {
    ...
    private static HashMap<String, Integer> calcFluidContainers() {
        ...
        for (int i = 0; i < dat.length; i++) {
            ...
            String is1 = empty != null ? String.format(
                             "%s (%d:%d)",
                             empty.getDisplayName(),
                             Item.getIdFromItem(empty.getItem()),
                             empty.getItemDamage()
                         )
                                       : "null";
            String is2 = full != null ? String.format(
                             "%s (%d:%d)",
                             full.getDisplayName(),
                             Item.getIdFromItem(full.getItem()),
                             full.getItemDamage()
                         )
                                      : "null";
            String sg
                = String.format("%d of %s in %s > %s", fs.amount, f.getName(), is1, is2);
