...
public class ReplacementCraftingHandler {
    private static final MultiMap<Integer, IRecipe> mainData
        = new MultiMap(CollectionType.LIST, new ConcurrencyDeterminator());
    ...
    public static List<IRecipe> getRecipeList(
    ) { //Pass recipe objects directly by reference to allow easy reflection of edits
        ...
        li.tracking = true;
        cachedRecipeList = li;
        //ReikaJavaLibrary.pConsole("DRAGONAPI: Recipe list completed in
    ...
    public static void fireEventsForVanillaRecipes() {
        ...
        //DragonAPICore.log("Filtering "+getRecipeList().size()+" pre-init recipes through
        //event");
        for (IRecipe r : getRecipeList()) {
            ...
            if (ReikaRecipeHelper.verifyRecipe(r)) {
                if (ReikaRecipeHelper.isNonVForgeRecipeClass(r)) {
                    DragonAPICore.log(
                        "Found a modded recipe registered in pre-init! This is a design error, as it can trigger a Forge bug and break the recipe! Recipe="
                        + ReikaRecipeHelper.toString(r)
                    );
                }
                ...
                DragonAPICore.logError(
                    "Found an invalid recipe in the list, with either nulled inputs or outputs! This is invalid! Class="
                    + r.getClass()
                );
            }
        }
    ...
    public static void sortRecipes() {
        ...
        DragonAPICore.log(
            "Recipe sort completed in " + (System.currentTimeMillis() - time) + " ms."
        );
    }
    ...
    private static class BackWritableList<E> extends ArrayList<E> {
        ...
        public boolean add(E obj) {
            if (tracking) {
                ...
                this.log(
                    "Logged a direct crafting recipe list operation, adding "
                    + ReikaRecipeHelper.toString((IRecipe) obj)
                );
            }
        ...
        public boolean remove(Object obj) {
            if (tracking) {
                ...
                this.log(
                    "Logged a direct crafting recipe list operation, removing "
                    + ReikaRecipeHelper.toString((IRecipe) obj)
                );
            }
        ...
        private void log(String s) {
            //if (DragonAPIInit.instance != null && DragonAPIInit.instance.getModLogger()
            //!= null) { //called too early to be safe 	DragonAPICore.log(s);
            //}
            //else {
