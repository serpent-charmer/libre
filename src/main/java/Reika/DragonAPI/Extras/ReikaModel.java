...
public class ReikaModel extends ModifiedPlayerModel {
    ...
    public void renderAll(
        Entity e, float par2, float par3, float par4, float par5, float par6, float par7
    ) {
    ...
    public void bindTexture() {
        ReikaTextureHelper.bindFinalTexture(
            DragonAPICore.class, "/Reika/DragonAPI/Resources/reika_tex.png"
        );
    }

    @Override
    public void renderBodyParts(EntityPlayer ep, float tick) {
        if (ep.equals(Minecraft.getMinecraft().thePlayer)
            && !ReikaPlayerAPI.isReika(Minecraft.getMinecraft().thePlayer))
            return;
    ...
    protected void setPartAngles(EntityPlayer ep, float tick) {
        ...
        float yawHead
            = -ep.rotationYaw % 360 - tick * (ep.rotationYaw - ep.prevRotationYaw);
        float yaw = -ep.renderYawOffset % 360
            - tick * (ep.renderYawOffset - ep.prevRenderYawOffset) + 180;
