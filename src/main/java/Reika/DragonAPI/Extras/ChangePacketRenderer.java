...
public class ChangePacketRenderer {
    ...
    public static void onBlockChangePacket(int x, int y, int z, Block b, int meta) {
        if (isActive) {
            //DragonAPICore.log("Received block change packet @ "+x+", "+y+", "+z+" for
            //"+b.getClass().getName()+" | "+b.getUnlocalizedName()+" meta "+meta);
    ...
    public static void
    onChunkRerender(int mx, int my, int mz, int px, int py, int pz, WorldRenderer r) {
        if (isActive) {
            if (py == 257 && my == -1 && px - mx == 17
                && pz - mz == 17) { //chunk (un)loading
                //long ckey = ChunkCoordIntPair.chunkXZ2Int(mx >> 4, mz >> 4);
                return;
            }
            //DragonAPICore.log("Triggering rerender @ "+new BlockBox(mx, my, mz, px, py,
            //pz)); Thread.dumpStack();
    ...
    private void renderPosition(Tessellator v5, RenderBox b) {
        ...
        double o = 0.03125;
        v5.setColorRGBA_I(
            b.renderColor, (int) (ReikaColorAPI.getAlpha(b.renderColor) * f)
        );
    ...
    private static class RenderBox {
        ...
        private RenderBox(Coordinate c, int clr, boolean re) {
            location = c;
            renderColor = clr;
            subchunk = new Coordinate(
                ReikaMathLibrary.roundDownToX(16, c.xCoord),
                ReikaMathLibrary.roundDownToX(16, c.yCoord),
                ReikaMathLibrary.roundDownToX(16, c.zCoord)
            );
            isReRender = re;
        }
    }
