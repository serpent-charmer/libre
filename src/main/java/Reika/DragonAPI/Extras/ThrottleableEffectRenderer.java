...
public class ThrottleableEffectRenderer extends EffectRenderer {
    ...
    private static AxisAlignedBB particleBox
    ...
    public void addEffect(EntityFX fx) {
        if (fx == null)
            throw new IllegalArgumentException(
                "You cannot spawn a null particle! This is a bug in the mod calling this code!"
            );
    ...
    private boolean isInWall(EntityFX fx) {
        ...
        if (b.isOpaqueCube() && b.renderAsNormalBlock() && b.getRenderType() == 0) {
            ...
            if (ReikaMathLibrary.isValueInsideBounds(minX, maxX, fx.posX)
                && ReikaMathLibrary.isValueInsideBounds(minY, maxY, fx.posY)
                && ReikaMathLibrary.isValueInsideBounds(minZ, maxZ, fx.posZ)) {
            ...
            if (!fxLayers[i].isEmpty()) {
                ...
                for (EntityFX fx : ((Collection<EntityFX>) fxLayers[i])) {
                    if (fx != null && isParticleVisible(fx)) {
                        //if (isEntityCloseEnough(fx, EntityFX.interpPosX,
                        //EntityFX.interpPosY, EntityFX.interpPosZ)) {
                        ...
                        if (!v5.isDrawing) {
                            DragonAPICore.logError(
                                "Particle " + fx
                                + " left the tessellator in a bad state, stopped drawing!"
                            );
                            v5.startDrawingQuads();
                        }
                        //}
    ...
    private void throwCrash(final int i, final EntityFX fx, Throwable throwable) {
        CrashReport crashreport
            = CrashReport.makeCrashReport(throwable, "Rendering Particle");
    ...
    public static AxisAlignedBB getBoundingBox(EntityFX fx) {
        return particleBox.setBounds(
            fx.posX - fx.particleScale,
            fx.posY - fx.particleScale,
            fx.posZ - fx.particleScale,
            fx.posX + fx.particleScale,
            fx.posY + fx.particleScale,
            fx.posZ + fx.particleScale
        );
