...
public class GenLayerControllableShore extends GenLayer {
        ...
        for (int bx = 0; bx < height; ++bx) {
            for (int bz = 0; bz < width; ++bz) {
                ...
                if (originalID == BiomeGenBase.mushroomIsland.biomeID) {
                    if (neighbor1 != BiomeGenBase.ocean.biomeID
                        && neighbor2 != BiomeGenBase.ocean.biomeID
                        && neighbor3 != BiomeGenBase.ocean.biomeID
                        && neighbor4 != BiomeGenBase.ocean.biomeID)
                        cache[bz + bx * width] = originalID;
                    else
                        cache[bz + bx * width] = this.getHookedShore(
                            original, BiomeGenBase.mushroomIslandShore.biomeID
                        );

                } else if (original != null && original.getBiomeClass() == BiomeGenJungle.class) {
                    if (this.func_151631_c(neighbor1) && this.func_151631_c(neighbor2)
                        && this.func_151631_c(neighbor3)
                        && this.func_151631_c(neighbor4)) {
                        if (this.hasNoAdjacentOcean(
                                neighbor1, neighbor2, neighbor3, neighbor4
                            ))
                            cache[bz + bx * width] = originalID;
                        else
                            cache[bz + bx * width] = this.getHookedShore(
                                original, BiomeGenBase.beach.biomeID
                            );
                    } else {
                    ...
                    if (original != null && original.func_150559_j()) {
                        this.applyBeachIfValid(
                            ...
                            width,
                            originalID,
                            this.getHookedShore(original, BiomeGenBase.coldBeach.biomeID)
                        );
                    }
					else if (originalID != BiomeGenBase.mesa.biomeID && originalID != BiomeGenBase.mesaPlateau_F.biomeID) {
                        if (originalID != BiomeGenBase.ocean.biomeID
                            && originalID != BiomeGenBase.deepOcean.biomeID
                            && originalID != BiomeGenBase.river.biomeID
                            && originalID != BiomeGenBase.swampland.biomeID) {
                            if (this.hasNoAdjacentOcean(
                                    neighbor1, neighbor2, neighbor3, neighbor4
                                ))
                                cache[bz + bx * width] = originalID;
                        ...
                        if (this.hasNoAdjacentOcean(
                            ...
                            if (this.isMesa(neighbor1) && this.isMesa(neighbor2)
                                && this.isMesa(neighbor3) && this.isMesa(neighbor4))
                                cache[bz + bx * width] = originalID;
                            else
                    ...
                    this.applyBeachIfValid(
                        ...
                        width,
                        originalID,
                        this.getHookedShore(original, BiomeGenBase.stoneBeach.biomeID)
                    );
                }
            }
        }
    ...
    private boolean hasNoAdjacentOcean(int n1, int n2, int n3, int n4) {
        return !isBiomeOceanic(n1) && !isBiomeOceanic(n2) && !isBiomeOceanic(n3)
            && !isBiomeOceanic(n4);
    }

    private void applyBeachIfValid(
        int[] data,
        int[] cache,
        int permutedX,
        int permutedZ,
    ...
    private boolean func_151631_c(int id) {
        return BiomeGenBase.getBiome(id) != null
                && BiomeGenBase.getBiome(id).getBiomeClass() == BiomeGenJungle.class
            ? true
            : id == BiomeGenBase.jungleEdge.biomeID || id == BiomeGenBase.jungle.biomeID
                || id == BiomeGenBase.jungleHills.biomeID
                || id == BiomeGenBase.forest.biomeID || id == BiomeGenBase.taiga.biomeID
                || isBiomeOceanic(id);
    }

    private boolean isMesa(int id) {
        return BiomeGenBase.getBiome(id) != null
            && BiomeGenBase.getBiome(id) instanceof BiomeGenMesa;
    }
}