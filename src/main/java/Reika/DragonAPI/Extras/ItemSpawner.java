    ...
    public boolean onItemUse(
        ...
        if (!this.isValidDimensionForSpawner(is, world)) {
            ReikaChatHelper.write(
                ReikaSpawnerHelper.getSpawnerFromItemNBT(is)
                + " cannot be placed in dimension " + world.provider.getDimensionName()
                + "!"
            );
            return false;
        }
        ...
        if (!ep.canPlayerEdit(x, y, z, 0, is))
            ...
            if (spw != null) {
                ...
                lgc.spawnDelay = Math.max(
                    lgc.minSpawnDelay,
                    itemRand.nextInt(Math.min(900, 1 + lgc.maxSpawnDelay))
                ); //20s delay
            }
        }
