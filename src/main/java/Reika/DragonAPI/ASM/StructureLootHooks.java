...
public class StructureLootHooks implements IClassTransformer {
        ...
        if (superClasses.contains(cn.name) || superClasses.contains(cn.superName)) {
            ...
            try {
                ...
                MethodNode m = ReikaASMHelper.getMethodByName(
                    ...
                    "(Lnet/minecraft/world/World;Lnet/minecraft/world/gen/structure/StructureBoundingBox;Ljava/util/Random;III[Lnet/minecraft/util/WeightedRandomChestContent;I)Z"
                );
                MethodInsnNode look = ReikaASMHelper.getFirstMethodCallByName(
                    cn,
                    m,
                    FMLForgePlugin.RUNTIME_DEOBF ? "func_76293_a"
                ...
                ReikaASMHelper.log(
                    "Skipping loot hooks on " + cn.name
                    + "; does not contain loot generation method"
                );
                ReikaASMHelper.activeMod = null;
                return bytes;
                ...
                ReikaASMHelper.log(
                    "Skipping loot hooks on " + cn.name
                    + "; does not contain call to generate loot"
                );
                ReikaASMHelper.activeMod = null;
                return bytes;
            } catch (Exception e) {
                ReikaASMHelper.log(
                    "Could not add loot hooks to " + cn.name + ": " + e.toString()
                );
                e.printStackTrace();
                ReikaASMHelper.activeMod = null;
                return bytes;
    ...
    public static void inject(ClassNode cn, MethodNode m, AbstractInsnNode loc) {
        int var = ((VarInsnNode) ReikaASMHelper.getLastOpcodeBefore(
                       m.instructions, m.instructions.indexOf(loc), Opcodes.ASTORE
                   ))
                      .var;
        ...
        li.add(new MethodInsnNode(
            ...
            "Reika/DragonAPI/Instantiable/Event/ChestLootEvent",
            "fire",
            "(Ljava/lang/Object;Lnet/minecraft/inventory/IInventory;)V",
            false
        ));
