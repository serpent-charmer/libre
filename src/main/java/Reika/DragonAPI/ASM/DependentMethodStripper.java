...
public class DependentMethodStripper implements IClassTransformer {
    private static final String baseString
        = "LReika/DragonAPI/ASM/DependentMethodStripper$";
    private static final boolean DEBUG = true;
    static final boolean runInDev
        = ReikaJVMParser.isArgumentPresent("-DragonAPI_ForceMethodStrip");
    ...
    public byte[] transform(String name, String transformedName, byte[] bytes) {
        ...
        if (!FMLForgePlugin.RUNTIME_DEOBF
            && !runInDev) //prevents needing to always reload game in dev env (ASM not run
                          //on src edit, so Eclipse thinks new methods)
            return bytes;
        ...
        try {
            boolean smartStripClass = ReikaASMHelper.memberHasAnnotationOfType(
                classNode, "LReika/DragonAPI/ASM/DependentMethodStripper$SmartStrip"
            );
            if (smartStripClass) {
                    ...
                    if (processSmart(field, false)) {
                        if (DEBUG) {
                            ReikaJavaLibrary.pConsole(String.format(
                                "DRAGONAPI ASM: Removing Field: '%s.%s'; Reason: %s",
                                classNode.name,
                                field.name,
                                "Refers to non-present class"
                            ));
                        }
                        fields.remove();
                    }
                ...
                while (methods.hasNext()) {
                    ...
                    if (processSmart(method, false)) {
                        if (DEBUG) {
                            ReikaJavaLibrary.pConsole(String.format(
                                "DRAGONAPI ASM: Removing Method: '%s.%s'; Reason: %s",
                                classNode.name,
                                method.name,
                                "Refers to non-present class"
                            ));
                ...
                while (methods.hasNext()) {
                    ...
                    if (a != null) {
                        if (DEBUG) {
                            ReikaJavaLibrary.pConsole(String.format(
                                "DRAGONAPI ASM: Removing Method: '%s.%s%s'; Reason: %s",
                                classNode.name,
                                method.name,
                                method.desc,
                                a.text
            ...
            if (a != null) {
                if (DEBUG) {
                    ReikaJavaLibrary.pConsole(String.format("DRAGONAPI ASM: Removing
            Method: %s.%s%s; Reason: %s", classNode.name, method.name, method.desc,
            a.text));
                }
                methods.remove();
            }
    ...
    private AnnotationFail
    parseAnnotation(ClassNode cn, Annotations a, AnnotationNode ann) {
    ...
    private AnnotationFail
    parseAnnotationEntry(ClassNode cn, Annotations a, Object values) {
    ...
    private static class InvalidStrippingAnnotationException extends ASMException {
        ...
        public final String getMessage() {
            ...
            sb.append(
                ...
                + " is not valid dependency annotation!"
            );
            return sb.toString();
        }
    }
    ...
    private static final AnnotationFail smartFail
        = new AnnotationFail(Annotations.SMART, "");
    ...
    private static boolean processSmart(MethodNode mn, boolean needAnnotation) {
        if (needAnnotation
            && !ReikaASMHelper.memberHasAnnotationOfType(
                mn, "LReika/DragonAPI/ASM/DependentMethodStripper$SmartStrip"
            ))
            return false;
        ArrayList<String> args = ReikaASMHelper.parseMethodArguments(mn);
        for (String s : args) {
            if (s.length() > 1
                && !ReikaASMHelper.checkForClass(s.substring(1, s.length() - 1)
                )) //> 1 to avoid primitives, substring to drop L- and -;
                return true;
        }
        return false;
    }
    ...
    private static boolean processSmart(FieldNode fn, boolean needAnnotation) {
        return (!needAnnotation
                || ReikaASMHelper.memberHasAnnotationOfType(
                    fn, "LReika/DragonAPI/ASM/DependentMethodStripper$SmartStrip"
                ))
            && !ReikaASMHelper.checkForClass(fn.desc);
    }
