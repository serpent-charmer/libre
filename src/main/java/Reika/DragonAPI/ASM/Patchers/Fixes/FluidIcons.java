...
public class FluidIcons extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        ...
        MethodNode m = ReikaASMHelper.getMethodByName(
            cn,
            "renderWorldBlock",
            "(Lnet/minecraft/world/IBlockAccess;IIILnet/minecraft/block/Block;ILnet/minecraft/client/renderer/RenderBlocks;)Z"
        );
        for (int i = 0; i < m.instructions.size(); i++) {
            AbstractInsnNode ain = m.instructions.get(i);
            if (ain.getOpcode() == Opcodes.INVOKEVIRTUAL) {
                MethodInsnNode min = (MethodInsnNode) ain;
                if (min.owner.equals("net/minecraft/block/Block") && min.name.equals(name)
                    && min.desc.equals("(II)Lnet/minecraft/util/IIcon;")) {
                    min.desc
