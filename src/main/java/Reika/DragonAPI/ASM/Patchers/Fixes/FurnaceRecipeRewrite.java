        ...
        for (MethodNode m : c) {
            if (m.name.equals("<init>")) {
                AbstractInsnNode ain
                    = ReikaASMHelper.getLastOpcode(m.instructions, Opcodes.INVOKEVIRTUAL);
                m.instructions.insert(
                    ain,
                    new MethodInsnNode(
                        Opcodes.INVOKESTATIC,
                        "Reika/DragonAPI/Extras/ReplacementSmeltingHandler",
                        "onSmeltingInit",
                        "(Lnet/minecraft/item/crafting/FurnaceRecipes;)V",
                        false
                    )
                );
                ...
                li.add(new MethodInsnNode(
                    ...
                    "Reika/DragonAPI/Extras/ReplacementSmeltingHandler",
                    "checkRecipe",
                    "(Lnet/minecraft/item/ItemStack;Lnet/minecraft/item/ItemStack;F)Z",
                    false
                ));
                ...
                if (redirect != null) {
                    ...
                    li.add(L1);
                    li.add(
                        new FrameNode(Opcodes.F_SAME, 0, new Object[0], 0, new Object[0])
                    );
                    li.add(new VarInsnNode(Opcodes.ALOAD, 0));
                    li.add(ReikaASMHelper.copyInsnList(args));
                    li.add(new MethodInsnNode(
                        Opcodes.INVOKEVIRTUAL, cn.name, m.name, m.desc, false
                    ));
                    li.add(L2);
                    String frame = ReikaASMHelper.convertLPrefixToPlain(
                        ReikaASMHelper.getMethodReturnType(m)
                    );
                    if (frame.equals("F")) {
                        frame = "float";
                    }
                    li.add(new FrameNode(
                        Opcodes.F_SAME1, 0, new Object[0], 1, new Object[] { frame }
                    ));
                    ...
                    ReikaASMHelper.log(
                        "Redirecting furnace smelting recipes method '" + orig
                        + "' to DragonAPI replacement '" + redirect + "' (delegating to '"
                        + m.name + "') with signature '" + m.desc + "'"
                    );
                }
            }
        }
