...
public class PotionPacketID1 extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        ...
        // break;
        FieldNode f
            = ReikaASMHelper.getFieldByName(cn, "field_149432_b", "field_149432_b");
        f.desc = "I";

        MethodNode m = ReikaASMHelper.getMethodByName(
            cn, "<init>", "(ILnet/minecraft/potion/PotionEffect;)V"
        );
        ...
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT) {
            try {
                m = ReikaASMHelper.getMethodByName(
                    cn, "func_149427_e", "func_149427_e", "()B"
                );
            } catch (NoSuchASMMethodException e1) {
                try {
                    ReikaASMHelper.getMethodByName(
                        cn, "func_149427_e", "func_149427_e", "()I"
                    );
