...
public class WorldRenderAlpha extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        //MethodNode m = ReikaASMHelper.getMethodByName(cn, "func_78471_a", "renderWorld",
        //"(FJ)V"); MethodInsnNode min = ReikaASMHelper.getFirstMethodCallByName(cn, m,
        //"glAlphaFunc");

        //LdcInsnNode ldc = (LdcInsnNode)min.getPrevious();
        //ldc.cst = 1/255F; //0.5F by default!? nuts!

        //IntInsnNode iin = (IntInsnNode)ReikaASMHelper.getLastInsnBefore(m.instructions,
        //m.instructions.indexOf(min), Opcodes.SIPUSH, 3008);
        //ReikaASMHelper.deleteFrom(m.instructions, iin, min);

        //MethodInsnNode enable =
        //(MethodInsnNode)ReikaASMHelper.getLastOpcodeBefore(m.instructions,
        //m.instructions.indexOf(min)-1, Opcodes.INVOKESTATIC); enable.name = "glDisable";
        ...
        MethodNode m = ReikaASMHelper.getMethodByName(
            cn, "func_147890_b", "preRenderBlocks", "(I)V"
        );
        MethodInsnNode ref = ReikaASMHelper.getFirstMethodCallByName(
            cn, m, FMLForgePlugin.RUNTIME_DEOBF ? "func_78382_b" : "startDrawingQuads"
        );
