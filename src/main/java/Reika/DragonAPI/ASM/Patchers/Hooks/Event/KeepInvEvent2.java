...
public class KeepInvEvent2 extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        ...
        li.add(new MethodInsnNode(
            ...
            "Reika/DragonAPI/Instantiable/Event/PlayerKeepInventoryEvent",
            "fire",
        ...
        MethodNode m = ReikaASMHelper.getMethodByName(
            cn, "func_70645_a", "onDeath", "(Lnet/minecraft/util/DamageSource;)V"
        );
        if (ReikaASMHelper.checkForClass("api.player.forge.PlayerAPITransformer")) {
            m = ReikaASMHelper.getMethodByName(
                cn, "localOnDeath", "(Lnet/minecraft/util/DamageSource;)V"
            ); //Try his method instead
        }

        AbstractInsnNode ain = ReikaASMHelper.getFirstInsnAfter(
            m.instructions, 0, Opcodes.LDC, "keepInventory"
        );
        if (ain == null)
            ReikaASMHelper.throwConflict(
                this.toString(), cn, m, "Could not find 'keepInventory' gamerule lookup"
            );
        AbstractInsnNode load = ReikaASMHelper.getLastOpcodeBefore(
