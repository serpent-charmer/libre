...
public class SetBlock extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        ...
        MethodNode m = ReikaASMHelper.getMethodByName(
            cn, "func_150807_a", "func_150807_a", "(IIILnet/minecraft/block/Block;I)Z"
        );
        for (int i = 0; i < m.instructions.size(); i++) {
            ...
            if (ain.getOpcode() == Opcodes.IRETURN) {
                if (ain.getPrevious().getOpcode() == Opcodes.ICONST_1) {
                    ...
                    /*
                    m.instructions.insertBefore(loc, new FieldInsnNode(Opcodes.GETSTATIC,
                    "net/minecraftforge/common/MinecraftForge", "EVENT_BUS",
                    "Lcpw/mods/fml/common/eventhandler/EventBus;"));
                    m.instructions.insertBefore(loc, new TypeInsnNode(Opcodes.NEW,
                    "Reika/DragonAPI/Instantiable/Event/SetBlockEvent$Post"));
                    ...
                    m.instructions.insertBefore(loc, new
                    MethodInsnNode(Opcodes.INVOKESPECIAL,
                    "Reika/DragonAPI/Instantiable/Event/SetBlockEvent$Post", "<init>",
        ...
        li.add(new MethodInsnNode(
            Opcodes.INVOKESTATIC,
            "Reika/DragonAPI/Instantiable/Event/SetBlockEvent$Pre",
            "fire",
            "(Lnet/minecraft/world/chunk/Chunk;IIILnet/minecraft/block/Block;I)V",
            false
        ));
        ...
        m = ReikaASMHelper.getMethodByName(
            cn, "func_76589_b", "setBlockMetadata", "(IIII)Z"
        );
        for (int i = 0; i < m.instructions.size(); i++) {
            AbstractInsnNode ain = m.instructions.get(i);
            if (ain.getOpcode() == Opcodes.IRETURN) {
