...
public class LiquidIconRB extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        String name = FMLForgePlugin.RUNTIME_DEOBF ? "func_147787_a"
                                                   : "getBlockIconFromSideAndMetadata";
        MethodNode m = ReikaASMHelper.getMethodByName(
            cn, "func_147721_p", "renderBlockLiquid", "(Lnet/minecraft/block/Block;III)Z"
        );
    ...
    private void replaceIconCall(
        ClassNode cn, InsnList li, int idx, MethodInsnNode min
    ) { //on stack: block, s, meta
        /*
        ...
        repl.add(new FieldInsnNode(Opcodes.GETFIELD, ReikaASMHelper.convertClassName(cn,
        false), FMLForgePlugin.RUNTIME_DEOBF ? "field_147845_a" : "blockAccess",
        "Lnet/minecraft/world/IBlockAccess;")); repl.add(new VarInsnNode(Opcodes.ILOAD,
        2)); repl.add(new VarInsnNode(Opcodes.ILOAD, 3)); repl.add(new
        VarInsnNode(Opcodes.ILOAD, 4)); repl.add(new VarInsnNode(Opcodes.ILOAD, vin.var));
        repl.add(new MethodInsnNode(Opcodes.INVOKEVIRTUAL,
        ReikaASMHelper.convertClassName(cn, false), call, sig, false));
        ...
        min.desc
            = "(Lnet/minecraft/client/renderer/RenderBlocks;Lnet/minecraft/block/Block;IIIII)Lnet/minecraft/util/IIcon;";
        min.itf = false;
    }
