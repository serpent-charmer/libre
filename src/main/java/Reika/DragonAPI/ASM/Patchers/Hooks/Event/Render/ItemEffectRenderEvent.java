...
public class ItemEffectRenderEvent extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        ...
        MethodNode m = ReikaASMHelper.getMethodByName(
            cn,
            "renderDroppedItem",
            "(Lnet/minecraft/entity/item/EntityItem;Lnet/minecraft/util/IIcon;IFFFFI)V"
        );
        String name = "hasEffect"; //Forge one //FMLForgePlugin.RUNTIME_DEOBF ?
        ...
        if (ain == null) {
            ReikaASMHelper.throwConflict(this.toString(), cn, m, "Could not find
        instruction pattern for ItemStack.hasEffect(int pass),
        Insns:\n"+ReikaASMHelper.clearString(m.instructions));
        }
        MethodInsnNode min =
        (MethodInsnNode)ain.getNext().getNext();//ReikaASMHelper.getFirstMethodCall(cn, m,
        "net/minecraft/item/ItemStack", name, "(I)Z");
         */
        ...
        for (int i = 0; i < m.instructions.size(); i++) {
            ...
            if (ain.getOpcode() == Opcodes.INVOKEVIRTUAL) {
                ...
                if (min.name.equals(name) && min.desc.equals("(I)Z")) {
                    min.owner
                        = "Reika/DragonAPI/Instantiable/Event/Client/ItemEffectRenderEvent";
                    min.name = "fire";
