...
public class SpawnerCheckPlayersEvent extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        MethodNode m
            = ReikaASMHelper.getMethodByName(cn, "func_98279_f", "isActivated", "()Z");
        m.instructions.clear();
        InsnList li = new InsnList();
        li.add(new VarInsnNode(Opcodes.ALOAD, 0));
        li.add(new MethodInsnNode(
            Opcodes.INVOKESTATIC,
            "Reika/DragonAPI/Instantiable/Event/SpawnerCheckPlayerEvent",
            "runCheck",
            "(Lnet/minecraft/tileentity/MobSpawnerBaseLogic;)Z",
            false
        ));
