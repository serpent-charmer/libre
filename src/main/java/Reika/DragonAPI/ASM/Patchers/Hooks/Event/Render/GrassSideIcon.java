...
public class GrassSideIcon extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        String seek
            = FMLForgePlugin.RUNTIME_DEOBF ? "func_149990_e" : "getIconSideOverlay";
        ...
        for (MethodNode m : cn.methods) {
            for (int i = 0; i < m.instructions.size(); i++) {
                ...
                if (ain.getOpcode() == Opcodes.INVOKESTATIC) {
                    ...
                    if (min.owner.equals("net/minecraft/block/BlockGrass")
                        && min.name.equals(seek) && min.desc.equals(seeksig)) {
    ...
    private void redirect(ClassNode cn, InsnList li, MethodInsnNode min) {
        ...
        min.name = "fireSide";
        min.desc
            = "(Lnet/minecraft/block/Block;Lnet/minecraft/world/IBlockAccess;III)Lnet/minecraft/util/IIcon;";
