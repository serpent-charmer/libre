...
public class ItemUpdate extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        ...
        fire.add(new FieldInsnNode(
            Opcodes.GETSTATIC,
            "net/minecraftforge/common/MinecraftForge",
            "EVENT_BUS",
            "Lcpw/mods/fml/common/eventhandler/EventBus;"
        ));
        fire.add(new TypeInsnNode(
            Opcodes.NEW, "Reika/DragonAPI/Instantiable/Event/ItemUpdateEvent"
        ));
        fire.add(new InsnNode(Opcodes.DUP));
        fire.add(new VarInsnNode(Opcodes.ALOAD, 0));
        fire.add(new MethodInsnNode(
            Opcodes.INVOKESPECIAL,
            "Reika/DragonAPI/Instantiable/Event/ItemUpdateEvent",
            "<init>",
        ...
        for (int i = 0; i < m.instructions.size(); i++) {
            ...
            if (ain.getOpcode() == Opcodes.INVOKESPECIAL) {
                ...
                if (min.name.equals(m.name)
                    && min.owner.equals("net/minecraft/entity/Entity")) {
