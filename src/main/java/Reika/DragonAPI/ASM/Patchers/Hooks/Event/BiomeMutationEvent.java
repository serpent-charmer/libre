...
public class BiomeMutationEvent extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        ...
        li.add(new MethodInsnNode(
            Opcodes.INVOKESTATIC,
            "Reika/DragonAPI/Instantiable/Event/BiomeMutationEvent",
            "fireTry",
            "(Lnet/minecraft/world/gen/layer/GenLayer;IIIII)Z",
            false
        ));
        ...
        AbstractInsnNode end
            = ReikaASMHelper.getFirstOpcode(m.instructions, Opcodes.IASTORE);
        AbstractInsnNode start = ReikaASMHelper.getLastInsnBefore(
            m.instructions, m.instructions.indexOf(end), Opcodes.ILOAD, var
        );
        ...
        li.add(new MethodInsnNode(
            Opcodes.INVOKESTATIC,
            "Reika/DragonAPI/Instantiable/Event/BiomeMutationEvent$GetMutatedBiomeEvent",
            "fireGet",
            "(Lnet/minecraft/world/gen/layer/GenLayer;IIIII)I",
            false
        ));
        ...
        AbstractInsnNode ain
            = ReikaASMHelper.getFirstInsnAfter(m.instructions, 0, Opcodes.IINC, var, 128);
        if (ain == null) {
            ReikaASMHelper.log(
                "Could not find normal IINC " + var + " 128 Insn. Checking for alternate."
            );
            ain = ReikaASMHelper.getLastInsn(m.instructions, Opcodes.SIPUSH, 128);
            start = ain.getPrevious();
            end = ReikaASMHelper.getFirstOpcodeAfter(
                m.instructions, m.instructions.indexOf(ain), Opcodes.ISTORE
            );
        } else {
            start = ain;
            end = start;
