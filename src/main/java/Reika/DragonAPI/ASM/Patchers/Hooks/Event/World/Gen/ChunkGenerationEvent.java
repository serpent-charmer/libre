...
public class ChunkGenerationEvent extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        MethodNode m = ReikaASMHelper.getMethodByName(
            cn, "originalLoadChunk", "(II)Lnet/minecraft/world/chunk/Chunk;"
        );
        int var = 5;
        if (CoreModDetection.BUKKIT.isInstalled())
            var = ((VarInsnNode) m.instructions.getLast().getPrevious().getPrevious())
                      .var;
        AbstractInsnNode ain
            = ReikaASMHelper.getLastInsn(m.instructions, Opcodes.ASTORE, var);
        InsnList li = new InsnList();
        li.add(new VarInsnNode(Opcodes.ALOAD, 5));
        li.add(new MethodInsnNode(
            Opcodes.INVOKESTATIC,
            "Reika/DragonAPI/Instantiable/Event/ChunkGenerationEvent",
            "fire",
