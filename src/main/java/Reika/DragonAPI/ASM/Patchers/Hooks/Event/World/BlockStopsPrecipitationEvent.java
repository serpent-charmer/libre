...
public class BlockStopsPrecipitationEvent extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        MethodNode m = ReikaASMHelper.getMethodByName(
            cn, "func_76626_d", "getPrecipitationHeight", "(II)I"
        );
        AbstractInsnNode ain
            = ReikaASMHelper.getFirstInsnAfter(m.instructions, 0, Opcodes.ALOAD, 7);
        AbstractInsnNode next = ReikaASMHelper.getFirstOpcodeAfter(
            m.instructions, m.instructions.indexOf(ain), Opcodes.INVOKEVIRTUAL
        );
        next = ReikaASMHelper.getFirstOpcodeAfter(
            m.instructions, m.instructions.indexOf(next) + 1, Opcodes.INVOKEVIRTUAL
        );
        ...
        li.add(new MethodInsnNode(
            ...
            "Reika/DragonAPI/Instantiable/Event/BlockStopsPrecipitationEvent",
            "fire",
            "(Lnet/minecraft/world/chunk/Chunk;Lnet/minecraft/block/Block;III)Z",
            false
        ));
