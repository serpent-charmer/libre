...
public class LightVisualBrightness extends BlockRenderBrightness {
    ...
    protected void apply(ClassNode cn) {
        MethodNode m = ReikaASMHelper.getMethodByName(
            cn, "func_72801_o", "getLightBrightness", "(III)F"
        );
        /*
        AbstractInsnNode ain = ReikaASMHelper.getLastOpcode(m.instructions,
        Opcodes.FRETURN); InsnList li = new InsnList(); li.add(new
        VarInsnNode(Opcodes.ALOAD, 0)); li.add(new VarInsnNode(Opcodes.ILOAD, 1));
        ...
        "Reika/DragonAPI/Instantiable/Event/LightVisualBrightnessEvent", "fire",
        "(FLnet/minecraft/world/World;III)F", false)); m.instructions.insertBefore(ain,
        li);
         */
        MethodInsnNode min = (MethodInsnNode
        ) ReikaASMHelper.getLastOpcode(m.instructions, Opcodes.INVOKEVIRTUAL);
        min.owner
            = "Reika/DragonAPI/Instantiable/Event/Client/LightVisualBrightnessEvent";
        min.name = "fire";
