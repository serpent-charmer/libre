...
public abstract class BlockSpreadingEvent extends Patcher {
    ...
    protected final void apply(ClassNode cn) {
        ...
        /*
        AbstractInsnNode ref = ReikaASMHelper.getLastOpcode(m.instructions,
        Opcodes.INVOKEVIRTUAL); AbstractInsnNode first = ref;//.getPrevious();//keep the
        block arg AbstractInsnNode last = ref.getNext(); ref =
        ref.getPrevious();//.getPrevious();

        ReikaASMHelper.deleteFrom(m.instructions, first, last);

        m.instructions.insert(ref, new MethodInsnNode(Opcodes.INVOKESTATIC,
        "Reika/DragonAPI/Instantiable/Event/BlockSpreadEvent", "fire",
        ...
        m.instructions.add(new MethodInsnNode(
            ...
            "Reika/DragonAPI/Instantiable/Event/BlockSpreadEvent",
            "fire",
            "(Lnet/minecraft/world/World;IIILjava/util/Random;Lnet/minecraft/block/Block;)V",
            false
        ));
