...
public class SnowRainRenderChoice extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        MethodNode m = ReikaASMHelper.getMethodByName(
            cn, "func_78474_d", "renderRainSnow", "(F)V"
        );
        String seek
            = FMLForgePlugin.RUNTIME_DEOBF ? "func_76939_a" : "getTemperatureAtHeight";
        MethodInsnNode min = ReikaASMHelper.getFirstMethodCallByName(cn, m, seek);
        VarInsnNode world = (VarInsnNode) ReikaASMHelper.getLastOpcodeBefore(
            m.instructions, m.instructions.indexOf(min), Opcodes.ALOAD
        );
        VarInsnNode height = (VarInsnNode) min.getPrevious();
        VarInsnNode temp = (VarInsnNode) height.getPrevious();
        AbstractInsnNode ref = ReikaASMHelper.getLastOpcodeBefore(
            m.instructions, m.instructions.indexOf(min), Opcodes.FSTORE
        );
        VarInsnNode z = (VarInsnNode) ReikaASMHelper.getLastOpcodeBefore(
            m.instructions, m.instructions.indexOf(ref), Opcodes.ILOAD
        );
        ...
        li.add(new MethodInsnNode(
            ...
            "Reika/DragonAPI/ASM/ASMCallsClient",
            "shouldRenderRainInsteadOfSnow",
            "(Lnet/minecraft/client/multiplayer/WorldClient;Lnet/minecraft/world/biome/BiomeGenBase;IIIFI)Z",
            false
        ));

        JumpInsnNode jump = (JumpInsnNode) ReikaASMHelper.getFirstOpcodeAfter(
            m.instructions, m.instructions.indexOf(min), Opcodes.IFLT
        );
