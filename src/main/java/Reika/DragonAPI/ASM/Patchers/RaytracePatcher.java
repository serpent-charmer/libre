...
public abstract class RaytracePatcher extends Patcher {
    ...
    protected final void apply(ClassNode cn) {
        /*
        MethodNode m = ReikaASMHelper.getMethodByName(cn, "func_70071_h_", "onUpdate",
        "()V");

        InsnList li = new InsnList();
        li.add(new VarInsnNode(Opcodes.ALOAD, 0));
        li.add(new MethodInsnNode(Opcodes.INVOKESTATIC,
        "Reika/DragonAPI/Instantiable/Event/EntityAboutToRayTraceEvent", "fire",
        "(Lnet/minecraft/entity/Entity;)V", false));

        String func1 = FMLForgePlugin.RUNTIME_DEOBF ? "func_149668_a" :
        "getCollisionBoundingBoxFromPool"; String func2 = FMLForgePlugin.RUNTIME_DEOBF ?
        "func_72933_a" : "rayTraceBlocks"; String func3 = FMLForgePlugin.RUNTIME_DEOBF ?
        "func_147447_a" : "func_147447_a";
        ...
        try {
            min1 = ReikaASMHelper.getFirstMethodCall(cn, m, "net/minecraft/block/Block",
        func1, "(Lnet/minecraft/world/World;III)Lnet/minecraft/util/AxisAlignedBB;");
        }
        catch (NoSuchASMMethodInstructionException e) {

        }
        try {
            min2 = ReikaASMHelper.getFirstMethodCall(cn, m, "net/minecraft/world/World",
        func2,
        ...
        try {
            min3 = ReikaASMHelper.getFirstMethodCall(cn, m, "net/minecraft/world/World",
        func3,
        ...
        AbstractInsnNode pre1 = min1 != null ?
        ReikaASMHelper.getLastNonZeroALOADBefore(m.instructions,
        m.instructions.indexOf(min1)) : null; AbstractInsnNode pre2 = min2 != null ?
        ReikaASMHelper.getLastFieldRefBefore(m.instructions, m.instructions.indexOf(min2),
        world).getPrevious() : null; AbstractInsnNode pre3 = min3 != null ?
        ReikaASMHelper.getLastFieldRefBefore(m.instructions, m.instructions.indexOf(min3),
        world).getPrevious() : null;
        ...
        if (min1 != null && pre1 == null) {
            ReikaASMHelper.log("WARNING: Have injectable raytrace code but no landmark
        1!");
        }
        ...
        MethodNode m
            = ReikaASMHelper.getMethodByName(cn, "func_70071_h_", "onUpdate", "()V");
        ...
        for (int i = 0; i < m.instructions.size(); i++) {
            ...
            if (ain.getOpcode() == Opcodes.INVOKEVIRTUAL) {
                ...
                if (min.name.equals(func1)) {
                    VarInsnNode pre = (VarInsnNode
                    ) ReikaASMHelper.getLastNonZeroALOADBefore(m.instructions, i);
                    //m.instructions.remove(pre);
                    pre.var = 0;
                    min.owner
                        = "Reika/DragonAPI/Instantiable/Event/EntityCollisionEvents";
                    ReikaASMHelper.addLeadingArgument(
                        min, "Lnet/minecraft/entity/Entity;"
                    );
