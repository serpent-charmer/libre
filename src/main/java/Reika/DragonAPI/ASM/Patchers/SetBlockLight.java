...
public class SetBlockLight extends Patcher {
    ...
    protected void apply(ClassNode cn) {
        if (CoreModDetection.FASTCRAFT.isInstalled()) {
            ReikaASMHelper.log(
                "Skipping " + this + " ASM handler 1; not compatible with FastCraft"
            );
        } else {
            MethodNode m = ReikaASMHelper.getMethodByName(
                cn, "func_147465_d", "setBlock", "(IIILnet/minecraft/block/Block;II)Z"
            );
            //ReikaJavaLibrary.pConsole(ReikaASMHelper.clearString(m.instructions));
            String cl
                = /*CoreModDetection.fastCraftInstalled() ? "fastcraft/J" : */ cn.name;
            String func
                = /*CoreModDetection.fastCraftInstalled() ? "d" : */ "func_147451_t";
            String sig = /*CoreModDetection.fastCraftInstalled() ?
            ...
            if (CoreModDetection.BUKKIT.isInstalled())
                m.instructions.remove(
                    min.getPrevious().getPrevious().getPrevious().getPrevious()
                );
        ...
        MethodNode m = ReikaASMHelper.getMethodByName(
            cn, "func_147451_t", "func_147451_t", "(III)Z"
        );
