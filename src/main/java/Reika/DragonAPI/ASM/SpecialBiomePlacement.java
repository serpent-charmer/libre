...
public class SpecialBiomePlacement implements IClassTransformer {
    ...
    public SpecialBiomePlacement() {
        relay.put(
            FMLForgePlugin.RUNTIME_DEOBF ? "field_150608_ab" : "mesaPlateau",
            "getBiome_Hot"
        );
        relay.put(
            FMLForgePlugin.RUNTIME_DEOBF ? "field_150607_aa" : "mesaPlateau_F",
            "getBiome_Hot2"
        );
        relay.put(
            FMLForgePlugin.RUNTIME_DEOBF ? "field_76782_w" : "jungle", "getBiome_Warm"
        );
        relay.put(
            FMLForgePlugin.RUNTIME_DEOBF ? "field_150578_U" : "megaTaiga", "getBiome_Cool"
        );
        relay.put(
            FMLForgePlugin.RUNTIME_DEOBF ? "field_76789_p" : "mushroomIsland",
            "getBiome_Cold"
        );
    }
    ...
    public byte[] transform(String name, String transformedName, byte[] bytes) {
        ...
        if (CLASS.equals(cn.name) || CLASS.equals(cn.superName)) {
            ...
            MethodNode m = ReikaASMHelper.getMethodByName(
                cn, "func_75904_a", "getInts", "(IIII)[I"
            );
            ...
            for (int i = 0; i < m.instructions.size(); i++) {
                ...
                if (ain.getOpcode() == Opcodes.SIPUSH && l1_Var == -1) {
                    ...
                    if (iin.operand == FLAG) {
                        l1_Var = ((VarInsnNode) ReikaASMHelper.getFirstOpcodeAfter(
                                      m.instructions, i, Opcodes.ISTORE
                                  ))
                                     .var;
                    }
                    ...
                    if (flag) {
                        ...
                        if (li != null) {
                            ...
                            ReikaASMHelper.log(
                                "Replaced placement of " + fin.name + " with '"
                                + relay.get(fin.name) + "' hook"
                            );
                        }
                    } else {
                        flag = true;
    ...
    private InsnList createRelay(FieldInsnNode original, int l1) {
        ...
        li.add(new MethodInsnNode(
            ...
            "Reika/DragonAPI/Auxiliary/SpecialBiomePlacementRegistry",
            call,
            "(I)I",
            false
        ));
