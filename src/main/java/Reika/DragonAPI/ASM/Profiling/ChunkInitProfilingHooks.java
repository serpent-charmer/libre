...
public class ChunkInitProfilingHooks implements IClassTransformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] bytes) {
        ...
        if (classNode.name.startsWith("net/minecraft/world/gen/")
            && !classNode.name.equals("net/minecraft/world/gen/ChunkProviderServer")
            && classNode.interfaces.contains("net/minecraft/world/chunk/IChunkProvider"
            )) {
            ...
            try {
                ...
                ReikaASMHelper.log(
                    "Injected " + (c.size() + 1) + " profiling hooks into "
                    + classNode.name
                );
            } catch (NoSuchASMMethodException e) {
                ReikaASMHelper.log(
                    "Skipping profiling hooks on " + classNode.name
                    + "; does not contain provideChunk method"
                );
            }
    ...
    private void inject(MethodNode m, AbstractInsnNode ain, boolean isPre) {
        ...
        li.add(new MethodInsnNode(
            ...
            isPre ? "startChunkInit" : "finishChunkInit",
