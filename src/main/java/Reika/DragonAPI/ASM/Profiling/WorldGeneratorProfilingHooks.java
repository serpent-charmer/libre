...
public class WorldGeneratorProfilingHooks implements IClassTransformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] bytes) {
        ...
        if (superClasses.contains(cn.superName)
            && !cn.interfaces.contains(Subgenerator.class.getName().replace(".", "/"))) {
            ...
            try {
                ...
                if ((m.access & Modifier.ABSTRACT) != 0) {
                    ...
                    ReikaASMHelper.log(
                        "Injected " + (c.size() + 1) + " profiling hooks into " + cn.name
                    );
            ...
            if (!flag) {
                ReikaASMHelper.log(
                    "Skipping profiling hooks on " + cn.name
                    + "; does not contain generate method"
                );
                if (!superClasses.contains(cn.name)) {
                    ReikaASMHelper.log(
                        "This class should be added to the superClass generator parent list!"
                    );
                }
            }
    ...
    private void inject(ClassNode cn, MethodNode m, AbstractInsnNode ain, boolean isPre) {
        ...
        if (isPre) {
            ...
            ); //cache X and Z in case they are modified across generate()
        ...
        li.add(new MethodInsnNode(
            ...
            "Reika/DragonAPI/Auxiliary/Trackers/WorldgenProfiler",
            isPre ? "startGenerator" : "onRunGenerator",
            "(Lnet/minecraft/world/World;Lnet/minecraft/world/gen/feature/WorldGenerator;II)V",
            false
        ));
