...
public class BlockPosWrapper {
    ...
    ///may want to change by having a mutable blockpos and always use and ref that; faster
    ///than constant obj creation

    private static final HashMap<String, ImmutablePair<String, String>> classes
        = new HashMap();
    ...
    public static void injectBlockPosWrapper(ClassNode cn, MethodNode mn, int minVar) {
        ...
        li.add(new MethodInsnNode(
            Opcodes.INVOKESPECIAL, BLOCKPOS_CLASS, "<init>", BLOCKPOS_CONSTR, false
        ));
        li.add(relay);
