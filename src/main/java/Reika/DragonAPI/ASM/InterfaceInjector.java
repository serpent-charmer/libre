...
public class InterfaceInjector implements IClassTransformer {
                    ...
                    for (int x = 0; x < ann.values.size() - 1; x += 2) {
                        ...
                        if (key instanceof String && key.equals("value")) {
                            if (values instanceof List && !((List) values).isEmpty()
                                && ((List) values).get(0) instanceof String) {
    ...
    private void tryInjectInterfaces(ClassNode cn, Collection<String> c) {
        if (DEBUG)
            ReikaJavaLibrary.pConsole(
                "DRAGONAPI ASM: Injecting " + c.size() + " interfaces into " + cn.name
                + ": " + c
            );
        for (String cl : c) {
            ...
            if (inter == null) {
                if (DEBUG)
                    ReikaJavaLibrary.pConsole(
                        "DRAGONAPI ASM: Interface class " + cl
                        + " not found. Injection failed."
                    );
            } else {
    ...
    private void tryInjectInterface(ClassNode cn, ClassNode inter) {
        ...
        for (MethodNode mn : inter.methods) {
            if (!ReikaASMHelper.classContainsMethod(cn, mn)
                && !ReikaASMHelper.checkIfClassInheritsMethod(cn, mn)) {
                missing.add(mn);
            }
        }
        if (!missing.isEmpty()) {
            ImproperImplementationException e
                = new ImproperImplementationException(cn, inter, missing);
            if (crashOnError)
                throw e;
            else {
                ReikaJavaLibrary.pConsole(
                    "DRAGONAPI ASM: Interface " + inter.name
                    + " could not be injected to " + cn.name
            ...
            if (DEBUG)
                ReikaJavaLibrary.pConsole(
                    "DRAGONAPI ASM: Interface class " + inter.name
                    + " successfully injected into " + cn.name + "."
                );
        }
    }
    ...
    private static class ImproperImplementationException extends ASMException {
        ...
        protected ImproperImplementationException(
            ClassNode cn, ClassNode interf, Collection<MethodNode> ms
        ) {
            super(cn);
            interface_ = interf;
            missingMethods = ms;
        ...
        public final String getMessage() {
            ...
            sb.append(
                node.name + " does not implement the following required methods:\n"
            );
            for (MethodNode m : missingMethods) {
            ...
            for (MethodNode m : node.methods) {
                ...
                if (missingMethodNames.contains(m.name)) {
                    sb.append(
                        " << Method matches a missing method name but not signature; this is likely the error."
                    );
                }
                sb.append("\n");
            }
