...
public class ASMCallsClient {
    public static void renderCursorStack(
        GuiContainer gui,
        ItemStack is,
        int x,
        int y,
        ...
        ir.renderItemOverlayIntoGUI(
            font, gui.mc.getTextureManager(), is, x, y - (drag == null ? 0 : 8), subtext
        );
        ir.zLevel = 0.0F;
    }

    public static IIcon getLiquidIconForRenderBlocks(
        RenderBlocks rb, Block b, int s, int meta, int x, int y, int z
    ) {
    ...
    public static void addRainParticlesAndSound(EntityRenderer er) {
        if (SpecialDayTracker.instance.loadXmasTextures())
            return;
        if (SpecialDayTracker.instance.getXmasWeatherStrength(
    ...
    public static boolean shouldRenderRainInsteadOfSnow(
        WorldClient world,
        BiomeGenBase biome,
        int x,
        int y,
        ...
        if (SpecialDayTracker.instance.getXmasWeatherStrength(world) >= 0.5)
            return false;
        return world.getWorldChunkManager().getTemperatureAtHeight(
                   biomeTemp, precipHeight
               )
            >= 0.15F;
    }
    ...
    public static void onRenderWorld(EntityRenderer er, float ptick, long systime) {
        ...
        ShaderRegistry.runShaderDomain(
            mc.getFramebuffer(),
            mc.displayWidth,
            mc.displayHeight,
