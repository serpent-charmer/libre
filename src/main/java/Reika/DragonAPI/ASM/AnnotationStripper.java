...
class AnnotationStripper {
    ...
    private static final boolean printClass
        = ReikaJVMParser.isArgumentPresent("-DragonAPI_printStripped");
    ...
    static byte[] parse(String name, String transformedName, byte[] bytes) {
        ...
        //ReikaJavaLibrary.pConsole(name+" // "+transformedName+":
        //:"+strippables.contains(name));
        if (doStrip(name)) {
            ...
            if (strip(cn)) {
                ...
                if (printClass) {
                    ReikaJavaLibrary.printClassSource(
                        "StrippedClasses/" + name.replace('.', '/'), bytes
                    );
                }
            }
        }
    ...
    static boolean strip(ClassNode cn) {
        //ReikaJavaLibrary.pConsole("Class: "+cn.name+" has
        //annotations"+ReikaASMHelper.clearAnnotations(cn.visibleAnnotations)+";
        //"+ReikaASMHelper.clearAnnotations(cn.invisibleAnnotations)+";
        //"+ReikaASMHelper.clearTypeAnnotations(cn.visibleTypeAnnotations)+";
        //"+ReikaASMHelper.clearTypeAnnotations(cn.invisibleTypeAnnotations));
        ...
        if (cn.visibleAnnotations != null) {
            for (AnnotationNode n : cn.visibleAnnotations) {
                ...
                if (node != null) {
                    ...
                    //ReikaJavaLibrary.pConsole("entry 0 for "+cn.name+";
                                ...
                                if (!classExists(clazz)) {
                                    ...
                                    if (DEBUG) {
                                        ReikaJavaLibrary.pConsole(
                                            "Removing interface " + cz + " from "
                                            + cn.name + "; class not present."
                                        );
                                    }
                                }
                            }
        ...
        if (altered) {
            if (DEBUG) {
                ReikaJavaLibrary.pConsole(
                    "Remaining interfaces on " + cn.name + ": " + cn.interfaces
                );
            }
        }
        return altered;
