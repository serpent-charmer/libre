...
public class ReikaRailCraftHelper extends DragonAPICore {
    ...
    /**
     * Get the energy liberated by the conversion of one block of steam to one bucket of
     * water.
     */
    public static double getSteamBoilingEnergy(int mB) {
        return ReikaThermoHelper.WATER_BOIL_ENTHALPY * mB
            * 1000; //2260 kJ/kg * 1000 kg * 1000 J/kJ
    }
    ...
    public static double getSteamBucketEnergyToHeat(int Tinit, int mB) {
        ...
        if (dT < 0)
            dT = 0;
        return ReikaThermoHelper.WATER_HEAT * mB * 1000
            * dT; //4.18 kJ/kgK * 1000 kg * 1000 J/kJ * dT K
    }
        ...
        public void load(TileEntity te) {
            if (System.identityHashCode(te) != tileID)
                throw new MisuseException(
                    "You cannot reuse a FireboxWrapper instance for different TileEntities!"
                );
            try {
