...
public abstract class LuaMethod {
    ...
    public LuaMethod(String name, Class requiredParent) {
        ...
        if (methods.containsKey(mk))
            throw new IllegalArgumentException(
                "This method is a duplicate of one that already exists!"
            );
        else
            methods.put(mk, this);
    }
    ...
    public static Object[] invokeCC(LuaMethod m, TileEntity te, Object[] args)
    ...
    public static Object[] invokeOC(LuaMethod m, TileEntity te, Object[] args)
    ...
    protected abstract Object[] invoke(TileEntity te, Object[] args)
    ...
    public final boolean equals(Object o) {
        if (o instanceof LuaMethod) {
            return ((LuaMethod) o).displayName.equals(displayName)
                && requiredClass == ((LuaMethod) o).requiredClass;
        } else
            return false;
    }
    ...
    public final String toString() {
        String name
            = requiredClass != null ? requiredClass.getSimpleName() : "Any TileEntity";
    ...
    public static void registerMethods(String folder) {
        try {
            for (Class c : ReikaJavaLibrary.getAllClassesFromPackage(
                     folder, LuaMethod.class, true, true
                 )) {
                if (c.isAnnotationPresent(ModTileDependent.class)) {
                    String[] vals
                        = ((ModTileDependent) c.getAnnotation(ModTileDependent.class))
                              .value();
                    for (String s : vals) {
                ...
                if (c.isAnnotationPresent(ModDependentMethod.class)) {
                    ModList mod
                        = ((ModDependentMethod) c.getAnnotation(ModDependentMethod.class))
                              .value();
                    if (!mod.isLoaded()) {
                        continue;
                    }
