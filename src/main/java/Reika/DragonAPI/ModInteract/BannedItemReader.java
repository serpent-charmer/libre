...
public final class BannedItemReader {
    ...
    public static final String PLUGIN_PATH
        = System.getProperty("user.dir").replaceAll("\\\\", "/") + "/plugins/";
    ...
    private KeyedItemStack createKey(ItemStack is) {
        return new KeyedItemStack(is).setSimpleHash(true).setIgnoreNBT(true).setSized(
            false
        );
    }
    ...
    private void parseDirectory(File f) throws Exception {
        ArrayList<File> li
            = ReikaFileReader.getAllFilesInFolder(f, "yml", "txt", "dat", "cfg", "json");
        for (File in : li) {
            this.parseFile(in);
        }
    }
                    ...
                    if (parts != null && parts.length > 1) {
                        try {
                            ...
                            try {
                                ...
                                int intmeta
                                    = meta.equals("*") ? -1 : Integer.parseInt(meta);
                                Item item = Item.getItemById(intid);
                                allEntries.add(
                                    intmeta >= 0
                                        ? this.createKey(new ItemStack(item, intmeta))
                                        : this.createKey(new ItemStack(item))
                                );
                                flag = true;
    ...
    private void parseJSONFile(File f) {
        ...
        if (e instanceof JsonObject) {
            ...
            if (j.getAsJsonPrimitive("enable").getAsBoolean()) {
                ...
                try {
                    WarpPoint p = new WarpPoint(id, new
            WorldLocation(dims.get(0).getAsInt(), x, y, z)); map.add(p);
                }
                catch (Exception ex) {
                    ChromatiCraft.logger.logError("Could not parse waypoint entry:
            "+e.toString()); ex.printStackTrace();
                }
            }*/
        }
    }
