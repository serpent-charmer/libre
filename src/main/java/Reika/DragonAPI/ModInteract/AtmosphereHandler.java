...
public class AtmosphereHandler {
        ...
        try {
            ...
            if (o == null) {
                DragonAPICore.logError(
                    "World #" + world.provider.dimensionId + " / " + world + " / "
                    + world.provider + " had a null atmo handler!"
                );
                return null;
            }
    ...
    /**
     * Note: Advanced Rocketry does not seal the actual blocks, only the areas bounded by
     * them, so give not tiles' location but at the input or output location
     */
    public static boolean
    isNoAtmo(World world, int x, int y, int z, Block b, boolean needsO2) {
        if (PlanetDimensionHandler.isAdvRWorld(world)) {
            ...
            if (needsO2) {
                if (OxygenHooks.noAtmosphericCombustion(world.provider)) {
                    if (!(OxygenHooks.inOxygenBubble(world, x + 0.5, y + 0.5, z + 0.5)
                          && OxygenHooks.checkTorchHasOxygen(world, b, x, y, z)))
                        return true;
                }
            } else {
                IGalacticraftWorldProvider igw
    ...
    static {
        if (ModList.ADVROCKET.isLoaded()) {
            try {
                ...
                get = handler.getDeclaredMethod(
                    "getAtmosphereType", int.class, int.class, int.class
                );
