...
public class SmelteryRecipeHandler {
    ...
    public static void
    addIngotMelting(ItemStack ingot, ItemStack render, int temp, String fluid) {
        addMelting(ingot, render, temp, INGOT_AMOUNT, fluid);
    }

    public static void
    addIngotMelting(ItemStack ingot, ItemStack render, int temp, Fluid fluid) {
        addMelting(ingot, render, temp, INGOT_AMOUNT, fluid);
    }

    public static void
    addMelting(ItemStack is, ItemStack render, int temp, int fluidAmount, String fluid) {
        addMelting(is, render, temp, fluidAmount, FluidRegistry.getFluid(fluid));
    }

    public static void
    addMelting(ItemStack is, ItemStack render, int temp, int fluidAmount, Fluid fluid) {
        if (fluid == null)
    ...
    public static void
    addMelting(ItemStack is, ItemStack render, int temp, FluidStack fluid) {
        ...
        try {
            addMelting.invoke(
                smelteryInstance, is, b, render.getItemDamage(), temp, fluid
            );
            DragonAPICore.log(
                "Adding smeltery melting of " + is + " into " + fluidToString(fluid) + "."
            );
        } catch (Exception e) {
            DragonAPICore.logError(
                "Could not add Smeltery Recipe for " + is + " to " + fluidToString(fluid)
                + " @ " + temp + "!"
            );
            e.printStackTrace();
        }
    ...
    public static void addIngotCasting(ItemStack out, String fluid, int delay) {
        addCasting(
            TinkerToolHandler.getInstance().getIngotCast(),
            out,
    ...
    public static void
    addCasting(ItemStack cast, ItemStack out, String fluid, int fluidAmount, int delay) {
        addCasting(cast, out, FluidRegistry.getFluid(fluid), fluidAmount, delay);
    }

    public static void
    addCasting(ItemStack cast, ItemStack out, Fluid fluid, int fluidAmount, int delay) {
        if (fluid == null)
    ...
    public static void
    addCasting(ItemStack cast, ItemStack out, FluidStack in, int delay) {
        addCasting(cast, out, in, delay, true);
    }

    public static void addCasting(
        ItemStack cast, ItemStack out, FluidStack in, int delay, boolean addCastRecipe
    ) {
        if (!isLoaded)
            return;
        try {
            ...
            DragonAPICore.log(
                "Adding casting of " + fluidToString(in) + " to " + out + " with " + cast
                + "."
            );
        } catch (Exception e) {
            DragonAPICore.logError(
                "Could not add Casting Recipe for " + fluidToString(in) + " to " + out
                + " with " + cast + "!"
            );
            e.printStackTrace();
        }
    ...
    public static void addReversibleCasting(
        ItemStack cast,
        ItemStack out,
        ItemStack render,
        int temp,
    ...
    public static void
    addBlockCasting(ItemStack block, int fluidAmount, String fluid, int delay) {
        addBlockCasting(block, fluidAmount, FluidRegistry.getFluid(fluid), delay);
    }

    public static void
    addBlockCasting(ItemStack block, int fluidAmount, Fluid fluid, int delay) {
        if (fluid == null)
    ...
    public static void addBlockCasting(ItemStack block, FluidStack fluid, int delay) {
        ...
        try {
            ...
            DragonAPICore.log(
                "Adding block casting of " + fluidToString(fluid) + " to " + block + "."
            );
        } catch (Exception e) {
            DragonAPICore.logError(
                "Could not add Block Casting Recipe for " + fluidToString(fluid) + " to "
                + block + "!"
            );
            e.printStackTrace();
        }
    ...
    public static void addAlloying(FluidStack out, FluidStack... in) {
        ...
        try {
            ...
            DragonAPICore.log(
                "Adding alloying of " + Arrays.toString(in) + " to " + fluidToString(out)
                + "."
            );
        } catch (Exception e) {
            DragonAPICore.logError(
                "Could not add alloying of " + Arrays.toString(in) + " to "
                + fluidToString(out) + "."
            );
            e.printStackTrace();
        }
    ...
    public static ItemStack getRecipeOutput(Object recipe) {
        ...
        if (recipe == null || recipe.getClass() != castingRecipe)
            throw new MisuseException(
                "You cannot get the output for a null or non-recipe!"
            );
        try {
    ...
    static {
        if (ModList.TINKERER.isLoaded() && Pulses.SMELTERY.isLoaded()) {
            try {
                ...
                addAlloying = smeltery.getMethod(
                    "addAlloyMixing", FluidStack.class, FluidStack[].class
                );

                Class casting
                    = Class.forName("tconstruct.library.crafting.LiquidCasting");
                addCasting = casting.getMethod(
                    "addCastingRecipe",
                    ItemStack.class,
                    FluidStack.class,
                    ItemStack.class,
                ...
                addBlockCasting = casting.getMethod(
                    "addCastingRecipe", ItemStack.class, FluidStack.class, int.class
                );
                getAllRecipes = casting.getMethod("getCastingRecipes");

                castingRecipe
                    = Class.forName("tconstruct.library.crafting.CastingRecipe");
