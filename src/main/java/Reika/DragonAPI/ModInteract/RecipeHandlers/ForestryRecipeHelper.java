        ...
        if (this.hasMod()) {
            if (!Loader.instance().hasReachedState(LoaderState.POSTINITIALIZATION))
                throw new MisuseException(
                    "You cannot load other mod's machine recipes before postload!"
                );
            try {
                Collection<ICentrifugeRecipe> c
                    = RecipeManagers.centrifugeManager.recipes();
                for (ICentrifugeRecipe r : c) {
                    ...
                    for (ItemStack is : out.keySet()) {
                        ...
                        for (int i = 0; i < is.stackSize; i++)
                            outputs.addItem(
                                ReikaItemHelper.getSizedItemStack(is, 1), chance
                            );
                    }
                ...
                for (ISqueezerRecipe in : RecipeManagers.squeezerManager.recipes()) {
                    ...
                    if (items.length == 1
                        && !FluidContainerRegistry.isFilledContainer(items[0])) {
                ...
                addFermenter = util.getDeclaredMethod(
                    "addFermenterRecipes", ItemStack.class, int.class, fluids
                );
            } catch (Exception e) {
            ...
            try {
                ...
                boolean p6 =
            SemanticVersionParser.isVersionAtLeast(this.getMod().getVersion(), "3.6");
                ...
                for (Object r : li) {
                    ...
                    for (ItemStack item : out.keySet()) {
                        ...
                        outputs.addItem(item, p6 ? chance.floatValue()*100 :
            chance.intValue()); //he changed the %/1 thing again T_T
                    }
                    outputs.lock();
                    centrifuge.put(in, outputs);
                }
                ...
                ReflectiveFailureTracker.instance.logModReflectiveFailure(ModList.FORESTRY,
            e); DragonAPICore.logError(this.getMod()+" class not found! "+e.getMessage());
                e.printStackTrace();
            }
            catch (ClassCastException e) {
                ReflectiveFailureTracker.instance.logModReflectiveFailure(ModList.FORESTRY,
            e); DragonAPICore.logError(this.getMod()+" classcast! "+e.getMessage());
                e.printStackTrace();
            }
            catch (NoSuchFieldException e) {
                ReflectiveFailureTracker.instance.logModReflectiveFailure(ModList.FORESTRY,
            e); DragonAPICore.logError(this.getMod()+" field not found! "+e.getMessage());
                e.printStackTrace();
            }
            catch (SecurityException e) {
                ReflectiveFailureTracker.instance.logModReflectiveFailure(ModList.FORESTRY,
            e); DragonAPICore.logError("Cannot read "+this.getMod()+" (Security
            Exception)! "+e.getMessage()); e.printStackTrace();
            }
            catch (IllegalArgumentException e) {
                ReflectiveFailureTracker.instance.logModReflectiveFailure(ModList.FORESTRY,
            e); DragonAPICore.logError("Illegal argument for reading "+this.getMod()+"!");
                e.printStackTrace();
            }
            catch (IllegalAccessException e) {
                ReflectiveFailureTracker.instance.logModReflectiveFailure(ModList.FORESTRY,
            e); DragonAPICore.logError("Illegal access exception for reading
            "+this.getMod()+"!"); e.printStackTrace();
            }
            catch (NullPointerException e) {
                ReflectiveFailureTracker.instance.logModReflectiveFailure(ModList.FORESTRY,
            e); DragonAPICore.logError("Null pointer exception for reading
