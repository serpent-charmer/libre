...
public class EnderIOFacadeHandler {
    ...
    private EnderIOFacadeHandler() {
        if (ModList.ENDERIO.isLoaded()) {
            try {
                ...
                paintConduit = c.getMethod(
                    "setSourceBlock", ItemStack.class, Block.class, int.class
                );
                ...
                getPaintConduitMeta
                    = c.getMethod("getSourceBlockMetadata", ItemStack.class);
                getPaintConduitMeta.setAccessible(true);

                conduitTileClass
                    = Class.forName("crazypants.enderio.conduit.TileConduitBundle");
    ...
    public Block getFacade(TileEntity te) {
        try {
            return te != null && conduitTileClass != null
                    && conduitTileClass.isAssignableFrom(te.getClass())
                ? (Block) facadeBlock.get(te)
                : null;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
