...
/**
 * Used to make NEI respect the result Item.getFontRenderer() when rendering tooltips,
 * something it normally does not do.
 */
public class NEIFontRendererHandler implements IContainerTooltipHandler {
    ...
    private class LineHandler implements ITooltipLineHandler {
        ...
        public Dimension getSize() {
            return new Dimension(
                ...
                string.endsWith(GuiDraw.TOOLTIP_LINESPACE) ? 12 : 10
            );
        }
    ...
    private List<String>
    interceptValuesAndRegisterHandlers(ItemStack itemstack, List<String> currenttip) {
        if (itemstack == null)
            return currenttip;
        item = itemstack;
        ...
        for (int i = 0; i < currenttip.size(); i++) {
            ...
            String pre
                = GuiDraw.TOOLTIP_HANDLER + GuiDraw.getTipLineId(new LineHandler(s));
            strings.add(pre);
        }
        return strings;
    }
