...
public class PlanetDimensionHandler {
    ...
    public static boolean isGalacticWorld(World world) {
        return ModList.GALACTICRAFT.isLoaded()
            && InterfaceCache.IGALACTICWORLD.instanceOf(world.provider);
    }

    //@ModDependent(ModList.ADVROCKET)
    public static boolean isAdvRWorld(World world) {
        return ModList.ADVROCKET.isLoaded()
            && (InterfaceCache.IPLANETWORLD.instanceOf(world.provider)
                || (advRDimensionData != null
                    && advRDimensionData.keySet().contains(world.provider.dimensionId)));
    }
    ...
    public static double getSunIntensity(World world) {
        if (isGalacticWorld(world)) {
            ...
            IDimensionProperties dim
                = AdvancedRocketryAPI.dimensionManager.getDimensionProperties(
