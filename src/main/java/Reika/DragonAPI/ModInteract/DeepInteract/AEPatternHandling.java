...
public class AEPatternHandling {
    public static boolean isCraftingRecipe(ItemStack is, World world) {
        ICraftingPatternDetails icpd
            = ((ICraftingPatternItem) is.getItem()).getPatternForItem(is, world);
    ...
    public static ItemStack getEncodedPattern(
        ItemStack out, boolean isCrafting, boolean allowOreSubs, ItemStack... in
    ) {
        return getEncodedPattern(
            ReikaJavaLibrary.makeListFrom(out), isCrafting, allowOreSubs, in
        );
    }

    public static ItemStack getEncodedPattern(
        Collection<ItemStack> out,
        boolean isCrafting,
        boolean allowOreSubs,
        ItemStack... in
    ) {
