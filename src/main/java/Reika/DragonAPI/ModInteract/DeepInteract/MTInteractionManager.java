...
/**
 * Used to prevent Minetweaker from interfering with the balance of a mod; mods can
 * register items whose recipes cannot be removed or items which cannot have additional
 * recipes added. It does not allow for the prevention of creating recipes that <i>use</i>
 * the items, as there is no harm that can result from such use. <br><br> <b>--Message
 * from Reika--</b> <br> As I can already imagine the complaints of some 500 upset server
 * admins and mod authors, who will object to the whole purpose of this class, I pose the
 * following question: <br> Given that the rules that I have in place on some of my mods,
 * which disallows using Minetweaker or other tools to change the core recipes - with the
 * reason for that being the abuse I received day in and day out from people screwing
 * things up and blaming me back before I put such rules in place, <a
 * href="https://sites.google.com/site/reikasminecraft/modifications">as detailed on my
 * website</a> - and the fact that, as I have recently seen firsthand, some people take
 * rules as suggestions and make such changes anyway, what <i>exactly</i> is the problem
 * with the fact I am now enforcing the rules? If you wish to say that this implementation
 * causes problems, that there is a better way to enforce this, then I will by all means
 * listen, but if your argument is one of moralistic proclamations that I have no right to
 * enforce my own rules, no matter what, or that I am committing some offence by having
 * rules in the first place, then I will still listen...but for my own amusement. <br><br>
 * This class is <u>NOT</u> an attack on Minetweaker or its author StanHebben (who I must
 * say left me extremely impressed with the complexity, versatility, and sophistication of
 * MT's internal workings and the ZenScript system), nor do I have any animosity towards
 * them (and I hope they feel the same for me). I am in full agreement that MT is a very
 * useful tool, and one I have used myself on several occasions, with excellent results.
 * This class is purely to provide me - and anyone else using DragonAPI with a similar
 * need - a means to save myself from once again dealing with hordes of people who make
 * changes to a mod with a dim understanding at best of their effects, then chew out the
 * developer of said mod when their pack or server balance implodes due to
 * exploits/missing dependencies/endgame shortcuts.
 */
public final class MTInteractionManager {
    public static final MTInteractionManager instance = new MTInteractionManager();

    private final OneWayMap<Prevention, OneWayMap<KeyedItemStack, String>> data
        = new OneWayMap();
    ...
    private static Method getReloadMethod(Class api) {
        String[] names = {
            "reload",  "reloadScripts",  "reloadAllScripts",
            "rebuild", "rebuildScripts", "rebuildAllScripts",
        };
    ...
    public void
    blacklistNewRecipesFor(ItemStack is, boolean ignoreMetadata, boolean ignoreNBT) {
    ...
    public void
    blacklistRecipeRemovalFor(ItemStack is, boolean ignoreMetadata, boolean ignoreNBT) {
    ...
    public void
    blacklistOreDictTagsFor(ItemStack is, boolean ignoreMetadata, boolean ignoreNBT) {
        this.addEntry(Prevention.OREDICT, is, ignoreMetadata, ignoreNBT);
    }

    private void
    addEntry(Prevention p, ItemStack is, boolean ignoreMetadata, boolean ignoreNBT) {
        ...
        if (li.containsKey(ks)) {
            for (Map.Entry<KeyedItemStack, String> e : li.entrySet()) {
                ...
                if (key.equals(ks) && key.contains(ks)) {
                    DragonAPICore.log(
                        "Note: This item, '" + is.getDisplayName() + "' (" + ks
                        + "), is already listed, but under the wider tag " + key + "."
                    );
                    return;
                }
            }
    ...
    private static final class MTItemEntry {
        ...
        private KeyedItemStack asKeyStack() {
            ...
            if (item == null)
                return null;
            KeyedItemStack ks = new KeyedItemStack(new ItemStack(item, 1, meta))
                                    .setSimpleHash(true)
                                    .lock();
            return ks;
        }
    ...
    private static final class MTScriptScanner extends LineEditor {
        ...
        private final OneWayMap<String, OneWaySet<MTItemEntry>> variables
            = new OneWayMap();
        ...
        @Override
        protected String
        getReplacementLine(String s, String newline, int idx) { //comment, plus note
            if (phase2)
                return "//" + s;
            ...
            if (!s.isEmpty()) {
                try {
                    ...
                    DragonAPICore.logError(
                        "Error parsing line '" + this.getOriginalLine(idx) + "' in '"
                        + script.getName() + "':"
                    );
                    e.printStackTrace();
                    return false;
        ...
        private String parseOutCommentedSections(String s, int lineNum) {
            ...
            if (s.contains("/*")) {
                ...
                return partialLine != null
                    ? ""
                    : s; //not just 's', so it skips lines between /* and */
            }
        }
        ...
        private boolean parseLine(String s) {
            if (s.startsWith("//") || s.startsWith("##")) { //comment
                ...
                int eq = s.indexOf("=");
                if (eq < 0) {
                    //this.logError(s, "is a variable declaration that lacks a variable
                    //definition.");
                } else {
                    ...
                    if (lb >= 0 && rb >= 0) {
                        ...
                        //this.logError(s, "is a variable definition with no item
                        //specified.");
                    }
                }
                return false;
                ...
                if (period >= 0) {
                    return this.parseTruncLine(
                        s.substring(0, period - 1), s.substring(period + 1)
                    );
        ...
        private void logError(String s, String desc) {
            DragonAPICore.logError(
                "Note that an invalid line has been found in your MT script '"
                + script.getName() + "':"
            );
            DragonAPICore.logError(
                "The line '" + s + "' " + desc + " Consider fixing this."
            );
        }
