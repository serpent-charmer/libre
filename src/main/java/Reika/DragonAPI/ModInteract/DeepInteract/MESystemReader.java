...
public class MESystemReader implements IMEMonitorHandlerReceiver<IAEItemStack> {
    ...
    public MESystemReader addCallback(ItemStack is, ChangeCallback call) {
        changeCallbacks.addValue(
            new KeyedItemStack(is).setSimpleHash(true).setIgnoreNBT(true), call
        );
    ...
    private IMEMonitor<IAEItemStack> getStorage() {
        if (node == null || node.getGrid() == null)
            return null;
        return ((IStorageGrid) node.getGrid().getCache(IStorageGrid.class))
            .getItemInventory();
    }
    ...
    public static HashSet<ICellProvider> getAllCellContainers(IStorageGrid isg) {
        try {
            if (isg.getClass() == gridCache) {
                HashSet<ICellProvider> set
                    = (HashSet<ICellProvider>) activeCellProviders.get(isg);
                return set;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public long removeItem(ItemStack is, boolean simulate, boolean nbt) {
        ...
        if (!nbt) {
            ...
            for (IAEItemStack iae : c) {
                if ((most == null || iae.getStackSize() >= most.getStackSize())
                    && is.getItem() == iae.getItem()
                    && (is.getItemDamage() == iae.getItemDamage()
     ...
     * @param allowMultiple
     */
    public ExtractedItemGroup removeItemFuzzy(
        ItemStack is,
        boolean simulate,
        FuzzyMode fz,
        boolean oredict,
        ...
        for (IAEItemStack iae : c) {
            if ((oredict || is.getItem() == iae.getItem())
                && (!nbt
                    || ReikaNBTHelper.areNBTTagsEqual(
                        is.stackTagCompound,
                        (iae.hasTagCompound()
                             ? iae.getTagCompound().getNBTTagCompoundCopy()
                             : null)
                    ))) {
                li.add(iae.copy());
            }
    ...
    private Collection<IAEItemStack> getFuzzyItemList(ItemStack is, FuzzyMode fz) {
        IMEMonitor<IAEItemStack> mon = this.getStorage();
        IAEItemStack ae = this.createAEStack(is);
        Collection<IAEItemStack> c = new ArrayList(mon.getStorageList().findFuzzy(ae, fz)
        ); //wrap in list so we can add entries
        if (is.getItemDamage()
            == OreDictionary
                   .WILDCARD_VALUE) { //since AE does not handle this, a simple hack
    ...
    private ExtractedItem extract(IAEItemStack ae, boolean simulate) {
        IAEItemStack is = this.getStorage().extractItems(
            ae, simulate ? Actionable.SIMULATE : Actionable.MODULATE, actionSource
        );
    ...
    public long addItem(ItemStack is, boolean simulate) {
        IAEItemStack ret = this.getStorage().injectItems(
            this.createAEStack(is),
    ...
    public long getItemCount(ItemStack is, boolean nbt) {
        return this.removeItem(
            ReikaItemHelper.getSizedItemStack(is, Integer.MAX_VALUE), true, nbt
        );
    }

    public long getFuzzyItemCount(ItemStack is, FuzzyMode fz, boolean ore, boolean nbt) {
        ExtractedItemGroup ei = this.removeItemFuzzy(
            ReikaItemHelper.getSizedItemStack(is, Integer.MAX_VALUE),
            true,
            fz,
            ore,
            nbt,
    ...
    public void triggerFuzzyCrafting(
        World world,
        ItemStack is,
        long amt,
        ICraftingCallback callback,
        CraftCompleteCallback callback2
    ) {
        this.triggerCrafting(world, is, amt, callback, callback2);
    }

    /**
     * Triggers the native crafting system to craft a given amount of a given item.
    ...
    public void triggerCrafting(
        ...
        Future<ICraftingJob> f = this.getCraftingGrid().beginCraftingJob(
            world, node.getGrid(), actionSource, iae, callback
        );
        if (callback2 != null)
    ...
    /**
     * You are required to call this on your reader if you want things like crafting
     * triggers to work.
     */
    public void tick() {
        ...
        if (cache != null) {
            ...
            for (Future<ICraftingJob> f : crafting.keySet()) {
                if (f.isDone()) {
                    try {
                        ...
                        ICraftingLink l
                            = cache.submitJob(job, requester, null, true, actionSource);
                        if (l == null) {
                            DragonAPICore.logError(
                                job + " to craft " + job.getOutput()
                                + " returned a null link!"
                            );
                        } else {
            ...
            MultiMap<CraftCompleteCallback, ICraftingLink> removeLinks
                = new MultiMap().setNullEmpty();
    ...
    public void postChange(
        ...
        for (IAEItemStack iae : change) {
            KeyedItemStack ks = new KeyedItemStack(iae.getItemStack())
                                    .setSimpleHash(true)
                                    .setIgnoreNBT(true);
    ...
    public static Collection<IGrid> getAllMENetworks() {
        try {
            return (Collection<IGrid>) getNetworks.invoke(tickHandlerInstance
            ); //TickHandler.INSTANCE.getGridList();
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public static interface ChangeCallback {
        /**
         * May be null if a total AE list rebuild. Assume that means what you care about
         * changed.
         */
    ...
    private static class AEStack implements IAEItemStack, Comparable<AEStack> {
        ...
        public boolean sameOre(IAEItemStack is) {
            return OreDictionary.getOreID(item) ==
    OreDictionary.getOreID(is.getItemStack());
        }
    ...
    public static enum MatchMode {
        ...
        public ExtractedItemGroup removeItems(
            MESystemReader net, ItemStack is, boolean simulate, boolean allowMultiple
        ) {
            switch (this) {
                case EXACT: {
                    long amt = net.removeItem(is, simulate, true);
                    return amt != 0 ? new ExtractedItemGroup(new ExtractedItem(is, amt))
                                    : null;
                }
                case EXACTNONBT: {
                ...
                case FUZZY:
                    return net.removeItemFuzzy(
                        is, simulate, FuzzyMode.IGNORE_ALL, false, true, allowMultiple
                    );
                case FUZZYORE:
                    return net.removeItemFuzzy(
                        is, simulate, FuzzyMode.IGNORE_ALL, true, true, allowMultiple
                    );
                case FUZZYNBT:
                    return net.removeItemFuzzy(
                        is, simulate, FuzzyMode.IGNORE_ALL, true, false, allowMultiple
                    );
            }
            return null;
        }
    ...
    public static final class ExtractedItem {
        ...
        public ItemStack getItem() {
            return ReikaItemHelper.getSizedItemStack(
                item, (int) Math.min(Integer.MAX_VALUE, amount)
            );
        }
    }
    ...
    public static abstract class ItemInSystemEffect implements MESystemEffect {
        ...
        public final void performEffect(IGrid grid) {
            if (grid.isEmpty() || grid.getNodes().isEmpty()
                || !grid.getNodes().iterator().hasNext())
                return;
            ...
            if (iah.isEmpty()) {
                ...
                try {
                    ...
                    DragonAPICore.logError(
                        "Detected invalid ME system when running " + this + ": "
