...
public class TwilightForestLootHooks {
    ...
    public static enum DungeonTypes {
        ...
        private final EnumMap<LootLevels, Object> treasureTables
            = new EnumMap(LootLevels.class);
        ...
        public void addItem(ItemStack item, LootLevels level, int rarity) {
            try {
                ...
                DragonAPICore.log(
                    "Added " + item + " to TF loot table " + this + " with rarity "
                    + rarity
                );
            } catch (Exception e) {
                DragonAPICore.logError(
                    "Could not add loot to TF dungeon loot table " + this + "!"
                );
                e.printStackTrace();
            }
        }
    ...
    private static Object generateTreasureEntry(ItemStack item, int rarity)
        throws Exception {
            ...
            try {
                ...
                for (int i = 0; i < DungeonTypes.list.length; i++) {
                    ...
                    try {
                        ...
                        DragonAPICore.logError(
                            "Could not load TF dungeon loot table " + type + "!"
                        );
                        e.printStackTrace();
