...
public class TinkerMaterialHelper {
    ...
    ) { //Breaks tools cheated in with uncraftable parts
        if (Pulses.TOOLS.isLoaded()) {
            EntityPlayer ep = evt.getPlayer();
            ItemStack tool = ep.getCurrentEquippedItem();
            try {
                if (tool != null
                    && InterfaceCache.TINKERTOOL.instanceOf(tool.getItem())) {
                    ...
                    if (t != null) {
                        for (int i = 0; i < ToolPartType.types.length; i++) {
                            ...
                            ToolParts p
                                = TinkerToolHandler.getInstance().getPart(t, type);
                            if (p != null) {
                                int id = TinkerToolHandler.getInstance().getToolMaterial(
                                    tool, type
                                );
                                ...
                                if (mat instanceof CustomTinkerMaterial) {
                                    ...
                                    if (cm.enforceNoCheating
                                        && !cm.toolParts.contains(p
                                        )) { //forbidden part, had to be spawned in
    ...
    TinkerToolHandler.getInstance().getTool(out.getItem()); if (tool != null) { if
    (!mat.tools.contains(tool)) { evt.setResult(Result.DENY);
                }
            }
            else {
                TinkerToolHandler.Weapons weapon =
    TinkerToolHandler.getInstance().getWeapon(out.getItem()); if (weapon != null) { if
    (!mat.weapons.contains(weapon)) { evt.setResult(Result.DENY);
                    }
                }
            }
        }
    ...
    static {
        try {
            ...
            registerMaterial = c.getDeclaredMethod(
                "addToolMaterial",
                int.class,
                String.class,
                int.class,
            ...
            registerDefaultMaterial
                = c.getDeclaredMethod("addDefaultToolPartMaterial", int.class);
            registerChunkMaterial
                = c.getDeclaredMethod("addDefaultShardMaterial", int.class);

            registerPartMapping = c.getDeclaredMethod(
                "addPartMapping", Item.class, int.class, int.class, ItemStack.class
            );

            registerBowstringMaterial = c.getDeclaredMethod(
            ...
            registerBowMaterial = c.getDeclaredMethod(
                "addBowMaterial", int.class, int.class, float.class
            );
            registerArrowMaterial = c.getDeclaredMethod(
                "addArrowMaterial", int.class, float.class, float.class
            );

            instance.addNativeToolMaterial(
                0,
                "Wood",
                1,
                97,
            ...
            if (ModList.THAUMCRAFT.isLoaded())
                instance.addNativeToolMaterial(
                    31, "Thaumium", 3, 400, 700, 2, 1.3F, 0, 0.0F, "\u00a75", 5325692
                );
        } catch (Exception e) {
        ...
        try {
            ...
            registerTextures = c.getDeclaredMethod(
                "addMaterialRenderMapping",
                int.class,
                String.class,
                String.class,
                boolean.class
        ...
        try {
            ...
            registerRepairMaterial = c.getDeclaredMethod(
                "registerFullMaterial",
                ItemStack.class,
                int.class,
                String.class,
                ItemStack.class,
    ...
    public CustomTinkerMaterial createMaterial(int id, DragonAPIMod mod, String name) {
        ...
        if (prev != null)
            throw new IDConflictException(
                mod,
                "Tool material ID " + id + " is already occupied by '" + prev.materialName
                    + "'!"
            );
        ...
        mat.disableWeaponPart(WeaponParts.FLETCHING)
            .disableWeaponPart(WeaponParts.BOWSTRING); //technical
    ...
    private void addNativeToolMaterial(
        int id,
        String name,
        int harvestLevel,
        int dura,
        ...
        NativeTinkerMaterial mat = new NativeTinkerMaterial(
            id,
            name,
            harvestLevel,
            dura,
    ...
    public boolean isPartEnabled(int id, ToolParts p) {
        AbstractMaterial mat = materialIDs.get(id);
        return mat instanceof CustomTinkerMaterial
            ? ((CustomTinkerMaterial) mat).toolParts.contains(p)
            : true;
    }
    ...
    public boolean isPartEnabled(int id, WeaponParts p) {
        AbstractMaterial mat = materialIDs.get(id);
        return mat instanceof CustomTinkerMaterial
            ? ((CustomTinkerMaterial) mat).weaponParts.contains(p)
            : true;
    }
    ...
    public static class CustomTinkerMaterial extends AbstractMaterial {
        ...
        /**
         * Pickaxe levels - 0: Wood, 1: Stone, 2: Redstone/Diamond, 3: Obsidian, 4:
        ...
        //private final EnumSet<TinkerToolHandler.Tools> tools =
        //EnumSet.allOf(TinkerToolHandler.Tools.class); private final
        //EnumSet<TinkerToolHandler.Weapons> weapons =
        //EnumSet.allOf(TinkerToolHandler.Weapons.class);

        private final EnumSet<TinkerToolHandler.ToolParts> toolParts
            = EnumSet.allOf(TinkerToolHandler.ToolParts.class);
        private final EnumSet<TinkerToolHandler.WeaponParts> weaponParts
            = EnumSet.allOf(TinkerToolHandler.WeaponParts.class);
        ...
        public CustomTinkerMaterial
        registerTexture(String path, boolean prependToolType) {
            try {
                registerTextures.invoke(
                    null, id, mod.getDisplayName(), path, prependToolType
                );
            } catch (Exception e) {
                e.printStackTrace();
                ReflectiveFailureTracker.instance.logModReflectiveFailure(
        ...
        public CustomTinkerMaterial registerRepairMaterial(ItemStack input) {
            try {
                registerRepairMaterial.invoke(
                    patternBuilder,
                    input,
                    2,
                    materialName,
                    new ItemStack(toolShard, 1, id),
                    new ItemStack(toolRod, 1, id),
                    id
                );
            } catch (Exception e) {
                e.printStackTrace();
        ...
        public CustomTinkerMaterial registerPatternBuilder(ItemStack input) {
            try {
                for (ToolParts p : toolParts) {
                    registerPartMapping.invoke(
                        null,
                        TinkerToolHandler.getInstance().toolWoodPattern,
                        p.castMeta,
                        id,
                        p.getItem(id)
                    );
                ...
                for (WeaponParts p : weaponParts) {
                    registerPartMapping.invoke(
                        null,
                        TinkerToolHandler.getInstance().weaponWoodPattern,
                        p.castMeta,
                        id,
                        p.getItem(id)
                    );
        ...
        public CustomTinkerMaterial registerSmelteryCasting(
            ItemStack input, Fluid molten, int temp, ItemStack render
        ) {
            ...
            for (ToolParts p : toolParts) {
                ...
                SmelteryRecipeHandler.addReversibleCasting(
                    cast, item, render, temp, molten, (int) (base * p.ingotCost), 80
                );
            }
        ...
        public CustomTinkerMaterial registerWeapons(
            ItemStack input,
            int drawSpeed,
            float drawSpeedMult,
            float arrowMass,
            ...
            try {
                registerBowstringMaterial.invoke(
                    null,
                    1,
                    2,
                    input,
                    new ItemStack(bowstring, 1, 1),
                    durability,
                    drawSpeedMult,
                    flightSpeed,
                    renderColor
                ...
                registerArrowMaterial.invoke(
                    null, id, arrowMass, reinforced == 10 ? 0 : breakChance
                );
            } catch (Exception e) {
                e.printStackTrace();
                ReflectiveFailureTracker.instance.logModReflectiveFailure(
