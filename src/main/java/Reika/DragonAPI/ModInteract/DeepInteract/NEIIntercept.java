...
public class NEIIntercept implements IContainerInputHandler {
        ...
        if (ModList.WAILA.isLoaded()) {
            try {
                ...
                DragonAPICore.logError(
                    "Could not interface with WAILA recipe lookup: " + e
                );
    ...
    public boolean keyTyped(GuiContainer gui, char keyChar, int keyCode) {
        return gui instanceof KeyConsumingGui
            && ((KeyConsumingGui) gui).consumeKey(keyChar, keyCode);
    }
    ...
    public boolean lastKeyTyped(GuiContainer gui, char keyChar, int keyID) {
        return keyID == NEIClientConfig.getKeyBinding("gui.recipe")
            && !NEIClientConfig.isHidden()
            && MinecraftForge.EVENT_BUS.post(new NEIRecipeCheckEvent(gui));
    }
                        ...
                        if (NEIController.manager.window instanceof GuiRecipe
                            ...
                            if (button == 0) {
                                return MinecraftForge.EVENT_BUS.post(
                                    new NEIRecipeCheckEvent(gui, item)
                                );
                            }
                        }
                    }
    ...
    @Override
    public void
    onMouseDragged(GuiContainer gui, int mousex, int mousey, int button, long heldTime) {}
            ...
            if (wailaNEILookup != null && key == wailaNEILookup.getKeyCode()) {
                ...
                if (mov != null) {
                    ...
                    ArrayList<ItemStack> li
                        = b.getDrops(world, mov.blockX, mov.blockY, mov.blockZ, meta, 0);
                    if (li != null && !li.isEmpty()) {
                        is = li.get(0);
                    }
                    GuiContainer prevscreen = mc.currentScreen instanceof GuiContainer
