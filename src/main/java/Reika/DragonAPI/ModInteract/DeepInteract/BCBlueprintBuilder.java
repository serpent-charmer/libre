...
public class BCBlueprintBuilder {
    public static Blueprint convertBA(World world, BlockArray ba) {
        ...
        for (Coordinate c : ba.keySet()) {
            ...
            SchematicBlockBase s
                = SchematicRegistry.INSTANCE.createSchematicBlock(b, meta);
    ...
    public static Blueprint convertBA(FilledBlockArray ba) {
        ...
        for (Coordinate c : ba.keySet()) {
            ...
            SchematicBlockBase s = SchematicRegistry.INSTANCE.createSchematicBlock(
                bk.blockID, bk.hasMetadata() ? bk.metadata : 0
            );
            s.idsToWorld(bpt.getMappingRegistry());
            //s.initializeFromObjectAt(bpt, c.xCoord, c.yCoord, c.zCoord);
            s.storeRequirements(bpt, c.xCoord, c.yCoord, c.zCoord);
            if (s instanceof SchematicBlock) {
                ...
                sb.storedRequirements
                    = new ItemStack[] { new ItemStack(bk.blockID, 1, sb.meta) };
            }
            ret.put(
                c.xCoord - ba.getMinX(),
                c.yCoord - ba.getMinY(),
    ...
    /**
     * Returns the blueprint item referencing the blueprint; will not create a new entry
     * if that file already exists.
     */
    public static ItemStack
    writeBlueprintToDisk(Blueprint b, String name, String author) {
    ...
    public static void registerBlockForDirectPlacement(Block b, int meta) {
        SchematicRegistry.INSTANCE.registerSchematicBlock(
            b, meta, SchematicDirect.class, new BlockKey(b, meta)
        );
    }
    ...
    private static class SchematicDirect extends SchematicBlock {
        ...
        @Override
        public void getRequirementsForPlacement(
            IBuilderContext context, LinkedList<ItemStack> requirements
        ) {
