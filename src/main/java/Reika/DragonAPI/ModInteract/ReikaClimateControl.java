...
@Reparent(
    value = { "climateControl.api.BiomeSettings",
              "Reika.DragonAPI.ModInteract.DummyBiomeSettings" }
)
public class ReikaClimateControl extends BiomeSettings {
    ...
    private Object biomesFromConfig
        = climateControlCategory.booleanSetting(biomesOnName, "", false);
    private Object biomesInNewWorlds = climateControlCategory.booleanSetting(
        this.startBiomesName(configName), "Use biome in new worlds and dimensions", true
    );
    ...
    public ReikaClimateControl() {
        ...
        DragonAPICore.log(
            "Constructing updated ClimateControl config with " + biomes.size()
            + " entries: " + biomes
        );
    }
    ...
    private void loadConfig() {
        ...
        biomesInNewWorlds = climateControlCategory.booleanSetting(
            this.startBiomesName(configName),
            "Use biome in new worlds and dimensions",
            true
        );
    }

    public static void
    registerBiome(BiomeGenBase b, int incidence, boolean hasVillages, String climate) {
        biomes.add(
            new ElementDelegate(b.biomeName, b.biomeID, incidence, hasVillages, climate)
        );
        DragonAPICore.log(
            "Registering ClimateControl biome registration of "
            + biomes.get(biomes.size() - 1)
        );
    }
    ...
    private static class ElementDelegate {
        ...
        public String toString() {
            return name + " @ " + ID + " = " + incidence + " / " + hasVillages + " / "
                + climate;
        }
    }
}
