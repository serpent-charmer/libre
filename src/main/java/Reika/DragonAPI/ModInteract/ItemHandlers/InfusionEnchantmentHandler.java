...
public class InfusionEnchantmentHandler {
    public static final InfusionEnchantmentHandler instance
        = new InfusionEnchantmentHandler();
    ...
    private void loadThaumcraft() {
        this.registerEnchant(
            ModList.THAUMCRAFT, Enchantment.enchantmentsList[ThaumcraftApi.enchantFrugal]
        );
        this.registerEnchant(
            ModList.THAUMCRAFT, Enchantment.enchantmentsList[ThaumcraftApi.enchantHaste]
        );
        this.registerEnchant(
            ModList.THAUMCRAFT, Enchantment.enchantmentsList[ThaumcraftApi.enchantPotency]
        );
        this.registerEnchant(
            ModList.THAUMCRAFT, Enchantment.enchantmentsList[ThaumcraftApi.enchantRepair]
        );
        this.registerEnchant(
            ModList.THAUMCRAFT,
            Enchantment.enchantmentsList[ThaumcraftApi.enchantWandFortune]
        );
    }
        ...
        try {
            ...
            for (Field f : fd) {
                if ((f.getModifiers() & Modifier.STATIC) != 0
                    && f.getType() == Enchantment.class) {
                    this.registerEnchant(
                        ModList.THAUMICTINKER, (Enchantment) f.get(null)
                    );
                }
            }
        } catch (Exception e) {
    ...
    private void registerEnchant(ModList mod, Enchantment e) {
        ...
        if (ReikaEnchantmentHelper.isVanillaEnchant(e)) {
            DragonAPICore.logError(
                "Detected a ThaumCraft enchantment registered to ID " + e.effectId
                + ", overwriting a vanilla ID!"
            );
            return;
        }
