...
public class QuantumOreHandler extends ModHandlerBase {
    ...
    public boolean isQuantumOre(ItemStack block) {
        if (!this.initializedProperly())
            return false;
        return ReikaItemHelper.matchStackWithBlock(block, quantumID)
            || ReikaItemHelper.matchStackWithBlock(block, quantumIDGlow);
    }
    ...
    public void forceOreRegistration() {
        if (!isOreDict) {
            ...
            DragonAPICore.log(
                "QCraft ores already registered to ore dictionary! No action taken!"
            );
