...
public final class ArsMagicaHandler extends ModHandlerBase {
    ...
    private final ArrayList<Class<? extends ISpellComponent>> spellComponents
        = new ArrayList();
    private final ArrayList<Class<? extends ISpellShape>> spellShapes = new ArrayList();
    private final ArrayList<Class<? extends ISpellModifier>> spellModifiers
        = new ArrayList();
        ...
        if (this.hasMod()) {
            try {
                ...
                for (Class c : ReikaJavaLibrary.getAllClassesFromPackage(
                         "am2.spell.components", null, false, true
                     )) {
                ...
                getSpellComponents = util.getDeclaredMethod(
                    "getComponentsForStage", ItemStack.class, int.class
                );
    ...
    public boolean isItem(ItemStack is) {
        if (!this.initializedProperly())
            return false;
        //return ReikaItemHelper.listContainsItemStack(items, is) && is.getItem() ==
        //itemID;
    ...
    public void forceOreRegistration() {
        if (!isOreDict) {
            ...
            DragonAPICore.log(
                "Ars Magica ores already registered to ore dictionary! No action taken!"
            );
    ...
    public ISpellComponent[] getSpellComponents(ItemStack is) {
        try {
            return (ISpellComponent[]
            ) getSpellComponents.invoke(spellUtilInstance, is, 0);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
