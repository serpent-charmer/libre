...
public class ThaumIDHandler extends ModHandlerBase {
            ...
            try {
                ...
                try {
                    ...
                    DragonAPICore.logError(
                        "Could not load field from ThaumCraft config class!"
                    );
                    e.printStackTrace();
                    this.logFailure(e);
                } catch (IllegalArgumentException e) {
                    DragonAPICore.logError(
                        "Could not read field from ThaumCraft config class!"
                    );
                    e.printStackTrace();
                    this.logFailure(e);
    ...
    public boolean initializedProperly() {
        return biomeSet.size() == Biomes.list.length
            && potionSet.size() == Potions.list.length && dimensionID != 0;
    }
