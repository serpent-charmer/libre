...
public class TwilightForestHandler extends ModHandlerBase {
    ...
    private TwilightForestHandler() {
        ...
        if (this.hasMod()) {
            try {
                ...
                DragonAPICore.logError(
                    "Twilight Forest class not found! Cannot read its contents!"
                );
                e.printStackTrace();
                this.logFailure(e);
            }
    ...
    public boolean isToughBlock(Block b) {
        return b == BlockEntry.MAZESTONE.getBlock() || b == BlockEntry.SHIELD.getBlock()
            || b == BlockEntry.DEADROCK.getBlock();
    }
}
