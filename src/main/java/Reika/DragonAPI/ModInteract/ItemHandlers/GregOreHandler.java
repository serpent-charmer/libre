...
public final class GregOreHandler extends ModHandlerBase {
    ...
    public boolean isGregOre(Block b) {
        ...
        return n.startsWith("gt.")
            && (n.startsWith("gt.blockores") || n.startsWith("gt.meta.ore.")
                || n.startsWith("gt.block.rockores"));
    }
    ...
    public static enum CounterpartOres {
        ...
        MINERALSAND(
            ReikaOreHelper.IRON, 3 / 14D, "BasalticMineralSand", "GraniticMineralSand"
        ),
        ...
        public boolean init() {
            ...
            for (String s : internalNames) {
                for (ItemStack is : OreDictionary.getOres(s)) {
                    if (ModList.GREGTECH.modLabel.equals(
        ...
        public String toString() {
            return this.name() + " (~" + counterpart.toString() + " x " + yieldFraction
                + ") < " + internalNames;
        }
    }
