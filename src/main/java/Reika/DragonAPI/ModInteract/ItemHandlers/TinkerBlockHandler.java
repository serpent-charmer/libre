...
public class TinkerBlockHandler extends ModHandlerBase {
    ...
    public enum SmelteryBlocks {
        ...
        public ItemStack getItem(boolean nether) {
            return new ItemStack(
                nether ? instance.netherSmelteryID : instance.smelteryBlockID,
                1,
                this.ordinal()
            );
        }
    ...
    public enum SmelteryTanks {
        ...
        public ItemStack getItem(boolean nether) {
            return new ItemStack(
                nether ? instance.netherTankID : instance.smelteryTankID, 1, metadata
            );
        }
    }
        ...
        if (this.hasMod()) {
            try {
                ...
                DragonAPICore.logError(
                    this.getMod() + " class not found! " + e.getMessage()
                );
                e.printStackTrace();
                this.logFailure(e);
            ...
            slimeDirt = BlockKey.fromItem(
                ReikaItemHelper.lookupBlock(this.getMod(), "CraftedSoil", 5)
            );
            slimeGrass = BlockKey.fromItem(
                ReikaItemHelper.lookupBlock(this.getMod(), "slime.grass", 5)
            );
            ...
            slimeTallGrass
                = GameRegistry.findBlock(this.getMod().modLabel, "slime.grass.tall");
        } else {
            this.noMod();
    ...
    public boolean initializedProperly() {
        return gravelOreID != null
            && stoneOreID != null; // && clearGlassID != null && clearPaneID != null;
    }
    ...
    public boolean isNetherOre(ItemStack block) {
        if (!this.initializedProperly())
            return false;
        return Block.getBlockFromItem(block.getItem()) == stoneOreID
            && block.getItemDamage() < 3;
    }

    public boolean isSlimeIslandBlock(Block b, int meta) {
        if (slimeDirt.match(b, meta) || slimeGrass.match(b, meta) || b == slimeWater
            || b == slimeTallGrass || b == congealedSlime)
            return true;
        return ModWoodList.getModWood(b, meta) == ModWoodList.SLIME
            || ModWoodList.getModWoodFromLeaf(b, meta) == ModWoodList.SLIME;
    }

    public boolean isSmelteryBlock(Block b) {
        return b == smelteryBlockID || b == smelteryTankID || b == netherSmelteryID
            || b == netherTankID;
    }
}
