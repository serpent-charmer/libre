...
public class ReikaBeeHelper {
    ...
    static {
        if (ModList.FORESTRY.isLoaded()) {
            try {
                beeHealth = Class.forName("forestry.core.genetics.IndividualLiving")
                                .getDeclaredField("health");
            ...
            try {
                butterflyStateEnum
                    = Class.forName("forestry.lepidopterology.entities.EnumButterflyState"
                    );
                butterfly
                    = Class.forName("forestry.lepidopterology.entities.EntityButterfly");
        ...
        if (ModList.GENDUSTRY.isLoaded()) {
            try {
                ...
                geneSampleCtr = geneSample.getDeclaredConstructor(
                    ISpeciesRoot.class, int.class, IAllele.class
                );
                geneSampleCtr.setAccessible(true);

                geneTemplate = Class.forName("net.bdew.gendustry.items.GeneTemplate$");
                addSample = geneTemplate.getDeclaredMethod(
                    "addSample", ItemStack.class, geneSample
                );
                addSample.setAccessible(true);

                geneSampleItem = Class.forName("net.bdew.gendustry.items.GeneSample$");
                geneSampleGetItem
                    = geneSampleItem.getDeclaredMethod("newStack", geneSample);
    ...
    public static void buildSpeciesList() {
        allBees.clear();
        for (IAllele ia :
             AlleleManager.alleleRegistry.getRegisteredAlleles(EnumBeeChromosome.SPECIES
             )) {
    ...
    public static String getRandomBeeSpecies() {
        return ReikaJavaLibrary.getRandomCollectionEntry(
            DragonAPICore.rand, getAllBeeSpecies()
        );
    }
    ...
    public static final ItemStack getBeeItem(World world, String bee, EnumBeeType type) {
        ...
        return root.getMemberStack(
            root.getBee(world, root.templateAsGenome(root.getTemplate(bee))),
            type.ordinal()
        );
    }
    ...
    getTreeItem(World world, String Tree, EnumGermlingType type) {
        ITreeRoot root = TreeManager.treeRoot;
        return root.getMemberStack(
            root.getTree(world, root.templateAsGenome(root.getTemplate(Tree))),
            type.ordinal()
        );
    }

    public static final ItemStack
    getButterflyItem(String Butterfly, EnumFlutterType type) {
        IButterflyRoot root = ButterflyManager.butterflyRoot;
        return root.getMemberStack(
            root.templateAsIndividual(root.getTemplate(Butterfly)), type.ordinal()
        );
    }
    ...
    private static void setBeeHealth(IAlvearyController iac, IBee bee, int health) {
        try {
            ...
            MinecraftForge.EVENT_BUS.post(
                new BeeSetHealthEvent(iac, iac.getBeekeepingLogic(), bee)
            );
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private static void
    setBeeHealth(IBeeHousing te, IBeekeepingLogic lgc, IBee bee, int health) {
        try {
            beeHealth.set(bee, health);
    ...
    public static boolean isGenedItem(ItemStack is) {
        return BeeManager.beeRoot.getType(is) != EnumBeeType.NONE
            || TreeManager.treeRoot.getType(is) != EnumGermlingType.NONE
            || ButterflyManager.butterflyRoot.getType(is) != EnumFlutterType.NONE;
    }

    public static boolean isBee(ItemStack is) {
        return is.getItem() == ForestryHandler.ItemEntry.DRONE.getItem()
            || is.getItem() == ForestryHandler.ItemEntry.PRINCESS.getItem()
            || is.getItem() == ForestryHandler.ItemEntry.QUEEN.getItem();
    }
    ...
    public static int getToleranceValue(EnumTolerance t) {
        return t == EnumTolerance.NONE
            ? 0
            : ReikaMathLibrary.getWithinBoundsElse(
                Character.getNumericValue(t.name().charAt(t.name().length() - 1)), 0, 5, 0
            );
    }
    ...
    public static void setGene(
        ItemStack queen,
        IButterflyGenome ibg,
        EnumButterflyChromosome gene,
        IAllele value,
        boolean inactive
    ) {
        boolean ana = AlleleManager.alleleRegistry.getIndividual(queen).isAnalyzed();
        ...
        IButterflyGenome repl
            = ButterflyManager.butterflyRoot.templateAsGenome(arr, arr2);
        saveBee(repl, queen);
        if (ana)
            analyzeBee(queen);
    }
    ...
    public static BeeGene getGeneEnum(EnumBeeChromosome gene, IBeeGenome ibg) {
        switch (gene) {
            ...
            case FERTILITY:
                return BeeAlleleRegistry
                    .getEnum(ibg.getActiveAllele(gene), Fertility.class);
            case FLOWERING:
                return BeeAlleleRegistry
                    .getEnum(ibg.getActiveAllele(gene), Flowering.class);
            ...
            case TERRITORY:
                return BeeAlleleRegistry
                    .getEnum(ibg.getActiveAllele(gene), Territory.class);
            default:
                return null;
        }
    }
    ...
    public static IAllele getToleranceGene(Tolerance d, int i) {
        String s = i > 0 ? String.format("%s%d", d.tag, Math.min(Math.abs(i), 5))
                         : Tolerance.NONE.tag;
        return AlleleManager.alleleRegistry.getAllele(s);
    }

    public static void runProductionCycle(IBeeHousing ibh
    ) { //skips frame damage, other checks, and pollination
    ...
    ) { //exact same as forestry code
        return (IAlleleBoolean) AlleleManager.alleleRegistry.getAllele(
            "forestry.bool" + WordUtils.capitalize(Boolean.toString(value))
        );
    }

    public static IAlleleInteger getIntegerAllele(int value
    ) { //exact same as forestry code
        return (IAlleleInteger
        ) AlleleManager.alleleRegistry.getAllele("forestry.i" + value + "d");
    }
    ...
    public static ArrayList<String> getGenesAsStringList(IGenome ig) {
        ...
        if (ig instanceof IBeeGenome) {
            ...
            EnumBeeChromosome[] order = { EnumBeeChromosome.SPECIES,
            ...
            for (int i = 0; i < order.length; i++) {
                ...
                li.add(
                    getGeneDisplay(ia1, order[i], true, true) + " / "
                    + getGeneDisplay(ia2, order[i], false, false)
                );
                if (order[i] == EnumBeeChromosome.EFFECT) {
                    String t1 = getTemperatureDisplay(
                        sp1,
                        (IAlleleTolerance
                        ) ig.getActiveAllele(EnumBeeChromosome.TEMPERATURE_TOLERANCE),
                        true,
                        true
                    );
                    String t2 = getTemperatureDisplay(
                        sp2,
                        (IAlleleTolerance
                        ) ig.getInactiveAllele(EnumBeeChromosome.TEMPERATURE_TOLERANCE),
                        false,
                        false
                    );
                    li.add(t1 + " / " + t2);
                    String h1 = getHumidityDisplay(
                        sp1,
                        (IAlleleTolerance
                        ) ig.getActiveAllele(EnumBeeChromosome.HUMIDITY_TOLERANCE),
                        true,
                        true
                    );
                    String h2 = getHumidityDisplay(
                        sp2,
                        (IAlleleTolerance
                        ) ig.getInactiveAllele(EnumBeeChromosome.HUMIDITY_TOLERANCE),
                        false,
                        false
                    );
                    li.add(h1 + " / " + h2);
            ...
            EnumTreeChromosome[] order = {
                EnumTreeChromosome.SPECIES,    EnumTreeChromosome.FERTILITY,
                EnumTreeChromosome.MATURATION, EnumTreeChromosome.HEIGHT,
                EnumTreeChromosome.GIRTH,      EnumTreeChromosome.YIELD,
                EnumTreeChromosome.SAPPINESS,  EnumTreeChromosome.FIREPROOF,
                EnumTreeChromosome.EFFECT,     EnumTreeChromosome.GROWTH,
                EnumTreeChromosome.PLANT,      EnumTreeChromosome.FRUITS,
            };
            ...
            for (int i = 0; i < order.length; i++) {
                ...
                IAllele ia2 = ig.getInactiveAllele(order[i]);
                li.add(
                    getGeneDisplay((ITreeGenome) ig, ia1, order[i], true, true) + " / "
                    + getGeneDisplay((ITreeGenome) ig, ia2, order[i], false, false)
                );
            }
            ...
            for (int i = 0; i < order.length; i++) {
                ...
                IAllele ia2 = ig.getInactiveAllele(order[i]);
                li.add(
                    getGeneDisplay((IButterflyGenome) ig, ia1, order[i], true, true)
                    + " / "
                    + getGeneDisplay((IButterflyGenome) ig, ia2, order[i], false, false)
                );
                if (order[i] == EnumButterflyChromosome.EFFECT) {
                    String t1 = getTemperatureDisplay(
                        sp1,
                        (IAlleleTolerance
                        ) ig.getActiveAllele(EnumButterflyChromosome.TEMPERATURE_TOLERANCE
                        ),
                        true,
                        true
                    );
                    String t2 = getTemperatureDisplay(
                        sp2,
                        (IAlleleTolerance) ig.getInactiveAllele(
                            EnumButterflyChromosome.TEMPERATURE_TOLERANCE
                        ),
                        false,
                        false
                    ...
                    String h1 = getHumidityDisplay(
                        sp1,
                        (IAlleleTolerance
                        ) ig.getActiveAllele(EnumButterflyChromosome.HUMIDITY_TOLERANCE),
                        true,
                        true
                    );
                    String h2 = getHumidityDisplay(
                        sp2,
                        (IAlleleTolerance
                        ) ig.getInactiveAllele(EnumButterflyChromosome.HUMIDITY_TOLERANCE
                        ),
                        false,
                        false
                    );
    ...
    public static String getTemperatureDisplay(
        IAlleleSpecies bee, IAlleleTolerance tol, boolean title, boolean primary
    ) {
        EnumChatFormatting ec
            = primary ? EnumChatFormatting.GREEN : EnumChatFormatting.RED;
        String ret = ec + AlleleManager.climateHelper.toDisplay(bee.getTemperature())
            + "/" + tol.getName() + EnumChatFormatting.RESET;
        if (title)
            ret = EnumChatFormatting.LIGHT_PURPLE
                + "Temperature: " + EnumChatFormatting.RESET + ret;
        return ret;
    }

    public static String getHumidityDisplay(
        IAlleleSpecies bee, IAlleleTolerance tol, boolean title, boolean primary
    ) {
        EnumChatFormatting ec
            = primary ? EnumChatFormatting.GREEN : EnumChatFormatting.RED;
        String ret = ec + AlleleManager.climateHelper.toDisplay(bee.getHumidity()) + "/"
            + tol.getName() + EnumChatFormatting.RESET;
        if (title)
            ret = EnumChatFormatting.LIGHT_PURPLE
                + "Humidity: " + EnumChatFormatting.RESET + ret;
        return ret;
    }

    public static String getGeneDisplay(
        IButterflyGenome itg,
        IAllele gene,
        EnumButterflyChromosome type,
        boolean title,
        boolean primary
    ) {
        EnumChatFormatting ec
        ...
        switch (type) {
            ...
            case FERTILITY:
                ...
                //val =
                //ReikaStringParser.capFirstChar(ButterflyAlleleRegistry.Fertility.getFromAllele(gene).name());
            ...
            case TERRITORY:
                ...
                val += " (" + ia.getValue()[0] + "x" + ia.getValue()[1] + "x"
                    + ia.getValue()[2] + ")";
                break;
            case FLOWER_PROVIDER:
                tag = "for.gui.flowers";
            ...
            case METABOLISM:
                ...
                //val =
                //ReikaStringParser.capFirstChar(Metabolisms.getFromAllele(gene).name());
                val = ((IAlleleInteger) gene).getValue() + "x";
                break;
            case SIZE:
                tag = "for.gui.size";
                val += " ("
                    + ((IAlleleFloat) gene).getValue()
                        / Size.AVERAGE.getAllele().getValue()
                    + "x)";
                break;
            case SPEED:
            ...
            case NOCTURNAL:
                ...
                val = StatCollector.translateToLocal(
                    ((IAlleleBoolean) gene).getValue() ? "for.yes" : "for.no"
                );
                break;
            case FIRE_RESIST:
        ...
        return title
            ? EnumChatFormatting.LIGHT_PURPLE + StatCollector.translateToLocal(tag) + ": "
                + EnumChatFormatting.RESET + ec + val + EnumChatFormatting.RESET
            : ec + val + EnumChatFormatting.RESET;
    }
    ...
    public static String getGeneDisplay(
        ...
        switch (type) {
            ...
            case YIELD:
                tag = "for.gui.yield";
                val += " ("
                    + ((IAlleleFloat) gene).getValue() / Yield.LOW.getAllele().getValue()
                    + "x)";
                break;
            case SAPPINESS:
                ...
                val += " ("
                    + ((IAlleleFloat) gene).getValue()
                        / Sappiness.LOW.getAllele().getValue()
                    + "x)";
                break;
            case GIRTH:
                tag = "for.gui.girth";
                val = ((IAlleleInteger) gene).getValue() + "x"
                    + ((IAlleleInteger) gene).getValue();
                break;
            case SPECIES:
                tag = "for.gui.species";
                break;
            ...
            case PLANT:
                tag = "for.gui.native";
                val = StatCollector.translateToLocal(
                    (primary ? itg.getPrimary().getPlantType().toString()
                             : itg.getSecondary().getPlantType().toString())
                );
                break;
            case GROWTH:
                tag = "for.gui.growth";
                val = StatCollector.translateToLocal(
                    ((IAlleleGrowth) gene).getProvider().getDescription()
                );
                break;
            case TERRITORY:
                tag = "for.gui.area";
    ...
    getGeneDisplay(IAllele gene, EnumBeeChromosome type, boolean title, boolean primary) {
        EnumChatFormatting ec
            = primary ? EnumChatFormatting.GREEN : EnumChatFormatting.RED;
        String tag = "";
        String val = gene.getName();
        switch (type) {
            ...
            case FLOWERING:
                ...
                val += " (" + ((IAlleleInteger) gene).getValue() * 20 * 60 / 550
                    + "/min)";
                break;
            case FLOWER_PROVIDER:
                tag = "for.gui.flowers";
    ...
    isDefaultJubilance(IAlleleBeeSpecies ias, IBeeGenome ibg, IBeeHousing ibh) {
    ...
    public static ItemStack getGeneTemplate(IIndividual ii) {
        return getGeneTemplate(
            getBeeRoot().getTemplate(ii.getIdent()),
            AlleleManager.alleleRegistry.getSpeciesRoot(ii.getClass())
        );
    }
    ...
    public static ItemStack getGeneSample(IChromosomeType ic, IAllele ie) {
        ...
        try {
            Object sample
                = geneSampleCtr.newInstance(ic.getSpeciesRoot(), ic.ordinal(), ie);
    ...
    public static Item getGendustrySampleItem() {
        return ModList.GENDUSTRY.isLoaded()
            ? GameRegistry.findItem(ModList.GENDUSTRY.modLabel, "GeneSample")
            : null;
    }

    public static Item getBinnieSampleItem() {
        return Loader.isModLoaded("Genetics") ? GameRegistry.findItem("Genetics", "serum")
                                              : null;
    }
    ...
    public static void setTree(TileEntity leaf, ITree tree) {
        try {
            ...
            leaf.worldObj.markBlockRangeForRenderUpdate(
                leaf.xCoord,
                leaf.yCoord,
                leaf.zCoord,
                leaf.xCoord,
    ...
    public static boolean hasFruit(TileEntity leaf) {
        try {
            return (boolean) treeHasFruit.invoke(leaf)
                && treeRipeness.getInt(leaf) >= treeRipeTime.getShort(leaf);
        } catch (Exception e) {
            e.printStackTrace();
            return false;
        }
    ...
    public static IAllelePlantType getAlleleForPlantType(EnumPlantType plantType) {
        return (IAllelePlantType) AlleleManager.alleleRegistry.getAllele(
            "forestry.plantType" + ReikaStringParser.capFirstChar(plantType.name())
        );
    }

    public static void attractButterflies(
        World world, double x, double y, double z, double r, EntityPathfinder path
    ) {
        AxisAlignedBB box
            = AxisAlignedBB.getBoundingBox(x - r, y - r, z - r, x + r, y + r, z + r);
        for (EntityCreature e : ((List<EntityCreature>) world.getEntitiesWithinAABB(
                 IEntityButterfly.class, box
             ))) {
            ...
            if (path != null) {
                if (path.isInRange(e)) {
                    ...
                    if (c != null) {
                        flag = true;
                        SphericalVector vec = SphericalVector.fromCartesian(
                        ...
                        ReikaPacketHelper.sendDataPacket(DragonAPIInit.packetChannel,
                        PacketIDs.PARTICLEWITHPOS.ordinal(), new
                        PacketTarget.RadiusTarget(e, 32),
                        ReikaParticleHelper.SMOKE.ordinal(), 4, xp[0], xp[1], yp[0],
                        yp[1], zp[0], zp[1]);
                         */
                    }
                } else {
            ...
            if (!flag) {
                if (e.getDistanceSq(x, y, z) <= 32 || true) {
                    ...
                    if (b.getMaterial().blocksMovement()
                        && b.getCollisionBoundingBoxFromPool(world, idx, idy, idz)
                            != null) {
                        vx *= -1;
                        vz *= -1;
                        vy += 0.25;
                    ...
                    if (e.tasks.taskEntries.size() != 1
                        || !(
                            ((EntityAITaskEntry) e.tasks.taskEntries.get(0)).action
                            instanceof AITaskSeekLocation
                        )) {
                        e.tasks.taskEntries.clear();
                        e.tasks.addTask(
                            0, new AITaskSeekLocation(e, 0.5, new Coordinate(x, y, z))
                        );
                    }
                }
            }
    ...
    public static void
    collectButterflies(World world, AxisAlignedBB box, EntityPlayer ep) {
        for (IEntityButterfly e : ((List<IEntityButterfly>) world.getEntitiesWithinAABB(
                 IEntityButterfly.class, box
             ))) {
            ...
            if (fly == null)
                continue;
            ItemStack is = ButterflyManager.butterflyRoot.getMemberStack(
                fly, EnumFlutterType.BUTTERFLY.ordinal()
            );
            if (is != null) {
                e.getEntity().setDead();
                ReikaSoundHelper.playSoundAtEntity(
                    world, e.getEntity(), "random.pop", 0.5F, 2F
                );
                is = is.copy();
                if (ep != null) {
    ...
    public static int[] getEffectiveTerritory(
        IBeeHousing ibh, ChunkCoordinates c, IBeeGenome ibg, long time
    ) {
