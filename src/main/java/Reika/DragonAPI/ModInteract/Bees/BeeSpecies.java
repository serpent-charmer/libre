...
public abstract class BeeSpecies
    extends GeneBase implements IAlleleBeeSpecies, IIconProvider {
    ...
    protected BeeSpecies(
        String name, String uid, String latinName, String creator, IClassification g
    ) {
        ...
        if (s.length < 2)
            throw new RuntimeException(
                "Bee latin names must be at least two words (genus and species)!"
            );
        genus = s[0];
        String scn = s[1];
    ...
    public void register() {
        ...
        AlleleManager.alleleRegistry.getClassification("family.apidae")
            .addMemberGroup(branch);
        isRegistered = true;
    }
    ...
    public final void addBreeding(String parent1, String parent2, int chance) {
        IAlleleBeeSpecies p1 = (IAlleleBeeSpecies
        ) AlleleManager.alleleRegistry.getAllele("forestry.species" + parent1);
        IAlleleBeeSpecies p2 = (IAlleleBeeSpecies
        ) AlleleManager.alleleRegistry.getAllele("forestry.species" + parent2);
        if (p1 == null)
            throw new MisuseException(
                "Error breeding from " + parent1 + ": You cannot breed a bee from null!"
            );
        if (p2 == null)
            throw new MisuseException(
                "Error breeding from " + parent2 + ": You cannot breed a bee from null!"
            );
        this.addBreeding(p1, p2, chance);
    }

    public final void
    addBreeding(String parent1, ModList mod1, String parent2, ModList mod2, int chance) {
        IAlleleBeeSpecies p1 = (IAlleleBeeSpecies) AlleleManager.alleleRegistry.getAllele(
            mod1.modLabel.toLowerCase(Locale.ENGLISH) + ".species" + parent1
        );
        IAlleleBeeSpecies p2 = (IAlleleBeeSpecies) AlleleManager.alleleRegistry.getAllele(
            mod2.modLabel.toLowerCase(Locale.ENGLISH) + ".species" + parent2
        );
        if (p1 == null)
            throw new MisuseException(
                "Error breeding from " + parent1 + ": You cannot breed a bee from null!"
    ...
    public final void
    addBreeding(String parent1, ModList mod1, BeeSpecies parent2, int chance) {
        IAlleleBeeSpecies p1 = (IAlleleBeeSpecies) AlleleManager.alleleRegistry.getAllele(
            mod1.modLabel.toLowerCase(Locale.ENGLISH) + ".species" + parent1
        );
        if (p1 == null)
    ...
    public final void addBreeding(BeeSpecies parent1, BeeSpecies parent2, int chance) {
        this.addBreeding(
            (IAlleleBeeSpecies) parent1, (IAlleleBeeSpecies) parent2, chance
        );
    }

    public final void
    addBreeding(IAlleleBeeSpecies p1, IAlleleBeeSpecies p2, int chance) {
        if (!isRegistered)
            throw new MisuseException(
                "You must register a bee before adding breeding pairs!"
            );
        if (p1 == null || p2 == null)
    ...
    public static class BeeBreeding implements IBeeMutation {
        ...
        protected BeeBreeding(
            IAlleleBeeSpecies p1, IAlleleBeeSpecies p2, int chance, BeeSpecies bee
        ) {
            parent1 = p1;
            parent2 = p2;
            this.chance = chance;
        ...
        public final boolean isPartner(IAllele ia) {
            return parent1.getUID().equals(ia.getUID())
                || parent2.getUID().equals(ia.getUID());
        }
        ...
        public final float getChance(
            IBeeHousing ibh,
            IAlleleBeeSpecies ia1,
            IAlleleBeeSpecies ia2,
            IBeeGenome ig1,
            IBeeGenome ig2
        ) {
            return this.isValidParents(ia1, ia2) && this.canBeBred(ibh, ig1, ig2)
                ? this.getModifiedChance(ibh, ig1, ig2)
                : 0;
        }
        ...
        protected final boolean isValidParents(IAllele ia1, IAllele ia2) {
            if (ia1.getUID().equals(parent1.getUID())
                && ia2.getUID().equals(parent2.getUID()))
                return true;
            if (ia1.getUID().equals(parent2.getUID())
                && ia2.getUID().equals(parent1.getUID()))
                return true;
            return false;
        }
        ...
        @Override
        @Deprecated
        public float getChance(IBeeHousing ibh, IAllele ia1, IAllele ia2, IGenome ig1,
        IGenome ig2) { return this.isValidParents(ia1, ia2) ? chance : 0;
        }
         */
    }
    ...
    protected final IAllele[] getSpeciesTemplate() {
        ...
        alleles[EnumBeeChromosome.SPEED.ordinal()]
            = this.getProductionSpeed().getAllele();
        alleles[EnumBeeChromosome.LIFESPAN.ordinal()] = this.getLifespan().getAllele();
        alleles[EnumBeeChromosome.TERRITORY.ordinal()]
            = this.getTerritorySize().getAllele();
        alleles[EnumBeeChromosome.FLOWERING.ordinal()]
            = this.getFloweringRate().getAllele();
        ...
        alleles[EnumBeeChromosome.NOCTURNAL.ordinal()]
            = ReikaBeeHelper.getBooleanAllele(this.isNocturnal());
        alleles[EnumBeeChromosome.CAVE_DWELLING.ordinal()]
            = ReikaBeeHelper.getBooleanAllele(this.isCaveDwelling());
        alleles[EnumBeeChromosome.TEMPERATURE_TOLERANCE.ordinal()]
            = ReikaBeeHelper.getToleranceGene(
                this.getTemperatureToleranceDir(), this.getTemperatureTolerance()
            );
        alleles[EnumBeeChromosome.HUMIDITY_TOLERANCE.ordinal()]
            = ReikaBeeHelper.getToleranceGene(
                this.getHumidityToleranceDir(), this.getHumidityTolerance()
            );
        alleles[EnumBeeChromosome.TOLERANT_FLYER.ordinal()]
            = ReikaBeeHelper.getBooleanAllele(this.isTolerantFlyer());
        return alleles;
    }
    ...
    public final void registerIcons(IIconRegister ico) {
        ...
        IIcon body1 = ico.registerIcon(
            mod + ":" + this.getIconFolderRoot() + "/" + iconType + body
        );
        IIcon larva = ico.registerIcon(
            mod + ":" + this.getIconFolderRoot() + "/" + iconType + "/"
            + EnumBeeType.LARVAE.name().toLowerCase(Locale.ENGLISH) + ".body"
        );

        for (int i = 0; i < EnumBeeType.VALUES.length; i++) {
            if (EnumBeeType.VALUES[i] != EnumBeeType.NONE) {
                String type = EnumBeeType.VALUES[i].name().toLowerCase(Locale.ENGLISH);
                String out = EnumBeeType.VALUES[i] != EnumBeeType.LARVAE
                        && this.simplifiedIconSystem()
                    ? "outline"
                    : type + ".outline";
                icons[i][0] = ico.registerIcon(
                    mod + ":" + this.getIconFolderRoot() + "/" + iconType + "/" + out
                );
                icons[i][1] = EnumBeeType.VALUES[i] == EnumBeeType.LARVAE ? larva : body1;
                String clas = this.simplifiedIconSystem() ? type : type + ".body2";
                icons[i][2] = ico.registerIcon(
                    mod + ":" + this.getIconFolderRoot() + "/" + iconType + "/" + clas
                );
            }
        }
    }
    ...
    @Override
    public ItemStack[] getResearchBounty(
        World paramWorld, GameProfile f, IIndividual paramIIndividual, int paramInt
    ) {
