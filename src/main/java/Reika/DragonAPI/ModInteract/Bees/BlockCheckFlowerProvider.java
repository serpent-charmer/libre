...
public abstract class BlockCheckFlowerProvider
    implements IFlowerProvider, IFlowerAcceptableRule {
    ...
    public BlockCheckFlowerProvider(BlockCheck bk, String id) {
        ...
        FlowerManager.flowerRegistry.registerAcceptableFlower(
            block.asBlockKey().blockID, block.asBlockKey().metadata, this.id
        );
    ...
    public final boolean isAcceptedFlower(World world, int x, int y, int z) {
        return block.matchInWorld(
            world, x, y, z
        ); //FlowerManager.flowerRegistry.isAcceptedFlower(id, world, x, y, z);
    }

    /*
    @Override
    public boolean isAcceptedFlower(World world, IIndividual individual, int x, int y, int
    z) { return block.matchInWorld(world, x, y, z);
    }
     */
    @Override
    ...
    growFlower(World world, IIndividual individual, int x, int y, int z) {
        return false;
    }
    ...
    public boolean growFlower(
        IFlowerGrowthHelper helper, String flowerType, World world, int x, int y, int z
    ) {
        return false;
    }

    public final boolean growFlower(
        IFlowerRegistry fr,
        String flowerType,
        World world,
