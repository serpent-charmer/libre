...
public class ReikaXPFluidHelper {
    ...
    static {
        ...
        addFluid(
            "openblocks.OpenBlocks$Fluids",
            "xpJuice",
            "openmods.utils.EnchantmentUtils",
            "RATIO"
        );
        addFluid(
            "crazypants.enderio.EnderIO",
            "fluidXpJuice",
            "crazypants.enderio.xp.XpUtil",
            "RATIO"
        );
        addFluid(
            "mods.immibis.lxp.LiquidXPMod",
            "defaultFluid",
            "mods.immibis.lxp.LiquidXPMod",
            "mbPerXp"
        );
    ...
    private static void addFluid(String cf, String sf, String c, String f) {
        try {
            ...
            DragonAPICore.log(
                "Could not load xp fluid type as loaded from " + cf + "#" + sf + ": " + e
            );
            //e.printStackTrace();
        }
    }
    ...
    private static void register(Fluid f, int amt) {
        if (loaded == null) {
            ...
            ratio = amt;
            DragonAPICore.log(
                "Loaded XP fluid " + f.getName() + " with ratio of " + amt + " mB/xp."
            );
        } else {
            DragonAPICore.log(
                "Rejected XP fluid " + f.getName() + " with ratio of " + amt
                + " mB/xp; a fluid is already loaded."
            );
        }
    }
