...
public class DragonAPIEventWatcher implements ProfileEventWatcher, BlockRenderWatcher {
    ...
    public boolean onBlockTriedRender(
        Block block, int x, int y, int z, WorldRenderer wr, RenderBlocks rb, int pass
    ) {
        if (block instanceof Submergeable) {
            ...
            if (s.isSubmergeable(rb.blockAccess, x, y, z) && s.renderLiquid(meta)) {
                if (pass == 1)
                    this.renderWaterInBlock(
                        rb.blockAccess, x, y, z, block, meta, Tessellator.instance
                    );
            }
        }
        return false;
    ...
    @SideOnly(Side.CLIENT)
    private void renderWaterInBlock(
        IBlockAccess world, int x, int y, int z, Block block, int meta, Tessellator v5
    ) {
        Block above = world.getBlock(x, y + 1, z);
        if (above != Blocks.water && above != Blocks.flowing_water
            && !ReikaWorldHelper.hasAdjacentWater(world, x, y + 1, z, false, false)) {
            boolean flag
                = ReikaWorldHelper.hasAdjacentWater(world, x, y, z, false, false);
            if (!flag) {
                for (int i = 2; i < 6 && !flag; i++) {
                    ...
                    flag = flag
                        || ReikaWorldHelper.hasAdjacentWater(
                            world, dx, dy, dz, false, true
                        );
                }
            }
            if (flag) {
                ...
                double d2
                    = RenderBlocks.getInstance().getLiquidHeight(x, y, z, Material.water);
                double d3 = world.getBlock(x, y, z + 1).getMaterial() == Material.water
                    ? RenderBlocks.getInstance().getLiquidHeight(
                        x, y, z + 1, Material.water
                    )
                    : 0.8875;
                double d4
                    = world.getBlock(x + 1, y, z + 1).getMaterial() == Material.water
                    ? RenderBlocks.getInstance().getLiquidHeight(
                        x + 1, y, z + 1, Material.water
                    )
                    : 0.8875;
                double d5 = world.getBlock(x + 1, y, z).getMaterial() == Material.water
                    ? RenderBlocks.getInstance().getLiquidHeight(
                        x + 1, y, z, Material.water
                    )
                    : 0.8875;
    ...
    public void monitorNewWorld(CreateSpawnPosition ev) {
        DragonAPICore.log(
            "Registering creation of new world " + ev.world + " in "
            + ev.world.getSaveHandler().getWorldDirectory()
        );
    ...
    public void tagDeathDrops(PlayerDropsEvent ev) {
        for (EntityItem ei : ev.drops) {
            ei.getEntityData().setString(
                ReikaItemHelper.PLAYER_DEATH_DROP_KEY,
                ev.entityPlayer.getUniqueID().toString()
            );
        }
    }
    ...
    private void showF3Extras() {
        ...
        mc.ingameGUI.drawString(
            mc.fontRenderer,
            sg,
            mc.displayWidth / ReikaRenderHelper.getGUIScale() - 10
                - mc.fontRenderer.getStringWidth(sg),
            73 + (len - 4) * (2 + mc.fontRenderer.FONT_HEIGHT),
            0xffffff
        );
    }
    ...
    public void xmasSky(SkyColorEvent evt) {
        ...
        if (f > 0) {
            evt.color
                = ReikaColorAPI.mixColors(WinterColorsEvent.getSkyColor(), evt.color, f);
        }
    }
                ...
                if (te instanceof IInventory) {
                    ...
                    for (int i = 0; i < 6; i++) {
                        HashMap<Integer, ItemStack> map
                            = ReikaInventoryHelper.getLocatedTransferrables(
                                ForgeDirection.VALID_DIRECTIONS[i], ii
                            );
                        for (int slot : map.keySet()) {
                            ...
                            if (evt.entityPlayer.isSneaking())
                                ...
                                flag = ReikaInventoryHelper.addToIInv(
                                    is, evt.entityPlayer.inventory
                                );
                            if (flag) {
                                ii.setInventorySlotContents(slot, null);
                                String s = "Collected " + is.stackSize + " "
                                    + is.getDisplayName();
                                ReikaPacketHelper.sendStringPacket(
                                    DragonAPIInit.packetChannel,
                                    PacketIDs.STRINGPARTICLE.ordinal(),
                                    s,
                                    te
                                );
                                flag2 = true;
                    ...
                    if (flag2) {
                        ReikaSoundHelper.playSoundAtEntity(
                            evt.world, evt.entityPlayer, "random.click", 1, 1.5F
                        );
                    }
                    evt.setCanceled(true);
                }
    ...
    public void modifyFireSpread(FireChanceEvent evt) {
        if (DragonOptions.BIOMEFIRE.getState()) {
            ...
            float humid = b.rainfall; //ranges from 0 (desert, nether, etc) to 1 (ocean)
                                      ////ReikaBiomeHelper.getBiomeHumidity(b);
            evt.spreadChance *= biomeHumidityFlammability.getValue(humid
            ); //*= 2-humid*1.5;  //1F/(0.5F+humid); //doubled for 0, unchanged for 0.5,
               //halved for 1
        }
    }
    ...
    public void fixRespirationFourPlusFog(EntityViewRenderEvent.FogColors evt) {
        ...
        if (helm != null
            && ReikaEnchantmentHelper.getEnchantmentLevel(Enchantment.respiration, helm)
                > 3) {
    ...
    public void fixRespirationFourPlusFog(EntityViewRenderEvent.FogDensity evt) {
        ...
        if (helm != null
            ...
            if (ep.isInsideOfMaterial(Material.water)
                || ep.isInsideOfMaterial(Material.lava)) {
                GL11.glFogi(GL11.GL_FOG_MODE, GL11.GL_EXP);
                evt.density = Math.min(
                    evt.density, ep.isInsideOfMaterial(Material.lava) ? 0.1F : 0.0025F
                );
                if (ep.isPotionActive(Potion.blindness))
                    evt.density = 0.9F;
                evt.setCanceled(true);
    ...
    public void catchNullOreDict(OreRegisterEvent evt) {
        if (evt.Ore == null || evt.Ore.getItem() == null)
            throw new WTFException(
                "Someone registered null to the OreDictionary under the name '" + evt.Name
                    + "'!",
                true
            );
        else if (evt.Name == null || evt.Name.isEmpty())
            throw new WTFException(
                "Someone registered " + evt.Ore + " under a null or empty OreDict name!",
                true
            );
        else {
            if (!oreDictLogSpamReductionStrings.contains(evt.Name)) {
                DragonAPICore.log(
                    "Logged OreDict registration of " + evt.Ore + " as '" + evt.Name
                    + "'."
                );
    ...
    public void onGameLoaded(GameFinishedLoadingEvent evt) throws InterruptedException {
        ...
        mc.getSoundHandler().playSound(
            new PositionedSoundRecord(new ResourceLocation("note.harp"), 1, 1, 0, 0, 0)
        );
        Thread.sleep(100);
        ...
        mc.getSoundHandler().playSound(
            new PositionedSoundRecord(new ResourceLocation("note.harp"), 1, f, 0, 0, 0)
        );
        ...
        while (!RemoteAssetLoader.instance.isDownloadComplete()) {
            if (time % 5000 == 0) {
                String p = String.format(
                    "%.2f", 100 * RemoteAssetLoader.instance.getDownloadProgress()
                );
                String s = "Remote asset downloads not yet complete (current = " + p
                    + "%). Pausing game load. Total delay: " + time + " ms.";
                DragonAPICore.log(s);
            }
            Thread.sleep(d);
            time += d;
    ...
    public void sendInteractToClient(PlayerInteractEvent evt) {
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER
            && !ReikaPlayerAPI.isFake(evt.entityPlayer)) {
            ReikaPacketHelper.sendDataPacket(
                DragonAPIInit.packetChannel,
                PacketIDs.PLAYERINTERACT.ordinal(),
                new PacketTarget.PlayerTarget((EntityPlayerMP) evt.entityPlayer),
                evt.x,
                evt.y,
                evt.z,
                evt.face,
    ...
    public void tagDroppedItems(ItemTossEvent evt) {
        if (evt.player != null) {
            ...
            //ReikaPacketHelper.sendStringIntPacket(packetChannel,
            //PacketIDs.ITEMDROPPER.ordinal(), new
            //PacketTarget.DimensionTarget(evt.entityItem.worldObj), s,
            //evt.entityItem.getEntityId());
        }
    }
    ...
    public void tagDroppedItems(EntityJoinWorldEvent evt) {
        if (evt.entity instanceof EntityItem && evt.world.isRemote) {
            //ReikaJavaLibrary.pConsole("Sending clientside request for Entity ID
            //"+evt.entity.getEntityId());
            ReikaPacketHelper.sendDataPacket(
                DragonAPIInit.packetChannel,
                PacketIDs.ITEMDROPPERREQUEST.ordinal(),
                PacketTarget.server,
                evt.entity.getEntityId()
            );
        }
    ...
    public void confirmNumericIDs(ChatEventPost evt) {
        if (System.currentTimeMillis() - IDMsgCooldown >= 30000
            && evt.chatMessage.startsWith(
                "Warning: Using numeric IDs will not be supported in the future"
            )) {
            String item1 = EnumChatFormatting.GOLD
                + "/give item.forestry.apiculture.bee.template.root3";
            String item2 = EnumChatFormatting.GOLD
                + "/give item.gregtech.machine.primary.transformer.hv.ruby";
            String c = EnumChatFormatting.LIGHT_PURPLE.toString();
            ReikaChatHelper.writeString(
                c + "Numeric IDs will remain functional as long as I am here,"
            );
        ...
        if (!evt.isVanillaPass) {
            try {
                if (!ReikaRecipeHelper.verifyRecipe(evt.recipe)) {
                    ...
                    if (evt.recipe.getRecipeOutput() != null
                        && evt.recipe.getRecipeOutput().getItem() != null)
                        msg += ", Output=" + evt.recipe.getRecipeOutput();
                    else if (evt.recipe.getRecipeOutput() != null)
                        msg += ", Output is a null-item ItemStack";
                    DragonAPICore.log(
                        "Invalid recipe, such as with nulled inputs, found. Removing to prevent crashes. "
                        + msg + "."
                    );
                    evt.setCanceled(true);
                }
        ...
        if (!evt.isVanillaPass) {
            try {
                ...
                if (in == null || in.getItem() == null) {
                    DragonAPICore.logError("Found a null-input (or null-item input)
    smelting recipe! "+null+" > "+out+"! This is invalid!"); Thread.dumpStack();
                    evt.markInvalid();
                }
                else if (out == null || out.getItem() == null) {
                    DragonAPICore.logError("Found a null-output (or null-item output)
    smelting recipe! "+in+" > "+null+"! This is invalid!"); Thread.dumpStack();
                    evt.markInvalid();
                }
                else if (!ReikaItemHelper.verifyItemStack(in, true)) {
                    DragonAPICore.logError("Found a smelting recipe with an invalid
    input!"); Thread.dumpStack(); evt.markInvalid();
                }
                else if (!ReikaItemHelper.verifyItemStack(out, true)) {
                    DragonAPICore.logError("Found a smelting recipe with an invalid
    output!"); Thread.dumpStack(); evt.markInvalid();
                }
            }
            catch (Exception e) {
    ...
    public void collateXP(XPUpdateEvent evt) {
        if (DragonOptions.XPMERGE.getState()) {
            //ReikaJavaLibrary.pConsole(evt.xp.worldObj.loadedEntityList.size(),
            //Side.SERVER);
            if (!evt.xp.isDead) {
                if (!evt.xp.worldObj.isRemote && evt.xp.xpOrbAge % 16 == 0) {
                    AxisAlignedBB box = ReikaAABBHelper.getEntityCenteredAABB(evt.xp, 1);
                    List<EntityXPOrb> li
                        = evt.xp.worldObj.getEntitiesWithinAABB(EntityXPOrb.class, box);
                    //ReikaJavaLibrary.pConsole(li.size()+":"+li);
                    if (li.size() > 1) {
                        EntityXPOrb xp
                            = ReikaEntityHelper.mergeXPOrbs(evt.xp.worldObj, li);
                    }
                }
            }
        }
