...
public class ChunkPregenerator implements TickHandler {
    ...
    public void
    addChunks(WorldServer world, double pertick, int ctrX, int ctrZ, int radius) {
    ...
    private ArrayList<ChunkCoordIntPair> spiralSort(int ctrX, int ctrZ, int radius) {
        ...
        BlockSpiral sp
            = new BlockSpiral(ctrX, 0, ctrZ, radius / 16).setGridSize(16).calculate();
    ...
    private static final class Pregenerator {
        ...
        private boolean tick(World world) {
            if (numberChunksPerTick >= 1
                || ReikaRandomHelper.doWithChance(numberChunksPerTick)) {
        ...
        private void generate(World world, ChunkCoordIntPair c) {
            ...
            try {
                ...
                if (p >= 5 + pregenPercent) {
                    pregenPercent += 5;
                    this.notifyAdmins(
                        "Pregeneration of area " + origin + " " + String.format("%.3f", p)
                        + " percent complete."
                    );
                }
                //ReikaJavaLibrary.pConsole("Generated chunk @ "+x+","+z+";
                //"+String.format("%.3f%% complete", p));
            } catch (Exception e) {
                DragonAPICore.logError(
                    "Tried and failed generating chunk @ " + x + "," + z + ";"
                );
                e.printStackTrace();
            }
        }
