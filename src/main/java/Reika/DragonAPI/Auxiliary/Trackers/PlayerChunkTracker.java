...
public class PlayerChunkTracker implements TickRegistry.TickHandler {
    ...
    public void tick(TickRegistry.TickType type, Object... tickData) {
        ...
        synchronized (trackedPlayers) {
            Iterator<Map.Entry<EntityPlayer, TrackerEntry>> iterator
            ...
            while (iterator.hasNext()) {
                ...
                if (entry.getKey().isDead
                    || !tracker.condition.shouldBeTracked(entry.getKey())) {
                    tracker.timeout--;
                } else {
    ...
    public static void
    startTrackingPlayer(EntityPlayer player, TrackingCondition condition) {
    ...
    public static void
    tryUpdatePlayerPertinentChunks(PlayerManager pm, EntityPlayerMP mp) {
