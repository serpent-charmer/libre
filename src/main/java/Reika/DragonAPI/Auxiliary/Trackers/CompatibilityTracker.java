...
public final class CompatibilityTracker {
    ...
    public void
    registerIncompatibility(ModList source, ModList issue, Severity level, String desc) {
        Incompatibility inc = new Incompatibility(source, issue, level, desc);
        if (!issues.contains(inc))
            issues.add(inc);
    }

    public void registerConditionalIncompatibility(
        ModList source, ModList issue, Severity level, String desc, boolean condition
    ) {
        Incompatibility inc
            = new Incompatibility(source, issue, level, desc).setConditional(condition);
        if (!issues.contains(inc))
            issues.add(inc);
    }

    public void registerHostileIncompatibility(
        ModList source, ModList issue, String desc, boolean crash
    ) {
        Incompatibility inc
            = new Incompatibility(source, issue, Severity.HOSTILITY, desc);
        if (crash)
            inc.setAlwaysCrash();
        if (!issues.contains(inc)) {
            issues.add(inc);
    ...
    public void test() {
        for (int i = 0; i < issues.size(); i++) {
            ...
            if (inc.isActive()) {
                ...
                if (inc.alwaysCrash) {
                    DragonAPICore.log(
                        inc.sourceMod.getDisplayName()
                        + " has detected incompatible modifications from "
                        + inc.issueMod.getDisplayName()
                        + " and was set to disallow continuing.\n"
                    );
                    DragonAPICore.log(
                        "It cannot function correctly. Check "
                        + inc.sourceMod.getDisplayName()
                        + " configs for an option to bypass this test and run anyways."
                    );
                    throw new RuntimeException();
                } else if (s == Severity.HOSTILITY) {
                    DragonAPICore.log(
                        inc.sourceMod.getDisplayName()
                        + " has detected hostile modifications from "
                        + inc.issueMod.getDisplayName()
                        + " but was set to allow continuing.\n"
                    );
                    DragonAPICore.log(
                        "However, it cannot function correctly. Contact the authors of the mods."
                    );
                } else if (s.throwException) {
                    new ModIncompatibilityException(
                        inc.sourceMod, inc.issueMod, inc.message, s.isFatal
                    );
                } else {
                    DragonAPICore.log(
                        inc.sourceMod.getDisplayName()
                        + " has logged a compatibility issue with "
                        + inc.issueMod.getDisplayName() + ".\n"
                    );
                    DragonAPICore.log(
                        "However, it is able to continue without ill effects."
                    );
                }
            }
        }
    ...
    private static class Incompatibility {
        ...
        public Incompatibility(
            ModList source, ModList issue, Severity level, String msg
        ) {
            sourceMod = source;
            issueMod = issue;
            severity = level;
