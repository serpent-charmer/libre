...
public final class IDCollisionTracker {
    ...
    public void
    addBiomeID(DragonAPIMod mod, int id, Class<? extends BiomeGenBase> biomeClass) {
        this.addID(mod, id, IDType.BIOME, biomeClass);
    }

    public void
    addEnchantmentID(DragonAPIMod mod, int id, Class<? extends Enchantment> enchClass) {
        this.addID(mod, id, IDType.ENCHANTMENT, enchClass);
    }

    public void
    addPotionID(DragonAPIMod mod, int id, Class<? extends Potion> potionClass) {
                ...
                for (int id : ids) {
                    ...
                    if (obj == null) {
                        ...
                        DragonAPICore.logError(
                            type.getName() + " ID " + id + " (" + dat.classes.get(id)
                            + ") was deleted post-registration!"
                        );
                    } else {
    ...
    protected String
    onConflict(DragonAPIMod mod, IDType type, int id, Class c, Class c1) {
        String s = type.getName() + " IDs: " + type.getValue(id) + " @ ID " + id + " ("
            + c.getSimpleName() + " & " + c1.getSimpleName() + ")";
