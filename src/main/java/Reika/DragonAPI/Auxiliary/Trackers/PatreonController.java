...
public final class PatreonController {
    ...
    public static final String reikaURL
        = "https://reikasminecraft.overminddl1.com/Donator/patreon_";
    ...
    public void registerMod(String dev, String root) {
        ...
        if (file == null) {
            DragonAPICore.logError(
                "Could not create URL to patreon file. Donators will not be loaded."
            );
            return;
        }
    ...
    private void addPatrons(String dev, List<String> lines) {
        for (String s : lines) {
            ...
            try {
                ...
                DragonAPICore.logError(
                    "Invalid patreon line: " + s + " for " + dev + ": " + e.toString()
                );
            }
        }
    }
    ...
    private static class DonatorFile implements ConnectionErrorHandler {
        ...
        public void onCertificateFailed() {
            DragonAPICore.logError(
                "Error accessing online file: Your HTTP certificate is invalid!"
            );
        }
    }
    ...
    private void addPatron(String dev, String name, String ingame, int amt) {
        ...
        DragonAPICore.log(
            "Adding patron to " + dev + ": " + name + " / " + ingame + " @ $" + amt
        );
    }
    ...
    public Collection<Donator> getModPatrons(String dev) {
        Patrons p = data.get(dev);
        return p != null ? Collections.unmodifiableCollection(p.data.keySet())
                         : new ArrayList();
    }
        ...
        public String toString() {
            ...
            for (Donator d : data.keySet()) {
                ...
                sb.append(String.format(
                    "  %s%s%s: %s%d%s\n",
                    this.getDisplayColor(amt).toString(),
                    this.getFormatting(amt),
                    d.toString(),
                    "$",
                    amt,
                    "/mo"
