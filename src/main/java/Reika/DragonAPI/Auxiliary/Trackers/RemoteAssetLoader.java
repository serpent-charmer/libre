...
public class RemoteAssetLoader {
    ...
    public void checkAndStartDownloads() {
        if (!downloadingAssets.isEmpty()) {
            ...
            for (RemoteAsset a : downloadingAssets) {
                a.log(
                    "Remote Asset '" + a.getDisplayName() + "' for "
                    + a.mod.getDisplayName()
                    + " is either missing or out of date. Redownloading..."
                );
                downloader.totalSize += a.data.size;
            }
            DragonAPICore.log(
                "Projected total download size: " + downloader.totalSize + " bytes in "
                + downloadingAssets.size() + " files."
            );
            downloadThread = new Thread(downloader, "Remote Asset Download");
            downloadThread.start();
        } else {
            MinecraftForge.EVENT_BUS.post(
                new RemoteAssetsDownloadCompleteEvent(downloadingAssets, 0)
            );
        }
    }
    ...
    public float getCurrentFileProgress() {
        return downloader != null ? Math.min(1, downloader.getCurrentFileCompletion())
                                  : 1F;
    }

    public boolean isDownloadComplete() {
        return downloadThread == null || !downloadThread.isAlive() || downloader == null
    ...
    public void onClientReceiveWarning(ClientLoginEvent evt) {
        for (BigWarning w : bigWarnings) {
            String sg = w.message + " the file server for remote asset repository '"
                + w.repository.getDisplayName()
                + "' may be inaccessible. Check your internet settings, and please notify "
                + w.repository.mod.getModAuthorName()
    ...
    private static class AssetDownloader implements Runnable, WriteCallback {
        ...
        public void run() {
            ...
            DragonAPICore.log(
                "All asset downloads complete. Elapsed time: "
                + ReikaDateHelper.millisToHMSms(duration)
            );
            isComplete = true;
            MinecraftForge.EVENT_BUS.post(new RemoteAssetsDownloadCompleteEvent(
                instance.downloadingAssets, totalSize
            ));
        }
        ...
        private void tryDownload(AssetData dat, int max) {
            for (int i = 0; i < max; i++) {
                try {
                    ...
                    /*dat.asset.mod.getModLogger()*/ DragonAPICore.logError(
                        "Could not read remote asset '" + dat.getDisplayName() + "'. "
                        + text
                    );
                    e.printStackTrace();
                    if (end) {
                    ...
                    /*dat.asset.mod.getModLogger()*/ DragonAPICore.logError(
                        "Could not save asset '" + dat.getDisplayName()
                        + "'. Skipping file."
                    );
                    e.printStackTrace();
                    break;
                    ...
                    /*dat.asset.mod.getModLogger()*/ DragonAPICore.logError(
                        "Could not download remote asset '" + dat.getDisplayName()
                        + "'. Skipping file."
                    );
                    e.printStackTrace();
                    break;
        ...
        private void download(AssetData dat) throws IOException {
            ...
            if (!ReikaFileReader.isFileWithin(f, DragonAPICore.getMinecraftDirectory())) {
                ...
                sb.append(
                    "Remote Asset " + dat.asset.getDisplayName()
                    + " attempted to download to " + f.getCanonicalPath() + "!"
                );
                sb.append(
            ...
            try (InputStream in = c.getInputStream();
                ...
                String s = "Download of '" + dat.getDisplayName() + "' to '"
                    + dat.asset.getLocalPath() + "' complete. Elapsed time: "
                    + ReikaDateHelper.millisToHMSms(duration);
                /*dat.asset.mod.getModLogger()*/ DragonAPICore.log(s);
                DragonAPICore.log(
                    "Remote asset downloads now "
                    + String.format(
                        "%.2f", Math.min(100, this.getTotalCompletion() * 100)
                    )
                    + "% complete."
                );
    ...
    public static abstract class RemoteAssetRepository implements ConnectionErrorHandler {
        ...
        private final void load() {
            ...
            List<String> li
                = ReikaFileReader.getFileAsLines(url, 10000, true, this, null);
            if (li == null) {
                if (!nonAccessible)
            ...
            this.writeList();
            DragonAPICore.log(
                assets.size() + " remote assets for " + mod.getDisplayName()
                + " found at " + this.getDisplayName() + ": " + assets
            );
        }
        ...
        private void writeList() {
            try {
                ...
                li.add(
                    "File list for remote asset repository '" + this.getDisplayName()
                    + "'"
                );
                li.add(
                    "Downloaded from " + this.getRepositoryURL() + " to "
                ...
                for (RemoteAsset a : assets) {
                    li.add(
                        a.getDisplayName() + " -> " + a.getLocalPath()
                        + " {Size=" + a.data.size + " B,  Hash=" + a.data.hash + "}"
                    );
                }
                ReikaFileReader.writeLinesToFile(f, li, true, Charsets.UTF_8);
                DragonAPICore.log(
                    "Writing file list for remote asset repository '"
                    + this.getDisplayName() + "' to disk."
                );
            } catch (IOException e) {
                DragonAPICore.logError(
                    "Remote asset repository '" + this.getDisplayName()
                    + "' could not save its file list to disk."
                );
                e.printStackTrace();
            }
        }
        ...
        private final void logError(String msg, boolean bigWarn) {
            nonAccessible = true;
            /*mod.getModLogger()*/ DragonAPICore.logError(
                "Error accessing online asset data file: " + msg
            );
            if (bigWarn) {
                instance.bigWarnings.add(
                    new BigWarning("Downloading the remote assets failed: " + msg, this)
                );
            }
        }
