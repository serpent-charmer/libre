        ...
        public static Key readFromConfig(DragonAPIMod mod, StringConfig cfg) {
            if (!cfg.isString())
                throw new MisuseException(
                    mod, "Cannot read a key from a non-string config!"
                );
            ...
            try {
                ...
                throw new InstallationException(
                    mod,
                    "Invalid specified keybind for config entry '" + cfg.getLabel()
                        + "'; no such key '" + s
                        + "' exists! Valid options: " + Arrays.toString(values())
                );
            }
        }
    }
    ...
    private static enum Keys { //THESE NEED TO MATCH THE KEY ENUM
        ...
        private void sendPacket() {
            ...
            if (flag)
                ...
                DragonAPICore.log(
                    "Could not send key " + this + " packet, as it was malformed."
                );
        }
                ...
                for (int i = 0; i < Keys.keyList.length; i++) {
                    ...
                    if (wasPressed != isPressed) {
                        ...
                        MinecraftForge.EVENT_BUS.post(
                            new RawKeyPressEvent(key.getServerKey(), ep)
                        );
                    }
                }
            }
