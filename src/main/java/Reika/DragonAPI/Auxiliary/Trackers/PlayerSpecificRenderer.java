...
public final class PlayerSpecificRenderer {
    ...
    private final MultiMap<UUID, PlayerRenderObj> renders
        = new MultiMap().setNullEmpty().setOrdered(new RenderComparator());
    ...
    private PlayerSpecificRenderer() {
        this.registerRenderer(
            DragonAPICore.Reika_UUID, new PlayerModelRenderer(modelReika)
        );
        this.registerRenderer(
            UUID.fromString("bca741d8-d934-4785-9c26-f6a4141be124"),
            new PlayerModelRenderer(modelSamaki)
        );

        this.registerGlow(DragonAPICore.Reika_UUID, "reika_glow");
        this.registerGlow(
            UUID.fromString("bca741d8-d934-4785-9c26-f6a4141be124"), "samaki_glow"
        );
        this.registerGlow(
            UUID.fromString("d859c5ea-37e9-43d7-b3b9-523e448bfda0"), "frey_glow"
        );
    }
    ...
    public void registerIntercept() {
        ...
        map.put(
            EntityPlayer.class, new CustomPlayerRenderer(map.get(EntityPlayer.class))
        );
    }
    ...
    public void registerRenderer(UUID uuid, PlayerRenderObj r) {
        ...
        //If anyone flips out over this and complains "OMG REIKA GIVES HIMSELF ALL THE
        //RENDERS IN THE DEV ENVIRONMENT! DRM!", You are: If you cannot understand Java:
        //making wild accusations based on your own ignorance If you can understand Java:
        //A disgrace to other programmers for harassing a developer over what you should
        //understand is harmless Update: Someone did it. Congratulations. Now go eat the
        //contents of your toilet. if (DragonAPICore.isReikasComputer() &&
    ...
    public void loadGlowFiles() {
        File f = new File(
            DragonAPICore.getMinecraftDirectory(), "config/Reika/glowrenders.dat"
        );
        if (f.exists()) {
            ...
            for (String s : li) {
                try {
                    ...
                    File img = new File(
                        ...
                        "config/Reika/glowrenders/" + parts[1]
                    );
                    if (!img.exists())
    ...
    private void renderAdditionalObjects(EntityPlayer ep, float ptick) {
        ...
        if (ep == Minecraft.getMinecraft().thePlayer
            && !DragonOptions.CUSTOMRENDER.getState())
            return;
    ...
    private static final class CustomPlayerRenderer extends RenderPlayer {
        ...
        @Override
        protected void
        rotateCorpse(EntityLivingBase ep, float par2, float par3, float partialTick) {
            ...
            if (MinecraftForgeClient.getRenderPass() == 1
                ...
                PlayerSpecificRenderer.instance.renderAdditionalObjects(
                    (EntityPlayer) ep, partialTick
                );
        }
        ...
        private void renderWithoutTextureBind(
            EntityLivingBase ep,
            float f1,
            float f2,
            float f3,
    ...
    public static class PlayerRotationData {
        ...
        private PlayerRotationData(EntityPlayer ep, float ptick) {
            ...
            renderYawHead = -ep.rotationYaw % 360
                - partialTick * (ep.rotationYaw - ep.prevRotationYaw);
            renderYaw = -ep.renderYawOffset % 360
                - partialTick * (ep.renderYawOffset - ep.prevRenderYawOffset) + 180;

            interpYaw = prevRotationYaw + (rotationYaw - prevRotationYaw) * partialTick;
            interpYawHead = prevRotationYawHead
                + (rotationYawHead - prevRotationYawHead) * partialTick;
            interpPitch
                = prevRotationPitch + (rotationPitch - prevRotationPitch) * partialTick;
