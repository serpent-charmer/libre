...
public final class CommandableUpdateChecker {
    public static final CommandableUpdateChecker instance
        = new CommandableUpdateChecker();

    public static final String reikaURL
        = "https://reikasminecraft.overminddl1.com/versions";
    ...
    public void checkAll() {
        Runnable r = () -> {
            for (UpdateChecker c : checkers) {
                ...
                if (latest == null) {
                    c.mod.getModLogger().logError(
                        "Could not access online version reference. Please notify "
                        + c.mod.getModAuthorName()
                    );
                    return;
                } else {
                    c.mod.getModLogger().log(
                        "Found latest mod version: " + latest + " compared to current "
                        + c.mod.getModVersion()
                    );
                }
                ...
                if (this.shouldCheck(mod)) {
                    ...
                    if (latest instanceof ErroredVersion) {
                        ...
                        ReikaJavaLibrary.pConsole(
                            "-----------------------" + mod.getTechnicalName()
                            + "-----------------------"
                        );
                        ReikaJavaLibrary.pConsole(
                            "Could not connect to version server. Please check your internet settings,"
                        );
                        ReikaJavaLibrary.pConsole(
                            "and if the server is unavailable please contact "
                        ...
                        ReikaJavaLibrary.pConsole(
                            "This version of the mod (" + version + ") is out of date."
                        );
                        ReikaJavaLibrary.pConsole(
                            "This version is likely to contain bugs, crashes, and/or exploits."
                        );
                        ReikaJavaLibrary.pConsole(
                            "No technical support whatsoever will be provided for this version."
                        );
                        ReikaJavaLibrary.pConsole(
                            "Update to " + latest
                            + " as soon as possible; there is no good reason not to."
                        );
                        ReikaJavaLibrary.pConsole(
                            "------------------------------------------------------------------------"
                        );
    ...
    private void markUpdate(DragonAPIMod mod, ModVersion version, ModVersion latest) {
        if (latest instanceof ErroredVersion) {
            ...
            FMLInterModComms.sendRuntimeMessage(
                mod.getModContainer().getModId(), "VersionChecker", "addUpdate", nbt
            );
        }
    }
    ...
    public void registerMod(DragonAPIMod mod) {
        ...
        if (mod.getUpdateCheckURL() == null)
            return;
        String url = mod.getUpdateCheckURL() + "_"
            + Loader.MC_VERSION.replaceAll("\\.", "-") + ".txt";
        URL file = this.getURL(url);
        if (file == null) {
            mod.getModLogger().logError(
                "Could not create URL to update checker. Version will not be checked."
            );
            return;
        }
        UpdateChecker c = new UpdateChecker(mod, version, file);
        checkers.add(c);
        String label = ReikaStringParser.stripSpaces(
            mod.getDisplayName().toLowerCase(Locale.ENGLISH)
        );
    ...
    private void setChecker(DragonAPIMod mod, boolean enable) {
        File f = this.getFile();
        String name = ReikaStringParser.stripSpaces(
            mod.getDisplayName().toLowerCase(Locale.ENGLISH)
        );
        ModVersion latest = latestVersions.get(mod);
        if (f.exists()) {
        ...
        if (ep instanceof EntityPlayerMP) {
            ...
            for (DragonAPIMod mod : oldMods) {
                if (this.beAggressive(mod, (EntityPlayerMP) ep)) {
                    ReikaPacketHelper.sendStringPacket(
                        ...
                        PacketIDs.OLDMODS.ordinal(),
                        modNamesReverse.get(mod),
                        pt
                    );
                }
            }
            for (Entry<DragonAPIMod, String> e : noURLMods.entrySet()) {
                ReikaPacketHelper.sendStringPacket(
                    DragonAPIInit.packetChannel,
                    PacketIDs.OLDMODS.ordinal(),
                    "URL_" + modNamesReverse.get(e.getKey()) + "::" + e.getValue(),
                    pt
                );
            }
        }
    ...
    private boolean beAggressive(DragonAPIMod mod, EntityPlayerMP ep) { /*
         boolean abandonedPack =
         latestVersions.get(mod).majorVersion-mod.getModVersion().majorVersion > 1; if
         (!abandonedPack && DragonOptions.PACKONLYUPDATE.getState()) { return
         this.isPackMaker(mod, ep);
         }
         else */
    ...
    private UpdateHash genHash(DragonAPIMod mod, EntityPlayer ep) {
        return new UpdateHash(
            ep.getUniqueID(),
            ReikaFileReader.getRealPath(mod.getModContainer().getSource()),
    ...
    public void onClientReceiveOldModsNote(ClientLoginEvent evt) {
        if (evt.newLogin) {
            ...
            for (Entry<DragonAPIMod, String> e : erroredMods.entrySet()) {
                ...
                sb.append(
                    " could not verify its version; " + err
                    + ". Check your internet settings, and please notify "
                );
    ...
    private void sendMessages(EntityPlayer ep) {
        ...
        String sg2 = g + "To disable this notifcation for any mod, type \"/"
            + CheckerDisableCommand.tag + " disable [modname]\".";
    ...
    private String getChatMessage(DragonAPIMod mod) {
        ...
        return g + mod.getDisplayName() + r
            + " is out of date, likely has errors, and is no longer supported. Update to "
            + latest + ".";
    }
            ...
            if (args.length == 2) {
                ...
                if (mod != null) {
                    if (action.equals("disable")) {
                        ...
                        String sg = EnumChatFormatting.BLUE.toString()
                            + "Update checker for " + mod.getDisplayName() + " disabled.";
                        ReikaChatHelper.sendChatToPlayer(ep, sg);
                    } else if (action.equals("enable")) {
                        instance.setChecker(mod, true);
                        String sg = EnumChatFormatting.BLUE.toString()
                            + "Update checker for " + mod.getDisplayName() + " enabled.";
                        ReikaChatHelper.sendChatToPlayer(ep, sg);
                    } else {
                        String sg = EnumChatFormatting.RED.toString()
                            + "Invalid argument '" + action + "'.";
                        ReikaChatHelper.sendChatToPlayer(ep, sg);
                    }
                } else {
                    String sg = EnumChatFormatting.RED.toString() + "Mod '" + name
                        + "' not found.";
                    ReikaChatHelper.sendChatToPlayer(ep, sg);
                }
            } else {
    ...
    private static class UpdateChecker implements ConnectionErrorHandler, DataFetcher {
        ...
        private ModVersion fetchLatestVersion() {
            try {
                List<String> lines
                    = ReikaFileReader.getFileAsLines(checkURL, 10000, false, this, this);
        ...
        public void onNoInternet() {
            mod.getModLogger().logError(
                "Error accessing online file: Is your internet disconnected?"
            );
        }
        ...
        public void onCertificateFailed() {
            throw new VersionNotLoadableException(
                "Version server could not be contacted: HTTPS certificate issues"
            );
        }
        ...
        public void fetchData(URLConnection c) throws Exception {
            ...
            modified = new SimpleDateFormat("E, d MMM yyyy HH:mm:ss Z", Locale.ENGLISH)
                           .parse(lastModified);
        }
    }
    ...
    private static class UpdateHash {
        ...
        public String toString() {
            ...
            while (idx < time.length() || idx < id.length() || idx < filepath.length()) {
                ...
                idx++;
                //ReikaJavaLibrary.pConsole(c1+" & "+c2+" & "+c3+" > "+sum+" $
                //"+this.getStringForInt(sum));
                sb.append(this.getStringForInt(sum) + ":");
            }
            //ReikaJavaLibrary.pConsole("Final: "+time+" & "+id+" & "+filepath+" >
            //"+sb.toString());
            return sb.toString();
        }
