...
public class PopupWriter {
    ...
    public void drawOverlay(RenderGameOverlayEvent evt) {
        if (!list.isEmpty() && evt.type == ElementType.HELMET) {
            ...
            ReikaTextureHelper.bindFinalTexture(
                DragonAPICore.class, "Resources/buttons.png"
            );
            v5.startDrawingQuads();
    ...
    public void click(MouseEvent evt) {
        if (!list.isEmpty() && evt.buttonstate && evt.button == 0 && ungrabbed
            && buttonX > 0 && buttonY > 0) {
            int x = evt.x;
            int y = Minecraft.getMinecraft().displayHeight - evt.y;
            //ReikaJavaLibrary.pConsole(x+","+y+ " / "+buttonX+","+buttonY+ " ?
            //"+(x/(double)buttonX)+", "+(y/(double)buttonY));
    ...
    public static class Warning {
        ...
        private static int calcMinSizeForText(String s
        ) { //at w=192, 74 chars becomes 4 lines, or about 18 chars a line (1 char =
            //11px); ideally keep line count <= 6
            int w = 192;
            int c = 18;
