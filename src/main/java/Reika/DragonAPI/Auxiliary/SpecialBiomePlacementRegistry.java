...
public class SpecialBiomePlacementRegistry {
    public static final SpecialBiomePlacementRegistry instance
        = new SpecialBiomePlacementRegistry();
    ...
    /**
     * Note well: You are REPLACING some of the placement regions of the default biome of
     * the category; each variant is 1/16th of it.
     */
    public void registerID(DragonAPIMod mod, Category c, int idx, int biomeID) {
        if (idx < 0 || idx >= VARIANTS)
            throw new RegistrationException(
                mod, "Special biome variant indices can only be within 0-15!"
            );
        if (IDs.containsKeyV(c, idx))
            throw new RegistrationException(
                mod,
                "Special biome variant " + idx + " for " + c + " is already occupied!"
            );
