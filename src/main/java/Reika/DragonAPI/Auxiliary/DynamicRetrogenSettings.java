...
public class DynamicRetrogenSettings {
    ...
    public void loadConfig() {
        File f = new File(
            DragonAPIInit.config.getConfigFolder(), "DragonAPI_Aux_Retrogen.cfg"
        );
        ...
        for (String s : li) {
            ...
            try {
                ...
                throw new InstallationException(
                    ...
                    "Invalid weight '" + parts[1] + "' for generator '" + parts[0] + "'!"
                );
            }
            if (RetroGenController.instance.getActiveRetroGenerators().contains(parts[0]
                ...
                throw new InstallationException(
                    DragonAPIInit.instance,
                    "Retrogen for generator '" + parts[0] + "' already active!"
                );
            }
            Generator gen = this.createGenerator(parts[0], wt);
            if (gen == null) {
                throw new InstallationException(
                    DragonAPIInit.instance, "Generator '" + parts[0] + "' not found!"
                );
            }
    ...
    private static class Generator implements RetroactiveGenerator {
        ...
        public boolean canGenerateAt(World world, int chunkX, int chunkZ) {
            return generator instanceof RetroactiveGenerator
                ? ((RetroactiveGenerator) generator).canGenerateAt(world, chunkX, chunkZ)
                : true;
        }
