...
public class LoggingFilters {
    ...
    public abstract static class CoreFilter implements Filter {
        ...
        /**
         * Return null to silence the message. Return anything else to replace the
         * message, except an empty string for "original message"
         */
        ...
        protected String parse(Message msg, Level lvl) {
            ...
            if (sg.contains(PREFIX)) {
                ...
                ReikaJavaLibrary.pConsole(
                    "ERROR: Texture Map could not find texture '" + tex
                    + "'; File not found."
                );
                return null; //"ERROR: Texture Map could not find texture '"+tex+"'; File
                             //not found.";
            }
            return "";
        }
    ...
    public static void registerFilter(Filter f, LoggerType type) {
        ...
        if (log != null) {
            ...
            throw new RuntimeException(
                "Logger interception is not working! Notify Reika immediately!"
            );
        }
    }
