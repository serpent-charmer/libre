    ...
    public void addLogger(DragonAPIMod mod, String label) {
        ...
        if (loggers.containsKey(label))
            throw new RegistrationException(
                mod, "Modular logger name '" + label + "' is already taken!"
            );
    ...
    public void log(String logger, String msg) {
        ...
        if (e == null) {
            DragonAPICore.logError(
                "Tried to use an unregistered logger '" + logger + "'!"
            );
        } else {
            if (e.enabled) {
        ...
        public void processCommand(ICommandSender ics, String[] args) {
            if (args.length != 2) {
                this.sendChatToSender(
                    ics,
                    EnumChatFormatting.RED + "You must specify a logger ID and a status!"
                );
                return;
            }
            ...
            if (e == null) {
                this.sendChatToSender(
                    ics,
                    EnumChatFormatting.RED + "Unrecognized logger ID '" + args[0] + "'!"
                );
                return;
            }
            e.enabled = args[1].equalsIgnoreCase("yes")
                || args[1].equalsIgnoreCase("enable") || args[1].equalsIgnoreCase("1")
                || Boolean.parseBoolean(args[1]);
            String status = e.enabled ? "enabled" : "disabled";
            this.sendChatToSender(
                ics, EnumChatFormatting.GREEN + "Logger '" + args[0] + "' " + status + "."
            );
            ReikaPacketHelper.sendStringIntPacket(
                DragonAPIInit.packetChannel,
                PacketIDs.MODULARLOGGER.ordinal(),
