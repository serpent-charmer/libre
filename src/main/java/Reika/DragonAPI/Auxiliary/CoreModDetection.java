...
public enum CoreModDetection {
    ...
    LITELOADER(
        ...
        "sounds and textures",
        "Try reloading resources (F3+T) to fix this."
    ),
    ...
    COLOREDLIGHTS(
        "coloredlightscore.src.asm.ColoredLightsCoreLoadingPlugin", "rendering", ""
    ),
        ...
        for (int i = 0; i < values().length; i++) {
            ...
            if (c.isInstalled()) {
                ReikaJavaLibrary.pConsole(
                    "DRAGONAPI: " + c + " detected. Loading compatibility features."
                );
                ReikaJavaLibrary.pConsole(
                    "\t\tNote that some parts of the game, especially " + c.warning
                    + ", may error out."
                );
