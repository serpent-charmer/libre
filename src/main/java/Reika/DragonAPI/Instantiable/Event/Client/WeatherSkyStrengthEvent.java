...
public class WeatherSkyStrengthEvent extends Event {
    ...
    public static float fire_Rain(World world, float ptick) {
        WeatherSkyStrengthEvent evt
            = new WeatherSkyStrengthEvent(world, world.getRainStrength(ptick), ptick);
        MinecraftForge.EVENT_BUS.post(evt);
        return evt.returnValue;
    }

    public static float fire_Thunder(World world, float ptick) {
        WeatherSkyStrengthEvent evt = new WeatherSkyStrengthEvent(
            world, world.getWeightedThunderStrength(ptick), ptick
        );
        MinecraftForge.EVENT_BUS.post(evt);
        return evt.returnValue;
    }
