...
public class SoundVolumeEvent extends Event {
    ...
    private float getDefaultVolume(ISound snd, SoundPoolEntry pool, SoundCategory cat) {
        return (float) MathHelper.clamp_double(
            snd.getVolume() * pool.getVolume() * this.getSoundCategoryVolume(cat),
            0.0D,
            1.0D
        );
    }
    ...
    private float getSoundCategoryVolume(SoundCategory cat) {
        return cat != null && cat != SoundCategory.MASTER
            ? Minecraft.getMinecraft().gameSettings.getSoundLevel(cat)
            : 1.0F;
    }
