...
public class LightVisualBrightnessEvent extends WorldPositionEvent {
    ...
    public static float fire(float orig, World world, int x, int y, int z) {
        LightVisualBrightnessEvent evt = new LightVisualBrightnessEvent(world, x, y, z,
    orig); return evt.brightness;
    }
     */

    public static float fire(World world, int x, int y, int z) {
        int light = world.getBlockLightValue(x, y, z);
        LightVisualBrightnessEvent evt
            = new LightVisualBrightnessEvent(world, x, y, z, light);
        MinecraftForge.EVENT_BUS.post(evt);
        return evt.brightness;
    }

    public static int
    fireMixed(int orig, IBlockAccess world, int x, int y, int z, int sky, int block) {
        LightMixedBrightnessEvent evt
            = new LightMixedBrightnessEvent(world, x, y, z, block, sky, orig);
        MinecraftForge.EVENT_BUS.post(evt);
        return evt.value;
    }
    ...
    public static class LightMixedBrightnessEvent extends PositionEventBase {
        ...
        public LightMixedBrightnessEvent(
            IBlockAccess world, int x, int y, int z, int bl, int skyl, int orig
        ) {
            super(world, x, y, z);
            blockLight = bl;
            skyLight = skyl;
