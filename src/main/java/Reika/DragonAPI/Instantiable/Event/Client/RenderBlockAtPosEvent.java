...
public class RenderBlockAtPosEvent {
    ...
    public static void addListener(BlockRenderWatcher l) {
        listeners
            = ReikaArrayHelper.addToFastArray(listeners, l, BlockRenderWatcher.class);
        Arrays.sort(listeners, EventWatchers.comparator);
        if (l instanceof AdvancedBlockRenderWatcher) {
            advancedListeners = ReikaArrayHelper.addToFastArray(
                advancedListeners,
                (AdvancedBlockRenderWatcher) l,
                AdvancedBlockRenderWatcher.class
            );
    ...
    fire(RenderBlocks rb, Block b, int x, int y, int z, WorldRenderer wr, int pass) {
        if (listeners != null) {
    ...
    /*
    @SideOnly(Side.CLIENT)
    public static int getMixedBrightnessForBlock(Block b, IBlockAccess iba, int x, int y,
    int z) { if (listeners != null) { for (BlockRenderWatcher l : listeners) { if
    (l.onBlockTriedRender(b, x, y, z, wr, rb, pass)) { continueRendering = false; return
    false;
                }
            }
        }
    ...
    public static interface BlockRenderWatcher extends EventWatcher {
        ...
        @SideOnly(Side.CLIENT)
        boolean onBlockTriedRender(
            Block b, int x, int y, int z, WorldRenderer wr, RenderBlocks rb, int pass
        );
    }
    ...
    public static interface AdvancedBlockRenderWatcher extends BlockRenderWatcher {
        ...
        //boolean getMixedBrightnessForBlock(Block b, IBlockAccess iba, int x, int y, int
        //z);
    }
}
