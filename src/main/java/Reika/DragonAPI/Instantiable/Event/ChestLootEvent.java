...
public class ChestLootEvent extends Event {
    ...
    public static void fire(
        Object struct, IInventory ii
    ) { //called after the call to WeightedRandomChestContent.generateChestContents inside
        ...
        if (table != null) {
            ...
            //ReikaJavaLibrary.pConsole("Generated a "+table+" in "+(struct instanceof
            //StructureComponent ? struct+" -
            //"+((StructureComponent)struct).getBoundingBox() : struct.toString()));
        } else {
            DragonAPICore.logError(
                "Tried to fire an event for chest loot from an unrecognized structure "
                + struct + "!"
            );
        }
    }
