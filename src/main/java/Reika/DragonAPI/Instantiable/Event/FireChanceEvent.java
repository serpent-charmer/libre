...
public class FireChanceEvent extends WorldPositionEvent {
    /**
     * An arbitrarily large number (ie not a 0-1 decimal); higher means more chance to
     * spread. Values of 400 or so guarantee a spread event.
     */
    public int spreadChance;
    public final int baseSpreadChance;

    /**
     * In some versions of Minecraft/Forge, this is <i>always</i> "UP", so do not rely on
     * it to be accurate.
     */
    ...
    public FireChanceEvent(
        World world, int x, int y, int z, ForgeDirection side, int base
    ) {
        super(world, x, y, z);
    ...
    public static int
    fire(World world, int x, int y, int z, ForgeDirection side, int base) {
        FireChanceEvent evt = new FireChanceEvent(world, x, y, z, side, base);
        MinecraftForge.EVENT_BUS.post(evt);
        lastRatio = evt.baseSpreadChance > 0
            ? (float) evt.spreadChance / evt.baseSpreadChance
            : 0;
