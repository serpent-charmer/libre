...
/**
 * Do NOT register global or persistent handlers here, or you will make ALL TEs run
 * through it and hurt performance!
 */
public class TileUpdateEvent {
    ...
    public static boolean fire(TileEntity te) {
        if (te.isInvalid() || !te.hasWorldObj()
            || !te.worldObj.blockExists(te.xCoord, te.yCoord, te.zCoord))
            return true;
        if (watchers == null)
            return false;
