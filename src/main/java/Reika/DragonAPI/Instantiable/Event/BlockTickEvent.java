...
/**
 * This method is fired whenever blocks are ticked via updateTick() via ambient world
 * block ticks and any of my mods' code running forced ticks. The event may or may not be
 * cancelable (check isCancelable() first), in which case the tick will not occur.
 */
public class BlockTickEvent extends WorldPositionEvent {
    ...
    public static void
    fire(World world, int x, int y, int z, Random rand, UpdateFlags flag) {
        fire(world.getBlock(x, y, z), world, x, y, z, rand, flag);
    }

    public static void
    fire(Block b, World world, int x, int y, int z, Random rand, UpdateFlags flag) {
        fire(b, world, x, y, z, rand, flag.flag);
    }

    public static void
    fire(Block b, World world, int x, int y, int z, Random rand, int flags) {
        if (!disallowAllUpdates && canTickAt(b, world, x, y, z, flags)
    ...
    canTickAt(Block b, World world, int x, int y, int z, int flags) {
        if (UpdateFlags.FORCED.isFlagPresent(flags)
            || UpdateFlags.REQUIRE.isFlagPresent(flags))
            return true;
        return DragonOptions.STOPUNLOADSPREAD.getState() && requireLoadedArea(b)
            ? hasLoadedRadius(world, x, y, z)
            : true;
    }

    private static boolean requireLoadedArea(Block b) {
        return ReikaBlockHelper.isLiquid(b) || b instanceof BlockGrass
