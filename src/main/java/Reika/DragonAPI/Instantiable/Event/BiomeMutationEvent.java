...
public class BiomeMutationEvent extends Event {
    ...
    fireTry(GenLayer gen, int chunkX, int chunkZ, int x, int z, int originalID) {
        BiomeMutationEvent evt = new BiomeMutationEvent(
            gen, (chunkX << 4) + x, (chunkZ << 4) + z, originalID
        );
        return !MinecraftForge.EVENT_BUS.post(evt)
            && BiomeGenBase.biomeList[evt.originalMutatedID] != null
            && isValidMutant(originalID, evt.originalMutatedID);
    }
    ...
    private static boolean isValidMutant(int originalID, int originalMutatedID) {
        ...
        return child instanceof BiomeGenMutated
    ...
    public static class GetMutatedBiomeEvent extends BiomeMutationEvent {
        ...
        public static int
        fireGet(GenLayer gen, int chunkX, int chunkZ, int x, int z, int originalID) {
            GetMutatedBiomeEvent evt = new GetMutatedBiomeEvent(
                gen, (chunkX << 4) + x, (chunkZ << 4) + z, originalID
            );
