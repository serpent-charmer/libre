...
public class Spline {
    ...
    public List<DecimalPosition> get(int fineness, boolean closed) {
        if (fineness < 2) {
            throw new IllegalArgumentException(
                "The fineness parameter must be greater than 2 (current " + fineness
                + "), since 2 points is just the linear segment."
            );
        }
    ...
    private List<DecimalPosition> interpolate(int fine, boolean closed) {
        ...
        if (closed) {
            /**
            Reika fix: this algorithm above assumes that "closed" is only and always true
            if start and end points match, rather than just a boolean flag indicating "try
            and close the loop". As such it needs correction, though the general logic
            still applies.
             */
     ...
     * DecimalPositioninate that is only used for controling the spline and is not
     * visualized.
     ...
     */
    private List<DecimalPosition>
    interpolatePoints(List<DecimalPosition> points, int index, int pointsPerSegment) {
        List<DecimalPosition> result = new ArrayList();
        ...
        for (int i = 1; i < segments; i++) {
            double xi
                = this.interpolate(x, time, tstart + (i * (tend - tstart)) / segments);
            double yi
                = this.interpolate(y, time, tstart + (i * (tend - tstart)) / segments);
            double zi = this.interpolate(
                z, time, tstart + (i * (tend - tstart)) / segments
            ); //z1+(dz*i)/segments;
    ...
    private double interpolate(double[] p, double[] time, double t) {
        double L01 = p[0] * (time[1] - t) / (time[1] - time[0])
            + p[1] * (t - time[0]) / (time[1] - time[0]);
        double L12 = p[1] * (time[2] - t) / (time[2] - time[1])
            + p[2] * (t - time[1]) / (time[2] - time[1]);
        double L23 = p[2] * (time[3] - t) / (time[3] - time[2])
            + p[3] * (t - time[2]) / (time[3] - time[2]);
        double L012 = L01 * (time[2] - t) / (time[2] - time[0])
            + L12 * (t - time[0]) / (time[2] - time[0]);
        double L123 = L12 * (time[3] - t) / (time[3] - time[1])
            + L23 * (t - time[1]) / (time[3] - time[1]);
        double C12 = L012 * (time[2] - t) / (time[2] - time[1])
    ...
    public static class BasicSplinePoint implements SplineAnchor {
        ...
        public final DecimalPosition asPosition() {
            return relative != null ? relative.offset(posX, posY, posZ)
                                    : new DecimalPosition(posX, posY, posZ);
        }
    ...
    public static class BasicVariablePoint extends BasicSplinePoint {
        ...
        private void move(double dx, double dy, double dz) {
            //ReikaJavaLibrary.pConsole(dx+":"+dy+":"+dz+" from
