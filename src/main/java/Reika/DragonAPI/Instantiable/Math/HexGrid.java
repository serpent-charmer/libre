...
public class HexGrid {
    private static final List<Hex> directions
        = Collections.unmodifiableList(ReikaJavaLibrary.makeListFrom(
            new Hex(1, 1, -1, 0),
            new Hex(1, 0, -1, 1),
            new Hex(1, -1, 0, 1),
            new Hex(1, -1, 1, 0),
    ...
    private static final List<Hex> diagonals
        = Collections.unmodifiableList(ReikaJavaLibrary.makeListFrom(
            new Hex(1, 2, -1, -1),
            new Hex(1, 1, -2, 1),
            new Hex(1, -1, -1, 2),
    ...
    private static final Orientation angled = new Orientation(
        Math.sqrt(3D),
        Math.sqrt(3D) / 2D,
        0D,
        3D / 2D,
    ...
    private static final Orientation flat = new Orientation(
        3D / 2D,
        0D,
        Math.sqrt(3D) / 2D,
        Math.sqrt(3D),
    ...
    public Hex getHex(int q, int r, int s) {
        return hexes.get(new Coordinate(q, r, s)
        ); //this.getHex(shape.getI(q, r, s), shape.getK(q, r, s));
    }

    public Hex getRandomEdgeCell(Random rand
    ) { //alternative: keep picking random cells until find one without six neighbors
    ...
    public void drawFilledHex(Tessellator v5, Hex h, int color) {
        ...
        for (double a = Math.toDegrees(style.start_angle); a <= 360 + style.start_angle;
             a += 60) {
            double dx = r + 0.1 + r * Math.cos(Math.toRadians(a));
            double dy = r - 1 + r * Math.sin(Math.toRadians(a));
            v5.addVertex(dx, dy, 0);
    ...
    public void drawTexturedGrid(Tessellator v5) {
        ...
        for (Hex h : hexes.values()) {
            ...
            for (double a = Math.toDegrees(style.start_angle);
                 a <= 360 + style.start_angle;
                 a += 60) {
...
public ArrayList<Point> polygonCorners(Hex h) {
    ...
    private static final class FractionalHex {
        ...
        public static FractionalHex
        hexLerp(double size, FractionalHex a, FractionalHex b, double t) {
        ...
        public static ArrayList<Hex> hexLinedraw(double size, Hex a, Hex b) {
            ...
            FractionalHex a_nudge = new FractionalHex(
                a.size, a.q + 0.000001, a.r + 0.000001, a.s - 0.000002
            );
            FractionalHex b_nudge = new FractionalHex(
                a.size, b.q + 0.000001, b.r + 0.000001, b.s - 0.000002
            );
            ArrayList<Hex> results = new ArrayList();
            double step = 1.0 / Math.max(N, 1);
            for (int i = 0; i <= N; i++) {
                results.add(
                    FractionalHex.hexLerp(size, a_nudge, b_nudge, step * i).hexRound()
                );
            }
            return results;
        }
    ...
    public static final class Point {
        ...
        public Point rotate(double r, int ox, int oz) {
            Vec3 ret = ReikaVectorHelper.rotateVector(
                Vec3.createVectorHelper(x - ox, 0, y - oz), 0, r, 0
            );
