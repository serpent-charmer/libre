...
/**
 * Adapted from the Open Simplex Noise Generator by Kurt Spencer, stripped to only the 2D
 * system and reworked for MC worldgen application. All '//' comments are his.
 */
public class SimplexNoiseGenerator extends NoiseGeneratorBase {
    ...
    //Generates a proper permutation (i.e. doesn't merely perform N successive pair swaps
    //on a base array) Uses a simple 64-bit LCG.
    ...
    calcValue(double x, double y, double z, double f, double a, Object obj) {
        ...
        if (inSum <= 1) { //We're inside the triangle (2-Simplex) at (0,0)
            double dins = 1 - inSum;
            if (dins > xins
                || dins > zins) { //(0,0) is one of the closest two triangular vertices
                if (xins > zins) {
                    xsv_ext = xsb + 1;
                    zsv_ext = zsb - 1;
                    dx_ext = dx0 - 1;
        ...
        if (attn_ext > 0) {
            ...
            value += attn_ext * attn_ext
                * this.extrapolate(xsv_ext, zsv_ext, dx_ext, dz_ext);
        }
