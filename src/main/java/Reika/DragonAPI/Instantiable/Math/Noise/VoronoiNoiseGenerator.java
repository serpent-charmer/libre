...
public class VoronoiNoiseGenerator extends NoiseGeneratorBase {
    ...
    private int IntValueNoise3D(int x, int y, int z, int rseed) {
        ...
        int n = (X_NOISE_GEN * x + Y_NOISE_GEN * y + Z_NOISE_GEN * z
                 + SEED_NOISE_GEN * rseed)
            & 0x7fffffff;
        n = (n >> 13) ^ n;
    ...
    @Override
    protected double
    calcValue(double x, double y, double z, double f, double a, Object obj) {
        if (f != 1 && f > 0) {
            x *= f;
            y *= f;
            z *= f;
        ...
        for (int zCur = zInt - 2; zCur <= zInt + 2; zCur++) {
            for (int yCur = yInt - 2; yCur <= yInt + 2; yCur++) {
                for (int xCur = xInt - 2; xCur <= xInt + 2; xCur++) {
                    ...
                    // this unit cube.
                    double xPos = xCur
                        + randomFactor * this.ValueNoise3D(xCur, yCur, zCur, (int) seed);
                    double yPos = yCur
                        + randomFactor
                            * this.ValueNoise3D(xCur, yCur, zCur, (int) seed + 1);
                    double zPos = zCur
                        + randomFactor
                            * this.ValueNoise3D(xCur, yCur, zCur, (int) seed + 2);
                    double xDist = xPos - x;
                    double yDist = yPos - y;
                    double zDist = zPos - z;
        ...
        return value
            + this.ValueNoise3D(
                MathHelper.floor_double(candidate.position.xCoord),
                MathHelper.floor_double(candidate.position.yCoord),
                MathHelper.floor_double(candidate.position.zCoord),
                0
            );
    }
    ...
    public DecimalPosition getClosestRoot(double x, double y, double z) {
        ...
        return new DecimalPosition(
            raw.xCoord / inputFactor, raw.yCoord / inputFactor, raw.zCoord / inputFactor
        );
    }
            ...
            for (int yCur = yInt - 2; yCur <= yInt + 2; yCur++) {
                for (int xCur = xInt - 2; xCur <= xInt + 2; xCur++) {
                    ...
                    double zPos = zCur
                        + randomFactor
                            * this.ValueNoise3D(xCur, yCur, zCur, (int) seed + 2);
                    ret.add(new DecimalPosition(
                        xPos / inputFactor, yPos / inputFactor, zPos / inputFactor
                    ));
                }
            }
        }
    ...
    public Collection<DecimalPosition>
    getCellsWithin3D(double x, double y, double z, double r) {
        x *= inputFactor;
        y *= inputFactor;
        z *= inputFactor;
        r *= inputFactor;
            ...
            for (int yCur = yInt - dr; yCur <= yInt + dr; yCur++) {
                for (int xCur = xInt - dr; xCur <= xInt + dr; xCur++) {
                    ...
                    if (d.getDistanceTo(x, y, z) <= r)
                        ret.add(new DecimalPosition(
                            d.xCoord / inputFactor,
                            d.yCoord / inputFactor,
                            d.zCoord / inputFactor
                        ));
                }
            }
        }
    ...
    public Collection<DecimalPosition> getCellsWithin2D(double x, double z, double r) {
        ...
        for (int zCur = zInt - dr; zCur <= zInt + dr; zCur++) {
            for (int xCur = xInt - dr; xCur <= xInt + dr; xCur++) {
                double xPos
                    = xCur + randomFactor * this.ValueNoise3D(xCur, 0, zCur, (int) seed);
                double zPos
                    = zCur + randomFactor * this.ValueNoise3D(xCur, 0, zCur, (int) ~seed);
                DecimalPosition d = new DecimalPosition(xPos, 0, zPos);
                if (d.getDistanceTo(x, 0, z) <= r)
                    ret.add(new DecimalPosition(
                        d.xCoord / inputFactor, 0, d.zCoord / inputFactor
                    ));
            }
        }
    ...
    public double getEdgeRatio(double x, double z) {
        ...
        // dist from point to line defined by two points:
    ...
    public boolean chunkContainsCenter(int x, int z) {
        ...
        return pos.xCoord >= x && pos.zCoord >= z && pos.xCoord < x + 16
            && pos.yCoord < z + 16;
    }
