...
public abstract class NoiseGeneratorBase {
    ...
    public final double getValue(double x, double y, double z) {
        return this.calculateValues(
            x * inputFactor, y * inputFactor, z * inputFactor, null
        );
    }
    ...
    calcValue(double x, double y, double z, double freq, double amp, Object obj);
    ...
    public final NoiseGeneratorBase
    addOctave(double relativeFrequency, double relativeAmplitude) {
        return this.addOctave(relativeFrequency, relativeAmplitude, 0);
    }

    public final NoiseGeneratorBase
    addOctave(double relativeFrequency, double relativeAmplitude, double phaseShift) {
    ...
    public final NoiseGeneratorBase
    setDisplacementSimple(long seedX, double fx, long seedZ, double fz, double s) {
        return this.setDisplacement(
            new SimplexNoiseGenerator(seedX).setFrequency(fx),
            s,
            null,
            s,
            new SimplexNoiseGenerator(seedZ).setFrequency(fz),
            s
        );
    }

    public final NoiseGeneratorBase setDisplacementSimple(
        long seedX, double fx, long seedY, double fy, long seedZ, double fz, double s
    ) {
        return this.setDisplacement(
            new SimplexNoiseGenerator(seedX).setFrequency(fx),
            s,
            new SimplexNoiseGenerator(seedY).setFrequency(fy),
            s,
            new SimplexNoiseGenerator(seedZ).setFrequency(fz),
            s
        );
    ...
    public final NoiseGeneratorBase setDisplacement(
        NoiseGeneratorBase x, NoiseGeneratorBase y, NoiseGeneratorBase z, double s
    ) {
        return this.setDisplacement(x, s, y, s, z, s);
    }

    public final NoiseGeneratorBase setDisplacement(
        NoiseGeneratorBase x,
        double xs,
        NoiseGeneratorBase y,
        double ys,
        NoiseGeneratorBase z,
        double zs
    ) {
        xNoise = x;
        yNoise = y;
