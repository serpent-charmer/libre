...
/**
 * Cloned from the vanilla FontRenderer to allow for rewrites and to avoid Optifine
 * compat issues
 */
@SideOnly(Side.CLIENT)
public abstract class BasicFontRenderer
    extends FontRenderer implements IResourceManagerReloadListener {
    ...
    /**
     * Array of the start/end column (in upper/lower nibble) for every glyph in the /font
     * directory.
     */
    protected byte[] glyphWidth = new byte[65536];
    /**
     * Array of RGB triplets defining the 16 standard chat colors followed by 16 darker
     * version of the same colors for drop shadows.
     */
    ...
    /**
     * If true, strings should be rendered with Unicode fonts instead of the default.png
     * font
     */
    private boolean unicodeFlag;
    /**
     * If true, the Unicode Bidirectional Algorithm should be run before rendering any
     * string.
     */
    ...
    protected float renderCharAtPos(int charIndex, char c, boolean italic, int index) {
        if (wipe != null && wipe.lastChar == index + 1) {
            GL11.glColor4f(
                ReikaColorAPI.getRed(wipe.wipeColor) / 255F,
                ReikaColorAPI.getGreen(wipe.wipeColor) / 255F,
                ReikaColorAPI.getBlue(wipe.wipeColor) / 255F,
                ReikaColorAPI.getAlpha(wipe.wipeColor) / 255F
            );
        } else {
        ...
        return c == 32
            ? 4.0F
            : (!unicodeFlag && charMap.contains(c)
                   ? this.renderCharFraction(charIndex, italic, this.getFraction(index))
                   : this.renderUnicodeChar(c, italic));
    }
    ...
    private ResourceLocation getUnicodePageLocation(int gl) {
        if (unicodePageLocations[gl] == null) {
            unicodePageLocations[gl] = new ResourceLocation(
                String.format("textures/font/unicode_page_%02x.png", new Object[] { gl })
            );
        }
    ...
    /**
     * Load one of the /font/glyph_XX.png into a new GL texture and store the texture ID
     * in glyphTextureName array.
     */
    ...
    public final int
    drawStringFloatPos(String sg, float x, float y, int color, boolean shadow) {
        wipe = null;
        return this.doDrawString(sg, x, y, color, shadow);
    }
    ...
    public final int drawFractionalString(
        String sg, int x, int y, int color, int cflash, boolean shadow, float fraction
    ) {
        if (fraction >= 1) {
    ...
    /**
     * Apply Unicode Bidirectional Algorithm to string and return a new possibly reordered
     * string for visual rendering.
     */
    ...
    /**
     * Reset all style flag fields in the class to false; called at the start of string
     * rendering
     */
    ...
    protected boolean renderCharInString(
        String sg, int idx, boolean shadow
    ) { //return true if hit a formatter
        ...
        if (c0 == 167 && idx + 1 < sg.length()) {
            ...
            if (j < 16) {
                ...
                textColor = k;
                GL11.glColor4f(
                    (k >> 16) / 255.0F, (k >> 8 & 255) / 255.0F, (k & 255) / 255.0F, alpha
                );
            } else if (j == 16) {
                randomStyle = true;
            } else if (j == 17) {
     ...
     */
    private int renderStringAligned(
        String sg, int x, int y, int p_78274_4_, int color, boolean shadow
    ) {
        if (bidiFlag) {
    ...
    /**
     * Perform actual work of rendering a multi-line string with wordwrap and with darker
     * drop shadow color if flag is set
     */
    ...
    /**
     * Set unicodeFlag controlling whether strings should be rendered with Unicode fonts
     * instead of the default.png font.
     */
    @Override
    ...
    /**
     * Get unicodeFlag controlling whether strings should be rendered with Unicode fonts
     * instead of the default.png font.
     */
    @Override
    ...
    /**
     * Set bidiFlag to control if the Unicode Bidirectional Algorithm should be run before
    ...
    /**
     * Get bidiFlag that controls if the Unicode Bidirectional Algorithm should be run
