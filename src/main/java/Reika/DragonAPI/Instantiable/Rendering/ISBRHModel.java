...
public class ISBRHModel extends ModelBase {
    ...
    public ISBRHModel
    addBox(int x, int y, int z, float px, float py, float pz, int tx, int ty) {
        return this.addBox(x, y, z, px, py, pz, 0, 0, 0, 0, 0, 0, tx, ty);
    }

    public ISBRHModel addBox(
        int x,
        int y,
        int z,
        float px,
    ...
    public ISBRHModel addBox(
        ...
        ModelBox box = new ModelBoxNoTess(
            new NoTessModelBox(this, tx, ty), tx, ty, px, py, pz, x, y, z, 0
        );
        /*
    ...
    private static class NoTessQuad extends TexturedQuad {
        ...
        public void draw(Tessellator v5, float sc) {
            ...
            Vec3 vec31
                = vertexPositions[1].vector3D.subtract(vertexPositions[2].vector3D);
            Vec3 vec32 = vec31.crossProduct(vec3).normalize();
            //v5.startDrawingQuads();

            //if (invertNormal) { //never used or accessible
            //	v5.setNormal(-((float)vec32.xCoord), -((float)vec32.yCoord),
            //-((float)vec32.zCoord));
            //}
            //else {
            v5.setNormal(
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                v5.addVertexWithUV(
                    (float) positiontexturevertex.vector3D.xCoord * sc,
                    (float) positiontexturevertex.vector3D.yCoord * sc,
                    (float) positiontexturevertex.vector3D.zCoord * sc,
