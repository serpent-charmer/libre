...
public class TessellatorVertexList {
    ...
    public void
    addVertexWithUVColor(double x, double y, double z, double u, double v, int color) {
    ...
    public static class TessellatorVertex {
        ...
        public TessellatorVertex(
            double x, double y, double z, double u, double v, int color
        ) {
            posX = x;
            posY = y;
            posZ = z;
        ...
        public final boolean equals(Object o) {
            if (o instanceof TessellatorVertex) {
                ...
                if (v.posX == posX && v.posY == posY && v.posZ == posZ) {
                    return hasUV ? (v.hasUV && v.posU == posU && v.posV == posV)
                                 : !v.hasUV;
                }
            }
            return false;
        ...
        public TessellatorVertex copy() {
            ...
            return hasUV ? new TessellatorVertex(posX, posY, posZ, posU, posV)
                         : new TessellatorVertex(posX, posY, posZ);
        }
        ...
        public void
        rotate(double rx, double ry, double rz, double ox, double oy, double oz) {
            Vec3 ret = ReikaVectorHelper.rotateVector(
                Vec3.createVectorHelper(posX - ox, posY - oy, posZ - oz), rx, ry, rz
            );
            posX = ox + ret.xCoord;
            posY = oy + ret.yCoord;
            posZ = oz + ret.zCoord;
    ...
    public void
    clamp(double minx, double miny, double minz, double maxx, double maxy, double maxz) {
        for (TessellatorVertex v : data) {
    ...
    public void rotateNonOrthogonal(
