...
public class ColorVariance {
    ...
    public ColorVariance(int c, float hue, float sat, float lum) {
        ...
        float[] hsb = Color.RGBtoHSB(
            ReikaColorAPI.getRed(c),
            ReikaColorAPI.getGreen(c),
            ReikaColorAPI.getBlue(c),
            null
        );
        hueRoot = hsb[0];
        satRoot = hsb[1];
    ...
    public int[] generateColors(int n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (whiteVariation > 0) {
                c2 = ReikaColorAPI.mixColors(
                    0xffffff, c2, rand.nextFloat() * whiteVariation
                );
            }
    ...
    private float getHue() {
        return hueVariation > 0 ? Math.min(
                   1F, (float) ReikaRandomHelper.getRandomPlusMinus(hueRoot, hueVariation)
               )
                                : hueRoot;
    }

    private float getSat() {
        return satVariation > 0 ? Math.min(
                   1F, (float) ReikaRandomHelper.getRandomPlusMinus(satRoot, satVariation)
               )
                                : satRoot;
    }

    private float getLum() {
        return lumVariation > 0 ? Math.min(
                   1F, (float) ReikaRandomHelper.getRandomPlusMinus(lumRoot, lumVariation)
               )
                                : lumRoot;
    }

    private int getAlpha() {
        return alphaVariation > 0 ? Math.min(
                   255, ReikaRandomHelper.getRandomPlusMinus(alphaRoot, alphaVariation)
               )
                                  : alphaRoot;
    }
    ...
    public String toString() {
        return String.format(
            "%s = {%d,%.1f,%.1f,%.1f} > [%d,%.1f,%.1f,%.1f] / %.3f",
            Integer.toHexString(rootColor),
            alphaRoot,
            hueRoot * 360,
            satRoot * 256,
            lumRoot * 256,
