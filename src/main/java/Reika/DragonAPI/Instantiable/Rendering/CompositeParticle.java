...
/**
 * A compilation of several EntityFX objects, intended for rendering in the TESR to
 * reduce the number of individual particles that need to be spawned.
 */
...
public class CompositeParticle {
    ...
    public CompositeParticle(
        World world, int x, int y, int z, int n, SubParticleCreator p
    ) {
        frameLength = n;
        data = new FXState[n];
    ...
    private static class FXState {
        ...
        private void
        render(float ptick, float yaw, float f5, float pitch, float f3, float f4) {
            for (CacheableFX fx : particles) {
                ((EntityFX) fx)
                    .renderParticle(Tessellator.instance, ptick, yaw, f5, pitch, f3, f4);
            }
        }
    }
