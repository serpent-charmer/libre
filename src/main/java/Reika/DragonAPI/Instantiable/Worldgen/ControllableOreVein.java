...
public abstract class ControllableOreVein extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int l = 0; l <= veinSize; l++) {
            ...
            double range_horiz
                = (MathHelper.sin(l * (float) Math.PI / veinSize) + 1F) * range + 1D;
            double range_vert
                = (MathHelper.sin(l * (float) Math.PI / veinSize) + 1F) * range + 1D;
                            ...
                            for (int dz = minz; dz <= maxz; dz++) {
                                ...
                                if (rx * rx + ry * ry + rz * rz < 1D) {
                                    if (world.getBlock(dx, dy, dz)
                                            .isReplaceableOreGen(
                                                world, dx, dy, dz, target
                                            )) {
                                        //if (!world.checkChunksExist(dx, dy, dz, dx, dy,
                                        //dz)) {
    ...
    public static class ExposedOreVein extends ControllableOreVein {
        ...
        public boolean canPlaceBlockHere(World world, int x, int y, int z) {
            return ReikaWorldHelper.checkForAdjMaterial(world, x, y, z, Material.air)
                != null;
        }
    }
    ...
    public static final class BlockAdjacentOreVein extends ControllableOreVein {
        ...
        @Override
        public boolean canPlaceBlockHere(World world, int x, int y, int z) {
            return ReikaWorldHelper.checkForAdjBlock(
                       world, x, y, z, check.blockID, check.metadata
                   )
                != null;
        }
    }
