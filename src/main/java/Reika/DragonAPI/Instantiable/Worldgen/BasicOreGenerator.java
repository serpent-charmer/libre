    ...
    generateOre(OreEnum ore, Random random, World world, int chunkX, int chunkZ) {
        if (random.nextInt(chunksPerVein) == 0) {
            for (int i = 0; i < veinsPerChunk; i++) {
                ...
                int posY = ore.getRandomGeneratedYCoord(
                    world, posX, posZ, random
                ); //ore.minY+random.nextInt(ore.maxY-ore.minY+1);

                if (ore.canGenAt(world, posX, posY, posZ)) {
                    if ((new WorldGenMinable(id, meta, veinSize, replaceable))
