...
public abstract class ModifiableBigTree extends WorldGenAbstractTree {
     ...
     * 'major' index - 1 and 2 for 0, 0 and 2 for 1, and 0 and 1 for 2.
     */
    ...
    /**
     * Sets the distance limit for how far away the generator will populate leaves from
     * the base leaf node.
     */
    ...
    private final AxisAlignedBB boundingBox = AxisAlignedBB.getBoundingBox(
            ...
            if (f < 0.0F) {
                ...
                for (double d0 = 0.5D; j1 < i; ++j1) {
                    ...
                    if (this.checkBlockLine(world, pos1, pos2) == -1) {
                        ...
                        double d3 = Math.sqrt(
                            Math.pow(Math.abs(basePos[0] - pos1[0]), 2.0D)
                            + Math.pow(Math.abs(basePos[2] - pos1[2]), 2.0D)
                        );
    ...
    private void
    placeLeafNode(World world, int x, int y, int z, float size, byte p_150529_5_) {
        ...
        for (pos2[p_150529_5_] = pos1[p_150529_5_]; i1 <= l; ++i1) {
            ...
            while (j1 <= l) {
                double d0 = Math.pow(Math.abs(i1) + 0.5D, 2.0D)
                    + Math.pow(Math.abs(j1) + 0.5D, 2.0D);
                ...
                if (d0 > size * size) {
                    ...
                    if (!block1.isAir(world, pos2[0], pos2[1], pos2[2])
                        && !block1.isLeaves(world, pos2[0], pos2[1], pos2[2])) {
                        ++j1;
                    } else {
                        BlockKey leaf = this.getLeafBlock(pos2[0], pos2[1], pos2[2]);
                        this.setBlockAndNotifyAdequately(
                            world, pos2[0], pos2[1], pos2[2], leaf.blockID, leaf.metadata
                        );
                        this.addCoord(pos2);
    ...
    protected float layerSize(int layer) {
        if (layer < (heightLimit) *0.3D) {
            ...
            if (f1 == 0.0F) {
                ...
                f2 = (float
                ) Math.sqrt(Math.pow(Math.abs(f), 2.0D) - Math.pow(Math.abs(f1), 2.0D));
            }
    ...
    protected float leafSize(int r) {
        return r >= 0 && r < leafDistanceLimit
            ? (r != 0 && r != leafDistanceLimit - 1 ? 3.0F : 2.0F)
            : -1.0F;
    }
    ...
    private void generateTrunkColumn(World world, int[] root, int[] top) {
        ...
        if (pos[mainAxis] != 0) {
            ...
            for (int j = pos[mainAxis] + b4; i != j; i += b4) {
                ...
                this.setBlockAndNotifyAdequately(
                    world, pos3[0], pos3[1], pos3[2], log.blockID, b5
                );
                this.addCoord(pos3);
    ...
    /**
     * Generates additional wood blocks to fill out the bases of different leaf nodes that
    ...
    /**
     * Checks a line of blocks in the world from the first coordinate to triplet to the
     * second, returning the distance (in blocks) before a non-air, non-leaf block is
     * encountered and/or the end is encountered.
     */
     ...
     * spanning basePos to to the height limit, is valid.
     */
    ...
    protected boolean isValidUnderBlock(World world, int x, int y, int z) {
        return world.getBlock(x, y, z).canSustainPlant(
            world, x, y, z, ForgeDirection.UP, (BlockSapling) Blocks.sapling
        );
    }
    ...
    protected boolean func_150523_a(Block b) {
        ...
        return m == Material.air || m == Material.leaves || b == Blocks.grass
            || b == Blocks.dirt || this.isMatchingLog(b) || this.isMatchingSapling(b)
            || b == Blocks.vine;
    }
