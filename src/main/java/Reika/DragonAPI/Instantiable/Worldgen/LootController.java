...
public class LootController {
    private static final HashMap<String, ChestGenLootLocation> locationsByID
        = new HashMap();
    private static final HashMap<String, ChestGenLootLocation> locationsByObject
        = new HashMap();
    ...
    static {
        ...
        locationsByObject.put(
            StructureNetherBridgePieces.Piece.class.getName(), netherFortress
        );
        locationsByObject.put(
            StructureMineshaftPieces.Corridor.class.getName(), Location.MINESHAFT
        );
        locationsByObject.put(
            StructureStrongholdPieces.Crossing.class.getName(),
            Location.STRONGHOLD_CROSSING
        );
        locationsByObject.put(
            StructureStrongholdPieces.RoomCrossing.class.getName(),
            Location.STRONGHOLD_CROSSING
        );
        locationsByObject.put(
            StructureStrongholdPieces.Library.class.getName(), Location.STRONGHOLD_LIBRARY
        );
        locationsByObject.put(
            StructureStrongholdPieces.ChestCorridor.class.getName(),
            Location.STRONGHOLD_HALLWAY
        );
        locationsByObject.put(
            ComponentScatteredFeaturePieces.JunglePyramid.class.getName(),
            Location.JUNGLE_PUZZLE
        );
        locationsByObject.put(
            ComponentScatteredFeaturePieces.DesertPyramid.class.getName(),
            Location.PYRAMID
        );
        locationsByObject.put(
            StructureVillagePieces.House2.class.getName(), Location.VILLAGE
        );
    ...
    public void registerToWorldGen(DragonAPIMod mod, int tier) {
        if (tier <= 0) {
            mod.getModLogger().log(
                "Skipping dungeon loot generation, as it has been disabled."
            );
            return;
        }
        for (LootStack ls : items) {
            ...
            if (ls.lootTier <= tier) {
                ReikaGenHelper.addChestLoot(
                    ls.chestLocation.tag, ls.item, ls.minSize, ls.maxSize, ls.weight
                );
                mod.getModLogger().log("Adding " + itemName + " to " + ls.chestLocation);
            } else {
                mod.getModLogger().log(
                    "Not adding " + itemName + " to " + ls.chestLocation
                    + ", as its generation tier (" + ls.lootTier
                    + ") is greater than the configured tier of " + tier + "."
                );
            }
        }
    }
    ...
    public void
    addItem(int tier, Location loc, ItemStack is, int min, int max, int chance) {
    ...
    class LootStack {
        ...
        public LootStack(
            int tier, Location loc, ItemStack is, int min, int max, int chance
        ) {
            lootTier = tier;
            chestLocation = loc;
            item = is;
        ...
        public boolean equals(Object o) {
            if (o instanceof LootStack) {
                ...
                return ls.chestLocation == chestLocation
                    && ReikaItemHelper.matchStacks(item, ls.item);
            }
            return false;
        }
    ...
    public static ChestGenLootLocation getLocationForStructure(Object struct) {
        if (struct instanceof StructureNetherBridgePieces
            || struct instanceof StructureNetherBridgePieces.Piece)
    ...
    public static ArrayList<WeightedRandomChestContent> getCGHItems(String tag)
        throws Exception {
        return getCGHItems(ChestGenHooks.getInfo(tag));
    }

    public static ArrayList<WeightedRandomChestContent> getCGHItems(ChestGenHooks entry)
        throws Exception {
        return (ArrayList<WeightedRandomChestContent>) chestContents.get(entry);
    }

    public static void setCGHItems(String tag, WeightedRandomChestContent[] items)
        throws Exception {
        setCGHItems(ChestGenHooks.getInfo(tag), items);
    }

    public static void
    setCGHItems(ChestGenHooks entry, WeightedRandomChestContent[] items)
        throws Exception {
    ...
    public static enum ModdedStructures implements ChestGenLootLocation {
        ...
        APIARIST(
            ModList.FORESTRY,
            "forestry.apiculture.worldgen.ComponentVillageBeeHouse",
            "naturalistChest",
            ReferenceType.TABLE
        ),
        TINKERVILLAGE(
            ModList.TINKERER,
            "tconstruct.world.village.ComponentToolWorkshop",
            "TinkerHouse",
            ReferenceType.TABLE
        ),
        APOTHECARY(
            ModList.WITCHERY,
            "com.emoniph.witchery.worldgen.ComponentVillageApothecary",
            "villageApothecaryChestContents",
            ReferenceType.FIELD
        ),
        BOOKSHOP(
            ModList.WITCHERY,
            "com.emoniph.witchery.worldgen.ComponentVillageBookShop",
            "bookshopChestContents",
            ReferenceType.FIELD
        ),
        WKEEP(
            ModList.WITCHERY,
            "com.emoniph.witchery.worldgen.ComponentVillageKeep",
            "villageTowerChestContents",
            ReferenceType.FIELD
        ),
        PHOTOSHOP(
            new BasicModEntry("WitchingGadgets"),
            "witchinggadgets.common.world.VillageComponentPhotoshop",
            "WG:PHOTOWORKSHOP",
            ReferenceType.TABLE
        ),
        LIBRARY(
        ...
        private ModdedStructures(
            ModEntry mod, String partName, String lootTable, ReferenceType type
        ) {
            sourceMod = mod;
            dataType = type;
            className = partName;
        ...
        public WeightedRandomChestContent[] getContents() throws Exception {
            if (!this.exists())
                throw new IllegalStateException(
                    "Mod '" + sourceMod.getDisplayName() + "' is not loaded!"
                );
    ...
    private static enum ReferenceType {
        ...
        private WeightedRandomChestContent[] getData(Class c, String name)
            throws Exception {
            switch (this) {
                case FIELD:
        ...
        private void setData(Class c, String name, WeightedRandomChestContent[] items)
            throws Exception {
            switch (this) {
                case FIELD:
                    Field f = c.getDeclaredField(name);
