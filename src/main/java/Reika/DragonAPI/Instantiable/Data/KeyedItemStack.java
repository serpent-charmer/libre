...
public final class KeyedItemStack implements Comparable<KeyedItemStack> {
    ...
    private boolean match(KeyedItemStack ks, boolean force) {
        for (int i = 0; i < Criteria.list.length; i++) {
            Criteria c = Criteria.list[i];
            if ((force || (enabledCriteria[i] && ks.enabledCriteria[i]))
                && !c.match(this, ks))
                return false;
        }
        return true;
    ...
    private static enum Criteria {
        ...
        public int hash(KeyedItemStack ks) {
            switch (this) {
                ...
                case NBT:
                    return ks.item.stackTagCompound != null
                        ? ks.item.stackTagCompound.hashCode()
                        : -1;
                default:
                    return 0;
            }
        ...
        private boolean match(KeyedItemStack k1, KeyedItemStack k2) {
            switch (this) {
                ...
                case METADATA:
                    if (k1.item.getItem().getHasSubtypes()
                        || k2.item.getItem().getHasSubtypes()) {
                        int m1 = k1.item.getItemDamage();
                        int m2 = k2.item.getItemDamage();
                        return m1 == m2 || m1 == OreDictionary.WILDCARD_VALUE
                            || m2 == OreDictionary.WILDCARD_VALUE;
                    } else
                        return true;
                case SIZE:
    ...
    public static KeyedItemStack readFromNBT(NBTTagCompound nbt) {
        return new KeyedItemStack(ItemStack.loadItemStackFromNBT(nbt))
            .setIgnoreMetadata(nbt.getBoolean("ignoremeta"))
            .setIgnoreNBT(nbt.getBoolean("ignorenbt"))
            .setSized(nbt.getBoolean("sized"))
            .setSimpleHash(nbt.getBoolean("simplehash"));
    }
