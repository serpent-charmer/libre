...
public final class DecimalLineSegment {
    ...
    public static final DecimalLineSegment
    getFromXYZDir(double x1, double y1, double z1, ForgeDirection dir, double len) {
        return new DecimalLineSegment(
            x1,
            y1,
            z1,
            x1 + len * dir.offsetX,
    ...
    public static final DecimalLineSegment
    getFromXYZDir(double x1, double y1, double z1, CubeDirections dir, double len) {
        return new DecimalLineSegment(
            x1,
            y1,
            z1,
            MathHelper.floor_double(x1 + len * dir.offsetX),
            y1,
            MathHelper.floor_double(z1 + len * dir.offsetZ)
        );
    }
    ...
    public DecimalPosition findIntersection2D(DecimalLineSegment l) {
        ...
        if ((px < origin.xCoord && px < target.xCoord)
            || (px > origin.xCoord && px > target.xCoord)) //intersection is outside lines
            return null;
        if ((pz < origin.zCoord && pz < target.zCoord)
            || (pz > origin.zCoord && pz > target.zCoord))
            return null;
        if ((px < l.origin.xCoord && px < l.target.xCoord)
            || (px > l.origin.xCoord && px > l.target.xCoord))
            return null;
        if ((pz < l.origin.zCoord && pz < l.target.zCoord)
            || (pz > l.origin.zCoord && pz > l.target.zCoord))
            return null;
