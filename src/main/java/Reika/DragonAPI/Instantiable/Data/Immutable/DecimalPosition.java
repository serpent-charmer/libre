...
public final class DecimalPosition implements Location, Comparable<DecimalPosition> {
    ...
    public DecimalPosition(Entity e, float ptick) {
        this(
            e.lastTickPosX + ptick * (e.posX - e.lastTickPosX),
            e.lastTickPosY + ptick * (e.posY - e.lastTickPosY),
            e.lastTickPosZ + ptick * (e.posZ - e.lastTickPosZ)
        );
    }
    ...
    @Override
    public int hashCode() {
        return Coordinate.coordHash(
            (int) (xCoord * 1000), (int) (yCoord * 1000), (int) (zCoord * 1000)
        ); //(int)(xCoord + (zCoord * 256) + (yCoord * 65536));
    }
    ...
    public Coordinate getCoordinate() {
        return new Coordinate(
            MathHelper.floor_double(xCoord),
            MathHelper.floor_double(yCoord),
    ...
    public boolean isEmpty(IBlockAccess world) {
        return this.getBlock(world).isAir(
            world,
            MathHelper.floor_double(xCoord),
            MathHelper.floor_double(yCoord),
            MathHelper.floor_double(zCoord)
    ...
    public static DecimalPosition
    interpolate(DecimalPosition p1, DecimalPosition p2, double f) {
        return interpolate(
            p1.xCoord, p1.yCoord, p1.zCoord, p2.xCoord, p2.yCoord, p2.zCoord, f
        );
    }

    public static DecimalPosition interpolate(
        double x1, double y1, double z1, double x2, double y2, double z2, double f
    ) {
        return new DecimalPosition(
            x1 + (x2 - x1) * f, y1 + (y2 - y1) * f, z1 + (z2 - z1) * f
        );
    }
    ...
    public static DecimalPosition getRandomWithin(Coordinate c, Random rand) {
        return new DecimalPosition(
            c.xCoord + rand.nextDouble(),
            c.yCoord + rand.nextDouble(),
            c.zCoord + rand.nextDouble()
        );
    }
