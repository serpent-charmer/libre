...
public final class MultiMap<K, V> {
    ...
    private boolean addValue(K key, V value, boolean load, boolean copy) {
        ...
        if (copy || !factory.allowsDuplicates
            || /*!li.contains(value)*/ !this.containsValueForKey(key, value)) {
    ...
    public Collection<V> get(K key) {
        ...
        if (c == null && this.nullEmpty)
            return null;
        return c != null ? (this.modifiable ? c : Collections.unmodifiableCollection(c))
                         : this.factory.createCollection(); //Internal NPE protection
    }
    ...
    public void writeToNBT(NBTTagList li, NBTIO<K> converterK, NBTIO<V> converterV) {
        for (Entry<K, Collection<V>> e : data.entrySet()) {
            ...
            ReikaNBTHelper.writeCollectionToNBT(
                e.getValue(), entry, "values", converterV
            );
            li.appendTag(entry);
        }
    }
    ...
    public static final class ConcurrencyDeterminator<K, V>
        implements MapDeterminator<K, V> {
        @Override
        public Map<K, V> getMapType() {
