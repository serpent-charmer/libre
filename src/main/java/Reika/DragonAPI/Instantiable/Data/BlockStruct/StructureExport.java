...
public class StructureExport extends NBTFile {
    ...
    public StructureExport(String name) {
        super(
            name,
            new File(
                DragonAPICore.getMinecraftDirectory(), "StructureData/" + name + ".struct"
            )
        );
    }
        ...
        for (BlockData dat : data.values()) {
            if (dat.hasTileEntity && dat.tileData != null) {
                ...
                if (tag != null) {
                    dat.tileData.setTag(
                        s,
                        call.getOverriddenValue(
                            dat.position,
                            dat.block,
                            s,
                            tag.copy(),
                            (NBTTagCompound) dat.tileData.copy()
                        )
                    );
                }
            }
        ...
        for (BlockData dat : data.values()) {
            ...
            if (placeCallback != null) {
                placeCallback.onPlace(
                    dat.position,
                    dat.block,
                    dat.tileData != null ? (NBTTagCompound) dat.tileData.copy() : null
                );
            }
        }
    }
    ...
    private static class BlockData {
        ...
        private BlockData(
            ...
            int z,
            HashSet<String> tags,
            HashMap<String, Object> extraTags,
            HashMap<String, NBTCallback> overrides
        ) {
            ...
            if (te != null) {
                ...
                for (String s : overrides.keySet()) {
                    ...
                    if (tag != null) {
                        tag = overrides.get(s).getOverriddenValue(
                            position,
                            block,
                            s,
                            tag.copy(),
                            (NBTTagCompound) tileData.copy()
                        );
        ...
        private BlockData(
            Coordinate c, BlockKey bk, boolean tile, NBTTagCompound tileDat
        ) {
            position = c;
            block = bk;
            hasTileEntity = tile;
        ...
        private static BlockData readFromNBT(
            NBTTagCompound NBT, HashSet<String> tags, HashMap<String, Object> extraTags
        ) {
