...
public abstract class AbstractSearch {
    ...
    public final FoundPath getResult() {
        return result == null || result.isEmpty() ? new FoundPath(null)
                                                  : new FoundPath(result);
    }
    ...
    isValidLocation(World world, int x, int y, int z, Coordinate from) {
        return propagation.isValidLocation(world, x, y, z, from)
            || termination.isValidTerminus(world, x, y, z);
    }
    ...
    public static final class LocationTerminus
        implements TerminationCondition, FixedPositionTarget {
    ...
    public static final class WalkablePropagation implements PropagationCondition {
        ...
        public boolean
        isValidLocation(World world, int x, int y, int z, Coordinate from) {
            return PassablePropagation.instance.isValidLocation(world, x, y, z, from)
                && (!PassablePropagation.instance.isValidLocation(
                        world, x, y - 1, z, from
                    )
                    || !PassablePropagation.instance.isValidLocation(
                        world, x, y - 2, z, from
    ...
    public class FoundPath {
        ...
        private boolean validatePath(World world) {
            for (Coordinate c : path) {
                if (!AbstractSearch.this.isValidLocation(
                        world, c.xCoord, c.yCoord, c.zCoord, root
                    ))
                    return false;
            }
            return true;
