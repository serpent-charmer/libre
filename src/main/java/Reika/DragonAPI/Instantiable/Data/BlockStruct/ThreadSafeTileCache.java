...
public final class ThreadSafeTileCache extends ThreadSafeSet<WorldLocation> {
    /**
     * The class tiles at the locations have to extend to be valid. May be null to
     * indicate even no tile is valid.
     */
    ...
    public boolean
    lookForMatch(World world, boolean skipOtherDimension, TileEntityMatchCheck check) {
        return this.lookForMatch(world, skipOtherDimension, check, null);
    }

    public boolean lookForMatch(
        World world,
        boolean skipOtherDimension,
        TileEntityMatchCheck check,
        ValidityFailHandler errorHandle
    ) {
        synchronized (data) {
            Iterator<WorldLocation> it
                = data.iterator(); // Must be in the synchronized block
            while (it.hasNext()) {
    ...
    public void
    applyToMatches(World world, boolean skipOtherDimension, TileEntityMatchEffect check) {
        this.applyToMatches(world, skipOtherDimension, check, null);
    }

    public void applyToMatches(
        World world,
        boolean skipOtherDimension,
        TileEntityMatchEffect check,
        ValidityFailHandler errorHandle
    ) {
        synchronized (data) {
            Iterator<WorldLocation> it
    ...
    public TileEntity
    returnMatch(World world, boolean skipOtherDimension, TileEntityMatchCheck check) {
        return this.returnMatch(world, skipOtherDimension, check, null);
    }

    public TileEntity returnMatch(
        World world,
        boolean skipOtherDimension,
        TileEntityMatchCheck check,
        ValidityFailHandler errorHandle
