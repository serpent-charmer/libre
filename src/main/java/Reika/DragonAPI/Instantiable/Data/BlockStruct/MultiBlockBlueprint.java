...
public class MultiBlockBlueprint {
    ...
    public MultiBlockBlueprint
    addCenteredBlockAt(int x, int y, int z, Block id, int meta) {
            ...
            for (int j = 0; j < ySize; j++) {
                for (int k = 0; k < zSize; k++) {
                    ...
                    if (b != IDs[i][j][k])
                        return false;
                    if (meta != metas[i][j][k]
                        && metas[i][j][k] != OreDictionary.WILDCARD_VALUE)
                        return false;
                }
            }
        }
                ...
                for (int k = 0; k < zSize; k++) {
                    ...
                    if (id != null) {
                        ...
                        if (this.canPlaceBlockAt(world, x0 + i, y0 + j, z0 + k)) {
                            //ReikaJavaLibrary.pConsole("Creating "+id+":"+meta+" @
                            //"+(x0+i)+", "+(y0+j)+", "+(z0+k));
