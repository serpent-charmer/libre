...
public class SlicedBlockBlueprint {
            ...
            if (i > 0)
                if (array[i].length() != array[i - 1].length())
                    throw new MisuseException(
                        "You must only register properly shaped slices!"
                    );
        }
    ...
    public String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append(
            "Size " + this.getLength() + "x" + this.getWidth() + "x" + this.getHeight()
            + ":\n"
        );
        ...
        for (int i = 0; i < ids.length; i++) {
            for (int k = 0; k < ids[i].length; k++) {
                ...
                if (id == null) {
                    ...
                    if (id != null) {
                        if (id == id2) {
                            //ReikaJavaLibrary.pConsole(slice+" w aID: "+id+"&"+id2+" @
                            //"+i+", "+k+" >> "+dx+","+dy+","+dz);
                            if (call != null)
                                call.onBlockFailure(
                                    world, dx, dy, dz, new EmptyCheck(false, false)
                                );
                            return false;
                        }
                        if (meta != -1) {
                            if (meta == meta2) {
                                //ReikaJavaLibrary.pConsole(slice+" w ameta:
                                //"+meta+"%"+meta2+" @ "+i+", "+k+" >>
                                //"+dx+","+dy+","+dz);
                                if (call != null)
                                    call.onBlockFailure(
                    ...
                    if (id != id2) {
                        //ReikaJavaLibrary.pConsole(slice+" w ID: "+id+"&"+id2+" @ "+i+",
                        //"+k+" >> "+dx+","+dy+","+dz); world.setBlock(dx, dy, dz, 49);
                        if (call != null)
                            call.onBlockFailure(
                                world, dx, dy, dz, new BlockKey(id, meta)
                            );
                        return false;
                    }
                    if (meta != -1) {
                        if (meta != meta2) {
                            //ReikaJavaLibrary.pConsole(slice+" w meta: "+meta+"%"+meta2+"
                            //@ "+i+", "+k+" >> "+dx+","+dy+","+dz); world.setBlock(dx,
                            //dy, dz, 49);
                            if (call != null)
                                call.onBlockFailure(
                                    world, dx, dy, dz, new BlockKey(id, meta)
