...
public class OpenPathFinder implements PropagationCondition {
    ...
    public static final Collection<PassRules> defaultRules
        = Collections.unmodifiableSet(EnumSet.of(PassRules.SOFT));
    ...
    isEmptyBlock(World world, int x, int y, int z, Collection<PassRules> rules) {
    ...
    private static boolean isSmallPassable(World world, int x, int y, int z, Block b) {
        ...
        double thresh = b.getCollisionBoundingBoxFromPool(world, x, y, z) == null
            ? 0.0625
            : 0.0078125;
        return vol <= thresh;
    }
