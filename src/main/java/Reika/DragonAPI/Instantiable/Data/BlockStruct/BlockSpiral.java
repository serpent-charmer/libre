...
public class BlockSpiral {
    ...
    public BlockSpiral calculate() {
        ...
        while (Math.abs(posX - originX) <= radius * gridSize
               && Math.abs(posY - originY) <= radius * gridSize
               && Math.abs(posZ - originZ) <= radius * gridSize) {
            posX += step.offsetX * gridSize;
            posY += step.offsetY * gridSize;
            posZ += step.offsetZ * gridSize;
            blocks.addBlockCoordinate(posX, posY, posZ);
            ForgeDirection dir = rightHanded ? ReikaDirectionHelper.getRightBy90(step)
                                             : ReikaDirectionHelper.getLeftBy90(step);
            if (!blocks.hasBlock(
                    posX + dir.offsetX * gridSize,
                    posY + dir.offsetY * gridSize,
                    posZ + dir.offsetZ * gridSize
                )) {
                step = dir;
            }
        }
