...
public class PolygonalGuiButton extends ImagedGuiButton {
    ...
    public PolygonalGuiButton(
        int par1, String par4Str, Class mod, DoublePolygon p, ComplexSubdividedTexture tex
    ) {
    ...
    */
    public PolygonalGuiButton(
        ...
        Class mod,
        DoublePolygon p,
        ComplexSubdividedTexture tex
    ) {
        super(
            par1,
            (int) p.getBounds().x,
            (int) p.getBounds().y,
            (int) p.getBounds().width,
            (int) p.getBounds().height,
            0,
            0,
            par6Str,
            par9,
    ...
    /**
     * Draw a Gui Button with an image background and text tooltip. Args: id, filepath,
     * text tooltip, text color, shadow
     */
    public PolygonalGuiButton(
        int par1,
        String file,
    ...
    public PolygonalGuiButton setLabelPosition(int x, int y, boolean relative) {
        ...
        if (relativeLabel) {
            ...
            labelY += shape.getBounds().y + shape.getBounds().height / 2
    ...
    protected void renderButton() {
        ...
        Tessellator.instance.startDrawing(
            shape.npoints == 4 ? GL11.GL_QUADS : GL11.GL_TRIANGLE_FAN
        );
        ...
        for (int i = 0; i < shape.npoints; i++) {
            ...
            //double dy = y-cy;
            //double d = 1.15;
            //double rx = MathHelper.clamp_double(cx+dx*d, shape.getBounds().x,
            //shape.getBounds().x+shape.getBounds().width); double ry =
            //MathHelper.clamp_double(cy+dy*d, shape.getBounds().y,
            //shape.getBounds().y+shape.getBounds().height); texture.addVertex(rx, ry);
        }
        Tessellator.instance.draw();

        if (displayString != null && alignment != TextAlign.CENTER) {
            //ReikaGuiAPI.instance.drawLine((int)cx, (int)cy, this.getLabelX(),
            //this.getLabelY(), c);
            GL11.glLineWidth(3);
            int ty = labelY + renderer.FONT_HEIGHT;
            int x2 = alignment == TextAlign.RIGHT
                ? labelX - renderer.getStringWidth(displayString) - 2
                : labelX + renderer.getStringWidth(displayString) + 2;
            ...
            ReikaGuiAPI.instance.drawLine(
                (int) Math.round(cx + dx * dr), (int) Math.round(cy + dy * dr), x22, ty, c
            );
        }
    }
    ...
    protected int getLabelX() {
        //ReikaJavaLibrary.pConsole(displayString+" > "+alignment.getDX(renderer,
        //displayString));
