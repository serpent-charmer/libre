...
public class BlockInWorldGui extends Block {
    ...
    public boolean onBlockActivated(
        ...
        //ReikaJavaLibrary.pConsole(String.format("%.2f, %.2f, %.2f -> %.2f, %.2f", a, b,
        //c, fx, fy));
        if (fx >= 0 && fy >= 0) {
    ...
    public static final class TileEntityInWorldGui extends TileEntity {
        ...
        private void activateArea(float fx, float fy) {
            if (gui != null) {
                ...
                //ReikaJavaLibrary.pConsole(String.format("[%.0f, %.0f : %.0f, %.0f]",
                //xmin*gui.xSize, ymin*gui.ySize, (xmin+width)*gui.xSize,
                //(ymin+height)*gui.ySize));
                //ReikaJavaLibrary.pConsole(String.format("%.2f, %.2f -> %.2f, %.2f -> %d,
                //%d", fx, fy, rx, ry, px, py));
                gui.click(px, py, 1);
            }
        }
