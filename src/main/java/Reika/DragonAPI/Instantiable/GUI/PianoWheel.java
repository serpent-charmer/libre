...
public class PianoWheel {
    ...
    public PianoWheel(
        ...
        if (!linearDivision) {
            ...
            if (octaveRange % 2 == 0) {
                for (int i = 0; i < mid; i++) {
                    //ReikaJavaLibrary.pConsole(i+" + "+d+" > "+(mid-1-i)+" & "+(mid+i)+"
                    //$ "+split[mid-1-i]+" & "+split[mid+i]);
                    split[mid - 1 - i] += d * (i + 1);
                    split[mid + i] -= d * (i + 1);
                }
            } else { //has ctr point
    ...
    public void draw(boolean label, boolean verbose, boolean renderLines) {
        ...
        if (label) {
            ...
            fr.drawString(
                s,
                originX - fr.getStringWidth(s) / 2,
                originY - fr.FONT_HEIGHT / 2,
                root.getNote().isPure() ? 0x000000 : 0xffffff
            );
            GL11.glDisable(GL11.GL_TEXTURE_2D);
        }
            ...
            for (int k = 0; k < 12; k++) {
                ...
                if (key == null) {
                    ReikaJavaLibrary.pConsole(
                        (1 + k + i * 12) + " above " + root + " is null??  i=" + i
                        + ",  k=" + k
                    );
                    continue;
                }
                ...
                if (label && (verbose || key.getNote() == root.getNote())) {
                    ...
                    double px = originX + midr * Math.sin(Math.toRadians(180 - mida - 15))
                        - fr.getStringWidth(s) / 2D;
                    double py = originY + midr * Math.cos(Math.toRadians(180 - mida - 15))
                        - fr.FONT_HEIGHT / 2D;
                    fr.drawString(
                        s,
                        (int) px,
                        (int) py,
                        key.getNote().isPure() ? 0x000000 : 0xffffff
                    );
                    GL11.glDisable(GL11.GL_TEXTURE_2D);
                }
            }
