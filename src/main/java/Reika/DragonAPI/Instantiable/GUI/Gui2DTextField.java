...
public class Gui2DTextField extends GuiTextField {
    ...
    public Gui2DTextField(
        FontRenderer f, int x, int y, int w, int h, int width, int lines
    ) {
        super(f, x, y, w, h);
        fr = f;
        guiX = x;
    ...
    public void drawTextBox()
    {
        if (this.getVisible())
        {
            ...
            String s = fr.trimStringToWidth(text.substring(lineScrollOffset),
    this.getWidth()); boolean flag = j >= 0 && j <= s.length(); boolean flag1 = isFocused
    && cursorCounter / 6 % 2 == 0 && flag; int l = enableBackgroundDrawing ? guiX + 4 :
    guiX; int i1 = enableBackgroundDrawing ? guiY + (guiH - 8) / 2 : guiY; int j1 = l;
            ...
            boolean flag2 = cursorPosition < text.length() || text.length() >=
    this.getMaxStringLength(); int k1 = j1;
