...
public class GuiPainter {
    ...
    private MultiMap<PaintElement, Point> locations
    ...
    public void onRenderTick(int mx, int my) {
        if (activeElement != null && Mouse.isButtonDown(0)) {
            if (mx >= posX && my >= posY && mx < posX + data.length * pixelSize
                && my < posY + data[0].length * pixelSize) {
                int x = mx - posX;
                int y = my - posY;
    ...
    public void draw() {
        ...
        ReikaGuiAPI.instance.drawRectFrame(
            posX, posY, data.length * pixelSize, data[0].length * pixelSize, 0xffffff
        );
    }
    ...
    public void clear() {
        for (int i = 0; i < data.length; i++) {
            for (int k = 0; k < data[i].length; k++) {
                data[i][k]
                    = init ? this.getDefaultEntry(i, k) : this.getFallbackEntry(i, k);
            }
        }
    }
    ...
    public static enum Brush {
        ...
        public GuiButton getButton(int id, int x, int y) {
            ImagedGuiButton b = new ImagedGuiButton(
                id,
                x,
                y,
                16,
        ...
        public Collection<Point> getFill() {
            ...
            switch (this) {
                ...
                case SPRAY:
                    ...
                    for (int i = 0; i < n; i++) {
                        c.add(new Point(
                            ReikaRandomHelper.getRandomPlusMinus(0, 4),
                            ReikaRandomHelper.getRandomPlusMinus(0, 4)
                        ));
                    }
                    break;
