...
public interface GuiItemDisplay extends Comparable<GuiItemDisplay> {
    ...
    public static class GuiStackDisplay implements GuiItemDisplay {
        ...
        public void draw(FontRenderer fr, int x, int y) {
            if (item != null)
                ReikaGuiAPI.instance.drawItemStackWithTooltip(
                    ReikaGuiAPI.itemRenderer, fr, item, x, y
                );
        }
        ...
        public int compareTo(GuiItemDisplay o) {
            return o instanceof GuiStackDisplay
                ? ReikaItemHelper.comparator.compare(item, ((GuiStackDisplay) o).item)
                : 1;
        }
    ...
    public static class GuiStackListDisplay extends GuiMultiStackDisplay {
        ...
        @Override
        public int compareTo(GuiItemDisplay o) {
            return o instanceof GuiStackListDisplay
                ? ReikaItemHelper.itemListComparator.compare(
                    items, ((GuiStackListDisplay) o).items
                )
                : 1;
        }
    }
    ...
    public static class GuiIconDisplay implements GuiItemDisplay {
        ...
        private IIcon getIcon() {
            return iconFetch == null ? ReikaTextureHelper.getMissingIcon()
                                     : iconFetch.getIcon();
        }
        ...
        public void draw(FontRenderer fr, int x, int y) {
            ...
            ReikaGuiAPI.instance.drawTexturedModelRectFromIcon(
                x, y, this.getIcon(), 16, 16
            );
        }

        @Override
        public int compareTo(GuiItemDisplay o) {
            return o instanceof GuiIconDisplay
                ? this.getIcon().getIconName().compareToIgnoreCase(
                    ((GuiIconDisplay) o).getIcon().getIconName()
                )
                : -1;
        }
