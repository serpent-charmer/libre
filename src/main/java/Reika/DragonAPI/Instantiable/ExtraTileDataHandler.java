...
/**
 * Used to add extra data to a TileEntity whose code you do not control, and have NBT
 * hooks to make that data persistent if necessary. ONLY ONE of these may be added per
 * class!
 * */
public abstract class ExtraTileDataHandler {
    ...
    public static final InsnList injectHandler(
        ClassNode cn,
        Class<? extends ExtraTileDataHandler> object,
        boolean construct,
        boolean addNBT
    ) {
    ...
    public static final InsnList injectHandler(
        ...
        boolean construct,
        boolean addNBT,
        Function<String, String> nbtFunc
    ) {
    ...
    private static final void injectNBTHandling(
        ClassNode cn,
        Class<? extends ExtraTileDataHandler> object,
        InsnList get,
        Function<String, String> nbtFunc
        ...
        MethodNode write
            = getMethodByName(cn, "func_145841_b", "writeToNBT", sig, nbtFunc);
        ...
        AbstractInsnNode loc1
            = ReikaASMHelper.getFirstInsnAfter(write.instructions, 0, Opcodes.ALOAD, 1);
        write.instructions.insertBefore(loc1, save);

        MethodNode read
            = getMethodByName(cn, "func_145839_a", "readFromNBT", sig, nbtFunc);
        ...
        AbstractInsnNode loc2
            = ReikaASMHelper.getFirstInsnAfter(read.instructions, 0, Opcodes.ALOAD, 1);
        read.instructions.insertBefore(loc2, load);
    }

    private static MethodNode getMethodByName(
        ClassNode cn,
        String obf,
        String deobf,
        String sig,
    ...
    private static final InsnList
    getLoadOfHandler(ClassNode cn, Class<? extends ExtraTileDataHandler> object) {
        InsnList li = new InsnList();
        li.add(new VarInsnNode(Opcodes.ALOAD, 0));
        li.add(new FieldInsnNode(
            Opcodes.GETFIELD,
            cn.name,
            FIELD_NAME,
            ReikaASMHelper.convertClassName(object, true)
        ));
        return li;
    }
}
