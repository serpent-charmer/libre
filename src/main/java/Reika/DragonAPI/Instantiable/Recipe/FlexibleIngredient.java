...
public final class FlexibleIngredient {
    ...
    public void addItem(ItemStack is) {
        if (filter != null) {
            filter.addItem(new KeyedItemStack(is)
    ...
    public ItemStack getItemForDisplay(boolean size) {
        ...
        return ReikaItemHelper.getSizedItemStack(
            filter.getCycledItem(), size ? numberToUse : 1
        );
    }
    ...
    public String fullID(IngredientIDHandler id) {
        return this.exists() ? id.fullIDForItems(filter.getItemList()) + " x"
                + numberToUse + "@" + chanceToUse + "%"
                             : "Empty";
    }
    ...
    public static FlexibleIngredient
    parseLua(CustomRecipeList crl, LuaBlock b, boolean allowEmptyItemList) {
        if (b == null)
            return EMPTY;
        Collection<ItemStack> li = b.hasChild("items")
            ? crl.parseItemCollection(b.getChild("items").getDataValues(), true)
            : null;
