...
public class AITaskSeekLocation extends EntityAIBase implements ComparableAI {
    ...
    private MotionTracker motionTracker
        = new MotionTracker(20, 5); //5s, in 1/4-second steps
    ...
    public void updateTask() {
        ...
        isStuck = false;
        if (motionTracker.getLastMoved() > 40
            || motionTracker.getTotalTravelDistanceSince(20)
                < 1) { //2s with no movement or 5s with < 1 blocks movement
            isStuck = true;
        }
        if (!isStuck) {
            ...
            if (b.getMaterial().blocksMovement()
                && b.getCollisionBoundingBoxFromPool(entity.worldObj, idx, idy, idz)
                    != null) {
                isStuck = true;
            }
        }
        if (isStuck && !wasStuck) {
            //entity.getNavigator().tryMoveToXYZ(entity.posX, entity.posY+4, entity.posZ,
            //speed);
    ...
    public boolean match(ComparableAI ai) {
        return ai instanceof AITaskSeekLocation
            && ((AITaskSeekLocation) ai).target.equals(target);
    }
}
