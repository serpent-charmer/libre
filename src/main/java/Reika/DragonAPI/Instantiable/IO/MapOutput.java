...
public abstract class MapOutput<V> {
            ...
            for (int dx = originX - range; dx <= originX + range; dx += resolution) {
                for (int dz = originZ - range; dz <= originZ + range; dz += resolution) {
                    ...
                    if ((flag1 || flag2) && ((flag1 && flag2) || fullGrid)) {
                        data[i][k] = ReikaColorAPI.mixColors(
                            data[i][k],
                            i2 == 0 && k2 == 0 ? 0xffff0000 : 0xffffffff,
                            0.25F
                        );
                        if (i - 1 >= 0)
                            data[i - 1][k] = ReikaColorAPI.mixColors(
                                data[i - 1][k], 0xff000000, 0.5F
                            );
                        if (i + 1 < data.length)
                            data[i + 1][k] = ReikaColorAPI.mixColors(
                                data[i + 1][k], 0xff000000, 0.5F
                        ...
                        if (k - 1 >= 0)
                            data[i][k - 1] = ReikaColorAPI.mixColors(
                                data[i][k - 1], 0xff000000, 0.5F
                            );
    ...
    public String createImage() throws IOException {
        ...
        f.createNewFile();
        BufferedImage img
            = new BufferedImage(data.length, data.length, BufferedImage.TYPE_INT_ARGB);
    ...
    private String getFilename() {
        String ret = this.getFilepath() + originX + ", " + originZ + " "
            + this.getFileNameDetails();
        return ret;
    }

    protected String getFileNameDetails() {
        String sr = String.valueOf(range * 2 + 1);
        return " (" + sr + "x" + sr + "; [R=" + resolution + " b-px, G=" + gridSize + "-"
            + fullGrid + "]).png";
    }

    private String getFilepath() {
        String ret = this.getClass().getSimpleName() + "/" + worldName + "/DIM"
            + dimensionID + "/";
