...
public class CustomRecipeList {
    ...
    private final LuaBlock exampleBlock
        = new ExampleLuaBlock("exampleRoot", null, new LuaBlockDatabase());
    ...
    public final boolean load() {
        ...
        ArrayList<File> files
            = ReikaFileReader.getAllFilesInFolder(folder, this.getExtension());
        this.load(files);
        return true;
    }
    ...
    public final void createExampleFile() {
        try {
            ...
            ReikaFileReader.writeLinesToFile(
                f, exampleBlock.writeToStrings(), true, Charsets.UTF_8
            );
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void parseLuaBlocks() {
        ...
        for (LuaBlock b : root.getChildren()) {
            try {
                ...
                mod.getModLogger().debug(
                    "Loaded recipe prototype:\n" + b.getString("type")
                );
                entries.add(b);
            } catch (Exception e) {
                mod.getModLogger().logError(
                    "Could not parse custom recipe section " + b.getString("type") + ": "
                );
                e.printStackTrace();
            }
        }
        if (!Strings.isNullOrEmpty(recipeType))
            mod.getModLogger().log(
                "All custom " + recipeType + " recipe entries parsed."
            );
    }
    ...
    private final File getBaseFilepath() {
        return new File(
            mod.getConfigFolder(), mod.getDisplayName() + "_" + this.getFolderName()
        );
    }
    ...
    public static final Object parseObjectString(String item) {
        if (item.equals("null") || item.equals("empty") || item.equals("~")
            || item.equals("-"))
            return null;
    ...
    public static final Collection<ItemStack>
    parseItemCollection(Collection<String> in, boolean tolerateNull) {
            ...
            if (s.startsWith("ore:")) {
                ...
                if (li.isEmpty() && !tolerateNull)
                    throw new IllegalArgumentException(
                        "Ore dictionary tag '" + s + "' has no items!"
                    );
                else
                    c.addAll(li);
            } else {
    ...
    parseItemString(String s, LuaBlock nbt, boolean tolerateNull) {
        ...
        if (m.find() && m.group(2) != null) {
            ...
            if (amt > 64) {
                throw new IllegalArgumentException(
                    "Stack size of " + amt + " is too large!"
                );
            }
            if (amt <= 0) {
        ...
        if (key.equals("delegate")) {
            ...
            if (delegate == null)
                throw new IllegalArgumentException(
                    "No such Delegate Lookup '" + lookup + "'!"
                );
            ItemStack is = delegate.getItem(nbt);
            if (is == null && !tolerateNull)
                throw new IllegalArgumentException(
                    "Delegate Lookup '" + lookup + "' yielded no item!"
                );
            return is;
    ...
    public static final IRecipe parseCraftingRecipe(LuaBlock lb, ItemStack output) {
        ...
        if (shaped) {
            String input1 = lb.containsKey("input_top")
                ? lb.getString("input_top").replaceAll(" ", "")
                : null;
            String input2 = lb.containsKey("input_middle")
                ? lb.getString("input_middle").replaceAll(" ", "")
                : null;
            String input3 = lb.containsKey("input_bottom")
                ? lb.getString("input_bottom").replaceAll(" ", "")
                : null;
            ...
            if (w > 3) {
                throw new IllegalArgumentException(
                    "Rows must be at most three entries long!"
                );
            }
            ...
            if (parts.length > 9)
                throw new IllegalArgumentException(
                    "You cannot have more than nine items in recipes!"
                );
            ...
            for (int i = 0; i < parts.length; i++) {
                ...
                if (o == null) {
                    throw new IllegalArgumentException(
                        "You cannot have blank spaces in shapeless recipes!"
                    );
                }
                inputs[i] = o;
            }
    ...
    public static final String fullID(Object o) {
        ...
        if (o instanceof Item)
            return Item.itemRegistry.getNameForObject(o) + "["
                + ReikaItemHelper.getRegistrantMod((Item) o) + "]";
        if (o instanceof Block)
            return Block.blockRegistry.getNameForObject(o) + "["
                + ReikaItemHelper.getRegistrantMod((Block) o) + "]";
    ...
    private static class BeeLookup implements DelegateLookup {
        ...
        public ItemStack getItem(LuaBlock data) {
            EnumBeeType type
                = EnumBeeType.valueOf(data.getString("class").toUpperCase(Locale.ENGLISH)
                );
                    ...
                    if (data.containsKey(key)) {
                        ...
                        switch (ec) {
                            ...
                            case TEMPERATURE_TOLERANCE:
                                EnumTolerance et = EnumTolerance.valueOf(
                                    data.getString(key).toUpperCase(Locale.ENGLISH)
                                );
    ...
    private static class TreeLookup implements DelegateLookup {
        @Override
        public ItemStack getItem(LuaBlock data) {
            EnumGermlingType type = EnumGermlingType.valueOf(
                data.getString("class").toUpperCase(Locale.ENGLISH)
            );
    ...
    private static class ButterflyLookup implements DelegateLookup {
        @Override
        public ItemStack getItem(LuaBlock data) {
            EnumFlutterType type = EnumFlutterType.valueOf(
                data.getString("class").toUpperCase(Locale.ENGLISH)
            );
            ItemStack ret
                = ReikaBeeHelper.getButterflyItem(data.getString("species"), type);
                    ...
                    if (data.containsKey(key)) {
                        ...
                        switch (ec) {
                            ...
                            case SPEED:
                                String val = data.getString(key);
                                ButterflyGene bg
                                    = ButterflyAlleleRegistry.getEnum(ec, val);
                                ia = bg.getAllele();
                                break;
                            case HUMIDITY_TOLERANCE:
                            case TEMPERATURE_TOLERANCE:
                        ...
                        if (ia != null) {
                            IButterflyGenome ibg
                                = (IButterflyGenome) ReikaBeeHelper.getGenome(ret);
                            ReikaBeeHelper.setGene(ret, ibg, ec, ia, false);
                            ReikaBeeHelper.setGene(ret, ibg, ec, ia, true);
                        }
                    }
