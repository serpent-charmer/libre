...
public abstract class LuaBlock {
    ...
    private final LinkedHashMap<LuaBlockKey, LuaBlock> children
        = new LinkedHashMap(); //linked to keep order
    ...
    protected LuaBlock(String n, LuaBlock parent, LuaBlockDatabase db) {
        ...
        if (tree == null)
            throw new MisuseException(
                "You cannot create a LuaBlock without a containing tree!"
            );
        if (parent != null) {
    ...
    private LuaBlockKey createKey(String n) {
        return tree.hasDuplicateKeys
            ? new LuaBlockKey(n, n + "_" + parent.children.size())
            : new LuaBlockKey(n);
    }
    ...
    private boolean isString(String s) {
        ...
        //return !Character.isDigit(s.charAt(s.charAt(0) == '-' && s.length() > 1 ? 1 :
        //0)) && !s.equalsIgnoreCase("true") && !s.equalsIgnoreCase("false");
    }
        ...
        for (LuaBlockKey s : steps) {
            if (lb.children.containsKey(s))
                ...
                throw new IllegalStateException(
                    "'" + orig.parent.name + "/" + orig.name
                    + "' tried to inherit property '" + key + "', but could not."
                );
        }
        //ReikaJavaLibrary.pConsole("'"+b.getString("type")+"' inheriting property
        //'"+steps+"/"+key+"' from parent '"+inherit+"'");
    ...
    private String toString(int indent) {
        ...
        if (!children.isEmpty()) {
            sb.append("\n");
            sb.append(
                this.getIndent("====", indent) + "=============CHILDREN=============\n"
            );
    ...
    public static final class ItemStackLuaBlock extends LuaBlock {
        ...
        public void write(ItemStack is, boolean writeSize) {
            ...
            LuaBlock nbt = is.stackTagCompound != null
                ? new NBTLuaBlock("nbt", this, tree, is.stackTagCompound, true)
                : null;
        }
    }
    ...
    public static class LuaBlockDatabase {
                ...
                if (s.contains("{")) {
                    ...
                    try {
                        ...
                        DragonAPICore.logError(
                            "Failed to construct proper child LuaBlock for " + s + "' in "
                            + this + ": "
                        );
                        e.printStackTrace();
                    ...
                    if (activeBlock.containsKey("type") && activeBlock.parent != null
                        && activeBlock.parent.isRoot)
        ...
        private LuaBlock createChild(String s, LuaBlock parent) throws Exception {
            ...
            if (parent.isRoot && defaultBlockType != null)
                c = defaultBlockType;
            Constructor<LuaBlock> ctr = (Constructor<LuaBlock>) c.getDeclaredConstructor(
                String.class, LuaBlock.class, LuaBlockDatabase.class
            );
    ...
    private NBTBase asNBT(boolean allowList) {
        if (!allowList && this.isList())
            throw new IllegalArgumentException(
                "The top-level LuaBlock must be a map type (root NBTTagCompound)!"
            );
        if (isList && !isRoot && allowList) {
            ...
            for (String s : data.keySet()) {
                tag.setTag(
                    s, ReikaNBTHelper.getTagForObject(this.parseObject(data.get(s)))
                );
            }
    ...
    private Object getObject(LuaBlock b) {
        return b.isList() && b.data.size() == 1 && b.children.isEmpty()
            ? this.parseObject(b.data.values().iterator().next())
            : b.isList() ? b.asList()
                         : b.asHashMap();
    }
    ...
    public static boolean isErrorCode(String s) {
        return NULL_DATA.equals(s) || NULL_KEY_INHERIT.equals(s)
            || NULL_PARENT_INHERIT.equals(s);
    }
    ...
    private static class LuaBlockKey {
        ...
        public boolean equals(Object o) {
            return o instanceof LuaBlockKey
                && ((LuaBlockKey) o).lookupKey.equals(lookupKey);
        }
