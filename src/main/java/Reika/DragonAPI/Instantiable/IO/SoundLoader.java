...
public class SoundLoader {
    ...
    private void registerSound(SoundEnum e, SoundResource sr) {
        String p = e.getPath();
        boolean stream
            = e instanceof StreamableSound && ((StreamableSound) e).isStreamed();
        DirectResourceManager.getInstance().registerCustomPath(
            p, e.getCategory(), stream
        );
        this.onRegister(e, p);
        if (e.preload()) {
            try {
                sr.resource
                    = (DirectResource) DirectResourceManager.getInstance().getResource(
                        sr.reference
                    );
                if (stream)
    ...
    private static class SoundResource {
        ...
        private SoundResource(SoundEnum s) {
            sound = s;
            reference = FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT
                ? getReference(s)
                : null;
        }
