...
public class PacketPipeline {
    ...
    public void sendToAllAround(
        PacketObj p, World world, double x, double y, double z, double range
    ) {
        this.sendToAllAround(p, world.provider.dimensionId, x, y, z, range);
    }

    public void
    sendToAllAround(PacketObj p, int world, double x, double y, double z, double range) {
    ...
    public void sendToAllAround(PacketObj p, Entity e, double range) {
        TargetPoint pt = new TargetPoint(
            e.worldObj.provider.dimensionId, e.posX, e.posY, e.posZ, range
        );
        //channels.get(Side.SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGET).set(FMLOutboundHandler.OutboundTarget.ALLAROUNDPOINT);
        //channels.get(Side.SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGETARGS).set(pt);
        //channels.get(Side.SERVER).writeAndFlush(p);
    ...
    public void sendToAllAround(PacketObj p, WorldLocation loc, double range) {
        TargetPoint pt
            = new TargetPoint(loc.dimensionID, loc.xCoord, loc.yCoord, loc.zCoord, range);
        //channels.get(Side.SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGET).set(FMLOutboundHandler.OutboundTarget.ALLAROUNDPOINT);
        //channels.get(Side.SERVER).attr(FMLOutboundHandler.FML_MESSAGETARGETARGS).set(pt);
        //channels.get(Side.SERVER).writeAndFlush(p);
        wrapper.sendToAllAround(p, pt);
