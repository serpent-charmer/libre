...
public class ConfigReader {
    ...
    private void setFile() {
        ...
        if (!configFile.exists()) {
            //throw new MisuseException("Config for "+modName+" does not exist at
            //"+filepath+"!\nThis reader is only designed for force-reading of existing
            //data!");
            DragonAPICore.logError(
                "Config for " + modName + " does not exist at " + filepath
    ...
    public int getConfigInt(String cat, String key) {
        if (!config.hasKey(cat, key))
            throw new MisuseException(
                "Config for " + modName + " at " + filepath
                + " does not contain category " + cat + " and key " + key
                + "\nThis reader is only designed for force-reading of existing data!"
            );
