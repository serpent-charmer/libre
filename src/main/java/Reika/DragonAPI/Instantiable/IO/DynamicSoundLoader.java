...
public class DynamicSoundLoader extends SoundLoader {
    ...
    public DynamicSoundLoader(
        Class<? extends DynamicSound> c, RemoteSourcedAssetRepository repo
    ) {
        super(c);
        dataSource = repo;
    }
    ...
    protected void onRegister(SoundEnum e, String p) {
        DirectResourceManager.getInstance().registerDynamicAsset(
            p, dataSource.createAsset(((DynamicSound) e).getRelativePath())
        );
    }
}
