...
public class BoundedValue<N extends Number> {
    ...
    private BoundedValue(N min, N max, double init) {
        ...
        this.isDecimalType
            = (min instanceof Double || min instanceof Float || min instanceof BigDecimal
            );
    }

    private BoundedValue(
        double min, double max, double init, double step, boolean dec, Class type
    ) {
        this.minValue = min;
        this.maxValue = max;
        this.value = init;
    ...
    public static BoundedValue readFromNBT(NBTTagCompound tag) {
        try {
            return new BoundedValue(
                ...
                tag.getDouble("val"),
                tag.getDouble("step"),
                tag.getBoolean("decimal"),
                Class.forName(tag.getString("type"))
            );
        } catch (Exception e) {
            return null;
        }
    ...
    public static BoundedValue readFromLuaBlock(LuaBlock lb) {
        try {
            return new BoundedValue(
                ...
                lb.getDouble("val"),
                lb.getDouble("step"),
                lb.getBoolean("decimal"),
                Class.forName(lb.getString("type"))
            );
        } catch (Exception e) {
            return null;
        }
    ...
    public String toString() {
        return "[" + this.minValue + " > " + this.maxValue + "] @ " + this.value + "x"
            + this.step + ", " + this.isDecimalType;
    }
}
