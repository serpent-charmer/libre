...
public class CorkscrewMotionController implements PositionController {
    ...
    public CorkscrewMotionController(
        double x,
        double y,
        double z,
        double len,
    ...
    public double getPositionX(Entity e) {
        return originX + ReikaPhysicsHelper.polarToCartesian(pos, theta, phi)[0]
            + spiralRadius * Math.cos(Math.toRadians(angle))
            * Math.cos(Math.toRadians(theta));
    }

    @Override
    public double getPositionY(Entity e) {
        return originY + ReikaPhysicsHelper.polarToCartesian(pos, theta, phi)[1]
            + spiralRadius * Math.sin(Math.toRadians(angle))
            * Math.sin(Math.toRadians(theta));
    }

    @Override
    public double getPositionZ(Entity e) {
        return originZ + ReikaPhysicsHelper.polarToCartesian(pos, theta, phi)[2]
            + spiralRadius * Math.cos(Math.toRadians(angle))
            * Math.cos(Math.toRadians(theta));
    }
}
