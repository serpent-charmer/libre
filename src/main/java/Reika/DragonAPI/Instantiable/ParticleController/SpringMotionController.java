...
public class SpringMotionController implements PositionController {
    ...
    public SpringMotionController(
        double theta, double phi, double a, double v, double d, DecimalPosition p
    ) {
        this(ReikaPhysicsHelper.polarToCartesian(1, theta, phi), a, v, d, p);
    }

    public SpringMotionController(
        double[] xyz, double a, double v, double d, DecimalPosition p
    ) {
        xComponent = xyz[0];
        yComponent = xyz[1];
        zComponent = xyz[2];
    ...
    public void update(Entity e) {
        tick++;
        linearPosition = maxAmplitude * Math.pow(Math.E, -damping * tick)
            * Math.cos(angularSpeed * tick);
    }
