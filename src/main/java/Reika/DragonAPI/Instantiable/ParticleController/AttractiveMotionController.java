...
public class AttractiveMotionController implements MotionController {
    ...
    public AttractiveMotionController(
        TileEntity te, double axz, double vy, double damping
    ) {
        this(te.xCoord + 0.5, te.yCoord + 0.5, te.zCoord + 0.5, axz, vy, damping);
    }

    public AttractiveMotionController(
        double x, double y, double z, double axz, double vy, double damping
    ) {
        targetX = x;
        targetY = y;
        targetZ = z;
    ...
    public double getMotionX(Entity e) {
        return -(e.posX - targetX) * velocityXZ
            / e.getDistance(targetX, targetY, targetZ);
    }
    ...
    public double getMotionZ(Entity e) {
        return -(e.posZ - targetZ) * velocityXZ
            / e.getDistance(targetX, targetY, targetZ);
    }
}
