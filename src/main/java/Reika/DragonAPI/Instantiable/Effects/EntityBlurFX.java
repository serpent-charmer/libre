...
public class EntityBlurFX extends EntityFX implements CustomRenderFX {
    ...
    public final EntityBlurFX setColor(int rgb) {
        return this.setColor(
            ReikaColorAPI.getRed(rgb),
            ReikaColorAPI.getGreen(rgb),
            ReikaColorAPI.getBlue(rgb)
        );
    }
        ...
        if (fx instanceof EntityBlurFX) {
            ...
            if (!bfx.getRenderMode().equals(this.getRenderMode()))
                DragonAPICore.logError(
                    "Cannot accurately lock two different particle render types: " + fx
                    + " & " + this
                );
            bfx.locks.add(this);
        }
    ...
    public void onUpdate() {
        ...
        if (fadeColor != -1) {
            int c = ReikaColorAPI.mixColors(
                fadeColor, preColor, age / (float) particleMaxAge
            );
            this.setColor(c);
        }
        ...
        if (alphaFade > 0) {
            ...
            if (rapidExpand) {
                f
                    = (particleMaxAge / age >= 12 ? age * 12F / particleMaxAge
                                                  : 1 - age / (float) particleMaxAge);
            } else {
            ...
            if (rapidExpand)
                ...
                particleScale = scale
                    * MathHelper.sin((float) Math.toRadians(180D * age / particleMaxAge));
            //if (particleAge < 10)
        ...
        if (bounds != null) {
            ...
            if ((posX <= bounds.minX && motionX < 0)
                || (posX >= bounds.maxX && motionX > 0)) {
                motionX = bounce ? -motionX : 0;
                if (cull)
                    this.setDead();
            }
            if ((posY <= bounds.minY && motionY < 0)
                || (posY >= bounds.maxY && motionY > 0)) {
                motionY = bounce ? -motionY : 0;
                if (cull)
                    this.setDead();
            }
            if ((posZ <= bounds.minZ && motionZ < 0)
                || (posZ >= bounds.maxZ && motionZ > 0)) {
    ...
    public void moveEntity(double vx, double vy, double vz) {
        if (noClip) {
        ...
        } else { //streamlined, removed everything that was never applicable or desirable
                 //for a particle
            ySize *= 0.4F;
            ...
            List<AxisAlignedBB> list = worldObj.getCollidingBoundingBoxes(
                this, boundingBox.addCoord(vx, vy, vz)
            );
    ...
    /*
    @Override
    public void renderParticle(Tessellator v5, float par2, float par3, float par4, float
    par5, float par6, float par7)
    {
        v5.draw();
    ...
    public final RenderMode getRenderMode() {
        if (renderMode == null)
            renderMode
                = new RenderMode()
                      .setFlag(RenderModeFlags.FOG, false)
                      .setFlag(RenderModeFlags.ADDITIVE, additiveBlend)
                      .setFlag(RenderModeFlags.DEPTH, depthTest)
                      .setFlag(RenderModeFlags.LIGHT, false)
                      .setFlag(
                          RenderModeFlags.ALPHACLIP, alphaTest && additiveBlend
                      ); //additiveBlend ? RenderMode.ADDITIVEDARK : RenderMode.LIT;
        return renderMode;
    }
