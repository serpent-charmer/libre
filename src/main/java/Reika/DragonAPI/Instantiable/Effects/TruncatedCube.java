...
public class TruncatedCube {
    ...
    public TruncatedCube cache(boolean startCenter, double x0, double y0, double z0) {
        faces = new List[6];
        for (int i = 0; i < 6; i++) {
            faces[i] = Collections.unmodifiableList(this.getFaceVertices(
                ForgeDirection.VALID_DIRECTIONS[i], startCenter, x0, y0, z0
            ));
        }
    ...
    public List<DecimalPosition> getFaceVertices(
        ForgeDirection face, boolean startCenter, double x0, double y0, double z0
    ) {
        if (faces != null)
    ...
    public List<ArrayList<DecimalPosition>>
    getCornerVertices(double x0, double y0, double z0) {
        if (corners != null)
            return corners;
    ...
    public void
    render(double x, double y, double z, int c1, int c2, boolean edge, float pdist) {
