...
public class Orbit {
    ...
    getPosition(double x0, double y0, double z0, double time, double mu) {
    ...
    public DecimalPosition getPosition(double x0, double y0, double z0, double dtheta) {
        ...
        double dd = semimajorAxis * (1 - eccentricity * eccentricity)
            / (1 + eccentricity * Math.cos(Math.toRadians(theta)));
        double x = dd
            * (Math.cos(RAAN) * Math.cos(Math.toRadians(theta) + argumentOfPerigee)
               - Math.sin(RAAN) * Math.sin(Math.toRadians(theta) + argumentOfPerigee)
                   * Math.cos(inclination));
        double y = dd
            * (Math.sin(RAAN) * Math.cos(Math.toRadians(theta) + argumentOfPerigee)
               + Math.cos(RAAN) * Math.sin(Math.toRadians(theta) + argumentOfPerigee))
            * Math.cos(inclination);
        double z = dd * Math.sin(Math.toRadians(theta) + argumentOfPerigee)
