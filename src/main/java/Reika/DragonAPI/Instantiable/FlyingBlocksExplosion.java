...
public class FlyingBlocksExplosion extends Explosion {
                ...
                for (int j = y - r; j <= y + r; j++) {
                    for (int k = z - r; k <= z + r; k++) {
                        if (ModList.CHROMATICRAFT.isLoaded()
                            && this.isLocationProtected(x, y, z))
                            continue;
    ...
    private boolean isLocationProtected(int x, int y, int z) {
        return TileEntityExplosionShield.isLocationProtected(
            world, x, y, z, explosionSize
        );
    }

    protected final Effect
    calcEffect(World world, int x, int y, int z, Block b, int meta) {
        Effect e = this.getEffect(world, x, y, z, b, meta);
        if (e == Effect.NOTHING)
            return e;
        double dd = ReikaMathLibrary.py3d(
            x + 0.5 - explosionX, y + 0.5 - explosionY, z + 0.5 - explosionZ
        );
        return dd <= explosionSize + 0.5 ? e : Effect.NOTHING;
    }

    protected final Effect
    getEffect(World world, int x, int y, int z, Block b, int meta) {
        if (b == Blocks.air)
        ...
        if (ReikaWorldHelper.softBlocks(world, x, y, z))
            return Effect.BREAK;
        if (b.getRenderType() != 0 && !b.renderAsNormalBlock()
            && !b.isOpaqueCube()) { //To prevent weird looking flying sand entities
            ; //return false;
        }
    ...
    private static enum Effect {
        ...
        public void trigger(
            ...
            int meta,
            TumbleCreator c,
            List<EntityFallingBlock> li
        ) {
            switch (this) {
                case BREAK:
                    ReikaWorldHelper.dropAndDestroyBlockAt(
                        world, x, y, z, null, true, false
                    );
                    break;
                case ENTITIZE:
                    EntityFallingBlock e = c != null
                        ? c.createBlock(world, x, y, z, b, meta)
                        : new EntityFallingBlock(world, x, y, z, b, meta);
                    li.add(e);
