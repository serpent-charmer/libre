...
/**
 * A tank class that can handle direct operations as well as standard Forge Liquid
 * operations.
 */
public class HybridTank extends FluidTank {
    ...
    public final NBTTagCompound writeToNBT(NBTTagCompound NBT) {
        ...
        if (repl != null && FluidRegistry.getFluid(repl) != null
            && !fluidName.equals(repl)) {
            tankData.setString("FluidName", repl);
            DragonAPICore.log(
                "Tank " + this + " has replaced its FluidName of '" + fluidName
                + "' with '" + repl + "', as the fluid has changed names."
            );
        }
    ...
    public final FluidTank readFromNBT(NBTTagCompound NBT) {
        ...
        ) { //"Empty String not allowed!" caused by fluid save failure
            DragonAPICore.logError(
                "Loading HybridTank '" + name
                + "' has errored, its machine will not keep its fluid!"
            );
            e.printStackTrace();
        }
        return this;
    ...
    public String toString() {
        ...
        return "Tank " + name + ", containing " + this.getLevel() + " mB of "
            + this.getActualFluid().getLocalizedName();
    }
    ...
    public boolean canTakeIn(FluidStack fs) {
        int amt = fs.amount;
        return this.isEmpty() ? capacity >= amt
                              : this.getRemainingSpace() >= amt
                && this.getActualFluid().equals(fs.getFluid());
    }
    ...
    public int getNBTInt(String key) {
        return this.getFluid() != null && this.getFluid().tag != null
            ? this.getFluid().tag.getInteger(key)
            : 0;
    }

    public String getNBTString(String key) {
        return this.getFluid() != null && this.getFluid().tag != null
            ? this.getFluid().tag.getString(key)
            : "";
    }

    public boolean getNBTBoolean(String key) {
        return this.getFluid() != null && this.getFluid().tag != null
            ? this.getFluid().tag.getBoolean(key)
            : false;
    }
}
