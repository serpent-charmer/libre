...
public abstract class PiecewiseExpression extends MathExpression {
    ...
    protected final void addComponent(
        double min, boolean inclmin, double max, boolean inclmax, MathExpression exp
    ) {
    ...
    private static class Range implements Comparable<Range> {
        ...
        public int hashCode() {
            return (int) (lowerLimit * 100)
                ^ (int) (upperLimit * 100) + (inclLower ? 50000 : 0)
                + (inclUpper ? 1000000 : 0);
        }
        ...
        public boolean equals(Object o) {
            if (o instanceof Range) {
                Range r = (Range) o;
                return r.lowerLimit == lowerLimit && r.upperLimit == upperLimit
                    && r.inclLower == inclLower && r.inclUpper == inclUpper;
            }
            return false;
        }
        ...
        @Override
        public final String toString() {
            return (inclLower ? "[" : "(") + lowerLimit + "," + upperLimit
                + (inclUpper ? "]" : ")");
        }
    }
}
