    ...
    public void processCommand(ICommandSender ics, String[] args) {
        ...
        if (args.length == 1) {
            if (args[0].equals("cleartags")) {
                ...
                this.sendChatToSender(
                    ics, EnumChatFormatting.GREEN + "NBT tags cleared."
                );
            } else {
                ...
                map.put(args[0], !e);
                this.sendChatToSender(
                    ics, EnumChatFormatting.GREEN + "Flag '" + args[0] + "' toggled."
                );
            }
            return;
            ...
            this.sendChatToSender(
                ics,
                EnumChatFormatting.GREEN + String.valueOf(args.length) + " tags added."
            );
            return;
        }
        ...
        if (click.containsKey(ep)) {
            ...
            try {
                s.save();
                this.sendChatToSender(
                    ics, EnumChatFormatting.GREEN + "Structure exported."
                );
            } catch (IOException e) {
                this.sendChatToSender(
                    ics,
                    EnumChatFormatting.RED + "Structure not exported: " + e.toString()
                );
                e.printStackTrace();
            }
        } else {
