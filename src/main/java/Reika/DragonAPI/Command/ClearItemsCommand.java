...
public class ClearItemsCommand extends DragonCommandBase {
    ...
    public void processCommand(ICommandSender ics, String[] args) {
        ...
        clearAll = false;
        if (args.length == 0
            || (args.length == 1 && args[0].toLowerCase(Locale.ENGLISH).equals("scan"))) {
            this.sendChatToSender(
                ics,
                EnumChatFormatting.RED.toString()
                    + "You must specify at least one ID, or a '*'!"
            );
            return;
        }
        boolean scan = false;
    ...
    private void parseArgs(ICommandSender ics, String[] args, boolean scan) {
        if (args[0].equals("*")) {
            ...
            this.sendChatToSender(
                ics, EnumChatFormatting.GREEN.toString() + "Cleared all dropped items."
            );
        } else {
            if (scan) {
                ...
                for (int i = 0; i < args.length; i++) {
                    ...
                    KeyedItemStack ks = new KeyedItemStack(new ItemStack(item))
            ...
            this.sendChatToSender(
                ics,
                EnumChatFormatting.GREEN.toString() + "Cleared all items with IDs '"
    ...
    public static boolean clearItem(EntityItem ei) {
        return System.currentTimeMillis() - clearTime < 1000
            && (clearAll
                || clearIDs.contains(Item.getIdFromItem(ei.getEntityItem().getItem())));
    }
}
