...
public class PopulateMinimapCommand extends DragonCommandBase {
        ...
        if (args.length < 1 || args.length > 3) {
            this.sendChatToSender(
                ...
                EnumChatFormatting.RED
                    + "Invalid arguments. Format: /populatemap [range] <stepsize> <delay>"
            );
            return;
        }
        if (!init) {
    ...
    private static class PlayerMovement {
        ...
        private boolean update() {
            ...
            if (flag1) {
                posZ += stepSize;
                //ReikaJavaLibrary.pConsole(posY+" > "+posZ+":"+range+" >
            ...
            DragonAPICore.log(
                "Populating map: Stepping player " + player.getCommandSenderName()
                + " to " + posX + ", " + posZ
            );
            return false;
        }
