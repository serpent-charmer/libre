...
public class LookInfoCommand extends DragonCommandBase {
    ...
    public void processCommand(ICommandSender ics, String[] args) {
        ...
        for (double d = 0; d < 16; d += 0.25) {
            ...
            entities.addAll(ep.worldObj.getEntitiesWithinAABB(
                Entity.class,
                AxisAlignedBB.getBoundingBox(dx, dy, dz, dx, dy, dz).expand(1, 1, 1)
            ));
            ...
            this.sendChatToSender(
                ics,
                EnumChatFormatting.GREEN + "Data for block at " + x + ", " + y + ", " + z
                    + ":"
            );
            this.sendChatToSender(
                ics,
                "Block ID: " + Block.getIdFromBlock(b) + " = '" + b.getUnlocalizedName()
                    + "', C=" + b.getClass()
            );
            this.sendChatToSender(
                ics, "Metadata: " + ep.worldObj.getBlockMetadata(x, y, z)
            );
            this.sendChatToSender(
                ics,
                "H=" + b.getBlockHardness(ep.worldObj, x, y, z) + "/E="
                    + b.getExplosionResistance(null, ep.worldObj, x, y, z, x, y, z)
                    + "/L=" + b.getLightValue(ep.worldObj, x, y, z)
                    + "/O=" + b.getLightOpacity(ep.worldObj, x, y, z)
            );
            ...
            if (entities.isEmpty()) {
                ...
                for (Entity e : entities) {
                    EntityRegistration er
                        = EntityRegistry.instance().lookupModSpawn(e.getClass(), true);
                    this.sendChatToSender(
                        ics,
                        (er != null ? er.getContainer().getName() : "[No Mod]") + " : "
                            + e.getClass().getName()
                    );
                }
            }
