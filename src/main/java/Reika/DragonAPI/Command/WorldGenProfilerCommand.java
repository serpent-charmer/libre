        ...
        switch (args[0].toLowerCase(Locale.ENGLISH)) {
            case "disable":
                ...
                this.sendChatToSender(
                    ics, EnumChatFormatting.GREEN + "Profiling finished."
                );
                break;
            case "enable":
                World world = ics instanceof EntityPlayerMP
                    ? this.getCommandSenderAsPlayer(ics).worldObj
                    : DimensionManager.getWorld(Integer.parseInt(args[1]));
                if (WorldgenProfiler.enableProfiling(world))
                    this.sendChatToSender(
                        ics,
                        EnumChatFormatting.GREEN + "Profiling started in world #"
                    ...
                    this.sendChatToSender(
                        ...
                        EnumChatFormatting.RED
                            + "Profiling already running or its hooks are not enabled!"
                    );
                break;
            case "display":
                ...
                String desc = "Profiling data for world #" + worldid + " contains "
                    + li.size() + " generators across " + chunks + " chunks, total time "
                    + totalt + " ms:";
                //this.sendChatToSender(ics, desc);
                DragonAPICore.log(desc);
                for (GeneratorProfile g : li) {
                    long time = g.getAverageTime();
                    String s = ReikaStringParser.padToLength(
                        "'" + g.identifier + "'", 96, " "
                    );
                    double percent = g.getTotalTime() * 100D / total;
                    //String sg = String.format("Generator %s - Average Time Per Chunk:
                    //%7.3f microseconds (%2.3f%s); Spilled chunks: %4d; Block changes:
                    //%7d", s, time/1000D, percent, "%%", g.getSpilledChunks(),
                    //g.getBlockChanges());
                    String sg = String.format(
                        "Generator %s - Average Time Per Chunk: %7.3f microseconds (%2.3f%s); Spilled chunks: %4d",
                        s,
                        time / 1000D,
                        percent,
                        "%%",
                ...
                this.sendChatToSender(
                    ics, EnumChatFormatting.GREEN + "Profiling data written to log."
                );
                break;
            default:
                this.sendChatToSender(
                    ics,
                    EnumChatFormatting.RED
                        + "Invalid argument. Specify 'disable', 'enable', or 'display'."
                );
                break;
        }
    }
