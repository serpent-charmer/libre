...
public class FindBiomeCommand extends DragonCommandBase {
    ...
    public void processCommand(ICommandSender ics, String[] args) {
        ...
        int[] rstep = target != null ? new int[] { 2048, 8192, 16384, 32768, 65536 }
                                     : new int[] { 4096 };
        boolean found = false;
        ...
        for (int i = 0; i < rstep.length && !found; i++) {
            ...
            for (int x = -r; x <= r; x += step) {
                for (int z = -r; z <= r; z += step) {
                    BiomeGenBase b = ReikaWorldHelper.getNaturalGennedBiomeAt(
                        ep.worldObj, x0 + x, z0 + z
                    );
            ...
            if (target == null) {
                for (String n : map.keySet()) {
                    this.sendChatToSender(
                        ics, "Search R=" + r + ": " + n + ": " + map.get(n)
                    );
                }
            } else {
                this.sendChatToSender(
                    ics,
                    "Search R=" + r + ": " + target.biomeName + ": "
                        + map.get(target.biomeName)
                );
            }
        }
    }
