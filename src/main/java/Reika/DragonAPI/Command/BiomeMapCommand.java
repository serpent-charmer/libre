...
public class BiomeMapCommand extends DragonCommandBase {
    ...
    public void processCommand(ICommandSender ics, String[] args) {
        ...
        if (args.length < 2) {
            this.sendChatToSender(
                ...
                ) + "Illegal arguments. Use [seed=<seed>] [range] [resolution] <grid> <fullGrid>."
            );
            return;
        }
    ...
    public static void triggerBiomeMap(
        EntityPlayerMP ep,
        int x,
        int z,
        int range,
        int res,
        int grid,
        MapCompleteCallback call
    ) {
        instance.generateMap(
            new WorldBiomes(ep.worldObj),
            ep,
    ...
    public static void triggerBiomeMap(EntityPlayerMP ep, int range, int res, int grid) {
        instance.processCommand(
            ep,
            new String[] {
                String.valueOf(range), String.valueOf(res), String.valueOf(grid) }
        );
    }
    ...
    private void generateMap(
        ...
        if (DragonAPICore.isSinglePlayer()) {
            this.startCollecting(
                hash, bp.getName(), dim, x, z, range, res, grid, fullGrid
            );
        } else {
            ReikaPacketHelper.sendStringIntPacket(
                DragonAPIInit.packetChannel,
                PacketIDs.BIOMEPNGSTART.ordinal(),
                ep,
                bp.getName(),
                hash,
                dim,
        ...
        for (int dx = x - range; dx <= x + range; dx += res) {
            for (int dz = z - range; dz <= z + range; dz += res) {
                ...
                //PacketIDs.BIOMEPNGDAT.ordinal(), ep, hash, dx, dz, b.biomeID);
                n++;
                if (DragonAPICore.isSinglePlayer()) {
        ...
        if (dat.size() > 1) {
            ...
            if (DragonAPICore.isSinglePlayer()) {
                ...
                ReikaPacketHelper.sendDataPacket(
                    DragonAPIInit.packetChannel, PacketIDs.BIOMEPNGDAT.ordinal(), ep, dat
                );
                n = 0;
                dat.clear();
                dat.add(hash);
        ...
        if (DragonAPICore.isSinglePlayer()) {
            ...
            ReikaPacketHelper.sendDataPacket(
                DragonAPIInit.packetChannel, PacketIDs.BIOMEPNGEND.ordinal(), ep, hash
            );
        }
    }
        ...
        if (map != null) {
            try {
                ...
                ReikaChatHelper.sendChatToPlayer(
                    ...
                    EnumChatFormatting.GREEN + "File created in " + dur + " ms: " + path
                );
            } catch (IOException e) {
                ReikaChatHelper.sendChatToPlayer(
                    Minecraft.getMinecraft().thePlayer,
                    EnumChatFormatting.RED + "Failed to create file: " + e.toString()
                );
                e.printStackTrace();
            }
        }
    ...
    private static interface BiomeProvider {
        //public String getFileName(long seed, String name, int x, int z, int range, int
        //res, int grid, boolean fullGrid);
    ...
    private static class WorldBiomes implements BiomeProvider {
        ...
        public int getBiome(int x, int z) {
            if (world.provider instanceof CustomBiomeDistributionWorld) {
                return ((CustomBiomeDistributionWorld) world.provider)
                    .getBiomeID(world, x, z);
            }
            BiomeGenBase b = world.getChunkProvider().chunkExists(x >> 4, z >> 4)
    ...
    private static class SeedBiomes implements BiomeProvider {
        ...
        @Override
        public String getFileName(long seed, String name, int x, int z, int range, int
        res, int grid, boolean fullGrid) { return "BiomeMap/Forced/"+worldName+"; "+x+",
        "+z+" ("+sr+"x"+sr+"; [R="+res+" b-px, G="+grid+"-"+fullGrid+"]).png";
        }*/
    ...
    private static class BiomeMap extends MapOutput<Integer> {
        private BiomeMap(
            String name, int dim, int x, int z, int r, int res, int grid, boolean fgrid
        ) {
        ...
        private void
        createLegendEntry(int b, int x, int y, Graphics g, BufferedImage img, int hpb) {
            BiomeGenBase biome = BiomeGenBase.biomeList[b];
            g.drawString(
                biome.biomeName + " [" + biome.biomeID + "]", x + hpb + 4, y + hpb / 2 + 4
            );
            for (int i = -1; i <= hpb; i++) {
                for (int k = -1; k <= hpb; k++) {
                    int clr = i == -1 || k == -1 || i == hpb || k == hpb
                        ? 0xff000000
                        : 0xff000000 | getBiomeColor(i * 12, k * 12, biome);
    ...
    public static int getBiomeColor(int x, int z, BiomeGenBase b) {
        ...
        if (b instanceof CustomMapColorBiome)
            return ((CustomMapColorBiome) b)
                .getMapColor(Minecraft.getMinecraft().theWorld, x, z);
        ...
        if (mutate) {
            ...
            c = c == 0xffffff ? 0xd0d0d0
                              : ReikaColorAPI.getColorWithBrightnessMultiplier(c, 1.125F);
        }
