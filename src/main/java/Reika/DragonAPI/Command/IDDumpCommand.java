...
public class IDDumpCommand extends DragonCommandBase {
    ...
    public void processCommand(ICommandSender ics, String[] args) {
        if (args.length != 2) {
            this.sendChatToSender(
                ...
                EnumChatFormatting.RED + "Invalid arguments. Use /"
                    + this.getCommandString() + " <idtype> <side>."
            );
            return;
        }
        ...
        try {
            ...
            sb.append(
                EnumChatFormatting.RED + "Invalid id type. Use one of the following: "
            );
        ...
        try {
            ...
            StringBuilder sb = new StringBuilder();
            sb.append(
                EnumChatFormatting.RED + "Invalid side. Use one of the following: "
            );
    ...
    public static void dumpClientside(int type) {
        ...
        for (String s : data.keySet()) {
            String sg = String.format(
                "Client %s ID %d = %s",
                ReikaStringParser.capFirstChar(id.name()),
                data.get(s),
                s
            );
        ...
        switch (side) {
            ...
            case SERVER:
                ...
                for (String s : data.keySet()) {
                    String sg = String.format(
                        "%s %s ID %d = %s",
                        ReikaStringParser.capFirstChar(side.name()),
                        type.getName(),
                        data.get(s),
                        s
                    );
    ...
    private void sendPacket(EntityPlayerMP ep, IDType type) {
        ReikaPacketHelper.sendDataPacket(
            DragonAPIInit.packetChannel, PacketIDs.IDDUMP.ordinal(), ep, type.ordinal()
        );
    }
