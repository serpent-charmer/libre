        ...
        if (args.length != 5) {
            this.sendChatToSender(
                ...
                EnumChatFormatting.RED
                    + "Invalid arguments. Use [dim] [perTick] [ctrX] [ctrZ] [radius]."
            );
            return;
        }
        try {
            ...
            this.sendChatToSender(
                ics,
                EnumChatFormatting.GREEN + "Queued a pregen of chunks R=" + radius
                    + " around " + ctrX + "," + ctrZ + " in DIM" + dim + ". Genning "
                    + pertick + " c/t."
            );
        } catch (NumberFormatException e) {
            this.sendChatToSender(
                ics, EnumChatFormatting.RED + "Error parsing argument: " + e.getMessage()
            );
            e.printStackTrace();
        }
    }
