...
public class MemoryUsageCommand extends DragonCommandBase {
    ...
    public void processCommand(ICommandSender ics, String[] args) {
        ...
        sendChatToSender(
            ics,
            "Used memory: " + MathHelper.floor_double(((double) used / max) * 100)
                + "%% (" + this.bytesToMB(used) + "MB) of " + this.bytesToMB(max) + "MB"
        );
        sendChatToSender(
            ics,
            "Allocated memory: "
                + MathHelper.floor_double(((double) allocated / max) * 100) + "%% ("
                + this.bytesToMB(allocated) + " MB)"
        );
    }
