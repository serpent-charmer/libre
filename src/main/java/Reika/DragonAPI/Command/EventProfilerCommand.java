        ...
        switch (args[0].toLowerCase(Locale.ENGLISH)) {
            ...
            case "enable":
                if (args.length < 2) {
                    this.sendChatToSender(
                        ics,
                        EnumChatFormatting.RED + "You must specify an event type (class)!"
                    );
                    return;
                }
                    ...
                    case SUCCESS:
                        this.sendChatToSender(
                            ...
                            EnumChatFormatting.GREEN
                                + "Profiling started for events of type " + args[1]
                        );
                        break;
                    case ALREADYRUNNING:
                        this.sendChatToSender(
                            ics, EnumChatFormatting.RED + "Profiling already running!"
                        );
                        break;
                    case NOSUCHCLASS:
                        this.sendChatToSender(
                            ics,
                            EnumChatFormatting.RED + "No such class '" + args[1] + "'!"
                        );
                        break;
                    case NOTANEVENT:
                        this.sendChatToSender(
                            ics,
                            EnumChatFormatting.RED + "Class '" + args[1]
            ...
            case "display":
                ...
                String desc = "Profiling data for event type " + type + " contains "
                    + li.size() + " handlers across " + fires
                    + " event fires, total time " + totalt + " ms:";
                ...
                for (EventProfile g : li) {
                    ...
                    String sg = String.format(
                        "Handler %s - Average Time Per Fire: %7.3f microseconds (%2.3f%s)",
                        s,
                        time / 1000D,
                        percent,
                        "%%"
