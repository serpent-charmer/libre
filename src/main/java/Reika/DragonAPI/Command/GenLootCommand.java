...
public class GenLootCommand extends DragonCommandBase {
            ...
            if (inv != null) {
                if (args.length == 3 && args[1].equalsIgnoreCase("preset")) {
                    if (inv instanceof IInventory) {
                        try {
                            Presets p
                                = Presets.valueOf(args[2].toUpperCase(Locale.ENGLISH));
                            p.generate(DragonAPICore.rand, (IInventory) inv);
                        } catch (IllegalArgumentException e) {
                            this.sendChatToSender(
                                ics,
                                EnumChatFormatting.RED + "Preset '" + args[2]
                                    + "' does not exist. Options: "
                                    + Arrays.toString(Presets.values())
                            );
                        }
                    } else {
                        this.sendChatToSender(
                            ...
                            EnumChatFormatting.RED
                                + "Presets only work with inventory targets."
                        );
                    }
                } else {
                    WeightedRandomChestContent[] types
                        = ChestGenHooks.getItems(args[0], ep.getRNG());
                    if (types.length == 0) {
                        this.sendChatToSender(
                            ics,
                            EnumChatFormatting.RED + "Loot table " + args[0]
                                + " is empty."
                        );
                    } else {
                        int count = ChestGenHooks.getCount(args[0], ep.getRNG());
                        int tries
                            = args.length > 1 && ReikaJavaLibrary.isValidInteger(args[1])
                ...
                this.sendChatToSender(
                    ics, EnumChatFormatting.RED + "No selected inventory."
                );
            }
        } else {
            try {
                ...
                HashMap<String, ChestGenHooks> map
                    = (HashMap<String, ChestGenHooks>) f.get(null);
                this.sendChatToSender(
                    ics, EnumChatFormatting.RED + "You must specify a loot table."
                );
    ...
    private void addItems(
        ...
        if (InterfaceCache.GRIDHOST.instanceOf(inv)) {
            ...
            this.sendChatToSender(
                ics,
                EnumChatFormatting.GREEN + "Generated loot type " + type
                    + " in ME System @ " + new Coordinate((TileEntity) inv)
            );
        } else if (inv instanceof EntityMinecartChest) {
            WeightedRandomChestContent.generateChestContents(
                ep.getRNG(), types, (EntityMinecartChest) inv, count
            );
            this.sendChatToSender(
                ics,
                EnumChatFormatting.GREEN + "Generated loot type " + type
                    + " in minecart @ " + new Coordinate((Entity) inv)
            );
        } else if (inv instanceof IInventory) {
            WeightedRandomChestContent.generateChestContents(
                ep.getRNG(), types, (IInventory) inv, count
            );
            this.sendChatToSender(
                ics,
                EnumChatFormatting.GREEN + "Generated loot type " + type + " in chest @ "
                    + new Coordinate((TileEntity) inv)
            );
        }
    }

    @ModDependent(ModList.APPENG)
    private void handleAE(
        EntityPlayer ep,
        Random rand,
        WeightedRandomChestContent[] types,
        IGridHost te,
