...
public class EditNearbyInventoryCommand extends DragonCommandBase {
    ...
    public void processCommand(ICommandSender ics, String[] args) {
        ...
        if (args.length != 5) {
            ReikaChatHelper.sendChatToPlayer(
                ...
                EnumChatFormatting.RED
                    + "You must specify an action, a range, and an ItemStack."
            );
            return;
        }
        ...
        if (is == null) {
            ReikaChatHelper.sendChatToPlayer(
                ep, EnumChatFormatting.RED + "No such item '" + args[2] + "'."
            );
            return;
        }
        ...
        if (mode == null) {
            ReikaChatHelper.sendChatToPlayer(
                ep, EnumChatFormatting.RED + "No such action '" + args[0] + "'."
            );
            return;
        }
        ...
        if (r <= 0) {
            ReikaChatHelper.sendChatToPlayer(
                ep, EnumChatFormatting.RED + "Invalid range '" + args[1] + "'."
            );
            return;
        }
        ...
        if (num < 0) {
            ReikaChatHelper.sendChatToPlayer(
                ep, EnumChatFormatting.RED + "No inventories within range."
            );
            return;
        }
        String base
            = "%sSuccessfully %s inventories for %s within%s%s%d blocks of %d, %d, %d. %s%s";
        String act = mode == Mode.COUNT ? num + " items present."
                                        : num + "/" + is.stackSize + " changes made.";
    ...
    private static enum Mode {
        ...
        private static Mode get(String mode) {
            for (int i = 0; i < values().length; i++) {
                ...
                if (m.name()
                        ...
                        .equals(mode.toLowerCase(Locale.ENGLISH)))
                    return m;
            }
        ...
        public int perform(World world, int x, int y, int z, ItemStack is, int r) {
            Collection<IInventory> c
                = ReikaWorldHelper.getAllInventories(world, x, y, z, r);
            if (c.isEmpty())
                return -1;
            int count = 0;
            for (IInventory ii : c) {
                ...
                if (this == ADD) {
                    ItemStack is2
                        = ReikaItemHelper.getSizedItemStack(is, is.stackSize - count);
                    count += is2.stackSize
                        - ReikaInventoryHelper.addToInventoryWithLeftover(is2, ii, false);
