    ...
    public void processCommand(ICommandSender ics, String[] args) {
        if (!Minecraft.getMinecraft().gameSettings.snooperEnabled) {
            this.sendChatToSender(
                ics, EnumChatFormatting.RED + "You need snooper enabled for this to work!"
            );
            return;
        }
        ...
        if (f == null) {
            this.sendChatToSender(
                ...
                EnumChatFormatting.RED
                    + "Could not export environment file. Check your logs for errors."
            );
        } else {
            this.sendChatToSender(
                ics, EnumChatFormatting.GREEN + "Game environment exported to file:"
            );
