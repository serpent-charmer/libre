...
public class ConfigReloadCommand extends DragonCommandBase {
    ...
    public void processCommand(ICommandSender ics, String[] args) {
        if (args[0].equals("all") || args[0].equals("*")) {
            ...
            this.sendChatToSender(
                ...
                EnumChatFormatting.GREEN
                    + "All configs reloaded. Note that some settings may require restart."
            );
            return;
        }
        ...
        if (c == null) {
            this.sendChatToSender(
                ics, EnumChatFormatting.RED + "No such config for mod '" + args[0] + "'!"
            );
        } else {
            c.reload();
            ...
            this.sendChatToSender(
                ics,
                EnumChatFormatting.GREEN + "Config for '" + args[0]
                    + "' reloaded. Note that some settings may require restart."
            );
        }
    }
