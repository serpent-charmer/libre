...
public class ContainerBasicStorage extends CoreContainer {
    ...
    public ContainerBasicStorage(EntityPlayer player, TileEntity te) {
        ...
        for (int i = 0; i < numRows; i++) {
            for (int j = 0; j < 9; j++) {
                this.addSlotToContainer(
                    this.getSlotOfType(lowerInv, j + i * 9, 8 + j * 18, 18 + i * 18)
                );
            }
        }
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 9; j++) {
                this.addSlotToContainer(this.getSlotOfType(
                    player.inventory, j + i * 9 + 9, 8 + j * 18, 103 + i * 18 + dy
                ));
            }
        }
        ...
        for (int i = 0; i < 9; i++) {
            this.addSlotToContainer(
                this.getSlotOfType(player.inventory, i, 8 + i * 18, 161 + dy)
            );
        }
    }
