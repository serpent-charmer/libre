...
public abstract class DragonAPIMod {
    ...
    protected DragonAPIMod() {
        ...
        if (isDeObf) {
            ReikaJavaLibrary.pConsole(
                this.getDisplayName() + " is running in a deobfuscated environment!"
            );
        } else {
            ReikaJavaLibrary.pConsole(
                this.getDisplayName() + " is not running in a deobfuscated environment."
            );
        }
        ...
        ReikaJavaLibrary.pConsole(
            ...
            + ": Constructed; Active Classloader is: " + this.getClass().getClassLoader()
        );
    ...
    /**
     * Fired after preload (right after the last DragonAPIMod fires preinit) but before
     * load
     */
    ...
    protected final void basicSetup(FMLPreInitializationEvent evt) {
        ...
        fileHash = this.isSource()
            ? "Source"
            : ReikaFileReader.getHash(this.getModFile(), HashType.SHA256);
    ...
    private static void checkFinalPreload(DragonAPIMod mod) {
        preInitSet.add(mod);
        DragonAPICore.log(
            "Pre-initialized " + mod.getTechnicalName() + "; preloaded "
            + preInitSet.size() + "/" + mods.size() + " mods."
        );
        if (preInitSet.size() == mods.size()) {
            DragonAPICore.log(
                "Finished all main preinit phases. Running post-pre init phase on "
                + preInitSet.size() + " mods."
            );
    ...
    private void verifyHash() {
        if (this.getModVersion().isCompiled()) {
            try {
                ...
                InputStream folder = ReikaFileReader.getFileInsideJar(
                    jf, ReikaJavaLibrary.getTopLevelPackage(this.getClass()) + "/"
                );
                String hash
                    = "@TEMPHASH@"; //ReikaFileReader.getHash(folder, HashType.SHA256);
                Manifest mf = jf.getManifest();
                if (mf == null)
                    throw new InvalidBuildException(
                        this, this.getModFile(), "No manifest found!"
                    );
                ...
                if (attr == null || hash == null || !hash.equals(attr))
                    throw new InvalidBuildException(
                        this,
                        this.getModFile(),
                        attr == null ? "Manifest does not contain hash!"
                                     : "Mismatched compile hash!"
                    );
            } catch (IOException e) {
                throw new InvalidBuildException(
                    this, this.getModFile(), "Threw IOException: " + e.toString()
                );
            }
        }
    }
    ...
    private final void verifyVersions() {
        ...
        if (mod.verify()) {
            if (mod.majorVersion != apiVersion.majorVersion
                || mod.isNewerMinorVersion(apiVersion)) {
                throw new APIMismatchException(
                    this, mod, apiVersion, DragonAPICore.last_API_Version
                );
            }
    ...
    public final URL getWikiLink() {
        try {
            ...
            throw new RegistrationException(
                this, "The mod provided a malformed URL for its documentation site!"
            );
        }
    }
    ...
    protected void hasNoDragonAPI() {
        throw new InstallationException(
            this, "This mod needs DragonAPI to function correctly!"
        );
    }
    ...
    public final boolean equals(Object o) {
        return o.getClass() == this.getClass()
            && ((DragonAPIMod) o)
                   .getTechnicalName()
                   .equalsIgnoreCase(this.getTechnicalName());
    }
    ...
    public static final class LoadProfiler {
        ...
        private void startTiming(LoadPhase p) {
            if (time != -1)
                throw new IllegalStateException(
                    mod.getTechnicalName() + " is already profiling phase " + phase + "!"
                );
            if (loaded.containsKey(p) && loaded.get(p))
                throw new IllegalStateException(
                    mod.getTechnicalName() + " already finished profiling phase " + phase
                    + "!"
                );
            phase = p;
        ...
        protected void finishTiming() {
            ...
            if (time == -1)
                throw new IllegalStateException(
                    mod.getTechnicalName() + " cannot stop profiling before it starts!"
                );
            time = -1;
            String s = ReikaDateHelper.millisToHMSms(duration);
            ReikaJavaLibrary.pConsole(
                mod.getTechnicalName() + ": Completed loading phase " + phase + " in "
                + duration + " ms (" + s + ")."
            );
            if (duration > 1800000) { //30 min
                ReikaJavaLibrary.pConsole(
                    "Loading time exceeded thirty minutes, indicating very weak hardware. Beware of low framerates."
                );
            } else if (duration > 300000) { //5 min
                ReikaJavaLibrary.pConsole(
                    "Loading time exceeded five minutes, indicating weaker hardware. Consider reducing settings."
                );
            }
            total += duration;
            if (phase == LoadPhase.POSTLOAD)
                ReikaJavaLibrary.pConsole(
                    "Total mod loading time: " + total + " ms ("
                    + ReikaDateHelper.millisToHMSms(total) + ")."
                );
            phase = null;
        }
