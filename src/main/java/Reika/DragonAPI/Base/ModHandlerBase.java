...
public abstract class ModHandlerBase {
    ...
    private void verifyInstanceField() {
        if (!ReikaReflectionHelper.checkForField(
                this.getClass(),
                "instance",
                Modifier.PRIVATE,
                Modifier.STATIC,
            ...
            throw new MisuseException(
                "A mod handler must have a private static final 'instance' field!"
            );
        if (!ReikaReflectionHelper.checkForMethod(
                this.getClass(),
                "getInstance",
                new Class[0],
                Modifier.PUBLIC,
            ...
            throw new MisuseException(
                "A mod handler must have a public static 'getInstance()' method!"
            );
    }
    ...
    public static final class SemanticVersionHandler implements VersionHandler {
        ...
        public boolean acceptVersion(String version) {
            boolean min
                = (minimum == null
                   || SemanticVersionParser.isVersionAtLeast(minimum, version));
            boolean max
                = (maximum == null
                   || SemanticVersionParser.isVersionAtMost(version, maximum));
            return min && max;
        }
