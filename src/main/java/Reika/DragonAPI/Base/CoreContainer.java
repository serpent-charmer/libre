...
public class CoreContainer extends Container {
    ...
    protected void addPlayerInventoryWithOffset(EntityPlayer player, int dx, int dy) {
        for (int i = 0; i < 3; i++) {
            for (int k = 0; k < 9; k++) {
                this.addSlotToContainer(new Slot(
                    player.inventory, k + i * 9 + 9, dx + 8 + k * 18, dy + 84 + i * 18
                ));
            }
        }
        ...
        for (int j = 0; j < 9; j++) {
            this.addSlotToContainer(
                new Slot(player.inventory, j, dx + 8 + j * 18, dy + 142)
            );
        }
    }
    ...
    public void detectAndSendChanges() {
        ...
        for (int i = 0; i < inventorySlots.size(); ++i) {
            ...
            //DragonAPICore.log("TRY: "+itemstack+":"+itemstack1, itemstack != null &&
        itemstack1 != null); if (!ItemStack.areItemStacksEqual(itemstack1, itemstack) ||
        true) { //the true is to force a sync itemstack1 = itemstack == null ? null :
        itemstack.copy(); inventoryItemStacks.set(i, itemstack1);
    ...
    public final boolean isStandard8mReach(EntityPlayer player) {
        double dist = ReikaMathLibrary.py3d(
            tile.xCoord + 0.5 - player.posX,
            tile.yCoord + 0.5 - player.posY,
            tile.zCoord + 0.5 - player.posZ
        );
        return (dist <= 8);
    }
    ...
    public final ItemStack transferStackInSlot(EntityPlayer player, int slot) {
        ...
        if (fromSlot != null && fromSlot.getHasStack()) {
            ...
            if (toPlayer) {
                for (int i = invsize + base;
                     i < inventorySlots.size() && is.stackSize > 0;
                     i++) {
                    //DragonAPICore.log(i);
                    ...
                    if (toSlot.isItemValid(is) && this.canAdd(is, toSlot.getStack())) {
                        if (!toSlot.getHasStack()) {
                            ...
                            ItemStack toAdd = ReikaItemHelper.getSizedItemStack(
                                is, inToSlot.stackSize + add
                            );
                            //DragonAPICore.log(is+" to "+inToSlot+" for "+toAdd+", by
                            //"+add);
                ...
                for (int i = base; i < ((IInventory) tile).getSizeInventory()
                     && i < list.size() && is.stackSize > 0;
                     i++) {
                    Slot toSlot = list.get(i);
                    int lim = ((IInventory) tile).getInventoryStackLimit();
                    //DragonAPICore.log(i+" "+toSlot+":"+toSlot.getSlotIndex()+" E
                    //["+base+", "+((IInventory)tile).getSizeInventory()+") >
                    //"+toSlot.isItemValid(is), Side.SERVER);
                    if (toSlot.isItemValid(is)
                        && (((IInventory) tile).isItemValidForSlot(i, is))
                        && this.canAdd(is, toSlot.getStack())) {
                        if (!toSlot.getHasStack()) {
                            if (is.stackSize <= lim) {
                                toSlot.putStack(is.copy());
                            ...
                            ItemStack inToSlot = toSlot.getStack();
                            int add = Math.min(
                                inToSlot.getMaxStackSize() - inToSlot.stackSize,
                                lim - inToSlot.stackSize
                            );
                            if (add > is.stackSize)
                                add = is.stackSize;
                            toSlot.putStack(ReikaItemHelper.getSizedItemStack(
                                is, inToSlot.stackSize + add
                            ));
                            is.stackSize -= add;
                        }
    ...
    private boolean canAdd(ItemStack is, ItemStack inslot) {
        ...
        return ReikaItemHelper.matchStacks(is, inslot)
            && ItemStack.areItemStackTagsEqual(is, inslot);
    }

    @Override //To avoid a couple crashes with some mods (or vanilla packet system) not
              //checking array bounds
              public Slot getSlot(int index) {
        if (index >= inventorySlots.size() || index < 0) {
            String o = "A mod tried to access an invalid slot " + index
                + " for TileEntity " + tile + ".";
            String o2
                = "It is likely assuming the TileEntity has an inventory when it does not.";
            String o3
                = "Check for any inventory-modifying mods and items you are carrying.";
            String o4 = "Slot List = " + inventorySlots.size() + ": "
            ...
            return new Slot(
                fakeChest, 0, -20, -20
            ); //create new slot off screen; hacky fix, but should work
        }
