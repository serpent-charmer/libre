...
public abstract class TileEntityRenderBase extends TileEntitySpecialRenderer {
    ...
    public final boolean isValidMachineRenderPass(TileEntityBase te) {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            int c = te.worldObj.getLightBrightnessForSkyBlocks(te.xCoord+dir.offsetX,
        te.yCoord+dir.offsetY, te.zCoord+dir.offsetZ, 0); b = Math.max(c, b);
        }
        if (te.hasWorldObj()) {
            int j = b % 65536;
            int k = b / 65536;
    ...
    public final int bindTextureByName(String tex) {
        ...
        if (this.loadXmasTextures()) {
            ...
            BufferedImage ret
                = ReikaImageLoader.readImage(this.getModClass(), xmas, null);
            String bind
                = ret != null && ret != ReikaImageLoader.getMissingTex() ? xmas : tex;
    ...
    public final int bindImageByName(String img) {
        return ReikaTextureHelper.bindTexture(
            this.getModClass(), this.getTextureFolder() + img
        );
    }
    ...
    protected final FontRenderer getFontRenderer() {
        return this.func_147498_b() != null ? this.func_147498_b()
