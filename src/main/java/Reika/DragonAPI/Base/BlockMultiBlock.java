...
public abstract class BlockMultiBlock<R>
    extends Block implements Transducerable, CustomCopyBehavior {
    ...
    public abstract R checkForFullMultiBlock(
    ...
    onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase e, ItemStack is) {
        if (!world.isRemote
            && this.canTriggerMultiBlockCheck(
                world, x, y, z, world.getBlockMetadata(x, y, z)
            )) {
            if (e instanceof EntityPlayer) {
                R ret = this.checkForFullMultiBlock(
                    world,
                    x,
                    y,
                    z,
                    ReikaEntityHelper.getDirectionFromEntityLook(e, false),
                    null
                ); //TODO: dummy callback?
    ...
    @Override
    public final void
    breakBlock(World world, int x, int y, int z, Block oldid, int oldmeta) {
    ...
    onCreateFullMultiBlock(World world, int x, int y, int z, R ret);
    ...
    public final IIcon getIcon(IBlockAccess world, int x, int y, int z, int side) {
        int index
            = this.getTextureIndex(world, x, y, z, side, world.getBlockMetadata(x, y, z));
