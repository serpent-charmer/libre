...
public class DragonAPICore {
    ...
    public static final String last_API_Version = "@MAJOR_VERSION@"
        + "@MINOR_VERSION@";
    ...
    public static final GameProfile serverProfile = new GameProfile(
        UUID.fromString("b9a1b954-6651-4bb8-af54-452a4d9fd5a4"), "[SERVER]"
    );
    private static GameProfile sessionUser = serverProfile;

    private static final String MINFORGE
        = "required-after:Forge@[10.13.4.1614,);"; //was 1205/1231/1291/1558
    ...
    public static final String FORUM_PAGE
        = "http://www.minecraftforum.net/topic/1969694-";
    public static final String GITHUB_PAGE
        = "https://github.com/ReikaKalseki/Reika_Mods_Issues/issues?q=";
    public static final UUID Reika_UUID
        = UUID.fromString("e5248026-6874-4954-9a02-aa8910d08f31");
    ...
    public static URL getReikaForumPage() {
        try {
            ...
            throw new RegistrationException(
                ...
                "Reika's mods provided a malformed URL for their documentation site!",
                e
            );
        }
    }
    ...
    public static URL getReikaGithubPage() {
        try {
            ...
            throw new RegistrationException(
                DragonAPIInit.instance,
                "Reika's mods provided a malformed URL for their github site!",
                e
            );
        }
    }
    ...
    public static File getServerRootFolder() {
        ...
        if (props.exists()) {
            String name
                = new PropertyManager(props).getStringProperty("level-name", "world");
    ...
    static {
        if (isReikasComputer())
            ReikaJavaLibrary.pConsole(
                "DRAGONAPI: Loading on Reika's computer; Dev features enabled."
            );
    ...
    private static void validateForgeVersions() {
        ...
        if (build < recbuild) {
            ...
            log(String.format(
                "Consider updating to at least %d.%d.%d.%d.", major, minor, rev, recbuild
            ));
        }
    }
    ...
    public static boolean isRemoteSinglePlayer() {
        return isOnActualServer()
            && MinecraftServer.getServer().getConfigurationManager().getMaxPlayers() == 1;
    }
    ...
    public static void debug(Object s) {
        ModLogger log = DragonAPIInit.instance == null
            ? null
            : DragonAPIInit.instance.getModLogger();
        if (log == null)
    ...
    public static void openURL(String url) throws Exception {
        ...
        oclass.getMethod("browse", new Class[] { URI.class })
            .invoke(object, new Object[] { new URI(url) });
    }
