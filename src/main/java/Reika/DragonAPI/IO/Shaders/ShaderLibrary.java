...
public class ShaderLibrary {
    ...
    private static final HashMap<String, Constructor<ComputedLibrary>> computed
        = new HashMap();
    ...
    static void loadLibraries() {
        if (libraries.isEmpty()) {
            try (
                InputStream in
                = DragonAPICore.class.getResourceAsStream("Resources/Shader/liblist.txt")
            ) {
                if (in == null)
                    ShaderRegistry.error(
                        DragonAPIInit.instance,
                        null,
                        "Shader library manifest not found",
                        null
                    );
                List<String> li
                    = ReikaFileReader.getFileAsLines(in, true, Charsets.UTF_8);
                for (String s : li) {
                ...
                ShaderRegistry.error(
                    DragonAPIInit.instance,
                    null,
                    "Failed to load shader library manifest",
                    null,
                    e
                );
            }
    ...
    protected List<String> load() {
        try (
            InputStream in = DragonAPICore.class.getResourceAsStream(
                "Resources/Shader/lib_" + name + ".txt"
            )
        ) {
            return ReikaFileReader.getFileAsLines(in, true, Charset.defaultCharset());
        } catch (IOException e) {
            ShaderRegistry.error(
                DragonAPIInit.instance, name, "Failed to load shader library", null, e
            );
    ...
    public static void registerComputedLibrary(
        DragonAPIMod mod, String id, Class<? extends ComputedLibrary> c
    ) {
        if ((c.getModifiers() & Modifier.ABSTRACT) != 0)
            throw new RegistrationException(
                mod,
                "Invalid computed library patch " + id + "/" + c.getName()
                    + ": class is abstract"
            );
        ...
        try {
            ...
            throw new RegistrationException(
                mod, "Invalid computed library patch " + id + "/" + c.getName(), e
            );
        }
    }
            ...
            for (int i = -r; i <= r; i++) {
                for (int k = -r; k <= r; k++) {
                    ...
                    if (f > 0) {
                        ...
                        li.add("sum += f;");
                        li.add(
                            "duv = uv+vec2(float(" + i + ")/float(screenWidth), float("
                            + k + ")/float(screenHeight));"
                        );
