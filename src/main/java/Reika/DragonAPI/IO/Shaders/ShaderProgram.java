...
public final class ShaderProgram implements Comparable<ShaderProgram> {
    ...
    public void load() throws IOException {
        ...
        try (InputStream vin = this.getShaderData(ShaderTypes.VERTEX);
             InputStream fin = this.getShaderData(ShaderTypes.FRAGMENT)) {
            Collection<ShaderLibrary> libs = new ArrayList();
            vertexID = ShaderRegistry.constructShader(
                owner, identifier, vin, ShaderTypes.VERTEX, libs
            );
            imports.addValues(ShaderTypes.VERTEX, libs);
            libs = new ArrayList();
            fragmentID = ShaderRegistry.constructShader(
                owner, identifier, fin, ShaderTypes.FRAGMENT, libs
            );
    ...
    public ShaderProgram setMatricesToCurrent() {
        return this.setMatrices(
            ReikaRenderHelper.getModelviewMatrix(),
            ReikaRenderHelper.getProjectionMatrix()
        );
    }
    ...
    addFocus(double x, double y, double z, Matrix4f model, Matrix4f proj) {
        ...
        compoundLocation.add(
            new RenderState(new Vector3f((float) x, (float) y, (float) z), model, proj)
        );
        return this;
    }

    public ShaderProgram
    modifyLastCompoundFocus(float intensity, HashMap<String, Object> vars) {
    ...
    boolean run() {
        ...
        if (focusLocation != null && modelview != null && projection != null) {
            Vector4f target = new Vector4f(focusLocation.x*0, focusLocation.y*0,
        focusLocation.z*0, 1.0F); Vector4f clipspace = Matrix4f.transform(modelview,
        target, null); clipspace = Matrix4f.transform(projection, clipspace, null);
            Vector3f ndcspace = new Vector3f(clipspace.x/clipspace.w,
        clipspace.y/clipspace.w, clipspace.z/clipspace.w); //should be [-1,+1] Vector2f
        screenXY = new Vector2f(ndcspace.x/2 + 0.5F, ndcspace.y/2 + 0.5F);
    ...
    void checkForError() {
        if (!errorChecked) {
            ...
            if (!data.isEmpty())
                ShaderRegistry.error(
                    owner,
                    identifier,
                    "Shader " + this + " threw errors: " + data + "!",
                    null
                );
        }
    }
    ...
    private void applyField(String f, Object val) {
        ...
        if (val instanceof Integer) {
            ...
            Vec3 vec = (Vec3) val;
            GL20.glUniform3f(
                loc, (float) vec.xCoord, (float) vec.yCoord, (float) vec.zCoord
            );
    ...
    private void register() {
        ...
        if (programID == 0) {
            ShaderRegistry.error(
                owner, identifier, "Shader program could not be assigned an ID!", null
            );
        }
        ...
        if (GL20.glGetProgrami(programID, GL20.GL_LINK_STATUS) == GL11.GL_FALSE) {
            ShaderRegistry.error(
                owner,
                identifier,
                "Shader was not linked properly: " + ShaderRegistry.parseError(programID),
                null
            );
        }
        ...
        if (GL20.glGetProgrami(programID, GL20.GL_VALIDATE_STATUS) == GL11.GL_FALSE) {
            ShaderRegistry.error(
                owner,
                identifier,
                "Shader failed to validate: " + ShaderRegistry.parseError(programID),
                null
            );
        }
    }
    ...
    public static class RenderState {
        ...
        private RenderState(
            Vector3f vec, Matrix4f m, Matrix4f p, float f, HashMap<String, Object> vars
        ) {
            position = vec;
            modelview = m;
            projection = p;
