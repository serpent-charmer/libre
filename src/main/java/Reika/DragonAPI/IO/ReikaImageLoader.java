...
public final class ReikaImageLoader {
    ...
    public static BufferedImage readImage(Class root, String name, ImageEditor editor) {
        DragonAPICore.log(
            "Pipelining texture from " + root.getCanonicalName() + " to " + name
        );

        try (InputStream inputfile = root.getResourceAsStream(name)) {
            if (inputfile == null) {
                DragonAPICore.logError(
                    "Image filepath at " + name
                    + " not found. Loading \"MissingTexture\"."
                );
                return missingtex;
            }
    ...
    getImageFromResourcePack(String path, IResourcePack res, ImageEditor editor) {
        if (DragonOptions.LOGLOADING.getState())
            DragonAPICore.log(
                "Loading image at " + path + " from resourcepack " + res.getPackName()
            );
        ...
        try (InputStream in = ReikaTextureHelper.getStreamFromTexturePack(path, pack)) {
            if (in == null) {
                if (DragonOptions.LOGLOADING.getState())
                    DragonAPICore.logError(
                        "Texture pack image at " + path + " not found in "
                        + res.getPackName() + "."
                    );
                return null;
            }
            ...
            if (DragonOptions.LOGLOADING.getState())
                DragonAPICore.logError(
                    "Texture pack image at " + path + " not found in " + res.getPackName()
                    + "."
                );
            //e.printStackTrace();
            return null;
    ...
    /*
    public static BufferedImage readTexturePackImage(ITexturePack ip, String name) throws
    IOException
    {
        ...
        if (inputfile == null) {
            throw new IOException("IOException on loading texture pack image variant for
    texturepack "+ip.getTexturePackID()+". Loading default textures.");
        }
    ...
    public static void
    unstitchIconsFromSheet(IIcon[] icons, IIconRegister ico, String name, int numIcons) {
        IIcon[] icos = unstitchIcons(ico, name, numIcons);
        System.arraycopy(icos, 0, icons, 0, icos.length);
    }

    public static void unstitchIconsFromSheet(
        IIcon[][] icons, IIconRegister ico, String name, int cols, int rows
    ) {
    ...
    private static class IconSheet extends TextureAtlasSprite {
        ...
        public boolean load(IResourceManager manager, ResourceLocation location) {
            ...
            BufferedImage image;
            try {
                IResource res = manager.getResource(
                    new ResourceLocation(location.getResourceDomain(), fileName)
                );
                try (InputStream in = res.getInputStream()) {
                    image = ImageIO.read(in);
                } catch (IOException ex) {
                    DragonAPICore.logError(
                        "Failed to load sub-texture from " + fileName + ": "
                ...
                DragonAPICore.logError(
                    "Failed to parse sub-texture from " + fileName + ": "
                    + ex.getLocalizedMessage()
                );
                return true;
            }
    ...
    public static BufferedImage copyImage(BufferedImage buf) {
        BufferedImage ret
            = new BufferedImage(buf.getWidth(), buf.getHeight(), buf.getType());
