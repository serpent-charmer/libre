...
public final class DelegateFontRenderer extends FontRenderer {
    ...
    private static int currentID
        = 512; //char 512; everything from here is a foreign-language char or diacritic
    ...
    public String addRenderer(BasicFontRenderer f) {
        if (currentID >= maxID) {
            throw new MisuseException(
                "Delegate Font Renderer has run out of IDs! All " + maxID
                + " IDs occupied!"
            );
        }
    ...
    private FontKey getRenderer(String sg) {
        if (sg != null && sg.length() > 2) {
            ...
            if (index >= 0 && index < sg.length() - 1) {
                ...
                if (f != null) {
                    return new FontKey(
                        f, sg.substring(0, index) + sg.substring(index + 2, sg.length())
                    );
                }
            }
        }
    ...
    public static DelegateFontRenderer getRegisteredInstance() {
        if (!Loader.instance().hasReachedState(LoaderState.LOADING))
            throw new MisuseException(
                "Tried to access the delegate font renderer too early!"
            );
    ...
    public static String stripFlags(String sg) {
        ...
        while (idx >= 0) {
            sg = sg.replaceAll(
                String.valueOf(keyChar) + String.valueOf(sg.charAt(idx + 1)), ""
            );
