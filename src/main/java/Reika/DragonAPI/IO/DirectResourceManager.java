...
public class DirectResourceManager
    ...
    public IResource getResource(ResourceLocation loc) throws IOException {
        ...
        DirectResource ret
            = rem != null ? new DynamicDirectResource(rem) : new DirectResource(path);
    ...
    public void initToSoundRegistry() {
        ...
        if (sh == null) {
            DragonAPICore.logError(
                "Attempted to initialize sound entries before the sound handler was created!"
            );
            return;
        }
        ...
        if (srg == null) {
            DragonAPICore.logError(
                "Attempted to initialize sound entries before the sound registry was created!"
            );
            return;
        }
