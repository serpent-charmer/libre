...
public final class ReikaMIDIReader {
    ...
    private static Sequence readMIDIFromFile(InputStream in)
        throws IOException, InvalidMidiDataException {
        return MidiSystem.getSequence(in);
    }

    /**
     * Reads a parameter from a MIDI Sequence. Args: Sequence, channel (1-16), time, task
     * (0 = note, 1 = voice, 2 = volume)
     */
    /*
...
1 MC tick = 1 32nd; 2 MC tick = 1 16th; 4 MC tick = 1 8th; 8 MC tick = 1 qtr; 16 MC = 1
half; 32 MC tick = 1 whole = 1920 32 = 1920 1 MC tick = 60 MIDI ticks
*/ /*
    time *= 60;
    time += 120;
        ...
        if (step != -1) {
            ...
            if (msg instanceof ShortMessage) {
                ...
                if (sm.getCommand() == NOTE_ON) {
                    //DragonAPICore.log(channel+" @  "+tr[channel].get(step).getTick()+"
for  "+time); int key = sm.getData1(); int octave = (key / 12)-1; while (octave > 5)
                        octave -= 5;
                    note = (key%12);
                    if (octave == 5 && note != 0)
                        octave = 0;
    ...
    //DragonAPICore.log("Channel "+channel+" has "+voice+" playing "+note+" @
"+tr[channel].get(step).getTick()/60); switch (task) { case 0: return note; case 1: return
voice; case 2: return vol; default: return 0;
    }
}
...
public static boolean hasNoteOn(Sequence seq, int channel, int time) {
            ...
            for (int i = 0; i < track.size(); i++) {
                ...
                if (message instanceof ShortMessage) {
                    ...
                    if (sm.getCommand() == NOTE_ON) {
                        ...
                        System.out.println(
                            "Note on, " + noteName + octave + " key=" + key
                            + " velocity: " + velocity
                        );
                        ...
                        int velocity = sm.getData2();
                        System.out.println(
                            "Note off, " + noteName + octave + " key=" + key
                            + " velocity: " + velocity
                        );
    ...
    public static int[][][] readMIDIFileToArray(Sequence seq) {
        ...
        for (int i = 0; i < tr.length; i++) {
            for (int j = 0; j < tr[i].size(); j++) {
                ...
                //DragonAPICore.log(event.getTick()+" midi to "+time+" MC, out of length
                //"+getSequenceLength(seq)+" ("+getSequenceLength(seq)/20F+") s");
                MidiMessage message = event.getMessage();
                if (message instanceof ShortMessage) {
                    ShortMessage sm = (ShortMessage) message;
                    channel = i - 1;
                    if (channel == -1) {
                        throw new RuntimeException(
                            "Invalid MIDI has notes in the tempo track (track 0)!"
                        );
                    }
                    ...
                    if (dataline[channel][0] != 0 && sm.getCommand() == NOTE_ON) {
                        //DragonAPICore.log("Event "+sm.getCommand()+" at time: "+time+";
                        //Channel "+channel+": "+dataline[channel][0]+"
                        //"+dataline[channel][1]+"  "+dataline[channel][2]);
                        for (int k = 0; k < 16; k++) {
                            for (int l = 0; l < 3; l++) {
                    ...
                    int val = (bytes[0] & 0xff) << 16 | (bytes[1] & 0xff) << 8
                        | (bytes[2] & 0xff);
                    switch (mm.getType()) {
                        case TEMPO:
                            tempo = 60000000 / val;
    ...
    public static MusicScore readMIDIFileToScore(Sequence seq, boolean readPercussion) {
        ...
        for (int i = 0; i < tr.length; i++) {
            for (int j = 0; j < tr[i].size(); j++) {
                ...
                //DragonAPICore.log(tick+" midi to "+time+" MC, out of length
                //"+getSequenceLength(seq)+" ("+getSequenceLength(seq)/20F+") s");
                MidiMessage message = event.getMessage();
                if (message instanceof ShortMessage) {
                    ShortMessage sm = (ShortMessage) message;
                    ...
                    //if (channel == 0) {
                    //	throw new RuntimeException("Invalid MIDI has notes in the tempo
                    //track (track 0)!");
                    //}
                    ...
                    if (channel >= 16) {
                        throw new RuntimeException(
                            "Invalid MIDI has more than 16 tracks!?!"
                        );
                    }
                    activeChannels.add(channel);
                    //ReikaJavaLibrary.pConsole(sm.getCommand()+" on channel "+channel+" @
                    //"+time);
                    switch (sm.getCommand()) {
                        case NOTE_ON:
                            key = sm.getData1();
                            ...
                            if (lastOn[channel][key] != null) {
                                DragonAPICore.log(
                                    "WARNING: MIDI has stacked notes on channel "
                                    + (channel + 1) + " note " + key + " @ " + tick
                                    + "! This will cause truncation!"
                                );
                            }
                            lastOn[channel][key] = new MIDINote(tick, key, instru, vol);
                            //ReikaJavaLibrary.pConsole("ON: "+key+" @ "+time+":
                            //"+lastOn[channel][key]);
                            break;
                        case NOTE_OFF:
                    ...
                    //ReikaJavaLibrary.pConsole("OFF: "+key+" @ "+time+":
                    //"+lastOn[channel][key], sm.getCommand() == NOTE_OFF);
                    if (lastOn[channel][key] != null && sm.getCommand() == NOTE_OFF) {
                        ...
                        //ReikaJavaLibrary.pConsole("Note "+note+" on channel "+channel+"
                        //@ "+time+" (event time="+event.getTick()+")");
                        //ReikaJavaLibrary.pConsole("Note "+note+" on channel "+channel+"
                        //("+m+"), tick off="+time+"/tick_"+event.getTick()+", length =
                        //"+len);
                    ...
                    if (bytes.length == 3) {
                        ...
                        switch (mm.getType()) {
                            case TEMPO:
                                tempo = 60000000 / val;
                                //ReikaJavaLibrary.pConsole("changing tempo @ tick
                                //"+tick+" to "+tempo);
        ...
        for (int i = 0; i < 16; i++) {
            ...
            if (!li.isEmpty()) {
                for (NoteData n : li) {
                    ...
                    if (note == null)
                        DragonAPICore.log(
                            "WARNING: MIDI has note out of range (> C8): " + n.toString()
                        );
                    else
                        data.addNote(
                            timeOn, i, note, n.voice, n.velocity, timeOff - timeOn, i == 9
                        );
                }
            }
        }
    ...
    private static int
    getTimeAtTick(Sequence seq, TreeMap<Long, Integer> tempos, long tick) {
    ...
    private static class MIDINote {
        ...
        public String toString() {
            return String.format(
                "Tick: %d, P:%d, Vel:%d, Voice:%d", tickOn, pitch, velocity, voice
            );
        }
    }
