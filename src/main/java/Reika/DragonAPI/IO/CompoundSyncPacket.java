...
public final class CompoundSyncPacket extends Packet implements DataSync {
    ...
    private final HashMap<WorldLocation, HashMap<String, NBTBase>> oldData
        = new HashMap();
    private final HashMap<WorldLocation, HashMap<String, NBTBase>> changes
        = new HashMap();
    ...
    public void setData(TileEntity te, boolean force, NBTTagCompound NBT) {
        if (dispatch) {
            if (DragonOptions.LOGSYNCCME.getState()) {
                DragonAPICore.log(
                    "The compound sync packet for " + te
                    + " would have just CME'd, as the"
                );
                DragonAPICore.log(
                    "Server-Thread data-writing code has overlapped with the Network-Thread byte[] dispatch."
                );
                DragonAPICore.log(
                    "Seeing this message frequently could indicate a serious issue.\n"
                );
            }
            return;
        }
        ...
        while (it.hasNext()) {
            ...
            if (name == null) {
                DragonAPICore.log(
                    "An NBT tag with a null key is being sent to the compound sync packet from "
                    + te
                );
            } else {
    ...
    private void addData(WorldLocation loc, String key, NBTBase value, boolean force) {
        ...
        if (force || !this.match(prev, value)) {
            //DragonAPICore.log("Changing '"+key+"' from "+prev+" to "+value+" @
            //"+loc.getTileEntity());
    ...
    private void populateFromStream(WorldLocation loc, NBTTagCompound local) {
        ...
        DragonAPICore.log(
            "Reading " + data.get(loc) + " from " + local + " @ " + loc.getTileEntity()
        );
    }
    ...
    public static class CompoundSyncPacketTracker implements TickHandler {
        public static final CompoundSyncPacketTracker instance
            = new CompoundSyncPacketTracker();
