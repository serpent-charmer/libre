...
public class DragonAPIInit extends DragonAPIMod {
    ...
    @SidedProxy(
        clientSide = "Reika.DragonAPI.APIProxyClient",
        serverSide = "Reika.DragonAPI.APIProxy"
    )
    ...
    public static final ControlledConfig config
        = new ControlledConfig(instance, DragonOptions.optionList, null);
    ...
    public void preload(FMLPreInitializationEvent evt) {
        ...
        if (val > 0) {
            ...
            logger.log(
                "Preparing to filter unneeded and/or trivial 'can't keep up' server log messages with delays of less than "
                + val + " ms."
            );
        }
        ...
        logger.log(
            "Initializing libraries with max recursion depth of "
        ...
        ReikaPacketHelper.registerPacketHandler(
            instance, packetChannel, new APIPacketHandler()
        );

        int id = DragonOptions.SYNCPACKET.getValue();
        ReikaPacketHelper.registerVanillaPacketType(
            this, id, SyncPacket.class, Side.SERVER, EnumConnectionState.PLAY
        );
        //if (DragonOptions.COMPOUNDSYNC.getState())
        //	ReikaPacketHelper.registerVanillaPacketType(this, id+1,
        //CompoundSyncPacket.class, Side.SERVER, EnumConnectionState.PLAY);

        if (!DragonOptions.CHECKSANITY.getState())
            CrashNotifications.instance.addNotification(
                null, new SanityCheckNotification()
            );
    ...
    protected void postPreLoad() {
        ...
        BannedItemReader.instance.initWith(
            BannedItemReader.PLUGIN_PATH + "/BanItem", "*"
        );
        BannedItemReader.instance.initWith(
            BannedItemReader.PLUGIN_PATH + "/ItemBan", "*"
        );
        BannedItemReader.instance.initWith(
            BannedItemReader.PLUGIN_PATH + "/TekkitCustomizerData", "*"
        );
        BannedItemReader.instance.initWith(
            BannedItemReader.PLUGIN_PATH + "/TekkitCustomizer", "*"
        );
        BannedItemReader.instance.initWith(
            DragonAPICore.getServerRootFolder(), "ItemBlacklist.json"
        );
        BannedItemReader.instance.initWith(
            Loader.instance().getConfigDir(), "ItemBlacklist.json"
        );
    }
    ...
    private void rebuildAndRegisterVanillaRecipes() {
        ...
        for (IRecipe r : li) {
            if (ReikaRecipeHelper.verifyRecipe(r)) {
                if (ReikaRecipeHelper.isNonVForgeRecipeClass(r)) {
                    DragonAPICore.log("Found a modded recipe registered in pre-init! This
        is a design error, as it can trigger a Forge bug and break the recipe!
        Recipe="+ReikaRecipeHelper.toString(r));
                }
                //if (!DragonAPIMod.EARLY_BUS.post(new AddRecipeEvent(r, true)))
                CraftingManager.getInstance().getRecipeList().add(r);
            }
            else {
                DragonAPICore.logError("Found an invalid recipe in the list, with either
        nulled inputs or outputs! This is invalid! Class="+r.getClass());
            }
        }
         */
        ReplacementCraftingHandler.fireEventsForVanillaRecipes();
        ((EventRecipeList) CraftingManager.getInstance().getRecipeList())
            .filterVanillaRecipes();
        ...
        for (Object o : FurnaceRecipes.smelting().getSmeltingList().keySet()) {
            ...
            ItemStack is2 =
        (ItemStack)FurnaceRecipes.smelting().getSmeltingList().get(is); Object[] dat =
        {is2, FurnaceRecipes.smelting().func_151398_b(is2)}; map.put(is, dat);
        }
    ...
    private int getRecursionDepth() {
        ...
        ); //DragonOptions.RECURSE.getValue();
        if (config == -1)
            config = 0;
        int min = 500;
        int max = 50000;
        if (config < min && config != 0) {
            logger.logError(
                "Specified recursion limit of " + config
                + " is far too low, and has been clamped to " + min + "."
            );
            config = min;
        }
        if (config > max && config != 0) {
            logger.logError(
                "Specified recursion limit of " + config
                + " is far too high, and has been clamped to " + max + "."
            );
            config = max;
        }
    ...
    private void increaseChunkCap() {
        ...
        Property modTC
            = cfg.get(this.getModContainer().getModId(), "maximumTicketCount", 1000);
        Property modCPT
            = cfg.get(this.getModContainer().getModId(), "maximumChunksPerTicket", 2000);
        cfg.save();
    }

    /**
     * Registers all the vanilla technical blocks (except air and block 36) to have items
     * so as to avoid crashes when rendering them in the inventory.
     */
    private void registerTechnicalBlocks() {
        Block[] technicalBlocks = { Blocks.brewing_stand,
                                    Blocks.bed,
                                    Blocks.nether_wart,
                                    Blocks.cauldron,
                                    Blocks.flower_pot,
        ...
        DragonAPICore.log(
            "Added technical blocks for " + Arrays.toString(technicalBlocks)
        );
        DragonAPICore.log(
            "Note that if you remove DragonAPI, you will need to install the patch from Reika's website for the first launch afterwards."
        );
    }
    ...
    public void preventMappingCorruption(FMLMissingMappingsEvent evt) {
        for (MissingMapping m : evt.getAll()) {
            if (m.type == Type.ITEM && (m.id <= 164 ||
    ReikaMathLibrary.isValueInsideBounds(170, 175, m.id))) {
    ...
    private void increasePotionCount() {
        ...
        if (count > newsize) {
            logger.log(
                "Did not increase potion size array, as some other mod already did."
            );
            return;
        }
        ...
        if (Potion.potionTypes.length == newsize)
            logger.log(
                "Overriding the vanilla PotionTypes array to allow for potion IDs up to "
                + (newsize - 1) + " (up from " + (count - 1) + ")."
            );
        else
            logger.logError(
                "Could not increase potion ID limit from " + count + " to " + newsize
    ...
    public void load(FMLInitializationEvent event) {
        ...
        ReikaRegistryHelper.registerModEntity(
            this, EntityTumblingBlock.class, "Tumbling Block", true, 128
        );
        ...
        for (int i = 0; i < 4; i++)
            VillagerRegistry.instance().registerVillageTradeHandler(
                i, VillageTradeHandler.instance
            );
        ...
        //if (DragonOptions.COMPOUNDSYNC.getState())
        //	TickRegistry.instance.registerTickHandler(CompoundSyncPacketTracker.instance,
        //Side.SERVER);

        FMLInterModComms.sendMessage(
            "Waila",
            "register",
            "Reika.DragonAPI.ModInteract.LegacyWailaHelper.registerObjects"
        );
        FMLInterModComms.sendMessage(
            "Waila",
            "register",
            "Reika.DragonAPI.ModInteract.WailaTechnicalOverride.registerOverride"
        );

        FMLInterModComms.sendMessage(
            "Mystcraft",
            "API",
            "Reika.DragonAPI.ModInteract.DeepInteract.ReikaMystcraftHelper.receiveAPI"
        );
        ...
        logger.log(
            "Credit to Techjar for hosting the version file and remote asset server."
        );
    ...
    public void postload(FMLPostInitializationEvent evt) {
        ...
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT) {
            ClientCommandHandler.instance.registerCommand(
                new ToggleBlockChangePacketCommand()
            );
        ...
        if (DragonOptions.BIOMEFIRE.getState()) {
            BiomeGenBase.desert.rainfall
                = BiomeGenBase.desertHills.rainfall; //to differentiate hell and desert
            BiomeGenBase.ocean.rainfall = BiomeGenBase.deepOcean.rainfall = 1F;
        }

        if (DragonOptions.PLAYERMOBCAP.getState()) {
            String f = ReikaObfuscationHelper.isDeObfEnvironment() ? "maxNumberOfCreature"
                                                                   : "field_75606_e";
            try {
                ReikaReflectionHelper.setFinalField(
                    EnumCreatureType.class, f, EnumCreatureType.monster, Integer.MAX_VALUE
                );
            } catch (Exception e) {
        ...
        ); //needs to be re-set due to CoFH overwriting
    ...
    private void initFlowerSeedsRegistration() {
        if (ModList.BOP.isLoaded()
            && BoPBlockHandler.getInstance().initializedProperly()) {
            for (int i = 0; i < BoPBlockHandler.Flower1Types.values().length; i++) {
                OreDictionary.registerOre(
                    "flower", new ItemStack(BoPBlockHandler.getInstance().flower1, 1, i)
                );
            }
            for (int i = 0; i < BoPBlockHandler.Flower2Types.values().length; i++) {
                OreDictionary.registerOre(
                    "flower", new ItemStack(BoPBlockHandler.getInstance().flower2, 1, i)
                );
            }
        }
        ...
        for (int i = 0; i < ModCropList.cropList.length; i++) {
            ...
            if (mod.seedID != null) {
                OreDictionary.registerOre(
                    "seed", new ItemStack(mod.seedID, 1, mod.seedMeta)
                );
            }
        }
    ...
    public void registerCommands(FMLServerStartingEvent evt) {
        ...
        try {
            for (Class c : ReikaJavaLibrary.getAllClassesFromPackage(
                     "Reika.DragonAPI.Command", DragonCommandBase.class, true, true
                 )) {
    ...
    public void printPackData(FMLServerStartedEvent evt) {
        ...
        DragonAPICore.log(
            "Total Crafting Recipes: "
            + CraftingManager.getInstance().getRecipeList().size()
        );
        DragonAPICore.log(
            "Dimensions Present: "
            + Arrays.toString(DimensionManager.getStaticDimensionIDs())
        );
        DragonAPICore.log("Mods Present: " + Loader.instance().getActiveModList().size());
        DragonAPICore.log(
            "ASM Transformers Loaded: " + Launch.classLoader.getTransformers().size()
        );
        if (MinecraftServer.getServer() != null)
            DragonAPICore.log(
                "Commands Loaded: " + ReikaCommandHelper.getCommandList().size()
            );
    }
    ...
    private void alCompat() { //Why the hell are there three standards for aluminum?
        ...
        for (int i = 0; i < al.size(); i++) {
            if (!ReikaItemHelper.collectionContainsItemStack(
                    OreDictionary.getOres("ingotAluminum"), al.get(i)
                ))
                OreDictionary.registerOre("ingotAluminum", al.get(i));
            if (!ReikaItemHelper.collectionContainsItemStack(
                    OreDictionary.getOres("ingotAluminium"), al.get(i)
                ))
        ...
        for (int i = 0; i < al.size(); i++) {
            if (!ReikaItemHelper.collectionContainsItemStack(
                    OreDictionary.getOres("ingotNaturalAluminum"), al.get(i)
                ))
        ...
        for (int i = 0; i < al.size(); i++) {
            if (!ReikaItemHelper.collectionContainsItemStack(
                    OreDictionary.getOres("oreAluminum"), al.get(i)
                ))
    ...
    private void loadHandlers() {
        ...
        this.registerHandler(
            ModList.TRANSITIONAL, TransitionalOreHandler.class, "Handler"
        );
        ...
        this.registerHandler(
            ModList.FORESTRY, ForestryRecipeHelper.class, "Recipe Handler"
        );
        ...
        this.registerHandler(
            ModList.MAGICCROPS,
            MagicCropHandler.class,
            "Handler",
            new SearchVersionHandler("4.0.0_PUBLIC_BETA")
        ); //Newest
        this.registerHandler(
            ModList.MAGICCROPS,
            LegacyMagicCropHandler.class,
            "Handler",
            new SearchVersionHandler("4.0.0_BETA")
        ); //Private Beta
        this.registerHandler(
            ModList.MAGICCROPS,
            VeryLegacyMagicCropHandler.class,
            "Handler",
            new SearchVersionHandler("1.7.2 - 0.1 ALPHA")
        ); //1.7.10 alpha
        ...
        this.registerHandler(
            ModList.FACTORIZATION, FactorizationHandler.class, "Handler"
        );
        ...
        this.registerHandler(
            ModList.PNEUMATICRAFT, PneumaticPlantHandler.class, "Handler"
        );
        this.registerHandler(ModList.BOP, BoPBlockHandler.class, "Handler");
        this.registerHandler(
            ModList.HUNGEROVERHAUL, HungerOverhaulHandler.class, "Handler"
        );
    ...
    private void
    registerHandler(ModList mod, Class<? extends ModHandlerBase> c, String id) {
        this.registerHandler(mod, c, id, new VersionIgnore());
    }

    private boolean registerHandler(
        ModList mod, Class<? extends ModHandlerBase> c, String id, VersionHandler vh
    ) {
        if (mod.isLoaded()) {
            try {
                ...
                if (vh.acceptVersion(ver)) {
                    ...
                    logger.log(
                        "Loading handler " + c + " for mod " + mod + " " + ver + "."
                    );
                    return true;
                } else {
                    logger.log(
                        "Not loading handler " + c + " for " + mod.getDisplayName()
                        + "; Version " + ver + " not compatible with " + vh.toString()
                        + "."
                    );
                    return false;
                }
                ...
                logger.logError(
                    "Class version mismatch error! Could not load handler for "
                    + mod.name()
                );
                e.printStackTrace();
                return false;
            ...
            logger.log(
                "Not loading handler for " + mod.getDisplayName() + "; Mod not present."
            );
            return false;
        }
    }

    private void initHandler(ModList mod, Class<? extends ModHandlerBase> c, String id)
        throws Exception {
    ...
    private static void loadLogParsers() {
        ...
        if (Loader.isModLoaded("endercore")) {
            ...
            if (enderCore != null) {
                enderCore.addFilter(new ReplyFilter(enderCore, "Removed 0 missing texture
        stacktraces. Tada!", "DRAGONAPI: Congratulations. Have 0 cookies."));
            }
        }
         */
    }
