...
public class ReikaLiquidRenderer {
    ...
    public static IIcon getFluidTexture(Fluid fluid, boolean flowing) {
        ...
        if (icon == null) {
            icon = ((TextureMap)
    Minecraft.getMinecraft().getTextureManager().getTexture(TextureMap.locationBlocksTexture)).getAtlasSprite("missingno");
        }
        return icon;
    }*/
    ...
    public static IIcon getFluidIconSafe(Fluid f, boolean flowing) {
        ...
        if (ico == null) {
            DragonAPICore.logError(
                "Fluid " + f.getID() + " (" + f.getLocalizedName() + ") exists (block ID "
                + f.getBlock()
                + ") but has no icon! Registering missingtex as a placeholder!"
            );
