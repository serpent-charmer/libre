...
public class ReikaColorAPI {
    ...
    public static int getPackedIntForColoredLight(int color, int lightval) {
        ...
        return lightval | (blue << 15) | (green << 10)
            | (red << 5); //(getBlue(color) << 15) | (getGreen(color) << 10) |
                          //(getRed(color) << 5) | lightval;
    }
    ...
    public static int fromVec3(Vec3 vec) {
        return RGBtoHex(
            (int) (vec.xCoord * 255), (int) (vec.yCoord * 255), (int) (vec.zCoord * 255)
        );
    }
