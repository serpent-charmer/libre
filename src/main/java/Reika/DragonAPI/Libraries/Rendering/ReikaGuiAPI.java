...
public final class ReikaGuiAPI extends GuiScreen {
     ...
     */
    public void
    drawTexturedModalRectInvert(int x, int y, int u, int v, int w, int h, int scale) {
        y += ySize / 2;
        y -= scale;
        v -= scale;
        h = scale;
    ...
    /**
     * Draws a solid color rectangle with the specified coordinates and color. Modified
     * and simplified from the original in that it automatically handles alpha channel
     * (makes completely transparent full opaque) and changes 4-point method to
     * x-y-width-height. Args: x, y-topleft, width, height, color, alpha on/off
     */
    ...
    public void
    drawLine(double x, double y, double x2, double y2, int color, LineType type) {
     ...
     */
    public void fillBar(
        int par1, int par2, int par3, int par4, int par5, int par6, int par7, boolean par8
    ) {
        int var5;
        if (!par8) {
            int color = par5;
            ...
            par5 = 0xff000000
                + (color - par5); //subtract original color alpha, then add FF
        }
    ...
    public void drawCustomRecipeList(
        RenderItem render, FontRenderer f, List<IRecipe> lr, int x, int y, int x2, int y2
    ) {
        ...
        Object ir = lr.get(k);
        IRecipe ire = ir instanceof WrappedRecipe ? ((WrappedRecipe) ir).getRecipe()
                                                  : (IRecipe) ir;
    ...
    /**
     * Draw a crafting recipe in the GUI. Args: x in, y in; items of: top-left, top,
     * top-right, left, center, right, bottom-left, bottom, bottom right; x out, y out;
     * output item, shapeless t/f. Input items MUST be a size-9 array!
     */
        ...
        for (int ii = 0; ii < 3; ii++) {
            for (int jj = 0; jj < 3; jj++) {
                if (in[ii * 3 + jj] != null) {
                    ...
                    this.drawItemStackWithTooltip(
                        render, f, in[ii * 3 + jj], x + j + 18 * jj, y + k + 18 * ii
                    );
                }
            }
        }
    ...
    public void drawSmelting(
        RenderItem render, FontRenderer f, ItemStack out, int x, int y, int x2, int y2
    ) {
        int j = this.getScreenXInset();
        int k = this.getScreenYInset();
    ...
    /**
     * Note that this must be called after any and all texture and text rendering, as the
     * lighting conditions are left a bit off
     */
    public void
    drawItemStack(RenderItem renderer, FontRenderer fr, ItemStack is, int x, int y) {
    ...
    public void drawItemStack(
        ...
        OpenGlHelper.setLightmapTextureCoords(
            OpenGlHelper.lightmapTexUnit, short1 / 1.0F, short2 / 1.0F
        );
        GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
        ...
        renderer.renderItemOverlayIntoGUI(
            ...
            x,
            y,
            forceStackSize ? String.valueOf(is.stackSize) : null
        );
    ...
    { this.drawItemStackWithTooltip(renderer, Minecraft.getMinecraft().fontRenderer, is,
    x, y);
    }
     */

    public void drawItemStackWithTooltip(
        RenderItem renderer, FontRenderer fr, ItemStack is, int x, int y
    ) {
        GL11.glTranslatef(0.0F, 0.0F, 32.0F);
        FontRenderer f2
            = Minecraft.getMinecraft().fontRenderer; //is.getItem().getFontRenderer(is);
        if (f2 != null)
            fr = f2;
        ...
        if (this.isMouseInBox(x, x + 16, y, y + 16)) {
            ...
            if (right)
                this.drawTooltipAt(
                    fr,
                    sg,
                    this.getMouseRealX() + fr.getStringWidth(sg) + 12,
                    this.getMouseRealY()
                );
            else
    ...
    public void
    drawMultilineTooltip(List<String> li, int x, int y, int spacing, boolean center) {
        GL11.glTranslatef(0.0F, 0.0F, 64.0F);
        int dy = y;
        for (String s : li) {
    ...
    public void drawTooltipAt(FontRenderer f, String s, int mx, int my) {
        ...
        this.drawGradientRect(
            j2 + k + 2, k2 - 3 + 1, j2 + k + 3, k2 + i1 + 3 - 1, k1, l1
        );
    ...
    public void drawSplitTooltipAt(FontRenderer f, List<String> li, int mx, int my) {
        ...
        for (int i = 0; i < li.size(); i++) {
            ...
            if (cacheRenders)
                tooltips.addItem(
                    s, mx, my + 8 + i * 10, f.getStringWidth(s) + 24, f.FONT_HEIGHT + 8
                );
        }
    ...
    public void
    drawTexturedModalRectWithDepth(int x, int y, int u, int v, int w, int h, int o) {
        GL11.glPushMatrix();
    ...
    public void
    drawRectFrame(int minx, int miny, int w, int h, int color, LineType type) {
        int maxx = minx + w;
        int maxy = miny + h;
    ...
    public void
    drawVanillaHealthBar(EntityPlayer ep, ScaledResolution sr, int rowHeight) {
        this.drawVanillaHealthBar(ep, sr, rowHeight, null);
    }

    public void drawVanillaHealthBar(
        EntityPlayer ep,
        ScaledResolution sr,
        int rowHeight,
        ...
        int healthRows
            = MathHelper.ceiling_float_int((healthMax + absorb) / 2.0F / 10.0F);
        ...
        for (int i = MathHelper.ceiling_float_int((healthMax + absorb) / 2.0F) - 1;
            ...
            if (highlight) {
                if (i * 2 + 1 < healthLast)
                    this.drawTexturedModalRect(
                        x, y, margin + 54, upper, 9, 9
                    ); //draw full heart
                else if (i * 2 + 1 == healthLast)
                    this.drawTexturedModalRect(
                        x, y, margin + 63, upper, 9, 9
                    ); //draw half heart
            }
            ...
            if (absorbRemaining > 0) {
                if (absorbRemaining == absorb && absorb % 2.0F == 1.0F)
                    this.drawTexturedModalRect(
                        x, y, margin + 153, upper, 9, 9
                    ); //draw full heart
                else
                    this.drawTexturedModalRect(
                        x, y, margin + 144, upper, 9, 9
                    ); //draw half heart
                ...
                if (i * 2 + 1 < health) //heart is filled
                    this.drawTexturedModalRect(
                        x, y, margin + 36, upper, 9, 9
                    ); //draw full heart
                else if (i * 2 + 1 == health)
                    this.drawTexturedModalRect(
                        x, y, margin + 45, upper, 9, 9
                    ); //draw half heart
