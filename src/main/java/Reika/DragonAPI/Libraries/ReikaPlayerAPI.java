...
public final class ReikaPlayerAPI extends DragonAPICore {
    ...
    /**
     * Get the block a player is looking at. Args: Player, Range, Detect 'soft' blocks
     * yes/no
     *//*
    ...
    getLookedAtEntity(EntityPlayer ep, double reach, double boxSize) {
        Vec3 vec
            = Vec3.createVectorHelper(ep.posX, (ep.posY + 1.62) - ep.yOffset, ep.posZ);
        ...
        for (double d = 0; d <= reach; d += boxSize * 2) {
            ...
            AxisAlignedBB box
                = AxisAlignedBB.getBoundingBox(x - s, y - s, z - s, x + s, y + s, z + s);
    ...
    getLookedAtBlock(EntityPlayer ep, double reach, boolean liq) {
        Vec3 vec
            = Vec3.createVectorHelper(ep.posX, (ep.posY + 1.62) - ep.yOffset, ep.posZ);
    ...
    public static FakePlayer
    getFakePlayerByNameAndUUID(WorldServer world, String name, UUID uuid) {
        /*
    ...
    /**
     * Returns true if the player has the given ID and metadata in their inventory, or is
     * in creative mode. Args: Player, stack
     */
    ...
    playerCanBreakAt(WorldServer world, BlockArray b, EntityPlayerMP ep) {
    ...
    public static boolean
    playerCanBreakAt(WorldServer world, int x, int y, int z, EntityPlayerMP ep) {
        Block b = world.getBlock(x, y, z);
        int meta = world.getBlockMetadata(x, y, z);
        return playerCanBreakAt(world, x, y, z, b, meta, ep);
    }

    public static boolean playerCanBreakAt(
        WorldServer world, int x, int y, int z, Block id, int meta, EntityPlayerMP ep
    ) {
        if (ep == null) {
    ...
    public static boolean playerCanBreakAt(
        WorldServer world, int x, int y, int z, Block id, int meta, String name, UUID uuid
    ) {
        if (name == null) {
            DragonAPICore.logError("Cannot check permissions of a null player!");
            return false;
    ...
    public static boolean
    playerCanBreakAt(WorldServer world, int x, int y, int z, String name, UUID uuid) {
        Block b = world.getBlock(x, y, z);
        int meta = world.getBlockMetadata(x, y, z);
    ...
    public static void syncCustomData(EntityPlayerMP ep) {
        ReikaPacketHelper.sendNBTPacket(
            ...
            PacketIDs.PLAYERDATSYNC.ordinal(),
            ep.getEntityData(),
            new PlayerTarget(ep)
        );
    }
    ...
    public static void syncAttributes(EntityPlayerMP ep) {
        ...
        for (IAttributeInstance iai : c) {
            nbt.setDouble(iai.getAttribute().getAttributeUnlocalizedName(),
        iai.getAttributeValue());
        }
        ReikaPacketHelper.sendNBTPacket(DragonAPIInit.packetChannel,
        PacketIDs.PLAYERATTRSYNC.ordinal(), ep, nbt);*/
        Set set
            = ((ServersideAttributeMap) ep.getAttributeMap()).getAttributeInstanceSet();
        ep.playerNetServerHandler.sendPacket(
            new S20PacketEntityProperties(ep.getEntityId(), set)
        );
    }
    ...
    public static void sendCustomDataFromClient(EntityPlayer ep) {
        ReikaPacketHelper.sendNBTPacket(
            DragonAPIInit.packetChannel,
            PacketIDs.PLAYERDATSYNC_CLIENT.ordinal(),
            ep.getEntityData(),
            PacketTarget.server
        );
    }
    ...
    public static void requestCustomDataSyncFromClient(EntityPlayer ep) {
        ReikaPacketHelper.sendDataPacket(
            DragonAPIInit.packetChannel,
            PacketIDs.PLAYERDATSYNCREQ_CLIENT.ordinal(),
            PacketTarget.server
        );
    }
    ...
    public static NBTTagCompound getDeathPersistentNBT(EntityPlayer ep) {
        NBTTagCompound nbt
            = ep.getEntityData().getCompoundTag(EntityPlayer.PERSISTED_NBT_TAG);
    ...
    public static boolean
    isFakeOrNotInteractable(EntityPlayer ep, double x, double y, double z, double dist) {
    ...
    public static boolean isFake(EntityPlayer ep) {
        if (ep instanceof FakePlayer)
            return true;
        if (ep.getCommandSenderName().contains("CoFH")
            || ep.getCommandSenderName().contains("Thaumcraft"))
            return true;
        String s = ep.getClass().getName().toLowerCase(Locale.ENGLISH);
        if (s.contains("fake") || s.contains("dummy"))
            return true;
        if (ep instanceof EntityPlayerMP
            && ((EntityPlayerMP) ep).playerNetServerHandler == null)
            return true;
        return false;
    }
    ...
    public static List<EntityPlayerMP> getPlayersWithin(World world, AxisAlignedBB box) {
        ...
        for (Object o : world.playerEntities) {
            if (o instanceof EntityPlayerMP
                && ((EntityPlayerMP) o).boundingBox.intersectsWith(box)) {
                EntityPlayerMP ep = (EntityPlayerMP) o;
    ...
    public static void addOrDropItem(ItemStack is, EntityPlayer ep) {
        if (ReikaInventoryHelper.addToIInv(is, ep.inventory))
            ep.onItemPickup(
                new EntityItem(ep.worldObj, ep.posX, ep.posY, ep.posZ, is), is.stackSize
            );
        else
    ...
    public static boolean findAndDecrItem(EntityPlayer ep, ItemStack is) {
        ...
        int slot = ReikaInventoryHelper.locateInInventory(
            is, ep.inventory.mainInventory, false
        );
        if (slot != -1) {
    ...
    public static GameProfile getClientProfile() {
        if (clientProfile == null && DragonAPICore.hasGameLoaded()
            && Minecraft.getMinecraft().thePlayer != null
            && Minecraft.getMinecraft().thePlayer.ticksExisted > 100
            && Minecraft.getMinecraft().getSession().getPlayerID() != null)
    ...
    public static boolean isMiningWith(EntityPlayerMP ep, ItemStack is) {
        return ep.theItemInWorldManager.isDestroyingBlock
            && ItemStack.areItemStacksEqual(is, ep.getCurrentEquippedItem());
    }
}
