...
public class ReikaSortingHelper {
    ...
    /**
     * Sorts a list by every possible pairing. Very slow but very effective for
    ...
    public static void allPairSort(ArrayList li, Comparator c) {
        ...
        DragonAPICore.log(
            "About to perform a very slow sorting! If your game locks up here, come to Reika with this message!"
        );
    ...
    sortListByPairMap(ArrayList li, HashMap<ImmutablePair, Integer> map) {
        for (int i = 0; i < li.size(); i++) {
            Object o1 = li.get(i);
            for (int k = 0; k < li.size(); k++) {
                ...
                if (rel == null)
                    continue;
                if (debug) {
                    //DragonAPICore.log("Comparing "+o1+" @ "+i+" and "+o2+" @ "+k+"; Map
                    //contains "+rel); DragonAPICore.log("");
                }
                if ((rel < 0 && i > k) || (rel > 0 && i < k)) { //swap
                    if (debug) {
                        String s = (rel < 0 && i > k) ? "after when it should be before"
                                                      : "before when it should be after";
                        DragonAPICore.log(
                            "Swapping " + o1 + " and " + o2 + ": o1 was " + s
                        );
                    }
                    li.set(i, o2);
                    li.set(k, o1);
