        ...
        public String encryptInto(String... parts) {
            ...
            for (int i = 0; i < parts.length; i++) {
                ...
                for (int k = 0; k < s.length(); k++) {
                    int c = s.charAt(k);
                    chars[k] = (char
                    ) ReikaMathLibrary.addAndRollover(chars[k], c, 0x0020, 0x007E);
                }
            }
        ...
        public String encryptInto(String... parts) {
            ...
            for (int i = 1; i < parts.length; i++) {
                ...
                for (int k = 0; k < s.length(); k++) {
                    ...
                    int d = c - chars[k];
                    if (d != 0) {
                        chars[k] = (char) ReikaMathLibrary.addAndRollover(
                            chars[k], d / 2, 0x0020, 0x007E
                        );
                    }
                }
            }
        ...
        public String encryptInto(String... parts) {
            ...
            for (int i = 0; i < parts.length; i++) {
                ...
                for (int k = 0; k < s.length(); k++) {
                    ...
                    chars[k] = (char) ReikaMathLibrary.addAndRollover(
                        chars[k], s.length(), 0x0020, 0x007E
                    );
                }
            }
            return new String(chars);
