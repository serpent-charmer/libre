...
public class ReikaRandomHelper extends DragonAPICore {
    ...
    /**
     * Returns a random number less than {@code n} such that the probability of a given
     * value {@code val} is equal to <br> {@code (val+1)/n!}.
     */
    ...
    /**
     * Returns a random number less than {@code n} such that the probability of a given
     * value {@code val} is equal to <br> {@code (n-val+1)/n!}.
     */
    ...
    getRandomSphericalPosition(double x, double y, double z, int r) {
        double dr = rand.nextDouble() * r;
        double[] d = ReikaPhysicsHelper.polarToCartesian(
            dr, rand.nextInt(360), rand.nextInt(360)
        );
