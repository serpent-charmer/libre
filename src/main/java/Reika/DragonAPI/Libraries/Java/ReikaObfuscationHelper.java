...
public class ReikaObfuscationHelper {
    ...
    private static final HashSet<ReflectiveAccessExceptionHandler> errorHandlers
        = new HashSet();

    private static boolean testDeobf() { /*
         try {
             Method m = ItemHoe.class.getMethod("onItemUse", ItemStack.class,
         EntityPlayer.class, World.class, int.class, int.class, int.class, int.class,
         float.class, float.class, float.class); return true;
         }
    ...
    public static String getLabelName(String deobf) {
        ...
        if (sg == null)
            throw new IllegalArgumentException(
                "Tried to get obfuscated name for non-mapped deobf field/method " + deobf
                + "!"
            );
    ...
    private static void addClass(String key, String n) {
        try {
            ...
            ReikaJavaLibrary.pConsole(
                "DRAGONAPI: Registering reflexive field access to class " + c
            ); //cannot use logger
        } catch (ClassNotFoundException e) {
            throw new IllegalArgumentException(
                "Tried to register nonexistent class " + n, e
            );
        }
    }
    ...
    private static void addField(String deobf, String obf, boolean isVisible, Class c) {
        try {
            ...
            ReikaJavaLibrary.pConsole(
                "DRAGONAPI: Registering reflexive field access to " + c + "." + deobf
                + " (obfuscated as " + obf + ")"
            ); //cannot use logger
        } catch (NoSuchFieldException e) {
            throw new IllegalArgumentException(
                "Tried to register nonexistent field " + deobf + "/" + obf, e
            );
        }
    }

    private static void
    addMethod(String deobf, String obf, boolean isVisible, Class c, Class... args) {
        try {
            String sg = isDeObfEnvironment() ? deobf : obf;
            Method m;
            if (isVisible)
            ...
            labels.put(deobf, obf);
            ReikaJavaLibrary.pConsole(
                "DRAGONAPI: Registering reflexive method access to " + c + "." + deobf
                + " (obfuscated as " + obf + ")"
            ); //cannot use logger
        } catch (NoSuchMethodException e) {
            //throw new VanillaIntegrityException("Tried to register nonexistent method
            //"+deobf+"/"+obf+". Check signature.");
    ...
    static {
        ...
        //addMethod("onItemUse", "func_77648_a", true, Item.class, ItemStack.class,
        //EntityPlayer.class, World.class, int.class, int.class, int.class, int.class,
        ...
        addMethod(
            "dropRareDrop", "func_70600_l", false, EntityLivingBase.class, int.class
        );
        ...
        if (isClientSide()) {
         ...
         if (isClientSide()) {
             addField("field_110859_k", "field_110859_k", false, RenderBiped.class);
         //armor texture map addField("scaleAmount", "field_77092_a", false,
         RenderSlime.class); addField("mainModel", "field_77045_g", false,
         RendererLivingEntity.class); addField("modelBipedMain", "field_77109_a", false,
         RenderPlayer.class);
         }
