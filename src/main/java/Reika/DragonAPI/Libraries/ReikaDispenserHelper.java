...
public class ReikaDispenserHelper {
    ...
    private static void
    addDispenserAction(ItemStack is, DispenserAction action, boolean override) {
        ...
        boolean flag = o == null || o instanceof StackDataSensitiveEntry
            || o.getClass() == BehaviorDefaultDispenseItem.class;
        if (!flag && !override)
            return;
        StackDataSensitiveEntry e
            = flag && o.getClass() != BehaviorDefaultDispenseItem.class ? (
                  StackDataSensitiveEntry
              ) o
                                                                        : null;
        if (e == null) {
    ...
    public static void
    overrideDispenserAction(KeyedItemStack is, DispenserAction action) {
        addDispenserAction(is, action, true);
    }

    private static void
    addDispenserAction(KeyedItemStack is, DispenserAction action, boolean override) {
        Object o = BlockDispenser.dispenseBehaviorRegistry.getObject(
            is.getItemStack().getItem()
        );
        boolean flag = o == null || o instanceof StackDataSensitiveEntry
            || o.getClass() == BehaviorDefaultDispenseItem.class;
        if (!flag && !override)
    ...
    public static abstract class DispenserAction implements IBehaviorDispenseItem {
        ...
        public final ItemStack dispense(IBlockSource ibs, ItemStack is) {
            ...
            return this.doAction(
                ...
                z,
                is,
                ForgeDirection.VALID_DIRECTIONS[facing.ordinal()],
                ibs
            );
        }

        protected abstract ItemStack doAction(
            World world,
            int x,
            int y,
            int z,
    ...
    private static class StackDataSensitiveEntry extends DispenserAction {
        ...
        public ItemStack doAction(
            ...
            if (ibd == null)
                ibd = fallback;
            if (ibd != null) {
                return ibd instanceof DispenserAction
                    ? ((DispenserAction) ibd).doAction(world, x, y, z, is, dir, ref)
                    : ibd.dispense(ref, is);
            }
            return is;
        }
    ...
    private static final GameProfile fakePlayerProfile
        = new GameProfile(UUID.randomUUID(), "dispenser");

    public static final FakePlayer getDispenserPlayer(IBlockSource ibs, ItemStack is) {
        return ibs.getWorld() instanceof WorldServer
            ? FakePlayerFactory.get((WorldServer) ibs.getWorld(), fakePlayerProfile)
            : null;
    }
    ...
    public static IBehaviorDispenseItem rightClickEffect = new IBehaviorDispenseItem() {
        ...
        public ItemStack dispense(IBlockSource ibs, ItemStack is) {
            ...
            world.getBlock(x, y, z).onBlockActivated(world, x, y, z, null,
    ReikaDirectionHelper.getOpposite(facing).ordinal(), 0, 0, 0);
