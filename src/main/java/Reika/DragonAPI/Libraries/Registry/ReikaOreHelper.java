...
public enum ReikaOreHelper implements OreType {
    ...
    LAPIS(
        "Lapis Lazuli",
        Blocks.lapis_ore,
        ReikaDyeHelper.BLUE.getStackOf(),
        1,
        "oreLapis",
        "gemLapis",
        OreRarity.SCARCE
    ...
    private ReikaOreHelper(
        String n, Block b, ItemStack is, int nd, String d, String d2, OreRarity r
    ) {
        name = n;
        ore = b;
        drop = is.copy();
    ...
    private ReikaOreHelper(
        String n, Block b, Item i, int nd, String d, String d2, OreRarity r
    ) {
    ...
    public ItemStack getOreBlock() {
        if (ore == null)
            throw new IllegalStateException(
                "Some mod deleted the vanilla ore block for " + this.getDisplayName()
                + "!"
            );
    ...
    public Collection<ItemStack> getAllOreBlocks() {
        return Collections.unmodifiableCollection(
            OreDictionary.getOres(this.getOreDictName())
        );
    }
    ...
    public Block getOreGenBlock() {
        return this.isEnd()   ? Blocks.end_stone
            : this.isNether() ? Blocks.netherrack
                              : Blocks.stone;
    }
    ...
    private void refresh() {
        ...
        ); //wrap in new list to get rid of Forge's immutable
