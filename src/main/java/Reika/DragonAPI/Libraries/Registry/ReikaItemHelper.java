...
public final class ReikaItemHelper extends DragonAPICore {
    ...
    public static final BlockKey stoneDoubleSlab
        = new BlockKey(Blocks.double_stone_slab, 0);
    public static final BlockKey sandstoneDoubleSlab
        = new BlockKey(Blocks.double_stone_slab, 1);
    public static final BlockKey cobbleDoubleSlab
        = new BlockKey(Blocks.double_stone_slab, 3);
    public static final BlockKey brickDoubleSlab
        = new BlockKey(Blocks.double_stone_slab, 4);
    public static final BlockKey stonebrickDoubleSlab
        = new BlockKey(Blocks.double_stone_slab, 5);
    public static final BlockKey netherDoubleSlab
        = new BlockKey(Blocks.double_stone_slab, 6);
    public static final BlockKey quartzDoubleSlab
        = new BlockKey(Blocks.double_stone_slab, 7);
    ...
    public static final BlockKey oakDoubleSlab
        = new BlockKey(Blocks.double_wooden_slab, 0);
    public static final BlockKey spruceDoubleSlab
        = new BlockKey(Blocks.double_wooden_slab, 1);
    public static final BlockKey birchDoubleSlab
        = new BlockKey(Blocks.double_wooden_slab, 2);
    public static final BlockKey jungleDoubleSlab
        = new BlockKey(Blocks.double_wooden_slab, 3);
    ...
    public static boolean matchStacks(ItemStack a, Object b) {
        if (a == b) {
            ...
            return matchStackWithBlock(a, bk.blockID)
                && (!bk.hasMetadata() || a.getItemDamage() == bk.metadata);
        } else if (b instanceof Collection) {
            return ReikaItemHelper.listContainsItemStack(
                (Collection<ItemStack>) b, a, false
            );
        } else if (b instanceof String) {
            return ReikaItemHelper.listContainsItemStack(
                OreDictionary.getOres((String) b), a, false
            );
    ...
    areMetasCombinable(ItemStack a, ItemStack b, boolean checkDurability) {
        int d1 = a.getItemDamage();
        int d2 = b.getItemDamage();
        if (d1 == d2 || d1 == OreDictionary.WILDCARD_VALUE
            || d2 == OreDictionary.WILDCARD_VALUE)
            return true;
        if (a.getHasSubtypes())
            return false;
    ...
    public static EntityItem
    dropItem(World world, double x, double y, double z, ItemStack is) {
        return dropItem(world, x, y, z, is, 1);
    }

    public static EntityItem
    dropItem(World world, double x, double y, double z, ItemStack is, double vscale) {
        if (is == null)
            return null;
    ...
    public static void
    dropItems(World world, double x, double y, double z, Collection<ItemStack> li) {
        for (ItemStack is : li)
    ...
    collectionContainsItemStack(Collection<ItemStack> li, ItemStack is) {
        return listContainsItemStack(li, is, false);
    }

    public static boolean
    listContainsItemStack(Collection<ItemStack> li, ItemStack is, boolean NBT) {
        for (ItemStack is2 : li) {
            if (matchStacks(is, is2)
                && (!NBT || ItemStack.areItemStackTagsEqual(is, is2)))
                return true;
        }
        return false;
    }
            ...
            for (int i = 0; i < ii.getSizeInventory(); i++) {
                ...
                do {
                    ...
                    if (itemstack.hasTagCompound())
                        ei.getEntityItem().setTagCompound(
    ...
    public static BlockKey getWorldBlockFromItem(ItemStack is) {
        ...
        if (matchStackWithBlock(is, Blocks.piston)
            || matchStackWithBlock(is, Blocks.sticky_piston))
            meta = 0;
    ...
    private static ItemStack calculateContainerForFluid(Fluid fluid) {
        ...
        ItemStack fill = FluidContainerRegistry.fillFluidContainer(
            new FluidStack(fluid, Integer.MAX_VALUE), is
        );
        if (fill != null) {
            return fill;
        }
        FluidContainerData[] dat
            = FluidContainerRegistry.getRegisteredFluidContainerData();
        for (int i = 0; i < dat.length; i++) {
            FluidContainerData fcd = dat[i];
            if (fcd.fluid != null && fcd.fluid.getFluid() == fluid
                && fcd.filledContainer != null) {
    ...
    public static boolean isVanillaBlock(Block b) {
        return isVanillaItem(Item.getItemFromBlock(b))
            || Block.getIdFromBlock(b) <= Block.getIdFromBlock(Blocks.double_plant);
    }
    ...
    public static String getNamespace(Item i) {
        ...
        if (s == null) {
            DragonAPICore.logError(
                "Found null ID string for non-null item '" + i.getClass().getName() + "'!"
            );
            return "ERROR";
        }
        return s.substring(0, s.indexOf(':'));
    ...
    public static EntityPlayer getDropper(EntityItem ei) {
        if (ei.getEntityData().hasKey("dropper")) {
            try {
                return ei.worldObj.func_152378_a(
                    UUID.fromString(ei.getEntityData().getString("dropper"))
                );
    ...
    static {
        try {
            Class c = Class.forName(
                "net.minecraftforge.oredict.OreDictionary$UnmodifiableArrayList"
            );
    ...
    public static ArrayList<ItemStack> collateItemList(Collection<ItemStack> c) {
        ...
        for (ItemStack is : c) {
            KeyedItemStack ks
                = new KeyedItemStack(is).setSimpleHash(true).setIgnoreNBT(false);
    ...
    public static boolean
    matchStackCollections(Collection<ItemStack> c1, Collection<ItemStack> c2) {
        if (c1.size() != c2.size())
            return false;
    ...
    public static ItemStack lookupItem(String s) {
        if (Strings.isNullOrEmpty(s))
            throw new IllegalArgumentException(
                "Invalid item string lookup is null or empty"
            );
        String[] parts = s.split(":");
        int m = 0;
        if (parts.length == 3) {
            try {
                m = parts[2].equalsIgnoreCase("*") ? OreDictionary.WILDCARD_VALUE
    ...
    public static ItemStack dechargeItem(ItemStack is) {
        if (is != null) {
            ...
            if (InterfaceCache.GASITEM.instanceOf(i))
                ...
                ((IMuseElectricItem) i).extractEnergy(is, Integer.MAX_VALUE, false);
            else if (InterfaceCache.IELECTRICITEM.instanceOf(i))
                ElectricItem.manager.discharge(
                    is, Double.POSITIVE_INFINITY, Integer.MAX_VALUE, true, false, false
                );
        }
        return is;
    }
    ...
    public static boolean areStacksCombinable(ItemStack is1, ItemStack is2, int limit) {
        ...
        return is1 != null && is2 != null && matchStacks(is1, is2, true)
            && ItemStack.areItemStackTagsEqual(is1, is2)
            && is1.stackSize + is2.stackSize <= limit;
    }
    ...
    public static ItemStack parseItem(Object o, boolean useWildcards) {
        if (o instanceof ItemStack) {
            ...
            return new ItemStack(
                (Item) o, 1, useWildcards ? OreDictionary.WILDCARD_VALUE : 0
            );
        } else if (o instanceof Block) {
            return new ItemStack(
                (Block) o, 1, useWildcards ? OreDictionary.WILDCARD_VALUE : 0
            );
    ...
    public static boolean isToolOrArmor(Item i) {
        return i instanceof ItemTool || i instanceof ItemSword || i instanceof ItemShears
            || i instanceof ItemHoe || i instanceof ItemArmor;
    }
            ...
            if (o1.getItem() == o2.getItem()) {
                if (o1.getItemDamage() == o2.getItemDamage()) {
                    if (o1.stackSize == o2.stackSize) {
                        if (o1.stackTagCompound == o2.stackTagCompound
                            || (o1.stackTagCompound != null
                                && o1.stackTagCompound.equals(o2.stackTagCompound))) {
                            return 0;
                        } else {
                            if (o1.stackTagCompound == null
                                && o2.stackTagCompound != null) {
                                return -1;
                            } else if (o2.stackTagCompound == null && o1.stackTagCompound != null) {
                                return 1;
                            } else {
                                return ReikaNBTHelper.compareNBTTags(
                                    o1.stackTagCompound, o2.stackTagCompound
                                );
                            }
                        }
                    } else {
                ...
                return Integer.compare(
                    Item.getIdFromItem(o1.getItem()), Item.getIdFromItem(o2.getItem())
                );
            }
        }
    }
    ...
    /**
     * Suitable for either raw ItemStacks or lists thereof, like what is found inside an
     * OreRecipe.
     */
    ...
    public static boolean isDenseArmor(ItemStack is) {
        if (is.getItem() instanceof ItemArmor) {
            ...
            return am == ArmorMaterial.GOLD
                || am.getDamageReductionAmount(1)
                >= 6; //gold or at least 3 hearts worth from chestplate
        }
        return false;
    }

    public static ItemStack cookFood(ItemStack food) {
        ItemStack smelted = food.getItem() instanceof ItemFood
            ? FurnaceRecipes.smelting().getSmeltingResult(food)
            : null;
