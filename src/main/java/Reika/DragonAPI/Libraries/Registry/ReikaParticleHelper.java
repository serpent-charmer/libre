...
public enum ReikaParticleHelper {
    ...
    public void
    spawnAt(World world, double x, double y, double z, double vx, double vy, double vz) {
    ...
    public void spawnAroundBlockWithOutset(
        World world, int x, int y, int z, int number, double outset
    ) {
        this.spawnAroundBlockWithOutset(world, x, y, z, 0, 0, 0, number, outset);
    }

    public void spawnAroundBlock(
        World world, int x, int y, int z, double vx, double vy, double vz, int number
    ) {
    ...
    public static void spawnColoredParticles(
        World world, int x, int y, int z, ReikaDyeHelper color, int number
    ) {
    ...
    public static void spawnColoredParticles(
        World world, int x, int y, int z, double r, double g, double b, int number
    ) {
        REDSTONE.spawnAroundBlock(world, x, y, z, r, g, b, number);
    }

    public static void spawnColoredParticlesWithOutset(
        World world,
        int x,
        int y,
        int z,
    ...
    public static void
    spawnColoredParticleAt(World world, double x, double y, double z, int color) {
        REDSTONE.spawnAt(
            world,
            x,
            y,
            z,
            ReikaColorAPI.getRed(color) / 255D,
            ReikaColorAPI.getGreen(color) / 255D,
            ReikaColorAPI.getBlue(color) / 255D
        );
    }

    public static void spawnColoredParticleAt(
        World world, double x, double y, double z, double r, double g, double b
    ) {
