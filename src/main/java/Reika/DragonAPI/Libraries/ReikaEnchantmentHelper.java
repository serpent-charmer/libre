...
public class ReikaEnchantmentHelper extends DragonAPICore {
    public static final Comparator<Enchantment> enchantmentNameSorter
        = new EnchantmentNameComparator();
    public static final Comparator<Enchantment> enchantmentTypeSorter
        = new EnchantmentTypeComparator();
    ...
    /**
     * Returns true iff the new enchantment is compatible with all the other
     * enchantments.
     */
    public static boolean
    isCompatible(Collection<Enchantment> enchantments, Enchantment addition) {
        Iterator<Enchantment> it = enchantments.iterator();
        Iterator<Enchantment> it2 = enchantments.iterator();
        while (it.hasNext()) {
            Enchantment e = it.next();
    ...
    public static void addEnchantment(
        NBTTagCompound tag, Enchantment enchantment, int level, boolean append
    ) {
    ...
    getRandomEnchantment(EnumEnchantmentType cat, boolean modded) {
        int idx = rand.nextInt(Enchantment.enchantmentsList.length);
        Enchantment e = Enchantment.enchantmentsList[idx];
        while (e == null || (cat != null && e.type != cat)
               || (!modded && !isVanillaEnchant(e))) {
