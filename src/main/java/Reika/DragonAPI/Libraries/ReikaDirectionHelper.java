...
public class ReikaDirectionHelper extends DragonAPICore {
    ...
    getDirectionBetween(int x1, int y1, int z1, int x2, int y2, int z2) {
        int dx = x2 - x1;
        int dy = y2 - y1;
        int dz = z2 - z1;
        for (int i = 0; i < 6; i++) {
            ForgeDirection dir = ForgeDirection.VALID_DIRECTIONS[i];
            if (Math.signum(dir.offsetX) == Math.signum(dx)
                && Math.signum(dir.offsetY) == Math.signum(dy)
                && Math.signum(dir.offsetZ) == Math.signum(dz))
                return dir;
        }
    ...
    public static ForgeDirection getDirectionBetween(Coordinate c1, Coordinate c2) {
        return getDirectionBetween(
            c1.xCoord, c1.yCoord, c1.zCoord, c2.xCoord, c2.yCoord, c2.zCoord
        );
    }
    ...
    /**
     * Returns the two positive direction vectors perpendicular to the supplied
     * direction.
     */
    ...
    public static ForgeDirection
    getSideOfBox(int i, int j, int k, boolean vertical, int size) {
        if (i == size)
            return ForgeDirection.EAST;
        else if (i == 0)
            return ForgeDirection.WEST;
    ...
    public static enum CubeDirections {
        ...
        public ForgeDirection getCardinal() {
            return this.isCardinal()
                ? ReikaDirectionHelper.getByDirection(directionX, directionZ)
                : null;
        }
    ...
    public static ForgeDirection
    getImpactedSide(World world, int x, int y, int z, Entity e) {
    ...
    public static ForgeDirection
    getFromLookDirection(EntityLivingBase ep, boolean vertical) {
    ...
    public static ForgeDirection getApproximateDirection(
        int x, int y, int z, int x2, int y2, int z2, boolean vertical
    ) {
        int dx = x2 - x;
        int dy = y2 - y;
        int dz = z2 - z;
    ...
    public static String getDirectionInfoAsString() {
        ...
        for (int i = 0; i < 6; i++) {
            ...
            sb.append(": ");
            sb.append(
                ReikaStringParser.capFirstChar(ForgeDirection.VALID_DIRECTIONS[i].name())
            );
            sb.append("\n");
        }
        return sb.toString();
