...
public final class ReikaMathLibrary extends DragonAPICore {
    ...
    /**
     * Returns the pythagorean sum of the three inputs. Used mainly for vector
     * magnitudes. Args: x,y,z
     */
    ...
    /**
     * Returns true if the input is within [target-range,target+range]. Args: input,
     * target, range
     */
    ...
    /**
     * Returns the value of an integer raised to a POSITIVE integer power. Args: Base,
     * power
     */
    ...
    /**
     * Returns the long value of an integer raised to a POSITIVE integer power. Args:
     * Base, power
     */
    ...
    public static boolean isPowerOfTwo(long num) {
        return num > 0
            && (num & (num - 1)) == 0; //alternate: num > 0 && (num & -num) == num
    }
    ...
    /**
     * Returns the nearest higher power of 2 or "sort of" power of 2 (=1.5*prev power of
     * 2, so eg 192 between 128 and 256)
     */
    ...
    /**
     * Splits a number of items into an array; index 0 is number of stacks, index 1 is
     * leftover Args: Number items, MaxStack size
     */
     ...
     * power distribution. (y = ax^n). Args: Peak x, peak y, required x, falloff factor,
     * power
     */
    public static double
    powerFalloff(double peakx, double peaky, double pos, double factor, double power) {
     ...
     * exponential distribution (y = a*n^x). Args: Peak x, peak y, required x, falloff
     * factor, base
     */
    public static double
    expFalloff(double peakx, double peaky, double pos, double factor, double base) {
        double distance = pos - peakx;
        if (distance < 0)
            distance = -distance;
    ...
    /**
     * Returns true if the value is not inside the bounds (inclusive). Args: Low Bound,
     * Upper Bound, Value
     */
    ...
    /**
     * Returns true if the value is inside the bounds (not inclusive). Args: Low Bound,
     * Upper Bound, Value
     */
    ...
    /**
     * Returns true if the value is inside the bounds (inclusive). Args: Low Bound, Upper
     * Bound, Value
     */
    ...
    /**
     * Returns a double expressed in scientific notation (ret[0] x 10^ret[1]). Args:
     * Value
     */
    ...
    /**
     * Simple test to see if two number ranges overlap. Args: range 1 min/max; range 2
     * min/max
     */
    ...
    isPointInsideEllipse(double x, double y, double z, double ra, double rb, double rc) {
        return (ra > 0 ? ((x * x) / (ra * ra)) : 0) + (rb > 0 ? ((y * y) / (rb * rb)) : 0)
            + (rc > 0 ? ((z * z) / (rc * rc)) : 0)
            <= 1;
    }

    public static boolean isPointInsidePowerEllipse(
        double x, double y, double z, double rx, double ry, double rz, double pow
    ) {
        x = Math.abs(x);
        y = Math.abs(y);
        z = Math.abs(z);
        return (rx > 0 ? (Math.pow(x, pow) / Math.pow(rx, pow)) : 0)
            + (ry > 0 ? (Math.pow(y, pow) / Math.pow(ry, pow)) : 0)
            + (rz > 0 ? (Math.pow(z, pow) / Math.pow(rz, pow)) : 0)
            <= 1;
    }

    public static double
    linterpolate(double x, double x1, double x2, double y1, double y2) {
        return linterpolate(x, x1, x2, y1, y2, false);
    }

    public static double
    linterpolate(double x, double x1, double x2, double y1, double y2, boolean clamp) {
        if (clamp && x <= x1)
            return y1;
        if (clamp && x >= x2)
            return y2;
    ...
    /** Whole wave! */
    public static double
    cosInterpolation(double min, double max, double val, double y1, double y2) {
        return y1 + (y2 - y1) * cosInterpolation(min, max, val);
    }

    /** Half a wave rather than the whole one */
    public static double
    cosInterpolation2(double min, double max, double val, double y1, double y2) {
        double dx = max - min;
        double mm = min + dx * 2;
    ...
    public static double
    normalizeToBounds(double val, double min, double max, double low, double high) {
    ...
    public static boolean
    isPointInsidePolygon(double x, double z, double n, double rx, double rz) {
        if (x * x + z * z > rx * rz)
            return false;
        double da = 360D / (2 * n);
    ...
    public static double
    ellipticalInterpolation(double x, double x1, double x2, double y1, double y2) {
        return (y2 - y1) * Math.sqrt(Math.pow(x2 - x1, 2) - Math.pow(x - x1, 2))
            / (x2 - x1);
    }

    public static double powerInterpolation(
        double x, double x1, double x2, double y1, double y2, double power
    ) {
        return (y2 - y1)
            * Math.pow(Math.pow(x2 - x1, power) - Math.pow(x - x1, power), 1D / power)
            / (x2 - x1);
    }
    ...
    public static int countDecimalPlaces(float f) {
        if (f == Float.NaN || f == Float.NEGATIVE_INFINITY
