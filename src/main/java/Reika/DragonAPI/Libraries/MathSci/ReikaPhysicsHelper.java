...
public final class ReikaPhysicsHelper extends DragonAPICore {
    ...
    /**
     * Converts 3D polar coordinates into cartesian ones. Use angles in degrees. Args:
     * magnitude, theta, phi
     */
    ...
    /**
     * Converts 3D cartesian coordinates into polar ones. Returns angles in degrees,
     * mapped 0-360. Args: x, y, z; Returns: Dist, Theta, Phi
     */
    ...
    /**
     * Calculates the required velocity (in xyz cartesian coordinates) required to travel
     * in projectile motion from point A to point B. Args: start x,y,z end x,y,z, double g
     */
    public static double[] targetPosn(
        double x, double y, double z, double x2, double y2, double z2, double ag
    ) {
    ...
    /**
     * Returns a modified value for the inverse-square law, based on the distance and
     * initial magnitude. Args: Distance x,y,z, initial magnitude
     */
    ...
    /**
     * Returns a float value for MC-scaled explosion power, based off the input energy in
     * joules. Recall TNT has a float power of 4F, corresponding to a real-energy value
     * of 12.4 Gigajoules. Args: Energy
     */
    ...
    getProjectileVelocity(double dist, double ang, double dy, double gravity) {
    ...
    public static double
    getProjectileRange(double vel, double ang, double dy, double gravity) {
        ang = Math.toRadians(ang);
    ...
    public static void reflectEntitySpherical(double x, double y, double z, Entity e) {
        ...
        vec.zCoord /= l;
        double vel
            = vec.dotProduct(Vec3.createVectorHelper(e.motionX, e.motionY, e.motionZ));
