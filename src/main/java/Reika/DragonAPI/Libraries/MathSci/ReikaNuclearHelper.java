...
public class ReikaNuclearHelper {
    ...
    public static double getUraniumFissionNeutronE() {
        return 0.5 * NEUTRON_ESCAPE_SPEED * NEUTRON_ESCAPE_SPEED
            * ReikaPhysicsHelper.NEUTRON_MASS * AVOGADRO;
    }

    public static double getWasteDecayHeat() {
        return ReikaThermoHelper.getTemperatureIncrease(
            1, 15000, AVOGADRO * getEnergyJ(0.5)
        );
    }
    ...
    public static boolean shouldDecay(Isotopes iso, double multiplier) {
        ...
        //ReikaJavaLibrary.pConsole("@ "+multiplier+"x: "+iso+" >
        //"+iso.getHalfLifeAsDisplay()+" > "+chance+" (averages "+trials+" trials
        //("+ReikaDateHelper.getSecondsAsClock(trials/20)+") for 99% chance)");
