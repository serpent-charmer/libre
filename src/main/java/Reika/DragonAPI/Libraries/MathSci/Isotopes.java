...
public enum Isotopes implements ChemicalElement {
    ...
    Xe136(
        10e21, ReikaTimeHelper.YEAR, "Xenon-136", ElementGroup.NONMETAL
    ), //basically stable
    ...
    //stable Cs133("Cesium-133", ElementGroup.ALKALI), //so stable it is used in atomic
    //clocks
    ...
    Ru103(
        1.69, ReikaTimeHelper.TICK, "Ruthenium-103", ElementGroup.TRANSITION
    ), //actually millis
    ...
    private Isotopes(
        double t, ReikaTimeHelper time, String n, ElementGroup g, boolean d
    ) {
        half = t;
        base = time;
        name = n;
    ...
    public String getHalfLifeAsDisplay() {
        return String.format(
            "%.3f%s%s",
            ReikaMathLibrary.getThousandBase(half),
            ReikaEngLibrary.getSIPrefix(half),
            base.getAbbreviation()
        );
    }
    ...
    private static DecayData computeDecayData(Isotopes s) {
        ArrayList<ChemicalElement> li
            = new ArrayList(ReikaJavaLibrary.makeListFromArray(Isotopes.isoList));
        ...
        for (ChemicalElement s2 : li) {
            if ((s2 instanceof Isotopes && s2 == s)
                || s2.getAtomicWeight() > s.atomicWeight - 3)
                continue;
        ...
        return use == null
            ? null
            : new DecayData(use, s.atomicWeight / (double) use.getAtomicWeight());
    }
}
