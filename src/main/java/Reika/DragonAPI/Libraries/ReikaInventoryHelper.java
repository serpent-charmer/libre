...
public final class ReikaInventoryHelper extends DragonAPICore {
        ...
        for (int i = 0; i < inv.length; i++) {
            ...
            if (in != null) {
                if (in.getItem() == id) {
                    ...
                    if (checkForItem(
                            id, ((ActivatedInventoryItem) in.getItem()).getInventory(in)
                        ))
                        return true;
                }
            }
            ...
            if (in != null) {
                if (in.getItem() == id && in.getItemDamage() == dmg) {
                    ...
                    if (checkForItemStack(
                            id,
                            dmg,
                            ((ActivatedInventoryItem) in.getItem()).getInventory(in)
                        ))
                        return true;
                }
            }
    ...
    checkForItemStack(ItemStack is, ItemStack[] inv, boolean matchsize) {
        for (int i = 0; i < inv.length; i++) {
            ItemStack in = inv[i];
            if (in != null) {
                ...
                if (matchsize) {
                    ...
                    if (ItemStack.areItemStackTagsEqual(is, in)
                        && ReikaItemHelper.matchStacks(is, in))
                        return true;
                }
            }
        }
    ...
    public static boolean checkForItemStack(KeyedItemStack is, ItemStack[] inv) {
        for (int i = 0; i < inv.length; i++) {
            ...
            if (in != null) {
                if (in.getItem() instanceof ActivatedInventoryItem) {
                    if (checkForItemStack(
                            is, ((ActivatedInventoryItem) in.getItem()).getInventory(in)
                        ))
                        return true;
                }
                if (is.match(in))
     ...
     */
    public static boolean
    checkForItemStack(ItemStack is, IInventory inv, boolean matchsize) {
        for (int i = 0; i < inv.getSizeInventory(); i++) {
     ...
     */
    public static int
    locateInInventory(ItemStack is, ItemStack[] inv, boolean matchsize) {
        for (int i = 0; i < inv.length; i++) {
            ItemStack in = inv[i];
            if (in != null) {
                if (in.getItem() instanceof ActivatedInventoryItem) {
    ...
    public static int countItem(Item id, int meta, IInventory inv) {
        ...
        for (int i = 0; i < inv.getSizeInventory(); i++) {
            if (inv.getStackInSlot(i) != null) {
                if (inv.getStackInSlot(i).getItem() == id
                    && inv.getStackInSlot(i).getItemDamage() == meta) {
    ...
    /**
     * Returns the existence of an item in the specified inventory, and decrements it if
     * found. Args: Item ID, Item Metadata, Inventory. Set meta to -1 for any.
     */
    public static boolean findAndDecrStack(Item id, int meta, ItemStack[] inv) {
        ItemStack is = meta >= 0 ? new ItemStack(id, 1, meta)
                                 : new ItemStack(id, 1, OreDictionary.WILDCARD_VALUE);
    ...
    public static int countNumStacks(Item id, int meta, ItemStack[] inv) {
        ...
        for (int i = 0; i < inv.length; i++) {
            if (inv[i] != null) {
                if (meta != -1) {
                    //ModLoader.getMinecraftInstance().thePlayer.addChatMessage(String.valueOf(id)+"
                    //= "+String.valueOf(inv[i].getItem()));
                    if (inv[i].getItem() == id && inv[i].getItemDamage() == meta)
                        count++;
                } else {
                    //ModLoader.getMinecraftInstance().thePlayer.addChatMessage(String.valueOf(id)+"
    ...
    /**
     * Counts the number of instances of an ItemStack in an inventory. Does not care
     * about size. Args: ItemStack, inventory. Set meta to -1 for any.
     */
     ...
     */
    public static boolean
    putStackInInventory(ItemStack is, IInventory ii, boolean overrideValid) {
        ...
        for (int i = 0; i < slots.size() && stack.stackSize > 0; i++) {
            ...
            if (overrideValid || ii.isItemValidForSlot(slot, stack)) {
                ...
                if (ReikaItemHelper.matchStacks(stack, in)
     ...
     */
    public static boolean
    putStackInInventory(Item id, int meta, int size, ItemStack[] inventory) {
        if (meta == -1) {
     ...
     * returns the number of "leftover" items that did not fit. Set metadata to -1 for
     * any. Args: Item ID, item metadata, number of items, inventory
     */
    public static int
    addToInventoryWithLeftover(Item id, int meta, int size, ItemStack[] inventory) {
        if (meta == -1) {
    ...
    private static int
    addToInventoryWithLeftover(Item id, int size, ItemStack[] inventory) {
        int slot = locateInInventory(id, inventory);
        int empty = findEmptySlot(inventory);
        if (slot == -1) {
            if (empty == -1)
    ...
    public static int addToInventoryWithLeftover(ItemStack stack, ItemStack[] inventory) {
        int leftover = addToInventoryWithLeftover(
            stack.getItem(), stack.getItemDamage(), stack.stackSize, inventory
        );
        return leftover;
    }

    public static int
    addToInventoryWithLeftover(ItemStack stack, IInventory inventory, boolean simulate) {
            ...
            if (in == null) {
                ...
                if (!simulate)
                    inventory.setInventorySlotContents(
                        i, ReikaItemHelper.getSizedItemStack(stack, add)
                    );
                left -= add;
                if (left <= 0)
                    return 0;
    ...
    /*
    public static boolean addToDSU(IDeepStorageUnit dsu, ItemStack stack, boolean
    simulate) { ItemStack in = dsu.getStoredItemType(); if (in != null &&
    !ReikaItemHelper.matchStacks(in, stack)) return false; int has = in != null ?
    in.stackSize : 0; int space = dsu.getMaxStoredCount()-has; if (space >=
    stack.stackSize) { if (!simulate) { dsu.setStoredItemType(stack, has+stack.stackSize);
            }
            return true;
        }
        else {
     ...
     */
    public static int
    addToDSUWithLeftover(IDeepStorageUnit dsu, ItemStack stack, boolean simulate) {
    ...
    /**
     * Returns true if the inventory is empty in the slot range specified. Args:
     * Inventory, int min, int max
     */
    ...
    public static void decrStack(int slot, ItemStack[] inv, int amt) {
        if (slot >= inv.length) {
            ReikaChatHelper.write(
                "Tried to access Slot " + slot + ", which is larger than the inventory."
            );
            return;
        }
        if (slot < 0) {
    ...
    /**
     * Checks a crafting inventory for a specific ID and metadata (-1 for any). Args:
     * InventoryCrafting, int ID
     */
    ...
    /**
     * Converts a crafting inventory to a standard ItemStack array. Args:
     * InventoryCrafting
     */
    ...
    /**
     * Returns the highest metadata of a specific item/block ID in the specified
     * inventory. If there is none of that ID, it will return -1. Args: ID, inventory
     */
    ...
    /**
     * Returns true if the inventory has space for more of a specific Items. Args: ID,
     * metadata, inventory
     */
    public static boolean canAcceptMoreOf(ItemStack is, IInventory inv) {
        ...
        int num = 0;
        int maxnum = new ItemStack(id, 1, meta).getMaxStackSize()*countNumStacks(id, meta,
        inv); for (int i = 0; i < inv.length; i++) { if (inv[i].getItem() == id &&
        inv[i].getItemDamage() == meta) num += inv[i].stackSize;
        }
        return (num < maxnum);*/
        int space = 0;
        for (int i = 0; i < inv.getSizeInventory(); i++) {
            if (inv.isItemValidForSlot(i, is)) {
                ...
                if (in == null)
                    ...
                    if (ReikaItemHelper.matchStacks(in, is)
                        && ItemStack.areItemStackTagsEqual(is, in)) {
                        int max = Math.min(
                            in.getMaxStackSize(), inv.getInventoryStackLimit()
                        );
    ...
    /**
     * Returns the number of unique itemstacks in the inventory after sorting and
     * cleaning. Args: Inventory
     */
    ...
    /**
     * Sorts and cleans an inventory, combining identical items and arranging them. Args:
     * Inventory
     *//*
    ...
    public static boolean addToIInv(
        ItemStack is, IInventory ii, boolean overrideValid, int firstSlot, int maxSlot
    ) { /*
if (InterfaceCache.DSU.instanceOf(ii))
return addToDSU((IDeepStorageUnit)ii, is, false);*/
        InventoryHandle h = getHandle(ii);
        if (h != iInventory
            || (!overrideValid && firstSlot == 0 && maxSlot == ii.getSizeInventory())) {
            ...
            if (overrideValid || ii.isItemValidForSlot(i, is)) {
                ...
                if (in == null) {
                    ...
                    ii.setInventorySlotContents(
                        i, ReikaItemHelper.getSizedItemStack(is, added)
                    );
                    return true;
                } else {
    ...
    public static boolean
    hasSpaceFor(ItemStack is, IInventory ii, boolean overrideValid) {
        return hasSpaceFor(is, ii, overrideValid, 0, ii.getSizeInventory());
    }

    public static boolean hasSpaceFor(
        ItemStack is, IInventory ii, boolean overrideValid, int firstSlot, int maxSlot
    ) {
    ...
    public static boolean hasItemStack(ItemStack is, IInventory ii) {
        for (int i = 0; i < ii.getSizeInventory(); i++) {
            if (ii.getStackInSlot(i) != null)
                if (ii.getStackInSlot(i).getItem() == is.getItem()
                    && ii.getStackInSlot(i).getItemDamage() == is.getItemDamage())
                    return true;
        }
        return false;
    }
    ...
    public static int
    findMaxMetadataOfIDWithinMaximum(Item id, ItemStack[] inv, int maxmeta) {
        int max = -1;
        for (int i = 0; i < inv.length; i++) {
            if (inv[i] != null && inv[i].getItem() == id) {
    ...
    public static void convertItems(Item id0, int m0, Item id1, int m1, ItemStack[] inv) {
        ...
        for (int i = 0; i < inv.length; i++) {
            if (inv[i] != null) {
                if ((inv[i].getItem() == id0 || id0 == null)
                    && (inv[i].getItemDamage() == m0 || m0 == -1)) {
                    if (id1 == null && m1 == -1) {
                        return;
                    } else if (m1 == -1) {
    ...
    public static void damageInventory(ItemStack[] inv) {
        ...
        for (int i = 0; i < inv.length; i++) {
            if (inv[i] != null) {
                to = new ItemStack(
                    inv[i].getItem(), inv[i].stackSize, inv[i].getItemDamage() + 1
                );
                inv[i] = to;
            }
        }
    ...
    public static boolean addOrSetStack(ItemStack is, IInventory inv, int slot) {
        ...
        if (!ReikaItemHelper.areStacksCombinable(
                is, at, Math.min(inv.getInventoryStackLimit(), is.getMaxStackSize())
            ))
            return false;
    ...
    /**
     * Adds a certain amount of a specified ID and metadata to an inventory slot,
     * creating the itemstack if necessary. Returns true if the whole stack fit and was
     * added. Args: ID, number, metadata (-1 for any), inventory, slot
     */
    public static boolean
    addOrSetStack(Item id, int size, int meta, ItemStack[] inv, int slot) {
        return addOrSetStack(new ItemStack(id, size, meta), inv, slot);
    }

    public static boolean
    addOrSetStack(Block id, int size, int meta, ItemStack[] inv, int slot) {
    ...
    /**
     * Add multiple items to an inventory. Args: IInventory, Items. Returns the ones that
     * could not be added.
     */
    ...
    /**
     * Gets the first block in an inventory, optionally consuming one. Args: Inventory,
     * Decr yes/no
     */
    ...
    /**
     * Spills the entire inventory of an ItemStack[] at the specified coordinates with a
     * 1-block spread. Args: World, x, y, z, inventory
     */
    public static void
    spillAndEmptyInventory(World world, int x, int y, int z, ItemStack[] inventory) {
        EntityItem ei;
        ItemStack is;
        for (int i = 0; i < inventory.length; i++) {
            is = inventory[i];
    ...
    /**
     * Spills the entire inventory of an IInventory at the specified coordinates with a
     * 1-block spread. Args: World, x, y, z, IInventory
     */
    public static void
    spillAndEmptyInventory(World world, int x, int y, int z, IInventory ii) {
    ...
    /**
     * Adds an ItemStack to an inventory and returns how many items were successfully
     * added.
     */
    ...
    public static int
    addStackAndReturnCount(ItemStack stack, IInventory ii, ForgeDirection side) {
        return addStackAndReturnCount(stack, ii, 0, ii.getSizeInventory() - 1);
    }

    public static int
    addStackAndReturnCount(ItemStack stack, IInventory ii, int slotMin, int slotMax) {
        return addStackAndReturnCount(stack, ii, slotMin, slotMax, null);
    }

    public static int addStackAndReturnCount(
        ItemStack stack, IInventory ii, int slotMin, int slotMax, ForgeDirection side
    ) {
        int[] slots = side != null && ii instanceof ISidedInventory
            ? ((ISidedInventory) ii).getAccessibleSlotsFromSide(side.ordinal())
            : ReikaArrayHelper.getLinearArray(slotMin, slotMax);
        int transferred = 0;
        for (int idx = 0; idx < slots.length && stack.stackSize > 0; idx++) {
            ...
            if (is == null) {
                ...
                if (ReikaItemHelper.areStacksCombinable(
                        stack, is, ii.getInventoryStackLimit()
                    )) {
                    int max
                        = Math.min(stack.getMaxStackSize(), ii.getInventoryStackLimit());
    ...
    getAllTransferrables(ForgeDirection from, IInventory source) {
        ArrayList<ItemStack> li = new ArrayList();
    ...
    public static HashMap<Integer, ItemStack>
    getLocatedTransferrables(ForgeDirection from, IInventory source) {
    ...
    getSlotsWithItemStack(ItemStack is, IInventory ii, boolean matchSize) {
    ...
    public static ArrayList<Integer>
    getSlotsWithItemStack(ItemStack is, ItemStack[] inv, boolean matchSize) {
        ArrayList<Integer> li = new ArrayList();
        for (int i = 0; i < inv.length; i++) {
            ItemStack in = inv[i];
            if (ReikaItemHelper.matchStacks(is, in)) {
    ...
    public static HashSet<Integer> getSlotsBetweenWithItemStack(
        ItemStack is, IInventory ii, int min, int max, boolean matchSize
    ) {
    ...
    public static void
    generateMultipliedLoot(int bonus, Random r, String s, IInventory te) {
        for (int n = 0; n < bonus; n++) {
            ...
            WeightedRandomChestContent.generateChestContents(
                r, loot, ii, ChestGenHooks.getCount(s, r)
            );
            for (int i = 0; i < ii.getSizeInventory(); i++) {
                ItemStack in = ii.getStackInSlot(i);
                if (in != null) {
    ...
    public static boolean isAutomatableInventory(TileEntity te) {
        return te instanceof IInventory && !(te instanceof InertIInv)
            && !isBlacklistedInvTE(te);
    }
    ...
    public static final InventoryHandle arrayInventory
        = new InventoryHandle<ItemStack[]>() {
              @Override
              public int addItem(
                  ItemStack[] obj, ItemStack is, boolean requireWholeFit, boolean doAdd
              ) {
                  is = is.copy();
              ...
              public ItemStack takeItem(
                  ItemStack[] obj, ItemMatch find, int amt, boolean doRemove
              ) {
                  for (int i = 0; i < obj.length; i++) {
                      ItemStack in = obj[i];
    ...
    public static final InventoryHandle iInventory = new InventoryHandle<IInventory>() {
        @Override
        public int addItem(
            IInventory ii, ItemStack is, boolean requireWholeFit, boolean doAdd
        ) {
            is = is.copy();
            int max = Math.min(ii.getInventoryStackLimit(), is.getMaxStackSize());
            int added = 0;
            for (int i = 0; i < ii.getSizeInventory(); i++) {
                if (ii.isItemValidForSlot(i, is)) {
                    ...
                    if (in == null) {
                        ...
                        if (doAdd)
                            ii.setInventorySlotContents(
                                i, ReikaItemHelper.getSizedItemStack(is, add)
                            );
                    } else {
                        if (ReikaItemHelper.areStacksCombinable(is, in, max)) {
                            int space = max - in.stackSize;
        ...
        @Override
        public ItemStack takeItem(
            IInventory ii, ItemMatch find, int amt, boolean doRemove
        ) {
            for (int i = 0; i < ii.getSizeInventory(); i++) {
                ItemStack in = ii.getStackInSlot(i);
                if (in != null && find.match(in)) {
    ...
    public static final InventoryHandle dsuInventory
        = new InventoryHandle<IDeepStorageUnit>() {
              @Override
              @ClassDependent("powercrystals.minefactoryreloaded.api.IDeepStorageUnit")
              public int addItem(
                  IDeepStorageUnit obj,
                  ItemStack is,
                  boolean requireWholeFit,
                  boolean doAdd
              ) {
              ...
              @ClassDependent("powercrystals.minefactoryreloaded.api.IDeepStorageUnit")
              public ItemStack takeItem(
                  IDeepStorageUnit obj, ItemMatch find, int amt, boolean doRemove
              ) {
                  ItemStack in = obj.getStoredItemType();
                  if (in == null || !find.match(in))
                      return null;
    ...
    public static final InventoryHandle drawerInventory
        = new InventoryHandle<IDrawerGroup>() {
              @Override
              @ModDependent(ModList.STORAGEDRAWERS)
              public int addItem(
                  IDrawerGroup obj, ItemStack is, boolean requireWholeFit, boolean doAdd
              ) {
                  for (int i = 0; i < obj.getDrawerCount(); i++) {
                      ...
                      if (!dwr.canItemBeStored(is))
                          continue;
                      if (dwr.isEmpty()
                          || ReikaItemHelper.areStacksCombinable(
                              dwr.getStoredItemPrototype(), is, Integer.MAX_VALUE
                          )) {
                          int space = dwr.isEmpty() ? dwr.getMaxCapacity(is)
                                                    : dwr.getRemainingCapacity();
              ...
              @ModDependent(ModList.STORAGEDRAWERS)
              public ItemStack takeItem(
                  IDrawerGroup obj, ItemMatch find, int amt, boolean doRemove
              ) {
                  for (int i = 0; i < obj.getDrawerCount(); i++) {
                      IDrawer dwr = obj.getDrawer(i);
                      if (dwr.isEmpty())
