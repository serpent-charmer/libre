...
public class ReikaSpawnerHelper {
    ...
    public static void
    setSpawnerFromItemNBT(ItemStack is, TileEntityMobSpawner spw, boolean keepLogic) {
        if (is == null)
            return;
    ...
    private static void
    setSpawnerLogic(TileEntityMobSpawner spw, MobSpawnerBaseLogic lgc) {
    ...
    public static void generateSpawnerTooltip(ItemStack is, List li) {
        if (is.stackTagCompound.hasKey("Spawner")) {
            li.add(
                "Spawns "
                + ReikaEntityHelper.getEntityDisplayName(
                    is.stackTagCompound.getString("Spawner")
                )
            );
        } else {
        ...
        if (is.stackTagCompound.hasKey("logic")) {
            if (GuiScreen.isShiftKeyDown()) {
                ...
                lgc.readFromNBT(is.stackTagCompound.getCompoundTag("logic"));
                li.add(
                    "Min Delay: " + EnumChatFormatting.WHITE + lgc.minSpawnDelay
                    + " ticks"
                );
                li.add(
                    "Max Delay: " + EnumChatFormatting.WHITE + lgc.maxSpawnDelay
                    + " ticks"
                );
                li.add(
                    "Max Near Mobs: " + EnumChatFormatting.WHITE + lgc.maxNearbyEntities
                );
                li.add("Spawn Count: " + EnumChatFormatting.WHITE + lgc.spawnCount);
                li.add("Spawn Range: " + EnumChatFormatting.WHITE + lgc.spawnRange + "m");
                li.add(
                    "Activation Range: " + EnumChatFormatting.WHITE
                    + lgc.activatingRangeFromPlayer + "m"
                );
            } else {
                li.add(
                    EnumChatFormatting.LIGHT_PURPLE + "Hold LSHIFT for spawner parameters"
                );
            }
        } else {
    ...
    public static void setSpawnerItemNBT(
        ItemStack is,
        int minDelay,
        int maxDelay,
        int maxNear,
        ...
        setSpawnerItemNBT(
            is,
            new TemporarySpawnerLogic(
                minDelay, maxDelay, maxNear, spawnCount, spawnRange, activeRange
            ),
            force,
            true
        );
    ...
    public static void setSpawnerItemNBT(
        ItemStack is, MobSpawnerBaseLogic lgc, boolean force, boolean keepLogic
    ) {
        if (is.stackTagCompound == null)
            is.setTagCompound(new NBTTagCompound());
        if (force || !is.stackTagCompound.hasKey("Spawner")) {
    ...
    /**
     * Forcibly runs the spawn cycle for a spawner. Args: Spawner TileEntity, number of
     * mobs, Potion effects
     */
    public static void
    forceSpawn(TileEntityMobSpawner spw, int num, PotionEffect... potions) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (e == null) {
                DragonAPICore.logError(
                    "Could not spawn spawner-based entity '" + name + "'"
                );
                continue;
            }
    ...
    private static boolean isDefaultParams(MobSpawnerBaseLogic lgc) {
        ...
        return lgc.minSpawnDelay == lgc2.minSpawnDelay
            && lgc.maxSpawnDelay == lgc2.maxSpawnDelay
            && lgc.maxNearbyEntities == lgc2.maxNearbyEntities
            && lgc.spawnCount == lgc2.spawnCount && lgc.spawnRange == lgc2.spawnRange
            && lgc.activatingRangeFromPlayer == lgc2.activatingRangeFromPlayer;
    }
    ...
    private static class ConstructableSpawnerLogic extends MobSpawnerBaseLogic {
        ...
        public void func_98267_a(int val) {
            this.getSpawnerWorld().addBlockEvent(
                this.getSpawnerX(),
                this.getSpawnerY(),
                this.getSpawnerZ(),
                Blocks.mob_spawner,
