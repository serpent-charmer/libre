...
public final class ReikaChatHelper extends DragonAPICore {
    ...
    public static void clearChat(EntityPlayerMP ep) {
        if (FMLCommonHandler.instance().getEffectiveSide() == Side.CLIENT) {
            ...
            ReikaPacketHelper.sendDataPacket(
                DragonAPIInit.packetChannel, PacketIDs.CLEARCHAT.ordinal(), ep
            );
        }
    }
    ...
    public static void writeItemStack(World world, ItemStack is) {
        ...
        if (is == null)
            ...
            msg = String.format(
                "%d, %d, %d", is.getItem(), is.stackSize, is.getItemDamage()
            );
        writeString(msg);
    }
    ...
    public static void writeBlockAtCoords(World world, int x, int y, int z) {
        ...
        sb.append(
            String.format(
                "Block %s (ID %d Metadata %d) @ x=%d, y=%d, z=%d",
                name,
    ...
    public static void writeEntity(World world, Entity ent) {
        ...
        if (ent == null)
            ...
            writeString(
                ...
                + String.format("%.2f, %.2f, %.2f", ent.posX, ent.posY, ent.posZ)
            );
    }
    ...
    public static void writeItem(World world, Item id, int dmg) {
        ...
        //	writeBlock(world, id, dmg);
            ...
            writeString(
                id + ":" + dmg + " is "
                + id.getItemStackDisplayName(new ItemStack(id, 1, dmg))
            );
    }
            ...
            if (cfg != null) {
                for (int i = 0; i < parts.length; i++) {
                    ChatComponentTranslation chat
                        = new ChatComponentTranslation(parts[i]);
                    cfg.sendChatMsg(chat);
                }
            } else {
                DragonAPICore.logError(
                    "Something tried to send chat to a server with null configurations!"
                );
