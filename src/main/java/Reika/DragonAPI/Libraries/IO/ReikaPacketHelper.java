...
public final class ReikaPacketHelper extends DragonAPICore {
    ...
    public static void
    registerPacketHandler(DragonAPIMod mod, String channel, PacketHandler handler) {
    ...
    public static void
    sendNIntPacket(String ch, int id, PacketTarget p, List<Integer> data) {
    ...
    public static void
    sendDataPacket(String ch, int id, EntityPlayerMP ep, List<Integer> data) {
        int npars;
        if (data == null)
            npars = 4;
        else
    ...
    public static void
    sendDataPacket(String ch, int id, TileEntity te, EntityPlayerMP ep, int... data) {
        int npars;
        if (data == null)
            npars = 4;
        else
    ...
    public static void sendDataPacket(
        String ch,
        int id,
        World world,
        int x,
        ...
        if (side == Side.SERVER) {
            ...
            PacketTarget pt
                = new PacketTarget.RadiusTarget(world, x + 0.5, y + 0.5, z + 0.5, radius);
    ...
    public static void
    sendDataPacket(String ch, int id, PacketTarget pt, List<Integer> data) {
        int npars;
        if (data == null)
            npars = 4;
        else
    ...
    public static void
    sendDataPacketToEntireServer(String ch, int id, List<Integer> data) {
        int npars;
        if (data == null)
            npars = 4;
        else
    ...
    public static void sendLongDataPacket(
        String ch, int id, World world, int x, int y, int z, List<Long> data
    ) {
        int npars;
        if (data == null)
            npars = 4;
    ...
    public static void
    sendUUIDPacket(String ch, int id, World world, int x, int y, int z, UUID data) {
        ByteArrayOutputStream bos
            = new ByteArrayOutputStream(2 * 8 + 3 * 4); //4 bytes an int + 8 bytes a long
        DataOutputStream outputStream = new DataOutputStream(bos);
        try {
            outputStream.writeInt(id);
    ...
    public static void sendPacketToServer(String ch, int id, int... data) {
        sendDataPacket(
            ...
            0,
            PacketTarget.server,
            ReikaJavaLibrary.makeIntListFromArray(data)
        );
    }
    ...
    public static void sendDataPacketWithRadius(
        String ch, int id, World world, Coordinate c, int radius, int... data
    ) {
        sendDataPacket(
            ch,
            id,
    ...
    public static void sendDataPacketWithRadius(
        String ch, int id, WorldLocation c, int radius, int... data
    ) {
        sendDataPacket(
            ch,
            id,
    ...
    public static void
    sendDataPacketWithRadius(String ch, int id, TileEntity te, int radius, int... data) {
        sendDataPacket(
            ch,
            id,
            te.worldObj,
    ...
    public static void
    sendDataPacketWithRadius(String ch, int id, Entity e, int radius, int... data) {
        sendDataPacketWithRadius(
            ch,
            id,
            e.worldObj,
    ...
    public static void sendDataPacketWithRadius(
        String ch, int id, World world, int x, int y, int z, int radius, int... data
    ) {
        sendDataPacket(
            ch,
            id,
    ...
    public static void sendDataPacket(
        String ch, int id, World world, int x, int y, int z, PacketTarget pt, int... data
    ) {
        sendDataPacket(
            ch, id, world, x, y, z, pt, ReikaJavaLibrary.makeIntListFromArray(data)
        );
    }
    ...
    public static void sendLongDataPacket(String ch, int id, TileEntity te, long data) {
        sendLongDataPacket(
            ...
            te.yCoord,
            te.zCoord,
            ReikaJavaLibrary.makeListFrom(data)
        );
    }

    public static void writeDirectSound(
        String ch,
        int id,
        World world,
        double x,
        ...
        try {
            ...
            throw new RuntimeException(
                "Sound Packet for sound '" + name + "' @ " + x + ", " + y + ", " + z
                + " threw a packet exception!"
            );
        }
        ...
        if (side == Side.SERVER) {
            ...
            //PacketDispatcher.sendPacketToAllAround(x, y, z, 20,
    ...
    private static int getSoundDistance(boolean atten, SoundEnum s) {
        if (atten) {
            float d = s instanceof CustomDistanceSound
                ? ((CustomDistanceSound) s).getAudibleDistance()
                : 16;
    ...
    public static void sendStringPacket(String ch, int id, String sg, PacketTarget pt) {
        ...
        try {
            ...
            ex.printStackTrace();
            throw new RuntimeException(
                "String Packet for " + sg + " threw a packet exception!"
            );
        }
    ...
    public static void sendStringPacket(String ch, int id, String sg, TileEntity te) {
        ...
        if (side == Side.SERVER) {
            ...
            //PacketDispatcher.sendPacketToServer(packet);
            //PacketDispatcher.sendPacketToAllInDimension(packet,
            //te.worldObj.provider.dimensionId);
    ...
    public static void
    sendStringPacket(String ch, int id, String sg, World world, int x, int y, int z) {
        int length = 0;
        ByteArrayOutputStream bos = new ByteArrayOutputStream(length);
        DataOutputStream outputStream = new DataOutputStream(bos);
        try {
    ...
    public static void
    sendStringIntPacket(String ch, int id, EntityPlayerMP ep, String sg, int... data) {
        ...
        try {
            ...
            ex.printStackTrace();
            //throw new RuntimeException("String Packet for "+sg+" threw a packet
            //exception!");
        }
    ...
    public static void
    sendStringIntPacket(String ch, int id, TileEntity te, String sg, int... data) {
        int length = data.length * 4;
        ByteArrayOutputStream bos = new ByteArrayOutputStream(length);
        DataOutputStream outputStream = new DataOutputStream(bos);
        try {
    ...
    public static void
    sendStringIntPacket(String ch, int id, PacketTarget p, String sg, int... data) {
        int length = data.length * 4;
        ByteArrayOutputStream bos = new ByteArrayOutputStream(length);
        DataOutputStream outputStream = new DataOutputStream(bos);
        try {
    ...
    public static void
    sendStringPacketWithRadius(String ch, int id, TileEntity te, int radius, String sg) {
        int length = 0;
        ByteArrayOutputStream bos = new ByteArrayOutputStream(length);
        DataOutputStream outputStream = new DataOutputStream(bos);
        try {
    ...
    public static void
    sendUpdatePacket(String ch, int id, TileEntity te, PacketTarget pt) {
        sendUpdatePacket(ch, id, te.xCoord, te.yCoord, te.zCoord, pt);
    }

    public static void
    sendUpdatePacket(String ch, int id, int x, int y, int z, PacketTarget pt) {
        ...
        try {
            ...
            throw new RuntimeException(
                "Packet " + ch + "/" + id + " @ " + x + "," + y + "," + z
                + " threw an update packet exception!"
            );
        }
    ...
    public static void
    sendFloatPacket(String ch, int id, World world, int x, int y, int z, float data) {
        ByteArrayOutputStream bos = new ByteArrayOutputStream(20);
        DataOutputStream outputStream = new DataOutputStream(bos);
        try {
            outputStream.writeInt(id);
    ...
    public static void
    sendPositionPacket(String ch, int id, Entity e, PacketTarget pt, int... data) {
    ...
    public static void sendPositionPacket(
        ...
        sendPositionPacket(
            ch, id, world, x, y, z, new PacketTarget.RadiusTarget(world, x, y, z, r), data
        );
    }
    ...
    public static void
    sendSyncPacket(String ch, TileEntity te, String field, boolean forceClient) {
        int x = te.xCoord;
        int y = te.yCoord;
        int z = te.zCoord;
        int length = 0;
        ...
        if (side == Side.SERVER) {
            ...
            if (forceClient)
                ...
                DragonAPICore.logError(
                    te + " sent a sync packet from the client! This is not allowed!"
                );
        } else {
            // We are on the Bukkit server.
        }
    ...
    public static void sendTankSyncPacket(String ch, TileEntity te, String tankField) {
        ...
        try {
            ...
            //ex.printStackTrace();
            DragonAPICore.logError(
                te + " tried to sync its tank, but it is not a HybridTank instance!"
            );
        } catch (IllegalAccessException ex) {
            ex.printStackTrace();
        } catch (IOException ex) {
    ...
    public static void
    sendNBTPacket(String ch, int id, NBTTagCompound nbt, PacketTarget pt) {
    ...
    public static void updateTileEntityData(
        World world, int x, int y, int z, String name, DataInputStream in
    ) {
    ...
    public static void
    updateTileEntityTankData(World world, int x, int y, int z, String name, int level) {
        if (world.checkChunksExist(x, y, z, x, y, z)) {
            TileEntity te = world.getTileEntity(x, y, z);
            if (te == null) {
    ...
    private static void writeString(String par0Str, DataOutput par1DataOutput)
        throws IOException {
    ...
    public static abstract class PacketObj implements IMessage {
        ...
        public String toString() {
            String hd = handler.getClass().getCanonicalName() + " (ID " + this.handlerID()
                + ")";
            return "type " + this.getType() + "; Data: " + this.getDataAsString()
                + " from " + hd;
        }
        ...
        private void close() {
            try {
                ...
                DragonAPIInit.instance.getModLogger().logError(
                    "Error closing packet " + this + ". Memory may leak."
                );
                e.printStackTrace();
            }
        }
        ...
        protected final byte[] writeNBTTagCompoundToBytes(NBTTagCompound tag)
            throws IOException {
        ...
        protected final NBTTagCompound readNBTTagCompoundFromBuffer(byte[] bytes)
            throws IOException {
            ...
            if (short1 < 0)
                ...
                buf.readFully(abyte);
                return CompressedStreamTools.func_152457_a(
                    abyte, NBTSizeTracker.field_152451_a
                );
            }
        }
    ...
    public static void registerVanillaPacketType(
        DragonAPIMod mod,
        int id,
            ...
            case CLIENT:
                if (state.func_150753_a().containsKey(id))
                    throw new IDConflictException(
                        mod,
                        "Packet " + c + " ID " + id + " is already occupied by "
                            + state.func_150753_a().get(id) + "!"
                    );
        ...
        mod.getModLogger().log(
            "Registering vanilla-type packet " + c + " with ID " + id + " on side " + s
        );
    }
    ...
    public static void syncTileEntity(TileEntity tile) {
        NBTTagCompound NBT = new NBTTagCompound();
        tile.writeToNBT(NBT);
        List<EntityPlayerMP> li = tile.worldObj.getEntitiesWithinAABB(
            EntityPlayerMP.class,
            ReikaAABBHelper.getBlockAABB(tile.xCoord, tile.yCoord, tile.zCoord)
                .expand(4, 4, 4)
        );
        for (EntityPlayerMP ep : li)
            sendNBTPacket(
                DragonAPIInit.packetChannel,
                PacketIDs.VTILESYNC.ordinal(),
                NBT,
                new PlayerTarget(ep)
            );
    }
