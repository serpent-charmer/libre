...
public abstract class CraftingContainer<V> extends CoreContainer {
    ...
    public CraftingContainer(
        EntityPlayer player, CraftingTile<V> te, World worldObj, boolean gui
    ) {
    ...
    public final ItemStack slotClick(int slot, int par2, int action, EntityPlayer ep) {
        ...
        if (wr != null && crafter.isReadyToCraft() && slot == 13) {
            ...
            if (drop != null
                && (!ReikaItemHelper.matchStacks(drop, craft)
                    || drop.stackSize + craft.stackSize > drop.getMaxStackSize()))
                return is;
            this.craft(wr, ep);
