...
public class StupidIDException extends DragonAPIException {
    public StupidIDException(DragonAPIMod mod, Property id, Type req) {
        ...
        message.append(
            "ID '" + id.getString() + "' is completely invalid, as it is the wrong type ("
            + id.getType() + " when it should be " + req + ").\n"
        );
        ...
        message.append(
            "This is NOT a mod bug. Do not post it or ask for support or you will look extremely foolish."
        );
        this.crash();
    }
    ...
    public StupidIDException(DragonAPIMod mod, int ID, IDType type) {
        message.append(mod.getDisplayName() + " was not installed correctly:\n");
        message.append(
            type.getName() + " ID " + ID + " is completely invalid, as it is "
            + this.getError(ID, type) + ".\n"
        );
        message.append("Please learn how IDs work before attempting to modify configs.\n"
        );
        if (ID > 100000)
            message.append(
                "No sane ID would be this large, and you would do well to realize this.\n"
            );
        message.append(
            "This is NOT a mod bug. Do not post it or ask for support or you will look extremely foolish."
        );
