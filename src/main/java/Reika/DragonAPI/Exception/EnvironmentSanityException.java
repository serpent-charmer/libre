...
public class EnvironmentSanityException extends DragonAPIException {
    ...
    public static enum ErrorType {
        ...
        public String getString(Object... data) {
            switch (this) {
                case NULLREG:
                    return data[0] + " (" + data[0].getClass()
                        + ") was assigned a null name in the GameRegistry!";
                case NULLENTRY:
                    return "Null was registered to the GameRegistry as '" + data[0]
                        + "'!";
                case IDMISMATCH:
                    return data[0] + " (" + data[0].getClass() + ") occupies an ID ("
                        + data[1] + ") that does not match its stored value (" + data[2]
                        + ")!";
                case INVALIDVALUE:
                    return data[0] + " (" + data[0].getClass() + ") returns an invalid ("
                        + data[1] + ") value for a critical field or function ('"
                        + data[2] + "')!";
                case UNPARSEABLE:
                ...
                case OREDICT:
                    return EnvironmentSanityChecker.getSafeItemString((ItemStack) data[0])
                        + " (" + data[0].getClass() + ") registered to the OreDict as '"
                        + data[1] + "', but is an invalid item, throwing " + data[2]
                        + " when parsing '" + data[3] + "'!";
                case LOOT:
                    return EnvironmentSanityChecker.getSafeItemString((ItemStack) data[0])
                        + " was registered to the loot table '" + data[1]
                        + "', and is invalid: " + data[2];
            }
