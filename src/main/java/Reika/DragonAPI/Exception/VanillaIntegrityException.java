...
public class VanillaIntegrityException extends DragonAPIException {
    public VanillaIntegrityException(DragonAPIMod mod, String missing) {
        ...
        message.append(
            mod.getDisplayName() + " relies on this " + missing
            + ", and thus you must fix that other mod or remove it.\n"
        );
        this.crash();
    }
    ...
    public VanillaIntegrityException(Class<? extends Block> cl, int id, boolean isBlock) {
        ...
        message.append(
            "ID " + id + " (normally " + cl.getSimpleName()
            + ") was deleted by another mod!\n"
        );
        message.append(
            "One or more of the DragonAPI mods rely on this " + sg
            + ", and thus you must fix that other mod or remove it.\n"
        );
        this.crash();
    }
    ...
    public VanillaIntegrityException(Method m) {
        ...
        message.append(
            "If you are using MCPC/Cauldron, try to replicate the crash in SSP!\n"
        );
        message.append(
            "If the crash does not occur there, you must fix your copy of MCPC/Cauldron.\n"
        );
    ...
    public VanillaIntegrityException(String methodName, Class src, Class... argTypes) {
        ...
        message.append(
            "Could not find " + methodName + "(" + Arrays.toString(argTypes)
            + " in class " + src.getCanonicalName() + "\n"
        );
        message.append(
            "If you are using MCPC/Cauldron, try to replicate the crash in SSP!\n"
        );
