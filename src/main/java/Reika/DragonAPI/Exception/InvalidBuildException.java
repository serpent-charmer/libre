...
public class InvalidBuildException extends DragonAPIException {
    ...
    public InvalidBuildException(DragonAPIMod mod, File jar, String detail) {
        ...
        message.append(
            jar.getPath() + " is likely not a valid compiled copy of the mod.\n"
        );
        if (detail != null) {
            message.append(detail + "\n");
        }
        message.append(
            "If you are attempting to make a custom build of the code, consult "
            + mod.getModAuthorName() + ".\n"
        );
        message.append(
            "Note that not all mods permit this, or of distributed versions of custom code, for security reasons.\n"
        );
        message.append(
